...
EXTERN void __tgt_sycl_create_interop_wrapper(omp_interop_t interop) {
   ...
   *SyclWrapperObj->SyclDevice = sycl::ext::oneapi::level_zero::make_device(
                       *SyclWrapperObj->SyclPlatform, reinterpret_cast<pi_native_handle>(ZeDevice));

   SyclWrapperObj->SyclContext = new sycl::context;;
   *SyclWrapperObj->SyclContext = sycl::make_context<sycl::backend::ext_oneapi_level_zero>(
                       {ZeContext, {*SyclWrapperObj->SyclDevice}, sycl::ext::oneapi::level_zero::ownership::keep});

   SyclWrapperObj->ZeQueue = ZeQueue;
   SyclWrapperObj->SyclQueue = new sycl::queue;
   ze_command_queue_handle_t ZeQueueT = static_cast<ze_command_queue_handle_t>(ZeQueue);
   *SyclWrapperObj->SyclQueue = sycl::make_queue<sycl::backend::ext_oneapi_level_zero>(
                        {ZeQueueT, sycl::ext::oneapi::level_zero::ownership::keep}, *SyclWrapperObj->SyclContext);
