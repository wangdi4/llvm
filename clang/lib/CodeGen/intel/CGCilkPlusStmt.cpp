...
void CodeGenFunction::EmitCilkForGrainsizeStmt(const CilkForGrainsizeStmt &S) {
...
class InlinedCilkForRegion {
  CodeGenFunction &CGF;
  CodeGenFunction::CGCapturedStmtInfo *PrevCapturedStmtInfo;

public:
  InlinedCilkForRegion(CodeGenFunction &CGF,
                       CodeGenFunction::CGCilkForStmtInfo *CFSI)
      : CGF(CGF), PrevCapturedStmtInfo(CGF.CapturedStmtInfo) {
    CGF.CapturedStmtInfo = CFSI;
  };
  ~InlinedCilkForRegion() { CGF.CapturedStmtInfo = PrevCapturedStmtInfo; };
};

// We need this function to be sure
//    that RAII object will be destroyed in proper time
static llvm::Value *getLoopCountValue(CodeGenFunction &CGF,
                                      const CilkForStmt &S,
                                      CodeGenFunction::CGCilkForStmtInfo *SI) {
  // create RAII object with required CapturedStmtInfo
  InlinedCilkForRegion CFRegion(CGF, SI);
  // get LoopCount value
  return CGF.EmitAnyExpr(S.getLoopCount()).getScalarVal();
}

void CodeGenFunction::EmitCilkForStmt(const CilkForStmt &S,
                                      llvm::Value *Grainsize) {
  // if (cond) {
  //   count = loop_count;
  //   grainsize = gs;
  //   initialize context
  ...
  EmitBlock(ThenBlock);
  {
    ...
    llvm::Value *LoopCount = getLoopCountValue(*this, S, &CSInfo);
    ...
    llvm::FunctionType *FTy = 0;
    {
      ...
      uint64_t SizeInBits =
          getContext().getTypeSize(S.getLoopCount()->getType());
      if (SizeInBits <= 32u) {
        FTy = llvm::TypeBuilder<void(void(void *, uint32_t, uint32_t), void *,
                                     uint32_t, int),
                                false>::get(Ctx);
        CilkForABI = M.getOrInsertFunction("__cilkrts_cilk_for_32", FTy);
      } else if (SizeInBits <= 64u) {
        FTy = llvm::TypeBuilder<void(void(void *, uint64_t, uint64_t), void *,
                                     uint64_t, int),
                                false>::get(Ctx);
    ...
    if (!isa<DeclStmt>(S.getInit())) {
      ...
      } else if (const BinaryOperator *Inc =
                     dyn_cast<BinaryOperator>(IncExpr)) {
...
void CodeGenFunction::CGPragmaSimd::emitInit(CodeGenFunction &CGF,
  ...
  const VarDecl *LoopControlVar =
      cast<SIMDForStmt>(this->getStmt())->getLoopControlVar();
  ...
  LCVStrideVal = CGF.EmitAnyExpr(cast<SIMDForStmt>(this->getStmt())
                                     ->getLoopStride()).getScalarVal();
...
void CodeGenFunction::CGPragmaSimd::emitIntelIntrinsic(
    CodeGenFunction *CGF, CodeGenModule *CGM, llvm::Value *LoopIndex,
...
static Expr *GetLinearStep(const SIMDForStmt &SS, VarDecl *SIMDVar) {
  ...
  for (ArrayRef<Attr *>::iterator I = Attrs.begin(), E = Attrs.end(); I != E;
       ++I) {
    Attr *A = *I;
...
bool CodeGenFunction::CGPragmaSimd::walkLocalVariablesToEmit(
  ...
  for (SIMDForStmt::simd_var_iterator I = SS.simd_var_begin(),
    ...
    if (I->isLinear()) {
      ...
      LValue LV =
          CGF->MakeNaturalAlignAddrLValue(Info->getContextValue(), TagType);
        ...
        for (SIMDForStmt::simd_var_iterator I = SimdFor->simd_var_begin(),
          ...
          if (I->isLinear() || I->isLastPrivate()) {
            EmitUpdateExpr(*this, I->getUpdateExpr(),
                           I->getLocalVar()->getType(), I->getArrayIndexVars(),
                           0);
          }
        }
...
static void EmitRecursiveCilkRankedStmt(CodeGenFunction &CGF,
  ...
  if (Rank < S.getRank()) {
    ...
    if (isSigned) {
      ...
      CGF.Builder.CreateCondBr(
          CGF.Builder.CreateICmpSGT(
              ValLength, llvm::ConstantInt::getSigned(ValLength->getType(), 0)),
          EnterLoop, ExitLoop);
      CGF.EmitBlock(EnterLoop);
    }
    if (Rank == S.getRank() - 1) {
      for (Stmt::const_child_range ChRange = S.getInits()->children(); ChRange;
           ++ChRange) {
    ...
    if (Rank == S.getRank() - 1) {
      ...
      Args.push_back(
          llvm::MDString::get(CGF.CGM.getLLVMContext(), "CEAN_LOOP"));
      llvm::Value *RealArg = llvm::MetadataAsValue::get(
    ...
    CGF.Builder.CreateCondBr(
        CGF.Builder.CreateICmpNE(
            CGF.Builder.CreateLoad(CGF.GetAddrOfLocalVar(VD)), ValLength),
        MainLoop, ExitLoop);
...
void CodeGenFunction::EmitPragmaStmt(const PragmaStmt &S) {
  ...
  case (IntelPragmaInitSeg): {
    ...
    /*fakeCall =*/Builder.CreateCall(
        fn, llvm::MetadataAsValue::get(CGM.getLLVMContext(), Node));
    // Special processing for pragma inline
