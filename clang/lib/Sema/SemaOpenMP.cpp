...
void Sema::ActOnOpenMPRegionStart(OpenMPDirectiveKind DKind, Scope *CurScope) {
  ...
  case OMPD_target_parallel_loop: {
    ...
    // Start a captured region for 'parallel'.
    ActOnCapturedRegionStart(DSAStack->getConstructLoc(), CurScope, CR_OpenMP,
                             ParamsParallel, /*OpenMPCaptureLevel=*/2);
    Sema::CapturedParamNameType ParamsLoop[] = {
        std::make_pair(StringRef(), QualType()) // __context with shared vars
    };
    // Start a captured region for 'loop' with no implicit parameters.
    ActOnCapturedRegionStart(DSAStack->getConstructLoc(), CurScope, CR_OpenMP,
                             ParamsLoop, /*OpenMPCaptureLevel=*/3);
    break;
  }
  case OMPD_parallel_loop: {
    ...
    // Start a captured region for 'loop' with no implicit parameters.
    ActOnCapturedRegionStart(DSAStack->getConstructLoc(), CurScope, CR_OpenMP,
                             ParamsLoop, /*OpenMPCaptureLevel=*/1);
    break;
  }
...
Expr *OpenMPIterationSpaceChecker::buildNumIterations(
  ...
  ExprResult Diff = calculateNumIters(SemaRef, S, DefaultLoc, Lower, Upper,
                                      Step, VarType, StepIsPositive, //INTEL
                                      TestIsStrictOp,                //INTEL
...
public:
  ...
  bool checkCondExprStmt(BinaryOperator *BO, unsigned DiagId, unsigned NoteId);
  bool checkCondUpdateStmt(IfStmt *If, unsigned DiagId, unsigned NoteId);
...
bool OpenMPAtomicCompareChecker::checkCondExprStmt(BinaryOperator *BO,
  ...
  // case 1: x = expr ordop x ? expr : x;
  // case 2: x = x ordop expr ? expr : x;
  //    set: X, E and either IsCompareMin or IsCompareMax to true
  //
  // case 3: x = x == e ? d : x;
  //    set: X, Expected, Desired (IsCompareMin/IsCompareMax both false)
  //

  if (BO->getType()->isScalarType() || BO->isInstantiationDependent()) {
...
bool OpenMPAtomicCompareChecker::checkCondUpdateStmt(IfStmt *If,
  ...
  // case 1: if (expr ordop x) { x = expr; }
  // case 2: if (x ordop expr) { x = expr; }
  //    set: X, E and either IsCompareMin or IsCompareMax to true
  //
  // case 3: if(x == e) { x = d; }
  //    set: X, Expected, Desired (IsCompareMin/IsCompareMax both false)
...
bool OpenMPAtomicCompareChecker::checkStatement(Stmt *S, unsigned DiagId,
  ...
  if (auto *BO = dyn_cast<BinaryOperator>(S)) {
    if (checkCondExprStmt(BO, DiagId, NoteId))
      return true;
  } else if (auto *If = dyn_cast<IfStmt>(S)) {
    if (checkCondUpdateStmt(If, DiagId, NoteId))
      return true;
  } else {
...
public:
  ...
  /// Return the 'r' lvalue part of the source atomic expression.
...
=======
    ...
    case OMPC_update:
    case OMPC_capture: {
>>>>>>> d762c3d905baf18d0f17a24799c9c4517b3c95fa
...
StmtResult Sema::ActOnOpenMPUnrollDirective(ArrayRef<OMPClause *> Clauses,
  ...
  LoopHintAttr *UnrollHintAttr =
      LoopHintAttr::CreateImplicit(Context, LoopHintAttr::UnrollCount,
                                   LoopHintAttr::Numeric, MakeFactorExpr(), // INTEL
                                   /*LoopExprValue=*/ nullptr);             // INTEL
...
static OpenMPDirectiveKind getOpenMPCaptureRegionForClause(
  ...
  case OMPC_ompx_places:
    ...
    case OMPD_target_data:
    case OMPD_target_variant_dispatch:
      // Do not capture ompx_places-clause expressions.
      break;
    case OMPD_teams_distribute_parallel_for:
    case OMPD_teams_distribute_parallel_for_simd:
    case OMPD_teams:
    ...
    case OMPD_distribute_simd:
    case OMPD_requires:
      llvm_unreachable("Unexpected OpenMP directive with ompx_places clause");
    case OMPD_unknown:
    default:
      llvm_unreachable("Unknown OpenMP directive");
    }
...
OMPClause *Sema::ActOnOpenMPOmpxPlacesClause(
  ...
  // Start value is only required argument to ompx_places clause.
  if (!StartExpr)
    return nullptr;

  //  The start expression must evaluate to a non-negative integer value.
  if (!isNonNegativeIntegerValue(StartExpr, *this, OMPC_ompx_places,
                                 /*StrictlyPositive=*/false))
    return nullptr;
  ...
  OpenMPDirectiveKind DKind = DSAStack->getCurrentDirective();
  OpenMPDirectiveKind CaptureRegion =
      getOpenMPCaptureRegionForClause(DKind, OMPC_ompx_places, LangOpts.OpenMP);
  if (CaptureRegion != OMPD_unknown && !CurContext->isDependentContext()) {
    llvm::MapVector<const Expr *, DeclRefExpr *> Captures;
    StartExpr = MakeFullExpr(StartExpr).get();
    StartExpr = tryBuildCapture(*this, StartExpr, Captures).get();
  ...
  return new (Context)
      OMPOmpxPlacesClause(Modifier, StartExpr, LengthExpr, StrideExpr,
                          HelperValStmt, CaptureRegion, StartLoc, EndLoc);
}
