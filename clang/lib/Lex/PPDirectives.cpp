...
static void getTypelibGuid(const char *Name, GUID *GuidPtr,
  ...
  if (OriginalName.starts_with_insensitive("libid:")) {
    Name += 6;
    while (isspace(*Name))
      Name++;
    strcpy(GuidStr, "{");
    ...
    strcat(GuidStr, "}");
  } else if (OriginalName.starts_with_insensitive("progid:")) {
...
static std::string getTypelibName(StringRef Filename, unsigned short Major,
  ...
  if (Guid == GUID_NULL) {
    ...
    if (ResultRef.starts_with_insensitive("libid:") ||
        ResultRef.starts_with_insensitive("progid:"))
      return "";
  }
...
void Preprocessor::HandleMicrosoftImportIntelDirective(SourceLocation HashLoc,
  ...
  llvm::Expected<FileEntryRef> ExpectedFileRef =
      HeaderInfo.getFileMgr().getFileRef(HeaderFilename,
                                         /*OpenFile=*/true);
  if (ExpectedFileRef) {
    FileEntryRef FE = ExpectedFileRef.get();
    CharSourceRange FilenameRange =
        CharSourceRange::getCharRange(FilenameTok.getLocation(), CharEnd);
    ...
    FileID FID =
        SourceMgr.createFileID(FE, FilenameTok.getLocation(), SrcMgr::C_System);
