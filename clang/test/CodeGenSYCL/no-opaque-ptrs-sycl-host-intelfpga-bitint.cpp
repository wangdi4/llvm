...
// RUN: %clang_cc1 -no-opaque-pointers -fsycl-is-host -fintelfpga -triple x86_64-pc-windows-msvc -aux-triple spir64_fpga -emit-llvm %s -o - | FileCheck %s
...
// CHECK: define{{.*}} void @{{.*}}foo{{.*}}(i4096* {{.*}} sret(i4096) align 8 %agg.result, i4096* {{.*}} %[[ARG1:[0-9]+]], i4096* {{.*}} %[[ARG2:[0-9]+]])
signed _BitInt(4096) foo(signed _BitInt(4096) a, signed _BitInt(4096) b) {
  // CHECK-DAG: %[[VAR_A:a]].addr = alloca i4096, align 8
  // CHECK-DAG: %[[VAR_B:b]].addr = alloca i4096, align 8
  // CHECK: %[[VAR_A]] = load i4096, i4096* %[[ARG1]], align 8
  // CHECK: %[[VAR_B]] = load i4096, i4096* %[[ARG2]], align 8
  // CHECK-DAG: store i4096 %[[VAR_A]], i4096* %[[VAR_A]].addr, align 8
  // CHECK-DAG: store i4096 %[[VAR_B]], i4096* %[[VAR_B]].addr, align 8
