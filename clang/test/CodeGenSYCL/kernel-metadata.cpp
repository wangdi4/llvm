...
// RUN: %clang_cc1 -fsycl-is-device -triple spir64_x86_64-unknown-unknown-sycldevice -emit-llvm %s -o - | FileCheck %s -check-prefix=CHECK-CPU
// CHECK-CPU: define {{.*}}spir_kernel void @{{.*}}kernel_function{{.*}} !kernel_arg_addr_space {{.*}} !kernel_arg_access_qual {{.*}} !kernel_arg_type {{.*}} !kernel_arg_base_type {{.*}} !kernel_arg_type_qual {{.*}}
