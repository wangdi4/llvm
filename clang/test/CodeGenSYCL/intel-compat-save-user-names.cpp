// RUN:  %clang_cc1 -fintel-compatibility -fsycl-is-device -internal-isystem %S/Inputs -triple spir64-unknown-unknown -disable-llvm-passes -emit-llvm %s -o - | FileCheck %s

// Test to verify that user specified names are retained in SPIR
// kernel argument names.

// This test is cloned from the test save-user-names.cpp, and uses the -fintel-compatibility flag in addition.
...
int main() {
  ...
  q.submit([&](sycl::handler &cgh) {
    ...
    NestedSimple NestedSimpleObj;
    NestedComplex NestedComplexObj;
    sycl::accessor<char, 1, sycl::access::mode::read> CapturedAcc1, CapturedAcc2;
...
// INTEL_CUSTOMIZATION
// CHECK-SAME: %_arg_AccField1{{.*}}%_arg_AccField11{{.*}}%_arg_AccField12{{.*}}%_arg_AccField14
// CHECK-SAME: %_arg_AccField2{{.*}}%_arg_AccField26{{.*}}%_arg_AccField27{{.*}}%_arg_AccField29
// end INTEL_CUSTOMIZATION
// CHECK-SAME: %_arg_NestedSimpleObj
// CHECK-SAME: %_arg_NestedComplexField
// INTEL_CUSTOMIZATION
// CHECK-SAME: %_arg_NestedAccField{{.*}}%_arg_NestedAccField11{{.*}}%_arg_NestedAccField12{{.*}}%_arg_NestedAccField14
// end INTEL_CUSTOMIZATION
...
// INTEL_CUSTOMIZATION
// CHECK-SAME: %_arg_CapturedAcc1{{.*}}%_arg_CapturedAcc11{{.*}}%_arg_CapturedAcc12{{.*}}%_arg_CapturedAcc14
// CHECK-SAME: %_arg_CapturedAcc2{{.*}}%_arg_CapturedAcc26{{.*}}%_arg_CapturedAcc27{{.*}}%_arg_CapturedAcc29
// end INTEL_CUSTOMIZATION
// CHECK-SAME: %_arg_NestedSimpleObj
// CHECK-SAME: %_arg_NestedComplexField
// INTEL_CUSTOMIZATION
