...
// INTEL_CUSTOMIZATION
// RUN: %clangxx -fsycl -target x86_64 -O2 -fsycl-device-only %s -flegacy-pass-manager -mllvm -debug-pass=Structure -emit-llvm -o - 2>&1 | FileCheck %s --check-prefixes=CHECK-EARLYOPT --implicit-check-not "Transform sin and cos calls"
...
// INTEL_CUSTOMIZATION
// RUN: %clangxx -fsycl -target x86_64 -O2 -fsycl -fsycl-device-only -fno-sycl-early-optimizations %s -flegacy-pass-manager -mllvm -debug-pass=Structure -emit-llvm -o - 2>&1 | FileCheck %s --check-prefix=CHECK-NOEARLYOPT --implicit-check-not "Transform sin and cos calls"
// end INTEL_CUSTOMIZATION
...
//
//
>>>>>>> 36f0cc8f92c1ac89c0c6d6278863f371aff008e2
