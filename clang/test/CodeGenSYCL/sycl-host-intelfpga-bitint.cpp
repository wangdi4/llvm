...
// RUN: %clang_cc1 -fsycl-is-host -fintelfpga -triple x86_64-pc-windows-msvc -aux-triple spir64_fpga -emit-llvm %s -o - | FileCheck %s
...
// CHECK: define{{.*}} void @{{.*}}foo{{.*}}(ptr {{.*}} sret(i4096) align 8 %agg.result, ptr {{.*}} %[[ARG1:[0-9]+]], ptr {{.*}} %[[ARG2:[0-9]+]])
signed _BitInt(4096) foo(signed _BitInt(4096) a, signed _BitInt(4096) b) {
  ...
  // CHECK-DAG: store i4096 %[[VAR_A]], ptr %[[VAR_A]].addr, align 8
  // CHECK-DAG: store i4096 %[[VAR_B]], ptr %[[VAR_B]].addr, align 8
