...
// CHECK: define dso_local spir_kernel void @_ZTS4fooA({{.*}}%[[ARG:.*]])
...
// CHECK: %[[ADDR_CAST:.*]] = addrspacecast ptr addrspace(1) %[[LOAD_ARG]] to ptr addrspace(4)
...
// CHECK-DAG: %[[GEP:.*]] = getelementptr inbounds
// CHECK-NEXT: store float %[[LOAD_ARG]], ptr %[[GEP]], align 4
    fooB(float _f) : f(_f) {}
    void operator()() const {}
};
