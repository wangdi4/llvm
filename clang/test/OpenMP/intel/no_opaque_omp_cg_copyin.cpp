// RUN: %clang_cc1 -no-opaque-pointers -emit-llvm -o - -fopenmp -fintel-compatibility -fopenmp-late-outline -triple x86_64-unknown-linux-gnu %s | FileCheck %s
// RUN: %clang_cc1 -no-opaque-pointers -emit-llvm -o - -fopenmp -fintel-compatibility -fopenmp-late-outline -fopenmp-threadprivate-legacy -triple x86_64-unknown-linux-gnu %s| FileCheck %s -check-prefix CHECK-TPL
