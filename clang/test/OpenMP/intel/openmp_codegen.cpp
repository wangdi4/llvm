...
#pragma omp parallel for shared(argv) private(s1, s2, arr1, arr2, arr3) \
                         linear(val(argc) : argc)
  for (int i = 0; i < argc; ++i)
    // CHECK: call void @_Z3foov()
    foo();
// CHECK: region.exit{{.*}}"DIR.OMP.END.PARALLEL.LOOP"()

// NOCHECK: [[TARGCREF:%.+]] = load i32*, i32** [[ARGCREF_ADDR]],
// NOCHECK-NEXT: store i32* [[TARGCREF]], i32** [[ARGCREFTMP:%tmp[0-9]+]]
...
#pragma omp simd private(s1, s2, arr1, arr2, arr3) linear(uval(argcref)) \
                 linear(argv)
  for (int i = 0; i < argc; ++i)
    // CHECK: call void @_Z3foov()
    foo();
...
#pragma omp parallel reduction(+:n1) reduction(-:n2) reduction(max:n3) \
                     reduction (min:n4) reduction(&:n5) reduction (|:n6) \
                     reduction(&&:n7) reduction(||:n8) reduction(^:n9) \
                     reduction(*:n10)
  // CHECK: call void @_Z3foov()
  foo();
// CHECK: region.exit{{.*}}"DIR.OMP.END.PARALLEL"()
...
#pragma omp parallel for schedule(nonmonotonic,simd:dynamic,16)
  ...
  long **input2 = 0;
  long result[M][M][N];
// Add VLA tests here when we resolve ARRSIZE issue
// CHECK: [[TV:%[0-9]+]] = call token{{.*}}region.entry() [ "DIR.OMP.TARGET"()
// CHECK: region.exit(token [[TV]]) [ "DIR.OMP.END.TARGET"() ]
#pragma omp target map(to : input1[2:N], input2[0:N][:M]) \
                   map(from : result[2:][:M][0:])
...
// CHECK: [[TV:%[0-9]+]] = call {{.*}}region.entry() [ "DIR.OMP.TARGET.DATA"(),
// CHECK-SAME: "QUAL.OMP.MAP.TOFROM"(i32* %z{{.*}}),
// CHECK-SAME: "QUAL.OMP.USE_DEVICE_PTR"(i32** %a{{.*}}, i32** %b{{.*}}) ]
// CHECK: region.exit(token [[TV]]) [ "DIR.OMP.END.TARGET.DATA"() ]
    #pragma omp target data map(tofrom:z) use_device_ptr(a,b)
    {
    }
// CHECK: [[TV:%[0-9]+]] = call {{.*}}region.entry{{.*}}"DIR.OMP.TARGET.UPDATE"
// CHECK-SAME: "QUAL.OMP.TO"(i32* %z{{.*}}), "QUAL.OMP.FROM"(i32* %y{{.*}}) ]
// CHECK: region.exit(token [[TV]]) [ "DIR.OMP.END.TARGET.UPDATE"() ]
    #pragma omp target update to(z) from(y)
  }
  {
    int local_int = 1;
// CHECK: [[TV:%[0-9]+]] = call token{{.*}}DIR.OMP.TARGET
...
// CHECK: region.exit(token [[TV]]) [ "DIR.OMP.END.TARGET"() ]
    ...
    S4 s4;
// CHECK: [[TV:%[0-9]+]] = call token{{.*}}DIR.OMP.TARGET
// CHECK-SAME: MAP.TOFROM{{.*}}%struct.S4
// CHECK: region.exit(token [[TV]]) [ "DIR.OMP.END.TARGET"() ]
    #pragma omp target
    {
      s4.i = 1;
...
// CHECK: [[TV:%[0-9]+]] = call token{{.*}}DIR.OMP.TARGET
// CHECK-SAME: DEFAULTMAP.TOFROM.SCALAR{{.*}}MAP.TOFROM{{.*}}glob_int
// CHECK-SAME: MAP.TOFROM{{.*}}local_int
// CHECK: region.exit(token [[TV]]) [ "DIR.OMP.END.TARGET"() ]
...
// CHECK: [[TV:%[0-9]+]] = call token{{.*}}region.entry() [ "DIR.OMP.TARGET"()
// CHECK: [[TE_TV:%[0-9]+]] = call token{{.*}}region.entry() [ "DIR.OMP.TEAMS
// CHECK-SAME: "QUAL.OMP.NUM_TEAMS"(i32 16), "QUAL.OMP.THREAD_LIMIT"(i32 4)
...
// CHECK: region.exit(token [[FLUSH_TOKENVAL]]) [ "DIR.OMP.END.FLUSH"
  ...
  {
    int fli = 3, flj = 4;
// CHECK: [[TV:%[0-9]+]] = call token{{.*}}DIR.OMP.FLUSH
// CHECK-SAME: QUAL.OMP.FLUSH{{.*}}fli{{.*}}flj
// CHECK: region.exit(token [[TV]]) [ "DIR.OMP.END.FLUSH"
    #pragma omp flush(fli,flj)
  }

// CHECK: [[SECT1ATV:%[0-9]+]] = call token{{.*}}DIR.OMP.PARALLEL
// CHECK: [[SECT1BTV:%[0-9]+]] = call token{{.*}}DIR.OMP.SECTIONS
// CHECK-SAME: NOWAIT{{.*}}PRIVATE{{.*}}sect1{{.*}}FIRSTPRIVATE{{.*}}sect2
// CHECK-SAME: LASTPRIVATE{{.*}}sect3{{.*}}REDUCTION.ADD{{.*}}sect4
...
// CHECK: [[SECT2BTV:%[0-9]+]] = call token{{.*}}DIR.OMP.PARALLEL.SECTIONS
// CHECK-SAME: PRIVATE{{.*}}sect1{{.*}}FIRSTPRIVATE{{.*}}sect2
// CHECK-SAME: LASTPRIVATE{{.*}}sect3{{.*}}REDUCTION.ADD{{.*}}sect4
...
// CHECK: define internal [10 x %struct.S1]*
// CHECK-SAME: @_ZTSA10_2S1.omp.def_constr([10 x %struct.S1]*)
...
// CHECK: define internal [10 x %struct.S2]*
// CHECK-SAME: @_ZTSA10_2S2.omp.def_constr([10 x %struct.S2]*)
