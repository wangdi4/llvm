...
//RUN: %clang_cc1 -no-opaque-pointers -emit-llvm -o - %s -std=c++14 -fopenmp -fintel-compatibility -fopenmp-late-outline -triple x86_64-pc-windows-msvc | FileCheck %s
//RUN: %clang_cc1 -no-opaque-pointers -emit-llvm -o - %s -std=c++14 -fopenmp -fintel-compatibility -fopenmp-late-outline -triple i386-pc-windows-msvc | FileCheck %s
