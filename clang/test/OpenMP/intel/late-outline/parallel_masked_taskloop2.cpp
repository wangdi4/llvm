...
#ifdef COMBINED
#pragma omp parallel masked taskloop filter(tid) if(parallel: a) default(none) shared(a, b, argc) final(b) priority(5) num_tasks(argc) reduction(*: g)
#else
#pragma omp parallel if (a)
#pragma omp masked taskloop filter(tid) default(none) shared(a, b, argc) final(b) priority(5) num_tasks(argc) reduction(*: g)
#endif
  for (int i = 0; i < 2; ++i)
    a = 2;
  ...
  // CHECK-NEXT: DIR.OMP.END.PARALLEL
#ifdef COMBINED
#pragma omp parallel masked taskloop filter(tid) private(argc, b), firstprivate(argv, c), lastprivate(d, f) collapse(2) shared(g) if(argc) mergeable priority(argc) grainsize(argc) reduction(max: a, e)
#else
#pragma omp parallel if (argc)
#pragma omp masked taskloop filter(tid) private(argc, b), firstprivate(argv, c), lastprivate(d, f) collapse(2) shared(g) if(argc) mergeable priority(argc) grainsize(argc) reduction(max: a, e)
#endif
  for (int i = 0; i < 10; ++i)
    for (int j = 0; j < 10; ++j)
      foo();
