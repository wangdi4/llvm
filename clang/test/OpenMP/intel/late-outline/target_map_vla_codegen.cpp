...
int vla_test() {
  auto array_size = 100;
  int *C[array_size];
  // CHECK: [[L1:%[0-9]+]] = load i32, ptr %array_size, align 4
  // CHECK-NEXT: [[L2:%[0-9]+]] = zext i32 [[L1]]
  // CHECK: [[VLA_ADDR:%.+]] = alloca ptr,
...
int vla_test1() {
  ...
  // CHECK: [[L1:%[0-9]+]] = load i32, ptr %array_size, align 4
  // CHECK-NEXT: [[L2:%[0-9]+]] = zext i32 [[L1]]
  // CHECK: [[L3:%[0-9]+]] = load i32, ptr %aaa, align 4
...
int vla_test2() {
  ...
  // CHECK: [[ARRIDX:%.+]] = getelementptr inbounds i32, ptr %vla, i64 %6
  // CHECK: [[L6:%[0-9]+]] = mul nuw i64 [[L2]], [[L4]]
  // CHECK-NEXT: [[SIZE:%[0-9]+]] = mul nuw i64 [[L6]], 4
  // CHECK: [[T:%[0-9]+]] = {{.*}}region.entry{{.*}}DIR.OMP.TARGET
  // CHECK-SAME: "QUAL.OMP.MAP.FROM:VARLEN"(ptr [[VLA_ADDR]], ptr [[ARRIDX]], i64 [[SIZE]],  i64 34
...
int vla_test3() {
  ...
  auto aaa = 10;
  int C[array_size][aaa];
  // CHECK: [[L1:%[0-9]+]] = load i32, ptr %aaa, align 4
  // CHECK-NEXT: [[L2:%[0-9]+]] = zext i32 [[L1]]
  // CHECK: [[VLA_ADDR:%.+]] = alloca i32,
