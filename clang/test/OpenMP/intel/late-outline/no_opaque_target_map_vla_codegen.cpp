...
int vla_test() {
  ...
  // CHECK-SAME: "QUAL.OMP.MAP.FROM:VARLEN"(i32** [[VLA_ADDR]], i32** [[ARRIDX]], i64 [[SIZE]], i64 34
...
int vla_test3() {
  ...
  // CHECK: [[T:%[0-9]+]] = {{.*}}region.entry{{.*}}DIR.OMP.TARGET
  // CHECK-SAME: "QUAL.OMP.MAP.FROM:VARLEN"(i32* [[VLA_ADDR]], i32* %arrayidx2, i64 [[SIZE]], i64 34
  // CHECK: region.exit(token [[T]]) [ "DIR.OMP.END.TARGET"() ]
...
int vla_test4() {
  ...
  int C[array_size][aaa];
  // CHECK: [[T:%[0-9]+]] = {{.*}}region.entry{{.*}}DIR.OMP.TARGET
  // CHECK-SAME: "QUAL.OMP.MAP.FROM:VARLEN"(i32* %vla, i32* %arrayidx2, i64 4, i64 34
  // CHECK: region.exit(token [[T]]) [ "DIR.OMP.END.TARGET"() ]
