...
#else
#pragma omp parallel
#pragma omp master taskloop simd if(a) default(none) shared(a, b, argc) final(b) priority(5) num_tasks(argc) reduction(*: g) aligned(argv: 8) linear(c:b)
#endif
  for (int i = 0; i < 2; ++i)
    a = 2;
...
#else
#pragma omp parallel reduction(max: a, e)
