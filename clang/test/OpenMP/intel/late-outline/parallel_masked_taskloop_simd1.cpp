...
#ifdef COMBINED
#pragma omp parallel masked taskloop simd filter(tid) if(taskloop: argc > N) default(shared) untied priority(N) grainsize(N) reduction(+:g) allocate(g) simdlen(8)
#else
#pragma omp parallel default(shared)
#pragma omp masked taskloop simd filter(tid) if(taskloop: argc > N) default(shared) untied priority(N) grainsize(N) reduction(+:g) allocate(g) simdlen(8)
#endif
  for (int i = 0; i < 2; ++i)
    a = 2;
  ...
  // CHECK-NEXT: DIR.OMP.END.PARALLEL
#ifdef COMBINED
#pragma omp parallel masked taskloop simd filter(tid) private(argc, b), firstprivate(c, d), lastprivate(d, f) collapse(N) shared(g) if (c) final(d) mergeable priority(f) nogroup num_tasks(N) safelen(8)
#else
#pragma omp parallel if (c)
#pragma omp masked taskloop simd filter(tid) private(argc, b), firstprivate(c, d), lastprivate(d, f) collapse(N) shared(g) if (c) final(d) mergeable priority(f) nogroup num_tasks(N) safelen(8)
#endif
  for (int i = 0; i < 2; ++i)
    for (int j = 0; j < 2; ++j)
      for (int j = 0; j < 2; ++j)
