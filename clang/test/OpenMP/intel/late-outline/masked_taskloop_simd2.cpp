...
#ifdef COMBINED
#pragma omp masked taskloop simd filter(tid) if(taskloop: a) default(none) shared(a) final(b) priority(5) safelen(8) linear(b), aligned(argv) num_tasks(argc) reduction(*: g) in_reduction(+: d)
#else
...
#pragma omp taskloop simd if(taskloop: a) default(none) shared(a) final(b) priority(5) safelen(8) linear(b), aligned(argv) num_tasks(argc) reduction(*: g) in_reduction(+: d)
  ...
  // CHECK: DIR.OMP.END.MASKED
#ifdef COMBINED
#pragma omp masked taskloop simd filter(tid) private(argv, b), firstprivate(argc, c), lastprivate(d, f) collapse(2) shared(g) if(argc) mergeable priority(argc) simdlen(16) grainsize(argc) reduction(max: a, e)
#else
#pragma omp masked filter(tid)
