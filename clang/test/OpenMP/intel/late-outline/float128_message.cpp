...
#pragma omp target map(f)
  // expected-error@+1 3{{'f' requires 128 bit size '__float128' type support, but device 'spir64' does not support it}}
  f = 1.0000;
  return f;
}

// expected-note@+1 3{{'d' defined here}}
long double zoo(long double d)
{
#pragma omp target map(d)
  // expected-error@+1 3{{'d' requires 128 bit size 'long double' type support, but device 'spir64' does not support it}}
  d = 1.0;
  return d;
}
// end INTEL_COLLAB
