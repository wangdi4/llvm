...
int main(int argc, char **argv) {
  ...
  // check-same: QUAL.OMP.INREDUCTION.MUL same here about reduction
  // CHECK: DIR.OMP.SIMD
  // CHECK-SAME: QUAL.OMP.REDUCTION.MUL
...
#ifdef COMBINED
#pragma omp parallel masked taskloop simd filter(tid) if(parallel: a) default(none) shared(a, b, argc) final(b) priority(5) num_tasks(argc) reduction(*: g) aligned(argv: 8) linear(c:b)
#else
#pragma omp parallel if (a)
#pragma omp masked taskloop simd filter(tid) default(none) shared(a, b, argc) final(b) priority(5) num_tasks(argc) reduction(*: g) aligned(argv: 8) linear(c:b)
#endif
  for (int i = 0; i < 2; ++i)
    a = 2;
  ...
  // CHECK-NEXT: DIR.OMP.END.PARALLEL
#ifdef COMBINED
#pragma omp parallel masked taskloop simd filter(tid) private(argc, b), firstprivate(argv, c), lastprivate(d, f) collapse(2) shared(g) if(argc) mergeable priority(argc) grainsize(argc) reduction(max: a, e)
#else
#pragma omp parallel if (argc) reduction(max: a, e)
#pragma omp masked taskloop simd filter(tid) private(argc, b), firstprivate(argv, c), lastprivate(d, f) collapse(2) shared(g) if(argc) mergeable priority(argc) grainsize(argc) reduction(max: a, e)
#endif
  for (int i = 0; i < 10; ++i)
    for (int j = 0; j < 10; ++j)
      foo();
