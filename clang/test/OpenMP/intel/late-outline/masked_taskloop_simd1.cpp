...
#pragma omp masked taskloop simd filter(tid) allocate(d) if(taskloop: argc > N) default(shared) untied priority(N) safelen(N) simdlen(N) linear(c) aligned(ptr) grainsize(N) reduction(+:g) in_reduction(+: d) mergeable final(d)
#else
#pragma omp taskgroup task_reduction(+: d) allocate(d)
...
#pragma omp taskloop simd allocate(d) if(taskloop: argc > N) default(shared) untied priority(N) safelen(N) simdlen(N) linear(c) aligned(ptr) grainsize(N) reduction(+:g) in_reduction(+: d) mergeable final(d)
  ...
  // CHECK: DIR.OMP.END.MASKED
#ifdef COMBINED
#pragma omp masked taskloop simd filter(tid) private(argc, b), firstprivate(c, d), lastprivate(d, f) collapse(N) shared(g) if (c) final(d) mergeable priority(f) simdlen(N) nogroup num_tasks(N)
#else
#pragma omp masked filter(tid)
