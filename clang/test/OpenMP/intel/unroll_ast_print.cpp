...
// RUN: %clang_cc1 -triple x86_64-pc-linux-gnu -fopenmp -fsyntax-only \
// RUN: -fopenmp-late-outline -fopenmp-intel-unroll -verify %s
// expected-no-diagnostics

// Check AST and unparsing
// RUN: %clang_cc1 -triple x86_64-pc-linux-gnu -fopenmp -ast-dump  \
// RUN: -fopenmp-late-outline -fopenmp-intel-unroll %s | FileCheck \
// RUN: %s --check-prefix=DUMP

// RUN: %clang_cc1 -triple x86_64-pc-linux-gnu -fopenmp -ast-print %s \
// RUN: -fopenmp-late-outline -fopenmp-intel-unroll | FileCheck %s \
// RUN: --check-prefix=PRINT --match-full-lines

// Check same results after serialization round-trip
// RUN: %clang_cc1 -triple x86_64-pc-linux-gnu -fopenmp -emit-pch \
// RUN: -fopenmp-late-outline -fopenmp-intel-unroll -o %t %s

// RUN: %clang_cc1 -triple x86_64-pc-linux-gnu -fopenmp -include-pch %t \
// RUN: -fopenmp-late-outline -fopenmp-intel-unroll -ast-dump-all %s \
// RUN: | FileCheck %s --check-prefix=DUMP

// RUN: %clang_cc1 -triple x86_64-pc-linux-gnu -fopenmp -include-pch %t \
// RUN: -fopenmp-late-outline -fopenmp-intel-unroll -ast-print  %s \
// RUN: | FileCheck %s --check-prefix=PRINT --match-full-lines
