...
void TestCharString() {
  ...
  // CHECK_INTEL: invoke void @_CxxThrowException({{.*}} @_TIC2PEAD)
  // CHECK_INTEL_NOPEDANTIC: invoke void @_CxxThrowException({{.*}} @_TI2PEAD)
...
void TestWCharString() {
  ...
  // CHECK_INTEL: invoke void @_CxxThrowException({{.*}} @_TIC2PEA_W)
  // CHECK_INTEL_NOPEDANTIC: invoke void @_CxxThrowException({{.*}} @_TI2PEA_W)
...
void TestUTF16String() {
  try { throw u"abc"; }
  catch (const char16_t *s) {}
  // CHECK-LABEL: TestUTF16String
  // CHECK_INTEL: invoke void @_CxxThrowException({{.*}} @_TIC2PEA_S)
  // CHECK_INTEL_NOPEDANTIC: invoke void @_CxxThrowException({{.*}} @_TI2PEA_S)
...
void TestUTF32String() {
  try { throw U"abc"; }
  catch (const char32_t *s) {}
  // CHECK-LABEL: TestUTF32String
  // CHECK_INTEL: invoke void @_CxxThrowException({{.*}} @_TIC2PEA_U)
  // CHECK_INTEL_NOPEDANTIC: invoke void @_CxxThrowException({{.*}} @_TI2PEA_U)
