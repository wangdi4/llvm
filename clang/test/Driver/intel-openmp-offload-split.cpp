...
// CHK-COMMANDS: clang{{.*}} "-cc1" "-triple" "spir64" "-aux-triple" "x86_64-unknown-linux-gnu" "-disable-lifetime-markers" "-Wspir-compat" "-emit-llvm-bc"{{.*}} "-internal-isystem" "{{.*}}bin{{[/\\]+}}..{{[/\\]+}}include{{[/\\]+}}sycl"{{.*}} "-fopenmp"{{.*}} "-fsycl-instrument-device-code" {{.*}} "-fopenmp-is-target-device" "-fopenmp-host-ir-file-path" "[[BCFILE]]"{{.*}} "-mllvm" "-paropt=63"{{.*}} "-fopenmp-targets=spir64" "-o" "[[OFFBCFILE:.+\.bc]]"
...
// CHK-COMMANDS: sycl-post-link{{.*}} "-split=kernel" "--ompoffload-link-entries" "--ompoffload-sort-entries" "--ompoffload-make-globals-static" "-symbols" "-emit-exported-symbols" "-split-esimd" "-lower-esimd" "-O2" "-spec-const=native" "-device-globals" "-o" "[[POSTLINKFILE:.+\.table]]" "[[LINKEDBCFILE]]"
...
// CHK-UBJOBS: sycl-post-link{{.*}} "-split=kernel" "--ompoffload-link-entries" "--ompoffload-sort-entries" "--ompoffload-make-globals-static" "-symbols" "-emit-exported-symbols" "-split-esimd" "-lower-esimd" "-O2" "-spec-const=native" "-device-globals" "-o" "[[POSTLINKFILE:.+\.table]]" "[[LINKEDBCFILE]]"
