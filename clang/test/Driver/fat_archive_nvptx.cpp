...
// INTEL_CUSTOMIZATION
// RUN: not %clang -O2 -target x86_64-unknown-linux-gnu -### -fopenmp -fno-openmp-new-driver -fopenmp-targets=nvptx64-nvidia-cuda %s -L%S/Inputs/openmp_static_device_link -lFatArchive 2>&1 | FileCheck %s
...
// INTEL_CUSTOMIZATION
// CHECK: clang-nvlink-wrapper{{.*}}"-o" "{{.*}}.bc" "-arch" "[[GPU]]" "{{.*}}[[DEVICESPECIFICARCHIVE]]"
// end INTEL_CUSTOMIZATION
