...
// STATIC_LIB: clang-offload-bundler{{.*}} "-type=aoo" {{.*}} "-inputs=[[INPUTA]]" "-outputs=[[OUTLIB:.+\.a]]"
...
// STATIC_LIB_NOSRC: clang-offload-bundler{{.*}} "-type=aoo" "-targets=openmp-spir64" "-inputs=[[INPUTLIB]]" "-outputs=[[OUTLIB:.+\.a]]" "-unbundle"
