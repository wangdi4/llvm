...
// RUN: not %clang -c -Xclang -fintel-compatibility-doc= %s 2>&1 | FileCheck %s
// CHECK: invalid value '' in '-fintel-compatibility-doc='

// -fintel-compatibility-doc= must be followed by at least one valid option
// RUN: not %clang -c -Xclang -fintel-compatibility-doc=foobar %s 2>&1 | FileCheck --check-prefix=CHECK_WRONG2 %s
