...
// INTEL RUN: env CINDEXTEST_FAILONERROR=1 c-index-test -file-refs-at=%s:5:17 %s -include-pch %t.h.pch \
...
// INTEL RUN:   c-index-test -file-refs-at=%s:5:17 %s -include-pch %t.h.pch \
...
// INTEL RUN: env CINDEXTEST_FAILONERROR=1 c-index-test -file-refs-at=%S/targeted-top.h:14:7 %s -include-pch %t.h.pch \
// RUN:    -Xclang -error-on-deserialized-decl=NestedVar1  \
// RUN:  | FileCheck %s -check-prefix=TOP

// RUN: env CINDEXTEST_FAILONERROR=1 CINDEXTEST_EDITING=1 CINDEXTEST_COMPLETION_NO_CACHING=1 \
// INTEL RUN:   c-index-test -file-refs-at=%S/targeted-top.h:14:7 %s -include-pch %t.h.pch \
// RUN:    -Xclang -error-on-deserialized-decl=PreambleVar  \
// RUN:    -Xclang -error-on-deserialized-decl=NestedVar1  \
// RUN:  | FileCheck %s -check-prefix=TOP
...
// INTEL RUN: env CINDEXTEST_FAILONERROR=1 c-index-test -file-refs-at=%S/targeted-nested1.h:2:16 %s -include-pch %t.h.pch \
// RUN:    -Xclang -error-on-deserialized-decl=TopVar  \
// RUN:  | FileCheck %s -check-prefix=NESTED

// RUN: env CINDEXTEST_FAILONERROR=1 CINDEXTEST_EDITING=1 CINDEXTEST_COMPLETION_NO_CACHING=1 \
// INTEL RUN:   c-index-test -file-refs-at=%S/targeted-nested1.h:2:16 %s -include-pch %t.h.pch \
// RUN:    -Xclang -error-on-deserialized-decl=PreambleVar  \
// RUN:    -Xclang -error-on-deserialized-decl=TopVar  \
// RUN:  | FileCheck %s -check-prefix=NESTED
...
// RUN: env CINDEXTEST_FAILONERROR=1 CINDEXTEST_EDITING=1 CINDEXTEST_COMPLETION_NO_CACHING=1 \
// INTEL RUN:   c-index-test -file-refs-at=%S/targeted-preamble.h:2:15 %s -include-pch %t.h.pch \
// RUN:    -Xclang -error-on-deserialized-decl=NestedVar1  \
// RUN:    -Xclang -error-on-deserialized-decl=TopVar  \
