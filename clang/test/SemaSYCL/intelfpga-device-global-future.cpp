...
// This test - created from sycl-restrict.cpp - tests that
// when the error diagnostic: 
//   "SYCL kernel cannot use a non-const static data variable"
// is generated, and -fintelfpga option is enabled, we also
// generate the note:
//   "future revisions of the Intel FPGA Compiler will support
//    'device_global' variables, which retain state between invocations"

// This note is expected to be temporary and when the feature is supported
// in the future, this message (and this test too) will be removed.
...
typedef struct A {
  ...
  int fm(void) {
    ...
    // fpga-note@+1{{future revisions of the Intel FPGA Compiler will support 'device_global' variables, which retain state between invocations}}
    return stat_member; 
  }
} a_type;
