// RUN: %clang_cc1 -triple x86_64-unknown-linux-gnu -emit-llvm -no-opaque-pointers -o - %s | FileCheck %s -check-prefix=X64
// RUN: %clang_cc1 -triple i386-unknown-linux-gnu -emit-llvm -no-opaque-pointers -o - %s | FileCheck %s -check-prefix=X86
