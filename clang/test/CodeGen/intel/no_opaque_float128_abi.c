// RUN: %clang_cc1 -triple -x86_64-linux-gnu %s -emit-llvm -no-opaque-pointers -o - | FileCheck %s -check-prefix=LINUX
// RUN: %clang_cc1 -triple -x86_64-windows-msvc -fintel-compatibility %s --extended_float_types -emit-llvm -no-opaque-pointers -o - | FileCheck %s -check-prefix=WINDOWS
