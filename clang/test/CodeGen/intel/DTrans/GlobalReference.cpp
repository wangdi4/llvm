...
// RUN: %clang_cc1 -disable-llvm-passes -O2 -triple x86_64-linux-gnu -emit-dtrans-info -fintel-compatibility -emit-llvm %s -o - | FileCheck %s -check-prefixes=CHECK-LINUX,CHECK-LINUX-OPQ
// RUN: %clang_cc1 -disable-llvm-passes -O2 -triple x86_64-windows-msvc -emit-dtrans-info -fintel-compatibility -emit-llvm %s -o - | FileCheck %s -check-prefixes=CHECK-WINDOWS,CHECK-WINDOWS-OPQ
