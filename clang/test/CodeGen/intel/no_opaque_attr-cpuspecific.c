// RUN: %clang_cc1 -triple x86_64-linux-gnu -fintel-compatibility -emit-llvm -no-opaque-pointers -o - %s | FileCheck %s --check-prefixes=CHECK,LINUX
// RUN: %clang_cc1 -triple x86_64-windows-pc -fintel-compatibility -fms-compatibility -emit-llvm -no-opaque-pointers -o - %s | FileCheck %s --check-prefixes=CHECK,WINDOWS
...
// LINUX: [[INIT_BODY]]:
// LINUX-NEXT: call intel_features_init_cc void @__intel_cpu_features_init_x()
...
// WINDOWS: [[INIT_BODY]]:
// WINDOWS-NEXT: call intel_features_init_cc void @__intel_cpu_features_init_x()
