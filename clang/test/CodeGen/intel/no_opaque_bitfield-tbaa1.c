// RUN: %clang_cc1 -triple x86_64-apple-darwin -fintel-compatibility -fintel-compatibility-enable=IntelTBAABF -O1 -disable-llvm-optzns %s -emit-llvm -no-opaque-pointers -o - | FileCheck %s
// RUN: %clang_cc1 -triple x86_64-apple-darwin -fintel-compatibility -O1 -disable-llvm-optzns %s -emit-llvm -no-opaque-pointers -o - | FileCheck %s
// RUN: %clang_cc1 -triple x86_64-apple-darwin -fintel-compatibility -fintel-compatibility-disable=IntelTBAABF -O1 -disable-llvm-optzns %s -emit-llvm -no-opaque-pointers -o - | FileCheck %s -check-prefix=CHECK-DISABLED
struct in_t {
  int x,y,w;
