...
// RUN:-no-struct-path-tbaa -disable-llvm-optzns %s -emit-llvm -no-opaque-pointers -o - | \
// RUN:FileCheck %s
// RUN: %clang_cc1 -triple x86_64-apple-darwin -fintel-compatibility -O1 \
// RUN:                     -disable-llvm-optzns %s -emit-llvm -no-opaque-pointers -o - | \
// RUN:FileCheck %s -check-prefix=PATH
void foo(_Complex double *dest) {
  long l1;
  // CHECK: for.cond:{{.*}}
  // CHECK: {{.*}} load i64, i64* %l1, {{.*}} [[TAG_l1_tbaa:!.*]]
