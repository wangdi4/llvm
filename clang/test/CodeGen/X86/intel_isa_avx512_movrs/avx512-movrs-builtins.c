...
// RUN: %clang_cc1 -ffreestanding %s -triple=x86_64-apple-darwin -target-feature +avx512movrs -target-feature +avx512vl -emit-llvm -o - -Wall -Werror | FileCheck %s
...
__m512i test_mm512_vmovadvisew_load_epi8(__m512i const *a) {
  ...
  // CHECK: call <16 x i32> @llvm.x86.avx512.vmovadvisew.load.512(ptr %{{.*}}, i8 16)
...
void test_mm512_vmovadvisew_store_epi8(__m512i *a, __m512i m) {
  ...
  // CHECK: call void @llvm.x86.avx512.vmovadvisew.store.512(ptr %{{.*}}, <16 x i32> %{{.*}}, i8 16)
...
void test_mm512_vmemadvise_epi8(__m512i *a) {
  ...
  // CHECK: call void @llvm.x86.avx512.vmemadvise.512(ptr %{{.*}}, i8 16)
