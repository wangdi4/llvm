...
// RUN: %clang_cc1 %s -ffreestanding -triple=x86_64-unknown-unknown -target-feature +amx-tile -target-feature +amx-convert -emit-llvm -o - -Wall -Werror -pedantic \
...
void test_tile_amxconvert_cvtps2bf16(void *A, size_t B) {
  ...
  // CHECK: call void @llvm.x86.amxconvert.tcvtps2bf16(ptr %0, i64 %1, i8 2)
  _tile_amxconvert_cvtps2bf16(A, B, 2);
}

void test_tile_cvtps2ph(void *A, size_t B) {
  // CHECK-LABEL: @test_tile_cvtps2ph
  // CHECK: call void @llvm.x86.tcvtps2ph(ptr %0, i64 %1, i8 3)
