...
// RUN: %clang_cc1 -ffreestanding -flax-vector-conversions=none %s -triple=x86_64-unknown-unknown -target-feature +avx512bf16ne -emit-llvm -no-opaque-pointers -o - -Wall -Werror | FileCheck %s
...
__m128bf16 test_mm_load_sbf16(void const *A) {
  ...
  // CHECK: %{{.*}} = call <8 x bfloat> @llvm.masked.load.v8bf16.p0v8bf16(<8 x bfloat>* %{{.*}}, i32 1, <8 x i1> bitcast (<1 x i8> <i8 1> to <8 x i1>), <8 x bfloat> %{{.*}})
...
__m128bf16 test_mm_mask_load_sbf16(__m128bf16 __A, __mmask8 __U, const void *__W) {
  ...
  // CHECK: %{{.*}} = call <8 x bfloat> @llvm.masked.load.v8bf16.p0v8bf16(<8 x bfloat>* %{{.*}}, i32 1, <8 x i1> %{{.*}}, <8 x bfloat> %{{.*}})
...
void test_mm256_storeu_pbf16(void *p, __m256bf16 a) {
  ...
  // CHECK: store <16 x bfloat> %{{.*}}, <16 x bfloat>* %{{.*}}, align 1{{$}}
  // CHECK-NEXT: ret void
...
void test_mm_storeu_pbf16(void *p, __m128bf16 a) {
  ...
  // CHECK: store <8 x bfloat> %{{.*}}, <8 x bfloat>* %{{.*}}, align 1{{$}}
  // CHECK-NEXT: ret void
