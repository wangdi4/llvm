...
// RUN: %clang_cc1 -ffreestanding -triple x86_64-unknown-unknown -target-feature +prefetchst2 -emit-llvm -o - %s | FileCheck %s
...
void test_m_prefetch_s(void *p) {
  ...
  // CHECK: call void @llvm.prefetch.p0({{.*}}, i32 2, i32 1, i32 1)
}
