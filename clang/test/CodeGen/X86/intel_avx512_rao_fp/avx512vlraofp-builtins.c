...
// RUN: %clang_cc1 %s -ffreestanding -triple=x86_64-unknown-unknown -target-feature +avx512f -target-feature +avx512raofp -target-feature +avx512vl \
...
void test_mm_vaadd_pbh(__m128bh *__A, __m128bh __B) {
  ...
  // CHECK: call void @llvm.x86.vaaddpbf16128(ptr %{{.*}}, <8 x i16> %{{.*}})
...
void test_mm256_mask_vaadd_ps(__m256 *__A, __mmask8 __B, __m256 __C) {
  ...
  // CHECK: call void @llvm.x86.mask.vaaddps256(ptr %{{.*}}, <8 x float> %{{.*}}, i8 %{{.*}})
