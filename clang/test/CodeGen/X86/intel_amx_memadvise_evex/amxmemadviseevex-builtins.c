...
// RUN: %clang_cc1 %s -ffreestanding -triple=x86_64-unknown-unknown -target-feature +amx-tile -target-feature +amx-memadvise-evex \
...
void test_tile_t2rpntlvwz0advisee(const void *A, size_t B) {
  ...
  // CHECK: call void @llvm.x86.t2rpntlvwz0advisee(i8 1, ptr %{{.*}}, i64 %{{.*}}, i32 127)
...
void test_tile_t2rpntlvwz1advisee(const void *A, size_t B) {
  ...
  // CHECK: call void @llvm.x86.t2rpntlvwz1advisee(i8 1, ptr %{{.*}}, i64 %{{.*}}, i32 127)
