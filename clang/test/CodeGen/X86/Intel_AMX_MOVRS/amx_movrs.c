...
void test_tile_movadvise(void *A) {
  ...
  // CHECK: call void @llvm.x86.tmovadvise.load(i8 1, ptr %{{.*}}, i64 32, i8 16)
  _tile_movadvise_load(1, A, 32, 16);

  // CHECK: call void @llvm.x86.tmovadvise.store(ptr %{{.*}}, i64 32, i8 7, i8 18)
...
void test_tile_t2rpntlvwz0advise(const void *A, size_t B) {
  ...
  // CHECK: call void @llvm.x86.t2rpntlvwz0advise(i8 1, ptr %{{.*}}, i64 %{{.*}}, i32 127)
...
void test_tile_t2rpntlvwz1advise(const void *A, size_t B) {
  ...
  // CHECK: call void @llvm.x86.t2rpntlvwz1advise(i8 1, ptr %{{.*}}, i64 %{{.*}}, i32 127)
