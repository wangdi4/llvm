...
int addit(float a, float b) {
  ...
  // CHECKFAST-NEXT: [[P:%.*]] = alloca ptr, align 8
  // CHECKFAST-NEXT: [[I:%.*]] = alloca i32, align 4
  // CHECKFAST-NEXT: [[K:%.*]] = alloca ptr, align 8
  // CHECKFAST-NEXT: store float {{.*}}, ptr [[A]], align 4
  // CHECKFAST-NEXT: store float {{.*}}, ptr [[B]], align 4
  // CHECKFAST-NEXT: [[GEP_CD1_R:%.*]] = getelementptr inbounds { double, double }, ptr [[CD1]], i32 0, i32 0
  ...
  // CHECKFAST-NEXT: [[L2:%.*]] = load <2 x float>, ptr [[VEC2]], align 8
  ...
  // CHECKPP: store <2 x float> [[TMP7]], ptr [[VEC2]], align 8
  ...
  // CHECKPP: store i32 [[CONV1]], ptr @v, align 4

  // CHECKNPP: [[CONV2:%.*]] = fptosi float [[TMP13]] to i32
  // CHECKNPP: store i32 [[CONV2]], ptr @v, align 4

  v = a + (b * b);
  // CHECKFAST-NEXT: [[TMP15:%.*]] = load float, ptr [[A]], align 4
  ...
  // CHECKPP: store i32 [[CONV4]], ptr @v, align 4
  ...
  int *p;
  int i = __fence(1 + 2);
  // CHECKFAST: store i32 3, ptr [[I]], align 4
  // CHECK-NOT: call{{.*}} float @llvm.arithmetic.fence.int(float {{.*}})

  int *k = __fence(i + p);
  // CHECKFAST-NEXT: [[TMP26:%.*]] = load i32, ptr [[I]], align 4
  // CHECKFAST-NEXT: [[TMP27:%.*]] =  load ptr, ptr [[P]], align 8
  // CHECKFAST-NEXT: [[IDX:%.*]] = sext i32 [[TMP26]] to i64
  // CHECKFAST-NEXT: [[GEP11:%.*]] = getelementptr inbounds i32, ptr [[TMP27]], i64 [[IDX]]
  // CHECKFAST: store ptr [[GEP11]], ptr [[K]], align 8
