...
// RUN: %clang_cc1 -triple x86_64-linux-gnu -mllvm -opt-bisect-limit=0 %s -fdebug-pass-manager -emit-llvm -o /dev/null 2>&1 -fopenmp -fopenmp-late-outline -mllvm -paropt=31 | FileCheck %s ;INTEL
