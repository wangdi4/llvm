// RUN: %clang_cc1 -fintel-compatibility -triple x86_64-unknown-linux-gnu -emit-llvm -x c %s %s -o - | FileCheck -check-prefix=CHECK-C %s
// RUN: %clang_cc1 -fintel-compatibility -fsyntax-only -emit-llvm -x c++ -std=c++11 %s -o - | FileCheck %s --check-prefixes CHECK-C,CHECK-CPP
...
void code_align() {
  ...
  // CHECK-C-NEXT: ![[MD_code_align]] = !{!"llvm.loop.intel.align", i32 4}
  [[clang::code_align(4)]]
  for(int I=0; I<128; ++I) { bar(I); }

  // CHECK-C: ![[MD_FP_1]] = distinct !{![[MD_FP_1]], ![[MP]], ![[MD_code_align_1:[0-9]+]]}
  // CHECK-C-NEXT: ![[MD_code_align_1]] = !{!"llvm.loop.intel.align", i32 16}
  int i = 0;
  ...
  // CHECK-C-NEXT: ![[MD_code_align_2]] = !{!"llvm.loop.intel.align", i32 8}
  int b = 10;
  ...
  // CHECK-C-NEXT: ![[MD_code_align_3]] = !{!"llvm.loop.intel.align", i32 64}
  [[clang::code_align(64)]]
  for(int I=0; I<128; ++I) { bar(I); }
}
...
void code_align_cpp() {
  ...
  // CHECK-CPP-NEXT: ![[MD_code_align_4]] = !{!"llvm.loop.intel.align", i32 32}
