// INTEL - added -mllvm -enable-lv -mllvm -loopopt=0
// INTEL RUN: %clang_cc1 -O3 -mllvm -enable-lv -mllvm -loopopt=0 -triple x86_64-unknown-linux-gnu -target-cpu x86-64 %s -o %t -opt-record-file %t.yaml -emit-obj
// RUN: cat %t.yaml | FileCheck %s
// RUN: llvm-profdata merge %S/Inputs/opt-record.proftext -o %t.profdata
// INTEL RUN: %clang_cc1 -O3 -mllvm -enable-lv -mllvm -loopopt=0 -triple x86_64-unknown-linux-gnu -target-cpu x86-64 -fprofile-instrument-use-path=%t.profdata %s -o %t -opt-record-file %t.yaml -emit-obj
...
// INTEL RUN: %clang_cc1 -O3 -mllvm -enable-lv -mllvm -loopopt=0 -triple x86_64-unknown-linux-gnu -target-cpu x86-64 %s -o %t -opt-record-file %t.yaml -opt-record-format yaml -emit-obj
// RUN: cat %t.yaml | FileCheck %s
