// RUN: %clang_cc1 -ffreestanding -triple i686-unknown-linux-gnu -emit-llvm -no-opaque-pointers %s -o - | FileCheck %s --check-prefix=X86
// RUN: %clang_cc1 -ffreestanding -triple x86_64-unknown-linux-gnu -emit-llvm -no-opaque-pointers %s -o - | FileCheck %s --check-prefix=X64
