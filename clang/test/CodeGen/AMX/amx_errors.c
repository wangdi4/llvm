...
void test_amx(void *data) {
  _tile_zero(16); // expected-error {{argument value 16 is outside the valid range [0, 15]}}  // INTEL
  _tile_loadd(19, data, 16); // expected-error {{argument value 19 is outside the valid range [0, 15]}}  // INTEL
  _tile_stream_loadd(23, data, 1); // expected-error {{argument value 23 is outside the valid range [0, 15]}} INTEL
  _tile_stored(88, data, 1); // expected-error {{argument value 88 is outside the valid range [0, 15]}}  // INTEL
  _tile_dpbssd(16, 2, 3); // expected-error {{argument value 16 is outside the valid range [0, 15]}} // INTEL
  _tile_dpbssd(0, 16, 3); // expected-error {{argument value 16 is outside the valid range [0, 15]}} // INTEL
  _tile_dpbuud(0, 2, 16); // expected-error {{argument value 16 is outside the valid range [0, 15]}} // INTEL
