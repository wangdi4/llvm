...
// RUN: %clang_cc1 -triple x86_64-linux-gnu -O0 %s -flegacy-pass-manager -mllvm -debug-pass=Structure -emit-llvm -o /dev/null -fcoroutines 2>&1 | FileCheck %s --check-prefix=LEGACYPM
...
// RUN: %clang_cc1 -triple x86_64-linux-gnu -flegacy-pass-manager -mllvm -opt-bisect-limit=0 %s -mllvm -debug-pass=Structure -emit-llvm -o /dev/null -fcoroutines 2>&1 | FileCheck %s --check-prefix=LEGACYPM
