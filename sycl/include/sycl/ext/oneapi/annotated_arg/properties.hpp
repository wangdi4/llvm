...
struct buffer_location_key {
  template<int K>
  using value_t = property_value<buffer_location_key, std::integral_constant<int, K>>;
};
...
enum class read_write_mode_enum : std::uint16_t {
  read_write,
  read,
  write
};
...
template<int W>
inline constexpr awidth_key::value_t<W> awidth;
template<int W>
inline constexpr dwidth_key::value_t<W> dwidth;
template<int N>
...
template <> struct IsCompileTimeProperty<buffer_location_key> : std::true_type {};
...
template <> struct IsCompileTimeProperty<read_write_mode_key> : std::true_type {};
