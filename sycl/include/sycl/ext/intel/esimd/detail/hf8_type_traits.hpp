...
//==-------------- hf8_type_traits.hpp - DPC++ Explicit SIMD API ---------==//
//
...
//
//===----------------------------------------------------------------------===//
// Implementation of SIMD element type traits for the hf8 type.
//===----------------------------------------------------------------------===//
...
inline namespace _V1 {
  ...
  template <> struct element_type_traits<hf8> {
  ...
  template <int N> struct vector_conversion_traits<hf8, N> {
    using StdT = __cpp_t<hf8>;
    using RawT = __raw_t<hf8>;
    ...
    convert_to_raw(vector_type_t<StdT, N> Val) {
      ...
      vector_type_t<RawT, N> Result =
          __esimd_hf8_cvt<N, RawT, __ESIMD_DNS::__raw_t<half>>(Input);
      return Result;
#else
      vector_type_t<RawT, N> Output = 0;

      for (int i = 0; i < N; i++) {
        Output[i] = hf8::from_float(Val[i]);
      }
      return Output;
#endif
    }
    ...
    convert_to_cpp(vector_type_t<RawT, N> Val) {
      ...
      vector_type_t<__ESIMD_DNS::__raw_t<half>, N> Result =
          __esimd_hf8_cvt<N, __ESIMD_DNS::__raw_t<half>, RawT>(Val);
  ...
  template <> struct scalar_conversion_traits<hf8> {
    using RawT = __raw_t<hf8>;

    static ESIMD_INLINE RawT bitcast_to_raw(hf8 Val) {
      return sycl::bit_cast<RawT>(Val);
    }

    static ESIMD_INLINE hf8 bitcast_to_wrapper(RawT Val) {
      return sycl::bit_cast<hf8>(Val);
    }
  };
