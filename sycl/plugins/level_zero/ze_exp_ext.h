...
typedef struct _ze_module_program_exp_desc_t {
  ...
  const size_t *inputSizes; ///< [in][range(0, count)] sizes of each input IL
                            ///< module in pInputModules.
  const uint8_t *
      *pInputModules; ///< [in][range(0, count)] pointer to an array of IL (e.g.
                      ///< SPIR-V modules). Valid only for SPIR-V input.
  const char **pBuildFlags; ///< [in][optional][range(0, count)] array of
                            ///< strings containing build flags. See pBuildFlags
                            ///< in ::ze_module_desc_t.
  const ze_module_constants_t *
      *pConstants; ///< [in][optional][range(0, count)] pointer to array of
                   ///< specialization constant strings. Valid only for SPIR-V
                   ///< input. This must be set to nullptr if no specialization
