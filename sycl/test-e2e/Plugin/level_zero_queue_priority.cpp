...
// RUN: env ZE_DEBUG=1 SYCL_PI_LEVEL_ZERO_USE_IMMEDIATE_COMMANDLISTS=0 %t.out 2>&1 | FileCheck %s --check-prefixes=CHECK-STD
// RUN: env ZE_DEBUG=1 SYCL_PI_LEVEL_ZERO_USE_IMMEDIATE_COMMANDLISTS=1 %t.out 2>&1 | FileCheck %s --check-prefixes=CHECK-IMM
//
