...
template <typename T>
void testAlign(sycl::queue &q, unsigned alignment) {
  const sycl::context &Ctx = q.get_context();
  auto dev = q.get_device();
...
//   TEST_ALIGN((aligned_alloc_device_annotated<T>), std::lcm(2, sizeof(T)), alignment, N, q, properties{alignment<2>});
//   TEST_ALIGN((aligned_alloc_host_annotated<T>), std::lcm(alignment, std::lcm(8, sizeof(T))), alignment, N, q, properties{alignment<8>});
//   if (dev.has(sycl::aspect::usm_shared_allocations)) {
//     TEST_ALIGN((aligned_alloc_shared_annotated<T>), std::lcm(alignment, std::lcm(16, sizeof(T))), alignment, N, q, properties{alignment<16>});
//   }
}
