...
template <typename T> void testUsmKind(sycl::queue &q) {
  ...
  auto check_usm_kind = [&](alloc Expected, auto AllocFn, int Line = __builtin_LINE(),
                    int Case = 0) {
  ...
  CheckUsmKindAll(alloc::device, std::tuple{
    [&]() { return MDevice(q); }
  ...
  CheckUsmKindAll(alloc::host, std::tuple{
    [&]() { return MHost(q); }
    , [&]() { return MHost(Ctx); }
  ...
  CheckUsmKindAll(alloc::shared, std::tuple{
    [&]() { return MShared(q); }
