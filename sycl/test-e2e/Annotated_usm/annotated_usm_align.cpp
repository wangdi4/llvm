...
template <typename T>
void testAlign(sycl::queue &q, unsigned align) {
  ...
  auto check_align = [&q](size_t Alignment, auto AllocFn, int Line = __builtin_LINE(),
                    int Case = 0) {
  ...
  CheckAlignAll(1, std::tuple{
    ...
    , [&]() { return MAnnotated(dev, Ctx, properties{usm_kind_device, alignment<0>}); }
    , [&]() { return MAnnotated(dev, Ctx, properties{usm_kind_host}); }
    , [&]() { return MAnnotated(dev, Ctx, properties{usm_kind_host, alignment<0>}); }
    , [&]() { return MAnnotated(dev, Ctx, properties{usm_kind_shared}); }
    , [&]() { return MAnnotated(dev, Ctx, properties{usm_kind_shared, alignment<0>}); }

    // Case: `aligned_alloc_xxx` with no alignment constraint, this includes
  ...
  // Case: `malloc_xxx<T>` with no alignment constraint, the alignment is sizeof(T)
  ...
  auto check_align_lcm = [&q](std::vector<size_t> Alignments, auto AllocFn, int Line = __builtin_LINE(),
                    int Case = 0) {
  ...
  auto CheckAlignLcmAll = [&](std::vector<std::vector<size_t>> AlignVec, auto Funcs,
                      int Line = __builtin_LINE()) {
    std::apply(
        [&](auto... Fs) {
          int Case = 0, Idx = 0;
  ...
  CheckAlignAll(sizeof(T), std::tuple{
    ...
    , [&]() { return TAnnotated(dev, Ctx, properties{usm_kind_device, alignment<0>}); }
    , [&]() { return TAnnotated(dev, Ctx, properties{usm_kind_host}); }
    , [&]() { return TAnnotated(dev, Ctx, properties{usm_kind_host, alignment<0>}); }
    , [&]() { return TAnnotated(dev, Ctx, properties{usm_kind_shared}); }
    , [&]() { return TAnnotated(dev, Ctx, properties{usm_kind_shared, alignment<0>}); }

    // Case: `aligned_alloc_xxx<T>` with no alignment constraint, this includes
    // 1. no alignment property or `alignment<0>`
    // 2. alignment argument is 0
    , [&]() { return ATDevice(0, q); }
  ...
  CheckAlignLcmAll(
    ...
    std::tuple{
      ...
      , [&]() { return TAnnotated(dev, Ctx, properties{usm_kind_device, alignment<16>}); }
      , [&]() { return TAnnotated(dev, Ctx, properties{usm_kind_host, alignment<64>}); }
      , [&]() { return TAnnotated(dev, Ctx, properties{usm_kind_shared, alignment<256>}); }
  });


  // Case: aligned_alloc_xxx<T> with alignment argument K (K is a power of 2) and no alignment properties, the
  // alignment is the least-common-multiple of K and sizeof(T)
  CheckAlignLcmAll(
    {
      {2, sizeof(T)},
    ...
    std::tuple{
      ...
      , [&]() { return ATAnnotated(512, Ctx, alloc::shared, ALN512); }
  });
  

  // Case: aligned_alloc_xxx<T> with compile-time alignment<K> (K is a power of 2),
...
// Test cases that are expected to return null
  ...
  CheckNullAll(std::tuple{
    // Case: malloc_xxx with compile-time alignment<K> (K is not a power of 2),
    // nullptr is returned
    ...
    // Case: malloc_xxx<T> with compile-time alignment<K> (K is not a power of 2),
    // nullptr is returned
    ...
    // Case: aligned_alloc_xxx<T> with no alignment property, and the alignment
    // argument is not a power of 2, the result is nullptr
    , [&]() { return ATDevice(3, q); }
