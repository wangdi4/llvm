...
//
//   x in [1/2,1] interval.
//      On [1/2,1) interval acos(x) can be rewritten as follows:
//
...
//   4) 0 <= *a < 1/2
//      4.1) "Near 0" path
//           0 <= *a < NEAR0_BOUND_U, where NEAR0_BOUND_U = 2^(-53) in this
//           implementation.
//
//           On this path acos(x) is approximated as Pi/2-x. This subtle case
//           is used to avoid raising underflow on the main path.
//
//             dbResLo := PI2_LO - x * (1+x)
//             *r := PI2_HI - dbResLo
//
//      4.2) Main path
//           NEAR0_BOUND_U <= *a < 1/2, where NEAR0_BOUND_U = 2^(-53) in this
//           implementation.
//
...
//                 further details.
//
//           Series of splittings, multiplications and accurate summations is
...
//
//           (11) Add small term dbZLo*A0 - part of multiprecision calculation of
//               (A0+A0L)*x^3:
//
...
//
//           (13) Calculate x+dbT1 exactly in target precision as sum of high and
...
//
//      5.1) Calculating s=sqrt(y2) in multiprecision, where y2=y/2.
...
//
//           a) Range reduction
//               Decompose y2 into y2=M1*2^iN1, where iN1 is an integer, 1<=M1<2.
...
//
//      5.2) b0 + b1*y + b2*y^2 + y^3*P1(y) valuation
...
//
//           (6) Split dbZHi into dbWHi and dbWLo using IML_SPLIT() macro.
//           (7) Calculate high parts of (_B1+B1L)*y and (_B2+B2L)*y^2 and perform
...
//           (14) Add low part dbS0 (we got during accurate summation) to dbResLo
//           (15) Final result:
//
...
namespace __imf_impl_acos_s_ha {
namespace {
static const union {
  uint32_t w;
  float f;
} __sacos_ha_c5 = {0x3d2edc07u};
static const union {
  uint32_t w;
  float f;
} __sacos_ha_c4 = {0x3cc32a6bu};
static const union {
  uint32_t w;
  float f;
} __sacos_ha_c3 = {0x3d3a9ab4u};
static const union {
  uint32_t w;
  float f;
} __sacos_ha_c2 = {0x3d997c12u};
static const union {
  uint32_t w;
  float f;
} __sacos_ha_c1 = {0x3e2aaaffu};
static const union {
  uint32_t w;
  float f;
} __sacos_ha_c0 = {0xb19b5f15u};
static const union {
  uint32_t w;
  float f;
} __sacos_ha_small_float = {0x01800000u};
// 2.0
static const union {
  uint32_t w;
  float f;
} __sacos_ha_two = {0x40000000u};
// pi/2
static const union {
  uint32_t w;
  float f;
} __sacos_ha_pi2h = {0x3FC90FDBu};
static const union {
  uint32_t w;
  float f;
} __sacos_ha_pi2l = {0xB33BBD2Eu};
// pi
static const union {
  uint32_t w;
  float f;
} __sacos_ha_pih = {0x40490FDBu};
static const union {
  uint32_t w;
  float f;
} __sacos_ha_pil = {0xB3BBBD2Eu};

inline int __devicelib_imf_internal_sacos(const float *pxin, float *pres) {
  int nRet = 0;
  float xin = *pxin;
  union {
    uint32_t w;
  ...
  } y, res;
  {
    union {
      uint32_t w;
      float f;
    } x, xa, RS, Shh2, High, Low, R0;
    union {
      uint32_t w;
      float f;
    } fcorr;
    int32_t sgn_x;
    float R, E, poly, Sh, yf;
    x.f = xin;
    // absolute values
    xa.w = x.w & 0x7fffffff;
    // input sign
    ...
    Low.f = (xa.f <= 0.5f) ? __sacos_ha_pi2l.f : Low.f;
    // rsqrt((1-|x|)/2)
    // fixup for |x|<=0.5 (set RS=0)
    fcorr.w = (xa.f <= 0.5f) ? 0x7f800000 : 0; // 0x3f800000;
    ...
    Low.f = __fma(E, RS.f, Low.f);
    // polynomial
    poly = __fma(__sacos_ha_c5.f, R, __sacos_ha_c4.f);
    poly = __fma(poly, R, __sacos_ha_c3.f);
    poly = __fma(poly, R, __sacos_ha_c2.f);
    poly = __fma(poly, R, __sacos_ha_c1.f);
    poly = __fma(poly, R, __sacos_ha_c0.f);
    // R0 = sel_mask? xa.f : Shh2.w
    R0.f = (xa.f <= 0.5f) ? x.f : Shh2.f;
    // -poly*R0 + Low
    poly = __fma(-(poly), R0.f, Low.f);
    // High + R0 + (poly*R0 + Low)
    res.f = poly - R0.f;
    res.f = High.f + res.f;
  }
...
DEVICE_EXTERN_C_INLINE float __devicelib_imf_acosf(float x) {
  using namespace __imf_impl_acos_s_ha;
  float r;
  __devicelib_imf_internal_sacos(&x, &r);
  return r;
}
