...
namespace __imf_impl_llround_s_xa {
namespace {
typedef struct {
  VUINT32 _sSignMask;
  VUINT32 _sOneHalf;
  VUINT32 _sOne;
  VUINT32 _s2p23;
} __devicelib_imf_internal_sllround_data_t;
static const __devicelib_imf_internal_sllround_data_t
    __devicelib_imf_internal_sllround_data = {
...
DEVICE_EXTERN_C_INLINE int64_t __devicelib_imf_llroundf(float x) {
  using namespace __imf_impl_llround_s_xa;
  int64_t r;
  VUINT32 vm;
  float va1;
  VUINT64 vr1;
  va1 = x;
  {
    ...
    sSignMask = as_float(__devicelib_imf_internal_sllround_data._sSignMask);
    sSign = as_float((as_uint(sSignMask) & as_uint(va1)));
    sX = as_float((~(as_uint(sSignMask)) & as_uint(va1)));
    sRoundX = __rint(sX);
    sFractionX = (sX - sRoundX);
    sOneHalf = as_float(__devicelib_imf_internal_sllround_data._sOneHalf);
    sIsOneHalf = as_float(((VUINT32)(-(VSINT32)(sFractionX == sOneHalf))));
    sOne = as_float(__devicelib_imf_internal_sllround_data._sOne);
