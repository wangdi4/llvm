...
namespace __imf_impl_norm3d_s_la {
namespace {
typedef struct {
  VUINT32 _AbsMask;
  VUINT32 _MaxNorm;
  ...
  VUINT32 _c2;
  VUINT32 _c1;
} __devicelib_imf_internal_snorm3d_data_t;
static const __devicelib_imf_internal_snorm3d_data_t
    __devicelib_imf_internal_snorm3d_data = {
        /* AbsMask */ 0x7FFFFFFFu,
        /* MaxNorm */ 0x7F7FFFFFu,
        /* Zero */ 0x00000000u,
        /* Inf */ 0x7F800000u,
...
DEVICE_EXTERN_C_INLINE float __devicelib_imf_norm3df(float a, float b,
                                                     float c) {
  using namespace __imf_impl_norm3d_s_la;
  VUINT32 vm;
  float va1;
  float va2;
  float va3;
  ...
  va3 = c;
  {
    ...
    float ac1;
    vm = 0;
    AbsMask = as_float(__devicelib_imf_internal_snorm3d_data._AbsMask);
    // absolute values
    ...
    asum = (asum + az);
    // asum <= max_norm?
    MaxNorm = as_float(__devicelib_imf_internal_snorm3d_data._MaxNorm);
    SpecialMask = as_float(((VUINT32)(-(VSINT32)(!(asum <= MaxNorm)))));
    // asum == 0?
    Zero = as_float(__devicelib_imf_internal_snorm3d_data._Zero);
    ...
    Inf = as_float(__devicelib_imf_internal_snorm3d_data._Inf);
    H0 = as_float((as_uint(t0) & as_uint(Inf)));
    // 2^(-expon_t0+ ((t0>=2)?1:(-1)))
    // expon0.w = 0x7e800000 + ((expon0.w & 0x40000000) >> 6)- expon0.w;
    exponl = as_uint(H0);
    Biasx2 = (__devicelib_imf_internal_snorm3d_data._Biasx2m1);
    Two62 = (__devicelib_imf_internal_snorm3d_data._Two62);
...
*/
    Sh = (sum * RS);
    ac1 = as_float(__devicelib_imf_internal_snorm3d_data._ac1); // ac1=+0.5
    RS2 = (RS * ac1);
    // eps = sum - Sh^2
    ...
    Biasx2 = (__devicelib_imf_internal_snorm3d_data._Biasx2);
