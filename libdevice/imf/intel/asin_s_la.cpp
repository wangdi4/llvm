...
namespace __imf_impl_asin_s_la {
namespace {
static const union {
  uint32_t w;
  float f;
} __sasin_la_c5 = {0x3d2edc07u};
static const union {
  uint32_t w;
  float f;
} __sasin_la_c4 = {0x3cc32a6bu};
static const union {
  uint32_t w;
  float f;
} __sasin_la_c3 = {0x3d3a9ab4u};
static const union {
  uint32_t w;
  float f;
} __sasin_la_c2 = {0x3d997c12u};
static const union {
  uint32_t w;
  float f;
} __sasin_la_c1 = {0x3e2aaaffu};
static const union {
  uint32_t w;
  float f;
} __sasin_la_c0 = {0x3f800000u};
static const union {
  uint32_t w;
  float f;
} __sasin_la_small_float = {0x01800000u};
// 2.0
static const union {
  uint32_t w;
  float f;
} __sasin_la_two = {0x40000000u};
// pi/2
static const union {
  uint32_t w;
  float f;
} __sasin_la_pi2h = {0x3FC90FDBu};
static const union {
  uint32_t w;
  float f;
} __sasin_la_pi2l = {0xB33BBD2Eu};
inline int __devicelib_imf_internal_sasin(const float *pxin, float *pres) {
  int nRet = 0;
  float xin = *pxin;
  union {
    uint32_t w;
  ...
  // float asinf(float xin)
  {
    ...
    sgn_x = x.w ^ xa.w;
    // (1-|x|)/2
    y.f = __fma(-(0.5f), xa.f, 0.5f);
    // prepare polynomial argument
    R = xin * xin;
    R = __fmin(R, y.f);
    // rsqrt((1-|x|)/2)
    RS.f = 1.0f / __sqrt(y.f + __sasin_la_small_float.f);
    // Sh ~ -2.0*sqrt((1-|x|)/2)
    Sh.f = y.f * RS.f;
    Sh.f *= -2.0f;
    // polynomial
    poly = __fma(__sasin_la_c5.f, R, __sasin_la_c4.f);
    poly = __fma(poly, R, __sasin_la_c3.f);
    poly = __fma(poly, R, __sasin_la_c2.f);
    poly = __fma(poly, R, __sasin_la_c1.f);
    poly = __fma(poly, R, __sasin_la_c0.f);
    // R0 = sel_mask? Shh2 : xa.f
    R0.f = (xa.f <= 0.5f) ? xa.f : Sh.f;
    // High = sel_mask ? pi/2 : 0
    High.f = (xa.f <= 0.5f) ? 0 : __sasin_la_pi2h.f;
    // poly*R0 + High
    res.f = __fma(poly, R0.f, High.f);
    res.w |= sgn_x;
  }
  *pres = res.f;
  nRet = (y.f >= 0) ? 0 : 1;
...
DEVICE_EXTERN_C_INLINE float __devicelib_imf_asinf(float a) {
  using namespace __imf_impl_asin_s_la;
  float r;
  __devicelib_imf_internal_sasin(&a, &r);
  return r;
}
