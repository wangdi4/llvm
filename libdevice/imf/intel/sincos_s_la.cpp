...
namespace __imf_impl_sincos_s_la {
namespace {
typedef struct {
  VUINT32 _sPtable[256][3];
} __ssincos_la_ReductionTab_t;
static const __ssincos_la_ReductionTab_t
    __devicelib_imf_internal_ssincos_reduction_data = {{
        /*     P_hi                  P_med               P_lo                */
...
typedef struct {
  ...
  VUINT32 _sOneHalf;
  VUINT32 _sOne;
} __devicelib_imf_internal_ssincos_data_t;
static const __devicelib_imf_internal_ssincos_data_t
    __devicelib_imf_internal_ssincos_data = {
        {
        ...
        0x000000ffu, /* high accuracy table index mask  */
        0x00000040u, /* 2^(k-1) */
        0x80000000u, /* sign field mask */
...
static const _iml_sp_union_t __ssincos_la__vmlsSinCosHATab[2] = {
    0x00000000, /* ZERO = 0.0 */
    0x7F800000  /* INF = 0x7f800000 */
};

inline int __devicelib_imf_internal_ssincos(const float *a, float *r1,
                                            float *r2) {
  int nRet = 0;
  float absx;
         ...
         0xFF))) /********* |(*a)| = Inf,NaN *************/
  {
    if (((_iml_sp_union_t *)&(absx))->hex[0] ==
        ((const _iml_sp_union_t *)&(
             ((const float *)__ssincos_la__vmlsSinCosHATab)[1]))
            ->hex[0]) /****** |(*a)|=Inf (path 1) ***********/
    {
      /* Return NaN with invalid exception */
      (*r2) = (*r1) = (*a) * ((const float *)__ssincos_la__vmlsSinCosHATab)[0];
...
DEVICE_EXTERN_C_INLINE void __devicelib_imf_sincosf(float x, float *y,
                                                    float *z) {
  using namespace __imf_impl_sincos_s_la;
  ;
  VUINT32 vm;
  float va1;
  float vr1;
  ...
  ;
  {
    ...
    float sAbsMask;
    vm = 0;
    sAbsMask = as_float(__devicelib_imf_internal_ssincos_data._sAbsMask);
    ...
    sAbsX = as_float((as_uint(va1) & as_uint(sAbsMask)));
    {
      ...
      float sNearZero;
      float sOne;
      sInvPI = as_float(__devicelib_imf_internal_ssincos_data._sInvPI);
      sRShifter = as_float(__devicelib_imf_internal_ssincos_data._sRShifter);
      //    c) Getting octant Y by 2/Pi multiplication
      //    d) Add "Right Shifter" value
      sSinN = __fma(sAbsX, sInvPI, sRShifter);
      //    e) Treat obtained value as integer S for destination sign setting.
      sSinSign = as_float(((VUINT32)as_uint(sSinN) << (31)));
      //    g) Subtract "Right Shifter" (0x4B000000) value
      sSinN = (sSinN - sRShifter);
      sPI1 = as_float(__devicelib_imf_internal_ssincos_data._sPI1_FMA);
      sPI2 = as_float(__devicelib_imf_internal_ssincos_data._sPI2_FMA);
      //    h) Subtract Y*(PI/2) from X argument, where PI/2 divided to 3 parts:
      //       X = X - Y*PI1 - Y*PI2 - Y*PI3;
      sSinR = __fma(-(sSinN), sPI1, sAbsX);
      sSinR = __fma(-(sSinN), sPI2, sSinR);
      sSignMask = as_float(__devicelib_imf_internal_ssincos_data._sSignMask);
      sSinRSign = as_float((as_uint(sSignMask) & as_uint(sSinR)));
      sOneHalf = as_float(__devicelib_imf_internal_ssincos_data._sOneHalf);
      sOneHalf = as_float((as_uint(sOneHalf) ^ as_uint(sSinRSign)));
      // Add correction term 0.5 for cos() part
      sCosN = (sSinN + sOneHalf);
      sCosR = __fma(-(sCosN), sPI1, sAbsX);
      sCosR = __fma(-(sCosN), sPI2, sCosR);
      sPI3 = as_float(__devicelib_imf_internal_ssincos_data._sPI3_FMA);
      sSinR = __fma(-(sSinN), sPI3, sSinR);
      sCosR = __fma(-(sCosN), sPI3, sCosR);
      ...
      // input = 0?
      sNearZero = as_float(((VUINT32)(-(VSINT32)(va1 == sSignX))));
      // 2) Polynomial (minimax for sin within  [-Pi/4; +Pi/4] interval)
      //    a) Calculate X^2 = X * X
      //    b) Calculate 2 polynomials for sin and cos:
      //       RS = X * ( A0 + X^2 * (A1 + x^2 * (A2 + x^2 * (A3))));
      //       RC = B0 + X^2 * (B1 + x^2 * (B2 + x^2 * (B3 + x^2 * (B4))));
      sA13 = as_float(__devicelib_imf_internal_ssincos_data._sA13);
      sA11 = as_float(__devicelib_imf_internal_ssincos_data._sA11);
      sA9 = as_float(__devicelib_imf_internal_ssincos_data._sA9);
      sA7 = as_float(__devicelib_imf_internal_ssincos_data._sA7);
      sA5 = as_float(__devicelib_imf_internal_ssincos_data._sA5);
      sA3 = as_float(__devicelib_imf_internal_ssincos_data._sA3);
      sSinP = __fma(sA13, sSinR2, sA11);
      sCosP = __fma(sA13, sCosR2, sA11);
      sSinP = __fma(sSinP, sSinR2, sA9);
      sCosP = __fma(sCosP, sCosR2, sA9);
      sSinP = __fma(sSinP, sSinR2, sA7);
      sCosP = __fma(sCosP, sCosR2, sA7);
      sSinP = __fma(sSinP, sSinR2, sA5);
      sCosP = __fma(sCosP, sCosR2, sA5);
      sSinP = __fma(sSinP, sSinR2, sA3);
      sCosP = __fma(sCosP, sCosR2, sA3);
      sSinP = (sSinP * sSinR2);
      vr1 = __fma(sSinP, sSinR, sSinR);
      vr1 = as_float((as_uint(vr1) ^ as_uint(sSignX)));
      sCosP = (sCosP * sCosR2);
      vr2 = __fma(sCosP, sCosR, sCosR);
      // cos_result=1.0 if arg.=0
      sOne = as_float(__devicelib_imf_internal_ssincos_data._sOne);
      vr2 = as_float((((~as_uint(sNearZero)) & as_uint(vr2)) |
                      (as_uint(sNearZero) & as_uint(sOne))));
    }
    sRangeReductionVal =
        as_float(__devicelib_imf_internal_ssincos_data._sRangeReductionVal);
    sRangeReductionMask =
    ...
    if (__builtin_expect((mRangeReductionMask) != 0, 0)) {
      ...
      sRangeVal = as_float(__devicelib_imf_internal_ssincos_data._sRangeVal);
      ...
      sS2 = as_float(__devicelib_imf_internal_ssincos_data._sS2);
      sS1 = as_float(__devicelib_imf_internal_ssincos_data._sS1);
      sPS = __fma(sS2, sRp2, sS1);
      sPS = (sPS * sRp2);
      sPS = (sPS * sR);
      sC2 = as_float(__devicelib_imf_internal_ssincos_data._sC2);
      sC1 = as_float(__devicelib_imf_internal_ssincos_data._sC1);
      sPC = __fma(sC2, sRp2, sC1);
      sPC = (sPC * sRp2);
      /*........................... Sin ............................*/
      /*..................... Table look-up ........................*/
      sCHL = as_float(
          ((const VUINT32 *)(__devicelib_imf_internal_ssincos_data
      ...
      sResLo = __fma(sPS, sD, sResLo);
      sResLo = (sResLo + sResLo0);
      /*.................. Final reconstruction ...................*/
      sRes1Large = (sResHi + sResLo);
      sRes1Large = as_float((as_uint(sRes1Large) ^ as_uint(sSignX)));
      /*..................... Cos:  Table look-up ........................*/
      i2pK_1 = (__devicelib_imf_internal_ssincos_data._i2pK_1);
      icIndex = (iIndex + i2pK_1);
      iIndexMask = (__devicelib_imf_internal_ssincos_data._iIndexMask);
      icIndex = (icIndex & iIndexMask);
      /*..................... Table look-up ........................*/
      scCHL = as_float((
          (const VUINT32 *)(__devicelib_imf_internal_ssincos_data
                                ._dT))[(((0 + icIndex) * (4 * 4)) >> (2)) + 0]);
      scSHi = as_float((
          (const VUINT32 *)(__devicelib_imf_internal_ssincos_data
                                ._dT))[(((0 + icIndex) * (4 * 4)) >> (2)) + 1]);
      scSLo = as_float((
          (const VUINT32 *)(__devicelib_imf_internal_ssincos_data
                                ._dT))[(((0 + icIndex) * (4 * 4)) >> (2)) + 2]);
      scSigma = as_float((
          (const VUINT32 *)(__devicelib_imf_internal_ssincos_data
                                ._dT))[(((0 + icIndex) * (4 * 4)) >> (2)) + 3]);
      scD = (scCHL + scSigma);
      scResInt = __fma(sR, scSigma, scSHi);
      scResHi = __fma(scCHL, sR, scResInt);
      scK0 = (scSHi - scResInt);
      scK2 = __fma(sR, scSigma, scK0);
      scK1 = (scResInt - scResHi);
      scK3 = __fma(scCHL, sR, scK1);
      scResLo0 = (scK3 + scK2);
      /*................. Reconstruction: res_lo=corr+polS+polC+res_lo0
       * ...................*/
      scD = __fma(-(sR), scSHi, scD);
      scCorr = __fma(sE, scD, scSLo);
      scResLo = __fma(sPC, scSHi, scCorr);
      scResLo = __fma(sPS, scD, scResLo);
      ...
      vr1 = as_float((((~as_uint(sRangeReductionMask)) & as_uint(vr1)) |
                      (as_uint(sRangeReductionMask) & as_uint(sRes1Large))));
      vr2 = as_float((((~as_uint(sRangeReductionMask)) & as_uint(vr2)) |
                      (as_uint(sRangeReductionMask) & as_uint(sRes2Large))));
    }
  }
  if (__builtin_expect((vm) != 0, 0)) {
    ...
    __devicelib_imf_internal_ssincos(&__cout_a1, &__cout_r1, &__cout_r2);
