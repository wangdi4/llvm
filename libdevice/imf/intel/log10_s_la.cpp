...
namespace __imf_impl_log10_s_la {
namespace {
typedef struct {
  VUINT32 One;
  VUINT32 coeff4[16];
  ...
  VUINT32 coeff1[16];
  VUINT32 L2;
} __devicelib_imf_internal_slog10_data_avx512_t;
static const __devicelib_imf_internal_slog10_data_avx512_t
    __devicelib_imf_internal_slog10_data_avx512 = {
        /*== One ==*/
        0x3f800000u
        // c4
        ,
        ...
        /*== L2 ==*/
        ,
        0x3e9a209bu}; /*dLn_Table*/
typedef struct {
  VUINT32 Log_HA_table[(1 << 8) + 2];
  VUINT32 ha_poly_coeff[3];
  VUINT32 ExpMask;
  VUINT32 Two10;
  VUINT32 MinNorm;
  VUINT32 MaxNorm;
  ...
  VUINT32 sOnes[2];
  VUINT32 sZeros[2];
} __devicelib_imf_internal_slog10_data_t;
static const __devicelib_imf_internal_slog10_data_t
    __devicelib_imf_internal_slog10_data = {
        /* Log_HA_table */
...
static const _iml_sp_union_t __slog10_la_CoutTab[212] = {
...
inline int __devicelib_imf_internal_slog10(const float *a, float *r) {
  float x, y, u, q;
  float fP;
  float fAbsU;
  ...
  if (((((_iml_sp_union_t *)&(*a))->bits.exponent) != 0xFF)) {
...
*/
    ...
    if ((((_iml_sp_union_t *)&x)->bits.exponent) == 0) {
      ...
      x *= ((const float *)__slog10_la_CoutTab)[200];
      iN -= 40;
    }
    /* Starting from this point x is finite normalized number */
    if (x > ((const float *)__slog10_la_CoutTab)[201]) {
      ...
      if (fAbsU > ((const float *)__slog10_la_CoutTab)[199]) {
        /* 6) "Main" path */
        /* a) Range reduction */
...
*/
        fNLg2Hi = (fN * ((const float *)__slog10_la_CoutTab)[195]);
        fNLg2Lo = (fN * ((const float *)__slog10_la_CoutTab)[196]);
        // printf("val=%.20e\n",fNLg2Lo);
        /* Get y */
        y = x;
        (((_iml_sp_union_t *)&y)->bits.exponent = 0x7F);
        /* Obtain j */
        fTmp = (y + ((const float *)__slog10_la_CoutTab)[197]);
        j = (((_iml_sp_union_t *)&fTmp)->bits.significand) &
            ((1 << (6 + 1)) - 1);
        /* Get table values of B, LgRcprYHi, LgRcprYLo */
        fB = ((const float *)__slog10_la_CoutTab)[3 * (j)];
        fLgRcprYHi = ((const float *)__slog10_la_CoutTab)[3 * (j) + 1];
        fLgRcprYLo = ((const float *)__slog10_la_CoutTab)[3 * (j) + 2];
        /* Calculate WHi and WLo */
        fWHi = (fNLg2Hi + fLgRcprYHi);
        fWLo = (fNLg2Lo + fLgRcprYLo);
        /* Get YHi and YLo */
        fTmp = (y + ((const float *)__slog10_la_CoutTab)[198]);
        fYHi = (fTmp - ((const float *)__slog10_la_CoutTab)[198]);
        fYLo = (y - fYHi);
        /* Get QHi, QLo and q */
        fQHi = (fB * fYHi - ((const float *)__slog10_la_CoutTab)[203]);
        fQLo = (fB * fYLo);
        q = (fQHi + fQLo);
        /* b) Approximation */
        fP = (((((((((const float *)__slog10_la_CoutTab)[211] * q +
                    ((const float *)__slog10_la_CoutTab)[210]) *
                       q +
                   ((const float *)__slog10_la_CoutTab)[209]) *
                      q +
                  ((const float *)__slog10_la_CoutTab)[208]) *
                     q +
                 ((const float *)__slog10_la_CoutTab)[207]) *
                    q +
                ((const float *)__slog10_la_CoutTab)[206]) *
                   q +
               ((const float *)__slog10_la_CoutTab)[205]) *
                  q +
              ((const float *)__slog10_la_CoutTab)[204]);
        /* c) Reconstruction */
        fResHi = (fWHi + fQHi);
        fResLo = (fWLo + fP * fQLo);
        fResLo = (fResLo + fQLo);
        ...
        /* Calculate q */
        q = (u * ((const float *)__slog10_la_CoutTab)[203]);
        fP = (((((((((const float *)__slog10_la_CoutTab)[211] * q +
                    ((const float *)__slog10_la_CoutTab)[210]) *
                       q +
                   ((const float *)__slog10_la_CoutTab)[209]) *
      ...
      if (x == ((const float *)__slog10_la_CoutTab)[201]) {
        ...
        (*r) = (float)(-((const float *)__slog10_la_CoutTab)[202] /
                       ((const float *)__slog10_la_CoutTab)[201]);
        nRet = 2;
      } else {
        /* Path 4). Here if argument is negative number */
        (*r) = (float)(((const float *)__slog10_la_CoutTab)[201] /
                       ((const float *)__slog10_la_CoutTab)[201]);
        nRet = 1;
      }
    }
    ...
    if (((((_iml_sp_union_t *)&(*a))->bits.sign) == 1) &&
        ((((_iml_sp_union_t *)&(*a))->bits.significand) == 0)) {
...
DEVICE_EXTERN_C_INLINE float __devicelib_imf_log10f(float a) {
  using namespace __imf_impl_log10_s_la;
  VUINT32 vm;
  float va1;
  float vr1;
  float r;
  ...
  ;
  {
    ...
    iBrkValue = (__devicelib_imf_internal_slog10_data.iBrkValue);
    iOffExpoMask = (__devicelib_imf_internal_slog10_data.iOffExpoMask);
    iX = as_uint(va1);
    iX = (iX - iBrkValue);
    ...
    iN = ((VSINT32)iX >> (23));
    iR = (iR + iBrkValue);
    FpExpon = ((float)((VINT32)(iN)));
    sR = as_float(iR);
    MinNorm = as_float(__devicelib_imf_internal_slog10_data.MinNorm);
    MaxNorm = as_float(__devicelib_imf_internal_slog10_data.MaxNorm);
    BrMask1 = as_float(((VUINT32)(-(VSINT32)(va1 < MinNorm))));
    BrMask2 = as_float(((VUINT32)(-(VSINT32)(!(va1 <= MaxNorm)))));
    ...
    vm = 0;
    vm = BrMask;
    One = as_float(__devicelib_imf_internal_slog10_data.One);
    // Range reduction R = R - 1
    sR = (sR - One);
    sPoly[8] = as_float(__devicelib_imf_internal_slog10_data.sPoly[0]);
    sPoly[7] = as_float(__devicelib_imf_internal_slog10_data.sPoly[1]);
    sPoly[6] = as_float(__devicelib_imf_internal_slog10_data.sPoly[2]);
    sPoly[5] = as_float(__devicelib_imf_internal_slog10_data.sPoly[3]);
    // P78 = C8*R + C7
    sP78 = __fma(sPoly[8], sR, sPoly[7]);
    // P56 = C6*R + C5
    sP56 = __fma(sPoly[6], sR, sPoly[5]);
    // R2 = R*R
    sR2 = (sR * sR);
    sPoly[4] = as_float(__devicelib_imf_internal_slog10_data.sPoly[4]);
    sPoly[3] = as_float(__devicelib_imf_internal_slog10_data.sPoly[5]);
    // P34 = C4*R + C3
    sP34 = __fma(sPoly[4], sR, sPoly[3]);
    // P = P78*R2 + P56
    sP = __fma(sP78, sR2, sP56);
    sPoly[2] = as_float(__devicelib_imf_internal_slog10_data.sPoly[6]);
    sPoly[1] = as_float(__devicelib_imf_internal_slog10_data.sPoly[7]);
    // P12 = C2*R + C1
    sP12 = __fma(sPoly[2], sR, sPoly[1]);
    // P = P*R2 + P34
    sP = __fma(sP, sR2, sP34);
    // P = P*R2 + P12
    sP = __fma(sP, sR2, sP12);
    sPoly[0] = as_float(__devicelib_imf_internal_slog10_data.sPoly[8]);
    // P = P*R + C0
    sP = __fma(sP, sR, sPoly[0]);
    L2H = as_float(__devicelib_imf_internal_slog10_data.L2H);
    L2L = as_float(__devicelib_imf_internal_slog10_data.L2L);
    // Kl = exponent * Log(2)Lo
    Kl = (FpExpon * L2L);
    // P = P*R + Kl
    vr1 = __fma(sP, sR, Kl);
    // Result = exponent*Log(2)Hi + P
    vr1 = __fma(FpExpon, L2H, vr1);
  }
  if (__builtin_expect((vm) != 0, 0)) {
    ...
    __devicelib_imf_internal_slog10(&__cout_a1, &__cout_r1);
...
float __svml_device_log10f(float x) { return __devicelib_imf_log10f(x); }
