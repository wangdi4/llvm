...
static const __scos_la_ReductionTab_t
    __devicelib_imf_internal_scos_reduction_data = {{
        /*     P_hi                  P_med               P_lo                */
...
typedef struct {
  ...
  VUINT32 _sOneHalf;
  VUINT32 _sOne;
} __devicelib_imf_internal_scos_data_t;
static const __devicelib_imf_internal_scos_data_t
    __devicelib_imf_internal_scos_data = {
        {
        ...
        0x40490000u, /* PI high part */
        0x3A7DA000u, /* PI mid part 1 */
        0x34222000u, /* PI mid part 2 */
        0x2CB4611Au, /* PI low part */
        // PI1, PI2, and PI3 when FMA is available
        0x40490FDBu, /* PI high part (when FMA available) */
        0xB3BBBD2Eu, /* PI mid part  (when FMA available) */
        0xA7772CEDu, /* PI low part  (when FMA available) */
        // Polynomial constants for work w/o FMA, relative error ~ 2^(-26.625)
        /* > Polynomial coefficients: */
        // IMF target (DPC++) requires better than 2.0 ulp accuracy - more
        // coefficients
        0xbe2aaaabu,
        0x3c088889u,
        0xb9500cfeu,
...
static const _iml_sp_union_t __scos_la__vmlsCosCoutTab[2] = {
    0x00000000, /* ZEROVAL = 0.0 */
    0x7F800000  /* INFVAL = 0x7ff00000 */
};

inline int __devicelib_imf_internal_scos(const float *a, float *r) {
  float absx;
  int nRet = 0;
         ...
         0xFF))) /********* |(*a)| = Inf,NaN *************/
  {
    if (((_iml_sp_union_t *)&(absx))->hex[0] ==
        ((const _iml_sp_union_t *)&(
             ((const float *)__scos_la__vmlsCosCoutTab)[1]))
            ->hex[0]) /****** |(*a)|=Inf (path 1) ***********/
    {
      /* Return NaN with invalid exception */
      (*r) = (float)((*a) * ((const float *)__scos_la__vmlsCosCoutTab)[0]);
...
DEVICE_EXTERN_C_INLINE float __devicelib_imf_cosf(float x) {
  using namespace __imf_impl_cos_s_la;
  float r;
  VUINT32 vm;
  float va1;
  float vr1;
  va1 = x;
  {
    ...
    float sX; // VARG+HalfPI
    float sHalfPI;
    float sRangeReductionVal;
    float sAbsMask;
    vm = 0;
    sHalfPI = as_float(__devicelib_imf_internal_scos_data._sHalfPI);
    sAbsMask = as_float(__devicelib_imf_internal_scos_data._sAbsMask);
    ...
    sRangeReductionVal =
        as_float(__devicelib_imf_internal_scos_data._sRangeReductionVal);
    sRangeReductionMask =
        as_float(((VUINT32)(-(VSINT32)(!(sAX <= sRangeReductionVal)))));
    {
      float sN;
      float sSign;
      float sR;
      ...
      float sR3;
      // _VARG1 near 0
      sNearZero = as_float(((VUINT32)(-(VSINT32)(sX == sHalfPI))));
      sInvPI = as_float(__devicelib_imf_internal_scos_data._sInvPI);
      sRShifter = as_float(__devicelib_imf_internal_scos_data._sRShifter);
      // 1) Range reduction to [-Pi/2; +Pi/2] interval
      //    c) Getting octant Y by 1/Pi multiplication
      //    d) Add "Right Shifter" (0x4B000000) value
      sN = __fma(sX, sInvPI, sRShifter);
      //    e) Treat obtained value as integer for destination sign setting.
      //       Shift first bit of this value to the last (sign) position (S <<
      //       31)
      sSign = as_float(((VUINT32)as_uint(sN) << (31)));
      //    f) Subtract "Right Shifter" (0x4B000000) value
      sN = (sN - sRShifter);
      sOneHalf = as_float(__devicelib_imf_internal_scos_data._sOneHalf);
      //    g) Subtract 0.5 from result for octant correction
      sN = (sN - sOneHalf);
      sPI1 = as_float(__devicelib_imf_internal_scos_data._sPI1_FMA);
      //    h) Subtract Y*PI from X argument, where PI divided to 4 parts:
      //       X = X - Y*PI1 - Y*PI2 - Y*PI3
      sR = __fma(-(sN), sPI1, sAX);
      sPI2 = as_float(__devicelib_imf_internal_scos_data._sPI2_FMA);
      sR = __fma(-(sN), sPI2, sR);
      sPI3 = as_float(__devicelib_imf_internal_scos_data._sPI3_FMA);
      sR = __fma(-(sN), sPI3, sR);
      //    a) Calculate X^2 = X * X
      sR2 = (sR * sR);
      // 3) Destination sign setting
      ...
      // IMF target (DPC++) requires better than 2.0 ulp accuracy - more
      // coefficients
      sA13 = as_float(__devicelib_imf_internal_scos_data._sA13);
      sA11 = as_float(__devicelib_imf_internal_scos_data._sA11);
      sA9 = as_float(__devicelib_imf_internal_scos_data._sA9);
      sA7 = as_float(__devicelib_imf_internal_scos_data._sA7);
      sA5 = as_float(__devicelib_imf_internal_scos_data._sA5);
      sA3 = as_float(__devicelib_imf_internal_scos_data._sA3);
      sP = __fma(sA13, sR2, sA11);
      sP = __fma(sP, sR2, sA9);
      sP = __fma(sP, sR2, sA7);
      sP = __fma(sP, sR2, sA5);
      sP = __fma(sP, sR2, sA3);
      ...
      sOne = as_float(__devicelib_imf_internal_scos_data._sOne);
      vr1 = as_float((((~as_uint(sNearZero)) & as_uint(vr1)) |
                      (as_uint(sNearZero) & as_uint(sOne))));
    }
    ...
    if (__builtin_expect((mRangeReductionMask) != 0, 0)) {
      ...
      sRangeVal = as_float(__devicelib_imf_internal_scos_data._sRangeVal);
      sAX = as_float((as_uint(sRangeVal) & as_uint(sAX)));
      sRangeMask = as_float(((VUINT32)(-(VSINT32)(sAX == sRangeVal))));
      ...
      /*..................... Table look-up ........................*/
      sCHL = as_float(
          ((const VUINT32 *)(__devicelib_imf_internal_scos_data
                                 ._dT))[(((0 + iIndex) * (4 * 4)) >> (2)) + 0]);
      sSHi = as_float(
          ((const VUINT32 *)(__devicelib_imf_internal_scos_data
                                 ._dT))[(((0 + iIndex) * (4 * 4)) >> (2)) + 1]);
      sSLo = as_float(
          ((const VUINT32 *)(__devicelib_imf_internal_scos_data
                                 ._dT))[(((0 + iIndex) * (4 * 4)) >> (2)) + 2]);
      sSigma = as_float(
          ((const VUINT32 *)(__devicelib_imf_internal_scos_data
                                 ._dT))[(((0 + iIndex) * (4 * 4)) >> (2)) + 3]);
      /*................. Reconstruction: res_hi ...................*/
      sResInt = __fma(sR, sSigma, sSHi);
      sResHi = __fma(sCHL, sR, sResInt);
      sK0 = (sSHi - sResInt);
      sK2 = __fma(sR, sSigma, sK0);
      sK1 = (sResInt - sResHi);
      sK3 = __fma(sCHL, sR, sK1);
      sResLo0 = (sK3 + sK2);
      /*............... Polynomial approximation ...................*/
      sS2 = as_float(__devicelib_imf_internal_scos_data._sS2);
      sS1 = as_float(__devicelib_imf_internal_scos_data._sS1);
      sPS = __fma(sS2, sRp2, sS1);
      sPS = (sPS * sRp2);
      sPS = (sPS * sR);
      sD = (sCHL + sSigma);
      sC2 = as_float(__devicelib_imf_internal_scos_data._sC2);
      sC1 = as_float(__devicelib_imf_internal_scos_data._sC1);
      sPC = __fma(sC2, sRp2, sC1);
      sPC = (sPC * sRp2);
      /*.......... Reconstruction: res_lo=corr+polS+polC+res_lo0 ......*/
      sD = __fma(-(sR), sSHi, sD);
      sCorr = __fma(sE, sD, sSLo);
      sResLo = __fma(sPC, sSHi, sCorr);
      sResLo0 = __fma(sPS, sD, sResLo0);
  ...
  if (__builtin_expect((vm) != 0, 0)) {
    ...
    __devicelib_imf_internal_scos(&__cout_a1, &__cout_r1);
