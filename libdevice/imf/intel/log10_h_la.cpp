...
namespace __imf_impl_log10_h_la {
namespace {
typedef struct {
  VUINT16 __L2;
  VUINT16 __c4;
...
static const hfn_Table_Type_la __devicelib_imf_internal_hlog10_data = {
    0x34d1u, //__L2
    0x2badu, //__cs4
    0xaf2bu, //__cs3
    0x30b4u, //__cs2
...
inline _iml_half __devicelib_imf_log10f16_impl(_iml_half x) {
  _iml_half r;
  VUINT32 vm;
  _iml_half va1;
  _iml_half vr1;
  va1 = x;
  {
    ...
    InfExpon = (__devicelib_imf_internal_hlog10_data._InfExpon);
    not_inf_neg_nanmask = (iexpon < InfExpon) ? 0x1L : 0x0L;
    // predicate for x==0
    zero = as_half(__devicelib_imf_internal_hlog10_data._Zero);
    ...
    I1 = (__devicelib_imf_internal_hlog10_data._I1);
    iexpon2 = (iexpon2 + I1);
    iexpon2 = ((VUINT32)(iexpon2) >> (1));
    ibias = (__devicelib_imf_internal_hlog10_data._Ibias);
    ...
    mant = as_float(imant);
    // R = mantissa - 1
    sOne = as_float(__devicelib_imf_internal_hlog10_data._sOne);
    R = (mant - sOne);
    // polynomial ~ log2(1+R)/R
    c4 = as_float(__devicelib_imf_internal_hlog10_data._sc4);
    c3 = as_float(__devicelib_imf_internal_hlog10_data._sc3);
    c2 = as_float(__devicelib_imf_internal_hlog10_data._sc2);
    c1 = as_float(__devicelib_imf_internal_hlog10_data._sc1);
    c0 = as_float(__devicelib_imf_internal_hlog10_data._sc0);
    spoly = __fma(c4, R, c3);
    ...
    // log2(x)*log(2)
    L2 = as_float(__devicelib_imf_internal_hlog10_data._sL2);
    spoly = (spoly * L2);
    vr1 = ((_iml_half)(spoly));
    // response for neg., NaN, Inf
    hNInf = as_half(__devicelib_imf_internal_hlog10_data._hNInf);
...
DEVICE_EXTERN_C_INLINE _iml_half_internal __devicelib_imf_log10f16(_iml_half_internal x) {
  using namespace __imf_impl_log10_h_la;
  _iml_half _warg1(x);
  _iml_half _wres = __devicelib_imf_log10f16_impl(_warg1);
