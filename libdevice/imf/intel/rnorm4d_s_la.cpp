...
namespace __imf_impl_rnorm4d_s_la {
namespace {
typedef struct {
  VUINT32 _AbsMask;
  VUINT32 _MaxNorm;
  ...
  VUINT32 _c2;
  VUINT32 _c1;
} __devicelib_imf_internal_srnorm4d_data_t;
static const __devicelib_imf_internal_srnorm4d_data_t
    __devicelib_imf_internal_srnorm4d_data = {
        /* AbsMask */ 0x7FFFFFFFu,
        /* MaxNorm */ 0x7F7FFFFFu,
        /* Zero */ 0x00000000u,
        /* Inf */ 0x7F800000u,
...
DEVICE_EXTERN_C_INLINE float __devicelib_imf_rnorm4df(float a, float b, float c,
                                                      float _VAPI_ARG4) {
  using namespace __imf_impl_rnorm4d_s_la;
  VUINT32 vm;
  float va1;
  float va2;
  float va3;
  ...
  va4 = _VAPI_ARG4;
  {
    ...
    float ac1;
    vm = 0;
    AbsMask = as_float(__devicelib_imf_internal_srnorm4d_data._AbsMask);
    // absolute values
    ...
    asum = (asum + aw);
    // asum <= max_norm?
    MaxNorm = as_float(__devicelib_imf_internal_srnorm4d_data._MaxNorm);
    SpecialMask = as_float(((VUINT32)(-(VSINT32)(!(asum <= MaxNorm)))));
    // asum == 0?
    Zero = as_float(__devicelib_imf_internal_srnorm4d_data._Zero);
    ...
    Inf = as_float(__devicelib_imf_internal_srnorm4d_data._Inf);
    H0 = as_float((as_uint(t0) & as_uint(Inf)));
    // 2^(-expon_t0+ ((t0>=2)?1:0))
    // expon0.w = 0x7f000000 + ((expon0.w & 0x40000000) >> 7)- expon0.w;
    exponl = as_uint(H0);
    Biasx2 = (__devicelib_imf_internal_srnorm4d_data._Biasx2);
    Two62 = (__devicelib_imf_internal_srnorm4d_data._Two62);
...
*/
    ...
    One = as_float(__devicelib_imf_internal_srnorm4d_data._One);
    me0 = __fma(sum, RS2h, -(One));
    m_eps = __fma(sum, RS2l, me0);
    ac1 = as_float(__devicelib_imf_internal_srnorm4d_data._ac1); // ac1=-0.5
    poly = (m_eps * ac1);
    RS = __fma(RS, poly, RS);
