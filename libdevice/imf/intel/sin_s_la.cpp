...
static const __ssin_la_ReductionTab_t
    __devicelib_imf_internal_ssin_reduction_data = {{
        /*     P_hi                  P_med               P_lo                */
...
typedef struct {
  ...
  VUINT32 _sInvPI;
  VUINT32 _sRShifter;
} __devicelib_imf_internal_ssin_data_t;
static const __devicelib_imf_internal_ssin_data_t
    __devicelib_imf_internal_ssin_data = {
        {
...
static const _iml_sp_union_t __ssin_la__vmlsSinHATab[2] = {
    0x00000000, /* ZEROVAL = 0.0 */
    0x7F800000  /* INFVAL = 0x7ff00000 */
};

inline int __devicelib_imf_internal_ssin(const float *a, float *r) {
  int nRet = 0;
         ...
         0xFF))) /********* |sp_a| = Inf,NaN *************/
  {
    if (((_iml_sp_union_t *)&(absx))->hex[0] ==
        ((const _iml_sp_union_t *)&(
             ((const float *)__ssin_la__vmlsSinHATab)[1]))
            ->hex[0]) /****** |sp_a|=Inf (path 1) ***********/
    {
      /* Return NaN with invalid exception */
      (*r) = (float)(sp_a * ((const float *)__ssin_la__vmlsSinHATab)[0]);
      /* Set ERRDOM error code */
      nRet = 1;
      return nRet;
    }    /* |sp_a| = Inf */
    else /**************************** |sp_a|=NaN (path 2) ***********/
    {
...
DEVICE_EXTERN_C_INLINE float __devicelib_imf_sinf(float x) {
  using namespace __imf_impl_sin_s_la;
  float r;
  VUINT32 vm;
  float va1;
  float vr1;
  va1 = x;
  {
    ...
    float sAbsMask;
    sAbsMask = as_float(__devicelib_imf_internal_ssin_data._sAbsMask);
    //        b) Remove sign using AND operation
    ...
    vm = 0;
    {
      ...
      float sRShifter;
      float sRangeVal;
      sInvPI = as_float(__devicelib_imf_internal_ssin_data._sInvPI);
      sRShifter = as_float(__devicelib_imf_internal_ssin_data._sRShifter);
      //    c) Getting octant Y by 1/Pi multiplication
      //    d) Add "Right Shifter" value
      sN = __fma(sAbsX, sInvPI, sRShifter);
      //    e) Treat obtained value as integer for destination sign setting.
      //       Shift first bit of this value to the last (sign) position
      sSign = as_float(((VUINT32)as_uint(sN) << (31)));
      //    g) Subtract "Right Shifter" value
      sN = (sN - sRShifter);
      sPI1 = as_float(__devicelib_imf_internal_ssin_data._sPI1_FMA);
      //    h) Subtract Y*PI from X argument, where PI divided to 4 parts:
      //       X = X - Y*PI1 - Y*PI2 - Y*PI3;
      sR = __fma(-(sN), sPI1, sAbsX);
      sPI2 = as_float(__devicelib_imf_internal_ssin_data._sPI2_FMA);
      sR = __fma(-(sN), sPI2, sR);
      sPI3 = as_float(__devicelib_imf_internal_ssin_data._sPI3_FMA);
      sR = __fma(-(sN), sPI3, sR);
      // 2) Polynomial (minimax for sin within [-Pi/2; +Pi/2] interval)
      //    a) Calculate X^2 = X * X
      //    b) Calculate polynomial:
      ...
      // IMF target (DPC++) requires better than 2.0 ulp accuracy - more
      // coefficients
      sA13 = as_float(__devicelib_imf_internal_ssin_data._sA13);
      sA11 = as_float(__devicelib_imf_internal_ssin_data._sA11);
      sA9 = as_float(__devicelib_imf_internal_ssin_data._sA9);
      sA7 = as_float(__devicelib_imf_internal_ssin_data._sA7);
      sA5 = as_float(__devicelib_imf_internal_ssin_data._sA5);
      sA3 = as_float(__devicelib_imf_internal_ssin_data._sA3);
    ...
    sRangeReductionVal =
        as_float(__devicelib_imf_internal_ssin_data._sRangeReductionVal);
    sRangeReductionMask =
    ...
    if (__builtin_expect((mRangeReductionMask) != 0, 0)) {
      ...
      sRangeVal = as_float(__devicelib_imf_internal_ssin_data._sRangeVal);
      sExpX = as_float((as_uint(sRangeVal) & as_uint(sAbsX)));
      sRangeMask = as_float(((VUINT32)(-(VSINT32)(sExpX == sRangeVal))));
      ...
      /*..................... Table look-up ........................*/
      sCHL = as_float(
          ((const VUINT32 *)(__devicelib_imf_internal_ssin_data
      ...
      sS2 = as_float(__devicelib_imf_internal_ssin_data._sS2);
      sS1 = as_float(__devicelib_imf_internal_ssin_data._sS1);
      sPS = __fma(sS2, sRp2, sS1);
      sPS = (sPS * sRp2);
      sPS = (sPS * sR);
      sD = (sCHL + sSigma);
      sC2 = as_float(__devicelib_imf_internal_ssin_data._sC2);
      sC1 = as_float(__devicelib_imf_internal_ssin_data._sC1);
  ...
  if (__builtin_expect((vm) != 0, 0)) {
    ...
    __devicelib_imf_internal_ssin(&__cout_a1, &__cout_r1);
