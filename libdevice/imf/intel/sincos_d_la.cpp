...
namespace __imf_impl_sincos_d_la {
namespace {
static const union {
  uint64_t w;
  uint32_t w32[2];
...
static const uint64_t __dsincos_la_AbsMask = 0x7fffffffffffffffUL;
static const uint64_t __dsincos_la_zero = 0x0000000000000000UL;
// 2^1152/(2*Pi)
static const uint32_t __dsincos_la_InvPi_tbl[] = {
    ...
    0,           0,           0,           0};
// (2*pi)*2^61
static const uint32_t __dsincos_la_two_pi[2] = {0x2168C235u, 0xC90FDAA2u};
// unsigned 32-bit shift
// signed 32-bit shift
// unsigned 64-bit shift
// signed 64-bit shift
// reduce argument to (-2*pi/(2^kbits), 2*pi/(2^kbits)) range
static inline double __dsincos_la_trig_reduction(double x, int kbits,
  ...
  *pinterv = (interv ^ (int32_t)sgn_x) - (int32_t)sgn_x;
  ...
  Sl = (uint32_t)S[1];
  R = (((uint64_t)((uint32_t)(Sh))) * ((uint32_t)(__dsincos_la_two_pi[1])));
  P2 = (((uint64_t)((uint32_t)(Sh))) * ((uint32_t)(__dsincos_la_two_pi[0])));
  P3 = (((uint64_t)((uint32_t)(Sl))) * ((uint32_t)(__dsincos_la_two_pi[1])));
  // accumulate terms
  P23 = P2 + P3;
  // add carry
  if (P23 < P3)
...
inline int __devicelib_imf_internal_dsincos(const double *a, double *psin,
                                            double *pcos) {
  int nRet = 0;
  double xin = *a;
  unsigned leading_xh;
  ...
  uint64_t sgn_x, R_sgn, C_sgn, index, lindex;
  x0.f = xin;
  ...
  dS.f = __fma(x.f, (__dsincos_la_INV_PI64).f,
                                (__dsincos_la_SHIFTER).f);
  // N ~ x*(1/pi)
  ...
  index = dS.w & 1;
  // Rh = x - N*PI1
  R = __fma(-dN, (__dsincos_la_PI1_BITS).f, x.f);
  // Rm = Rh - N*PI2
  R = __fma(-dN, (__dsincos_la_PI2_BITS).f, R);
  // R = Rm - N*PI3
  R = __fma(-dN, (__dsincos_la_PI3_BITS).f, R);
  R2 = __fma(R, R, 0.0);
SINCOS_MAIN_PATH:
  c_poly = __fma(R2, (__dsincos_la_c_coeff[7]).f,
                                  (__dsincos_la_c_coeff[6]).f);
  s_poly = __fma(R2, (__dsincos_la_s_coeff[7]).f,
                                  (__dsincos_la_s_coeff[6]).f);
  c_poly = __fma(R2, c_poly, (__dsincos_la_c_coeff[5]).f);
  s_poly = __fma(R2, s_poly, (__dsincos_la_s_coeff[5]).f);
  c_poly = __fma(R2, c_poly, (__dsincos_la_c_coeff[4]).f);
  s_poly = __fma(R2, s_poly, (__dsincos_la_s_coeff[4]).f);
  c_poly = __fma(R2, c_poly, (__dsincos_la_c_coeff[3]).f);
  s_poly = __fma(R2, s_poly, (__dsincos_la_s_coeff[3]).f);
  c_poly = __fma(R2, c_poly, (__dsincos_la_c_coeff[2]).f);
  s_poly = __fma(R2, s_poly, (__dsincos_la_s_coeff[2]).f);
  c_poly = __fma(R2, c_poly, (__dsincos_la_c_coeff[1]).f);
  s_poly = __fma(R2, s_poly, (__dsincos_la_s_coeff[1]).f);
  c_poly = __fma(R2, c_poly, (__dsincos_la_c_coeff[0]).f);
  s_poly = __fma(R, s_poly, R);
...
SINCOS_SPECIAL:
  ...
  R = __dsincos_la_trig_reduction(x.f, 2, &Rl, &idx);
  R2 = R * R;
  lindex = index = (uint64_t)idx;
  lindex <<= 63;
  C_sgn = (lindex);
  lindex = (index >> 1);
  lindex <<= 63;
...
DEVICE_EXTERN_C_INLINE void __devicelib_imf_sincos(double x, double *y,
                                                   double *z) {
  using namespace __imf_impl_sincos_d_la;
  ;
  __devicelib_imf_internal_dsincos(&x, y, z);
  return;
}
