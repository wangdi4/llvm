...
//       into a number of subintervals.  On each subinterval we approximate
//     tanh(.)
...
namespace __imf_impl_tanh_s_ha {
namespace {
typedef struct {
  VUINT32 _sC[32];
  VUINT32 _sP0[32];
  ...
  VUINT32 _sP6[32];
  VUINT32 _sP7[32];
  VUINT32 _iExpMantMask_int;
  VUINT32 _iMinIdxOfsMask_int;
  VUINT32 _iMaxIdxMask_int;
  // Use large table
  VUINT64 _dbP[(27 * 8)];
  VUINT32 _sSignMask;
  VUINT32 _sAbsMask;
...
static const __devicelib_imf_internal_stanh_data_t
    __devicelib_imf_internal_stanh_data = {
        {/*== _sC ==*/
        ...
        0x7fe00000u,                /* _iExpMantMask_int     */
        0x3d400000u,                /* _iMinIdxOfsMask_int   */
        0x03e00000u,                /* _iMaxIdxMask_int      */
        {
...
static const _iml_sp_union_t __stanh_ha__imlsTanhTab[2] = {
    0x3F800000, /* ONES(0)       =  1.0    */
    0xBF800000, /* ONES(1)       = -1.0    */
};
inline int __devicelib_imf_internal_stanh(const float *a, float *r) {
  int nRet = 0;
  float sSign;
  sSign =
      ((const float *)
           __stanh_ha__imlsTanhTab)[((((_iml_sp_union_t *)&a[0])->bits.sign))];
...
DEVICE_EXTERN_C_INLINE float __devicelib_imf_tanhf(float a) {
  using namespace __imf_impl_tanh_s_ha;
  VUINT32 vm;
  float va1;
  float vr1;
  float r;
  ...
  ;
  {
    ...
    VUINT32 iMaxIdxMask;
    sSignMask = as_float(__devicelib_imf_internal_stanh_data._sSignMask);
    sAbsMask = as_float(__devicelib_imf_internal_stanh_data._sAbsMask);
    iExpMask = (__devicelib_imf_internal_stanh_data._iExpMask);
    iExpMantMask = (__devicelib_imf_internal_stanh_data._iExpMantMask_int);
    iMinIdxOfsMask = (__devicelib_imf_internal_stanh_data._iMinIdxOfsMask_int);
    iMaxIdxMask = (__devicelib_imf_internal_stanh_data._iMaxIdxMask_int);
    iZero = 0;
    // Absolute argument
    ...
    iIndex = (((VINT32)(iIndex) > (VINT32)(iZero)) ? iIndex : iZero);
    iIndex =
        (((VINT32)(iIndex) < (VINT32)(iMaxIdxMask)) ? iIndex : iMaxIdxMask);
    // Scale index
    iIndex = ((VUINT32)(iIndex) >> (19));
    // Load sC
    sC = as_float(
        ((const VUINT32 *)((const float *)(&__devicelib_imf_internal_stanh_data
                                                ._sC)))[iIndex >> 2]);
    // |x| = |x| - sC
    sAbsX = (sAbsX - sC);
    ...
    sP[7] = as_float(
        ((const VUINT32 *)((const float *)(&__devicelib_imf_internal_stanh_data
                                                ._sP7)))[iIndex >> 2]);
    // Compute polynomial
    sPoly = __fma(sP[7], sAbsX, sP[6]);
    sPoly = __fma(sPoly, sAbsX, sP[5]);
    sPoly = __fma(sPoly, sAbsX, sP[4]);
    sPoly = __fma(sPoly, sAbsX, sP[3]);
    /* sP[1] is the lower part of constant term sP[0] */
    sP[1] = as_float(
        ((const VUINT32 *)((const float *)(&__devicelib_imf_internal_stanh_data
                                                ._sP1)))[iIndex >> 2]);
    sPoly = (sPoly * sAbsX);
    sPoly = __fma(sPoly, sAbsX, sP[1]);
    sPoly = __fma(sAbsX, sP[2], sPoly);
    sPoly = (sPoly + sP[0]);
    // Set result sign
  ...
  if (__builtin_expect((vm) != 0, 0)) {
    ...
    __devicelib_imf_internal_stanh(&__cout_a1, &__cout_r1);
