...
namespace __imf_impl_log1p_s_ha {
namespace {
typedef struct {
  ...
  VUINT32 L2H;
  VUINT32 L2L;
} __devicelib_imf_internal_slog1p_data_avx512_t;
static const __devicelib_imf_internal_slog1p_data_avx512_t
    __devicelib_imf_internal_slog1p_data_avx512 = {
        {
...
typedef struct {
  ...
  VUINT32 sOnes[2];
  VUINT32 sZeros[2];
} __devicelib_imf_internal_slog1p_data_t;
static const __devicelib_imf_internal_slog1p_data_t
    __devicelib_imf_internal_slog1p_data = {
        /* Log_HA_table */
...
static const _iml_sp_union_t __slog1p_ha_CoutTab[210] = {
...
inline int __devicelib_imf_internal_slog1p(const float *a, float *r) {
  float fap1, *ap1 = (&fap1);
  float x, y, u;
  float fP;
  float fAbsU;
  ...
  *ap1 = (*a) + 1.0f;
  ...
  if (((((_iml_sp_union_t *)&ap1[0])->bits.exponent) != 0xFF)) {
...
*/
    ...
    if ((((_iml_sp_union_t *)&x)->bits.exponent) == 0) {
      ...
      x *= ((const float *)__slog1p_ha_CoutTab)[200];
      iN -= 40;
    }
    /* Starting from this point x is finite normalized number */
    if (x > ((const float *)__slog1p_ha_CoutTab)[201]) {
      ...
      if (fAbsU > ((const float *)__slog1p_ha_CoutTab)[199]) {
        /* 6) "Main" path */
        /* ap1) Range reduction */
...
*/
        if (isDenorm == 1) {
          fNLn2Hi = (fN * (((const float *)__slog1p_ha_CoutTab)[195] +
                           ((const float *)__slog1p_ha_CoutTab)[196]));
          fNLn2Lo = 0.0f;
        } else {
          fNLn2Hi = (fN * ((const float *)__slog1p_ha_CoutTab)[195]);
          fNLn2Lo = (fN * ((const float *)__slog1p_ha_CoutTab)[196]);
        }
        /* Get y */
        y = x;
        (((_iml_sp_union_t *)&y)->bits.exponent = 0x7F);
        /* Obtain j */
        fTmp = (y + ((const float *)__slog1p_ha_CoutTab)[197]);
        j = (((_iml_sp_union_t *)&fTmp)->bits.significand) &
            ((1 << (6 + 1)) - 1);
        /* Get table values of RcprY, LnRcprYHi, LnRcprYLo */
        fRcprY = ((const float *)__slog1p_ha_CoutTab)[3 * (j)];
        fLnRcprYHi = ((const float *)__slog1p_ha_CoutTab)[3 * (j) + 1];
        fLnRcprYLo = ((const float *)__slog1p_ha_CoutTab)[3 * (j) + 2];
        ...
        ;
        /* Get YHi and YLo */
        fTmp = (y + ((const float *)__slog1p_ha_CoutTab)[198]);
        fYHi = (fTmp - ((const float *)__slog1p_ha_CoutTab)[198]);
        fYLo = (y - fYHi);
        /* Get UHi, ULo and U */
        ...
        u = (fUHi + fULo);
        /* b) Approximation */
        fP = ((((((((const float *)__slog1p_ha_CoutTab)[209] * u +
                   ((const float *)__slog1p_ha_CoutTab)[208]) *
                      u +
                  ((const float *)__slog1p_ha_CoutTab)[207]) *
                     u +
                 ((const float *)__slog1p_ha_CoutTab)[206]) *
                    u +
                ((const float *)__slog1p_ha_CoutTab)[205]) *
                   u +
               ((const float *)__slog1p_ha_CoutTab)[204]) *
                  u +
              ((const float *)__slog1p_ha_CoutTab)[203]);
        fP = (fP * u * u);
        /* c) Reconstruction */
        fResHi = (fWHi + fUHi);
      ...
      if (x == ((const float *)__slog1p_ha_CoutTab)[201]) {
        ...
        r[0] = -((const float *)__slog1p_ha_CoutTab)[202] /
               ((const float *)__slog1p_ha_CoutTab)[201];
        nRet = 2;
      } else {
        /* Path 4). Here if argument is negative number */
        r[0] = ((const float *)__slog1p_ha_CoutTab)[201] /
               ((const float *)__slog1p_ha_CoutTab)[201];
        nRet = 1;
      }
    }
    ...
    if (((((_iml_sp_union_t *)&ap1[0])->bits.sign) == 1) &&
        ((((_iml_sp_union_t *)&ap1[0])->bits.significand) == 0)) {
...
DEVICE_EXTERN_C_INLINE float __devicelib_imf_log1pf(float a) {
  using namespace __imf_impl_log1p_s_ha;
  VUINT32 vm;
  float va1;
  float vr1;
  float r;
  ...
  ;
  {
    ...
    float B;
    float dR2;
    One = as_float(__devicelib_imf_internal_slog1p_data.sOne);
    SgnMask = as_float(__devicelib_imf_internal_slog1p_data.SgnMask);
    XThreshold = as_float(__devicelib_imf_internal_slog1p_data.XThreshold);
    XhMask = as_float(__devicelib_imf_internal_slog1p_data.XhMask);
    ...
    Xl = (A - X);
    Xl = (Xl + B);
    ExpMask = as_float(__devicelib_imf_internal_slog1p_data.ExpMask);
    Two10 = as_float(__devicelib_imf_internal_slog1p_data.Two10);
    ...
    MinLog1p = as_float(__devicelib_imf_internal_slog1p_data.MinLog1p);
    MaxLog1p = as_float(__devicelib_imf_internal_slog1p_data.MaxLog1p);
    ExpMask0 = (__devicelib_imf_internal_slog1p_data.ExpMask0);
    ExpMask2 = (__devicelib_imf_internal_slog1p_data.ExpMask2);
    ...
    /* exponent bits */
    IExpon = as_uint(X);
    IExpon = ((VUINT32)(IExpon) >> (23));
    /* check range */
    BrMask1 = as_float(((VUINT32)(-(VSINT32)(va1 < MinLog1p))));
    BrMask2 = as_float(((VUINT32)(-(VSINT32)(!(va1 <= MaxLog1p)))));
    ...
    R = __fma(Xl, DblRcp1, Rh);
    Rlh = (R - Rh);
    Rl = __fma(Xl, DblRcp1, -(Rlh));
    /* prepare table index */
    Index = as_uint(DblRcp);
    /* table lookup */
    Index = ((VUINT32)(Index) >> (23 - 7 - 3));
    THL[0] = as_float(
        ((const VUINT32 *)((const char *)(&__devicelib_imf_internal_slog1p_data
                                               .Log_HA_table[0]) -
                           0x21800))[Index >> 2]);
    THL[1] = as_float(
        ((const VUINT32 *)((const char *)(&__devicelib_imf_internal_slog1p_data
                           ...
                           0x21800))[(Index >> 2) + 1]);
    /* exponent*log(2.0) */
    L2H = as_float(__devicelib_imf_internal_slog1p_data.L2H);
    L2L = as_float(__devicelib_imf_internal_slog1p_data.L2L);
    Kh = __fma(FpExpon, L2H, THL[0]);
    Kl = __fma(FpExpon, L2L, THL[1]);
    /* polynomial */
    // VLOAD_CONST( S, poly_coeff[2], TAB.ha_poly_coeff[0] );
    poly_coeff[1] =
        as_float(__devicelib_imf_internal_slog1p_data.ha_poly_coeff[0]);
    poly_coeff[0] =
        as_float(__devicelib_imf_internal_slog1p_data.ha_poly_coeff[1]);
    // VQFMA( S, dP, poly_coeff[2], R, poly_coeff[1] );
    dP = __fma(poly_coeff[1], R, poly_coeff[0]);
    dR2 = (R * R);
    dP = __fma(dP, dR2, Rl);
    /* reconstruction */
    THL[0] = (Kh + R);
  ...
  if (__builtin_expect((vm) != 0, 0)) {
    ...
    __devicelib_imf_internal_slog1p(&__cout_a1, &__cout_r1);
...
float __svml_device_log1pf(float x) { return __devicelib_imf_log1pf(x); }
