...
namespace __imf_impl_cosh_d_ha {
namespace {
typedef struct {
  ...
  VUINT64 _dPC7_UISA;
  /************************************************************/
  VUINT64 _dbT[(1 << 7)][3]; // HA: <dTp_h,dTp_l,dTn_h>
  VUINT64 _dbInvLn2;
  VUINT64 _dbLn2hi;
  VUINT64 _dbLn2lo;
  VUINT64 _dbShifter;
  VUINT32 _iIndexMask; //(1<<K)1-
  VUINT64 _dPC1;       //=1
  VUINT64 _dPC2;
  VUINT64 _dPC3;
  VUINT64 _dPC4;
...
static const __devicelib_imf_internal_dcosh_data_t
    __devicelib_imf_internal_dcosh_data = {
        {/*== _dTp_h ==*/
...
static const _iml_dp_union_t __dcosh_ha_CoutTab[144] = {
...
//
*/
inline int __devicelib_imf_internal_dcosh(const double *a, double *r) {
  double M, Nj;
  ...
  scale = ((const double *)__dcosh_ha_CoutTab)[141];
  ...
  if (((((_iml_dp_union_t *)&(*a))->bits.exponent) != 0x7FF)) {
    ...
    if ((((_iml_dp_union_t *)&absAi)->bits.exponent) > (0x3FF - 54)) {
      /* Here if argument is not within "Near 0" interval */
      /* Check if cosh((*a)) overflows */
      if (absAi < ((const double *)__dcosh_ha_CoutTab)[137]) {
        /* Here if cosh((*a)) doesn't overflow */
        /* Check if |(*a)| is big enough                */
        /* to approximate cosh((*a)) by exp( |(*a)| )/2 */
        if (absAi >= ((const double *)__dcosh_ha_CoutTab)[143]) {
          /* Path 5 */
          /* Range Reduction part, 5a) */
          tmp = absAi * ((const double *)__dcosh_ha_CoutTab)[130];
          Nj = (tmp + ((const double *)__dcosh_ha_CoutTab)[131]);
          M = (Nj - ((const double *)__dcosh_ha_CoutTab)[131]);
          RHi = (M * ((const double *)__dcosh_ha_CoutTab)[138]);
          RLo = (M * ((const double *)__dcosh_ha_CoutTab)[139]);
          R = (absAi - RHi);
          R = (R - RLo);
          /* Approximation part: polynomial series, 5b) */
          p = (((((((const double *)__dcosh_ha_CoutTab)[136] * R +
                   ((const double *)__dcosh_ha_CoutTab)[135]) *
                      R +
                  ((const double *)__dcosh_ha_CoutTab)[134]) *
                     R +
                 ((const double *)__dcosh_ha_CoutTab)[133]) *
                    R +
                ((const double *)__dcosh_ha_CoutTab)[132]) *
               R);
          p = (p * R + R);
          ...
          N = (N + 0x3FF);
          /* p = (T[j] * p +  D[j]) + T[j] */
          p = ((const double *)__dcosh_ha_CoutTab)[2 * (j)] * p;
          p = (p + ((const double *)__dcosh_ha_CoutTab)[2 * (j) + 1]);
          p = (p + ((const double *)__dcosh_ha_CoutTab)[2 * (j)]);
          N = ((N - 1) & 0x7FF);
          ...
          if (N <= (0x7FF - 1)) {
            ...
            p = (p * scale);
            (*r) = ((const double *)__dcosh_ha_CoutTab)[128] * p;
          }
        } else {
          /* Path 4 */
          ...
          rsq = (R * R);
          podd = (((((const double *)__dcosh_ha_CoutTab)[135] * rsq +
                    ((const double *)__dcosh_ha_CoutTab)[133]) *
                   rsq) *
                  R);
          peven = ((((((const double *)__dcosh_ha_CoutTab)[136] * rsq +
                      ((const double *)__dcosh_ha_CoutTab)[134]) *
                     rsq) +
                    ((const double *)__dcosh_ha_CoutTab)[132]) *
                   rsq);
          ...
          N = N & 0x7FF;
          (((_iml_dp_union_t *)&scale)->bits.exponent = N - 1);
          TpHi = ((const double *)__dcosh_ha_CoutTab)[2 * (j)] * scale;
          TpLo = ((const double *)__dcosh_ha_CoutTab)[2 * (j) + 1] * scale;
          /* Obtain scale = 2^{-N - 2 + bias} */
          N = 2 * 0x3FF - N;
          (((_iml_dp_union_t *)&scale)->bits.exponent = N - 2);
          TmHi = ((const double *)__dcosh_ha_CoutTab)[2 * (64 - (j))] * scale;
          TmLo =
              ((const double *)__dcosh_ha_CoutTab)[2 * (64 - (j)) + 1] * scale;
        ...
        (*r) = ((const double *)__dcosh_ha_CoutTab)[140] *
               ((const double *)__dcosh_ha_CoutTab)[140];
      ...
      (*r) = ((const double *)__dcosh_ha_CoutTab)[142] + absAi;
    }
  } else {
...
DEVICE_EXTERN_C_INLINE double __devicelib_imf_cosh(double x) {
  using namespace __imf_impl_cosh_d_ha;
  double r;
  VUINT32 vm;
  double va1;
  double vr1;
  va1 = x;
  {
    ...
    dbInvLn2 = as_double(__devicelib_imf_internal_dcosh_data._dbInvLn2);
    dbShifter = as_double(__devicelib_imf_internal_dcosh_data._dbShifter);
    iIndexMask = (__devicelib_imf_internal_dcosh_data._iIndexMask);
    // VLOAD_CONST( D, dPC[0],         TAB._dPC1 );
    dPC[1] = as_double(__devicelib_imf_internal_dcosh_data._dPC2);
    dPC[2] = as_double(__devicelib_imf_internal_dcosh_data._dPC3);
    dPC[3] = as_double(__devicelib_imf_internal_dcosh_data._dPC4);
    dPC[4] = as_double(__devicelib_imf_internal_dcosh_data._dPC5);
    dXSign = as_double(__devicelib_imf_internal_dcosh_data._dSign);
    iDomainRange = (__devicelib_imf_internal_dcosh_data._iDomainRange);
    dbLn2[0] = as_double(__devicelib_imf_internal_dcosh_data._dbLn2hi);
    dbLn2[1] = as_double(__devicelib_imf_internal_dcosh_data._dbLn2lo);
    /* ............... Abs argument ............................ */
    dAbsX = as_double((~(as_ulong(dXSign)) & as_ulong(va1)));
    /* ............... Load argument ........................... */
    dM = __fma(dAbsX, dbInvLn2,
                                dbShifter); // dM = x*2^K/log(2) + RShifter
    ...
    dTp[0] = as_double(
        ((const VUINT64 *)(__devicelib_imf_internal_dcosh_data
    ...
    dR = __fma(-(dbLn2[0]), dN,
                                dAbsX); // dR = dX - dN*Log2_hi/2^K
    dR = __fma(-(dbLn2[1]), dN, dR); // dR = dX -
                                                      // dN*Log2_hi/2^K
    ...
    /* ............... dTp,dTn * 2^N,2^(-N) .................... */
    lM = ((VUINT64)(lM) << ((52 - 7)));
    lExpMask = (__devicelib_imf_internal_dcosh_data._lExpMask);
    ...
    dM = __fma(dPC[4], dR2, dPC[2]);   // dM=(a3 +a5*r^2)
    dOut = __fma(dPC[3], dR2, dPC[1]); // dOut=(a2 + a4*r^2)
    ...
    dM = __fma(dM, dR, dR); // dM= r + r^3*(a3 +a5*r^2)
    dOut = __fma(
  ...
  if (__builtin_expect((vm) != 0, 0)) {
    ...
    __devicelib_imf_internal_dcosh(&__cout_a1, &__cout_r1);
