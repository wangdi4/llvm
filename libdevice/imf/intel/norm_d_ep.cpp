...
namespace __imf_impl_norm_d_ep {
namespace {

inline int __devicelib_imf_internal_dnorm(int n, const double *x, double *r) 
{
  int nRet = 0;
  double fres = 0.0, fsum = 0.0, fax = 0.0, famaxx = 0.0, fdownscale = 1.0,
         fupscale = 1.0;
  uint64_t iamaxxexp = 0ul, idownscale = 0ul, iupscale = 0ul;
  int i = 0;
  ...
  idownscale =
      (((iamaxxexp & 0x4000000000000000ul) >> 9) + 0x7fd0000000000000ul) -
      iamaxxexp;
  // doubleing point scale
  fdownscale = as_double(idownscale);
  ...
  iupscale = (0x7fe0000000000000ul - idownscale);
  fupscale = as_double(iupscale);
  // Result = sqrt(sum)*2^(K)
...
DEVICE_EXTERN_C_INLINE double __devicelib_imf_norm(int n, const double *x)
{
  using namespace __imf_impl_norm_d_ep;
  double r;
  __devicelib_imf_internal_dnorm(n, x, &r);
  return r;
}
