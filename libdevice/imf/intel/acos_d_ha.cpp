...
namespace __imf_impl_acos_d_ha {
namespace {
static const union {
  uint64_t w;
  uint32_t w32[2];
...
inline int __devicelib_imf_internal_dacos(const double *pxin, double *pres) {
  int nRet = 0;
  double xin = *pxin;
  union {
    uint64_t w;
  ...
  // double acos(double xin)
  {
    ...
    x.f = xin;
    // absolute values
    xa.f = __fabs(x.f); // xa.w = x.w & 0x7fffffffffffffffUL;
    // input sign
    sgn_x = x.w ^ xa.w;
    // (1-|x|)/2
    ...
    R0.f = __fma(-(E), RS.f, Shh2.f);
    // polynomial
    poly = __fma(__dacos_ha_c12.f, R, __dacos_ha_c11.f);
    poly = __fma(poly, R, __dacos_ha_c10.f);
    poly = __fma(poly, R, __dacos_ha_c9.f);
    poly = __fma(poly, R, __dacos_ha_c8.f);
    poly = __fma(poly, R, __dacos_ha_c7.f);
    poly = __fma(poly, R, __dacos_ha_c6.f);
    poly = __fma(poly, R, __dacos_ha_c5.f);
    poly = __fma(poly, R, __dacos_ha_c4.f);
    poly = __fma(poly, R, __dacos_ha_c3.f);
    poly = __fma(poly, R, __dacos_ha_c2.f);
    poly = __fma(poly, R, __dacos_ha_c1.f);
    poly = __fma(poly, R, __dacos_ha_c0.f);
    // -poly*R0 + Low
...
DEVICE_EXTERN_C_INLINE double __devicelib_imf_acos(double x) {
  using namespace __imf_impl_acos_d_ha;
  double r;
  __devicelib_imf_internal_dacos(&x, &r);
  return r;
}
