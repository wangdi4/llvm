...
static const _VAPI_DATA_TYPE_ATS _VAPI_DATA_NAME_ATS = {
    // OneEighth = 0.125
...
static const _iml_dp_union_t __dlog1p_ha_CoutTab[210] = {
...
//
*/
inline int __devicelib_imf_internal_dlog1p(const double *a0, double *r) {
  ...
  if (((((_iml_dp_union_t *)&a[0])->bits.exponent) != 0x7FF)) {
    ...
    */
    ...
    if ((((_iml_dp_union_t *)&x)->bits.exponent) == 0) {
      ...
      x *= ((const double *)__dlog1p_ha_CoutTab)[200];
      iN -= 60;
    }
    /* Starting from this point x is finite normalized number */
    if (x > ((const double *)__dlog1p_ha_CoutTab)[201]) {
      ...
      if (dbAbsU > ((const double *)__dlog1p_ha_CoutTab)[199]) {
        /* 6) "Main" path */
        /* a) Range reduction */
        ...
        */
        dbNLn2Hi = (dbN * ((const double *)__dlog1p_ha_CoutTab)[195]);
        dbNLn2Lo = (dbN * ((const double *)__dlog1p_ha_CoutTab)[196]);
        /* Get y */
        y = x;
        (((_iml_dp_union_t *)&y)->bits.exponent = 0x3FF);
        /* Obtain j */
        dbTmp = (y + ((const double *)__dlog1p_ha_CoutTab)[197]);
        j = (((_iml_dp_union_t *)&dbTmp)->bits.lo_significand) &
            ((1 << (6 + 1)) - 1);
        /* Get table values of RcprY, LnRcprYHi, LnRcprYLo */
        dbRcprY = ((const double *)__dlog1p_ha_CoutTab)[3 * (j)];
        dbLnRcprYHi = ((const double *)__dlog1p_ha_CoutTab)[3 * (j) + 1];
        dbLnRcprYLo = ((const double *)__dlog1p_ha_CoutTab)[3 * (j) + 2];
        /* Calculate WHi and WLo */
        dbWHi = (dbNLn2Hi + dbLnRcprYHi);
        dbWLo = (dbNLn2Lo + dbLnRcprYLo);
        /* Get YHi and YLo */
        dbTmp = (y + ((const double *)__dlog1p_ha_CoutTab)[198]);
        dbYHi = (dbTmp - ((const double *)__dlog1p_ha_CoutTab)[198]);
        dbYLo = (y - dbYHi);
        /* Get UHi, ULo and U */
        ...
        u = (dbUHi + dbULo);
        /* b) Approximation */
        dbP = (((((((const double *)__dlog1p_ha_CoutTab)[209] * u +
                   ((const double *)__dlog1p_ha_CoutTab)[208]) *
                      u +
                  ((const double *)__dlog1p_ha_CoutTab)[207]) *
                     u +
                 ((const double *)__dlog1p_ha_CoutTab)[206]) *
                    u +
                ((const double *)__dlog1p_ha_CoutTab)[205]) *
                   u +
               ((const double *)__dlog1p_ha_CoutTab)[204]) *
                  u +
              ((const double *)__dlog1p_ha_CoutTab)[203];
        dbP = (dbP * u * u);
        /* c) Reconstruction */
        dbResHi = (dbWHi + dbUHi);
      ...
      if (x == ((const double *)__dlog1p_ha_CoutTab)[201]) {
        ...
        r[0] = -((const double *)__dlog1p_ha_CoutTab)[202] /
               ((const double *)__dlog1p_ha_CoutTab)[201];
        nRet = 2;
      } else {
        /* Path 4). Here if argument is negative number */
        r[0] = ((const double *)__dlog1p_ha_CoutTab)[201] /
               ((const double *)__dlog1p_ha_CoutTab)[201];
        nRet = 1;
      }
    }
...
DEVICE_EXTERN_C_INLINE double __devicelib_imf_log1p(double x) {
  using namespace __imf_impl_log1p_d_ha;
  double r;
  VUINT32 vm;
  double va1;
  double vr1;
  va1 = x;
  {
    ...
    OneEighth = as_double(_VAPI_DATA_NAME_ATS.OneEighth);
    t = (OneEighth * va1);
    t = (t + OneEighth);
    ...
    // 2^-1023 <= s <= 2^53
    //
    ExpMask = (_VAPI_DATA_NAME_ATS.ExpMask);
    ExpInv = (_VAPI_DATA_NAME_ATS.ExpInv);
    lt = as_ulong(t);
    te = (lt & ExpMask);
    ls = (ExpInv - te);
    s = as_double(ls);
    ...
    //
    OneHalf = as_double(_VAPI_DATA_NAME_ATS.OneHalf);
    tt = (s * t);
    tt = (tt + OneHalf);
    ...
    //
    Eight = as_double(_VAPI_DATA_NAME_ATS.Eight);
    ConvExp = (_VAPI_DATA_NAME_ATS.ConvExp);
    ConvConst = as_double(_VAPI_DATA_NAME_ATS.ConvConst);
    sss = (ss * Eight);
    ls = as_ulong(sss);
    ls = ((VUINT64)(ls) >> (20));
    ls = (ls | ConvExp);
    ...
    //
    One = as_double(_VAPI_DATA_NAME_ATS.One);
    q = (One - ss);
    r = __fma(ss, va1, -(q));
    // We have -0.25 <= r <= 0.5
    //
    ...
    //
    Shifter = as_double(_VAPI_DATA_NAME_ATS.Shifter);
    ThreeQuarters = as_double(_VAPI_DATA_NAME_ATS.ThreeQuarters);
    sm = (Shifter + r);
    mm = (sm - Shifter);
    m = (mm * ThreeQuarters);
    // Compute
    ...
    //
    du = as_double(_VAPI_DATA_NAME_ATS.CoeffD7);
    dg = as_double(_VAPI_DATA_NAME_ATS.CoeffD6);
    cdu = as_double(_VAPI_DATA_NAME_ATS.CoeffD5);
    cdg = as_double(_VAPI_DATA_NAME_ATS.CoeffD4);
    du = __fma(v, du, cdu);
    dg = __fma(v, dg, cdg);
    cdu = as_double(_VAPI_DATA_NAME_ATS.CoeffD3);
    cdg = as_double(_VAPI_DATA_NAME_ATS.CoeffD2);
    du = __fma(v, du, cdu);
    dg = __fma(v, dg, cdg);
    dd = __fma(m, du, dg);
    // Polynomial evaluation of p(z) = z + w * pp(z)
    //
    pgg = as_double(_VAPI_DATA_NAME_ATS.CoeffP12);
    puu = as_double(_VAPI_DATA_NAME_ATS.CoeffP11);
    pug = as_double(_VAPI_DATA_NAME_ATS.CoeffP10);
    pgu = as_double(_VAPI_DATA_NAME_ATS.CoeffP9);
    cpgg = as_double(_VAPI_DATA_NAME_ATS.CoeffP8);
    cpuu = as_double(_VAPI_DATA_NAME_ATS.CoeffP7);
    cpug = as_double(_VAPI_DATA_NAME_ATS.CoeffP6);
    cpgu = as_double(_VAPI_DATA_NAME_ATS.CoeffP5);
    pgg = __fma(ww, pgg, cpgg);
    puu = __fma(ww, puu, cpuu);
    pug = __fma(ww, pug, cpug);
    pgu = __fma(ww, pgu, cpgu);
    cpgg = as_double(_VAPI_DATA_NAME_ATS.CoeffP4);
    cpuu = as_double(_VAPI_DATA_NAME_ATS.CoeffP3);
    cpug = as_double(_VAPI_DATA_NAME_ATS.CoeffP2);
    pgg = __fma(ww, pgg, cpgg);
    puu = __fma(ww, puu, cpuu);
    pug = __fma(ww, pug, cpug);
    pg = __fma(w, pgg, pug);
    ...
    //
    LogTwoHi = as_double(_VAPI_DATA_NAME_ATS.LogTwoHi);
    LogTwoLo = as_double(_VAPI_DATA_NAME_ATS.LogTwoLo);
    lkhi = (k * LogTwoHi);
    lklo = (k * LogTwoLo);
    ...
    tlo = (mpzhi - tts);
    // Lower parts
    ttlo = __fma(pp, w, lklo);
    tttlo = __fma(dd, v, mpzlo);
    ttttlo = (ttlo + tttlo);
    ylo = (tlo + ttttlo);
    // Final summation
    vr1 = (yhi + ylo);
    // Compute rangemask for main-path arguments: -1.0 < x < 2^1024
    MinArg = as_double(_VAPI_DATA_NAME_ATS.MinArg);
    MaxArg = as_double(_VAPI_DATA_NAME_ATS.MaxArg);
    MaskMin = as_double((VUINT64)((va1 < MinArg) ? 0xffffffffffffffff : 0x0));
    MaskMax =
        as_double((VUINT64)(((!(va1 <= MaxArg)) ? 0xffffffffffffffff : 0x0)));
  ...
  if (__builtin_expect((vm) != 0, 0)) {
    ...
    __devicelib_imf_internal_dlog1p(&__cout_a1, &__cout_r1);
