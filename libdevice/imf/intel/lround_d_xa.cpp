...
namespace __imf_impl_lround_d_xa {
namespace {
typedef struct {
  VUINT64 _dSignMask;
  VUINT64 _dOneHalf;
  VUINT64 _dOne;
  VUINT64 _d2p52;
} __devicelib_imf_internal_dlround_data_t;
static const __devicelib_imf_internal_dlround_data_t
    __devicelib_imf_internal_dlround_data = {
...
DEVICE_EXTERN_C_INLINE int64_t __devicelib_imf_lround(double x) {
  using namespace __imf_impl_lround_d_xa;
  int64_t r;
  VUINT32 vm;
  double va1;
  VUINT64 vr1;
  va1 = x;
  {
    ...
    dSignMask = as_double(__devicelib_imf_internal_dlround_data._dSignMask);
    dSign = as_double((as_ulong(dSignMask) & as_ulong(va1)));
    dX = as_double((~(as_ulong(dSignMask)) & as_ulong(va1)));
    vm = 0;
    dRoundX = __rint(dX);
    dFractionX = (dX - dRoundX);
    dOneHalf = as_double(__devicelib_imf_internal_dlround_data._dOneHalf);
    dIsOneHalf = as_double(
        (VUINT64)((dFractionX == dOneHalf) ? 0xffffffffffffffff : 0x0));
    dOne = as_double(__devicelib_imf_internal_dlround_data._dOne);
