...
; RUN: opt < %s -dtrans-meminittrimdown -enable-dtrans-meminittrimdown -whole-program-assume -intel-libirc-allowed -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -debug-only=dtrans-meminittrimdown -disable-output 2>&1 | FileCheck %s
...
; "f1->resize(1);" call in "F" is the main reason why MemInitTrimDown
; transformation isn't triggered. "capacity" field is updated in "resize"
; function. MemInitTrimDown expects "resize" to be called from AppendElem
; (i.e add) only.
;
;CHECK: Failed: Resize has more than one use.
...
entry:
  ...
  %cap2 = tail call i32 @_ZN3ArrIPiE11getCapacityEv(%struct.Arr* nonnull %0)
  tail call void @_ZN3ArrIPiE3setEiPS0_(%struct.Arr* %7, i32 %cap2, i32** %call6)
  ...
  tail call void @_ZN3ArrIPiE6resizeEi(%struct.Arr* %d, i32 1)
