...
; This test verifies the parameters to realloc memory allocations
...
; updated for realloc with a new multiple of the size.
define internal void @test01() {
; CHECK-LABEL: define internal void @test01

  %alloc1 = call i8* @malloc(i64 240)
; CHECK: %alloc1 = call i8* @malloc(i64 120)

  %dep01_mem1 = bitcast i8* %alloc1 to %struct.test01dep*

  %alloc2 = call i8* @realloc(i8* %alloc1, i64 480)
; CHECK: %alloc2 = call i8* @realloc(i8* %alloc1, i64 240)

  %dep01_mem2 = bitcast i8* %alloc2 to %struct.test01dep*

  ret void
}

; Verify the constant multiple gets updated when constants are
; not used as the parameters to realloc.
...
; CHECK: %alloc1 = call i8* @malloc(i64 %multiple1)

  %dep01_mem1 = bitcast i8* %alloc1 to %struct.test01dep*
  %multiple2 = mul i64 %n, 96
; CHECK:  %multiple2 = mul i64 %n, 48

  %alloc2 = call i8* @realloc(i8* %alloc1, i64 %multiple2)
; CHECK: %alloc2 = call i8* @realloc(i8* %alloc1, i64 %multiple2)
