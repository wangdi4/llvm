; This test verifies that BlockSize is changed by the transformation.
;
; This test is same as memmanage-functionality-01.ll except the below.
; 1. memmanage-functionality-01.ll failed to apply the transformation
; due to heuristic. We can’t prove the original BlockSize is 10 for
; memmanage-functionality-01.ll. For this test, we can detect that the
; original BlockSize is 10 from the function call below.
; _ZN11xalanc_1_1022ReusableArenaAllocatorINS_13XStringCachedEEC2ERN11xercesc_2_713MemoryManagerEtb(%"ReusableArenaAllocator"* nonnull dereferenceable(41) %4, %"MemoryManager"* nonnull align 8 dereferenceable(8) %1, i16 10, i1 zeroext false)
;
; 2. memmanage-functionality-01.ll checks for only debug output to verify
; that all functions are recognized and BlockSize heuristic failed.
; This test just checks for transformed code but doesn’t check for any
; debug messages.
;
; RUN: opt < %s -dtrans-memmanagetrans -enable-dtrans-memmanagetrans -whole-program-assume -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -S 2>&1 | FileCheck %s
; RUN: opt < %s -passes=dtrans-memmanagetrans -enable-dtrans-memmanagetrans -whole-program-assume -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -S 2>&1 | FileCheck %s

; CHECK: define internal void @_ZN11xalanc_1_1022ReusableArenaAllocatorINS_13XStringCachedEEC2ERN11xercesc_2_713MemoryManagerEtb
; CHECK-NOT: store i16 %arg2, i16* %i4, align 8
; CHECK: store i16 60, i16* %i4, align 8
...
define internal void @_ZN11xalanc_1_1022XStringCachedAllocatorC2ERN11xercesc_2_713MemoryManagerEt.8894(%"XStringCachedAllocator"* nocapture nonnull dereferenceable(48) %0, %"MemoryManager"* nonnull align 8 dereferenceable(8) %1, i16 zeroext %2) unnamed_addr align 2 {
  ...
  tail call void @_ZN11xalanc_1_1022ReusableArenaAllocatorINS_13XStringCachedEEC2ERN11xercesc_2_713MemoryManagerEtb(%"ReusableArenaAllocator"* nonnull dereferenceable(41) %4, %"MemoryManager"* nonnull align 8 dereferenceable(8) %1, i16 10, i1 zeroext false)
  ret void
}
