...
; RUN: opt < %s -whole-program-assume -intel-libirc-allowed -dtransanalysis -dtrans-test-paddedmalloc -dtrans-print-types -padded-pointer-prop -dtrans-paddedmalloc -S 2>&1 | FileCheck %s
; RUN: opt < %s -whole-program-assume -intel-libirc-allowed -passes='require<dtransanalysis>,padded-pointer-prop,dtrans-paddedmalloc' -dtrans-test-paddedmalloc -dtrans-print-types  -S 2>&1 | FileCheck %s

; This test case tests the runtime badcast validation.

; CHECK-LABEL: define{{.*}}init_with_coder1
; CHECK: %2 = getelementptr %struct.mynextcoder, %struct.mynextcoder* %0, i64 0, i32 0
; CHECK: %3 = load i8*, i8** %2
; CHECK: %4 = icmp ne i8* %3, null
; CHECK: br i1 %4, label %5, label %6

; CHECK: store i32 251, i32* @__Intel_PaddedMallocCounter
; CHECK: br label %6

; ===================================

; CHECK-LABEL: define{{.*}}init_with_coder2
; CHECK: %2 = getelementptr %struct.mynextcoder, %struct.mynextcoder* %0, i64 0, i32 0
; CHECK: %3 = load i8*, i8** %2
; CHECK: %4 = icmp ne i8* %3, null
; CHECK: br i1 %4, label %5, label %6
...
define dso_local noalias i8* @alloc(i64 %size) {
entry:
...
t5:                                      ; preds = %t1
  %t6 = tail call noalias i8* @alloc(i64 16) #4
...
define internal zeroext i1 @searchloop(%struct.mycoder2* %p1, %struct.mycoder2* %p2) {
entry:
  %pp1 = getelementptr inbounds %struct.mycoder2, %struct.mycoder2* %p1, i64 0, i32 0
  %pv1 = load i32*, i32** %pp1
  %pp2 = getelementptr inbounds %struct.mycoder2, %struct.mycoder2* %p2, i64 0, i32 0
  %pv2 = load i32*, i32** %pp2
  br label %bb2
...
bb2:
  ...
  %t5 = getelementptr inbounds i32, i32* %pv1, i64 %t4
  %t6 = load i32, i32* %t5, align 4
  %t7 = getelementptr inbounds i32, i32* %pv2, i64 %t4
