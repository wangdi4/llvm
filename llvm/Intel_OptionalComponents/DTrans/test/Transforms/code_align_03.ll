...
;
; RUN: opt < %s -dtrans-codealign -whole-program-assume -intel-libirc-allowed -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -disable-output -debug-only=dtrans-codealign 2>&1 | FileCheck %s
; RUN: opt < %s -passes=dtrans-codealign -whole-program-assume -intel-libirc-allowed -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -disable-output -debug-only=dtrans-codealign 2>&1 | FileCheck %s
