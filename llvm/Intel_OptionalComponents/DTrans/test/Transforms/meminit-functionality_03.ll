...
; recognizing destructor, set, get, getCapacity and getSize member
...
; destructor: Not all elements are freed. Loop starts at 1.
;             "for(unsigned i = 1"
;
; set: "if (i >= size)" check is missing.
;
; get: Not returning expected element of array.
;      Returning "base[i - 1]" instead of "base[i]".
;
; getCapacity: Returning "capacity << 1" instead of "capacity".
;
; getSize: Returning "size + 1" instead of "size".
;
;  struct Mem {
...
;CHECK-DAG:  Functionality of _ZN3ArrIPiE7getSizeEv: Failed to recognize as GetSizeOrCapacity
;CHECK-DAG:  Functionality of _ZN3ArrIPiE11getCapacityEv: Failed to recognize as GetSizeOrCapacity
...
if.then:                                          ; preds = %entry
  ...
  %2 = bitcast float*** %arrayidx to i8**
  %3 = load i8*, i8** %2
  tail call void @free(i8* %3)
  br label %if.end
