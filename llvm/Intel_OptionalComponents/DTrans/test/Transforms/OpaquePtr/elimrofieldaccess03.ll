; REQUIRES: asserts
; RUN: opt -force-opaque-pointers < %s -whole-program-assume -dtrans-elim-ro-field-access-op -debug-only=elim-ro-field-access -disable-output 2>&1 | FileCheck %s
; RUN: opt -force-opaque-pointers < %s -whole-program-assume -passes=dtrans-elim-ro-field-access-op -debug-only=elim-ro-field-access -disable-output 2>&1 | FileCheck %s
