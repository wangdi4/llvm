...
; recognize _ZN3ArrIPiEC2EiP3Mem as constructor since both size and
; capacity fields are assigned with non-zero constants.
...
;      : flag(false), capacity(c), size(c), base(0), mem(mem) {
...
; Both i32 fields (size and capacity) are initialized with non-zero
; constants. So, this will not be considered as valid constructor.
...
declare !intel.dtrans.func.type !36 noalias "intel_dtrans_func_index"="1" ptr @malloc(i64)
