...
; REQUIRES: asserts
;  RUN: opt < %s -disable-output -dtrans-simple-dynclone-enable=false -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -whole-program-assume -dtrans-dyncloneop -debug-only=dtrans-dynclone-reencoding 2>&1 | FileCheck %s
;  RUN: opt < %s -disable-output -dtrans-simple-dynclone-enable=false -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -whole-program-assume -passes=dtrans-dyncloneop -debug-only=dtrans-dynclone-reencoding 2>&1 | FileCheck %s
