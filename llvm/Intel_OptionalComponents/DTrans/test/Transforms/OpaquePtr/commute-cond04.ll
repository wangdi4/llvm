; This test verifies that the CommuteCond transformation is triggered for
; this test when -dtrans-commute-cond-ignore-heuristic is passed.
; Verify that the Condition and TrueValue operands of %sel1 are swapped.

;  RUN: opt < %s -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -whole-program-assume -dtrans-commutecondop -dtrans-commute-cond-ignore-heuristic -S 2>&1 | FileCheck %s
;  RUN: opt < %s -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -whole-program-assume -passes=dtrans-commutecondop -dtrans-commute-cond-ignore-heuristic  -S 2>&1 | FileCheck %s
