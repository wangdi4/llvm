; RUN: opt < %s -opaque-pointers -S -passes='lto-pre-link<O3>' -enable-npm-dtrans -inline-report=0xe807 -dtrans-inline-heuristics -inline-for-xmain 2>&1 | FileCheck --check-prefixes=CHECK-SUPP,CHECK-SUPP-CL %s
; RUN: opt < %s -opaque-pointers -S -passes='lto<O3>' -enable-npm-dtrans -inline-report=0xe807 -dtrans-inline-heuristics -inline-for-xmain 2>&1 | FileCheck --check-prefixes=CHECK-NINL,CHECK-NINL-CL %s
; RUN: opt < %s -S -opaque-pointers -passes='lto-pre-link<O3>' -enable-npm-dtrans -inline-report=0xe886 -dtrans-inline-heuristics -inline-for-xmain 2>&1 | FileCheck --check-prefixes=CHECK-SUPP,CHECK-SUPP-MD %s
; RUN: opt < %s -S -opaque-pointers -passes='lto<O3>' -enable-npm-dtrans -inline-report=0xe886 -dtrans-inline-heuristics -inline-for-xmain 2>&1 | FileCheck --check-prefixes=CHECK-NINL,CHECK-NINL-MD %s
