...
; RUN:          -passes='internalize,require<dtrans-safetyanalyzer>,function(require<soatoaosop-approx>)'            \
; RUN:       2>&1 | FileCheck --check-prefix=CHECK-DEP %s
; RUN: opt < %s -whole-program-assume -disable-output                                           \
; RUN:          -debug-only=dtrans-soatoaosop,dtrans-soatoaosop-struct                              \
; RUN:          -passes='internalize,require<dtrans-safetyanalyzer>,function(require<soatoaosop-approx>,require<soatoaosop-struct-methods>)' \
