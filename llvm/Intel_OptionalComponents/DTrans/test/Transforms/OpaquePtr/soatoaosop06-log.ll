; RUN: opt < %S/soatoaosop06-exe.ll -S -whole-program-assume -dtrans-soatoaosop -disable-output         \
...
; RUN: opt < %S/soatoaosop06-exe.ll -S -whole-program-assume -passes=dtrans-soatoaosop -disable-output  \
; RUN:          -dtrans-soatoaosop-size-heuristic=false                       \
; RUN:          -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2                            \
; RUN:          -debug-only=dtrans-soatoaosop,dtrans-soatoaosop-arrays,dtrans-soatoaosop-struct           \
; RUN:  2>&1 | FileCheck %s
