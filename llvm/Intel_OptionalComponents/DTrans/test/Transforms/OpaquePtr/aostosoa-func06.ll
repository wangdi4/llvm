...
; Test AOS-to-SOA transformation uses special typed parameter attributes
; (such as: byref, byval, sret, ...). These should get removed when the
; type gets transformed from being a pointer parameter. This is the same
; as aostosoa-func01.ll, but contains the special typed attribute.
...
define i32 @test01() {
  ...
  call void @test02(%struct.test01* byref(%struct.test01) %v0)
  ret i32 0
}
...
define void @test02(%struct.test01* "intel_dtrans_func_index"="1" byref(%struct.test01) %in) !intel.dtrans.func.type !3 {
