; REQUIRES: asserts
; RUN: opt < %s -force-opaque-pointers -whole-program-assume -dtrans-elim-ro-field-access-op -S -debug-only=elim-ro-field-access 2>&1 | FileCheck %s
; RUN: opt < %s -force-opaque-pointers -whole-program-assume -passes=dtrans-elim-ro-field-access-op -S -debug-only=elim-ro-field-access 2>&1 | FileCheck %s
