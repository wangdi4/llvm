; REQUIRES: asserts
; RUN: opt -opaque-pointers -S -whole-program-assume -intel-libirc-allowed -passes=dtrans-aostosoaop -dtrans-aostosoaop-index32=true -dtrans-aostosoaop-typelist=struct.test01 -dtrans-aostosoaop-qual-override=true %s 2>&1 | FileCheck %s
...
; CHECK: %alloc_idx1 = call ptr @llvm.ptr.annotation.p0(ptr %p2, ptr @__intel_dtrans_aostosoa_index, ptr @__intel_dtrans_aostosoa_filename, i32 0, ptr null)
...
; CHECK-SAME: !invariant.load
; CHECK: %[[ZEXT2:[0-9]+]] = zext i32 %p1 to i64
; CHECK: %p3 = getelementptr i32, ptr %[[LOAD2]], i64 %[[ZEXT2]]
