; RUN: opt < %s -dtransop-allow-typed-pointers -dtrans-soatoaosop -whole-program-assume -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -debug-only=dtrans-soatoaosop -disable-output 2>&1 | FileCheck %s
; RUN: opt < %s -dtransop-allow-typed-pointers -passes=dtrans-soatoaosop -whole-program-assume -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -debug-only=dtrans-soatoaosop -disable-output 2>&1 | FileCheck %s
...
; This test is same as soatoaos_cand_01.ll except nocapture attribute is
; missing for arguments of _ZN3ArrIPiEC2EiP3Mem.
; This test verifies that %class.F is not considered as candidate due to
; missing nocapture attribute.
...
; int main() { F *f = new F(); }
;
; CHECK: dtrans-soatoaos-layout: array method captures unexpected parameter.
...
define void @_ZN1FC2Ev(%class.F* "intel_dtrans_func_index"="1" %this) align 2 personality i8* bitcast (i32 (...)* @__gxx_personality_v0 to i8*) !intel.dtrans.func.type !25 {
entry:
  %call = call i8* @_Znwm(i64 32)
...
define void @_ZN3ArrIPiEC2EiP3Mem(%struct.Arr* "intel_dtrans_func_index"="1" %this, i32 %c, %struct.Mem* "intel_dtrans_func_index"="2" %mem) align 2 !intel.dtrans.func.type !38 {
entry:
