...
; track uses of memory allocation because a store instruction, which
; stores %tp3 to @globvar, is not processed because it is not in
; expected location like AOSTOSOA glob variable.
...
; This variable is used to store a use of memory allocation.
@globvar = internal global %struct.test.01* zeroinitializer, align 8, !intel_dtrans_type !6

; This routine is selected as InitRoutine.
; Uses of %call1 memory allocation pointer (%tp1 and  %tp2) are stored in
; 2nd and 3rd array fields of @n. But, DynCloning is disabled due to %tp3.
...
; Store %tp3 to @globvar. This is not in expected form (i.e SimplePtrStore
; or ComplexPtrStor). This causes DynClone to be disabled.
  store %struct.test.01* %tp3, %struct.test.01** @globvar
