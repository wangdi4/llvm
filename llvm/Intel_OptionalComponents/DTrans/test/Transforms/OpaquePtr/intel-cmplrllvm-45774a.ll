; REQUIRES: asserts
; RUN: opt < %s -passes='module(dtrans-force-inline-op)' -S -debug-only=dtrans-soatoaosop 2>&1 | FileCheck %s

; Test that the types starting with "%eh." or "%rtti." are recognized as
; skippable types. This is necessary because the C/C++ front end will not
; emit DTrans metadata for them.

; CHECK: Skippable type %eh.CatchableTypeArray.3
; CHECK: Skippable type %eh.ThrowInfo
; CHECK: Skippable type %eh.CatchableType
; CHECK: Skippable type %rtti.TypeDescriptor43
