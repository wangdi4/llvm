...
;  RUN: opt < %s -S -opaque-pointers -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -whole-program-assume -dtrans-commutecondop 2>&1 | FileCheck %s
;  RUN: opt < %s -S -opaque-pointers -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -whole-program-assume -passes=dtrans-commutecondop 2>&1 | FileCheck %s
