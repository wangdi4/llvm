...
; used with memintrisic calls. These could be supported, but are not currently
; needed, and require a lot of code to rewrite the memintrinsic call site.
;
; %struct.test01 is not supported because is used for memset, which is
...
define void @test02() {
  ...
  %raw = bitcast %struct.test01* %st to i8*
  call void @llvm.memset.p0i8.i64(i8* %raw, i8 0, i64 240, i1 false)
  ret void
}
