...
%struct.dep = type { i32, ptr }
@g_del = private global %struct.test { i32 100, i64 1000, i32 10000 }, align 4
@g_dep = private global %struct.dep { i32 1, ptr @g_del }, align 4


; Check order with opaque pointers is different because only one variable is cloned.
; CHECK-DAG: %struct.dep = type { i32, ptr }
; CHECK-DAG: %__DFT_struct.test = type { i32, i32 }
; CHECK-DAG: @g_dep = private global %struct.dep { i32 1, ptr @g_del }, align 4
; CHECK-DAG: @g_del = private global %__DFT_struct.test { i32 100, i32 10000 }, align 4
...
define i32 @main(i32 %argc, ptr "intel_dtrans_func_index"="1" %argv) !intel.dtrans.func.type !5 {
  ...
  ; transformed.
  %val = load i32, ptr getelementptr inbounds (%struct.dep, ptr @g_dep, i64 0, i32 0)

  %ptr = load ptr, ptr getelementptr inbounds (%struct.dep, ptr @g_dep, i64 0, i32 1)

  %same = icmp eq ptr %ptr, @g_del
...
t1:
  ...
  %p = load ptr, ptr getelementptr inbounds (%struct.dep, ptr @g_dep, i64 0, i32 1)

  ; read A and C
  %valA = load i32, ptr getelementptr inbounds (%struct.test, ptr @g_del, i64 0, i32 0)
  %valC = load i32, ptr getelementptr inbounds (%struct.test, ptr @g_del, i64 0, i32 2)

  ; write B
  store i64 3, ptr getelementptr (%struct.test, ptr @g_del, i64 0, i32 1)
