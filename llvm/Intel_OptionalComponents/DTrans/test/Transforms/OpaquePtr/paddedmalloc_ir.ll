; RUN: opt < %s -whole-program-assume -intel-libirc-allowed -passes=dtrans-paddedmallocop -dtrans-test-paddedmalloc  -S 2>&1 | FileCheck %s
; RUN: opt < %s -whole-program-assume -intel-libirc-allowed -passes="dtrans-paddedmallocop,padded-pointer-prop-op" -S 2>&1 | FileCheck %s --check-prefix=CHECK-PROP
