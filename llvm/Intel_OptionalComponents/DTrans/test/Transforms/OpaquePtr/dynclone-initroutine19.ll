...
; called in loop. init routine has only one unsafe call.
...
; This is used to store pointer of %struct.test.01.
%struct.netw = type { %struct.test.01*, %struct.test.01* }
@glob = internal global %struct.netw zeroinitializer, align 8

; This routine has only one unsafe call that is called in loop.
