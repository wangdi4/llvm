...
; RUN: opt < %s -dtrans-soatoaosop-prepare  -whole-program-assume -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -S 2>&1 | FileCheck %s
; RUN: opt < %s -passes=dtrans-soatoaosop-prepare  -whole-program-assume -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -S 2>&1 | FileCheck %s
; RUN: opt < %s -opaque-pointers -dtrans-soatoaosop-prepare  -whole-program-assume -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -S 2>&1 | FileCheck --check-prefix=CHECK-OP %s
; RUN: opt < %s -opaque-pointers -passes=dtrans-soatoaosop-prepare  -whole-program-assume -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -S 2>&1 | FileCheck --check-prefix=CHECK-OP %s
...
; CHECK: define linkonce_odr dso_local void @_ZN6RefArrIPsEC2EjbP3Mem(%struct.RefArr* nocapture  "intel_dtrans_func_index"="1" %this
...
; CHECK: define linkonce_odr dso_local "intel_dtrans_func_index"="1" i16** @_ZN7BaseArrIPsE3getEj(%struct.BaseArr* nocapture  "intel_dtrans_func_index"="2" %this
; CHECK: %size = getelementptr inbounds %struct.BaseArr, %struct.BaseArr* %this, i64 0, i32 3


; Checking for cloned routines starts here.

; CHECK: define linkonce_odr dso_local void @_ZN1FC2Ev{{.*}}(%_DPRE_class.F*  "intel_dtrans_func_index"="1" %this)
...
; CHECK: define linkonce_odr dso_local void @_ZN7BaseArrIPsEC2EjbP3Mem{{.*}}{{.*}}(%_DPRE__REP_struct.RefArr* nocapture  "intel_dtrans_func_index"="1" %this, i32 %c, %struct.Mem* "intel_dtrans_func_index"="2" %mem, i1 %0)
; CHECK-NEXT: entry:
...
; Make sure GEPs are fixed correctly in cloned routines.
; CHECK: define linkonce_odr dso_local "intel_dtrans_func_index"="1" i16** @_ZN7BaseArrIPsE3getEj{{.*}}{{.*}}(%_DPRE__REP_struct.RefArr*
; CHECK-NEXT: entry:
; CHECK-NEXT: %size = getelementptr inbounds %_DPRE__REP_struct.RefArr, %_DPRE__REP_struct.RefArr* %this, i64 0, i32 2

; CHECK: define linkonce_odr dso_local void @_ZN7BaseArrIPsE3setEjPS0_{{.*}}{{.*}}(%_DPRE__REP_struct.RefArr
...
; with cloned calls.
; CHECK: define linkonce_odr dso_local void @_ZN7BaseArrIPsE3addEPS0_{{.*}}(%_DPRE__REP_struct.RefArr
; CHECK-NEXT: entry:
...
; CHECK: define linkonce_odr dso_local void @_ZN7BaseArrIPsE6resizeEi{{.*}}(%_DPRE__REP_struct.RefArr*
...
; Make sure all member functions are cloned
; CHECK: define linkonce_odr dso_local i32 @_ZN7BaseArrIPsE7getSizeEv{{.*}}{{.*}}(%_DPRE__REP_struct.RefArr*

; CHECK: define linkonce_odr dso_local i32 @_ZN7BaseArrIPsE11getCapacityEv{{.*}}{{.*}}(%_DPRE__REP_struct.RefArr*
; CHECK-NEXT: entry:
...
; CHECK-NOT: define linkonce_odr dso_local void @_ZN6RefArrIPsED0Ev{{.*}}{{.*}}(%_DPRE__REP_struct.RefArr*

; Make sure use of flag is replaced by constant 1.
; CHECK: define linkonce_odr dso_local void @_ZN6RefArrIPsED2Ev{{.*}}{{.*}}(%_DPRE__REP_struct.RefArr*
; CHECK:  %tobool = icmp eq i8 1, 0
...
; CHEC-OP: tail call void @_ZN1FC2Ev(

; CHECK-OP: define linkonce_odr dso_local void @_ZN1FC2Ev(ptr "intel_dtrans_func_index"="1" %this)

; Check allocation size is updated
; CHECK-OP:  %call5 = tail call ptr @_Znwm(i64 32)

; Make sure dead flag argument is moved to the end of arg list and
; value of flag argument is changed from true to false.
; CHECK-OP: call void @_ZN7BaseArrIPsEC2EjbP3Mem{{.*}}{{.*}}(ptr %call5, i32 10, ptr null, i1 zeroext false)

; Check all member function calls are replaced with cloned calls.
; CHECK-OP: call ptr @_ZN7BaseArrIPsE3getEj{{.*}}{{.*}}
; CHECK-OP: call void @_ZN7BaseArrIPsE3setEjPS0_{{.*}}{{.*}}
; CHECK-OP: call void @_ZN7BaseArrIPsE3addEPS0_{{.*}}{{.*}}
; CHECK-OP: call i32 @_ZN7BaseArrIPsE7getSizeEv{{.*}}{{.*}}

; CHECK-OP: call void @_ZN7BaseArrIPsEC2EjbP3Mem{{.*}}{{.*}}
; Make sure Ctor wrapper is not called here
; CHECK-OP-NOT: call void @_ZN6RefArrIPsEC2EjbP3Mem{{.*}}{{.*}}

; CHECK-OP: call i32 @_ZN7BaseArrIPsE11getCapacityEv{{.*}}{{.*}}
; CHECK-OP: call void @_ZN7BaseArrIPsE3addEPS0_{{.*}}{{.*}}

; Make sure Dtor wrapper is not called.
; CHECK-OP: call void @_ZN6RefArrIPsED2Ev{{.*}}{{.*}}
; CHECK-OP-NOT: call void @_ZN6RefArrIPsED0Ev{{.*}}{{.*}}


; Make sure Ctor wrapper is deleted.
; CHECK-OP-NOT: define void @_ZN6RefArrIPsEC2EjbP3Mem{{.*}}{{.*}}(ptr
; Make sure original member function is not modified.
; CHECK-OP: define linkonce_odr dso_local void @_ZN6RefArrIPsEC2EjbP3Mem(ptr nocapture  "intel_dtrans_func_index"="1" %this
; CHECK-OP: %0 = getelementptr inbounds %struct.RefArr, ptr %this, i64 0, i32 0
; CHECK-OP: %1 = getelementptr inbounds %struct.RefArr, ptr %this, i64 0, i32 0, i32 0

; Make sure field accesses are not modified in original member function.
; CHECK-OP: define linkonce_odr dso_local "intel_dtrans_func_index"="1" ptr @_ZN7BaseArrIPsE3getEj(ptr nocapture  "intel_dtrans_func_index"="2" %this
; CHECK-OP: %size = getelementptr inbounds %struct.BaseArr, ptr %this, i64 0, i32 3
...
; Make sure GEPs are fixed correctly in cloned routines.
; Make sure unused flag argument is moved to the end of arg list.
; CHECK-OP: define linkonce_odr dso_local void @_ZN7BaseArrIPsEC2EjbP3Mem.{{[0-9]+}}(ptr nocapture  "intel_dtrans_func_index"="1" %this, i32 %c, ptr "intel_dtrans_func_index"="2" %mem, i1 %0) #{{[0-9]+}} !intel.dtrans.func.type ![[FMD1:[0-9]+]] {
; CHECK-OP-NEXT: entry:
; CHECK-OP-NEXT: %flag = getelementptr inbounds %_DPRE__REP_struct.RefArr, ptr %this, i64 0, i32 0
; Make sure zero is saved to the %flag field.
; CHECK-OP-NEXT: store i8 0, ptr %flag

; Make sure GEPs are fixed correctly in cloned routines.
; CHECK-OP: define linkonce_odr dso_local "intel_dtrans_func_index"="1" ptr @_ZN7BaseArrIPsE3getEj.{{[0-9]+}}(ptr nocapture "intel_dtrans_func_index"="2" %this, i32 %i) #{{[0-9]+}} !intel.dtrans.func.type ![[FMD2:[0-9]+]] {
; CHECK-OP-NEXT: entry:
; CHECK-OP-NEXT: %size = getelementptr inbounds %_DPRE__REP_struct.RefArr, ptr %this, i64 0, i32 2

; CHECK-OP: define linkonce_odr dso_local void @_ZN7BaseArrIPsE3setEjPS0_{{.*}}(ptr
; Make sure value of flag is replaced by constant 1.
; CHECK-OP: %tobool = icmp eq i8 1, 0
; CHECK-OP-NOT:  %flag = getelementptr inbounds %struct.BaseArr, ptr %this, i64 0, i32 1
; CHECK-OP-NOT:  %1 = load i8, ptr %flag, align 8
; CHECK-OP-NOT:  %tobool = icmp eq i8 %1, 0
; Make sure GEPs are fixed correctly in cloned routines.
; CHECK-OP: %base = getelementptr inbounds %_DPRE__REP_struct.RefArr, ptr %this, i64 0, i32 3

; Checking that original calls in cloned member functions are replaced
; with cloned calls.
; CHECK-OP: define linkonce_odr dso_local void @_ZN7BaseArrIPsE3addEPS0_.{{[0-9]+}}(ptr nocapture "intel_dtrans_func_index"="1" %this, ptr "intel_dtrans_func_index"="2" %val) #{{[0-9]+}} !intel.dtrans.func.type ![[FMD3:[0-9]+]] {
; CHECK-OP-NEXT: entry:
; CHECK-OP-NEXT: tail call void @_ZN7BaseArrIPsE6resizeEi{{.*}}(ptr %this, i32 1)

; Make sure all member functions are cloned
; CHECK-OP: define linkonce_odr dso_local void @_ZN7BaseArrIPsE6resizeEi{{.*}}(ptr
; Make sure peephole optimization is done by converting
; from "shl + add" to "add + shl".
; CHECK-OP:  [[A0:%[0-9]*]] = add i32 %spec.select,
; CHECK-OP:  [[ZEXT1:%[0-9]*]] = zext i32 [[A0]]
; CHECK-OP:  [[ADD1:%[0-9]*]] = add nuw nsw i64 [[ZEXT1]], 1
; CHECK-OP:  [[SHL1:%[0-9]*]] = shl nuw nsw i64 [[ADD1]], 3
; CHECK-OP:  call void @llvm.memset.p0.i64(ptr nonnull align 8 dereferenceable(1) %scevgep, i8 0, i64 [[SHL1]], i1 false)

; Make sure all member functions are cloned
; CHECK-OP: define linkonce_odr dso_local i32 @_ZN7BaseArrIPsE7getSizeEv{{.*}}(ptr

; CHECK-OP: define linkonce_odr dso_local i32 @_ZN7BaseArrIPsE11getCapacityEv{{.*}}(ptr
; CHECK-OP-NEXT: entry:
; CHECK-OP-NEXT: %capacity = getelementptr inbounds %_DPRE__REP_struct.RefArr, ptr %this, i64 0, i32 1

; Make sure llvm.assume and llvm.type.test calls are removed.
; CHECK-OP-NOT: %tt2 = tail call i1 @llvm.type.test(ptr %bc2, metadata !"typeId1")
; CHECK-OP-NOT: tail call void @llvm.assume(i1 %tt2)

; Make sure Dtor wrapper is deleted.
; CHECK-OP-NOT: define linkonce_odr dso_local void @_ZN6RefArrIPsED0Ev{{.*}}(%_DPRE__REP_struct.RefArr*

; Make sure use of flag is replaced by constant 1.
; CHECK-OP: define linkonce_odr dso_local void @_ZN6RefArrIPsED2Ev.{{[0-9]+}}(ptr nocapture "intel_dtrans_func_index"="1" %this) #{{[0-9]+}} !intel.dtrans.func.type ![[FMD4:[0-9]+]]  {
; CHECK-OP:  %tobool = icmp eq i8 1, 0
; CHECK-OP-NOT:  %flag = getelementptr inbounds %struct.BaseArr, ptr %1, i64 0, i32 1
; CHECK-OP-NOT:  %2 = load i8, ptr %flag
; CHECK-OP-NOT:  %tobool = icmp eq i8 %2, 0


; CHECK-OP: ![[S1:[0-9]+]] = !{%struct.Mem zeroinitializer, i32 1}
; CHECK-OP: ![[S2:[0-9]+]] = !{%_DPRE__REP_struct.RefArr zeroinitializer, i32 1}
; CHECK-OP: ![[M1:[0-9]+]] = !{i32 0, i32 0}
; CHECK-OP: ![[M2:[0-9]+]] = !{i8 0, i32 0}
; CHECK-OP: ![[M3:[0-9]+]] = !{i16 0, i32 3}
; CHECK-OP: = !{!"S", %_DPRE__REP_struct.RefArr zeroinitializer, i32 5, ![[M2]], ![[M1]], ![[M1]], ![[M3]], ![[S1]]}
; CHECK-OP: ![[M4:[0-9]+]] = !{i16 0, i32 2}
; CHECK-OP: ![[FMD1]] = distinct !{![[S2]], ![[S1]]}
; CHECK-OP: ![[FMD2]] = distinct !{![[M4]], ![[S2]]}
; CHECK-OP: ![[FMD3]] = distinct !{![[S2]], ![[M4]]}
; CHECK-OP: ![[FMD4]] = distinct !{![[S2]]}
