...
; CHECK: define linkonce_odr dso_local void @_ZN1FC2Ev(ptr "intel_dtrans_func_index"="1" %this)

; Check allocation size is updated
; CHECK:  %call5 = tail call ptr @_Znwm(i64 32)

; Make sure dead flag argument is moved to the end of arg list and
; value of flag argument is changed from true to false.
; CHECK: call void @_ZN7BaseArrIPsEC2EjbP3Mem{{.*}}{{.*}}(ptr %call5, i32 10, ptr null, i1 zeroext false)

; Check all member function calls are replaced with cloned calls.
; CHECK: call ptr @_ZN7BaseArrIPsE3getEj{{.*}}{{.*}}
...
; CHECK-NOT: define void @_ZN6RefArrIPsEC2EjbP3Mem{{.*}}{{.*}}(ptr
; Make sure original member function is not modified.
; CHECK: define linkonce_odr dso_local void @_ZN6RefArrIPsEC2EjbP3Mem(ptr nocapture  "intel_dtrans_func_index"="1" %this
; CHECK: %i = getelementptr inbounds %struct.RefArr, ptr %this, i64 0, i32 0
; CHECK: %i1 = getelementptr inbounds %struct.RefArr, ptr %this, i64 0, i32 0, i32 0

; Make sure field accesses are not modified in original member function.
; CHECK: define linkonce_odr dso_local "intel_dtrans_func_index"="1" ptr @_ZN7BaseArrIPsE3getEj(ptr nocapture  "intel_dtrans_func_index"="2" %this
; CHECK: %size = getelementptr inbounds %struct.BaseArr, ptr %this, i64 0, i32 3
...
; CHECK: define linkonce_odr dso_local void @_ZN7BaseArrIPsEC2EjbP3Mem.{{[0-9]+}}(ptr nocapture  "intel_dtrans_func_index"="1" %this, i32 %c, ptr "intel_dtrans_func_index"="2" %mem, i1 %0) #{{[0-9]+}} !intel.dtrans.func.type ![[FMD1:[0-9]+]] {
; CHECK-NEXT: entry:
; CHECK-NEXT: %flag = getelementptr inbounds %_DPRE__REP_struct.RefArr, ptr %this, i64 0, i32 0
; Make sure zero is saved to the %flag field.
; CHECK-NEXT: store i8 0, ptr %flag

; Make sure GEPs are fixed correctly in cloned routines.
; CHECK: define linkonce_odr dso_local "intel_dtrans_func_index"="1" ptr @_ZN7BaseArrIPsE3getEj.{{[0-9]+}}(ptr nocapture "intel_dtrans_func_index"="2" %this, i32 %i) #{{[0-9]+}} !intel.dtrans.func.type ![[FMD2:[0-9]+]] {
; CHECK-NEXT: entry:
; CHECK-NEXT: %size = getelementptr inbounds %_DPRE__REP_struct.RefArr, ptr %this, i64 0, i32 2

; CHECK: define linkonce_odr dso_local void @_ZN7BaseArrIPsE3setEjPS0_{{.*}}(ptr
; Make sure value of flag is replaced by constant 1.
; CHECK: %tobool = icmp eq i8 1, 0
; CHECK-NOT:  %flag = getelementptr inbounds %struct.BaseArr, ptr %this, i64 0, i32 1
; CHECK-NOT:  %i1 = load i8, ptr %flag, align 8
; CHECK-NOT:  %tobool = icmp eq i8 %i1, 0
; Make sure GEPs are fixed correctly in cloned routines.
; CHECK: %base = getelementptr inbounds %_DPRE__REP_struct.RefArr, ptr %this, i64 0, i32 3

; Checking that original calls in cloned member functions are replaced
; with cloned calls.
; CHECK: define linkonce_odr dso_local void @_ZN7BaseArrIPsE3addEPS0_.{{[0-9]+}}(ptr nocapture "intel_dtrans_func_index"="1" %this, ptr "intel_dtrans_func_index"="2" %val) #{{[0-9]+}} !intel.dtrans.func.type ![[FMD3:[0-9]+]] {
; CHECK-NEXT: entry:
; CHECK-NEXT: tail call void @_ZN7BaseArrIPsE6resizeEi{{.*}}(ptr %this, i32 1)

; Make sure all member functions are cloned
; CHECK: define linkonce_odr dso_local void @_ZN7BaseArrIPsE6resizeEi{{.*}}(ptr
; Make sure peephole optimization is done by converting
; from "shl + add" to "add + shl".
; CHECK:  [[A0:%i[0-9]*]] = add i32 %spec.select,
; CHECK:  [[ZEXT1:%i[0-9]*]] = zext i32 [[A0]]
; CHECK:  [[ADD1:%[0-9]*]] = add nuw nsw i64 [[ZEXT1]], 1
; CHECK:  [[SHL1:%i[0-9]*]] = shl nuw nsw i64 [[ADD1]], 3
; CHECK:  call void @llvm.memset.p0.i64(ptr nonnull align 8 dereferenceable(1) %scevgep, i8 0, i64 [[SHL1]], i1 false)

; Make sure all member functions are cloned
; CHECK: define linkonce_odr dso_local i32 @_ZN7BaseArrIPsE7getSizeEv{{.*}}(ptr

; CHECK: define linkonce_odr dso_local i32 @_ZN7BaseArrIPsE11getCapacityEv{{.*}}(ptr
; CHECK-NEXT: entry:
; CHECK-NEXT: %capacity = getelementptr inbounds %_DPRE__REP_struct.RefArr, ptr %this, i64 0, i32 1

; Make sure llvm.assume and llvm.type.test calls are removed.
; CHECK-NOT: %tt2 = tail call i1 @llvm.type.test(ptr %vtable55, metadata !"typeId1")
...
; CHECK: define linkonce_odr dso_local void @_ZN6RefArrIPsED2Ev.{{[0-9]+}}(ptr nocapture "intel_dtrans_func_index"="1" %this) #{{[0-9]+}} !intel.dtrans.func.type ![[FMD4:[0-9]+]]  {
; CHECK:  %tobool = icmp eq i8 1, 0
; CHECK-NOT:  %flag = getelementptr inbounds %struct.BaseArr, ptr %i1, i64 0, i32 1
; CHECK-NOT:  %i2 = load i8, ptr %flag
; CHECK-NOT:  %tobool = icmp eq i8 %i2, 0
...
; CHECK: ![[M4:[0-9]+]] = !{i16 0, i32 2}
; CHECK: ![[FMD1]] = distinct !{![[S2]], ![[S1]]}
; CHECK: ![[FMD2]] = distinct !{![[M4]], ![[S2]]}
; CHECK: ![[FMD3]] = distinct !{![[S2]], ![[M4]]}
; CHECK: ![[FMD4]] = distinct !{![[S2]]}
...
delete.notnull54:                                 ; preds = %delete.end51
  ...
  %tt2 = tail call i1 @llvm.type.test(ptr %vtable55, metadata !"typeId1")
...
declare !intel.dtrans.func.type !62 i1 @llvm.type.test(ptr, metadata) #2
