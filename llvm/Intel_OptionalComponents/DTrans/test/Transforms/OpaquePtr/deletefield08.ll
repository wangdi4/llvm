...
; CHECK: %p = call {{.*}} @calloc(i64 8, i64 8)
...
; CHECK: %p = call {{.*}} @calloc(i64 %sz, i64 8)
...
; CHECK: %sz = mul i64 %val, 8
; CHECK: %p = call {{.*}} @calloc(i64 8, i64 %sz)
...
; CHECK: %p = call {{.*}} @calloc(i64 %sz64, i64 8)
...
; CHECK: %sz = mul i32 %val, 8
; CHECK: %sz64 = sext i32 %sz to i64
; CHECK: %p = call {{.*}} @calloc(i64 8, i64 %sz64)
...
; CHECK: %p = call {{.*}} @calloc(i64 8, i64 16)
...
; CHECK: %p = call {{.*}} @calloc(i64 %sz.dt, i64 %sz)
...
; CHECK: %p = call {{.*}} @calloc(i64 %sz.dt, i64 %other)
