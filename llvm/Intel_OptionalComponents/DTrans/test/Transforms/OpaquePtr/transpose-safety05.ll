...
bb:
  ...
  store ptr @test_var03, ptr %"var$14_$field0$", align 8
  store i64 1, ptr %"var$14_$field3$", align 8
  call void @test03dv(ptr %"var$14", ptr %otherval)
  ret void
}
...
altpath1:                                         ; No predecessors!
  br label %altpath2

altpath2:                                         ; preds = %altpath1, %entry
;TEST_SAFE_PHI:  %addr = phi ptr [  %"AnotherMYBLOCK_$field0$1", %entry], [%"MYBLOCK_$field0$1", %altpath1]
;TEST_UNSAFE_PHI:  %addr = phi ptr [  %othervar, %entry], [%"MYBLOCK_$field0$1", %altpath1]
  br label %merge

merge:                                            ; preds = %altpath2, %entry
  %addr2 = phi ptr [ %"AnotherMYBLOCK_$field0$1", %entry ], [ %addr, %altpath2 ]
  %"AnotherMYBLOCK_$field0$2" = load ptr, ptr %"MYBLOCK_$field0$", align 8
;TEST_SAFE_SELECT:  %addr3 = select i1 undef, ptr %addr2, ptr %"AnotherMYBLOCK_$field0$2"
;TEST_UNSAFE_SELECT:  %addr3 = select i1 undef, ptr %othervar, ptr %"AnotherMYBLOCK_$field0$2"
  %t4 = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 1, i64 1, i64 %t3, ptr elementtype(i32) %addr3, i64 5)
