...
;  RUN: opt < %s -opaque-pointers -dtrans-dynclone-shrunken-type-width=16 -S -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -whole-program-assume -intel-libirc-allowed -internalize -dtrans-dyncloneop 2>&1 | FileCheck %s
...
; Fixed alloc size
  %tp1 = tail call ptr @calloc(i64 8, i64 48)
; CHECK: %tp1 = tail call ptr @calloc(i64 8, i64 26)

; Assignment for candidate fields dependancy.
  %I1 = getelementptr %struct.test.01, ptr %tp1, i32 0, i32 1
  %L = load i64, ptr %I1
  %I6 = getelementptr %struct.test.01, ptr %tp1, i32 0, i32 6
  store i64 %L, ptr %I6

; Fixed alloc size
  %tp2 = tail call ptr @malloc(i64 480)
; CHECK: %tp2 = tail call ptr @malloc(i64 260)

  %J1 = getelementptr %struct.test.01, ptr %tp2, i32 0, i32 1
  %K = load i64, ptr %J1
  %J6 = getelementptr %struct.test.01, ptr %tp2, i32 0, i32 6
  store i64 %K, ptr %J6

; Fixed alloc size
  %tp3 = tail call ptr @realloc(ptr %tp2, i64 48)
; CHECK: %tp3 = tail call ptr @realloc(ptr %tp2, i64 26)

  %M1 = getelementptr %struct.test.01, ptr %tp3, i32 0, i32 1
  %N = load i64, ptr %M1
  %M6 = getelementptr %struct.test.01, ptr %tp3, i32 0, i32 6
  store i64 %N, ptr %M6

; Fixed memset size
  call void @llvm.memset.p0.i64(ptr %tp3, i8 0, i64 48, i1 false)
; CHECK: call void @llvm.memset.p0.i64(ptr %tp3, i8 0, i64 26, i1 false)

; Fixed memcpy size
  call void @llvm.memcpy.p0.p0.i64(ptr %tp2, ptr %tp3, i64 48, i1 false)
; CHECK: call void @llvm.memcpy.p0.p0.i64(ptr %tp2, ptr %tp3, i64 26, i1 false)

; Fixed memmove size
  call void @llvm.memmove.p0.p0.i64(ptr %tp2, ptr %tp3, i64 48, i1 false)
; CHECK: call void @llvm.memmove.p0.p0.i64(ptr %tp2, ptr %tp3, i64 26, i1 false)

  %p1 = ptrtoint ptr %tp1 to i64
  %p2 = ptrtoint ptr %tp2 to i64
  %diff1 = sub i64  %p1, %p2
...
; routine.
; CHECK-LABEL:   define internal void @proc1{{.*}}
; CHECK: %tp1 = tail call ptr @calloc(i64 8, i64 48)
; CHECK: %tp2 = tail call ptr @malloc(i64 480)
; CHECK: %tp3 = tail call ptr @realloc(ptr %tp2, i64 48)
; CHECK: call void @llvm.memset.p0.i64(ptr %tp3, i8 0, i64 48, i1 false)
; CHECK: call void @llvm.memcpy.p0.p0.i64(ptr %tp2, ptr %tp3, i64 48, i1 false)
; CHECK: call void @llvm.memmove.p0.p0.i64(ptr %tp2, ptr %tp3, i64 48, i1 false)
