; This test case verifies that the Transpose optimization didn't run because
; opaque pointers are enabled.
;
; TODO: This test case needs to be updated when the transformation supports
; opaque pointers.

; RUN: opt < %s -S -opaque-pointers -dtrans-transpose -dtrans-transpose-override=test_var01,0,1 2>&1 | FileCheck %s
; RUN: opt < %s -S -opaque-pointers -passes=dtrans-transpose -dtrans-transpose-override=test_var01,0,1 2>&1 | FileCheck %s

; ModuleID = 'transpose-transform.ll'
source_filename = "transpose-transform.ll"
...
; CHECK: call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 1, i64 1, i64 36, ptr elementtype(i32) @test_var01, i64 0)
; CHECK: call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 0, i64 1, i64 4, ptr elementtype(i32) %ptr_part1, i64 0)
...
; CHECK: store i64 4, ptr %t0, align 8
...
; CHECK: store i64 36, ptr %t3, align 8
...
; CHECK: %test02dv_col_addr = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 1, i64 1, i64 %t3, ptr elementtype(i32) %"MYBLOCK_$field0$1", i64 5)
; CHECK: %test02dv_ptr_addr = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 0, i64 1, i64 %t1, ptr elementtype(i32) %test02dv_col_addr, i64 5)
...
; CHECK: %test02ul_col_addr = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 1, i64 1, i64 %stride1, ptr elementtype(i32) %"ul_arg_0.GEP_$field0$30", i64 0)
; CHECK: %test02ul_ptr_addr = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 0, i64 1, i64 %stride0, ptr elementtype(i32) %test02ul_col_addr, i64 0)
