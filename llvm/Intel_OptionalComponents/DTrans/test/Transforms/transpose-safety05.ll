...
; RUN: sed -e s/.TEST_UNSAFE_SELECT:// %s | sed -e s/.TEST_SAFE_PHI:// | opt -disable-output -dtrans-transpose -dtrans-transpose-print-candidates 2>&1 | FileCheck --check-prefix=CHECK-UNSAFE %s
; RUN: sed -e s/.TEST_UNSAFE_SELECT:// %s | sed -e s/.TEST_SAFE_PHI:// | opt -disable-output -dtrans-transpose -dtrans-transpose-print-candidates 2>&1 | FileCheck --check-prefix=CHECK-UNSAFE %s
...
;TEST_UNSAFE_SELECT:  %addr3 = select i1 undef, i32* %othervar, i32* %"AnotherMYBLOCK_$field0$2"
  ...
  %t4 = tail call i32* @llvm.intel.subscript.p0i32.i64.i64.p0i32.i64(i8 1, i64 1, i64 %t3, i32* %addr3, i64 5)
