; REQUIRES: asserts
; RUN: opt < %s -debug-only=dtrans-transpose-transform -disable-output -dtrans-transpose -dtrans-transpose-print-candidates  2>&1 | FileCheck %s
; RUN: opt < %s -debug-only=dtrans-transpose-transform -disable-output -passes=dtrans-transpose -dtrans-transpose-print-candidates 2>&1 | FileCheck %s
...
; transpose-debug-transform16.ll but it adds a call to the vararg function @foo.
