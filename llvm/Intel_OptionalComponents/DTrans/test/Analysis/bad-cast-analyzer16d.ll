...
; store. This bad bitcast comes from @mycharstring.
...
@mycharstring = internal global [6 x i8] c"hello\00", align 1
...
define internal fastcc void @init_with_coder1(%struct.mynextcoder* nocapture) unnamed_addr {
  ...
  store i8* getelementptr inbounds ([6 x i8], [6 x i8]* @mycharstring, i64 0, i64 0), i8** %myptr, align 8
...
t5:                                      ; preds = %t1
  ...
  %t6.0 = load i8*, i8** %myptr, align 8
  %t13.2 = bitcast i8* %t6.0 to %struct.mycoder1*
  br label %t11

t11:                                     ; preds = %t5, %t1
  %t12 = phi %struct.mycoder1* [ %t13.0, %1 ], [ %t13.2, %t5 ]
...
; CHECK: %t13.2 = bitcast i8* %t6.0 to %struct.mycoder1*
