...
; RUN: opt < %s -whole-program-assume -intel-libirc-allowed -dtrans-outofboundsok=false -dtransanalysis -dtrans-print-immutable-types -disable-output 2>&1 | FileCheck %s
...
; analysis pass and verifies if the analysis process is turned on by default.
...
; CHECK:   Field 0:
; CHECK:     Likely Values:
; CHECK:     Likely Indirect Array Values:
; CHECK:   Field 1:
; CHECK:     Likely Values:
; CHECK:     Likely Indirect Array Values:
