...
; flattened GEP. In this case we are checking when the field of interest is
; wrapped in another structure.
...
define void @baz(%class.TestClass* %0) {
  ...
  %tmp2 = bitcast %class.TestClass* %tmp1 to %"class.boost::array"*
  %tmp3 = load %"class.boost::array", %"class.boost::array"* %tmp2
  ret void
}
