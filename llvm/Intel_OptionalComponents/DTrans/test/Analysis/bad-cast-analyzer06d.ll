...
; This test case tests the bad cast analyzer on a case which has an ICmp that
; tests the candidate field against a non-null ptr. It should fail with a
...
@myglobalstring = internal global i8 15, align 4
...
define internal fastcc void @init_with_coder1(%struct.mynextcoder* nocapture) unnamed_addr {
  ...
  %t4 = icmp eq i8* %t3, @myglobalstring
...
; CHECK: dtrans-bca: (L) ICmp not in spec guard cond basic block:   %t4 = icmp eq i8* %t3, @myglobalstring
