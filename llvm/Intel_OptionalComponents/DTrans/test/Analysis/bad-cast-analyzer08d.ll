...
; casting and Unsafe pointer store violations.
...
t5:                                      ; preds = %t1
  ...
  %t13.2 = ptrtoint i8* %t6 to i64
  %t13.1 = inttoptr i64 %t13.2 to %struct.mycoder1*
...
; CHECK: dtrans-bca: (SV) [0] Store not from alloc or nullptr:   store i8* %t6, i8** %t2, align 8
; CHECK: %t2 = getelementptr inbounds %struct.mynextcoder, %struct.mynextcoder* %0, i64 0, i32 0
; CHECK: %t6 = tail call noalias i8* @malloc(i64 16)
; CHECK: dtrans-bca: Bad casting (pending) -- unknown pointer cast to type of interest:
