...
; includes related types" was printed for the call to "new" in function @test.
; The reason is because the pointer to the new memory space allocated won't
; have a dominant aggregate type since the field 0 of %class.TestClass.Inner
; is a base class, but is used in instruction %14 as a padded class.

; CHECK: dtrans-safety: Bad casting (related types) -- Allocation includes related types
; CHECK:   [foo]   %11 = invoke noalias noundef nonnull ptr @_Znwm(i64 noundef %7)
...
invoke.good:
  ...
  store ptr %12, ptr %0
  %13 = getelementptr inbounds %class.TestClass.Inner_vect, ptr %0, i64 0, i32 1
  store ptr %12, ptr %13
