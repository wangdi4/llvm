; REQUIRES: asserts
; RUN: opt  < %s -opaque-pointers -whole-program-assume -intel-libirc-allowed -debug-only=dtrans-alloc-collector -dtrans-ptrtypeanalyzertest -disable-output 2>&1 | FileCheck %s
