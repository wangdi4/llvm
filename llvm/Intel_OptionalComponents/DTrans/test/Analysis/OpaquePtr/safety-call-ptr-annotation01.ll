; REQUIRES: asserts
; RUN: opt -disable-output -whole-program-assume -dtrans-safetyanalyzer -dtrans-print-types < %s 2>&1 | FileCheck %s
; RUN: opt -disable-output -whole-program-assume -passes='require<dtrans-safetyanalyzer>' -dtrans-print-types < %s 2>&1 | FileCheck %s
