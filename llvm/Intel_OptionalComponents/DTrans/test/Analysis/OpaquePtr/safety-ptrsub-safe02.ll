; REQUIRES: asserts
; RUN: opt -whole-program-assume -dtrans-safetyanalyzer -debug-only=dtrans-safetyanalyzer-verbose -disable-output %s 2>&1 | FileCheck %s
; RUN: opt -whole-program-assume -passes='require<dtrans-safetyanalyzer>' -debug-only=dtrans-safetyanalyzer-verbose -disable-output %s 2>&1 | FileCheck %s
