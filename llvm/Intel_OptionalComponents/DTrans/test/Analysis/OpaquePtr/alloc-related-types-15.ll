...
; result has non-divide by size use" wasn't set for the call to "new" in @foo,
; nor was the safety data "Bad pointer manipulation" set for any of the
; types. The reason is we can prove that the result from the
; subtraction in %7 will be a multiple of the expected structure's size that
; is being allocated.

; CHECK-NOT: dtrans-safety: Bad pointer manipulation -- Pointer subtract result has non-divide by size use
