...
; %struct.test.a.base are not set to "Bad memfunc manipulation (related types)"
; since the we can't identify the dominant type for the input pointers to
; the memcpy's call, nor can we identify that %class.TestClass.Outer
; encapsulates all the declaration and use aliases.

; CHECK-NOT: dtrans-safety: Bad memfunc manipulation (related types) -- memcpy/memmove - source or destination may contain related types
; CHECK: dtrans-safety: Ambiguous pointer target -- memcpy/memmove - unidentified type for src or dest
; CHECK: [test01]   call void @llvm.memcpy.p0.p0.i64(ptr %ptr, ptr %ptr2, i64 8, i1 false)

; CHECK-LABEL: LLVMType: %class.Simple = type { i32, i32, [4 x i8] }
...
; CHECK: Safety data: Ambiguous GEP | Ambiguous pointer target | Contains nested structure{{ *$}}
...
; CHECK: Safety data: Ambiguous GEP | Ambiguous pointer target | Structure may have ABI padding{{ *$}}

; CHECK-LABEL: LLVMType: %struct.test.a.base = type { i32, i32 }
; CHECK: Related padded structure: struct.test.a
; CHECK: Safety data: Ambiguous GEP | Ambiguous pointer target | Nested structure | Structure could be base for ABI padding{{ *$}}
...
%class.Simple = type { i32, i32, [4 x i8] }
...
entry:
  ...
  %1 = getelementptr inbounds %class.Simple, ptr %ptr2, i64 0, i32 0
  %2 = load i32, ptr %0
  %3 = load i32, ptr %1
...
!18 = !{!"S", %class.Simple zeroinitializer, i32 3, !6, !6, !7}