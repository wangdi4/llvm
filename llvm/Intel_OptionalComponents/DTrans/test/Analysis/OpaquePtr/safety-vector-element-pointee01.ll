...
; pointer type in the vector matches the pointer type it is used as.
...
define float @test1(%struct.test02* "intel_dtrans_func_index"="1" %in) !intel.dtrans.func.type !7 {
  ...
  %field = getelementptr %struct.test01, ptr %test01ptr, i64 0, i32 1
  %ret = load float, ptr %field
  ret float %ret
}

; Same as @test1, but written with an alternative form of the GEP that combines
; all the indices into a single GEP.
define float @test2(%struct.test02* "intel_dtrans_func_index"="1" %in) !intel.dtrans.func.type !7 {
  %b = getelementptr %struct.test02, ptr %in, i64 0, i32 1, i64 1
  %test01ptr = load ptr, ptr %b
  %field = getelementptr %struct.test01, ptr %test01ptr, i64 0, i32 1
  %ret = load float, ptr %field
  ret float %ret
