...
; RUN: opt < %s -whole-program-assume -intel-libirc-allowed -passes='require<dtrans-safetyanalyzer>' -dtrans-print-types -disable-output -debug-only=dtrans-bca 2>&1 | FileCheck %s

; This test case tests the bad cast analyzer on a case where a function
; pointer field is assigned a nullptr. The test case should run without
; core dumping.

; CHECK: dtrans-bca: Begin bad casting analysis
; CHECK: dtrans-bca: Candidate Root Type: struct._ZTS11mynextcoder.mynextcoder
; CHECK: dtrans-bca: Bad casting (pending) -- unknown pointer virtually cast to type of interest:
; CHECK: store ptr %call, ptr %field0, align 8
; CHECK: dtrans-bca: Bad casting (pending) -- unknown pointer virtually cast to type of interest:
; CHECK: store ptr %call, ptr %field0, align 8
; CHECK: dtrans-bca: Unsafe pointer store (pending) -- unmatched store of aliased value:
; CHECK: store ptr %call, ptr %field0, align 8
; CHECK: dtrans-bca: Unsafe pointer store (pending) -- unmatched store of aliased value:
; CHECK: store ptr %call, ptr %field0, align 8
; CHECK: dtrans-bca: Unsafe pointer store (pending) -- unmatched store of aliased value:
; CHECK: store ptr %call, ptr %field0, align 8
; CHECK: dtrans-bca: Mismatched element access (pending) -- incompatible type for field load/store value:
; CHECK: store ptr %call, ptr %field0, align 8
; CHECK: dtrans-bca: Bad casting (pending) -- unknown pointer virtually cast to type of interest:
; CHECK: store ptr %call, ptr %field0, align 8
; CHECK: dtrans-bca: Bad casting (pending) -- unknown pointer virtually cast to type of interest:
...
; CHECK: dtrans-bca: Mismatched element access (pending) -- incompatible type for field load/store value:
; CHECK: store ptr %call, ptr %field0, align 8
; CHECK: dtrans-bca: (CS) init_with_coder1 struct._ZTS8mycoder1.mycoder1
; CHECK: store ptr %call, ptr %field0, align 8
; CHECK: dtrans-bca: (CS) init_with_coder2 struct._ZTS8mycoder2.mycoder2
; CHECK: store ptr %call, ptr %field0, align 8
; CHECK: dtrans-bca: Found conditional bad cast, unsafe pointer store, or mismatched element access
; CHECK: dtrans-bca: End bad casting analysis: (OK)
...
if.end:                                           ; preds = %if.then, %entry
  ...
  store ptr null, ptr %field14, align 8
  ret void
}
