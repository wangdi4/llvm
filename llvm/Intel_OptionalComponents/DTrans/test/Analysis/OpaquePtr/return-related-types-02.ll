...
; RUN: opt  < %s -whole-program-assume -intel-libirc-allowed -passes='require<dtrans-safetyanalyzer>' -disable-output -debug-only=dtrans-safetyanalyzer-verbose 2>&1 | FileCheck %s
...
; Return value type encapsulates related types" was Not printed for the return
; instruction in function @test. The reason is because the pointer to the new
; memory space allocated is used as %class.SimpleClass, and this class is
; not part of the zero element nor related types of %%class.TestClass.Outer.

; CHECK-NOT: dtrans-safety: Bad casting (related types) -- Return type contains related types
; CHECK-NOT: dtrans-safety: Bad casting (related types) -- Return value type encapsulates related types

; CHECK: dtrans-safety: Bad casting -- Return of ambiguous type
; CHECK:   [foo]   ret ptr %0
; CHECK:     LocalPointerInfo: CompletelyAnalyzed
; CHECK:     Declared Types:
; CHECK:       Aliased types:
; CHECK:         %class.SimpleClass*
...
; CHECK:         i8*
; CHECK:       No element pointees.
; CHECK: dtrans-safety: Bad casting -- Return of aggregate type that does not match expected type
; CHECK:   [foo]   ret ptr %0
; CHECK:     LocalPointerInfo: CompletelyAnalyzed
; CHECK:     Declared Types:
; CHECK:       Aliased types:
...
; CHECK:         i8*
; CHECK:       No element pointees.
; CHECK: dtrans-safety: Bad casting -- Return value type did not match this type
; CHECK:   [foo]   ret ptr %0
...
entry:
  ...
  %2 = getelementptr inbounds %class.SimpleClass, ptr %0, i64 0, i32 0
  ret ptr %0
}
...
!intel.dtrans.types = !{!5, !6, !8, !11, !15}
...
!15 = !{!"S", %class.SimpleClass zeroinitializer, i32 3, !3, !0, !4} ; %class.SimpleClass = type { [4 x i32], i32, [4 x i8]}