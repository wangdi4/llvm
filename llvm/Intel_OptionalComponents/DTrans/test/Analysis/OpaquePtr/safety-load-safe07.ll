...
; Verify loading element zero of a structure that is contained within a nested
; array using the address of an array element results in the field being set as
; "Read" and "NonGEPAccess"

%struct.test = type { i64, i64, i64 }
@gStructArray = internal global [10 x [14 x %struct.test]] zeroinitializer
...
define i64 @test(i64 %idx) {
  ...
  %addr0 = getelementptr inbounds [10 x [14 x %struct.test]], ptr @gStructArray, i64 0, i64 0, i64 %idx
  %v0 = load i64, ptr %addr0

  ; Load a field from a structure, where the GEP indexing is the field address.
  %addr1 = getelementptr inbounds [10 x [14 x %struct.test]], ptr @gStructArray, i64 0, i64 0, i64 %idx, i32 1
