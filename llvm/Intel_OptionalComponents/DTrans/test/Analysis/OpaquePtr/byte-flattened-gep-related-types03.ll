...
; This test case checks that the byte flattened GEP %4 is not marked as "Bad
; pointer manipulation (related types) -- Byte flattened GEP used for accessing
; fields in related types" since the offset 34 won't cover the whole field 1
; for %class.TestClass.Inner_vect_imp.

; CHECK-NOT: Bad pointer manipulation (related types) -- Byte flattened GEP used for accessing fields in related types
; CHECK: dtrans-safety: Bad pointer manipulation -- Byte flattened GEP could not be resolved
; CHECK-NEXT:   [foo]   %4 = getelementptr i8, ptr %0, i64 34
...
define dso_local void @bar(ptr noundef "intel_dtrans_func_index"="1" %ptr) !intel.dtrans.func.type !18 {
entry:
