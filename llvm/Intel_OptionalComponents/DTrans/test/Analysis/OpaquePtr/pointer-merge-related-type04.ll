...
; RUN: opt  < %s -whole-program-assume -intel-libirc-allowed -passes='require<dtrans-safetyanalyzer>' -disable-output -debug-only=dtrans-safetyanalyzer-verbose -dtrans-print-types 2>&1 | FileCheck %s
...
; not set for %class.MainClass and %class.MainClass.base due to an access
; to %class.MainClass's padded field in @bar. The main goal of this test
; case is to check that the related types utility reverts "Unsafe pointer
; merge (related types)" into "Unsafe pointer merge" correctly.
...
; CHECK:  Safety data: Ambiguous GEP | Unsafe pointer merge | Nested structure{{ *$}}
...
define dso_local void @bar(ptr "intel_dtrans_func_index"="1" %ptr) !intel.dtrans.func.type !12 {
entry:
  %0 = load ptr, ptr %ptr
  %1 = getelementptr inbounds %class.TestClass.Inner, ptr %0, i64 0, i32 0
  %2 = getelementptr inbounds %class.MainClass, ptr %1, i64 0, i32 2
