...
; expected type size" was set for the call to "new" in @foo, and the
; safety data "Bad alloc size" was set for %class.MainClass.base. The reason is
; we can't prove that the result from the subtraction in %7 won't be a
; multiple of the expected structure's size that is being allocated (division
; in %8 is not exact to the structure's size).

; CHECK: dtrans-safety: Bad alloc size -- Allocation size does not match expected type size
