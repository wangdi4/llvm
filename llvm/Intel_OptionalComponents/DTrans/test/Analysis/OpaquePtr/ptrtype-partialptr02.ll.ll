...
entry:
  ...
  %identify = bitcast ptr %p to ptr
  %tmp = call i1 @test01identify(ptr %identify)
  %other = getelementptr i8, ptr %p, i64 8
  br label %pre_swap

pre_swap:
  %Cast1 = bitcast ptr %p to ptr
  %Cast2 = bitcast ptr %other to ptr
  br label %swap

swap:
  %Count = phi i64 [ 2, %pre_swap ], [ %NextCount, %swap ]
  %HalfPtr1 = phi ptr [ %Cast1, %pre_swap ], [ %NextHalf1, %swap ]
  %HalfPtr2 = phi ptr [ %Cast2, %pre_swap ], [ %NextHalf2, %swap ]
...
; CHECK:  %HalfPtr1 = phi ptr [ %Cast1, %pre_swap ], [ %NextHalf1, %swap ]
...
; CHECK:  %HalfPtr2 = phi ptr [ %Cast2, %pre_swap ], [ %NextHalf2, %swap ]
; CHECK:      LocalPointerInfo:
; CHECK-SAME: PARTIAL PTR
; CHECK:      Aliased types:
; CHECK:        %struct.test01**{{ *}}
...
entry:
  ; This is here to establish what %p actually points to.
  %identify = bitcast ptr %p to ptr
  %tmp = call i1 @test02identify(ptr %identify)
  %other = getelementptr i8, ptr %p, i64 8
  br label %swap
...
entry:
  ; This is here to establish what %p actually points to.
  %identify = bitcast ptr %p to ptr
  %tmp = call i1 @test03identify(ptr %identify)
  %offset = mul i64 %n, 8
...
entry:
  ; This is here to establish what %p actually points to.
  %identify = bitcast ptr %p to ptr
  %tmp = call i1 @test04identify(ptr %identify)
  %other = getelementptr i8, ptr %p, i64 8
  br label %pre_swap
...
swap:
  %p1 = bitcast ptr %cur_p to ptr
  %val1 = load i32, ptr %p1
  %p2 = bitcast ptr %prev_p to ptr
  %val2 = load i32, ptr %p2
  store i32 %val2, ptr %p1
  store i32 %val1, ptr %p2
...
; CHECK:  %p1 = bitcast ptr %cur_p to ptr
; CHECK:      LocalPointerInfo:
; CHECK:      Aliased types:
; CHECK:        %struct.test04**{{ *}}
; CHECK:        i32*{{ *}}
...
; CHECK: %p2 = bitcast ptr %prev_p to ptr
; CHECK:      LocalPointerInfo:
; CHECK:      Aliased types:
; CHECK:        %struct.test04**{{ *}}
; CHECK:        i32*{{ *}}
...
entry:
  ; This is here to establish what %p actually points to.
  %identify = bitcast ptr %p to ptr
  %tmp = call i1 @test05identify(ptr %identify)
  %other = getelementptr i8, ptr %p, i64 8
  br label %pre_swap
...
swap:
  %HalfPtr1 = phi ptr [ %Cast1, %pre_swap ], [ %other_p1, %other_bb ]
  %HalfPtr2 = phi ptr [ %Cast2, %pre_swap ], [ %other_p2, %other_bb ]
  %HalfVal1 = load i32, ptr %HalfPtr1
  %HalfVal2 = load i32, ptr %HalfPtr2
  %NextHalf1 = getelementptr inbounds i32, ptr %HalfPtr1, i64 1
  store i32 %HalfVal2, ptr %HalfPtr1
...
other_bb:
  ...
  %other_p1 = bitcast ptr %tmp1 to ptr
  %other_p2 = bitcast ptr %tmp2 to ptr
  br i1 undef, label %swap, label %exit
...
; CHECK:  %HalfPtr1 = phi ptr [ %Cast1, %pre_swap ], [ %other_p1, %other_bb ]
; CHECK:      Aliased types:
; CHECK:        %struct.test05**{{ *}}
; CHECK:        i32*{{ *}}
; CHECK:        i8*{{ *}}
...
; CHECK:  %HalfPtr2 = phi ptr [ %Cast2, %pre_swap ], [ %other_p2, %other_bb ]
; CHECK:      Aliased types:
; CHECK:        %struct.test05**{{ *}}
; CHECK:        i32*{{ *}}
; CHECK:        i8*{{ *}}
...
entry:
  ; This is here to establish what %p actually points to.
  %identify = bitcast ptr %p to ptr
  %tmp = call i1 @test06identify(ptr %identify)
  %other = getelementptr i8, ptr %p, i64 8
  br label %pre_swap
...
entry:
  ; This is here to establish what %p actually points to.
  %identify = bitcast ptr %p to ptr
  %tmp = call i1 @test07identify(ptr %identify)
  %offset = mul i64 %n, 8
  %init_count = mul i64 %n, 2
  %other = getelementptr i8, ptr %p, i64 %offset
  br label %pre_swap
...
entry:
  ; This is here to establish what %p actually points to.
  %identify = bitcast ptr %p to ptr
  %tmp = call i1 @test08identify(ptr %identify)
  %other = getelementptr i8, ptr %p, i64 8
  br label %pre_swap
...
entry:
  ; This is here to establish what %p actually points to.
  %identify = bitcast ptr %p to ptr
  %tmp = call i1 @test09identify(ptr %identify)
  %other = getelementptr i8, ptr %p, i64 8
  br label %pre_swap
