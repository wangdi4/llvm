; REQUIRES: asserts
; RUN: opt  < %s -opaque-pointers -whole-program-assume -intel-libirc-allowed -dtrans-safetyanalyzer -dtrans-print-callinfo -disable-output 2>&1 | FileCheck %s
; RUN: opt  < %s -opaque-pointers -whole-program-assume -intel-libirc-allowed -passes='require<dtrans-safetyanalyzer>' -dtrans-print-callinfo -disable-output 2>&1 | FileCheck %s
...
; CHECK: Instruction:   tail call void @_ZN10MemManager10deallocateEPv(ptr %ld1, ptr nonnull %gep1)
; CHECK: FreeCallInfo:
; CHECK: Kind: UserFreeThis

; CHECK: Function: _ZN6MemorynwEmPNS_13MemoryManagerE
; CHECK: Instruction:   %alloc1 = tail call ptr @_ZN10MemManager8allocateEl(ptr nonnull %mm, i64 %new.size)
; CHECK: AllocCallInfo:
; CHECK:   Kind: UserMallocThis

; CHECK: Function: test01user
; CHECK: Instruction:   %res = call ptr @_ZN6MemorynwEmPNS_13MemoryManagerE(i64 16, ptr %mm)
; CHECK: AllocCallInfo:
; CHECK:   Kind: UserMalloc0

; CHECK: Function: test01user
; CHECK: Instruction:   call void @_ZN6MemorydlEPv(ptr %res)
; CHECK: FreeCallInfo:
