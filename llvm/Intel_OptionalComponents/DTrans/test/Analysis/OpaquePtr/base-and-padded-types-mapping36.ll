...
; since we can't identify the dominant type for the input pointers
; to the memcpy's call, but we can identify that %class.TestClass.Outer
...
; used in the memcpy is the same as the zero element of
; %struct.test.a.base (4).

; CHECK: dtrans-safety: Bad memfunc manipulation (related types) -- memcpy/memmove - source or destination may contain related types
; CHECK: [test01]   call void @llvm.memcpy.p0.p0.i64(ptr %ptr, ptr %ptr2, i64 4, i1 false)
