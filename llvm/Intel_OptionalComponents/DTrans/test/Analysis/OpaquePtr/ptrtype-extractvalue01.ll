...
; Test pointer type recovery on an extract value instruction that produces a
; pointer type.
...
bad:
  %lp = landingpad { i8*, i32 }
        cleanup
  %extract = extractvalue { i8*, i32 } %lp, 0
  resume { i8*, i32 } %lp
}
...
; CHECK-NONOPAQUE: %extract = extractvalue { i8*, i32 } %lp, 0
; CHECK-OPAQUE: %extract = extractvalue { ptr, i32 } %lp, 0
; CHECK-NEXT: LocalPointerInfo:
; CHECK-NEXT: Aliased types:
; CHECK-NEXT:   i8*
; CHECK-NEXT: No element pointees
