...
define void @test01(ptr "intel_dtrans_func_index"="1" %in) !intel.dtrans.func.type !3 {
  %tmp = getelementptr %struct.test01, ptr %in
  %f0 = getelementptr %struct.test01, ptr %tmp, i64 0, i32 0
  store i32 0, ptr %f0
  ret void
}
; CHECK-LABEL: void @test01
; CHECK: %tmp = getelementptr %struct.test01, ptr %in
; CHECK-NEXT: LocalPointerInfo:
; CHECK-NEXT: Aliased types:
; CHECK-NEXT:   %struct.test01*{{ *$}}
; CHECK-NEXT: No element pointees.

; CHECK: %f0 = getelementptr %struct.test01, ptr %tmp, i64 0, i32 0
; CHECK-NEXT: LocalPointerInfo:
; CHECK-NEXT: Aliased types:
; CHECK-NEXT:   i32*{{ *$}}
...
define internal void @test02(ptr "intel_dtrans_func_index"="1" %in) !intel.dtrans.func.type !6 {
...
; CHECK: %p2p = getelementptr ptr, ptr %in, i64 5
; CHECK-NEXT: LocalPointerInfo:
; CHECK-NEXT: Aliased types:
; CHECK-NEXT:   %struct.test02**{{ *$}}
; CHECK-NEXT: No element pointees.

; CHECK: %ptr = load ptr, ptr %p2p
; CHECK-NEXT: LocalPointerInfo:
; CHECK-NEXT: Aliased types:
; CHECK-NEXT:   %struct.test02*{{ *$}}
; CHECK-NEXT: No element pointees.

; CHECK: %field_addr = getelementptr %struct.test02, ptr %ptr, i64 0, i32 1
; CHECK-NEXT: LocalPointerInfo:
; CHECK-NEXT: Aliased types:
; CHECK-NEXT:   i64*{{ *$}}
; CHECK-NEXT: Element pointees:
