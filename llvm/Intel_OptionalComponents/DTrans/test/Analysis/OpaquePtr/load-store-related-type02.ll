...
; for field load/store contains related types" was not set for the load
; and store instructions in @foo. The reason is because field 0 in
; %class.TestClass.Outer is a pointer to %class.TestClass, but then the
; pointer is used as %class.SimpleClass, which is not a zero element nor a
; related type of %class.TestClass.
