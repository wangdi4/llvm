...
; This test case tests the bad cast analyzer on a case where an inconsistent
; pointer type is passed to a field in the mynextcoder structure. It should
; fail with a safety violation and retain the Bad casting, Unsafe pointer
; store, and Mismatched element access violations.

; CHECK: LLVMType: %struct._ZTS11mynextcoder.mynextcoder
; CHECK: Bad casting | Mismatched element access | Global instance | Has function ptr{{ *$}}
