...
; RUN: opt  < %s -whole-program-assume -intel-libirc-allowed -passes='require<dtrans-safetyanalyzer>' -dtrans-print-types -disable-output -debug-only=dtrans-safetyanalyzer-verbose 2>&1 | FileCheck %s

; This test case checks that the base-padded relationship between
; %struct.test.b.base and %struct.test.b is set, but the relationship between
; %struct.test.a.base and %struct.test.a isn't set because there is an access
...
; innermost nested structure failed the analysis for base-padded structures.
...
; CHECK: Related base structure: struct.test.b.base
...
; CHECK: Related padded structure: struct.test.b
...
define "intel_dtrans_func_index"="1" i32 @foo(ptr "intel_dtrans_func_index"="2" %ptr) local_unnamed_addr #0 !intel.dtrans.func.type !18 {
  %bgep1 = getelementptr %struct.test.a, ptr %ptr, i64 0, i32 2
