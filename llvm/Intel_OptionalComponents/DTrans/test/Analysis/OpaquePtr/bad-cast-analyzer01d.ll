; REQUIRES: asserts
; RUN: opt -opaque-pointers < %s -whole-program-assume -intel-libirc-allowed -dtrans-safetyanalyzer -dtrans-print-types -debug-only=dtrans-bca -disable-output 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers < %s -whole-program-assume -intel-libirc-allowed -passes='require<dtrans-safetyanalyzer>' -dtrans-print-types -debug-only=dtrans-bca -disable-output 2>&1 | FileCheck %s
...
; CHECK: dtrans-bca: Bad casting (pending)
; CHECK: dtrans-bca: Unsafe pointer store (pending)
; CHECK: dtrans-bca: Mismatched element access (pending)
