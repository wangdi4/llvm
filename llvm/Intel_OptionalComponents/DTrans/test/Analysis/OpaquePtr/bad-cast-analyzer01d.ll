; REQUIRES: asserts
; RUN: opt -opaque-pointers < %s -whole-program-assume -dtrans-safetyanalyzer -dtrans-print-types -debug-only=dtrans-bca -disable-output 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers < %s -whole-program-assume -passes='require<dtrans-safetyanalyzer>' -dtrans-print-types -debug-only=dtrans-bca -disable-output 2>&1 | FileCheck %s
