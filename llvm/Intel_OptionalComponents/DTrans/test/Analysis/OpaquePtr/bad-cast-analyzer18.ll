...
; RUN: opt < %s -whole-program-assume -intel-libirc-allowed -passes='require<dtrans-safetyanalyzer>' -debug-only=dtrans-bca -dtrans-print-types -disable-output 2>&1 | FileCheck %s
...
; Also check that there are no pending stores at the end of the analysis.

; CHECK: dtrans-bca: Begin bad casting analysis
; CHECK-NOT: dtrans-bca: Pending Store
; CHECK-NOT: dtrans-bca: End bad casting analysis: (NOT OK)
