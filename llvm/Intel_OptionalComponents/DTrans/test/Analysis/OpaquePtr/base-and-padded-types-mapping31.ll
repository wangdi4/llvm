...
; RUN: opt  < %s -whole-program-assume -intel-libirc-allowed -dtrans-outofboundsok=false -passes='require<dtrans-safetyanalyzer>' -disable-output -debug-only=dtrans-safetyanalyzer-verbose -dtrans-print-types 2>&1 | FileCheck %s

; This test case checks that the structure %class.TestClass.Inner was set as
; "Bad pointer manipulation" and not "Bad pointer manipulation (related types)".
; The reason is that there is read in the padded field of %class.MainClass,
; therefore the relationship with %class.MainClass.base must be broken and the
; safety violation needs to be reverted.
...
; CHECK:  Safety data: Bad casting | Bad pointer manipulation | Mismatched element access | Unsafe pointer store | Nested structure{{.*}}

; CHECK:  LLVMType: %class.TestClass.Inner = type { %class.MainClass.base, [4 x i8] }
; CHECK:  Safety data: Bad casting | Bad pointer manipulation | Mismatched element access | Unsafe pointer store | Nested structure | Contains nested structure{{.*}}
...
define dso_local i8 @bar(ptr noundef "intel_dtrans_func_index"="1" %ptr) !intel.dtrans.func.type !17 {
entry:
  %0 = getelementptr inbounds %class.TestClass.Outer, ptr %ptr, i64 0, i32 1
  %1 = getelementptr inbounds %class.TestClass.Inner_vect, ptr %0, i64 0, i32 0
