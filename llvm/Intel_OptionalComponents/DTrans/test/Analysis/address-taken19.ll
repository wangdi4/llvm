...
; Check that %struct.s2 has <complete> results in fields 0 and 2,
; even though @getenv has the potential to create a false alias.
; Also, check that neither %struct.s1 nor %struct.s2 are Address taken.
...
; CHECK: Multiple Value: [ @f1, null ] <complete>
; CHECK: 2)Field LLVM Type: i32 (%struct.s2*)*
; CHECK: Multiple Value: [ @f2, null ] <complete>
; CHECK-NOT: Safety data:{{.*}}Address taken
