...
; to the candidate field is bitcast and then passed to a function argument
; which can be bitcast to more than one type.  It should fail with a safety
; violation and retain the Bad casting and Unsafe pointer store violations.
...
t5:
  %t2.1 = bitcast i8* %0 to %struct.mycoder1*
  %t33.1 = getelementptr inbounds %struct.mycoder1, %struct.mycoder1* %t2.1, i64 0, i32 0
  store i32 150, i32* %t33.1, align 8
  ret void
t11:
  %t2.2 = bitcast i8* %0 to %struct.mycoder2*
  %t33.2 = getelementptr inbounds %struct.mycoder2, %struct.mycoder2* %t2.2, i64 0, i32 1
  store i32 122, i32* %t33.2, align 8
  ret void
}
...
; CHECK: %t13.0 = bitcast i8* %t3 to %struct.mycoder1*
; CHECK: dtrans-bca: Unsafe pointer store --  (pending)   Unmatch store of aliased value:
; CHECK: dtrans-bca: (SF) [1] Fxn ptr arg not a single type: void (i8*)* @coder1_startup
; CHECK: dtrans-bca: Bad casting (pending) -- unknown pointer cast to type of interest:
...
; CHECK: Safety data: Bad casting | Ambiguous GEP | Unsafe pointer store
; CHECK: LLVMType: %struct.mycoder2
; CHECK: Bad casting | Ambiguous GEP | Unsafe pointer store
