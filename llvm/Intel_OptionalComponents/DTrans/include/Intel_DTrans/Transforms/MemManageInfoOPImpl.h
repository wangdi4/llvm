...
getClassTypeOrUnknown(const Function *F, FunctionTypeResolver &TypeResolver) {
  ...
  if (!DFnTy) {
    // We can ignore functions that have no uses, even if we cannot determine
    // the parameter type due to the lack of DTrans metadata. This happens with
    // the "branch_funnel" functions generated by the WholeProgramDevirtPass.
    if (F->use_empty())
      return {nullptr, false};
    return {nullptr, true};
  }
