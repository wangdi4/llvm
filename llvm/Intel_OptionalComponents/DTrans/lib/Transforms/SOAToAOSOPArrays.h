...
// for SOA-to-AOS-OP: method analysis and transformations.
//
//===----------------------------------------------------------------------===//

#ifndef INTEL_DTRANS_TRANSFORMS_SOATOAOSOPARRAYS_H
#define INTEL_DTRANS_TRANSFORMS_SOATOAOSOPARRAYS_H

#if !INTEL_FEATURE_SW_DTRANS
#error SOAToAOSOPArrays.h include in an non-INTEL_FEATURE_SW_DTRANS build.
#endif
...
#define DTRANS_SOAARR "dtrans-soatoaosop-arrays"
...
struct ArraySummaryForIdiom : public SummaryForIdiom {
  ...
  DTransPointerType *ElementType;

  ArraySummaryForIdiom(DTransStructType *A, DTransPointerType *E,
                       DTransStructType *MI, const Function *F,
                       DTransSafetyInfo *DTInfo)
      : SummaryForIdiom(A, MI, F, DTInfo), ElementType(E) {}

  ArraySummaryForIdiom(const SummaryForIdiom &S, DTransPointerType *E)
...
protected:
  ...
  static bool isElementValueFromArg(const Dep *D,
    ...
    auto *ATy = DFnTy->getArgType(A->Const);

    if (D->Kind == Dep::DK_Load)
      return isa<DTransPointerType>(ATy) &&
...
private:
  ...
  private:
    static bool checkTransformRestriction(const ArraySummaryForIdiom &S,
      ...
      for (unsigned I = 0, E = DFnTy->getNumArgs(); I != E; ++I) {
        auto *P = DFnTy->getArgType(I);
        if (P->isIntegerTy())
          continue;
        if (P == S.ElementType) {
          ElemArgOff = I;
          break;
        } else if (auto *Ptr = dyn_cast<DTransPointerType>(P)) {
          auto *Pointee = Ptr->getPointerElementType();
      ...
      return !(UnknownArg || ElemArgOff != DFnTy->getNumArgs() - 1);
    }
  ...
  bool checkElementAccess(const Value *Address, const char *Desc) const {
    if (isSafeBitCast(DL, Address, S.DTInfo->getPtrTypeAnalyzer()))
    ...
    if (!PTy || PTy->getPointerElementType() != S.ElementType)
      return false;
  ...
  bool checkMemset(const MemSetInst *MS) const {
    ...
    return dtrans::isValueMultipleOfSize(
        MS->getLength(), DL.getTypeStoreSize(S.ElementType->getLLVMType()));
  }
  ...
  bool checkLoadStoreAddress(const Instruction *I) const {
    ...
    if (auto *BC = dyn_cast<BitCastInst>(Address)) {
      if (!isSafeBitCast(DL, BC, S.DTInfo->getPtrTypeAnalyzer()))
        return false;
...
public:
  ArrayMethodTransformation(
      const DataLayout &DL, DTransSafetyInfo &DTInfo,
  ...
  using ClonedElemLoadStoreMapTy =
      SmallDenseMap<Instruction *, SmallDenseMap<unsigned, Instruction *>, 4>;
  ...
  void updateBasePointerInsts(bool IsCombined, int NumArrays,
                              DTransPointerType *NewDTBaseType,
                              DTransPointerType *PDTNewBaseType) {
    IRBuilder<> Builder(Context);
    // TODO: For opaque pointer, NewBaseType and PNewBaseType are just
    // pointers and exactly same. Also. isSafeBitCast checks can be deleted
    // once we move to opaque pointers completely. This code can be
    // simplified once we move to opaque pointers.
    llvm::PointerType *NewBaseType =
        cast<PointerType>(NewDTBaseType->getLLVMType());
    llvm::PointerType *PNewBaseType =
  ...
  // LoadInst/StoreInst instructions for remaining elements since type of
  // ValueOperand/PointerOperand may not match with mutated GEPs. So, new
  // LoadInst/StoreInst are created for all elements before processing any
  // element and saved in ClonedElemLoadStoreMap, which will be used later in
  // rawCopyAndRelink instead of creating new LoadInst/StoreInst on the fly.
  void earlyCloneElemLoadStoreInst(
      unsigned Off, ClonedElemLoadStoreMapTy &ClonedElemLoadStoreMap) {
    IRBuilder<> Builder(Context);
    for (auto *I : InstsToTransform.ElementInstToTransform) {
      ...
      if (auto *NewStore = dyn_cast<StoreInst>(NewI)) {
        ...
        ClonedElemLoadStoreMap[NewStore][Off] = CopyStore;
      } else if (auto *NewLoad = dyn_cast<LoadInst>(NewI)) {
        Builder.SetInsertPoint(NewLoad);
        auto *CopyLoad = Builder.CreateAlignedLoad(
            NewLoad->getType(), NewLoad->getPointerOperand(),
            NewLoad->getAlign(), false, "copy");
        ClonedElemLoadStoreMap[NewLoad][Off] = CopyLoad;
      }
    }
  ...
  void rawCopyAndRelink(OrigToCopyTy &OrigToCopy, bool UniqueInsts,
                        unsigned NumArrays, DTransType *OtherDTElemType,
                        unsigned NewParamOffset,
                        ClonedElemLoadStoreMapTy &ClonedElemLoadStoreMap,
                        unsigned Off) {
    IRBuilder<> Builder(Context);

    // TODO: This code can be removed once we have moved to opaque pointers
    // completely.
    auto ProcessSafeBitCast = [&](const Value *Old,
                                  BitCastInst *NewBC) -> void {
      assert(isSafeBitCast(DL, Old, DTInfo.getPtrTypeAnalyzer()) &&
    ...
    Type *OtherElemType = OtherDTElemType->getLLVMType();
    for (auto *I : InstsToTransform.ElementFromArg) {
      auto *NewI = cast<Instruction>((Value *)VMap[I]);
      ...
      Builder.SetInsertPoint(NewLoad);
      LoadInst *CopyLoad = cast<LoadInst>(ClonedElemLoadStoreMap[NewLoad][Off]);
    ...
    for (auto *I : InstsToTransform.ElementInstToTransform) {
      ...
      if (auto *NewLoad = dyn_cast<LoadInst>(NewI)) {
        ...
        Builder.SetInsertPoint(NewLoad);
        LoadInst *CopyLoad =
            cast<LoadInst>(ClonedElemLoadStoreMap[NewLoad][Off]);
        OrigToCopy[NewLoad] = CopyLoad;
        if (auto *NewBC = dyn_cast<BitCastInst>(NewPtr))
          ProcessSafeBitCast(cast<LoadInst>(I)->getPointerOperand(), NewBC);
        else {
        ...
        StoreInst *CopyStore =
            cast<StoreInst>(ClonedElemLoadStoreMap[NewStore][Off]);
  ...
  void gepRAUW(bool Copy, const OrigToCopyTy &OrigToCopy, unsigned Offset,
    ...
    auto *NewBaseType = cast<PointerType>(NewDTBaseType->getLLVMType());
    ...
    for (auto *I : ElementPtrGEP) {
      ...
      if (auto *NewGEP = dyn_cast<GetElementPtrInst>(NewI)) {
        ...
        NewGEP->setResultElementType(
            NewDTBaseType->getPointerElementType()->getLLVMType());
	...
	if (InstsToTransform.BasePtrInst.count(cast<Instruction>(U.getUser())))
        ...
        if (!ReplGEP)
          ReplGEP = insertElemGEP(NewI, Offset, NewDTBaseType);
    ...
    for (auto *I : InstsToTransform.ElementInstToTransform) {
      ...
      if (auto *L = dyn_cast<LoadInst>(I))
        Address = const_cast<Value *>(L->getPointerOperand());
      else if (auto *SI = dyn_cast<StoreInst>(I))
        Address = const_cast<Value *>(SI->getPointerOperand());
      else if (isa<MemSetInst>(I))
        continue;
      else if (auto *R = dyn_cast<ReturnInst>(I))
        Address = const_cast<Value *>(R->getOperand(0));
      else
        llvm_unreachable("Unexpected instruction encountered.");

      if (isSafeBitCast(DL, Address, DTInfo.getPtrTypeAnalyzer()))
        Address =
            const_cast<Value *>(cast<BitCastInst>(Address)->getOperand(0));
      ...
      NewI->replaceUsesOfWith(NewAddress,
                              insertElemGEP(NewAddress, Offset, NewDTBaseType));
    }
  }
  ...
  mapNewAppendType(const Function &Method,
    ...
    for (unsigned I = 0, E = DTFuncTy->getNumArgs(); I != E; ++I) {
      ...
      if (auto *Ptr = dyn_cast<DTransPointerType>(Param)) {
        if (Ptr->getPointerElementType() == ArrType) {
          Params.push_back(TM.getOrCreatePointerType(NewArray));
          continue;
        }
        ...
        if (Ptr->getPointerElementType() == ElementType) {
          ...
          ElemOffset = Params.size();
          for (auto *Fld : Elements)
            Params.push_back(TM.getOrCreatePointerType(Fld));
          continue;
        }
      }
...
private:
  Value *insertElemGEP(Instruction *Base, unsigned Offset,
    ...
    return Builder.CreateInBoundsGEP(
        NewDTBaseType->getPointerElementType()->getLLVMType(), Base,
...
extern cl::opt<unsigned> DTransSOAToAOSOPBasePtrOff;
ArraySummaryForIdiom
getParametersForSOAToAOSArrayMethodsCheckDebug(Function &F, DTransSafetyInfo *);
#endif // !defined(NDEBUG) || defined(LLVM_ENABLE_DUMP)
} // namespace soatoaosOP
...
#endif // INTEL_DTRANS_TRANSFORMS_SOATOAOSOPARRAYS_H
