...
private:
  ...
  // This set will store the new types generated when applying the
  // transformation. The post-processing function will use this information to
  // clean the redundant instructions.
  SetVector<StructType *> NewTypesGenerated;
...
// This function cleans up the IR after applying the transformation. It removes
// redundant instructions that were generated due to merging the types.
void ResolveTypesImpl::postprocessFunction(Function &OrigFunc, bool isCloned) {

  // Clean up any redundant BitCast instruction that was generated by merging
  // the types. For example:
  //
  //   Before resolve types:
  //     %temp4 = bitcast %"QNCA_a0$double*$rank3$"* %temp2 to
  //                      %"QNCA_a0$double*$rank3$.7.39"*
  //
  //   After resolve types:
  //     %temp4 = bitcast %"__DTRT_QNCA_a0$double*$rank3$"* %temp2 to
  //                      %"__DTRT_QNCA_a0$double*$rank3$"*
  //
  // The example above shows that after applying resolve types the BitCast
  // %temp4 becomes redundant. This means that the source type will be the same
  // as the destination type. We can get rid of these BitCast instructions and
  // replace all the uses with the source pointer (%temp2).
  //
  // The input BitCast instruction will be stored in the vector InstsToDelete.
  // This vector will be iterated over after the function is analyzed to delete
  // the instructions since there is no use for them.
  auto postprocessBitCast = [&, this](BitCastInst *BC,
                                std::vector<Instruction *> &InstsToDelete) {
    assert(BC && "Null reference while post-processing a BitCast instruction");
    ...
    // We are going to process only those BitCast instructions generated by
    // the transformation. This means that the source and destination types
    // should be in the NewTypesGenerated set.
    if (!NewTypesGenerated.contains(cast<StructType>(SrcTy)))
      return;

    Value *Op = BC->getOperand(0);
    BC->replaceAllUsesWith(Op);

    InstsToDelete.push_back(BC);
  };
  ...
  for (auto &I : instructions(F))
    if (auto *BC = dyn_cast<BitCastInst>(&I))
      postprocessBitCast(BC, InstsToDelete);
...
void ResolveTypesImpl::addTypeMapping(StructType *SrcTy, StructType *DestTy) {
  ...
  // Store the new types generated, this will be used later during the
  // post-processing to clean up redundant instructions.
  NewTypesGenerated.insert(ActualDestTy);
}
