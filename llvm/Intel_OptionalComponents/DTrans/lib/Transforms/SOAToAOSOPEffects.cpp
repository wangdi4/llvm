...
cl::opt<bool>
    DTransSOAToAOSOPComputeAllDep("enable-dtrans-soatoaosop-alldeps",
                                  ...
                                  cl::desc("Enable DTrans SOAToAOSOP"));
  ...
  auto IsFieldAccessGEP = [ClassTy](const Value *V) -> unsigned {
    if (auto *GEP = dyn_cast<GetElementPtrInst>(V))
      if (GEP->getSourceElementType() ==
          cast<llvm::StructType>(ClassTy->getLLVMType()))
  ...
  case Instruction::Call: {
    ...
    if (Info) {
      if (Info->getCallInfoKind() == dtrans::CallInfo::CIK_Alloc) {
        ...
        auto FK = cast<dtrans::FreeCallInfo>(Info)->getFreeKind();
    ...
    else if (dtrans::isTypeTestRelatedIntrinsic(I))
      Rep = Dep::mkCall(DM, Dep::mkArgList(DM, Remaining), true);
    else {
      Rep = Dep::mkCall(DM, Dep::mkArgList(DM, Remaining),
                        F && getOPStructTypeOfMethod(F, &DTInfo) == ClassType);
    }
    break;
  }
...
bool DepCompute::computeDepApproximation() const {
  ...
  assert(getOPStructTypeOfMethod(const_cast<Function *>(Method), &DTInfo) ==
             ClassType &&
  ...
  for (auto *S : Sinks)
    ...
    for (auto SCCIt = scc_begin(AllDepGraph<const Value *>(S));
      ...
      for (auto *PtrI : *SCCIt) {
        ...
        if (Rep->isBottom() && !DTransSOAToAOSOPComputeAllDep)
          return false;
...
char SOAToAOSOPApproximationDebug::PassID;

// Provide a definition for the static class member used to identify passes.
AnalysisKey SOAToAOSOPApproximationDebug::Key;

SOAToAOSOPApproximationDebug::Ignore::Ignore(
    SOAToAOSOPApproximationDebugResult *Ptr)
    : Ptr(Ptr) {}
SOAToAOSOPApproximationDebug::Ignore::Ignore(Ignore &&Other)
    : Ptr(std::move(Other.Ptr)) {}
const SOAToAOSOPApproximationDebugResult *
SOAToAOSOPApproximationDebug::Ignore::get() const {
  return Ptr.get();
}
SOAToAOSOPApproximationDebug::Ignore::~Ignore() {}
