...
private:
  ...
  // Map of FunctionKind to Function. Only one function is allowed
  // for each FunctionKind.
...
bool MemManageTransImpl::analyzeCandidates(void) {
  ...
  auto IsLibFunction = [this](const CallBase *Call,
...
bool MemManageTransImpl::categorizeFunctions() {
  ...
  auto CategorizeFunctionUsingSignature =
      [&, IsCalledFromInterfaceFunction](Function *F) -> FunctionKind {
    bool NoReturn = false;
  ...
  // Detect FunctionKind for each interface function.
  unsigned NumInterfacefunctions = 0;
  for (auto F : Cand->interface_functions()) {
    NumInterfacefunctions++;
    FunctionKind FKind = CategorizeFunctionUsingSignature(F);
    DEBUG_WITH_TYPE(DTRANS_MEMMANAGETRANS, {
