...
// Returns false if \p I is not supported instruction.
bool ReuseFieldImpl::IsInstructionSupported(Instruction *I) {
...
bool ReuseFieldImpl::collectCandidateStructFields(
    StructField2FieldsMapTy &StructField2FieldsMap,
    CandidateStructFieldsTy &CandidateStructFields) {
  ...
  for (dtrans::TypeInfo *TI : DTInfo->type_info_entries()) {
    ...
    if (CandidateFieldsList.size() >= 2) {
      ...
      for (auto Field : CandidateFieldsList)
        StructField2FieldsMap[std::make_pair(StInfo->getLLVMType(), (size_t)Field)] =
        CandidateFieldsList;
...
bool ReuseFieldImpl::classifyCandStoreInst(
  ...
  auto isCandidateStructField = [&](StructFieldTy StructField) {
    ...
    auto* StInfo = dyn_cast<dtrans::StructInfo>(DTInfo->getTypeInfo(Ty));
  ...
  auto getAccessStructField = [&](GEPOperator *GEP) {
    ...
    if (NumIndices > 2)
      return std::make_pair((Type *)nullptr, (size_t)0);
    if (NumIndices == 1)
  ...
  auto AnalyzeMultiElemLdSt = [&](Instruction *I) {
    ...
    if (isCandidateStructField(TElem) || isCandidateStructField(FElem))
      return false;
  ...
  auto isValidPointerOperand = [&](Value* PointerOperand) {
    ...
    // Only support 2 indices.
    int32_t NumIndices = GEP->getNumIndices();
    if (NumIndices != 2) {
      LLVM_DEBUG(dbgs() << "Only support 2 indices.\n");
      return false;
    }
...
// value.
// %37 = getelementptr %struct.arc, %struct.arc* %25, i64 0, i32 1 <- LdGEP
// %38 = load i64, i64* %37, align 8                               <- Load1
// %39 = getelementptr %struct.arc, %struct.arc* %32, i64 0, i32 1 <- StGEP1
// store i64 %38, i64* %39, align 8                                <- SI1
// %40 = load i64, i64* %37, align 8                               <- Load2
// %41 = getelementptr %struct.arc, %struct.arc* %32, i64 0, i32 8 <- StGEP2
// store i64 %40, i64* %41, align 8                                <- SI2
//
...
// equal to %38.
void ReuseFieldImpl::foldToSameValue(SF2BP2VIMapTy &SF2BP2VIMap) {
...
// reached legally.
bool ReuseFieldImpl::storesReachLegallyWithSameValue(
    ...
    while (Count++ < Limit && II != nullptr) {
      ...
      if (II->mayWriteToMemory()) {
        if (auto SI = dyn_cast<StoreInst>(II)) {
          ...
          if (IIElem.first == ToElem.first &&
              IIElem.second == ToElem.second)
            return false;
        } else
          return false;
  ...
  if (!(ReachLegally(SI0, SI1) || ReachLegally(SI1, SI0))) {
    LLVM_DEBUG(dbgs() << "Bail Out: SI0 can't reach to SI2 legally.\n");
    return false;
  }
...
void ReuseFieldImpl::classifyFieldsByValue(
  ...
  for (auto SF2BP2VIIt = SF2BP2VIMap.begin(), SF2BP2VIE = SF2BP2VIMap.end();
    ...
    for (auto BP2VIIt0 = BP2VI0.begin(), BP2VIE0 = BP2VI0.end();
...

      {
        ...
        // the same value with the miss field.
...
bool ReuseFieldImpl::isValidPtrOfPtr(Module &M, Value *V,
  ...
  auto StructIndex = dyn_cast<GetElementPtrInst>(StructField->getPointerOperand());
  ...
  if (DTInfo->testSafetyData(DTInfo->getTypeInfo(GVTy),
                             dtrans::DT_ReuseFieldPtrOfPtr))
    return false;

  auto GVField =
    dyn_cast<GetElementPtrInst>(LoadAddress->getPointerOperand());
  ...
  for (auto GVUser : GV->users()) {
    ...
    auto IndexOfPtrOfPtr = dyn_cast<GetElementPtrInst>(*LoadAddress->user_begin());
...
bool ReuseFieldImpl::isValidPtr(Module &M, Value *V, Value *CorePtr) {
        ...
        if (auto LI = dyn_cast<LoadInst>(GVFieldUser)) {
          ...
          for (auto LoadUser : LI->users()) {
            if (auto GEP = dyn_cast<GetElementPtrInst>(LoadUser)) {
              ...
              auto CI = dyn_cast<CallInst>(*BitCast->user_begin());
              if (!CI)
                return false;
...
// We know when the key field of StructField2FieldsMap is set, which fields will be set
// together, but we don't know when the mapped fields are set, if the key field
// is also set. In this function, we calculate the set whose elements are always
// set together each other.
void ReuseFieldImpl::calcFieldsIntersect(
  ...
  LLVM_DEBUG(
    ...
    for (auto Field2FeildsA : StructField2FieldsMap) {
      ...
      dbgs() << "LLVM Type: "; StructTy->print(dbgs(), true);
...
bool ReuseFieldImpl::doCollection(Module &M) {
  ...
  for (Function &F : M) {
    ...
    for (auto BI = F.begin(), BE = F.end(); BI != BE; ++BI) {
      ...
      classifyFieldsByValue(SF2BP2VIMap, StructField2FieldsMap, MissingFieldsList);
    }
  }
...
// mapped field.
bool ReuseFieldImpl::doTransformation(Module& M) {
  ...
  for (auto StructField2LoadsPair : StructField2Loads) {
    ...
    if (ToFieldIt != ReuseFieldMap.end() &&
      ...
      for (auto LI : Loads) {
        ...
        // GEP only has 1 user, fix the field directly.
        if (GEP->hasOneUser()) {
          GEP->setOperand(2, ConstantInt::get(Idx->getType(), ToFieldIt->second));
          LLVM_DEBUG(dbgs() << "GEP (after): \n"; GEP->dump(););
          // Fix the alignment of load.
          dtrans::resetLoadStoreAlignment(AffectedGEP, DL, IsPacked);
        } else {
          // Fix the field with the new cloned GEP when the original GEP has
          // multiple users.
          auto NewGEP = GetElementPtrInst::Create(
              GEP->getSourceElementType(), GEP->getPointerOperand(),
              {Constant::getNullValue(Type::getInt64Ty(M.getContext())),
               ConstantInt::get(Idx->getType(), ToFieldIt->second)});
...
bool ReuseFieldImpl::run(Module& M) {
  if (!doCollection(M))
    return false;
...
bool dtrans::ReuseFieldPass::runImpl(
  ...
  ReuseFieldImpl Transformer(DTInfo, GetTLI);
