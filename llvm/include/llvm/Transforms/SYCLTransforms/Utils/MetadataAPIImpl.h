...
#ifndef LLVM_TRANSFORMS_SYCLTRANSFORMS_UTILS_METADATA_API_IMPL_H
#define LLVM_TRANSFORMS_SYCLTRANSFORMS_UTILS_METADATA_API_IMPL_H
...
template <class T, typename C = void> struct MDValueTraits {
...
template <> struct MDValueTraits<std::string, void> {
...
template <> struct MDValueTraits<bool, void> {
...
template <> struct MDValueTraits<int32_t, void> {
...
template <> struct MDValueTraits<llvm::Type, void> {
...
template <> struct MDValueTraits<llvm::Function, void> {
...
template <class T, class Traits = MDValueTraits<T>> class MDValue {
public:
  typedef typename Traits::value_type value_type;
...
template <typename T> class NamedMDValueAccessor {
...
template <typename T> class VecTypeHintTupleMDListAccessor {
public:
...
template <typename T> class WorkgroupSizeMDAccessor {
public:
