...
public:
  ...
  /// which is an optionally AddrSpaceCast'ed AllocaInst.
  static bool isOMPItemLocalVAR(const Value *V);

  /// Returns the pointer type of a local variable referenced
  /// in an OpenMP clause as defined by the corresponding AllocaInst.
  /// isOMPItemLocalVAR() is asserted for \p V.
  static PointerType *getOMPItemLocalVARPointerType(const Value *V);
};
