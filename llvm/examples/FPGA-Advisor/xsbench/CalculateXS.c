                         ...
                         double * RESTRICT( macro_xs_vector) )
{
  ...
  for( int j = 0; j < num_nucs[mat]; j++ )
	...
	if( energy_grid[idx].xs_ptrs[p_nuc] == n_gridpoints - 1 )
	  low = &nuclide_grids[p_nuc][energy_grid[idx].xs_ptrs[p_nuc] - 1];
	else
	  low = &nuclide_grids[p_nuc][energy_grid[idx].xs_ptrs[p_nuc]];
