; RUN: opt < %s -function-splitting -function-splitting-min-size=20 -function-splitting-only-hot=false -S | FileCheck %s
...
if.then:                                          ; preds = %entry
  %puts35 = call i32 @puts(i8* getelementptr inbounds ([9 x i8], [9 x i8]* @str.2, i64 0, i64 0))
