; RUN: opt -opaque-pointers=0 < %s -passes=function-splitting -function-splitting-min-size=25 -function-splitting-only-hot=false -S | FileCheck %s

; This is a basic test for the Intel Function Splitting pass.
;
; In this test, the code under the 'if.else' tag is never executed, however
; the control flow under this case has part of the code resuming, and part
; part of the code returning from the function. In the current implementation
; of the function splitting, this should be detected as not being a single-entry/
; single-exit region, and should not be split.
;
; This test verifies function splitting does NOT occur on this case.

; CHECK-LABEL: define void @test(i32 %x, i32* %y)
; CHECK-NOT: codeRepl:
; CHECK-NOT: define internal void @test_if.else
...
cleanup:                                          ; preds = %if.then, %if.else16
  ret void
}
