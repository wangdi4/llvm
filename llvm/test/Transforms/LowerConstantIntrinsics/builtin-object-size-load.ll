; NOTE: Assertions have been autogenerated by utils/update_test_checks.py
<<<<<<< HEAD
; INTEL_CUSTOMIZATION
; Added -opaque-pointers -xmain-enable-gep0-removal
; RUN: opt -opaque-pointers -xmain-enable-gep0-removal --instcombine -lower-constant-intrinsics -S < %s | FileCheck %s
; end INTEL_CUSTOMIZATION
=======
; RUN: opt -passes=instcombine,lower-constant-intrinsics -S < %s | FileCheck %s
>>>>>>> 136872a93a5b5639c971824973e423ac2493a615

target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

declare i64 @llvm.objectsize.i64.p0(ptr, i1 immarg, i1 immarg, i1 immarg)


define dso_local i64 @check_store_load(i1 %cond) local_unnamed_addr {
; CHECK-LABEL: @check_store_load(
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[PTR01:%.*]] = alloca [10 x i8], align 1
; CHECK-NEXT:    br i1 [[COND:%.*]], label [[IF_THEN:%.*]], label [[IF_END:%.*]]
; CHECK:       if.then:
; CHECK-NEXT:    br label [[RETURN:%.*]]
; CHECK:       if.end:
; CHECK-NEXT:    [[PTR12:%.*]] = alloca [12 x i8], align 1
; CHECK-NEXT:    br label [[RETURN]]
; CHECK:       return:
; CHECK-NEXT:    [[STOREMERGE:%.*]] = phi ptr [ [[PTR12]], [[IF_END]] ], [ [[PTR01]], [[IF_THEN]] ]
; CHECK-NEXT:    ret i64 12
;
entry:
  %holder = alloca ptr
  %ptr0 = alloca i8, i64 10
  br i1 %cond, label %if.then, label %if.end

if.then:
  store ptr %ptr0, ptr %holder
  br label %return

if.end:
  %ptr1 = alloca i8, i64 12
  store ptr %ptr1, ptr %holder
  br label %return

return:
  %held = load ptr, ptr %holder
  %objsize = call i64 @llvm.objectsize.i64.p0(ptr %held, i1 false, i1 true, i1 false)
  ret i64 %objsize

}
