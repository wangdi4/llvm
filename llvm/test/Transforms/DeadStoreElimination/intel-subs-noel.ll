...
; CHECK: store double %"fill2_$Q[][][][]_fetch", double* %"fill2_$Q[]31[][][]", align 1
; CHECK: store double %"fill2_$Q[][][][]_fetch", double* %"fill2_$Q[]66[][][]", align 1
; CHECK: store double %"fill2_$Q[]112[][][]_fetch", double* %"fill2_$Q[]131[][][]", align 1
; CHECK: store double %"fill2_$Q[]112[][][]_fetch", double* %"fill2_$Q[]163[][][]", align 1
...
define void @fill2_(double* noalias nocapture dereferenceable(8) %"fill2_$Q", i32* noalias nocapture readonly dereferenceable(4) %"fill2_$NX", i32* noalias nocapture readonly dereferenceable(4) %"fill2_$NY", i32* noalias nocapture readonly dereferenceable(4) %"fill2_$NZL") local_unnamed_addr #0 {
alloca_0:
  %"fill2_$NX_fetch" = load i32, i32* %"fill2_$NX", align 1
  %"fill2_$NY_fetch" = load i32, i32* %"fill2_$NY", align 1
...
attributes #0 = { nofree nounwind uwtable "intel-lang"="fortran" "min-legal-vector-width"="0" "pre_loopopt" "target-cpu"="core-avx2" "target-features"="+avx,+avx2,+bmi,+bmi2,+cx16,+cx8,+f16c,+fma,+fsgsbase,+fxsr,+invpcid,+lzcnt,+mmx,+movbe,+pclmul,+popcnt,+rdrnd,+sahf,+sse,+sse2,+sse3,+sse4.1,+sse4.2,+ssse3,+x87,+xsave,+xsaveopt" }
