...
; Source code compiled with: -O3 -ffast-math -fimf-precision=high
; #include <math.h>
...
; CHECK-NEXT:    [[TMP0:%.*]] = tail call fast float @llvm.sin.f32(float [[CONV1]]) [[ATTR2:#.*]]
; CHECK-NEXT:    [[TMP1:%.*]] = tail call fast float @llvm.cos.f32(float [[CONV1]]) [[ATTR2]]
; CHECK-NEXT:    [[CONV7:%.*]] = fmul fast float [[FARG]], 1.000000e+01
; CHECK-NEXT:    [[TMP2:%.*]] = tail call fast float @llvm.sin.f32(float [[CONV7]]) [[ATTR2]]
; CHECK-NEXT:    [[TMP3:%.*]] = tail call fast float @llvm.cos.f32(float [[CONV7]]) [[ATTR2]]
...
; CHECK-NEXT:    [[TMP6:%.*]] = tail call fast float @llvm.sin.f32(float [[MUL14]]) [[ATTR2]]
; CHECK-NEXT:    [[TMP7:%.*]] = tail call fast float @llvm.cos.f32(float [[MUL14]]) [[ATTR2]]
...
attributes #2 = { "imf-precision"="high" }
