...
; . precision: low
; . fast-math: yes
; . disable transformation of sinf/cosf to sinpif/cospif 
;
; Compilation options:
; icpx -O2 -ffast-math -fimf-precision=low -mllvm -enable transform-sin-cos=0
;
; Compilation options:
;
...
; }
;
; RUN: opt < %s -transform-sin-cos-calls -enable-transform-sin-cos=0 -S 2>&1 | FileCheck %s
; RUN: opt -passes="transform-sin-cos-calls" -enable-transform-sin-cos=0 -S 2>&1 < %s | FileCheck %s
;
