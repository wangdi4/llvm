; REQUIRES: asserts
; RUN: opt -whole-program-assume -passes=iparraytranspose -debug-only=iparraytranspose -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -S < %s 2>&1 | FileCheck %s

; Check the debug trace for the transformation of "new" unoptimized SCEVs 

; CHECK: TransposedNumRows: 33816576  TransposedNumCols: 20
