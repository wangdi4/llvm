; This test verifies that array transpose is not enabled because %gep1,
; which doesn't have constant index, is escaped to "bar".
...
b1:                                                ; preds = %b2, %b0
  ...
  tail call void @bar(double* %gep1)
...
define void @bar(double* %in2) {
b0:
  %bc3 = bitcast double* %in2 to i8*
  br label %b1
