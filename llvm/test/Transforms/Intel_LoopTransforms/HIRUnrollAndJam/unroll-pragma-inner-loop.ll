...
; Verify that unroll & jam is skipped when unroll pragma is specified on the
; inner loop.

; + DO i1 = 0, %n + -1, 1   <DO_LOOP>  <MAX_TC_EST = 100>
; |   + DO i2 = 0, %n + -1, 1   <DO_LOOP>  <MAX_TC_EST = 100> <LEGAL_MAX_TC = 2147483647> <unroll = 4>
; |   |   %0 = (@B)[0][i2];
; |   |   %1 = (@A)[0][i1];
; |   |   (@A)[0][i1] = %0 + %1;
; |   + END LOOP
...
; CHECK: |   + DO i2 = 0, %n + -1, 1   <DO_LOOP>  <MAX_TC_EST = 100> <LEGAL_MAX_TC = 2147483647> <unroll = 4>
; CHECK: |   |   %0 = (@B)[0][i2];
; CHECK: |   |   %1 = (@A)[0][i1];
; CHECK: |   |   (@A)[0][i1] = %0 + %1;
; CHECK: |   + END LOOP
...
for.body3:                                        ; preds = %for.body3.lr.ph, %for.body3
  ...
  br i1 %cmp2, label %for.body3, label %for.cond1.for.inc6_crit_edge, !llvm.loop !0
