...
; CHECK: |   |   + DO i3 = 0, %N + -1, 1   <DO_LOOP>  <MAX_TC_EST = 1024>
; CHECK: |   |   |   %0 = (@c)[0][2 * i1][8 * i2];
; CHECK: |   |   |   %1 = (@a)[0][2 * i1][i3];
; CHECK: |   |   |   %2 = (@b)[0][i3][8 * i2];
; CHECK: |   |   |   %mul = %1  *  %2;
; CHECK: |   |   |   %add = %0  +  %mul;
; CHECK: |   |   |   (@c)[0][2 * i1][8 * i2] = %add;
; CHECK: |   |   |   %0 = (@c)[0][2 * i1 + 1][8 * i2];
; CHECK: |   |   |   %1 = (@a)[0][2 * i1 + 1][i3];
; CHECK: |   |   |   %2 = (@b)[0][i3][8 * i2];
; CHECK: |   |   |   %mul = %1  *  %2;
; CHECK: |   |   |   %add = %0  +  %mul;
; CHECK: |   |   |   (@c)[0][2 * i1 + 1][8 * i2] = %add;
; CHECK: |   |   |   %0 = (@c)[0][2 * i1][8 * i2 + 1];
; CHECK: |   |   |   %1 = (@a)[0][2 * i1][i3];
; CHECK: |   |   |   %2 = (@b)[0][i3][8 * i2 + 1];
; CHECK: |   |   |   %mul = %1  *  %2;
; CHECK: |   |   |   %add = %0  +  %mul;
; CHECK: |   |   |   (@c)[0][2 * i1][8 * i2 + 1] = %add;
; CHECK: |   |   |   %0 = (@c)[0][2 * i1 + 1][8 * i2 + 1];
; CHECK: |   |   |   %1 = (@a)[0][2 * i1 + 1][i3];
; CHECK: |   |   |   %2 = (@b)[0][i3][8 * i2 + 1];
; CHECK: |   |   |   %mul = %1  *  %2;
; CHECK: |   |   |   %add = %0  +  %mul;
; CHECK: |   |   |   (@c)[0][2 * i1 + 1][8 * i2 + 1] = %add;
; CHECK: |   |   |   %0 = (@c)[0][2 * i1][8 * i2 + 2];
; CHECK: |   |   |   %1 = (@a)[0][2 * i1][i3];
; CHECK: |   |   |   %2 = (@b)[0][i3][8 * i2 + 2];
; CHECK: |   |   |   %mul = %1  *  %2;
; CHECK: |   |   |   %add = %0  +  %mul;
; CHECK: |   |   |   (@c)[0][2 * i1][8 * i2 + 2] = %add;
; CHECK: |   |   |   %0 = (@c)[0][2 * i1 + 1][8 * i2 + 2];
; CHECK: |   |   |   %1 = (@a)[0][2 * i1 + 1][i3];
; CHECK: |   |   |   %2 = (@b)[0][i3][8 * i2 + 2];
; CHECK: |   |   |   %mul = %1  *  %2;
; CHECK: |   |   |   %add = %0  +  %mul;
; CHECK: |   |   |   (@c)[0][2 * i1 + 1][8 * i2 + 2] = %add;
; CHECK: |   |   |   %0 = (@c)[0][2 * i1][8 * i2 + 3];
; CHECK: |   |   |   %1 = (@a)[0][2 * i1][i3];
; CHECK: |   |   |   %2 = (@b)[0][i3][8 * i2 + 3];
; CHECK: |   |   |   %mul = %1  *  %2;
; CHECK: |   |   |   %add = %0  +  %mul;
; CHECK: |   |   |   (@c)[0][2 * i1][8 * i2 + 3] = %add;
; CHECK: |   |   |   %0 = (@c)[0][2 * i1 + 1][8 * i2 + 3];
; CHECK: |   |   |   %1 = (@a)[0][2 * i1 + 1][i3];
; CHECK: |   |   |   %2 = (@b)[0][i3][8 * i2 + 3];
; CHECK: |   |   |   %mul = %1  *  %2;
; CHECK: |   |   |   %add = %0  +  %mul;
; CHECK: |   |   |   (@c)[0][2 * i1 + 1][8 * i2 + 3] = %add;
; CHECK: |   |   |   %0 = (@c)[0][2 * i1][8 * i2 + 4];
; CHECK: |   |   |   %1 = (@a)[0][2 * i1][i3];
; CHECK: |   |   |   %2 = (@b)[0][i3][8 * i2 + 4];
; CHECK: |   |   |   %mul = %1  *  %2;
; CHECK: |   |   |   %add = %0  +  %mul;
; CHECK: |   |   |   (@c)[0][2 * i1][8 * i2 + 4] = %add;
; CHECK: |   |   |   %0 = (@c)[0][2 * i1 + 1][8 * i2 + 4];
; CHECK: |   |   |   %1 = (@a)[0][2 * i1 + 1][i3];
; CHECK: |   |   |   %2 = (@b)[0][i3][8 * i2 + 4];
; CHECK: |   |   |   %mul = %1  *  %2;
; CHECK: |   |   |   %add = %0  +  %mul;
; CHECK: |   |   |   (@c)[0][2 * i1 + 1][8 * i2 + 4] = %add;
; CHECK: |   |   |   %0 = (@c)[0][2 * i1][8 * i2 + 5];
; CHECK: |   |   |   %1 = (@a)[0][2 * i1][i3];
; CHECK: |   |   |   %2 = (@b)[0][i3][8 * i2 + 5];
; CHECK: |   |   |   %mul = %1  *  %2;
; CHECK: |   |   |   %add = %0  +  %mul;
; CHECK: |   |   |   (@c)[0][2 * i1][8 * i2 + 5] = %add;
; CHECK: |   |   |   %0 = (@c)[0][2 * i1 + 1][8 * i2 + 5];
; CHECK: |   |   |   %1 = (@a)[0][2 * i1 + 1][i3];
; CHECK: |   |   |   %2 = (@b)[0][i3][8 * i2 + 5];
; CHECK: |   |   |   %mul = %1  *  %2;
; CHECK: |   |   |   %add = %0  +  %mul;
; CHECK: |   |   |   (@c)[0][2 * i1 + 1][8 * i2 + 5] = %add;
; CHECK: |   |   |   %0 = (@c)[0][2 * i1][8 * i2 + 6];
; CHECK: |   |   |   %1 = (@a)[0][2 * i1][i3];
; CHECK: |   |   |   %2 = (@b)[0][i3][8 * i2 + 6];
; CHECK: |   |   |   %mul = %1  *  %2;
; CHECK: |   |   |   %add = %0  +  %mul;
; CHECK: |   |   |   (@c)[0][2 * i1][8 * i2 + 6] = %add;
; CHECK: |   |   |   %0 = (@c)[0][2 * i1 + 1][8 * i2 + 6];
; CHECK: |   |   |   %1 = (@a)[0][2 * i1 + 1][i3];
; CHECK: |   |   |   %2 = (@b)[0][i3][8 * i2 + 6];
; CHECK: |   |   |   %mul = %1  *  %2;
; CHECK: |   |   |   %add = %0  +  %mul;
; CHECK: |   |   |   (@c)[0][2 * i1 + 1][8 * i2 + 6] = %add;
; CHECK: |   |   |   %0 = (@c)[0][2 * i1][8 * i2 + 7];
; CHECK: |   |   |   %1 = (@a)[0][2 * i1][i3];
; CHECK: |   |   |   %2 = (@b)[0][i3][8 * i2 + 7];
; CHECK: |   |   |   %mul = %1  *  %2;
; CHECK: |   |   |   %add = %0  +  %mul;
; CHECK: |   |   |   (@c)[0][2 * i1][8 * i2 + 7] = %add;
; CHECK: |   |   |   %0 = (@c)[0][2 * i1 + 1][8 * i2 + 7];
; CHECK: |   |   |   %1 = (@a)[0][2 * i1 + 1][i3];
; CHECK: |   |   |   %2 = (@b)[0][i3][8 * i2 + 7];
; CHECK: |   |   |   %mul = %1  *  %2;
; CHECK: |   |   |   %add = %0  +  %mul;
; CHECK: |   |   |   (@c)[0][2 * i1 + 1][8 * i2 + 7] = %add;
; CHECK: |   |   + END LOOP
; CHECK: |   + END LOOP
; CHECK: |
; CHECK: |
...
; CHECK: |   |   |   %mul = %1  *  %2;
; CHECK: |   |   |   %add = %0  +  %mul;
; CHECK: |   |   |   (@c)[0][2 * i1][i2] = %add;
; CHECK: |   |   |   %0 = (@c)[0][2 * i1 + 1][i2];
; CHECK: |   |   |   %1 = (@a)[0][2 * i1 + 1][i3];
; CHECK: |   |   |   %2 = (@b)[0][i3][i2];
; CHECK: |   |   |   %mul = %1  *  %2;
; CHECK: |   |   |   %add = %0  +  %mul;
; CHECK: |   |   |   (@c)[0][2 * i1 + 1][i2] = %add;
; CHECK: |   |   + END LOOP
; CHECK: |   + END LOOP
; CHECK: + END LOOP
