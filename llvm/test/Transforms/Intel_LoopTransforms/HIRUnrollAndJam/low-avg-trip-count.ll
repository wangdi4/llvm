...
; RUN: opt -passes="hir-ssa-deconstruction,hir-unroll-and-jam,print<hir>" -debug-only=hir-unroll-and-jam < %s 2>&1 | FileCheck %s

; Verify that we skip unroll & jam of loop with low avg trip count.


; CHECK: Skipping unroll & jam of small trip count loop!


; CHECK: + DO i1 = 0, %n + -1, 1   <DO_LOOP>  <MAX_TC_EST = 100>  <LEGAL_MAX_TC = 2147483647> <avg_trip_count = 6>
; CHECK: |   + DO i2 = 0, %n + -1, 1   <DO_LOOP>  <MAX_TC_EST = 100>  <LEGAL_MAX_TC = 2147483647>
; CHECK: |   |   %0 = (@B)[0][i2];
; CHECK: |   |   %1 = (@A)[0][i1];
; CHECK: |   |   (@A)[0][i1] = %0 + %1;
; CHECK: |   + END LOOP
