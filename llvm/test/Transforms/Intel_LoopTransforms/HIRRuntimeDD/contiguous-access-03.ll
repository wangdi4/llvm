...
; This test case checks that HIRRuntimeDD was applied since the the number of
; entries accessed contiguously is larger than the IV coefficient.
...
;        |   %7 = (%B)[i1 + 2];
;        |   (%A)[2 * i1 + sext.i32.i64(%t) + 2] = %7;
;        |   %10 = (%B)[i1 + 3];
;        |   (%A)[2 * i1 + sext.i32.i64(%t) + 3] = %10;
;        + END LOOP
;  END REGION
...
; CHECK:        %mv.test1 = &((%A)[sext.i32.i64(%t) + 201]) >=u &((%B)[0]);
; CHECK:        %mv.and = %mv.test  &  %mv.test1;
; CHECK:        if (%mv.and == 0)
; CHECK:        {
; CHECK:           + DO i1 = 0, 99, 1   <DO_LOOP>  <MVTag: 31>
; CHECK:           |   %1 = (%B)[i1];
; CHECK:           |   (%A)[2 * i1 + sext.i32.i64(%t)] = %1;
; CHECK:           |   %4 = (%B)[i1 + 1];
; CHECK:           |   (%A)[2 * i1 + sext.i32.i64(%t) + 1] = %4;
; CHECK:           |   %7 = (%B)[i1 + 2];
; CHECK:           |   (%A)[2 * i1 + sext.i32.i64(%t) + 2] = %7;
; CHECK:           |   %10 = (%B)[i1 + 3];
; CHECK:           |   (%A)[2 * i1 + sext.i32.i64(%t) + 3] = %10;
; CHECK:           + END LOOP
; CHECK:        }
; CHECK:        else
; CHECK:        {
; CHECK:           + DO i1 = 0, 99, 1   <DO_LOOP>  <MVTag: 31> <nounroll> <novectorize>
; CHECK:           |   %1 = (%B)[i1];
; CHECK:           |   (%A)[2 * i1 + sext.i32.i64(%t)] = %1;
; CHECK:           |   %4 = (%B)[i1 + 1];
; CHECK:           |   (%A)[2 * i1 + sext.i32.i64(%t) + 1] = %4;
