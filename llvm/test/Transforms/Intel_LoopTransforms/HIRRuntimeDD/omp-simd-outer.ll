...
;       |   @llvm.lifetime.start.p0i8(8,  &((i8*)(%block_id.linear.iv)[0]));
...
;       |      + END LOOP
;       |   }
;       |   @llvm.lifetime.end.p0i8(8,  &((i8*)(%block_id.linear.iv)[0]));
;       + END LOOP
;
...
define dso_local void @foo(double* nocapture %lema, double* nocapture %sema, i32 %trip_count, i32 %midPoint, i64 %block_end, double* nocapture readonly %sfLema, double* nocapture readonly %sfSema) local_unnamed_addr #0 {
entry:
...
DIR.OMP.SIMD.2:                                   ; preds = %entry
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.SIMD"(), "QUAL.OMP.SIMDLEN"(i32 64), "QUAL.OMP.NORMALIZED.IV"(i8* null), "QUAL.OMP.NORMALIZED.UB"(i8* null), "QUAL.OMP.LINEAR:IV"(i64* %block_id.linear.iv, i32 1) ]
  %1 = bitcast i64* %block_id.linear.iv to i8*
  %cmp540 = icmp slt i32 %trip_count, 1
  %conv = sitofp i32 %midPoint to double
  %2 = add nuw nsw i32 %trip_count, 1
...
omp.inner.for.body:                               ; preds = %DIR.OMP.SIMD.2, %for.cond.cleanup
  ...
  store i64 %.omp.iv.local.034, i64* %block_id.linear.iv, align 8, !tbaa !2
...
for.body:                                         ; preds = %for.body.preheader, %for.body
  ...
  %ptridx = getelementptr inbounds double, double* %sfLema, i64 %indvars.iv
  %3 = load double, double* %ptridx, align 8, !tbaa !6
  %ptridx7 = getelementptr inbounds double, double* %lema, i64 %indvars.iv
  %4 = load double, double* %ptridx7, align 8, !tbaa !6
  %sub8 = fsub double %conv, %4
  %mul9 = fmul double %3, %sub8
  %add12 = fadd double %4, %mul9
  store double %add12, double* %ptridx7, align 8, !tbaa !6
  %ptridx14 = getelementptr inbounds double, double* %sfSema, i64 %indvars.iv
  %5 = load double, double* %ptridx14, align 8, !tbaa !6
  %ptridx17 = getelementptr inbounds double, double* %sema, i64 %indvars.iv
  %6 = load double, double* %ptridx17, align 8, !tbaa !6
  %sub18 = fsub double %conv, %6
  %mul19 = fmul double %5, %sub18
  %add22 = fadd double %6, %mul19
  store double %add22, double* %ptridx17, align 8, !tbaa !6
