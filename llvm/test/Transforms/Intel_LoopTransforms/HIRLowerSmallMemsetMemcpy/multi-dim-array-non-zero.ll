...
; and transformed to a loop. Check that non-zero innermost index is transformed correctly.
; Also check that loop is correctly formed on level 2.

; HIR:
;            BEGIN REGION { }
;                  + DO i1 = 0, 9, 1   <DO_LOOP>
;                  |   @llvm.memset.p0i8.i64(&((%a)[0][2][3]),  97,  6,  0);
;                  + END LOOP
;            END REGION
...
entry:
  ...
  %arraydecay = getelementptr inbounds [3 x [11 x i8]], [3 x [11 x i8]]* %a, i64 0, i64 2, i64 3
  br label %for.body
