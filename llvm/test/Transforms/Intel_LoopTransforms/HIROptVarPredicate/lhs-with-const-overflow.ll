...
; Test checks that Opt Var Predicate can't handle non-constant RHS in presence of constant
; in LHS of condition that overflows the LHS type.
...
;                  |   %1 = (@B)[0][i1 + 1];
;                  |   (@A)[0][i1 + 1] = %1 + 1;
;                  |   if (i1 + 9223372036854775804 > %m)
;                  |   <RVAL-REG> LINEAR i64 i1 + 9223372036854775804 {sb:2}
;                  |   <RVAL-REG> LINEAR sext.i32.i64(%m) {sb:2}
;                  |      <BLOB> LINEAR i32 %m {sb:14}
;                  |   {
;                  |      (@C)[0][i1 + 1] = %1 + -1;
;                  |   }
;                  + END LOOP
...
for.body:                                         ; preds = %for.body.preheader, %for.inc
  ...
  %add2 = add i64 %indvars.iv, 9223372036854775803
  %cmp3 = icmp sgt i64 %add2, %0
