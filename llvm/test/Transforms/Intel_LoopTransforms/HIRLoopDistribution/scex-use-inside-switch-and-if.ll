...
; expanding it. Note that although use of %t43 is inside a switch, its
; immediate parent is an HLIf and the existing analysis can place the
; definition based on the HLIf parent.
...
define fastcc void @brthso_(ptr %t0, i32 %t1, i1 %t2, i1 %t3, i1 %t4) {
entry:
  br label %loop
...
t42:                                               ; preds = %t30
  ...
  switch i32 %t1, label %latch [
    i32 0, label %nested.if
  ]

nested.if:
  br i1 %t4, label %t35, label %latch
