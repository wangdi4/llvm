;RUN: opt -hir-ssa-deconstruction -hir-temp-cleanup   -hir-loop-distribute-memrec -hir-verify-cf-def-level -print-after=hir-loop-distribute-memrec  < %s 2>&1 | FileCheck %s
;RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-loop-distribute-memrec,print<hir>" -aa-pipeline="basic-aa"  -hir-verify-cf-def-level  < %s 2>&1 | FileCheck %s
