...
;
; Check the distributed loops inherit pragma trip count info from the original loop
;
;          BEGIN REGION { }
;<29>         + DO i1 = 0, 99998, 1   <DO_LOOP>
;<28>         |   + DO i2 = 0, 99998, 1   <DO_LOOP> <min_trip_count = 50> <avg_trip_count = 75> <max_trip_count = 100>
;<5>          |   |   %0 = (@B)[0][i1 + 1][i2 + 1];
;<7>          |   |   %1 = (@C)[0][i1 + 1][i2 + 1];
;<8>          |   |   %add = %0  +  %1;
...
; CHECK-NEXT: DO i2 = 0, 99998, 1   <DO_LOOP> <min_trip_count = 50> <avg_trip_count = 75> <max_trip_count = 100>
; CHECK: (@A)[0][i1 + 1][i2 + 1] =
; CHECK-NEXT: END LOOP

; CHECK: DO i2 = 0, 99998, 1   <DO_LOOP> <min_trip_count = 50> <avg_trip_count = 75> <max_trip_count = 100>
; CHECK: (@A)[0][i1 + 1][i2]
; CHECK: END LOOP
; CHECK-NEXT: END LOOP
...
for.body.3:                                       ; preds = %for.body.3, %for.cond.1.preheader
  ...
  br i1 %exitcond, label %for.inc.23, label %for.body.3, !llvm.loop !0
