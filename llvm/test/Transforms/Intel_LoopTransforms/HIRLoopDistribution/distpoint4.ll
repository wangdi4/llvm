...
;RUN: opt -passes="loop-simplify,hir-ssa-deconstruction,hir-loop-distribute-memrec,hir-cg,loop-simplifycfg,intel-ir-optreport-emitter" -aa-pipeline="basic-aa" -intel-loop-optreport=low 2>&1 < %s -S | FileCheck %s  -check-prefix=OPTREPORT
;
;  for (i =0 ; i<n ; i++) {
;    a1[i] += b2  - 11.0;
