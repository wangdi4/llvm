...
; RUN: opt -hir-create-function-level-region -passes="hir-ssa-deconstruction,hir-cross-loop-array-contraction,print<hir>" -aa-pipeline="basic-aa" -disable-output < %s 2>&1 | FileCheck %s

; Check that the first loop is not eliminated as dead by contraction pass.
; The analysis missed the second loop as use loop when analyzing the first loop
; as def loop and incorrectly deduced %A to be dead.
; The candidate loop for (%A)[0][i1][i3][i4][i6][i5] was set as the i1 loop due
; to presence of i1.
...
; CHECK: + UNKNOWN LOOP i1  <MAX_TC_EST = 100>
; CHECK: |   <i1 = 0>
; CHECK: |   do.body:
; CHECK: |
...
do.body:                                          ; preds = %do.cond, %entry
  %do.iv = phi i64 [ 0, %entry ], [ %do.inc, %do.cond ]
  br label %for.body36
...
for.body97:                                       ; preds = %for.body93, %for.inc119
  ...
  %arrayidx99 = getelementptr inbounds [100 x [100 x [100 x [10 x [10 x i32]]]]], [100 x [100 x [100 x [10 x [10 x i32]]]]]* %A, i64 0, i64 %do.iv
