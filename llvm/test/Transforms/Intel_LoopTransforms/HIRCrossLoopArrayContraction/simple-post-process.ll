; RUN: opt -hir-create-function-level-region -hir-ssa-deconstruction -hir-cross-loop-array-contraction -print-after=hir-cross-loop-array-contraction -disable-output -S < %s 2>&1 | FileCheck %s
; RUN: opt -hir-create-function-level-region -passes="hir-ssa-deconstruction,require<hir-loop-statistics>,hir-cross-loop-array-contraction,print<hir>" -aa-pipeline="basic-aa" -disable-output < %s 2>&1 | FileCheck %s
...
for.body16:                                       ; preds = %for.cond13.preheader, %for.body16
  ...
  %1 = add nuw nsw i64 %indvars.iv157, %indvars.iv161
...
for.body61:                                       ; preds = %for.cond58.preheader, %for.body61
  ...
  %4 = load i32, i32* %arrayidx71, align 4, !tbaa !2
  %add72 = add nsw i32 %4, 1
