; RUN: opt -disable-hir-cross-loop-array-contraction=false -disable-hir-pragma-bailout -hir-create-function-level-region -hir-ssa-deconstruction  -hir-loop-distribute-loopnest -hir-cross-loop-array-contraction -hir-cg -force-hir-cg -print-before=hir-cross-loop-array-contraction -print-after=hir-cross-loop-array-contraction -disable-output -S < %s 2>&1 | FileCheck %s
; RUN: opt -disable-hir-cross-loop-array-contraction=false -disable-hir-pragma-bailout -hir-create-function-level-region -passes="hir-ssa-deconstruction,require<hir-loop-statistics>,hir-loop-distribute-loopnest,print<hir>,hir-cross-loop-array-contraction,print<hir>,hir-cg" -force-hir-cg -aa-pipeline="basic-aa" -disable-output < %s 2>&1 | FileCheck %s
