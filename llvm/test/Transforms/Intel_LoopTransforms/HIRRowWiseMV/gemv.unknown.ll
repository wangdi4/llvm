...
; RUN: opt -opaque-pointers=0 -whole-program-assume -intel-libirc-allowed -hir-create-function-level-region -passes='require<dtransanalysis>,function(hir-ssa-deconstruction,hir-temp-cleanup,hir-rowwise-mv)' -print-before=hir-rowwise-mv -print-after=hir-rowwise-mv -disable-output 2>&1 < %s | FileCheck %s
