...
; Check that we bail out for if with multiple predicates which we cannot invert.
...
bb2:                                              ; preds = %bb6_endif, %bb2.preheader
  ...
  %rel.2 = fcmp reassoc ninf nsz arcp contract afn one float %"sub_$A_entry[]_fetch.4", 1.000000e+02
  %rel.3 = fcmp reassoc ninf nsz arcp contract afn one float %"sub_$A_entry[]_fetch.4", 2.000000e+02
  %c2 = and i1 %rel.2, %rel.3
  br i1 %c2, label %bb_new3_then, label %bb_new4_else
