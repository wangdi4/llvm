...
define void @sub_(float* noalias nocapture writeonly dereferenceable(4) %"sub_$A", float* noalias nocapture readonly dereferenceable(4) %"sub_$B", i32* noalias nocapture readonly dereferenceable(4) %"sub_$N", i32 %"sub_$N_fetch.1") #0 {
alloca_0:
...
bb2:                                              ; preds = %bb6_endif, %bb2.preheader
  ...
  %"sub_$B_entry[]10" = tail call float* @llvm.intel.subscript.p0f32.i64.i64.p0f32.i64(i8 0, i64 1, i64 0, float* nonnull elementtype(float) %"sub_$B", i64 %indvars.iv)
  %"sub_$B_entry[]_fetch.4" = load float, float* %"sub_$B_entry[]10", align 1
...
bb_new4_else:                                     ; preds = %bb2
  %"sub_$B_entry[]4" = tail call float* @llvm.intel.subscript.p0f32.i64.i64.p0f32.i64(i8 0, i64 1, i64 0, float* nonnull elementtype(float) %"sub_$B", i64 %1)
  %"sub_$B_entry[]_fetch.9" = load float, float* %"sub_$B_entry[]4", align 1
  %"sub_$A_entry[]7" = tail call float* @llvm.intel.subscript.p0f32.i64.i64.p0f32.i64(i8 0, i64 1, i64 4, float* nonnull elementtype(float) %"sub_$A", i64 %indvars.iv)
  store float %"sub_$B_entry[]_fetch.9", float* %"sub_$A_entry[]7", align 1
  br label %bb6_endif
