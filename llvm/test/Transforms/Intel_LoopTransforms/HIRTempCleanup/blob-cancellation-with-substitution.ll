...
; CHECK: |   %src1.19 = 0;
; CHECK: |   if (%idx + -1 * ptrtoint.i8*.i64(%pLineSrc.191713) + ptrtoint.i8*.i64(%pLineSrc.191713.out) + 1 < %i258)
; CHECK: |   {
...
define void @foo(i32 %i3, i32 %i9, i8* %add.ptr, i32 %spec.select, i64 %idx, i32* %m_nStride) {
entry:
  br label %for.body802

for.body802:                                      ; preds = %if.end836, %entry
  %i258 = phi i32 [ %i263, %if.end836 ], [ %i9, %entry ]
  %pLineSrc.191713 = phi i8* [ %add.ptr850, %if.end836 ], [ %add.ptr, %entry ]
  %yy.191712 = phi i32 [ %inc852, %if.end836 ], [ %spec.select, %entry ]
  %add.ptr806 = getelementptr inbounds i8, i8* %pLineSrc.191713, i64 %idx
  %i259 = load i8, i8* %add.ptr806, align 1
  %conv808 = zext i8 %i259 to i32
  %srcPtr.91701 = getelementptr inbounds i8, i8* %add.ptr806, i64 1
  %shl8371703 = shl nuw nsw i32 %conv808, 8
  %sub.ptr.lhs.cast826 = ptrtoint i8* %srcPtr.91701 to i64
  %sub.ptr.rhs.cast827 = ptrtoint i8* %pLineSrc.191713 to i64
