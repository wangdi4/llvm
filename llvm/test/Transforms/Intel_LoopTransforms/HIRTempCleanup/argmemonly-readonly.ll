...
; Verify that we are able to forward substitute load (%B)[i1] in the presence of
; argmemonly function call @bar() which has readonly attribute on the argument.
...
define void @foo(float* noalias %A, float* noalias %B, i32 %n) {
entry:
...
declare void @bar(float* readonly) #1
