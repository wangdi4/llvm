...
; Verify that we are able to forward substitute non-aliasing load (%B)[i1]
; but give up on (%A)[i1] in the presence of aliasing store (%A)[0].
...
define void @foo(float* noalias %A, float* noalias %B, i32 %n, float %f) {
entry:
