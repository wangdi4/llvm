; RUN: opt -hir-ssa-deconstruction -hir-temp-cleanup -hir-loop-interchange -hir-loop-blocking -print-after=hir-loop-blocking -hir-loop-blocking-algo=kandr -print-before=hir-loop-blocking < %s 2>&1 | FileCheck %s --check-prefix=KANDR
; RUN: opt -hir-ssa-deconstruction -hir-temp-cleanup -hir-loop-interchange -hir-loop-blocking -print-after=hir-loop-blocking  -hir-loop-blocking-algo=outer -print-before=hir-loop-blocking  < %s 2>&1 | FileCheck %s --check-prefix=OUTER
...
; RUN: opt -hir-ssa-deconstruction -hir-temp-cleanup -hir-loop-interchange -hir-loop-blocking -print-after=hir-loop-blocking  -print-before=hir-loop-blocking -hir-loop-blocking-blocksize=24 < %s 2>&1 | FileCheck %s --check-prefix=USERBS

; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-loop-interchange,print<hir>,hir-loop-blocking,print<hir>" -aa-pipeline="basic-aa"  -hir-loop-blocking-algo=kandr 2>&1 < %s | FileCheck %s --check-prefix=KANDR
; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-loop-interchange,print<hir>,hir-loop-blocking,print<hir>" -aa-pipeline="basic-aa"  -hir-loop-blocking-algo=outer 2>&1 < %s | FileCheck %s --check-prefix=OUTER
...
; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-loop-interchange,print<hir>,hir-loop-blocking,print<hir>" -aa-pipeline="basic-aa"  -hir-loop-blocking-blocksize=24 2>&1 < %s | FileCheck %s --check-prefix=USERBS
...
for.body.6.lr.ph:                                 ; preds = %for.cond.4.for.inc.14_crit_edge, %for.cond.4.preheader.preheader
  ...
  %arrayidx7 = getelementptr inbounds [200 x [200 x double]], [200 x [200 x double]]* @c, i64 0, i64 %i.042, i64 %j.039
...
for.body.6:                                       ; preds = %for.body.6, %for.body.6.lr.ph
  ...
  %arrayidx9 = getelementptr inbounds [200 x [200 x double]], [200 x [200 x double]]* @a, i64 0, i64 %i.042, i64 %k.037
  %1 = load double, double* %arrayidx9, align 8, !tbaa !1
  %arrayidx11 = getelementptr inbounds [200 x [200 x double]], [200 x [200 x double]]* @b, i64 0, i64 %k.037, i64 %j.039
