; RUN: opt -scoped-noalias-aa -hir-ssa-deconstruction -hir-temp-cleanup -hir-sinking-for-perfect-loopnest -hir-loop-interchange -hir-loop-blocking -print-before=hir-loop-blocking -print-after=hir-loop-blocking -hir-print-only=41 %s 2>&1 | FileCheck %s
; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-sinking-for-perfect-loopnest,hir-loop-interchange,print<hir>,hir-loop-blocking,print<hir>" -hir-print-only=41 -aa-pipeline="scoped-noalias-aa" %s 2>&1 | FileCheck %s
