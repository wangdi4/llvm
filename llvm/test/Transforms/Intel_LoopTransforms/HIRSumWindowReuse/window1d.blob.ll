...
; window sum with a CE containing blob terms.
...
; CHECK:       |   {
; CHECK:       |      + DO i2 = 0, 7, 1   <DO_LOOP>
; CHECK:       |      |   %[[WSUM]] = %[[WSUM]]  +  (%A)[i2 + %offset];
; CHECK:       |      + END LOOP
; CHECK:       |   }
; CHECK:       |   else
; CHECK:       |   {
; CHECK:       |      %[[WSUM]] = %[[WSUM]]  -  (%A)[i1 + %offset + -1];
; CHECK:       |      %[[WSUM]] = %[[WSUM]]  +  (%A)[i1 + %offset + 7];
; CHECK:       |   }
; CHECK:       |   %sum = %sum  +  %[[WSUM]]
; CHECK:       |   (%B)[i1] = %sum;
; CHECK:       + END LOOP
...
define void @window1d(double* %A, double*noalias %B, i64 %offset) {
...
L2:
  ...
  %Aijoffs = getelementptr inbounds double, double* %Aijptr, i64 %offset
  %Aij = load double, double* %Aijoffs, align 8
