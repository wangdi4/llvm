...
; window sum without removing the inner loop if it is not dead.
...
; CHECK:       |   %stridesum = 0.000000e+00;
; CHECK:       |
; CHECK:       |   + DO i2 = 0, 7, 1   <DO_LOOP>
; CHECK:       |   |   %sum = %sum  +  (%A)[i1 + i2];
; CHECK:       |   |   %stridesum = %stridesum  +  (%A)[i1 + 2 * i2];
; CHECK:       |   + END LOOP
; CHECK:       |
; CHECK:       |   %fullsum = %sum  +  %stridesum;
; CHECK:       |   (%B)[i1] = %fullsum;
; CHECK:       + END LOOP
; CHECK: END REGION
