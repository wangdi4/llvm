...
; sum with a term load behind a uitofp cast.
...
; CHECK:       |   |   %Aij = uitofp.i16.double((%A)[i1 + i2]);
; CHECK:       |   |   %sum = %sum  +  %Aij;
; CHECK:       |   + END LOOP
; CHECK:       |
; CHECK:       |   (%B)[i1] = %sum;
...
; CHECK:       |   {
; CHECK:       |      + DO i2 = 0, 7, 1   <DO_LOOP>
; CHECK:       |      |   %[[CAST0:[A-Za-z0-9_.]+]] = uitofp.i16.double((%A)[i2]);
; CHECK:       |      |   %[[WSUM]] = %[[WSUM]]  +  %[[CAST0]];
; CHECK:       |      + END LOOP
; CHECK:       |   }
; CHECK:       |   else
; CHECK:       |   {
; CHECK:       |      %[[CAST1:[A-Za-z0-9_.]+]] = uitofp.i16.double((%A)[i1 + -1]);
; CHECK:       |      %[[WSUM]] = %[[WSUM]]  -  %[[CAST1]];
; CHECK:       |      %[[CAST2:[A-Za-z0-9_.]+]] = uitofp.i16.double((%A)[i1 + 7]);
; CHECK:       |      %[[WSUM]] = %[[WSUM]]  +  %[[CAST2]];
; CHECK:       |   }
; CHECK:       |   %sum = %sum  +  %[[WSUM]]
; CHECK:       |   (%B)[i1] = %sum;
; CHECK:       + END LOOP
...
define void @window1d(i16* %A, double*noalias %B) {
...
L2:
  ...
  %Aijptr = getelementptr inbounds i16, i16* %A, i64 %ij
  %Aiji16 = load i16, i16* %Aijptr, align 2
  %Aij = uitofp i16 %Aiji16 to double
