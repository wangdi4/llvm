...
;}
;
;// add_neighbour() function inside cpu2017/541.leela/FastBoard.cpp, AFTER pattern matching
;void FastBoard::add_neighbour(const int i, const int color) {       
;  assert(color == WHITE || color == BLACK || color == EMPTY);
;
;  std::tr1::array<int, 4> nbr_pars;
...
;  //k=1:
;  int ai1 = i + m_dirs[1];
;  m_neighbours[ai1] += (1 << (NBR_SHIFT * color)) - (1 << (NBR_SHIFT * EMPTY));
;  m_libs[m_parent[ai1]]--;
;
;  //k=2:
;  int ai2 = i + m_dirs[2];
;  m_neighbours[ai2] += (1 << (NBR_SHIFT * color)) - (1 << (NBR_SHIFT * EMPTY));
;  m_libs[m_parent[ai2]]--;
;
;  //k=3:
;  int ai3 = i + m_dirs[3];
;  m_neighbours[ai3] += (1 << (NBR_SHIFT * color)) - (1 << (NBR_SHIFT * EMPTY));
;  m_libs[m_parent[ai3]]--;
;}
;
;
...
; CHECK:        |   {
; CHECK:        |      + DO i2 = 0, smax(1, sext.i32.i64(%nbr_par_cnt.060)) + -1, 1   <DO_MULTI_EXIT_LOOP>  <MAX_TC_EST = 4>
; CHECK:        |      |   if ((%nbr_pars)[0].0[i2] == %5)
; CHECK:        |      |   {
; CHECK:        |      |      goto if.end31.loopexit;
; CHECK:        |      |   }
; CHECK:        |      + END LOOP
; CHECK:        |      
; CHECK:        |      goto if.then18;
; CHECK:        |      if.end31.loopexit:
; CHECK:        |      goto if.end31;
; CHECK:        |   }
; CHECK:        |   if.then18:
...
; CHECK:        |   %nbr_par_cnt.060 = %nbr_par_cnt.060  +  1;
; CHECK:        |   (%nbr_pars)[0].0[%nbr_par_cnt.060.out] = %9;
; CHECK:        |   if.end31:
; CHECK:        + END LOOP
; CHECK:  END REGION
;
...
@llvm.global_ctors = appending global [1 x { i32, void ()*, i8* }] [{ i32, void ()*, i8* } { i32 65535, void ()* @_GLOBAL__sub_I_Test0.cpp, i8* null }]
...
define void @_ZN9FastBoard13add_neighbourEii(%class.FastBoard* %this, i32 %i, i32 %color) local_unnamed_addr #2 align 2 {
entry:
...
for.body:                                         ; preds = %if.end31, %entry
  %indvars.iv63 = phi i64 [ 0, %entry ], [ %indvars.iv.next64, %if.end31 ]
  %nbr_par_cnt.060 = phi i32 [ 0, %entry ], [ %nbr_par_cnt.1, %if.end31 ]
  %arrayidx.i = getelementptr inbounds %class.FastBoard, %class.FastBoard* %this, i64 0, i32 12, i32 0, i64 %indvars.iv63
  %2 = load i32, i32* %arrayidx.i, align 4, !tbaa !1
  %add = add nsw i32 %2, %i
  %conv2 = sext i32 %add to i64
  %arrayidx.i55 = getelementptr inbounds %class.FastBoard, %class.FastBoard* %this, i64 0, i32 10, i32 0, i64 %conv2
  %3 = load i16, i16* %arrayidx.i55, align 2, !tbaa !7
  %conv4 = zext i16 %3 to i32
  %add5 = add nuw nsw i32 %sub, %conv4
  %conv6 = trunc i32 %add5 to i16
  store i16 %conv6, i16* %arrayidx.i55, align 2, !tbaa !7
  %cmp957 = icmp sgt i32 %nbr_par_cnt.060, 0
  ...
  %6 = sext i32 %nbr_par_cnt.060 to i64
  br i1 %cmp957, label %for.body11.preheader, label %if.then18
...
for.body11:                                       ; preds = %for.cond8, %for.body11.preheader
  ...
  %arrayidx.i54 = getelementptr inbounds %"class.boost::array.2", %"class.boost::array.2"* %nbr_pars, i64 0, i32 0, i64 %indvars.iv
  %7 = load i32, i32* %arrayidx.i54, align 4, !tbaa !1
...
if.then18:                                        ; preds = %if.then18.loopexit, %for.body
  ...
  %arrayidx.i51 = getelementptr inbounds %class.FastBoard, %class.FastBoard* %this, i64 0, i32 8, i32 0, i64 %conv22
  %8 = load i16, i16* %arrayidx.i51, align 2, !tbaa !11
  %dec = add i16 %8, -1
  store i16 %dec, i16* %arrayidx.i51, align 2, !tbaa !11
  %9 = load i16, i16* %4, align 2, !tbaa !11
  %conv27 = zext i16 %9 to i32
  %inc28 = add nsw i32 %nbr_par_cnt.060, 1
  %arrayidx.i49 = getelementptr inbounds %"class.boost::array.2", %"class.boost::array.2"* %nbr_pars, i64 0, i32 0, i64 %6
  store i32 %conv27, i32* %arrayidx.i49, align 4, !tbaa !1
  br label %if.end31
...
if.end31:                                         ; preds = %if.end31.loopexit, %if.then18
  %nbr_par_cnt.1 = phi i32 [ %inc28, %if.then18 ], [ %nbr_par_cnt.060, %if.end31.loopexit ]
  %indvars.iv.next64 = add nuw nsw i64 %indvars.iv63, 1
  %exitcond = icmp eq i64 %indvars.iv.next64, 4
...
define internal void @_GLOBAL__sub_I_Test0.cpp() #5 section ".text.startup" {
entry:
