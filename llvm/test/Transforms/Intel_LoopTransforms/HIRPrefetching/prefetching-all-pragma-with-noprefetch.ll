...
; CHECK-NEXT:      |   @llvm.prefetch.p0i8(&((i8*)(@C)[0][i1 + 20]),  0,  1,  1);
; CHECK-NEXT:      |   @llvm.prefetch.p0i8(&((i8*)(@A)[0][i1 + 20]),  0,  1,  1);
; CHECK-NEXT:      + END LOOP
;
; CHECK:           ret &((undef)[0]);
; CHECK:     END REGION
...
entry:
  %0 = call token @llvm.directive.region.entry() [ "DIR.PRAGMA.PREFETCH_LOOP"(), "QUAL.PRAGMA.ENABLE"(i32 1), "QUAL.PRAGMA.VAR"(i8* null), "QUAL.PRAGMA.HINT"(i32 3), "QUAL.PRAGMA.DISTANCE"(i32 20), "QUAL.PRAGMA.ENABLE"(i32 0), "QUAL.PRAGMA.VAR"([1000 x float]* @B), "QUAL.PRAGMA.HINT"(i32 -1), "QUAL.PRAGMA.DISTANCE"(i32 -1)]
  br label %for.body
