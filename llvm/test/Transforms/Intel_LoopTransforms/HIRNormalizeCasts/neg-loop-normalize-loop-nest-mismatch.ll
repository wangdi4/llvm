...
; This test case checks that the zext casting for both loop upperbounds weren't
; converted into sext. The reason is that %n is used as zext in the instruction
; %t2 in the innermost loop. It was created from the following C++ code, but
; the casting was added in the IR:

; unsigned foo(unsigned *a, int n, unsigned m) {
;   unsigned res = 0;
;   for (int i = 0; i < n; i++ ) {
;     for (int j = 0; j < n; j++) {
;       res += a[m * i + n *j];
