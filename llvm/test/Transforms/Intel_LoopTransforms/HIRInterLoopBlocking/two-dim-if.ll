; RUN: opt -hir-ssa-deconstruction -hir-temp-cleanup -hir-inter-loop-blocking -hir-inter-loop-blocking-force-test -print-before=hir-inter-loop-blocking -print-after=hir-inter-loop-blocking -hir-create-function-level-region < %s 2>&1 | FileCheck %s
; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,print<hir>,hir-inter-loop-blocking,print<hir>" -hir-inter-loop-blocking-force-test -print-before=hir-inter-loop-blocking -print-after=hir-inter-loop-blocking -hir-create-function-level-region < %s 2>&1 | FileCheck %s
