...
;
; RUN: opt -opaque-pointers -hir-ssa-deconstruction -hir-temp-cleanup -disable-hir-inter-loop-blocking=false -hir-inter-loop-blocking -debug-only=hir-inter-loop-blocking-profit -print-before=hir-inter-loop-blocking -hir-inter-loop-blocking-stripmine-size=2 -print-after=hir-inter-loop-blocking   < %s 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers -disable-hir-inter-loop-blocking=false -passes="hir-ssa-deconstruction,hir-temp-cleanup,print<hir>,hir-inter-loop-blocking,print<hir>" -aa-pipeline="basic-aa" -debug-only=hir-inter-loop-blocking-profit -hir-inter-loop-blocking-stripmine-size=2 2>&1 < %s | FileCheck %s
;
...
bb33:                                             ; preds = %bb33, %bb29
  ...
  %"sub1_$A[]32[]" = tail call double* @llvm.intel.subscript.p0f64.i64.i64.p0f64.i64(i8 0, i64 1, i64 8, double* elementtype(double) %"sub1_$A[]32", i64 %"sub1_$J.1")
  %"sub1_$A[]32[]_fetch" = load double, double* %"sub1_$A[]32[]", align 1
  %add34 = fadd double %"sub1_$A[]32[]_fetch", 2.000000e+00
  %"sub1_$B[]43[]" = tail call double* @llvm.intel.subscript.p0f64.i64.i64.p0f64.i64(i8 0, i64 1, i64 8, double* elementtype(double) %"sub1_$B[]43", i64 %"sub1_$J.1")
