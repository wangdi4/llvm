...
;CHECK: region.0:
;CHECK: {{loop.[0-9]+:}}
;CHECK: [[A_ADDR:%.*]] = getelementptr inbounds i32, ptr %A
;CHECK: [[A_LOAD:%.*]] = load i32, ptr [[A_ADDR]]
;CHECK: store i32 [[A_LOAD]], ptr [[A_SLOT:%t[0-9]+]]

;CHECK: [[B_ADDR:%.*]] = getelementptr inbounds i32, ptr %B
;CHECK: [[B_LOAD:%.*]] = load i32, ptr [[B_ADDR]]
;CHECK: store i32 [[B_LOAD]], ptr [[B_SLOT:%t[0-9]+]]

; A[i] < B[i]
;CHECK: [[CMP1:%hir.cmp.[0-9]+]] = icmp slt i32 [[A_SLOT]].{{[0-9]*}}, [[B_SLOT]].{{[0-9]*}}
;CHECK: store i1 [[CMP1]], ptr [[CMP1_SLOT:%t[0-9]+]]

;tobool = small.030 != 0
;CHECK: [[CMP2:%hir.cmp.[0-9]+]] = icmp ne i8 {{.*}}, 0
;CHECK: store i1 [[CMP2]], ptr [[CMP2_SLOT:%t[0-9]+]]

;extend cmp3 to i8 for or op. Done in original IR
;CHECK: [[CMP1_LOAD:%.*]] = load i1, ptr [[CMP1_SLOT]]
;CHECK: [[CMP1_EXT:%.*]] = zext i1 [[CMP1_LOAD]] to i8

;zext the ne 0 cmp
;CHECK: [[CMP2_LOAD:%.*]] = load i1, ptr [[CMP2_SLOT]]
;CHECK: [[CMP2_EXT:%.*]] = zext i1 [[CMP2_LOAD]] to i8

;CHECK: [[OR_CMP:%.*]] = or i8 [[CMP1_EXT]], [[CMP2_EXT]]
;CHECK: store i8 [[OR_CMP]], ptr [[OR_SLOT:%t[0-9]+]]


;CHECK: [[OR_LOAD:%.*]] = load i8, ptr [[OR_SLOT]]
...
define void @_Z3fooPiS_i(ptr nocapture %A, ptr nocapture %B, i32 %n) {
entry:
