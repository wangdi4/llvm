...
; CHECK: |   |   (@D)[0:i1:544(%base_type:0)].0[0:i2:4(float:15)] = %cast;
; CHECK: |   + END LOOP
; CHECK: + END LOOP
...
; CHECK: = load i64, ptr %i1
; CHECK: [[IV:%.*]] = load i64, ptr %i1.i64, align 4
; CHECK:  [[MUL:%.*]] = mul nsw i64 544, [[IV]]
; CHECK: [[GEP1:%.*]] = getelementptr inbounds i8, ptr @D, i64 [[MUL]]
; CHECK: = getelementptr inbounds %base_type, ptr [[GEP1]]
...
bb2.i:                                            ; preds = %loop_exit7.i, %entry
  ...
  %noexact.sub = tail call ptr @llvm.intel.subscript.nonexact.p0.i64.i64.p0.i64(i8 0, i64 1, i64 544, ptr elementtype(%base_type) @D, i64 %indvars.iv)
  %gep = getelementptr inbounds %base_type, ptr %noexact.sub, i64 0, i32 0, i64 0
...
loop_body6.i:                                     ; preds = %loop_body6.i, %bb2.i
  ...
  %subs = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 0, i64 1, i64 4, ptr elementtype(float) %gep, i64 %iv)
  store float %cast, ptr %subs, align 1
...
declare ptr @llvm.intel.subscript.nonexact.p0.i64.i64.p0.i64(i8, i64, i64, ptr, i64) #0
