...
; Verifies UB is incremented before loop
; Verifies UB is correctly sext
; Verifies UB and IV increment are correctly used in bt
...
; CHECK-NEXT:  [[UB0:%[0-9]+]] = add i64 [[SEXT_OP1]], -1
; CHECK-NEXT:  [[UB1:%[0-9]+]] = add i64 [[UB0]], 1
...
; make sure ub and iv update is used in bt of loop
; CHECK: %cond[[L1Label]] = icmp ne i64 [[IV_UPDATE]], [[UB1]]
...
entry:
  ...
  br i1 %cmp6, label %SIMD.1, label %for.end
