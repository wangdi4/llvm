...
;
; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-store-result-into-temp-array,print<hir>" -hir-create-function-level-region -disable-output -hir-details 2>&1 < %s | FileCheck %s
;
...
; CHECK:           |   <LVAL-REG> NON-LINEAR ptr %TempArray
; CHECK:           |   <RVAL-REG> NON-LINEAR i64 %array_size5
;                  |
...
; CHECK:           |   |   |   |   <LVAL-REG> (LINEAR ptr %TempArray{def@1})[LINEAR i64 i2][LINEAR i64 i3][LINEAR i64 i4] inbounds
; CHECK:           |   |   |   |      <BLOB> LINEAR ptr %TempArray{def@1}
; CHECK:           |   |   |   |      <BLOB> LINEAR i32 %10
; CHECK:           |   |   |   |      <BLOB> LINEAR i32 %9
; CHECK:           |   |   |   |   <RVAL-REG> NON-LINEAR double %72
; CHECK:           |   |   |   + END LOOP
; CHECK:           |   |   + END LOOP
; CHECK:           |   + END LOOP
;                  |
...
; CHECK:           |   |   |   |   <LVAL-REG> NON-LINEAR double %73
; CHECK:           |   |   |   |   <RVAL-REG> (LINEAR ptr %TempArray{def@1})[LINEAR i64 i2][LINEAR i64 i3][LINEAR i64 i4] inbounds
; CHECK:           |   |   |   |      <BLOB> LINEAR ptr %TempArray{def@1}
; CHECK:           |   |   |   |      <BLOB> LINEAR i32 %10
; CHECK:           |   |   |   |      <BLOB> LINEAR i32 %9
;                  |   |   |   |
; CHECK:           |   |   |   |   %89 = (%TempArray)[i2 + 1][zext.i32.i64(%47) + -2][zext.i32.i64(%57) + -2];
; CHECK:           |   |   |   |   <LVAL-REG> NON-LINEAR double %89
; CHECK:           |   |   |   |   <RVAL-REG> (LINEAR ptr %TempArray{def@1})[LINEAR i64 i2 + 1][LINEAR i64 zext.i32.i64(%47) + -2{def@3}][NON-LINEAR i64 zext.i32.i64(%57) + -2] inbounds
; CHECK:           |   |   |   |      <BLOB> LINEAR ptr %TempArray{def@1}
; CHECK:           |   |   |   |      <BLOB> LINEAR i32 %10
; CHECK:           |   |   |   |      <BLOB> LINEAR i32 %9
; CHECK:           |   |   |   |      <BLOB> NON-LINEAR i32 %57
; CHECK:           |   |   |   |      <BLOB> LINEAR i32 %47{def@3}
;                  |   |   |   |
; CHECK:           |   |   |   |   %90 = %34  +  %73;
; CHECK:           |   |   |   |   %34 = %90  +  %89;
; CHECK:           |   |   |   + END LOOP
...
35:                                               ; preds = %96, %32
  ...
  %39 = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 3, i64 1, i64 %14, ptr elementtype(double) nonnull %0, i64 %38)
  %40 = add nuw nsw i64 %36, 2
  %41 = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 3, i64 1, i64 %14, ptr elementtype(double) nonnull %0, i64 %40)
  br label %42
...
42:                                               ; preds = %93, %35
  ...
  %49 = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 2, i64 1, i64 %12, ptr elementtype(double) nonnull %39, i64 %48)
  %50 = zext i32 %47 to i64
  %51 = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 2, i64 1, i64 %12, ptr elementtype(double) nonnull %41, i64 %50)
  br label %52
...
52:                                               ; preds = %52, %42
  ...
  %59 = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 1, i64 1, i64 40, ptr elementtype(double) nonnull %49, i64 %58)
  %60 = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 0, i64 1, i64 8, ptr elementtype(double) nonnull %59, i64 1)
  %61 = load double, ptr %60, align 1
  %62 = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 0, i64 1, i64 8, ptr elementtype(double) nonnull %59, i64 2)
  %63 = load double, ptr %62, align 1
  %64 = fdiv reassoc ninf nsz arcp contract afn double %63, %61
  %65 = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 0, i64 1, i64 8, ptr elementtype(double) nonnull %59, i64 3)
  %66 = load double, ptr %65, align 1
  ...
  %75 = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 1, i64 1, i64 40, ptr elementtype(double) nonnull %51, i64 %74)
  %76 = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 0, i64 1, i64 8, ptr elementtype(double) nonnull %75, i64 1)
  %77 = load double, ptr %76, align 1
  %78 = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 0, i64 1, i64 8, ptr elementtype(double) nonnull %75, i64 2)
  %79 = load double, ptr %78, align 1
  %80 = fdiv reassoc ninf nsz arcp contract afn double %79, %77
  %81 = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 0, i64 1, i64 8, ptr elementtype(double) nonnull %75, i64 3)
...
103:                                              ; preds = %99
  ...
  %108 = call i32 (ptr, i32, i64, ptr, ptr, ...) @for_write_seq_lis(ptr nonnull %6, i32 -1, i64 1239157112576, ptr nonnull %7, ptr nonnull %8) #4
  br label %109
