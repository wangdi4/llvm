...
; CHECK: |         <LVAL-REG> {al:8}(LINEAR double* %A)[LINEAR zext.i32.i64(i1 + 1)]
; CHECK: |   }
; CHECK: |   else
; CHECK: |   {
; CHECK: |      (%A)[i1 + 1] = %init;
; CHECK: |         <LVAL-REG> {al:8}(LINEAR double* %A)[LINEAR i64 i1 + 1]
; CHECK: |   }
; CHECK: + END LOOP
...
define void @foo(double* %A, double %init, i1 %cond, i64 %n) {
