...
; transformation works since the reference of %a[i] isn't used inside the 
; inner conditionals, even if the conditional doesn't have an else branch.
...
for.inc:                                          ; preds = %if.then, %if.else10, %if.then5
  %.pn = phi i32 [ %add1, %if.then ], [ %add8, %if.else ], [ %add9, %if.then5 ]
