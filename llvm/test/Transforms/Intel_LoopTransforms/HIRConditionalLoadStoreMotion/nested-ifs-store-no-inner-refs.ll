...
; This test case checks that the transformation sunk the stores of
; %a[i] because the reference was found in all outer branches of the conditionals.
...
if.else6:                                         ; preds = %if.else
  %add7 = add nsw i32 %res.018, 1
  br label %for.inc

for.inc:                                          ; preds = %if.then, %if.else6, %if.then3
  %res.1 = phi i32 [ %res.018, %if.then ], [ %add5, %if.then3 ], [ %add7, %if.else6 ]
