...
; This test case checks that the transformation didn't hoisted the loads of
; %a[i] in the nested Ifs. The reason is that there is not a reference of %a[i]
; in an outer If, but there are references of %a[i] inside the inner If.
...
if.else10:                                        ; preds = %if.else
  %mul2 = mul nsw i32 %i.030, %m
  %cmp5 = icmp eq i32 %mul2, 7
  br i1 %cmp5, label %if.then15, label %if.else20

if.then15:                                         ; preds = %if.else
  %arrayidx8 = getelementptr inbounds i32, ptr %a, i64 %indvars.iv
  %5 = load i32, ptr %arrayidx8
  %add9 = add nsw i32 %5, -3
  br label %for.inc

if.else20:                                        ; preds = %if.else
  %arrayidx12 = getelementptr inbounds i32, ptr %a, i64 %indvars.iv
  %6 = load i32, ptr %arrayidx12
  %add13 = add nsw i32 %6, 1
  br label %for.inc

for.inc:                                          ; preds = %if.then, %if.else10, %if.then5
  %.pn = phi i32 [ %3, %if.then ], [ %add8, %if.then5 ], [ %add9, %if.then15], [ %add13, %if.else20 ]
