...
; the nested Ifs, but the load that is outside of the conditionals (%6) wasn't
; transformed.
...
for.inc:                                          ; preds = %if.then, %if.else10, %if.then5
  ...
  %6 = load i32, ptr %arrayidx13
  %temp.res = add nsw i32 %6, %res.029
  %res.1 = add nsw i32 %.pn, %temp.res
