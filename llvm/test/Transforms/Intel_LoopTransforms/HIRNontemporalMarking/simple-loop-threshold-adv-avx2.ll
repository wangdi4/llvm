; RUN: opt -opaque-pointers=0 -enable-intel-advanced-opts -intel-libirc-allowed -mtriple=i686-- -mattr=+avx2 -S -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-nontemporal-marking,print<hir>" -aa-pipeline="basic-aa" -hir-details < %s 2>&1 | FileCheck %s
