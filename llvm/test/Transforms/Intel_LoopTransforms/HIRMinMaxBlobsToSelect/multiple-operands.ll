...
; that represents a max. The reason is because the temp %0 is used in the
; addition 5 + %0, and this not a temp blob in smax. The test case was created
; from the following C++ test case, but added the write in the LLVM IR:
...
; CHECK:          + DO i1 = 0, zext.i32.i64(%n_res) + -1, 1   <DO_LOOP>  <MAX_TC_EST = 1000>  <LEGAL_MAX_TC = 2147483647>  <MVTag: 18>
; CHECK:          |   %0 = %limm;
; CHECK:          |   %1 = (@mapped)[0][i1];
; CHECK:          |   %limm = smax((5 + %0), %1);
