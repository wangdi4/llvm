...
; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-loop-rematerialize,hir-cg,simplifycfg,intel-ir-optreport-emitter" -aa-pipeline="basic-aa" -intel-opt-report=low -force-hir-cg  -hir-allow-loop-materialization-regions=true -disable-output < %s 2>&1 | FileCheck %s -check-prefix=OPTREPORT
