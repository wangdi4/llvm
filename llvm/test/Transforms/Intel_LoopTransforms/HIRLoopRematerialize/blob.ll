...
;
; RUN: opt -force-opaque-pointers -hir-ssa-deconstruction -hir-temp-cleanup -hir-loop-rematerialize -print-before=hir-loop-rematerialize -print-after=hir-loop-rematerialize -hir-loop-rematerialize-tc-lb=1 < %s 2>&1 | FileCheck %s
; RUN: opt -force-opaque-pointers -passes="hir-ssa-deconstruction,hir-temp-cleanup,print<hir>,hir-loop-rematerialize,print<hir>" -aa-pipeline="basic-aa" -hir-loop-rematerialize-tc-lb=1 < %s 2>&1 | FileCheck %s
;
; CHECK:Function: Vsub
...
define dso_local void @Vsub(double* noalias nocapture %a, double* noalias nocapture readonly %b, double %n) local_unnamed_addr #0 {
entry:
  %mul = fmul double %n, %n
  %0 = load double, double* %b, align 8, !tbaa !2
  %mul1 = fmul double %mul, %0
  store double %mul1, double* %a, align 8, !tbaa !2
  ...
  %arrayidx8 = getelementptr inbounds double, double* %b, i64 2
