...
; Test checks that min/max is not recognized in presence of the else branch.
...
;     |   |   else
;     |   |   {
;     |   |      (%BUF_3D)[-1][%BLOCK][%K][i1][i2] = 1.000000e+00;
;     |   |   }
;     |   + END LOOP
;     + END LOOP
;   END REGION
...
where_mask_else11:                                ; preds = %loop_body14
  store double 1.0, double* %"tf_$TAVG_BUF_3D[][][][][]12", align 1 
  br label %where_mask_endif12

where_mask_endif12:                               ; preds = %where_mask_then11, %where_mask_else11
...
loop_body14:                                      ; preds = %loop_body14.preheader, %where_mask_endif12
  ...
  br i1 %rel.7, label %where_mask_then11, label %where_mask_else11
