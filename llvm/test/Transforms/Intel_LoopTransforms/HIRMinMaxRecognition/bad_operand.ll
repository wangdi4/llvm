...
define void @tf_(ptr noalias nocapture readonly dereferenceable(8) %ARRAY, ptr noalias nocapture readonly dereferenceable(8) %"tf_$RRE", ptr noalias nocapture readonly dereferenceable(4) %"tf_$BLOCK", ptr noalias nocapture readonly dereferenceable(4) %"tf_$K", ptr noalias nocapture readonly dereferenceable(4) %"tf_$NX_BLOCK", ptr noalias nocapture readonly dereferenceable(4) %"tf_$NY_BLOCK", ptr noalias nocapture dereferenceable(8) %BUF_3D) local_unnamed_addr {
alloca_0:
...
where_mask_then11:                                ; preds = %loop_body14
  %"tf_$RRE[][]30" = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 0, i64 1, i64 8, ptr nonnull elementtype(double) %"tf_$RRE[]29", i64 %"$loop_ctr.062")
  store double %X, ptr %"tf_$RRE[][]30", align 1
...
loop_test13.preheader:                            ; preds = %loop_test13.preheader.preheader, %loop_exit15
  ...
  %"tf_$RRE[]29" = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 1, i64 1, i64 %mul.1, ptr nonnull elementtype(double) %"tf_$RRE", i64 %"$loop_ctr1.064")
