...
; Test checks that min/max is not recognized when operands of 'if' and 'store' do not match.
...
; CHECK-CHANGED-NOT: Dump After HIRMinMaxRecognition
...
define void @tf_(double* noalias nocapture readonly dereferenceable(8) %ARRAY, double* noalias nocapture readonly dereferenceable(8) %"tf_$RRE", i32* noalias nocapture readonly dereferenceable(4) %"tf_$BLOCK", i32* noalias nocapture readonly dereferenceable(4) %"tf_$K", i32* noalias nocapture readonly dereferenceable(4) %"tf_$NX_BLOCK", i32* noalias nocapture readonly dereferenceable(4) %"tf_$NY_BLOCK", double* noalias nocapture dereferenceable(8) %BUF_3D) local_unnamed_addr {
alloca_0:
...
where_mask_then11:                                ; preds = %loop_body14
  %"tf_$RRE[][]30" = tail call double* @llvm.intel.subscript.p0f64.i64.i64.p0f64.i64(i8 0, i64 1, i64 8, double* nonnull elementtype(double) %"tf_$RRE[]29", i64 %"$loop_ctr.062")
  store double %X, double* %"tf_$RRE[][]30", align 1
...
loop_test13.preheader:                            ; preds = %loop_test13.preheader.preheader, %loop_exit15
  ...
  %"tf_$RRE[]29" = tail call double* @llvm.intel.subscript.p0f64.i64.i64.p0f64.i64(i8 1, i64 1, i64 %mul.1, double* nonnull elementtype(double) %"tf_$RRE", i64 %"$loop_ctr1.064")
