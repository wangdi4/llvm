...
; Test checks that following code is recognized as minnum.f64() pattern:
...
;         |   |   if (%X < %Y)
;         |   |   {
;         |   |      %"(float)tf_$ARRAY[][]_fetch.30$" = fptrunc.double.float(%X);
;         |   |      (%BUF_3D)[-1][%BLOCK][%K][i1][i2] = %"(float)tf_$ARRAY[][]_fetch.30$";
;         |   |   }
;         |   + END LOOP
;         + END LOOP
;       END REGION
...
;             |   |   %X = (%ARRAY)[i1][i2];
;             |   |   %Y = fpext.float.double((%BUF_3D)[-1][%BLOCK][%K][i1][i2]);
; CHECK:      |   |   %call = @llvm.minnum.f64(%X,  %Y);
;             |   |   %"(float)tf_$ARRAY[][]_fetch.30$" = fptrunc.double.float(%call);
;             |   |   (%BUF_3D)[-1][%BLOCK][%K][i1][i2] = %"(float)tf_$ARRAY[][]_fetch.30$";
;             |   + END LOOP
;             + END LOOP
;            END REGION
...
define void @tf_(double* noalias nocapture readonly dereferenceable(8) %ARRAY, i32* noalias nocapture readonly dereferenceable(4) %"tf_$BLOCK", i32* noalias nocapture readonly dereferenceable(4) %"tf_$K", i32* noalias nocapture readonly dereferenceable(4) %"tf_$NX_BLOCK", i32* noalias nocapture readonly dereferenceable(4) %"tf_$NY_BLOCK", float* noalias nocapture dereferenceable(4) %BUF_3D) local_unnamed_addr {
alloca_0:
...
where_mask_then11:                                ; preds = %loop_body14
  %"(float)tf_$ARRAY[][]_fetch.30$" = fptrunc double %X to float
  store float %"(float)tf_$ARRAY[][]_fetch.30$", float* %"tf_$TAVG_BUF_3D[][][][][]12", align 1
...
loop_body14:                                      ; preds = %loop_body14.preheader, %where_mask_endif12
  ...
  %rel.7 = fcmp reassoc ninf nsz arcp contract afn olt double %X, %Y
...
loop_test13.preheader:                            ; preds = %loop_test13.preheader.preheader, %loop_exit15
  ...
  %"tf_$ARRAY[]6" = tail call double* @llvm.intel.subscript.p0f64.i64.i64.p0f64.i64(i8 1, i64 1, i64 %mul.9, double* nonnull elementtype(double) %ARRAY, i64 %"$loop_ctr1.064")
