...
; Verify that we do not sink invariant 'reduction like' instruction if the
; reduction temp escapes (is used elsewhere inside the loop).

; %add7 is used in unrelated store (@C)[0][i1] = %add7.
...
for.body:                                         ; preds = %for.body, %entry
  ...
  %arrayidx9 = getelementptr inbounds [100 x float], [100 x float]* @C, i64 0, i64 %indvars.iv
