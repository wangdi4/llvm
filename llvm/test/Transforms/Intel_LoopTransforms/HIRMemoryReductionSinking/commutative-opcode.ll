...
; This case handles when there is another possible dependency, but the opcode
; can still be handled as commutative (one instruction is an add and the
; other is a subtract).
...
define dso_local void @_Z3fooPiS_(ptr nocapture noalias noundef %a, ptr nocapture noundef readonly %b, float %m) {
entry:
