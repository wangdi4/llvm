...
; as specified by @llvm.experimental.noalias.scope.decl() calls in the incoming
; IR. The new scopes lists are added to the outer loop. They will be translated
; into calls during CG.
...
; CHECK: |   + DO i64 i2 = 0, 3, 1   <DO_LOOP> <unroll>
; CHECK: |   |   (%b)[i2] = (%a)[i2];
...
; CHECK: + NoAlias scope lists: [[NEW_SCOPE1:.*]], [[NEW_SCOPE2:.*]], [[NEW_SCOPE3:.*]], [[NEW_SCOPE4:.*]], [[NEW_SCOPE5:.*]], [[NEW_SCOPE6:.*]], [[NEW_SCOPE7:.*]], [[NEW_SCOPE8:.*]]
; CHECK: + DO i64 i1 = 0, 39, 1   <DO_LOOP>
; CHECK: |   (%b)[0] = (%a)[0];
...
; CHECK: |
; CHECK: |   (%b)[2] = (%a)[2];
; CHECK: |   <LVAL-REG> {al:4}(LINEAR ptr %b)[i64 2] inbounds  !alias.scope [[NEW_SCOPE5]] !noalias [[NEW_SCOPE6]]
; CHECK: |   <RVAL-REG> {al:4}(LINEAR ptr %a)[i64 2] inbounds  !alias.scope [[NEW_SCOPE6]] !noalias [[NEW_SCOPE5]]
; CHECK: |
; CHECK: |   (%b)[3] = (%a)[3];
; CHECK: |   <LVAL-REG> {al:4}(LINEAR ptr %b)[i64 3] inbounds  !alias.scope [[NEW_SCOPE7]] !noalias [[NEW_SCOPE8]]
; CHECK: |   <RVAL-REG> {al:4}(LINEAR ptr %a)[i64 3] inbounds  !alias.scope [[NEW_SCOPE8]] !noalias [[NEW_SCOPE7]]
; CHECK: + END LOOP
