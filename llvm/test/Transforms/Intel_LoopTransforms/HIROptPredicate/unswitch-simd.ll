...
;       |   |   @llvm.lifetime.start.p0(4,  &((i8*)(%j.linear.iv)[0]));
;       |   |   if (%n != 20)
;       |   |   {
;       |   |      (%a)[i2] = i2;
;       |   |   }
;       |   |   @llvm.lifetime.end.p0(4,  &((i8*)(%j.linear.iv)[0]));
;       |   + END LOOP
...
; CHECK:          |   |   @llvm.lifetime.start.p0(4,  &((%j.linear.iv)[0]));
; CHECK:          |   |   (%a)[i2] = i2;
; CHECK:          |   |   @llvm.lifetime.end.p0(4,  &((%j.linear.iv)[0]));
; CHECK:          |   + END LOOP
...
define dso_local void @foo(ptr nocapture %a, i32 %n, i64 %m) local_unnamed_addr #0 {
entry:
...
omp.inner.for.body:                               ; preds = %omp.inner.for.inc, %omp.inner.for.body.lr.ph
  ...
  call void @llvm.lifetime.start.p0(i64 4, ptr nonnull %j.linear.iv)
...
omp.inner.for.inc:                                ; preds = %omp.inner.for.body, %if.then
  ...
  call void @llvm.lifetime.end.p0(i64 4, ptr nonnull %j.linear.iv)
