; RUN: opt -hir-ssa-deconstruction -disable-output -hir-opt-predicate -print-after=hir-opt-predicate -hir-cg < %s 2>&1 | FileCheck %s
; RUN: opt -passes="hir-ssa-deconstruction,hir-opt-predicate,print<hir>,hir-cg" -aa-pipeline="basic-aa" -disable-output < %s 2>&1 | FileCheck %s

; HIR:
; BEGIN REGION { }
;      + DO i1 = 0, %argc + -2, 1   <DO_MULTI_EXIT_LOOP>  <MAX_TC_EST = 2147483646>
;       |   if (%call == 0)
;       |   {
