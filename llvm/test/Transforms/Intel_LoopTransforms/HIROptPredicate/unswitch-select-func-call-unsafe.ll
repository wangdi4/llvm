...
; This test case checks that the select instruction "%add.5" is not converted
; as an if/else because the left hand side is passed as a parameter to a
; function that is not readonly (unsafe function).

; Original test is in Fortran, the MERGE function will produce a select
; instruction. The call to foo was added in the IR to make a function call.
...
; HIR shouldn't be transformed:

; CHECK: Function: loop_merge_compute_
; CHECK: BEGIN REGION { }
...
; CHECK:       |   |   |   %add.5 = (%"loop_merge_compute_$BLKIDX[][]_fetch.19" > 0) ? %"loop_merge_compute_$ARRAY[][][]_fetch.38" : 0.000000e+00;
; CHECK:       |   |   |   @foo(%add.5);
