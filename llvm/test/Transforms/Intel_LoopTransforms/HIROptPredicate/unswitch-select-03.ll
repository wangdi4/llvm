...
; This change set checks that the Select instruction %slct.8 was unswitched,
; but the Select instruction %slct.9 wasn't because it doesn't have the same
; condition. It was created from the following Fortran code, but one of the
; Selects was modified:
...
;        |      |      |   %slct.9 = (%"BLKIDX[][]_fetch.118" > 1) ? %neg.1 : 0.000000e+00;
...
;                    |   |      |   %neg.1 =  - %"ARRAY[][][]_fetch.137";
; CHECK:             |   |      |   %slct.9 = (%"BLKIDX[][]_fetch.118" > 1) ? %neg.1 : 0.000000e+00;
;                    |   |      |   %add.26 = %slct.8  +  %slct.9;
;                    |   |      |   %add.27 = %slct.8  +  %add.26;
;                    |   |      |   (%ARRAY)[%slct.7][i2 + sext.i32.i64(%I_STARTLEV_fetch.106) + -1][i3 + sext.i32.i64(%I_STARTIDX_fetch.109) + -1] = %add.27;
;                    |   |      + END LOOP
...
; Check the output from the debug information
...
; CHECK-DBG-NOT:     if (%"BLKIDX[][]_fetch.118" > 1)
; CHECK-DBG-NOT:        %slct.9 = %neg.1;
; CHECK-DBG-NOT:        %slct.9 = 0.000000e+00;
...
bb30.preheader:                                   ; preds = %bb26
  ...
  %rel.29 = icmp sgt i32 %"BLKIDX[][]_fetch.118", 1
...
bb30:                                             ; preds = %bb30.preheader, %bb30
  ...
  %slct.9 = select i1 %rel.29, double %neg.1, double 0.000000e+00
