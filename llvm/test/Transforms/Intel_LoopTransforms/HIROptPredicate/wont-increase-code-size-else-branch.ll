...
; increase code size because the else branch won't have a side effect (no temp
; live out, no side effect instructions, no stores). It was created from the
; following test in C++:
...
for.inc:                                          ; preds = %if.then, %if.else
  %temp1.1 = phi i32 [ %temp1.07, %if.then ], [ %inc, %if.else ]
