...
;
;RUN: opt -force-opaque-pointers -hir-ssa-deconstruction -hir-temp-cleanup -hir-mv-variable-stride -print-after=hir-mv-variable-stride  -print-before=hir-mv-variable-stride < %s 2>&1 | FileCheck %s
; RUN: opt -force-opaque-pointers -passes="hir-ssa-deconstruction,hir-temp-cleanup,print<hir>,hir-mv-variable-stride,print<hir>" -aa-pipeline="basic-aa" < %s 2>&1 | FileCheck %s
; 
