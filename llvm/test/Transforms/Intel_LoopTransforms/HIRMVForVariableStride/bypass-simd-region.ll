; RUN: opt -hir-ssa-deconstruction -hir-temp-cleanup -hir-mv-variable-stride -print-before=hir-mv-variable-stride -print-after=hir-mv-variable-stride -hir-print-only=0 < %s 2>&1 | FileCheck %s
; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,print<hir>,hir-mv-variable-stride,print<hir>" -aa-pipeline="basic-aa" -hir-print-only=0 < %s 2>&1 | FileCheck %s

; Make sure MV doesn't work within simd-entry/exit
; Blind MV results in broken simd region struction. simd-entry/exit should share the same immediate parent with the loop directed to be vectorixed by the simd-entry/exit.
...
;
; CHECK:              + DO i1 = 0, sext.i32.i64(%"step2d_tile_$M2_fetch") + -1 * sext.i32.i64(%"step2d_tile_$M1_fetch"), 1   <DO_LOOP> <simd>
; CHECK:              |   + DO i2 = 0, sext.i32.i64(%"step2d_tile_$M2_fetch") + -1 * sext.i32.i64(%"step2d_tile_$M1_fetch"), 1   <DO_LOOP>
; CHECK:              |   |   %func_result70_fetch = (%"step2d_tile_$ZETA_$field0$_fetch")[%"step2d_tile_$M1_fetch"][i1 + sext.i32.i64(%"step2d_tile_$M1_fetch")][i2 + sext.i32.i64(%"step2d_tile_$M1_fetch")];
; CHECK:              |   |   %func_result112_fetch = (%"step2d_tile_$H_$field0$_fetch")[i1 + sext.i32.i64(%"step2d_tile_$M1_fetch")][i2 + sext.i32.i64(%"step2d_tile_$M1_fetch")];
; CHECK:              |   |   %add114 = %func_result70_fetch  +  %func_result112_fetch;
; CHECK:              |   |   (%"step2d_tile_$DRHS")[i1 + sext.i32.i64(%"step2d_tile_$M1_fetch")][i2 + sext.i32.i64(%"step2d_tile_$M1_fetch")] = %add114;
; CHECK:              |   + END LOOP
; CHECK:              + END LOOP
;
