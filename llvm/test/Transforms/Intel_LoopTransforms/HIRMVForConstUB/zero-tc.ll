...
;         |   if (%tmp556 != 0)
;         |   {
;         |      @llvm.memcpy.p0i8.p0i8.i64(&((i8*)(%tmp8)[0][%tmp113][%tmp471][%tmp553.out]),  &((i8*)(%tmp10)[0][i1 + zext.i32.i64(%tmp528)]),  104,  0);
;         |   }
...
%struct.snork = type { i32, i32, i32, i32, i32, i32, i32, i32, i32, float, %struct.snork.0, %struct.eggs, %struct.wobble, %struct.quux*, i32*, float*, float*, i32*, %struct.blam*, double }
%struct.snork.0 = type { i32*, i32*, i32*, i32** }
%struct.eggs = type { i32*, i32*, i32*, i32* }
...
define dso_local void @wobble.bb469(i16 %tmp460, i16 %tmp461, <16 x i32> %tmp462, <16 x i32> %tmp111, <16 x float> %tmp366, [2 x [2 x i32]]* %tmp9, i64 %tmp113, [2 x [2 x %struct.spam.1]]* %tmp5, [2 x [2 x %struct.spam.1]]* %tmp6, [2 x [2 x %struct.quux.2]]* %tmp7, i16 %tmp374, [16 x %struct.zot]* %tmp10, [2 x [2 x [16 x %struct.zot]]]* %tmp8, %struct.snork* %arg, i32 %tmp96) #2 {
newFuncRoot:
  br label %bb469
...
bb469:                                            ; preds = %newFuncRoot, %bb571
  ...
  %tmp477 = getelementptr inbounds [2 x [2 x i32]], [2 x [2 x i32]]* %tmp9, i64 0, i64 %tmp113, i64 %tmp471
  %tmp478 = load i32, i32* %tmp477, align 4
  %tmp479 = shl i32 65535, %tmp478
  %tmp480 = trunc i32 %tmp479 to i16
  %tmp481 = getelementptr inbounds [2 x [2 x %struct.spam.1]], [2 x [2 x %struct.spam.1]]* %tmp5, i64 0, i64 %tmp113, i64 %tmp471, i32 0
  %tmp482 = bitcast <8 x i64>* %tmp481 to <16 x i32>*
  %tmp483 = load <16 x i32>, <16 x i32>* %tmp482, align 64
  %tmp484 = bitcast i16 %tmp480 to <16 x i1>
  %tmp486 = bitcast <16 x i32> %tmp483 to <8 x i64>
  store <16 x i32> %tmp483, <16 x i32>* %tmp482, align 64
  %tmp487 = getelementptr inbounds [2 x [2 x %struct.spam.1]], [2 x [2 x %struct.spam.1]]* %tmp6, i64 0, i64 %tmp113, i64 %tmp471, i32 0
  %tmp488 = bitcast <8 x i64>* %tmp487 to <16 x i32>*
  %tmp489 = load <16 x i32>, <16 x i32>* %tmp488, align 64
  %tmp491 = bitcast <16 x i32> %tmp489 to <8 x i64>
  store <16 x i32> %tmp489, <16 x i32>* %tmp488, align 64
  %tmp492 = getelementptr inbounds [2 x [2 x %struct.quux.2]], [2 x [2 x %struct.quux.2]]* %tmp7, i64 0, i64 %tmp113, i64 %tmp471, i32 0
  %tmp493 = load <16 x float>, <16 x float>* %tmp492, align 64
  store <16 x float> %tmp493, <16 x float>* %tmp492, align 64
...
bb526:                                            ; preds = %bb525, %bb523, %bb497
  ...
  store i32 %tmp530, i32* %tmp477, align 4
  %tmp531 = icmp sgt i32 %tmp530, 15
  br i1 %tmp531, label %bb532, label %bb571

bb532:                                            ; preds = %bb526
  %tmp533 = getelementptr inbounds [2 x [2 x [16 x %struct.zot]]], [2 x [2 x [16 x %struct.zot]]]* %tmp8, i64 0, i64 %tmp113, i64 %tmp471, i64 0
  %tmp534 = trunc i64 %tmp471 to i32
  %tmp535 = add nsw i32 %tmp530, -16
  store i32 %tmp535, i32* %tmp477, align 4
...
bb558:                                            ; preds = %bb551
  %tmp559 = getelementptr inbounds [16 x %struct.zot], [16 x %struct.zot]* %tmp10, i64 0, i64 %tmp552
  %tmp560 = zext i32 %tmp553 to i64
  %tmp561 = getelementptr inbounds [2 x [2 x [16 x %struct.zot]]], [2 x [2 x [16 x %struct.zot]]]* %tmp8, i64 0, i64 %tmp113, i64 %tmp471, i64 %tmp560
  %tmp562 = bitcast %struct.zot* %tmp561 to i8*
  %tmp563 = bitcast %struct.zot* %tmp559 to i8*
  call void @llvm.memcpy.p0i8.p0i8.i64(i8* noundef nonnull align 8 dereferenceable(104) %tmp562, i8* noundef nonnull align 8 dereferenceable(104) %tmp563, i64 104, i1 false)
  br label %bb564
