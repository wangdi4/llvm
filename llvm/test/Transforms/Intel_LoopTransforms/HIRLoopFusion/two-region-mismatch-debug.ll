...
;RUN: opt -passes="loop-simplify,hir-ssa-deconstruction,hir-loop-fusion,hir-cg,loop-simplifycfg,intel-ir-optreport-emitter" -aa-pipeline="basic-aa" -intel-loop-optreport=low 2>&1 < %s -S | FileCheck %s  -check-prefix=OPTREPORT

; OPTREPORT: LOOP BEGIN at t1.c (6, 3)
; OPTREPORT:     LOOP BEGIN at t1.c (7, 5)
; OPTREPORT:         Remark: Loops have been fused with (11,15)
; OPTREPORT:     LOOP END
; OPTREPORT:     LOOP BEGIN at t1.c (7, 5)
; OPTREPORT:         Remark: Peeled loop after fusion
; OPTREPORT:     LOOP END
