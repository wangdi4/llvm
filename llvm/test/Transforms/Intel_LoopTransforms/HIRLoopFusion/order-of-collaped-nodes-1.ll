; RUN: opt -disable-hir-create-fusion-regions=0 -hir-ssa-deconstruction -hir-temp-cleanup -disable-output -hir-loop-fusion -print-after=hir-loop-fusion < %s 2>&1 | FileCheck %s
...
define void @foo_(float* noalias nocapture dereferenceable(4) %"foo_$T1", float* noalias nocapture dereferenceable(4) %"foo_$T2", float* noalias nocapture dereferenceable(4) %"foo_$T3", float* noalias nocapture readonly dereferenceable(4) %"foo_$INA", float* noalias nocapture readonly dereferenceable(4) %"foo_$INA2", float* noalias nocapture readonly dereferenceable(4) %"foo_$INA3", float* noalias nocapture readonly dereferenceable(4) %"foo_$INA4", float* noalias nocapture dereferenceable(4) %"foo_$C") local_unnamed_addr #0 {
alloca_0:
  br label %bb14
