; RUN: opt -hir-create-function-level-region -aa-pipeline="basic-aa" -passes="hir-ssa-deconstruction,hir-dead-store-elimination,print<hir-dd-analysis>" -hir-dd-analysis-verify=Region -hir-dead-store-elimination-deduce-region-local-alloca=true 2>&1 < %s -disable-output | FileCheck %s
