...
; RUN: opt -hir-ssa-deconstruction -hir-post-vec-complete-unroll -hir-vec-dir-insert -hir-vplan-vec -hir-cg -intel-opt-report=low -simplifycfg -intel-ir-optreport-emitter -vplan-force-vf=4 2>&1 < %s -S | FileCheck %s -check-prefix=OPTREPORT --strict-whitespace
...
; RUN: opt -hir-ssa-deconstruction -hir-post-vec-complete-unroll -hir-vec-dir-insert -hir-vplan-vec -hir-cg -vplan-force-vf=4 -intel-opt-report=low < %s -S | FileCheck %s
