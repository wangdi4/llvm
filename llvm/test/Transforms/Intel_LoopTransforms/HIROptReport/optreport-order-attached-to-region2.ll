...
; RUN: opt -hir-ssa-deconstruction -hir-post-vec-complete-unroll -hir-vec-dir-insert -VPODriverHIR -hir-cg -intel-loop-optreport=low -simplifycfg -intel-ir-optreport-emitter 2>&1 < %s -S | FileCheck %s -check-prefix=OPTREPORT --strich-whitespace
...
; CHECK: [[M10]] = distinct !{!"intel.loop.optreport", [[M11:!]]}
...
; CHECK: [[M14]] = distinct !{!"llvm.loop.optreport", [[M15:!.*]]}
; CHECK: [[M15]] = distinct !{!"intel.loop.optreport", [[M16:!.*]], [[M19:!.*]]}
; CHECK: [[M16]] = !{!"intel.optreport.next_sibling", [[M17:!.*]]}
; CHECK: [[M17]] = distinct !{!"llvm.loop.optreport", [[M18:!.*]]}
; CHECK: [[M18]] = distinct !{!"intel.loop.optreport", [[M6]]}
