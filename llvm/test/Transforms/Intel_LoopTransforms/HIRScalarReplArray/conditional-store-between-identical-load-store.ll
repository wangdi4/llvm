...
; Verify that we can correctly handle the loop-independant group with conditional store
; by still recognizing the max index load and min index store of the group.
; Max index load is the load with the minimum top sort number and the maximum index
; within the group.
; Min index store is the store with the maximum top sort number and the minimum index
; within the group.
