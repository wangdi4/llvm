...
; Verify that we are able to scalar-replace group {(%A)[i1][i2],
; (%A)[i1][i2 + 1], (%A)[i1][i2 + 2]} in the presence of load
; (@A)[0][%t][i2 + 1] which doesn't have constant distance w.r.t other refs but
; overlaps with load (%A)[i1][i2 + 1] of the group at index (i2 + 1).
...
; CHECK: |      %scalarepl2 = (@A)[0][i1][1];
; CHECK: |   + DO i2 = 0, 97, 1   <DO_LOOP>
; CHECK: |   |   %0 = %scalarepl;
; CHECK: |   |   %1 = %scalarepl2;
; CHECK: |   |   %ld = (@A)[0][%t][i2 + 1];
; CHECK: |   |   %scalarepl3 = %0 + %1 + %ld;
; CHECK: |   |   (@A)[0][i1][i2 + 2] = %scalarepl3;
...
for.body3:                                        ; preds = %for.cond1.preheader, %for.body3
  ...
  %arrayidx9 = getelementptr inbounds [100 x [100 x i32]], ptr @A, i64 0, i64 %indvars.iv31, i64 %indvars.iv.next
  ...
  %ld = load i32, ptr %arrayidx15, align 4
  %add2 = add nsw i32 %ld, %add1
  %arrayidx3 = getelementptr inbounds [100 x [100 x i32]], ptr @A, i64 0, i64 %indvars.iv31, i64 %2
