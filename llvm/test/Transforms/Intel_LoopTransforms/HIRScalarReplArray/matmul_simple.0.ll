; RUN: opt -hir-ssa-deconstruction -hir-temp-cleanup -hir-sinking-for-perfect-loopnest -hir-loop-blocking -hir-lmm -hir-unroll-and-jam -hir-loop-blocking-blocksize=128 -hir-vec-dir-insert -hir-vplan-vec -hir-scalarrepl-array  -print-after=hir-scalarrepl-array -disable-output -S < %s 2>&1 | FileCheck %s
; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-sinking-for-perfect-loopnest,hir-loop-blocking,hir-lmm,hir-unroll-and-jam,hir-vec-dir-insert,hir-vplan-vec,hir-scalarrepl-array,print<hir>" -aa-pipeline="basic-aa" -hir-loop-blocking-blocksize=128 -disable-output < %s 2>&1 | FileCheck %s
;
