...
; This test case checks that the invariant bitcast instruction is not hoisted
; out of the loop after applying LMM. The reason is that multiple loads can
; use different size and can produce issues. The load to src[0] can still
; be hoisted.
...
; CHECK:       |   %conv = uitofp.i8.float(%limm);
; CHECK:       |   (%0)[i1] = %conv;
; CHECK:       |   (%1)[i1] = %limm;
; CHECK:       + END LOOP
; CHECK: END REGION
...
@data.a_ = dso_local local_unnamed_addr global ptr null
