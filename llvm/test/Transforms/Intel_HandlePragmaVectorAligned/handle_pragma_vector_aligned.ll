; RUN: opt -opaque-pointers=0 -S -passes=handle-pragma-vector-aligned < %s | FileCheck %s

target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

define void @foo(float* %P1, float* %P2, i64 %N) {
entry:
...
for.body:
  ...
  %ptridx = getelementptr inbounds float, float* %P1, i64 %I.08
  %0 = load float, float* %ptridx, align 4
  %conv = sitofp i64 %I.08 to float
  %add = fadd fast float %0, %conv
  %ptridx1 = getelementptr inbounds float, float* %P2, i64 %I.08
  store float %add, float* %ptridx1, align 4
