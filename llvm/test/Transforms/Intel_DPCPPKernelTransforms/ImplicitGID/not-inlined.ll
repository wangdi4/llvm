...
define dso_local void @foo(i32 addrspace(1)* noalias noundef %dst) #0 !dbg !6 {
entry:
; CHECK-LABEL: @foo
...
; CHECK-NEXT: store volatile i64 %GlobalID_2, i64* %__ocl_dbg_gid2, align 8
  ...
  call void @llvm.dbg.declare(metadata i32 addrspace(1)** %dst.addr, metadata !12, metadata !DIExpression()), !dbg !13
  %0 = load i32 addrspace(1)*, i32 addrspace(1)** %dst.addr, align 8, !dbg !14
  %call = call i64 @_Z13get_global_idj(i32 noundef 0) #4, !dbg !15
  %arrayidx = getelementptr inbounds i32, i32 addrspace(1)* %0, i64 %call, !dbg !14
  store i32 0, i32 addrspace(1)* %arrayidx, align 4, !dbg !16
  ret void, !dbg !17
}
...
; Function Attrs: convergent noinline norecurse nounwind optnone
define dso_local void @test(i32 addrspace(1)* noalias noundef align 4 %dst) #3 !dbg !18 !kernel_arg_addr_space !20 !kernel_arg_access_qual !21 !kernel_arg_type !22 !kernel_arg_base_type !22 !kernel_arg_type_qual !23 !kernel_arg_name !24 !kernel_arg_host_accessible !25 !kernel_arg_pipe_depth !26 !kernel_arg_pipe_io !23 !kernel_arg_buffer_location !23 !no_barrier_path !27 !kernel_has_sub_groups !25 !kernel_execution_length !28 !kernel_has_global_sync !25 !recommended_vector_length !20 {
entry:
; CHECK-LABEL: @test
; CHECK: %__ocl_dbg_gid0 = alloca i64, align 8
...
; CHECK-NEXT: store volatile i64 %GlobalID_2, i64* %__ocl_dbg_gid2, align 8
  ...
  %dst.addr = alloca i32 addrspace(1)*, align 8
  store i32 addrspace(1)* %dst, i32 addrspace(1)** %dst.addr, align 8
  call void @llvm.dbg.declare(metadata i32 addrspace(1)** %dst.addr, metadata !29, metadata !DIExpression()), !dbg !30
  %0 = load i32 addrspace(1)*, i32 addrspace(1)** %dst.addr, align 8, !dbg !31
  call void @foo(i32 addrspace(1)* noundef %0) #5, !dbg !32
  ret void, !dbg !33
}
