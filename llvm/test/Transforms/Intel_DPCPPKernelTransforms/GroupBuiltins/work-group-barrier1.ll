; RUN: opt -dpcpp-kernel-split-on-barrier -dpcpp-kernel-barrier -S < %s -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: opt -passes=dpcpp-kernel-split-on-barrier,dpcpp-kernel-barrier -S < %s -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: opt -dpcpp-kernel-split-on-barrier -dpcpp-kernel-barrier -S < %s | FileCheck %s
; RUN: opt -passes=dpcpp-kernel-split-on-barrier,dpcpp-kernel-barrier -S < %s | FileCheck %s
