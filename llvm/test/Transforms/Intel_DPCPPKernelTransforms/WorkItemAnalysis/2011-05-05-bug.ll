...
; CHECK-NEXT: PTR   %3 = getelementptr inbounds float, float addrspace(1)* %input, i64 %2
; CHECK-NEXT: RND   %4 = load float, float addrspace(1)* %3, align 4
; CHECK-NEXT: RND   %5 = tail call float @_Z5acoshf(float %4) #0
; CHECK-NEXT: PTR   %6 = getelementptr inbounds float, float addrspace(1)* %output, i64 %2
; CHECK-NEXT: RND   store float %5, float addrspace(1)* %6, align 4
...
; CHECK-NEXT: RND   %3 = getelementptr inbounds float, float addrspace(1)* %input, i64 %2
; CHECK-NEXT: RND   %4 = load float, float addrspace(1)* %3, align 4
; CHECK-NEXT: RND   %5 = tail call float @_Z5acoshf(float %4) #0
; CHECK-NEXT: RND   %6 = getelementptr inbounds float, float addrspace(1)* %output, i64 %2
; CHECK-NEXT: RND   store float %5, float addrspace(1)* %6, align 4
; CHECK-NEXT: UNI   ret void
