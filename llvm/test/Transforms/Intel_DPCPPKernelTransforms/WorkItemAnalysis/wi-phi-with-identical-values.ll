...
; CHECK-NEXT: RND   %call1 = tail call i32 (i8 addrspace(2)*, ...) @printf(i8 addrspace(2)* getelementptr inbounds ([13 x i8], [13 x i8] addrspace(2)* @.str, i32 0, i32 0))
...
; CHECK-NEXT: UNI   %arrayidx = getelementptr inbounds float, float addrspace(1)* %a, i32 %i.01
; CHECK-NEXT: UNI   %load = load float, float addrspace(1)* %arrayidx, align 4
; CHECK-NEXT: UNI   %inc = fadd float %load, 1.000000e+00
; CHECK-NEXT: UNI   store float %inc, float addrspace(1)* %arrayidx, align 4
