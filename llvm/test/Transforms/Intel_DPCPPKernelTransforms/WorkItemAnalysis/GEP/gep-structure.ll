...
declare void @create_linked_lists(%struct.Node addrspace(1)*, i32 addrspace(1)*, i32) #0
...
; CHECK-NEXT: PTR   %arrayidx = getelementptr inbounds %struct.Node, %struct.Node addrspace(1)* %pNodes, i64 %call
; CHECK-NEXT: SEQ   %conv = trunc i64 %call to i32
; CHECK-NEXT: STR   %global_id = getelementptr inbounds %struct.Node, %struct.Node addrspace(1)* %arrayidx, i64 0, i32 0
; CHECK-NEXT: RND   store i32 %conv, i32 addrspace(1)* %global_id, align 4
; CHECK-NEXT: RND   %position_in_list = getelementptr inbounds %struct.Node, %struct.Node addrspace(1)* %pNodes, i64 %call, i32 1
; CHECK-NEXT: RND   store i32 0, i32 addrspace(1)* %position_in_list, align 4
...
; CHECK-NEXT: RND   %pNode.02 = phi %struct.Node addrspace(1)* [ %arrayidx3, %for.body ], [ %arrayidx, %for.body.preheader ]
; CHECK-NEXT: RND   %call2 = tail call i32 @_Z10atomic_incPU3AS1Vi(i32 addrspace(1)* %allocation_index) #0
; CHECK-NEXT: RND   %idxprom = sext i32 %call2 to i64
; CHECK-NEXT: RND   %arrayidx3 = getelementptr inbounds %struct.Node, %struct.Node addrspace(1)* %pNodes, i64 %idxprom
; CHECK-NEXT: RND   %global_id5 = getelementptr inbounds %struct.Node, %struct.Node addrspace(1)* %arrayidx3, i64 0, i32 0
; CHECK-NEXT: RND   store i32 %conv, i32 addrspace(1)* %global_id5, align 4
; CHECK-NEXT: RND   %position_in_list6 = getelementptr inbounds %struct.Node, %struct.Node addrspace(1)* %pNodes, i64 %idxprom, i32 1
; CHECK-NEXT: RND   store i32 %j.03, i32 addrspace(1)* %position_in_list6, align 4
; CHECK-NEXT: RND   %pNext = getelementptr inbounds %struct.Node, %struct.Node addrspace(1)* %pNode.02, i64 0, i32 2
; CHECK-NEXT: RND   store %struct.Node addrspace(1)* %arrayidx3, %struct.Node addrspace(1)* addrspace(1)* %pNext, align 8
...
for.body:                                         ; preds = %for.body, %for.body.preheader
  ...
  %call2 = tail call i32 @_Z10atomic_incPU3AS1Vi(i32 addrspace(1)* %allocation_index) #0
