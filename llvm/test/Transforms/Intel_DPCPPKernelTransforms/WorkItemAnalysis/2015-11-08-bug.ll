...
; CHECK-NEXT: PTR   %arrayidx = getelementptr i32, i32 addrspace(1)* %early_exit, i64 %add
; CHECK-NEXT: RND   %0 = load i32, i32 addrspace(1)* %arrayidx, align 4
; CHECK-NEXT: RND   %cmp = icmp eq i32 %0, 2147483647
; CHECK-NEXT: RND   br i1 %cmp, label %if.else, label %if.then
; CHECK-NEXT: PTR   %arrayidx4 = getelementptr float, float addrspace(1)* %in_select_arg1, i64 %add
; CHECK-NEXT: RND   %1 = load float, float addrspace(1)* %arrayidx4, align 4
; CHECK-NEXT: PTR   %arrayidx5 = getelementptr float, float addrspace(1)* %in_select_arg2, i64 %add
; CHECK-NEXT: RND   %2 = load float, float addrspace(1)* %arrayidx5, align 4
; CHECK-NEXT: RND   %call7 = tail call float @_Z6selectffi(float %1, float %2, i32 1)
; CHECK-NEXT: RND   %mul8 = fmul float %call7, %call7
; CHECK-NEXT: PTR   %arrayidx9 = getelementptr float, float addrspace(1)* %out_dets, i64 %add
; CHECK-NEXT: RND   store float %mul8, float addrspace(1)* %arrayidx9, align 4
; CHECK-NEXT: UNI   br label %if.end
; CHECK-NEXT: PTR   %arrayidx10 = getelementptr float, float addrspace(1)* %out_dets, i64 %add
; CHECK-NEXT: RND   store float 0.000000e+00, float addrspace(1)* %arrayidx10, align 4
; CHECK-NEXT: UNI   br label %if.end
; CHECK-NEXT: UNI   ret void

define void @generateHitDataPrim(i32 addrspace(1)* noalias nocapture %early_exit, float addrspace(1)* nocapture %in_select_arg1, float addrspace(1)* nocapture %in_select_arg2, float addrspace(1)* nocapture %out_dets) {
entry:
