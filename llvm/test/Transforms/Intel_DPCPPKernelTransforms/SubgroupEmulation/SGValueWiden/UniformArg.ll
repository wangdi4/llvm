; RUN: opt -dpcpp-kernel-sg-emu-value-widen -S %s -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: opt -dpcpp-kernel-sg-emu-value-widen -S %s | FileCheck %s
...
; CHECK-NEXT: %[[#OP1:]] = getelementptr <16 x i32>, <16 x i32>* %w.x.addr, i32 0, i32 %sg.lid.1
; CHECK-NEXT: store i32 %x, i32* %[[#OP1]], align 4
; CHECK-NEXT: %sg.lid. = call i32 @_Z22get_sub_group_local_idv()
; CHECK-NEXT: %[[#OP2:]] = getelementptr <16 x i32>, <16 x i32>* %w.x.addr, i32 0, i32 %sg.lid.
; CHECK-NEXT: %[[#OP3:]] = load i32, i32* %[[#OP2]], align 4
; CHECK-NEXT: %sg.lid.3 = call i32 @_Z22get_sub_group_local_idv()
; CHECK-NEXT: %[[#OP4:]] = getelementptr <16 x i32>, <16 x i32>* %w., i32 0, i32 %sg.lid.3
; CHECK-NEXT: store i32 %[[#OP3]], i32* %[[#OP4]], align 4
...
; CHECK: %[[#OP5:]] = load <16 x i32>, <16 x i32>* %w., align 64
