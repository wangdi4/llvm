; RUN: opt -dpcpp-kernel-sg-emu-barrier-simplify -S %s -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: opt -dpcpp-kernel-sg-emu-barrier-simplify -S %s | FileCheck %s
; RUN: opt -passes=dpcpp-kernel-sg-emu-barrier-simplify -S %s -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: opt -passes=dpcpp-kernel-sg-emu-barrier-simplify -S %s | FileCheck %s
