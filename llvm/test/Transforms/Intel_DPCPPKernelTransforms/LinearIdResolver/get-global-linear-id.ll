; RUN: opt -dpcpp-kernel-linear-id-resolver %s -S -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: opt -passes=dpcpp-kernel-linear-id-resolver %s -S -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: opt -dpcpp-kernel-linear-id-resolver %s -S | FileCheck %s
; RUN: opt -passes=dpcpp-kernel-linear-id-resolver %s -S | FileCheck %s
