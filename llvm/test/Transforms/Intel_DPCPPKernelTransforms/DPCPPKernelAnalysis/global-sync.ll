...
; RUN: opt -dpcpp-kernel-builtin-lib=%s.rtl -enable-new-pm=0 -dpcpp-kernel-analysis %s -S -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: opt -dpcpp-kernel-builtin-lib=%s.rtl -passes=dpcpp-kernel-analysis %s -S | FileCheck %s
; RUN: opt -dpcpp-kernel-builtin-lib=%s.rtl -enable-new-pm=0 -dpcpp-kernel-analysis %s -S | FileCheck %s
