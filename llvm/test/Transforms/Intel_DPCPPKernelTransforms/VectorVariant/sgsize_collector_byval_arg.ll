; RUN: opt %s -dpcpp-enable-direct-function-call-vectorization -dpcpp-enable-direct-subgroup-function-call-vectorization -dpcpp-enable-byval-byref-function-call-vectorization -dpcpp-kernel-sg-size-collector -S | FileCheck %s
; RUN: opt %s -dpcpp-kernel-sg-size-collector -S | FileCheck %s -check-prefix CHECK-NO-FLAG
...
; RUN: opt %s -enable-debugify -disable-output -dpcpp-enable-direct-function-call-vectorization -dpcpp-enable-direct-subgroup-function-call-vectorization -dpcpp-enable-byval-byref-function-call-vectorization -dpcpp-kernel-sg-size-collector -S 2>&1 | FileCheck %s -check-prefix=DEBUGIFY
