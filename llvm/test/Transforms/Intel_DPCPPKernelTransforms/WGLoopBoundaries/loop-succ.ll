...
; RUN: opt -passes="dpcpp-kernel-analysis,dpcpp-kernel-wg-loop-bound" %s -S | FileCheck %s
...
; RUN: opt -dpcpp-kernel-analysis -dpcpp-kernel-wg-loop-bound %s -S | FileCheck %s
...
define void @test(i32 addrspace(1)* nocapture %dst, i32 %count) nounwind {
entry:
