...
; RUN: opt -passes=dpcpp-kernel-wg-loop-bound %s -S -debug -disable-output 2>&1 | FileCheck %s
; RUN: opt -dpcpp-kernel-wg-loop-bound %s -S -enable-debugify -disable-output 2>&1 | FileCheck %s -check-prefix=DEBUGIFY
; RUN: opt -dpcpp-kernel-wg-loop-bound %s -S -debug -disable-output 2>&1 | FileCheck %s
