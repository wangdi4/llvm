...
entry:
  ...
  %call = tail call <2 x i32> @_Z10isnotequalDv2_fS_(<2 x float> %0, <2 x float> %1) #2
  store <2 x i32> %call, <2 x i32>* %c, align 8, !tbaa !8
  ret void
;CHECK: define void @foo_float2
;CHECK-NOT: call <2 x i32> @_Z10isnotequalDv2_dS_
;CHECK: [[cmp2:%[a-zA-Z0-9]+]] = fcmp une <2 x float> %0, %1
;CHECK-NEXT: [[sext2:%[a-zA-Z0-9]+]] = sext <2 x i1> [[cmp2]] to <2 x i32>
;CHECK: ret void
}

; Function Attrs: nounwind readnone
declare <2 x i32> @_Z10isnotequalDv2_fS_(<2 x float>, <2 x float>) #1
...
entry:
  ...
  %call = tail call <3 x i32> @_Z10isnotequalDv3_fS_(<3 x float> %extractVec, <3 x float> %extractVec3) #2
...
;CHECK-NOT: call <3 x i32> @_Z10isnotequalDv3_dS_
;CHECK: [[cmp3:%[a-zA-Z0-9]+]] = fcmp une <3 x float> %extractVec, %extractVec3
;CHECK-NEXT: [[sext3:%[a-zA-Z0-9]+]] = sext <3 x i1> [[cmp3]] to <3 x i32>
;CHECK: ret void
}

; Function Attrs: nounwind readnone
declare <3 x i32> @_Z10isnotequalDv3_fS_(<3 x float>, <3 x float>) #1
...
entry:
  ...
  %call = tail call <4 x i32> @_Z10isnotequalDv4_fS_(<4 x float> %0, <4 x float> %1) #2
  store <4 x i32> %call, <4 x i32>* %c, align 16, !tbaa !8
  ret void
;CHECK: define void @foo_float4
;CHECK-NOT: call <4 x i32> @_Z10isnotequalDv4_dS_
;CHECK: [[cmp4:%[a-zA-Z0-9]+]] = fcmp une <4 x float> %0, %1
;CHECK-NEXT: [[sext4:%[a-zA-Z0-9]+]] = sext <4 x i1> [[cmp4]] to <4 x i32>
;CHECK: ret void
}

; Function Attrs: nounwind readnone
declare <4 x i32> @_Z10isnotequalDv4_fS_(<4 x float>, <4 x float>) #1
...
entry:
  ...
  %call = tail call <8 x i32> @_Z10isnotequalDv8_fS_(<8 x float> %0, <8 x float> %1) #2
  store <8 x i32> %call, <8 x i32>* %c, align 32, !tbaa !8
  ret void
;CHECK: define void @foo_float8
;CHECK-NOT: call <8 x i32> @_Z10isnotequalDv8_dS_
;CHECK: [[cmp8:%[a-zA-Z0-9]+]] = fcmp une <8 x float> %0, %1
...
entry:
  ...
  %call = tail call <16 x i32> @_Z10isnotequalDv16_fS_(<16 x float> %0, <16 x float> %1) #2
  store <16 x i32> %call, <16 x i32>* %c, align 64, !tbaa !8
  ret void
;CHECK: define void @foo_float16
;CHECK-NOT: call <16 x i32> @_Z10isnotequalDv16_dS_
;CHECK: [[cmp16:%[a-zA-Z0-9]+]] = fcmp une <16 x float> %0, %1
;CHECK-NEXT: [[sext16:%[a-zA-Z0-9]+]] = sext <16 x i1> [[cmp2]] to <16 x i32>
;CHECK: ret void
}

; Function Attrs: nounwind readnone
declare <16 x i32> @_Z10isnotequalDv16_fS_(<16 x float>, <16 x float>) #1
