...
; NOSKIP: @PhiCanonCase7
; NOSKIP-NOT: %{{[a-z\.0-9]}} %{{[a-z\.0-9]}} %{{[a-z\.0-9]}}
; NOSKIP: phi-split-bb:                                     ; preds = %if.then, %if.then3
; NOSKIP: ret

define void @PhiCanonCase7(i32 %arg1, i32 %arg2, float addrspace(1)* nocapture %a, float addrspace(1)* nocapture %b) nounwind {
entry:
...
; DEBUGIFY-NOSKIP: WARNING: Instruction with empty DebugLoc in function PhiCanonCase7 --  br label %for.header
