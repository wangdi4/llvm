...
; CHECK:        entry:
; CHECK-NEXT:   %call01 = call float @_Z6cos_rmf(float %f1)
; CHECK-NEXT:   %call02 = call <2 x float> @_Z6cos_rmDv2_f(<2 x float> %f2)
; CHECK-NEXT:   %call03 = call <3 x float> @_Z6cos_rmDv3_f(<3 x float> %f3)
; CHECK-NEXT:   %call04 = call <4 x float> @_Z6cos_rmDv4_f(<4 x float> %f4)
; CHECK-NEXT:   %call05 = call <8 x float> @_Z6cos_rmDv8_f(<8 x float> %f8)
; CHECK-NEXT:   %call06 = call <16 x float> @_Z6cos_rmDv16_f(<16 x float> %f16)
; CHECK-NEXT:   ret void

; CHECK:        declare float @_Z6cos_rmf(float)
; CHECK:        declare <2 x float> @_Z6cos_rmDv2_f(<2 x float>)
; CHECK:        declare <3 x float> @_Z6cos_rmDv3_f(<3 x float>)
