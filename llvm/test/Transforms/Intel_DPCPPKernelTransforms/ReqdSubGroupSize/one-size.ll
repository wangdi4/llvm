...
; RUN: opt -enable-new-pm=0 -dpcpp-kernel-reqd-sub-group-size -dpcpp-reqd-sub-group-size="foo(8)" %s -S -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: opt -enable-new-pm=0 -dpcpp-kernel-reqd-sub-group-size -dpcpp-reqd-sub-group-size="foo(8)" %s -S -o - | FileCheck %s
