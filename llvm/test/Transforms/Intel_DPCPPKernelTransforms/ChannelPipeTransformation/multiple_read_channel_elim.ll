...
; CHECK: call i32 @__read_pipe_2_bl_fpga({{.*}} %[[CAST_STAR_PIPE_ARR]], {{.*}}, i32 8, i32 4)
;
; CHECK: %[[LOAD_BAR_PIPE1:.*]] = load {{.*}} @[[PIPE_BAR]]
; CHECK: %[[CAST_BAR_PIPE1:.*]] = bitcast %opencl.pipe_rw_t{{.*}} %[[LOAD_BAR_PIPE1]] to %opencl.pipe_ro_t
; CHECK: call i32 @__read_pipe_2_bl_fpga({{.*}} %[[CAST_BAR_PIPE1]], {{.*}}, i32 4, i32 4)
;
...
; CHECK: call i32 @__read_pipe_2_bl_fpga({{.*}} %[[CAST_STAR_PIPE_ARR1]], {{.*}}, i32 8, i32 4)
...
entry:
  ...
  call void @_Z18read_channel_intel11ocl_channel3Foo(%struct.Foo* sret(%struct.Foo) %stst, %opencl.channel_t addrspace(1)* %3) #4
  ...
  call void @_Z18read_channel_intel11ocl_channel3Foo(%struct.Foo* sret(%struct.Foo) %stst1, %opencl.channel_t addrspace(1)* %7) #4
