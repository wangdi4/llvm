...
; INTEL_CUSTOMIZATION
; RUN: opt -S -opaque-pointers -function-attrs -enable-nonnull-arg-prop %s | FileCheck %s --check-prefixes=FNATTR,FNATTR-NO-SS
; RUN: opt -S -opaque-pointers -passes=function-attrs -enable-nonnull-arg-prop %s | FileCheck %s --check-prefixes=FNATTR,FNATTR-NO-SS
; RUN: opt -S -opaque-pointers -convert-to-subscript %s | opt -S -function-attrs -enable-nonnull-arg-prop | FileCheck %s --check-prefixes=FNATTR,FNATTR-SS
; RUN: opt -S -opaque-pointers -passes=convert-to-subscript %s | opt -S -passes=function-attrs -enable-nonnull-arg-prop | FileCheck %s --check-prefixes=FNATTR,FNATTR-SS
...
; INTEL_CUSTOMIZATION
; FNATTR-NO-SS-NEXT: [[TMP6:%.*]] = getelementptr inbounds i32, ptr [[TMP0:%.*]], i64 [[TMP5]]
; FNATTR-SS-NEXT:    [[TMP6:%.*]] = call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 0, i64 0, i64 4, ptr elementtype(i32) [[TMP0:%.*]], i64 [[TMP5]])
; end INTEL_CUSTOMIZATION
