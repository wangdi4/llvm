...
; INTEL_CUSTOMIZATION
; RUN: opt -S -convert-to-subscript %s | opt -S -functionattrs -enable-nonnull-arg-prop | FileCheck %s --check-prefixes=BOTH,FNATTR,OLD
; RUN: opt -S -passes=convert-to-subscript %s | opt -S -passes=function-attrs -enable-nonnull-arg-prop | FileCheck %s --check-prefixes=BOTH,FNATTR,OLD
