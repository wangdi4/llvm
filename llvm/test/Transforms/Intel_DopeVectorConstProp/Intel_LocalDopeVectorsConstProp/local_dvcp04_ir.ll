; RUN: opt -opaque-pointers=0 < %s -passes=dopevectorconstprop -dope-vector-local-const-prop  -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2  -S 2>&1 | FileCheck %s
...
; CHECK: %"assumedshape_arrs_$ARRAY_TARGET.addr_a0$_fetch.61[]" = tail call float* @llvm.intel.subscript.p0f32.i64.i64.p0f32.i64(i8 2, i64 %"assumedshape_arrs_$ARRAY_TARGET.dim_info$.lower_bound$[]_fetch.68", i64 %"assumedshape_arrs_$ARRAY_TARGET.dim_info$.spacing$[]_fetch.67", float* elementtype(float) %"assumedshape_arrs_$ARRAY_TARGET.addr_a0$_fetch.61", i64 %indvars.iv518)

; CHECK: %"assumedshape_arrs_$ARRAY_SOURCE.addr_a0$_fetch.31[][]" = tail call float* @llvm.intel.subscript.p0f32.i64.i64.p0f32.i64(i8 1, i64 %"assumedshape_arrs_$ARRAY_SOURCE.dim_info$.lower_bound$[]_fetch.35", i64 %"assumedshape_arrs_$ARRAY_SOURCE.dim_info$.spacing$[]_fetch.34", float* elementtype(float) %"assumedshape_arrs_$ARRAY_SOURCE.addr_a0$_fetch.31[]", i64 %indvars.iv515)
; CHECK: %"assumedshape_arrs_$ARRAY_ADD.addr_a0$_fetch.46[][]" = tail call float* @llvm.intel.subscript.p0f32.i64.i64.p0f32.i64(i8 1, i64 %"assumedshape_arrs_$ARRAY_ADD.dim_info$.lower_bound$[]_fetch.50", i64 %"assumedshape_arrs_$ARRAY_ADD.dim_info$.spacing$[]_fetch.49", float* elementtype(float) %"assumedshape_arrs_$ARRAY_ADD.addr_a0$_fetch.46[]", i64 %indvars.iv515)
