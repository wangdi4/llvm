; REQUIRES: asserts
; RUN: opt < %s -opaque-pointers -dope-vector-local-const-prop=false -disable-output -dopevectorconstprop -debug-only=dopevectorconstprop 2>&1 | FileCheck %s
; RUN: opt < %s -opaque-pointers -dope-vector-local-const-prop=false -disable-output -passes=dopevectorconstprop -debug-only=dopevectorconstprop 2>&1 | FileCheck %s
