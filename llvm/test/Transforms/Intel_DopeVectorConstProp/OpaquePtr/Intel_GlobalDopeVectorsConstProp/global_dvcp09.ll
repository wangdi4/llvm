; REQUIRES: asserts
; RUN: opt -opaque-pointers < %s -disable-output -dopevectorconstprop -dope-vector-global-const-prop=true -debug-only=dope-vector-global-const-prop -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers < %s -disable-output -passes=dopevectorconstprop -dope-vector-global-const-prop=true -debug-only=dope-vector-global-const-prop -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 2>&1 | FileCheck %s
...
; Function Attrs: nofree nounwind uwtable
define internal void @arr_mod_mp_print_info_(ptr noalias nocapture readonly %arg, ptr noalias nocapture readonly dereferenceable(4) %arg1, ptr noalias nocapture readonly dereferenceable(4) %arg2, ptr noalias nocapture readonly dereferenceable(4) %arg3, ptr nocapture dereferenceable(96) "assumed_shape" %arg4, ptr nocapture dereferenceable(120) "assumed_shape" %arg5, ptr nocapture dereferenceable(72) "assumed_shape" %arg6) #0 {
bb:
