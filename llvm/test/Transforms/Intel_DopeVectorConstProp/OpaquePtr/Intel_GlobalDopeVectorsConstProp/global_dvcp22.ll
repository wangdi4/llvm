...
; RUN: opt -opaque-pointers < %s -disable-output -dopevectorconstprop -dope-vector-global-const-prop=true -debug-only=dope-vector-global-const-prop -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -dvcp-tile-mv-min-calls=3 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers < %s -disable-output -passes=dopevectorconstprop -dope-vector-global-const-prop=true -debug-only=dope-vector-global-const-prop -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -dvcp-tile-mv-min-calls=3 2>&1 | FileCheck %s
...
bb:
  ...
  %i7 = tail call i32 @for_allocate_handle(i64 400, ptr @arr_mod_mp_a_, i32 262144, ptr null) #3
  ret void
}
...
bb:
  %i = load ptr, ptr @arr_mod_mp_a_, align 16
  %i1 = load i64, ptr getelementptr inbounds (%"QNCA_a0$float*$rank2$", ptr @arr_mod_mp_a_, i64 0, i32 3), align 8
  ...
  %i9 = and i32 %i8, 65011712
  %i10 = or i32 %i6, %i4
  %i11 = or i32 %i10, %i9
  %i12 = or i32 %i11, 262144
  %i13 = load i64, ptr getelementptr inbounds (%"QNCA_a0$float*$rank2$", ptr @arr_mod_mp_a_, i64 0, i32 5), align 8
  %i14 = inttoptr i64 %i13 to ptr
  %i15 = tail call i32 @for_dealloc_allocatable_handle(ptr %i, i32 %i12, ptr %i14) #3
  %i16 = icmp eq i32 %i15, 0
  br i1 %i16, label %bb17, label %bb20

bb17:                                             ; preds = %bb
  store ptr null, ptr @arr_mod_mp_a_, align 16
  %i18 = load i64, ptr getelementptr inbounds (%"QNCA_a0$float*$rank2$", ptr @arr_mod_mp_a_, i64 0, i32 3), align 8
  %i19 = and i64 %i18, -1099243194370
  store i64 %i19, ptr getelementptr inbounds (%"QNCA_a0$float*$rank2$", ptr @arr_mod_mp_a_, i64 0, i32 3), align 8
  br label %bb20
