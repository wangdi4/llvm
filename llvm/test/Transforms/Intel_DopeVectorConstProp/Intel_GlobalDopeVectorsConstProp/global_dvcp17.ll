; REQUIRES: asserts
; RUN: opt -opaque-pointers=0 < %s -disable-output -passes=dopevectorconstprop -debug-only=dopevectorconstprop -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 2>&1 | FileCheck %s
...
; CHECK: DV FOUND: ARG #0 myoutput_.1 2 x float
; CHECK: VALID
; CHECK: REPLACING 4 LOADS WITH 96
; CHECK: REPLACING 4 LOADS WITH 1
; CHECK: REPLACING 1 LOAD WITH 10
; CHECK: NOT REPLACING LOAD IN FXN myoutput_.1   %16 = load i64, i64* %15, align 1
; CHECK: REPLACING 2 LOADS WITH 4
; CHECK: REPLACING 2 LOADS WITH 1
; CHECK: REPLACING 1 LOAD WITH 10
; CHECK: NOT REPLACING LOAD IN FXN myoutput_.1   %18 = load i64, i64* %17, align 1
