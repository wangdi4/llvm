...
; RUN: opt < %s -disable-output -intel-dvcp-relaxed -dopevectorconstprop -dope-vector-global-const-prop=true -debug-only=dope-vector-global-const-prop -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -dvcp-tile-mv-min-calls=3 2>&1 | FileCheck %s
; RUN: opt < %s -disable-output -intel-dvcp-relaxed -passes=dopevectorconstprop -dope-vector-global-const-prop=true -debug-only=dope-vector-global-const-prop -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -dvcp-tile-mv-min-calls=3 2>&1 | FileCheck %s
...
; CHECK: DVCP FOR LOWER BOUND FIELD IS DISABLED
