; REQUIRES: asserts
; RUN: opt < %s -opaque-pointers -disable-output -dopevectorconstprop -dope-vector-global-const-prop=true -debug-only=dopevectorconstprop -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 2>&1 | FileCheck %s
; RUN: opt < %s -opaque-pointers -disable-output -passes=dopevectorconstprop -dope-vector-global-const-prop=true -debug-only=dopevectorconstprop -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 2>&1 | FileCheck %s

; This test case checks that DVCP for global dope vectors is disabled when
; opaque pointers are enabled. The test case needs to be removed once we
; start the development of the transformation for opaque pointers.
