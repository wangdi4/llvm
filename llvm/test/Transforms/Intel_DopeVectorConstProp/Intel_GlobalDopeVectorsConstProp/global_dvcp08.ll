; REQUIRES: asserts
; RUN: opt -opaque-pointers=0 < %s -disable-output -passes=dopevectorconstprop -dope-vector-global-const-prop=true -debug-only=dope-vector-global-const-prop -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 2>&1 | FileCheck %s

; This test case checks that the information for the global dope vector
; and the nested dope vectors wasn't collected nor propagated since
; the nested dope vectors are passed to a function without IR. This
; test case was generated from the following example but function
; PRINT_INFO is created as declare:
...
; Function Attrs: nofree nounwind uwtable
declare dso_local void @arr_mod_mp_print_info_(i32* noalias nocapture readonly %0, i32* noalias nocapture readonly dereferenceable(4) %1, i32* noalias nocapture readonly dereferenceable(4) %2, i32* noalias nocapture readonly dereferenceable(4) %3, %"QNCA_a0$float*$rank2$"* noalias nocapture readonly dereferenceable(96) "assumed_shape" "ptrnoalias" %4, %"QNCA_a0$float*$rank3$"* noalias nocapture readonly dereferenceable(120) "assumed_shape" "ptrnoalias" %5, %"QNCA_a0$float*$rank1$"* noalias nocapture readonly dereferenceable(72) "assumed_shape" "ptrnoalias" %6) #0
