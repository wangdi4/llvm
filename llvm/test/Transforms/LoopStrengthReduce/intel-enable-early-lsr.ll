...
; CHECK-NEXT:    [[SCEVGEP:%.*]] = getelementptr float, float addrspace(1)* [[VLA_ASCAST:%.*]], i64 [[TMP4]]
; CHECK-NEXT:    [[TMP5:%.*]] = sext i32 [[TMP3]] to i64
; CHECK-NEXT:    [[TMP6:%.*]] = shl nsw i64 [[TMP5]], 2
; CHECK-NEXT:    [[TMP7:%.*]] = mul i32 [[TMP2]], 7
; CHECK-NEXT:    [[TMP8:%.*]] = mul i32 [[TMP3]], 7
; CHECK-NEXT:    br label [[OMP_INNER_FOR_BODY:%.*]]
; CHECK:       omp.inner.for.body:
; CHECK-NEXT:    [[LSR_IV3:%.*]] = phi i32 [ [[LSR_IV_NEXT:%.*]], [[OMP_INNER_FOR_BODY]] ], [ [[TMP7]], [[OMP_INNER_FOR_BODY_PREHEADER]] ]
; CHECK-NEXT:    [[LSR_IV:%.*]] = phi float addrspace(1)* [ [[TMP9:%.*]], [[OMP_INNER_FOR_BODY]] ], [ [[SCEVGEP]], [[OMP_INNER_FOR_BODY_PREHEADER]] ]
; CHECK-NEXT:    [[DOTOMP_IV_ASCAST_LOCAL_047:%.*]] = phi i32 [ [[ADD8:%.*]], [[OMP_INNER_FOR_BODY]] ], [ [[TMP2]], [[OMP_INNER_FOR_BODY_PREHEADER]] ]
; CHECK-NEXT:    [[LSR_IV1:%.*]] = bitcast float addrspace(1)* [[LSR_IV]] to i1 addrspace(1)*
; CHECK-NEXT:    [[CONV:%.*]] = sitofp i32 [[LSR_IV3]] to float
; CHECK-NEXT:    store float [[CONV]], float addrspace(1)* [[LSR_IV]], align 4, !tbaa [[TBAA8:![0-9]+]], !alias.scope !12, !noalias !17, !llvm.access.group [[ACC_GRP31:![0-9]+]]
; CHECK-NEXT:    [[ADD8]] = add nsw i32 [[DOTOMP_IV_ASCAST_LOCAL_047]], [[TMP3]]
; CHECK-NEXT:    [[SCEVGEP2:%.*]] = getelementptr i1, i1 addrspace(1)* [[LSR_IV1]], i64 [[TMP6]]
; CHECK-NEXT:    [[TMP9]] = bitcast i1 addrspace(1)* [[SCEVGEP2]] to float addrspace(1)*
; CHECK-NEXT:    [[LSR_IV_NEXT]] = add i32 [[LSR_IV3]], [[TMP8]]
...
define weak dso_local spir_kernel void @__omp_offloading_3b_d7a6801f__Z3foo_l8(float addrspace(1)* noalias %vla.ascast, i64 %N.addr.ascast.val.zext, i64 %omp.vla.tmp.ascast.val, i64 %.addr.ascast.val) local_unnamed_addr #0 {
...
omp.inner.for.body:                               ; preds = %omp.inner.for.body.preheader, %omp.inner.for.body
  ...
  %arrayidx7 = getelementptr inbounds float, float addrspace(1)* %vla.ascast, i64 %idxprom
  store float %conv, float addrspace(1)* %arrayidx7, align 4, !tbaa !8, !alias.scope !12, !noalias !17, !llvm.access.group !31
