...
; Tests if the intrinsic llvm.public.type.test was converted correctly
; into llvm.type.test with whole program safe.


; RUN: opt -opaque-pointers=0 < %s -passes='module(intel-fold-wp-intrinsic)' -whole-program-assume -S 2>&1 | FileCheck %s
...
; CHECK: br i1 true, label %if.whpr, label %if.end
; CHECK-NOT: @llvm.intel.wholeprogramsafe()
; CHECK: @llvm.type.test(i8* %vtable, metadata !"_ZTS1A")
