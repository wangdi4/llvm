...
; Note: remove XFAIL when CMPLRLLVM-49320 is fixed and stores to "fake"
; memory are supported in VecClone for uval args at -O0.

; Check code generated from VecClone for uval vector variant for unoptimized LLVM
; Stride should be applied to value loaded indirectly from the linear arg local
; memory (alias)

; Function Attrs: mustprogress noinline nounwind optnone uwtable
define dso_local noundef i64 @_Z3fooRl(ptr noundef nonnull align 8 dereferenceable(8) %x) #0 {
; CHECK:  define dso_local noundef <8 x i64> @_ZGVbN8U2__Z3fooRl(ptr noundef nonnull align 8 dereferenceable(8) [[X0:%.*]]) #1 {
; CHECK-NEXT:  entry:
...
; CHECK-NEXT:    [[RET_CAST0:%.*]] = bitcast ptr [[VEC_RETVAL0]] to ptr
; CHECK-NEXT:    br label [[SIMD_BEGIN_REGION0:%.*]]
; CHECK-EMPTY:
; CHECK-NEXT:  simd.begin.region:
; CHECK-NEXT:    [[ENTRY_REGION0:%.*]] = call token @llvm.directive.region.entry() [ "DIR.OMP.SIMD"(), "QUAL.OMP.SIMDLEN"(i32 8), "QUAL.OMP.LINEAR:TYPED"(ptr [[ALLOCA_X0]], ptr null, i32 1, i32 2), "QUAL.OMP.PRIVATE:TYPED"(ptr [[X_ADDR0]], ptr null, i32 1) ]
...
; CHECK-NEXT:    store ptr [[LOAD_X0]], ptr [[X_ADDR0]], align 8
