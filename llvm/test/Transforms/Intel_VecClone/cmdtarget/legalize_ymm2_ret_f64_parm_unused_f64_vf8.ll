...
; Note that first argument is unused in the scalar routine.
; It still needs to be legalized but there is no any code that reads the argument.
; ISA class: YMM2
; "vector-variants"="_ZGVYN8vv_bar,_ZGVYM8vv_bar"

define dso_local nofpclass(nan inf) double @bar(double noundef nofpclass(nan inf) %y,double noundef nofpclass(nan inf) %x) local_unnamed_addr #0 {
; CHECK:  define dso_local x86_regcallcc { <4 x double>, <4 x double> } @_ZGVYN8vv_bar(<4 x double> noundef nofpclass(nan inf) [[Y_00:%.*]], <4 x double> noundef nofpclass(nan inf) [[Y_10:%.*]], <4 x double> noundef nofpclass(nan inf) [[X_00:%.*]], <4 x double> noundef nofpclass(nan inf) [[X_10:%.*]]) local_unnamed_addr #1 {
; CHECK-NEXT:  entry:
...
; CHECK-NEXT:  }
;
; CHECK:  define dso_local x86_regcallcc { <4 x double>, <4 x double> } @_ZGVYM8vv_bar(<4 x double> noundef nofpclass(nan inf) [[Y_00]], <4 x double> noundef nofpclass(nan inf) [[Y_10]], <4 x double> noundef nofpclass(nan inf) [[X_00]], <4 x double> noundef nofpclass(nan inf) [[X_10]], <4 x double> [[MASK_00:%.*]], <4 x double> [[MASK_10:%.*]]) local_unnamed_addr #1 {
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[VEC_X0]] = alloca <8 x double>, align 64
...
attributes #0 = { mustprogress nofree noinline norecurse nosync nounwind willreturn memory(none) uwtable "vector-variants"="_ZGVYN8vv_bar,_ZGVYM8vv_bar" }
