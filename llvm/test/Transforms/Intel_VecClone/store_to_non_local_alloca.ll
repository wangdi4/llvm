...
define dso_local void @foo1(ptr nocapture nonnull align 4 dereferenceable(4) %p, i32 %i) local_unnamed_addr #0 {
; CHECK:  define dso_local void @_ZGVbN2vl_foo1(<2 x ptr> nocapture nonnull align 4 dereferenceable(4) [[P0:%.*]], i32 [[I0:%.*]]) local_unnamed_addr #2 {
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[ALLOCA_I0:%.*]] = alloca i32, align 4
; CHECK-NEXT:    store i32 [[I0]], ptr [[ALLOCA_I0]], align 4
...
; CHECK-NEXT:    [[ENTRY_REGION0:%.*]] = call token @llvm.directive.region.entry() [ "DIR.OMP.SIMD"(), "QUAL.OMP.SIMDLEN"(i32 2), "QUAL.OMP.LINEAR:TYPED.PTR_TO_PTR"(ptr [[ALLOCA_I0]], i8 0, i32 1, i32 1) ]
; CHECK-NEXT:    br label [[SIMD_LOOP_PREHEADER0:%.*]]
; CHECK-EMPTY:
; CHECK-NEXT:  simd.loop.preheader:
; CHECK-NEXT:    [[LOAD_I0:%.*]] = load i32, ptr [[ALLOCA_I0]], align 4
...
; CHECK-NEXT:    store i32 [[STRIDE_ADD0]], ptr [[VEC_P_ELEM0]], align 4
...
define dso_local void @foo2(ptr nocapture %p, i32 %i) local_unnamed_addr #1 {
; CHECK:  define dso_local void @_ZGVbN2vl_foo2(<2 x ptr> nocapture [[P0:%.*]], i32 [[I0:%.*]]) local_unnamed_addr #2 {
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[ALLOCA_I0:%.*]] = alloca i32, align 4
; CHECK-NEXT:    store i32 [[I0]], ptr [[ALLOCA_I0]], align 4
; CHECK-NEXT:    [[VEC_P0:%.*]] = alloca <2 x ptr>, align 16
...
; CHECK-NEXT:    [[VEC_P_CAST_GEP0:%.*]] = getelementptr ptr, ptr [[VEC_P0]], i32 [[INDEX0]]
; CHECK-NEXT:    [[VEC_P_ELEM0:%.*]] = load ptr, ptr [[VEC_P_CAST_GEP0]], align 8
; CHECK-NEXT:    [[PTRIDX0:%.*]] = getelementptr inbounds i32, ptr [[VEC_P_ELEM0]]
; CHECK-NEXT:    [[STRIDE_MUL0:%.*]] = mul i32 1, [[INDEX0]]
; CHECK-NEXT:    [[STRIDE_ADD0:%.*]] = add i32 [[LOAD_I0]], [[STRIDE_MUL0]]
; CHECK-NEXT:    store i32 [[STRIDE_ADD0]], ptr [[PTRIDX0]], align 4
; CHECK-NEXT:    br label [[SIMD_LOOP_LATCH0]]
; CHECK-EMPTY:
; CHECK-NEXT:  simd.loop.latch:
; CHECK-NEXT:    [[INDVAR0]] = add nuw nsw i32 [[INDEX0]], 1
...
;
entry:
  %ptridx = getelementptr inbounds i32, ptr %p
  store i32 %i, ptr %ptridx, align 4
  ret void
}

; CHECK: attributes #2 = { nounwind memory(readwrite) uwtable "frame-pointer"="none" "may-have-openmp-directive"="true" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+{{.*}}" }
