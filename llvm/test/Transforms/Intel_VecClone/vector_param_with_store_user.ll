...
; CHECK-NEXT:    store <2 x float> [[C_COERCE0]], ptr [[C0]], align 4
; CHECK-NEXT:    store i32 [[MAX_ITER0]], ptr [[MAX_ITER_ADDR0]], align 4
; CHECK-NEXT:    ret void
; CHECK-NEXT:  }
;
...
; CHECK-NEXT:    store i32 [[MAX_ITER0]], ptr [[ALLOCA_MAX_ITER0]], align 4
; CHECK-NEXT:    [[VEC_C_COERCE0:%.*]] = alloca <16 x float>, align 64
; CHECK-NEXT:    [[C0]] = alloca { float, float }, align 4
; CHECK-NEXT:    [[MAX_ITER_ADDR0]] = alloca i32, align 4
; CHECK-NEXT:    store <16 x float> [[C_COERCE0]], ptr [[VEC_C_COERCE0]], align 64
; CHECK-NEXT:    br label [[SIMD_BEGIN_REGION0:%.*]]
; CHECK-EMPTY:
; CHECK-NEXT:  simd.begin.region:
; CHECK-NEXT:    [[ENTRY_REGION0:%.*]] = call token @llvm.directive.region.entry() [ "DIR.OMP.SIMD"(), "QUAL.OMP.SIMDLEN"(i32 8), "QUAL.OMP.UNIFORM:TYPED"(ptr [[ALLOCA_MAX_ITER0]], i32 0, i32 1), "QUAL.OMP.PRIVATE:TYPED"(ptr [[C0]], { float, float } zeroinitializer, i32 1), "QUAL.OMP.PRIVATE:TYPED"(ptr [[MAX_ITER_ADDR0]], i32 0, i32 1) ]
; CHECK-NEXT:    br label [[SIMD_LOOP_PREHEADER0:%.*]]
; CHECK-EMPTY:
; CHECK-NEXT:  simd.loop.preheader:
; CHECK-NEXT:    [[LOAD_MAX_ITER0:%.*]] = load i32, ptr [[ALLOCA_MAX_ITER0]], align 4
...
; CHECK-NEXT:    [[VEC_C_COERCE_CAST_GEP0:%.*]] = getelementptr <2 x float>, ptr [[VEC_C_COERCE0]], i32 [[INDEX0]]
; CHECK-NEXT:    [[VEC_C_COERCE_ELEM0:%.*]] = load <2 x float>, ptr [[VEC_C_COERCE_CAST_GEP0]], align 8
; CHECK-NEXT:    store <2 x float> [[VEC_C_COERCE_ELEM0]], ptr [[C0]], align 4
; CHECK-NEXT:    store i32 [[LOAD_MAX_ITER0]], ptr [[MAX_ITER_ADDR0]], align 4
...
entry:
  ...
  store <2 x float> %c.coerce, ptr %c, align 4
  store i32 %max_iter, ptr %max_iter.addr, align 4
  ret void
}
