...
; CHECK:       %tmp1 = bitcast i1 (%class.Base*, i32)* %tmp2 to i8*
; CHECK-NEXT:  %tmp3 = bitcast i1 (%class.Derived*, i32)* @_ZN7Derived3fooEi to i8*
; CHECK-NEXT:  %tmp4 = icmp eq i8* %tmp1, %tmp3
; CHECK-NEXT:  br i1 %tmp4, label %BBDevirt__ZN7Derived3fooEi_0_0, label %BBDevirt__ZN8Derived23fooEi_0_0
...
; CHECK:        %tmp6 = tail call zeroext i1 bitcast (i1 (%class.Derived2*, i32)* @_ZN8Derived23fooEi to i1 (%class.Base*, i32)*)(%class.Base* %.4, i32 %argc)
; CHECK-NEXT:   br label %MergeBB_0_0

; We need to collect back the result and generate the PhiNode
; CHECK-LABEL: MergeBB_0_0:
; CHECK-NEXT:   %tmp7 = phi i1 [ %tmp5, %BBDevirt__ZN7Derived3fooEi_0_0 ], [ %tmp6, %BBDevirt__ZN8Derived23fooEi_0_0 ]
; CHECK-NEXT:   br label %bb

; Now check that the users were replaced correctly
; CHECK-LABEL: bb:
; CHECK-NEXT:   %call.i = tail call dereferenceable(272) %"class.std::basic_ostream"* @_ZNSo9_M_insertIbEERSoT_(%"class.std::basic_ostream"* nonnull @_ZSt4cout, i1 zeroext %tmp7)
