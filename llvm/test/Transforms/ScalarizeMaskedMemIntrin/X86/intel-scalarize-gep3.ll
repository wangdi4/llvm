...
define void @mm_mp_gentrs_(float* noalias nocapture readonly dereferenceable(4) %PTRST, i32* noalias nocapture readonly dereferenceable(4) %NCLS, float* noalias nocapture writeonly dereferenceable(4) %RPICT) local_unnamed_addr #0 {
; CHECK-LABEL: @mm_mp_gentrs_(
; CHECK-NEXT:  alloca_1:
; CHECK-NEXT:    [[NCLS_FETCH_1:%.*]] = load i32, i32* [[NCLS:%.*]], align 1
...
; CHECK-NEXT:    [[TMP4:%.*]] = bitcast float* [[PTRST:%.*]] to i8*
; CHECK-NEXT:    [[TMP5:%.*]] = add i64 [[I1_I64_0]], 1
; CHECK-NEXT:    [[TMP6:%.*]] = mul i64 [[MUL_1]], [[TMP5]]
; CHECK-NEXT:    [[PTR0:%.*]] = getelementptr i8, i8* [[TMP4]], i64 [[TMP6]]
; CHECK-NEXT:    [[TMP7:%.*]] = add i64 [[I1_I64_0]], 1
; CHECK-NEXT:    [[TMP8:%.*]] = bitcast i8* [[PTR0]] to float*
; CHECK-NEXT:    [[PTR01:%.*]] = getelementptr float, float* [[TMP8]], i64 [[TMP7]]
; CHECK-NEXT:    [[LOAD0:%.*]] = load float, float* [[PTR01]], align 1
; CHECK-NEXT:    [[RES0:%.*]] = insertelement <2 x float> undef, float [[LOAD0]], i64 0
; CHECK-NEXT:    [[TMP9:%.*]] = bitcast float* [[PTRST]] to i8*
; CHECK-NEXT:    [[TMP10:%.*]] = add i64 [[I1_I64_0]], 2
; CHECK-NEXT:    [[TMP11:%.*]] = mul i64 [[MUL_1]], [[TMP10]]
; CHECK-NEXT:    [[PTR1:%.*]] = getelementptr i8, i8* [[TMP9]], i64 [[TMP11]]
; CHECK-NEXT:    [[TMP12:%.*]] = add i64 [[I1_I64_0]], 2
; CHECK-NEXT:    [[TMP13:%.*]] = bitcast i8* [[PTR1]] to float*
; CHECK-NEXT:    [[PTR12:%.*]] = getelementptr float, float* [[TMP13]], i64 [[TMP12]]
; CHECK-NEXT:    [[LOAD1:%.*]] = load float, float* [[PTR12]], align 1
; CHECK-NEXT:    [[RES1:%.*]] = insertelement <2 x float> [[RES0]], float [[LOAD1]], i64 1
; CHECK-NEXT:    [[TMP14:%.*]] = add nuw nsw i64 [[I1_I64_0]], 1
; CHECK-NEXT:    [[TMP15:%.*]] = getelementptr inbounds float, float* [[RPICT:%.*]], i64 [[TMP14]]
; CHECK-NEXT:    [[TMP16:%.*]] = bitcast float* [[TMP15]] to <2 x float>*
; CHECK-NEXT:    store <2 x float> [[RES1]], <2 x float>* [[TMP16]], align 1
...
; CHECK-NEXT:    [[TMP18:%.*]] = getelementptr inbounds float, float* [[RPICT]], i64 [[TMP17]]
; CHECK-NEXT:    [[TMP19:%.*]] = mul nsw i64 [[TMP17]], [[INT_SEXT]]
; CHECK-NEXT:    [[TMP20:%.*]] = getelementptr inbounds float, float* [[PTRST]], i64 [[TMP19]]
; CHECK-NEXT:    [[TMP21:%.*]] = getelementptr inbounds float, float* [[TMP20]], i64 [[TMP17]]
; CHECK-NEXT:    [[GEPLOAD:%.*]] = load float, float* [[TMP21]], align 1
; CHECK-NEXT:    store float [[GEPLOAD]], float* [[TMP18]], align 1
; CHECK-NEXT:    [[CONDLOOP_51_NOT:%.*]] = icmp eq i64 [[I1_I64_1]], [[TMP3]]
; CHECK-NEXT:    br i1 [[CONDLOOP_51_NOT]], label [[BB3]], label [[LOOP_51]]
;
alloca_1:
  %NCLS_fetch.1 = load i32, i32* %NCLS, align 1
...
loop.16.preheader:                                ; preds = %bb2.preheader
  %.splatinsert22 = insertelement <2 x float*> poison, float* %PTRST, i64 0
  ...
  %3 = bitcast <2 x float*> %.splatinsert22 to <2 x i8*>
...
loop.16:                                          ; preds = %loop.16.preheader, %loop.16
  ...
  %8 = getelementptr inbounds i8, <2 x i8*> %4, <2 x i64> %7
  %9 = bitcast <2 x i8*> %8 to <2 x float*>
  %10 = getelementptr inbounds float, <2 x float*> %9, <2 x i64> %6
  %11 = call <2 x float> @llvm.masked.gather.v2f32.v2p0f32(<2 x float*> %10, i32 1, <2 x i1> <i1 true, i1 true>, <2 x float> undef)
  %12 = add nuw nsw i64 %i1.i64.0, 1
  %13 = getelementptr inbounds float, float* %RPICT, i64 %12
  %14 = bitcast float* %13 to <2 x float>*
  store <2 x float> %11, <2 x float>* %14, align 1
...
loop.51:                                          ; preds = %loop.51, %hir.L.24
  ...
  %16 = getelementptr inbounds float, float* %RPICT, i64 %15
  %17 = mul nsw i64 %15, %int_sext
  %18 = getelementptr inbounds float, float* %PTRST, i64 %17
  %19 = getelementptr inbounds float, float* %18, i64 %15
  %gepload = load float, float* %19, align 1
  store float %gepload, float* %16, align 1
  %condloop.51.not = icmp eq i64 %i1.i64.1, %5
  br i1 %condloop.51.not, label %bb3, label %loop.51
}

; Function Attrs: nocallback nofree nosync nounwind readonly willreturn
declare <2 x float> @llvm.masked.gather.v2f32.v2p0f32(<2 x float*>, i32 immarg, <2 x i1>, <2 x float>) #2
