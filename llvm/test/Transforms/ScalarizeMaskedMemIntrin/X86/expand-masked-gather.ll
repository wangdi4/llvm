...
; INTEL_CUSTOMIZATION
define <2 x i64> @scalarize_v2i64_ones_mask_gep(ptr %p, <2 x i64> %passthru) {
; CHECK-LABEL: @scalarize_v2i64_ones_mask_gep(
; CHECK-NEXT:    [[PTR0:%.*]] = getelementptr i64, ptr [[P:%.*]], i64 0
; CHECK-NEXT:    [[LOAD0:%.*]] = load i64, ptr [[PTR0]], align 8
; CHECK-NEXT:    [[RES0:%.*]] = insertelement <2 x i64> [[PASSTHRU:%.*]], i64 [[LOAD0]], i64 0
; CHECK-NEXT:    [[PTR1:%.*]] = getelementptr i64, ptr [[P]], i64 2
; CHECK-NEXT:    [[LOAD1:%.*]] = load i64, ptr [[PTR1]], align 8
; CHECK-NEXT:    [[RES1:%.*]] = insertelement <2 x i64> [[RES0]], i64 [[LOAD1]], i64 1
; CHECK-NEXT:    ret <2 x i64> [[RES1]]
;
  %vptr0 = insertelement <2 x ptr> undef, ptr %p, i32 0
  %vptr1 = shufflevector <2 x ptr> %vptr0, <2 x ptr> undef, <2 x i32> zeroinitializer
  %vptr2 = getelementptr i64, <2 x ptr> %vptr1, <2 x i64> <i64 0, i64 2>
  %ret = call <2 x i64> @llvm.masked.gather.v2i64.v2p0(<2 x ptr> %vptr2, i32 8, <2 x i1> <i1 true, i1 true>, <2 x i64> %passthru)
  ret <2 x i64> %ret
}

define <2 x i64> @scalarize_v2i64_var_mask_gep(ptr %p, <2 x i64> %passthru, <2 x i1> %mask) {
; CHECK-LABEL: @scalarize_v2i64_var_mask_gep(
; CHECK-NEXT:    [[VPTR0:%.*]] = insertelement <2 x ptr> undef, ptr [[P:%.*]], i32 0
; CHECK-NEXT:    [[VPTR1:%.*]] = shufflevector <2 x ptr> [[VPTR0]], <2 x ptr> undef, <2 x i32> zeroinitializer
; CHECK-NEXT:    [[VPTR2:%.*]] = getelementptr i64, <2 x ptr> [[VPTR1]], <2 x i64> <i64 0, i64 2>
; CHECK-NEXT:    [[SCALAR_MASK:%.*]] = bitcast <2 x i1> [[MASK:%.*]] to i2
; CHECK-NEXT:    [[TMP1:%.*]] = and i2 [[SCALAR_MASK]], 1
; CHECK-NEXT:    [[TMP2:%.*]] = icmp ne i2 [[TMP1]], 0
; CHECK-NEXT:    br i1 [[TMP2]], label [[COND_LOAD:%.*]], label [[ELSE:%.*]]
...
;
  ...
  %vptr1 = shufflevector <2 x ptr> %vptr0, <2 x ptr> undef, <2 x i32> zeroinitializer
  %vptr2 = getelementptr i64, <2 x ptr> %vptr1, <2 x i64> <i64 0, i64 2>
  %ret = call <2 x i64> @llvm.masked.gather.v2i64.v2p0(<2 x ptr> %vptr2, i32 8, <2 x i1> %mask, <2 x i64> %passthru)
  ret <2 x i64> %ret
}
