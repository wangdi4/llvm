...
; INTEL_CUSTOMIZATION
; RUN: opt -enable-new-pm=0 -S -indvars-predicate-loops=0 -indvars %s | FileCheck %s --check-prefixes=CHECK,OLDPM
; RUN: opt -S -indvars-predicate-loops=0 -passes='loop(indvars)' %s | FileCheck %s --check-prefixes=CHECK,NEWPM
; xmain is able to perform last-value on most of these loops.
; Old and new PM may have differing results because the PMs will process the
; loops in opposite order.
...
; CHECK-NEXT:    br i1 [[CMP2]], label [[LOOP2]], label [[LOOP2_END:%.*]]
; CHECK:       loop2.end:
; NEWPM-NEXT:    [[K2_ADD_LCSSA:%.*]] = add i16 [[ARG]], 4 ;INTEL
; OLDPM-NEXT:    [[K2_ADD_LCSSA:%.*]] = add i16 [[TMP0]], 2 ;INTEL
; CHECK-NEXT:    ret i16 [[K2_ADD_LCSSA]]
;
entry:
  br label %loop1
