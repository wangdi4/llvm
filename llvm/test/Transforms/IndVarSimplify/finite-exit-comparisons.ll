...
; CHECK-NEXT:  entry:
; Trip count evaluated in i8 type instead of i16 type due to intel customization. ; INTEL
; CHECK-NEXT:    [[TMP0:%.*]] = trunc i16 [[N_RAW:%.*]] to i8 ;INTEL
; CHECK-NEXT:    [[SMAX:%.*]] = call i8 @llvm.smax.i8(i8 [[TMP0]], i8 1) ;INTEL
; CHECK-NEXT:    br label [[FOR_BODY:%.*]]
; CHECK:       for.body:
; CHECK-NEXT:    [[IV:%.*]] = phi i8 [ [[IV_NEXT:%.*]], [[FOR_BODY]] ], [ 0, [[ENTRY:%.*]] ] ;INTEL
; CHECK-NEXT:    [[IV_NEXT]] = add nuw nsw i8 [[IV]], 1 ;INTEL
; CHECK-NEXT:    [[EXITCOND:%.*]] = icmp ne i8 [[IV_NEXT]], [[SMAX]] ;INTEL
...
; CHECK-NEXT:    [[IV_NEXT]] = add i8 [[IV]], 1
; Trip count evaluated in i8 type instead of i16 type due to intel customization. ; INTEL
; CHECK-NEXT:    [[EXITCOND:%.*]] = icmp ne i8 [[IV_NEXT]], -1 ;INTEL
; CHECK-NEXT:    br i1 [[EXITCOND]], label [[FOR_BODY]], label [[FOR_END:%.*]] ;INTEL
; CHECK:       for.end:
; CHECK-NEXT:    ret void
;
entry:
