...
; CHECK-NEXT:    [[TMP0:%.*]] = trunc i16 [[N_RAW:%.*]] to i8
; CHECK-NEXT:    [[SMAX:%.*]] = call i8 @llvm.smax.i8(i8 [[TMP0]], i8 1)
...
; CHECK-NEXT:    [[EXITCOND:%.*]] = icmp ne i8 [[IV_NEXT]], [[SMAX]]
; end INTEL_CUSTOMIZATION
...
; INTEL_CUSTOMIZATION
; Trip count evaluated in i8 type instead of i16 type due to intel customization.
; CHECK-NEXT:    [[EXITCOND:%.*]] = icmp ne i8 [[IV_NEXT]], -1
; CHECK-NEXT:    br i1 [[EXITCOND]], label [[FOR_BODY]], label [[FOR_END:%.*]]
; end INTEL_CUSTOMIZATION
; CHECK:       for.end:
; CHECK-NEXT:    ret void
...
; INTEL_CUSTOMIZATION
; Trip count evaluated in i8 type instead of i16 type due to intel customization.
; CHECK-NEXT:    [[UMAX:%.*]] = call i8 @llvm.umax.i8(i8 [[TMP0]], i8 -2)
; CHECK-NEXT:    [[TMP1:%.*]] = trunc i16 254 to i8
; end INTEL_CUSTOMIZATION
; CHECK-NEXT:    br label [[FOR_BODY:%.*]]
; CHECK:       for.body:
; CHECK-NEXT:    [[IV:%.*]] = phi i8 [ [[IV_NEXT:%.*]], [[FOR_BODY]] ], [ [[START]], [[ENTRY:%.*]] ]
; CHECK-NEXT:    [[IV_NEXT]] = add i8 [[IV]], 1
; INTEL_CUSTOMIZATION
; CHECK-NEXT:    [[CMP:%.*]] = icmp ult i8 [[IV_NEXT]], [[TMP1]]
; end INTEL_CUSTOMIZATION
; CHECK-NEXT:    br i1 [[CMP]], label [[FOR_BODY]], label [[FOR_END:%.*]]
; CHECK:       for.end:
; INTEL_CUSTOMIZATION
; CHECK-NEXT:    [[TMP2:%.*]] = zext i8 [[UMAX]] to i16
