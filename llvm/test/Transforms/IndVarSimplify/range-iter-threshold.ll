...
; INTEL the iter threshold doesn't change the decisions made (widening vs not)

define i32 @test(i1 %c.0, i32 %m) {
; COMMON-LABEL: define i32 @test ;INTEL
; COMMON-SAME: (i1 [[C_0:%.*]], i32 [[M:%.*]]) { ;INTEL
; COMMON-NEXT:  entry:
; COMMON-NEXT:    br label [[OUTER_HEADER:%.*]]
; COMMON:       outer.header:
; COMMON-NEXT:    [[INDVARS_IV:%.*]] = phi i64 [ [[INDVARS_IV_NEXT:%.*]], [[OUTER_LATCH:%.*]] ], [ 0, [[ENTRY:%.*]] ] ;INTEL
...
; COMMON:       inner.1:
; COMMON-NEXT:    [[C_1:%.*]] = icmp ult i64 0, [[INDVARS_IV]] ;INTEL
...
; COMMON-NEXT:    br i1 [[C_0]], label [[OUTER_LATCH]], label [[INNER_2_LATCH]] ;INTEL
; COMMON:       inner.2.latch:
; COMMON-NEXT:    [[IV_3_NEXT]] = add i32 [[IV_3]], 1
; COMMON-NEXT:    [[C_2:%.*]] = icmp ugt i32 [[IV_3]], [[IV_1]] ;INTEL
; COMMON-NEXT:    br i1 [[C_2]], label [[OUTER_LATCH]], label [[INNER_2_HEADER]] ;INTEL
; COMMON:       outer.latch:
; COMMON-NEXT:    [[MAX_1]] = phi i32 [ [[M]], [[INNER_2_LATCH]] ], [ 0, [[INNER_2_HEADER]] ] ;INTEL
; COMMON-NEXT:    [[INDVARS_IV_NEXT]] = add nuw nsw i64 [[INDVARS_IV]], 1 ;INTEL
; COMMON-NEXT:    [[IV_1_NEXT]] = add nuw nsw i32 [[IV_1]], 1 ;INTEL
