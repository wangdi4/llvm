...
; INTEL_CUSTOMIZATION
; RUN: opt -opaque-pointers -loop-rotate -instcombine                   -enable-knowledge-retention -S < %s  | FileCheck %s --check-prefixes=OLDPM
; RUN: opt -opaque-pointers -passes='loop(loop-rotate),instcombine' -enable-knowledge-retention -S < %s  | FileCheck %s --check-prefixes=NEWPM
...
; NEWPM-NEXT:    [[I4:%.*]] = getelementptr inbounds [[TMP0:%.*]], ptr [[I1]], i64 0, i32 0
; NEWPM-NEXT:    [[I5]] = load ptr, ptr [[I4]], align 8
