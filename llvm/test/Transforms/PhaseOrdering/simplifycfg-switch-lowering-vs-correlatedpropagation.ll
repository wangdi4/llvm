...
; RUN: opt -xmain-enable-gep0-removal -passes='default<O1>' -S < %s | FileCheck %s ;INTEL
; RUN: opt -xmain-enable-gep0-removal -passes='default<O2>' -S < %s | FileCheck %s ;INTEL
; RUN: opt -xmain-enable-gep0-removal -passes='default<O3>' -S < %s | FileCheck %s ;INTEL
...
; CHECK-NEXT:    [[TMP0:%.*]] = select i1 [[COND]], i64 10, i64 0 ;INTEL
; CHECK-NEXT:    ret i64 [[TMP0]] ;INTEL
;
entry:
