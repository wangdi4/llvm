...
; NOTE: Assertions have been autogenerated by utils/update_test_checks.py UTC_ARGS: --version 2 ; INTEL
; RUN: opt -passes="default<O3>" -xmain-enable-gep0-removal -S            < %s | FileCheck %s --check-prefix=SSE ;INTEL
; RUN: opt -passes="default<O3>" -xmain-enable-gep0-removal -S -mattr=avx < %s | FileCheck %s --check-prefix=AVX ;INTEL
...
; SSE-LABEL: define dso_local noundef <4 x float> @ConvertVectors_ByRef ;INTEL
; SSE-SAME: (ptr nocapture noundef nonnull readonly align 16 dereferenceable(16) [[TMP0:%.*]]) local_unnamed_addr #[[ATTR0:[0-9]+]] { ;INTEL
; SSE-NEXT:    [[TMP2:%.*]] = load <4 x float>, ptr [[TMP0]], align 16 ;INTEL
; SSE-NEXT:    [[TMP3:%.*]] = shufflevector <4 x float> [[TMP2]], <4 x float> poison, <4 x i32> <i32 0, i32 1, i32 2, i32 2>
; SSE-NEXT:    ret <4 x float> [[TMP3]]
;
; AVX-LABEL: define dso_local noundef <4 x float> @ConvertVectors_ByRef ;INTEL
; AVX-SAME: (ptr nocapture noundef nonnull readonly align 16 dereferenceable(16) [[TMP0:%.*]]) local_unnamed_addr #[[ATTR0:[0-9]+]] { ;INTEL
; AVX-NEXT:    [[TMP2:%.*]] = load <4 x float>, ptr [[TMP0]], align 16 ;INTEL
...
; SSE-LABEL: define noundef <4 x float> @ConvertVectors_ByVal ;INTEL
; SSE-SAME: (ptr nocapture noundef nonnull readonly align 16 dereferenceable(16) [[V:%.*]]) local_unnamed_addr #[[ATTR0]] { ;INTEL
; SSE-NEXT:  entry:
; INTEL_CUSTOMIZATION
...
;
; AVX-LABEL: define noundef <4 x float> @ConvertVectors_ByVal ;INTEL
; AVX-SAME: (ptr nocapture noundef nonnull readonly align 16 dereferenceable(16) [[V:%.*]]) local_unnamed_addr #[[ATTR0]] { ;INTEL
; AVX-NEXT:  entry:
; INTEL_CUSTOMIZATION
...
; SSE-LABEL: define void @PR51397 ;INTEL
; SSE-SAME: (ptr nocapture noundef writeonly [[DST:%.*]], ptr nocapture noundef readonly [[SRCP:%.*]]) local_unnamed_addr #[[ATTR1:[0-9]+]] { ;INTEL
; SSE-NEXT:    [[TMP1:%.*]] = load <4 x i32>, ptr [[SRCP]], align 16 ;INTEL
; SSE-NEXT:    [[TMP2:%.*]] = sitofp <4 x i32> [[TMP1]] to <4 x float>
; SSE-NEXT:    store <4 x float> [[TMP2]], ptr [[DST]], align 16 ;INTEL
; SSE-NEXT:    ret void
;
; AVX-LABEL: define void @PR51397 ;INTEL
; AVX-SAME: (ptr nocapture noundef writeonly [[DST:%.*]], ptr nocapture noundef readonly [[SRCP:%.*]]) local_unnamed_addr #[[ATTR1:[0-9]+]] { ;INTEL
; AVX-NEXT:    [[TMP1:%.*]] = load <4 x i32>, ptr [[SRCP]], align 16 ;INTEL
; AVX-NEXT:    [[TMP2:%.*]] = sitofp <4 x i32> [[TMP1]] to <4 x float>
; AVX-NEXT:    store <4 x float> [[TMP2]], ptr [[DST]], align 16 ;INTEL
; AVX-NEXT:    ret void
;
