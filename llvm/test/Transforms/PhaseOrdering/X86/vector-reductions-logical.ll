...
; INTEL_CUSTOMIZATION
; This test is done with -O2, so there are many differences with llorg.
; * No common.ret blocks. Only return blocks.
; * Ordering of extracts/ors/fcmp in anyof tests are different.
; * Ordering of phis is different.
; * Some select conversion is skipped.
; * freeze may be skipped.
...
; CHECK-NEXT:    br i1 [[TMP2]], label [[RETURN:%.*]], label [[LOR_LHS_FALSE:%.*]] ;INTEL
; CHECK:       lor.lhs.false: ;INTEL
...
; INTEL_CUSTOMIZATION
; CHECK-NEXT:    [[TMP7:%.*]] = select i1 [[TMP5]], float 0.000000e+00, float [[ADD]]
; CHECK-NEXT:    br label [[RETURN]]
; CHECK:       return:
; CHECK-NEXT:    [[RETVAL_0:%.*]] = phi float [ 0.000000e+00, [[ENTRY:%.*]] ], [ [[TMP7]], [[LOR_LHS_FALSE]] ]
; end INTEL_CUSTOMIZATION
...
; CHECK-NEXT:    [[OR_COND:%.*]] = or i1 [[CMP14]], [[CMP19]]
; CHECK-NEXT:    [[CMP24:%.*]] = fcmp ogt float [[VECEXT2]], 1.000000e+00
; CHECK-NEXT:    [[OR_COND1:%.*]] = or i1 [[OR_COND]], [[CMP24]]
; CHECK-NEXT:    [[CMP29:%.*]] = fcmp ogt float [[VECEXT7]], 1.000000e+00
; CHECK-NEXT:    [[OR_COND2:%.*]] = or i1 [[OR_COND1]], [[CMP29]]
; CHECK-NEXT:    br i1 [[OR_COND2]], label [[RETURN]], label [[LOR_LHS_FALSE31:%.*]]
; CHECK:       lor.lhs.false31:
; CHECK-NEXT:    [[CMP34:%.*]] = fcmp ogt float [[VECEXT12]], 1.000000e+00
; CHECK-NEXT:    [[ADD:%.*]] = fadd float [[VECEXT]], [[VECEXT2]]
; CHECK-NEXT:    [[TMP0:%.*]] = select i1 [[CMP34]], float 0.000000e+00, float [[ADD]]
; CHECK-NEXT:    br label [[RETURN]]
; CHECK:       return:
; CHECK-NEXT:    [[RETVAL_0:%.*]] = phi float [ [[TMP0]], [[LOR_LHS_FALSE31]] ], [ 0.000000e+00, [[LOR_LHS_FALSE11]] ], [ 0.000000e+00, [[LOR_LHS_FALSE6]] ], [ 0.000000e+00, [[LOR_LHS_FALSE]] ], [ 0.000000e+00, [[ENTRY:%.*]] ]
; end INTEL_CUSTOMIZATION
; CHECK-NEXT:    ret float [[RETVAL_0]]
;
entry:
...
; CHECK-NEXT:    [[TMP1:%.*]] = bitcast <4 x i1> [[TMP0]] to i4
; CHECK-NEXT:    [[TMP2:%.*]] = icmp eq i4 [[TMP1]], 0
; CHECK-NEXT:    br i1 [[TMP2]], label [[RETURN:%.*]], label [[IF_END:%.*]] ;INTEL
; CHECK:       if.end: ;INTEL
; CHECK-NEXT:    [[TMP3:%.*]] = fcmp ule <4 x float> [[T_FR]], <float 1.000000e+00, float 1.000000e+00, float 1.000000e+00, float 1.000000e+00>
; CHECK-NEXT:    [[TMP4:%.*]] = bitcast <4 x i1> [[TMP3]] to i4
; CHECK-NEXT:    [[TMP5:%.*]] = icmp eq i4 [[TMP4]], 0
...
; CHECK-NEXT:    br label [[RETURN]]
; CHECK:       return:
; CHECK-NEXT:    [[RETVAL_0:%.*]] = phi float [ 0.000000e+00, [[ENTRY:%.*]] ], [ [[TMP7]], [[IF_END]] ]
; end INTEL_CUSTOMIZATION
; CHECK-NEXT:    ret float [[RETVAL_0]]
;
entry:
...
; CHECK-NEXT:    [[OR_COND:%.*]] = or i1 [[CMP14]], [[CMP18]]
; CHECK-NEXT:    [[CMP23:%.*]] = fcmp ogt float [[VECEXT2]], 1.000000e+00
; CHECK-NEXT:    [[OR_COND1:%.*]] = or i1 [[OR_COND]], [[CMP23]]
; CHECK-NEXT:    [[CMP28:%.*]] = fcmp ogt float [[VECEXT7]], 1.000000e+00
; CHECK-NEXT:    [[OR_COND2:%.*]] = or i1 [[OR_COND1]], [[CMP28]]
; CHECK-NEXT:    br i1 [[OR_COND2]], label [[RETURN]], label [[LOR_LHS_FALSE30:%.*]]
; CHECK:       lor.lhs.false30:
; CHECK-NEXT:    [[CMP33:%.*]] = fcmp ogt float [[VECEXT12]], 1.000000e+00
; CHECK-NEXT:    [[ADD:%.*]] = fadd float [[VECEXT]], [[VECEXT2]]
; CHECK-NEXT:    [[TMP0:%.*]] = select i1 [[CMP33]], float 0.000000e+00, float [[ADD]]
; CHECK-NEXT:    br label [[RETURN]]
; CHECK:       return:
; CHECK-NEXT:    [[RETVAL_0:%.*]] = phi float [ 0.000000e+00, [[LOR_LHS_FALSE11]] ], [ 0.000000e+00, [[LOR_LHS_FALSE6]] ], [ 0.000000e+00, [[LOR_LHS_FALSE]] ], [ 0.000000e+00, [[ENTRY:%.*]] ], [ [[TMP0]], [[LOR_LHS_FALSE30]] ]
; end INTEL_CUSTOMIZATION
; CHECK-NEXT:    ret float [[RETVAL_0]]
;
entry:
...
; INTEL_CUSTOMIZATION
; CHECK-NEXT:    [[TMP7:%.*]] = select i1 [[TMP5]], float 0.000000e+00, float [[CONV]]
; CHECK-NEXT:    br label [[RETURN]]
; CHECK:       return:
; CHECK-NEXT:    [[RETVAL_0:%.*]] = phi float [ [[TMP7]], [[LOR_LHS_FALSE]] ], [ 0.000000e+00, [[ENTRY:%.*]] ]
; end INTEL_CUSTOMIZATION
; CHECK-NEXT:    ret float [[RETVAL_0]]
;
entry:
...
; CHECK-NEXT:    [[ADD:%.*]] = extractelement <4 x i32> [[TMP6]], i64 0
; CHECK-NEXT:    [[CONV:%.*]] = sitofp i32 [[ADD]] to float
; CHECK-NEXT:    [[TMP7:%.*]] = select i1 [[DOTNOT]], float [[CONV]], float 0.000000e+00 ;INTEL
; CHECK-NEXT:    ret float [[TMP7]] ;INTEL
;
entry:
  %vecext = extractelement <4 x i32> %t, i32 0
  %cmp = icmp slt i32 %vecext, 1
...
; CHECK-NEXT:    [[TMP6:%.*]] = add nsw <4 x i32> [[T_FR]], [[SHIFT]]
; CHECK-NEXT:    [[ADD:%.*]] = extractelement <4 x i32> [[TMP6]], i64 0
; CHECK-NEXT:    [[TMP7:%.*]] = select i1 [[TMP5]], i32 0, i32 [[ADD]] ;INTEL
; CHECK-NEXT:    br label [[RETURN]]
; CHECK:       return:
; CHECK-NEXT:    [[RETVAL_0:%.*]] = phi i32 [ 0, [[ENTRY:%.*]] ], [ [[TMP7]], [[IF_END]] ] ;INTEL
...
; CHECK-NEXT:    [[TMP5:%.*]] = select i1 [[DOTNOT6]], i32 [[ADD]], i32 0 ;INTEL
; CHECK-NEXT:    br label [[RETURN]]
; CHECK:       return:
; CHECK-NEXT:    [[RETVAL_0:%.*]] = phi i32 [ 0, [[ENTRY:%.*]] ], [ [[TMP5]], [[IF_END]] ] ;INTEL
; CHECK-NEXT:    ret i32 [[RETVAL_0]]
;
entry:
  %vecext = extractelement <4 x i32> %t, i32 0
