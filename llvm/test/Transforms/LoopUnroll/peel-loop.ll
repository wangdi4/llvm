...
; RUN: opt < %s -S -passes='require<opt-remark-emit>,loop-unroll,loop-simplifycfg,instcombine' -unroll-force-peel-count=3 -verify-dom-info | FileCheck %s
; RUN: opt < %s -S -passes='require<opt-remark-emit>,loop-unroll<peeling;no-runtime>,loop-simplifycfg,instcombine' -unroll-force-peel-count=3 -verify-dom-info | FileCheck %s
