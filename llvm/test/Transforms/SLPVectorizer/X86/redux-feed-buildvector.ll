...
; INTEL_CUSTOMIZATION
; Customization note: add threshold to overcome FMA-breakage cost penalty, which community code
; does not account so far. There are also customizations for masked gather loads costs in TTI.
; RUN: opt < %s -mtriple=x86_64 -passes=slp-vectorizer -S -mcpu=skylake-avx512 -slp-threshold=-2 | FileCheck %s
