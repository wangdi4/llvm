...
; RUN: opt -force-opaque-pointers < %s -ip-cloning -ip-gen-cloning-force-enable-dtrans -inline -S 2>&1 | FileCheck %s
; RUN: opt -force-opaque-pointers < %s -passes='module(ip-cloning),cgscc(inline)' -ip-gen-cloning-force-enable-dtrans -S 2>&1 | FileCheck %s
