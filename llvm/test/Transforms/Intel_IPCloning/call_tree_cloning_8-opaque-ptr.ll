...
; RUN: opt < %s -opaque-pointers -passes='module(call-tree-clone)' -enable-intel-advanced-opts=1 -mtriple=i686-- -mattr=+avx2 -call-tree-clone-do-mv=false -S | FileCheck %s
; RUN: opt < %s -opaque-pointers -call-tree-clone -enable-intel-advanced-opts=1 -mtriple=i686-- -mattr=+avx2 -call-tree-clone-do-mv=false -S | FileCheck %s

; CHECK:call fastcc void @"foo.1|7"(), !dbg !61
;                                      ^^^^^^^^

; This is the same test case as call_tree_cloning_8.ll, but it checks for
; opaque pointers.

; ModuleID = 'call_tree_cloning_8-opaque-ptr.ll'
