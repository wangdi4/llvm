...
; RUN: opt -force-opaque-pointers < %s -S -ip-cloning -ip-spec-cloning-min-loops=2 -ip-gen-cloning-force-enable-dtrans -debug-only=ipcloning 2>&1 | FileCheck %s
; RUN: opt -force-opaque-pointers < %s -S -passes='module(ip-cloning)' -ip-spec-cloning-min-loops=2 -ip-gen-cloning-force-enable-dtrans -debug-only=ipcloning 2>&1 | FileCheck %s
