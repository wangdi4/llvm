...
; RUN: opt < %s -debug-only=call-tree-clone -call-tree-clone -ctcmv-model-user-calls=1 -ctcmv-num-user-calls-modeled=5201 -enable-intel-advanced-opts=1 -mtriple=i686-- -mattr=+avx2 -call-tree-clone-mv-bypass-coll-for-littest=0 -S -disable-output 2>&1 | FileCheck %s
; RUN: opt < %s -debug-only=call-tree-clone -passes='module(call-tree-clone)' -ctcmv-model-user-calls=1 -ctcmv-num-user-calls-modeled=5201 -enable-intel-advanced-opts=1 -mtriple=i686-- -mattr=+avx2 -call-tree-clone-mv-bypass-coll-for-littest=0 -S -disable-output 2>&1 | FileCheck %s
