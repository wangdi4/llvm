; Checks that the Multi-Versioning (MV) transformation triggers properly on get_ref(), even though the size of get_ref()
; is actually much larger than before.
;
; This verifies that multi-versioning of get_ref() won't depend on the size of get_ref().
;
; This also checks that the multi-versioning (MV) on get_ref() has proper code generation for
; - 2-variable clones;
; - 1-variable clones;
; and
...
; ModuleID = 'call_tree_cloning_11-opaque-ptr.ll'
...
; CHECK:  "get_ref|_._._.16.16"
; CHECK:  "get_ref|_._._.8.8"
; CHECK:  "get_ref|_._._.8.16"
; CHECK:  "get_ref|_._._.16.8"
; CHECK:  tail call fastcc void @get_ref(ptr %i12, i32 %i13, i32 %i14, i32 %i.010, i32 %sub)
...
; CHECK-LABEL:  define internal fastcc void @get_ref(ptr nocapture %i_dst_stride, i32 %mvx, i32 %mvy, i32 %i_width, i32 %i_height) unnamed_addr #1 {
;
; checks 4 2-variable clones
;
; CHECK:        %0 = icmp eq i32 %i_width, 8
; CHECK:        %1 = icmp eq i32 %i_height, 8
; CHECK:        call fastcc void @"get_ref|_._._.8.8"(ptr %i_dst_stride, i32 %mvx, i32 %mvy)
;
; CHECK:        %3 = icmp eq i32 %i_width, 16
; CHECK:        %4 = icmp eq i32 %i_height, 16
; CHECK:        call fastcc void @"get_ref|_._._.16.16"(ptr %i_dst_stride, i32 %mvx, i32 %mvy)
;
; CHECK:        %6 = icmp eq i32 %i_width, 16
; CHECK:        %7 = icmp eq i32 %i_height, 8
; CHECK:        call fastcc void @"get_ref|_._._.16.8"(ptr %i_dst_stride, i32 %mvx, i32 %mvy)
;
; CHECK:        %9 = icmp eq i32 %i_width, 8
; CHECK:        %10 = icmp eq i32 %i_height, 16
; CHECK:        call fastcc void @"get_ref|_._._.8.16"(ptr %i_dst_stride, i32 %mvx, i32 %mvy)
;
; checks 4 1-variable clones
;
; CHECK:        %12 = icmp eq i32 %i_width, 20
; CHECK:        call fastcc void @"get_ref|_._._.20._"(ptr %i_dst_stride, i32 %mvx, i32 %mvy, i32 %i_height)
;
; CHECK:        %13 = icmp eq i32 %i_width, 16
; CHECK:        call fastcc void @"get_ref|_._._.16._"(ptr %i_dst_stride, i32 %mvx, i32 %mvy, i32 %i_height)
;
; CHECK:        %14 = icmp eq i32 %i_width, 12
; CHECK:        call fastcc void @"get_ref|_._._.12._"(ptr %i_dst_stride, i32 %mvx, i32 %mvy, i32 %i_height)
;
; CHECK:        %15 = icmp eq i32 %i_width, 8
; CHECK:        call fastcc void @"get_ref|_._._.8._"(ptr %i_dst_stride, i32 %mvx, i32 %mvy, i32 %i_height)
;
;
; checks the default-fallthrough path
;
; CHECK:        call fastcc void @"get_ref|_._._._._"(ptr %i_dst_stride, i32 %mvx, i32 %mvy, i32 %i_width, i32 %i_height)
;
;
