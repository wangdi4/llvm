...
; RUN: opt -force-opaque-pointers < %s -ip-cloning -force-ip-manyreccalls-splitting -debug-only=ipcloning -S 2>&1 | FileCheck %s
; RUN: opt -force-opaque-pointers < %s -passes='module(ip-cloning)' -force-ip-manyreccalls-splitting -debug-only=ipcloning -S 2>&1 | FileCheck %s
...
; CHECK-DAG: MRCS:   %i173 = phi ptr [ %i145, %bb167 ], [ %i68, %bb78 ]
; CHECK-DAG: MRCS:   %i178 = getelementptr inbounds %struct._CacheInfo, ptr %i13, i64 0, i32 6
...
; CHECK: MRCS: Validated split insts:  %i173 = phi ptr [ %i145, %bb167 ], [ %i68, %bb78 ]
