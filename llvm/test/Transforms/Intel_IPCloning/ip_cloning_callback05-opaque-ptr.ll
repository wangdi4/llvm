...
; RUN: opt -force-opaque-pointers < %s -ip-cloning -ip-cloning-after-inl -ip-cloning-force-heuristics-off -S -debug-only=ipcloning 2>&1 | FileCheck %s
; RUN: opt -force-opaque-pointers < %s -passes='module(post-inline-ip-cloning)' -ip-cloning-force-heuristics-off -S -debug-only=ipcloning 2>&1 | FileCheck %s
