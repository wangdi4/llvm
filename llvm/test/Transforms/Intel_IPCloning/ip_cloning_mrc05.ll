; REQUIRES: asserts
; RUN: opt < %s -S -ip-manyreccalls-cloning-min-rec-callsites=2 -ip-cloning -debug-only=ipcloning -ip-manyreccalls-splitting=false 2>&1 | FileCheck %s
...
; cloning candidate, because there are not enough good SWITCH candidates

; Check the -ip-cloning trace output

; CHECK: MRC Cloning: Testing: foo
; CHECK: MRC Cloning: Skipping: Not enough good SWITCH candidates
...
if.then:                                          ; preds = %entry
  ...
  %call = call i32 @foo(i32 1, i32 1, i32 0)
  br label %return
