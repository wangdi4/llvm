...
; CHECK: Cloned callback in foo.1:{{.*}}@__kmpc_fork_call{{.*}}@foo.DIR.OMP.PARALLEL.LOOP.2.split5.[[R0:[0-9]+]]
; CHECK: Cloned callback in foo.2:{{.*}}@__kmpc_fork_call{{.*}}@foo.DIR.OMP.PARALLEL.LOOP.2.split5.[[R1:[0-9]+]]
