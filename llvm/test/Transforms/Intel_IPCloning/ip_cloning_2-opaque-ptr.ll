...
; RUN: opt -opaque-pointers < %s -ip-cloning -ip-cloning-after-inl -ip-cloning-loop-heuristic -debug-only=ipcloning -disable-output 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers < %s -passes='module(post-inline-ip-cloning)' -ip-cloning-loop-heuristic -debug-only=ipcloning -disable-output 2>&1 | FileCheck %s
