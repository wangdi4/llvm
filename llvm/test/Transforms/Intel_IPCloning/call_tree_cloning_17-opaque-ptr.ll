; Checks that the Call Tree Cloning transformation does not clone expected
; functions, when -call-tree-clone-max-direct-callsites is set explicitly,
; but the number is too low.

; RUN: opt < %s -opaque-pointers -passes='module(call-tree-clone)' -call-tree-clone-max-direct-callsites=5 -call-tree-clone-do-mv=false -enable-intel-advanced-opts=1 -mtriple=i686-- -mattr=+avx2 -S | FileCheck %s
; RUN: opt < %s -opaque-pointers -call-tree-clone -call-tree-clone-max-direct-callsites=5 -call-tree-clone-do-mv=false -enable-intel-advanced-opts=1 -mtriple=i686-- -mattr=+avx2 -S | FileCheck %s
...
; This is the same test case as call_tree_cloning_17.ll, but it checks for
; opaque pointers.

; ModuleID = 'call_tree_cloning_17-opaque-ptr.ll'
