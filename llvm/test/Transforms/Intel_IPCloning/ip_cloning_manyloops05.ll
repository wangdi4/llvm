; REQUIRES: asserts
; RUN: opt < %s -S -ip-cloning -ip-spec-cloning-min-loops=1 -ip-gen-cloning-force-enable-dtrans=false -debug-only=ipcloning 2>&1 | FileCheck %s
; RUN: opt < %s -S -passes='module(ip-cloning)' -ip-spec-cloning-min-loops=1 -ip-gen-cloning-force-enable-dtrans=false -debug-only=ipcloning 2>&1 | FileCheck %s

; Check that many loops specialization cloning was NOT performed, because
; DTRANS heuristics are not selected.

; CHECK-NOT: MLSC: Testing aer_rad_props_mp_aer_rad_props_sw_:
