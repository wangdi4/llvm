...
; RUN: opt -opaque-pointers < %s -ip-gen-cloning-enable-morphology -ip-cloning -ip-cloning-after-inl -ip-cloning-if-heuristic -ip-cloning-switch-heuristic -ip-gen-cloning-force-if-switch-heuristic -ip-gen-cloning-min-if-count=2 -ip-gen-cloning-min-switch-count=1 -mtriple=i686-- -mattr=+avx2 -S 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers < %s -ip-gen-cloning-enable-morphology -passes='module(post-inline-ip-cloning)' -ip-cloning-if-heuristic -ip-cloning-switch-heuristic -ip-gen-cloning-force-if-switch-heuristic -ip-gen-cloning-min-if-count=2 -ip-gen-cloning-min-switch-count=1 -mtriple=i686-- -mattr=+avx2 -S 2>&1 | FileCheck %s
