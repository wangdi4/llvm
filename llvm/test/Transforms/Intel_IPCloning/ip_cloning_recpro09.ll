...
; RUN: opt < %s -ip-rec-pro-cloning-skip-cpu-check -ip-cloning -ip-gen-cloning-force-enable-dtrans -debug-only=ipcloning -S 2>&1 | FileCheck %s
; RUN: opt < %s -ip-rec-pro-cloning-skip-cpu-check -passes='module(ip-cloning)' -ip-gen-cloning-force-enable-dtrans -debug-only=ipcloning -S 2>&1 | FileCheck %s
