...
; Check that GetVirtualPixelsFromNexus is not a candidate because it cannot
; find storebacks for more than one PHINode.

; CHECK: MRCS: START ANALYSIS: myoriginal
; CHECK: MRCS: EXIT: canSplitBlocks: No return value
...
@myalias = dso_local alias void (ptr), ptr @myoriginal
...
define void @myoriginal(ptr %arg) {
bb:
  ret void
}
