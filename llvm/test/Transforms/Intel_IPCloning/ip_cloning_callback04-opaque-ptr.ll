...
; RUN: opt -force-opaque-pointers < %s -ip-cloning -ip-cloning-after-inl -ip-cloning-force-heuristics-off -ip-gen-cloning-force-on-callback-cloning -S -debug-only=ipcloning 2>&1 | FileCheck %s
; RUN: opt -force-opaque-pointers < %s -passes='module(post-inline-ip-cloning)' -ip-cloning-force-heuristics-off -ip-gen-cloning-force-on-callback-cloning -S -debug-only=ipcloning 2>&1 | FileCheck %s

; Check that callback cloning is not attempted for @test because it is a
; varags function, even though -ip-gen-cloning-force-on-callback-cloning
; is specified.

; CHECK: Not attempting callback cloning for test
; CHECK: Cloned call:     %i8 = tail call i32 (i32, ...) @test.1(i32 2, i64 4294967296, i64 12884901890, i64 4294967296, i64 12884901890)
; CHECK: Cloned call:     %i3 = tail call i32 (i32, ...) @test.1(i32 2, i64 4294967296, i64 12884901890, i64 4294967296, i64 12884901890)
; CHECK: Cloned call:     %i = tail call i32 (i32, ...) @test.2(i32 1, i64 8589934592, i64 25769803780)
