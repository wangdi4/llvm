...
; RUN: opt -opaque-pointers < %s -ip-cloning -ip-gen-cloning-force-enable-dtrans -debug-only=ipcloning -enable-intel-advanced-opts=1 -mtriple=i686-- -mattr=+avx2 -S 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers < %s -passes='module(ip-cloning)' -ip-gen-cloning-force-enable-dtrans -debug-only=ipcloning -enable-intel-advanced-opts=1 -mtriple=i686-- -mattr=+avx2 -S 2>&1 | FileCheck %s
