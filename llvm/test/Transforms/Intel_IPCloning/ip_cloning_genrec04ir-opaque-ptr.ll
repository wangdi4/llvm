...
; RUN: opt -force-opaque-pointers < %s -ip-cloning -ip-gen-cloning-force-if-switch-heuristic -ip-gen-cloning-min-rec-callsites=4 -S 2>&1 | FileCheck %s
; RUN: opt -force-opaque-pointers < %s -passes='module(ip-cloning)' -ip-gen-cloning-force-if-switch-heuristic -ip-gen-cloning-min-rec-callsites=4 -S 2>&1 | FileCheck %s

; Test that foo is not selected for generic cloning of a recursive routine
; because it has more than one possible clone.
; This is the same test as ip_cloning_genrec04-opaque-ptr.ll, but checks for
