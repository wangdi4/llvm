...
; RUN: opt -force-opaque-pointers < %s -S -ip-cloning -ip-manyreccalls-splitting=false 2>&1 | FileCheck %s
; RUN: opt -force-opaque-pointers < %s -S -passes='module(ip-cloning)' -ip-manyreccalls-splitting=false 2>&1 | FileCheck %s
...
; This is the same test as ip_cloning_mrc02-opaque-ptr.ll, but checks for IR
