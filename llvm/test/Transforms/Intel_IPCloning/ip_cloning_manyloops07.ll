...
; RUN: opt -opaque-pointers=0 < %s -S -passes='module(ip-cloning)' -ip-spec-cloning-min-loops=1 -ip-gen-cloning-force-enable-dtrans -debug-only=ipcloning 2>&1 | FileCheck %s

; Check that many loops specialization cloning was performed. This is similar to
; ip_cloning_manyloops01.ll, but the cloned function has a return value.
...
; CHECK: define i32 @mymain(%"PHYSICS_TYPES$.btPHYSICS_STATE"* nonnull %0)
; CHECK: %[[S2:[0-9a-z]+]] = getelementptr %"PHYSICS_TYPES$.btPHYSICS_STATE", %"PHYSICS_TYPES$.btPHYSICS_STATE"* %0, i32 0, i32 1
; CHECK: %[[S3:[0-9a-z]+]] = load i32, i32* %[[S2]], align 4
; CHECK: %[[S4:[0-9a-z]+]] = icmp eq i32 %[[S3]], 4
; CHECK: br i1 %[[S4]], label %[[L5:[0-9]+]], label %[[L6:[0-9]+]]
; CHECK: [[L5]]:
; CHECK: %[[S5:[0-9a-z]+]] = call i32 @aer_rad_props_mp_aer_rad_props_sw_(i32* nonnull @anon, %"PHYSICS_TYPES$.btPHYSICS_STATE"* nonnull %0)
; CHECK:  br label %[[L7:[0-9]+]]
; CHECK: [[L6]]:
; CHECK: %[[S6:[0-9a-z]+]] = call i32 @aer_rad_props_mp_aer_rad_props_sw_.1(i32* nonnull @anon, %"PHYSICS_TYPES$.btPHYSICS_STATE"* nonnull %0)
; CHECK:  br label %[[L7]]
; CHECK: [[L7]]:
...
define i32 @mymain(%"PHYSICS_TYPES$.btPHYSICS_STATE"* nonnull %0) #0 {
  %t0 = call i32 @aer_rad_props_mp_aer_rad_props_sw_(i32* nonnull @anon, %"PHYSICS_TYPES$.btPHYSICS_STATE"* nonnull %0)
  ret i32 %t0
}
