...
; RUN: opt -force-opaque-pointers < %s -S -ip-cloning -ip-cloning-after-inl -ip-cloning-ivdep-min=2 2>&1 | FileCheck %s
; RUN: opt -force-opaque-pointers < %s -S -passes='module(post-inline-ip-cloning)' -ip-cloning-ivdep-min=2 2>&1 | FileCheck %s
