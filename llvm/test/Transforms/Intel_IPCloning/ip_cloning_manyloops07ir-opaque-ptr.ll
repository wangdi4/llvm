...
; Check that many loops specialization cloning was performed. This is similar to
; ip_cloning_manyloops07-opaque-ptr.ll, but only checks the IR.
...
; CHECK: define i32 @mymain(ptr nonnull %arg)
; CHECK: %[[S2:[0-9a-z]+]] = getelementptr %"PHYSICS_TYPES$.btPHYSICS_STATE", ptr %arg, i32 0, i32 1
; CHECK: %[[S3:[0-9a-z]+]] = load i32, ptr %[[S2]], align 4
; CHECK: %[[S4:[0-9a-z]+]] = icmp eq i32 %[[S3]], 4
; CHECK: br i1 %[[S4]], label %[[L5:[0-9]+]], label %[[L6:[0-9]+]]
; CHECK: [[L5]]:
; CHECK: %[[S5:[0-9a-z]+]] = call i32 @aer_rad_props_mp_aer_rad_props_sw_(ptr nonnull @anon, ptr nonnull %arg)
; CHECK:  br label %[[L7:[0-9]+]]
; CHECK: [[L6]]:
; CHECK: %[[S6:[0-9a-z]+]] = call i32 @aer_rad_props_mp_aer_rad_props_sw_.1(ptr nonnull @anon, ptr nonnull %arg)
; CHECK:  br label %[[L7]]
; CHECK: [[L7]]:
; CHECK: %.manyloops.phi = phi i32 [ %[[S6]], %[[L6]] ], [ %[[S5]], %[[L5]]
; CHECK:  ret i32 %.manyloops.phi

; Check that the original was optimized

; CHECK: define internal i32 @aer_rad_props_mp_aer_rad_props_sw_
...
; CHECK: define internal i32 @aer_rad_props_mp_aer_rad_props_sw_.1
...
define i32 @mymain(ptr nonnull %arg) #0 {
bb:
  %t0 = call i32 @aer_rad_props_mp_aer_rad_props_sw_(ptr nonnull @anon, ptr nonnull %arg)
  ret i32 %t0
}

define internal i32 @aer_rad_props_mp_aer_rad_props_sw_(ptr noalias nocapture readonly dereferenceable(4) %arg, ptr noalias nocapture readonly dereferenceable(20552) %arg1) #0 {
L0:
