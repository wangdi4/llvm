...
; RUN: opt -force-opaque-pointers < %s -S -ip-cloning -ip-spec-cloning-min-loops=1 -ip-gen-cloning-force-enable-dtrans=false 2>&1 | FileCheck %s
; RUN: opt -force-opaque-pointers < %s -S -passes='module(ip-cloning)' -ip-spec-cloning-min-loops=1 -ip-gen-cloning-force-enable-dtrans=false 2>&1 | FileCheck %s

; Check that many loops specialization cloning was NOT performed, because
; DTRANS heuristics are not selected.
; This test is similar to ip_cloning_manyloops05-opaque-ptr.ll, bot only checks
; the IR.
