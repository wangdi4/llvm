...
; This test is same as argument_align_9.ll except "null" is also stored into
; the malloced location in addition to return address of "calloc".

; RUN: opt < %s -intel-argument-alignment -whole-program-assume -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -debug-only=intel-argument-alignment -disable-output 2>&1 | FileCheck %s
; RUN: opt < %s -passes=intel-argument-alignment -whole-program-assume -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -debug-only=intel-argument-alignment -disable-output 2>&1
