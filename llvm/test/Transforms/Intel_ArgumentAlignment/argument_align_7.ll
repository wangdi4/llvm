...
; RUN: opt < %s -intel-argument-alignment -debug-only=intel-argument-alignment -S 2>&1 | FileCheck %s
; RUN: opt < %s -passes=intel-argument-alignment -debug-only=intel-argument-alignment  -S 2>&1
