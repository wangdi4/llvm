; This test checks that the argument alignment wasn't applied since it
...
; RUN: opt < %s -passes=intel-argument-alignment -whole-program-assume -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -S 2>&1

; CHECK: void @foo

; Check that first AND was converted
; CHECK-NOT: and i64 8, 7
; CHECK: and i64 %2, 7

; Check that second AND was converted
; CHECK-NOT: and i64 8, 7
; CHECK: and i64 %9, 7
...
entry:
  %2 = tail call noalias i8* @calloc(i64 %0, i64 9)
