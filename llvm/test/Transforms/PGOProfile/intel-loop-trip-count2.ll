; Test that a loop already annotated with the intel.loopcount metadata
; is not changed, even if the PGO data disagrees with the existing marking.

; RUN: llvm-profdata merge %S/Inputs/intel-loop-trip-count.proftext -o %t.profdata
; RUN: opt %s -S -pgo-looptc-annotate=true -passes=pgo-instr-use -pgo-test-profile-file=%t.profdata | FileCheck %s --check-prefix=CHECK
