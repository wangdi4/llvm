; Basic test for annotating loops with loop trip count data based on PGO
; value profiling.

; RUN: llvm-profdata merge %S/Inputs/intel-loop-trip-count.proftext -o %t.profdata
; RUN: opt %s -S -pgo-looptc-annotate=true -passes=pgo-instr-use -pgo-test-profile-file=%t.profdata | FileCheck %s --check-prefix=CHECK --check-prefix=ENABLED

; Use flag to inhibit updating the !loop metadata
; RUN: opt %s -S -pgo-looptc-annotate=false -passes=pgo-instr-use -pgo-test-profile-file=%t.profdata | FileCheck %s --check-prefix=CHECK --check-prefix=INHIBITED

; Use a minimum execution count to inhibit updating the !loop metadata
; RUN: opt %s -S -pgo-looptc-min-executions=40000000000 -passes=pgo-instr-use -pgo-test-profile-file=%t.profdata | FileCheck %s --check-prefix=CHECK --check-prefix=INHIBITED

; Use a maximum trip count value to inhibit updating the !loop metadata
; RUN: opt %s -S -pgo-looptc-max-trip-count=3 -passes=pgo-instr-use -pgo-test-profile-file=%t.profdata | FileCheck %s --check-prefix=CHECK --check-prefix=INHIBITED
...
; ENABLED: ![[LOOP_MD]] = distinct !{![[LOOP_MD]], ![[LOOP_MUST_PROGRESS:[0-9]+]], ![[LOOP_COUNT:[0-9]+]]}
; ENABLED: ![[LOOP_COUNT]] = !{!"llvm.loop.intel.loopcount", i32 4}
; INHIBITED: ![[LOOP_MD]] = distinct !{![[LOOP_MD]], ![[LOOP_MUST_PROGRESS:[0-9]+]]}
