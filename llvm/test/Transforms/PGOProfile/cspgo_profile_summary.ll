...
; 1) xmain has replaced the SimplifyCFG function FoldTwoEntryPHINode
; with the function FoldPHIEntries. This replacement function is
; unable to preserve the variable names when replacing a PHI node
; with a select instruction. This test could be updated to start
; matching the IR starting with the 'select' to overcome this for
; the affected CHECK lines.
; 2) This test is taking previously created profile data from the
; input file Inputs/cspgo.proftext, however the passes run in xmain
; prior to the PGO instrumentation/feedback phase for the context
; sensitive PGO have some differences from the community version.
; This causes the CFG to be different, preventing profile data feedback.
; There is not a good mechanism to recreate and maintain profile
; data for the xmain passes for this test, as the complete original
...
>>>>>>> 1299945b81284680829d39b2839087dc71f3d176
