...
; RUN: opt %s -mtriple=x86_64-- -codegenprepare -S -o - | FileCheck %s ;INTEL
@g = global i32 0
@effect = global i32 0
...
; CHECK-NEXT:    ]
; CHECK:       case_13:
; CHECK-NEXT:    br label [[DOTSPLIT9:%.*]] ;INTEL
; CHECK:       .split9: ;INTEL
; CHECK-NEXT:    [[MERGE11:%.*]] = phi i32 [ [[X]], [[CASE_13]] ], [ [[X_LOOPBACK:%.*]], [[CASE_7]] ] ;INTEL
; CHECK-NEXT:    store i32 13, ptr @effect, align 4
; CHECK-NEXT:    br label [[DOTSPLIT6:%.*]] ;INTEL
; CHECK:       case_42:
; INTEL_CUSTOMIZATION
...
; CHECK-NEXT:  bb0:
; CHECK-NEXT:    br i1 [[C:%.*]], label [[BB1:%.*]], label [[DOTSPLIT:%.*]] ;INTEL
; CHECK:       bb1:
; CHECK-NEXT:    [[COND:%.*]] = icmp eq i32 [[X:%.*]], 13
; CHECK-NEXT:    br i1 [[COND]], label [[CASE_13:%.*]], label [[DEFAULT:%.*]] ;INTEL
; CHECK:       case_13:
; INTEL_CUSTOMIZATION
; CHECK-NEXT:    br label [[DOTSPLIT]]
; CHECK:       .split:
...
; CHECK-NEXT:    ]
; CHECK:       case_13:
; CHECK-NEXT:    br label [[DOTSPLIT4:%.*]] ;INTEL
; CHECK:       .split4: ;INTEL
; CHECK-NEXT:    [[MERGE6:%.*]] = phi i64 [ [[TMP0]], [[CASE_13]] ], [ [[X7:%.*]], [[CASE_7]] ] ;INTEL
; CHECK-NEXT:    store i64 13, ptr @effect64, align 4
; CHECK-NEXT:    br label [[DOTSPLIT1:%.*]] ;INTEL
; CHECK:       case_42:
; INTEL_CUSTOMIZATION
; CHECK-NEXT:    br label [[DOTSPLIT1]]
; CHECK:       .split1:
