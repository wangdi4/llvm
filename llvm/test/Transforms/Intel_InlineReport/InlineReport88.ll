...
; RUN: opt -enable-new-pm=0 -wholeprogramanalysis -inline -inline-report=0xe807 --whole-program-assume-read -lto-inline-cost -dtrans-inline-heuristics -inline-expose-local-arrays-min-args=2 -inline-expose-local-arrays-min-calls=2 < %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-BEFORE
; RUN: opt -passes='require<wholeprogram>,cgscc(inline)' -inline-report=0xe807 --whole-program-assume-read -lto-inline-cost -dtrans-inline-heuristics -inline-expose-local-arrays-min-args=2 -inline-expose-local-arrays-min-calls=2 < %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-BEFORE
; RUN: opt -inlinereportsetup -inline-report=0xe886 < %s -S | opt -enable-new-pm=0 -wholeprogramanalysis -whole-program-assume-read -inline -lto-inline-cost -inline-report=0xe886 -dtrans-inline-heuristics -inline-expose-local-arrays-min-args=2 -inline-expose-local-arrays-min-calls=2 -S | opt -inlinereportemitter -inline-report=0xe886 -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-AFTER
; RUN: opt -passes='inlinereportsetup' -inline-report=0xe886 < %s -S | opt -passes='require<wholeprogram>,cgscc(inline)' -whole-program-assume-read -lto-inline-cost -inline-report=0xe886 -dtrans-inline-heuristics -inline-expose-local-arrays-min-args=2 -inline-expose-local-arrays-min-calls=2 -S | opt -inlinereportemitter -inline-report=0xe886 -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-AFTER
...
define dso_local void @MAIN__() local_unnamed_addr #0 {
  %1 = alloca [100 x [100 x [100 x float]]], align 8
  %2 = alloca [100 x [100 x [100 x float]]], align 8
  %3 = getelementptr inbounds [100 x [100 x [100 x float]]], [100 x [100 x [100 x float]]]* %1, i64 0, i64 0, i64 0, i64 0
  %4 = getelementptr inbounds [100 x [100 x [100 x float]]], [100 x [100 x [100 x float]]]* %2, i64 0, i64 0, i64 0, i64 0
  tail call fastcc void @mycopy_(float* %3, float* %4)
  tail call fastcc void @mycopy_(float* %3, float* %4)
  ret void
}
