...
; RUN: opt -passes='inlinereportsetup' -inline-report=0x86 < %s -S | opt -passes='require<wholeprogram>,cgscc(inline)' -whole-program-assume-read -lto-inline-cost -inline-report=0x86 -dtrans-inline-heuristics -inline-expose-local-arrays-min-args=2 -inline-expose-local-arrays-min-calls=2 -S | opt -inlinereportemitter -inline-report=0x86 -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-AFTER
