; RUN: opt -O3 -inline-report=0x807 < %s -disable-output 2>&1 | FileCheck %s

; Check that at -O3, if we select the standard inlining report for the legacy
; pass manager, that we will get multiple inlining reports.
