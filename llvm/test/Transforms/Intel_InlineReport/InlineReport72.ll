; RUN: opt -O3 -inline-report=7 -inline-threshold=-100 < %s -S 2>&1 | FileCheck %s
; RUN: opt -passes='default<O3>' -inline-report=7 -inline-threshold=-100 < %s -S 2>&1 | FileCheck %s
; RUN: opt -passes='inlinereportsetup' -inline-report=0x86 < %s -S | opt -passes='default<O3>' -inline-report=0x86 -inline-threshold=-100 -S | opt -passes='inlinereportemitter' -inline-report=0x86 -disable-output 2>&1 | FileCheck %s
