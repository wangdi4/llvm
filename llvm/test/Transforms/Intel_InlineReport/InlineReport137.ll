; RUN: opt -opaque-pointers=0 -passes='function(mem2reg),print<inline-report>' -disable-output -inline-report=0xea07 < %s 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers=0 -passes='inlinereportsetup,function(mem2reg),inlinereportemitter' -inline-report=0xea86 -S < %s 2>&1 | FileCheck %s
