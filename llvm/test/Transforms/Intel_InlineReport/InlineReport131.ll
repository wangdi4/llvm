...
; RUN: opt -passes=inline -pre-lto-inline-cost -inlining-huge-lto-odr-bb-count=0 -inline-threshold=0 -sycl-host -sycl-optimization-mode -S -inline-report=0xf867 < %s 2>&1 | FileCheck %s --check-prefixes=CHECK-SM,CHECK-SM-CL
; RUN: opt -passes=inline -pre-lto-inline-cost -inlining-huge-lto-odr-bb-count=0 -inline-threshold=0 -sycl-host -S -inline-report=0xf867 < %s 2>&1 | FileCheck %s --check-prefixes=CHECK-SM,CHECK-SM-CL
; RUN: opt -passes=inline -pre-lto-inline-cost -inlining-huge-lto-odr-bb-count=0 -inline-threshold=0 -sycl-optimization-mode -S -inline-report=0xf867 < %s 2>&1 | FileCheck %s --check-prefixes=CHECK-SM,CHECK-SM-CL
