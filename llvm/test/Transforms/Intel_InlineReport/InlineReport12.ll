...
; Inline report
; RUN: opt -inline -inline-report=0xe807 -new-double-callsite-inlining-heuristics=true < %s -S 2>&1 | FileCheck %s
; RUN: opt -passes='cgscc(inline)' -inline-report=0xe807 -new-double-callsite-inlining-heuristics=true < %s -S 2>&1 | FileCheck %s
; Inline report via metadata
; RUN: opt -inlinereportsetup -inline-report=0xe886 < %s -S | opt -inline -inline-report=0xe886 -new-double-callsite-inlining-heuristics=true -S | opt -inlinereportemitter -inline-report=0xe886 -S 2>&1 | FileCheck %s
; RUN: opt -passes='inlinereportsetup' -inline-report=0xe886 < %s -S | opt -passes='cgscc(inline)' -inline-report=0xe886 -S -new-double-callsite-inlining-heuristics=true | opt -passes='inlinereportemitter' -inline-report=0xe886 -S 2>&1 | FileCheck %s
