; RUN: opt -passes='module(pgo-icall-prom),inline' -disable-output -inline-report=0xf847 < %s 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-CL
; RUN: opt -passes='inlinereportsetup,module(pgo-icall-prom),inline,inlinereportemitter' -disable-output -inline-report=0xf8c6 < %s 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-MD
