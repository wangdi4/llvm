; Inline report
; RUN: opt -opaque-pointers -inline -inline-report=1 < %s -disable-output 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers -passes='cgscc(inline)' -inline-report=1 < %s -disable-output 2>&1 | FileCheck %s
; Inline report via metadata
; RUN: opt -opaque-pointers -inlinereportsetup -inline-report=0x80 < %s -S | opt -inline -inline-report=0x80 -S | opt -inlinereportemitter -inline-report=0x80 -disable-output 2>&1 | FileCheck %s
