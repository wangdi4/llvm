; RUN: opt -opaque-pointers -inline -inline-report=0x7 < %s -S 2>&1 | FileCheck --check-prefixes=CHECK-OLD,CHECK-CODE-BEFORE %s
...
; RUN: opt -opaque-pointers -inlinereportsetup -inline-report=0xe886 < %s -S | opt -inline -inline-report=0xe886 -S | opt -inlinereportemitter -inline-report=0xe886 -S 2>&1 | FileCheck --check-prefixes=CHECK-NEW,CHECK-CODE-AFTER  %s
