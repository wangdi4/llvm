...
; when inner function does not meet criteria (Triad 0 test fails).
...
; CHECK-MD: trial_move_smc{{.*}}Inlining is not profitable
...
; CHECK-MD: add_bond_forces{{.*}}Inlining is not profitable
; CHECK-MD: add_bond_forces{{.*}}Inlining is not profitable
...
; CHECK-MD: call void @trial_move_smc
...
; CHECK-MD: call void @add_bond_forces
; CHECK-MD: call void @add_bond_forces
...
entry:
  ...
  call void @add_bond_forces(ptr %p, i64 0, i32 %ibead, double %add15, double %add16, double %add17, ptr nonnull %nfx, ptr nonnull %nfy, ptr nonnull %nfz)
