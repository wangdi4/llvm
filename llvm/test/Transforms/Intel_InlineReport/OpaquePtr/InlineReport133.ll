; RUN: opt -opaque-pointers < %s -inline -inline-threshold=225 -inlinehint-threshold=325 -enable-new-pm=0 -inline-report=0xf847 -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-CL
; RUN: opt -opaque-pointers < %s -passes='cgscc(inline)' -inline-threshold=225 -inlinehint-threshold=325 -inline-report=0xf847 -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-CL
; RUN: opt -opaque-pointers -inlinereportsetup -inline-report=0xf8c6 < %s -S | opt -inline -inline-threshold=225 -inlinehint-threshold=325 -S -enable-new-pm=0 -inline-report=0xf8c6 -S | opt -inlinereportemitter -inline-report=0xf8c6 -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-MD
; RUN: opt -opaque-pointers -passes='inlinereportsetup' -inline-report=0xf8c6 < %s -S | opt -passes='cgscc(inline)' -inline-threshold=225 -inlinehint-threshold=325 -inline-report=0xf8c6 -S | opt -passes='inlinereportemitter' -inline-report=0xef8c6 -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-MD
