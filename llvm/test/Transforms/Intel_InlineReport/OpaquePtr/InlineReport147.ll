...
; RUN: opt -opaque-pointers -passes='cgscc(inline)' -lto-inline-cost -inlining-dyn-alloca-adjustable=true -inline-report=0xe807 -disable-output < %s 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers -passes='inlinereportsetup' -inline-report=0xe886 < %s -S | opt -passes='cgscc(inline)' -lto-inline-cost -inlining-dyn-alloca-adjustable=true -inline-report=0xe886 -S | opt -passes='inlinereportemitter' -inline-report=0xe886 -S 2>&1 | FileCheck %s
