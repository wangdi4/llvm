...
; Check that all instances of @daxpy are inlined due to the inline budget and
; single callsite local linkage heuristics.

; CHECK-BEFORE-NOT: call{{.*}}daxpy_
; CHECK: INLINE: daxpy_{{.*}}Has inline budget for small application
; CHECK: INLINE: daxpy_{{.*}}Has inline budget for small application
; CHECK: INLINE: daxpy_{{.*}}Has inline budget for small application
; CHECK: INLINE: daxpy_{{.*}}Callee has single callsite and local linkage
; CHECK-AFTER-NOT: call{{.*}}daxpy_
...
bb54:                                             ; preds = %bb54, %bb53
  ...
  call fastcc void @daxpy_(ptr nonnull %i2, ptr nonnull %i, ptr nonnull %i51, ptr %i63)
  %i64 = add i64 %i55, 1
...
bb18:                                             ; preds = %bb14, %bb4
  ...
  call fastcc void @daxpy_(ptr nonnull %i1, ptr nonnull %i, ptr %i23, ptr %i24)
