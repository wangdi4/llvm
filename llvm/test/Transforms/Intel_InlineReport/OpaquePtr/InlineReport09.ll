...
; Inline report
; RUN: opt -opaque-pointers -inline -inline-threshold=15 -inline-report=0xe807 < %s -disable-output 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers -passes='cgscc(inline)' -inline-report=0xe807 < %s -disable-output 2>&1 | FileCheck %s
; Inline report via metadata
; RUN: opt -opaque-pointers -inlinereportsetup -inline-report=0xe886 < %s -S | opt -inline -inline-report=0xe886 -inline-threshold=15 -S | opt -inlinereportemitter -inline-report=0xe886 -inline-threshold=15 -S 2>&1 | FileCheck %s
