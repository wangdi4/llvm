; RUN: opt -opaque-pointers -inline -argpromotion -inline-report=0xe807 < %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK-NO-MD
...
; RUN: opt -opaque-pointers -inlinereportsetup -inline-report=0xe886 < %s -S | opt -inline -argpromotion -inline-report=0xe886 -S | opt  -inlinereportemitter -inline-report=0xe886 -S 2>&1 | FileCheck %s --check-prefixes=CHECK-MD
; RUN: opt -opaque-pointers -passes='inlinereportsetup' -inline-report=0xe886 < %s -S | opt -passes='cgscc(inline)',argpromotion -inline-report=0xe886 -S | opt -passes='inlinereportemitter' -inline-report=0xe886 -S 2>&1 | FileCheck %s --check-prefixes=CHECK-MD
...
; CHECK-NO-MD-NEXT:   %.val = load float, ptr %2, align 4
...
define internal float @bar(ptr %0) #0 {
  %2 = load float, ptr %0
  ret float %2
}
