...
; Inline report
; RUN: opt -opaque-pointers -passes='require<wholeprogram>,lto<O3>' -whole-program-assume-read -inline-report=0xe807 -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 < %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-BEFORE
; Inline report via metadata
; RUN: opt -opaque-pointers -inlinereportsetup -inline-report=0xe886 < %s -S | opt -passes='require<wholeprogram>,lto<O3>' -whole-program-assume-read -lto-inline-cost -inline-report=0xe886 -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -S | opt -inlinereportemitter -inline-report=0xe886 -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-AFTER
...
; callsite local linkage heuristic.

; This test is similar to InlineReport66.ll, but checks that with lto<O3> that LinkForLTO is true.

; CHECK-BEFORE-NOT: call{{.*}}@perdida_m_mp_perdida_
; CHECK-BEFORE-NOT: call{{.*}}@perdida_m_mp_generalized_hookes_law_
; CHECK: INLINE: perdida_m_mp_perdida_{{.*}}Callee has single callsite and local linkage
; CHECK-DAG: INLINE: perdida_m_mp_generalized_hookes_law_{{.*}}Callee has single callsite and local linkage
; CHECK-DAG: INLINE: perdida_m_mp_generalized_hookes_law_{{.*}}Has inline budget for small application
