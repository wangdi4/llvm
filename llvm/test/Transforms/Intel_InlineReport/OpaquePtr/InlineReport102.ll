...
; RUN: opt -opaque-pointers < %s -S -inline -inline-report=0xe807 -pre-lto-inline-cost -min-callee-args-double-external=4 -min-callee-ivdep-loops-double-external=2 2>&1 | FileCheck --check-prefixes=CHECK,CHECK-OLD %s
...
; RUN: opt -opaque-pointers -inlinereportsetup -inline-report=0xe886 < %s -S | opt -S -inline -inline-report=0xe886 -pre-lto-inline-cost -min-callee-args-double-external=4 -min-callee-ivdep-loops-double-external=2 | opt -inlinereportemitter -inline-report=0xe886 -S 2>&1 | FileCheck --check-prefixes=CHECK,CHECK-NEW %s
