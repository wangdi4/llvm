...
; RUN: opt -opaque-pointers -passes='inlinereportsetup' -inline-report=0x2898 < %s -S | opt -passes='cgscc(inline)' -inline-report=0x2898 -S | opt -passes='inlinereportemitter' -inline-report=0x2898 -S 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers -passes='inlinereportsetup' -inline-report=0xf8d8 < %s -S | opt -passes='cgscc(inline)' -inline-report=0xf8d8 -S | opt -passes='inlinereportemitter' -inline-report=0xf8d8 -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-IND
