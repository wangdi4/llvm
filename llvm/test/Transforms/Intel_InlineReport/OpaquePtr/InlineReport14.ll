...
; Inline report
; RUN: opt -opaque-pointers -new-double-callsite-inlining-heuristics=true -inline -inline-report=0xe807 < %s -S 2>&1 | FileCheck %s
...
; RUN: opt -opaque-pointers -inlinereportsetup -inline-report=0xe886 < %s -S | opt -inline -new-double-callsite-inlining-heuristics=true -inline-report=0xe886 -S | opt -inlinereportemitter -inline-report=0xe886 -S 2>&1 | FileCheck %s
