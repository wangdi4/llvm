; RUN: opt < %s -passes='sample-profile,print<inline-report>' -sample-profile-file=%S/Inputs/InlineReport160.prof -sample-profile-prioritized-inline -profile-sample-accurate -sample-profile-use-preinliner=0 -disable-output -inline-report=0xf847 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-CL,CHECK-OFF
; RUN: opt < %s -passes='sample-profile,print<inline-report>' -sample-profile-file=%S/Inputs/InlineReport160.prof -sample-profile-prioritized-inline -profile-sample-accurate -sample-profile-use-preinliner=1 -disable-output -inline-report=0xf847 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-CL,CHECK-ON
; RUN: opt < %s -passes='inlinereportsetup,sample-profile,inlinereportemitter' -sample-profile-file=%S/Inputs/InlineReport160.prof -sample-profile-prioritized-inline -profile-sample-accurate -sample-profile-use-preinliner=0 -disable-output -inline-report=0xf8c6 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-MD,CHECK-OFF
; RUN: opt < %s -passes='inlinereportsetup,sample-profile,inlinereportemitter' -sample-profile-file=%S/Inputs/InlineReport160.prof -sample-profile-prioritized-inline -profile-sample-accurate -sample-profile-use-preinliner=1 -disable-output -inline-report=0xf8c6 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-MD,CHECK-ON

; Test the output of the classic and metadata inlining report reports with sample based profiling.
; Both standard and context-sensitive sample based profiling are tested.

; CHECK-CL: COMPILE FUNC: _Z5funcBi
; CHECK-CL: _Z8funcLeafi {{.*}}Sample-based inlining is not beneficial
; CHECK: COMPILE FUNC: main
; CHECK: _Z5funcBi {{.*}}Callee has noinline attribute
; CHECK: INLINE: _Z5funcAi {{.*}}Sample-based inlining is beneficial
; CHECK-OFF: _Z8funcLeafi {{.*}}Sample-based inlining is not beneficial
; CHECK-ON: _Z8funcLeafi {{.*}}CS-sample-based inlining is beneficial
; CHECK-MD: COMPILE FUNC: _Z5funcBi
; CHECK-MD: _Z8funcLeafi {{.*}}Sample-based inlining is not beneficial
