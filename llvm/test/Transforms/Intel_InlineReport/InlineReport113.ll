; RUN: opt < %s -disable-output -inline -simplifycfg -inline-report=0xe807 2>&1 | FileCheck %s 
; RUN: opt < %s -disable-output -passes='cgscc(inline),simplifycfg' -inline-report=0xe807 2>&1 | FileCheck %s 
; RUN: opt -inlinereportsetup -inline-report=0xe886 < %s -S | opt -inline -simplifycfg -inline-report=0xe886 -S | opt -inlinereportemitter -inline-report=0xe886 -S 2>&1 | FileCheck %s
