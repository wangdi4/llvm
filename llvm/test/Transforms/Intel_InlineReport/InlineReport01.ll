; Inline report
; RUN: opt -opaque-pointers -passes='cgscc(inline)' -inline-report=0x2801 < %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-INLREP
; Inline report via metadata
; RUN: opt -opaque-pointers -passes='inlinereportsetup,cgscc(inline),inlinereportemitter' -inline-report=0x2880 < %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-MD-INLREP
