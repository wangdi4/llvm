...
; Inline report
; RUN: opt -opaque-pointers -passes='lto-pre-link<O3>' -whole-program-assume-read -lto-inline-cost -inline-report=0xe807 -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 < %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK-PRE,CHECK-PRE-BEFORE
; RUN: opt -opaque-pointers -passes='lto<O3>' -whole-program-assume-read -lto-inline-cost -inline-report=0xe807 -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 < %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK-POST,CHECK-POST-BEFORE
; Inline report via metadata
; RUN: opt -opaque-pointers -passes='lto-pre-link<O3>' -whole-program-assume-read -lto-inline-cost -inline-report=0xe886 -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -S < %s 2>&1 | FileCheck %s --check-prefixes=CHECK-PRE,CHECK-PRE-AFTER
; RUN: opt -opaque-pointers -passes='lto<O3>' -whole-program-assume-read -lto-inline-cost -inline-report=0xe886 -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -S < %s 2>&1 | FileCheck %s --check-prefixes=CHECK-POST,CHECK-POST-AFTER
...
; in the new pass manager, when inlining is applied on the link step, but
; not on the compile step.

; CHECK-PRE-BEFORE: call{{.*}}sw_IP_ddx_
; CHECK-PRE-BEFORE: call{{.*}}sw_IP_ddy_
; CHECK-PRE-DAG: sw_IP_ddx_{{.*}}Inlining is not profitable
; CHECK-PRE-DAG: sw_IP_ddy_{{.*}}Inlining is not profitable
; CHECK-PRE-DAG: sw_IP_ddx_{{.*}}Inlining is not profitable
; CHECK-PRE-DAG: sw_IP_ddy_{{.*}}Inlining is not profitable
; CHECK-PRE-AFTER: call{{.*}}sw_IP_ddx_
; CHECK-PRE-AFTER: call{{.*}}sw_IP_ddy_

; CHECK-POST-BEFORE-NOT: call{{.*}}sw_IP_ddx_
; CHECK-POST-BEFORE-NOT: call{{.*}}sw_IP_ddy_
; CHECK-POST-DAG: INLINE: sw_IP_ddx_{{.*}}Has inline budget for small application
; CHECK-POST-DAG: INLINE: sw_IP_ddy_{{.*}}Has inline budget for small application
; CHECK-POST-DAG: INLINE: sw_IP_ddx_{{.*}}Callee has single callsite and local linkage
; CHECK-POST-DAG: INLINE: sw_IP_ddy_{{.*}}Callee has single callsite and local linkage
; CHECK-POST-AFTER-NOT: call{{.*}}sw_IP_ddx_
; CHECK-POST-AFTER-NOT: call{{.*}}sw_IP_ddy_
...
L1161:                                            ; preds = %L1161, %entry
  ...
  call fastcc void @sw_IP_ddx_(ptr nonnull %t2, ptr nonnull %t177, ptr nonnull %t180)
  call fastcc void @sw_IP_ddy_(ptr nonnull %t2, ptr nonnull %t179, ptr nonnull %t182)
