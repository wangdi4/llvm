...
; RUN: opt -wholeprogramanalysis -inline -inline-report=0xe807 --whole-program-assume-read -lto-inline-cost -dtrans-inline-heuristics -inline-expose-local-arrays-min-args=2 < %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-BEFORE
...
; RUN: opt -inlinereportsetup -inline-report=0xe886 < %s -S | opt -wholeprogramanalysis -whole-program-assume-read -inline -lto-inline-cost -inline-report=0xe886 -dtrans-inline-heuristics -inline-expose-local-arrays-min-args=2 -S | opt -inlinereportemitter -inline-report=0xe886 -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-AFTER
