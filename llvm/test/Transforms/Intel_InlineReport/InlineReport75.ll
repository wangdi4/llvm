...
; RUN: opt -passes='cgscc(inline)' -disable-output -inline-report=0xe807 -dtrans-inline-heuristics -intel-libirc-allowed -pre-lto-inline-cost < %s 2>&1 | FileCheck %s
; RUN: opt -passes='inlinereportsetup,cgscc(inline),inlinereportemitter' -disable-output -inline-report=0xe886 -dtrans-inline-heuristics -intel-libirc-allowed -pre-lto-inline-cost < %s 2>&1 | FileCheck %s
