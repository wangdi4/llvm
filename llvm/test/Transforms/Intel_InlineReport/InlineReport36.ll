...
; Inline report
; RUN: opt < %s -enable-new-pm=0 -dtrans-inline-heuristics -inline -inline-report=0xe807 -S 2>&1 | FileCheck --check-prefixes=CHECK,CHECK-OLD %s
; RUN: opt < %s -dtrans-inline-heuristics -passes='cgscc(inline)' -inline-report=0xe807 -S 2>&1 | FileCheck --check-prefixes=CHECK,CHECK-NEW %s
; Inline report via metadata
; RUN: opt -inlinereportsetup -inline-report=0xe886 < %s -S | opt -enable-new-pm=0 -inline -inline-report=0xe886 -dtrans-inline-heuristics -S | opt -inlinereportemitter -inline-report=0xe886 -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-OLD
; RUN: opt -passes='inlinereportsetup' -inline-report=0xe886 < %s -S | opt -passes='cgscc(inline)' -inline-report=0xe886 -dtrans-inline-heuristics -S | opt -passes='inlinereportemitter' -inline-report=0xe886 -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-OLD
...
define dso_local i32 @foo(i32 %arg) personality i8* bitcast (i32 (...)* @__gxx_personality_v0 to i8*) {
entry:
...
for.end:                                          ; preds = %for.cond
  %t6 = load i32, i32* %s, align 4
  %t7 = bitcast i32* %s to i8*
  call void @llvm.lifetime.end.p0i8(i64 4, i8* %t7) #2
  %t8 = bitcast i32* %i to i8*
  call void @llvm.lifetime.end.p0i8(i64 4, i8* %t8) #2
  ret i32 %t6
...
for.body:                                         ; preds = %for.cond
  %3 = load i32, i32* %i, align 4
  %call = invoke i32 @foo(i32 %3) to label %ilabel1 unwind label %lpad
