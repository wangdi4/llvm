; RUN: opt <%s -S -inline -inline-report=7 2>&1 | FileCheck %s
; RUN: opt <%s -S -passes='cgscc(inline)' -inline-report=7 2>&1 | FileCheck %s
; RUN: opt -inlinereportsetup -inline-report=0x86 < %s -S | opt -inline -inline-report=0x86 -S | opt -inlinereportemitter -inline-report=0x86 -S 2>&1 | FileCheck %s
