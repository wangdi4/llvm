; RUN: opt -opaque-pointers -passes='require<anders-aa>,indirectcallconv,inline' -intel-ind-call-force-andersen -disable-output -inline-report=0xf847 < %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-CL
; RUN: opt -opaque-pointers -passes='require<anders-aa>,inlinereportsetup,indirectcallconv,inline,inlinereportemitter' -intel-ind-call-force-andersen -disable-output -inline-report=0xf8c6 < %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-MD
