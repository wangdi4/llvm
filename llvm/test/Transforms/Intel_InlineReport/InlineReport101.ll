; RUN: opt -opaque-pointers=0 -passes='cgscc(inline)',argpromotion -inline-report=0xe807 < %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK-NO-MD
; RUN: opt -opaque-pointers=0 -passes='inlinereportsetup,cgscc(inline),argpromotion,inlinereportemitter' -inline-report=0xe886 -S < %s 2>&1 | FileCheck %s --check-prefixes=CHECK-MD
