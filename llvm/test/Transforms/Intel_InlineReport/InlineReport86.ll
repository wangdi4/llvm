...
; RUN: opt -passes='inlinereportsetup,cgscc(inline),module(post-inline-ip-cloning),inlinereportemitter' -ip-cloning-force-heuristics-off -inline-report=0xe886 -S < %s 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-MD
