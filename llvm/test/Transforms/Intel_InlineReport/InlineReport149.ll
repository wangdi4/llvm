...
; RUN: opt -passes='cgscc(inline)' -inline-report=0xe807 -lto-inline-cost -inlining-dyn-alloca-special-arg-count=3 < %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-GOOD,CHECK-GOOD-CL
; RUN: opt -passes='inlinereportsetup,cgscc(inline),inlinereportemitter' -inline-report=0xe886 -lto-inline-cost -inlining-dyn-alloca-special-arg-count=3 < %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-GOOD,CHECK-GOOD-ML
; RUN: opt -passes='cgscc(inline)' -inline-report=0xe807 -lto-inline-cost -inlining-dyn-alloca-special-arg-count=4 < %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-BAD,CHECK-BAD-CL
; RUN: opt -passes='inlinereportsetup,cgscc(inline),inlinereportemitter' -inline-report=0xe886 -lto-inline-cost -inlining-dyn-alloca-special-arg-count=4 < %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-BAD,CHECK-BAD-ML
