...
; Inline report
; RUN: opt -enable-new-pm=0 -wholeprogramanalysis -whole-program-assume-read -instcombine -inline -lto-inline-cost -inline-report=0xe807 -forced-inline-opt-level=3  -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 < %s -S 2>&1 |FileCheck %s --check-prefixes=CHECK,CHECK-BEFORE
...
; RUN: opt -instcombine -inlinereportsetup -inline-report=0xe886 < %s -S | opt -enable-new-pm=0 -wholeprogramanalysis -whole-program-assume-read -inline -lto-inline-cost -inline-report=0xe886 -forced-inline-opt-level=3  -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -S | opt -inlinereportemitter -inline-report=0xe886 -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-AFTER
