...
; RUN: opt -enable-new-pm=0 -wholeprogramanalysis -inline -inline-report=0xe807 --whole-program-assume-read -lto-inline-cost -dtrans-inline-heuristics -inline-expose-local-arrays-min-args=2 -inline-expose-local-arrays-min-calls=2 -inline-expose-local-arrays-min-dims=4 < %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-BEFORE
...
; RUN: opt -inlinereportsetup -inline-report=0xe886 < %s -S | opt -enable-new-pm=0 -wholeprogramanalysis -whole-program-assume-read -inline -lto-inline-cost -inline-report=0xe886 -dtrans-inline-heuristics -inline-expose-local-arrays-min-args=2 -inline-expose-local-arrays-min-calls=2 -inline-expose-local-arrays-min-dims=4 -S | opt -inlinereportemitter -inline-report=0xe886 -S 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-AFTER
