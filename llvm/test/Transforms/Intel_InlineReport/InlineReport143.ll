; RUN: opt -opaque-pointers=0 -passes='function(instcombine),print<inline-report>' -disable-output -inline-report=0xea07 < %s 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers=0 -passes='inlinereportsetup,function(instcombine),inlinereportemitter' -inline-report=0xea86 -S < %s 2>&1 | FileCheck %s
