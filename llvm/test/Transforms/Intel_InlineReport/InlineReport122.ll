...
; RUN: opt < %s -dtrans-inline-heuristics -passes='cgscc(inline)' -inline-report=0xf847 -S 2>&1 | FileCheck %s --check-prefix=CHECK-CL
; RUN: opt -passes='inlinereportsetup' -inline-report=0xf8c6  < %s -S | opt -dtrans-inline-heuristics -passes='cgscc(inline)' -inline-report=0xf8c6 -S | opt -passes='inlinereportemitter' -inline-report=0xf8c6 -S 2>&1 | FileCheck %s --check-prefix=CHECK-MD
