...
; RUN: opt -opaque-pointers -passes='inlinereportsetup,module(call-tree-clone),inlinereportemitter' -enable-intel-advanced-opts=1 -mtriple=i686-- -march=core-avx2 -call-tree-clone-mv-bypass-coll-for-littest=1 -inline-report=0xe886 < %s 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-MD
