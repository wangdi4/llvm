...
; CHECK-LABEL: @test3(
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[TMP0:%.*]] = load ptr, ptr [[X:%.*]], align 8 ;INTEL
; CHECK-NEXT:    br i1 [[F:%.*]], label [[IF_END57:%.*]], label [[IF_END57]]
; CHECK:       if.end57:
; CHECK-NEXT:    [[TMP1:%.*]] = phi ptr [ [[TMP0]], [[ENTRY:%.*]] ], [ [[TMP0]], [[ENTRY]] ] ;INTEL
; CHECK-NEXT:    [[TOBOOL59:%.*]] = icmp eq ptr [[TMP1]], null ;INTEL
...
; CHECK-LABEL: @test8(
; CHECK-NEXT:  ret2:
; CHECK-NEXT:    [[A:%.*]] = load i32, ptr [[TMP0:%.*]], align 4, !tbaa [[TBAA0]], !range [[RNG4:![0-9]+]], !noundef !5 ;INTEL
