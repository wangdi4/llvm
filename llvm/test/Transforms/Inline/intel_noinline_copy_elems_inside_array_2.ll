...
; RUN: opt < %s -disable-output -inline -inline-report=7 -dtrans-inline-heuristics -inline-for-xmain -pre-lto-inline-cost 2>&1 | FileCheck %s
; Same as above except using metadata
; RUN: opt < %s -disable-output -inlinereportsetup -inline -inline-report=0x86 -dtrans-inline-heuristics -inline-for-xmain -pre-lto-inline-cost -inlinereportemitter 2>&1 | FileCheck %s

; New PM command
; RUN: opt < %s -disable-output  -passes='cgscc(inline)' -inline-report=7 -dtrans-inline-heuristics -inline-for-xmain -pre-lto-inline-cost  2>&1 | FileCheck %s
; Same as above except using metadata
; RUN: opt < %s -disable-output -passes='inlinereportsetup,cgscc(inline),inlinereportemitter' -inline-report=0x86 -dtrans-inline-heuristics -inline-for-xmain -pre-lto-inline-cost 2>&1 | FileCheck %s
