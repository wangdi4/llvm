...
; RUN: opt < %s -whole-program-assume -intel-libirc-allowed -agginliner -inline -inline-report=0xe807 -inline-threshold=-50 -S 2>&1 | FileCheck %s
; RUN: opt < %s -whole-program-assume -intel-libirc-allowed -passes='module(agginliner),cgscc(inline)' -inline-report=0xe807 -inline-threshold=-50 -S 2>&1 | FileCheck %s
