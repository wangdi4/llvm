...
; RUN: opt -inline -inline-threshold=20 -inlining-for-deep-ifs=true -inlining-min-if-depth=4 -inline-report=0xe807 < %s -S 2>&1 | FileCheck %s
; RUN: opt -passes='cgscc(inline)' -inline-threshold=20 -inlining-for-deep-ifs=true -inlining-min-if-depth=4 -inline-report=0xe807 < %s -S 2>&1 | FileCheck %s
