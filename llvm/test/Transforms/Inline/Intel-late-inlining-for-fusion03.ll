...
; RUN: opt -passes='inlinereportsetup,cgscc(inline),inlinereportemitter' -pre-lto-inline-cost=false -inlining-for-fusion-heuristics=true -inline-threshold=20 -inline-for-fusion-min-arg-refs=3 -dtrans-inline-heuristics=false -inline-report=0xe886 -S < %s 2>&1 | FileCheck --check-prefix=CHECK-META %s
