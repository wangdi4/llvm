; REQUIRES: asserts
; RUN: opt < %s -whole-program-assume -agginliner -inline -debug-only=agginliner -inline-threshold=-50 -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -disable-output 2>&1 | FileCheck %s
; RUN: opt < %s -whole-program-assume -passes='module(agginliner),cgscc(inline)' -debug-only=agginliner -inline-threshold=-50 -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -disable-output 2>&1 | FileCheck %s
