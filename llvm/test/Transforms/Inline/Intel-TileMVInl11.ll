...
; RUN: opt < %s -S -intel-libirc-allowed=false -tilemvinlmarker -debug-only=tilemvinlmarker -tile-candidate-min=4 -tile-candidate-arg-min=3 -tile-candidate-sub-arg-min=2 -tile-candidate-mark=false -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 2>&1 | FileCheck %s
; RUN: opt < %s -S -intel-libirc-allowed=false -passes='tilemvinlmarker' -debug-only=tilemvinlmarker -tile-candidate-min=4 -tile-candidate-arg-min=3 -tile-candidate-sub-arg-min=2 -tile-candidate-mark=false -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 2>&1 | FileCheck %s
