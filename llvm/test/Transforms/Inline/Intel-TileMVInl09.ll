; REQUIRES: asserts
; RUN: opt < %s -S -tilemvinlmarker -tile-candidate-mark -debug-only=tilemvinlmarker -tile-candidate-test -tile-candidate-min=4 -tile-candidate-arg-min=2 -tile-candidate-sub-arg-min=2 2>&1 | FileCheck %s
; RUN: opt < %s -S -passes='tilemvinlmarker' -tile-candidate-mark -debug-only=tilemvinlmarker -tile-candidate-test -tile-candidate-min=4 -tile-candidate-arg-min=2 -tile-candidate-sub-arg-min=2 2>&1 | FileCheck %s
...
; CHECK: TMVINL: fun01_ Loop Index   %13 = add nuw nsw i64 %9, 1
; CHECK: TMVINL: fun01_ Loop Inc   %9 = phi i64 [ 3, %5 ], [ %13, %8 ]
; CHECK: TMVINL: fun01_ Arg %0(2,1)
; CHECK: TMVINL: Tile Candidate fun01_
; CHECK: TMVINL: fun00_ Loop Index   %13 = add nuw nsw i64 %9, 1
; CHECK: TMVINL: fun00_ Loop Inc   %9 = phi i64 [ 3, %5 ], [ %13, %8 ]
...
8:                                                ; preds = %4
  %load02a = load i32, i32* %2, align 4
  tail call fastcc void @fun00_(double* %0, double* %1, i32 %load02a)
  br label %10

9:                                                ; preds = %4
  %load02b = load i32, i32* %2, align 4
  tail call fastcc void @fun01_(double* %0, double* %1, i32 %load02b)
  br label %10
...
define internal fastcc void @fun01_(double* noalias nocapture %0, double* noalias nocapture readonly %1, i32 %2) unnamed_addr #0 {
  %4 = icmp slt i32 %2, 5
...
define internal fastcc void @fun00_(double* noalias nocapture %0, double* noalias nocapture readonly %1, i32 %2) unnamed_addr #0 {
  %4 = icmp slt i32 %2, 5
  br i1 %4, label %22, label %5
