...
; RUN: opt -inline -disable-inlined-alloca-merging=true -S < %s 2>&1 | FileCheck --check-prefixes=CHECK,CHECK-OFF %s
; RUN: opt -inline -disable-inlined-alloca-merging=false -S < %s 2>&1 | FileCheck --check-prefixes=CHECK,CHECK-ON %s
