...
; RUN: opt -passes='inlinereportsetup,cgscc(inline),inlinereportemitter' -pre-lto-inline-cost=false -inlining-for-fusion-heuristics=true -inline-threshold=20 -inline-for-fusion-min-arg-refs=3 -dtrans-inline-heuristics -intel-libirc-allowed=true -inline-for-fusion-small-app-function-limit=0 -inline-report=0xe886 -S < %s 2>&1 | FileCheck --check-prefix=CHECK-META %s
