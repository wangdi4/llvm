...
; RUN: opt -enable-new-pm=0 -wholeprogramanalysis -inline -pre-lto-inline-cost=false -inlining-for-fusion-heuristics=true -inline-threshold=20 -inline-for-fusion-min-arg-refs=3 -dtrans-inline-heuristics -intel-libirc-allowed -inline-report=0xe807 < %s -S 2>&1 | FileCheck --check-prefix=CHECK-OLD %s
; RUN: opt -passes='require<wholeprogram>,cgscc(inline)' -pre-lto-inline-cost=false -inlining-for-fusion-heuristics=true -inline-threshold=20 -inline-for-fusion-min-arg-refs=3 -dtrans-inline-heuristics -intel-libirc-allowed -inline-report=0xe807 < %s -S 2>&1 | FileCheck --check-prefix=CHECK-NEW %s
; RUN:  opt -inlinereportsetup -inline-report=0xe886 < %s -S | opt -enable-new-pm=0 -wholeprogramanalysis -inline -pre-lto-inline-cost=false -inlining-for-fusion-heuristics=true -inline-threshold=20 -inline-for-fusion-min-arg-refs=3 -dtrans-inline-heuristics -intel-libirc-allowed -inline-report=0xe886 -S  | opt -inlinereportemitter -inline-report=0xe886 -S 2>&1 | FileCheck --check-prefix=CHECK-META %s
; RUN: opt -inlinereportsetup -inline-report=0xe886 < %s -S | opt -passes='require<wholeprogram>,cgscc(inline)' -pre-lto-inline-cost=false -inlining-for-fusion-heuristics=true -inline-threshold=20 -inline-for-fusion-min-arg-refs=3 -dtrans-inline-heuristics -intel-libirc-allowed -inline-report=0xe886 | opt -inlinereportemitter -inline-report=0xe886 -S 2>&1 | FileCheck --check-prefix=CHECK-META %s

target triple = "x86_64-unknown-linux-gnu"

; Test link compile step loop fusion heuristic
; This test is similar to Intel-late-inlining-for-fusion01.ll, but also checks
; that when whole program analysis is run @foo will be marked with
; "prefer-function-level-region".

; Checks for old pass manager with old inline report

; CHECK-OLD: define void @bar_
; CHECK-OLD: define void @foo_{{.*}}#1
...
; CHECK-OLD: attributes #1 = { "prefer-function-level-region" }
...
; CHECK-NEW: attributes #1 = { "prefer-function-level-region" }
