...
; devirtualization with multiversioning already ran. This is the same test
; case as intel_partial_inline_virtual_funcs_off.ll, but it checks for
; opaque pointers.

; RUN: opt < %s -opaque-pointers -enable-new-pm=0 -partial-inliner -skip-partial-inlining-cost-analysis -partial-inline-virtual-functions -S 2>&1 | FileCheck %s
; RUN: opt < %s -opaque-pointers -passes='module(partial-inliner)' -skip-partial-inlining-cost-analysis -partial-inline-virtual-functions -S 2>&1 | FileCheck %s
...
; CHECK:   %tmp8 = tail call zeroext i1 @_ZN7Derived3fooEPvi(ptr %tmp4, ptr nonnull %call.i8, i32 10), !_Intel.Devirt.Call !26

; Call to Derived2::foo
; CHECK: BBDevirt__ZN8Derived23fooEPvi:
; CHECK:   %tmp9 = tail call zeroext i1 @_ZN8Derived23fooEPvi(ptr %tmp4, ptr nonnull %call.i8, i32 10), !_Intel.Devirt.Call !26
