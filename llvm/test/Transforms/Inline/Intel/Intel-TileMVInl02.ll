...
; RUN: opt -opaque-pointers < %s -S -passes='tilemvinlmarker' -debug-only=tilemvinlmarker -tile-candidate-test -tile-candidate-min=4 -tile-candidate-arg-min=3 -tile-candidate-sub-arg-min=2 2>&1 | FileCheck %s
...
; CHECK: TMVINL: fun3_ Loop Index   %i11 = add nuw nsw i64 %i7, 1
; CHECK: TMVINL: fun3_ Loop Inc   %i7 = phi i64 [ 2, %bb4 ], [ %i11, %bb6 ]
...
bb:
  ...
  tail call fastcc void @fun3_(ptr %arg, ptr %arg1, ptr %arg2)
...
define internal fastcc void @fun3_(ptr noalias nocapture readonly %arg, ptr noalias nocapture %arg1, ptr noalias nocapture readonly %arg2) unnamed_addr {
bb:
  %i = load i32, ptr %arg2, align 4
  %i3 = icmp slt i32 %i, 3
  br i1 %i3, label %bb20, label %bb4
