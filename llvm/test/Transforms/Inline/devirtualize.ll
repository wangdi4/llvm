;; INTEL specific RUN lines.  Added -inlineoptsize-threshold=75
; RUN: opt -S -inlineoptsize-threshold=75 -Os < %s | FileCheck %s
; RUN: opt -S -inlineoptsize-threshold=75 -aa-pipeline=basic-aa -passes='default<Os>' < %s | FileCheck %s
