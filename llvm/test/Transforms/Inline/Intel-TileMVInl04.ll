...
; RUN: opt -opaque-pointers=0 < %s -S -passes='tilemvinlmarker,cgscc(inline)' -tile-candidate-mark -debug-only=tilemvinlmarker -tile-candidate-test -tile-candidate-min=4 -tile-candidate-arg-min=3 -tile-candidate-sub-arg-min=2 2>&1 | FileCheck %s
