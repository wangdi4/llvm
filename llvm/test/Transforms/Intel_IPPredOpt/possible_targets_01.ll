...
; This test verifies that possible targets are collected for indirect virtual
; calls.
...
; Possible targets: Derived::foo and Derived2::foo

; RUN: opt < %s  -disable-output -ippred-dump-target-functions -whole-program-assume -enable-intel-advanced-opts=1 -mtriple=i686-- -mattr=+avx2 -passes=ippredopt 2>&1 | FileCheck %s

; CHECK: main  --    %call
; CHECK-DAG:        _ZN7Derived3fooEi
; CHECK-DAG:        _ZN8Derived23fooEi
