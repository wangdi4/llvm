...
; RUN: opt < %s -passes='function(loop(loop-rotate),vpo-cfg-restructuring,vpo-paropt-prepare,loop-simplifycfg,loop(loop-simplifycfg),sroa,vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt,function(loop-simplifycfg)' -switch-to-offload  -S | FileCheck %s
