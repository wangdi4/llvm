; RUN: opt -opaque-pointers=1 -enable-new-pm=0 -switch-to-offload -vpo-cfg-restructuring -vpo-paropt -S -vpo-paropt-atomic-free-reduction-slm=false -vpo-paropt-atomic-free-reduction-par-global=true <%s | FileCheck -check-prefix=NOSLM %s
...
; RUN: opt -opaque-pointers=1 -enable-new-pm=0 -switch-to-offload -vpo-cfg-restructuring -vpo-paropt -S -vpo-paropt-atomic-free-reduction-slm=true -vpo-paropt-atomic-free-reduction-par-global=true <%s | FileCheck -check-prefix=SLM %s
