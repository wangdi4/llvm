; INTEL_CUSTOMIZATION
; REQUIRES: asserts
; RUN: opt -opaque-pointers=1 -enable-new-pm=0 -vpo-cfg-restructuring -vpo-paropt-apply-config -vpo-paropt-config=%S/Inputs/Intel_thread_limit_config.yaml -debug-only=vpo-paropt-apply-config -S %s 2>&1 | FileCheck %s
