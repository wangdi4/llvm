...
; RUN: opt < %s -passes='function(loop(loop-rotate),vpo-cfg-restructuring,vpo-paropt-prepare,vpo-restore-operands,loop-simplifycfg,loop(loop-simplifycfg),sroa,vpo-cfg-restructuring),vpo-paropt,function(loop-simplifycfg)' -switch-to-offload -S -pass-remarks=vpo-paropt-transform -pass-remarks-missed=vpo-paropt-transform 2>&1 | FileCheck %s
