...
; This test is used to check reduction with bool or operation + m128 type.
; #include <stdio.h>
; #include <xmmintrin.h>
; #include <float.h>
; #include <math.h>
...
; ModuleID = 'reduction_bool_or_m128.cpp'
source_filename = "reduction_bool_or_m128.cpp"
...
entry:
  ...
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.PARALLEL.LOOP"(), "QUAL.OMP.REDUCTION.OR"(<4 x float>* %v3), "QUAL.OMP.PRIVATE"(i32* %i), "QUAL.OMP.SHARED"(<4 x float>* %v4), "QUAL.OMP.NORMALIZED.IV"(i32* %.omp.iv), "QUAL.OMP.FIRSTPRIVATE"(i32* %.omp.lb), "QUAL.OMP.NORMALIZED.UB"(i32* %.omp.ub) ]
...
; CHECK-NEXT: %[[BOOL_VAL:[^,]+]] = select <4 x i1> %[[ORIG_BOOL]], <4 x i1> %[[ORIG_BOOL]], <4 x i1> %[[RED_BOOL]]
...
for.body:                                         ; preds = %for.cond
  %12 = load float*, float** %test, align 8
  %13 = load i32, i32* %i, align 4
  %idxprom = sext i32 %13 to i64
  %ptridx = getelementptr inbounds float, float* %12, i64 %idxprom
  %14 = load float, float* %ptridx, align 4
  %conv = fpext float %14 to double
  %sub = fsub double %conv, 1.000000e+00
  %conv6 = fpext double %sub to x86_fp80
  %15 = call x86_fp80 @llvm.fabs.f80(x86_fp80 %conv6)
  %cmp7 = fcmp ogt x86_fp80 %15, 0xK3FD78CBCCC096F508800
  br i1 %cmp7, label %if.then, label %if.end

if.then:                                          ; preds = %for.body
  %call8 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([8 x i8], [8 x i8]* @.str, i64 0, i64 0))
