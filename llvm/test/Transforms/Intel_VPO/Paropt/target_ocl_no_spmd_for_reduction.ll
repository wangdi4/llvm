; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -switch-to-offload -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -vpo-paropt-atomic-free-reduction-slm=true -S %s | FileCheck %s
; RUN: opt -opaque-pointers=0 -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare,vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt' -switch-to-offload -vpo-paropt-atomic-free-reduction-slm=true -S %s | FileCheck %s
...
; Check that SPMD mode was not applied:
...
define hidden spir_func i32 @foo() #0 {
entry:
...
omp.inner.for.body:                               ; preds = %omp.inner.for.cond
  ...
  %7 = load i32, i32 addrspace(4)* %s.ascast, align 4
  %add1 = add nsw i32 %7, %6
  store i32 %add1, i32 addrspace(4)* %s.ascast, align 4
...
omp.loop.exit:                                    ; preds = %omp.inner.for.end
  ...
  %9 = load i32, i32 addrspace(4)* %s.ascast, align 4
  ret i32 %9
}
...
!0 = !{i32 0, i32 2052, i32 55999569, !"foo", i32 3, i32 0, i32 0}
