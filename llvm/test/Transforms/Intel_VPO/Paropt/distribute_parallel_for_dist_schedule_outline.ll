; RUN: opt -opaque-pointers=1 -bugpoint-enable-legacy-pm -loop-rotate -vpo-cfg-restructuring -vpo-paropt-prepare -sroa -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -simplifycfg -S %s | FileCheck %s
; RUN: opt -opaque-pointers=1 -passes='function(loop(loop-rotate),vpo-cfg-restructuring,vpo-paropt-prepare,loop-simplify,sroa,vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt,function(simplifycfg)' -S %s | FileCheck %s
...
; PHB:
; CHECK: [[PHB]]:
; CHECK: call void @__kmpc_team_static_init_4({{.*}}, i32 %[[TID:[^,]+]], ptr %[[PTEAMISLAST:[^,]+]], ptr %[[PTEAMLB:[^,]+]], ptr %[[PTEAMUB:[^,]+]], ptr %[[PTEAMST:[^,]+]], i32 1, i32 55)
...
; CHECK: call void @__kmpc_for_static_init_4({{.*}}, i32 %[[TID]], i32 34, ptr %[[PISLAST:[^,]+]], ptr %[[PLB:[^,]+]], ptr %[[PUB:[^,]+]], ptr %[[PST:[^,]+]], i32 1, i32 1)
...
entry:
  ...
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.DISTRIBUTE.PARLOOP"(),
    "QUAL.OMP.DIST_SCHEDULE.STATIC"(i32 55),
