...
bb9:                                              ; preds = %bb7
  %func_result2 = call i32 @f_(ptr @1)
  br label %bb6

bb6:                                              ; preds = %bb9
  store i32 %func_result2, ptr %"omp_test_$R"
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.PARALLEL"(),
    "QUAL.OMP.REDUCTION.NEQV:TYPED"(ptr %"omp_test_$R", i32 0, i64 1) ]
  br label %bb10

; CHECK:  store i32 0, ptr %"omp_test_$R.red"
...
bb16:                                             ; preds = %bb14
  %func_result4 = call i32 @f_(ptr @2)
  br label %bb13

bb13:                                             ; preds = %bb16
  %xor = xor i32 %"omp_test_$R_fetch", %func_result4
  store i32 %xor, ptr %"omp_test_$R"
  call void @llvm.directive.region.exit(token %0) [ "DIR.OMP.END.PARALLEL"() ]
  br label %bb1

bb10:                                             ; preds = %bb6
  %"omp_test_$R_fetch" = load i32, ptr %"omp_test_$R"
  br label %bb11

; CRITICAL:  %{{[0-9]+}} = load i32, ptr %"omp_test_$R.red"
; FASTRED:  %{{[0-9]+}} = load i32, ptr %"omp_test_$R.fast_red"
; ALL-NEXT:  %{{[0-9]+}} = load i32, ptr %"omp_test_$R"
; ALL-NEXT:  %{{[0-9]+}} = xor i32 %{{[0-9]+}}, %{{[0-9]+}}
; ALL-NEXT:  store i32 %{{[0-9]+}}, ptr %"omp_test_$R"
...
define i32 @f_(ptr noalias %"f_$X") {
alloca:
...
bb19:                                             ; preds = %bb18
  ...
  store i32 %slct, ptr %"f_$F"
  br label %bb17

bb17:                                             ; preds = %bb19
  %"f_$F_fetch" = load i32, ptr %"f_$F"
  ret i32 %"f_$F_fetch"
}
