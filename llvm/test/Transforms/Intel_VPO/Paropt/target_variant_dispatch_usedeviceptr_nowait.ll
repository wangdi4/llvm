...
;   3. Omit code to release the target buffers (in the asynchronous case the
;       async_handler callback routine will release the buffers)

; 1a. Allocate AsyncObj (type: %__struct.AsyncObj) and create a pointer to it
;
; CHECK: [[ASYNCOBJ:%[a-zA-Z._0-9]+]] = call i8* @__kmpc_omp_task_alloc(%__struct.ident_t* @.kmpc_loc{{.*}}, i32 0, i32 16, i64 24, i64 16, i8* null)
...
DIR.OMP.TARGET.VARIANT.DISPATCH.1:                ; preds = %entry
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.TARGET.VARIANT.DISPATCH"(), "QUAL.OMP.USE_DEVICE_PTR"(float** @aaa, double** @bbb), "QUAL.OMP.NOWAIT"() ]
