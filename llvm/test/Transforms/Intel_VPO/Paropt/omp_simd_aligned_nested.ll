...
; CHECK:   call{{.*}} @__kmpc_fork_call({{.+}}, i32 4, ptr [[FOO_OUTLINED:@.+]], ptr {{.*}}, ptr {{.*}}, i64 {{.*}}, ptr {{.*}})
; CHECK: }

; CHECK: define internal void [[FOO_OUTLINED]](ptr %{{.+}}, ptr %{{.+}}, ptr [[AADDR:%.+]], ptr %{{.+}}, i64 %{{.+}}, ptr %{{.+}}) #{{[0-9]+}} {
; CHECK:   [[TOKEN:%.+]] = call token @llvm.directive.region.entry() [ "DIR.OMP.SIMD"(), "QUAL.OMP.ALIGNED:PTR_TO_PTR"(ptr null, i32 32)
;
; CHECK:   [[PTR:%.+]] = load ptr, ptr [[AADDR]]
; CHECK:   call void @llvm.assume(i1 true) [ "align"(ptr [[PTR]], i64 32) ]
;
