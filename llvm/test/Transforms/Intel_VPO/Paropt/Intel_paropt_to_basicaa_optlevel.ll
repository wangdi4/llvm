; INTEL_CUSTOMIZATION
; REQUIRES: asserts
; RUN: opt < %s -O0 -vpo-cfg-restructuring -vpo-paropt -debug-only=basicaa-opt-level  -S 2>&1 | FileCheck %s
...
; CHECK: BasicAAResult: using OptLevel = 2
; Check for the opt-level 0 is propagated from Paropt to basic-aa:
; CHECK-NEXT: BasicAAResult: using OptLevel = 0
