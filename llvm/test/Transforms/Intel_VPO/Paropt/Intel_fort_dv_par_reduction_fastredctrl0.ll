; INTEL_CUSTOMIZATION
; RUN: opt -bugpoint-enable-legacy-pm -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -vpo-paropt-fast-reduction-ctrl=0 -S %s | FileCheck %s
...
; CHECK: [[FAST_RED_DV_ARR_SIZE:[^ ]+]] = call i64 @_f90_dope_vector_init2(ptr [[FAST_RED_DV]], ptr %"foo_$A")
...
; CHECK: %"foo_$A.addr_a0$_fetch" = load ptr, ptr [[FAST_RED_DV]], align 1
...
; CHECK: {{[^ ]+}} = call i32 @__kmpc_reduce(ptr {{[^ ,]+}}, i32 {{[^ ,]+}}, i32 1, i32 120, ptr {{[^ ,]+}}, ptr @foo{{[^ ,]*}}tree_reduce{{[^ ,]*}}, ptr {{[^ ,]+}})
