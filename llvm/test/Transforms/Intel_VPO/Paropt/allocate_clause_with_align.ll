...
;                allocate(align(16),allocator(omp_large_cap_mem_alloc):array) allocate(align(8):scalar)
;    {
;       array[50] = scalar;
;    }
;    return 0;
...
;   %0 = call i8* @__kmpc_aligned_alloc(i32 %my.tid1, i64 8, i64 4, i64 %default_allocator)
;
; CHECK: [[TID:%[^ ]+]] = load i32, i32* %tid, align 4
; CHECK: [[DEFALLOC:%[^ ]+]] = call i64 @omp_get_default_allocator()
; CHECK-NEXT:  call i8* @__kmpc_aligned_alloc(i32 [[TID]], i64 8, i64 4, i64 [[DEFALLOC]])
...
;   %1 = call i8* @__kmpc_aligned_alloc(i32 %my.tid, i64 16, i64 400, i64 2)
;   %array.priv = bitcast i8* %1 to [100 x float]*
;
; CHECK: [[TID2:%[^ ]+]] = load i32, i32* %tid, align 4
; CHECK-NEXT:  call i8* @__kmpc_aligned_alloc(i32 [[TID2]], i64 16, i64 400, i64 2)
;
...
DIR.OMP.PARALLEL.2:                               ; preds = %DIR.OMP.PARALLEL.1
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.PARALLEL"(), "QUAL.OMP.PRIVATE"([100 x float]* %array), "QUAL.OMP.FIRSTPRIVATE"(float* %scalar), "QUAL.OMP.ALLOCATE"(i64 16, [100 x float]* %array, i32 2), "QUAL.OMP.ALLOCATE"(i64 8, float* %scalar) ]
