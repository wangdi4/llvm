...
entry:
; CHECK:  %{{[a-zA-Z._0-9]+}} = call i16 @__kmpc_atomic_fixed2_swp({{[^,]+}}, i32 %{{[a-zA-Z._0-9]+}}, ptr @x, i16 %{{[a-zA-Z._0-9]+}})
...
entry:
; CHECK:  %{{[a-zA-Z._0-9]+}} = call i16 @__kmpc_atomic_fixed2_add_cpt_fp({{[^,]+}}, i32 %{{[a-zA-Z._0-9]+}}, ptr @x, fp128 %{{[a-zA-Z._0-9]+}}, i32 0)
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.ATOMIC"(),
    "QUAL.OMP.CAPTURE"() ]
  fence acquire
  %1 = load i16, ptr @x, align 2
  ...
  %3 = load fp128, ptr @e, align 16
  %add = fadd fp128 %conv2, %3
  %conv3 = fptosi fp128 %add to i16
  store i16 %conv3, ptr @x, align 2
  fence release
  call void @llvm.directive.region.exit(token %0) [ "DIR.OMP.END.ATOMIC"() ]
  ret void
}
...
entry:
; CHECK:  %{{[a-zA-Z._0-9]+}} = call i16 @__kmpc_atomic_fixed2_add_cpt_fp({{[^,]+}}, i32 %{{[a-zA-Z._0-9]+}}, ptr @x, fp128 %{{[a-zA-Z._0-9]+}}, i32 1)
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.ATOMIC"(),
    "QUAL.OMP.CAPTURE"() ]
  fence acquire
  %1 = load i16, ptr @x, align 2
...
entry:
; CHECK: call void @__kmpc_atomic_fixed2_add_fp({{[^,]+}}, i32 %{{[a-zA-Z._0-9]+}}, ptr @x, fp128 %{{[a-zA-Z._0-9]+}})
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.ATOMIC"(),
    "QUAL.OMP.UPDATE"() ]
  fence acquire
  %1 = load i16, ptr @x, align 2
...
entry:
; CHECK: call void @__kmpc_atomic_fixed2_sub_rev_fp({{[^,]+}}, i32 %{{[a-zA-Z._0-9]+}}, ptr @x, fp128 %{{[a-zA-Z._0-9]+}})
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.ATOMIC"(),
    "QUAL.OMP.UPDATE"() ]
  fence acquire
  %1 = load fp128, ptr @e, align 16
  %2 = load i16, ptr @x, align 2
...
entry:
; CHECK:  %{{[a-zA-Z._0-9]+}} = call i16 @__kmpc_atomic_fixed2_rd({{[^,]+}}, i32 %{{[a-zA-Z._0-9]+}}, ptr @x)
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.ATOMIC"(),
    "QUAL.OMP.READ"() ]
  fence acquire
  %1 = load i16, ptr @x, align 2
  %conv = sitofp i16 %1 to fp128
  store fp128 %conv, ptr @e, align 16
  fence release
  call void @llvm.directive.region.exit(token %0) [ "DIR.OMP.END.ATOMIC"() ]
  ret void
}
...
entry:
; CHECK: call void @__kmpc_atomic_fixed2_wr({{[^,]+}}, i32 %{{[a-zA-Z._0-9]+}}, ptr @x, i16 %{{[a-zA-Z._0-9]+}})
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.ATOMIC"(),
    "QUAL.OMP.WRITE"() ]
  fence acquire
  %1 = load fp128, ptr @e, align 16
...
; CHECK: %{{[a-zA-Z._0-9]+}} = call x86_fp80 @__kmpc_atomic_float10_add_cpt({{[^,]+}}, i32 %{{[a-zA-Z._0-9]+}}, ptr %x.addr, x86_fp80 %{{[a-zA-Z._0-9]+}}, i32 0)
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.ATOMIC"(),
    "QUAL.OMP.CAPTURE"() ]
  fence acquire
  %1 = load x86_fp80, ptr %x.addr, align 16
  %conv = fptosi x86_fp80 %1 to i64
  store i64 %conv, ptr @v, align 8
  %2 = load fp128, ptr @e, align 16
  %3 = load x86_fp80, ptr %x.addr, align 16
  %conv1 = fpext x86_fp80 %3 to fp128
  %add = fadd fp128 %conv1, %2
  %conv2 = fptrunc fp128 %add to x86_fp80
  store x86_fp80 %conv2, ptr %x.addr, align 16
  fence release
  call void @llvm.directive.region.exit(token %0) [ "DIR.OMP.END.ATOMIC"() ]
; CHECK: %{{[a-zA-Z._0-9]+}} = call x86_fp80 @__kmpc_atomic_float10_mul_cpt({{[^,]+}}, i32 %{{[a-zA-Z._0-9]+}}, ptr %x.addr, x86_fp80 %{{[a-zA-Z._0-9]+}}, i32 0)
  %4 = call token @llvm.directive.region.entry() [ "DIR.OMP.ATOMIC"(),
    "QUAL.OMP.CAPTURE"() ]
  fence acquire
  %5 = load x86_fp80, ptr %x.addr, align 16
  %conv3 = fptosi x86_fp80 %5 to i64
  store i64 %conv3, ptr @v, align 8
  %6 = load fp128, ptr @e, align 16
  %7 = load x86_fp80, ptr %x.addr, align 16
  %conv4 = fpext x86_fp80 %7 to fp128
  %mul = fmul fp128 %conv4, %6
  %conv5 = fptrunc fp128 %mul to x86_fp80
  store x86_fp80 %conv5, ptr %x.addr, align 16
  fence release
  call void @llvm.directive.region.exit(token %4) [ "DIR.OMP.END.ATOMIC"() ]
; CHECK: %{{[a-zA-Z._0-9]+}} = call x86_fp80 @__kmpc_atomic_float10_sub_cpt({{[^,]+}}, i32 %{{[a-zA-Z._0-9]+}}, ptr %x.addr, x86_fp80 %{{[a-zA-Z._0-9]+}}, i32 0)
  %8 = call token @llvm.directive.region.entry() [ "DIR.OMP.ATOMIC"(),
    "QUAL.OMP.CAPTURE"() ]
  fence acquire
  %9 = load x86_fp80, ptr %x.addr, align 16
  %conv6 = fptosi x86_fp80 %9 to i64
  store i64 %conv6, ptr @v, align 8
  %10 = load fp128, ptr @e, align 16
  %11 = load x86_fp80, ptr %x.addr, align 16
  %conv7 = fpext x86_fp80 %11 to fp128
  %sub = fsub fp128 %conv7, %10
  %conv8 = fptrunc fp128 %sub to x86_fp80
  store x86_fp80 %conv8, ptr %x.addr, align 16
  fence release
  call void @llvm.directive.region.exit(token %8) [ "DIR.OMP.END.ATOMIC"() ]
; CHECK: %{{[a-zA-Z._0-9]+}} = call x86_fp80 @__kmpc_atomic_float10_div_cpt({{[^,]+}}, i32 %{{[a-zA-Z._0-9]+}}, ptr %x.addr, x86_fp80 %{{[a-zA-Z._0-9]+}}, i32 0)
  %12 = call token @llvm.directive.region.entry() [ "DIR.OMP.ATOMIC"(),
    "QUAL.OMP.CAPTURE"() ]
  fence acquire
  %13 = load x86_fp80, ptr %x.addr, align 16
  %conv9 = fptosi x86_fp80 %13 to i64
  store i64 %conv9, ptr @v, align 8
  %14 = load fp128, ptr @e, align 16
  %15 = load x86_fp80, ptr %x.addr, align 16
  %conv10 = fpext x86_fp80 %15 to fp128
  %div = fdiv fp128 %conv10, %14
  %conv11 = fptrunc fp128 %div to x86_fp80
  store x86_fp80 %conv11, ptr %x.addr, align 16
  fence release
