...
@.kmpc_loc.0.0.6 = private unnamed_addr global %struct.ident_t { i32 0, i32 838860802, i32 0, i32 0, i8* getelementptr inbounds ([22 x i8], [22 x i8]* @.source.0.0.5, i32 0, i32 0) }
@.source.0.0.7 = private unnamed_addr constant [22 x i8] c";unknown;unknown;0;0;;"
@.kmpc_loc.0.0.8 = private unnamed_addr global %struct.ident_t { i32 0, i32 838860802, i32 0, i32 0, i8* getelementptr inbounds ([22 x i8], [22 x i8]* @.source.0.0.7, i32 0, i32 0) }
...
entry:
  ...
  %tid.val = tail call i32 @__kmpc_global_thread_num(%struct.ident_t* nonnull @.kmpc_loc.0.0.8)
  ...
  %fork.test = tail call i32 @__kmpc_ok_to_fork(%struct.ident_t* nonnull @.kmpc_loc.0.0.6)
  %fork.test20 = icmp eq i32 %fork.test, 0
  br i1 %fork.test20, label %if.else.call.4, label %if.then.fork.4

if.then.fork.4:                                   ; preds = %entry
  call void (%struct.ident_t*, i32, void (i32*, i32*, ...)*, ...) @__kmpc_fork_call(%struct.ident_t* nonnull @.kmpc_loc.0.0.4, i32 4, void (i32*, i32*, ...)* bitcast (void (i32*, i32*, float*, i32*, double*, i32*)* @foo.DIR.OMP.PARALLEL.LOOP.2.split17 to void (i32*, i32*, ...)*), float* nonnull %p, i32* nonnull %.omp.lb, double* nonnull %q, i32* nonnull %.omp.ub)
...
; CHECK: call void @bar(i32 %{{.+}}, float %{{.+}}, double 5.000000e+00)
...
loop.region.exit:                                 ; preds = %omp.inner.for.body, %omp.inner.for.body.lr.ph
  call void @__kmpc_for_static_fini(%struct.ident_t* nonnull @.kmpc_loc.0.0.2, i32 %my.tid)
...
declare !callback !0 void @__kmpc_fork_call(%struct.ident_t*, i32, void (i32*, i32*, ...)*, ...) local_unnamed_addr

declare i32 @__kmpc_ok_to_fork(%struct.ident_t*) local_unnamed_addr
