; RUN: opt < %s -switch-to-offload -vpo-cfg-restructuring -vpo-paropt-optimize-data-sharing -S | FileCheck %s
; RUN: opt < %s -switch-to-offload -passes='vpo-cfg-restructuring,vpo-paropt-optimize-data-sharing' -S | FileCheck %s
;
; Original code:
;
;struct C {
...
define hidden spir_func void @_Z3foo1CS_(%struct.C addrspace(4)* %c1, %struct.C addrspace(4)* %c2) #0 {
entry:
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.TARGET"(), "QUAL.OMP.OFFLOAD.ENTRY.IDX"(i32 0), "QUAL.OMP.PRIVATE:NONPOD"(%struct.C addrspace(4)* %c1, %struct.C addrspace(4)* (%struct.C addrspace(4)*)* @_ZTS1C.omp.def_constr, void (%struct.C addrspace(4)*)* @_ZTS1C.omp.destr), "QUAL.OMP.FIRSTPRIVATE:NONPOD"(%struct.C addrspace(4)* %c2, void (%struct.C addrspace(4)*, %struct.C addrspace(4)*)* @_ZTS1C.omp.copy_constr, void (%struct.C addrspace(4)*)* @_ZTS1C.omp.destr), "QUAL.OMP.MAP.TO"(%struct.C addrspace(4)* %c2, %struct.C addrspace(4)* %c2, i64 4, i64 161, i8* null, i8* null) ]
...
define internal spir_func %struct.C addrspace(4)* @_ZTS1C.omp.def_constr(%struct.C addrspace(4)* %0) #2 {
entry:
  %retval = alloca %struct.C addrspace(4)*, align 8
  %retval.ascast = addrspacecast %struct.C addrspace(4)** %retval to %struct.C addrspace(4)* addrspace(4)*
  ...
  %1 = load %struct.C addrspace(4)*, %struct.C addrspace(4)* addrspace(4)* %retval.ascast, align 8
  ret %struct.C addrspace(4)* %1
}

; Function Attrs: convergent nounwind
define internal spir_func void @_ZTS1C.omp.destr(%struct.C addrspace(4)* %0) #2 {
entry:
  %.addr = alloca %struct.C addrspace(4)*, align 8
  %.addr.ascast = addrspacecast %struct.C addrspace(4)** %.addr to %struct.C addrspace(4)* addrspace(4)*
  store %struct.C addrspace(4)* %0, %struct.C addrspace(4)* addrspace(4)* %.addr.ascast, align 8, !tbaa !8
