...
; Check that 'vpo-paropt-loop-collapse' pass could handle rotated OpenMP loops.
...
; CHECK: [[IV1:%[0-9]+]] = load i32, ptr [[UNCOLLAPSED_IV_1]], align 4
; CHECK: [[UB1:%[0-9]+]] = load i32, ptr [[UNCOLLAPSED_UB_1]], align 4
; CHECK: [[CMP4:%cmp]] = icmp sle i32 [[IV1]], [[UB1]]
; CHECK: br i1 [[CMP4]], label %omp.uncollapsed.loop.lh, label %omp.uncollapsed.loop.end{{.*}}

; CHECK-LABEL: omp.uncollapsed.loop.lh:
; CHECK: br label %omp.uncollapsed.loop.lh.split

; CHECK-LABEL: omp.uncollapsed.loop.lh.split:   
...
; CHECK: br i1 %{{.*}}, label %omp.uncollapsed.loop.body7, label %omp.uncollapsed.loop.end_crit_edge
;
; CHECK-LABEL: omp.uncollapsed.loop.end_crit_edge:               ; preds = %omp.uncollapsed.loop.inc
; CHECK: br label %omp.uncollapsed.loop.end
...
; CHECK: br label %omp.collapsed.loop.cond
; 
; CHECK: omp.collapsed.loop.postexit:
; CHECK: br label %omp.uncollapsed.loop.end{{.*}}

; CHECK: omp.uncollapsed.loop.end{{.*}}:
...
entry:
  ...
  br i1 %cmp, label %omp.uncollapsed.loop.lh, label %omp.uncollapsed.loop.end16

omp.uncollapsed.loop.lh:                          ; preds = %entry
  br label %omp.uncollapsed.loop.body

omp.uncollapsed.loop.body:                        ; preds = %omp.uncollapsed.loop.inc12, %omp.uncollapsed.loop.lh
...
omp.uncollapsed.loop.inc:                         ; preds = %omp.body.continue
  ...
  br i1 %cmp11, label %omp.uncollapsed.loop.body7, label %omp.uncollapsed.loop.end_crit_edge
...
omp.uncollapsed.loop.inc12:                       ; preds = %omp.uncollapsed.loop.end
  ...
  br i1 %cmp14, label %omp.uncollapsed.loop.body, label %omp.uncollapsed.loop.end_crit_edge15

omp.uncollapsed.loop.end_crit_edge15:             ; preds = %omp.uncollapsed.loop.inc12
  br label %omp.uncollapsed.loop.end16

omp.uncollapsed.loop.end16:                       ; preds = %omp.uncollapsed.loop.end_crit_edge15, %entry
