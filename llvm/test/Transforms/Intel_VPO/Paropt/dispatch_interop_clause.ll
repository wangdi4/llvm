...
; RUN: opt -vpo-paropt-dispatch-codegen-version=1 -bugpoint-enable-legacy-pm -vpo-cfg-restructuring -vpo-paropt-prepare -S <%s | FileCheck %s -check-prefix=NCG
; RUN: opt -vpo-paropt-dispatch-codegen-version=1 -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare)' -S <%s | FileCheck %s -check-prefix=NCG
...
; CHECK: [[DEVICE:%[^ ]+]] = call i64 @omp_get_interop_int(ptr %i0, i32 -5, ptr null)
; CHECK: call i32 @__tgt_is_device_available(i64 [[DEVICE]], ptr inttoptr (i64 15 to ptr))

; Check that the interop obj '%i0' is passed to the variant function
; CHECK-DAG: call void @_Z7foo_gpuiPv(i32 123, ptr %i0)
