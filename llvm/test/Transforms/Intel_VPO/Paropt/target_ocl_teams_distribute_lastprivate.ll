...
; CHECK-DAG: store i32 0, ptr [[L0_IS_LAST_AI]]
; CHECK-DAG: store i32 0, ptr [[L1_IS_LAST_AI]]

; CHECK: load i32, ptr %loop0.lower.bnd
; CHECK: load i32, ptr %loop0.upper.bnd
; CHECK: [[L0_LB:%[a-zA-Z._0-9]+]] = load i32, ptr %loop0.lower.bnd
; CHECK: [[L0_UB:%[a-zA-Z._0-9]+]] = load i32, ptr %loop0.upper.bnd
...
; CHECK: store i32 [[L0_LI_VAL]], ptr [[L0_IS_LAST_AI]]

; CHECK: load i32, ptr %loop1.lower.bnd
; CHECK: load i32, ptr %loop1.upper.bnd
; CHECK: [[L1_LB:%[a-zA-Z._0-9]+]] = load i32, ptr %loop1.lower.bnd
; CHECK: [[L1_UB:%[a-zA-Z._0-9]+]] = load i32, ptr %loop1.upper.bnd
...
; CHECK: store i32 [[L1_LI_VAL]], ptr [[L1_IS_LAST_AI]]

; CHECK: [[TMP1:%[a-zA-Z._0-9]+]] = load i32, ptr [[L1_IS_LAST_AI]]
; CHECK: [[TMP2:%[a-zA-Z._0-9]+]] = load i32, ptr [[L0_IS_LAST_AI]]
...
; CHECK: [[LAST_THEN]]:
; CHECK: [[LAST_VAL:%[a-zA-Z._0-9]+]] = load i32, ptr [[LPRIV:%x.ascast.lpriv]]
; CHECK: store i32 [[LAST_VAL]], ptr addrspace(1) [[VAR:%x.ascast]]
...
entry:
  ...
  store i32 -1, ptr addrspace(4) %x.ascast, align 4
  ...
  store i32 100, ptr addrspace(4) %.omp.uncollapsed.ub2.ascast, align 4
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.TARGET"(),
    "QUAL.OMP.OFFLOAD.ENTRY.IDX"(i32 0),
    "QUAL.OMP.MAP.FROM"(ptr addrspace(4) %x.ascast, ptr addrspace(4) %x.ascast, i64 4, i64 34),
  ...
  %2 = call token @llvm.directive.region.entry() [ "DIR.OMP.DISTRIBUTE"(),
    "QUAL.OMP.LASTPRIVATE:TYPED"(ptr addrspace(4) %x.ascast, i32 0, i32 1),
...
omp.uncollapsed.loop.body7:                       ; preds = %omp.uncollapsed.loop.cond5
  ...
  store i32 %mul10, ptr addrspace(4) %x.ascast, align 4
...
omp.uncollapsed.loop.inc:                         ; preds = %omp.body.continue
  %13 = load i32, ptr addrspace(4) %.omp.uncollapsed.iv4.ascast, align 4
  %add11 = add nsw i32 %13, 1
  store i32 %add11, ptr addrspace(4) %.omp.uncollapsed.iv4.ascast, align 4
  br label %omp.uncollapsed.loop.cond5
...
omp.uncollapsed.loop.inc12:                       ; preds = %omp.uncollapsed.loop.end
  %14 = load i32, ptr addrspace(4) %.omp.uncollapsed.iv.ascast, align 4
  %add13 = add nsw i32 %14, 1
  store i32 %add13, ptr addrspace(4) %.omp.uncollapsed.iv.ascast, align 4
  br label %omp.uncollapsed.loop.cond
