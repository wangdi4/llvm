...
define void @foo_(ptr %"foo_$B") #0 {
alloca_0:
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.TARGET.DATA"(),
    "QUAL.OMP.USE_DEVICE_PTR"(ptr %"foo_$B"),
    "QUAL.OMP.MAP.TOFROM"(ptr %"foo_$B", ptr %"foo_$B", i64 40, i64 3, ptr null, ptr null) ]
...
; CHECK: %"foo_$B.updated.val" = load ptr, ptr [[GEP]]

; Check that call to outlined function for target data uses %array_device.new
; CHECK: call void @foo_.DIR.OMP.TARGET.DATA{{[^ ]+}}(ptr %"foo_$B.updated.val")

  %"foo_$B[]" = call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 0, i64 1, i64 4, ptr elementtype(i32) %"foo_$B", i64 2)
  store i32 20, ptr %"foo_$B[]", align 1
