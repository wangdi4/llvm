; RUN: opt -bugpoint-enable-legacy-pm -vpo-paropt-tpv -mem2reg -simplifycfg -S %s | FileCheck %s
; RUN: opt -passes="vpo-paropt-tpv,mem2reg,function(simplifycfg)" -S %s | FileCheck %s
...
; CHECK: @__tpv_ptr_b = internal global ptr null, align 64
; CHECK:  %{{.*}} = call ptr @__kmpc_threadprivate_cached({{.*}}@__tpv_ptr_a)
; CHECK:  %{{.*}} = call ptr @__kmpc_threadprivate_cached({{.*}}@__tpv_ptr_b)
