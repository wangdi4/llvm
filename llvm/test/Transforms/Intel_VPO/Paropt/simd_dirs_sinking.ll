...
; RUN: opt -opaque-pointers=0 -enable-new-pm=0 -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -simplifycfg -instcombine -hir-ssa-deconstruction -hir-temp-cleanup -hir-last-value-computation -hir-vplan-vec -disable-output -print-before=hir-vplan-vec %s 2>&1 | FileCheck %s
