...
; Check that we  map the global @a
; CHECK: %[[A_MAP_GEP:[^ ]+]] = getelementptr inbounds [1 x ptr], ptr %.offload_baseptrs, i32 0, i32 0
; CHECK: store ptr @a, ptr %[[A_MAP_GEP]], align 8
...
; CHECK: %[[A_UPDATED:[^ ]+]] = load ptr, ptr %[[A_MAP_GEP]], align 8
; CHECK: call void @[[OUTLINED_FUNC:main.DIR.OMP.TARGET.DATA[^ ]*]](ptr %[[A_UPDATED]])

; Check that the passed-in value of A is used in the outlined function.
; CHECK-DAG: define internal void @[[OUTLINED_FUNC]](ptr %[[A_PASS:[^ ]+]])
; CHECK: call i32 (ptr, ...) @printf(ptr @.str, ptr %[[A_PASS]])
...
entry:
  %call = call i32 (ptr, ...) @printf(ptr @.str, ptr @a)

  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.TARGET.DATA"(),
    "QUAL.OMP.USE_DEVICE_PTR"(ptr @a) ]

  %call1 = call i32 (ptr, ...) @printf(ptr @.str, ptr @a) #2
