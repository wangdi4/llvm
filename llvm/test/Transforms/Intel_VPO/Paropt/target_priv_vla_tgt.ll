; REQUIRES: asserts
; RUN: opt -opaque-pointers=1 -enable-new-pm=0 -switch-to-offload -vpo-paropt-target-capture-non-pointers-using-map-to=true -vpo-cfg-restructuring -vpo-paropt -debug-only=WRegionUtils,vpo-paropt-transform -S %s 2>&1 | FileCheck %s
