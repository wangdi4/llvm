; RUN: opt -opaque-pointers=1 -enable-new-pm=0 -switch-to-offload -vpo-cfg-restructuring -vpo-paropt -S -vpo-paropt-atomic-free-reduction-par-global=false -vpo-paropt-atomic-free-red-global-combiner-selector=last-team-inlined <%s | FileCheck -check-prefix=LASTTEAMINLINED %s
...
; RUN: opt -opaque-pointers=1 -enable-new-pm=0 -switch-to-offload -vpo-cfg-restructuring -vpo-paropt -S -vpo-paropt-atomic-free-reduction-par-global=false -vpo-paropt-atomic-free-red-global-combiner-selector=team-zero-rtl <%s | FileCheck -check-prefix=TEAMZERORTL %s
...
; RUN: opt -opaque-pointers=1 -enable-new-pm=0 -switch-to-offload -vpo-cfg-restructuring -vpo-paropt -S -vpo-paropt-atomic-free-reduction-par-global=false -vpo-paropt-atomic-free-red-global-combiner-selector=last-team-rtl <%s | FileCheck -check-prefix=LASTTEAMRTL %s
...
; LASTTEAMINLINED: store i32 %[[UPD_CNTR]], ptr addrspace(3) @.broadcast.ptr.__local, align 4

; LASTTEAMINLINED: master.thread.fallthru{{.*}}:
; LASTTEAMINLINED: %.new = load i32, ptr addrspace(3) @.broadcast.ptr.__local, align 4
...
; TEAMZERORTL: store i1 %[[UPD_CNTR]], ptr addrspace(3) @.broadcast.ptr.__local, align 1
; TEAMZERORTL: br label %master.thread.fallthru{{.*}}

; TEAMZERORTL: master.thread.fallthru{{[0-9]+}}:
; TEAMZERORTL: call spir_func void @_Z{{.*}}__spirv_ControlBarrieriii(i32 {{.*}}, i32 {{.*}}, i32 {{.*}})
; TEAMZERORTL: %.new = load i1, ptr addrspace(3) @.broadcast.ptr.__local, align 1
...
; LASTTEAMRTL: store i1 %[[UPD_CNTR]], ptr addrspace(3) @.broadcast.ptr.__local, align 1
; LASTTEAMRTL: br label %master.thread.fallthru{{.*}}

; LASTTEAMRTL: master.thread.fallthru{{[0-9]+}}:
; LASTTEAMRTL: call spir_func void @_Z{{.*}}__spirv_ControlBarrieriii(i32 {{.*}}, i32 {{.*}}, i32 {{.*}})
; LASTTEAMRTL: %.new = load i1, ptr addrspace(3) @.broadcast.ptr.__local, align 1
