; RUN: opt < %s -switch-to-offload -vpo-paropt-enable-device-simd-codegen -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt  -S | FileCheck %s
; RUN: opt < %s -switch-to-offload -vpo-paropt-enable-device-simd-codegen -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare,vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt' -S | FileCheck %s
...
; This test is identical to target_simd_priv_spir64.ll, except that it uses
; a "for" construct instead of "simd".

; Check that when explicit SIMD is enabled, the address space of the store to
; %x.priv is simplified (which doesn't happen in target_simd_priv_spir64.ll).

; CHECK-NOT: call token @llvm.directive.region.entry()
; CHECK: store float 0.000000e+00, float* %xtmp.ascast{{[^ ,]+}}, align 4
...
entry:
  ...
  %1 = call token @llvm.directive.region.entry() [ "DIR.OMP.LOOP"(), "QUAL.OMP.NORMALIZED.IV"(i32 addrspace(4)* %.omp.iv.ascast), "QUAL.OMP.NORMALIZED.UB"(i32 addrspace(4)* %.omp.ub.ascast), "QUAL.OMP.LINEAR:IV"(i32 addrspace(4)* %v.ascast, i32 1), "QUAL.OMP.PRIVATE"(float addrspace(4)* %xtmp.ascast) ]
