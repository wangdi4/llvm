...
; This is a version of target_fp_map_vla_tgt.ll, but the TYPED firstprivate
; clause uses the same size value as the alloca of the vla.

; In this case, the map clause will capture the VLA's size again. So the size will be
; captured twice to avoid host/device compilation mismatch, even though it's the same value.
...
; CHECK:     captureAndAddCollectedNonPointerValuesToSharedClause: Added implicit shared/map(to) clause for: 'ptr addrspace(4) [[SIZE_ADDR1:%n.val.addr.*]]'
