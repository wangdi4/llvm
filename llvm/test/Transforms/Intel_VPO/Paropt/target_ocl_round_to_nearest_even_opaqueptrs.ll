; RUN: opt -switch-to-offload -bugpoint-enable-legacy-pm -vpo-cfg-restructuring -vpo-paropt -S <%s | FileCheck %s --check-prefix=DEFAULT
; RUN: opt -switch-to-offload -passes='function(vpo-cfg-restructuring),vpo-paropt' -S <%s | FileCheck %s --check-prefix=DEFAULT
;
; RUN: opt -vpo-paropt-spirv-offload-rne -switch-to-offload -bugpoint-enable-legacy-pm -vpo-cfg-restructuring -vpo-paropt -S <%s | FileCheck %s --check-prefix=DEFAULT
; RUN: opt -vpo-paropt-spirv-offload-rne -switch-to-offload -passes='function(vpo-cfg-restructuring),vpo-paropt' -S <%s | FileCheck %s --check-prefix=DEFAULT
;
; RUN: opt --vpo-paropt-enable-device-simd-codegen=true -vpo-paropt-spirv-offload-rne -switch-to-offload -bugpoint-enable-legacy-pm -vpo-cfg-restructuring -vpo-paropt -S <%s | FileCheck %s --check-prefix=RNE
; RUN: opt --vpo-paropt-enable-device-simd-codegen=true -vpo-paropt-spirv-offload-rne -switch-to-offload -passes='function(vpo-cfg-restructuring),vpo-paropt' -S <%s | FileCheck %s --check-prefix=RNE
;
