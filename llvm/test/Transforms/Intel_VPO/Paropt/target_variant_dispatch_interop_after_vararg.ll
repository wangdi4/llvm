; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-paropt-put-interop-after-vararg=true -S <%s | FileCheck %s
; RUN: opt -opaque-pointers=0 -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare)' -vpo-paropt-put-interop-after-vararg=true -S <%s | FileCheck %s

; Check that target variant dispatch with interop works with vararg functions.
; This test is for the "-vpo-paropt-put-interop-after-vararg=true" behavior,
; where the interop obj is the last argument of the variant call, and the
; func decl doesn't have an explicit formal parameter for the interop obj
; (because it's part of the ... vararg list).
;
; // C source
; #include <stdio.h>
; #include <omp.h>
;
; void __attribute__((nothrow,noinline))  foo_gpu(int a, int b, ...) {
...
; ModuleID = 'target_variant_dispatch_interop_after_vararg.cpp'
source_filename = "target_variant_dispatch_interop_after_vararg.cpp"
...
define dso_local void @_Z7foo_gpuiiz(i32 noundef %a, i32 noundef %b, ...) #0 {
entry:
...
; variant call: interop obj is the last arg
; CHECK-DAG: call void (i32, i32, ...) @_Z7foo_gpuiiz(i32 111, i32 222, i32 333, i8* %interop.obj{{.*}})
...
attributes #2 = { mustprogress noinline nounwind optnone uwtable "approx-func-fp-math"="true" "frame-pointer"="all" "loopopt-pipeline"="light" "min-legal-vector-width"="0" "no-infs-fp-math"="true" "no-nans-fp-math"="true" "no-signed-zeros-fp-math"="true" "no-trapping-math"="true" "openmp-variant"="name:_Z7foo_gpuiiz;construct:target_variant_dispatch;arch:gen" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" "unsafe-fp-math"="true" }
