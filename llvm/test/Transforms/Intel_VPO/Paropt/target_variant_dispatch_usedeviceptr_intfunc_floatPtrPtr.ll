; RUN: opt -vpo-paropt-prepare -vpo-paropt-use-interop=false -vpo-paropt-use-mapper-api=false -vpo-paropt-parameter-homing=false -S < %s | FileCheck %s
; RUN: opt < %s -passes='function(vpo-paropt-prepare)' -vpo-paropt-use-interop=false -vpo-paropt-use-mapper-api=false -vpo-paropt-parameter-homing=false -S | FileCheck %s
