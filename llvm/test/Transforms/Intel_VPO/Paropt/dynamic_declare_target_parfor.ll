...
; RUN: opt < %s -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare,vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt' -S -switch-to-offload 2>&1 | FileCheck %s
...
; CHECK: warning:{{.*}}'parallel loop' construct, in a declare target function, was ignored for calls from target regions.
