; REQUIRES: asserts
; RUN: opt -opaque-pointers=0 -enable-new-pm=0 -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -S -vpo-paropt-force-member-of-to-point-to-base=true %s | FileCheck %s -check-prefix=TFORM
...
; RUN: opt -opaque-pointers=0 -enable-new-pm=0 -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -debug-only=vpo-paropt-target -S -vpo-paropt-force-member-of-to-point-to-base=true %s 2>&1 | FileCheck %s -check-prefix=DBG
