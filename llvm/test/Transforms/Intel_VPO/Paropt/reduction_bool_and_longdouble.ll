...
; This test is used to check reduction with bool and operation + longdouble type.
; #include <stdio.h>
; #include <math.h>
;
; #define eps         1.0e-12
...
; ModuleID = 'reduction_bool_and_longdouble.c'
source_filename = "reduction_bool_and_longdouble.c"
...
entry:
  ...
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.PARALLEL.LOOP"(), "QUAL.OMP.REDUCTION.AND"(x86_fp80* %v3), "QUAL.OMP.PRIVATE"(i32* %i), "QUAL.OMP.NORMALIZED.IV"(i32* %.omp.iv), "QUAL.OMP.FIRSTPRIVATE"(i32* %.omp.lb), "QUAL.OMP.NORMALIZED.UB"(i32* %.omp.ub) ]
...
land.end:                                         ; preds = %land.rhs, %omp.inner.for.body
  ...
  %conv = sitofp i32 %land.ext to x86_fp80
