; RUN: opt -enable-new-pm=0 -opaque-pointers -vpo-restore-operands -S %s | FileCheck %s -check-prefixes=RESTR,ALL
; RUN: opt -opaque-pointers -passes='function(vpo-restore-operands)' -S %s | FileCheck %s -check-prefixes=RESTR,ALL
; RUN: opt -enable-new-pm=0 -opaque-pointers -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -S %s | FileCheck %s -check-prefixes=TFORM,ALL
