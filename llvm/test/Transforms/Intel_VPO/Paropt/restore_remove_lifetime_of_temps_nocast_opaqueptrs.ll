; RUN: opt -opaque-pointers=1 -enable-new-pm=0 -vpo-restore-operands -S %s | FileCheck %s -check-prefixes=RESTR,ALL
; RUN: opt -opaque-pointers=1 -passes='function(vpo-restore-operands)' -S %s | FileCheck %s -check-prefixes=RESTR,ALL
; RUN: opt -opaque-pointers=1 -enable-new-pm=0 -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -S %s | FileCheck %s -check-prefixes=TFORM,ALL
