; REQUIRES: asserts
; RUN: opt -bugpoint-enable-legacy-pm -vpo-paropt-target-capture-non-pointers-using-map-to -switch-to-offload -vpo-cfg-restructuring -vpo-paropt -debug-only=WRegionUtils,vpo-paropt-transform -S %s 2>&1 | FileCheck %s
