...
omp.precond.then:                                 ; preds = %arrayctor.cont
  ...
  %7 = call token @llvm.directive.region.entry() [ "DIR.OMP.PARALLEL.LOOP"(),
    "QUAL.OMP.FIRSTPRIVATE:NONPOD.TYPED"(ptr %vla, %class.A zeroinitializer, i64 %1, ptr @_ZTS1A.omp.copy_constr, ptr @_ZTS1A.omp.destr),
...
; Copy constructor
; CHECK:  %[[TO:[^,]+]] = getelementptr inbounds %class.A, ptr %vla.fpriv, i32 0
; CHECK-NEXT:  %[[FROM:[^,]+]] = getelementptr inbounds %class.A, ptr %vla, i32 0
; CHECK-NEXT:  %[[END:[^,]+]] = getelementptr %class.A, ptr %[[TO]], i64 %[[VLA_LEN:[^,]+]]
...
; Destructor
; CHECK:  %[[DESTR_BEGIN:[^,]+]] = getelementptr inbounds %class.A, ptr %vla.fpriv, i32 0
; CHECK-NEXT:  %[[DESTR_END:[^,]+]] = getelementptr %class.A, ptr %[[DESTR_BEGIN]], i64 %[[VLA_LEN]]
...
define internal void @_ZTS1A.omp.copy_constr(ptr noundef %0, ptr noundef %1) #4 {
entry:
