...
; RUN: opt < %s -passes='function(loop(loop-rotate),vpo-cfg-restructuring,vpo-paropt-prepare,loop-simplifycfg,loop(loop-simplifycfg),sroa,vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt' -vpo-paropt-fast-reduction=false -S | FileCheck %s --check-prefix=CRITICAL
...
; RUN: opt < %s -passes='function(loop(loop-rotate),vpo-cfg-restructuring,vpo-paropt-prepare,loop-simplifycfg,loop(loop-simplifycfg),sroa,vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt'   -S | FileCheck %s --check-prefix=FASTRED
