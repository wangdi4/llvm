; RUN: opt -vpo-cfg-restructuring -vpo-paropt-prepare -S < %s | FileCheck %s -check-prefix=BUFFPTR
; RUN: opt < %s -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare)' -S | FileCheck %s -check-prefix=BUFFPTR
; RUN: opt -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-paropt-use-raw-dev-ptr=true -S < %s | FileCheck %s -check-prefix=RAWPTR
; RUN: opt < %s -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare)' -vpo-paropt-use-raw-dev-ptr=true -S | FileCheck %s -check-prefix=RAWPTR
