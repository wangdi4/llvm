; RUN: opt -opaque-pointers=0 -enable-new-pm=0 -vpo-cfg-restructuring -vpo-paropt-loop-collapse -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -vpo-implicit-multiple-teams=true -S %s | FileCheck -check-prefix=TRUE %s
; RUN: opt -opaque-pointers=0 -passes='function(vpo-cfg-restructuring,vpo-paropt-loop-collapse,vpo-cfg-restructuring,vpo-paropt-prepare,vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt' -vpo-implicit-multiple-teams=true -S %s | FileCheck -check-prefix=TRUE %s
; RUN: opt -opaque-pointers=0 -enable-new-pm=0 -vpo-cfg-restructuring -vpo-paropt-loop-collapse -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -vpo-implicit-multiple-teams=false -S %s | FileCheck -check-prefix=FALSE %s
; RUN: opt -opaque-pointers=0 -passes='function(vpo-cfg-restructuring,vpo-paropt-loop-collapse,vpo-cfg-restructuring,vpo-paropt-prepare,vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt' -vpo-implicit-multiple-teams=false -S %s | FileCheck -check-prefix=FALSE %s
...
; Check that ND-range descriptor is passed with -vpo-implicit-multiple-teams=true,
; and not passed with -vpo-implicit-multiple-teams=false:
; TRUE: @{{.*}}offload_maptypes = {{.*}}i64 2048{{.*}}
; FALSE-NOT: @{{.*}}offload_maptypes = {{.*}}i64 2048{{.*}}
...
entry:
  ...
  store i32 999, i32* %.omp.ub, align 4, !tbaa !7
  %2 = load i32*, i32** %a.addr, align 8
  %3 = load i32*, i32** %a.addr, align 8, !tbaa !3
  %arrayidx = getelementptr inbounds i32, i32* %3, i64 0
  %4 = call token @llvm.directive.region.entry() [ "DIR.OMP.TARGET"(), "QUAL.OMP.OFFLOAD.ENTRY.IDX"(i32 0), "QUAL.OMP.MAP.TOFROM:AGGRHEAD"(i32* %2, i32* %arrayidx, i64 4000), "QUAL.OMP.PRIVATE"(i32* %.omp.iv), "QUAL.OMP.PRIVATE"(i32* %i), "QUAL.OMP.FIRSTPRIVATE"(i32* %.omp.lb), "QUAL.OMP.FIRSTPRIVATE"(i32* %.omp.ub), "QUAL.OMP.PRIVATE"(i32** %a.map.ptr.tmp), "QUAL.OMP.PRIVATE"(i32* %tmp) ]
  store i32* %2, i32** %a.map.ptr.tmp, align 8
  %5 = bitcast i32* %.omp.iv to i8*
  call void @llvm.lifetime.start.p0i8(i64 4, i8* %5) #1
  %6 = call token @llvm.directive.region.entry() [ "DIR.OMP.PARALLEL.LOOP"(), "QUAL.OMP.FIRSTPRIVATE"(i32* %.omp.lb), "QUAL.OMP.NORMALIZED.IV"(i32* %.omp.iv), "QUAL.OMP.NORMALIZED.UB"(i32* %.omp.ub), "QUAL.OMP.PRIVATE"(i32* %i), "QUAL.OMP.SHARED"(i32** %a.map.ptr.tmp) ]
  %7 = load i32, i32* %.omp.lb, align 4, !tbaa !7
  store i32 %7, i32* %.omp.iv, align 4, !tbaa !7
  br label %omp.inner.for.cond

omp.inner.for.cond:                               ; preds = %omp.inner.for.inc, %entry
  %8 = load i32, i32* %.omp.iv, align 4, !tbaa !7
  %9 = load i32, i32* %.omp.ub, align 4, !tbaa !7
...
omp.inner.for.body:                               ; preds = %omp.inner.for.cond
  ...
  %11 = load i32, i32* %.omp.iv, align 4, !tbaa !7
  %mul = mul nsw i32 %11, 1
  %add = add nsw i32 0, %mul
  store i32 %add, i32* %i, align 4, !tbaa !7
  %12 = load i32*, i32** %a.map.ptr.tmp, align 8, !tbaa !3
...
omp.inner.for.inc:                                ; preds = %omp.body.continue
  %15 = load i32, i32* %.omp.iv, align 4, !tbaa !7
  %add2 = add nsw i32 %15, 1
  store i32 %add2, i32* %.omp.iv, align 4, !tbaa !7
  br label %omp.inner.for.cond
