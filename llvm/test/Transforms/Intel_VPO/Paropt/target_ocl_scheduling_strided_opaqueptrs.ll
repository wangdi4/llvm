; RUN: opt -opaque-pointers=1 -bugpoint-enable-legacy-pm -switch-to-offload -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -vpo-paropt-ocl-loop-processing=strided -S %s | FileCheck %s
; RUN: opt -opaque-pointers=1 -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare,vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt' -switch-to-offload -vpo-paropt-ocl-loop-processing=strided -S %s | FileCheck %s
...
; This test checks parallel loop scheduling code for the case of strided processing

; CHECK-LABEL: DIR.OMP.PARALLEL.LOOP{{.*}}:
; CHECK: [[LB:%[^,]+]] = load i32, ptr %loop0.lower.bnd
; CHECK: [[UB:%[^,]+]] = load i32, ptr %loop0.upper.bnd
; CHECK: [[LSize:%[^,]+]] = call spir_func i64 @_Z14get_local_sizej(i32 0)
; CHECK: [[LSizeTrunc:%[^,]+]] = trunc i64 [[LSize]] to i32
; CHECK: [[UBLBDiff:%[^,]+]] = sub i32 [[UB]], [[LB]]
; CHECK: [[UBLBDiffPlusLSize:%[^,]+]] = add i32 [[UBLBDiff]], [[LSizeTrunc]]
; CHECK: [[LoopChunkSize:%[^,]+]] = sdiv i32 [[UBLBDiffPlusLSize]], [[LSizeTrunc]]
; CHECK: [[LId:%[^,]+]] = call spir_func i64 @_Z12get_local_idj(i32 0)
; CHECK: [[LIdTrunc:%[^,]+]] = trunc i64 [[LId]] to i32
; CHECK: [[LoopChunkOffset:%[^,]+]] = mul i32 [[LIdTrunc]], [[LoopChunkSize]]
; CHECK: [[NewLB:%[^,]+]] = add i32 [[LB]], [[LoopChunkOffset]]
