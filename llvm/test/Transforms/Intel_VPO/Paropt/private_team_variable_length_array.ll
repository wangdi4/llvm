...
; ModuleID = 'private_team_variable_length_array.cpp'
source_filename = "private_team_variable_length_array.cpp"
...
arrayctor.cont:                                   ; preds = %entry, %arrayctor.loop
  ...
  %3 = call token @llvm.directive.region.entry() [ "DIR.OMP.TARGET"(), "QUAL.OMP.OFFLOAD.ENTRY.IDX"(i32 0), "QUAL.OMP.PRIVATE:NONPOD"(%class.A addrspace(4)* %vla.ascast, %class.A addrspace(4)* (%class.A addrspace(4)*)* @_ZTS1A.omp.def_constr, void (%class.A addrspace(4)*)* @_ZTS1A.omp.destr), "QUAL.OMP.FIRSTPRIVATE"(i32 addrspace(4)* %n.addr.ascast), "QUAL.OMP.PRIVATE"(i32 addrspace(4)* %d.ascast), "QUAL.OMP.FIRSTPRIVATE"(i64 addrspace(4)* %omp.vla.tmp.ascast) ]
  %4 = call token @llvm.directive.region.entry() [ "DIR.OMP.TEAMS"(), "QUAL.OMP.PRIVATE:NONPOD"(%class.A addrspace(4)* %vla.ascast, %class.A addrspace(4)* (%class.A addrspace(4)*)* @_ZTS1A.omp.def_constr, void (%class.A addrspace(4)*)* @_ZTS1A.omp.destr), "QUAL.OMP.SHARED"(i32 addrspace(4)* %n.addr.ascast), "QUAL.OMP.PRIVATE"(i32 addrspace(4)* %d.ascast), "QUAL.OMP.SHARED"(i64 addrspace(4)* %omp.vla.tmp.ascast) ]
...
for.inc:                                          ; preds = %for.body
  %8 = load i32, i32 addrspace(4)* %d.ascast, align 4
  %inc = add nsw i32 %8, 1
...
!0 = !{i32 0, i32 2065, i32 5393282, !"_Z3fn1i", i32 10, i32 0, i32 0}
