; INTEL_CUSTOMIZATION
; RUN: opt -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -vpo-paropt-parameter-homing=false -S < %s | FileCheck %s
; RUN: opt -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare,vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt' -vpo-paropt-parameter-homing=false -S < %s  | FileCheck %s
...
bb31:                                             ; preds = %alloca
  ...
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.TARGET"(), "QUAL.OMP.OFFLOAD.ENTRY.IDX"(i32 0), "QUAL.OMP.MAP.TO"(%"QNCA_a0$i32*$rank1$.0"* %A, %"QNCA_a0$i32*$rank1$.0"* %A, i64 72, i64 33), "QUAL.OMP.PRIVATE:F90_DV"(%"QNCA_a0$i32*$rank1$.0"* %A) ]
