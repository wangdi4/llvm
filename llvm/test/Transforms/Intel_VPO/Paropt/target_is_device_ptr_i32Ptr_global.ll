; RUN: opt -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -vpo-paropt-use-mapper-api=false -vpo-paropt-parameter-homing=false -S < %s | FileCheck %s
; RUN: opt < %s -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare,vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt' -vpo-paropt-use-mapper-api=false -vpo-paropt-parameter-homing=false -S | FileCheck %s

; This test check that we can handle global is_device_ptr operands without a PTR_TO_PTR modifier.
...
; Check for the map created for a.
...
; CHECK: call void @__omp_offloading{{[^ ]*}}main{{[^ ]*}}(i32* %[[A_CAST1]])

; Check that the "a" passed in to the outlined function is used in the print in the region.
; CHECK-DAG: define internal void @__omp_offloading{{[^ ]*}}main{{[^ ]*}}(i32* %[[A_PASS:a[^ ,]*]])
...
entry:
  ...
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.TARGET"(), "QUAL.OMP.OFFLOAD.ENTRY.IDX"(i32 0), "QUAL.OMP.IS_DEVICE_PTR"(i32* @a) ]
...
!0 = !{i32 0, i32 2055, i32 154530606, !"_Z4main", i32 11, i32 0, i32 0}
