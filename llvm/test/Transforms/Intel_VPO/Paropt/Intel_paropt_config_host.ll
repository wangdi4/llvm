; RUN: opt -bugpoint-enable-legacy-pm -vpo-cfg-restructuring -vpo-paropt-apply-config -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -vpo-paropt-config=%S/Inputs/Intel_thread_limit_config.yaml -S %s | FileCheck %s
...
; CHECK: call i32 @__tgt_target_teams_mapper({{.*}}, ptr @__omp_offloading_805_b42e4b__Z3foo_l2.region_id,{{.*}}, i32 0, i32 33)
