...
; ModuleID = 'lastpriv_variable_length_array.cpp'
source_filename = "lastpriv_variable_length_array.cpp"
...
omp.precond.then:                                 ; preds = %arrayctor.cont
  ...
  %7 = call token @llvm.directive.region.entry() [ "DIR.OMP.PARALLEL.LOOP"(), "QUAL.OMP.LASTPRIVATE:NONPOD"(%class.A* %vla, %class.A* (%class.A*)* @_ZTS1A.omp.def_constr, void (%class.A*, %class.A*)* @_ZTS1A.omp.copy_assign, void (%class.A*)* @_ZTS1A.omp.destr), "QUAL.OMP.NORMALIZED.IV"(i32* %.omp.iv), "QUAL.OMP.FIRSTPRIVATE"(i32* %.omp.lb), "QUAL.OMP.NORMALIZED.UB"(i32* %.omp.ub), "QUAL.OMP.PRIVATE"(i32* %d), "QUAL.OMP.SHARED"(i64* %omp.vla.tmp) ]
...
; Constructor
; CHECK:  %[[CONSTR_BEGIN:[^,]+]] = getelementptr inbounds %class.A, %class.A* %vla.lpriv, i32 0
...
; Copy Assignment
; CHECK:  %[[TO:[^,]+]] = getelementptr inbounds %class.A, %class.A* %vla, i32 0
; CHECK-NEXT:  %[[FROM:[^,]+]] = getelementptr inbounds %class.A, %class.A* %vla.lpriv, i32 0
; CHECK-NEXT:  %[[END:[^,]+]] = getelementptr %class.A, %class.A* %[[TO]], i64 %{{.*}}
; CHECK-NEXT:  %priv.cpyassn.isempty = icmp eq %class.A* %[[TO]], %[[END]]
; CHECK-NEXT:  br i1 %priv.cpyassn.isempty, label %priv.cpyassn.done, label %priv.cpyassn.body
; CHECK-LABEL: priv.cpyassn.body:
; CHECK-NEXT:  %priv.cpy.dest.ptr{{.*}} = phi %class.A* [ %[[TO]], %{{.*}} ], [ %priv.cpy.dest.inc{{.*}}, %priv.cpyassn.body ]
; CHECK-NEXT:  %priv.cpy.src.ptr = phi %class.A* [ %[[FROM]], %{{.*}} ], [ %priv.cpy.src.inc, %priv.cpyassn.body ]
; CHECK-NEXT:  call void @_ZTS1A.omp.copy_assign(%class.A* %priv.cpy.dest.ptr{{.*}}, %class.A* %priv.cpy.src.ptr)
; CHECK-NEXT:  %priv.cpy.dest.inc{{.*}} = getelementptr %class.A, %class.A* %priv.cpy.dest.ptr{{.*}}, i32 1
...
; Destructor
; CHECK:  %[[DESTR_BEGIN:[^,]+]] = getelementptr inbounds %class.A, %class.A* %vla.lpriv, i32 0
...
; Function Attrs: noinline uwtable
define internal void @_ZTS1A.omp.copy_assign(%class.A* %0, %class.A* %1) #3 {
entry:
  %.addr = alloca %class.A*, align 8
