; RUN: opt -bugpoint-enable-legacy-pm -switch-to-offload -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt --vpo-paropt-atomic-free-reduction-ctrl=1 -vpo-paropt-atomic-free-red-local-buf-size=0 -S %s | FileCheck %s
; RUN: opt -switch-to-offload -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare,vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt' --vpo-paropt-atomic-free-reduction-ctrl=1 -vpo-paropt-atomic-free-red-local-buf-size=0 -S %s | FileCheck %s
...
; CHECK: %[[GLOBAL_SUM_CAST:[^,]+]] = addrspacecast ptr addrspace(1) %[[GLOBAL_SUM:[^,]+]] to ptr addrspace(4)
...
; CHECK: %[[GLOBAL_SUM_VAL:[^,]+]] = load volatile i32, ptr addrspace(4) %[[GLOBAL_SUM_CAST]]
; CHECK: %[[RED_VALUE:[^,]+]] = add i32 %[[GLOBAL_SUM_VAL]], %[[LOCAL_SUM_VAL]]
; CHECK: store i32 %[[RED_VALUE]], ptr addrspace(1) %[[GLOBAL_SUM]]
...
!0 = !{i32 0, i32 53, i32 -1921486161, !"_Z4main", i32 4, i32 0, i32 0}
