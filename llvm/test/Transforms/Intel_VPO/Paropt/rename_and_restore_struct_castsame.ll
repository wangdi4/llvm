; RUN: opt -opaque-pointers=0 -enable-new-pm=0 -vpo-cfg-restructuring -vpo-paropt-prepare -S %s | FileCheck %s -check-prefix=PREPR
; RUN: opt -opaque-pointers=0 -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare)' -S %s | FileCheck %s -check-prefix=PREPR
; RUN: opt -opaque-pointers=0 -enable-new-pm=0 -vpo-cfg-restructuring -vpo-paropt-prepare -early-cse -instcombine -early-cse -S %s | FileCheck %s -check-prefix=INSTCMB
; RUN: opt -opaque-pointers=0 -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare,early-cse,instcombine,early-cse)' -S %s | FileCheck %s -check-prefix=INSTCMB
; RUN: opt -opaque-pointers=0 -enable-new-pm=0 -vpo-cfg-restructuring -vpo-paropt-prepare -early-cse -instcombine -vpo-restore-operands -S %s | FileCheck %s -check-prefix=RESTR
...
;   }
; }
; ModuleID = 'rename_and_restore_struct_castsame.c'
source_filename = "rename_and_restore_struct_castsame.c"
...
; RESTR: call void @print_str(i32* [[FRENAMED]])
  ...
  %2 = bitcast %struct._FourChars* %f to i32*
...
declare dso_local void @print_str(i32*) #2
