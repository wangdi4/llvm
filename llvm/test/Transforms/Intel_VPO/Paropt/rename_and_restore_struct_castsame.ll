...
; RUN: opt -vpo-cfg-restructuring -vpo-paropt-prepare -early-cse -instcombine -early-cse -S < %s | FileCheck %s -check-prefix=INSTCMB
; RUN: opt < %s -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare,early-cse,instcombine,early-cse)'  -S | FileCheck %s -check-prefix=INSTCMB
...
; RUN: opt < %s -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare,early-cse,instcombine,vpo-restore-operands)'  -S | FileCheck %s -check-prefix=RESTR
;
; Test Src:
;
; #include <stdio.h>
