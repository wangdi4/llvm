; RUN: opt -bugpoint-enable-legacy-pm -switch-to-offload -vpo-cfg-restructuring -vpo-paropt-loop-collapse -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt-optimize-data-sharing -vpo-paropt -S %s | FileCheck %s --check-prefixes=CHECK,CHECK-GLOBAL-LOCAL
...
define hidden spir_func void @test(ptr addrspace(4) %w, i32 %E) {
entry:
  %w.addr = alloca ptr addrspace(4), align 8
  %w.addr.ascast = addrspacecast ptr %w.addr to ptr addrspace(4)
  ...
  store ptr addrspace(4) %w, ptr addrspace(4) %w.addr.ascast, align 8
