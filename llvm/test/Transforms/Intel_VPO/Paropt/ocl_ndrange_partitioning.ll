; RUN: opt < %s -prepare-switch-to-offload -switch-to-offload=true -vpo-cfg-restructuring -vpo-paropt-loop-collapse -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -vpo-paropt-gpu-execution-scheme=1 -S -pass-remarks-missed=openmp -pass-remarks-output=%t 2>&1 | FileCheck %s
; RUN: FileCheck --input-file=%t --check-prefix=YAML %s
; RUN: opt < %s -passes='function(vpo-cfg-restructuring,vpo-paropt-loop-collapse,vpo-cfg-restructuring,vpo-paropt-prepare,vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt' -prepare-switch-to-offload -switch-to-offload -vpo-paropt-gpu-execution-scheme=1 -S -pass-remarks-missed=openmp -pass-remarks-output=%t  2>&1 | FileCheck %s
; RUN: FileCheck --input-file=%t --check-prefix=YAML %s
