; RUN: opt -bugpoint-enable-legacy-pm -switch-to-offload -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -S %s | FileCheck %s --check-prefix=CTORDTOR
...
; RUN: opt -bugpoint-enable-legacy-pm -switch-to-offload -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -S -vpo-paropt-emit-target-priv-ctor-dtor=false %s | FileCheck %s --check-prefix=NOCTORDTOR
...
; Constructor
; CTORDTOR:  %[[CONSTR_BEGIN:[^,]+]] = getelementptr inbounds %class.A, ptr %vla.ascast.priv, i32 0
; CTORDTOR-NEXT:  %[[END:[^,]+]] = getelementptr %class.A, ptr %[[CONSTR_BEGIN]], i64 %[[VLA_LEN:[^,]+]]
; CTORDTOR-NEXT:  %priv.constr.isempty = icmp eq ptr %[[CONSTR_BEGIN]], %[[END]]
; CTORDTOR-NEXT:  br i1 %priv.constr.isempty, label %priv.constr.done, label %priv.constr.body
; CTORDTOR-LABEL: priv.constr.body:
; CTORDTOR-NEXT:  %priv.cpy.dest.ptr = phi ptr [ %[[CONSTR_BEGIN]], %{{.*}} ], [ %priv.cpy.dest.inc, %{{.*}} ]
; CTORDTOR:  call spir_func ptr addrspace(4) @_ZTS1A.omp.def_constr(ptr addrspace(4) %{{.*}})
...
for.cond:                                         ; preds = %for.inc, %arrayctor.cont
  %5 = load i32, ptr addrspace(4) %d.ascast, align 4
...
for.end:                                          ; preds = %for.cond
  ...
  %7 = load ptr, ptr addrspace(4) %saved_stack.ascast, align 8
...
!0 = !{i32 0, i32 43, i32 -1925675490, !"_Z3fn1i", i32 10, i32 0, i32 0, i32 0}
