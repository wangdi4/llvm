...
;   %0 = call i8* @omp_alloc(i64 4, i64 %default_allocator)
;   %scalar.fpriv = bitcast i8* %0 to float*
;
; CHECK: [[DEFALLOC:%[a-zA-Z._0-9]+]] = call i64 @omp_get_default_allocator()
; CHECK-NEXT:  call i8* @omp_alloc(i64 4, i64 [[DEFALLOC]])

; The IR to privatize "private(array) allocate(omp_large_cap_mem_alloc:array)" is
; (where omp_large_cap_mem_alloc is an enum == 2)
;   %1 = call i8* @omp_alloc(i64 400, i64 2)
;   %array.priv = bitcast i8* %1 to [100 x float]*
;
; CHECK: call i8* @omp_alloc(i64 400, i64 2)
...
DIR.OMP.PARALLEL.2:                               ; preds = %DIR.OMP.PARALLEL.1
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.PARALLEL"(), "QUAL.OMP.PRIVATE"([100 x float]* %array), "QUAL.OMP.FIRSTPRIVATE"(float* %scalar), "QUAL.OMP.ALLOCATE"([100 x float]* %array, i32 2), "QUAL.OMP.ALLOCATE"(float* %scalar) ]
