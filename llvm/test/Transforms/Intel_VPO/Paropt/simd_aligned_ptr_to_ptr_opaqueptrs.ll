...
; #pragma omp simd aligned(A:32) aligned(B)
;   for (int I = 0; I < N; ++I) {
;     A[I] = I;
;     B[I] = I;
;   }
...
; CHECK:         [[TOK:%.+]] = call token @llvm.directive.region.entry() [ "DIR.OMP.SIMD"(), "QUAL.OMP.ALIGNED:PTR_TO_PTR"(ptr null, i32 32),
; CHECK-NEXT:    br label %[[LOOPBODY:[^,]+]]

; CHECK:       [[LOOPBODY]]:
; CHECK:         [[PTR_A_ADDR_LOAD:%.+]] = load ptr, ptr [[PTR_A]].addr, align 8
; CHECK:         call void @llvm.assume(i1 true) [ "align"(ptr [[PTR_A_ADDR_LOAD]], i64 32) ]
; CHECK:         [[PTR_B_ADDR_LOAD:%.+]] = load ptr, ptr [[PTR_B]].addr, align 8
; CHECK:         call void @llvm.assume(i1 true) [ "align"(ptr [[PTR_B_ADDR_LOAD]], i64 64) ]
...
DIR.OMP.SIMD.1:                                   ; preds = %omp.precond.then
  %4 = call token @llvm.directive.region.entry() [ "DIR.OMP.SIMD"(),
    "QUAL.OMP.ALIGNED:PTR_TO_PTR"(ptr %A.addr, i32 32),
    "QUAL.OMP.ALIGNED:PTR_TO_PTR"(ptr %B.addr, i32 0),
