...
; ModuleID = 'private_variable_length_array.cpp'
source_filename = "private_variable_length_array.cpp"
...
omp.precond.then:                                 ; preds = %arrayctor.cont
  ...
  %7 = call token @llvm.directive.region.entry() [ "DIR.OMP.PARALLEL.LOOP"(), "QUAL.OMP.PRIVATE:NONPOD"(%class.A* %vla, %class.A* (%class.A*)* @_ZTS1A.omp.def_constr, void (%class.A*)* @_ZTS1A.omp.destr), "QUAL.OMP.NORMALIZED.IV"(i32* %.omp.iv), "QUAL.OMP.FIRSTPRIVATE"(i32* %.omp.lb), "QUAL.OMP.NORMALIZED.UB"(i32* %.omp.ub), "QUAL.OMP.PRIVATE"(i32* %d), "QUAL.OMP.SHARED"(i64* %omp.vla.tmp) ]
...
; Constructor
; CHECK:  %[[CONSTR_BEGIN:[^,]+]] = getelementptr inbounds %class.A, %class.A* %vla.priv, i32 0
; CHECK-NEXT:  %[[CONSTR_END:[^,]+]] = getelementptr %class.A, %class.A* %[[CONSTR_BEGIN]], i64 %{{.*}}
...
; Destructor
; CHECK:  %[[DESTR_BEGIN:[^,]+]] = getelementptr inbounds %class.A, %class.A* %vla.priv, i32 0
; CHECK-NEXT:  %[[DESTR_END:[^,]+]] = getelementptr %class.A, %class.A* %[[DESTR_BEGIN]], i64 %{{.*}}
