; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -vpo-paropt-handle-firstprivate-on-teams=false -switch-to-offload -vpo-cfg-restructuring -vpo-paropt -S %s | FileCheck %s --check-prefix=IGNOREFP
; RUN: opt -opaque-pointers=0 -vpo-paropt-handle-firstprivate-on-teams=false -switch-to-offload -passes='function(vpo-cfg-restructuring),vpo-paropt' -S %s | FileCheck %s --check-prefix=IGNOREFP

; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -vpo-paropt-handle-firstprivate-on-teams=true -switch-to-offload -vpo-cfg-restructuring -vpo-paropt -S %s | FileCheck %s --check-prefix=HANDLEFP
; RUN: opt -opaque-pointers=0 -vpo-paropt-handle-firstprivate-on-teams=true -switch-to-offload -passes='function(vpo-cfg-restructuring),vpo-paropt' -S %s | FileCheck %s --check-prefix=HANDLEFP
