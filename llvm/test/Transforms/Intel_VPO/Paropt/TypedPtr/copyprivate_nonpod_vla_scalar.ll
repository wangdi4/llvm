; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -vpo-cfg-restructuring -vpo-paropt-prepare -verify -S %s | FileCheck %s
; RUN: opt -opaque-pointers=0 -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare,verify)' -S %s | FileCheck %s
...
; CHECK: %__struct.kmp_copy_privates_t = type { %[[ARRAY_INFO:__struct.kmp_copy_privates_t.array.*]], %class.goo* }
; CHECK: %[[ARRAY_INFO]] = type { %class.goo*, i64 }
...
; CHECK: %[[CP_A:.*]] = getelementptr inbounds %[[ARRAY_INFO]], %[[ARRAY_INFO]]* %[[CP_ARRAY_INFO]], i32 0, i32 0
...
; CHECK: %[[CP_C:.*]] = getelementptr inbounds %__struct.kmp_copy_privates_t, %__struct.kmp_copy_privates_t* %[[CP_STRUCT]], i32 0, i32 1
; CHECK: store %class.goo* %c, %class.goo** %[[CP_C]], align 8
; CHECK: %[[CP_STRUCT_BITCAST:.*]] = bitcast %__struct.kmp_copy_privates_t* %[[CP_STRUCT]] to i8*
; CHECK: call void @__kmpc_copyprivate(%struct.ident_t* @{{.*}}, i32 %{{.*}}, i32 24, i8* %[[CP_STRUCT_BITCAST]], i8* bitcast (void (%__struct.kmp_copy_privates_t*, %__struct.kmp_copy_privates_t*)* @[[CPRIV_CALLBACK:_Z3fool_copy_priv.*]] to i8*), i32 %{{.*}})
...
; CHECK: %[[A_SRC_GEP:.*]] = getelementptr inbounds %[[ARRAY_INFO]], %[[ARRAY_INFO]]* %[[A_SRC_INFO_GEP]], i32 0, i32 0
; CHECK: %[[A_SRC:.*]] = load %class.goo*, %class.goo** %[[A_SRC_GEP]], align 8
; CHECK: %[[A_DST_GEP:.*]] = getelementptr inbounds %[[ARRAY_INFO]], %[[ARRAY_INFO]]* %[[A_DST_INFO_GEP]], i32 0, i32 0
; CHECK: %[[A_DST:.*]] = load %class.goo*, %class.goo** %[[A_DST_GEP]], align 8
; CHECK: %[[A_SRC_SIZE_GEP:.*]] = getelementptr inbounds %[[ARRAY_INFO]], %[[ARRAY_INFO]]* %[[A_SRC_INFO_GEP]], i32 0, i32 1
; CHECK: %[[A_SRC_SIZE:.*]] = load i64, i64* %[[A_SRC_SIZE_GEP]], align 8
; CHECK: %[[A_DST_BEGIN:.*]] = getelementptr inbounds %class.goo, %class.goo* %[[A_DST]], i32 0
; CHECK: %[[A_SRC_BEGIN:.*]] = getelementptr inbounds %class.goo, %class.goo* %[[A_SRC]], i32 0
; CHECK: %[[A_CP_DST_PTR:.*]] = phi %class.goo* {{.*}}[ %[[A_DST_BEGIN]], %{{.*}} ]
; CHECK: %[[A_CP_SRC_PTR:.*]] = phi %class.goo* {{.*}}[ %[[A_SRC_BEGIN]], %{{.*}} ]
; CHECK: call void @_ZTS3goo.omp.copy_assign(%class.goo* %[[A_CP_DST_PTR]], %class.goo* %[[A_CP_SRC_PTR]])

; Check loading the scalar C's addr
; CHECK: %[[C_SRC_GEP:.*]] = getelementptr inbounds %__struct.kmp_copy_privates_t, %__struct.kmp_copy_privates_t* %[[STRUCT_SRC]], i32 0, i32 1
; CHECK: %[[C_DST_GEP:.*]] = getelementptr inbounds %__struct.kmp_copy_privates_t, %__struct.kmp_copy_privates_t* %[[STRUCT_DST]], i32 0, i32 1
; CHECK: %[[C_SRC:.*]] = load %class.goo*, %class.goo** %[[C_SRC_GEP]], align 8
; CHECK: %[[C_DST:.*]] = load %class.goo*, %class.goo** %[[C_DST_GEP]], align 8
; CHECK: call void @_ZTS3goo.omp.copy_assign(%class.goo* %[[C_DST]], %class.goo* %[[C_SRC]])
...
arrayctor.cont6:                                  ; preds = %arrayctor.loop2
  call void @_ZN3gooC2Ev(%class.goo* noundef nonnull align 4 dereferenceable(4) %c)
  %2 = call token @llvm.directive.region.entry() [ "DIR.OMP.SINGLE"(),
    ...
    "QUAL.OMP.COPYPRIVATE:NONPOD"(%class.goo* %c, void (%class.goo*, %class.goo*)* @_ZTS3goo.omp.copy_assign) ]
  fence acquire
  fence release
