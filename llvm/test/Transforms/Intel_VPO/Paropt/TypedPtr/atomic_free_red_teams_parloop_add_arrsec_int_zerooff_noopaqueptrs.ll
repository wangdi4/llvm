...
; CHECK: define weak dso_local spir_kernel void @__omp_offloading{{.*}}main{{.*}}([2 x i32] addrspace(1)* noalias %[[RESULT_PTR:sum.*]], i32 addrspace(1)* %red_local_buf, i32 addrspace(1)* %[[RED_GLOBAL_BUF:red_buf.*]], i32 addrspace(1)* %[[TEAMS_COUNTER_PTR:teams_counter.*]], i64 %.omp.lb
...
; CHECK: %[[LOCAL_SUM_LD:[^,]+]] = load i32, i32 addrspace(1)* %[[RED_VAR_PTR_CAST]]
; CHECK: %[[LOCAL_RED_ADD:[^,]+]] = add i32 %[[LOCAL_SUM_LD]]
; CHECK: store i32 %[[LOCAL_RED_ADD]], i32 addrspace(1)* %[[RED_VAR_PTR_CAST]]
