; REQUIRES: asserts
; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -vpo-paropt -debug -S %s 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers=0 -passes='vpo-paropt' -debug -S %s 2>&1 | FileCheck %s
