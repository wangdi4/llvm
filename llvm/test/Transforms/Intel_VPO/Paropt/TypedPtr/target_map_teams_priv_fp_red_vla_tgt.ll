; REQUIRES: asserts
; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -vpo-paropt-handle-firstprivate-on-teams -switch-to-offload -vpo-cfg-restructuring -vpo-paropt -debug-only=WRegionUtils,vpo-paropt-transform -S %s 2>&1 | FileCheck %s -check-prefixes=LOCAL,ALL
; RUN: opt -opaque-pointers=0 -vpo-paropt-handle-firstprivate-on-teams -switch-to-offload -aa-pipeline=basic-aa -passes='function(vpo-cfg-restructuring),vpo-paropt' -debug-only=WRegionUtils,vpo-paropt-transform -S %s 2>&1 | FileCheck %s -check-prefixes=LOCAL,ALL

; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -vpo-paropt-teams-non-wilocal-vla-alloc-mode=module -vpo-paropt-handle-firstprivate-on-teams -switch-to-offload -vpo-cfg-restructuring -vpo-paropt -debug-only=WRegionUtils,vpo-paropt-transform -S %s 2>&1 | FileCheck %s -check-prefixes=MODULE,ALL
; RUN: opt -opaque-pointers=0 -vpo-paropt-teams-non-wilocal-vla-alloc-mode=module -vpo-paropt-handle-firstprivate-on-teams -switch-to-offload -aa-pipeline=basic-aa -passes='function(vpo-cfg-restructuring),vpo-paropt' -debug-only=WRegionUtils,vpo-paropt-transform -S %s 2>&1 | FileCheck %s -check-prefixes=MODULE,ALL

; RUN: opt -opaque-pointers=0 -vpo-paropt-handle-firstprivate-on-teams -bugpoint-enable-legacy-pm -switch-to-offload -vpo-cfg-restructuring -vpo-paropt -disable-output -pass-remarks-missed=openmp %s 2>&1 | FileCheck %s -check-prefix=REMARK
; RUN: opt -opaque-pointers=0 -vpo-paropt-handle-firstprivate-on-teams -switch-to-offload -aa-pipeline=basic-aa -passes='function(vpo-cfg-restructuring),vpo-paropt' -disable-output -pass-remarks-missed=openmp %s 2>&1 | FileCheck %s -check-prefix=REMARK
...
; LOCAL:     %vla1.ascast.fpriv = alloca i32, i64 [[NV1]], align 4
; LOCAL:     [[VLA1:%vla1.ascast.fpriv.ascast]] = addrspacecast i32* %vla1.ascast.fpriv to i32 addrspace(4)*

; LOCAL:     %vla.ascast.priv = alloca i32, i64 [[NV]], align 4
; LOCAL:     [[VLA:%vla.ascast.priv.ascast]] = addrspacecast i32* %vla.ascast.priv to i32 addrspace(4)*

; Check that the private copies are used inside the region.
; LOCAL:       %oclPrint = call i32 (i8 addrspace(2)*, ...) @_Z18__spirv_ocl_printfPU3AS2cz({{.*}}, i32 addrspace(4)* [[VLA]], i32 addrspace(4)* [[VLA1]], i32 addrspace(4)* [[VLA2]])
; MODULE:      %oclPrint = call i32 (i8 addrspace(2)*, ...) @_Z18__spirv_ocl_printfPU3AS2cz({{.*}}, i32 addrspace(4)* addrspacecast (i32 addrspace(3)* [[VLA]] to i32 addrspace(4)*), i32 addrspace(4)* addrspacecast (i32 addrspace(3)* [[VLA1]] to i32 addrspace(4)*), i32 addrspace(4)* addrspacecast (i32 addrspace(3)* [[VLA2]] to i32 addrspace(4)*))
...
entry:
  ...
  %n.v = load i64, i64 addrspace(4)* %n.ascast, align 8
  %vla = alloca i32, i64 %n.v, align 4
  %vla.ascast = addrspacecast i32* %vla to i32 addrspace(4)*

  %n.v1 = load i64, i64 addrspace(4)* %n.ascast, align 8
  %vla1 = alloca i32, i64 %n.v1, align 4
  %vla1.ascast = addrspacecast i32* %vla1 to i32 addrspace(4)*

  %n.v2 = load i64, i64 addrspace(4)* %n.ascast, align 8
  %vla2 = alloca i32, i64 %n.v2, align 4
  %vla2.ascast = addrspacecast i32* %vla2 to i32 addrspace(4)*

  %map_size1 = mul nuw i64 %n.v1, 4
  %arrayidx = getelementptr inbounds i32, i32 addrspace(4)* %vla1.ascast, i64 0
  %map_size2 = mul nuw i64 %n.v2, 4
  %arrayidx5 = getelementptr inbounds i32, i32 addrspace(4)* %vla2.ascast, i64 0
  %map_size3 = mul nuw i64 %n.v, 4
  %arrayidx6 = getelementptr inbounds i32, i32 addrspace(4)* %vla.ascast, i64 0

  %dir1 = call token @llvm.directive.region.entry() [ "DIR.OMP.TARGET"(),
    "QUAL.OMP.OFFLOAD.ENTRY.IDX"(i32 0),
    "QUAL.OMP.MAP.TOFROM"(i32 addrspace(4)* %vla1.ascast, i32 addrspace(4)* %arrayidx, i64 %map_size1, i64 35, i8* null, i8* null),
    "QUAL.OMP.MAP.TOFROM"(i32 addrspace(4)* %vla2.ascast, i32 addrspace(4)* %arrayidx5, i64 %map_size2, i64 35, i8* null, i8* null),
    "QUAL.OMP.MAP.TOFROM"(i32 addrspace(4)* %vla.ascast, i32 addrspace(4)* %arrayidx6, i64 %map_size3, i64 3, i8* null, i8* null) ]

  %dir2 = call token @llvm.directive.region.entry() [ "DIR.OMP.TEAMS"(),
    "QUAL.OMP.PRIVATE"(i32 addrspace(4)* %vla.ascast),
    "QUAL.OMP.FIRSTPRIVATE"(i32 addrspace(4)* %vla1.ascast),
    "QUAL.OMP.REDUCTION.ADD"(i32 addrspace(4)* %vla2.ascast) ]

  %call = call spir_func i32 (i8 addrspace(4)*, ...) @printf(i8 addrspace(4)* noundef getelementptr inbounds ([10 x i8], [10 x i8] addrspace(4)* addrspacecast ([10 x i8] addrspace(1)* @.str to [10 x i8] addrspace(4)*), i64 0, i64 0), i32 addrspace(4)* noundef %vla.ascast, i32 addrspace(4)* noundef %vla1.ascast, i32 addrspace(4)* noundef %vla2.ascast)
