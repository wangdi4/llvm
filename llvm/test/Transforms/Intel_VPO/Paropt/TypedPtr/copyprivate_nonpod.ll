...
; CHECK: %[[SINGLE_THREAD:.*]] = load i32, i32* %is.single.thread
...
; CHECK: store %class.goo* %a, %class.goo** %[[CP_A]], align 8
; CHECK: %[[CP_STRUCT_BITCAST:.*]] = bitcast %__struct.kmp_copy_privates_t* %[[CP_STRUCT]] to i8*
; CHECK-COUNT-1: call void @__kmpc_copyprivate(%struct.ident_t* @{{.*}}, i32 %{{.*}}, i32 16, i8* %[[CP_STRUCT_BITCAST]], i8* bitcast (void (%__struct.kmp_copy_privates_t*, %__struct.kmp_copy_privates_t*)* @[[CPRIV_CALLBACK:_Z3foov_copy_priv.*]] to i8*), i32 %[[SINGLE_THREAD]])
; CHECK: define internal void @[[CPRIV_CALLBACK]](%__struct.kmp_copy_privates_t* %[[DST:.*]], %__struct.kmp_copy_privates_t* %[[SRC:.*]])
; CHECK: %[[A_SRC_GEP:.*]] = getelementptr inbounds %__struct.kmp_copy_privates_t, %__struct.kmp_copy_privates_t* %[[SRC]]
; CHECK: %[[A_DST_GEP:.*]] = getelementptr inbounds %__struct.kmp_copy_privates_t, %__struct.kmp_copy_privates_t* %[[DST]]
; CHECK: %[[A_SRC:.*]] = load %class.goo*, %class.goo** %[[A_SRC_GEP]]
; CHECK: %[[A_DST:.*]] = load %class.goo*, %class.goo** %[[A_DST_GEP]]
; CHECK: call void @_ZTS3goo.omp.copy_assign(%class.goo* %[[A_DST]], %class.goo* %[[A_SRC]])
...
entry:
  ...
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.SINGLE"(),
    "QUAL.OMP.COPYPRIVATE:NONPOD"(%class.goo* %a, void (%class.goo*, %class.goo*)* @_ZTS3goo.omp.copy_assign),
    "QUAL.OMP.COPYPRIVATE"(i32* %b) ]
  fence acquire
  fence release
