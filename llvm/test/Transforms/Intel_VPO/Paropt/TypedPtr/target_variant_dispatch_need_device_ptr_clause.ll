; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -vpo-paropt-prepare -S <%s | FileCheck %s --check-prefix=CHECK-BEFORE --check-prefix=CHECK-ALL
...
; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -vpo-paropt-prepare -vpo-paropt-put-interop-after-vararg=true -S <%s | FileCheck %s --check-prefix=CHECK-AFTER --check-prefix=CHECK-ALL
