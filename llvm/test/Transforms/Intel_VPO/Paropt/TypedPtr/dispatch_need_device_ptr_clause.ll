; RUN: opt -opaque-pointers=0 -vpo-paropt-dispatch-codegen-version=0 -bugpoint-enable-legacy-pm -vpo-paropt-prepare -S <%s | FileCheck %s
; RUN: opt -opaque-pointers=0 -vpo-paropt-dispatch-codegen-version=0 -passes='function(vpo-paropt-prepare)' -S <%s | FileCheck %s
...
define dso_local void @_Z3fooPfS_S_(float* noundef %aaa, float* noundef %bbb, float* noundef %ccc) #0 {
entry:
;
; CHECK: {{.*}}.offload_baseptrs = alloca [3 x i8*], align 8
...
DIR.OMP.DISPATCH.2:                               ; preds = %DIR.OMP.DISPATCH.1
  %1 = load float*, float** %ccc.addr, align 8
  call void (float**, float**, ...) @_Z3barRPfS0_z(float** noundef nonnull align 8 dereferenceable(8) %aaa.addr, float** noundef nonnull align 8 dereferenceable(8) %bbb.addr, float* noundef %1, i32 noundef 4) #1 [ "QUAL.OMP.DISPATCH.CALL"() ]

; CHECK:  call void (float**, float**, i8*, ...) @_Z11bar_variantRPfS0_Pvz(float** %aaa.addr.new, float** %bbb.addr.new, i8* %interop.obj.sync, float* {{.*}}.updated.val, i32 4)
...
declare dso_local void @_Z3barRPfS0_z(float** noundef nonnull align 8 dereferenceable(8), float** noundef nonnull align 8 dereferenceable(8), ...) #2
