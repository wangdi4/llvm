...
; is zero constant float point.
;
; #include <stdio.h>
; #include <omp.h>
; void foo ( )
; {
; #pragma omp task final(0.0)
;   {
;     if(!omp_in_final()) {
;     printf("passed\n");
;     }
...
; The flag of 3rd argument in __kmpc_omp_task_alloc is expected to 1.
; CHECK: call i8* @__kmpc_omp_task_alloc(%struct.ident_t* {{.*}}, i32 {{.*}}, i32 1, {{.*}}
...
entry:
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.TASK"(),
    "QUAL.OMP.FINAL"(double 0.000000e+00) ]
