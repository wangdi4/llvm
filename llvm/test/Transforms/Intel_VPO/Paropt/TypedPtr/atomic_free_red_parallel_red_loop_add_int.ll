...
; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -switch-to-offload -vpo-cfg-restructuring -vpo-paropt-loop-collapse -vpo-cfg-restructuring -vpo-paropt-prepare -S %s | FileCheck -check-prefixes=CHECK %s
; RUN: opt -opaque-pointers=0 -switch-to-offload -passes='function(vpo-cfg-restructuring,vpo-paropt-loop-collapse,vpo-cfg-restructuring,vpo-paropt-prepare)' -S %s | FileCheck -check-prefixes=CHECK %s
...
; This is a hand-modified version of the IR for the code above to check
; that no specific ND-range partitioning kicks in when there's an atomicfree-capable
; reduction clause on the parallel construct enclosing a loop construct.
; Now this issue only manifests itself in Fortran compiler, so firstprivate
; clauses for LB & UB were manually added to the target construct to trigger
; the issue on CFE output.
