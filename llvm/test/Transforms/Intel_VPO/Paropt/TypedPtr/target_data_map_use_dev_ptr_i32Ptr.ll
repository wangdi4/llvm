; RUN: opt -opaque-pointers=0 -enable-new-pm=0 -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -vpo-paropt-use-mapper-api=false -S %s | FileCheck %s
...
; This test IR was hand-modified to make the clause operands of the map
; clause and use_device_ptr clause same.
...
;   {
; #pragma omp target data map(tofrom: array_device[0:10]) use_device_ptr(array_device)
;     {
