; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -switch-to-offload -vpo-cfg-restructuring -vpo-paropt-loop-collapse -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -vpo-implicit-multiple-teams=true -S %s | FileCheck -check-prefix=TRUE %s
...
; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -switch-to-offload -vpo-cfg-restructuring -vpo-paropt-loop-collapse -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -vpo-implicit-multiple-teams=false -S %s | FileCheck -check-prefix=FALSE %s
...
; TRUE: @{{.*}}kernel_info = {{.*}}], i64 2,{{.*}}
; TRUE: call{{.*}}_Z13get_global_idj
; FALSE-NOT: @{{.*}}kernel_info = {{.*}}], i64 2,{{.*}}
...
define hidden spir_func void @foo(i32 addrspace(4)* %a) {
entry:
  ...
  %2 = call token @llvm.directive.region.entry() [ "DIR.OMP.TARGET"(),
    ...
    "QUAL.OMP.MAP.TOFROM:AGGRHEAD"(i32 addrspace(4)* %0, i32 addrspace(4)* %arrayidx, i64 4000),
