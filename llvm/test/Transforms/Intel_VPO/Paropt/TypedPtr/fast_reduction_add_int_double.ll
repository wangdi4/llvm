...
; ModuleID = 'fast_reduction_add_int_double.c'
source_filename = "fast_reduction_add_int_double.c"
...
entry:
  ...
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.PARALLEL.LOOP"(), "QUAL.OMP.REDUCTION.ADD"(i32* %sum), "QUAL.OMP.REDUCTION.ADD"(double* %f), "QUAL.OMP.PRIVATE"(i32* %i), "QUAL.OMP.NORMALIZED.IV"(i32* %.omp.iv), "QUAL.OMP.FIRSTPRIVATE"(i32* %.omp.lb), "QUAL.OMP.NORMALIZED.UB"(i32* %.omp.ub) ]
...
; ALL: call void @__kmpc_end_reduce({{.*}})
; ALL-NEXT: br label %[[ATOMIC_REDUCE_EXIT]]
