; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -switch-to-offload -vpo-cfg-restructuring -vpo-paropt -S -vpo-paropt-atomic-free-reduction-par-global=false -vpo-paropt-atomic-free-red-global-combiner-selector=last-team-inlined <%s | FileCheck -check-prefix=LASTTEAMINLINED %s
; RUN: opt -opaque-pointers=0 -switch-to-offload -passes="function(vpo-cfg-restructuring),vpo-paropt" -S -vpo-paropt-atomic-free-reduction-par-global=false -vpo-paropt-atomic-free-red-global-combiner-selector=last-team-inlined <%s | FileCheck -check-prefix=LASTTEAMINLINED %s

; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -switch-to-offload -vpo-cfg-restructuring -vpo-paropt -S -vpo-paropt-atomic-free-reduction-par-global=false -vpo-paropt-atomic-free-red-global-combiner-selector=team-zero-rtl <%s | FileCheck -check-prefix=TEAMZERORTL %s
; RUN: opt -opaque-pointers=0 -switch-to-offload -passes="function(vpo-cfg-restructuring),vpo-paropt" -S -vpo-paropt-atomic-free-reduction-par-global=false -vpo-paropt-atomic-free-red-global-combiner-selector=team-zero-rtl <%s | FileCheck -check-prefix=TEAMZERORTL %s

; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -switch-to-offload -vpo-cfg-restructuring -vpo-paropt -S -vpo-paropt-atomic-free-reduction-par-global=false -vpo-paropt-atomic-free-red-global-combiner-selector=last-team-rtl <%s | FileCheck -check-prefix=LASTTEAMRTL %s
; RUN: opt -opaque-pointers=0 -switch-to-offload -passes="function(vpo-cfg-restructuring),vpo-paropt" -S -vpo-paropt-atomic-free-reduction-par-global=false -vpo-paropt-atomic-free-red-global-combiner-selector=last-team-rtl <%s | FileCheck -check-prefix=LASTTEAMRTL %s
...
bb2:                                              ; preds = %bb1
  %tmp3 = call token @llvm.directive.region.entry() [ "DIR.OMP.TEAMS"(),
    "QUAL.OMP.REDUCTION.ADD"(i32 addrspace(1)* @b) ]

  store i32 111, i32 addrspace(4)* addrspacecast (i32 addrspace(1)* @b to i32 addrspace(4)*), align 4
  br label %bb4
