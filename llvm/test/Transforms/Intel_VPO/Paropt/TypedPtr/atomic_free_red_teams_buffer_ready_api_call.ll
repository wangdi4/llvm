; RUN: opt -opaque-pointers=0 -enable-new-pm=0 -switch-to-offload -vpo-cfg-restructuring -vpo-paropt -S -vpo-paropt-atomic-free-reduction-par-global=false -vpo-paropt-atomic-free-red-global-combiner-selector=last-team-inlined <%s | FileCheck -check-prefix=LASTTEAMINLINED %s
...
; RUN: opt -opaque-pointers=0 -enable-new-pm=0 -switch-to-offload -vpo-cfg-restructuring -vpo-paropt -S -vpo-paropt-atomic-free-reduction-par-global=false -vpo-paropt-atomic-free-red-global-combiner-selector=team-zero-rtl <%s | FileCheck -check-prefix=TEAMZERORTL %s
...
; RUN: opt -opaque-pointers=0 -enable-new-pm=0 -switch-to-offload -vpo-cfg-restructuring -vpo-paropt -S -vpo-paropt-atomic-free-reduction-par-global=false -vpo-paropt-atomic-free-red-global-combiner-selector=last-team-rtl <%s | FileCheck -check-prefix=LASTTEAMRTL %s
