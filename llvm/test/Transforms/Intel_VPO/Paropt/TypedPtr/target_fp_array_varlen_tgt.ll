...
; CHECK:     define {{.*}} void @__omp_offloading{{.*}}main{{.*}}([2 x i32] addrspace(1)* %vla.ascast, i64 [[SIZE_ADDR]].val)

; Check that no extra local copy is made for the VLA inside the kernel, and the argument passed-in is used directly.
; CHECK:       [[VLA_CAST:%.+]] = addrspacecast [2 x i32] addrspace(1)* %vla.ascast to [2 x i32] addrspace(4)*
; CHECK:       [[SIZE_ADDR]].fpriv = alloca i64, align 8
; CHECK:       store i64 [[SIZE_ADDR]].val, i64* [[SIZE_ADDR]].fpriv, align 8
; CHECK:       call {{.*}} @_Z18__spirv_ocl_printfPU3AS2ci({{.*}}, [2 x i32] addrspace(4)* [[VLA_CAST]])
...
entry:
  ...
  %vla = alloca [ 2 x i32], align 4
  %vla.ascast = addrspacecast [2 x i32]* %vla to [2 x i32] addrspace(4)*

  %dir = call token @llvm.directive.region.entry() [ "DIR.OMP.TARGET"(),
    "QUAL.OMP.OFFLOAD.ENTRY.IDX"(i32 0),
    "QUAL.OMP.FIRSTPRIVATE:VARLEN"([2 x i32] addrspace(4)* %vla.ascast) ]

  %call = call spir_func i32 (i8 addrspace(4)*, ...) @printf(i8 addrspace(4)* noundef getelementptr inbounds ([4 x i8], [4 x i8] addrspace(4)* addrspacecast ([4 x i8] addrspace(1)* @.str to [4 x i8] addrspace(4)*), i64 0, i64 0), [2 x i32] addrspace(4)* noundef %vla.ascast)
