; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -switch-to-offload -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -vpo-paropt-atomic-free-reduction-ctrl=3 -vpo-paropt-atomic-free-red-local-buf-size=0 -S %s | FileCheck %s
; RUN: opt -opaque-pointers=0 -switch-to-offload -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare,vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt' -vpo-paropt-atomic-free-reduction-ctrl=3 -vpo-paropt-atomic-free-red-local-buf-size=0 -S %s | FileCheck %s
...
; CHECK: %[[LOCAL_BUF_BASE:team.buf.baseptr[^,]*]] = getelementptr inbounds [1 x i32], [1 x i32] addrspace(1)* %[[RED_GLOBAL_BUF]]
; CHECK: %[[MINUS_OFFSET:[^,]+]] = getelementptr i32, i32 addrspace(1)* %[[LOCAL_BUF_BASE]], i64 -[[CONST_OFFSET:[^,]+]]
; CHECK: %[[MINUS_OFFSET_CAST:[^,]+]] =  bitcast i32 addrspace(1)* %[[MINUS_OFFSET]] to [2 x i32] addrspace(1)*
...
; CHECK: %[[MINUS_OFFSET_CAST_BACK:[^,]+]] = bitcast [2 x i32] addrspace(1)* %[[MINUS_OFFSET_CAST]] to i32 addrspace(1)*
; CHECK: %[[DST_PTR_BASE:[^,]+]] = getelementptr i32, i32 addrspace(1)* %[[MINUS_OFFSET_CAST_BACK]], i64 [[CONST_OFFSET]]
; CHECK-LABEL: atomic.free.red.local.update.update.body:
; CHECK: %[[DST_PTR:red.cpy.dest.ptr[^,]+]] = phi i32 addrspace(1)* [ %[[DST_PTR_BASE]]
; CHECK: %[[SRC_PTR:red.cpy.src.ptr[^,]+]] = phi i32*
; CHECK: %[[DST_PTR_ASCAST:[^,]+]] = addrspacecast i32 addrspace(1)* %[[DST_PTR]] to i32 addrspace(4)*
; CHECK: %[[SRC_PTR_ASCAST:[^,]+]] = addrspacecast i32* %[[SRC_PTR]] to i32 addrspace(4)*
