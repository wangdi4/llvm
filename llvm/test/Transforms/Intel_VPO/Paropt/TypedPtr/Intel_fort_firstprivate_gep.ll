; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -switch-to-offload -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -vpo-paropt-opt-scalar-fp=false -S %s | FileCheck %s
; RUN: opt -opaque-pointers=0 -switch-to-offload -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare,vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt' -vpo-paropt-opt-scalar-fp=false -S %s | FileCheck %s
...
define void @test_target_enter_data_components_alloc_IP_test_map_derived_type_alloc_(%uplevel_type.0* nest %uplevel_ptr0) {
alloca:
...
bb3:                                              ; preds = %alloca
  %2 = call token @llvm.directive.region.entry() [ "DIR.OMP.TARGET"(),
    ...
    "QUAL.OMP.MAP.ALLOC"(i32 addrspace(4)* addrspace(4)* %"test_map_derived_type_alloc$MYPTR$_0", i32 addrspace(4)* addrspace(4)* %"test_map_derived_type_alloc$MYPTR$_0", i64 4, i64 32, i8 addrspace(4)* null, i8 addrspace(4)* null),
