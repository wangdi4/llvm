...
; RUN: opt -passes="function(vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt" -S --vpo-utils-add-typed-privates=false <%s 2>&1 | FileCheck %s --check-prefixes=CHECK,UNTYPED
...
; RUN: opt -passes="function(vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt" -S <%s 2>&1 | FileCheck %s --check-prefixes=CHECK,TYPED
