; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -S --vpo-utils-add-typed-privates=false <%s 2>&1 | FileCheck %s --check-prefixes=CHECK,UNTYPED
...
; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -S <%s 2>&1 | FileCheck %s --check-prefixes=CHECK,TYPED
