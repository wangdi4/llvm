...
entry:
  ...
  store float 1.000000e+00, float* %v3, align 4
  store i32 0, i32* %.omp.lb, align 4
  store i32 2, i32* %.omp.ub, align 4
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.PARALLEL.LOOP"(), "QUAL.OMP.REDUCTION.AND"(float* %v3), "QUAL.OMP.PRIVATE"(i32* %i), "QUAL.OMP.NORMALIZED.IV"(i32* %.omp.iv), "QUAL.OMP.FIRSTPRIVATE"(i32* %.omp.lb), "QUAL.OMP.NORMALIZED.UB"(i32* %.omp.ub) ]


; CHECK-NOT: QUAL.OMP.REDUCTION.AND
; CHECK: %[[RED:[^,]+]] = load float, float* %v3.fast_red, align 4
; CHECK-NEXT: %[[ORIG:[^,]+]] = load float, float* %v3, align 4
...
; CHECK-NEXT: store float %[[CONV]], float* %v3, align 4
...
land.end:                                         ; preds = %land.rhs, %omp.inner.for.body
  ...
  store float %conv, float* %v3, align 4
...
omp.loop.exit:                                    ; preds = %omp.inner.for.end
  ...
  %8 = load float, float* %v3, align 4
  %sub = fsub float %8, 1.000000e+00
  %conv2 = fpext float %sub to x86_fp80
  %9 = call x86_fp80 @llvm.fabs.f80(x86_fp80 %conv2)
  %cmp3 = fcmp ogt x86_fp80 %9, 0xK3FD78CBCCC096F508800
