...
; CHECK: %[[BUF1_BASE:[^,]+]] = getelementptr inbounds float, float addrspace(1)* %red_local_buf, i64
; CHECK: %[[BUF2_BASE:[^,]+]] = getelementptr inbounds i32, i32 addrspace(1)* %red_local_buf{{.+}}, i64
...
; CHECK: %[[BUF2:[^,]+]] = getelementptr inbounds i32, i32 addrspace(1)* %[[BUF2_BASE]], i64 %[[IDX_PHI]]
; CHECK: %[[BUF1:[^,]+]] = getelementptr inbounds float, float addrspace(1)* %[[BUF1_BASE]], i64 %[[IDX_PHI]]
; CHECK: %[[VAL1_PRIV:[^,]+]] = load float, float addrspace(1)* %[[BUF1]]
; CHECK: %[[VAL1_BASE:[^,]+]] = load volatile float, float addrspace(1)* %[[BUF1_BASE]]
; CHECK: %[[VAL1_RES:[^,]+]] = fadd float %[[VAL1_BASE]], %[[VAL1_PRIV]]
; CHECK: store float %[[VAL1_RES]], float addrspace(1)* %[[BUF1_BASE]]
; CHECK: %[[VAL2_PRIV:[^,]+]] = load i32, i32 addrspace(1)* %[[BUF2]]
; CHECK: %[[VAL2_BASE:[^,]+]] = load volatile i32, i32 addrspace(1)* %[[BUF2_BASE]]
; CHECK: %[[VAL2_RES:[^,]+]] = add i32 %[[VAL2_BASE]], %[[VAL2_PRIV]]
; CHECK: store i32 %[[VAL2_RES]], i32 addrspace(1)* %[[BUF2_BASE]]
...
omp.inner.for.body:                               ; preds = %omp.inner.for.cond
  ...
  %8 = load float, float addrspace(4)* %sum.ascast, align 4
  %add1 = fadd fast float %8, %conv
  store float %add1, float addrspace(4)* %sum.ascast, align 4
  %9 = load i32, i32 addrspace(4)* %i.ascast, align 4
  %mul2 = mul nsw i32 2, %9
  %10 = load i32, i32 addrspace(4)* %sum2.ascast, align 4
  %add3 = add nsw i32 %10, %mul2
  store i32 %add3, i32 addrspace(4)* %sum2.ascast, align 4
