; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -switch-to-offload -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -vpo-paropt-atomic-free-reduction-slm=false -vpo-paropt-atomic-free-red-local-buf-size=0  -vpo-paropt-atomic-free-reduction-par-global=false -S %s | FileCheck %s
; RUN: opt -opaque-pointers=0 -switch-to-offload -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare,vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt' -vpo-paropt-atomic-free-reduction-slm=false -vpo-paropt-atomic-free-red-local-buf-size=0  -vpo-paropt-atomic-free-reduction-par-global=false -S %s | FileCheck %s
...
; CHECK: %[[LOCAL_SUM_VAL:[^,]+]] = load volatile i32, i32 addrspace(4)* %[[LOCAL_SUM_GEP_AC]]
