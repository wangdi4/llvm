; REQUIRES: asserts
; RUN: opt -vpo-cfg-restructuring -vpo-paropt -debug-only=vpo-paropt-target -vpo-paropt-parameter-homing=false -S < %s 2>&1 | FileCheck %s
; RUN: opt < %s -passes='function(vpo-cfg-restructuring),vpo-paropt' -debug-only=vpo-paropt-target -vpo-paropt-parameter-homing=false -S 2>&1 | FileCheck %s
