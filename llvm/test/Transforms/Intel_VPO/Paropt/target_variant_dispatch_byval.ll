; RUN: opt -opaque-pointers=1 -bugpoint-enable-legacy-pm -vpo-cfg-restructuring -vpo-paropt-prepare -S %s 2>&1 | FileCheck %s
...
;
; wrapper call:
; CHECK: call void @bar.fooGPU.wrapper(ptr byval(%struct.A) align 8 %aaa)
;
; base call:
; CHECK: call void @foo(ptr noundef byval(%struct.A) align 8 %aaa)
;
; wrapper function:
; CHECK: define internal void @bar.fooGPU.wrapper(ptr byval(%struct.A) align 8 %aaa)
;
; variant call in the wrapper function:
; CHECK: call void @fooGPU(ptr byval(%struct.A) align 8 %aaa, ptr %interop.obj.sync)
...
entry:
  ...
  call void @foo(ptr noundef byval(%struct.A) align 8 %aaa) #1
...
attributes #2 = { "approx-func-fp-math"="true" "frame-pointer"="all" "loopopt-pipeline"="light" "no-infs-fp-math"="true" "no-nans-fp-math"="true" "no-signed-zeros-fp-math"="true" "no-trapping-math"="true" "openmp-variant"="name:fooGPU;construct:target_variant_dispatch;arch:gen" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" "unsafe-fp-math"="true" }
