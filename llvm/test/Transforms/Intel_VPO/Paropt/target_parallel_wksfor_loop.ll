; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -switch-to-offload -vpo-paropt -S %s | FileCheck %s -check-prefix=DEFAULT -check-prefix=ALL
; RUN: opt -opaque-pointers=0 -passes='vpo-paropt' -switch-to-offload -S %s | FileCheck %s -check-prefix=DEFAULT -check-prefix=ALL
; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -switch-to-offload -vpo-paropt -vpo-paropt-emit-wks-loops-implicit-barrier-for-target=false -S %s | FileCheck %s -check-prefix=NO_BARRIER -check-prefix=ALL
