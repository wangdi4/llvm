...
; CHECK: [[NUMTEAMS:%[0-9A-Za-z._]+]] = load i32, ptr %x
; CHECK: [[THREADLIMIT:%[0-9A-Za-z._]+]] = load i32, ptr %x
; CHECK: call void @__kmpc_push_num_teams(ptr @{{.*}}, i32 %{{.*}}, i32 [[NUMTEAMS]], i32 [[THREADLIMIT]])
...
entry:
  ...
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.TEAMS"(),
    ...
    "QUAL.OMP.THREAD_LIMIT:TYPED"(ptr %x, i32 0) ]
