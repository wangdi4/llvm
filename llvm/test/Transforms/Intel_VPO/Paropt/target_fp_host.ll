; RUN: opt -opaque-pointers=1 -enable-new-pm=0 -vpo-paropt-target-non-wilocal-fp-alloc-mode=rtl -vpo-cfg-restructuring -vpo-paropt -S %s | FileCheck %s --check-prefix=RTL
...
; RUN: opt -opaque-pointers=1 -enable-new-pm=0 -vpo-paropt-target-non-wilocal-fp-alloc-mode=module -vpo-cfg-restructuring -vpo-paropt -S %s | FileCheck %s --check-prefix=NOTRTL
...
; RUN: opt -opaque-pointers=1 -enable-new-pm=0 -vpo-paropt-target-non-wilocal-fp-alloc-mode=wilocal -vpo-cfg-restructuring -vpo-paropt -S %s | FileCheck %s --check-prefix=NOTRTL
