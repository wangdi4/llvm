...
; CHECK: [[PHB]]:
; CHECK: call void @__kmpc_for_static_init_4({{.*}}, i32 %[[TID:[^,]+]], i32 91, ptr %[[PISLAST:[^,]+]], ptr %[[PLB:[^,]+]], ptr %[[PUB:[^,]+]], ptr %[[PST:[^,]+]], i32 1, i32 33)
...
; DISPINC:
; CHECK: [[DISPINC]]:
; CHECK: [[INC:%.+]] = load i32, ptr %[[PST]]
; CHECK: [[LB:%.+]] = load i32, ptr %[[PLB]]
; CHECK: [[LBINC:%.+]] = add i32 [[LB]], [[INC]]
; CHECK: [[UB:%.+]] = load i32, ptr %[[PUB]]
; CHECK: [[UBINC:%.+]] = add i32 [[UB]], [[INC]]
; CHECK: store i32 [[LBINC]], ptr %[[PLB]]
; CHECK: store i32 [[UBINC]], ptr %[[PUB]]
...
entry:
  ...
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.DISTRIBUTE"(),
    "QUAL.OMP.DIST_SCHEDULE.STATIC"(i32 33),
