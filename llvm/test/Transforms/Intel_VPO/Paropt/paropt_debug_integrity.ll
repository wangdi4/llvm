; REQUIRES: asserts
; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -debugify -vpo-cfg-restructuring -check-debugify -S %s 2>&1 | FileCheck %s
...
; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -debugify -vpo-cfg-restructuring -vpo-paropt-prepare -check-debugify -S %s 2>&1 | FileCheck %s
...
; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -debugify -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -check-debugify -S %s 2>&1 | FileCheck %s --check-prefixes=CHECK,PAROPT
