...
; This test is used to check reduction with bool and operation + double type.
; #include <stdio.h>
; #include <math.h>
;
; #define eps         1.0e-12
...
; ModuleID = 'reduction_bool_and_double.c'
source_filename = "reduction_bool_and_double.c"
...
entry:
  ...
  store double 1.000000e+00, double* %v3, align 8
  store i32 0, i32* %.omp.lb, align 4
  store i32 2, i32* %.omp.ub, align 4
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.PARALLEL.LOOP"(), "QUAL.OMP.REDUCTION.AND"(double* %v3), "QUAL.OMP.PRIVATE"(i32* %i), "QUAL.OMP.NORMALIZED.IV"(i32* %.omp.iv), "QUAL.OMP.FIRSTPRIVATE"(i32* %.omp.lb), "QUAL.OMP.NORMALIZED.UB"(i32* %.omp.ub) ]

; CHECK-NOT: QUAL.OMP.REDUCTION.AND
; CHECK: %[[RED:[^,]+]] = load double, double* %v3.fast_red, align 8
; CHECK-NEXT: %[[ORIG:[^,]+]] = load double, double* %v3, align 8
; CHECK-NEXT: %[[ORIG_BOOL:[^,]+]] = fcmp une double %[[ORIG]], 0.000000e+00
; CHECK-NEXT: %[[RED_BOOL:[^,]+]] = fcmp une double %[[RED]], 0.000000e+00
; CHECK-NEXT: %[[BOOL_VAL:[^,]+]] = select i1 %[[ORIG_BOOL]], i1 %[[RED_BOOL]], i1 %[[ORIG_BOOL]]
; CHECK-NEXT: %[[CONV:[^,]+]] = uitofp i1 %[[BOOL_VAL]] to double
; CHECK-NEXT: store double %[[CONV]], double* %v3, align 8
...
omp.inner.for.body:                               ; preds = %omp.inner.for.cond
  ...
  %5 = load double, double* %v3, align 8
...
land.end:                                         ; preds = %land.rhs, %omp.inner.for.body
  ...
  store double %conv, double* %v3, align 8
...
omp.loop.exit:                                    ; preds = %omp.inner.for.end
  ...
  %8 = load double, double* %v3, align 8
  %sub = fsub double %8, 1.000000e+00
  %conv2 = fpext double %sub to x86_fp80
