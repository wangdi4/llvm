; REQUIRES: asserts
; RUN: opt -opaque-pointers=1 -bugpoint-enable-legacy-pm -vpo-cfg-restructuring -vpo-paropt-prepare -S -debug-only=vpo-paropt-target -disable-output %s 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers=1 -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare)' -S -debug-only=vpo-paropt-target -disable-output %s 2>&1 | FileCheck %s
