...
; RUN: opt < %s -aa-pipeline=basic-aa -passes='function(loop(loop-rotate),vpo-cfg-restructuring,vpo-paropt-prepare,simplify-cfg,loop-simplifycfg,sroa,vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt,function(simplify-cfg)' -switch-to-offload  -S | FileCheck %s
;
