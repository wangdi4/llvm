; RUN: opt -opaque-pointers=0 -enable-new-pm=0 -switch-to-offload -vpo-cfg-restructuring -vpo-paropt-prepare -vpo-restore-operands -vpo-cfg-restructuring -vpo-paropt -vpo-paropt-atomic-free-reduction=true -S %s | FileCheck %s
; RUN: opt -opaque-pointers=0 -passes='function(vpo-cfg-restructuring,vpo-paropt-prepare,vpo-restore-operands,vpo-cfg-restructuring),vpo-paropt' -switch-to-offload -vpo-paropt-atomic-free-reduction=true -S %s | FileCheck %s
...
; CHECK: @__omp_offloading_805_b43487__Z3foo_l3_kernel_info = weak target_declare addrspace(1) constant %[[KINFO_TYPE]] { i32 5, i32 3, [3 x %[[KINFO_ARGMAP_TYPE]]] [%[[KINFO_ARGMAP_TYPE]] { i32 0, i32 8 }, %[[KINFO_ARGMAP_TYPE]] { i32 0, i32 8 }, %[[KINFO_ARGMAP_TYPE]] { i32 0, i32 8 }], i64 1, i64 0, i64 0 }
