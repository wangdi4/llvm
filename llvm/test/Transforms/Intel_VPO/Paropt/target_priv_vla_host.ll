; RUN: opt -opaque-pointers=1 -bugpoint-enable-legacy-pm -vpo-paropt-target-capture-non-pointers-using-map-to=true -vpo-cfg-restructuring -vpo-paropt -S %s | FileCheck %s
; RUN: opt -opaque-pointers=1 -vpo-paropt-target-capture-non-pointers-using-map-to=true -aa-pipeline=basic-aa -passes='function(vpo-cfg-restructuring),vpo-paropt' -S %s | FileCheck %s
...
; CHECK: @.offload_maptypes = private unnamed_addr constant [3 x i64] [i64 160, i64 33, i64 33]

; Check that we capture vla size (once for the typed size, and once for the base) and emit a
; map for the private VLA on target.
...
; CHECK: define internal void @__omp_offloading{{.*}}main{{.*}}(ptr noalias %vla, ptr noalias [[SIZE_ADDR]], ptr noalias [[SIZE_ADDR1]])
...
; CHECK:   %call = call i32 (ptr, ...) @printf({{.*}}, ptr noundef %vla.priv)
...
entry:
  ...
  %dir = call token @llvm.directive.region.entry() [ "DIR.OMP.TARGET"(),
    ...
    "QUAL.OMP.PRIVATE:TYPED"(ptr %vla, i32 0, i64 %n.val) ]
