; RUN: opt -bugpoint-enable-legacy-pm -vpo-paropt-target-capture-non-pointers-using-map-to=true -vpo-cfg-restructuring -vpo-paropt -S %s | FileCheck %s
; RUN: opt -vpo-paropt-target-capture-non-pointers-using-map-to=true -aa-pipeline=basic-aa -passes='function(vpo-cfg-restructuring),vpo-paropt' -S %s | FileCheck %s
