; RUN: opt %s -disable-output -passes=vplan-vec --debug-only=LoopVectorizationPlanner_vec_lengths --debug-only=LoopVectorizationPlanner 2>&1 | FileCheck %s
; REQUIRES: asserts
;
...
; void foo(int *arr){
;   #pragma omp simd
;   #pragma vector vectorlength(1, 0, 4, 1, 8)
...
!10 = !{!"llvm.loop.intel.vector.vectorlength", i64 1, i64 0, i64 4, i64 1, i64 8}
