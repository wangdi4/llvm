...
; optimized IR (no empty block for the false branch)
;
; REQUIRES: asserts
;
...
;
; RUN: opt -disable-output -passes=hir-ssa-deconstruction,hir-temp-cleanup,hir-vplan-vec -vplan-print-after-all-zero-bypass -vplan-all-zero-bypass-region-threshold=10000 %s 2>&1 | FileCheck %s
;
; CHECK:        i1 [[VP_COND:%.*]] = fcmp une float [[VP0:%.*]] float 0.000000e+00
; CHECK-NEXT:   br all.zero.bypass.begin9
;
; CHECK:      all.zero.bypass.begin9:
; CHECK-NEXT:   i1 [[VP_ZCHECK:%.*]] = all-zero-check i1 [[VP_COND]]
; CHECK-NEXT:   br i1 [[VP_ZCHECK]], all.zero.bypass.end11, [[BB3:BB[0-9]+]]
...
simd.loop.header:                                 ; preds = %simd.loop.latch, %simd.loop.preheader
  ...
  br i1 %mask.cond, label %simd.loop.then, label %simd.loop.latch
