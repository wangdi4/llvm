...
; REQUIRES: asserts
; RUN: opt %s -S -mattr=+avx512vl,+avx512cd -hir-ssa-deconstruction -hir-temp-cleanup -hir-vec-dir-insert -hir-vplan-vec -debug-only=parvec-analysis,VPlanHCFGBuilder 2>&1 | FileCheck %s
...
define dso_local float @_Z4foo1PfPi(float* noalias nocapture %A, i32* noalias nocapture readonly %B) local_unnamed_addr #0 {
entry:
  br label %for.body
...
for.body:                                         ; preds = %entry, %for.body
  ...
  store float %flt, float* %ptridx2, align 4
