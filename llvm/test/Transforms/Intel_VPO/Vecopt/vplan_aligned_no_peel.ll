...
; And will generate aligned stores/loads for "#pragma vector aligned"
...
; RUN: opt %s -S -passes='hir-ssa-deconstruction,hir-vec-dir-insert,hir-vplan-vec,print<hir>,hir-cg,simplifycfg,intel-ir-optreport-emitter' -vplan-enable-peeling -vplan-force-vf=8 -intel-opt-report=high 2>&1 | FileCheck %s --check-prefix=O3

; O2-NOT: peel.check
; O2-NOT: masked unaligned unit stride
; O2: masked aligned unit stride
; O2: %wide.load = load <8 x i32>, ptr {{%.*}}, align 32
; O2: store <8 x i32> {{%.*}}, ptr {{%.*}}, align 32
; O3-NOT: vector-peel
; O3-NOT: masked unaligned unit stride
; O3: masked aligned unit stride
; O3: %gepload = load <8 x i32>, ptr {{%.*}}, align 32
; O3: store <8 x i32> {{%.*}}, ptr {{%.*}}, align 32
