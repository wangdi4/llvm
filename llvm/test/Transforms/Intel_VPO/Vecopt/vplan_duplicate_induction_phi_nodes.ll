...
; RUN: opt -loopopt=0 -VPlanDriver -vpo-vplan-build-stress-test -vplan-print-plain-cfg -vplan-entities-dump -disable-output < %s 2>&1 | FileCheck %s
; RUN: opt -loopopt=0 -passes="vplan-driver" -vpo-vplan-build-stress-test -vplan-print-plain-cfg -vplan-entities-dump -disable-output < %s 2>&1 | FileCheck %s
