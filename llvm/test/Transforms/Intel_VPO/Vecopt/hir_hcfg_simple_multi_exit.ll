...
; RUN: opt -vplan-print-terminator-inst=false -hir-ssa-deconstruction -hir-vec-dir-insert -VPlanDriverHIR -vplan-print-after-initial-transforms -disable-output -vplan-force-vf=2 < %s 2>&1 | FileCheck %s
; RUN: opt -vplan-print-terminator-inst=false -passes="hir-ssa-deconstruction,hir-vec-dir-insert,vplan-driver-hir" -vplan-print-after-initial-transforms -disable-output -vplan-force-vf=2 < %s 2>&1 | FileCheck %s
