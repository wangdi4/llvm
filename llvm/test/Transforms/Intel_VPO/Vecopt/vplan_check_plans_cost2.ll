...
; RUN: opt -S -mattr=+avx512f -vplan-vec -vplan-print-after-evaluator -vplan-enable-peeling -vplan-enable-general-peeling-cost-model -vplan-enable-masked-variant -vplan-enable-vectorized-peel -disable-output %s 2>&1 | FileCheck %s --check-prefix=VECTOR-PEEL

; RUN: opt -S -mattr=+avx512f -vplan-vec -vplan-print-after-evaluator -vplan-enable-peeling -vplan-enable-general-peeling-cost-model -vplan-enable-masked-variant -vplan-enable-vectorized-peel -vplan-enable-masked-vectorized-remainder -disable-output %s 2>&1 | FileCheck %s --check-prefix=MASKED-PEEL-REMAINDER
