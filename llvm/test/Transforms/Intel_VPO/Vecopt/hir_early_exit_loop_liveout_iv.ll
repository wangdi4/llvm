...
; RUN: opt %s -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-last-value-computation,hir-vec-dir-insert,print<hir>,hir-vplan-vec" -disable-hir-last-value-computation-multi-exit-loop -vplan-print-after-transformed-early-exit-loop -vplan-enable-early-exit-loops -disable-output 2>&1 | FileCheck %s
...
; CHECK-NEXT:     i64 [[VP__PRIV_FINAL:%.*]] = private-final-uc i64 [[VP3]] i32 [[VP_EARLY_EXIT_LANE]]
; CHECK-NEXT:     i32 [[VP_EARLY_EXIT_ID:%.*]] = early-exit-id i32 [[VP_EXIT_ID_PHI]] i32 [[VP_EARLY_EXIT_LANE]]
; CHECK-NEXT:     i1 [[VP8:%.*]] = icmp eq i32 [[VP_EARLY_EXIT_ID]] i32 1
...
; CHECK-NEXT:  Id: 0   i64 [[VP__PRIV_FINAL]] -> [[VP9:%.*]] = {%indvars.iv.out}
;
entry:
  br label %for.body
