...
; RUN: opt -enable-new-pm=0 -hir-ssa-deconstruction -hir-framework -hir-vplan-vec -print-after=hir-vplan-vec -disable-output < %s 2>&1 -vplan-enable-new-cfg-merge-hir=1 | FileCheck %s --check-prefix=HIR-CHECK
; RUN: opt -passes='vplan-vec' -print-after=vplan-vec -disable-output < %s 2>&1 | FileCheck %s --check-prefix=IR-CHECK
; RUN: opt -passes='hir-ssa-deconstruction,print<hir>,hir-vplan-vec,print<hir>' -disable-output < %s 2>&1 -vplan-enable-new-cfg-merge-hir=0 | FileCheck %s --check-prefix=HIR-CHECK
; RUN: opt -passes='hir-ssa-deconstruction,print<hir>,hir-vplan-vec,print<hir>' -disable-output < %s 2>&1 -vplan-enable-new-cfg-merge-hir=1 | FileCheck %s --check-prefix=HIR-CHECK
;
