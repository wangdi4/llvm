...
; CHECK:   [DA: Div] float {{%.*}} = call ptr {{%.*}} ptr @cdfnormf [Serial]
; CHECK:   [DA: Div] float {{%.*}} = call ptr {{%.*}} ptr {{%.*}} ptr @atan2pi [Serial]
...
; CHECK-NEXT:    |   %extract.1. = extractelement &((<2 x ptr>)(%A)[i1 + <i64 0, i64 1> + 1]),  1;
...
; CHECK-NEXT:    |   {
; CHECK-NEXT:    |      %extract.1.7 = extractelement &((<2 x ptr>)(%A)[i1 + <i64 0, i64 1> + 1]),  1;
; CHECK-NEXT:    |      %extract.1.8 = extractelement &((<2 x ptr>)(%B)[i1 + <i64 0, i64 1> + 1]),  1;
...
define void @foo(ptr noalias dereferenceable(4) %A, ptr noalias dereferenceable(4) %B, ptr noalias nocapture writeonly dereferenceable(4) %R) local_unnamed_addr {
bb2.preheader:
...
bb2:                                              ; preds = %bb2.preheader, %if.end
  ...
  %A.ptr = getelementptr inbounds float, ptr %A, i64 %indvars.iv
  %invalidcdfnorm = call float @cdfnormf(ptr %A.ptr) #1
  %B.ptr = getelementptr inbounds float, ptr %B, i64 %indvars.iv
  %cond = icmp ugt i64 %indvars.iv, 14
  br i1 %cond, label %if.then, label %if.end

if.then:
  %func_result.i = tail call float (...) @atan2pi(ptr nonnull %A.ptr, ptr nonnull %B.ptr) #1
  br label %if.end

if.end:
  %res = phi float [ %func_result.i, %if.then ], [ 0.000000, %bb2 ]
  %R.ptr = getelementptr inbounds float, ptr %R, i64 %indvars.iv
  store float %res, ptr %R.ptr, align 1
...
declare dso_local float @cdfnormf(ptr) local_unnamed_addr
