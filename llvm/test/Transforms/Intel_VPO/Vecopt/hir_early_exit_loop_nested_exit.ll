...
; with nested exit condition as unsafe to vectorize.
...
; CHECK: EarlyExitVecSafety: Early exit condition is nested.
...
for.body:                                         ; preds = %for.body.preheader, %for.inc
  ...
  br i1 %cmp1, label %for.inc, label %nest.exit

nest.exit:                 ; preds = %for.body
  %cmp2 = icmp eq i8 %ld, %val2
  br i1 %cmp2, label %for.inc, label %cleanup.loopexit

for.inc:                                          ; preds = %for.body, %nest.exit
  %indvars.iv.next = add nuw nsw i64 %indvars.iv, 1
  %cmp = icmp slt i64 %indvars.iv.next, %0
  br i1 %cmp, label %for.body, label %cleanup.loopexit

cleanup.loopexit:                                 ; preds = %for.inc, %nest.exit
  br label %cleanup
