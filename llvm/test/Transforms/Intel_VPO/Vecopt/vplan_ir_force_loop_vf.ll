...
; RUN: opt < %s -VPlanDriver -S -vplan-force-vf=4 -vplan-force-loop-vf=2:2 -vplan-force-loop-vf=3:8  -debug-only=LoopVectorizationPlanner  2>&1 | FileCheck %s --check-prefixes=CHECK,LLVM --implicit-check-not load
; RUN: opt < %s -hir-ssa-deconstruction -hir-framework -VPlanDriverHIR -disable-output -vplan-force-vf=4 -vplan-force-loop-vf=2:2 -vplan-force-loop-vf=3:8  -debug-only=LoopVectorizationPlanner -print-after=VPlanDriverHIR  -enable-vp-value-codegen-hir=0 2>&1 | FileCheck %s --check-prefixes=CHECK,HIR
; RUN: opt < %s -hir-ssa-deconstruction -hir-framework -VPlanDriverHIR -disable-output -vplan-force-vf=4 -vplan-force-loop-vf=2:2 -vplan-force-loop-vf=3:8  -debug-only=LoopVectorizationPlanner -print-after=VPlanDriverHIR  -enable-vp-value-codegen-hir 2>&1 | FileCheck %s --check-prefixes=CHECK,HIR


; CHECK:      LVP: ForcedVF: 4
; CHECK-NEXT: LVP: Safelen: 4294967295
; CHECK-NEXT: LVP: MinVF: 4 MaxVF: 4
; CHECK-NEXT: Selecting VF for VPlan #1
; CHECK-NEXT: There is only VPlan with VF=4, selecting it.
; CHECK-NEXT: LVP: ForcedVF: 2
; CHECK-NEXT: LVP: Safelen: 4294967295
; CHECK-NEXT: LVP: MinVF: 2 MaxVF: 2
; CHECK-NEXT: Selecting VF for VPlan #2
; CHECK-NEXT: There is only VPlan with VF=2, selecting it.
; CHECK-NEXT: LVP: ForcedVF: 8
; CHECK-NEXT: LVP: Safelen: 4294967295
; CHECK-NEXT: LVP: MinVF: 8 MaxVF: 8
; CHECK-NEXT: Selecting VF for VPlan #3
