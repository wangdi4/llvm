...
; RUN: opt -vplan-print-terminator-inst=false -hir-ssa-deconstruction -hir-vec-dir-insert -hir-temp-cleanup -hir-last-value-computation -VPlanDriverHIR -disable-vplan-codegen -vplan-print-after-vpentity-instrs -vplan-enable-inmemory-entities -disable-output < %s 2>&1 | FileCheck %s
; RUN: opt -vplan-print-terminator-inst=false -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-last-value-computation,hir-vec-dir-insert,vplan-driver-hir" -disable-vplan-codegen -vplan-print-after-vpentity-instrs -vplan-enable-inmemory-entities -disable-output < %s 2>&1 | FileCheck %s
; REQUIRES: asserts
...
; CHECK-EMPTY:
; CHECK-NEXT:    [[BB1]]:
; CHECK-NEXT:     i32* [[VP5:%.*]] = allocate-priv i32*
...
; CHECK-NEXT:     i32 [[VP10:%.*]] = load i32* [[PTR_ADDR_PROMOTED0:%.*]]
; CHECK-NEXT:     i32 [[VP11:%.*]] = sext i8 [[C_PROMOTED0:%.*]] to i32
; CHECK-NEXT:     i32 [[VP12:%.*]] = mul i32 [[VP10]] i32 [[VP11]]
...
; CHECK-EMPTY:
; CHECK-NEXT:    [[BB3]]:
; CHECK-NEXT:     i32 [[VP14:%.*]] = load i32* [[VP5]]
; CHECK-NEXT:     i32 [[VP__RED_FINAL:%.*]] = reduction-final{u_add} i32 [[VP14]]
