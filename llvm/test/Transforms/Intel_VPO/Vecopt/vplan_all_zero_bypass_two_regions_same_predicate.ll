...
; REQUIRES: asserts
; RUN: opt -vplan-print-terminator-inst=false < %s -VPlanDriver -mtriple=x86_64-unknown-linux-gnu -tti -mattr=+sse4.2 -enable-intel-advanced-opts -vplan-enable-all-zero-bypass-non-loops -vplan-print-after-all-zero-bypass -vplan-force-vf=4 -vplan-all-zero-bypass-region-threshold=1 -disable-output -S 2>&1 | FileCheck %s
; RUN: opt -vplan-print-terminator-inst=false < %s -passes="vplan-driver" -mtriple=x86_64-unknown-linux-gnu -mattr=+sse4.2 -enable-intel-advanced-opts -vplan-enable-all-zero-bypass-non-loops -vplan-print-after-all-zero-bypass -vplan-force-vf=4 -vplan-all-zero-bypass-region-threshold=1 -disable-output -S 2>&1 | FileCheck %s
