; Check that proper HIR code is generated by vectorizer for non-masked uniform store with vector RHS (non-constant).
; NOTE: The innermost loop gets fully unrolled, so RHS of the uniform-store is a vector of the 2nd loop's indices.
...
; CHECK-NEXT: ([[ArrB]])[1] = [[E1]];
; CHECK-NEXT: [[E2:%.*]] = extractelement i2 + <i64 0, i64 1, i64 2, i64 3>,  3;
; CHECK-NEXT: ([[ArrB]])[2] = [[E2]];
; CHECK-NEXT: [[E3:%.*]] = extractelement i2 + <i64 0, i64 1, i64 2, i64 3>,  3;
; CHECK-NEXT: ([[ArrB]])[3] = [[E3]];
; CHECK-NEXT: [[E4:%.*]] = extractelement i2 + <i64 0, i64 1, i64 2, i64 3>,  3;
; CHECK-NEXT: ([[ArrB]])[4] = [[E4]];
; CHECK: END LOOP

; Check LLVM-IR
; CHECK: [[VecCast:%.*]] = bitcast i32* {{%.*}} to <4 x i32>*
; CHECK: [[E0:%.*]] = extractelement <4 x i32> {{%.*}}, i64 3
; CHECK-NEXT: store i32 [[E0]], i32* {{%.*}}
; CHECK: [[E1:%.*]] = extractelement <4 x i32> {{%.*}}, i64 3
; CHECK-NEXT: store i32 [[E1]], i32* {{%.*}}
; CHECK: [[E2:%.*]] = extractelement <4 x i32> {{%.*}}, i64 3
; CHECK-NEXT: store i32 [[E2]], i32* {{%.*}}
; CHECK: [[E3:%.*]] = extractelement <4 x i32> {{%.*}}, i64 3
; CHECK-NEXT: store i32 [[E3]], i32* {{%.*}}
; CHECK: [[E4:%.*]] = extractelement <4 x i32> {{%.*}}, i64 3
; CHECK-NEXT: store i32 [[E4]], i32* {{%.*}}
...
for.body9.us:                                     ; preds = %for.body9.us, %for.body4.us
  ...
  %2 = trunc i64 %indvars.iv22 to i32             ; store innermost loop index to %B
