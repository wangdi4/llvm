...
; RUN: opt -opaque-pointers=0 %s -S -passes=vplan-vec -vplan-force-vf=4 -vplan-force-uf=3 -vplan-print-after-unroll | FileCheck %s
; RUN: opt -opaque-pointers=0 %s -S -passes='hir-ssa-deconstruction,hir-temp-cleanup,hir-vplan-vec,print<hir>' -vplan-force-vf=4 -hir-details -disable-output 2>&1 | FileCheck %s --check-prefix=CHECK-HIR
