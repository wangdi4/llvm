...
; RUN: opt -opaque-pointers=0 < %s -passes=vplan-vec -disable-output -vplan-print-after-predicator -vplan-force-vf=2 | FileCheck %s --check-prefix=LLVM
; RUN: opt -opaque-pointers=0 < %s -S -passes=vplan-vec -vplan-force-vf=2 | FileCheck %s --check-prefix=LLVM-CG

; RUN: opt -opaque-pointers=0 < %s -passes=hir-vplan-vec -disable-output -vplan-print-after-predicator -vplan-force-vf=2 | FileCheck %s --check-prefix=HIR
; RUN: opt -opaque-pointers=0 < %s -S -passes=hir-vplan-vec  -vplan-force-vf=2 | FileCheck %s --check-prefix=HIR-CG
