...
; RUN: opt -hir-ssa-deconstruction -hir-framework -hir-vplan-vec -print-after=hir-vplan-vec -disable-output < %s 2>&1 | FileCheck %s --check-prefix=HIR-CHECK
; RUN: opt -vplan-vec -S < %s 2>&1 | FileCheck %s --check-prefix=LLVM-CHECK
;
...
; LLVM-CHECK-NEXT:    [[BROADCAST_SPLATINSERT20:%.*]] = insertelement <4 x i7*> poison, i7* [[ARRI70]], i32 0
; LLVM-CHECK-NEXT:    [[BROADCAST_SPLAT30:%.*]] = shufflevector <4 x i7*> [[BROADCAST_SPLATINSERT20]], <4 x i7*> poison, <4 x i32> zeroinitializer
