...
; RUN: opt -opaque-pointers=0 -passes='hir-ssa-deconstruction,hir-vplan-vec,print<hir>' -disable-output < %s 2>&1 | FileCheck %s --check-prefix=HIR-CHECK
; RUN: opt -opaque-pointers=0 -passes='vplan-vec' -S < %s 2>&1 | FileCheck %s --check-prefix=LLVM-CHECK
;
