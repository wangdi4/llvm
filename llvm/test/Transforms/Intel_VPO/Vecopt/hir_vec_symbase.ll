...
; RUN: opt -hir-ssa-deconstruction -hir-vec-dir-insert -VPlanDriverHIR -vplan-force-vf=4 -enable-vp-value-codegen-hir -print-before=VPlanDriverHIR -print-after=VPlanDriverHIR -hir-details -S -disable-output < %s 2>&1 | FileCheck %s
; RUN: opt -passes="hir-ssa-deconstruction,hir-vec-dir-insert,print<hir>,vplan-driver-hir,print<hir>" -aa-pipeline="basic-aa" -vplan-force-vf=4 -enable-vp-value-codegen-hir -hir-details -S -disable-output < %s 2>&1 | FileCheck %s
