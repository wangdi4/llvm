...
; RUN: opt -hir-ssa-deconstruction -hir-vec-dir-insert -hir-vplan-vec -vplan-force-vf=4 -enable-vp-value-codegen-hir -print-before=hir-vplan-vec -print-after=hir-vplan-vec -hir-details -S -disable-output < %s 2>&1 | FileCheck %s
; RUN: opt -passes="hir-ssa-deconstruction,hir-vec-dir-insert,print<hir>,hir-vplan-vec,print<hir>" -aa-pipeline="basic-aa" -vplan-force-vf=4 -enable-vp-value-codegen-hir -hir-details -S -disable-output < %s 2>&1 | FileCheck %s
