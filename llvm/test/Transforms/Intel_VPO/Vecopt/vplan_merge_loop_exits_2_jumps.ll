; Test merge loop exits transformation when the loop has two outside of the loop.
...
; CHECK-NEXT: SUCCESSORS(2):{{BB[0-9]+}}(i1 [[CMP_OUT5]]), {{IfBlock[0-9]+}}(!i1 [[CMP_OUT5]])
; CHECK-NEXT: PREDECESSORS(1): {{NewLoopLatch[0-9]+}}
; CHECK-EMPTY:

; CHECK-NEXT: {{IfBlock[0-9]+}} (BP: NULL) :
; CHECK-NEXT: i1 [[CMP_OUT6:%vp[0-9]+]] = icmp
; CHECK-NEXT: SUCCESSORS(2):{{BB[0-9]+}}(i1 [[CMP_OUT6]]), {{BB[0-9]+}}(!i1 [[CMP_OUT6]])
...
; CHECK-NEXT: {{BB[0-9]+}} (BP: NULL) :
; CHECK-NEXT: i32 [[PHI_OUT9:%vp[0-9]+]] = phi
; CHECK-NEXT: SUCCESSORS(1):{{BB[0-9]+}}
; CHECK-NEXT: PREDECESSORS(2): {{BB[0-9]+}} {{BB[0-9]+}}
; CHECK-EMPTY:
...
loop2_latch:
  ...
  br i1 %cmp3, label %loop2, label %bb6
...
bb7:
  %m6 = phi i32 [ 0, %bb6 ], [ 1, %bb5 ]
  br label %loop1_latch

loop1_latch:
  %i_inc = add nsw i32 %i, %m6
