...
; CHECK-NEXT:   (FloatMinimum) Start: float [[TMP0:%.*]] Exit: float [[VP__MIN_0:%.*]]
; CHECK-NEXT:    Linked values: float [[VP_MIN_014:%.*]], float [[VP__MIN_0]], float [[VP_REDMINMAX_RED_INIT:%.*]], float [[VP_REDMINMAX_RED_FINAL:%.*]],
...
; CHECK:    [[TMP6]] = call <4 x float> @llvm.minimum.v4f32(<4 x float> [[WIDE_LOAD0I:%.*]], <4 x float> [[VEC_PHI50]])

; CHECK:  VPlannedBB5:
; CHECK:    [[TMP11:%.*]] = call float @llvm.vector.reduce.fminimum.v4f32(<4 x float> [[TMP6]])
...
for.cond.cleanup.loopexit:
  %.Min.0.lcssa = phi float [ %.Min.0, %for.body ]
  br label %end.simd
