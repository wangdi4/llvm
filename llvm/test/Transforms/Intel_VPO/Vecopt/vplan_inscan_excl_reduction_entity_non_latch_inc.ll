...
; CHECK-NEXT:     i64 [[VP_INDVARS_IV]] = phi  [ i64 [[VP_INDVARS_IV_IND_INIT]], [[BB2]] ],  [ i64 [[VP_INDVARS_IV_NEXT]], new_latch ]
; CHECK-NEXT:     float [[VP_INSCAN_ACCUM:%.*]] = phi  [ float [[VP_X_REDINSCAN_RED_INIT]], [[BB2]] ],  [ float [[VP3:%.*]], new_latch ]
; CHECK-NEXT:     i32 [[VP4:%.*]] = phi  [ i32 [[VP_I_LINEAR_IV_IND_INIT]], [[BB2]] ],  [ i32 [[VP5:%.*]], new_latch ]
; CHECK-NEXT:     call i64 4 ptr [[VP_I_LINEAR_IV]] ptr @llvm.lifetime.start.p0
...
; CHECK-NEXT:     i32 [[VP6:%.*]] = load ptr [[VP_I_LINEAR_IV]]
; CHECK-NEXT:     i64 [[VP_IDXPROM1:%.*]] = sext i32 [[VP6]] to i64
...
; CHECK-NEXT:     float [[VP9:%.*]] = load ptr [[VP_X_RED]]
; CHECK-NEXT:     ptr [[VP_ARRAYIDX:%.*]] = getelementptr inbounds float, ptr [[B0:%.*]] i64 [[VP_INDVARS_IV]]
; CHECK-NEXT:     store float [[VP9]] ptr [[VP_ARRAYIDX]]
...
; CHECK-NEXT:  Id: 0     [[ADD3_LCSSA0:%.*]] = phi float [ [[ADD30:%.*]], [[DIR_VPO_END_GUARD_MEM_MOTION_40:%.*]] ] float [[VP_ADD3_PRIV_FINAL]] -> float [[ADD30]]
; CHECK-EMPTY:
...
DIR.VPO.END.GUARD.MEM.MOTION.6:                   ; preds = %DIR.OMP.END.SCAN.2
