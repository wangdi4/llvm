...
; RUN: opt -vplan-print-terminator-inst=false -vector-library=SVML -hir-ssa-deconstruction -hir-framework -VPlanDriverHIR -vplan-force-vf=4 -vplan-force-uf=2 -vplan-print-after-unroll -print-after=VPlanDriverHIR -disable-output -enable-vp-value-codegen-hir=0 < %s 2>&1 | FileCheck %s
; RUN: opt -vplan-print-terminator-inst=false -vector-library=SVML -hir-ssa-deconstruction -hir-framework -VPlanDriverHIR -vplan-force-vf=4 -vplan-force-uf=2 -vplan-print-after-unroll -print-after=VPlanDriverHIR -disable-output -enable-vp-value-codegen-hir < %s 2>&1 | FileCheck %s
