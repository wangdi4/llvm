...
; RUN: opt -vector-library=SVML -hir-ssa-deconstruction -hir-framework -hir-vplan-vec -vplan-force-vf=4 -vplan-force-uf=2 -vplan-print-after-unroll -print-after=hir-vplan-vec -disable-output < %s 2>&1 -vplan-enable-new-cfg-merge-hir=0 | FileCheck %s
; RUN: opt -passes="hir-ssa-deconstruction,hir-vplan-vec,print<hir>" -vector-library=SVML -vplan-force-vf=4 -vplan-force-uf=2 -vplan-print-after-unroll -disable-output < %s 2>&1 -vplan-enable-new-cfg-merge-hir=0 | FileCheck %s
; RUN: opt -vector-library=SVML -hir-ssa-deconstruction -hir-framework -hir-vplan-vec -vplan-force-vf=4 -vplan-force-uf=2 -vplan-print-after-unroll -print-after=hir-vplan-vec -disable-output < %s 2>&1 -vplan-enable-new-cfg-merge-hir=1 | FileCheck %s
; RUN: opt -passes="hir-ssa-deconstruction,hir-vplan-vec,print<hir>" -vector-library=SVML -vplan-force-vf=4 -vplan-force-uf=2 -vplan-print-after-unroll -disable-output < %s 2>&1 -vplan-enable-new-cfg-merge-hir=1 | FileCheck %s
...
; CHECK-NEXT:     [DA: Uni] i64 [[VP_VECTOR_TRIP_COUNT:%.*]] = vector-trip-count i64 80, UF = 2
