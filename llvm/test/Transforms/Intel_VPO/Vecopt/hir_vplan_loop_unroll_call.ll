...
; RUN: opt -vector-library=SVML -hir-ssa-deconstruction -hir-framework -hir-vplan-vec -vplan-force-vf=4 -vplan-force-uf=2 -vplan-print-after-unroll -print-after=hir-vplan-vec -disable-output -enable-vp-value-codegen-hir < %s 2>&1 | FileCheck %s
; RUN: opt -passes="hir-ssa-deconstruction,hir-vplan-vec,print<hir>" -vector-library=SVML -vplan-force-vf=4 -vplan-force-uf=2 -vplan-print-after-unroll -disable-output -enable-vp-value-codegen-hir < %s 2>&1 | FileCheck %s
