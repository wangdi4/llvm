; REQUIRES: asserts
; RUN: opt < %s -VPlanDriver -disable-output -vplan-dump-da -vplan-force-vf=2 2>&1 | FileCheck %s
...
; CHECK-NEXT: Uniform: [Shape: Uniform] i64 [[VAL12:%vp.*]] = add i64 [[VAL3:%vp.*]] i64 1
; CHECK-NEXT: Uniform: [Shape: Uniform] i1 [[VAL13:%vp.*]] = icmp i64 [[VAL12:%vp.*]] i64 [[VAL6:%n]]
; CHECK-LABEL: Basic Block: BB5
; CHECK-NEXT: Divergent: [Shape: Unit Stride, Stride: i64 1] i64 [[VAL2:%vp.*]] = add i64 [[VAL1:%vp.*]] i64 1
; CHECK-NEXT: Uniform: [Shape: Uniform] i1 [[VAL14:%vp.*]] = icmp i64 [[VAL2:%vp.*]] i64 [[VAL6:%n]]
