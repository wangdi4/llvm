...
define dso_local void @foo(ptr nocapture noalias readonly %ptr1,
; CHECK:    Cost 2 for i32 [[VP_LOAD:%.*]] = load ptr [[PTR10:%.*]]
; TODO: the second load is not broadcast but CM doesn't check SVA yet.
; The cost should be 1 for this load when CM is fixed.
; CHECK:    Cost 2 for i32 [[VP_LOAD_1:%.*]] = load ptr [[PTR20:%.*]]
; CHECK:    Cost 2 for store i32 [[VP_LOAD_1]] ptr [[PTR40:%.*]]
; CHECK:    Cost 2 for store i32 [[VP4:%.*]] ptr [[PTR30:%.*]]
;
  ptr nocapture noalias readonly %ptr2,
  ptr nocapture noalias writeonly %ptr3,
  ptr nocapture noalias writeonly %ptr4) {
entry:
...
for.body:                                         ; preds = %entry, %for.body
  ...
  %idx = getelementptr inbounds [1024 x i32], ptr @a, i64 0, i64 %iv
  %cnst.vec = load i32, ptr %ptr1, align 8
  %cnst.scal = load i32, ptr %ptr2, align 8
  %wvar = load i32, ptr %idx, align 16
  %cond = icmp ult i32 %cnst.vec, 0
  br i1 %cond, label %if.cond, label %else.cond

if.cond:
  store i32 %cnst.scal, ptr %ptr4, align 8
  br label %else.fi

else.cond:
  %res = add nsw nuw i32 %cnst.vec, %wvar
  store i32 %res, ptr %ptr3, align 8
  br label %else.fi
