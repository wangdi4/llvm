...
; RUN: opt -vplan-print-terminator-inst=false < %s -VPlanDriver -vplan-force-vf=2 -disable-output -vplan-print-after-linearization 2>&1 | FileCheck %s --check-prefixes=VPLAN
; RUN: opt -vplan-print-terminator-inst=false < %s -VPlanDriver -vplan-force-vf=2 -S 2>&1 | FileCheck %s --check-prefixes=CG
; RUN: opt -vplan-print-terminator-inst=false < %s -vplan-force-vf=2 -hir-ssa-deconstruction -hir-framework -VPlanDriverHIR -remove-region-directives -disable-output -print-after=VPlanDriverHIR 2>&1 | FileCheck %s --check-prefixes=HIR-CG
