...
; RUN: opt -S < %s -passes='function(vpo-cfg-restructuring),vplan-pragma-omp-ordered-simd-extract,print' -disable-output --vpo-utils-add-typed-privates=false 2>&1 | FileCheck %s --check-prefixes=CHECK,UNTYPED
; RUN: opt -S < %s -passes='function(vpo-cfg-restructuring),vplan-pragma-omp-ordered-simd-extract,print' -disable-output 2>&1 | FileCheck %s --check-prefixes=CHECK,TYPED
