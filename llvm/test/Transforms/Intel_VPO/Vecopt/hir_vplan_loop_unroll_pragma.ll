...
; RUN: opt -S < %s -hir-ssa-deconstruction -hir-temp-cleanup -hir-vplan-vec -vplan-force-vf=4 -print-after=hir-vplan-vec -disable-output 2>&1 | FileCheck %s -check-prefixes=VPVALCG,PM1
; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-vplan-vec" -S < %s -vplan-force-vf=4 -print-after=hir-vplan-vec -disable-output 2>&1 | FileCheck %s -check-prefixes=VPVALCG,PM2
