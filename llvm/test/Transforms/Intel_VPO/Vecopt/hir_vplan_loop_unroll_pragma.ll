...
; RUN: opt -vplan-print-terminator-inst=false -S < %s -hir-ssa-deconstruction -hir-temp-cleanup -VPlanDriverHIR -vplan-force-vf=4 -enable-vp-value-codegen-hir=0 -vplan-print-after-unroll -print-after=VPlanDriverHIR -disable-output 2>&1 | FileCheck %s
; RUN: opt -vplan-print-terminator-inst=false -S < %s -hir-ssa-deconstruction -hir-temp-cleanup -VPlanDriverHIR -vplan-force-vf=4 -enable-vp-value-codegen-hir -print-after=VPlanDriverHIR -disable-output 2>&1 | FileCheck %s --check-prefix=VPVALCG
