...
; RUN: opt -enable-new-pm=0 -hir-ssa-deconstruction -hir-temp-cleanup -hir-vplan-vec -vplan-force-vf=4 -vplan-print-after-unroll -print-after=hir-vplan-vec -disable-output < %s 2>&1 | FileCheck %s -check-prefixes=CHECK,VPVALCG
; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-vplan-vec,print<hir>" -vplan-force-vf=4 -vplan-print-after-unroll -disable-output < %s 2>&1 | FileCheck %s -check-prefixes=CHECK,VPVALCG
