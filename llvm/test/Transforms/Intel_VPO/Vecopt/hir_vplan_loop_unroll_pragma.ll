...
; RUN: opt -S < %s -hir-ssa-deconstruction -hir-temp-cleanup -VPlanDriverHIR -vplan-force-vf=4 -enable-vp-value-codegen-hir=0 -vplan-print-after-unroll -print-after=VPlanDriverHIR -disable-output 2>&1 | FileCheck %s
; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,vplan-driver-hir" -S < %s -vplan-force-vf=4 -enable-vp-value-codegen-hir=0 -vplan-print-after-unroll -print-after=vplan-driver-hir -disable-output 2>&1 | FileCheck %s

; RUN: opt -S < %s -hir-ssa-deconstruction -hir-temp-cleanup -VPlanDriverHIR -vplan-force-vf=4 -enable-vp-value-codegen-hir -print-after=VPlanDriverHIR -disable-output 2>&1 | FileCheck %s --check-prefix=VPVALCG
; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,vplan-driver-hir" -S < %s -vplan-force-vf=4 -enable-vp-value-codegen-hir -print-after=vplan-driver-hir -disable-output 2>&1 | FileCheck %s --check-prefix=VPVALCG
...
; CHECK-NEXT:     [DA: Uni] i64 [[VP_VECTOR_TRIP_COUNT:%.*]] = vector-trip-count i64 [[VP0]], UF = 3
