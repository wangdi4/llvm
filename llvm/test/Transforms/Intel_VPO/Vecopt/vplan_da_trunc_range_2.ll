...
; Loop iv is outside signed i32 range. Check that trunc is random.

define i32 @main() local_unnamed_addr {
;
; CHECK:  Divergent: [Shape: Random] i32 [[VP_CONV:%.*]] = trunc i64
;
DIR.OMP.SIMD.1:
  %i.linear.iv = alloca i64, align 8
  %b = alloca [2147483648 x i32], align 16
...
omp.inner.for.body:                               ; preds = %DIR.OMP.SIMD.117, %omp.inner.for.body
  ...
  %arrayidx = getelementptr inbounds [2147483648 x i32], ptr %b, i64 0, i64 %.omp.iv.local.013
  store i32 %conv, ptr %arrayidx, align 4
  %add1 = add nuw nsw i64 %.omp.iv.local.013, 1
  %exitcond.not = icmp eq i64 %add1, 2147483648
