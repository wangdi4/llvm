; RUN: opt -mtriple=x86_64 -passes=vplan-vec -vplan-force-vf=2 -vplan-print-after-call-vec-decisions -disable-output < %s | FileCheck %s
; RUN: opt -mtriple=x86_64 -passes=vplan-vec -vplan-force-vf=2 -S < %s | FileCheck %s -check-prefix=VPLANCG
; RUN: opt -mtriple=x86_64 -passes='hir-ssa-deconstruction,hir-vplan-vec,hir-cg' -vplan-force-vf=2 -S < %s | FileCheck %s -check-prefix=HIRCG
...
define dso_local void @run_test.P(ptr noundef %vals, ptr nocapture noundef readonly %src, ptr nocapture noundef writeonly %dst) local_unnamed_addr #2 {
; CHECK-LABEL: VPlan after CallVecDecisions analysis for merged CFG:
; CHECK-NEXT:  VPlan IR for: run_test.P:omp.inner.for.body.#1
; XMM (SSE2) vector variant call
; CHECK:       [DA: Div] double [[VP0:%.*]] = call double [[VP1:%.*]] ptr %vals _ZGVxN2vu_Interpolate [x 1]
;
; VPLANCG:  define dso_local void @run_test.P(ptr noundef [[VALS0:%.*]], ptr nocapture noundef readonly [[SRC0:%.*]], ptr nocapture noundef writeonly [[DST0:%.*]]) local_unnamed_addr #2 {
; VPLANCG:  [[TMP4:%.*]] = call fast x86_regcallcc nofpclass(nan inf) <2 x double> @_ZGVxN2vu_Interpolate(<2 x double> noundef nofpclass(nan inf) [[WIDE_LOAD0:%.*]], ptr noundef [[VALS0]]) #0
...
;
; HIRCG:    define dso_local void @run_test.P(ptr noundef [[VALS0:%.*]], ptr nocapture noundef readonly [[SRC0:%.*]], ptr nocapture noundef writeonly [[DST0:%.*]]) local_unnamed_addr #2 {
; HIRCG:    [[CALLRET0:%.*]] = call fast nofpclass(nan inf) double @Interpolate(double noundef nofpclass(nan inf) [[TMP2:%.*]], ptr noundef [[VALS0]]) #0
; HIRCG:    [[TMP14:%.*]] = call fast x86_regcallcc nofpclass(nan inf) <2 x double> @_ZGVxN2vu_Interpolate(<2 x double> noundef nofpclass(nan inf) [[T30_0:%.*]], ptr noundef [[VALS0]]) #0
;
entry:
...
attributes #2 = { noinline nounwind uwtable "advanced-optim"="true" "approx-func-fp-math"="true" "denormal-fp-math"="preserve-sign,preserve-sign" "loopopt-pipeline"="full" "may-have-openmp-directive"="true" "min-legal-vector-width"="0" "no-infs-fp-math"="true" "no-nans-fp-math"="true" "no-signed-zeros-fp-math"="true" "no-trapping-math"="true" "pre_loopopt" "stack-protector-buffer-size"="8" "target-cpu"="corei7" "target-features"="+crc32,+cx16,+cx8,+fxsr,+mmx,+popcnt,+sahf,+sse,+sse2,+sse3,+sse4.1,+sse4.2,+ssse3,+x87" "tune-cpu"="corei7" "unsafe-fp-math"="true" }
