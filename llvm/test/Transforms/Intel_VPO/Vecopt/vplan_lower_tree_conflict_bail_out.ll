...
; REQUIRES: asserts
; RUN: opt -S -enable-new-pm=0 -mattr=+avx512vl,+avx512cd -hir-ssa-deconstruction -hir-temp-cleanup -hir-vec-dir-insert -hir-vplan-vec -vplan-force-vf=4 -disable-output -debug-only=vplan-vec -debug-only=LoopVectorizationPlanner < %s 2>&1 | FileCheck %s
; RUN: opt -S -mattr=+avx512vl,+avx512cd -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-vec-dir-insert,hir-vplan-vec" -vplan-force-vf=4 -disable-output -debug-only=vplan-vec -debug-only=LoopVectorizationPlanner < %s 2>&1 | FileCheck %s
