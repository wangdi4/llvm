...
; RUN: opt -S -vector-library=SVML -vplan-vec -vplan-force-vf=4 -vplan-force-uf=3 \
...
; CHECK-NEXT:     [DA: Div] float [[VP2:%.*]] = load float* [[VP_ARRAYIDX_2]]
...
; CHECK-NEXT:    [[SCALAR_GEP40:%.*]] = getelementptr inbounds float, float* [[A0]], i64 [[TMP5]]
; CHECK-NEXT:    [[TMP7:%.*]] = bitcast float* [[SCALAR_GEP40]] to <4 x float>*
; CHECK-NEXT:    [[WIDE_LOAD50:%.*]] = load <4 x float>, <4 x float>* [[TMP7]], align 4
; CHECK-NEXT:    [[TMP8:%.*]] = call svml_cc <4 x float> @__svml_sinf4(<4 x float> [[WIDE_LOAD50]])
; CHECK-NEXT:    [[TMP9:%.*]] = bitcast float* [[SCALAR_GEP40]] to <4 x float>*
...
; CHECK-NEXT:    [[SCALAR_GEP70:%.*]] = getelementptr inbounds float, float* [[A0]], i64 [[TMP11]]
; CHECK-NEXT:    [[TMP13:%.*]] = bitcast float* [[SCALAR_GEP70]] to <4 x float>*
; CHECK-NEXT:    [[WIDE_LOAD80:%.*]] = load <4 x float>, <4 x float>* [[TMP13]], align 4
; CHECK-NEXT:    [[TMP14:%.*]] = call svml_cc <4 x float> @__svml_sinf4(<4 x float> [[WIDE_LOAD80]])
; CHECK-NEXT:    [[TMP15:%.*]] = bitcast float* [[SCALAR_GEP70]] to <4 x float>*
