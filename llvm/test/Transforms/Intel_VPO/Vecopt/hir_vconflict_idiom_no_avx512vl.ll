...
; REQUIRES: asserts
; RUN: opt -mattr=+avx512cd -hir-ssa-deconstruction -hir-vec-dir-insert -hir-vplan-vec -print-after=hir-vplan-vec -vplan-force-vf=8 < %s 2>&1 | FileCheck %s --check-prefix=VEC
; RUN: opt -mattr=+avx512cd -hir-ssa-deconstruction -hir-vec-dir-insert -hir-vplan-vec -print-after=hir-vplan-vec -vplan-force-vf=4 -debug-only=LoopVectorizationPlanner < %s 2>&1 | FileCheck %s --check-prefix=NO-VEC
...
define dso_local void @_Z4foo1PfPi(float* noalias nocapture %A, i64* noalias nocapture readonly %B) local_unnamed_addr #0 {
entry:
  br label %for.body
...
for.body:                                         ; preds = %entry, %for.body
  ...
  %ptridx = getelementptr inbounds i64, i64* %B, i64 %indvars.iv
  %0 = load i64, i64* %ptridx, align 4
  %ptridx2 = getelementptr inbounds float, float* %A, i64 %0
