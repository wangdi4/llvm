...
; pointers like (%src + %x + %i1) and (%dst + %x + %1) can be properly
...
;
;   void foo(int *dst, int *src, int x, int size) {
...
; CHECK-NEXT:        |   %1 = (%src)[i1 + 5 * %x + 7];
; CHECK-NEXT:        |   %2 = i1  *  i1;
; CHECK-NEXT:        |   %3 = trunc.i64.i32(%2);
; CHECK-NEXT:        |   (%dst)[i1 + 5 * %x + 7] = %1 + %3;
; CHECK-NEXT:        + END LOOP
; CHECK-NEXT:<{{.*}}>
...
entry:
  %cmp16 = icmp sgt i32 %size, 0
  br i1 %cmp16, label %for.body.lr.ph, label %for.cond.cleanup

for.body.lr.ph:                                   ; preds = %entry
  %mul = mul nsw i32 %x, 5
  %add = add i32 %mul, 7
  %wide.trip.count19 = zext i32 %size to i64
  br label %for.body
...
for.body:                                         ; preds = %for.body.lr.ph, %for.body
  ...
  %ptridx = getelementptr inbounds i32, i32* %src, i64 %idxprom
  %1 = load i32, i32* %ptridx, align 4
  %2 = mul nsw i64 %indvars.iv, %indvars.iv
  %3 = trunc i64 %2 to i32
  %add3 = add nsw i32 %1, %3
  %ptridx8 = getelementptr inbounds i32, i32* %dst, i64 %idxprom
