...
; <3>          |   %0 = (@a)[0][i1];
; <4>          |   %min.011 = @llvm.minimum.f32(%0,  %min.011); <Safe Reduction>
; <11>         + END LOOP
...
; CHECK-NEXT:               |   %llvm.minimum.v8f32 = @llvm.minimum.v8f32(%.vec,  %phi.temp);
; CHECK-NEXT:               |   %phi.temp = %llvm.minimum.v8f32;
; CHECK-NEXT:          + END LOOP
; CHECK:               %min.011 = @llvm.vector.reduce.fminimum.v8f32(%llvm.minimum.v8f32);
; CHECK:               END REGION
...
for.body:                                         ; preds = %entry, %for.body
  ...
  %1 = tail call fast float @llvm.minimum.f32(float %0, float %min.011)
