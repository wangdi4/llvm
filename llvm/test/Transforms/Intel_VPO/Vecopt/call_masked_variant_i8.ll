...
; RUN: opt -mtriple=x86_64 -passes=vplan-vec -vplan-force-vf=64 -S < %s | FileCheck %s -check-prefix=VPLANCGVF64
; RUN: opt -mtriple=x86_64 -passes=vplan-vec -vplan-force-vf=128 -S < %s | FileCheck %s -check-prefix=VPLANCGVF128
; RUN: opt -mtriple=x86_64 -passes='hir-ssa-deconstruction,hir-vplan-vec,print<hir>' -vplan-force-vf=64 -disable-output < %s 2>&1 | FileCheck %s -check-prefix=HIRVF64
; RUN: opt -mtriple=x86_64 -passes='hir-ssa-deconstruction,hir-vplan-vec,print<hir>' -vplan-force-vf=128 -disable-output < %s 2>&1 | FileCheck %s -check-prefix=HIRVF128
