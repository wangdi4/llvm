...
; RUN: opt -disable-output -vplan-vec -vplan-print-after-vpentity-instrs -vplan-entities-dump -disable-vplan-codegen  -vplan-force-vf=2 -disable-output -vplan-force-inscan-reduction-vectorization=true -S  < %s 2>&1 | FileCheck %s
; RUN: opt -disable-output -passes="vplan-vec" -vplan-print-after-vpentity-instrs -vplan-entities-dump -disable-vplan-codegen  -vplan-force-vf=2 -disable-output -vplan-force-inscan-reduction-vectorization=true -S  < %s 2>&1 | FileCheck %s
; REQUIRES: asserts
