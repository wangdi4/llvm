...
;       |   if ((%C)[i1] != 0)
;       |   {
;       |      (%B)[%j.014] = (%A)[i1 + sext.i32.i64(%j.014)];
;       |      %j.014 = %j.014  +  1;
;       |   }
;       + END LOOP
; END REGION

; CHECK:      [Compress/Expand Idiom] Increment {sb:3}+1 detected: <16>         %j.014 = %j.014  +  1;
; CHECK-NEXT: [Compress/Expand Idiom] IV found in UseMemRef: (%A)[i1 + sext.i32.i64(%j.014)]
...
if.then:                                          ; preds = %for.body
  ...
  %sum = add i64 %indvars.iv, %idxprom4
  %arrayidx3 = getelementptr inbounds double, double* %A, i64 %sum
