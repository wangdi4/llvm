...
; CHECK-NEXT: SUCCESSORS(2):{{BB[0-9]+}}(i1 [[CMP_OUT1]]), {{BB[0-9]+}}(!i1 [[CMP_OUT1]])
...
; CHECK-NEXT: {{BB[0-9]+}} (BP: NULL) :
; CHECK-NEXT: i32 [[ADD_OUT3:%vp.*]] = add
; CHECK-NEXT: i1 [[CMP_OUT3:%vp.*]] = icmp
; CHECK-NEXT: SUCCESSORS(2):{{BB[0-9]+}}(i1 [[CMP_OUT3]]), {{IntermediateBB[0-9]+}}(!i1 [[CMP_OUT3]])
; CHECK-NEXT: PREDECESSORS(2): {{BB[0-9]+}} {{BB[0-9]+}}
; CHECK-EMPTY:

; CHECK-NEXT: {{IntermediateBB[0-9]+}} (BP: NULL) :
; CHECK-NEXT: i32 [[PHI_OUT2:%vp.*]] = phi
...
; CHECK-NEXT: i32 [[PHI_OUT3:%vp.*]] = phi  [ i32 [[PHI_OUT1]], [[OrigLoopLatch]] ],  [ i32 1, {{IntermediateBB[0-9]+}} ]
; CHECK-NEXT: i1 [[PHI_OUT4:%vp.*]] = phi  [ i1 true, [[OrigLoopLatch]] ],  [ i1 false, {{IntermediateBB[0-9]+}} ]
...
; CHECK-NEXT: {{BB[0-9]+}} (BP: NULL) :
; CHECK-NEXT: i32 [[ADD_OUT5:%vp.*]] = add
; CHECK-NEXT: SUCCESSORS(1):{{BB[0-9]+}}
...
; CHECK-NEXT: i1 [[CMP_OUT7:%vp.*]] = icmp
; CHECK-NEXT: SUCCESSORS(2):{{BB[0-9]+}}(i1 [[CMP_OUT7]]), {{BB[0-9]+}}(!i1 [[CMP_OUT7]])
; CHECK-NEXT: PREDECESSORS(1): {{BB[0-9]+}}
; CHECK-EMPTY:
...
bb3:
  %m1 = add nsw i32 %j, 2
  %cmp3 = icmp eq i32 %m1, 16
  br i1 %cmp3, label %bb1, label %bb4
