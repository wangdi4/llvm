...
; Tests how #pragma omp ordered simd is processed for reductions.
...
; CHECK:       codeRepl:
; CHECK-NEXT:    call void @var_tripcount.ordered.simd.region(ptr [[IP:%.*]], i32 [[N]])
...
ordered.entry:
  ...
  br label %ordered.loop

ordered.loop:
  %indvars.iv.2 = phi i32 [ 0, %ordered.entry ], [ %indvars.iv.next.2, %ordered.loop ]
  %acc = phi i32 [ 0, %ordered.entry ], [ %sum, %ordered.loop ]
  %arrayidx.2 = getelementptr inbounds i32, ptr %ip, i32 %indvars.iv.2
  %ld = load i32, ptr %arrayidx.2, align 4
  %sum = add nsw i32 %ld, %acc
  %indvars.iv.next.2 = add nuw nsw i32 %indvars.iv.2, 1
  %exitcond.2 = icmp eq i32 %indvars.iv.next.2, %n
  br i1 %exitcond.2, label %ordered.exit, label %ordered.loop
