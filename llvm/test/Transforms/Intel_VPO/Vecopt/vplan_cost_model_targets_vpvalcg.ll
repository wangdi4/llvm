...
; RUN: opt -opaque-pointers=0 < %s -S -passes=vplan-vec  -mtriple=x86_64-unknown-unknown -mattr=+avx2 \
; RUN:     -vplan-cost-model-print-analysis-for-vf=8 -vplan-force-vf=8 -disable-output \
; RUN:     | FileCheck %s --check-prefix=VPLAN-CM-AVX2

; RUN: opt -opaque-pointers=0 < %s -S -passes=vplan-vec  -mtriple=x86_64-unknown-unknown -mattr=+sse2 \
; RUN:     -vplan-cost-model-print-analysis-for-vf=8 -vplan-force-vf=8 -disable-output \
...
; RUN: opt -opaque-pointers=0 < %s -S -passes='vplan-vec,instcombine,simplifycfg,print<cost-model>'  -mtriple=x86_64-unknown-unknown -mattr=+avx2 \
; RUN:     -vplan-force-vf=8 2>&1 | FileCheck %s --check-prefix=LLVM-CM-AVX2

; RUN: opt -opaque-pointers=0 < %s -S -passes='vplan-vec,instcombine,simplifycfg,print<cost-model>'  -mtriple=x86_64-unknown-unknown -mattr=+sse2 \
