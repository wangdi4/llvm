; RUN: opt -hir-ssa-deconstruction -hir-vec-dir-insert -hir-vplan-vec -print-after=hir-vplan-vec -hir-details -vplan-force-vf=8 -disable-output -enable-vp-value-codegen-hir < %s 2>&1 | FileCheck %s
; RUN: opt -passes="hir-ssa-deconstruction,hir-vec-dir-insert,hir-vplan-vec,print<hir>" -hir-details -vplan-force-vf=8 -disable-output -enable-vp-value-codegen-hir < %s 2>&1 | FileCheck %s
...
; CHECK: + DO i32 i1 = 0, 8 * %tgu + -1, 8   <DO_LOOP>  <MAX_TC_EST = 268435455>
...
; RUN: opt -hir-ssa-deconstruction -hir-vec-dir-insert -hir-vplan-vec -print-after=hir-vplan-vec -hir-details -vplan-force-vf=8 -hir-ignore-wraparound -disable-output -enable-vp-value-codegen-hir < %s 2>&1 | FileCheck %s --check-prefix=IGNORE-WRAP
; RUN: opt -passes="hir-ssa-deconstruction,hir-vec-dir-insert,hir-vplan-vec,print<hir>" -hir-details -vplan-force-vf=8 -hir-ignore-wraparound -disable-output -enable-vp-value-codegen-hir < %s 2>&1 | FileCheck %s --check-prefix=IGNORE-WRAP
