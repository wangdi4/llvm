...
; vectorizing peel.
;
; RUN: opt -disable-output -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-vec-dir-insert,hir-vplan-vec" -vplan-enable-peeling -vplan-enable-vectorized-peel -vplan-enable-peel-rem-strip=0 %s -print-after=hir-vplan-vec -vplan-force-vf=8 2>&1 | FileCheck %s
...
; Peel
; CHECK:        |   + DO i2 = 0, [[LOOP_UB0:%.*]], 8   <DO_LOOP>  <MAX_TC_EST = 1>  <LEGAL_MAX_TC = 1> <vector-peel> <nounroll> <novectorize> <max_trip_count = 1>
; CHECK:        |   + END LOOP
; Main loop
