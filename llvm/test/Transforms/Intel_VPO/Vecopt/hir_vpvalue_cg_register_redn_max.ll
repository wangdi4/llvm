; Test to check VPlan's VPValue-based HIR vector codegen for simple in-register max reduction, both integer and float.

; Incoming HIR into vectorizer (integer version)
; <0>    BEGIN REGION { }
...
; CHECK: Function: foo_int
; CHECK: if (0 <u 4 * [[UB:%.*]])
; CHECK: [[RED_VAR:%.*]] = [[RED_INIT:%.*]];
; CHECK: DO i1 = 0, 4 * [[UB]] + -1, 4   <DO_LOOP>
; CHECK: [[VEC_LD:%.*]] = (<4 x i32>*)(%ptr)
; CHECK: [[RED_VAR]] = ([[RED_VAR]] > [[VEC_LD]]) ? [[RED_VAR]] : [[VEC_LD]];
; CHECK: END LOOP
; CHECK: [[RED_INIT]] = @llvm.experimental.vector.reduce.smax.v4i32([[RED_VAR]]);

; CHECK: Function: foo_float
; CHECK: if (0 <u 4 * [[UB:%.*]])
; CHECK: [[RED_VAR:%.*]] = [[RED_INIT:%.*]];
; CHECK: DO i1 = 0, 4 * [[UB]] + -1, 4   <DO_LOOP>
...
; CHECK: [[RED_VAR]] = ([[RED_VAR]] > [[VEC_LD]]) ? [[RED_VAR]] : [[VEC_LD]];
; CHECK: END LOOP
; CHECK: [[RED_INIT]] = @llvm.experimental.vector.reduce.fmax.v4f32([[RED_VAR]]);
