...
; RUN: opt -S -passes="vplan-vec" -vplan-force-vf=16  -debug-only=VPlanLegality  < %s 2>&1 | FileCheck %s

; RUN: opt -disable-output -passes="hir-ssa-deconstruction,hir-vplan-vec,print<hir>" -print-after=hir-vplan-vec -vplan-force-vf=16 -debug-only=VPlanLegality < %s 2>&1 | FileCheck %s --check-prefix=HIR-CHECK
