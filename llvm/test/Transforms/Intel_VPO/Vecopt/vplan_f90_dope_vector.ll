...
; RUN: opt %s -S -vplan-vec -vplan-force-vf=16 -debug-only=vpo-ir-loop-vectorize-legality 2>&1 | FileCheck %s
; RUN: opt %s -hir-ssa-deconstruction -hir-framework -hir-temp-cleanup -disable-output -hir-vplan-vec -print-after=hir-vplan-vec -vplan-force-vf=16 -debug-only=vplan-vec 2>&1 | FileCheck %s --check-prefix=HIR-CHECK
