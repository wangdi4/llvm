...
; CHECK-NEXT: Uniform: [Shape: Uniform] i64 [[VAL3:%vp[0-9]+]] = phi [ i64 0, BB3 ], [ i64 [[VAL4:%vp[0-9]+]], BB4 ]
; CHECK-NEXT: Divergent: [Shape: Strided, Stride: ?] i64 [[VAL5:%vp[0-9]+]] = mul i64 [[VAL6:%n]] i64 [[VAL1:%vp[0-9]+]]
; CHECK-NEXT: Divergent: [Shape: Strided, Stride: ?] i64 [[VAL7:%vp[0-9]+]] = add i64 [[VAL5:%vp[0-9]+]] i64 [[VAL3:%vp[0-9]+]]
; CHECK-NEXT: Divergent: [Shape: Random] i32 [[VAL8:%vp[0-9]+]] = trunc i64 [[VAL7:%vp[0-9]+]]
; CHECK-NEXT: Divergent: [Shape: Random] float [[VAL9:%vp[0-9]+]] = sitofp i32 [[VAL8:%vp[0-9]+]]
; CHECK-NEXT: Divergent: [Shape: Strided, Stride: ?] float* [[VAL10:%vp[0-9]+]] = getelementptr inbounds float* [[VAL11:%ptr]] i64 [[VAL7:%vp[0-9]+]]
; CHECK-NEXT: Divergent: [Shape: Random] store float [[VAL9:%vp[0-9]+]] float* [[VAL10:%vp[0-9]+]]
; CHECK-NEXT: Uniform: [Shape: Uniform] i64 [[VAL11:%vp[0-9]+]] = add i64 [[VAL3:%vp[0-9]+]] i64 1
; CHECK-NEXT: Divergent: [Shape: Random] i1 [[VAL12:%vp[0-9]+]] = icmp i64 [[VAL4:%vp[0-9]+]] i64 [[VAL1:%vp[0-9]+]]
; CHECK-LABEL: Basic Block: BB5
; CHECK-NEXT: Divergent: [Shape: Unit Stride, Stride: i64 1] i64 [[VAL13:%vp[0-9]+]] = add i64 [[VAL1:%vp[0-9]+]] i64 1
; CHECK-NEXT: Uniform: [Shape: Uniform] i1 [[VAL14:%vp[0-9]+]] = icmp i64 [[VAL13:%vp[0-9]+]] i64 [[VAL6:%n]]
