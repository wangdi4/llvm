...
; RUN: opt < %s -S -passes=vplan-pragma-omp-ordered-simd-extract,print -disable-output --vpo-utils-add-typed-privates 2>&1 | FileCheck %s --check-prefixes=CHECK,TYPED
; RUN: opt < %s -S -passes=vplan-pragma-omp-ordered-simd-extract,print -disable-output 2>&1 | FileCheck %s --check-prefixes=CHECK,UNTYPED
