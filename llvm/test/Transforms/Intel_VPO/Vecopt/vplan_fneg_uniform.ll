...
; RUN: opt -S -passes=vplan-vec -vplan-force-vf=4 < %s 2>&1 | FileCheck %s -check-prefixes=CHECK-IR
; RUN: opt -S -passes=hir-ssa-deconstruction,hir-temp-cleanup,hir-vec-dir-insert,hir-vplan-vec,hir-cg -vplan-force-vf=4 < %s | FileCheck %s -check-prefix=CHECK-HIR
