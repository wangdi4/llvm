; RUN: opt -S -VPlanDriver -vplan-print-after-simplify-cfg -disable-vplan-codegen -disable-output < %s 2>&1 | FileCheck %s
; RUN: opt -S -passes="vplan-driver" -vplan-print-after-simplify-cfg -disable-vplan-codegen -disable-output < %s 2>&1 | FileCheck %s
...
; CHECK: Condition(external): i1 icmp eq (i8 addrspace(1)* bitcast (i32 addrspace(1)* @gint to i8 addrspace(1)*), i8 addrspace(1)* null)
; CHECK: Condition(external): i1 icmp eq (i8 addrspace(3)* bitcast (i32 addrspace(3)* @testKernel.lint to i8 addrspace(3)*), i8 addrspace(3)* null)
