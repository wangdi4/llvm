...
; RUN: opt -hir-ssa-deconstruction -hir-framework -hir-vplan-vec -print-after=hir-vplan-vec -disable-output < %s 2>&1 | FileCheck %s --check-prefix=HIRCHECK
; RUN: opt -vplan-vec -print-after=vplan-vec -disable-output < %s 2>&1 | FileCheck %s --check-prefix=IRCHECK
...
; IRCHECK-LABEL:  *** IR Dump After VPlan Vectorizer (vplan-vec) ***
;
