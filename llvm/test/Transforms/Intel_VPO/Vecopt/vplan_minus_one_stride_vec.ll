...
; RUN: opt -opaque-pointers=0 -passes='hir-ssa-deconstruction,print<hir>,hir-vplan-vec,print<hir>' -disable-output < %s 2>&1 | FileCheck %s --check-prefix=HIRCHECK
; RUN: opt -opaque-pointers=0 -passes='vplan-vec' -print-after=vplan-vec -disable-output < %s 2>&1 | FileCheck %s --check-prefix=IRCHECK
