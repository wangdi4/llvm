...
; RUN: opt -S %s -hir-ssa-deconstruction -hir-temp-cleanup -hir-vec-dir-insert -hir-vplan-vec -vplan-force-vf=4 -enable-vp-value-codegen-hir -print-before=hir-vplan-vec -print-after=hir-vplan-vec -disable-output 2>&1 | FileCheck %s
; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-vec-dir-insert,hir-vplan-vec" -S %s -vplan-force-vf=4 -enable-vp-value-codegen-hir -print-before=hir-vplan-vec -print-after=hir-vplan-vec -disable-output 2>&1 | FileCheck %s
