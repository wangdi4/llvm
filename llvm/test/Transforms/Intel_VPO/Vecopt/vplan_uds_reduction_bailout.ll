; RUN: opt -disable-output -passes="vplan-vec" -debug-only=vpo-ir-loop-vectorize-legality -debug-only=vplan-vec -vplan-force-uds-reduction-vectorization=false < %s 2>&1 | FileCheck %s
; REQUIRES: asserts
