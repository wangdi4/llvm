; Test merge loop exits transformation where inner loop has one jump outside of the loop.
...
; CHECK-NEXT: i32 [[PHI_OUT2:%vp[0-9]+]] = phi  [ i32 [[PHI_OUT1]], {{BB[0-9]+}} ],  [ i32 1, {{IntermediateBB[0-9]+}} ]
; CHECK-NEXT: i1 [[PHI_OUT3:%vp[0-9]+]] = phi  [ i1 [[CMP_OUT2]], {{BB[0-9]+}} ],  [ i1 false, {{IntermediateBB[0-9]+}} ]
...
; CHECK-NEXT: PREDECESSORS(2): {{BB[0-9]+}} {{IntermediateBB[0-9]+}}
; CHECK-EMPTY:
...
loop2_latch:
  ...
  br i1 %cmp2, label %loop2, label %bb3
