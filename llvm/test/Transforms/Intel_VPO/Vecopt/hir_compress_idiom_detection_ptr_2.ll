...
; CHECK-NEXT:    Indices:
; CHECK-NEXT:      ptr [[VP9]] = phi  [ ptr [[OUTP_0150]], [[BB1]] ],  [ ptr [[VP10]], [[BB2]] ]
; CHECK-EMPTY:
...
; CHECK-NEXT:     ptr [[VP17:%.*]] = compress-expand-index {i64, stride: 2} ptr [[VP9]]
; CHECK-NEXT:     ptr [[VP18:%.*]] = compress-expand-index {i32, stride: 2} ptr [[VP6]]
...
; CHECK-NEXT:       i64 [[VP19:%.*]] = compress-expand-mask
; CHECK-NEXT:       i32 [[VP8]] = trunc i64 [[VP5]] to i32
; CHECK-NEXT:       ptr [[VP_SUBSCRIPT_1]] = subscript inbounds ptr [[VP18]]
; CHECK-NEXT:       compress-store-nonu i32 [[VP8]] ptr [[VP_SUBSCRIPT_1]] i64 [[VP19]]
; CHECK-NEXT:       ptr [[VP_SUBSCRIPT_3]] = subscript inbounds ptr [[VP17]]
; CHECK-NEXT:       compress-store-nonu i64 [[VP_LOAD]] ptr [[VP_SUBSCRIPT_3]] i64 [[VP19]]
; CHECK-NEXT:       br [[BB2]]
; CHECK-EMPTY:
...
; CHECK-NEXT:        |   [[CAST0:%.*]] = bitcast.<4 x i1>.i4([[DOTVEC50]])
; CHECK-NEXT:        |   [[POPCNT0:%.*]] = @llvm.ctpop.i4([[CAST0]])
; CHECK-NEXT:        |   [[SHL0:%.*]] = -1  <<  [[POPCNT0]]
; CHECK-NEXT:        |   [[XOR0:%.*]] = [[SHL0]]  ^  -1
; CHECK-NEXT:        |   [[CAST60:%.*]] = bitcast.i4.<4 x i1>([[XOR0]])
; CHECK-NEXT:        |   [[COMPRESS0:%.*]] = @llvm.x86.avx512.mask.compress.v4i32(i1 + <i64 0, i64 1, i64 2, i64 3>,  poison,  [[DOTVEC50]])
; CHECK-NEXT:        |   @llvm.masked.scatter.v4i32.v4p0([[COMPRESS0]],  &((<4 x ptr>)([[PHI_TEMP20]])[<i64 0, i64 2, i64 4, i64 6>]),  0,  [[CAST60]])
; CHECK-NEXT:        |   [[COMPRESS70:%.*]] = @llvm.x86.avx512.mask.compress.v4i64([[DOTVEC40]],  poison,  [[DOTVEC50]])
; CHECK-NEXT:        |   @llvm.masked.scatter.v4i64.v4p0([[COMPRESS70]],  &((<4 x ptr>)([[PHI_TEMP0]])[<i64 0, i64 2, i64 4, i64 6>]),  0,  [[CAST60]])
; CHECK-NEXT:        |   [[SELECT0:%.*]] = ([[DOTVEC50]] == <i1 true, i1 true, i1 true, i1 true>) ? -1 : 0
; CHECK-NEXT:        |   [[SELECT80:%.*]] = ([[DOTVEC50]] == <i1 true, i1 true, i1 true, i1 true>) ? -1 : 0
; CHECK-NEXT:        |   [[CAST90:%.*]] = bitcast.<4 x i1>.i4([[SELECT0]])
; CHECK-NEXT:        |   [[POPCNT100:%.*]] = @llvm.ctpop.i4([[CAST90]])
; CHECK-NEXT:        |   [[ZEXT0:%.*]] = zext.i4.i64([[POPCNT100]])
; CHECK-NEXT:        |   [[MUL0:%.*]] = [[ZEXT0]]  *  2
; CHECK-NEXT:        |   [[COPY0:%.*]] = &(([[PHI_TEMP0]])[%mul])
; CHECK-NEXT:        |   [[CAST110:%.*]] = bitcast.<4 x i1>.i4([[SELECT80]])
; CHECK-NEXT:        |   [[POPCNT120:%.*]] = @llvm.ctpop.i4([[CAST110]])
; CHECK-NEXT:        |   [[ZEXT130:%.*]] = zext.i4.i64([[POPCNT120]])
; CHECK-NEXT:        |   [[MUL140:%.*]] = [[ZEXT130]]  *  2
; CHECK-NEXT:        |   [[COPY150:%.*]] = &(([[PHI_TEMP20]])[%mul14])
; CHECK-NEXT:        |   [[PHI_TEMP0]] = [[COPY0]]
; CHECK-NEXT:        |   [[PHI_TEMP20]] = [[COPY150]]
; CHECK-NEXT:        + END LOOP
; CHECK:             [[ROWP_ADDR_0140]] = [[COPY150]]
...
if.then:                                          ; preds = %for.body
  ...
  %incdec.ptr = getelementptr inbounds i32, ptr %rowp.addr.014, i64 2
  store i32 %conv, ptr %rowp.addr.014, !tbaa !3
  %incdec.ptr5 = getelementptr inbounds i64, ptr %outp.015, i64 2
...
!11 = !{!"llvm.loop.vectorize.ignore_profitability"}
