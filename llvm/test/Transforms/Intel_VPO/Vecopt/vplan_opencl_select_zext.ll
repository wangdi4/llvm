; RUN: opt %s -vector-library=SVML -VPlanDriver -disable-vplan-predicator -vplan-force-vf=4 -S | FileCheck %s
...
; CHECK: @test_select_zext
; CHECK: sext
; CHECK: _Z6selectDv4_fS_Dv4_i
; CHECK: ret void

define void @test_select_zext(<8 x float> %accumulated_occupancy, float addrspace(2)* %kernel_parameters) #5 {
simd.begin.region:
