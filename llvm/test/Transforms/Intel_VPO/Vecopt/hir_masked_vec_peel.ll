...
; RUN: opt -disable-output -enable-new-pm=0 -hir-ssa-deconstruction -hir-temp-cleanup -hir-runtime-dd -hir-vec-dir-insert -hir-vplan-vec -print-after=hir-vplan-vec -vplan-enable-peeling -vplan-enable-vectorized-peel -vplan-enable-peel-rem-strip=0  -vplan-vec-scenario="m8;v8;s1" %s 2>&1 | FileCheck %s
; RUN: opt -disable-output -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-runtime-dd,hir-vec-dir-insert,hir-vplan-vec" -print-after=hir-vplan-vec -vplan-enable-peeling -vplan-enable-vectorized-peel -vplan-enable-peel-rem-strip=0  -vplan-vec-scenario="m8;v8;s1" %s 2>&1 | FileCheck %s
;
