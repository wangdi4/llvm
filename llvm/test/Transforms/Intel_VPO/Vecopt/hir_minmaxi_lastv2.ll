...
; RUN: opt -enable-new-pm=0 -hir-ssa-deconstruction -hir-temp-cleanup -hir-vec-dir-insert -hir-vplan-vec -vplan-force-vf=4 -print-after=hir-vplan-vec -enable-mmindex=1 -disable-output < %s 2>&1 | FileCheck %s
; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-vec-dir-insert,hir-vplan-vec,print<hir>" -vplan-force-vf=4 -enable-mmindex=1 -disable-output < %s 2>&1 | FileCheck %s
...
; CHECK-NEXT:              {
; CHECK-NEXT:                 %red.init = %max.018;
; CHECK-NEXT:                 %red.init1 = %indmax.017;
...
; CHECK-NEXT:                 |   %.vec4 = (%.vec > %phi.temp2) ? i1 + <i64 0, i64 1, i64 2, i64 3> + 1 : %phi.temp;
; CHECK-NEXT:                 |   %.vec5 = (%.vec > %phi.temp2) ? %.vec : %phi.temp2;
; CHECK-NEXT:                 |   %phi.temp = %.vec4;
; CHECK-NEXT:                 |   %phi.temp2 = %.vec5;
; CHECK-NEXT:                 + END LOOP
; CHECK:                      %max.018 = @llvm.vector.reduce.smax.v4i64(%.vec5);
