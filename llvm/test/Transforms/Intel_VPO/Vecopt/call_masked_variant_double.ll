...
; RUN: opt -mtriple=x86_64 -passes=vplan-vec -vplan-force-vf=8 -S -vplan-cm-prohibit-zmm-low-pumping=0 < %s | FileCheck %s -check-prefix=VPLANCGVF8
...
; VPLANCGVF16-NEXT:    [[TMP4:%.*]] = call x86_regcallcc { <8 x double>, <8 x double> } @_ZGVZM16v_vfunc(<8 x double> noundef nofpclass(nan inf) [[WIDE_LOAD_PART_0_OF_2_0]], <8 x double> noundef nofpclass(nan inf) [[WIDE_LOAD_PART_1_OF_2_0]], i32 [[ZEXT0]], i32 [[ZEXT1]]) #0
;
...
; VPLANCGVF8-NEXT:    [[TMP4:%.*]] = call fast x86_regcallcc nofpclass(nan inf) <8 x double> @_ZGVZM8v_vfunc(<8 x double> noundef nofpclass(nan inf) [[WIDE_LOAD0]], i32 [[ZEXT0]]) #0
;
