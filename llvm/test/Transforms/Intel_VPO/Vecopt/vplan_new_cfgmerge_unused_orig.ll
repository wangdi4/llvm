...
;RUN: opt -VPlanDriver -vplan-force-vf=2 -S -print-after=VPlanDriver -vplan-vec-scenario="n0;v2;m2" -vplan-enable-masked-variant -vplan-enable-new-cfg-merge %s | FileCheck %s
;RUN: opt -passes="vplan-driver" -vplan-force-vf=2 -S -print-after=VPlanDriver --vplan-vec-scenario="n0;v2;m2" --vplan-enable-masked-variant -vplan-enable-new-cfg-merge %s | FileCheck %s
...
; CHECK-NEXT:    [[TMP3:%.*]] = icmp eq i32 [[TMP2]], 4096
...
; CHECK-NEXT:    [[BROADCAST_SPLATINSERT19:%.*]] = insertelement <2 x i1> poison, i1 [[TMP12]], i32 0
; CHECK-NEXT:    [[BROADCAST_SPLAT20:%.*]] = shufflevector <2 x i1> [[BROADCAST_SPLATINSERT19]], <2 x i1> poison, <2 x i32> zeroinitializer
; CHECK-NEXT:    [[BROADCAST_SPLAT20_EXTRACT_0_:%.*]] = extractelement <2 x i1> [[BROADCAST_SPLAT20]], i32 0
...
; CHECK-NEXT:    [[ADD2_LCSSA:%.*]] = phi float [ [[UNI_PHI23]], [[FINAL_MERGE]] ]
