...
; RUN: opt %s -S -hir-ssa-deconstruction -hir-temp-cleanup -hir-pre-vec-complete-unroll -hir-memory-reduction-sinking -hir-vec-dir-insert -hir-vplan-vec -print-after=hir-vplan-vec -disable-output 2>&1 -vplan-enable-new-cfg-merge-hir=0 | FileCheck %s --check-prefix=VPCHECK
; RUN: opt %s -S -hir-ssa-deconstruction -hir-temp-cleanup -hir-pre-vec-complete-unroll -hir-memory-reduction-sinking -hir-vec-dir-insert -hir-vplan-vec -print-after=hir-vplan-vec -disable-output 2>&1 -vplan-enable-new-cfg-merge-hir=1 | FileCheck %s --check-prefix=VPCHECK
