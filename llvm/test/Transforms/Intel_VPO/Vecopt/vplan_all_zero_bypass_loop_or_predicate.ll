...
; REQUIRES: asserts
; RUN: opt < %s -VPlanDriver -vplan-enable-all-zero-bypass-loops -vplan-print-after-all-zero-bypass \
; RUN: -vplan-force-vf=4 -disable-output -vplan-enable-new-cfg-merge -vplan-new-cfg-merge-list-only -S 2>&1 | FileCheck %s
; RUN: opt < %s -passes="vplan-driver" -vplan-enable-all-zero-bypass-loops -vplan-print-after-all-zero-bypass\
; RUN:  -vplan-force-vf=4 -disable-output -vplan-enable-new-cfg-merge -vplan-new-cfg-merge-list-only -S 2>&1 | FileCheck %s
...
; CHECK-NEXT:     [DA: Uni] i1 [[VP_EXITCOND29:%.*]] = icmp eq i64 [[VP_INDVARS_IV_NEXT]] i64 [[VP_VECTOR_TRIP_COUNT]]
