...
; REQUIRES: asserts
; RUN: opt < %s -VPlanDriver -vplan-enable-all-zero-bypass-loops -vplan-print-after-all-zero-bypass -vplan-force-vf=4 -disable-output -S 2>&1 | FileCheck %s
; RUN: opt < %s -passes="vplan-driver" -vplan-enable-all-zero-bypass-loops -vplan-print-after-all-zero-bypass -vplan-force-vf=4 -disable-output -S 2>&1 | FileCheck %s
...
; CHECK-NEXT:     [DA: Uni] i64 [[VP_VECTOR_LOOP_IV:%.*]] = phi  [ i64 0, [[BB1]] ],  [ i64 [[VP_VECTOR_LOOP_IV_NEXT:%.*]], [[BB3]] ]
...
; CHECK-NEXT:     [DA: Uni] br i1 [[VP_VECTOR_LOOP_EXITCOND]], [[BB21:BB[0-9]+]], [[BB2]]
; CHECK-EMPTY:
