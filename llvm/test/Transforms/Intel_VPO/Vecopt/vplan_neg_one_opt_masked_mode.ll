...
; RUN: opt -opaque-pointers=0 -disable-output %s -passes=vplan-vec -vplan-enable-masked-vectorized-remainder -vplan-vec-scenario="n0;v16;m16" -print-after=vplan-vec 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers=0 -disable-output %s -passes='hir-ssa-deconstruction,hir-temp-cleanup,hir-vec-dir-insert,hir-vplan-vec,print<hir>' -vplan-enable-masked-vectorized-remainder -vplan-vec-scenario="n0;v16;m16" 2>&1 | FileCheck %s -check-prefixes=HIR
