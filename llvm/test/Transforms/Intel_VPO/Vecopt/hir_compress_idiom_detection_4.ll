...
; CHECK:      [Compress/Expand Idiom] Increment {sb:3}+4 detected: <19>         %j.014 = 4  +  %j.014;
; CHECK-NOT:  [Compress/Expand Idiom] Increment rejected
; CHECK:      Idiom List
; CHECK-NEXT: CEIndexIncFirst: <16>         %j.014 = %j.014 + 2  +  3;
; CHECK-NEXT:   CEIndexIncNext: <19>         %j.014 = 4  +  %j.014;
; CHECK-DAG:    CEStore: <14>         (%B)[%j.014] = %1;
; CHECK-DAG:      CELdStIndex: %j.014
; CHECK-DAG:    CEStore: <18>         (%B)[%j.014] = %1;
; CHECK-DAG:      CELdStIndex: %j.014
