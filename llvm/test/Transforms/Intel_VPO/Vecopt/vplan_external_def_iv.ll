; RUN: opt -hir-ssa-deconstruction -hir-vec-dir-insert -VPlanDriverHIR -vplan-force-vf=4 -vplan-print-after-simplify-cfg -disable-vplan-codegen -disable-output < %s 2>&1 | FileCheck %s
...
;
;  BB4 (BP: NULL) :
;   i64 %vp27808 = semi-phi i64 0 i64 %vp31344
;   i32 %vp28032 = load i32 %vp27968
;   i32 %vp30928 = mul i32 %vp28032 i32 %i1
;   store i32 %vp30928 i32 %vp29232
;   i64 %vp31344 = add i64 %vp27808 i64 1
;   i1 %vp31600 = icmp i64 %vp31344 i64 1599
;  SUCCESSORS(1):BB7

; CHECK: i32 [[Load:%vp[0-9]*]] = load i32 %vp{{[0-9]*}}
; CHECK: i32 %vp{{[0-9]*}} = mul i32 [[Load]] i32 %i1
