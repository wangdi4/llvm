; RUN: opt -hir-ssa-deconstruction -hir-vplan-vec -vplan-print-after-vpentity-instrs -disable-output -S < %s  | FileCheck %s
...
entry:
  ...
  %call.i = tail call i64 @strtol(i8* nocapture noundef nonnull %1, i8** noundef null, i32 noundef 10) #3
...
DIR.OMP.SIMD.157:                                 ; preds = %entry
  %2 = call token @llvm.directive.region.entry() [ "DIR.OMP.SIMD"(), "QUAL.OMP.SIMDLEN"(i32 8), "QUAL.OMP.LINEAR:IV"(i64* %i9.linear.iv, i32 %conv.i) ]
...
omp.inner.for.body:                               ; preds = %DIR.OMP.SIMD.256, %omp.inner.for.body
  ...
  %arrayidx13 = getelementptr inbounds [128 x i64], [128 x i64]* %b, i64 0, i64 %mul
  store i64 %add.i, i64* %arrayidx13, align 8
...
declare dso_local i64 @strtol(i8* noundef readonly, i8** nocapture noundef, i32 noundef) local_unnamed_addr
