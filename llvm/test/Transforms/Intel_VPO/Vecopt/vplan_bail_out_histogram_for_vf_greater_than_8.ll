...
; REQUIRES: asserts
; RUN: opt -S -mattr=+avx512vl,+avx512cd -hir-ssa-deconstruction -hir-temp-cleanup -hir-vec-dir-insert -enable-vconflict-idiom -hir-vplan-vec -vplan-force-vf=16 -debug-only=LoopVectorizationPlanner -disable-output < %s 2>&1 | FileCheck %s

; Function Attrs: nofree norecurse nosync nounwind uwtable mustprogress
define dso_local void @foo(i32* noalias nocapture %A, i32* noalias nocapture readonly %B, i32 %TC) local_unnamed_addr #0 {
; CHECK: VConflict idiom cannot be optimized for VF > MaxVecRegSize / VConflictIndexSizeInBits.
;
entry:
