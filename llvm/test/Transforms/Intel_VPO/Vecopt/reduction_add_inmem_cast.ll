...
; HIR-NEXT:     float [[VP_LOAD_1:%.*]] = load ptr [[VP_SUBSCRIPT]]
...
; HIR-NEXT:     ptr [[VP_SUBSCRIPT_3:%.*]] = subscript inbounds ptr [[VP_SUM]]
; HIR-NEXT:     store float [[VP7]] ptr [[VP_SUBSCRIPT_3]]
...
; CHECK-NEXT:     float [[VP_A_I:%.*]] = load ptr [[VP_ARRAYIDX]]
; CHECK-NEXT:     float [[VP_SUM_LD:%.*]] = load ptr [[VP_SUM]]
; CHECK-NEXT:     float [[VP_ADD:%.*]] = fadd float [[VP_A_I]] float [[VP_SUM_LD]]
; CHECK-NEXT:     store float [[VP_ADD]] ptr [[VP_SUM]]
...
; CHECK:       VPlannedBB2:
; CHECK-NEXT:    call void @llvm.lifetime.start.p0(i64 8, ptr [[SUM_VEC:%.*]]) 
...
; CHECK-NEXT:    [[WIDE_LOAD0:%.*]] = load <2 x float>, ptr [[SCALAR_GEP0]], align 4
; CHECK-NEXT:    [[WIDE_LOAD40:%.*]] = load <2 x float>, ptr [[SUM_VEC_BASE_ADDR_EXTRACT_0_0:%.*]], align 4
; CHECK-NEXT:    [[TMP8:%.*]] = fadd fast <2 x float> [[WIDE_LOAD0]], [[WIDE_LOAD40]]
; CHECK-NEXT:    store <2 x float> [[TMP8]], ptr [[SUM_VEC_BASE_ADDR_EXTRACT_0_0]], align 4
...
; CHECK-NEXT:    store i32 [[TMP13]], ptr [[SUM0]], align 1
; CHECK-NEXT:    call void @llvm.lifetime.end.p0(i64 8, ptr [[SUM_VEC:%.*]])
...
for.body:
  ...
  %sum.ld = load float, ptr %sum, align 4
