; Test to check that #pragma vector dynamic_align effectively enforces dynamic peeling
...
; RUN: opt %s -disable-output -passes="vplan-vec" -vplan-enable-peeling=0 -mattr=+avx2 -debug-only=LoopVectorizationPlanner 2>&1 | FileCheck %s
; RUN: opt %s -disable-output -vplan-vec -vplan-enable-peeling=0 -mattr=+avx2 -debug-only=LoopVectorizationPlanner 2>&1 | FileCheck %s
