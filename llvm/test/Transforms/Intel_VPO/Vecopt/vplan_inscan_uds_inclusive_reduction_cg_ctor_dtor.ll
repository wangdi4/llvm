...
; CHECK:         [[RED_VEC_BASE_ADDR:%.*]] = getelementptr %struct.Dummy, ptr [[RED_VEC]], <2 x i32> <i32 0, i32 1>
; CHECK:         [[RED_VEC_BASE_ADDR_EXTRACT_1:%.*]] = extractelement <2 x ptr> [[RED_VEC_BASE_ADDR]], i32 1
; CHECK:         [[RED_VEC_BASE_ADDR_EXTRACT_0:%.*]] = extractelement <2 x ptr> [[RED_VEC_BASE_ADDR]], i32 0
...
; CHECK:         call void @.omp_combiner.(ptr [[RED_ORIG]], ptr [[RED_VEC_BASE_ADDR_EXTRACT_0]])
; CHECK-NEXT:    call void @.omp_initializer.(ptr [[RED_VEC_BASE_ADDR_EXTRACT_0]], ptr [[RED_ORIG]])
...
DIR.VPO.END.GUARD.MEM.MOTION.5:                   ; preds = %DIR.VPO.GUARD.MEM.MOTION.2
  call void @llvm.directive.region.exit(token %phase1.guard.start) [ "DIR.VPO.END.GUARD.MEM.MOTION"() ]
  br label %DIR.OMP.SCAN.7

DIR.OMP.SCAN.7:                                   ; preds = %DIR.VPO.END.GUARD.MEM.MOTION.5
  %5 = call token @llvm.directive.region.entry() [ "DIR.OMP.SCAN"(), "QUAL.OMP.INCLUSIVE"(ptr %red.red, i64 1) ]
...
DIR.OMP.END.SCAN.4:                               ; preds = %DIR.OMP.END.SCAN.9
