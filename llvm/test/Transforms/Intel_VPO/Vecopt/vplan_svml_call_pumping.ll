...
; Check that we bailout in HIR vectorizer.
; HIR-LABEL:    BEGIN REGION { }
; HIR:               %0 = @llvm.directive.region.entry(); [ DIR.OMP.SIMD(),  QUAL.OMP.SIMDLEN(128),  QUAL.OMP.NORMALIZED.IV(null),  QUAL.OMP.NORMALIZED.UB(null) ]

; HIR:               + DO i1 = 0, %N + -1, 1   <DO_LOOP> <MAX_TC_EST = 2147483647> <LEGAL_MAX_TC = 2147483647> <simd> <vectorize>
; HIR-NEXT:          |   %conv = sitofp.i32.float(i1);
; HIR-NEXT:          |   %call = @sinf(%conv);
; HIR-NEXT:          |   (%A)[i1] = %call;
; HIR-NEXT:          |   if (-1 * i1 == 0)
; HIR-NEXT:          |   {
; HIR-NEXT:          |      %maskcall = @sinf(%conv);
; HIR-NEXT:          |      (%B)[i1] = %maskcall;
; HIR-NEXT:          |   }
; HIR-NEXT:          + END LOOP

; HIR:               @llvm.directive.region.exit(%0); [ DIR.OMP.END.SIMD() ]
; HIR-NEXT:    END REGION
