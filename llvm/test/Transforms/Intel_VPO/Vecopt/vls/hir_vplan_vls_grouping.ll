...
; REQUIRES: asserts
; RUN: opt < %s -enable-new-pm=0 -O2 -hir-ssa-deconstruction -hir-vec-dir-insert -hir-vplan-vec -debug-only=ovls -disable-hir-complete-unroll -vplan-vls-level=always -vplan-force-vf=8 2>&1 | FileCheck %s
; RUN: opt < %s -passes='default<O2>,hir-ssa-deconstruction,hir-vec-dir-insert,hir-vplan-vec' -debug-only=ovls -disable-hir-complete-unroll -vplan-vls-level=always -vplan-force-vf=8 2>&1 | FileCheck %s
