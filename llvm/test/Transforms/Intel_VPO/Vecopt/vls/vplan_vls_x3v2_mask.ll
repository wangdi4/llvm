...
; RUN: opt -S -passes=vplan-vec -vplan-enable-all-zero-bypass-non-loops=false -debug-only=ovls -vplan-enable-masked-vectorized-remainder=0 -vplan-enable-non-masked-vectorized-remainder=0 < %s 2>&1 | FileCheck %s
; REQUIRES: asserts
