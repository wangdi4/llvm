; RUN: opt -tbaa -hir-ssa-deconstruction  -hir-vec-dir-insert -hir-vplan-vec -disable-output -print-before=hir-vplan-vec -print-after=hir-vplan-vec -vplan-force-vf=4  -hir-details < %s 2>&1 -vplan-enable-new-cfg-merge-hir=0 | FileCheck %s
; RUN: opt -tbaa -hir-ssa-deconstruction  -hir-vec-dir-insert -hir-vplan-vec -disable-output -print-before=hir-vplan-vec -print-after=hir-vplan-vec -vplan-force-vf=4  -hir-details < %s 2>&1 -vplan-enable-new-cfg-merge-hir=1 | FileCheck %s
