...
; CHECK-LABEL:   {{(HIR )?}}Cost Model for VPlan foo.{{.*}} with VF = 4:
; CHECK:         Cost Unknown for float {{%vp.*}} = call float [[ARG:%vp.*]] __svml_cospif4 [x 1]
; CHECK-NEXT:    Cost Unknown for float {{%vp.*}} = call float [[ARG]] __svml_sinpif4 [x 1]
