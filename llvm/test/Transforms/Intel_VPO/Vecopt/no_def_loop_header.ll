; RUN: opt -S -VPlanDriver -vpo-vplan-build-stress-test -vplan-dump-da < %s --disable-output 2>&1 | FileCheck %s
...
; CHECK: Divergent: [Shape: Random] i32 [[IV:%vp.*]] = phi  [ i32 [[IV_NEXT:%vp.*]], BB5 ],  [ i32 0, BB2 ]
  br label %exiting1

exiting1:
  %gep = getelementptr inbounds i32, i32* %a, i32 %iv
; CHECK: Divergent: [Shape: Random] i32* [[GEP:%vp.*]] = getelementptr inbounds i32* %a i32 [[IV]]
  %ld = load i32, i32* %gep, align 8
; CHECK: Divergent: [Shape: Random] i32 [[LD:%vp.*]] = load i32* [[GEP]]
  %cmp1 = icmp ne i32 %ld, 3
; CHECK: Divergent: [Shape: Random] i1 [[CMP1:%vp.*]] = icmp i32 [[LD]] i32 3
  br i1 %cmp1, label %exiting2, label %loop.exit

exiting2:
  %cmp2 = icmp slt i32 %iv, 42
; CHECK: Divergent: [Shape: Random] i1 [[CMP2:%vp.*]] = icmp i32 [[IV]] i32 42
