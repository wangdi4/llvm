...
;
; RUN: opt -passes='hir-ssa-deconstruction,hir-vec-dir-insert,hir-vplan-vec,print<hir>' -hir-details-no-verbose-indent -hir-details -vplan-force-vf=4 -vplan-enable-peeling -vplan-calc-min-profitable-dyn-peel-tc=false -mattr=+avx2 -disable-output < %s 2>&1 | FileCheck %s
;
; Incoming HIR:
...
; CHECK:    |   <LVAL-REG> {al:32}(<4 x i64>*)(LINEAR ptr @arr)[i64 0][i64 2][LINEAR i64 i1 + %n]
; CHECK:    + END LOOP
;
...
for.body:                                         ; preds = %entry, %for.body
  ...
  %arrayidx = getelementptr inbounds [10 x [1024 x i64]], ptr @arr, i64 0, i64 2, i64 %add
