...
; RUN: opt -opaque-pointers=0 -disable-output -disable-vplan-codegen -passes=vplan-vec -vplan-force-vf=4  -vplan-print-after-cfg-merge -vplan-enable-masked-vectorized-remainder=0 -vplan-enable-non-masked-vectorized-remainder=0 < %s  | FileCheck %s
; RUN: opt -opaque-pointers=0 -disable-output -passes=vplan-vec -vplan-force-vf=4 -vplan-enable-pushvf  -vplan-print-after-cfg-merge -vplan-enable-masked-vectorized-remainder=0 -vplan-enable-non-masked-vectorized-remainder=0 < %s  | FileCheck %s -check-prefixes CHECK
