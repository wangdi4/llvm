...
; CHECK: BB2 -> BB14 [ label="Branch If Not All Zero Recipe"]
; CHECK: BB14 [label = "{BB14}"]
; CHECK: BB14 -> BB3 [ label="Branch If Not All Zero Recipe" lhead=cluster_loop19]
; CHECK: BB14 -> BB4 [ label="!Branch If Not All Zero Recipe"]
...
; CHECK: BB5 -> BB20 [ label="Non-Uniform Cond Bit Recipe"]
; CHECK: BB5 -> BB21 [ label="!Non-Uniform Cond Bit Recipe"]
; CHECK: BB20 [label = "{BB20 | Vectorize VPInstIR:\n  %arrayidx4 = getelementptr inbounds float, float* %3, i64 %indvars.iv\n  %4 = load float, float* %arrayidx4, align 4 | UniformCBR4: %cmp5 = fcmp oeq float %4, 0.000000e+00}"]
; CHECK: BB20 -> BB16 [ label="UniformCBR4"]
; CHECK: BB16 [label = "{BB16}"]
; CHECK: BB16 -> BB6 [ label="UniformCBR4"]
; CHECK: BB16 -> BB7 [ label="!UniformCBR4"]
; CHECK: BB6 [label = "{BB6 | Vectorize VPInstIR:\n  %5 = add nsw i64 %indvars.iv, %indvars.iv41\n  %6 = trunc i64 %5 to i32}"]
; CHECK: BB6 -> BB8 [ label=""]
; CHECK: BB8 [label = "{BB8 | Vectorize VPInstIR:\n  %.sink16 = phi i64 [ %indvars.iv41, %if.else ], [ %indvars.iv, %if.then ]\n  %.sink = phi float* [ %7, %if.else ], [ %3, %if.then ]\n  %conv10.sink.in = phi i32 [ %mul, %if.else ], [ %6, %if.then ]\n  %conv10.sink = sitofp i32 %conv10.sink.in to float\n  %sext = shl i64 %.sink16, 32\n  %idxprom13 = ashr exact i64 %sext, 32\n  %arrayidx14 = getelementptr inbounds float, float* %.sink, i64 %idxprom13\n  store float %conv10.sink, float* %arrayidx14, align 4}"]
; CHECK: BB8 -> BB21 [ label="Branch If Not All Zero Recipe"]
; CHECK: BB21 [label = "{BB21 | Vectorize VPInstIR:\n  %indvars.iv.next = add nsw i64 %indvars.iv, 1\n  %add15 = add nsw i32 %j.035, 1 | IfNotAllZero: \n  %cmp1 = icmp slt i64 %indvars.iv, %1}"]
; CHECK: BB21 -> BB17 [ label="Branch If Not All Zero Recipe"]
; CHECK: BB17 [label = "{BB17}"]
; CHECK: BB17 -> BB5 [ label="Branch If Not All Zero Recipe"]
; CHECK: BB17 -> BB9 [ label="!Branch If Not All Zero Recipe"]
; CHECK: BB9 [label = "{BB9}"]
; CHECK: BB7 [label = "{BB7 | Vectorize VPInstIR:\n  %mul = mul nsw i32 %j.035, %2\n  %arrayidx12 = getelementptr inbounds float*, float** %a, i64 %indvars.iv\n  %7 = load float*, float** %arrayidx12, align 8}"]
; CHECK: BB7 -> BB8 [ label=""]
