...
; This test case checks that the reduction with the intrinsic llvm.smax
...
; Check VPlan vectorization and reduction

; CHECK: Reduction list
; CHECK:  signed (SIntMax) Start: i32 %max.013 Exit: i32 [[VP_LOAD:%.*]]
; CHECK:   Linked values: i32 [[VP1:%.*]], i32 [[VP_LOAD:%.*]], i32 [[VP2:%.*]], i32 [[VP3:%.*]],

; CHECK: i32 [[VP2:%.*]] = reduction-init i32 %max.013
; CHECK: i32 [[VP_LOAD:%.*]] = call i32 [[VP4:%.*]] i32 [[VP1:%.*]] i32 (i32, i32)* @llvm.smax.i32
; CHECK: i32 [[VP3:%.*]] = reduction-final{u_smax} i32 [[VP_LOAD:%.*]]
...
; CHECK:  |   %llvm.smax.v8i32 = @llvm.smax.v8i32(%.vec7,  %phi.temp5);
; CHECK:  |   %phi.temp5 = %llvm.smax.v8i32;
; CHECK:  + END LOOP
; CHECK:  %max.013 = @llvm.vector.reduce.smax.v8i32(%llvm.smax.v8i32);
