...
; In this test we have main value used in another select (even not an index for this one).
; REQUIRES: asserts
...
;CHECK-NEXT: [MinMax+Index] Skipped: dependency on comparison operand
;
;<0>          BEGIN REGION { }
;<17>               + DO i1 = 0, sext.i32.i64(%m) + -1, 1   <DO_LOOP>
;<5>                |   %0 = (%ordering)[i1];
;<9>                |   %tmp.027 = (%0 >= %tmp.027) ? %0 : %best.026;
;<10>               |   %best.026 = (%0 > %best.026) ? %0 : %best.026;
;<17>               + END LOOP
;<0>          END REGION
;
...
for.body:                                         ; preds = %for.body, %for.body.preheader
  ...
  %2 = select i1 %cmp2, i32 %0, i32 %best.026
