...
; RUN: opt -S -VPlanDriver -vplan-enable-peeling=true  -debug-only=LoopVectorizationPlanner -disable-output < %s 2>&1 | FileCheck %s --check-prefix=VPLAN-CM-PEELING
; RUN: opt -S -VPlanDriver -vplan-enable-peeling=false -debug-only=LoopVectorizationPlanner -disable-output < %s 2>&1 | FileCheck %s --check-prefix=VPLAN-CM-NO-PEELING
; REQUIRES: asserts
