...
; RUN: opt -vplan-vec-scenario="m4;v4;s1" -vplan-vec -vplan-enable-new-cfg-merge -vplan-force-vf=4 -vplan-enable-masked-variant -S < %s | FileCheck %s --check-prefix=MASKED-PEEL
; RUN: opt -vplan-vec-scenario="n0;v4;m4" -vplan-vec -vplan-enable-new-cfg-merge -vplan-force-vf=4 -vplan-enable-masked-variant -S < %s | FileCheck %s --check-prefix=MASKED-REMAINDER
; RUN: opt -vplan-vec-scenario="m4;v4;m4" -vplan-vec -vplan-enable-new-cfg-merge -vplan-force-vf=4 -vplan-enable-masked-variant -S < %s | FileCheck %s --check-prefix=MASKED-PEEL-REMAINDER
