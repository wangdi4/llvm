...
; RUN: opt -hir-ssa-deconstruction -hir-temp-cleanup -hir-vec-dir-insert -VPlanDriverHIR -vplan-force-vf=4 -print-after=VPlanDriverHIR -hir-cg -sroa -S < %s 2>&1 | FileCheck %s
...
; CHECK-LABEL: void @foo(i32 %constant)
; CHECK:       loop.40:
; CHECK-NEXT:    [[LOOP_IV:%.*]] = phi i64 [ 0, %region.0 ], [ [[IV_UPDATE:%.*]], %loop.40 ]
; CHECK-NOT:     phi <4 x i32>
; CHECK-NOT:     phi <4 x i1>
