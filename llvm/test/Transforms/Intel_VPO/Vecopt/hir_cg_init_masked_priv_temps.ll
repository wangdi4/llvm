...
; RUN: opt -hir-ssa-deconstruction -hir-temp-cleanup -hir-vec-dir-insert -hir-vplan-vec -vplan-force-vf=4 -print-after=hir-vplan-vec -hir-cg -sroa -enable-vp-value-codegen-hir -S < %s 2>&1 | FileCheck %s --check-prefixes=VPCHECK,IRCHECK
; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-vec-dir-insert,hir-vplan-vec,print<hir>,hir-cg,sroa" -vplan-force-vf=4 -enable-vp-value-codegen-hir -S < %s 2>&1 | FileCheck %s --check-prefixes=VPCHECK,IRCHECK
