...
; RUN: opt -hir-ssa-deconstruction -hir-vec-dir-insert -hir-vplan-vec -disable-output -vplan-print-after-plain-cfg -print-after=hir-vplan-vec -vplan-force-vf=4 -enable-vp-value-codegen-hir < %s 2>&1 | FileCheck %s -check-prefixes=PM1
; RUN: opt -passes="hir-ssa-deconstruction,hir-vec-dir-insert,hir-vplan-vec" -disable-output -vplan-print-after-plain-cfg -print-after=hir-vplan-vec -vplan-force-vf=4 -enable-vp-value-codegen-hir < %s 2>&1 | FileCheck %s -check-prefixes=PM2
