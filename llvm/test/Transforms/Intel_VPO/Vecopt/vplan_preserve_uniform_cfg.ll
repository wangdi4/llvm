; RUN: opt %s -VPlanDriver -enable-new-vplan-predicator=false -enable-vp-value-codegen=0 -disable-vplan-subregions=0 -vplan-predicator-report -vplan-force-vf=4 -S -o /dev/null 2>&1 | FileCheck %s -check-prefix=REPORT
; RUN: opt %s -VPlanDriver -enable-new-vplan-predicator=false -enable-vp-value-codegen=0 -disable-vplan-subregions=0 -S -vplan-force-vf=4 2>&1 | FileCheck %s -check-prefix=CG
; REQUIRES: asserts
