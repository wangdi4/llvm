...
; In this test we have non linear value for index (i1/3) while non-linear
...
;CHECK-NEXT: [MinMax+Index] Skipped: nonlinear rhs disabled (denom)
;
;<0>          BEGIN REGION { }
;<16>               + DO i1 = 0, sext.i32.i64(%m) + -1, 1   <DO_LOOP>
;<4>                |   %0 = (%ordering)[i1];
;<8>                |   %tmp.027 = (%0 > %best.026) ? (i1)/u3 : %tmp.027;
...
for.body:                                         ; preds = %for.body, %for.body.preheader
  ...
  %div = udiv i32 %1, 3
  %2 = select i1 %cmp1, i32 %div, i32 %tmp.027
