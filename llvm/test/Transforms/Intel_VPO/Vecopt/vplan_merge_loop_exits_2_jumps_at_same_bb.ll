...
; Test merge loop exits transformation when the loop has two exiting blocks into
; the same exit. Bottom test has dedicated exit though, because latch is
; treated specially in the current implementation of the transformation and
; might be a point of future unification/refactoring.
...
; CHECK-NEXT:      SUCCESSORS(2):[[BB3:BB[0-9]+]](i1 [[VP_CMP2]]), [[INTERMEDIATE_BB0]](!i1 [[VP_CMP2]])
; CHECK-NEXT:      PREDECESSORS(1): [[BB1]]
; CHECK-EMPTY:
; CHECK-NEXT:      [[INTERMEDIATE_BB0]]:
; CHECK-NEXT:       i32 [[VP_USE:%.*]] = phi  [ i32 0, [[BB1]] ],  [ i32 1, [[BB2]] ]
...
; CHECK-NEXT:     i1 [[VP_TAKE_BACKEDGE_COND:%.*]] = phi  [ i1 [[VP_BOTTOM_TEST]], [[BB3]] ],  [ i1 false, [[INTERMEDIATE_BB0]] ]
; CHECK-NEXT:    SUCCESSORS(2):[[BB1]](i1 [[VP_TAKE_BACKEDGE_COND]]), [[CASCADED_IF_BLOCK0:cascaded.if.block[0-9]+]](!i1 [[VP_TAKE_BACKEDGE_COND]])
; CHECK-NEXT:    PREDECESSORS(2): [[BB3]] [[INTERMEDIATE_BB0]]
; CHECK-EMPTY:
...
bb1:
  ...
  br i1 %cmp2, label %latch, label %early.exit
