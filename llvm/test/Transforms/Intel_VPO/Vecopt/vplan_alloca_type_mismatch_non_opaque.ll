...
; RUN: opt -opaque-pointers=0 -passes="vplan-vec" -vplan-force-vf=2 -S < %s 2>&1 | FileCheck %s
...
define void @repro(i32* %tid, i32* %bid, %"struct.std::complex"* %a.map.ptr.tmp.fp110.0.val, %"struct.std::complex"* %b.map.ptr.tmp.fp112.0.val, %"struct.std::complex"* %c.map.ptr.tmp.fp114.0.val, i32* %.omp.ub) {
...
omp.inner.for.body.lr.ph:                         ; preds = %DIR.OMP.SIMD.10
  ...
  br i1 %omp.ztt.not, label %DIR.OMP.END.SIMD.12.loopexit, label %DIR.OMP.SIMD.1
