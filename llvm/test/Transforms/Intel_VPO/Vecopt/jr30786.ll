...
; RUN: opt -hir-ssa-deconstruction -hir-framework -hir-vplan-vec -disable-output -enable-vplan-outer-loop-hir -print-after=hir-vplan-vec  -vplan-force-vf=4 -hir-details < %s 2>&1 | FileCheck %s
; RUN: opt -passes="hir-ssa-deconstruction,hir-vplan-vec,print<hir>" -disable-output -enable-vplan-outer-loop-hir -vplan-force-vf=4 -hir-details < %s 2>&1 | FileCheck %s
;
