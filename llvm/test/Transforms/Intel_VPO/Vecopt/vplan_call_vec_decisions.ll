...
; RUN: opt < %s -VPlanDriver -disable-output -vplan-print-after-linearization -vector-library=SVML | FileCheck %s
...
; CHECK-NEXT:     [DA: Div] float [[VP_SERIAL_CALL:%.*]] = call float [[VP0]] float (float)* @bar
; CHECK-NEXT:     [DA: Div] float [[VP_VEC_VARIANT:%.*]] = call float [[VP0]] float (float)* @simdBar
; CHECK-NEXT:     [DA: Div] double [[VP_CONV:%.*]] = fpext float [[VP0]] to double
; CHECK-NEXT:     [DA: Div] double [[VP_LIB_CALL:%.*]] = call double [[VP_CONV]] double (double)* @sin
...
; CHECK-NEXT:     [DA: Div] double [[VP_MASK_LIB_CALL:%.*]] = call double [[VP_CONV]] double (double)* @log
; CHECK-NEXT:     [DA: Div] float [[VP_MASK_LIB_TRUNC:%.*]] = fptrunc double [[VP_MASK_LIB_CALL]] to float
; CHECK-NEXT:     [DA: Div] float [[VP_MASK_VEC_VARIANT:%.*]] = call float [[VP0]] float (float)* @simdBar
...
; CHECK-NEXT:     [DA: Div] float [[VP_PUMP_CALL:%.*]] = call float [[VP_CONV]] float (float)* @sinf
