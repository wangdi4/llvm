...
; RUN: opt -opaque-pointers=0 < %s -passes=vplan-vec -disable-output -vplan-print-after-call-vec-decisions -vector-library=SVML | FileCheck %s --check-prefix=CALLVECDEC
; RUN: opt -opaque-pointers=0 < %s -passes=vplan-vec -disable-output -vplan-print-scalvec-results -vector-library=SVML | FileCheck %s --check-prefix=SVAPREFIX
