...
; RUN: opt -vplan-print-terminator-inst=false < %s -VPlanDriver -disable-output -vplan-print-after-call-vec-decisions -vector-library=SVML | FileCheck %s --check-prefix=CALLVECDEC
; RUN: opt -vplan-print-terminator-inst=false < %s -VPlanDriver -disable-output -vplan-print-scalvec-results -vector-library=SVML | FileCheck %s --check-prefix=SVA
