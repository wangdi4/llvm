...
; RUN: opt -hir-ssa-deconstruction -hir-vec-dir-insert -hir-vplan-vec -vplan-force-vf=4 -hir-cg -S -print-after=hir-vplan-vec -enable-vp-value-codegen-hir=1 < %s 2>&1  | FileCheck %s
; RUN: opt -passes="hir-ssa-deconstruction,hir-vec-dir-insert,hir-vplan-vec,print<hir>,hir-cg" -vplan-force-vf=4 -S -enable-vp-value-codegen-hir=1 < %s 2>&1 | FileCheck %s
