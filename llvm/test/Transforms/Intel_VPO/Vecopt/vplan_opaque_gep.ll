; RUN: opt -passes=vplan-vec -opaque-pointers -disable-output -vplan-print-after-plain-cfg < %s | FileCheck %s -check-prefix=VPLAN
; RUN: opt -passes=vplan-vec -opaque-pointers -S < %s | FileCheck %s -check-prefix=LLVM
