...
; RUN: opt -opaque-pointers=0 -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-vec-dir-insert,hir-vplan-vec" -enable-mmindex=1 -disable-nonlinear-mmindex=0 -vplan-force-vf=4 -S -print-after=hir-vplan-vec -vplan-enable-masked-vectorized-remainder=0 -vplan-enable-non-masked-vectorized-remainder=0 < %s 2>&1 | FileCheck --check-prefixes=CG_ENABLE %s
