...
; CHECK-NEXT:  entry:
; CHECK-NEXT:    br label [[PEEL_CHECKZ32:%.*]]
; CHECK:       peel.checkz32:
...
; CHECK-NEXT:    br i1 [[TMP4]], label [[MERGE_BLK30:%.*]], label [[PEEL_CHECKV33:%.*]]
; CHECK:       peel.checkv33:
...
; CHECK:       VPlannedBB:
; CHECK-NEXT:    br label [[MERGE_BLK30]]
; CHECK:       merge.blk30:
; CHECK-NEXT:    [[UNI_PHI:%.*]] = phi i64 [ 0, [[PEEL_CHECKZ32]] ], [ [[INDVARS_IV_NEXT_SL_CLONE:%.*]], [[VPLANNEDBB:%.*]] ]
...
; CHECK-NEXT:    br i1 [[TMP10]], label [[MERGE_BLK28:%.*]], label [[VPLANNEDBB3:%.*]]
; CHECK:       VPlannedBB3:
...
; CHECK-NEXT:    [[N_ADJST:%.*]] = sub nuw nsw i64 1024, [[UNI_PHI]]
...
; CHECK-NEXT:    [[SCALAR_GEP:%.*]] = getelementptr inbounds i64, ptr [[ARY]], i64 [[UNI_PHI6]]
; CHECK-NEXT:    [[TMP12:%.*]] = sext <4 x i32> [[BROADCAST_SPLAT8]] to <4 x i64>
; CHECK-NEXT:    [[TMP13:%.*]] = add <4 x i64> [[TMP12]], [[VEC_PHI]]
; CHECK-NEXT:    store <4 x i64> [[TMP13]], ptr [[SCALAR_GEP]], align 8, !intel.preferred_alignment !0
...
; CHECK:       VPlannedBB11:
; CHECK-NEXT:    [[N_ADJST12:%.*]] = sub nuw nsw i64 1024, [[UNI_PHI]]
...
; CHECK-NEXT:    br i1 [[TMP20]], label [[MERGE_BLK27:%.*]], label [[MERGE_BLK28]]
; CHECK:       merge.blk28:
; CHECK-NEXT:    [[UNI_PHI15:%.*]] = phi i64 [ [[TMP19]], [[VPLANNEDBB11]] ], [ [[UNI_PHI]], [[VPLANNEDBB2]] ]
; CHECK-NEXT:    br label [[VPLANNEDBB16:%.*]]
; CHECK:       VPlannedBB16:
; CHECK-NEXT:    [[BROADCAST_SPLATINSERT25:%.*]] = insertelement <2 x i32> poison, i32 [[C]], i64 0
...
; CHECK:       VPlannedBB17:
; CHECK-NEXT:    [[UNI_PHI15IND_START_BCAST_SPLATINSERT:%.*]] = insertelement <2 x i64> poison, i64 [[UNI_PHI15]], i64 0
...
; CHECK-NEXT:    [[N_ADJST18:%.*]] = sub nuw nsw i64 1024, [[UNI_PHI]]
...
; CHECK-NEXT:    [[SCALAR_GEP24:%.*]] = getelementptr inbounds i64, ptr [[ARY]], i64 [[UNI_PHI22]]
; CHECK-NEXT:    [[TMP22:%.*]] = sext <2 x i32> [[BROADCAST_SPLAT26]] to <2 x i64>
; CHECK-NEXT:    [[TMP23:%.*]] = add <2 x i64> [[TMP22]], [[VEC_PHI23]]
; CHECK-NEXT:    store <2 x i64> [[TMP23]], ptr [[SCALAR_GEP24]], align 8
...
; CHECK:       VPlannedBB28:
; CHECK-NEXT:    br label [[MERGE_BLK27]]
; CHECK:       merge.blk27:
; CHECK-NEXT:    [[UNI_PHI29:%.*]] = phi i64 [ [[TMP29]], [[VPLANNEDBB28]] ], [ [[TMP19]], [[VPLANNEDBB11]] ]
; CHECK-NEXT:    br label [[VPLANNEDBB30:%.*]]
; CHECK:       VPlannedBB30:
; CHECK-NEXT:    [[N_ADJST31:%.*]] = sub nuw nsw i64 1024, [[UNI_PHI]]
; CHECK-NEXT:    [[N_MOD_VF32:%.*]] = urem i64 [[N_ADJST31]], 2
; CHECK-NEXT:    [[N_VEC33:%.*]] = sub nuw nsw i64 1024, [[N_MOD_VF32]]
; CHECK-NEXT:    [[TMP30:%.*]] = icmp eq i64 1024, [[N_VEC33]]
; CHECK-NEXT:    br i1 [[TMP30]], label [[FINAL_MERGE:%.*]], label [[MERGE_BLK24]]
; CHECK:       merge.blk24:
; CHECK-NEXT:    [[UNI_PHI34:%.*]] = phi i64 [ [[UNI_PHI29]], [[VPLANNEDBB30]] ], [ 0, [[PEEL_CHECKV33]] ], [ [[UNI_PHI]], [[VPLANNEDBB1]] ]
...
; CHECK:       final.merge:
; CHECK-NEXT:    [[UNI_PHI36:%.*]] = phi i64 [ [[INDVARS_IV_NEXT:%.*]], [[VPLANNEDBB35:%.*]] ], [ [[UNI_PHI29]], [[VPLANNEDBB30]] ]
; CHECK-NEXT:    br label [[FOR_END:%.*]]
; CHECK:       for.body:
; CHECK-NEXT:    [[INDVARS_IV:%.*]] = phi i64 [ [[UNI_PHI34]], [[REMBLK15]] ], [ [[INDVARS_IV_NEXT]], [[FOR_BODY]] ]
