...
; RUN: opt -enable-new-pm=0 -hir-ssa-deconstruction -hir-temp-cleanup -hir-vec-dir-insert -hir-vplan-vec -disable-output -vplan-enable-masked-vectorized-remainder -print-after=hir-vplan-vec -hir-details -hir-verify=0 %s 2>&1 | FileCheck %s

; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-vec-dir-insert,hir-vplan-vec" -disable-output -vplan-enable-masked-vectorized-remainder -print-after=hir-vplan-vec -hir-details -hir-verify=0 %s 2>&1 | FileCheck %s
...
; FIXME: the CMP0 is livein but its symbase in not marked that way.
; CHECK:        [[CMP0:%.*]] = [[C10:%.*]] == [[C20:%.*]];
; CHECK:        <LVAL-REG> NON-LINEAR i1 [[CMP0]] {sb:21}
; CHECK:        <RVAL-REG> LINEAR i32 [[C10]] {sb:8}
; CHECK:        <RVAL-REG> LINEAR i32 [[C20]] {sb:10}
; CHECK:        + LiveIn symbases: 5, 8, 10, 11, 22, 24, 33
; CHECK:        + DO i64 i1 = 0, [[TC:.*]] + -1, 16   <DO_LOOP> <simd-vectorized> <nounroll> <novectorize>
; CHECK:        + END LOOP
; Here is the clone of CMP0 in remainder, it's correctly marked as livein
; CHECK:        [[CMP150:%.*]] = [[C10]] == [[C20]]
; CHECK:        <LVAL-REG> NON-LINEAR i1 [[CMP150]] {sb:33}
; CHECK:        <RVAL-REG> LINEAR i32 [[C10]] {sb:8}
; CHECK:        <RVAL-REG> LINEAR i32 [[C20]] {sb:10}
; CHECK:        + LiveIn symbases: 5, 8, 10, 11, 19, 33, 34
; CHECK:        + DO i64 i1 = [[LB:%.*]], %N + -1, 16   <DO_LOOP>  <MAX_TC_EST = 1>  <LEGAL_MAX_TC = 1> <nounroll> <novectorize> <max_trip_count = 1>
; CHECK:        + END LOOP
