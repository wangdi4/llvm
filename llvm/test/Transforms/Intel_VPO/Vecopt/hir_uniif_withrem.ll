...
; RUN: opt -enable-new-pm=0 -hir-ssa-deconstruction -hir-temp-cleanup -hir-vec-dir-insert -hir-vplan-vec -disable-output -vplan-enable-masked-vectorized-remainder -vplan-masked-remainder-gain-threshold=0 -print-after=hir-vplan-vec -hir-details -vplan-enable-peel-rem-strip=0 %s 2>&1 | FileCheck %s

; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-vec-dir-insert,hir-vplan-vec" -disable-output -vplan-enable-masked-vectorized-remainder -vplan-masked-remainder-gain-threshold=0 -print-after=hir-vplan-vec -hir-details -vplan-enable-peel-rem-strip=0 %s 2>&1 | FileCheck %s
