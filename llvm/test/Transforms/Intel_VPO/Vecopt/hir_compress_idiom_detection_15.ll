; RUN: opt %s -mattr=+avx512f,+avx512vl -passes='hir-ssa-deconstruction,hir-temp-cleanup,hir-vec-dir-insert,hir-vplan-vec,print<hir>' -disable-output -debug-only=parvec-analysis -vplan-print-after-plain-cfg -vplan-print-after-vpentity-instrs -vplan-entities-dump -vplan-force-vf=16 2>&1 | FileCheck %s
...
; CHECK-NEXT:       i64 [[VP10]] = zext i32 [[VP6]] to i64
; CHECK-NEXT:       i64 [[VP16:%.*]] = compress-expand-index i64 [[VP10]] i32 5
...
; CHECK-NEXT:     i32 [[VP21]] = compress-expand-index-inc i32 [[VP6]] i1 [[VP7]] i32 5
...
; CHECK-NEXT:        |   [[ZEXT0:%.*]] = zext.i16.i32([[POPCNT80]])
; CHECK-NEXT:        |   [[MUL0:%.*]] = [[ZEXT0]]  *  5
; CHECK-NEXT:        |   [[ADD90:%.*]] = [[PHI_TEMP0]]  +  [[MUL0]]
; CHECK-NEXT:        |   [[PHI_TEMP0]] = [[ADD90]]
; CHECK-NEXT:        + END LOOP
; CHECK:             [[K_0130]] = [[ADD90]]
...
; CM4: Cost 2 for i64 [[VP7:%.*]] = compress-expand-index i64 [[VP6:%.*]] i32 5
...
; CM4: Cost 3 for i32 [[VP15:%.*]] = compress-expand-index-inc i32 [[VP1:%.*]]
...
; CM8: Cost 2 for i64 [[VP7:%.*]] = compress-expand-index i64 [[VP6:%.*]] i32 5
...
; CM8: Cost 3 for i32 [[VP15:%.*]] = compress-expand-index-inc i32 [[VP1:%.*]]
