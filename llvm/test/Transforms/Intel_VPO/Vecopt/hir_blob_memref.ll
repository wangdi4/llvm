...
; RUN: opt -enable-new-pm=0 -hir-ssa-deconstruction -hir-vec-dir-insert -hir-vplan-vec -vplan-force-vf=4 -hir-cg -simplifycfg -sroa -print-after=hir-vplan-vec -print-after=sroa -disable-output < %s 2>&1 | FileCheck %s -check-prefixes=PM1
; RUN: opt -passes="hir-ssa-deconstruction,hir-vec-dir-insert,hir-vplan-vec,hir-cg,simplifycfg,sroa" -vplan-force-vf=4 -print-after=hir-vplan-vec -print-after=sroa -disable-output < %s 2>&1 | FileCheck %s -check-prefixes=PM2
