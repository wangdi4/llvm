...
; RUN: opt -hir-ssa-deconstruction -hir-vec-dir-insert -hir-vplan-vec -enable-vp-value-codegen-hir -vplan-force-vf=4 -hir-cg -simplifycfg -sroa -print-after=hir-vplan-vec -print-after=sroa -disable-output < %s 2>&1 | FileCheck %s -check-prefixes=PM1
; RUN: opt -passes="hir-ssa-deconstruction,hir-vec-dir-insert,hir-vplan-vec,hir-cg,simplifycfg,sroa" -enable-vp-value-codegen-hir -vplan-force-vf=4 -print-after=hir-vplan-vec -print-after=sroa -disable-output < %s 2>&1 | FileCheck %s -check-prefixes=PM2
