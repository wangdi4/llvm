...
; REQUIRES: asserts
; RUN: opt < %s -VPlanDriver -mtriple=x86_64-unknown-linux-gnu -tti -mattr=+sse4.2 -enable-intel-advanced-opts -vplan-enable-all-zero-bypass-non-loops -vplan-print-after-all-zero-bypass -vplan-force-vf=4 -vplan-all-zero-bypass-region-threshold=1 -disable-output -S -vplan-print-terminator-inst 2>&1 | FileCheck %s
; RUN: opt < %s -passes="vplan-driver" -mtriple=x86_64-unknown-linux-gnu -mattr=+sse4.2 -enable-intel-advanced-opts -vplan-enable-all-zero-bypass-non-loops -vplan-print-after-all-zero-bypass -vplan-force-vf=4 -vplan-all-zero-bypass-region-threshold=1 -disable-output -S -vplan-print-terminator-inst 2>&1 | FileCheck %s
...
; CHECK-NEXT:     [DA: Uni] br i1 [[VP_VECTOR_LOOP_EXITCOND]], [[BB5:BB[0-9]+]], [[BB2]]
; CHECK-EMPTY:
