...
; REQUIRES: asserts
; RUN: opt < %s -VPlanDriver -vplan-enable-all-zero-bypass -vplan-print-after-all-zero-bypass -vplan-force-vf=4 -vplan-all-zero-bypass-region-threshold=1 -disable-output -S -vplan-print-terminator-inst 2>&1 | FileCheck %s
; RUN: opt < %s -passes="vplan-driver" -vplan-enable-all-zero-bypass -vplan-print-after-all-zero-bypass -vplan-force-vf=4 -vplan-all-zero-bypass-region-threshold=1 -disable-output -S -vplan-print-terminator-inst 2>&1 | FileCheck %s
...
; CHECK-NEXT:     [DA: Div] br all.zero.bypass.begin9
; CHECK-EMPTY:
...
; CHECK-NEXT:       [DA: Div] br all.zero.bypass.end11
; CHECK-EMPTY:
