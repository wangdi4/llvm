; RUN: opt -disable-output -passes='hir-ssa-deconstruction,hir-vec-dir-insert,hir-vplan-vec' -vplan-force-vf=2 -vplan-entities-dump -vplan-print-after-vpentity-instrs -vplan-dump-induction-init-details < %s 2>&1 | FileCheck %s
; REQUIRES: asserts
...
define dso_local void @foo(ptr noalias nocapture %a, ptr noalias nocapture readonly %b, i32 %n) local_unnamed_addr #0
{
entry:
  br label %for.body
