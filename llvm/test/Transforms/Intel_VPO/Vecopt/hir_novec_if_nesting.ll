...
; RUN: opt -vplan-enable-new-cfg-merge-hir=false -enable-new-pm=0 -hir-ssa-deconstruction -analyze -hir-parvec-analysis -debug-only=parvec-analysis < %s 2>&1 | FileCheck %s
; RUN: opt -vplan-enable-new-cfg-merge-hir=false -passes='hir-ssa-deconstruction,print<hir-parvec-analysis>' -debug-only=parvec-analysis -disable-output < %s 2>&1 | FileCheck %s
; RUN: opt -vplan-enable-new-cfg-merge-hir -enable-new-pm=0 -hir-ssa-deconstruction -analyze -hir-parvec-analysis -debug-only=parvec-analysis < %s 2>&1 | FileCheck %s
; RUN: opt -vplan-enable-new-cfg-merge-hir -passes='hir-ssa-deconstruction,print<hir-parvec-analysis>' -debug-only=parvec-analysis -disable-output < %s 2>&1 | FileCheck %s
