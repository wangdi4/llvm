...
; RUN: opt -S -enable-new-pm=0 -disable-output -hir-ssa-deconstruction -hir-vec-dir-insert -hir-vplan-vec -print-after=hir-vplan-vec -vplan-force-vf=2 < %s 2>&1 | FileCheck %s
; RUN: opt -S -passes="hir-ssa-deconstruction,hir-vec-dir-insert,hir-vplan-vec,print<hir>" -disable-output -vplan-force-vf=2 < %s 2>&1 | FileCheck %s
...
; CHECK-NEXT:        + DO i1 = 0, 75, 2   <DO_LOOP> <auto-vectorized> <novectorize>
...
; CHECK-NEXT:        + END LOOP
; CHECK:             + DO i1 = 76, 76, 1   <DO_LOOP> <novectorize>
