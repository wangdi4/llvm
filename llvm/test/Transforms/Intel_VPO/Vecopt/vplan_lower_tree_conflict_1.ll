...
; Check that we do not crash when forcing an unroll factor
; RUN: opt -S -mattr=+avx512vl,+avx512cd -vplan-force-vf=4 -vplan-force-uf=2 -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-vec-dir-insert,hir-vplan-vec" -print-after=hir-vplan-vec -disable-output < %s 2>&1 | FileCheck %s --check-prefix=CHECK-VF4
...
; will need to be added. This goes for all other similar tests for tree conflict
; lowering.
