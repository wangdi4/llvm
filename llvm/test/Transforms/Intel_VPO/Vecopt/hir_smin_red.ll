...
;       |   %min.013 = @llvm.smin.i32((%a)[i1],  %min.013);
;       + END LOOP
; END REGION
...
; CHECK:  signed (SIntMin) Start: i32 %min.013 Exit: i32 [[VP_LOAD:%.*]]
; CHECK:   Linked values: i32 [[VP1:%.*]], i32 [[VP_LOAD:%.*]], i32 [[VP2:%.*]], i32 [[VP3:%.*]],

; CHECK: i32 [[VP2:%.*]] = reduction-init i32 %min.013
; CHECK: i32 [[VP_LOAD:%.*]] = call i32 [[VP4:%.*]] i32 [[VP1:%.*]] i32 (i32, i32)* @llvm.smin.i32
; CHECK: i32 [[VP3:%.*]] = reduction-final{u_smin} i32 [[VP_LOAD:%.*]]

; Check HIR after VPlan

; CHECK: %vec.tc4 = %tgu3  *  8;
; CHECK: %red.init = %min.013;
; CHECK: %phi.temp5 = %red.init;
; CHECK: %loop.ub = %vec.tc4  -  1;
; CHECK: + DO i1 = 0, %loop.ub, 8   <DO_LOOP>  <MAX_TC_EST = 268435455>  <LEGAL_MAX_TC = 268435455> <auto-vectorized> <nounroll> <novectorize>
; CHECK: |   %.vec7 = (<8 x i32>*)(%a)[i1];
; CHECK: |   %llvm.smin.v8i32 = @llvm.smin.v8i32(%.vec7,  %phi.temp5);
; CHECK: |   %phi.temp5 = %llvm.smin.v8i32;
; CHECK: + END LOOP
; CHECK: %min.013 = @llvm.vector.reduce.smin.v8i32(%llvm.smin.v8i32);
...
for.body:                                         ; preds = %for.body.preheader, %for.body
  ...
  %1 = tail call i32 @llvm.smin.i32(i32 %0, i32 %min.013)
