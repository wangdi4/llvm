...
; REQUIRES: asserts
; RUN: opt -vplan-enable-soa=false -S %s -VPlanDriver -vplan-dump-da 2>&1 -disable-output | FileCheck %s

; The run line below used to crash before CMPLRLLVM-22366 was fixed.
; RUN: opt -S %s -VPlanDriver -vplan-dump-da -vplan-enable-all-zero-bypass-loops=0 -disable-output 2>&1 | FileCheck %s
...
; CHECK-NEXT:  Uniform: [Shape: Uniform] br i1 [[VP_VECTOR_LOOP_EXITCOND]], [[BB6:BB[0-9]+]], [[BB0]]
; CHECK-EMPTY:
