...
; RUN:     -disable-vplan-cost-model=0 | FileCheck %s --check-prefix=CHECK-LLVM

; RUN: opt < %s -S -hir-ssa-deconstruction -hir-vec-dir-insert -VPlanDriverHIR \
; RUN:     -mtriple=x86_64-unknown-unknown -mattr=+avx2 \
; RUN:     -disable-vplan-cost-model=0 | FileCheck %s --check-prefix=CHECK-HIR
...
; CHECK-LLVM: fadd fast <4 x float>
; CHECK-HIR-LABEL: test_vectorize
; CHECK-HIR: fadd fast <4 x float>
entry:
