...
; RUN: opt -enable-intel-advanced-opts -mattr=+avx2 -xmain-opt-level=3 -hir-ssa-deconstruction -hir-temp-cleanup -hir-loop-concatenation -hir-vec-dir-insert -VPlanDriverHIR -hir-cg -vplan-print-after-plain-cfg -S -disable-output 2>&1 < %s | FileCheck %s
; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-loop-concatenation,hir-vec-dir-insert,vplan-driver-hir,hir-cg" -enable-intel-advanced-opts -mattr=+avx2 -xmain-opt-level=3 -vplan-print-after-plain-cfg -S -disable-output 2>&1 < %s | FileCheck %s
