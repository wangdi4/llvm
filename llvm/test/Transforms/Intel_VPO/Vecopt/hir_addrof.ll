...
; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-vec-dir-insert,hir-vplan-vec" -vplan-force-vf=4 -print-changed -disable-output < %s 2>&1 | FileCheck %s --check-prefix=CHECK-CHANGED

; CHECK:      DO i1 = 0, 1023, 4   <DO_LOOP>  <auto-vectorized> <novectorize>
; CHECK-NEXT:     (<4 x ptr>*)(@ip)[0][i1] = &((<4 x ptr>)(@arr)[0][i1 + <i64 0, i64 1, i64 2, i64 3>]);
; CHECK-NEXT: END LOOP

; Verify that hir-vec-dir-insert and hir-vplan-vec passes are dumped with
; print-changed when they trigger.


; CHECK-CHANGED: Dump Before HIRTempCleanup
; CHECK-CHANGED: Dump After HIRVecDirInsert
; CHECK-CHANGED: Dump After vpo::VPlanDriverHIR
...
@ip = common local_unnamed_addr global [1024 x ptr] zeroinitializer, align 16
...
for.body:                                         ; preds = %for.body, %entry
  ...
  %arrayidx2 = getelementptr inbounds [1024 x ptr], ptr @ip, i64 0, i64 %indvars.iv
  store ptr %arrayidx, ptr %arrayidx2, align 8, !tbaa !1
