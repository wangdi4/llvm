...
; RUN: opt -enable-new-pm=0 -hir-ssa-deconstruction -hir-temp-cleanup -hir-vec-dir-insert -hir-vplan-vec -vplan-enable-masked-vectorized-remainder -disable-output -hir-verify=0 -hir-details -print-after=hir-vplan-vec %s 2>&1 | FileCheck %s
; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-vec-dir-insert,hir-vplan-vec" -vplan-enable-masked-vectorized-remainder -disable-output -hir-verify=0 -hir-details -print-after=hir-vplan-vec %s 2>&1 | FileCheck %s
...
; Main loop
; CHECK:             |   + DO i64 i2 = 0, [[VEC_TC30:%.*]] + -1, 8   <DO_LOOP> <auto-vectorized> <nounroll> <novectorize>
; CHECK-NEXT:        |   | <RVAL-REG> LINEAR i64 [[VEC_TC30]] + -1{def@1} {sb:2}
; CHECK-NEXT:        |   |    <BLOB> LINEAR i64 %vec.tc3{def@1} {sb:20}
; skipping body
; CHECK:             |   + END LOOP
;
; Remainder, non-linear upper bound generated
; CHECK:             |   + DO i64 i2 = [[PHI_TEMP0:%.*]], -1 * %"val$[]671_fetch.1074" + [[N0:%.*]] + -1, 8   <DO_LOOP>  <MAX_TC_EST = 1>  <LEGAL_MAX_TC = 1> <nounroll> <novectorize> <max_trip_count = 1>
; CHECK-NEXT:        |   | <RVAL-REG> LINEAR i64 %phi.temp{def@1} {sb:17}
; CHECK-NEXT:        |   | <RVAL-REG> NON-LINEAR i64 -1 * %"val$[]671_fetch.1074" + [[N0]] + -1 {sb:2}
; CHECK-NEXT:        |   |    <BLOB> NON-LINEAR i64 %"val$[]671_fetch.1074" {sb:4}
; CHECK-NEXT:        |   |    <BLOB> LINEAR i64 [[N0]] {sb:6}
; skipping body
; CHECK:             |   + END LOOP
;
alloca_11:
