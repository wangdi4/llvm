...
; RUN: opt -passes="vplan-vec" -vplan-force-vf=2 -vplan-entities-dump -vplan-print-after-vpentity-instrs -vplan-dump-induction-init-details -S < %s 2>&1 | FileCheck %s
; REQUIRES: asserts
...
define dso_local void @foo(ptr %x, ptr %y, ptr %z) #0 {
...
; CHECK-NEXT:     ptr [[VP_LOAD_Y1_1:%.*]] = load ptr [[VP_Y_LINEAR_PTR]]
; CHECK-NEXT:     i32 [[VP_LOAD_Y2:%.*]] = load ptr [[VP_LOAD_Y1_1]]
...
; CHECK-NEXT:     ptr [[VP_LOAD_X1_1:%.*]] = load ptr [[VP_X_LINEAR_PTR]]
; CHECK-NEXT:     ptr [[VP_LOAD_Z1_1:%.*]] = load ptr [[VP_Z_LINEAR_PTR]]
; CHECK-NEXT:     call ptr [[VP_LOAD_X1_1]] ptr [[VP_LOAD_Z1_1]] ptr @_Z3bazPcPc
