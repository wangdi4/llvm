...
; RUN: opt %s -S -vplan-vec -mattr=avx2 -debug-only=VectorVariant -debug-only=VPlanCallVecDecisions 2>&1 | FileCheck %s
; RUN: opt %s -S -passes="vplan-vec" -mattr=avx2 -debug-only=VectorVariant -debug-only=VPlanCallVecDecisions 2>&1 | FileCheck %s
; REQUIRES: asserts
