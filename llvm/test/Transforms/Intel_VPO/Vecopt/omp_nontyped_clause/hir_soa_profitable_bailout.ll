...
; RUN: opt -opaque-pointers=0 -passes="hir-ssa-deconstruction,hir-vplan-vec,print<hir>" -disable-output -vplan-force-vf=4 < %s 2>&1 | FileCheck %s --check-prefix=SOA
;
...
; RUN: opt -opaque-pointers=0 -passes="hir-ssa-deconstruction,hir-vplan-vec,print<hir>" -disable-output -vplan-force-vf=4 -vplan-enable-soa-hir=0 < %s 2>&1 | FileCheck %s --check-prefix=NOSOA
;
...
DIR.OMP.SIMD.1:
  %tok = call token @llvm.directive.region.entry() [ "DIR.OMP.SIMD"(), "QUAL.OMP.PRIVATE"([100 x i64]* %arr) ]
  br label %for.body
