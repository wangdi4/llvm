...
; REQUIRES: asserts
; RUN: opt -enable-new-pm=0 -vplan-vec -enable-intel-advanced-opts -vplan-enable-all-zero-bypass-non-loops -vplan-print-after-all-zero-bypass -vplan-enable-masked-vectorized-remainder=0 -vplan-enable-non-masked-vectorized-remainder=0 \
; RUN: -vplan-all-zero-bypass-region-threshold=1 -disable-vplan-codegen -disable-output < %s 2>&1 | FileCheck %s
; RUN: opt -passes='vplan-vec' -enable-intel-advanced-opts -vplan-enable-all-zero-bypass-non-loops -vplan-print-after-all-zero-bypass -vplan-enable-masked-vectorized-remainder=0 -vplan-enable-non-masked-vectorized-remainder=0 \
; RUN: -vplan-all-zero-bypass-region-threshold=1 -disable-vplan-codegen -disable-output < %s  2>&1 | FileCheck %s
