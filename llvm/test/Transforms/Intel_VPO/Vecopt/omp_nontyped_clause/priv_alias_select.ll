...
;
; RUN: opt -opaque-pointers=0 -passes=vplan-vec -vplan-print-after-vpentity-instrs -vplan-force-vf=4 -disable-output %s | FileCheck %s
...
; CHECK:     i64* [[VP_PRIV2:%.*]] = allocate-priv i64*, OrigAlign = 8
; CHECK-NEXT: i8* [[VP_PRIV2_BCAST:%.*]] = bitcast i64* [[VP_PRIV2]]
...
; CHECK-NEXT:     i64* [[VP_SELECT:%.*]] = select i1 [[PRED0:%.*]] i64* [[VP_PRIV1]] i64* [[VP_PRIV2]]
...
simd.begin.region:
  %entry.region = call token @llvm.directive.region.entry() [ "DIR.OMP.SIMD"(), "QUAL.OMP.SIMDLEN"(i32 16), "QUAL.OMP.PRIVATE"(i64* %priv1), "QUAL.OMP.PRIVATE"(i64* %priv2), "QUAL.OMP.UNIFORM"(i64 %init1), "QUAL.OMP.UNIFORM"(i64 %init2), "QUAL.OMP.UNIFORM"(i64* %ptr), "QUAL.OMP.UNIFORM"(i1 %pred) ]
  br label %simd.loop.preheader

simd.loop.preheader:
  %select = select i1 %pred, i64* %priv1, i64* %priv2
  br label %simd.loop.header

simd.loop.header:
  %index = phi i64 [ 0, %simd.loop.preheader ], [ %indvar, %simd.loop.latch ]
  store i64 %init1, i64* %priv1, align 8
  store i64 %init2, i64* %priv2, align 8
