...
; RUN: opt -opaque-pointers=0 -passes=vplan-vec -vplan-force-vf=2 -vplan-enable-soa \
...
; CHECK-NEXT:  VPlan IR for: test_memref_transform:for.body.#{{[0-9]+}}
; CHECK-NEXT:    [[BB0:BB[0-9]+]]: # preds:
; CHECK-NEXT:     [DA: Uni] br [[BB1:BB[0-9]+]]
; CHECK-EMPTY:
; CHECK-NEXT:    [[BB1]]: # preds: [[BB0]]
; CHECK-NEXT:     [DA: Div] [1024 x i32]* [[VP_ARR_PRIV:%.*]] = allocate-priv [1024 x i32]*, OrigAlign = 4
; CHECK-NEXT:     [DA: Div] i8* [[VP0:%.*]] = bitcast [1024 x i32]* [[VP_ARR_PRIV]]
; CHECK-NEXT:     [DA: Div] call i64 4096 i8* [[VP0]] void (i64, i8*)* @llvm.lifetime.start.p0i8
...
; CHECK-NEXT:     [DA: Div] i8* [[VP1:%.*]] = bitcast [1024 x i32]* [[VP_ARR_PRIV]]
; CHECK-NEXT:     [DA: Div] call i64 4096 i8* [[VP1]] void (i64, i8*)* @llvm.lifetime.end.p0i8
...
; CHECK-NEXT:    [[SOA_VECTORGEP60:%.*]] = getelementptr inbounds [1024 x <2 x i32>], [1024 x <2 x i32>]* [[ARR_PRIV_SOA_VEC0]], <2 x i64> zeroinitializer, <2 x i64> [[VEC_PHI0]], <2 x i64> <i64 0, i64 1>
; CHECK-NEXT:    [[SOA_VECTORGEP70:%.*]] = getelementptr inbounds <2 x i32>, <2 x <2 x i32>*> [[SOA_VECTORGEP0]], <2 x i64> [[VEC_PHI0]], <2 x i64> <i64 0, i64 1>
...
; CHECK-NEXT:    call void @llvm.masked.scatter.v2i32.v2p0i32(<2 x i32> <i32 20, i32 20>, <2 x i32*> [[SOA_VECTORGEP70]], i32 4, <2 x i1> <i1 true, i1 true>)
; CHECK-NEXT:    [[TMP2:%.*]] = sext <2 x i32> [[WIDE_LOAD0]] to <2 x i64>
; CHECK-NEXT:    [[SOA_VECTORGEP80:%.*]] = getelementptr inbounds [1024 x <2 x i32>], [1024 x <2 x i32>]* [[ARR_PRIV_SOA_VEC0]], <2 x i64> zeroinitializer, <2 x i64> [[TMP2]], <2 x i64> <i64 0, i64 1>
; CHECK-NEXT:    [[WIDE_MASKED_GATHER90:%.*]] = call <2 x i32> @llvm.masked.gather.v2i32.v2p0i32(<2 x i32*> [[SOA_VECTORGEP80]], i32 4, <2 x i1> <i1 true, i1 true>, <2 x i32> poison)
; CHECK-NEXT:    call void @llvm.masked.scatter.v2i32.v2p0i32(<2 x i32> <i32 30, i32 30>, <2 x i32*> [[SOA_VECTORGEP80]], i32 4, <2 x i1> <i1 true, i1 true>)
...
; CHECK-NEXT:    br i1 [[TMP5]], label [[VECTOR_BODY0]], label [[VPLANNEDBB100:%.*]], !llvm.loop !0
; CHECK-EMPTY:
