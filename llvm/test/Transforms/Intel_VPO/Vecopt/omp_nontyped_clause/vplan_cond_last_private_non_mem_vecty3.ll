...
;
; RUN: opt -opaque-pointers=0 %s -disable-output -passes="vplan-vec" -debug-only=LoopVectorizationPlanner 2>&1 | FileCheck %s
...
b1:
  %entry.region = call token @llvm.directive.region.entry() [ "DIR.OMP.SIMD"(), "QUAL.OMP.LASTPRIVATE"(<2 x i64>* %mm), "QUAL.OMP.SIMDLEN"(i64 2) ]
  br label %for.body
