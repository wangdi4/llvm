...
; CHECK:          ptr [[VP_PRIV2:%.*]] = allocate-priv double, OrigAlign = 8
...
; CHECK-NEXT:     ptr [[VP_P2_ALIAS:%.*]] = bitcast ptr [[VP_PRIV2]]
; CHECK-NEXT:     ptr [[VP_SELECT:%.*]] = select i1 [[PRED0:%.*]] ptr [[VP_PRIV1]] ptr [[VP_P2_ALIAS]]

entry:
  %priv1 = alloca i64, align 8
  %priv2 = alloca double, align 8
  %priv3 = alloca double, align 8
  %p2_alias = bitcast ptr %priv2 to ptr

  br label %simd.begin.region

simd.begin.region:
  %entry.region = call token @llvm.directive.region.entry() [ "DIR.OMP.SIMD"(), "QUAL.OMP.SIMDLEN"(i32 16), "QUAL.OMP.PRIVATE:TYPED"(ptr %priv1, i64 0, i32 1), "QUAL.OMP.PRIVATE:TYPED"(ptr %priv2, double zeroinitializer, i32 1), "QUAL.OMP.UNIFORM"(i64 %init1), "QUAL.OMP.UNIFORM"(i64 %init2), "QUAL.OMP.UNIFORM:TYPED"(ptr %ptr, i64 0, i32 1), "QUAL.OMP.UNIFORM"(i1 %pred) ]
  br label %simd.loop.preheader

simd.loop.preheader:
  %select = select i1 %pred, ptr %priv1, ptr %p2_alias
  br label %simd.loop.header

simd.loop.header:
  %index = phi i64 [ 0, %simd.loop.preheader ], [ %indvar, %simd.loop.latch ]
  store i64 %init1, ptr %priv1, align 8
  store i64 %init2, ptr %p2_alias, align 8
