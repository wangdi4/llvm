...
; a vector variant _ZGVyN8ulv_foo of "foo".
; Check that 1st and 2dn arguments (uniform and linear respectively) remain scalar
; (hence do not require legalization) while the 3d vector argument is properly legalizated.
; The return value does require legalization.
; The test is simlar to vplan_legalize_vecvariant_vul.ll with only difference the arguments order.

declare i32 @foo(i32 %n, i32 %i, i32 %x) local_unnamed_addr #0
...
; CHECK-NEXT:    [[TMP1:%.*]] = call x86_regcallcc { <4 x i32>, <4 x i32> } @_ZGVyN8ulv_foo(i32 256, i32 [[DOTEXTRACT_0_0]], <4 x i32> [[WIDE_LOAD_PART_0_OF_2_0]], <4 x i32> [[WIDE_LOAD_PART_1_OF_2_0]])
...
; HIR-NEXT:        |   [[_ZGVYN8ULV_FOO0:%.*]] = @_ZGVyN8ulv_foo(256,  i1,  [[DOTEXTRACTED_SUBVEC0]],  [[DOTEXTRACTED_SUBVEC20]])
; HIR-NEXT:        |   [[EXTRACT_RESULT0:%.*]] = extractvalue [[_ZGVYN8ULV_FOO0]], 0
; HIR-NEXT:        |   [[EXTRACT_RESULT30:%.*]] = extractvalue [[_ZGVYN8ULV_FOO0]], 1
...
attributes #0 = { "vector-variants"="_ZGVyN8ulv_foo" }
