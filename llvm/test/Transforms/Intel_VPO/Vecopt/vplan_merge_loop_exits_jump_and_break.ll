; Test merge loop exits transformation when the loop has one jump and one break outside of the loop.

; REQUIRES: asserts
; RUN: opt -S %s -VPlanDriver -vplan-force-vf=4 -disable-vplan-codegen -debug 2>&1 | FileCheck %s
...
; CHECK-NEXT: {{BB[0-9]+}} (BP: NULL) :
; CHECK-NEXT: i32 [[PHI_OUT0:%vp[0-9]+]] = phi  [ i32 0, {{BB[0-9]+}} ], [ i32 {{%vp[0-9]+}}, {{NewLoopLatch[0-9]+}} ]
; CHECK-NEXT: i32 [[PHI_OUT1:%vp[0-9]+]] = phi  [ i32 {{%vp[0-9]+}}, {{NewLoopLatch[0-9]+}} ],  [ i32 0, {{BB[0-9]+}} ]
...
; CHECK-NEXT: PREDECESSORS(2): {{NewLoopLatch[0-9]+}} {{BB[0-9]+}}
; CHECK-EMPTY:
...
; CHECK-NEXT: i32 [[PHI_OUT2:%vp[0-9]+]] = phi  [ i32 [[PHI_OUT1]], {{BB[0-9]+}} ],  [ i32 1, {{IntermediateBB[0-9]+}} ],  [ i32 2, {{IntermediateBB[0-9]+}} ]
; CHECK-NEXT: i1 [[PHI_OUT3:%vp[0-9]+]] = phi  [ i1 [[CMP_OUT3]], {{BB[0-9]+}} ],  [ i1 false, {{IntermediateBB[0-9]+}} ],  [ i1 false, {{IntermediateBB[0-9]+}} ]
; CHECK-NEXT: i1 [[CMP_OUT4:%vp[0-9]+]] = icmp i32 [[PHI_OUT2]] i32 0
; CHECK-NEXT: i1 [[AND_OUT4:%vp[0-9]+]] = and i1 [[CMP_OUT4]] i1 [[PHI_OUT3]]
; CHECK-NEXT: SUCCESSORS(2):{{BB[0-9]+}}(i1 [[AND_OUT4]]), {{IfBlock[0-9]+}}(!i1 [[AND_OUT4]])
; CHECK-NEXT: PREDECESSORS(3): {{BB[0-9]+}} {{IntermediateBB[0-9]+}} {{IntermediateBB[0-9]+}}
; CHECK-EMPTY:
...
loop2_latch:
  ...
  br i1 %cmp3, label %loop2, label %bb5
