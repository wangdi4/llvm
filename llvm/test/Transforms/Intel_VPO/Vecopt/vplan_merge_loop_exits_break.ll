; Test merge loop exits transformation when the loop has one break outside of the loop.
...
; CHECK-NEXT: i1 [[CMP_OUT3:%vp[0-9]+]] = icmp i32 [[PHI_OUT2]] i32 0
; CHECK-NEXT: i1 [[AND_OUT3:%vp[0-9]+]] = and i1 [[CMP_OUT3]] i1 [[PHI_OUT3]]
; CHECK-NEXT: SUCCESSORS(2):[[LoopHeader]](i1 [[AND_OUT3]]), {{BB[0-9]+}}(!i1 [[AND_OUT3]])
...
; CHECK-NEXT: i1 [[CMP_OUT4:%vp[0-9]+]] = icmp
; CHECK-NEXT: SUCCESSORS(2):{{BB[0-9]+}}(i1 [[CMP_OUT4]]), {{BB[0-9]+}}(!i1 [[CMP_OUT4]])
; CHECK-NEXT: PREDECESSORS(1): {{BB[0-9]+}}
; CHECK-EMPTY:
...
loop2:
  ...
  br i1 %cmp1, label %bb3, label %loop2_latch
