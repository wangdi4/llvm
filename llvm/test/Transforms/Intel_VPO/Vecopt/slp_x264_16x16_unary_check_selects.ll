...
; RUN: opt < %s -slp-vectorizer -mtriple=x86_64-unknown-linux-gnu -pslp -mcpu=skylake-avx512 -x86-experimental-max-legal-vector-width=256 -tti -S | FileCheck %s -check-prefix=8WIDE
...
; 8WIDE-NEXT:    [[TMP161:%.*]] = sub <8 x i32> [[TMP160]], [[TMP90]]
; 8WIDE-NEXT:    [[TMP162:%.*]] = add <8 x i32> [[TMP160]], [[TMP90]]
; 8WIDE-NEXT:    [[TMP163:%.*]] = select <8 x i1> <i1 true, i1 false, i1 false, i1 true, i1 true, i1 false, i1 false, i1 true>, <8 x i32> [[TMP162]], <8 x i32> [[TMP161]]
