...
; CHECK:      [Compress/Expand Idiom] Increment {sb:3}+1 detected: <18>         %n2.031 = %n2.031  +  1;
; CHECK-NEXT: [Compress/Expand Idiom] Data-dependency on other memory found: (%B)[0]
; CHECK-NEXT: [Compress/Expand Idiom] Increment rejected: <18>         %n2.031 = %n2.031  +  1;
...
for.inc:                                          ; preds = %for.body, %if.else10, %if.then5
  %n1.1 = phi i32 [ %n1.030, %for.body ], [ %n1.030, %if.then5 ], [ %inc13, %if.else10 ]
  %n2.1 = phi i32 [ %n2.031, %for.body ], [ %n2.031, %if.then5 ], [ %inc, %if.else10 ]
  %res = phi double [ 1., %for.body ], [ 2., %if.then5 ], [ %3, %if.else10 ]
