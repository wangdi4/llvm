...
define dso_local void @run_test.V(ptr noundef %vals, ptr nocapture noundef readonly %src, ptr nocapture noundef writeonly %dst) local_unnamed_addr #3 {
; CHECK-LABEL: VPlan after CallVecDecisions analysis for merged CFG:
; CHECK-NEXT:  VPlan IR for: run_test.V:omp.inner.for.body.#1
; FIXME - Has to be YMM2(AVX2) vector variant call
...
omp.inner.for.body:                               ; preds = %omp.inner.for.body, %DIR.OMP.SIMD.117
  ...
  %callret = call fast nofpclass(nan inf) double @Interpolate(double noundef nofpclass(nan inf) %2, ptr noundef %vals) #2
