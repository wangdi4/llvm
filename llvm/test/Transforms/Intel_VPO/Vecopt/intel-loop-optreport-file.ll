...
; RUN: opt -vplan-vec -vplan-force-vf=4 -intel-loop-optreport=high -intel-ir-optreport-emitter -intel-loop-optreport-file=stdout < %s -disable-output | FileCheck %s --strict-whitespace -check-prefixes=LLVM
; RUN: opt -vplan-vec -vplan-force-vf=4 -intel-loop-optreport=high -intel-ir-optreport-emitter -intel-loop-optreport-file=stderr < %s -disable-output 2>&1 >%tout | FileCheck %s --strict-whitespace -check-prefixes=LLVM
; RUN: opt -vplan-vec -vplan-force-vf=4 -intel-loop-optreport=high -intel-ir-optreport-emitter -intel-loop-optreport-file=%t < %s -disable-output
...
; RUN: opt -vplan-vec -vplan-force-vf=4 -intel-loop-optreport=high -intel-ir-optreport-emitter -intel-loop-optreport-file=%S < %s -disable-output 2>%terr | FileCheck %s --strict-whitespace -check-prefixes=LLVM
