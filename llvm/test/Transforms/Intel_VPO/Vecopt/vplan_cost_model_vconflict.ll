...
; REQUIRES: asserts
; RUN: opt -mattr=+avx512vl,+avx512cd -S -hir-ssa-deconstruction -hir-temp-cleanup -hir-vec-dir-insert -enable-vconflict-idiom -hir-vplan-vec -disable-vplan-codegen -disable-output -vplan-cost-model-print-analysis-for-vf=1 < %s 2>&1 | FileCheck %s --check-prefix=CHECK-VF1
