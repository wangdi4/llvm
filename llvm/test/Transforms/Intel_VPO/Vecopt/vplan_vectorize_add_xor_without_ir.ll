...
; CHECK:       VPlannedBB:
; CHECK-NEXT:    [[VEC_PHI:%.*]] = phi <2 x i64> [ [[TMP19:%.*]], [[VPLANNEDBB:%.*]] ], [ undef, [[VECTOR_BODY:%.*]] ]
; CHECK-NEXT:    [[VEC_PHI1:%.*]] = phi <2 x i64> [ [[TMP18:%.*]], [[VPLANNEDBB]] ], [ undef, [[VECTOR_BODY]] ]
; CHECK-NEXT:    [[VEC_PHI2:%.*]] = phi <2 x i1> [ [[TMP17:%.*]], [[VPLANNEDBB]] ], [ undef, [[VECTOR_BODY]] ]
; CHECK-NEXT:    [[VEC_PHI3:%.*]] = phi <2 x i64> [ [[TMP8:%.*]], [[VPLANNEDBB]] ], [ zeroinitializer, [[VECTOR_BODY]] ]
; CHECK-NEXT:    [[VEC_PHI4:%.*]] = phi <2 x i32> [ [[PREDPHI:%.*]], [[VPLANNEDBB]] ], [ zeroinitializer, [[VECTOR_BODY]] ]
; CHECK-NEXT:    [[VEC_PHI5:%.*]] = phi <2 x i1> [ [[TMP3:%.*]], [[VECTOR_BODY]] ], [ [[TMP16:%.*]], [[VPLANNEDBB]] ]
; CHECK-NEXT:    [[TMP6:%.*]] = and <2 x i1> [[TMP5:%.*]], [[VEC_PHI5]]
; CHECK-NEXT:    [[MM_VECTORGEP:%.*]] = getelementptr inbounds i64, <2 x i64*> [[BROADCAST_SPLAT7:%.*]], <2 x i64> [[VEC_PHI3]]
...
; CHECK-NEXT:    [[TMP8]] = add nuw nsw <2 x i64> [[VEC_PHI3]], <i64 1, i64 1>
...
; CHECK-NEXT:    [[PREDPHI]] = select <2 x i1> [[TMP12]], <2 x i32> <i32 1, i32 1>, <2 x i32> [[VEC_PHI4]]
; CHECK-NEXT:    [[PREDPHI8:%.*]] = select <2 x i1> [[TMP12]], <2 x i1> [[TMP7]], <2 x i1> zeroinitializer
; CHECK-NEXT:    [[PREDPHI9:%.*]] = select <2 x i1> [[TMP12]], <2 x i64> [[TMP8]], <2 x i64> <i64 100, i64 100>
; CHECK-NEXT:    [[PREDPHI10:%.*]] = select <2 x i1> [[TMP12]], <2 x i64> [[VEC_PHI3]], <2 x i64> <i64 100, i64 100>
; CHECK-NEXT:    [[PREDPHI11:%.*]] = select <2 x i1> [[TMP12]], <2 x i1> <i1 true, i1 true>, <2 x i1> [[TMP11]]
; CHECK-NEXT:    [[TMP13:%.*]] = xor <2 x i1> [[VEC_PHI5]], <i1 true, i1 true>
; CHECK-NEXT:    [[TMP14:%.*]] = and <2 x i1> [[TMP5]], [[TMP13]]
; CHECK-NEXT:    [[TMP15:%.*]] = xor <2 x i1> [[PREDPHI11]], <i1 true, i1 true>
; CHECK-NEXT:    [[TMP16]] = and <2 x i1> [[TMP15]], [[VEC_PHI5]]
; CHECK-NEXT:    [[TMP17]] = select <2 x i1> [[VEC_PHI5]], <2 x i1> [[PREDPHI8]], <2 x i1> [[VEC_PHI2]]
; CHECK-NEXT:    [[TMP18]] = select <2 x i1> [[VEC_PHI5]], <2 x i64> [[PREDPHI9]], <2 x i64> [[VEC_PHI1]]
; CHECK-NEXT:    [[TMP19]] = select <2 x i1> [[VEC_PHI5]], <2 x i64> [[PREDPHI10]], <2 x i64> [[VEC_PHI]]
; CHECK-NEXT:    [[TMP20:%.*]] = and <2 x i1> [[TMP16]], [[TMP5]]
; CHECK-NEXT:    [[TMP21:%.*]] = bitcast <2 x i1> [[TMP20]] to i2
; CHECK-NEXT:    [[TMP22:%.*]] = icmp eq i2 [[TMP21]], 0
; CHECK-NEXT:    [[BROADCAST_SPLATINSERT12:%.*]] = insertelement <2 x i1> undef, i1 [[TMP22]], i32 0
; CHECK-NEXT:    [[BROADCAST_SPLAT13:%.*]] = shufflevector <2 x i1> [[BROADCAST_SPLATINSERT12]], <2 x i1> undef, <2 x i32> zeroinitializer
; CHECK-NEXT:    [[TMP23:%.*]] = extractelement <2 x i1> [[BROADCAST_SPLAT13]], i32 0
; CHECK-NEXT:    br i1 [[TMP23]], label [[VPLANNEDBB14:%.*]], label [[VPLANNEDBB]]
; CHECK:       VPlannedBB14:
; CHECK-NEXT:    [[TMP24:%.*]] = add <2 x i64> [[TMP19]], <i64 1, i64 1>
; CHECK-NEXT:    [[TMP25:%.*]] = add <2 x i64> [[TMP18]], <i64 1, i64 1>
; CHECK-NEXT:    [[TMP26:%.*]] = xor <2 x i1> [[TMP17]], <i1 true, i1 true>
; CHECK-NEXT:    [[TMP27:%.*]] = add <2 x i64> [[TMP24]], [[TMP25]]
; CHECK-NEXT:    [[TMP28:%.*]] = and <2 x i1> [[TMP26]], <i1 true, i1 true>
