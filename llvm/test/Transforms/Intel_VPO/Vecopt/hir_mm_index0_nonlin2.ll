...
; REQUIRES: asserts
; RUN: opt -vplan-enable-new-cfg-merge-hir=false -disable-output -hir-ssa-deconstruction -hir-temp-cleanup -hir-vec-dir-insert -hir-vplan-vec -vplan-entities-dump -enable-mmindex=1  -disable-nonlinear-mmindex=0 -vplan-print-after-vpentity-instrs -vplan-print-after-plain-cfg -print-after=hir-vplan-vec -vplan-force-vf=4 -S < %s 2>&1 | FileCheck %s
; RUN: opt -vplan-enable-new-cfg-merge-hir -disable-output -hir-ssa-deconstruction -hir-temp-cleanup -hir-vec-dir-insert -hir-vplan-vec -vplan-entities-dump -enable-mmindex=1  -disable-nonlinear-mmindex=0 -vplan-print-after-vpentity-instrs -vplan-print-after-plain-cfg -print-after=hir-vplan-vec -vplan-force-vf=4 -S < %s 2>&1 | FileCheck %s
