...
; In this test we have different conditions in selects for main value and index.
; REQUIRES: asserts
...
;CHECK-NEXT: [MinMax+Index] Skipped: dependency on not the same comparison
;
;<0>          BEGIN REGION { }
;<16>               + DO i1 = 0, sext.i32.i64(%m) + -1, 1   <DO_LOOP>
;<4>                |   %0 = (%ordering)[i1];
;<8>                |   %tmp.027 = (%0 >= %best.026) ? i1 : %tmp.027;
;<9>                |   %best.026 = (%0 > %best.026) ? %0 : %best.026;
;<16>               + END LOOP
;<0>          END REGION
;
...
for.body:                                         ; preds = %for.body, %for.body.preheader
  ...
  %cmp2 = icmp sge i32 %0, %best.026
  %1 = trunc i64 %indvars.iv to i32
  %2 = select i1 %cmp2, i32 %1,i32 %tmp.027
