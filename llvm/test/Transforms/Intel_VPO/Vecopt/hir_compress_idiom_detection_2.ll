...
; CHECK:      [Compress/Expand Idiom] Increment {sb:3}+2 detected: {{.*}} %j.014 = 2 + %j.014;
; CHECK-NOT:  [Compress/Expand Idiom] Increment rejected
; CHECK:      Idiom List
; CHECK-NEXT: CEIndexIncFirst: {{.*}} %j.014 = 2  +  %j.014;
; CHECK-NEXT:   CEStore: {{.*}} (%B)[%j.014] = (%A)[i1];
; CHECK-NEXT:     CELdStIndex: %j.014
