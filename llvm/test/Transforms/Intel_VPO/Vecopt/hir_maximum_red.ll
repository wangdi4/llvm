...
; <3>          |   %0 = (@a)[0][i1];
; <4>          |   %max.011 = @llvm.maximum.f32(%0,  %max.011); <Safe Reduction>
; <11>         + END LOOP
...
; CHECK-NEXT:               |   %llvm.maximum.v8f32 = @llvm.maximum.v8f32(%.vec,  %phi.temp);
; CHECK-NEXT:               |   %phi.temp = %llvm.maximum.v8f32;
; CHECK-NEXT:               + END LOOP
; CHECK:               %max.011 = @llvm.vector.reduce.fmaximum.v8f32(%llvm.maximum.v8f32);
; CHECK:          END REGION
...
for.body:                                         ; preds = %entry, %for.body
  ...
  %1 = tail call fast float @llvm.maximum.f32(float %0, float %max.011)
