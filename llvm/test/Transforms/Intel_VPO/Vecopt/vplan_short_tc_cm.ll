...
;
; RUN: opt -passes='vplan-vec,print' -disable-output -vplan-enable-masked-main-loop=1 < %s 2>&1 | FileCheck %s
; RUN: opt -passes='vpo-directive-cleanup,hir-ssa-deconstruction,hir-vec-dir-insert,hir-vplan-vec,print<hir>' -disable-output -vplan-enable-masked-main-loop=1 < %s 2>&1 | FileCheck %s
...
define dso_local i32 @_Z3fooPiii(i32* nocapture readnone %ptr, i32 %step, i32 %n) local_unnamed_addr #0 {
entry:
...
DIR.OMP.SIMD.128:                                 ; preds = %DIR.OMP.SIMD.1
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.SIMD"(), "QUAL.OMP.REDUCTION.ADD"(i32* %s.red), "QUAL.OMP.NORMALIZED.IV"(i8* null), "QUAL.OMP.NORMALIZED.UB"(i8* null), "QUAL.OMP.LINEAR:IV"(i32* %i.linear.iv, i32 1) ]
...
omp.inner.for.body:                               ; preds = %DIR.OMP.SIMD.2, %omp.inner.for.body
  ...
  %p = getelementptr inbounds i32, i32* %ptr, i32 %.omp.iv.local.019
  %l2 = load i32, i32* %p, align 4
...
DIR.OMP.END.SIMD.2:                               ; preds = %omp.inner.for.body
  ...
  store i32 %add7.lcssa, i32* %s.red, align 4
