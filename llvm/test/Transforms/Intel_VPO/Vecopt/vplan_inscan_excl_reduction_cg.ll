...
; CHECK-NEXT:    [[UNI_PHI0:%.*]] = phi i64 [ 0, [[VPLANNEDBB10]] ], [ [[TMP22:%.*]], [[NEW_LATCH0:%.*]] ]
; CHECK-NEXT:    [[VEC_PHI0:%.*]] = phi <4 x i64> [ <i64 0, i64 1, i64 2, i64 3>, [[VPLANNEDBB10]] ], [ [[TMP21:%.*]], [[NEW_LATCH0]] ]
...
; CHECK-NEXT:    br label [[VPLANNEDBB800:%.*]]
; CHECK-EMPTY:
...
; CHECK-NEXT:    [[TMP15:%.*]] = fadd fast <4 x float> [[WIDE_LOAD140]], [[TMP14]]
...
; CHECK-NEXT:    [[TMP18:%.*]] = shufflevector <4 x float> [[TMP17]], <4 x float> zeroinitializer, <4 x i32> <i32 4, i32 0, i32 1, i32 2>
; CHECK-NEXT:    [[TMP19:%.*]] = fadd fast <4 x float> [[TMP18]], [[BROADCAST_SPLAT160]]
; CHECK-NEXT:    [[DOTEXTRACT_3_0:%.*]] = extractelement <4 x float> [[TMP19]], i32 3
; CHECK-NEXT:    store <4 x float> [[TMP19]], ptr [[X_RED_VEC0]], align 1
...
; CHECK-NEXT:    br label [[VPLANNEDBB210:%.*]]
; CHECK-EMPTY:
; CHECK-NEXT:  VPlannedBB21:
; CHECK-NEXT:    br label [[NEW_LATCH0]]
; CHECK-EMPTY:
; CHECK-NEXT:  new_latch:
; CHECK-NEXT:    [[TMP21]] = add nuw nsw <4 x i64> [[VEC_PHI0]], <i64 4, i64 4, i64 4, i64 4>
; CHECK-NEXT:    [[TMP22]] = add nuw nsw i64 [[UNI_PHI0]], 4
; CHECK-NEXT:    [[TMP23:%.*]] = icmp uge i64 [[TMP22]], 1024
; CHECK-NEXT:    br i1 [[TMP23]], label [[VPLANNEDBB220:%.*]], label [[VECTOR_BODY0]], !llvm.loop !0
; CHECK-EMPTY:
; CHECK-NEXT:  VPlannedBB22:
; CHECK-NEXT:    store float [[TMP20]], ptr [[X_RED0]], align 1
;
DIR.OMP.SIMD.1:
