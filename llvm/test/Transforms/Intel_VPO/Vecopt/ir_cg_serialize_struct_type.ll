...
; RUN: opt -passes=vplan-vec -vplan-print-scalvec-results -vplan-force-vf=2 -S < %s | FileCheck %s
; RUN: opt -passes=vplan-vec,intel-ir-optreport-emitter -vplan-print-scalvec-results -vplan-force-vf=2 -intel-opt-report=high 2>&1 -disable-output -S < %s | FileCheck %s -check-prefixes=OPTREPORT
...
; CHECK-NEXT:     [DA: Div, SVA: ( V )] [[COMPLEX_64BIT0:%.*]] = type { float, float } [[VP_PR_FETCH:%.*]] = load ptr [[VP_PR]] (SVAOpBits 0->V )
...
; CHECK-NEXT:     [DA: Div, SVA: ( V )] store [[COMPLEX_64BIT0]] = type { float, float } [[VP_PR_FETCH]] ptr [[VP_PS]] (SVAOpBits 0->V 1->V )
