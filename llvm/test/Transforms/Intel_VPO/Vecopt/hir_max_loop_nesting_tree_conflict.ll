;
; RUN: opt -S -mattr=+avx512vl,+avx512cd -hir-ssa-deconstruction -hir-temp-cleanup -hir-vec-dir-insert -enable-vconflict-idiom -hir-vplan-vec -vplan-force-vf=4 -disable-output -debug-only=vplan-vec -print-after=hir-vplan-vec < %s 2>&1 | FileCheck %s
;
