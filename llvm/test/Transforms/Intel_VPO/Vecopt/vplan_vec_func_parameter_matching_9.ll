...
; RUN: opt %s -S -VPlanDriver -mattr=avx2 -debug-only=VectorVariant -debug-only=VPlanCallVecDecisions 2>&1 | FileCheck %s
; RUN: opt %s -S -passes="vplan-driver" -mattr=avx2 -debug-only=VectorVariant -debug-only=VPlanCallVecDecisions 2>&1 | FileCheck %s
; REQUIRES: asserts
