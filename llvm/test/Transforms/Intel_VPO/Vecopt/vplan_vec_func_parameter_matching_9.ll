...
; RUN: opt %s -S -VPlanDriver -mattr=avx2 -debug-only=VPlanCallVecDecisions 2>&1 | FileCheck %s
; RUN: opt %s -S -passes="vplan-driver" -mattr=avx2 -debug-only=VPlanCallVecDecisions 2>&1 | FileCheck %s
; REQUIRES: asserts
...
; CHECK-LABEL: Attempting to match variant parameters for: _ZGVdN2uvu_
; CHECK: MaxArg: 0
; CHECK-NEXT: Matched function: _ZGVdN2uvu_
; CHECK-NEXT: Score: 9
; CHECK-EMPTY:
