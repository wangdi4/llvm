...
; RUN: opt -S -vplan-vec -intel-ir-optreport-emitter -intel-loop-optreport=high -disable-output 2>&1 < %s | FileCheck %s
; RUN: opt -S -vplan-vec -intel-ir-optreport-emitter -intel-loop-optreport=high -disable-output -enable-intel-advanced-opts -mcpu=skylake-avx512  2>&1 < %s | FileCheck %s
