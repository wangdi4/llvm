...
; RUN:  opt < %s -hir-ssa-deconstruction -hir-framework -hir-vplan-vec -disable-output -print-after=hir-vplan-vec 2>&1 | FileCheck %s -check-prefix=CHECK-HIR

; RUN:  opt < %s -vplan-vec -disable-output -print-after=vplan-vec -vplan-force-uf=2 2>&1 | FileCheck %s -check-prefix=CHECK-LLVM-IR
...
; CHECK-HIR-LABEL:  *** IR Dump After VPlan HIR Vectorizer (hir-vplan-vec) ***
...
;
; CHECK-LLVM-IR-LABEL:  *** IR Dump After VPlan Vectorizer (vplan-vec) ***
;
