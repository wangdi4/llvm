...
; RUN:  opt -enable-new-pm=0 -hir-ssa-deconstruction -hir-framework -hir-vplan-vec -print-after=hir-vplan-vec -disable-output < %s 2>&1 -vplan-enable-new-cfg-merge-hir=0 | FileCheck %s -check-prefix=CHECK-HIR
; RUN:  opt -enable-new-pm=0 -hir-ssa-deconstruction -hir-framework -hir-vplan-vec -print-after=hir-vplan-vec -disable-output < %s 2>&1 -vplan-enable-new-cfg-merge-hir=1 | FileCheck %s -check-prefix=CHECK-HIR
...
; RUN:  opt -passes='hir-ssa-deconstruction,hir-vplan-vec,print<hir>' -disable-output < %s 2>&1 -vplan-enable-new-cfg-merge-hir=0 | FileCheck %s -check-prefix=CHECK-HIR
; RUN:  opt -passes='hir-ssa-deconstruction,hir-vplan-vec,print<hir>' -disable-output < %s 2>&1 -vplan-enable-new-cfg-merge-hir=1 | FileCheck %s -check-prefix=CHECK-HIR
