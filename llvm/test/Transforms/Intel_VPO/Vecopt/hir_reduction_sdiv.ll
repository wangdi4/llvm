; RUN: opt -passes="hir-ssa-deconstruction,hir-temp-cleanup,hir-vec-dir-insert,hir-vplan-vec" -vplan-force-vf=4 -vplan-vec-scenario="n0;v4;s1" -disable-output -print-after=hir-vplan-vec %s 2>&1 | FileCheck %s

; This test case checks that the loop was not vectorized since the safe reduction
; is a division with integers. It was created from the following source code:
...
; Check that the loop wasn't vectorized
