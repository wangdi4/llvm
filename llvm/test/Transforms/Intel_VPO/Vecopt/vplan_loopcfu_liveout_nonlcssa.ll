...
; CHECK-NEXT:     i1 [[VP_SKIP_LOOP:%.*]] = icmp i64 [[VP_OUTER_IV]] i64 [[MASK_OUT_INNER_LOOP0:%.*]]
...
; CHECK-NEXT:     <Empty Block>
; CHECK-NEXT:     Condition([[BB2]]): i1 [[VP_SKIP_LOOP]] = icmp i64 [[VP_OUTER_IV]] i64 [[MASK_OUT_INNER_LOOP0]]
; CHECK-NEXT:    SUCCESSORS(2):[[BB5:BB[0-9]+]](i1 [[VP_SKIP_LOOP]]), [[BB6:BB[0-9]+]](!i1 [[VP_SKIP_LOOP]])
; CHECK-NEXT:    no PREDECESSORS
; CHECK-EMPTY:
; CHECK-NEXT:      [[BB6]] (BP: NULL) :
; CHECK-NEXT:       i1 [[VP_CMP216:%.*]] = icmp i64 [[VP_OUTER_IV]] i64 0
; CHECK-NEXT:       i1 [[VP_CMP216_NOT:%.*]] = not i1 [[VP_CMP216]]
...
; CHECK-NEXT:       i64 [[VP_PHI_USE_LIVE_OUT_PREV:%.*]] = phi  [ i64 [[VP_PHI_USE_LIVE_OUT_BLEND:%.*]], [[BB9:BB[0-9]+]] ],  [ i64 undef, [[BB7]] ]
; CHECK-NEXT:       i64 [[VP_PHI_UPDATE_USE_LIVE_OUT_PREV:%.*]] = phi  [ i64 [[VP_PHI_UPDATE_USE_LIVE_OUT_BLEND:%.*]], [[BB9]] ],  [ i64 undef, [[BB7]] ]
; CHECK-NEXT:       i1 [[VP_NO_PHI_INST_USE_LIVE_OUT_PREV:%.*]] = phi  [ i1 [[VP_NO_PHI_INST_USE_LIVE_OUT_BLEND:%.*]], [[BB9]] ],  [ i1 undef, [[BB7]] ]
; CHECK-NEXT:       i64 [[VP_INNER_IV:%.*]] = phi  [ i64 [[VP_INNER_IV_NEXT:%.*]], [[BB9]] ],  [ i64 0, [[BB7]] ]
; CHECK-NEXT:       i32 [[VP0:%.*]] = phi  [ i32 [[VP1:%.*]], [[BB9]] ],  [ i32 0, [[BB7]] ]
; CHECK-NEXT:       i1 [[VP_LOOP_MASK:%.*]] = phi  [ i1 [[VP_CMP216_NOT]], [[BB7]] ],  [ i1 [[VP_LOOP_MASK_NEXT:%.*]], [[BB9]] ]
...
; CHECK-NEXT:       <Empty Block>
; CHECK-NEXT:       Condition([[BB8]]): i1 [[VP_LOOP_MASK]] = phi  [ i1 [[VP_CMP216_NOT]], [[BB7]] ],  [ i1 [[VP_LOOP_MASK_NEXT]], [[BB9]] ]
...
; CHECK-NEXT:         i1 [[VP_SOME_CMP:%.*]] = icmp i64 [[VP_LD]] i64 42
; CHECK-NEXT:         i64 [[VP_INNER_IV_NEXT]] = add i64 [[VP_INNER_IV]] i64 1
; CHECK-NEXT:        SUCCESSORS(1):[[REGION3:region[0-9]+]]
; CHECK-NEXT:        PREDECESSORS(1): [[BB10]]
; CHECK-EMPTY:
; CHECK-NEXT:        REGION: [[REGION3]] (BP: NULL)
; CHECK-NEXT:        [[BB13:BB[0-9]+]] (BP: NULL) :
; CHECK-NEXT:         <Empty Block>
; CHECK-NEXT:         Condition([[BB11]]): i1 [[VP_SOME_CMP]] = icmp i64 [[VP_LD]] i64 42
; CHECK-NEXT:        SUCCESSORS(2):Intermediate[[BB14:BB[0-9]+]](i1 [[VP_SOME_CMP]]), [[BB15:BB[0-9]+]](!i1 [[VP_SOME_CMP]])
; CHECK-NEXT:        no PREDECESSORS
; CHECK-EMPTY:
; CHECK-NEXT:          [[BB15]] (BP: NULL) :
; CHECK-NEXT:           i1 [[VP_EXITCOND:%.*]] = icmp i64 [[VP_INNER_IV_NEXT]] i64 [[VP_OUTER_IV]]
; CHECK-NEXT:          SUCCESSORS(1):NewLoopLatch13
; CHECK-NEXT:          PREDECESSORS(1): [[BB13]]
; CHECK-EMPTY:
; CHECK-NEXT:          Intermediate[[BB14]] (BP: NULL) :
; CHECK-NEXT:           <Empty Block>
; CHECK-NEXT:          SUCCESSORS(1):NewLoopLatch13
; CHECK-NEXT:          PREDECESSORS(1): [[BB13]]
; CHECK-EMPTY:
; CHECK-NEXT:        NewLoopLatch13 (BP: NULL) :
; CHECK-NEXT:         i32 [[VP1]] = phi  [ i32 [[VP0]], [[BB15]] ],  [ i32 1, Intermediate[[BB14]] ]
; CHECK-NEXT:         i1 [[VP_NO_PHI_INST_USE:%.*]] = phi  [ i1 [[VP_SOME_CMP]], Intermediate[[BB14]] ],  [ i1 false, [[BB15]] ]
; CHECK-NEXT:         i64 [[VP_PHI_UPDATE_USE:%.*]] = phi  [ i64 [[VP_INNER_IV_NEXT]], Intermediate[[BB14]] ],  [ i64 100, [[BB15]] ]
; CHECK-NEXT:         i64 [[VP_PHI_USE:%.*]] = phi  [ i64 [[VP_INNER_IV]], Intermediate[[BB14]] ],  [ i64 100, [[BB15]] ]
; CHECK-NEXT:         i1 [[VP2:%.*]] = phi  [ i1 [[VP_EXITCOND]], [[BB15]] ],  [ i1 true, Intermediate[[BB14]] ]
; CHECK-NEXT:        no SUCCESSORS
; CHECK-NEXT:        PREDECESSORS(2): [[BB15]] Intermediate[[BB14]]
; CHECK-EMPTY:
; CHECK-NEXT:        SUCCESSORS(1):[[BB12]]
; CHECK-NEXT:        END Region([[REGION3]])
; CHECK-EMPTY:
; CHECK-NEXT:      [[BB12]] (BP: NULL) :
; CHECK-NEXT:       i1 [[VP__NOT:%.*]] = not i1 [[VP2]]
; CHECK-NEXT:       i1 [[VP_LOOP_MASK_NEXT]] = and i1 [[VP__NOT]] i1 [[VP_LOOP_MASK]]
; CHECK-NEXT:       i1 [[VP_NO_PHI_INST_USE_LIVE_OUT_BLEND]] = select i1 [[VP_LOOP_MASK]] i1 [[VP_NO_PHI_INST_USE]] i1 [[VP_NO_PHI_INST_USE_LIVE_OUT_PREV]]
; CHECK-NEXT:       i64 [[VP_PHI_UPDATE_USE_LIVE_OUT_BLEND]] = select i1 [[VP_LOOP_MASK]] i64 [[VP_PHI_UPDATE_USE]] i64 [[VP_PHI_UPDATE_USE_LIVE_OUT_PREV]]
; CHECK-NEXT:       i64 [[VP_PHI_USE_LIVE_OUT_BLEND]] = select i1 [[VP_LOOP_MASK]] i64 [[VP_PHI_USE]] i64 [[VP_PHI_USE_LIVE_OUT_PREV]]
; CHECK-NEXT:       i1 [[VP3:%.*]] = all-zero-check i1 [[VP_LOOP_MASK_NEXT]]
; CHECK-NEXT:      no SUCCESSORS
...
; CHECK-NEXT:       <Empty Block>
; CHECK-NEXT:       Condition([[BB12]]): i1 [[VP3]] = all-zero-check i1 [[VP_LOOP_MASK_NEXT]]
; CHECK-NEXT:      SUCCESSORS(2):[[BB16:BB[0-9]+]](i1 [[VP3]]), [[BB8]](!i1 [[VP3]])
; CHECK-NEXT:      PREDECESSORS(1): mask_[[REGION2]]
; CHECK-EMPTY:
; CHECK-NEXT:      [[BB16]] (BP: NULL) :
; CHECK-NEXT:       i64 [[VP_PHI_USE_LIVE_OUT_LCSSA:%.*]] = phi  [ i64 [[VP_PHI_USE_LIVE_OUT_BLEND]], [[BB9]] ]
; CHECK-NEXT:       i64 [[VP_PHI_UPDATE_USE_LIVE_OUT_LCSSA:%.*]] = phi  [ i64 [[VP_PHI_UPDATE_USE_LIVE_OUT_BLEND]], [[BB9]] ]
; CHECK-NEXT:       i1 [[VP_NO_PHI_INST_USE_LIVE_OUT_LCSSA:%.*]] = phi  [ i1 [[VP_NO_PHI_INST_USE_LIVE_OUT_BLEND]], [[BB9]] ]
; CHECK-NEXT:       i64 [[VP_USE_A:%.*]] = add i64 [[VP_PHI_USE_LIVE_OUT_LCSSA]] i64 1
; CHECK-NEXT:       i64 [[VP_USE_B:%.*]] = add i64 [[VP_PHI_UPDATE_USE_LIVE_OUT_LCSSA]] i64 1
; CHECK-NEXT:       i1 [[VP_USE_C:%.*]] = xor i1 [[VP_NO_PHI_INST_USE_LIVE_OUT_LCSSA]] i1 true
; CHECK-NEXT:      no SUCCESSORS
...
; CHECK-NEXT:     <Empty Block>
; CHECK-NEXT:     Condition([[BB5]]): i1 [[VP_OUTER_EXIT_COND]] = icmp i64 [[VP_OUTER_IV_NEXT]] i64 [[N0]]
; CHECK-NEXT:    SUCCESSORS(2):[[BB17:BB[0-9]+]](i1 [[VP_OUTER_EXIT_COND]]), [[BB2]](!i1 [[VP_OUTER_EXIT_COND]])
