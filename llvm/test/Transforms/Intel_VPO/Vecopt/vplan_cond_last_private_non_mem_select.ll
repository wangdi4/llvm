...
; RUN: opt %s -disable-output -passes=vplan-vec -vplan-print-after-live-inout-list -vplan-dump-live-inout -vplan-entities-dump | FileCheck %s

define void @main(i64 %call.i, ptr %call1) {
...
; CHECK-NEXT:    Linked values: i16 [[VP0]], ptr [[RET_I_LPRIV0:%.*]], i16 [[VP__LCSSA109]], ptr [[VP_RET_I_LPRIV:%.*]], i16 [[VP__PRIV_FINAL:%.*]],
; CHECK-NEXT:   Memory: ptr [[RET_I_LPRIV0]]
; CHECK-EMPTY:
; CHECK-NEXT:    [[BB2:BB[0-9]+]]: # preds:
; CHECK-NEXT:     br [[BB3:BB[0-9]+]]
; CHECK-EMPTY:
; CHECK-NEXT:    [[BB3]]: # preds: [[BB2]]
; CHECK-NEXT:     ptr [[VP_RET_I_LPRIV]] = allocate-priv i16, OrigAlign = 2
; CHECK-NEXT:     call i64 2 ptr [[VP_RET_I_LPRIV]] ptr @llvm.lifetime.start.p0
...
; CHECK-NEXT:       ptr [[VP_ARRAYIDX_I:%.*]] = getelementptr inbounds i16, ptr [[TMP0:%.*]] i64 [[VP_INDVARS_IV114]]
; CHECK-NEXT:       i16 [[VP2:%.*]] = load ptr [[VP_ARRAYIDX_I]]
...
; CHECK-NEXT:     call i64 2 ptr [[VP_RET_I_LPRIV]] ptr @llvm.lifetime.end.p0
...
DIR.OMP.SIMD.1:                                   ; preds = %for.cond.cleanup
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.SIMD"(), "QUAL.OMP.LASTPRIVATE:CONDITIONAL.TYPED"(ptr %ret.i.lpriv, i16 0, i32 1) ]
  br label %DIR.OMP.SIMD.2

DIR.OMP.SIMD.2:                                   ; preds = %DIR.OMP.SIMD.1
  %div6.i130 = lshr i32 %cond, 1
  %cmp7.i105 = icmp sgt i32 %cond, 1
  %ret.i.lpriv.promoted108 = load i16, ptr %ret.i.lpriv
...
omp.inner.for.body.i:                             ; preds = %for.cond.cleanup.i, %DIR.OMP.SIMD.2
  ...
  %.lcssa109 = phi i16 [ %ret.i.lpriv.promoted108, %DIR.OMP.SIMD.2 ], [ %3, %for.cond.cleanup.i ]
  %1 = trunc i64 %indvars.iv114 to i32
  br i1 %cmp7.i105, label %for.body.i.lr.ph, label %for.cond.cleanup.i

for.body.i.lr.ph:                                 ; preds = %omp.inner.for.body.i
  %arrayidx.i = getelementptr inbounds i16, ptr %call1, i64 %indvars.iv114
  %2 = load i16, ptr %arrayidx.i
  %conv.i65 = sext i16 %2 to i32
  br label %for.body.i
...
for.cond.cleanup.i:                               ; preds = %for.cond.cleanup.i.loopexit, %omp.inner.for.body.i
  %3 = phi i16 [ %.lcssa109, %omp.inner.for.body.i ], [ %spec.select.lcssa, %for.cond.cleanup.i.loopexit ]
...
for.body.i:                                       ; preds = %for.body.i, %for.body.i.lr.ph
  ...
  %4 = phi i16 [ %.lcssa109, %for.body.i.lr.ph ], [ %spec.select, %for.body.i ]
  %add8.i = add nuw nsw i32 %storemerge.i106, %1
  %cmp9.i = icmp sgt i32 %add8.i, %conv.i65
  %spec.select = select i1 %cmp9.i, i16 %2, i16 %4
...
_ZN18ConditionalPrivateIs10vec_policyEclEiPKs.exit: ; preds = %for.cond.cleanup.i
  %.lcssa = phi i16 [ %3, %for.cond.cleanup.i ]
  store i16 %.lcssa, ptr %ret.i.lpriv
