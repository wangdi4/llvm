...
; RUN: opt -opaque-pointers=0 -S < %s -passes=vplan-vec -disable-output -vplan-enable-scalvec-analysis -vplan-print-scalvec-results | FileCheck %s --check-prefix=SVA-IR
; RUN: opt -opaque-pointers=0 -S < %s -passes='hir-ssa-deconstruction,hir-vec-dir-insert,hir-vplan-vec' -disable-output -vplan-enable-scalvec-analysis -vplan-print-scalvec-results | FileCheck %s --check-prefix=SVA-HIR
