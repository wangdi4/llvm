...
; RUN: opt -S < %s -VPlanDriver -disable-output -vplan-enable-scalvec-analysis -vplan-print-scalvec-results -enable-explicit-vplan-vls-hir | FileCheck %s --check-prefix=SVA-IR
; RUN: opt -S < %s -hir-ssa-deconstruction -hir-vec-dir-insert -VPlanDriverHIR -disable-output -vplan-enable-scalvec-analysis -vplan-print-scalvec-results -enable-explicit-vplan-vls-hir | FileCheck %s --check-prefix=SVA-HIR
...
; SVA-IR-NEXT:     [DA: Uni, SVA: (F  )] i1 [[VP_VEC_TC_CHECK:%.*]] = icmp eq i64 0 i64 [[VP_VECTOR_TRIP_COUNT]] (SVAOpBits 0->F 1->F )
...
; SVA-IR-NEXT:       [DA: Uni, SVA: (F  )] br middle.block (SVAOpBits 0->F )
; SVA-IR-EMPTY:
; SVA-IR-NEXT:      middle.block: # preds: [[BB3]]
; SVA-IR-NEXT:       [DA: Uni, SVA: (F  )] i1 [[VP_REMTC_CHECK:%.*]] = icmp ne i64 [[VP_ORIG_TRIP_COUNT]] i64 [[VP_VECTOR_TRIP_COUNT]] (SVAOpBits 0->F 1->F )
