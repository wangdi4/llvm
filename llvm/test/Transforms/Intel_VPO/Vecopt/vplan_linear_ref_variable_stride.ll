...
DIR.OMP.SIMD.2:
  ...
  %call = tail call i32 @atoi(i8* nocapture noundef %2) #6
...
DIR.OMP.SIMD.1:                                   ; preds = %DIR.OMP.SIMD.2
  %3 = call token @llvm.directive.region.entry() [ "DIR.OMP.SIMD"(), "QUAL.OMP.SIMDLEN"(i32 8), "QUAL.OMP.LINEAR:IV"(i64* %i.linear.iv, i32 %call) ]
  br label %DIR.OMP.SIMD.137

DIR.OMP.SIMD.137:                                 ; preds = %DIR.OMP.SIMD.1
  %4 = bitcast i64* %i.linear.iv to i8*
...
omp.inner.for.body:                               ; preds = %DIR.OMP.SIMD.137, %omp.inner.for.body
  ...
  %arrayidx6 = getelementptr inbounds [128 x i64], [128 x i64]* %a, i64 0, i64 %mul
  %call7 = call noundef i64 @_Z3fooiRl(i32 noundef %call, i64* noundef nonnull align 8 dereferenceable(8) %arrayidx6) #3
  %arrayidx8 = getelementptr inbounds [128 x i64], [128 x i64]* %b, i64 0, i64 %mul
