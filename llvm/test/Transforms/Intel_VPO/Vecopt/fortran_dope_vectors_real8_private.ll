...
; LLVMIR-NEXT:   %7 = udiv <2 x i64> %5, <i64 8, i64 8>
...
; LLVMIR-NEXT:   %.array.buffer.lane.0 = alloca double, i64 %.extract.0.6, align 8
; LLVMIR-NEXT:   %.array.buffer.insert.0 = insertelement <2 x ptr> undef, ptr %.array.buffer.lane.0, i64 0
; LLVMIR-NEXT:   %.array.buffer.lane.1 = alloca double, i64 %.extract.0.6, align 8
...
omp.pdo.body6:                                    ; preds = %DIR.OMP.SIMD.119, %omp.pdo.body6
  ...
  %"sum_$C1.addr_a0$_fetch.10[]" = call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 0, i64 1, i64 %"sum_$C1.dim_info$.spacing$[]_fetch.11", ptr elementtype(double) %"sum_$C1.addr_a0$_fetch.10", i64 %indvars.iv.next)
  store double %add.3, ptr %"sum_$C1.addr_a0$_fetch.10[]", align 1
...
DIR.OMP.SIMD.1:                                   ; preds = %allocated.then, %DIR.OMP.SIMD.2
  %rel.1.not13 = icmp slt i32 %"(i32)sum_$N_fetch.1$", 1
...
DIR.OMP.SIMD.119:                                 ; preds = %DIR.OMP.SIMD.118
  %"sum_$M_fetch.7" = load double, ptr %"sum_$M", align 1
  %"sum_$N_fetch.8" = load double, ptr %"sum_$N", align 1
...
DIR.OMP.END.SIMD.2:                               ; preds = %omp.pdo.body6
  %add.4.le = add nuw i32 %"(i32)sum_$N_fetch.1$", 1
...
DIR.OMP.END.SIMD.412:                             ; preds = %DIR.OMP.END.SIMD.220, %DIR.OMP.SIMD.1
  ret i32 1
}
