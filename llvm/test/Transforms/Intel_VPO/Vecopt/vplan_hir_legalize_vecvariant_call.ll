...
; RUN: opt -mtriple=x86_64 -mcpu=skylake-avx512 -passes="hir-ssa-deconstruction,hir-vec-dir-insert,hir-vplan-vec,print<hir>" -disable-output -vec-clone-legalize-enabled < %s 2>&1 | FileCheck %s

; Test case to check that call to vector variant is properly legalized in HIR path.
...
; CHECK-LABEL:  Function: _ZGVZN8vu_CallerFunc
; CHECK-EMPTY:
; CHECK-NEXT:  BEGIN REGION { modified }
; CHECK-NEXT:        [[LOAD_VALS0:%.*]] = ([[ALLOCA_VALS0:%.*]])[0]
; CHECK-NEXT:        [[DOTVEC0:%.*]] = (<8 x double>*)([[VEC_X0:%.*]])[0]
; CHECK-NEXT:        [[DOTEXTRACTED_SUBVEC0:%.*]] = shufflevector [[DOTVEC0]],  undef,  <i32 0, i32 1, i32 2, i32 3>
; CHECK-NEXT:        [[DOTEXTRACTED_SUBVEC20:%.*]] = shufflevector [[DOTVEC0]],  undef,  <i32 4, i32 5, i32 6, i32 7>
; CHECK-NEXT:        [[_ZGVYN8V_ORIGIN0:%.*]] = @_ZGVYN8v_Origin([[DOTEXTRACTED_SUBVEC0]],  [[DOTEXTRACTED_SUBVEC20]])
; CHECK-NEXT:        [[EXTRACT_RESULT0:%.*]] = extractvalue [[_ZGVYN8V_ORIGIN0]], 0
; CHECK-NEXT:        [[EXTRACT_RESULT30:%.*]] = extractvalue [[_ZGVYN8V_ORIGIN0]], 1
; CHECK-NEXT:        [[COMB_SHUF0:%.*]] = shufflevector [[EXTRACT_RESULT0]],  [[EXTRACT_RESULT30]],  <i32 0, i32 1, i32 2, i32 3, i32 4, i32 5, i32 6, i32 7>
; CHECK-NEXT:        (<8 x double>*)([[VEC_RETVAL0:%.*]])[0] = [[COMB_SHUF0]]
