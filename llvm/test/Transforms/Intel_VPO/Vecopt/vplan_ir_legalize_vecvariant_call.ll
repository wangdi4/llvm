...
; RUN: opt -passes=vplan-vec -mtriple=x86_64 -mcpu=skylake-avx512 -S < %s  | FileCheck %s
...
; CHECK:       vector.body:
;   FIXME -- <8 x double> should be passed/returned as two data chunks for AVX2.
...
; CHECK-NEXT:    [[TMP0:%.*]] = call fast x86_regcallcc nofpclass(nan inf) <8 x double> @_ZGVYN8v_Origin(<8 x double> noundef nofpclass(nan inf) [[WIDE_LOAD]]) #[[ATTR4:[0-9]+]]
; CHECK-NEXT:    [[SCALAR_GEP3:%.*]] = getelementptr double, ptr [[VEC_RETVAL:%.*]], i32 [[UNI_PHI]]
; CHECK-NEXT:    store <8 x double> [[TMP0]], ptr [[SCALAR_GEP3]], align 64
