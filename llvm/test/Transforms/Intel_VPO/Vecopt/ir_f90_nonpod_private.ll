...
define void @foo_(ptr noalias nocapture dereferenceable(4) %"foo_$A", ptr noalias nocapture dereferenceable(4) %"foo_$B", ptr noalias nocapture dereferenceable(4) %"foo_$N", ptr noalias dereferenceable(80) %"foo_$CL") {
; CHECK:         [[PRIV_VEC:%.*]] = alloca <2 x ptr>, align 16
; CHECK-NEXT:    [[PRIV_VEC_BASE_ADDR:%.*]] = getelementptr ptr, ptr [[PRIV_VEC]], <2 x i32> <i32 0, i32 1>
; CHECK-NEXT:    [[PRIV_VEC_BASE_ADDR_EXTRACT_1:%.*]] = extractelement <2 x ptr> [[PRIV_VEC_BASE_ADDR]], i32 1
; CHECK-NEXT:    [[PRIV_VEC_BASE_ADDR_EXTRACT_0:%.*]] = extractelement <2 x ptr> [[PRIV_VEC_BASE_ADDR]], i32 0
; CHECK:  [[BB1:BB[0-9]+]]:
; CHECK:         call void @"%FOO$.btI_TY.omp.mold_ctor_deref"(ptr [[PRIV_VEC_BASE_ADDR_EXTRACT_0]], ptr [[PRIV:%.*]])
; CHECK-NEXT:    call void @"%FOO$.btI_TY.omp.mold_ctor_deref"(ptr [[PRIV_VEC_BASE_ADDR_EXTRACT_1]], ptr [[PRIV]])
; CHECK:  [[BB2:BB[0-9]+]]:
; CHECK:         call void @"%FOO$.btI_TY.omp.dtor_deref"(ptr [[PRIV_VEC_BASE_ADDR_EXTRACT_0]])
; CHECK-NEXT:    call void @"%FOO$.btI_TY.omp.dtor_deref"(ptr [[PRIV_VEC_BASE_ADDR_EXTRACT_1]])
;
; HIR:        [[PRIV_MEM_BC0:%.*]] = &((ptr)([[PRIV_MEM0:%.*]])[0])
; HIR:        @"%FOO$.btI_TY.omp.mold_ctor_deref"(&((ptr)([[PRIV_MEM0]])[0]),  [[PRIV:%.*]])
...
DIR.OMP.SIMD.1:
  ...
  %"foo_$CL.priv" = alloca ptr, align 8
  call void @"%FOO$.btI_TY.omp.mold_ctor_deref"(ptr nonnull %"foo_$CL.priv", ptr nonnull %"foo_$CL")
...
DIR.OMP.SIMD.127:                                 ; preds = %DIR.OMP.SIMD.1
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.SIMD"(), "QUAL.OMP.LINEAR:IV.TYPED"(ptr %"foo_$I.linear.iv", i32 0, i32 1, i32 1), "QUAL.OMP.PRIVATE:F90_NONPOD.TYPED"(ptr %"foo_$CL.priv", ptr zeroinitializer, i32 1, ptr @"%FOO$.btI_TY.omp.mold_ctor_deref", ptr @"%FOO$.btI_TY.omp.dtor_deref"), "QUAL.OMP.LIVEIN"(ptr %"foo_$B"), "QUAL.OMP.LIVEIN"(ptr %"foo_$A"), "QUAL.OMP.LIVEIN"(ptr %"foo_$N") ]
  br label %DIR.OMP.SIMD.129

DIR.OMP.SIMD.129:                                 ; preds = %DIR.OMP.SIMD.127
  %"foo_$CL_fetch.18" = load ptr, ptr %"foo_$CL.priv", align 8
