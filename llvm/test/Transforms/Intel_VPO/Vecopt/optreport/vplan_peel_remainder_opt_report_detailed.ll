...
; RUN: opt -vplan-vec -vplan-vec-scenario="n0;v4;v2s1" -intel-opt-report=high -intel-ir-optreport-emitter -vplan-enable-masked-vectorized-remainder=0 < %s -disable-output 2>&1 | FileCheck %s --strict-whitespace -check-prefixes=CHECK,IR
; RUN: opt -hir-ssa-deconstruction -hir-framework -hir-vplan-vec -vplan-vec-scenario="n0;v4;v2s1" -intel-opt-report=high -hir-optreport-emitter -vplan-enable-masked-vectorized-remainder=0 < %s -disable-output 2>&1 | FileCheck %s --strict-whitespace
