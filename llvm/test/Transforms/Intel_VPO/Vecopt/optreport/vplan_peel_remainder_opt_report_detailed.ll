...
; RUN: opt -passes=hir-ssa-deconstruction,hir-vplan-vec,hir-optreport-emitter -vplan-vec-scenario="n0;v4;v2s1" -intel-opt-report=high -vplan-enable-masked-vectorized-remainder=0 < %s -disable-output 2>&1 | FileCheck %s -check-prefixes=VEC-REM
...
; RUN: opt -passes=hir-ssa-deconstruction,hir-vplan-vec,hir-optreport-emitter \
; RUN:     -vplan-vec-scenario="s4;v4;n0" \
; RUN:     -intel-opt-report=high < %s -disable-output 2>&1 \
; RUN:     | FileCheck %s -check-prefixes=PEEL,STA-PEEL
; RUN: opt -passes=vplan-vec,intel-ir-optreport-emitter \
