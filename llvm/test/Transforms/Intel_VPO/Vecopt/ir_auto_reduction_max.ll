; RUN: opt -VPlanDriver -disable-vplan-predicator -disable-vplan-subregions -vplan-force-vf=4 -S < %s  -instcombine | FileCheck %s
...
; CHECK:       vector.ph:
; CHECK:       [[BROADCAST_SPLATINSERT:%.*]] = insertelement <4 x i32> undef, i32 [[TMP0]], i32 0
; CHECK:       [[BROADCAST_SPLAT:%.*]] = shufflevector <4 x i32> [[BROADCAST_SPLATINSERT]], <4 x i32> undef, <4 x i32> zeroinitializer
; CHECK:       vector.body:
...
; CHECK:       VPlannedBB:
; CHECK:       [[TMP6:%.*]] = call i32 @llvm.experimental.vector.reduce.smax.v4i32(<4 x i32> [[TMP3]])
; CHECK:       scalar.ph:
