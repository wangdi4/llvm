...
; VEC-HIR-NEXT: <{{[0-9]+}}:8>             %red.init = call <4 x i32> @llvm.ssa.copy.v4i32(<4 x i32> undef)
; VEC-HIR-NEXT: <{{[0-9]+}}:8>             %red.init.insert = insertelement <4 x i32> undef, i32 undef, i64 0
; VEC-HIR-NEXT: <{{[0-9]+}}:11>             %.copy = call <4 x i32> @llvm.ssa.copy.v4i32(<4 x i32> undef)
; VEC-HIR:      <{{[0-9]+}}:8>             + DO i1 = 0, 1023, 4   <DO_LOOP> <auto-vectorized> <novectorize>
; VEC-HIR-NEXT: <{{[0-9]+}}:10>            |     %.vec = load <4 x float>, ptr undef, align 16
; VEC-HIR-NEXT: <{{[0-9]+}}:10>            |     %.vec1 = load <4 x float>, ptr undef, align 16
; VEC-HIR-NEXT: <{{[0-9]+}}:10>            |     %.vec2 = fmul fast <4 x float> undef, undef
; VEC-HIR-NEXT: <{{[0-9]+}}:10>            |     store <4 x float> undef, ptr undef, align 16
; VEC-HIR-NEXT: <{{[0-9]+}}:11>            |     %.vec3 = ashr exact <4 x i32> undef, undef
; VEC-HIR-NEXT: <{{[0-9]+}}:11>            |     %.vec4 = fcmp fast true <4 x float> undef, undef
; VEC-HIR-NEXT: <{{[0-9]+}}:11>            |     %.vec5 = add <4 x i32> undef, undef
; VEC-HIR-NEXT: <{{[0-9]+}}:11>            |     %.vec6 = add <4 x i32> undef, undef
; VEC-HIR-NEXT: <{{[0-9]+}}:11>            |     %.vec7 = select fast i1 undef, <4 x float> undef, <4 x float> undef
; VEC-HIR-NEXT: <{{[0-9]+}}:11>            |     store <4 x float> undef, ptr undef, align 16
; VEC-HIR-NEXT: <{{[0-9]+}}:8>             |     %.copy8 = call <4 x i32> @llvm.ssa.copy.v4i32(<4 x i32> undef)
; VEC-HIR-NEXT: <{{[0-9]+}}:8>             + END LOOP
; VEC-HIR:      <{{[0-9]+}}:11>            %vec.reduce = call i32 @llvm.vector.reduce.add.v4i32(<4 x i32> undef)
