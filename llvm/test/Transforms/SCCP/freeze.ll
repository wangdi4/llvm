...
; This test is like propagate_info_from_phi_of_select_through_freeze
; but not all inputs to the 'phi' are from 'select' instructions.
; (CMPLRLLVM-49030)
define void @dont_propagate_info_from_phi_of_select_through_freeze2(i32 %x, i32 %y) {
; CHECK-LABEL: @dont_propagate_info_from_phi_of_select_through_freeze2(
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[CMP:%.*]] = icmp eq i32 [[X:%.*]], 1
; CHECK-NEXT:    [[CMP_1:%.*]] = icmp eq i32 [[Y:%.*]], 1
; CHECK-NEXT:    [[SELECT:%.*]] = select i1 [[CMP]], i32 1, i32 2
...
; CHECK-NEXT:    br label [[MERGE]]
; CHECK:       merge:
; CHECK-NEXT:    [[P:%.*]] = phi i32 [ [[SELECT]], [[ENTRY:%.*]] ], [ [[ADD]], [[THEN]] ]
; CHECK-NEXT:    [[FREEZE:%.*]] = freeze i32 [[P]]
; CHECK-NEXT:    [[COND:%.*]] = icmp eq i32 [[FREEZE]], 0
...
merge:
  %p = phi i32 [ %select, %entry ], [ %add, %then ]
  %freeze = freeze i32 %p
  %cond = icmp eq i32 %freeze, 0
  br i1 %cond, label %true, label %false
