; REQUIRES: asserts
; RUN: opt < %s -qsortrecognizer -debug-only=qsortrecognizer -qsort-unit-test -qsort-test-pivot-sorter -disable-output 2>&1 | FileCheck %s
; RUN: opt < %s -passes='module(qsortrecognizer)' -debug-only=qsortrecognizer -qsort-unit-test -qsort-test-pivot-sorter -disable-output 2>&1 | FileCheck %s

declare i32 @arc_compare(%struct.arc** nocapture readonly, %struct.arc** nocapture readonly)

; Check that the two pivot mover candidates are recognized, but the pivot
; sorter enclosing them, is not because the swap sequence in the "loop back"
; block has a store with a bad value operand.

; CHECK: QsortRec: Pivot Mover Candidate PASSED Test.
; CHECK: QsortRec: Pivot Mover Candidate PASSED Test.
; CHECK-NOT: QsortRec: PivotSorter in qsort_sorter PASSED Test.
