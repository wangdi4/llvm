; REQUIRES: asserts
; RUN: opt < %s -qsortrecognizer -debug-only=qsortrecognizer -qsort-unit-test -qsort-test-recursion -disable-output 2>&1 | FileCheck %s
; RUN: opt < %s -passes='module(qsortrecognizer)' -debug-only=qsortrecognizer -qsort-unit-test -qsort-test-recursion -disable-output 2>&1 | FileCheck %s
...
declare void @foo(i8* %arg1, i64 %arg2)

; Function Attrs: nounwind uwtable
define internal fastcc void @qsort_rec(i8* %0, i64 %1) unnamed_addr {
...
311:                                              ; preds = %309
  %312 = lshr i64 %259, 3
  tail call void @foo(i8* %34, i64 %312)
  br label %313
