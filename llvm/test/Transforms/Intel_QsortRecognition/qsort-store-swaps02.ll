; REQUIRES: asserts
; RUN: opt < %s -qsortrecognizer -debug-only=qsortrecognizer -qsort-unit-test -qsort-test-store-swaps -disable-output 2>&1 | FileCheck %s
; RUN: opt < %s -passes='module(qsortrecognizer)' -debug-only=qsortrecognizer -qsort-unit-test -qsort-test-store-swaps -disable-output 2>&1 | FileCheck %s

; This test checks that all the store instructions are swapping values. It must
; fail since there is one store that isn't part of a swap, but all other
; related tests (pivot movers and sorter, and vect swap) must pass.
; This is the same test as qsort-sorter01.ll with one instruction modified.

; CHECK: QsortRec: Insertion Sort Candidate in qsort_sorter PASSED Test.
; CHECK: QsortRec: Insertion Sort Candidate in qsort_sorter PASSED Test.
; CHECK: QsortRec: Pivot Mover Candidate in qsort_sorter PASSED Test (UP)
; CHECK: QsortRec: qsort_sorter passed pivot test
; CHECK: QsortRec: Pivot Mover Candidate PASSED Test.
; CHECK: QsortRec: Pivot Mover Candidate PASSED Test.
; CHECK: QsortRec: PivotSorter in qsort_sorter PASSED Test.
; CHECK: QsortRec: Computation of swap in qsort_sorter PASSED Test.
; CHECK: QsortRec: Computation of swap in qsort_sorter PASSED Test.
; CHECK: QsortRec: All store instructions are NOT swaps
; CHECK: FAILED QSORT RECOGNITION UNIT TESTS
