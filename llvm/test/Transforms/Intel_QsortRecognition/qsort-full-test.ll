; REQUIRES: asserts
; RUN: opt < %s -qsortrecognizer -debug-only=qsortrecognizer -disable-output 2>&1 | FileCheck %s
; RUN: opt < %s -passes='module(qsortrecognizer)' -debug-only=qsortrecognizer -disable-output 2>&1 | FileCheck %s

; This test checks that qsort was recognized as well as the pieces that
; compose the analysis.

; CHECK: QsortRec: Insertion Sort Candidate in qsort PASSED Test
; CHECK: QsortRec: qsort passed pivot test
; CHECK: QsortRec: Pivot Mover Candidate PASSED Test.
; CHECK: QsortRec: Pivot Mover Candidate PASSED Test.
; CHECK: QsortRec: PivotSorter in qsort PASSED Test.
; CHECK: QsortRec: Computation of MIN in qsort PASSED Test.
; CHECK: QsortRec: Computation of MIN in qsort PASSED Test.
; CHECK: QsortRec: Computation of swap in qsort PASSED Test.
; CHECK: QsortRec: Computation of swap in qsort PASSED Test.
; CHECK: QsortRec: Recursion in qsort PASSED Test.
; CHECK: QsortRec: Recursion in qsort PASSED Test.
; CHECK: QsortRec: All store instructions are swaps
