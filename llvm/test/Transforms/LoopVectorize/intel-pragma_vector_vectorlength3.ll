...
; RUN: opt -passes="loop-vectorize" %s -S -force-vector-width=4 --debug-only=loop-vectorize-vec-lengths 2>&1 | FileCheck %s
; REQUIRES: asserts
;
; Cheks if the code with #pragma vector vectorlength(8) (!{!"llvm.loop.vector.vectorlength", i64 8} metadata)
; and with -force-vector-width=4 is being vectorized correctly with LoopVectorize
;
; void foo(int *arr)
; {
;   int i;
