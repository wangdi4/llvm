...
; RUN: opt < %s  -passes=loop-vectorize -enable-lv -enable-load-coalescing=false -mcpu=corei7-avx -S | FileCheck %s -check-prefix=AVX -check-prefix=AVX1
; RUN: opt < %s  -passes=loop-vectorize -enable-lv -enable-load-coalescing=false -mcpu=core-avx2 -S | FileCheck %s -check-prefix=AVX -check-prefix=AVX2
; RUN: opt < %s  -passes=loop-vectorize -enable-lv -enable-load-coalescing=false -mcpu=knl -S | FileCheck %s -check-prefix=AVX512
