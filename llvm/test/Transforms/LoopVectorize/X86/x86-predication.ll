...
; INTEL_CUSTOMIZATION
; xmain does not sink gather. Checks and check-prefix have been updated to
; reflect the same. Original checks have been retained to minimize future
; conflicts.
; RUN: opt -mcpu=skylake-avx512 -S -force-vector-width=8 -force-vector-interleave=1 -loop-vectorize < %s | FileCheck %s --check-prefix=GATHER
...
; GATHER-NEXT:   [[INS:%.+]] = insertelement <8 x i32> poison, i32 [[UDIV]], i32 0
; GATHER-NEXT:   br label %pred.udiv.continue
; GATHER:      pred.udiv.continue:
; GATHER-NEXT:   phi <8 x i32> [ poison, %vector.body ], [ [[INS]], %pred.udiv.if ]
; end INTEL_CUSTOMIZATION
