; RUN: opt -opaque-pointers -intel-ipo-dead-arg-elimination  %s -S 2>&1 | FileCheck %s
...
; test case as intel-ipo-dead-arg-07-IR.ll but it checks the IR for opaque
; pointers.
...
; CHECK-NEXT:   %6 = call float (ptr, ptr, i64, i64, ...) @foo(ptr %5, ptr %0, i64 %2, i64 %3)
; CHECK-NEXT:   %7 = fadd float %1, %6
; CHECK-NEXT:   ret float %7

; ModuleID = 'intel-ipo-dead-arg-07-IR.ll'
source_filename = "intel-ipo-dead-arg-07-IR.ll"
...
define float @bas(ptr %0, float %1, i64 %2, i64 %3) {
  ...
  %6 = call float (ptr, ptr, i64, i64, ...) @foo(ptr %5, ptr %0, i64 %2, i64 %3)
  %7 = fadd float %1, %6
  ret float %7
}
