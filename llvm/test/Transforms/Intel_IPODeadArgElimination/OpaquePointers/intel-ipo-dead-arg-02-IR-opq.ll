...
; in function @bas. This test case is the same as intel-ipo-dead-arg-02-IR.ll
; but it checks the IR with opaque pointers.

; CHECK: define internal float @foo(ptr %0, i64 %1, i64 %2)
; CHECK-NEXT:   %4 = load float, ptr %0, align 4
; CHECK-NEXT:   ret float %4

; CHECK: define internal float @bar(ptr %0, i64 %1, i64 %2)
; CHECK-NEXT:   %4 = load float, ptr %0, align 4
; CHECK-NEXT:   ret float %4

; CHECK: define internal float @bas(ptr %0, ptr %1, i64 %2, i64 %3)
; CHECK-NEXT:   %5 = call float @foo(ptr %0, i64 %2, i64 %3)
; CHECK-NEXT:   %6 = call float @bar(ptr %1, i64 %2, i64 %3)
; CHECK-NEXT:   %7 = fadd float %5, %6
; CHECK-NEXT:   ret float %7

; ModuleID = 'intel-ipo-dead-arg-02-IR.ll'
source_filename = "intel-ipo-dead-arg-02-IR.ll"
