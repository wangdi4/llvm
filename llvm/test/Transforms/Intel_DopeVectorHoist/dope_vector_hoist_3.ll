...
; REQUIRES: asserts
; RUN: opt -opaque-pointers=0 < %s -passes=dopevectorhoist -debug-only=dopevectorhoist -dopevector-hoist-enable=true -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx2 -S 2>&1 | FileCheck %s
