...
; RUN: opt -passes='loop-simplifycfg<no-keep-loops>' -S < %s | FileCheck %s
; INTEL
; RUN: opt -S < %s -convert-to-subscript | opt -simplifycfg -keep-loops=false -S | FileCheck --check-prefix=CHECK-SUBS %s
; RUN: opt -S < %s -convert-to-subscript | opt -passes='loop-simplifycfg<no-keep-loops>' -S | FileCheck --check-prefix=CHECK-SUBS %s
...
;
; INTEL
; CHECK-SUBS-LABEL: @upper_bound(
; CHECK-SUBS-NEXT:  entry:
; CHECK-SUBS-NEXT:    br label [[WHILE_COND:%.*]]
; CHECK-SUBS:       while.cond:
; CHECK-SUBS-NEXT:    [[HIGH_ADDR_0:%.*]] = phi i32 [ [[HIGH:%.*]], [[ENTRY:%.*]] ], [ [[DIV_HIGH_ADDR_0:%.*]], [[WHILE_BODY:%.*]] ]
; CHECK-SUBS-NEXT:    [[LOW_0:%.*]] = phi i32 [ 0, [[ENTRY]] ], [ [[LOW_0_ADD2:%.*]], [[WHILE_BODY]] ]
; CHECK-SUBS-NEXT:    [[CMP:%.*]] = icmp ult i32 [[LOW_0]], [[HIGH_ADDR_0]]
; CHECK-SUBS-NEXT:    br i1 [[CMP]], label [[WHILE_BODY]], label [[WHILE_END:%.*]]
; CHECK-SUBS:       while.body:
; CHECK-SUBS-NEXT:    [[ADD:%.*]] = add i32 [[LOW_0]], [[HIGH_ADDR_0]]
; CHECK-SUBS-NEXT:    [[DIV:%.*]] = udiv i32 [[ADD]], 2
; CHECK-SUBS-NEXT:    [[IDXPROM:%.*]] = zext i32 [[DIV]] to i64
; CHECK-SUBS-NEXT:    [[ARRAYIDX:%.*]] = call i32* @llvm.intel.subscript.p0i32.i64.i64.p0i32.i64(i8 0, i64 0, i64 4, i32* [[R:%.*]], i64 [[IDXPROM]]
; CHECK-SUBS-NEXT:    [[TMP0:%.*]] = load i32, i32* [[ARRAYIDX]]
; CHECK-SUBS-NEXT:    [[CMP1:%.*]] = icmp ult i32 [[K:%.*]], [[TMP0]]
; CHECK-SUBS-NEXT:    [[ADD2:%.*]] = add i32 [[DIV]], 1
; CHECK-SUBS-NEXT:    [[DIV_HIGH_ADDR_0]] = select i1 [[CMP1]], i32 [[DIV]], i32 [[HIGH_ADDR_0]]
; CHECK-SUBS-NEXT:    [[LOW_0_ADD2]] = select i1 [[CMP1]], i32 [[LOW_0]], i32 [[ADD2]]
; CHECK-SUBS-NEXT:    br label [[WHILE_COND]]
; CHECK-SUBS:       while.end:
; CHECK-SUBS-NEXT:    ret i32 [[LOW_0]]
;
entry:
  br label %while.cond
