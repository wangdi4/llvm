...
; INTEL
; RUN: opt -S < %s -passes=convert-to-subscript | opt -passes='simplifycfg<no-keep-loops>' -S | FileCheck --check-prefix=CHECK-SUBS %s
; end INTEL
...
; CHECK-SUBS-NEXT:    [[HIGH_ADDR_0:%.*]] = phi i32 [ [[HIGH:%.*]], [[ENTRY:%.*]] ], [ [[SPEC_SELECT:%.*]], [[WHILE_BODY:%.*]] ]
; CHECK-SUBS-NEXT:    [[LOW_0:%.*]] = phi i32 [ 0, [[ENTRY]] ], [ [[SPEC_SELECT1:%.*]], [[WHILE_BODY]] ]
...
; CHECK-SUBS-NEXT:    [[ARRAYIDX:%.*]] = call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 0, i64 0, i64 4, ptr elementtype(i32) [[R:%.*]], i64 [[IDXPROM]])
; CHECK-SUBS-NEXT:    [[TMP0:%.*]] = load i32, ptr [[ARRAYIDX]], align 4
; CHECK-SUBS-NEXT:    [[CMP1:%.*]] = icmp ult i32 [[K:%.*]], [[TMP0]]
; CHECK-SUBS-NEXT:    [[ADD2:%.*]] = add i32 [[DIV]], 1
; CHECK-SUBS-NEXT:    [[SPEC_SELECT]] = select i1 [[CMP1]], i32 [[DIV]], i32 [[HIGH_ADDR_0]]
; CHECK-SUBS-NEXT:    [[SPEC_SELECT1]] = select i1 [[CMP1]], i32 [[LOW_0]], i32 [[ADD2]]
; CHECK-SUBS-NEXT:    br label [[WHILE_COND]]
