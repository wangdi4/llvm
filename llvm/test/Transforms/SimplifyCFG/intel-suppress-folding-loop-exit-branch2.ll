...
; Note that IV phi is used in the loop exit condition.

; CHECK-LABEL: @iv-phi-preloopopt
; CHECK: loop:
; CHECK: latch:

define void @iv-phi-preloopopt(i32 %v) "pre_loopopt" {
entry:
  br label %loop
...
; CHECK-LABEL: @iv-phi-no-preloopopt
; CHECK: loop:
; CHECK-NOT: latch:
; CHECK: %or.cond
; CHECK: br i1 %or.cond, label %loop, label %exit

define void @iv-phi-no-preloopopt(i32 %v) {
entry:
  br label %loop
