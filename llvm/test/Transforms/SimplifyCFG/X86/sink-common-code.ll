...
; RUN: opt < %s -simplifycfg -simplifycfg-require-and-preserve-domtree=1 -sink-common-insts -phi-node-folding-threshold=2 -S | FileCheck %s ;INTEL
; RUN: opt < %s -passes='simplify-cfg<sink-common-insts>' -phi-node-folding-threshold=2 -S | FileCheck %s ;INTEL
