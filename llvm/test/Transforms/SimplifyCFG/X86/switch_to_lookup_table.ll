...
; INTEL_CUSTOMIZATION
; This large test has numerous differences which are better described here
; than marked.
; llorg version does not use "define" pattern on labels. Probably needs
; an scripted update.
; @overflow: xmain adds a default block to the switch.
; @undef,@linearmap*,@reuse_cmp*: temps renamed
;
...
; CHECK-LABEL: define {{[^@]+}}@earlyreturncrash(
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[TMP0:%.*]] = icmp ult i32 [[X:%.*]], 4
; CHECK-NEXT:    br i1 [[TMP0]], label [[SWITCH_LOOKUP:%.*]], label [[SW_EPILOG:%.*]]
; CHECK:       switch.lookup:
...
; CHECK-LABEL: define {{[^@]+}}@unreachable_case(
; CHECK-NEXT:  entry:
...
; CHECK-LABEL: define {{[^@]+}}@unreachable_default(
; CHECK-NEXT:  entry:
...
; CHECK-LABEL: define {{[^@]+}}@illegaltype(
; CHECK-NEXT:  entry:
...
; CHECK-LABEL: define {{[^@]+}}@nodefaultnoholes(
; CHECK-NEXT:  entry:
...
; CHECK-LABEL: define {{[^@]+}}@nodefaultwithholes(
; CHECK-NEXT:  entry:
...
; CHECK-LABEL: define {{[^@]+}}@threecasesholes(
; CHECK-NEXT:  entry:
; CHECK-NEXT:    switch i32 [[C:%.*]], label [[SW_DEFAULT:%.*]] [
...
; CHECK-NEXT:    [[TMP2:%.*]] = select i1 [[INVERTED_CMP]], i32 100, i32 [[TMP1]]
...
; CHECK-LABEL: define {{[^@]+}}@reuse_cmp2(
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[TMP0:%.*]] = icmp ult i32 [[X:%.*]], 4
; CHECK-NEXT:    [[X_:%.*]] = select i1 [[TMP0]], i32 [[X]], i32 4
; CHECK-NEXT:    [[CMP:%.*]] = icmp ne i32 [[X_]], 4
; CHECK-NEXT:    [[TMP1:%.*]] = select i1 [[TMP0]], i32 [[X_]], i32 100
; CHECK-NEXT:    ret i32 [[TMP1]]
;
entry:
  switch i32 %x, label %sw.default [
...
; CHECK-LABEL: define {{[^@]+}}@no_reuse_cmp2(
; CHECK-NEXT:  entry:
...
<<<<<<< HEAD
; CHECK-NEXT:    [[TMP1:%.*]] = select i1 [[TMP0]], i32 [[SWITCH_OFFSET]], i32 0
; CHECK-NEXT:    [[TMP2:%.*]] = select i1 [[EC]], i32 [[TMP1]], i32 100
; CHECK-NEXT:    [[CMP:%.*]] = icmp eq i32 [[TMP2]], 0
; CHECK-NEXT:    [[TMP3:%.*]] = select i1 [[CMP]], i32 100, i32 [[TMP2]]
...
; CHECK-LABEL: define {{[^@]+}}@covered_switch_with_bit_tests(
; CHECK-NEXT:  entry:
...
; CHECK-LABEL: define {{[^@]+}}@use_x_as_index(
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[TMP0:%.*]] = icmp ult i32 [[X:%.*]], 9
...
; CHECK-LABEL: define {{[^@]+}}@signed_overflow1(
; CHECK-NEXT:  start:
...
; CHECK-LABEL: define {{[^@]+}}@signed_overflow2(
; CHECK-NEXT:  start:
; CHECK-NEXT:    [[TRUNC:%.*]] = trunc i8 [[N:%.*]] to i2
...
; CHECK-LABEL: define {{[^@]+}}@signed_overflow_negative(
; CHECK-NEXT:  start:
; CHECK-NEXT:    [[TRUNC:%.*]] = trunc i8 [[N:%.*]] to i2
; CHECK-NEXT:    [[SWITCH_TABLEIDX:%.*]] = sub i2 [[TRUNC]], -2
