...
; RUN: opt < %s -passes='loop-simplifycfg<no-keep-loops;switch-to-lookup>' -S -mtriple=x86_64-unknown-linux-gnu | FileCheck %s
...
; CHECK-NEXT:  entry:
; CHECK-NEXT:    switch i32 [[TYPE:%.*]], label [[SW_DEFAULT:%.*]] [ ;INTEL
; CHECK-NEXT:    i32 -2147483645, label [[SW_BB3:%.*]] ;INTEL
; CHECK-NEXT:    i32 3, label [[SW_BB3]] ;INTEL
; CHECK-NEXT:    i32 1, label [[IF_END:%.*]] ;INTEL
...
; CHECK:       sw.bb3:
; CHECK-NEXT:    br label [[IF_END]]
; CHECK:       sw.default: ;INTEL
; CHECK-NEXT:    br label [[IF_END]] ;INTEL
; CHECK:       if.end:
; CHECK-NEXT:    [[DIRENT_TYPE_0:%.*]] = phi i32 [ 3, [[SW_DEFAULT]] ], [ 6, [[SW_BB3]] ], [ 5, [[SW_BB2]] ], [ 0, [[ENTRY:%.*]] ] ;INTEL
...
; CHECK-NEXT:    [[TMP0:%.*]] = icmp ult i32 [[SWITCH_TABLEIDX]], 3 ;INTEL
; CHECK-NEXT:    br i1 [[TMP0]], label [[SWITCH_LOOKUP:%.*]], label [[RETURN:%.*]]
; CHECK:       switch.lookup:
