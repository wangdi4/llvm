...
; RUN: opt < %s -xmain-enable-gep0-removal -passes=simplifycfg -simplifycfg-require-and-preserve-domtree=1 -phi-node-folding-threshold=2 -S | FileCheck %s ;INTEL

; phi folding algorithm is significantly different in xmain. ;INTEL
...
; CHECK-NEXT:    [[TMP0:%.*]] = select i1 [[T1]], i32 [[SPEC_SELECT]], i32 [[B]] ;INTEL
; CHECK-NEXT:    [[T5:%.*]] = sub i32 [[TMP0]], 1 ;INTEL
...
; CHECK-NEXT:    [[TMP0:%.*]] = select ninf i1 [[T2]], float [[T3]], float [[A]] ;INTEL
; CHECK-NEXT:    br label [[BB3]]
; CHECK:       bb3:
; CHECK-NEXT:    [[T4:%.*]] = phi ninf float [ [[B]], [[ENTRY:%.*]] ], [ [[TMP0]], [[BB1]] ] ;INTEL
...
define i32* @test5(i32 %a, i32 %b, i32 %c, i32* dereferenceable(10) %ptr1, i32* dereferenceable(10) %ptr2, i32** dereferenceable(10) align 8 %ptr3) nofree nosync { ;INTEL
; CHECK-LABEL: @test5(
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[T1:%.*]] = icmp eq i32 [[B:%.*]], 0
; CHECK-NEXT:    [[T2:%.*]] = icmp sgt i32 [[C:%.*]], 1
...
; CHECK-NEXT:    [[TMP0:%.*]] = select i1 [[T1]], ptr [[SPEC_SELECT]], ptr [[PTR1:%.*]] ;INTEL
; CHECK-NEXT:    ret ptr [[TMP0]] ;INTEL
;
entry:
  %t1 = icmp eq i32 %b, 0
  br i1 %t1, label %bb1, label %bb3
...
; CHECK-NEXT:    [[T1:%.*]] = fcmp oeq float [[B:%.*]], 0.000000e+00
; CHECK-NEXT:    [[T2:%.*]] = fcmp ogt float [[C:%.*]], 1.000000e+00
; CHECK-NEXT:    [[TMP0:%.*]] = select nsz i1 [[T2]], float [[C]], float [[B]] ;INTEL
; CHECK-NEXT:    [[TMP1:%.*]] = select nsz i1 [[T1]], float [[TMP0]], float [[A:%.*]] ;INTEL
