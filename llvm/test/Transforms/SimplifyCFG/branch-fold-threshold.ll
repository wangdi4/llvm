...
; RUN: opt %s -passes=loop-simplifycfg -S | FileCheck %s --check-prefix=NORMAL
; RUN: opt %s -passes='loop-simplifycfg<bonus-inst-threshold=2>' -S | FileCheck %s --check-prefix=AGGRESSIVE
; RUN: opt %s -passes='loop-simplifycfg<bonus-inst-threshold=4>' -S | FileCheck %s --check-prefix=WAYAGGRESSIVE
