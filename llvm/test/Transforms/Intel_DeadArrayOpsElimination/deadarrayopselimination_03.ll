...
; REQUIRES: asserts
; RUN: opt < %s  -deadarrayopselimination -dead-array-ops-functions="s_qsort,60" -debug-only=deadarrayopselimination  -mtriple=i686-- -mattr=+avx2 -whole-program-assume  -enable-intel-advanced-opts -disable-output 2>&1 | FileCheck %s
; RUN: opt < %s  -passes='module(deadarrayopselimination)' -dead-array-ops-functions="s_qsort,60" -debug-only=deadarrayopselimination  -mtriple=i686-- -mattr=+avx2 -whole-program-assume  -enable-intel-advanced-opts -disable-output 2>&1 | FileCheck %s
