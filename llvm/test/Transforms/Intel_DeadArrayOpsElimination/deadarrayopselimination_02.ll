...
; This test is same as deadarrayopselimination_01.ll except only s_qsort
; recursive call is cloned instead of entire %bb1 due to %t7, which is not
; related to the recursive call.
...
; CHECK:  %t3 = lshr i64 %t1, 3
; CHECK:  %t7 = getelementptr inbounds i8, ptr %t5, i64 2
; CHECK:  [[CMP1:%[0-9]+]] = icmp ult ptr %t6, %2
; CHECK:  br i1 [[CMP1]], label %4, label %5

; CHECK: 4:
; CHECK:  tail call fastcc void @s_qsort{{.*}}(ptr %t6, i64 %t3, ptr %2)
; CHECK:  br label %5
...
; CHECK:bb2:
; CHECK:  %t4 = icmp ugt i64 %t1, 4
; CHECK:  br i1 %t4, label %bb0, label %bb3
...
bb1:                                              ; preds = %bb0
  %t3 = lshr i64 %t1, 3
  %t7 = getelementptr inbounds i8, ptr %t5, i64 2
