; Check to see that __svml_cdfnorminvf4 is translated to the medium accuracy svml variant.

; RUN: opt -enable-new-pm=0 -vector-library=SVML -iml-trans -S < %s | FileCheck %s

; CHECK-LABEL: @vector_foo
; CHECK: call fast svml_cc <4 x float> @__svml_cdfnorminvf4(
; CHECK: ret
...
vector.body:                                      ; preds = %vector.body, %entry
  ...
  %3 = fmul fast <4 x float> %2, <float 0x3F505E1D20000000, float 0x3F505E1D20000000, float 0x3F505E1D20000000, float 0x3F505E1D20000000>
  %4 = call fast <4 x float> @__svml_cdfnorminvf4(<4 x float> %3)
...
declare <4 x float> @__svml_cdfnorminvf4(<4 x float>) #7
