; __svml_nearbyintf4 does not have an _ha variant. The iml query results in the same variant function name when using attributes of max-error=0.5 and precision="high". This
...
; CHECK: call svml_cc <4 x float> @__svml_nearbyintf4
; CHECK: ret
...
vector.body:                                      ; preds = %entry, %vector.body
  ...
  %2 = call fast <4 x float> @__svml_nearbyintf4(<4 x float> %wide.load)
...
for.body:                                         ; preds = %entry, %for.body
  ...
  %call = tail call fast float @nearbyintf(float %6) #6
...
declare float @nearbyintf(float) #2

; Function Attrs: nounwind readnone
declare <4 x float> @__svml_nearbyintf4(<4 x float>) #6
