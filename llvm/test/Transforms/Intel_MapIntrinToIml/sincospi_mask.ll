...
; CHECK: %{{.*}} = call svml_cc_avx512 { <16 x float>, <16 x float> } @__svml_sincospif16_ha_mask_z0({ <16 x float>, <16 x float> } %tmp2, <16 x i1> %tmp, <16 x float> %E)
; CHECK: ret
...
; CHECK: %{{.*}} = call svml_cc_avx512 { <8 x double>, <8 x double> } @__svml_sincospi8_ha_mask_z0({ <8 x double>, <8 x double> } %tmp2, <8 x i1> %tmp, <8 x double> %E)
; CHECK: ret
