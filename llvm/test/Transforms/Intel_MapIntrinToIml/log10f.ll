; Check to see that __svml_log10f4 is translated to the medium accuracy svml variant.

; RUN: opt -enable-new-pm=0 -vector-library=SVML -iml-trans -S < %s | FileCheck %s

; CHECK-LABEL: @vector_foo
; CHECK: call fast svml_cc <4 x float> @__svml_log10f4(
; CHECK: ret
...
vector.body:                                      ; preds = %vector.body, %entry
  ...
  %3 = call fast <4 x float> @__svml_log10f4(<4 x float> %2)
...
declare <4 x float> @__svml_log10f4(<4 x float>) #6
