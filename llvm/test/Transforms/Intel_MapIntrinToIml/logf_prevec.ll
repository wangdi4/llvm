; Check to see that the -fveclib=SVML option is applied and the logf call becomes __svml_logf.

; RUN: opt -vector-library=SVML -loop-vectorize -S < %s | FileCheck %s

; CHECK-LABEL: @vector_foo
; CHECK: call fast <4 x float> @__svml_logf4
; CHECK: ret
...
for.body:                                         ; preds = %for.body, %entry
  ...
  %call = tail call fast float @logf(float %conv) #5
