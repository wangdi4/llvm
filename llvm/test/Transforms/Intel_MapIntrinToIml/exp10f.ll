; Check to see that __svml_exp10f4 is translated to the medium accuracy svml variant.

; RUN: opt -enable-new-pm=0 -vector-library=SVML -iml-trans -S < %s | FileCheck %s

; CHECK-LABEL: @vector_foo
; CHECK: call fast svml_cc <4 x float> @__svml_exp10f4(
; CHECK: ret
...
vector.body:                                      ; preds = %vector.body, %entry
  ...
  %1 = srem <4 x i32> %induction9, <i32 8, i32 8, i32 8, i32 8>
  %2 = sitofp <4 x i32> %1 to <4 x float>
  %3 = call fast <4 x float> @__svml_exp10f4(<4 x float> %2)
...
declare <4 x float> @__svml_exp10f4(<4 x float>) #6
