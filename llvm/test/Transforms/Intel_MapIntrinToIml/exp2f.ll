; Check to see that __svml_exp2f4 is translated to the medium accuracy svml variant.

; RUN: opt -enable-new-pm=0 -vector-library=SVML -iml-trans -S < %s | FileCheck %s

; CHECK-LABEL: @vector_foo
; CHECK: call fast svml_cc <4 x float> @__svml_exp2f4(
; CHECK: ret
...
vector.body:                                      ; preds = %entry, %vector.body
  ...
  %2 = call fast <4 x float> @__svml_exp2f4(<4 x float> %wide.load)
...
for.body:                                         ; preds = %entry, %for.body
  ...
  %call = tail call fast float @exp2f(float %6) #6
...
; Function Attrs: nounwind readnone
declare <4 x float> @__svml_exp2f4(<4 x float>) #6
