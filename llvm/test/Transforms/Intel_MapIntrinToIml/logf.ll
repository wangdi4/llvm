; Check to see that __svml_logf4 is translated to the high accuracy svml variant.

; RUN: opt -enable-new-pm=0 -vector-library=SVML -iml-trans -S < %s | FileCheck %s

; CHECK-LABEL: @foo
; CHECK: call fast svml_cc <4 x float> @__svml_logf4(
; CHECK: ret
...
vector.body:                                      ; preds = %vector.body, %entry
  ...
  %2 = call fast <4 x float> @__svml_logf4(<4 x float> %1)
...
declare <4 x float> @__svml_logf4(<4 x float>) #3
