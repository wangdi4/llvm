...
; RUN: opt -mtriple=i686-unknown-linux-gnu -bugpoint-enable-legacy-pm -vector-library=SVML -mattr=+sse2 -iml-trans -S < %s | FileCheck %s -check-prefixes=CHECK,CHECK-X86
; RUN: opt -mtriple=x86_64-unknown-linux-gnu -bugpoint-enable-legacy-pm -vector-library=SVML -iml-trans -S < %s | FileCheck %s -check-prefixes=CHECK,CHECK-X86_64
; RUN: opt -mtriple=i686-unknown-linux-gnu -bugpoint-enable-legacy-pm -vector-library=SVML -mattr=+sse2 -iml-trans -enable-intel-advanced-opts -S < %s | FileCheck %s -check-prefixes=CHECK,CHECK-X86
; RUN: opt -mtriple=x86_64-unknown-linux-gnu -bugpoint-enable-legacy-pm -vector-library=SVML -iml-trans -enable-intel-advanced-opts -S < %s | FileCheck %s -check-prefixes=CHECK,CHECK-X86_64
