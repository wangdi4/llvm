; Check to see that __svml_hypotf4 is translated to the medium accuracy svml variant.

; RUN: opt -enable-new-pm=0 -vector-library=SVML -iml-trans -S < %s | FileCheck %s

; CHECK-LABEL: @vector_foo
; CHECK: call fast svml_cc <4 x float> @__svml_hypotf4(
; CHECK: ret
...
define void @vector_foo(float* nocapture %varray, float* nocapture readonly %side_a, float* nocapture readonly %side_b) #0 {
entry:
  %scevgep = getelementptr float, float* %varray, i64 999
  %scevgep14 = getelementptr float, float* %side_a, i64 999
  %scevgep17 = getelementptr float, float* %side_b, i64 999
  %bound0 = icmp uge float* %scevgep14, %varray
  %bound1 = icmp uge float* %scevgep, %side_a
  %found.conflict = and i1 %bound0, %bound1
  %bound019 = icmp uge float* %scevgep17, %varray
  %bound120 = icmp uge float* %scevgep, %side_b
...
vector.body:                                      ; preds = %entry, %vector.body
  ...
  %0 = getelementptr inbounds float, float* %side_a, i64 %index
  %1 = bitcast float* %0 to <4 x float>*
  %wide.load = load <4 x float>, <4 x float>* %1, align 4
  %2 = getelementptr inbounds float, float* %side_b, i64 %index
  %3 = bitcast float* %2 to <4 x float>*
  %wide.load22 = load <4 x float>, <4 x float>* %3, align 4
  %4 = call fast <4 x float> @__svml_hypotf4(<4 x float> %wide.load, <4 x float> %wide.load22)
...
for.body:                                         ; preds = %entry, %for.body
  ...
  %arrayidx = getelementptr inbounds float, float* %side_a, i64 %indvars.iv
  %8 = load float, float* %arrayidx, align 4
  %arrayidx2 = getelementptr inbounds float, float* %side_b, i64 %indvars.iv
  %9 = load float, float* %arrayidx2, align 4
  %call = tail call fast float @hypotf(float %8, float %9) #6
...
declare <4 x float> @__svml_hypotf4(<4 x float>, <4 x float>) #6
