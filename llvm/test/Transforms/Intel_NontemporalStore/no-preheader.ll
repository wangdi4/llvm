...
; CHECK-NEXT:    [[EMPTY:%.*]] = icmp eq ptr [[DEST]], null
...
; CHECK-NEXT:    br i1 [[EMPTY]], label [[EXIT:%.*]], label [[LOOP:%.*]]
; CHECK:       loop:
...
; CHECK:       exit:
; CHECK-NEXT:    [[ADDR_NT_BUF_EXIT_PHI:%.*]] = phi i64 [ [[ADDR_NT_BUF_POST_PHI]], [[LOOP_SPLIT]] ], [ 0, [[ENTRY]] ]
; CHECK-NEXT:    [[TMP7:%.*]] = mul i64 [[ADDR_NT_BUF_EXIT_PHI]], 64
; CHECK-NEXT:    call void @__libirc_nontemporal_store(ptr [[ADDR_NT_STORE_ALLOCA]], i64 [[TMP7]], i32 1)
; CHECK-NEXT:    ret void
;
entry:
  %empty = icmp eq ptr %dest, null
