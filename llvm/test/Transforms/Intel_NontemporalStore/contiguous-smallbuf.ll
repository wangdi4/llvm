...
; RUN: opt -enable-intel-advanced-opts -intel-libirc-allowed -S -passes 'unaligned-nontemporal,verify' -unaligned-nontemporal-buffer-size=1024 < %s | FileCheck %s
; RUN: opt -enable-intel-advanced-opts -intel-libirc-allowed -S -unaligned-nontemporal -verify -unaligned-nontemporal-buffer-size=1024 < %s | FileCheck %s
...
; CHECK-NEXT:    [[ADDR_NT_STORE_ALLOCA:%.*]] = alloca i8, i64 1112, align 64
...
; CHECK-NEXT:    [[TMP6:%.*]] = icmp eq i64 16, [[ADDR_NT_BUF_IDX2]]
; CHECK-NEXT:    br i1 [[TMP6]], label [[ADDR_NT_BUF_DRAIN:%.*]], label [[LOOP_SPLIT]]
; CHECK:       addr.nt_buf_drain:
; CHECK-NEXT:    call void @__libirc_nontemporal_store(%__nontemporal_buffer_data* [[ADDR_NT_STORE_STRUCT]], i64 1024, i32 0)
