...
; BS1:   call void @__libirc_nontemporal_store(ptr [[ADDR_NT_STORE_STRUCT:%.*]], i64 64, i32 0)

; Don't round if the buffer size is an exact multiple of the element size.
; BS512: call void @__libirc_nontemporal_store(ptr [[ADDR_NT_STORE_STRUCT:%.*]], i64 512, i32 0)

; Round down if the buffer size is less than halfway between two multiples.
; BS543: call void @__libirc_nontemporal_store(ptr [[ADDR_NT_STORE_STRUCT:%.*]], i64 512, i32 0)

; Also round down if the buffer size is exactly between two multiples.
; BS544: call void @__libirc_nontemporal_store(ptr [[ADDR_NT_STORE_STRUCT:%.*]], i64 512, i32 0)

; Round up if the buffer size is more than halfway between two multiples.
; BS545: call void @__libirc_nontemporal_store(ptr [[ADDR_NT_STORE_STRUCT:%.*]], i64 576, i32 0)
