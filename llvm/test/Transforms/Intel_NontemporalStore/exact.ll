; RUN: opt -enable-intel-advanced-opts -S -passes 'unaligned-nontemporal' < %s | FileCheck %s
; RUN: opt -enable-intel-advanced-opts -S -unaligned-nontemporal < %s | FileCheck %s
