...
; RUN: opt -opaque-pointers=0 -passes="addsub-reassoc" < %s -S | FileCheck %s -check-prefix=CHECK

; RUN: opt -opaque-pointers=0 -passes="addsub-reassoc" < %s -addsub-reassoc-max-distributed-instructions=0 -S | FileCheck %s -check-prefix=DISTDISABLED
; RUN: opt -opaque-pointers=0 -passes="addsub-reassoc" < %s -addsub-reassoc-canonicalize-group=false -S | FileCheck %s -check-prefix=NOGROUPCAN
; RUN: opt -opaque-pointers=0 -passes="addsub-reassoc" < %s -addsub-reassoc-max-distributed-instructions=3 -S | FileCheck %s -check-prefix=MULTIPLE
