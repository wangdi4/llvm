...
; RUN: opt < %s -addsub-reassoc -S | FileCheck %s -check-prefix=CHECK

; RUN: opt < %s -addsub-reassoc -addsub-reassoc-memcan-enable-unary-associations=false -S | FileCheck %s -check-prefix=DISTDISABLED
; RUN: opt < %s -addsub-reassoc -addsub-reassoc-canonicalize-group=false -S | FileCheck %s -check-prefix=NOGROUPCAN
; RUN: opt < %s -addsub-reassoc -addsub-reassoc-max-unary-associations=3 -S | FileCheck %s -check-prefix=MULTIPLE
...
; hence no pull-ins regardless of whether it is enabled or disabled.
; CHECK-LABEL: @test0(
; CHECK-NEXT:  entry:
