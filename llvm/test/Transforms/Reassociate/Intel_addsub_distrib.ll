...
; RUN: opt < %s -addsub-reassoc -addsub-reassoc-max-distributed-instructions=0 -S | FileCheck %s -check-prefix=DISTDISABLED
; RUN: opt < %s -addsub-reassoc -addsub-reassoc-canonicalize-group=false -S | FileCheck %s -check-prefix=NOGROUPCAN
; RUN: opt < %s -addsub-reassoc -addsub-reassoc-max-distributed-instructions=3 -S | FileCheck %s -check-prefix=MULTIPLE
