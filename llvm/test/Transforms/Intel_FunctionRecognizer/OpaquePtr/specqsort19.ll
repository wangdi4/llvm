...
; because the key loop PHIs are not OK.

; CHECK: FXNREC: SPEC_QSORT: spec_qsort: Key loop PHIs not OK.
...
loop:                                             ; preds = %if.then292, %entry
  %n.addr.0 = phi i64 [ %n, %entry ], [ %div296, %if.then292 ]
...
if.then292:                                       ; preds = %if.end286
  %div296 = udiv i64 %es, %sub.ptr.sub289
