...
; UNSUPPORTED: windows
; RUN: opt < %s -opaque-pointers -enable-dtrans -functionrecognizer -debug-only=functionrecognizer,functionrecognizer-verbose -S 2>&1 | FileCheck %s
; RUN: opt < %s -opaque-pointers -enable-dtrans -passes='function(functionrecognizer)' -debug-only=functionrecognizer,functionrecognizer-verbose -S 2>&1 | FileCheck %s

; Test that on Linux @spec_qsort is not recognized as a qsort spec_qsort,
; because it has a malformed first MIN computation.

; CHECK: FXNREC: SPEC_QSORT: spec_qsort: No first MIN computation.
...
if.then58:                                        ; preds = %if.then53
  ...
  %call64 = call ptr @med3(ptr %a.addr.0, ptr %add.ptr61, ptr %add.ptr63, ptr %cmp)
  ...
  %call68 = call ptr @med3(ptr %add.ptr66, ptr %add.ptr50, ptr %add.ptr67, ptr %cmp)
  ...
  %call74 = call ptr @med3(ptr %add.ptr71, ptr %add.ptr73, ptr %add.ptr55, ptr %cmp)
  br label %if.end75
...
if.else212:                                       ; preds = %if.else203
  ...
  %add.ptr214 = getelementptr inbounds i8, ptr %pl.2, i64 %idx.neg213
  %conv215 = trunc i64 %es to i32
  call void @swapfunc(ptr %pl.2, ptr %add.ptr214, i32 %conv215, i32 %cond4, i32 %cond17)
  br label %for.inc218
...
if.end225:                                        ; preds = %for.end173
  ...
  %sub.ptr.lhs.cast228 = ptrtoint ptr %a.addr.0 to i64
...
if.then247:                                       ; preds = %cond.end243
  ...
  call void @swapfunc(ptr %a.addr.0, ptr %add.ptr249, i32 %conv250, i32 %cond4, i32 %cond17)
  br label %if.end251
...
if.then274:                                       ; preds = %cond.end270
  ...
  call void @swapfunc(ptr %pb.1, ptr %add.ptr276, i32 %conv277, i32 %cond4, i32 %cond17)
  br label %if.end278
