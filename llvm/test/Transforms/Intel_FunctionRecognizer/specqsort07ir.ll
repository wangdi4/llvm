...
; This is the same test as specqsort07.ll, but does not require asserts.
...
for.cond95:                                       ; preds = %if.end169, %if.end90
  ...
  %pd.0 = phi ptr [ %add.ptr91, %if.end90 ], [ %pd.1, %if.end169 ]
  %pc.0 = phi ptr [ %add.ptr91, %if.end90 ], [ %add.ptr172, %if.end169 ]
  %pb.0 = phi ptr [ %add.ptr94, %if.end90 ], [ %add.ptr170, %if.end169 ]
  %pa.0 = phi ptr [ %add.ptr94, %if.end90 ], [ %pa.1, %if.end169 ]
  br label %while.cond
