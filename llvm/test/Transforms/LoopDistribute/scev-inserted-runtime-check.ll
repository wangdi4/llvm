...
<<<<<<< HEAD
; RUN: opt -aa-pipeline=basic-aa -passes=loop-distribute -enable-loop-distribute -S -enable-mem-access-versioning=0 < %s | FileCheck %s
=======
; RUN: opt -aa-pipeline=basic-aa -opaque-pointers=0 -passes=loop-distribute -enable-loop-distribute -S -enable-mem-access-versioning=0 < %s | FileCheck %s
