...
; RUN: opt < %s -instcombine -disable-select-fcmp-min-max-combine -S | FileCheck %s --check-prefixes=DISABLE-SELECT
...
;
; DISABLE-SELECT-LABEL: @test_or(
; DISABLE-SELECT-NEXT:  entry:
; DISABLE-SELECT-NEXT:    [[S0:%.*]] = fsub fast float [[A0:%.*]], [[B0:%.*]]
; DISABLE-SELECT-NEXT:    [[Z0:%.*]] = fmul fast float [[S0]], [[C0:%.*]]
; DISABLE-SELECT-NEXT:    [[S1:%.*]] = fsub fast float [[A1:%.*]], [[B1:%.*]]
; DISABLE-SELECT-NEXT:    [[Z1:%.*]] = fmul fast float [[S1]], [[C1:%.*]]
; DISABLE-SELECT-NEXT:    [[S2:%.*]] = fsub fast float [[A2:%.*]], [[B2:%.*]]
; DISABLE-SELECT-NEXT:    [[Z2:%.*]] = fmul fast float [[S2]], [[C2:%.*]]
; DISABLE-SELECT-NEXT:    [[O0:%.*]] = fcmp fast olt float [[Z0]], 0.000000e+00
; DISABLE-SELECT-NEXT:    [[O1:%.*]] = fcmp fast olt float [[Z1]], 0.000000e+00
; DISABLE-SELECT-NEXT:    [[OR0:%.*]] = select i1 [[O0]], i1 true, i1 [[O1]]
; DISABLE-SELECT-NEXT:    [[O2:%.*]] = fcmp fast olt float [[Z2]], 0.000000e+00
; DISABLE-SELECT-NEXT:    [[OR1:%.*]] = select i1 [[OR0]], i1 true, i1 [[O2]]
; DISABLE-SELECT-NEXT:    br i1 [[OR1]], label [[T:%.*]], label [[F:%.*]]
; DISABLE-SELECT:       t:
; DISABLE-SELECT-NEXT:    ret i1 false
; DISABLE-SELECT:       f:
; DISABLE-SELECT-NEXT:    ret i1 true
;
entry:
...
; DISABLE-SELECT-NEXT:    [[O0:%.*]] = fcmp fast olt float [[Z0]], 0.000000e+00
; DISABLE-SELECT-NEXT:    [[O1:%.*]] = fcmp fast olt float [[Z1]], 0.000000e+00
; DISABLE-SELECT-NEXT:    [[OR0:%.*]] = select i1 [[O0]], i1 [[O1]], i1 false
; DISABLE-SELECT-NEXT:    [[O2:%.*]] = fcmp fast olt float [[Z2]], 0.000000e+00
; DISABLE-SELECT-NEXT:    [[OR1:%.*]] = select i1 [[OR0]], i1 [[O2]], i1 false
; DISABLE-SELECT-NEXT:    br i1 [[OR1]], label [[T:%.*]], label [[F:%.*]]
; DISABLE-SELECT:       t:
; DISABLE-SELECT-NEXT:    ret i1 false
; DISABLE-SELECT:       f:
