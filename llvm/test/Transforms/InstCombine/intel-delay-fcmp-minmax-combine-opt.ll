...
; RUN: opt -passes=instcombine,slp-vectorizer,instcombine -mtriple=i686-linux -mattr=+avx512f,+avx512vl,+avx512dq -enable-intel-advanced-opts=true -mattr=-prefer-256-bit %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK,ENABLE
; RUN: opt -passes=instcombine,slp-vectorizer,instcombine -disable-fcmp-min-max-combine=true -mtriple=i686-linux -mattr=+avx512f,+avx512vl,+avx512dq -enable-intel-advanced-opts=true -mattr=-prefer-256-bit %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK,DISABLE
...
; DISABLE-NEXT:    [[TMP2:%.*]] = insertelement <4 x float> undef, float [[M]], i32 0
