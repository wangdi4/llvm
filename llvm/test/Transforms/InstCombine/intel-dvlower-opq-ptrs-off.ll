; RUN: opt -opaque-pointers -instcombine -instcombine-preserve-for-dtrans=true < %s -S 2>&1 | FileCheck %s
...
; CHECK: %phys_prop_mp_physprop__fetch.6 = load %"QNCA_a0$i32*$rank1$", ptr @phys_prop_mp_physprop_, align 8
; CHECK: store %"QNCA_a0$i32*$rank1$" %phys_prop_mp_physprop__fetch.6, ptr %"foo_$MYB", align 1
