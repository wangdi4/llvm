...
; INTEL_CUSTOMIZATION
; INTEL: 52292: The fpclassify folding that is based on "nofpclass" has been
; disabled. We set nofpclass(nan,inf) by default and that causes problems
...
; CHECK-NEXT:    ret float [[ARG]] ;INTEL
;
  ret float %arg
}
...
; CHECK-NEXT:    ret <2 x float> [[ARG]] ;INTEL
;
  ret <2 x float> %arg
}
...
; CHECK-NEXT:    ret float 0x7FF0000000000000 ;INTEL
;
...
; CHECK-NEXT:    ret float 0xFFF0000000000000 ;INTEL
;
  ret float 0xFFF0000000000000
}
...
; CHECK-NEXT:    [[SELECT:%.*]] = select i1 [[COND]], float [[X]], float [[Y]] ;INTEL
; CHECK-NEXT:    ret float [[SELECT]] ;INTEL
;
  %select = select i1 %cond, float %x, float %y
  ret float %select
}
...
; CHECK-NEXT:    [[SELECT:%.*]] = select i1 [[COND]], [3 x [2 x float]] [[X]], [3 x [2 x float]] [[Y]] ;INTEL
; CHECK-NEXT:    ret [3 x [2 x float]] [[SELECT]] ;INTEL
;
...
; CHECK-NEXT:    [[SELECT:%.*]] = select i1 [[COND]], float 0x7FF0000000000000, float [[X]] ;INTEL
; CHECK-NEXT:    ret float [[SELECT]] ;INTEL
;
...
; CHECK-SAME: (i1 [[COND:%.*]], float [[X:%.*]]) {
; CHECK-NEXT:    [[SELECT:%.*]] = select i1 [[COND]], float [[X]], float 0x7FF0000000000000 ;INTEL
; CHECK-NEXT:    ret float [[SELECT]] ;INTEL
;
  %select = select i1 %cond, float %x, float 0x7FF0000000000000
  ret float %select
...
; CHECK-SAME: (i1 [[COND:%.*]], float [[X:%.*]]) {
; CHECK-NEXT:    [[SELECT:%.*]] = select i1 [[COND]], float 0x7FF0000000000000, float 0xFFF0000000000000 ;INTEL
; CHECK-NEXT:    ret float [[SELECT]] ;INTEL
;
...
; CHECK-SAME: (i1 [[COND:%.*]], float [[X:%.*]]) {
; CHECK-NEXT:    [[SELECT:%.*]] = select i1 [[COND]], float 0xFFF0000000000000, float 0x7FF0000000000000 ;INTEL
; CHECK-NEXT:    ret float [[SELECT]] ;INTEL
;
...
; CHECK-SAME: (i1 [[COND:%.*]], float [[X:%.*]]) {
; CHECK-NEXT:    [[SELECT:%.*]] = select i1 [[COND]], float 0.000000e+00, float -0.000000e+00 ;INTEL
; CHECK-NEXT:    ret float [[SELECT]] ;INTEL
;
...
; CHECK-NEXT:    [[SELECT:%.*]] = select <2 x i1> [[COND]], <2 x float> <float 0x7FF0000000000000, float 0x7FF0000000000000>, <2 x float> [[X]] ;INTEL
; CHECK-NEXT:    ret <2 x float> [[SELECT]] ;INTEL
;
...
; CHECK-SAME: (<2 x i1> [[COND:%.*]], <2 x float> [[X:%.*]]) {
; CHECK-NEXT:    [[SELECT:%.*]] = select <2 x i1> [[COND]], <2 x float> <float 0x7FF0000000000000, float poison>, <2 x float> [[X]] ;INTEL
; CHECK-NEXT:    ret <2 x float> [[SELECT]] ;INTEL
;
...
; CHECK-SAME: (<2 x i1> [[COND:%.*]], <2 x float> [[X:%.*]]) {
; CHECK-NEXT:    [[SELECT:%.*]] = select <2 x i1> [[COND]], <2 x float> <float 0x7FF0000000000000, float 0xFFF0000000000000>, <2 x float> [[X]] ;INTEL
; CHECK-NEXT:    ret <2 x float> [[SELECT]] ;INTEL
;
...
; CHECK-SAME: (i1 [[COND:%.*]], float [[X:%.*]]) {
; CHECK-NEXT:    [[SELECT1:%.*]] = select i1 [[COND]], float 0x7FF0000000000000, float [[X]] ;INTEL
; CHECK-NEXT:    ret float [[SELECT1]] ;INTEL
;
...
; CHECK-SAME: (i1 [[COND:%.*]], float [[X:%.*]]) {
; CHECK-NEXT:    [[SELECT1:%.*]] = select i1 [[COND]], float 0x7FF0000000000000, float 0x7FF8000000000000 ;INTEL
; CHECK-NEXT:    ret float [[SELECT1]] ;INTEL
;
...
; CHECK-SAME: (i1 [[COND:%.*]], float [[X:%.*]]) {
; CHECK-NEXT:    [[SELECT1:%.*]] = select i1 [[COND]], float 0x7FF8000000000000, float 0x7FF0000000000000 ;INTEL
; CHECK-NEXT:    ret float [[SELECT1]] ;INTEL
;
  %select0 = select i1 %cond, float 0x7FF8000000000000, float %x
...
; CHECK-SAME: (i1 [[COND:%.*]], float [[X:%.*]]) {
; CHECK-NEXT:    [[SELECT:%.*]] = select i1 [[COND]], float [[X]], float 0xFFF0000000000000 ;INTEL
; CHECK-NEXT:    [[FABS:%.*]] = call float @llvm.fabs.f32(float [[SELECT]]) ;INTEL
...
; CHECK-NEXT:    [[SELECT:%.*]] = select i1 [[COND]], float [[X]], float 0x7FF0000000000000 ;INTEL
; CHECK-NEXT:    [[FABS:%.*]] = call float @llvm.fabs.f32(float [[SELECT]]) ;INTEL
; CHECK-NEXT:    ret float [[FABS]] ;INTEL
;
  %select = select i1 %cond, float %x, float 0x7FF0000000000000
  %fabs = call float @llvm.fabs.f32(float %select)
  ret float %fabs
...
; CHECK-SAME: (i1 [[COND:%.*]], float [[X:%.*]]) {
; CHECK-NEXT:    [[SELECT:%.*]] = select i1 [[COND]], float [[X]], float 0xFFF0000000000000 ;INTEL
; CHECK-NEXT:    [[FNEG:%.*]] = fneg float [[SELECT]] ;INTEL
...
; CHECK-SAME: (i1 [[COND:%.*]], float [[X:%.*]]) {
; CHECK-NEXT:    [[SELECT:%.*]] = select i1 [[COND]], float 0xFFF0000000000000, float [[X]] ;INTEL
; CHECK-NEXT:    [[FNEG:%.*]] = fneg float [[SELECT]] ;INTEL
; CHECK-NEXT:    ret float [[FNEG]]
;
...
; CHECK-NEXT:    [[SELECT:%.*]] = select i1 [[COND]], float [[X]], float 0x7FF0000000000000 ;INTEL
; CHECK-NEXT:    [[COPYSIGN:%.*]] = call float @llvm.copysign.f32(float [[SELECT]], float [[UNKNOWN_SIGN]]) ;INTEL
...
; CHECK-SAME: (i1 [[COND:%.*]], float [[X:%.*]]) {
; CHECK-NEXT:    [[SELECT:%.*]] = select i1 [[COND]], float [[X]], float 0x7FF0000000000000 ;INTEL
; CHECK-NEXT:    [[COPYSIGN:%.*]] = call float @llvm.fabs.f32(float [[SELECT]]) ;INTEL
; CHECK-NEXT:    ret float [[COPYSIGN]]
;
  %select = select i1 %cond, float %x, float 0x7FF0000000000000
...
; CHECK-SAME: (i1 [[COND:%.*]], float [[X:%.*]]) {
; CHECK-NEXT:    [[SELECT:%.*]] = select i1 [[COND]], float [[X]], float 0x7FF0000000000000 ;INTEL
; CHECK-NEXT:    [[TMP1:%.*]] = call float @llvm.fabs.f32(float [[SELECT]]) ;INTEL
...
; CHECK-NEXT:    [[COPYSIGN:%.*]] = call float @llvm.copysign.f32(float [[X]], float [[UNKNOWN_SIGN]]) ;INTEL
; CHECK-NEXT:    ret float [[COPYSIGN]]
;
...
; CHECK-SAME: (float [[X:%.*]], float [[UNKNOWN_SIGN:%.*]]) {
; CHECK-NEXT:    [[COPYSIGN:%.*]] = call nnan float @llvm.copysign.f32(float [[X]], float [[UNKNOWN_SIGN]]) ;INTEL
; CHECK-NEXT:    ret float [[COPYSIGN]]
;
...
; CHECK-SAME: (float [[X:%.*]]) {
; CHECK-NEXT:    [[IS_GT_ZERO:%.*]] = fcmp ogt float [[X]], 0.000000e+00 ;INTEL
; CHECK-NEXT:    [[SELECT:%.*]] = select i1 [[IS_GT_ZERO]], float 0.000000e+00, float [[X]] ;INTEL
; CHECK-NEXT:    ret float [[SELECT]] ;INTEL
;
  %is.gt.zero = fcmp ogt float %x, 0.0
  %select = select i1 %is.gt.zero, float 0.0, float %x
...
; CHECK-NEXT:    [[I4:%.*]] = fcmp olt float [[Y]], 0.000000e+00 ;INTEL
; CHECK-NEXT:    [[I5:%.*]] = select i1 [[I4]], float 0x7FF0000000000000, float 0.000000e+00 ;INTEL
; CHECK-NEXT:    [[I6:%.*]] = fcmp oeq float [[X]], 0.000000e+00
; CHECK-NEXT:    [[I7:%.*]] = select i1 [[I6]], float [[I5]], float [[I3]] ;INTEL
; CHECK-NEXT:    [[I8:%.*]] = fcmp oeq float [[Y]], 0.000000e+00
; CHECK-NEXT:    [[I9:%.*]] = select i1 [[I6]], float 0x7FF8000000000000, float 1.000000e+00 ;INTEL
; CHECK-NEXT:    [[I10:%.*]] = select i1 [[I8]], float [[I9]], float [[I7]] ;INTEL
...
; CHECK-NEXT:    [[I7:%.*]] = fcmp olt float [[I4]], 0.000000e+00 ;INTEL
; CHECK-NEXT:    [[I8:%.*]] = select i1 [[I7]], float 0x7FF0000000000000, float 0.000000e+00 ;INTEL
; CHECK-NEXT:    [[I9:%.*]] = fcmp ueq float [[I4]], 0.000000e+00 ;INTEL
; CHECK-NEXT:    [[I10:%.*]] = fcmp oeq float [[I2]], 0.000000e+00
; CHECK-NEXT:    [[I11:%.*]] = select i1 [[I9]], float 0x7FF8000000000000, float [[I8]] ;INTEL
; CHECK-NEXT:    [[I12:%.*]] = select i1 [[I10]], float [[I11]], float [[I6]] ;INTEL
...
; CHECK-NEXT:    [[I14:%.*]] = fcmp olt float [[ARG]], 0.000000e+00 ;INTEL
; CHECK-NEXT:    [[I15:%.*]] = select i1 [[I7]], float [[I13]], float 0x7FF8000000000000 ;INTEL
; CHECK-NEXT:    [[I16:%.*]] = select i1 [[I14]], float [[I15]], float [[I13]] ;INTEL
; CHECK-NEXT:    [[I17:%.*]] = fcmp oeq float [[ARG]], 0.000000e+00
; CHECK-NEXT:    [[I18:%.*]] = fcmp olt float [[ARG1]], 0.000000e+00 ;INTEL
; CHECK-NEXT:    [[I19:%.*]] = xor i1 [[I17]], [[I18]] ;INTEL
; CHECK-NEXT:    [[I20:%.*]] = select i1 [[I19]], float 0.000000e+00, float 0x7FF0000000000000 ;INTEL
; CHECK-NEXT:    [[I21:%.*]] = select i1 [[I11]], float [[ARG]], float 0.000000e+00
; CHECK-NEXT:    [[I22:%.*]] = tail call noundef nofpclass(nan sub norm) float @llvm.copysign.f32(float noundef [[I20]], float noundef [[I21]]) ;INTEL
; CHECK-NEXT:    [[I23:%.*]] = select i1 [[I17]], float [[I22]], float [[I16]] ;INTEL
...
; CHECK-NEXT:    [[SELECT:%.*]] = select i1 [[COND]], float [[MUST_BE_INF]], float [[Y]] ;INTEL
; CHECK-NEXT:    ret float [[SELECT]] ;INTEL
;
...
; CHECK-SAME: (i1 [[COND:%.*]], float [[Y:%.*]]) {
; CHECK-NEXT:    [[MUST_BE_INF:%.*]] = call nofpclass(nan zero sub norm) float @extern()
; CHECK-NEXT:    ret float [[MUST_BE_INF]] ;INTEL
;
  %must.be.inf = call nofpclass(nan norm zero sub) float @extern()
...
; CHECK-NEXT:    ret float [[MUST_BE_ZERO]] ;INTEL
;
...
; CHECK-NEXT:    br i1 [[LOOP_COND]], label [[RET]], label [[LOOP]]
; CHECK:       ret:
; CHECK-NEXT:    [[PHI_RET:%.*]] = phi float [ 0.000000e+00, [[ENTRY:%.*]] ], [ 0x7FF0000000000000, [[LOOP]] ] ;INTEL
; CHECK-NEXT:    ret float [[PHI_RET]] ;INTEL
;
entry:
  br i1 %cond0, label %loop, label %ret
...
; CHECK-SAME: (i1 [[COND:%.*]], float [[X:%.*]]) {
; CHECK-NEXT:    [[SELECT:%.*]] = select i1 [[COND]], float [[X]], float 0x7FF0000000000000 ;INTEL
; CHECK-NEXT:    [[FENCE:%.*]] = call float @llvm.arithmetic.fence.f32(float [[SELECT]]) ;INTEL
