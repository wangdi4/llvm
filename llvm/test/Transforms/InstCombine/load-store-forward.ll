...
; RUN: opt -S -opaque-pointers -passes=instcombine < %s | FileCheck %s --check-prefixes=CHECK,LITTLE
; RUN: opt -S -opaque-pointers -passes=instcombine -data-layout="E" < %s | FileCheck %s --check-prefixes=CHECK,BIG
...
; CHECK-NEXT:    call void @llvm.memset.p0.i64(ptr noundef nonnull align 1 dereferenceable(16) [[A:%.*]], i8 0, i64 16, i1 false)
; CHECK-NEXT:    [[GEP:%.*]] = getelementptr i32, ptr [[A]], i64 1
