...
; RUN: opt -opaque-pointers=0 %s -passes='loop-load-elim,instcombine' -S | FileCheck %s
; RUN: opt -opaque-pointers=0 %s -bugpoint-enable-legacy-pm -instcombine -S | FileCheck %s
; RUN: opt -opaque-pointers=0 %s -passes='lto<O3>' -S | FileCheck --check-prefix=LTOO3 %s
