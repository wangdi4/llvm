; RUN: opt -opaque-pointers=0 %s -sycl-enable-vector-variant-passes -passes=sycl-kernel-create-simd-variant-propagation -S -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: opt -opaque-pointers=0 %s -sycl-enable-vector-variant-passes -passes=sycl-kernel-create-simd-variant-propagation -S | FileCheck %s
