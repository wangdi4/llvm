...
; RUN: opt -opaque-pointers=0 %s -sycl-enable-direct-function-call-vectorization=true -passes=sycl-kernel-sg-size-collector -S -enable-debugify -disable-output 2>&1 | FileCheck %s -check-prefix=DEBUGIFY
; RUN: opt -opaque-pointers=0 %s -sycl-enable-direct-function-call-vectorization=true -passes=sycl-kernel-sg-size-collector -S | FileCheck %s
