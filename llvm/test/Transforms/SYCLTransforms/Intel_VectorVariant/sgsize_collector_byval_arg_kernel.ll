; RUN: opt -opaque-pointers=0 %s -sycl-enable-byval-byref-function-call-vectorization -passes=sycl-kernel-sg-size-collector -S | FileCheck %s
; RUN: opt -opaque-pointers=0 %s -passes=sycl-kernel-sg-size-collector -S | FileCheck %s
; RUN: opt -opaque-pointers=0 %s -enable-debugify -disable-output -sycl-enable-byval-byref-function-call-vectorization -passes=sycl-kernel-sg-size-collector -S 2>&1 | FileCheck %s -check-prefix=DEBUGIFY
