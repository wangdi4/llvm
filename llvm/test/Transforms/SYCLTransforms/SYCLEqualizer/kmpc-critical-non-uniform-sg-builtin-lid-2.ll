...
%"QNCA_a0$double*$rank1$.0" = type { double addrspace(4)*, i64, i64, i64, i64, i64, [1 x %structtype.3] }
%structtype.3 = type { i64, i64, i64 }

@__omp_offloading_entries_table = addrspace(1) constant [3 x %struct.__tgt_offload_entry] [%struct.__tgt_offload_entry { i8 addrspace(4)* addrspacecast (i8 addrspace(1)* null to i8 addrspace(4)*), i8 addrspace(2)* null, i64 3888, i32 0, i32 0, i64 53 }, %struct.__tgt_offload_entry { i8 addrspace(4)* addrspacecast (i8 addrspace(1)* null to i8 addrspace(4)*), i8 addrspace(2)* null, i64 8, i32 0, i32 0, i64 45 }, %struct.__tgt_offload_entry { i8 addrspace(4)* null, i8 addrspace(2)* null, i64 0, i32 0, i32 0, i64 40 }]
...
define spir_kernel void @__omp_offloading_801_663ae85_MAIN___l18(%"QNCA_a0$double*$rank1$.0" addrspace(1)* noalias %"ascastB$target_reduction_array_dp_1d_$ARRAY", %"QNCA_a0$double*$rank1$.0" addrspace(1)* noalias %"ascastB$target_reduction_array_dp_1d_$ASUM", i64 %omp.pdo.norm.lb.val389.zext, i64 %omp.pdo.norm.ub.val.zext, i64 %"ascast$target_reduction_array_dp_1d_$I.val.zext") {
newFuncRoot:
  %"ascastB$target_reduction_array_dp_1d_$ASUM.addr_a0$_fetch.46356.red.gep" = getelementptr [2 x double], [2 x double]* null, i64 0, i64 0
...
DIR.OMP.END.DISTRIBUTE.PARLOOP.4363.split.split379.split.clone: ; preds = %newFuncRoot
  call spir_func void @__kmpc_critical([8 x i32] addrspace(4)* null)
...
.lr.ph:                                           ; preds = %DIR.OMP.END.DISTRIBUTE.PARLOOP.4363.split.split379.split.clone
  %"ascastB$target_reduction_array_dp_1d_$ASUM.addr_a0$_fetch.46356.cast.plus.offset.clone" = getelementptr [1 x double], [1 x double] addrspace(4)* null, i64 0, i64 0
  br label %1
...
DIR.OMP.END.DISTRIBUTE.PARLOOP.4363.split.split.split.clone.split: ; preds = %DIR.OMP.END.DISTRIBUTE.PARLOOP.4363.split.split.split.clone.split.loopexit, %DIR.OMP.END.DISTRIBUTE.PARLOOP.4363.split.split379.split.clone
  call spir_func void @__kmpc_end_critical([8 x i32] addrspace(4)* null)
...
red.update.body.clone:                            ; preds = %red.update.body.clone, %red.update.body.clone.preheader
  %red.cpy.dest.ptr375.clone = phi double addrspace(4)* [ %red.cpy.dest.inc376.clone, %red.update.body.clone ], [ %"ascastB$target_reduction_array_dp_1d_$ASUM.addr_a0$_fetch.46356.cast.plus.offset.clone", %red.update.body.clone.preheader ]
  %red.cpy.src.ptr.clone = phi double* [ %red.cpy.src.inc.clone, %red.update.body.clone ], [ %"ascastB$target_reduction_array_dp_1d_$ASUM.addr_a0$_fetch.46356.red.gep", %red.update.body.clone.preheader ]
  %3 = call spir_func double @_Z32sub_group_non_uniform_reduce_addd(double 0.000000e+00)
  %red.cpy.dest.inc376.clone = getelementptr double, double addrspace(4)* %red.cpy.dest.ptr375.clone, i64 1
  %red.cpy.src.inc.clone = getelementptr double, double* %red.cpy.src.ptr.clone, i64 1
