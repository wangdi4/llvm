...
; RUN: opt -opaque-pointers=0 -sycl-kernel-builtin-lib=%t.rtl.bc -passes='require<sycl-kernel-builtin-info-analysis>,sycl-kernel-sin-cos-fold' -S < %s -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: opt -opaque-pointers=0 -sycl-kernel-builtin-lib=%t.rtl.bc -passes='require<sycl-kernel-builtin-info-analysis>,sycl-kernel-sin-cos-fold' -S < %s | FileCheck %s
