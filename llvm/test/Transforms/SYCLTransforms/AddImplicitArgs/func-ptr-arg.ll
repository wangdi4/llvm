...
; RUN: opt -opaque-pointers=0 -passes=sycl-kernel-add-implicit-args -enable-debugify -disable-output -S %s 2>&1 | FileCheck %s -check-prefix=DEBUGIFY
...
; CHECK-NONOPAQUE: call void @bar(i32 (i32, i32)* [[TMP1]], i8** [[BUF]], i8 addrspace(3)* noalias %pLocalMemBase, { i64, [3 x i64], [3 x i64], [2 x [3 x i64]], [3 x i64], {}*, {}*, [3 x i64], [2 x [3 x i64]], [3 x i64] }* noalias [[PWORKDIM:%.*]], i64* noalias [[PWGID:%.*]], [4 x i64] [[BASEGLBID:%.*]], i8* noalias [[PSPECIALBUF:%.*]], {}* noalias [[RUNTIMEHANDLE:%.*]])
; CHECK-OPAQUE:    call void @bar(ptr [[TMP1]], ptr [[BUF]], ptr addrspace(3) noalias %pLocalMemBase, ptr noalias [[PWORKDIM:%.*]], ptr noalias [[PWGID:%.*]], [4 x i64] [[BASEGLBID:%.*]], ptr noalias [[PSPECIALBUF:%.*]], ptr noalias [[RUNTIMEHANDLE:%.*]])
; CHECK-NEXT:      ret void
;
  %buf = alloca i8*
