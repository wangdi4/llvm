...
; CHECK: @t1
define void @t1( <4 x double> %arg1) {
entry:
  ret void
}

;; new func
;; double4 arg1 - expected alignment: 32
; CHECK: [[ARG0_BUFF_INDEX:%[a-zA-Z0-9]+]] = getelementptr i8, i8* %UniformArgs, i32 0
; CHECK-NEXT: [[ARG0_TYPECAST:%[a-zA-Z0-9]+]] = bitcast i8* [[ARG0_BUFF_INDEX]] to <4 x double>*
; CHECK-NEXT: %explicit_0 = load <4 x double>, <4 x double>* [[ARG0_TYPECAST]], align 32
;;implicit args
; CHECK: ret void

!sycl.kernels = !{!0}
!0 = !{void (<4 x double>)* @t1}
