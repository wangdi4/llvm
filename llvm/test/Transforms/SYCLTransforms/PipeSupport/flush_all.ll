...
; RUN: opt -sycl-kernel-builtin-lib=%t.rtl.bc -passes=always-inline,sycl-kernel-pipe-support %s -S -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: opt -sycl-kernel-builtin-lib=%t.rtl.bc -passes=always-inline,sycl-kernel-pipe-support %s -S | FileCheck %s
