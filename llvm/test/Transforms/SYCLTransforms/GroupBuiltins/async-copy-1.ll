...
define void @main(i32 %x, ptr addrspace(3) %localBuffer, ptr addrspace(1) %globalBuffer, i64 %count) nounwind {
  ...
  %y = xor i32 %x, %x
  call void @foo(i32 %x, ptr addrspace(3) %localBuffer, ptr addrspace(1) %globalBuffer, i64 %count)
...
; CHECK: %y = xor i32 %x, %x
; CHECK: call void @foo(i32 %x, ptr addrspace(3) %localBuffer, ptr addrspace(1) %globalBuffer, i64 %count)
; CHECK: call void @_Z17wait_group_eventsiP9ocl_event(i32 1, ptr %pEvent)
; CHECK: ret void
}

; CHECK: @foo
define void @foo(i32 %x, ptr addrspace(3) %localBuffer, ptr addrspace(1) %globalBuffer, i64 %count) nounwind {
  %pEvent = alloca ptr, align 8
  %event = call ptr @_Z29async_work_group_strided_copyPU3AS1cPU3AS3Kcmm9ocl_event(ptr addrspace(1) %globalBuffer, ptr addrspace(3) %localBuffer, i64 %count, i64 18, ptr null) nounwind
  store ptr %event, ptr %pEvent, align 8
  %y = xor i32 %x, %x
  call void @_Z17wait_group_eventsiP9ocl_event(i32 1, ptr %pEvent) nounwind
  ret void
...
; CHECK: %pEvent = alloca ptr, align 8
; CHECK: call void @_Z18work_group_barrierj(i32 1)
; CHECK: %event = call ptr @_Z29async_work_group_strided_copyPU3AS1cPU3AS3Kcmm9ocl_event(ptr addrspace(1) %globalBuffer, ptr addrspace(3) %localBuffer, i64 %count, i64 18, ptr null)
; CHECK: call void @dummy_barrier.
; CHECK: store ptr %event, ptr %pEvent, align 8
; CHECK: %y = xor i32 %x, %x
; CHECK: call void @_Z17wait_group_eventsiP9ocl_event(i32 1, ptr %pEvent)
