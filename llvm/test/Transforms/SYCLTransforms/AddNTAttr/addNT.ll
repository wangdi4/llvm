; RUN: opt -opaque-pointers=0 -passes=sycl-kernel-add-nt-attr -S < %s -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: opt -opaque-pointers=0 -passes=sycl-kernel-add-nt-attr -S < %s | FileCheck %s
