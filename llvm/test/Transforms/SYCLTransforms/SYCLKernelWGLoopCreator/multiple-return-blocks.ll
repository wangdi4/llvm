...
; CHECK-LEGACY: if.end:
; CHECK-LEGACY-NEXT: br label %UnifiedReturnBlock
; CHECK-LEGACY: if.end.2:
; CHECK-LEGACY-NEXT: br label %UnifiedReturnBlock
; CHECK-LEGACY: UnifiedReturnBlock:
; CHECK-LEGACY: ret void
; CHECK-LEGACY-NOT: ret void

; UnifyFunctionExitNodes is not enabled for new pass manager, and there is no
; unified return block emitted, so we just check the return blocks are
; unchanged.
; CHECK-NEW: if.end:
