...
; RUN: opt -opaque-pointers=0 -sycl-kernel-builtin-lib=%t.rtl.bc -passes=sycl-kernel-channel-pipe-transformation %s -S | FileCheck --implicit-check-not write_channel_intel %s
...
; CHECK: call i32 @__write_pipe_2_bl_fpga({{.*}} %[[CAST_FAR_PIPE]], {{.*}}, i32 1, i32 1)
;
; CHECK: %[[LOAD_STAR_PIPE:.*]] = load {{.*}} @[[PIPE_STAR]]
; CHECK: %[[CAST_STAR_PIPE:.*]] = bitcast %opencl.pipe_rw_t{{.*}} %[[LOAD_STAR_PIPE]] to %opencl.pipe_wo_t
; CHECK: call i32 @__write_pipe_2_bl_fpga({{.*}} %[[CAST_STAR_PIPE]], {{.*}}, i32 16, i32 8)
;
...
; CHECK: call i32 @__write_pipe_2_bl_fpga({{.*}} %[[CAST_BAR_PIPE_ARR]], {{.*}}, i32 4, i32 4)
;
...
; CHECK: call i32 @__write_pipe_2_bl_fpga({{.*}} %[[CAST_FAR_PIPE_ARR]], {{.*}}, i32 1, i32 1)
;
...
; CHECK: call i32 @__write_pipe_2_bl_fpga({{.*}} %[[CAST_STAR_PIPE_ARR]], {{.*}}, i32 16, i32 8)
...
entry:
  ...
  call void @_Z19write_channel_intel11ocl_channel3FooS_(%opencl.channel_t addrspace(1)* %7, %struct.Foo* byval(%struct.Foo) align 8 %st) #4
  ...
  call void @_Z19write_channel_intel11ocl_channel3FooS_(%opencl.channel_t addrspace(1)* %12, %struct.Foo* byval(%struct.Foo) align 8 %st) #4
...
; Function Attrs: convergent
declare void @_Z19write_channel_intel11ocl_channel3FooS_(%opencl.channel_t addrspace(1)*, %struct.Foo* byval(%struct.Foo) align 8) #2
