...
; RUN: opt -opaque-pointers=0 -sycl-kernel-builtin-lib=%t.rtl.bc -passes=sycl-kernel-channel-pipe-transformation %s -S | FileCheck --implicit-check-not helper2,helper3,helper4 %s
...
; CHECK: define spir_func void @helper(%opencl.pipe_rw_t addrspace(1)* %a, i32 %data)
