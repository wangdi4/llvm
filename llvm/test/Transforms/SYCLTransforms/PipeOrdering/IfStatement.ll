...
for.body:                                         ; preds = %for.cond
  %1 = load i32, ptr addrspace(1) %iters, align 4, !tbaa !7
  %cmp1 = icmp sgt i32 %1, 10
  br i1 %cmp1, label %if.then, label %if.else

if.then:                                          ; preds = %for.body
  %2 = load ptr addrspace(1), ptr addrspace(1) @ch.pipe, align 8, !tbaa !11
  %3 = load ptr addrspace(1), ptr addrspace(1) @ch, align 4, !tbaa !11
...
for.inc:                                          ; preds = %if.else, %if.then
  %i.1 = phi i32 [ %i.0, %if.then ], [ %inc, %if.else ]
  %inc2 = add nsw i32 %i.1, 1
  br label %for.cond
}
