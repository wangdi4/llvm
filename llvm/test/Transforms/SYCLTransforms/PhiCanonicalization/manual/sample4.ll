; RUN: opt -passes=sycl-kernel-phi-canonicalization %s -S -enable-debugify -disable-output 2>&1 | FileCheck %s -check-prefix=DEBUGIFY
; RUN: opt -passes=sycl-kernel-phi-canonicalization %s -S -o - | FileCheck %s
