...
; RUN: opt -opaque-pointers=0 -passes=sycl-kernel-coerce-types -mtriple x86_64-pc-linux -S %s -o - | FileCheck %s --check-prefixes=X64-LINUX,X64-LINUX-NONOPAQUE
...
; RUN: opt -opaque-pointers=0 -passes=sycl-kernel-coerce-types -opaque-pointers -mtriple x86_64-pc-linux -S %s -o - | FileCheck %s --check-prefixes=X64-LINUX,X64-LINUX-OPAQUE
