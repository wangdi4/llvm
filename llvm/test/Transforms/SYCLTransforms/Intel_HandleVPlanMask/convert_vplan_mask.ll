; RUN: opt -opaque-pointers=0 -passes=sycl-kernel-convert-vplan-mask %s -S -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: opt -opaque-pointers=0 -passes=sycl-kernel-convert-vplan-mask %s -S | FileCheck %s
