; RUN: opt -passes='require<sycl-kernel-builtin-info-analysis>,require<sycl-kernel-vec-dim-analysis>,sycl-kernel-vec-clone' -sycl-vector-variant-isa-encoding-override=AVX512Core -S %s -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s

; RUN: opt -passes='require<sycl-kernel-builtin-info-analysis>,require<sycl-kernel-vec-dim-analysis>,sycl-kernel-vec-clone' -sycl-vector-variant-isa-encoding-override=AVX512Core %s -S -o - | FileCheck %s --check-prefixes=CHECK,WithVecDimAnalysis

; RUN: opt -passes='sycl-kernel-vec-clone' -sycl-vector-variant-isa-encoding-override=AVX512Core %s -S -o - | FileCheck %s --check-prefixes=CHECK,WithoutVecDimAnalysis
