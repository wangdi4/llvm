...
; CHECK: WorkItemAnalysis for function recalculateEigenIntervals:
...
; CHECK-NEXT: UNI   br label %BB048
; CHECK-NEXT: RND   %BB048_loop_mask.0 = phi i1 [ %cmp33, %BB048.preheader ], [ %loop_mask67, %BB049 ]
; CHECK-NEXT: RND   %BB048_exit_mask.0 = phi i1 [ false, %BB048.preheader ], [ %1, %BB049 ]
; CHECK-NEXT: RND   %tmp9_prev = phi i32 [ undef, %BB048.preheader ], [ %pLoad109, %BB049 ]
; CHECK-NEXT: RND   %sub_prev = phi i32 [ undef, %BB048.preheader ], [ %sub, %BB049 ]
; CHECK-NEXT: UNI   %storemerge35_prev = phi i32 [ undef, %BB048.preheader ], [ %storemerge35, %BB049 ]
; CHECK-NEXT: RND   %BB048_Min = phi i1 [ %Mneg, %BB048.preheader ], [ %local_edge, %BB049 ]
; CHECK-NEXT: RND   %tmp13 = phi i32 [ %pLoad, %BB048.preheader ], [ %pLoad109, %BB049 ]
; CHECK-NEXT: RND   %currentIndex.034 = phi i32 [ %0, %BB048.preheader ], [ %sub, %BB049 ]
; CHECK-NEXT: UNI   %storemerge35 = phi i32 [ 1, %BB048.preheader ], [ %phitmp, %BB049 ]
...
; CHECK-NEXT: RND   %1 = or i1 %BB048_exit_mask.0, %who_left_tr
; CHECK-NEXT: RND   %loop_mask67 = or i1 %BB048_loop_mask.0, %who_left_tr
; CHECK-NEXT: RND   %shouldexit = call i1 @allOne3(i1 %loop_mask67) #0
; CHECK-NEXT: UNI   br label %BB049
; CHECK-NEXT: RND   %notCond = xor i1 %cmp, true
; CHECK-NEXT: RND   %local_edge = and i1 %BB048_Min, %notCond
; CHECK-NEXT: UNI   %phitmp = add i32 %storemerge35, 1
; CHECK-NEXT: RND   %leave = call i1 @allOne3(i1 %loop_mask67) #0
; CHECK-NEXT: RND   br i1 %leave, label %BB048, label %BB050.loopexit
; CHECK-NEXT: UNI   br label %BB050
; CHECK-NEXT: RND   %merge96 = select i1 %cmp33, i32 %0, i32 %out_sel102
; CHECK-NEXT: RND   %merge = select i1 %cmp33, i32 %pLoad, i32 %out_sel104
; CHECK-NEXT: RND   %BB050_Min87 = or i1 %1, %cmp33
; CHECK-NEXT: RND   %out_sel.op = shl i32 %out_sel, 1
...
; CHECK-NEXT: UNI   %cmp81.i = icmp ugt i32 %width, 1
; CHECK-NEXT: UNI   %Mneg69 = xor i1 %cmp81.i, true
; CHECK-NEXT: RND   %BB051_to_BB052 = and i1 %BB050_to_BB051, %Mneg69
; CHECK-NEXT: RND   %BB051_to_BB053 = and i1 %BB050_to_BB051, %cmp81.i
; CHECK-NEXT: UNI   br label %BB052
; CHECK-NEXT: RND   %conv.i42 = uitofp i1 %cmp.i to float
; CHECK-NEXT: RND   %sub.i443 = fsub float %pLoad112, %pLoad111
; CHECK-NEXT: RND   %cmp.i544 = fcmp olt float %sub.i443, 0.000000e+00
; CHECK-NEXT: RND   %cond.i645 = zext i1 %cmp.i544 to i32
; CHECK-NEXT: UNI   br label %BB053
; CHECK-NEXT: UNI   %tmp.i = add i32 %width, -1
; CHECK-NEXT: RND   %negIncomingLoopMask88 = xor i1 %BB051_to_BB053, true
; CHECK-NEXT: UNI   br label %BB054
; CHECK-NEXT: RND   %BB054_loop_mask.0 = phi i1 [ %negIncomingLoopMask88, %BB053 ], [ %loop_mask73, %BB054 ]
; CHECK-NEXT: RND   %BB054_exit_mask.0 = phi i1 [ false, %BB053 ], [ %2, %BB054 ]
; CHECK-NEXT: RND   %add33.i_prev = phi i32 [ undef, %BB053 ], [ %add33.i, %BB054 ]
; CHECK-NEXT: RND   %BB054_Min = phi i1 [ %BB051_to_BB053, %BB053 ], [ %local_edge76, %BB054 ]
; CHECK-NEXT: UNI   %indvar.i = phi i32 [ 0, %BB053 ], [ %tmp38, %BB054 ]
...
; CHECK-NEXT: RND   %cmp30.i = fcmp olt float %sub28.i, 0.000000e+00
; CHECK-NEXT: RND   %cond31.i = zext i1 %cmp30.i to i32
; CHECK-NEXT: RND   %add33.i = add i32 %cond31.i, %count.02.i
; CHECK-NEXT: UNI   %exitcond37 = icmp eq i32 %tmp38, %tmp.i
; CHECK-NEXT: UNI   %notCond70 = xor i1 %exitcond37, true
; CHECK-NEXT: RND   %who_left_tr71 = and i1 %BB054_Min, %exitcond37
; CHECK-NEXT: RND   %2 = or i1 %BB054_exit_mask.0, %who_left_tr71
; CHECK-NEXT: RND   %loop_mask73 = or i1 %BB054_loop_mask.0, %who_left_tr71
; CHECK-NEXT: RND   %shouldexit74 = call i1 @allOne3(i1 %loop_mask73) #0
; CHECK-NEXT: RND   %local_edge76 = and i1 %BB054_Min, %notCond70
; CHECK-NEXT: RND   br i1 %shouldexit74, label %BB055, label %BB054
; CHECK-NEXT: RND   %out_sel106 = select i1 %BB054_Min, i32 %add33.i, i32 %add33.i_prev
; CHECK-NEXT: RND   %conv.i = uitofp i32 %out_sel106 to float
; CHECK-NEXT: RND   %sub.i4 = fsub float %pLoad112, %pLoad111
; CHECK-NEXT: RND   %cmp.i5 = fcmp olt float %sub.i4, 0.000000e+00
; CHECK-NEXT: RND   %cond.i6 = zext i1 %cmp.i5 to i32
; CHECK-NEXT: UNI   %Mneg77 = xor i1 %cmp81.i, true
; CHECK-NEXT: RND   %BB055_to_BB058 = and i1 %2, %Mneg77
; CHECK-NEXT: RND   %BB055_to_BB056 = and i1 %2, %cmp81.i
; CHECK-NEXT: UNI   br label %BB063
; CHECK-NEXT: UNI   %tmp.i8 = add i32 %width, -1
; CHECK-NEXT: RND   %negIncomingLoopMask89 = xor i1 %BB055_to_BB056, true
; CHECK-NEXT: UNI   br label %BB057
; CHECK-NEXT: RND   %BB057_loop_mask.0 = phi i1 [ %negIncomingLoopMask89, %BB056 ], [ %loop_mask81, %BB057 ]
; CHECK-NEXT: RND   %BB057_exit_mask.0 = phi i1 [ false, %BB056 ], [ %3, %BB057 ]
; CHECK-NEXT: RND   %add33.i24_prev = phi i32 [ undef, %BB056 ], [ %add33.i24, %BB057 ]
; CHECK-NEXT: RND   %BB057_Min = phi i1 [ %BB055_to_BB056, %BB056 ], [ %local_edge84, %BB057 ]
; CHECK-NEXT: UNI   %indvar.i10 = phi i32 [ 0, %BB056 ], [ %tmp, %BB057 ]
...
; CHECK-NEXT: RND   %cmp30.i22 = fcmp olt float %sub28.i21, 0.000000e+00
; CHECK-NEXT: RND   %cond31.i23 = zext i1 %cmp30.i22 to i32
; CHECK-NEXT: RND   %add33.i24 = add i32 %cond31.i23, %count.02.i12
; CHECK-NEXT: RND   %out_sel108 = select i1 %BB057_Min, i32 %add33.i24, i32 %add33.i24_prev
; CHECK-NEXT: UNI   %exitcond = icmp eq i32 %tmp, %tmp.i8
; CHECK-NEXT: UNI   %notCond78 = xor i1 %exitcond, true
; CHECK-NEXT: RND   %who_left_tr79 = and i1 %BB057_Min, %exitcond
; CHECK-NEXT: RND   %3 = or i1 %BB057_exit_mask.0, %who_left_tr79
; CHECK-NEXT: RND   %loop_mask81 = or i1 %BB057_loop_mask.0, %who_left_tr79
; CHECK-NEXT: RND   %shouldexit82 = call i1 @allOne3(i1 %loop_mask81) #0
; CHECK-NEXT: RND   %local_edge84 = and i1 %BB057_Min, %notCond78
; CHECK-NEXT: RND   br i1 %shouldexit82, label %BB058.loopexit, label %BB057
; CHECK-NEXT: UNI   br label %phi-split-bb
; CHECK-NEXT: UNI   br label %BB058
; CHECK-NEXT: RND   %phi-split-bb_Min90 = or i1 %BB051_to_BB052, %3
; CHECK-NEXT: RND   %merge97 = select i1 %3, i32 %out_sel108, i32 %cond.i645
; CHECK-NEXT: RND   %merge98 = select i1 %3, float %conv.i, float %conv.i42
; CHECK-NEXT: RND   %merge100 = select i1 %BB055_to_BB058, i32 %cond.i6, i32 %merge97
; CHECK-NEXT: RND   %merge99 = select i1 %BB055_to_BB058, float %conv.i, float %merge98
; CHECK-NEXT: RND   %BB058_Min91 = or i1 %phi-split-bb_Min90, %BB055_to_BB058
; CHECK-NEXT: RND   %sub60 = fsub float %pLoad110, %pLoad111
; CHECK-NEXT: RND   %cmp62 = fcmp olt float %sub60, %tolerance
; CHECK-NEXT: RND   %Mneg85 = xor i1 %cmp62, true
...
; CHECK-NEXT: RND   %div121 = fdiv float %sub115, %conv
