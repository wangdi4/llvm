...
; RUN: opt -opaque-pointers=0 -sycl-kernel-builtin-lib=%t.rtl.bc -sycl-demangle-fpga-pipes -passes='sycl-kernel-rewrite-pipes,sycl-kernel-equalizer,sycl-kernel-pipe-io-transform' %s -S -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: opt -opaque-pointers=0 -sycl-kernel-builtin-lib=%t.rtl.bc -sycl-demangle-fpga-pipes -passes='sycl-kernel-rewrite-pipes,sycl-kernel-equalizer,sycl-kernel-pipe-io-transform' %s -S | FileCheck %s
