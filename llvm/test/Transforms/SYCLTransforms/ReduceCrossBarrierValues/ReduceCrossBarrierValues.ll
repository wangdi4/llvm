; RUN: opt -opaque-pointers=0 -S -sycl-barrier-copy-instruction-threshold=3 -passes="sycl-kernel-reduce-cross-barrier-values,adce" %s -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: opt -opaque-pointers=0 -S -sycl-barrier-copy-instruction-threshold=3 -passes="sycl-kernel-reduce-cross-barrier-values,adce" %s | FileCheck %s
...
; than the threshold.
define void @test_threshold(i8* %dst) {
...
; CHECK-NEXT: SyncBB:
; CHECK-NEXT:   call void @_Z7barrierj(i32 0)
; CHECK-NEXT:   %gep = getelementptr i8, i8* %dst, i64 %add3
...
SyncBB:
  call void @_Z7barrierj(i32 0)
  %gep = getelementptr i8, i8* %dst, i64 %add3
  store i8 1, i8* %gep
  ret void
}
...
define void @test_barrier_region(i8* %dst, i1 %c) {
...
; CHECK-NEXT:   %gepB = getelementptr i8, i8* %dst, i64 [[ID_B]]
; CHECK-NEXT:   store i8 1, i8* %gepB, align 1
; CHECK-NEXT:   br i1 %c, label %C, label %D
; CHECK-EMPTY:
; CHECK-NEXT: C:
; CHECK-NOT:    call
; CHECK-NEXT:   %gepC = getelementptr i8, i8* %dst, i64 [[ID_B]]
; CHECK-NEXT:   store i8 1, i8* %gepC, align 1
; CHECK-NEXT:   br label %E
; CHECK-EMPTY:
; CHECK-NEXT: E:
; CHECK-NEXT:   call void @_Z7barrierj(i32 0)
; CHECK-NEXT:   [[ID_E:%.*]] = call i64 @_Z12get_local_idj(i32 0)
; CHECK-NEXT:   %gepE = getelementptr i8, i8* %dst, i64 [[ID_E]]
; CHECK-NEXT:   store i8 1, i8* %gepE, align 1
; CHECK-NEXT:   br label %G
; CHECK-EMPTY:
; CHECK-NEXT: D:
; CHECK-NEXT:   [[ID_D:%.*]] = phi i64 [ [[ID_F:%.*]], %F ], [ [[ID_B]], %B ]
; CHECK-NEXT:   %iv = phi i64 [ 0, %B ], [ %iv.next, %F ]
; CHECK-NEXT:   %gepD = getelementptr i8, i8* %dst, i64 [[ID_D]]
; CHECK-NEXT:   store i8 1, i8* %gepD, align 1
; CHECK-NEXT:   br label %F
; CHECK-EMPTY:
; CHECK-NEXT: F:
; CHECK-NEXT:   call void @_Z7barrierj(i32 0)
; CHECK-NEXT:   [[ID_F]] = call i64 @_Z12get_local_idj(i32 0)
; CHECK-NEXT:   %gepF = getelementptr i8, i8* %dst, i64 [[ID_F]]
; CHECK-NEXT:   store i8 1, i8* %gepF, align 1
...
; CHECK-NEXT:   %gepG = getelementptr i8, i8* %dst, i64 [[ID_G]]
; CHECK-NEXT:   store i8 1, i8* %gepG, align 1
; CHECK-NEXT:   br label %H
; CHECK-EMPTY:
; CHECK-NEXT: H:
; CHECK-NEXT:   [[ID_H:%.*]] = phi i64 [ [[ID_G]], %G ], [ %id, %A ]
; CHECK-NEXT:   %gepH = getelementptr i8, i8* %dst, i64 [[ID_H]]
; CHECK-NEXT:   store i8 1, i8* %gepH, align 1
; CHECK-NEXT:   br label %I
; CHECK-EMPTY:
; CHECK-NEXT: I:
; CHECK-NOT:    call
; CHECK-NEXT:   %gepI = getelementptr i8, i8* %dst, i64 [[ID_H]]
; CHECK-NEXT:   store i8 1, i8* %gepI, align 1
; CHECK-NEXT:   ret void
A:
...
B:
  call void @_Z7barrierj(i32 0)
  %gepB = getelementptr i8, i8* %dst, i64 %id
  store i8 1, i8* %gepB
  br i1 %c, label %C, label %D

C:
  %gepC = getelementptr i8, i8* %dst, i64 %id
  store i8 1, i8* %gepC
  br label %E

E:
  call void @_Z7barrierj(i32 0)
  %gepE = getelementptr i8, i8* %dst, i64 %id
  store i8 1, i8* %gepE
  br label %G

D:
  %iv = phi i64 [ 0, %B ], [ %iv.next, %F ]
  %gepD = getelementptr i8, i8* %dst, i64 %id
  store i8 1, i8* %gepD
  br label %F

F:
  call void @_Z7barrierj(i32 0)
  %gepF = getelementptr i8, i8* %dst, i64 %id
  store i8 1, i8* %gepF
...
G:
  %gepG = getelementptr i8, i8* %dst, i64 %id
  store i8 1, i8* %gepG
  br label %H

H:
  %gepH = getelementptr i8, i8* %dst, i64 %id
  store i8 1, i8* %gepH
  br label %I

I:
  %gepI = getelementptr i8, i8* %dst, i64 %id
  store i8 1, i8* %gepI
  ret void
}

define void @test_multiple_uses(i8* %dst) {
...
; CHECK-NEXT:   %gep = getelementptr i8, i8* %dst, i64 %add
; CHECK-NEXT:   store i8 1, i8* %gep, align 1
...
; CHECK-NEXT:   %gep1 = getelementptr i8, i8* %dst, i64 %mul
; CHECK-NEXT:   %gep2 = getelementptr i8, i8* %dst, i64 %add2
; CHECK-NEXT:   store i8 1, i8* %gep1, align 1
; CHECK-NEXT:   store i8 2, i8* %gep2, align 1
; CHECK-NEXT:   ret void
entry:
  %id = call i64 @_Z12get_local_idj(i32 0)
  br label %SyncBB
...
SyncBB:
  call void @_Z7barrierj(i32 0)
  %add = add i64 %id, 1
  %gep = getelementptr i8, i8* %dst, i64 %add
  store i8 1, i8* %gep
  br label %SyncBB1
...
SyncBB1:
  ...
  %mul = mul i64 %add, 10
  %add2 = add i64 %id, 2
  %gep1 = getelementptr i8, i8* %dst, i64 %mul
  %gep2 = getelementptr i8, i8* %dst, i64 %add2
  store i8 1, i8* %gep1
  store i8 2, i8* %gep2
  ret void
}

define void @test_phi(i8* %dst, i1 %c) {
...
; CHECK-NEXT:   %add.copy = add i64 %id.copy, 1
; CHECK-NEXT:   %gep = getelementptr i8, i8* %dst, i64 %id.copy
; CHECK-NEXT:   store i8 1, i8* %gep, align 1
; CHECK-NEXT:   br label %ret
; CHECK-EMPTY:
; CHECK-NEXT: ret:
; CHECK-NEXT:   %offset = phi i64 [ %add.copy, %t ], [ %id, %entry ]
; CHECK-NEXT:   %gep1 = getelementptr i8, i8* %dst, i64 %offset
; CHECK-NEXT:   store i8 2, i8* %gep1, align 1
; CHECK-NEXT:   ret void
entry:
  %id = call i64 @_Z12get_local_idj(i32 0)
...
t:
  call void @_Z7barrierj(i32 0)
  %gep = getelementptr i8, i8* %dst, i64 %id
  store i8 1, i8* %gep
  br label %ret

ret:
  %offset = phi i64 [ %add, %t ], [ %id, %entry ]
  %gep1 = getelementptr i8, i8* %dst, i64 %offset
  store i8 2, i8* %gep1
  ret void
}
...
; remain untouched.
define void @test_unsafe(i8* %dst) {
; CHECK-LABEL: define void @test_unsafe
; CHECK-NEXT: entry:
; CHECK-NEXT:   %id = call i64 @_Z12get_local_idj(i32 0)
; CHECK-NEXT:   %f = call i64 @foo()
; CHECK-NEXT:   %gep0 = getelementptr i8, i8* %dst, i64 %id
; CHECK-NEXT:   %load = load i8, i8* %gep0
; CHECK-NEXT:   br label %st
; CHECK-EMPTY:
; CHECK-NEXT: st:
; CHECK-NEXT:   %gep1 = getelementptr i8, i8* %dst, i64 %f
; CHECK-NEXT:   store i8 1, i8* %gep1
; CHECK-NEXT:   br label %SyncBB
; CHECK-EMPTY:
; CHECK-NEXT: SyncBB:
; CHECK-NEXT:   call void @_Z7barrierj(i32 0)
; CHECK-NEXT:   %mul = mul i64 %f, %f
; CHECK-NEXT:   %gep2 = getelementptr i8, i8* %dst, i64 %mul
; CHECK-NEXT:   store i8 %load, i8* %gep2
; CHECK-NEXT:   ret void
entry:
  %id = call i64 @_Z12get_local_idj(i32 0)
  %f = call i64 @foo()
  %gep0 = getelementptr i8, i8* %dst, i64 %id
  %load = load i8, i8* %gep0
  br label %st

st:
  %gep1 = getelementptr i8, i8* %dst, i64 %f
  store i8 1, i8* %gep1
  br label %SyncBB
...
SyncBB:
  call void @_Z7barrierj(i32 0)
  %mul = mul i64 %f, %f
  %gep2 = getelementptr i8, i8* %dst, i64 %mul
  store i8 %load, i8* %gep2
  ret void
}

define void @test_dom_frontier(i8* %dst, i1 %c) {
...
; CHECK-NEXT:   %gep0 = getelementptr i8, i8* %dst, i64 %add.copy
; CHECK-NEXT:   store i8 1, i8* %gep0, align 1
...
; CHECK-NEXT:   %gep1 = getelementptr i8, i8* %dst, i64 %rem
; CHECK-NEXT:   store i8 2, i8* %gep1, align 1
; CHECK-NEXT:   ret void
entry:
  %id = call i64 @_Z12get_local_idj(i32 0)
...
SyncBB0:
  call void @_Z7barrierj(i32 0)
  %gep0 = getelementptr i8, i8* %dst, i64 %add
  store i8 1, i8* %gep0
  br label %ed
...
define void @test_dom_frontier_in_loop(i8* %dst, i1 %c) {
