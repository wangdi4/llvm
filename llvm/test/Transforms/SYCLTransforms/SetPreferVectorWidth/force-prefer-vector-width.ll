; RUN: opt -opaque-pointers=0 -sycl-force-prefer-vector-width=256 -passes=sycl-kernel-set-prefer-vector-width -S %s | FileCheck %s
; RUN: opt -opaque-pointers=0 -sycl-force-prefer-vector-width=256 -passes=sycl-kernel-set-prefer-vector-width -S %s -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
