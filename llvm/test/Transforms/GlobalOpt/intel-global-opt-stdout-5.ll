; CMPLRLLVM-24304: This test verifies that "nabout" in load instructions are
; replaced with "stdout" by globalopt.
; This is same as intel-global-opt-stdout-1.ll except BitCasts are not
; involved while modifying "nabout".

; RUN: opt < %s -passes='require<wholeprogram>,globalopt' -whole-program-assume -S | FileCheck %s

; CHECK-DAG: %l1 = load %struct._IO_FILE*, %struct._IO_FILE** @stdout, align 8
; CHECK-DAG: %l3 = load %struct._IO_FILE*, %struct._IO_FILE** @stdout, align 8

%struct._IO_FILE = type { i32, i8*, i8*, i8*, i8*, i8*, i8*, i8*, i8*, i8*, i8*, i8*, %struct._IO_marker*, %struct._IO_FILE*, i32, i32, i64, i16, i8, [1 x i8], i8*, i64, i8*, i8*, i8*, i8*, i64, i32, [20 x i8] }
%struct._IO_marker = type { %struct._IO_marker*, %struct._IO_FILE*, i32 }

@nabout = internal dso_local local_unnamed_addr global %struct._IO_FILE* null, align 8
@stdout = external dso_local local_unnamed_addr global %struct._IO_FILE*, align 8

define dso_local void @bar() {
entry:
  %l1 = load %struct._IO_FILE*, %struct._IO_FILE** @nabout, align 8
  %call1 = tail call i32 @fflush(%struct._IO_FILE* %l1)
  ret void
}

define dso_local i32 @main() {
entry:
  %l2 = load %struct._IO_FILE*, %struct._IO_FILE** @stdout, align 8
  store %struct._IO_FILE* %l2, %struct._IO_FILE** @nabout, align 8
  tail call void @bar()
  %l3 = load %struct._IO_FILE*, %struct._IO_FILE** @nabout, align 8
  %call1 = tail call i32 @fflush(%struct._IO_FILE* %l3)
  ret i32 0
}

declare dso_local i32 @fflush(%struct._IO_FILE* nocapture)
