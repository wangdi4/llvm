; RUN: opt -opaque-pointers -enable-new-pm=0 -argpromotion -S < %s | FileCheck %s --check-prefix=CHECK-ON
; RUN: opt -opaque-pointers -passes=argpromotion -S < %s | FileCheck %s --check-prefix=CHECK-ON
; RUN: opt -opaque-pointers -enable-new-pm=0 -argpromotion -argpro-single-level-recursive=false -S < %s | FileCheck %s --check-prefix=CHECK-OFF
...
; CHECK-ON: define internal noundef i32 @_Z3fooR9_MYSTRUCT(ptr %myarg.0.val, i32 %myarg.8.val)

; Check that arg promotion did not happen on the single argument recursive
; function @_Z3fooR9_MYSTRUCT because it was inhibited by the option
; -argpro-single-level-recursive=false.

; CHECK-OFF: define internal noundef i32 @_Z3fooR9_MYSTRUCT(ptr noundef nonnull align 8 dereferenceable(16) %myarg)
