...
; RUN: opt -opaque-pointers=0 < %s -passes=SYCLMutatePrintfAddrspace -S | FileCheck %s
; RUN: opt -opaque-pointers=0 < %s -passes=SYCLMutatePrintfAddrspace -S --enable-new-pm=1 | FileCheck %s
