; RUN: opt < %s -hir-cost-model-throttling=0 -hir-ssa-deconstruction -hir-avr-generate -hir-avr-predicate -debug -debug-only=avr-predicate -S -o /dev/null 2>&1| FileCheck %s
; REQUIRES: asserts
