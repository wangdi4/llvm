...
; RUN: opt -opaque-pointers=1 -passes='function(vpo-cfg-restructuring,print<vpo-wrncollection>)' -S -disable-output %s 2>&1 | FileCheck %s
...
; CHECK:       REDUCTION clause (size=2): (INSCAN<1>, UDR: BYREF(ptr %x.addr), TYPED (TYPE: %class.C = type { i32 }, NUM_ELEMENTS: i64 2, OFFSET: i64 5) )
; CHECK-SAME:                             (INSCAN<2>, ADD: ptr @y, TYPED (TYPE: i32, NUM_ELEMENTS: i32 1))
