; RUN: opt -opaque-pointers=1 -enable-new-pm=0 -vpo-wrncollection -analyze -S %s 2>&1 | FileCheck %s
...
;     init(prefer_type("level_zero", "opencl"), target, targetsync               \
;          : obj1)
; }
...
; CHECK-NEXT: DESTROY clause (size=1): (ptr %interop_obj2)
; CHECK-NEXT: INIT clause (size=1): (ptr %interop_obj) TARGET TARGETSYNC PREFER_TYPE < 6 (LEVEL0) 3 (OpenCL) >

; CHECK: BEGIN INTEROP ID=2 {
; CHECK:      DEVICE: UNSPECIFIED
; CHECK-NEXT: NOWAIT: false
; CHECK-NEXT: USE clause (size=1): (ptr %interop_obj)

; CHECK: BEGIN INTEROP ID=3 {
; CHECK:      DEVICE: UNSPECIFIED
; CHECK-NEXT: NOWAIT: false
; CHECK-NEXT: DESTROY clause (size=1): (ptr %interop_obj)
...
define dso_local i32 @foo(ptr %interop_obj, ptr %interop_obj2) #0 {
entry:
  %a = alloca i32, align 4
  %interop_obj.addr = alloca ptr, align 8
  store ptr %interop_obj, ptr %interop_obj.addr, align 8
  %interop_obj2.addr = alloca ptr, align 8
  store ptr %interop_obj2, ptr %interop_obj2.addr, align 8
  br label %DIR.OMP.INTEROP.1

DIR.OMP.INTEROP.1:                                ; preds = %entry
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.INTEROP"(),
    "QUAL.OMP.DESTROY"(ptr %interop_obj2),
    "QUAL.OMP.INIT:TARGET.TARGETSYNC.PREFER"(ptr %interop_obj, i32 6, i32 3) ]
...
DIR.OMP.INTEROP.7:                                ; preds = %DIR.OMP.INTEROP.6
  %2 = call token @llvm.directive.region.entry() [ "DIR.OMP.INTEROP"(),
    "QUAL.OMP.DESTROY"(ptr %interop_obj) ]
