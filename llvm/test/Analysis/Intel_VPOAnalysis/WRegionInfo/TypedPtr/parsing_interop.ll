; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -vpo-wrncollection -analyze -S %s 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers=0 -passes='function(print<vpo-wrncollection>)' -S %s 2>&1 | FileCheck %s
;
