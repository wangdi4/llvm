; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -vpo-wrncollection -analyze %s | FileCheck %s
; RUN: opt -opaque-pointers=0 -passes='function(print<vpo-wrncollection>)' -disable-output %s 2>&1 | FileCheck %s
