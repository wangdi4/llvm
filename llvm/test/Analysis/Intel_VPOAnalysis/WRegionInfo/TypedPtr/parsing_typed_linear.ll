; REQUIRES: asserts
; RUN: opt -opaque-pointers=0 -bugpoint-enable-legacy-pm -vpo-cfg-restructuring -vpo-wrncollection -analyze -S %s 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers=0 -passes='function(vpo-cfg-restructuring,print<vpo-wrncollection>)' -S %s 2>&1 | FileCheck %s

; This code tests TYPED for the LINEAR clause
; The test is passed if LINEAR:TYPED and PRIVATE:TYPED clauses are parsed correctly
...
; CHECK: LINEAR clause (size=1): (TYPED({{.*}}, TYPE: i32, NUM_ELEMENTS: i32 1), {{.*}}
...
entry:
  ...
  %3 = call token @llvm.directive.region.entry() [ "DIR.OMP.LOOP"(), "QUAL.OMP.LINEAR:TYPED"(i32* @y, i32 0, i32 1, i32 1), "QUAL.OMP.NORMALIZED.IV"(i32* %.omp.iv), "QUAL.OMP.FIRSTPRIVATE"(i32* %.omp.lb), "QUAL.OMP.NORMALIZED.UB"(i32* %.omp.ub), "QUAL.OMP.PRIVATE:TYPED"(i32* %i, i32 0, i32 1) ]
