; RUN: not opt -opaque-pointers=1 -bugpoint-enable-legacy-pm -vpo-cfg-restructuring -vpo-wrncollection -analyze -S %s 2>&1 | FileCheck %s
; RUN: not opt -opaque-pointers=1 -passes='function(vpo-cfg-restructuring,print<vpo-wrncollection>)' -S %s 2>&1 | FileCheck %s
...
; The IR is a hand-modified version of the above test using omp simd instead of omp for.
...
omp.inner.for.body:                               ; preds = %omp.inner.for.cond
  ...
  %6 = call token @llvm.directive.region.entry() [ "DIR.OMP.SCAN"(),
    "QUAL.OMP.EXCLUSIVE:TYPED"(ptr @x, i32 0, i32 1, i64 1) ]
  fence acq_rel
