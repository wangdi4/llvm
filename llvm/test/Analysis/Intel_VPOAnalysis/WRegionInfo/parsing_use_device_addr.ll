; REQUIRES: asserts
; RUN: opt -opaque-pointers=1 -bugpoint-enable-legacy-pm -vpo-wrncollection -analyze -debug-only=vpo-wrninfo -S %s 2>&1 | FileCheck %s
...
;     { printf("%p\n", &array_device[0]); } // end target data
;   }                                       // end parallel
; }
...
entry:
  ...
  %call = call i32 (ptr, ...) @printf(ptr noundef @.str, ptr noundef %arrayidx1)
...
DIR.OMP.PARALLEL.2:                               ; preds = %DIR.OMP.PARALLEL.1
  %1 = call token @llvm.directive.region.entry() [ "DIR.OMP.PARALLEL"(),
    ...
    "QUAL.OMP.SHARED:TYPED"(ptr %array_device, ptr null, i32 1) ]
...
DIR.OMP.TARGET.DATA.5:                            ; preds = %DIR.OMP.TARGET.DATA.4
  %4 = load ptr, ptr %array_device, align 8
  %arrayidx2 = getelementptr inbounds i32, ptr %4, i64 0
  %call3 = call i32 (ptr, ...) @printf(ptr noundef @.str, ptr noundef %arrayidx2) #2
