; REQUIRES: asserts
; RUN: opt -opaque-pointers=1 -enable-new-pm=0 -vpo-cfg-restructuring -vpo-wrncollection -analyze -S %s 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers=1 -passes='function(vpo-cfg-restructuring,print<vpo-wrncollection>)' -S %s 2>&1 | FileCheck %s
;
; Check parsing of the Order clause on Distribute
; Test IR was hand-modified by adding QUAL.OMP.ORDER.CONCURRENT
;
; Test src:
;
; void foo() {
...
; CHECK: BEGIN DISTRIBUTE ID=1 {
; CHECK:  ORDER: CONCURRENT
; CHECK:} END DISTRIBUTE ID=1
...
entry:
  ...
  %0 = call token @llvm.directive.region.entry() [ "DIR.OMP.DISTRIBUTE"(),
    "QUAL.OMP.ORDER.CONCURRENT"() ,"QUAL.OMP.PRIVATE:TYPED"(ptr %i, i32 0, i32 1),
