; REQUIRES: asserts
; RUN: opt -vpo-cfg-restructuring -vpo-wrncollection -analyze -debug-only=vpo-wrnnode -S < %s 2>&1 | FileCheck %s
; RUN: opt -passes='function(vpo-cfg-restructuring,print<vpo-wrncollection>)' -debug-only=vpo-wrnnode -S < %s 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers -vpo-cfg-restructuring -vpo-wrncollection -analyze -debug-only=vpo-wrnnode -S < %s 2>&1 | FileCheck %s
