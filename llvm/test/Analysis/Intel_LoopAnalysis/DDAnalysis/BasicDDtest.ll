; RUN:  opt < %s  -loop-simplify  -hir-de-ssa   -HIRCG   -dda  -dda-verify=Region  -analyze  | FileCheck %s 
...
; CHECK: (%q)[i1] --> (%p)[i1] ANTI [ * ]
; CHECK: (%p)[i1] --> (%q)[i1] FLOW [ * ]				
...
; CHECK: (%p)[100 * i1 + i2 + 101] --> (%p)[100 * i1 + i2 + 100] FLOW [ <= <> ]
...
; CHECK: (%p)[100 * i1 + i2 + 101] --> (%p)[100 * i1 + -1 * i2 + 110] FLOW [ <= <> ]
...
; CHECK: (%p)[2 * i1 + -4 * i2] --> (%p)[6 * i1 + 8 * i2] FLOW [ <= * ]
; CHECK: (%p)[6 * i1 + 8 * i2] --> (%p)[2 * i1 + -4 * i2] ANTI [ <= * ]
