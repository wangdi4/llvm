...
; RUN: opt < %s -force-opaque-pointers -loop-simplify -hir-ssa-deconstruction -mattr=+avx512f -enable-intel-advanced-opts -analyze -hir-cost-model-throttling=0 -force-hir-sparse-array-reduction-analysis -hir-sparse-array-reduction-analysis | FileCheck %s
; RUN: opt < %s -force-opaque-pointers -mattr=+avx512f -enable-intel-advanced-opts -aa-pipeline=basic-aa -passes="loop-simplify,hir-ssa-deconstruction,print<hir-sparse-array-reduction-analysis>" -hir-cost-model-throttling=0 -force-hir-sparse-array-reduction-analysis -disable-output 2>&1 | FileCheck %s
