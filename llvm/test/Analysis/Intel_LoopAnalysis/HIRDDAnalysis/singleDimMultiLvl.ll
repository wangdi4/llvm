...
;
;  Initial DDG built for 2 levels
;    FLOW (* <)  bwd  edge 
;    ANTI (* >)  fwd  edge
;  which is not precise. SIV test is invoked. Even Banerjee test is invoked,
;  same DV is obtained because input DV using for testing is (* *). We can obtain precise DV for flow as (< >) 
;  if we test different combinations of DV (9 times) which is expensive in terms of compile time.
;  By the time when it gets to the vectorizer, we should recompute again for input DV (= *). 
;  The check is to verify if the loop is vectorized.  
; 
;  do i=1, n
;     do j=1,n
;          a(j)  = a(j+1) + 1.0
...
;Module Before HIR
; ModuleID = 'singleDimMultipleLvl.f90'
source_filename = "singleDimMultipleLvl.f90"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

; Function Attrs: nofree norecurse nosync nounwind memory(argmem: readwrite) uwtable
define void @sub_(ptr noalias nocapture dereferenceable(4) %"sub_$A", ptr noalias nocapture readonly dereferenceable(4) %"sub_$N") local_unnamed_addr #0 {
alloca_0:
...
do.body6:                                         ; preds = %do.body2, %do.body6
  ...
  %"sub_$A[]_fetch.6" = load float, ptr %"sub_$A[]", align 1, !tbaa !6, !llfort.type_idx !8
  %add.2 = fadd reassoc ninf nsz arcp contract afn float %"sub_$A[]_fetch.6", 1.000000e+00
  %"sub_$A[]3" = tail call ptr @llvm.intel.subscript.p0.i64.i64.p0.i64(i8 0, i64 1, i64 4, ptr nonnull elementtype(float) %"sub_$A", i64 %indvars.iv), !llfort.type_idx !5
  store float %add.2, ptr %"sub_$A[]3", align 1, !tbaa !6
  %exitcond = icmp eq i64 %indvars.iv.next, %wide.trip.count
  br i1 %exitcond, label %do.end_do7, label %do.body6

do.end_do7:                                       ; preds = %do.body6
  %add.4 = add nuw nsw i32 %"sub_$I.0", 1
  %exitcond10 = icmp eq i32 %add.4, %0
  br i1 %exitcond10, label %do.end_do3.loopexit, label %do.body2
