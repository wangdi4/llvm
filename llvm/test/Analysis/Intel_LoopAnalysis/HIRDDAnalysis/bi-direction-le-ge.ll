...
; (%a)[0][i1 + i2] is identified as (<= >=). Then, the vector is selected as
...
; CHECK-DAG: (%a)[i1][i2] --> (%a)[0][i1 + i2] FLOW (= <=)
; CHECK-DAG: (%a)[0][i1 + i2] --> (%a)[i1][i2] ANTI (<= >=)
...
for.body4:                                        ; preds = %for.body4.lr.ph, %for.body4
  ...
  %iv.add = add nuw nsw i64 %indvars.iv, %indvars.iv23
  %arrayidx5 = getelementptr inbounds [100 x i32], ptr %a, i64 0, i64 %iv.add
