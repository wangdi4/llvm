...
; until we reach the load type of %struct.inner. Adding the extra dims/offsets
...
; that would result in the ref having a BitCastDestTy of %struct.inner* and
...
%struct.inner = type { [100 x i64] }
%struct.outer = type { %struct.inner, i32 }

@x = dso_local global %struct.outer zeroinitializer, align 4

define dso_local noundef %struct.inner @foo() {
entry:
  br label %bb

bb:
  %ld = load %struct.inner, ptr @x, align 4
  ret %struct.inner %ld
}
