...
define void @RemInt(ptr %p, i32 %n) #0 {
entry:
...
for.body.254:                                     ; preds = %for.cond.251.backedge, %for.end.217
  %0 = phi ptr [ %p, %for.end.217 ], [ %2, %for.cond.251.backedge ]
...
if.end.273:                                       ; preds = %for.body.254
  %add.ptr330 = getelementptr i16, ptr %0, i32 %storemerge.3205
...
for.inc.347:                                      ; preds = %for.inc.347, %for.inc.347.lr.ph
  %m.sroa.0.1.in190 = phi ptr [ %add.ptr330, %for.inc.347.lr.ph ], [ %incdec.ptr349, %for.inc.347 ]
  %k.0189 = phi i32 [ 0, %for.inc.347.lr.ph ], [ %inc348, %for.inc.347 ]
  %inc348 = add nuw i32 %k.0189, 1
  %incdec.ptr349 = getelementptr i16, ptr %m.sroa.0.1.in190, i32 1
  %1 = load i16, ptr %incdec.ptr349, align 2
