...
; CG:
; CHECK-CG: Module
; CHECK-CG-DAG: call void @llvm.dbg.declare(metadata float* {{.*}}, metadata !22, metadata !24), !dbg !25
; CHECK-CG-DAG: call void @llvm.dbg.declare(metadata float* {{.*}}, metadata !19, metadata !24), !dbg !26
; CHECK-CG-DAG: call void @llvm.dbg.declare(metadata float* {{.*}}, metadata !15, metadata !24), !dbg !27
; CHECK-CG: region.0: 

; Call + float instructions
; CHECK-CG:  {{.*}} = sitofp i32 {{.*}} to float, !dbg !33
; CHECK-CG:  {{.*}} = tail call float bitcast (float (...)* @bar to float ()*)() #3, !dbg !33
; CHECK-CG:  {{.*}} = fmul float {{.*}}, {{.*}}, !dbg !33
; CHECK-CG:  {{.*}} = fadd float {{.*}}, {{.*}}, !dbg !33
; CHECK-CG:  store float {{.*}}, float* {{.*}}, !dbg !33
  
; IF + complex predicate
; CHECK-CG:  %hir.cmp.7 = fcmp ogt float {{.*}}, 1.000000e+01, !dbg !35
; CHECK-CG:  %hir.cmp.8 = icmp slt i32 {{.*}}, 10, !dbg !37
; CHECK-CG:  %hir.cmp.10 = icmp ne i1 {{.*}}, false, !dbg !35
; CHECK-CG:  br i1 %hir.cmp.10, label %then.10, label %ifmerge.10, !dbg !35 

; IF + multiple predicates
; CHECK-CG:  %hir.cmp.22 = icmp sgt i32 {{.*}}, 0, !dbg !47
; CHECK-CG:  {{.*}} = load i32, i32* {{.*}}, !dbg !45
; CHECK-CG:  %hir.cmp.225 = icmp ne i32 {{.*}}, 0, !dbg !45
; CHECK-CG:  {{.*}} = and i1 %hir.cmp.22, %hir.cmp.225, !dbg !45
; CHECK-CG:  br i1 {{.*}}, label %then.22, label %ifmerge.22, !dbg !45

; Load ref + CE
; CHECK-CG:  {{.*}} = load i32, i32* %i1.i32, !dbg !26
; CHECK-CG:  {{.*}} = add i32 {{.*}}, 1, !dbg !26
; CHECK-CG:  %arrayIdx10 = getelementptr inbounds float, float* %a, i32 {{.*}}, !dbg !26
; CHECK-CG:  %gepload11 = load float, float* %arrayIdx10, align 4, !dbg !26

; Scalar store
; CHECK-CG:  store float {{.*}}, float* {{.*}}, !dbg !26

; Select
; CHECK-CG:  %hir.selcmp.37 = fcmp une float {{.*}}, 0.000000e+00, !dbg !25
; CHECK-CG:  {{.*}} = select i1 %hir.selcmp.37, float 1.000000e+01, float -1.000000e+01, !dbg !25

; Store ref
; CHECK-CG:  %arrayIdx13 = getelementptr inbounds float, float* %c, i32 {{.*}}, !dbg !52
; CHECK-CG:  {{.*}} = load float, float* {{.*}}, !dbg !52
; CHECK-CG:  store float {{.*}}, float* %arrayIdx13, align 4, !dbg !52
