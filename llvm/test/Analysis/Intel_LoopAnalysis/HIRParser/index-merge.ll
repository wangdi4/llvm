...
; CHECK: DO i1 = 0, 15
; CHECK-NEXT: %0 = {al:1}(%pSrc1)[%src1Step * i1];
; CHECK-NEXT: %1 = {al:1}(%pSrc2)[%src2Step * i1];
; CHECK-NEXT: {al:2}(%pDst)[(%dstStep * i1)/u2] = sext.i8.i16(%0) + sext.i8.i16(%1);
; CHECK-NEXT: %2 = {al:1}(%pSrc1)[%src1Step * i1 + 1];
; CHECK-NEXT: %3 = {al:1}(%pSrc2)[%src2Step * i1 + 1];
; CHECK-NEXT: {al:2}(%pDst)[(%dstStep * i1 + 2)/u2] = sext.i8.i16(%2) + sext.i8.i16(%3);
; CHECK-NEXT: %4 = {al:1}(%pSrc1)[%src1Step * i1 + 2];
; CHECK-NEXT: %5 = {al:1}(%pSrc2)[%src2Step * i1 + 2];
; CHECK-NEXT: {al:2}(%pDst)[(%dstStep * i1 + 4)/u2] = sext.i8.i16(%4) + sext.i8.i16(%5);
; CHECK-NEXT: %6 = {al:1}(%pSrc1)[%src1Step * i1 + 3];
; CHECK-NEXT: %7 = {al:1}(%pSrc2)[%src2Step * i1 + 3];
; CHECK-NEXT: {al:2}(%pDst)[(%dstStep * i1 + 6)/u2] = sext.i8.i16(%6) + sext.i8.i16(%7);
; CHECK-NEXT: %8 = {al:1}(%pSrc1)[%src1Step * i1 + 4];
; CHECK-NEXT: %9 = {al:1}(%pSrc2)[%src2Step * i1 + 4];
; CHECK-NEXT: {al:2}(%pDst)[(%dstStep * i1 + 8)/u2] = sext.i8.i16(%8) + sext.i8.i16(%9);
; CHECK-NEXT: %10 = {al:1}(%pSrc1)[%src1Step * i1 + 5];
; CHECK-NEXT: %11 = {al:1}(%pSrc2)[%src2Step * i1 + 5];
; CHECK-NEXT: {al:2}(%pDst)[(%dstStep * i1 + 10)/u2] = sext.i8.i16(%10) + sext.i8.i16(%11);
; CHECK-NEXT: %12 = {al:1}(%pSrc1)[%src1Step * i1 + 6];
; CHECK-NEXT: %13 = {al:1}(%pSrc2)[%src2Step * i1 + 6];
; CHECK-NEXT: {al:2}(%pDst)[(%dstStep * i1 + 12)/u2] = sext.i8.i16(%12) + sext.i8.i16(%13);
; CHECK-NEXT: %14 = {al:1}(%pSrc1)[%src1Step * i1 + 7];
; CHECK-NEXT: %15 = {al:1}(%pSrc2)[%src2Step * i1 + 7];
; CHECK-NEXT: {al:2}(%pDst)[(%dstStep * i1 + 14)/u2] = sext.i8.i16(%14) + sext.i8.i16(%15);
; CHECK-NEXT: %16 = {al:1}(%pSrc1)[%src1Step * i1 + 8];
; CHECK-NEXT: %17 = {al:1}(%pSrc2)[%src2Step * i1 + 8];
; CHECK-NEXT: {al:2}(%pDst)[(%dstStep * i1 + 16)/u2] = sext.i8.i16(%16) + sext.i8.i16(%17);
; CHECK-NEXT: %18 = {al:1}(%pSrc1)[%src1Step * i1 + 9];
; CHECK-NEXT: %19 = {al:1}(%pSrc2)[%src2Step * i1 + 9];
; CHECK-NEXT: {al:2}(%pDst)[(%dstStep * i1 + 18)/u2] = sext.i8.i16(%18) + sext.i8.i16(%19);
; CHECK-NEXT: %20 = {al:1}(%pSrc1)[%src1Step * i1 + 10];
; CHECK-NEXT: %21 = {al:1}(%pSrc2)[%src2Step * i1 + 10];
; CHECK-NEXT: {al:2}(%pDst)[(%dstStep * i1 + 20)/u2] = sext.i8.i16(%20) + sext.i8.i16(%21);
; CHECK-NEXT: %22 = {al:1}(%pSrc1)[%src1Step * i1 + 11];
; CHECK-NEXT: %23 = {al:1}(%pSrc2)[%src2Step * i1 + 11];
; CHECK-NEXT: {al:2}(%pDst)[(%dstStep * i1 + 22)/u2] = sext.i8.i16(%22) + sext.i8.i16(%23);
; CHECK-NEXT: %24 = {al:1}(%pSrc1)[%src1Step * i1 + 12];
; CHECK-NEXT: %25 = {al:1}(%pSrc2)[%src2Step * i1 + 12];
; CHECK-NEXT: {al:2}(%pDst)[(%dstStep * i1 + 24)/u2] = sext.i8.i16(%24) + sext.i8.i16(%25);
; CHECK-NEXT: %26 = {al:1}(%pSrc1)[%src1Step * i1 + 13];
; CHECK-NEXT: %27 = {al:1}(%pSrc2)[%src2Step * i1 + 13];
; CHECK-NEXT: {al:2}(%pDst)[(%dstStep * i1 + 26)/u2] = sext.i8.i16(%26) + sext.i8.i16(%27);
; CHECK-NEXT: %28 = {al:1}(%pSrc1)[%src1Step * i1 + 14];
; CHECK-NEXT: %29 = {al:1}(%pSrc2)[%src2Step * i1 + 14];
; CHECK-NEXT: {al:2}(%pDst)[(%dstStep * i1 + 28)/u2] = sext.i8.i16(%28) + sext.i8.i16(%29);
; CHECK-NEXT: %30 = {al:1}(%pSrc1)[%src1Step * i1 + 15];
; CHECK-NEXT: %31 = {al:1}(%pSrc2)[%src2Step * i1 + 15];
; CHECK-NEXT: {al:2}(%pDst)[(%dstStep * i1 + 30)/u2] = sext.i8.i16(%30) + sext.i8.i16(%31);
; CHECK-NEXT: END LOOP
