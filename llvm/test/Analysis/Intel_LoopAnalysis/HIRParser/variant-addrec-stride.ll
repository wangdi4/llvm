...
; CHECK: |   %t690 = (i32*)(%t687)[0];
; CHECK: |   (%t20)[0] = %t690;
; CHECK: |   %t696 = (-4 * i1 + 4 == 0) ? 4 * i1 + -4 : -4;
; CHECK: |   %t687 = &((%t687)[%t696]);
; CHECK: + END LOOP

define void @foo(i8* %t684, i32* %t20) {
entry:
  br label %loop

loop:                                              ; preds = %loop, %entry
  %t686 = phi i32 [ 8, %entry ], [ %t693, %loop ]
  %t687 = phi i8* [ %t684, %entry ], [ %t697, %loop ]
  %t688 = phi i32 [ 8, %entry ], [ %t692, %loop ]
  %t689 = bitcast i8* %t687 to i32*
  %t690 = load i32, i32* %t689, align 1
  store i32 %t690, i32* %t20, align 4
  ...
  %t697 = getelementptr inbounds i8, i8* %t687, i32 %t696
