...
define hidden void @main([16384 x [4097 x float]]* nocapture %a, float %conv, float %mul9, i32* %.omp.lb, i64 %t) {
entry:
  ...
  %t0 = load i32, i32* %.omp.lb, align 4
  %cmp70 = icmp sgt i32 %t0, 16383
  br i1 %cmp70, label %exit, label %loop.ph

loop.ph:                         ; preds = %entry
  %t4 = call token @llvm.directive.region.entry() [ "DIR.OMP.SIMD"(), "QUAL.OMP.NORMALIZED.IV"(i8* null), "QUAL.OMP.NORMALIZED.UB"(i8* null) ]
  br label %loop1
...
loop1:                              ; preds = %loop1, %loop.ph
  ...
  %arrayidx13 = getelementptr inbounds [16384 x [4097 x float]], [16384 x [4097 x float]]* %a, i64 0, i64 %t, i64 %indvars.iv
...
loop2.lr.ph:                       ; preds = %loop1
  ...
  %arrayidx17 = getelementptr inbounds [16384 x [4097 x float]], [16384 x [4097 x float]]* %a, i64 0, i64 %t, i64 4096
  store float 1.000000e+00, float* %arrayidx17, align 4
  %t7 = call token @llvm.directive.region.entry() [ "DIR.OMP.SIMD"(), "QUAL.OMP.NORMALIZED.IV"(i8* null), "QUAL.OMP.NORMALIZED.UB"(i8* null) ]
...
loop2:                             ; preds = %loop2, %loop2.lr.ph
  ...
  %arrayidx32 = getelementptr inbounds [16384 x [4097 x float]], [16384 x [4097 x float]]* %a, i64 0, i64 %t, i64 %indvars.iv.next7
  %t8 = load float, float* %arrayidx32, align 4
  ...
  %arrayidx42 = getelementptr inbounds [16384 x [4097 x float]], [16384 x [4097 x float]]* %a, i64 0, i64 %t, i64 %indvars.iv6
  store float %add38, float* %arrayidx42, align 4
