...
; single guarding condition in the incoming IR, the region exit directive stays
; in the postexit of the first loop. Before the fix, region exit directive was
; being moved to the preheader of the second loop which broke the simd region
; semantics.


; CHECK:    %simd.reg = @llvm.directive.region.entry()
; CHECK:    %tB = (%B)[0];
; CHECK: + Ztt: if (%n > 0)
...
; Verify that this is in the postexit of the first loop.
; CHECK-NEXT:   @llvm.directive.region.exit(%simd.reg)
...
for.body.preheader:                               ; preds = %entry
  %simd.reg = call token @llvm.directive.region.entry() [ "DIR.OMP.SIMD"() ]
...
for.body3.preheader:                              ; preds = %for.body3.preheader
  call void @llvm.directive.region.exit(token %simd.reg) [ "DIR.OMP.END.SIMD"() ]
