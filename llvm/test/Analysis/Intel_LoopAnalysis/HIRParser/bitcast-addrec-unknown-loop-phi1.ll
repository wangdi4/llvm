; RUN: opt < %s -hir-ssa-deconstruction -analyze -enable-new-pm=0 -hir-framework -hir-framework-debug=parser 2>&1 | FileCheck %s
...
; CHECK: |   if (&((%phi.ptr.out)[0]) != &((%end.ptr)[0]))
; CHECK: |   {
; CHECK: |      <i1 = i1 + 1>
; CHECK: |      goto do.loop;
; CHECK: |   }
...
do.loop:                                          ; preds = %do.loop, %entry
  ...
  %cmp44 = icmp eq %struct.Segment* %phi.ptr, %end.ptr
