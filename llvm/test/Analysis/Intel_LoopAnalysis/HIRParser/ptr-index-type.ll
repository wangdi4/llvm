; RUN: opt -opaque-pointers=0 < %s -passes="hir-ssa-deconstruction,print<hir>" -xmain-opt-level=3 -hir-framework-debug=parser -hir-details 2>&1 | FileCheck %s
...
; CHECK: |   <LVAL-REG> {al:4}(LINEAR i32* %p)[LINEAR i32 2 * (%d /u 4) * i1]
; CHECK: |   (%p)[2 * (%d /u 4) * i1 + (%d /u 4)] = 1;
; CHECK: + END LOOP

target datalayout = "e-m:m-p:40:64:64:32-i32:32-i16:16-i8:8-n32"

define void @test1(i32 %d, i32* nocapture %p) {
entry:
  %div = lshr i32 %d, 2
  br label %for.body
...
for.body:                                         ; preds = %for.body, %entry
  ...
  %p.addr.01 = phi i32* [ %p, %entry ], [ %add.ptr1, %for.body ]
  store i32 0, i32* %p.addr.01, align 4
  %add.ptr = getelementptr inbounds i32, i32* %p.addr.01, i32 %div
