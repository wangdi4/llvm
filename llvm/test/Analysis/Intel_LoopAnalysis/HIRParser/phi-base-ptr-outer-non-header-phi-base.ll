...
define i32 @sub(i32* nocapture %p, i1 %c) {
entry:
  br label %for.body.outer

for.body.outer:
  %iv.outer =  phi i32 [ 0, %entry ], [ %iv.outer.inc, %latch ]
  %p.outer = phi i32* [ %p, %entry ], [ %p.merge, %latch ]
  br i1 %c, label %then, label %else

then:
  %p.then.inc = getelementptr inbounds i32, i32* %p.outer, i64 1
  br label %for.body.pre

else:
  %p.else.inc = getelementptr inbounds i32, i32* %p.outer, i64 1
  br label %for.body.pre

for.body.pre:
  %p.merge = phi i32* [ %p.then.inc, %then ], [ %p.else.inc, %else ]
  br label %for.body

for.body:                                         ; preds = %for.body, %entry
  %i.06 = phi i32 [ 0, %for.body.pre ], [ %inc, %for.body ]
  %p.addr.05 = phi i32* [ %p.merge, %for.body.pre ], [ %incdec.ptr, %for.body ]
