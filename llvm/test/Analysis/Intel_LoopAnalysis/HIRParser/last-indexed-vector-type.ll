...
define void @foo(%struct* %vec.ptr.arr, <8 x i64> %val) {
entry:
  br label %bb

bb:                                      ; preds = %bb, %entry
  %vec.ptr.gep = getelementptr inbounds %struct, %struct* %vec.ptr.arr, i64 0, i32 0
  store <8 x i64> %val, <8 x i64>* %vec.ptr.gep
  ret void
}
