...
; RUN: opt < %s -loop-simplify -hir-ssa-deconstruction | opt -analyze -hir-parser -hir-details | FileCheck -check-prefix=DETAIL %s
; DETAIL: %0 = %0  +  %1
...
; ModuleID = 'self-recrursive-def.c'
...
entry:
  ...
  br i1 %cmp.18, label %for.body.3.lr.ph, label %for.end.8

for.body.3.lr.ph:                                 ; preds = %entry, %for.inc.6
  %indvars.iv20 = phi i64 [ %indvars.iv.next21, %for.inc.6 ], [ 0, %entry ]
...
for.inc.6:                                        ; preds = %for.body.3
  ...
  br i1 %exitcond23, label %for.end.8, label %for.body.3.lr.ph

for.end.8:                                        ; preds = %for.inc.6, %entry
  ret void
}
