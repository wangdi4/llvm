...
; CHECK: |   %1 = (%arraydecay.i)[i1];
; CHECK: |   %call2 = @printf(&((@.str)[0][0]),  %1);
; CHECK: + END LOOP
...
for.body:                                         ; preds = %entry, %for.body
  %__begin.010 = phi double* [ %arraydecay.i, %entry ], [ %incdec.ptr, %for.body ]
  %1 = load double, double* %__begin.010, align 8, !tbaa !5
  %call2 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([4 x i8], [4 x i8]* @.str, i64 0, i64 0), double %1)
  %incdec.ptr = getelementptr inbounds double, double* %__begin.010, i64 1
  %cmp = icmp eq double* %incdec.ptr, %add.ptr.i
