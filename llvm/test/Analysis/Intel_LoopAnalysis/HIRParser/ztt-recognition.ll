; RUN: opt < %s -instcombine -hir-ssa-deconstruction | opt -analyze -enable-new-pm=0 -hir-framework -hir-framework-debug=parser -debug-only=hir-framework | FileCheck %s
