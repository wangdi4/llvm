...
; sibling loop is not marked as SIMD. This case handles when the SIMD
; loop is placed after the non-SIMD loop.
...
; CHECK:       |   + END LOOP
; CHECK:       |
; CHECK:       |   %1 = @llvm.directive.region.entry(); [ DIR.OMP.SIMD(),  QUAL.OMP.NORMALIZED.IV(null),  QUAL.OMP.NORMALIZED.UB(null) ]
; CHECK:       |
...
if.sib.then:                                          ; preds = %inner.for.body
  ...
  %0 = trunc i64 %indvars.iv.sib to i32
...
inner.for.inc:                                ; preds = %inner.for.body, %if.sib.then
  ...
  br i1 %exitcond.sib, label %omp.inner.for.body.lr.ph, label %inner.for.body
