...
; outer loop despite the outermost loop not being included in the region because
; it contains atomics.
...
define dso_local double @foo(%struct.Pixel* nocapture readonly %pix1, %struct.Pixel* nocapture readonly %pix2, double* nocapture readonly %mul_ptr, i32 %n) local_unnamed_addr #0 {
entry:
...
for.cond4.preheader.preheader:                    ; preds = %for.cond1.preheader.lr.ph, %for.inc103
  ...
  %mul = load double, double* %mul_ptr, align 8
