...
; RUN: opt < %s -hir-ssa-deconstruction -analyze -hir-locality-analysis -hir-spatial-locality | FileCheck %s --check-prefix=SCALAR
...
; RUN: opt < %s -hir-ssa-deconstruction -hir-temp-cleanup -hir-vec-dir-insert -hir-vplan-vec -vplan-force-vf=8 -enable-vp-value-codegen-hir=0 -analyze -hir-locality-analysis -hir-spatial-locality | FileCheck %s --check-prefix=VECTOR
; RUN: opt < %s -hir-ssa-deconstruction -hir-temp-cleanup -hir-vec-dir-insert -hir-vplan-vec -vplan-force-vf=8 -enable-vp-value-codegen-hir -analyze -hir-locality-analysis -hir-spatial-locality | FileCheck %s --check-prefix=VECTOR
