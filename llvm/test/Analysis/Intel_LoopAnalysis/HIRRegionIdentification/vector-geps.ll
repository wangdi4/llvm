; REQUIRES: asserts
; RUN: opt < %s  -hir-region-identification -debug-only=hir-region-identification 2>&1 | FileCheck %s
...
define void @foo(<8 x float*> %broadcast.splat, <8 x i64> %broadcast.splat20, i64 %n.vec) {
entry:
...
vector.body:                                      ; preds = %vector.body, %entry
  ...
  %mm_vectorGEP = getelementptr inbounds float, <8 x float*> %broadcast.splat, <8 x i64> %t3
  call void @llvm.masked.scatter.v8f32.v8p0f32(<8 x float> %t2, <8 x float*> %mm_vectorGEP, i32 4, <8 x i1> <i1 true, i1 true, i1 true, i1 true, i1 true, i1 true, i1 true, i1 true>)
