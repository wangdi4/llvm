; REQUIRES: asserts
; RUN: opt %s -enable-new-pm=0 -disable-output -debug-only=whole-program-analysis -whole-program-advanced-opt-trace -wholeprogramanalysis -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx 2>&1 | FileCheck %s
; RUN: opt %s -disable-output -debug-only=whole-program-analysis -whole-program-advanced-opt-trace -passes='require<wholeprogram>' -enable-intel-advanced-opts -mtriple=i686-- -mattr=+avx 2>&1 | FileCheck %s
