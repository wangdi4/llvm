; This test verifies that points-to set of %i7 is computed as universal-set
; since %i7 is passed to indirect call which has non-internal function as
; target.
; RUN: opt < %s -passes='require<anders-aa>' -print-anders-points-to -disable-output 2>&1 | FileCheck %s
; CHECK: [1] main:i7       --> (0): <universal>
; REQUIRES: asserts
