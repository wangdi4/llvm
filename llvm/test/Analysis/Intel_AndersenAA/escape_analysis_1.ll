...
define i32 @foo(ptr %fooPtr, i32 %aconst) #0 {
entry:
...
if.then:                                          ; preds = %entry
  store ptr getelementptr inbounds ([2048 x i32], ptr @foo.local_fooBuf, i64 0, i64 0), ptr @foo.fooBuf, align 8
  store i1 true, ptr @foo.init, align 1
  br label %if.end

if.end:                                           ; preds = %entry.if.end_crit_edge, %if.then
  %0 = phi ptr [ %.pre, %entry.if.end_crit_edge ], [ getelementptr inbounds ([2048 x i32], ptr @foo.local_fooBuf, i64 0, i64 0), %if.then ]
...
for.inc.prol:                                     ; preds = %for.inc.prol, %for.inc.lr.ph
  ...
  store i32 %2, ptr %arrayidx3.prol, align 4
...
for.inc:                                          ; preds = %for.inc.lr.ph.split, %for.inc
  ...
  store i32 %lftr.wideiv.2, ptr %arrayidx3.3, align 4
