...
; CHECK:  NoAlias:	double* %arrayidx9, double** %data
...
%struct.TestStruct = type { i32, i32, double* }

declare void @dealloc(%struct.TestStruct* %arg)
declare dso_local noundef double* @_Z3barv()

; Function Attrs: mustprogress uwtable
define dso_local void @_Z3fooP10TestStructi(%struct.TestStruct* nocapture noundef %input_ptr, i32 noundef %n) local_unnamed_addr #0 {
entry:
  %Local = alloca %struct.TestStruct 
  %call = tail call noundef double* @_Z3barv()
  %rows = getelementptr inbounds %struct.TestStruct, %struct.TestStruct* %Local, i32 0, i32 0 
  store i32 3, i32* %rows 
  %cols = getelementptr inbounds %struct.TestStruct, %struct.TestStruct* %Local, i32 0, i32 1 
  store i32 3, i32* %cols 
  %data = getelementptr inbounds %struct.TestStruct, %struct.TestStruct* %Local, i32 0, i32 2 
  store double* %call, double** %data 
  %cmp26 = icmp sgt i32 %n, 0
  br i1 %cmp26, label %for.cond1.preheader.lr.ph, label %for.cond.cleanup

for.cond1.preheader.lr.ph:                        ; preds = %entry
  %data7 = getelementptr inbounds %struct.TestStruct, %struct.TestStruct* %input_ptr, i64 0, i32 2 
  %0 = load double*, double** %data7 
  %1 = zext i32 %n to i64
...
for.cond1.preheader:                              ; preds = %for.cond1.preheader.lr.ph, %for.cond.cleanup4
  ...
  %rows2 = load i32, i32* %rows
  %rowsz = zext i32 %rows2 to i64
  %datai = load double*, double** %data
  br label %for.body5
...
for.cond.cleanup:                                 ; preds = %for.cond.cleanup.loopexit, %entry
  call void @dealloc(%struct.TestStruct* %Local)
  ret void

for.cond.cleanup4:                                ; preds = %for.body5
  %add.lcssa = phi double [ %add, %for.body5 ]
  %arrayidx9 = getelementptr inbounds double, double* %0, i64 %indvars.iv28
  store double %add.lcssa, double* %arrayidx9 
...
for.body5:                                        ; preds = %for.cond1.preheader, %for.body5
  ...
  %arrayidx = getelementptr inbounds double, double* %datai, i64 %indvars.iv
