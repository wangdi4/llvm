...
; INTEL
; RUN: opt -passes="gvn,instcombine" < %s -aa-pipeline=basic-aa -S 2>&1 | FileCheck --check-prefix=CHECK-GEP %s
; RUN: opt -passes=convert-to-subscript -S < %s | opt -passes="gvn,instcombine" -aa-pipeline=basic-aa -S 2>&1 | FileCheck %s
...
define i32 @test5_as1_smaller_size(ptr addrspace(1) %p, i8 %i) {
  ...
  ret i32 %z
; INTEL
; Special treatment for GEP, i.e. sign extention
; CHECK-GEP-LABEL: @test5_as1_smaller_size(
; CHECK-GEP: sext
; CHECK-GEP: ret i32 0
}
