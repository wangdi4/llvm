...
; safely truncating the sign-extended operand %sext.iv. %sext.iv is a 
; sign-extend of IV and %upper has known constant range which fits in 32 bits.
...
; CHECK: %sext.iv
; CHECK-NEXT: (sext i32 {{[{]}}{1,+,10}<%loop.outer>,+,1}<%loop.inner> to i64)
...
loop.inner:                                        ; preds = %loop.inner, %loop.inner.preheader
  ...
  %sext.iv = sext i32 %trunc.iv to i64
  %exitcond = icmp ne i64 %sext.iv, %upper
