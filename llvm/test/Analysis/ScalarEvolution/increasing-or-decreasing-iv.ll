...
; CHECK-NEXT:    --> %step U: [1,0) S: [-1,2) ;INTEL
...
; CHECK-NEXT:    --> {(%step + %start),+,%step}<nw><%loop> U: [-128,256) S: [-128,256) Exits: ((128 * %step)<nsw> + %start) LoopDispositions: { %loop: Computable } ;INTEL
...
; CHECK-NEXT:    --> %step U: [8,-7) S: [-8,9) ;INTEL
; CHECK-NEXT:    %loop.iv = phi i32 [ 0, %entry ], [ %loop.iv.inc, %loop ]
...
; CHECK-NEXT:    %iv.next = add i32 %iv, %step
; CHECK-NEXT:    --> {(%step + %start),+,%step}<nw><%loop> U: [0,-7) S: [-128,249) Exits: ((16 * %step)<nsw> + %start) LoopDispositions: { %loop: Computable } ;INTEL
; CHECK-NEXT:    %loop.iv.inc = add i32 %loop.iv, 1
...
; CHECK:       Loop %loop: Trip multiple is 128
;
; INTEL - Note: in upstream the ranges for %iv.trunc are "full-set". ; INTEL
...
; CHECK-NEXT:    --> %step U: [0,-1) S: [-2,1) ;INTEL
; CHECK-NEXT:    %loop.iv = phi i16 [ 0, %entry ], [ %loop.iv.inc, %loop ]
; CHECK-NEXT:    --> {0,+,1}<nuw><nsw><%loop> U: [0,128) S: [0,128) Exits: 127 LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %iv = phi i32 [ %start, %entry ], [ %iv.next, %loop ]
...
; CHECK-NEXT:    --> (1 + %step)<nuw><nsw> U: [1,0) S: [-1,2) Exits: (1 + %step)<nuw><nsw> LoopDispositions: { %loop: Invariant } ;INTEL
; CHECK-NEXT:    %iv.next = add i32 %iv, %step.plus.one
; CHECK-NEXT:    --> {(1 + %step + %start),+,(1 + %step)<nuw><nsw>}<nw><%loop> U: [-128,256) S: [-128,256) Exits: (128 + (128 * %step)<nsw> + %start) LoopDispositions: { %loop: Computable } ;INTEL
; CHECK-NEXT:    %iv.sext = sext i32 %iv to i64
