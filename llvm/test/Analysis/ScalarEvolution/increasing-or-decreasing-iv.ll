...
; INTEL - added bunch of nsw flags.
define void @f0(i1 %c) {
; CHECK-LABEL: 'f0'
...
; CHECK-NEXT:    --> {%start,+,%step}<%loop> U: [0,128) S: [0,128) Exits: ((127 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %iv.next = add i32 %iv, %step
; CHECK-NEXT:    --> {(%step + %start)<nsw>,+,%step}<%loop> U: [-256,256) S: [-256,256) Exits: ((128 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
...
; CHECK-NEXT:    %iv = phi i32 [ %start, %entry ], [ %iv.next, %loop ]
; CHECK-NEXT:    --> {%start,+,%step}<%loop> U: [0,121) S: [0,121) Exits: ((15 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %iv.1 = add i32 %iv, 1
; CHECK-NEXT:    --> {(1 + %start)<nuw><nsw>,+,%step}<%loop> U: [1,122) S: [1,122) Exits: (1 + (15 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %iv.2 = add i32 %iv, 2
; CHECK-NEXT:    --> {(2 + %start)<nuw><nsw>,+,%step}<%loop> U: [2,123) S: [2,123) Exits: (2 + (15 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %iv.3 = add i32 %iv, 3
; CHECK-NEXT:    --> {(3 + %start)<nuw><nsw>,+,%step}<%loop> U: [3,124) S: [3,124) Exits: (3 + (15 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %iv.4 = add i32 %iv, 4
; CHECK-NEXT:    --> {(4 + %start)<nuw><nsw>,+,%step}<%loop> U: [4,125) S: [4,125) Exits: (4 + (15 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %iv.5 = add i32 %iv, 5
; CHECK-NEXT:    --> {(5 + %start)<nuw><nsw>,+,%step}<%loop> U: [5,126) S: [5,126) Exits: (5 + (15 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %iv.6 = add i32 %iv, 6
; CHECK-NEXT:    --> {(6 + %start)<nuw><nsw>,+,%step}<%loop> U: [6,127) S: [6,127) Exits: (6 + (15 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %iv.7 = add i32 %iv, 7
; CHECK-NEXT:    --> {(7 + %start)<nuw><nsw>,+,%step}<%loop> U: [7,128) S: [7,128) Exits: (7 + (15 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %iv.m1 = sub i32 %iv, 1
; CHECK-NEXT:    --> {(-1 + %start)<nsw>,+,%step}<%loop> U: [-1,120) S: [-1,120) Exits: (-1 + (15 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %iv.m2 = sub i32 %iv, 2
; CHECK-NEXT:    --> {(-2 + %start)<nsw>,+,%step}<%loop> U: [0,-1) S: [-2,119) Exits: (-2 + (15 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %iv.m3 = sub i32 %iv, 3
; CHECK-NEXT:    --> {(-3 + %start)<nsw>,+,%step}<%loop> U: [-3,118) S: [-3,118) Exits: (-3 + (15 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %iv.m4 = sub i32 %iv, 4
; CHECK-NEXT:    --> {(-4 + %start)<nsw>,+,%step}<%loop> U: [0,-3) S: [-4,117) Exits: (-4 + (15 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %iv.m5 = sub i32 %iv, 5
; CHECK-NEXT:    --> {(-5 + %start)<nsw>,+,%step}<%loop> U: [-5,116) S: [-5,116) Exits: (-5 + (15 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %iv.m6 = sub i32 %iv, 6
; CHECK-NEXT:    --> {(-6 + %start)<nsw>,+,%step}<%loop> U: [0,-1) S: [-6,115) Exits: (-6 + (15 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %iv.m7 = sub i32 %iv, 7
; CHECK-NEXT:    --> {(-7 + %start)<nsw>,+,%step}<%loop> U: [-7,114) S: [-7,114) Exits: (-7 + (15 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %iv.next = add i32 %iv, %step
; CHECK-NEXT:    --> {(%step + %start)<nsw>,+,%step}<%loop> U: [0,-7) S: [-256,361) Exits: ((16 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %loop.iv.inc = add i32 %loop.iv, 1
...
; CHECK-NEXT:    %iv.next = add i32 %iv, %step
; CHECK-NEXT:    --> {(%step + %start)<nsw>,+,%step}<nsw><%loop> U: [-256,256) S: [-256,256) Exits: ((128 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %loop.iv.inc = add i32 %loop.iv, 1
; CHECK-NEXT:    --> {1,+,1}<%loop> U: [1,129) S: [1,129) Exits: 128 LoopDispositions: { %loop: Computable }
...
; CHECK-NEXT:    --> {(%step + %start)<nuw><nsw>,+,%step}<nuw><%loop> U: [1,0) S: [1,0) Exits: ((128 * %step)<nuw> + %start) LoopDispositions: { %loop: Computable }
...
; CHECK-NEXT:    --> {0,+,1}<%loop> U: [0,128) S: [0,128) Exits: 127 LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %iv = phi i32 [ %start, %entry ], [ %iv.next, %loop ]
; CHECK-NEXT:    --> {%start,+,(1 + %step)<nuw><nsw>}<%loop> U: [0,128) S: [0,128) Exits: (127 + (127 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
...
; CHECK-NEXT:    --> {(1 + %step + %start)<nsw>,+,(1 + %step)<nuw><nsw>}<%loop> U: [-128,384) S: [-128,384) Exits: (128 + (128 * %step)<nsw> + %start)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %iv.sext = sext i32 %iv to i64
; CHECK-NEXT:    --> {(zext i32 %start to i64),+,(1 + (sext i32 %step to i64))<nuw><nsw>}<nsw><%loop> U: [0,128) S: [0,128) Exits: (127 + (zext i32 %start to i64) + (127 * (sext i32 %step to i64))<nsw>)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %loop.iv.inc = add i16 %loop.iv, 1
; CHECK-NEXT:    --> {1,+,1}<%loop> U: [1,129) S: [1,129) Exits: 128 LoopDispositions: { %loop: Computable }
...
; CHECK-NEXT:    --> {(1 + (trunc i32 %start to i16))<nuw><nsw>,+,(trunc i32 %step to i16)}<%loop> U: [1,129) S: [1,129) Exits: (1 + (trunc i32 %start to i16) + (127 * (trunc i32 %step to i16))<nsw>)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %iv.trunc.plus.two = add i16 %iv.trunc, 2
; CHECK-NEXT:    --> {(2 + (trunc i32 %start to i16))<nuw><nsw>,+,(trunc i32 %step to i16)}<%loop> U: [2,130) S: [2,130) Exits: (2 + (trunc i32 %start to i16) + (127 * (trunc i32 %step to i16))<nsw>)<nsw> LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %loop.iv.inc = add i16 %loop.iv, 1
; CHECK-NEXT:    --> {1,+,1}<%loop> U: [1,129) S: [1,129) Exits: 128 LoopDispositions: { %loop: Computable }
