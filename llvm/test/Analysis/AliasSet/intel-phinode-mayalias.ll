; RUN: opt -opaque-pointers=0 -S -passes='loop(loop-rotate),print-alias-sets,loop-mssa(licm)' -o - < %s 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers=0 -S -passes='loop(loop-rotate),print-alias-sets' -o - < %s 2>&1 | FileCheck %s
