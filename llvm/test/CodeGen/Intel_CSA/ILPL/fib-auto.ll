; RUN: llc -mtriple=csa -csa-ilpl-selection=automatic < %s | FileCheck %s --check-prefix=COMPLETION
; RUN: llc -mtriple=csa -csa-ilpl-use-completion-buf=false -csa-ilpl-selection=automatic < %s | FileCheck %s --check-prefix=TOKENLIC
...
; COMPLETION-DAG:  completion64 [[TOKENS:%.+]], [[ORDERED:%.+]], [[IDXIN:%.+]], [[VALIN:%.+]], [[NUMTOKENS:[0-9]+]]
;
...
; ... some number more, dependening on the implementation.
entry:
