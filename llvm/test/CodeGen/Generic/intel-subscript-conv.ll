...
; RUN: opt -opaque-pointers=0 -S -passes=convert-to-subscript %s 2>&1 | %lli | FileCheck -check-prefix=CHECK-EXEC %s
; RUN: opt -opaque-pointers=0 -S -opaque-pointers -passes=convert-to-subscript %s 2>&1 | %lli -opaque-pointers | FileCheck -check-prefix=CHECK-EXEC %s
; RUN: opt -opaque-pointers=0 -S %s 2>&1 | %lli | FileCheck -check-prefix=CHECK-EXEC %s
; CHECK-EXEC: passed

; RUN: opt -opaque-pointers=0 -S -passes=convert-to-subscript %s 2>&1 | FileCheck -check-prefix=CHECK-CONV %s
...
; RUN: opt -opaque-pointers=0 -S -passes=convert-to-subscript -opaque-pointers %s 2>&1 | FileCheck -check-prefix=OPAQUE-CONV %s
; @_Z3fooi (GetElementPtrInst)
