...
; RUN: opt -opaque-pointers=0 -S -passes=convert-to-subscript %s 2>&1 | FileCheck -check-prefix=CHECK %s
...
; RUN: opt -opaque-pointers=0 -S -passes=convert-to-subscript -opaque-pointers %s 2>&1 | FileCheck -check-prefix=OPAQUE %s
