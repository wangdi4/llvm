...
entry:
  ...
  %1 = bitcast <16 x i32> %xor.i.i to <16 x float>
  %2 = bitcast <32 x half> %rhs.coerce to <16 x float>
  %3 = tail call fast <16 x float> @llvm.x86.avx512fp16.mask.vfmulc.ph.512(<16 x float> %2, <16 x float> %1, <16 x float> zeroinitializer, i16 -1, i32 4) #2
  %4 = bitcast <16 x float> %3 to <32 x half>
  %add = fadd fast <32 x half> %4, %acc.coerce
  ret <32 x half> %add
}
...
; CHECK-LABEL: test6:
; CHECK:       # %bb.0: # %entry
; CHECK-NEXT:    vxorps {{.*}}(%rip){1to4}, %xmm1, %xmm1
...
; CHECK-NEXT:    retq
entry:
  %xor1.i = xor <8 x i64> %lhs.coerce.conj, <i64 576460752437641216, i64 576460752437641216, i64 576460752437641216, i64 576460752437641216, i64 576460752437641216, i64 576460752437641216, i64 576460752437641216, i64 576460752437641216>
  %0 = bitcast <8 x i64> %xor1.i to <16 x float>
  %1 = bitcast <32 x half> %rhs.coerce to <16 x float>
  %2 = tail call fast <16 x float> @llvm.x86.avx512fp16.mask.vfmulc.ph.512(<16 x float> %0, <16 x float> %1, <16 x float> zeroinitializer, i16 -1, i32 4) #2
