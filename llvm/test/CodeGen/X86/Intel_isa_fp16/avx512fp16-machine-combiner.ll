...
; RUN: llc -mtriple=x86_64-unknown-unknown -mcpu=x86-64 -mattr=avx512fp16 -enable-unsafe-fp-math -enable-no-nans-fp-math -enable-no-signed-zeros-fp-math -machine-combiner-verify-pattern-order=true < %s | FileCheck %s

; Incremental updates of the instruction depths should be enough for this test
; case.
; RUN: llc -mtriple=x86_64-unknown-unknown -mcpu=x86-64 -mattr=avx512fp16 -enable-unsafe-fp-math -enable-no-nans-fp-math -enable-no-signed-zeros-fp-math -machine-combiner-inc-threshold=0 < %s | FileCheck %s
...
; CHECK-NEXT:    retq
  %t0 = fdiv <8 x half> %x0, %x1
  %t1 = fadd <8 x half> %x2, %t0
  %t2 = fadd <8 x half> %x3, %t1
  ret <8 x half> %t2
}
...
; CHECK-NEXT:    retq
  %t0 = fdiv <16 x half> %x0, %x1
  %t1 = fadd <16 x half> %x2, %t0
  %t2 = fadd <16 x half> %x3, %t1
  ret <16 x half> %t2
}
...
; CHECK-NEXT:    retq
  ...
  %t1 = fmul <16 x half> %x2, %t0
  %t2 = fmul <16 x half> %x3, %t1
  ret <16 x half> %t2
}
...
; CHECK-NEXT:    retq
  %t0 = fdiv <32 x half> %x0, %x1
  %t1 = fadd <32 x half> %x2, %t0
  %t2 = fadd <32 x half> %x3, %t1
  ret <32 x half> %t2
}
...
; CHECK-NEXT:    retq
  ...
  %t1 = fmul <32 x half> %x2, %t0
  %t2 = fmul <32 x half> %x3, %t1
  ret <32 x half> %t2
}
