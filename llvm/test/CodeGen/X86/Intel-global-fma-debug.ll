...
; RUN: llc < %s -mtriple=x86_64-unknown-linux-gnu -mcpu=core-avx2 -fp-contract=fast -enable-unsafe-fp-math | FileCheck %s
...
; CHECK-LABEL: _Z8computexv:
; CHECK:       %bb.0: # %entry
; CHECK:       vmulsd {{.*xmm.*xmm.*xmm}}
; CHECK-NEXT:  vfmsub231sd {{.*#+}} xmm1 = (xmm0 * mem) - xmm1
; CHECK-NEXT:  vmulsd {{.*xmm.*xmm.*xmm0}}
; CHECK-NEXT:  .Ltmp
; CHECK-NEXT:  #DEBUG_VALUE: computex:fpair <- $xmm0
entry:
