...
; RUN: llc < %s -mtriple=x86_64-- -mattr=+avx512f -mattr=+avx512bw -enable-intel-advanced-opts -O3 | FileCheck %s
...
; CHECK-NEXT:    vaddss {{.*}}(%rip), %xmm0, %xmm2
; CHECK-NEXT:    vmovss %xmm2, {{[-0-9]+}}(%r{{[sb]}}p) # 4-byte Spill
; CHECK-NEXT:    movl $1, %eax
; CHECK-NEXT:    subl %edi, %eax
; CHECK-NEXT:    movq %rax, {{[-0-9]+}}(%r{{[sb]}}p) # 8-byte Spill
; CHECK-NEXT:    vmulsd {{.*}}(%rip), %xmm1, %xmm1
...
; CHECK-NEXT:    vaddss {{.*}}(%rip), %xmm0, %xmm1
