...
; INTEL_CUSTOMIZATION
; RUN: llc < %s -mtriple=x86_64-unknown-unknown -mattr=+sse4.1 -disable-iml-trans | FileCheck %s --check-prefix=CHECK --check-prefix=SSE
; RUN: llc < %s -mtriple=x86_64-unknown-unknown -mattr=+avx -disable-iml-trans | FileCheck %s --check-prefix=CHECK --check-prefix=AVX --check-prefix=AVX1
; RUN: llc < %s -mtriple=x86_64-unknown-unknown -mattr=+avx2 -disable-iml-trans | FileCheck %s --check-prefix=CHECK --check-prefix=AVX --check-prefix=AVX2
