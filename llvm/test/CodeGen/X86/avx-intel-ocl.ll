...
; X64: vmovups {{.*}}(%rsp), {{%ymm([8-9]|1[0-5])}} ## 32-byte Reload
; X64: vmovups {{.*}}(%rsp), {{%ymm([8-9]|1[0-5])}} ## 32-byte Reload
define intel_ocl_bicc_avx <16 x float> @test_prolog_epilog(<16 x float> %a, <16 x float> %b) nounwind { ;INTEL
...
define i32 @test_int(i32 %a, i32 %b) nounwind {
    %c1 = add i32 %a, %b
	%c2 = call intel_ocl_bicc_avx i32 @func_int(i32 %c1, i32 %a) ;INTEL
    %c = add i32 %c2, %b
	ret i32 %c
}
...
entry:
  ...
  %call.i = tail call intel_ocl_bicc_avx <4 x float> @func_float4(<4 x float> %0, <4 x float> %1, <4 x float> %2) nounwind ;INTEL
  ...
  %call.i2 = tail call intel_ocl_bicc_avx <4 x float> @func_float4(<4 x float> %4, <4 x float> %5, <4 x float> %6) nounwind ;INTEL
