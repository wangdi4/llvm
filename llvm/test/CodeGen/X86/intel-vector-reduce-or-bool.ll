...
; RUN: llc < %s -mtriple=x86_64-unknown-unknown -enable-intel-advanced-opts -mattr=+sse4.1 | FileCheck %s --check-prefixes=SSE,SSE41
; RUN: llc < %s -mtriple=x86_64-unknown-unknown -enable-intel-advanced-opts -mattr=+avx2 | FileCheck %s --check-prefixes=AVX,AVX2
; RUN: llc < %s -mtriple=x86_64-unknown-unknown -enable-intel-advanced-opts -mattr=+avx512f,+avx512bw,+avx512vl | FileCheck %s --check-prefixes=AVX512,AVX512VL
...
; AVX:       # %bb.0:
; AVX-NEXT:    vmovdqa {{.*#+}} ymm2 = [0,1,4,5,8,9,12,13,8,9,12,13,12,13,14,15,16,17,20,21,24,25,28,29,24,25,28,29,28,29,30,31]
; AVX-NEXT:    vpshufb %ymm2, %ymm1, %ymm1
; AVX-NEXT:    vpermq {{.*#+}} ymm1 = ymm1[0,2,2,3]
; AVX-NEXT:    vmovdqa {{.*#+}} xmm3 = [255,255,255,255,255,255,255,255]
; AVX-NEXT:    vpand %xmm3, %xmm1, %xmm1
; AVX-NEXT:    vpshufb %ymm2, %ymm0, %ymm0
; AVX-NEXT:    vpermq {{.*#+}} ymm0 = ymm0[0,2,2,3]
...
; AVX:       # %bb.0:
; AVX-NEXT:    vpsllw $7, %ymm1, %ymm1
; AVX-NEXT:    vpmovmskb %ymm1, %eax
; AVX-NEXT:    shlq $32, %rax
; AVX-NEXT:    vpsllw $7, %ymm0, %ymm0
; AVX-NEXT:    vpmovmskb %ymm0, %ecx
...
; AVX-NEXT:    vpmovmskb %ymm0, %eax
; AVX-NEXT:    vpcmpeqb %ymm2, %ymm1, %ymm0
; AVX-NEXT:    vpmovmskb %ymm0, %ecx
