; NOTE: Assertions have been autogenerated by utils/update_llc_test_checks.py
; REQUIRES: intel_feature_isa_avx512_convert
; RUN: llc < %s -mtriple=i686-unknown-unknown -mattr=+avx512vl,+avx512convert,+avx512fp16 --show-mc-encoding | FileCheck %s --check-prefixes=CHECK,X86
; RUN: llc < %s -mtriple=x86_64-unknown-unknown -mattr=+avx512vl,+avx512convert,+avx512fp16 --show-mc-encoding | FileCheck %s --check-prefixes=CHECK,X64

declare <8 x half> @llvm.x86.avx512.mask.vcvt2ps2ph.128(<4 x float>, <4 x float>, <8 x half>, i8)

define <8 x half>@test_int_x86_avx512_mask_vcvt2ps2ph_128(<4 x float> %x0, <4 x float> %x1, <8 x half> %x2, i8 %x3) {
; X86-LABEL: test_int_x86_avx512_mask_vcvt2ps2ph_128:
; X86:       # %bb.0:
; X86-NEXT:    kmovb {{[0-9]+}}(%esp), %k1 # encoding: [0xc5,0xf9,0x90,0x4c,0x24,0x04]
; X86-NEXT:    vcvt2ps2ph %xmm1, %xmm0, %xmm2 {%k1} # encoding: [0x62,0xf2,0x7d,0x09,0x67,0xd1]
; X86-NEXT:    vcvt2ps2ph %xmm1, %xmm0, %xmm0 # encoding: [0xc4,0xe2,0x79,0x67,0xc1]
; X86-NEXT:    vaddph %xmm0, %xmm2, %xmm0 # encoding: [0x62,0xf5,0x6c,0x08,0x58,0xc0]
; X86-NEXT:    retl # encoding: [0xc3]
;
; X64-LABEL: test_int_x86_avx512_mask_vcvt2ps2ph_128:
; X64:       # %bb.0:
; X64-NEXT:    kmovd %edi, %k1 # encoding: [0xc5,0xfb,0x92,0xcf]
; X64-NEXT:    vcvt2ps2ph %xmm1, %xmm0, %xmm2 {%k1} # encoding: [0x62,0xf2,0x7d,0x09,0x67,0xd1]
; X64-NEXT:    vcvt2ps2ph %xmm1, %xmm0, %xmm0 # encoding: [0xc4,0xe2,0x79,0x67,0xc1]
; X64-NEXT:    vaddph %xmm0, %xmm2, %xmm0 # encoding: [0x62,0xf5,0x6c,0x08,0x58,0xc0]
; X64-NEXT:    retq # encoding: [0xc3]
  %res =  call <8 x half> @llvm.x86.avx512.mask.vcvt2ps2ph.128(<4 x float> %x0, <4 x float> %x1, <8 x half> %x2, i8 %x3)
  %res1 = call <8 x half> @llvm.x86.avx512.mask.vcvt2ps2ph.128(<4 x float> %x0, <4 x float> %x1, <8 x half> %x2, i8 -1)
  %res2 = fadd <8 x half> %res, %res1
  ret <8 x half> %res2
}

declare <16 x half> @llvm.x86.avx512.mask.vcvt2ps2ph.256(<8 x float>, <8 x float>, <16 x half>, i16)

define <16 x half>@test_int_x86_avx512_mask_vcvt2ps2ph_256(<8 x float> %x0, <8 x float> %x1, <16 x half> %x2, i16 %x3) {
; X86-LABEL: test_int_x86_avx512_mask_vcvt2ps2ph_256:
; X86:       # %bb.0:
; X86-NEXT:    kmovw {{[0-9]+}}(%esp), %k1 # encoding: [0xc5,0xf8,0x90,0x4c,0x24,0x04]
; X86-NEXT:    vcvt2ps2ph %ymm1, %ymm0, %ymm2 {%k1} # encoding: [0x62,0xf2,0x7d,0x29,0x67,0xd1]
; X86-NEXT:    vcvt2ps2ph %ymm1, %ymm0, %ymm0 # encoding: [0xc4,0xe2,0x7d,0x67,0xc1]
; X86-NEXT:    vaddph %ymm0, %ymm2, %ymm0 # encoding: [0x62,0xf5,0x6c,0x28,0x58,0xc0]
; X86-NEXT:    retl # encoding: [0xc3]
;
; X64-LABEL: test_int_x86_avx512_mask_vcvt2ps2ph_256:
; X64:       # %bb.0:
; X64-NEXT:    kmovd %edi, %k1 # encoding: [0xc5,0xfb,0x92,0xcf]
; X64-NEXT:    vcvt2ps2ph %ymm1, %ymm0, %ymm2 {%k1} # encoding: [0x62,0xf2,0x7d,0x29,0x67,0xd1]
; X64-NEXT:    vcvt2ps2ph %ymm1, %ymm0, %ymm0 # encoding: [0xc4,0xe2,0x7d,0x67,0xc1]
; X64-NEXT:    vaddph %ymm0, %ymm2, %ymm0 # encoding: [0x62,0xf5,0x6c,0x28,0x58,0xc0]
; X64-NEXT:    retq # encoding: [0xc3]
  %res =  call <16 x half> @llvm.x86.avx512.mask.vcvt2ps2ph.256(<8 x float> %x0, <8 x float> %x1, <16 x half> %x2, i16 %x3)
  %res1 = call <16 x half> @llvm.x86.avx512.mask.vcvt2ps2ph.256(<8 x float> %x0, <8 x float> %x1, <16 x half> %x2, i16 -1)
  %res2 = fadd <16 x half> %res, %res1
  ret <16 x half> %res2
}

declare <8 x half> @llvm.x86.avx512.mask.vcvtbf162ph.128(<8 x i16>,  <8 x half>, i8)

define <8 x half>@test_int_x86_avx512_mask_vcvtbf162ph_128(<8 x i16> %x0, <8 x half> %x1, i8 %x2) {
; X86-LABEL: test_int_x86_avx512_mask_vcvtbf162ph_128:
; X86:       # %bb.0:
; X86-NEXT:    kmovb {{[0-9]+}}(%esp), %k1 # encoding: [0xc5,0xf9,0x90,0x4c,0x24,0x04]
; X86-NEXT:    vcvtbf162ph %xmm0, %xmm1 {%k1} # encoding: [0x62,0xf2,0x7e,0x09,0x67,0xc8]
; X86-NEXT:    vcvtbf162ph %xmm0, %xmm0 # encoding: [0xc4,0xe2,0x7a,0x67,0xc0]
; X86-NEXT:    vaddph %xmm0, %xmm1, %xmm0 # encoding: [0x62,0xf5,0x74,0x08,0x58,0xc0]
; X86-NEXT:    retl # encoding: [0xc3]
;
; X64-LABEL: test_int_x86_avx512_mask_vcvtbf162ph_128:
; X64:       # %bb.0:
; X64-NEXT:    kmovd %edi, %k1 # encoding: [0xc5,0xfb,0x92,0xcf]
; X64-NEXT:    vcvtbf162ph %xmm0, %xmm1 {%k1} # encoding: [0x62,0xf2,0x7e,0x09,0x67,0xc8]
; X64-NEXT:    vcvtbf162ph %xmm0, %xmm0 # encoding: [0xc4,0xe2,0x7a,0x67,0xc0]
; X64-NEXT:    vaddph %xmm0, %xmm1, %xmm0 # encoding: [0x62,0xf5,0x74,0x08,0x58,0xc0]
; X64-NEXT:    retq # encoding: [0xc3]
  %res =  call <8 x half> @llvm.x86.avx512.mask.vcvtbf162ph.128(<8 x i16> %x0, <8 x half> %x1, i8 %x2)
  %res1 = call <8 x half> @llvm.x86.avx512.mask.vcvtbf162ph.128(<8 x i16> %x0, <8 x half> %x1, i8 -1)
  %res2 = fadd <8 x half> %res, %res1
  ret <8 x half> %res2
}

declare <16 x half> @llvm.x86.avx512.mask.vcvtbf162ph.256(<16 x i16>,  <16 x half>, i16)

define <16 x half>@test_int_x86_avx512_mask_vcvtbf162ph_256(<16 x i16> %x0, <16 x half> %x1, i16 %x2) {
; X86-LABEL: test_int_x86_avx512_mask_vcvtbf162ph_256:
; X86:       # %bb.0:
; X86-NEXT:    kmovw {{[0-9]+}}(%esp), %k1 # encoding: [0xc5,0xf8,0x90,0x4c,0x24,0x04]
; X86-NEXT:    vcvtbf162ph %ymm0, %ymm1 {%k1} # encoding: [0x62,0xf2,0x7e,0x29,0x67,0xc8]
; X86-NEXT:    vcvtbf162ph %ymm0, %ymm0 # encoding: [0xc4,0xe2,0x7e,0x67,0xc0]
; X86-NEXT:    vaddph %ymm0, %ymm1, %ymm0 # encoding: [0x62,0xf5,0x74,0x28,0x58,0xc0]
; X86-NEXT:    retl # encoding: [0xc3]
;
; X64-LABEL: test_int_x86_avx512_mask_vcvtbf162ph_256:
; X64:       # %bb.0:
; X64-NEXT:    kmovd %edi, %k1 # encoding: [0xc5,0xfb,0x92,0xcf]
; X64-NEXT:    vcvtbf162ph %ymm0, %ymm1 {%k1} # encoding: [0x62,0xf2,0x7e,0x29,0x67,0xc8]
; X64-NEXT:    vcvtbf162ph %ymm0, %ymm0 # encoding: [0xc4,0xe2,0x7e,0x67,0xc0]
; X64-NEXT:    vaddph %ymm0, %ymm1, %ymm0 # encoding: [0x62,0xf5,0x74,0x28,0x58,0xc0]
; X64-NEXT:    retq # encoding: [0xc3]
  %res =  call <16 x half> @llvm.x86.avx512.mask.vcvtbf162ph.256(<16 x i16> %x0, <16 x half> %x1, i16 %x2)
  %res1 = call <16 x half> @llvm.x86.avx512.mask.vcvtbf162ph.256(<16 x i16> %x0, <16 x half> %x1, i16 -1)
  %res2 = fadd <16 x half> %res, %res1
  ret <16 x half> %res2
}

declare <8 x i16> @llvm.x86.avx512.mask.vcvtneph2bf16.128(<8 x half>, <8 x i16>, i8)

define <8 x i16>@test_int_x86_avx512_mask_vcvtneph2bf16_128(<8 x half> %x0, <8 x i16> %x1, i8 %x2) {
; X86-LABEL: test_int_x86_avx512_mask_vcvtneph2bf16_128:
; X86:       # %bb.0:
; X86-NEXT:    kmovb {{[0-9]+}}(%esp), %k1 # encoding: [0xc5,0xf9,0x90,0x4c,0x24,0x04]
; X86-NEXT:    vcvtneph2bf16 %xmm0, %xmm1 {%k1} # encoding: [0x62,0xf2,0x7f,0x09,0x67,0xc8]
; X86-NEXT:    vcvtneph2bf16 %xmm0, %xmm0 # encoding: [0xc4,0xe2,0x7b,0x67,0xc0]
; X86-NEXT:    vpaddw %xmm0, %xmm1, %xmm0 # EVEX TO VEX Compression encoding: [0xc5,0xf1,0xfd,0xc0]
; X86-NEXT:    retl # encoding: [0xc3]
;
; X64-LABEL: test_int_x86_avx512_mask_vcvtneph2bf16_128:
; X64:       # %bb.0:
; X64-NEXT:    kmovd %edi, %k1 # encoding: [0xc5,0xfb,0x92,0xcf]
; X64-NEXT:    vcvtneph2bf16 %xmm0, %xmm1 {%k1} # encoding: [0x62,0xf2,0x7f,0x09,0x67,0xc8]
; X64-NEXT:    vcvtneph2bf16 %xmm0, %xmm0 # encoding: [0xc4,0xe2,0x7b,0x67,0xc0]
; X64-NEXT:    vpaddw %xmm0, %xmm1, %xmm0 # EVEX TO VEX Compression encoding: [0xc5,0xf1,0xfd,0xc0]
; X64-NEXT:    retq # encoding: [0xc3]
  %res =  call <8 x i16> @llvm.x86.avx512.mask.vcvtneph2bf16.128(<8 x half> %x0, <8 x i16> %x1, i8 %x2)
  %res1 = call <8 x i16> @llvm.x86.avx512.mask.vcvtneph2bf16.128(<8 x half> %x0, <8 x i16> %x1, i8 -1)
  %res2 = add <8 x i16> %res, %res1
  ret <8 x i16> %res2
}

declare <16 x i16> @llvm.x86.avx512.mask.vcvtneph2bf16.256(<16 x half>, <16 x i16>, i16)

define <16 x i16>@test_int_x86_avx512_mask_vcvtneph2bf16_256(<16 x half> %x0, <16 x i16> %x1, i16 %x2) {
; X86-LABEL: test_int_x86_avx512_mask_vcvtneph2bf16_256:
; X86:       # %bb.0:
; X86-NEXT:    kmovw {{[0-9]+}}(%esp), %k1 # encoding: [0xc5,0xf8,0x90,0x4c,0x24,0x04]
; X86-NEXT:    vcvtneph2bf16 %ymm0, %ymm1 {%k1} # encoding: [0x62,0xf2,0x7f,0x29,0x67,0xc8]
; X86-NEXT:    vcvtneph2bf16 %ymm0, %ymm0 # encoding: [0xc4,0xe2,0x7f,0x67,0xc0]
; X86-NEXT:    vpaddw %ymm0, %ymm1, %ymm0 # EVEX TO VEX Compression encoding: [0xc5,0xf5,0xfd,0xc0]
; X86-NEXT:    retl # encoding: [0xc3]
;
; X64-LABEL: test_int_x86_avx512_mask_vcvtneph2bf16_256:
; X64:       # %bb.0:
; X64-NEXT:    kmovd %edi, %k1 # encoding: [0xc5,0xfb,0x92,0xcf]
; X64-NEXT:    vcvtneph2bf16 %ymm0, %ymm1 {%k1} # encoding: [0x62,0xf2,0x7f,0x29,0x67,0xc8]
; X64-NEXT:    vcvtneph2bf16 %ymm0, %ymm0 # encoding: [0xc4,0xe2,0x7f,0x67,0xc0]
; X64-NEXT:    vpaddw %ymm0, %ymm1, %ymm0 # EVEX TO VEX Compression encoding: [0xc5,0xf5,0xfd,0xc0]
; X64-NEXT:    retq # encoding: [0xc3]
  %res =  call <16 x i16> @llvm.x86.avx512.mask.vcvtneph2bf16.256(<16 x half> %x0, <16 x i16> %x1, i16 %x2)
  %res1 = call <16 x i16> @llvm.x86.avx512.mask.vcvtneph2bf16.256(<16 x half> %x0, <16 x i16> %x1, i16 -1)
  %res2 = add <16 x i16> %res, %res1
  ret <16 x i16> %res2
}
