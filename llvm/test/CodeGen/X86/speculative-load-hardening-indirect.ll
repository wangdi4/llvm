...
<<<<<<< HEAD
; RUN: llc < %s -cgp-split-switch-critical-edge=false -debug-entry-values -mtriple=x86_64-unknown-linux-gnu -x86-speculative-load-hardening -stop-after=machineverifier
=======
; RUN: llc < %s -mtriple=x86_64-unknown-linux-gnu -x86-speculative-load-hardening -stop-after=machineverifier
>>>>>>> a82d3e8a6e67473c94a5ce6345372748e9b61718
...
; RUN: llc < %s -cgp-split-switch-critical-edge=false -mtriple=x86_64-unknown-linux-gnu -x86-speculative-load-hardening -relocation-model pic -data-sections -verify-machineinstrs=0 | FileCheck %s --check-prefix=X64-PIC
; RUN: llc < %s -cgp-split-switch-critical-edge=false -mtriple=x86_64-unknown-linux-gnu -x86-speculative-load-hardening -data-sections -mattr=+retpoline | FileCheck %s --check-prefix=X64-RETPOLINE
;
