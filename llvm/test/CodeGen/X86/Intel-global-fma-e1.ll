...
; RUN: llc < %s -mtriple=x86_64-unknown-unknown -mcpu=core-avx2 -do-x86-global-fma -x86-global-fma-control=0x100 -fp-contract=fast -enable-unsafe-fp-math | FileCheck --check-prefix=ALL --check-prefix=AVX2 %s
; RUN: llc < %s -mtriple=x86_64-unknown-unknown -mcpu=skx       -do-x86-global-fma -x86-global-fma-control=0x100 -fp-contract=fast -enable-unsafe-fp-math | FileCheck --check-prefix=ALL --check-prefix=AVX512 %s
