...
; CHECK-NEXT:    addl {{[-0-9]+}}(%e{{[sb]}}p), %eax # 4-byte Folded Reload ;INTEL
; CHECK-NEXT:    movl %eax, {{[-0-9]+}}(%e{{[sb]}}p) # 4-byte Spill ;INTEL
; CHECK-NEXT:    adcl %edx, %esi
; CHECK-NEXT:    movl %esi, {{[-0-9]+}}(%e{{[sb]}}p) # 4-byte Spill ;INTEL
; CHECK-NEXT:    movl 24(%edi), %eax ;INTEL
; CHECK-NEXT:    movl %eax, {{[-0-9]+}}(%e{{[sb]}}p) # 4-byte Spill
; CHECK-NEXT:    movl $-1028477379, %ebx # imm = 0xC2B2AE3D ;INTEL
...
; CHECK-NEXT:    movl {{[-0-9]+}}(%e{{[sb]}}p), %ebx # 4-byte Reload ;INTEL
; CHECK-NEXT:    movl %ebx, %eax ;INTEL
; CHECK-NEXT:    mull %edx
; INTEL_CUSTOMIZATION
...
; CHECK-NEXT:    mull %ebx
; CHECK-NEXT:    imull $326129324, %edi, %esi # imm = 0x137056AC ;INTEL
; CHECK-NEXT:    addl %edx, %esi ;INTEL
