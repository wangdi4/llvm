...
; RUN: %gold -m elf_x86_64 -plugin %llvmshlibdir/LLVMgold%shlibext --plugin-opt=save-temps -plugin-opt=opaque-pointers \
...
; RUN:   -u foo %t/a.bc --start-lib %t/b.bc %t/c.bc --end-lib -o %t/abc --gc-sections -plugin-opt=opaque-pointers
; RUN: llvm-readelf -x .data %t/abc | FileCheck %s --check-prefix=DATA
