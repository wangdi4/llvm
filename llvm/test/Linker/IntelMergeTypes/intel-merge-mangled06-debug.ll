...
; RUN: llvm-link -irmover-enable-merge-by-mangled-names -irmover-enable-module-verify -irmover-type-merging=false -debug-only=irmover-mangled-names -S %S/Inputs/intel-merge-mangled06-a.ll %S/Inputs/intel-merge-mangled06-b.ll %S/Inputs/intel-merge-mangled06-c.ll 2>&1 | FileCheck %s
...
; but they are in different modules. This is the same test case as
; intel-merge-mangled06.ll, but checks the debug information. It was created
; from these sources:
...
; CHECK-SAME: Inputs/intel-merge-mangled06-a.ll
; CHECK: -------------------------------------------------------

; CHECK: Merging types from source module:
; CHECK-SAME: Inputs/intel-merge-mangled06-b.ll
; CHECK:   Source type: %struct._ZTS10TestStruct.TestStruct = type { i32, i32 }
; CHECK:     Destination type: None
; CHECK: -------------------------------------------------------

; CHECK: Merging types from source module:
; CHECK-SAME: Inputs/intel-merge-mangled06-c.ll
; CHECK:   Source type: %struct._ZTS10TestStruct.TestStruct.0 = type { double, double }
; CHECK:     Destination type: None
; CHECK: -------------------------------------------------------
