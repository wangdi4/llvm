...
; RUN: llvm-link -irmover-enable-merge-by-mangled-names -irmover-enable-module-verify -irmover-type-merging=false -debug-only=irmover-mangled-names -irmover-enable-full-dtrans-types-check -S %S/Inputs/intel-merge-mangled07-a.ll %S/Inputs/intel-merge-mangled07-b.ll 2>&1 | FileCheck %s -check-prefix=CHECK-AB
...
; type needs to be repaired before inserting it. It is the same test case
; as intel-merge-mangled07.ll but it checks for debug information. The
; test case was created using the following example:
...
; CHECK-AB-SAME: Inputs/intel-merge-mangled07-a.ll
; CHECK-AB:   Source type: %struct._ZTS11TestStructA.TestStructA = type { %"__Intel$Empty$Struct"* }
; CHECK-AB:     Destination type: None
; CHECK-AB:     Fields that will be repaired:
; CHECK-AB:       Field [0]:
; CHECK-AB:         Original: %"__Intel$Empty$Struct"*
; CHECK-AB:         New: i32 (%struct._ZTS11TestStructA.TestStructA*)*
; CHECK-AB: -------------------------------------------------------

; CHECK-AB: Merging types from source module:
; CHECK-AB-SAME: Inputs/intel-merge-mangled07-b.ll
; CHECK-AB:   Source type: %struct._ZTS11TestStructA.TestStructA.0 = type { i32 (%struct._ZTS11TestStructA.TestStructA.0*)* }
; CHECK-AB:     Destination type: %struct._ZTS11TestStructA.TestStructA = type { i32 (%struct._ZTS11TestStructA.TestStructA*)* }
; CHECK-AB: -------------------------------------------------------
...
; RUN: llvm-link -irmover-enable-merge-by-mangled-names -irmover-enable-module-verify -irmover-type-merging=false -debug-only=irmover-mangled-names -S %S/Inputs/intel-merge-mangled07-b.ll %S/Inputs/intel-merge-mangled07-a.ll 2>&1 | FileCheck %s -check-prefix=CHECK-BA
...
; CHECK-BA-SAME: Inputs/intel-merge-mangled07-b.ll
; CHECK-BA:   Source type: %struct._ZTS11TestStructA.TestStructA = type { i32 (%struct._ZTS11TestStructA.TestStructA*)* }
; CHECK-BA:     Destination type: None
; CHECK-BA: -------------------------------------------------------

; CHECK-BA: Merging types from source module:
; CHECK-BA-SAME: Inputs/intel-merge-mangled07-a.ll
; CHECK-BA:   Source type: %struct._ZTS11TestStructA.TestStructA.0 = type { %"__Intel$Empty$Struct"* }
; CHECK-BA:     Destination type: %struct._ZTS11TestStructA.TestStructA = type { i32 (%struct._ZTS11TestStructA.TestStructA*)* }
; CHECK-BA: -------------------------------------------------------
