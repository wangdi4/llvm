  ...
  while (!Worklist.empty()) {
    ...
    if (const SCEVNAryExpr *N = dyn_cast<SCEVNAryExpr>(S))
      ...
      for (const Use &U : V->uses()) {
        ...
        // This is a fix being put in place to enable a community pulldown.
        // The same fix will be proposed for upstreaming to LLVM trunk and may
        // be present by the next pulldown.  Ask Andy Kaylor for details.

        // If the use is a PHI node UseBB will be the incoming block for the
        // use, so we must also check the parent block of UseInst in that case.
        if (UseBB->getTerminator()->isEHPad() ||
            (isa<PHINode>(UserInst) &&
             UserInst->getParent()->getTerminator()->isEHPad()))
          continue;
