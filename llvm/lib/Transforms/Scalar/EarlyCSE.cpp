...
=======
        if (InVal.IsLoad)
          if (auto *I = dyn_cast<Instruction>(Op))
            combineMetadataForCSE(I, &Inst, false);
>>>>>>> 084ca632ac81437e5517ae5933b22d2b8e9a2bcd
        if (!Inst.use_empty())
...
bool EarlyCSE::run() {
#if INTEL_CUSTOMIZATION
  // llorg uses "non-optimized" MSSA form to save compile time. This misses
  // too many cases in large load-store blocks, like what is generated by the
  // Fortran FE.
  // "optimized" MSSA form uses AA to try to get the most accurate def
  // information, for a given use. Otherwise typically the closest def is
  // chosen and the client must refine the result.
  if (MSSA)
