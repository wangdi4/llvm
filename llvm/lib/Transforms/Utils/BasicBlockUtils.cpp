...
#if INTEL_COLLAB
    // The latch block may be a "double latch" handling two nested loops:
    // Edge A goes to the inner loop header, edge B goes to the outer loop
    // header. Passes that update the opt-report or rely on loop count info
    // may be sensitive to duplication of MDs, so we do not copy the MD to the
    // new latch, even though the community code prefers to do this.
    // These double-latches may be generated by OpenMP lowering in paropt.
    Loop *InnerLoop = LI->getLoopFor(OldLatch);
    if (InnerLoop && InnerLoop != L && InnerLoop->getLoopLatch() == OldLatch)
      OldLatch = nullptr;
#endif // INTEL_COLLAB
  } else
