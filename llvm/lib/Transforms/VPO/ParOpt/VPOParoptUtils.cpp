...
CallInst* VPOParoptUtils::genRTLKmpcBeginCall(
  Function    *F,
  Instruction *AI,
  StructType  *IdentTy
)
{
  ...
  Constant* FnC = M->getOrInsertFunction("__kmpc_begin",
                                       Type::getVoidTy(C),
...
CallInst* VPOParoptUtils::genRTLKmpcEndCall(
  Function    *F,
  Instruction *AI,
  StructType  *IdentTy
)
{
  ...
  Constant* FnC = M->getOrInsertFunction("__kmpc_end",
                                         Type::getVoidTy(C),
...
CallInst* VPOParoptUtils::genRTLKmpcGlobalThreadNumCall(
  Function    *F, 
  Instruction *AI, 
  StructType  *IdentTy
)
{
  ...
  FunctionType* FnGetTidTy = FunctionType::get(Type::getInt32Ty(C),
                                               PointerType::getUnqual(IdentTy),
                                               false);
...
AllocaInst* VPOParoptUtils::genKmpcLocfromDebugLoc(
  Function    *F, 
  Instruction *AI, 
  StructType  *IdentTy, 
  int Flags, 
...
)
{
  ...
  int vpo_emit_source_location = 1;
  ...
  switch (vpo_emit_source_location) {

    case 0: KmpLoc = ";unknown;unknown;0;0;;\00";
            break;

    case 1: KmpLoc = ";unknown;" + FnName.str() + ";" + std::to_string(SLine) + 
                     ";" + std::to_string(ELine) + ";;\00"; 
            break;

    case 2: KmpLoc =  ";" + Path.str() + "/" + File.str() + ";" + 
                      FnName.str() + ";" + std::to_string(SLine) + 
                      ";" + std::to_string(ELine) + ";;\00";
            break;
    default:
  ...
  Constant* VarLoc = new GlobalVariable(*M, LocStrTy, false,
  ...
  AllocaInst *KmpcLoc = new AllocaInst(IdentTy, 
                                       "loc." + std::to_string(SLine) + "."  
                                              + std::to_string(ELine), AI);
  KmpcLoc->setAlignment(8);

  GetElementPtrInst *FlagsPtr = GetElementPtrInst::Create(IdentTy,
                                   KmpcLoc, {ValueZero, ValueOne}, 
                                   "flags." + std::to_string(SLine) + "."  
                                            + std::to_string(ELine), AI);
  ...
  GetElementPtrInst *PSrcPtr = GetElementPtrInst::Create(IdentTy,
                                       KmpcLoc, {ValueZero, ValueFour}, 
                                       "psource." + std::to_string(SLine) + "." 
                                                  + std::to_string(ELine), AI);
...
AllocaInst* VPOParoptUtils::genKmpcLocforExplicitBarrier(
  Function    *F, 
  Instruction *AI, 
  StructType  *IdentTy, 
  BasicBlock  *BB
)
{
  ...
  AllocaInst *KmpcLoc = VPOParoptUtils::genKmpcLocfromDebugLoc(F, AI,
                                                  IdentTy, Flags, BB, BB);
  return KmpcLoc;
}

// Generate source location information for Implicit barrier
AllocaInst* VPOParoptUtils::genKmpcLocforImplicitBarrier(
  WRegionNode *W,
  Function    *F, 
  Instruction *AI, 
  StructType  *IdentTy, 
