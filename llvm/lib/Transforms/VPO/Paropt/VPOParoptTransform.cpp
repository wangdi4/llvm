...
bool VPOParoptTransform::genAtomicFreeReductionGlobalFini(
  ...
  auto *GepToSkip = cast<GetElementPtrInst>(ReductionValueLoc);
  ReductionValueLoc = GepToSkip->getPointerOperand();
...
bool VPOParoptTransform::genReductionScalarFini(
  ...
  if (UseGlobalUpdates && IsArraySection) {
    OptimizationRemark R(DEBUG_TYPE, "AtomicFreeReduction",
                         W->getEntryDirective());
    R << ore::NV("Kind", RedI->getOpName())
      << " global atomic-free GPU reduction is not supported for array sections "
         "yet";
    ORE.emit(R);
    UseGlobalUpdates = false;
  }
  ...
  bool UseExistingGlobalLoop = UseGlobalUpdates &&
                               AtomicFreeRedGlobalUpdateBBs.count(W);
...
bool VPOParoptTransform::genReductionFini(WRegionNode *W, ReductionItem *RedI,
  ...
  // in precense of SLM or tree local update
  if (LocalRedVar) {
...
bool VPOParoptTransform::genRedAggregateInitOrFini(
  ...
  auto GenArrSecLoopBegin = [NumElements, IsInit, &DT, &BodyBB, DestElementTy,
                             &DoneBB, &DestEnd,
