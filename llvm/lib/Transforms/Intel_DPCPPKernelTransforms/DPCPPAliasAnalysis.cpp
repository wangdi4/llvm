//===- DPCPPAliasAnalysis.cpp - addrspace based alias analysis ------------===//
//
...
DPCPPAAResult DPCPPAliasAnalysis::run(Function &, FunctionAnalysisManager &) {
  return DPCPPAAResult{};
}

DPCPPAAResult::AACallbackVH::AACallbackVH(Value *V, DPCPPAAResult *AAR)
    : CallbackVH(V), AAR(AAR) {}

bool DPCPPAAResult::invalidate(Function &, const PreservedAnalyses &PA,
                               FunctionAnalysisManager::Invalidator &) {
  // Check whether the analysis has been explicitly invalidated. Otherwise, it's
  // stateless and remains preserved.
  auto PAC = PA.getChecker<DPCPPAliasAnalysis>();
  return !PAC.preservedWhenStateless();
}

void DPCPPAAResult::AACallbackVH::deleted() {
  assert(AAR && "AACallbackVH called with a null OCLAliasAnalysis!");
  AAR->deleteValue(getValPtr());
}

void DPCPPAAResult::AACallbackVH::allUsesReplacedWith(Value *V) {
  assert(AAR && "AACallbackVH called with a null OCLAliasAnalysis!");
...
void DPCPPAAResult::deleteValue(Value *V) { ValueMap.erase(V); }
void DPCPPAAResult::copyValue(Value * /*From*/, Value * /*To*/) {
  // Do nothing, i.e. let new value compute in resolveAddressSpace.
}
void DPCPPAAResult::addEscapingUse(Use &U) {
  resolveAddressSpace(U.get(), true);
}

void DPCPPAAResult::rauwValue(Value *OldV, Value *NewV) {
...
DPCPPAAResult::ResolveResult DPCPPAAResult::cacheResult(SmallValueSet &Values,
                                                        ResolveResult RR) {
  for (auto *V : Values)
    ValueMap.insert(
...
// DPCPP/OpenCL address space although the address does belong to one (e.g.
...
// __generic address space DPCPPAA can not say for sure that another pointer
// is in disjoint address space.
//
// Since DPCPP/OpenCL's address spaces are disjoint, a single relation of an
...
// DPCPP/OpenCL address space).
DPCPPAAResult::ResolveResult DPCPPAAResult::resolveAddressSpace(const Value *V,
                                                                bool Force) {
  ...
  if (AP && BP) {
    ...
    if (RR1.isResolved() && RR1.isResolved()) {
      ...
      if (IsInSpace(APAddressSpace, DisjointASs) &&
        ...
        LLVM_DEBUG(dbgs() << "DPCPPAAResult NoAlias: " << *A << ", " << *B
                          << "\n");
