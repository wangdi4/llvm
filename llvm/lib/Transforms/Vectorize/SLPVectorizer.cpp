...
static LoadsState canVectorizeLoads(
  ...
  if (MaxSplitLoads > 0 && (Order.empty() || Order.size() == VL.size())) {
    ...
    auto &&IsConsecutive = [&SE, &DL, ScalarTy](Value *Ptr0, Value *PtrN,
                                                int Size) {
      Optional<int> Diff =
          getPointersDiff(ScalarTy, Ptr0, ScalarTy, PtrN, DL, SE);
    ...
    auto &&TryGroupSize = [ScalarTy, VF, &LoadGroups, IsConsecutive, &SE,
                           &DL](ArrayRef<Value *> Pointers,
    ...
    for (unsigned Split = 1; Split <= MaxSplitNums; ++Split) {
      unsigned GroupSize = VF >> Split;
      if (GroupSize < 2)
        break;
      if (TryGroupSize(PointerOps, Order, GroupSize))
        return LoadsState::LoadGroups;
    }
...
void BoUpSLP::buildTree_rec(ArrayRef<Value *> VL_, unsigned Depth, // INTEL
                            const EdgeInfo &UserTreeIdxC) {        // INTEL
#if INTEL_CUSTOMIZATION
      ...
      for (unsigned MinVF = getMinVecRegSize() / (2 * Sz); VF >= MinVF;
        ...
        for (unsigned Cnt = StartIdx, End = VL.size(); Cnt + VF <= End;
          ...
          if (!VectorizedLoads.count(Slice.front()) &&
            ...
            case LoadsState::LoadGroups: // INTEL
              // Mark the vectorized loads so that we don't vectorize them
              // again.
              if (LS == LoadsState::Vectorize || // INTEL
                  LS == LoadsState::LoadGroups)  // INTEL
                ++VectorizedCnt;
              else
                ++ScatterVectorizeCnt;
