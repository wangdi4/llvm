...
VPInstructionCost VPlanTTICostModel::getInsertExtractElementsCost(
  ...
  for(unsigned Idx = 0; Idx < VF; Idx++)
    Cost += TTI.getVectorInstrCost(Opcode, VecTy,
                                   TTI::TCK_RecipThroughput, Idx);
  return Cost;
}
...
VPlanTTICostModel::getTTICostForVF(const VPInstruction *VPInst, unsigned VF) {
  ...
  case Instruction::Select: {
    ...
    // Use cost of And for selects that are typically generated by the
    // predicator instead of And to avoid propagating poison value. These
    // selects instructions look like select i1 mask1, i1 mask2, i1 false.
    const VPConstant *ConstOp2Val = nullptr;
    if (OpTy->isIntegerTy(1) &&
        (ConstOp2Val = dyn_cast<VPConstant>(VPInst->getOperand(2))) &&
        ConstOp2Val->getConstant()->isZeroValue())
        return getArithmeticInstructionCost(
            Instruction::And, VPInst->getOperand(0), VPInst->getOperand(1),
            VPInst->getType(), VF);
      ...
      if (StructType *STy = dyn_cast<StructType>(OpTy)) {
        ...
        for(unsigned Idx = 0; Idx < VF; Idx++)
          ExtractCost += TTI.getVectorInstrCost(Instruction::ExtractElement,
            VecCondTy, TTI::TCK_RecipThroughput, Idx);
    ...
    if (!isa<VPConstant>(VPInd->getOperand(1))) {
      if (IsAdd)
        ...
        for (unsigned Index = 0; Index < VF; Index++)
          Cost += TTI.getVectorInstrCost(Instruction::InsertElement, StepVecTy,
                                         TTI::TCK_RecipThroughput, Index);
