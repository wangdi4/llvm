...
public:
  VPOCodeGenHIR(TargetLibraryInfo *TLI, TargetTransformInfo *TTI,
      ...
      : TLI(TLI), TTI(TTI), SRA(SRA), Plan(Plan), VLSA(VLSA), Fn(Fn),
        ...
        CFGInfo(CFGInfo), VPLoopDescrs(LoopDescrs) {}
...
private:
  ...
  // List of loops that should be stripped. We strip the loop leaving
  // one-iteration code.
  SmallVector<HLLoop *, 2> StripCandidates;
  ...
  // Descriptors of top VPLoop.
  VPLoopDescrMap &VPLoopDescrs;
