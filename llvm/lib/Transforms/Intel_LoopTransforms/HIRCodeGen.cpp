...
private:
  ...
  private:
    ...
    // TODO unclear how blobs defined in region will be handled, their scev
    // refers to values generated by hlinst creation, and yet its initial def
    // is a regular ref for which initial value is stored into symbase memory
    Value *getBlobValue(int BlobIdx, Type *Ty) {
      ...
      // Expander shouldnt create new Bblocks, new IP is enc of current bblock
    ...
    std::string getTempName(RegDDRef *Ref) {
      ...
      return "t" + std::to_string(Ref->getSymBase());
    }
  };
...
Value *HIRCodeGen::CGVisitor::visitScalar(RegDDRef *Ref) {
  ...
  // For rval linear temps, we shouldnt need a load,
  // generate value directly from CE
  if (ScalarCE->isProperLinear() && Ref->isRval())
    return visitCanonExpr(ScalarCE);

  AllocaInst *Alloca;
  std::string TempName = getTempName(Ref);
  if (!NamedValues.count(TempName)) {
    Alloca = CreateEntryBlockAlloca(TempName, Ref->getType());
    NamedValues[TempName] = Alloca;
  } else {
    Alloca = NamedValues[TempName];
  }

  // For lvals return address of temp, but for rvals we want the loaded value
  if (Ref->isLval()) {
    return Alloca;
  } else {
...
Value *HIRCodeGen::CGVisitor::visitLoop(HLLoop *L) {
  ...
  if (!NamedValues.count(IVName)) {
    Alloca = CreateEntryBlockAlloca(IVName, L->getIVType());
