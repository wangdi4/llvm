...
bool intersects(const BasePtrIndexSetTy &Cur,
                const BasePtrIndexSetTy &History) {

  return any_of(
      Cur, [&History](unsigned BaseIndex) { return History.count(BaseIndex); });
}
...
bool InnermostLoopAnalyzer::areMostlyStructuallyStencilRefs(
    RefGroupVecTy &Groups) const {

  // 0.5 is an exprimental choice.
  unsigned StencilGroupCountThreshold = static_cast<float>(Groups.size()) * 0.5;
...
const RegDDRef *InnermostLoopAnalyzer::getALval() const {
...
unsigned Transformer::getNumByStripLoops(ArrayRef<unsigned> StripmineSizes) {
  return count_if(StripmineSizes, [](unsigned Size) { return Size; });
}

bool Transformer::checkDimsToLoops(
    ArrayRef<unsigned> StripmineSizes,
    const LoopToDimInfoTy &InnermostLoopToDimInfos) {
...
bool Transformer::rewrite() {
  ...
  if (!computeByStripLoopBounds(InnermostLoopToAdjustingRef,
                                OrigToCloneIndexMap, AuxRefsForByStripBounds)) {
...
void Transformer::prepareAdjustingRefs(
    LoopToRefTy &InnermostLoopToAdjustingRef) const {
...
void Transformer::collectAdditionalLiveInsToByStripLoops(
    unsigned StartTopSortNum, unsigned LastTopSortNum) {
...
void Transformer::updateDefAtLevelOfSpatialLoops(HLNode *Node) const {
...
void Transformer::alignSpatialLoopBody(
    const LoopAndDimInfoTy &LoopAndDimInfo,
    const LoopToRefTy &InnermostLoopToAdjustingRef) const {

  HLLoop *InnermostLoop = LoopAndDimInfo.first;
  // Get a Ref to be used for Adjusting
  const RegDDRef *AdjustingRef = InnermostLoopToAdjustingRef.at(InnermostLoop);
...
const HLInst *Transformer::tracebackToLoad(const HLInst *Copy,
                                           DDGraph DDG) const {
...
void Transformer::printDDEdges(const HLInst *LoadInst, DDGraph DDG) {
  ...
  for (auto *BlobRef :
    ...
    if (DDG.incoming_edges_begin(BlobRef) != DDG.incoming_edges_end(BlobRef)) {
      dbgs() << "= incoming edges\n";
      BlobRef->dump();
      dbgs() << "\n";
...
const std::pair<const HLInst *, const HLInst *>
Transformer::findLoad(const HLDDNode *SrcNode, DDGraph DDG) const {
...
void Transformer::findLoadsOfTemp(
    DDGraph DDG, const RegDDRef *DestRef, unsigned AnchorNodeTopSortNum,
    SmallPtrSetImpl<const HLInst *> &LoadInsts,
    std::map<const HLInst *, const HLInst *> &CopyToLoadMap) const {
  ...
  for (auto *BlobRef : make_range(DestRef->blob_begin(), DestRef->blob_end())) {
...
void Transformer::collectLoadsToClone(
    unsigned AnchorNodeTopSortNum,
    SmallPtrSetImpl<const HLInst *> &LoadInstsToClone,
    SmallVectorImpl<std::pair<unsigned, unsigned>> &CopyToLoadIndexMap) {
...
bool Transformer::computeByStripLoopBounds(
  ...
  for (int DimNum = 1; DimNum <= GlobalNumDims; DimNum++) {
    ...
    for (auto &E : InnermostLoopToDimInfos) {
      ...
      (void)Replaced;
      AlignedLowerBounds[DimNum - 1].push_back(NewLBRef->getSingleCanonExpr());
      AuxRefs.push_back(TargetLoop->getLowerDDRef());

      LLVM_DEBUG(dbgs() << "LB after (" << Replaced << ") "; NewLBRef->dump(1);
                 dbgs() << "\n");

      RegDDRef *NewUBRef = TargetLoop->getUpperDDRef()->clone();
      Replaced = NewUBRef->replaceTempBlobs(BlobIndexMap);
      AlignedUpperBounds[DimNum - 1].push_back(NewUBRef->getSingleCanonExpr());
...
HLLoop *Transformer::addByStripLoops(
    HLNode *AnchorNode, const SmallPtrSetImpl<const HLInst *> &LoadInstsToClone,
    ArrayRef<const RegDDRef *> AuxRefsFromSpatialLoops) {
  ...
  for (unsigned DimNum = StripmineSizes.size(); DimNum >= 1; DimNum--) {
    ...
    ByStrip->getLowerDDRef()->populateTempBlobSymbases(LiveInsOfAllSpatialLoop);
    ByStrip->getUpperDDRef()->populateTempBlobSymbases(LiveInsOfAllSpatialLoop);
...
void Transformer::applyBlockingGuardsToSpatialLoops(
  ...
  for (unsigned DimNum = 1, Sizes = DimInfos.size(); DimNum <= Sizes;
    ...
    HLLoop *TargetLoop = getLoopMatchingDimNum(DimNum, DimInfos, InnermostLoop);
    if (!TargetLoop) {
...
bool Transformer::isNoBlockDim(unsigned DimNum,
                               ArrayRef<unsigned> StripmineSizes) {
...
// For a dimension, where blocking is not done for the corresponding loop,
// stripmine size is set to zero.
bool Transformer::isNoBlockDim(unsigned DimNum) const {
  return isNoBlockDim(DimNum, StripmineSizes);
}

void Transformer::removeDupCanonExprs(SmallVectorImpl<CanonExpr *> &CEs) {
...
// Return the loop matching DimNum.
// InnermostLoop and DimInfos are data to consult with.
const HLLoop *Transformer::getLoopMatchingDimNum(unsigned DimNum,
                                                 ArrayRef<DimInfoTy> DimInfos,
                                                 const HLLoop *InnermostLoop) {
...
HLLoop *Transformer::getLoopMatchingDimNum(unsigned DimNum,
                                           ArrayRef<DimInfoTy> DimInfos,
                                           HLLoop *InnermostLoop) {
...
unsigned Transformer::addLoopBoundsGuards(HLLoop *Loop, unsigned DimNum) const {
...
std::pair<BlobTy, unsigned>
Transformer::getConstantOrSingleBlob(const CanonExpr *CE) {
...
template <bool IsMin>
std::pair<BlobTy, unsigned>
Transformer::getGlobalMinMaxBlob(ArrayRef<CanonExpr *> Bounds) {
  assert(!Bounds.empty());
...
void ProfitablityAndLegalityChecker::visit(HLLoop *Loop) {
  markVisited(Loop);
...
bool ProfitablityAndLegalityChecker::analyzeLegality(HLLoop *Loop) {
...
bool ProfitablityAndLegalityChecker::stopAndWork(int CallSiteLoc,
                                                 const HLLoop *StopLoop) {
...
bool ProfitablityAndLegalityChecker::verifyTopSortOrderOfInnermostLoops()
    const {
  unsigned PrevNum = 0;
...
bool ProfitablityAndLegalityChecker::postCheck(const HLLoop *StopLoop) const {
