...
HLSwitch* HLNodeUtils::createHLSwitch() {
  return new HLSwitch();
}
...
HLInst* HLNodeUtils::createHLInst(Instruction* In) {
  return new HLInst(In);
}

HLIf* HLNodeUtils::createHLIf() {
  return new HLIf();
}

HLLoop* HLNodeUtils::createHLLoop(HLIf* ZttIf, bool isDoWh, 
  unsigned NumEx) {

  return new HLLoop(ZttIf, isDoWh, NumEx);
}
...
struct HLNodeUtils::LoopFinderUpdater {
  ...
  LoopFinderUpdater(bool IsFinder): FinderMode(IsFinder)
  , FoundLoop(false) { }
...
void HLNodeUtils::insertInternal(HLContainerTy& InsertContainer, 
  HLContainerTy::iterator Pos, HLContainerTy* RemoveContainer, 
  HLContainerTy::iterator First, HLContainerTy::iterator Last) {
...
void HLNodeUtils::insertImpl(HLNode* Parent, HLContainerTy::iterator Pos, 
  HLContainerTy* RemoveContainer, HLContainerTy::iterator First, 
  HLContainerTy::iterator Last, bool UpdateSeparator, bool PostExitSeparator) {

  assert( (!isa<HLRegion>(Pos) && !isa<HLRegion>(First)) && 
    "Transformations should not add/reorder regions!");
...
void HLNodeUtils::insertAsPreheaderPostexitImpl(HLLoop* Loop,
  HLContainerTy* RemoveContainer, HLContainerTy::iterator First,
...
bool HLNodeUtils::IsInPreheaderPostexitImpl(HLLoop* Loop, HLNode* Node, 
  bool Preheader) {
...
bool HLNodeUtils::IsInPreheader(HLLoop* Loop, HLNode* Node) {
  return IsInPreheaderPostexitImpl(Loop, Node, true);
}

bool HLNodeUtils::IsInPostexit(HLLoop* Loop, HLNode* Node) {
  return IsInPreheaderPostexitImpl(Loop, Node, false);
}

bool HLNodeUtils::IsInPreheaderOrPostexit(HLLoop* Loop, HLNode* Node) {
...
void HLNodeUtils::removeImpl(HLContainerTy::iterator First, 
  HLContainerTy::iterator Last, HLContainerTy* MoveContainer, bool Erase) {
  ...
  if (Erase) {
    ...
    MoveContainer->splice(MoveContainer->end(), *RemoveContainer, First, 
      Last);
  }
...
void HLNodeUtils::moveBefore(HLNode* Pos,
  HLContainerTy::iterator First, HLContainerTy::iterator Last) {
  assert(Pos && "Pos is null!");
...
void HLNodeUtils::moveAfter(HLNode* Pos,
  HLContainerTy::iterator First, HLContainerTy::iterator Last) {
  assert(Pos && "Pos is null!");
...
void HLNodeUtils::moveAsFirstIfChildren(HLIf* If, HLContainerTy::iterator First,
  HLContainerTy::iterator Last, bool IsThenChild) {
  HLContainerTy TempContainer;
...
void HLNodeUtils::moveAsLastIfChildren(HLIf* If, HLContainerTy::iterator First,
  ...
  removeImpl(First, Last, &TempContainer);
  insertImpl(If, IsThenChild ? If->then_end() : If->else_end(), 
    &TempContainer, TempContainer.begin(), TempContainer.end(), 
    !IsThenChild);  
}
