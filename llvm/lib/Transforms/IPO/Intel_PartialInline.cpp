...
// TODO: This flag will be enabled from the driver
...
bool PartialInliner::goToExit(BranchInst *BrInst) {
...
bool PartialInliner::isUsedForExit(Instruction *Inst,
                                   SetVector<Instruction *> &VisitedInst) {
...
Instruction *PartialInliner::getPHIUser(User *Usr, LoopInfo &LI,
                                        SetVector<Instruction *> &VisitedInst) {
...
bool PartialInliner::identifyLoopRegion(Function &F, Value *ArgVal) {
...
Value *PartialInliner::identifyEntryRegion(Function &F) {
...
bool PartialInliner::canSplitFunctionIntoRegions(Function &F) {
  ...
  // block
  Value *ArgValue = identifyEntryRegion(F);
  ...
  if (!identifyLoopRegion(F, ArgValue))
    return false;
...
// return true.
bool PartialInliner::checkFunctionProperties(Function &F) {
...
// means if we can split the input function in three regions,
//  *Arguments
//  *Loop
//  *Exit
// Else return false.
bool PartialInliner::isPartialInlineCandidate(Function &F, Module &M) {
  ...
  return canSplitFunctionIntoRegions(F);
}
...
bool PartialInliner::runImpl() {
  ...
  for (Function &F : M) {
    ...
    if (isPartialInlineCandidate(F, M)) {
