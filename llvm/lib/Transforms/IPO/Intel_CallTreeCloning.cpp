...
// Flag to print the progress log in detailed mode for Call-Tree Cloning (CTC)
static cl::opt<bool>
    CTCDetailedLog("call-tree-clone-detail-log", cl::init(false),
                   cl::ReallyHidden,
                   cl::desc("Call-Tree Cloning (CTC) detailed progress log"));

// Flag to print the progress log in detailed mode for Post Processing (PP)
static cl::opt<bool>
    PPDetailedLog("post-processing-detail-log", cl::init(false),
                  cl::ReallyHidden,
                  cl::desc("Post Processing (PP) detailed progress log"));

// Flag to print the progress log in detailed mode for MultiVersioning (MV)
static cl::opt<bool>
...
public:
  ...
  ParamIndSet &operator=(const ParamIndSet &) = delete;
...
void print_MVSeeds(
  ...
  for (auto &MVSeed : MVSeeds) {
    ...
    // print the mapped SetOfParamIndSets:
    dbgs() << SPIS.toString() << "\n";
  }
...
class CallTreeCloningImpl {
  ...
  ProgressLog CTCTracer;
#endif

public:
  CallTreeCloningImpl() {
    LLVM_DEBUG({
      // Initialize/Adjust Call-Tree Cloning Tracer:
      CTCTracer.setPassName(CTCSectionName);
      if (CTCDetailedLog)
...
public:
  PostProcessor(
    ...
    LLVM_DEBUG({
      // Initialize/Adjust Post-Process Tracer:
      Tracer.setPassName(PPSectionName);
      if (PPDetailedLog)
...
public:
  MultiVersionImpl(
    ...
    LLVM_DEBUG({
      ...
      Tracer.setPassName(MVSectionName);
      if (MVDetailedLog)
        Tracer.setDetailMode(true);
    });
  }
...
#if !defined(NDEBUG) || defined(LLVM_ENABLE_DUMP)
  ProgressLog Tracer; // Progress Tracer
#endif
}; // namespace
...
bool CallTreeCloningImpl::run(
  ...
  if (!checkThreshold(M)) {
    LLVM_DEBUG({
      dbgs() << "Disable CallTreeClone pass due to potential "
                "callgraph's size over threshold\n";
      CTCTracer.dump();
    });
    return false;
  }
  LLVM_DEBUG(CTCTracer.setStage(ProgressLog::PreliminaryAnalysis););
  ...
  LLVM_DEBUG({
    if (AlgSeeds.size())
      CTCTracer.setStage(ProgressLog::Collection);
  });
  ...
  // To check if Call-Tree Cloning (CTC) triggers or not, do the following:
  //
  // option: -debug-only=call-tree-clone
  // search "Call-Tree Cloning Triggered" or "Call-Tree Cloning NOT Triggered"
  // in the debug output.
  //
  // In case Call-Tree Cloning does not trigger, and there is a need to know
  // which stage the pass bailed out, do the following:
  //
  // options: -debug-only=call-tree-clone -mllvm -call-tree-clone-detail-log=1
  // search the following strings in the output:
  // "Call-Tree Cloning preliminary test", "Call-Tree Cloning collection",
  // "Call-Tree Cloning analysis", and/or "Call-Tree Cloning transformation".
  //
  // E.g.
  // The following strings will be available if Call-Tree Cloning triggers in
  // detailed mode:
  // Call-Tree Cloning preliminary analysis:  good
  // Call-Tree Cloning collection:  good
  // Call-Tree Cloning analysis:  good
  // Call-Tree Cloning transformation:  good
  // Call-Tree Cloning Triggered
  //
  // There are also prints of cloned function details.
  LLVM_DEBUG({
    CTCTracer.setStage(ProgressLog::Analysis);
    CTCTracer.setStage(ProgressLog::Transform);
    CTCTracer.dump();
    if (CTCDetailedLog)
      print_CloneRegistry("Function Clone details:", Clones);
  });
...
bool MultiVersionImpl::run(void) {
  ...
  // MV has no preliminary analysis stage, so give it a free pass.
  LLVM_DEBUG(Tracer.setStage(ProgressLog::PreliminaryAnalysis););
  ...
  // To check if MultiVersioning (MV) triggers or not, do the following:
  //
  // option: -debug-only=call-tree-clone
  // search "MultiVersioning Triggered" or "MultiVersioning NOT Triggered"
  // in the debug output.
  //
  // In case MultiVersioning does not trigger, and there is a need to know
  // which stage the pass bailed out, do the following:
  //
  // options: -debug-only=call-tree-clone -mllvm -multiversioning-detail-log=1
  // search the following strings in the output:
  // "MultiVersioning preliminary test", "MultiVersioning collection",
  // "MultiVersioning analysis", and/or "MultiVersioning transformation".
  //
  // E.g.
  // The following strings will be available if MultiVersioning triggers in
  // detailed mode:
  //
  // MultiVersioning preliminary analysis:  good
  // MultiVersioning collection:  good
  // MultiVersioning analysis:  good
  // MultiVersioning transformation:  good
  // MultiVersioning Triggered
  //
  // There are also prints of cloned function detailes.
  LLVM_DEBUG({
    Tracer.setStage(ProgressLog::Transform);
    Tracer.dump();
    if (MVDetailedLog) {
      print_MVSeeds("Multi-Versioning Function Seeds details:\n", MVSeeds);
      print_CloneRegistry("Clones: ", Clones);
    }
  });
