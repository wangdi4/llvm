...
void Verifier::visitDIDerivedType(const DIDerivedType &N) {
  ...
  CheckDI(N.getTag() == dwarf::DW_TAG_typedef ||
              ...
              N.getTag() == dwarf::DW_TAG_template_type_parameter || // INTEL
...
void Verifier::visitTemplateParams(const MDNode &N, const Metadata &RawParams) {
  ...
  for (Metadata *Op : Params->operands()) {
    CheckDI(((Op) && (isa<DITemplateParameter>(Op))) ||
            ...
            "invalid template parameter", &N, Params, Op); // INTEL
  }
}
...
void Verifier::visitSubscriptInst(SubscriptInst &I) {
  ...
  unsigned Width = 0;
  if (IsVector) {
    Width = PtrTy->isVectorTy() ? cast<FixedVectorType>(PtrTy)->getNumElements()
                                : 0;
...
#if INTEL_CUSTOMIZATION
      // CMPLRLLVM-46524: Intrinsics like @llvm.seh.scope.begin and
      // @llvm.seh.scope.end which do not throw do not need funclets
      // in catchpads or cleanuppads.
      auto *CalledFn =
