...
// The instrinsic @llvm.codemotion.fence is generated by this function.
// The purpose is to inhibit the codemotion for certain instructions.
// Given an example as follows.
//
// %arrayidx = getelementptr inbounds [10 x i32],
//                                    [10 x i32]* %pvtPtr, i64 0, i64 5
//
// DIR.OMP.TARGET.15:
// %1 = call token @llvm.directive.region.entry()
//      [ "DIR.OMP.TARGET"(), "QUAL.OMP.PRIVATE"([10 x i32]* %pvtPtr) ]
//
// %arrayidx1 = getelementptr inbounds [10 x i32],
//                                     [10 x i32]* %pvtPtr, i64 0, i64 5
//
// The instruction %arrayidx1 can be eliminated due to the cse opportunity
// between the instruction %arrayidx and %arrayidx1.
// The compiler has to inhibit such transformation since it is across
// the omp region. Therefore the instrinsic @llvm.codemotion.fence is
// introduced to serve this purpose. Here is the output.
//
//
// %arrayidx = getelementptr inbounds [10 x i32],
//                                    [10 x i32]* %pvtPtr, i64 0, i64 5
...
// %1 = call token @llvm.directive.region.entry()
//      [ "DIR.OMP.TARGET"(), "QUAL.OMP.PRIVATE"([10 x i32]* %pvtPtr) ]
// %2 = call [10 x i32]* @llvm.codemotion.fence.p0a10i32([10 x i32]* %pvtPtr)
// %arrayidx1 = getelementptr inbounds [10 x i32],
//                                     [10 x i32]* %2, i64 0, i64 5
//
Instruction *IRBuilderBase::CreateCodemotionFence(Value *Ptr) {
  Value *CPtr = Ptr;
  if (StructType *STyp =
  ...
  Value *Ops[] = { CPtr };
  Module *M = BB->getParent()->getParent();
  Value *Fn = Intrinsic::getDeclaration(M, Intrinsic::codemotion_fence, Types);
  Instruction *Ret = createCallHelper(Fn, Ops, this);
...
// The intrinsic llvm.intel.std.container.ptr or 
// llvm.intel.std.container.ptr.iter is generated by this function.
// One example is as follows.
// %0 = call i32** @llvm.intel.std.container.ptr.iter.p0p0i32(
// i32** nonnull getelementptr in bounds (%"class.std::vector", 
// %"class.std::vector"* @myvector2, i64 0, i32 0, i32 0, i32 0))
//
Instruction *IRBuilderBase::CreateStdContainerCall(Value *Ptr, bool ITER) {
  Value *CPtr = Ptr;
  if (StructType *STyp =
          dyn_cast<StructType>(CPtr->getType()->getPointerElementType())) {
    if (STyp->isLiteral())
  ...
  Value *Ops[] = {CPtr};
  Module *M = BB->getParent()->getParent();
  Value *Fn = Intrinsic::getDeclaration(
      M, ITER ? Intrinsic::intel_std_container_ptr_iter
              : Intrinsic::intel_std_container_ptr,
      Types);
  Instruction *Ret = createCallHelper(Fn, Ops, this);
  cast<CallInst>(Ret)->setDoesNotThrow();
