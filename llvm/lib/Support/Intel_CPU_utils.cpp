...
public:
  ...
  const FeatureDefType &getFeatureDef(IntelLibFeatureId id) const {
    return proc_info_features[id];
  }
};
} // namespace
...
#define CPU_FEATURE_ADD_ISA_NAME(name)                                         \
  proc_info_features[__curr_isa__].isa_names = StringRef(name);
...
X86::getCpuFeatureBitmap(ArrayRef<StringRef> CpuFeatures) {
  ...
  for (auto CpuFeature : CpuFeatures) {
    ...
    FOR_EACH_LIBIRC_FEATURE(feature_id) {
      if (PIF->getFeatureDef(feature_id).isa_names == CpuFeature &&
          PIF->getFeatureDef(feature_id).autogenerated) {
        int bitpos = PIF->getFeatureDef(feature_id).bitpos;
