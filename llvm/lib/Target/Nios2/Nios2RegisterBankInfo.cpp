...
#include "Nios2GenRegisterBank.inc"

// This file will be TableGen'ed at some point.
#include "Nios2GenRegisterBankInfo.def"
...
Nios2RegisterBankInfo::Nios2RegisterBankInfo(const TargetRegisterInfo &TRI)
    : Nios2GenRegisterBankInfo() {
  static bool AlreadyInit = false;
  // We have only one set of register banks, whatever the subtarget
  // is. Therefore, the initialization of the RegBanks table should be
  // done only once. Indeed the table of all register banks
  // (Nios2::RegBanks) is unique in the compiler. At some point, it
  // will get tablegen'ed and the whole constructor becomes empty.
  if (AlreadyInit)
    return;
  AlreadyInit = true;

  const RegisterBank &RBGPR = getRegBank(Nios2::GPRRegBankID);
  (void)RBGPR;
  assert(&Nios2::GPRRegBank == &RBGPR &&
         "The order in RegBanks is messed up");

  // The GPR register bank is fully defined by all the registers in
  // GR64all + its subclasses.
  assert(RBGPR.covers(*TRI.getRegClass(Nios2::CPURegsRegClassID)) &&
         "Subclass not added?");
  assert(RBGPR.getSize() == 32 && "GPRs should hold up to 64-bit");
...
#define CHECK_VALUEMAP_CROSSREGCPY(RBNameDst, RBNameSrc, Size)                 \
  do {                                                                         \
    ...
    const ValueMapping *Map = getCopyMapping(                                  \
        Nios2::RBNameDst##RegBankID, Nios2::RBNameSrc##RegBankID, Size);  \
    (void)Map;                                                                 \
    assert(Map[0].BreakDown ==                                                 \
               &Nios2GenRegisterBankInfo::PartMappings[PartialMapDstIdx] &&  \
           Map[0].NumBreakDowns == 1 && #RBNameDst #Size                       \
           " Dst is incorrectly initialized");                                 \
    assert(Map[1].BreakDown ==                                                 \
               &Nios2GenRegisterBankInfo::PartMappings[PartialMapSrcIdx] &&  \
...
const RegisterBank &Nios2RegisterBankInfo::getRegBankFromRegClass(
    const TargetRegisterClass &RC) const {
  switch (RC.getID()) {
  case Nios2::CPURegsRegClassID:
  case Nios2::GPROutRegClassID:
    return getRegBank(Nios2::GPRRegBankID);
  default:
...
Nios2RegisterBankInfo::getInstrAlternativeMappings(
  ...
  case TargetOpcode::G_BITCAST: {
    ...
    InstructionMapping GPRMapping(
        /*ID*/ 1, /*Cost*/ 1,
        getCopyMapping(Nios2::GPRRegBankID, Nios2::GPRRegBankID, Size),
        /*NumOperands*/ 2);
...
void Nios2RegisterBankInfo::applyMappingImpl(
...
Nios2RegisterBankInfo::getSameKindOfOperandsMapping(const MachineInstr &MI) {
  ...
  PartialMappingIdx RBIdx = PMI_FirstGPR;
  ...
  for (unsigned Idx = 1; Idx != NumOperands; ++Idx) {
    ...
    assert(
        Nios2GenRegisterBankInfo::getRegBankBaseIdxOffset(
            RBIdx, OpTy.getSizeInBits()) ==
            Nios2GenRegisterBankInfo::getRegBankBaseIdxOffset(RBIdx, Size) &&
...
unsigned Nios2RegisterBankInfo::copyCost(const RegisterBank &A,
...
RegisterBankInfo::InstructionMapping
Nios2RegisterBankInfo::getInstrMapping(const MachineInstr &MI) const {
  const unsigned Opc = MI.getOpcode();
  const MachineFunction &MF = *MI.getParent()->getParent();
  const MachineRegisterInfo &MRI = MF.getRegInfo();
  ...
  case TargetOpcode::G_BITCAST: {
    ...
    const RegisterBank &DstRB = Nios2::GPRRegBank;
    const RegisterBank &SrcRB = Nios2::GPRRegBank;
  ...
  case TargetOpcode::G_BITCAST:
    ...
    if (OpRegBankIdx[0] != OpRegBankIdx[1])
      Cost = copyCost(
          *Nios2GenRegisterBankInfo::PartMappings[OpRegBankIdx[0]].RegBank,
          *Nios2GenRegisterBankInfo::PartMappings[OpRegBankIdx[1]].RegBank,
          OpSize[0]);
    break;
  case TargetOpcode::G_LOAD:
