//===-- Nios2AnalyzeImmediate.cpp - Analyze Immediates ---------------------===//
//
...
Nios2AnalyzeImmediate::Inst::Inst(unsigned O, unsigned I) : Opc(O), ImmOpnd(I) {}

// Add I to the instruction sequences.
void Nios2AnalyzeImmediate::AddInstr(InstSeqLs &SeqLs, const Inst &I) {
...
void Nios2AnalyzeImmediate::GetInstSeqLsADDiu(uint64_t Imm, unsigned RemSize,
                                             InstSeqLs &SeqLs) {
  GetInstSeqLs((Imm + 0x8000ULL) & 0xffffffffffff0000ULL, RemSize, SeqLs);
  AddInstr(SeqLs, Inst(ADDiu, Imm & 0xffffULL));
}

void Nios2AnalyzeImmediate::GetInstSeqLsORi(uint64_t Imm, unsigned RemSize,
                                           InstSeqLs &SeqLs) {
  GetInstSeqLs(Imm & 0xffffffffffff0000ULL, RemSize, SeqLs);
  AddInstr(SeqLs, Inst(ORi, Imm & 0xffffULL));
}

void Nios2AnalyzeImmediate::GetInstSeqLsSHL(uint64_t Imm, unsigned RemSize,
                                           InstSeqLs &SeqLs) {
...
void Nios2AnalyzeImmediate::GetInstSeqLs(uint64_t Imm, unsigned RemSize,
                                        InstSeqLs &SeqLs) {
...
// are replaced with
//  LUi 0x444
void Nios2AnalyzeImmediate::ReplaceADDiuSHLWithLUi(InstSeq &Seq) {
...
void Nios2AnalyzeImmediate::GetShortestSeq(InstSeqLs &SeqLs, InstSeq &Insts) {
...
const Nios2AnalyzeImmediate::InstSeq
&Nios2AnalyzeImmediate::Analyze(uint64_t Imm, unsigned Size,
                               bool LastInstrIsADDiu) {
  this->Size = Size;

  ADDiu = Nios2::ADDI_R1; // TODO: Nios2 R2 support
  ORi = Nios2::ORI_R1; // TODO: Nios2 R2 support
  SHL = Nios2::ROL_R1; // TODO: Nios2 R2 support
  LUi = Nios2::ORHI_R1; // TODO: Nios2 R2 support
