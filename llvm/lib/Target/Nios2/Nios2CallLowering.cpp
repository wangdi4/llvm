//===--- Nios2CallLowering.cpp - Call lowering --------------------------===//
//
...
Nios2CallLowering::Nios2CallLowering(const Nios2TargetLowering &TLI)
  : CallLowering(&TLI) {}
...
struct OutgoingArgHandler : public CallLowering::ValueHandler {
  ...
  unsigned getStackAddress(uint64_t Size, int64_t Offset,
    ...
    MIRBuilder.buildCopy(SPReg, Nios2::SP);
...
void Nios2CallLowering::splitToValueTypes(
    const ArgInfo &OrigArg, SmallVectorImpl<ArgInfo> &SplitArgs,
    const DataLayout &DL, MachineRegisterInfo &MRI,
    const SplitArgTy &PerformArgSplit) const {
  const Nios2TargetLowering &TLI = *getTLI<Nios2TargetLowering>();
...
bool Nios2CallLowering::lowerReturn(MachineIRBuilder &MIRBuilder,
                                      const Value *Val, unsigned VReg) const {
  MachineFunction &MF = MIRBuilder.getMF();
  const Function &F = *MF.getFunction();

  auto MIB = MIRBuilder.buildInstrNoInsert(Nios2::RET_R1);
...
bool Nios2CallLowering::lowerFormalArguments(MachineIRBuilder &MIRBuilder,
                                               const Function &F,
  ...
  if (F.isVarArg()) {
//    if (!MF.getSubtarget<Nios2Subtarget>().isTargetDarwin()) {
...
bool Nios2CallLowering::lowerCall(MachineIRBuilder &MIRBuilder,
  ...
  auto CallSeqStart = MIRBuilder.buildInstr(Nios2::ADJCALLSTACKDOWN);

  // Create a temporarily-floating call instruction so we can add the implicit
  // uses of arg registers.
  auto MIB = MIRBuilder.buildInstrNoInsert(Callee.isReg() ? Nios2::CALLR_R1
                                                          : Nios2::CALL_R1);
  MIB.add(Callee);
  ...
  MIRBuilder.buildInstr(Nios2::ADJCALLSTACKUP)
