...
void Nios2AsmPrinter::EmitInstrWithMacroNoAT(const MachineInstr *MI) {
  MCInst TmpInst;

  MCInstLowering.Lower(MI, TmpInst);
  OutStreamer->EmitRawText(StringRef("\t.set\tmacro"));
  if (Nios2FI->getEmitNOAT())
...
bool Nios2AsmPrinter::lowerOperand(const MachineOperand &MO, MCOperand &MCOp) {
  MCOp = MCInstLowering.LowerOperand(MO);
  return MCOp.isValid();
}

#include "Nios2GenMCPseudoLowering.inc"

#ifdef ENABLE_GPRESTORE
void Nios2AsmPrinter::emitPseudoCPRestore(MCStreamer &OutStreamer,
...
#ifdef ENABLE_GPRESTORE
    if (I->getOpcode() == Nios2::CPRESTORE) {
...
void Nios2AsmPrinter::printSavedRegsBitmask(raw_ostream &O) {
  ...
  unsigned CPURegSize = TRI->getRegSizeInBits(Nios2::CPURegsRegClass) / 8;
...
void Nios2AsmPrinter::printHex32(unsigned Value, raw_ostream &O) {
  O << "0x";
  for (int i = 7; i >= 0; i--)
...
/// Emit Set directives.
const char *Nios2AsmPrinter::getCurrentABIString() const {
  switch (static_cast<Nios2TargetMachine &>(TM).getABI().GetEnumValue()) {
  case Nios2ABIInfo::ABI::O32:  return "abiO32";
  case Nios2ABIInfo::ABI::S32:  return "abiS32";
  default: llvm_unreachable("Unknown Nios2д207я ABI");
  }
}
...
void Nios2AsmPrinter::EmitFunctionEntryLabel() {
  if (OutStreamer->hasRawTextSupport() && false /*agrische*/)
    OutStreamer->EmitRawText("\t.global\t" + Twine(CurrentFnSym->getName()));
...
//agrische  emitFrameDirective();
  bool EmitCPLoad = (MF->getTarget().getRelocationModel() == Reloc::PIC_) &&
    Nios2FI->globalBaseRegSet() &&
  ...
  if (OutStreamer->hasRawTextSupport() && false /*agrische*/) {
    ...
    if (Nios2FI->getEmitNOAT())
      OutStreamer->EmitRawText(StringRef("\t.set\tnoat"));
  } else if (EmitCPLoad && false /*agrische*/) {
    SmallVector<MCInst, 4> MCInsts;
...
void Nios2AsmPrinter::EmitStartOfAsmFile(Module &M) {
  ...
  if (/*agrische*/false && OutStreamer->hasRawTextSupport())
...
bool Nios2AsmPrinter::PrintAsmMemoryOperand(const MachineInstr *MI,
  ...
  O << Offset << "($" << Nios2InstPrinter::getRegisterName(MO.getReg()) << ")";
...
void Nios2AsmPrinter::printOperand(const MachineInstr *MI, int opNum,
  ...
  case Nios2II::MO_ABS_HI:   O << "%hiadj(";     break;
  case Nios2II::MO_ABS_LO:   O << "%lo(";     break;
  case Nios2II::MO_GOT_HI16: O << "%got_hi16("; break;
  case Nios2II::MO_GOT_LO16: O << "%got_lo16("; break;
  }
...
void Nios2AsmPrinter::PrintDebugValueComment(const MachineInstr *MI,
                                           raw_ostream &OS) {
  // TODO: implement
  OS << "PrintDebugValueComment()";
}

bool Nios2AsmPrinter::isLongBranchPseudo(int Opcode) const {
 return false;
/*agrische  return (Opcode == Nios2::LONG_BRANCH_LUi
          || Opcode == Nios2::LONG_BRANCH_ADDiu);*/
}
