...
struct CSASeqHeader {
  ...
  CSASeqHeader() : pickerInit(NULL), pickerMov1(NULL), compareInst(NULL) {}

  void init(MachineInstr *pickerInit_, MachineInstr *pickerMov1_,
            MachineInstr *compareInst_, unsigned pickerChannel_,
            unsigned switcherChannel_, bool pickerSense_, bool switcherSense_) {
  ...
  static int pick_top_op_idx() { return 0; }
  static int pick_select_op_idx() { return 1; }
  static int switch_select_op_idx() { return 2; }
  static int switch_bottom_op_idx() { return 3; }
  ...
  int pick_input_op_idx() const { return 2 + this->pickerSense; }
  int pick_loopback_op_idx() const { return 3 - this->pickerSense; }
  ...
  int switch_output_op_idx() const { return this->switcherSense; }
  int switch_loopback_op_idx() const { return 1 - this->switcherSense; }
};
...
struct CSASeqCandidate {
  ...
  CSASeqCandidate(MachineInstr *pickI, MachineInstr *switchI)
      : pickInst(pickI), switchInst(switchI), transformInst(NULL),
        stype(UNKNOWN), top(0), bottom(0), saved_op(NULL),
        opcode(CSASeqCandidate::INVALID_OPCODE), negate_input(false) {}
  ...
  inline MachineOperand *get_pick_input_op(const CSASeqHeader &header) const {
  ...
  inline MachineOperand *get_fma_mul_op(int input_idx) const {
...
public:
  CSASeqLoopInfo()
      : loop_id(-1), num_valid_sequences(0), cmp0_channel(CSA::IGN),
        cmp1_channel(CSA::IGN), cmp0_idx(-1), cmp1_idx(-1), indvar_idx(-1),
        bound_idx(-1), compare_sense(0), valid_to_transform(false),
        seq_opcode(CSASeqCandidate::INVALID_OPCODE) {}

  ~CSASeqLoopInfo() {}

  // Get and set the number of valid sequences in this loop.
  void set_valid_sequence_count(int val) { this->num_valid_sequences = val; }
  int get_valid_sequence_count() const { return this->num_valid_sequences; }

  // Accessor methods for some of the index fields in this data
  // structure.
  int cmp0Idx() const { return cmp0_idx; }
  int cmp1Idx() const { return cmp1_idx; }

  // Operand index in the compare instruction where we can find the
  // bound.
  inline int boundOpIdx() const { return 2 - this->compare_sense; }

  int indvarIdx() const { return indvar_idx; }
  int boundIdx() const { return bound_idx; }

  bool sequence_transform_is_valid() const { return valid_to_transform; }

  unsigned get_seq_opcode() const { return seq_opcode; }
  ...
  //   compare bound indvar
  //
  bool commute_compare_operands() const { return this->compare_sense; }

  // 2. If the switcher sense is 1 (so it expects 0, 0, 0, ... 1 for
  // its control), then we need to negate the output of the compare.
  bool negate_compare_output() const { return this->header.switcherSense; }
  ...
  CmpMatchType match_candidate_with_cmp(unsigned bottom, int candidate_idx) {
    // Errors conditions.
  ...
  bool find_induction_variable() {
    ...
    // either cmp0 or cmp1.
    if ((this->cmp0Idx() >= 0) && this->candidates[this->cmp0Idx()].stype ==
                                    CSASeqCandidate::SeqType::STRIDE) {
      this->indvar_idx    = this->cmp0Idx();
      this->bound_idx     = this->cmp1Idx();
      this->compare_sense = 0;
      return true;
    } else if ((this->cmp1Idx() >= 0) &&
               this->candidates[this->cmp1Idx()].stype ==
                 CSASeqCandidate::SeqType::STRIDE) {
  ...
  bool has_valid_bound() const {
    ...
    bool found_bound_channel =
      ((this->bound_idx >= 0) && this->candidates[this->bound_idx].stype ==
                                   CSASeqCandidate::SeqType::REPEAT);
  ...
  bool sequence_opcode_transform_check(const CSAInstrInfo &TII) {
    if (this->indvar_idx >= 0) {
      ...
      this->valid_to_transform = CSASeqLoopInfo::compute_matching_seq_opcode(
        compare_opcode, transform_opcode, this->commute_compare_operands(),
        this->negate_compare_output(), TII, &this->seq_opcode);
