//===-- LPU.cpp - Library for LPU code generation --------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This library converts LLVM code to LPU simulator executable code.
//
//===----------------------------------------------------------------------===//
...
static std::tr1::unordered_map<int, std::string> VarNamesMap;

static bool PrintSigned = false;
...
namespace {
  ...
  /// LPUWriter - This class is the main chunk of code that converts an LLVM
  /// module to a C translation unit.
  class LPUWriter : public FunctionPass, public InstVisitor<LPUWriter> {
  ...
  public:
    static char ID;
    explicit LPUWriter(formatted_raw_ostream &o)
    ...
    virtual const char *getPassName() const { return "LPU"; }
    ...
    raw_ostream &printTypedef(raw_ostream &Out,
  ...
  private :
    ...
    /// intrinsics which need to be explicitly defined in the LPU.
    ...
    void printInstOps(BasicBlock * BB, int i);
    ...
    friend class InstVisitor<LPUWriter>;
...
std::string LPUWriter::getStructName(StructType *ST) {
...
void LPUWriter::printStructReturnPointerFunctionType(raw_ostream &Out,
                                                   const AttributeSet &PAL,
                                                   PointerType *TheTy) {
...
raw_ostream &
LPUWriter::printSimpleType(raw_ostream &Out, Type *Ty, bool isSigned,
                         const std::string &NameSoFar) {
...
// Prints out the typedef'd type name and variable name.
raw_ostream &LPUWriter::printTypedef(raw_ostream &Out,
                                     const std::string &typedefName,
                                     const std::string &NameSoFar)
{
  //std::cout << "printTypedef: printing " << typedefName << std::endl;
  Out << typedefName << " " << NameSoFar;
  return Out;
}
...
// declaration.
//
raw_ostream &LPUWriter::printType(raw_ostream &Out, Type *Ty,
...
void LPUWriter::printConstantArray(ConstantArray *CPA, bool Static) {
  Out << "{ ";
...
void LPUWriter::printConstantVector(ConstantVector *CP, bool Static) {
  Out << "{ ";
...
void LPUWriter::printConstantDataSequential(ConstantDataSequential *CDS,
                                          bool Static) {
...
/// @brief Print a cast
void LPUWriter::printCast(unsigned opc, Type *SrcTy, Type *DstTy) {
...
void LPUWriter::printConstant(Constant *CPV, bool Static) {
    ...
    default:
      DEBUG (errs() << "LPUWriter Error: Unhandled constant expression: " << *CE << "\n");
      llvm_unreachable(0);
    }
...
bool LPUWriter::printConstExprCast(const ConstantExpr* CE, bool Static) {
...
void LPUWriter::printConstantWithCast(Constant* CPV, unsigned Opcode) {
...
std::string LPUWriter::GetValueName(const Value *Operand) {
...
void LPUWriter::writeInstComputationInline(Instruction &I) {
  ...
  if (Ty->isIntegerTy() && (Ty!=Type::getInt1Ty(I.getContext()) &&
      ...
      report_fatal_error("The LPU does not currently support integer "
  ...
  //printf("writeInstComputationInline: op=%s\n", I.getOpcodeName());
  visit(I);
...
//Instruction is updated in this routine.  The if contains =, not ==!
//
void LPUWriter::writeOperandInternal(Value *Operand, bool Static) {
...
void LPUWriter::writeOperand(Value *Operand, bool Static) {
...
// for the Instruction.
bool LPUWriter::writeInstructionCast(const Instruction &I) {
...
void LPUWriter::writeOperandWithCast(Value* Operand, unsigned Opcode) {
...
// being used.
void LPUWriter::writeOperandWithCast(Value* Operand, const ICmpInst &Cmp) {
...
// this boiler-plate is needed for any real apps to build properly.
...
bool LPUWriter::doInitialization(Module &M) {
...
/// Output all floating point constants that cannot be printed accurately...
void LPUWriter::printFloatingPointConstants(Function &F) {
...
void LPUWriter::printFloatingPointConstants(const Constant *C) {
...
///
void LPUWriter::printModuleTypes() {
  /*
...
//
void LPUWriter::printContainedStructs(Type *Ty,
                                SmallPtrSet<Type *, 16> &StructPrinted) {
...
void LPUWriter::printFunctionSignature(const Function *F, bool Prototype) {
...
void LPUWriter::printFunction(Function &F) {
  /// isStructReturn - Should this function actually return a struct by-value?
  bool isStructReturn = F.hasStructRetAttr();
...
// Print the Opcode of each instruction in the basic block.
// this is for debugging/investigation purposes only.
void LPUWriter::printInstOps(BasicBlock * BB, int i)
{
...
void LPUWriter::printLoop(Loop *L) {
  ...
  for (unsigned i = 0, e = L->getBlocks().size(); i != e; ++i) {
    ...
    switch (Tp){
     ...
     case IF :          
      {
#if 0   // goto restoration: do not create for/while loop
...
void LPUWriter::printLoopInst(BasicBlock *BB){
...
// goto restoration: do not convert to for/while loop
void LPUWriter::printBasicBlockLoop(BasicBlock *BB)
{
  // kdg: TODO: Deal with visited
  /*
  if (BB->getVisited())
    return;
  else
...
#if 0 // goto restoration: do not create for/while loops.
void LPUWriter::printBasicBlockLoop(BasicBlock *BB){
  int x = 0;
  int BBSize = BB->size();
  Out << "/* BasicBlock Size: " << BBSize << "*/ \n";
...
void LPUWriter::visitBasicBlockByType(BasicBlock *BB){
...
void LPUWriter::printBasicBlockNoVisit(BasicBlock *BB)
{
...
// gail added "visited" field to BasicBlock class to handle TODO.
// TODO: Stop printing out the BasicBlocks which contain the bodies that we
//   have already visited in conditional statements
void LPUWriter::printBasicBlock(BasicBlock *BB) {
...
//
void LPUWriter::visitReturnInst(ReturnInst &I) {
...
void LPUWriter::visitSwitchInst(SwitchInst &SI) {
  InSwitch = true;
...
void LPUWriter::visitIndirectBrInst(IndirectBrInst &IBI) {
  Out << "  goto *(void*)(";
  writeOperand(IBI.getOperand(0));
  Out << ");\n";
}

void LPUWriter::visitUnreachableInst(UnreachableInst &I) {
  Out << "  /*UNREACHABLE*/;\n";
}


bool LPUWriter::isGotoCodeNecessary(BasicBlock *From, BasicBlock *To) {
  GotoSet.insert(To);
...
#if 0
  // this removes extraneous goto/label pairs for fall through.
  // before it can be enabled, the cmp condition must be flipped.
  if (From != To->getSinglePredecessor()) {
    return true;
  }
#endif
  
#if 0 // undid this as part of the restoration of go-tos.
...
void LPUWriter::printPHICopiesForSuccessor (BasicBlock *CurBlock,
...
void LPUWriter::printBranchToBlock(BasicBlock *CurBB, BasicBlock *Succ,
                                 unsigned Indent) {
...
void LPUWriter::addToGoToSet(BasicBlock *To){
  GotoSet.insert(To);
}


void LPUWriter::visitBranchInst(BranchInst &I){
  ...
  else {  // added unconditional goto as part of goto restoration
    printBranchToBlock(I.getParent(), I.getSuccessor(0), 0);
    addToGoToSet(I.getSuccessor(0));
  }
}

#if 0  // rewritten as part of goto restoration.
void LPUWriter::visitBranchInst(BranchInst &I){
...
// the PHI.
void LPUWriter::visitPHINode(PHINode &I) {
  Out << "/* " << I << " */ ";
  writeOperand(&I);
  Out << "__PHI_TEMPORARY";
}
...
void LPUWriter::visitBinaryOperator(Instruction &I) {
...
void LPUWriter::visitICmpInst(ICmpInst &I) {
...
void LPUWriter::visitFCmpInst(FCmpInst &I) {
...
void LPUWriter::visitCastInst(CastInst &I) {
  Type *DstTy = I.getType();
  Type *SrcTy = I.getOperand(0)->getType();
  if (isFPIntBitCast(I)) {
    Out << '(';
...
void LPUWriter::visitSelectInst(SelectInst &I) {
  Out << "((";
  writeOperand(I.getCondition());
  Out << ") ? (";
...
void LPUWriter::printIntrinsicDefinition(const Function &F, raw_ostream &Out) {
  ...
  assert(isSupportedIntegerSize(*elemT) &&
         "LPU does not support arbitrary size integers.");
...
void LPUWriter::lowerIntrinsics(Function &F) {
...
void LPUWriter::visitCallInst(CallInst &I) {
...
bool LPUWriter::visitBuiltinCall(CallInst &I, Intrinsic::ID ID,
                               bool &WroteCallee) {
  switch (ID) {
  default: {
    // If this is an intrinsic that directly corresponds to a GCC
...
void LPUWriter::visitAllocaInst(AllocaInst &I) {
  Out << '(';
  printType(Out, I.getType());
  Out << ") alloca(sizeof(";
...
void LPUWriter::printGEPExpression(Value *Ptr, gep_type_iterator I,
                                 gep_type_iterator E, bool Static) {
...
void LPUWriter::writeMemoryAccess(Value *Operand, Type *OperandType,
                                bool IsVolatile, unsigned Alignment) {
...
void LPUWriter::visitLoadInst(LoadInst &I) {
  writeMemoryAccess(I.getOperand(0), I.getType(), I.isVolatile(),
                    I.getAlignment());
}

void LPUWriter::visitStoreInst(StoreInst &I) {
...
void LPUWriter::visitGetElementPtrInst(GetElementPtrInst &I) {
  printGEPExpression(I.getPointerOperand(), gep_type_begin(I),
                     gep_type_end(I), false);
}

void LPUWriter::visitVAArgInst(VAArgInst &I) {
  Out << "va_arg(*(va_list*)";
  writeOperand(I.getOperand(0));
  Out << ", ";
  printType(Out, I.getType());
...
void LPUWriter::visitInsertElementInst(InsertElementInst &I) {
...
void LPUWriter::visitExtractElementInst(ExtractElementInst &I) {
...
void LPUWriter::visitShuffleVectorInst(ShuffleVectorInst &SVI) {
  Out << "(";
...
void LPUWriter::visitInsertValueInst(InsertValueInst &IVI) {
...
void LPUWriter::visitExtractValueInst(ExtractValueInst &EVI) {
  Out << "(";
...
bool LPUTargetMachine::addPassesToEmitFile(PassManagerBase &PM,
