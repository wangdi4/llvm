...
namespace {
  ...
  public:
    ...
    /*
    bool MatchAddress(SDValue N, LPUISelAddressMode &AM);
    bool MatchWrapper(SDValue N, LPUISelAddressMode &AM);
    bool MatchAddressBase(SDValue N, LPUISelAddressMode &AM);
...
FunctionPass *llvm::createLPUISelDag(LPUTargetMachine &TM,
  ...
  return new LPUDAGToDAGISel(TM, OptLevel);
    ...
    /*
/// MatchWrapper - Try to match LPUISD::Wrapper node into an addressing mode.
/// These wrap things that will resolve down into a symbol reference.  If no
/// match is possible, this returns true, otherwise it returns false.
bool LPUDAGToDAGISel::MatchWrapper(SDValue N, LPUISelAddressMode &AM) {
...
bool LPUDAGToDAGISel::MatchAddressBase(SDValue N, LPUISelAddressMode &AM) {
  // Is the base register already occupied?
  if (AM.BaseType != LPUISelAddressMode::RegBase || AM.Base.Reg.getNode()) {
    // If so, we cannot select it.
    return true;
  }

  // Default, generate it as a register.
  AM.BaseType = LPUISelAddressMode::RegBase;
  AM.Base.Reg = N;
  return false;
}

bool LPUDAGToDAGISel::MatchAddress(SDValue N, LPUISelAddressMode &AM) {
  ...
  case ISD::FrameIndex:
    if (AM.BaseType == LPUISelAddressMode::RegBase
        && AM.Base.Reg.getNode() == nullptr) {
      AM.BaseType = LPUISelAddressMode::FrameIndexBase;
...
/// match by reference.
bool LPUDAGToDAGISel::SelectAddr(SDValue N,
                                    SDValue &Base, SDValue &Disp) {
  LPUISelAddressMode AM;
  ...
  Base  = (AM.BaseType == LPUISelAddressMode::FrameIndexBase) ?
...
SDNode *LPUDAGToDAGISel::SelectIndexedLoad(SDNode *N) {
  ...
  switch (VT.SimpleTy) {
  case MVT::i8:
    Opcode = LPU::MOV8rm_POST;
    break;
  case MVT::i16:
    Opcode = LPU::MOV16rm_POST;
    break;
  default:
    return nullptr;
  }
...
SDNode *LPUDAGToDAGISel::SelectIndexedBinOp(SDNode *Op,
...
SDNode *LPUDAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::FrameIndex: {
    ...
    if (Node->hasOneUse())
      return CurDAG->SelectNodeTo(Node, LPU::ADD16ri, MVT::i16,
                                  TFI, CurDAG->getTargetConstant(0, MVT::i16));
    return CurDAG->getMachineNode(LPU::ADD16ri, dl, MVT::i16,
                                  TFI, CurDAG->getTargetConstant(0, MVT::i16));
  }
  case ISD::LOAD:
  ...
  case ISD::ADD:
    if (SDNode *ResNode =
        SelectIndexedBinOp(Node,
                           ...
                           LPU::ADD8rm_POST, LPU::ADD16rm_POST))
      return ResNode;
  ...
  case ISD::SUB:
    if (SDNode *ResNode =
        SelectIndexedBinOp(Node,
                           Node->getOperand(0), Node->getOperand(1),
                           LPU::SUB8rm_POST, LPU::SUB16rm_POST))
      return ResNode;
  ...
  case ISD::AND:
    if (SDNode *ResNode =
        SelectIndexedBinOp(Node,
                           Node->getOperand(0), Node->getOperand(1),
                           LPU::AND8rm_POST, LPU::AND16rm_POST))
      return ResNode;
    else if (SDNode *ResNode =
             SelectIndexedBinOp(Node, Node->getOperand(1), Node->getOperand(0),
                                LPU::AND8rm_POST, LPU::AND16rm_POST))
  ...
  case ISD::OR:
    if (SDNode *ResNode =
        SelectIndexedBinOp(Node,
                           Node->getOperand(0), Node->getOperand(1),
                           LPU::OR8rm_POST, LPU::OR16rm_POST))
      return ResNode;
    else if (SDNode *ResNode =
             SelectIndexedBinOp(Node, Node->getOperand(1), Node->getOperand(0),
                                LPU::OR8rm_POST, LPU::OR16rm_POST))
  ...
  case ISD::XOR:
    if (SDNode *ResNode =
        SelectIndexedBinOp(Node,
                           Node->getOperand(0), Node->getOperand(1),
                           LPU::XOR8rm_POST, LPU::XOR16rm_POST))
      return ResNode;
    else if (SDNode *ResNode =
             SelectIndexedBinOp(Node, Node->getOperand(1), Node->getOperand(0),
                                LPU::XOR8rm_POST, LPU::XOR16rm_POST))
