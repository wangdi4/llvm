...
public:
  LPUAsmStreamer(MCContext &Context, formatted_raw_ostream &os,
...
void LPUAsmStreamer::ChangeSection(const MCSection *Section,
...
void LPUAsmStreamer::EmitELFSize(MCSymbol *Symbol, const MCExpr *Value) {
...
void LPUAsmStreamer::EmitZerofill(const MCSection *Section, MCSymbol *Symbol,
...
// e.g. _a.
void LPUAsmStreamer::EmitTBSSSymbol(const MCSection *Section, MCSymbol *Symbol,
                                   uint64_t Size, unsigned ByteAlignment) {
  AssignSection(Symbol, Section);
...
MCStreamer *createLpuAsmStreamer(MCContext &Context,
  ...
  if (WrapAssembly) {
    ...
    return new LPUAsmStreamer(Context, OS, isVerboseAsm, useDwarfDirectory,
                              IP, CE, MAB, ShowInst);
  } else {

    // Use MCAsmStream to generat a raw LPU assembly file
    return llvm::createAsmStreamer(Context, OS, isVerboseAsm, useDwarfDirectory,
                                   IP, CE, MAB, ShowInst);
  }
}
