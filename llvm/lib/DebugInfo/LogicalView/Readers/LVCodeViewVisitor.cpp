...
Error LVLogicalVisitor::visitKnownRecord(CVType &Record,
  ...
  LLVM_DEBUG({
    printTypeBegin(Record, TI, Element, StreamTPI);
    printTypeIndex("ElementType", Dim.getElementType(), StreamIPI);
    printTypeIndex("DimInfo", Dim.getDimInfo(), StreamIPI);
    W.printString("Name", Dim.getName());
    printTypeEnd(Record);
  });
  return Error::success();
}
...
Error LVLogicalVisitor::visitKnownRecord(CVType &Record,
  ...
  LLVM_DEBUG({
    printTypeBegin(Record, TI, Element, StreamTPI);
    printTypeIndex("IndexType", DCL.getIndexType(), StreamIPI);
    W.printNumber("Rank", DCL.getRank());
    ListScope Bounds(W, "Bounds");
    for (int64_t Bnd : DCL.getBounds())
      W.printNumber("Bound", Bnd);
    printTypeEnd(Record);
  });
  return Error::success();
}
...
Error LVLogicalVisitor::visitKnownRecord(CVType &Record,
  ...
  LLVM_DEBUG({
    printTypeBegin(Record, TI, Element, StreamTPI);
    W.printEnum("OEMId", unsigned(OEM.getOEMIdentifier()),
                ArrayRef(LeafTypeNames));
    W.printEnum("OEMType", unsigned(OEM.getOEMTypeID()),
                ArrayRef(LeafTypeNames));

    ArrayRef<TypeIndex> TypeIndices = OEM.getTypeIndices();
    uint32_t TISize = TypeIndices.size();
    W.printNumber("Type Count", TISize);
    if (TISize > 0) {
      ListScope Types(W, "Types");
      for (uint32_t I = 0; I < TISize; ++I)
        printTypeIndex("Type", TypeIndices[I], StreamIPI);
    }

    ArrayRef<uint32_t> Data = OEM.getData();
    uint32_t DSize = Data.size();
    if (DSize > 0) {
      ListScope AdditionalData(W, "Additional Data");
