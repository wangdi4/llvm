...
private:
  ...
  public:
    ...
    Andersens() : ModulePass(ID) {
        initializeAndersensPass(*PassRegistry::getPassRegistry());
    }
...
bool doInitialization(Module &M) override {
  return true;
    ...
    ModRefInfo getModRefInfo(ImmutableCallSite CS,
                                       const MemoryLocation &Loc,
                                       AliasAnalysis *AAChain) override;
...
INITIALIZE_AG_PASS(Andersens, AliasAnalysis, "anders-aa",
                   "Andersen Interprocedural AA", false, true, false)
...
AliasResult Andersens::alias(const MemoryLocation &LocA,
...
Andersens::getModRefInfo(ImmutableCallSite CS1, ImmutableCallSite CS2) {
  ...
  // parameter.
  ModRefInfo R = AliasAnalysis::getModRefInfo(CS1, CS2);
  if (PrintAndersModRefQueries) {
      errs() << "Result: " << getModRefResultStr(R) << "\n";
      errs() << " getModRefInfo_end\n";
  }
...
/// return true.
///
bool Andersens::pointsToConstantMemory(const MemoryLocation &Loc, bool OrLocal) {
  NumPtrQuery++;
...
void Andersens::CollectConstraints(Module &M) {
  ...
  for (Module::iterator F = M.begin(), E = M.end(); F != E; ++F) {
    ...
    Constraints.push_back(Constraint(Constraint::AddressOf, ValueNodes[F], ValueNodes[F]));
    Constraints.push_back(Constraint(Constraint::Store, ValueNodes[F], ValueNodes[F]));
...
void Andersens::visitInsertValueInst(InsertValueInst &AI) {
...
void Andersens::visitExtractValueInst(ExtractValueInst &AI) {
    if (!AI.getType()->isPointerTy()) {
        return;
    }
...
void Andersens::visitAtomicRMWInst(AtomicRMWInst &AI) {
...
void Andersens::visitBinaryOperator(BinaryOperator &AI) {
    if (!AI.getType()->isPointerTy()) {
        return;
    }
    Constraints.push_back(Constraint(Constraint::Copy, getNodeValue(AI),
...
void Andersens::visitPtrToIntInst(PtrToIntInst &AI) {
    Constraints.push_back(Constraint(Constraint::Copy, 
                   getNode(AI.getOperand(0)), UniversalSet));
}

void Andersens::visitIntToPtrInst(IntToPtrInst &AI) {
    Constraints.push_back(Constraint(Constraint::Copy, getNodeValue(AI), 
                   UniversalSet));
}

void Andersens::visitExtractElementInst(ExtractElementInst &AI) {
  if (!AI.getType()->isPointerTy()) {
      return;
  }
  Constraints.push_back(Constraint(Constraint::Load, getNodeValue(AI),
...
void Andersens::visitInsertElementInst(InsertElementInst &AI) {
    if (!AI.getType()->isPointerTy()) {
        return;
    }
    Constraints.push_back(Constraint(Constraint::Copy, getNodeValue(AI),
...
void Andersens::visitShuffleVectorInst(ShuffleVectorInst &AI) {
    if (!AI.getType()->isPointerTy()) {
        return;
    }
    Constraints.push_back(Constraint(Constraint::Copy, getNodeValue(AI),
...
void Andersens::visitLandingPadInst(LandingPadInst &AI) {
  if (!AI.getType()->isPointerTy()) {
      return;
  }
  Constraints.push_back(Constraint(Constraint::Copy, getNodeValue(AI),
...
void Andersens::visitAtomicCmpXchgInst(AtomicCmpXchgInst &AI) {
...
//
void Andersens::AddConstraintsForDirectCall(CallSite CS, Function *F)
{
...
//
void Andersens::AddConstraintsForInitActualsToUniversalSet(CallSite CS) {
...
void Andersens::visitCallSite(CallSite CS) {
  ...
  // malloc-like calls.   
  if (CS.getCalledFunction() && (CS.getCalledFunction()->getName() == "malloc" || CS.getCalledFunction()->getName() == "calloc")) {
...
//
void Andersens::AddEdgeInGraph(unsigned N1, unsigned N2) {
  N1 = FindNode(N1);
  N2 = FindNode(N2);
...
//
void Andersens::InitIndirectCallActualsToUniversalSet(CallSite CS) {
...
//
void Andersens::IndirectCallActualsToFormals(CallSite CS, Function *F) {
...
//
void Andersens::ProcessIndirectCall(CallSite CS) {
...
//
void Andersens::ProcessIndirectCalls() {
...
unsigned Andersens::getPointsToSet(const Value *V, std::vector<llvm::Value*>& PtVec)
{
    Node *N1;
...
void Andersens::printValueNode(const Value *V)
{
