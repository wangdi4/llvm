...
// This class is used to recreate original SCEV form of a value given the scoped
// SCEV form. This is done by re-parsing SCEVUnkowns in the incoming SCEV.
...
const SCEV *ScalarEvolution::getNonScopedSCEV(const SCEV *SC) {
  if (auto *ScopedSE = dyn_cast<ScopedScalarEvolution>(this)) {
    return OriginalSCEVCreator::create(SC, ScopedSE->getOrigSE());
  }
...
#if INTEL_CUSTOMIZATION
  // Return unknown if phi is outside all outermost loops.
  if (auto *ScopedSE = dyn_cast<ScopedScalarEvolution>(this)) {
    bool IsAllowed = false;

    for (auto *Lp : ScopedSE->getOutermostLoops()) {
      if (Lp->contains(L)) {
        IsAllowed = true;
        break;
      }
...
#if INTEL_CUSTOMIZATION
// This code was removed by: https://reviews.llvm.org/D149344
// It blocks "faulting" instructions such as "/u 4" from being hoisted out
// of conditionals, even if they can be converted to non-faulting shr.
// This is generally better for analysis, but it is creating too much
// instruction pressure inside the loop. It could be left to x86CG to
// simplify the code, but it seems simpler to revert the code.
// CMPLRLLVM-49654
...
const SCEV *ScalarEvolution::createNodeForIdenticalOperandsPHI(PHINode *PN) {
  ...
  if ((NumOperands > 1) && (FirstIncomingVal != PN) &&
    ...
    ValueExprMapType::iterator It = ValueExprMap.find_as(PN);
...
#if INTEL_CUSTOMIZATION
  // If we are in 'scoped' mode, take the intersection of range info from
  // regular and scoped mode as we don't know who can provide better range info.
  auto *NonScopedSCEV = getNonScopedSCEV(S);
  if (S != NonScopedSCEV)
    ConservativeResult = ConservativeResult.intersectWith(
        cast<ScopedScalarEvolution>(this)->getOrigSE().getRangeRef(
            NonScopedSCEV, SignHint),
        RangeType);
#endif // INTEL_CUSTOMIZATION
...
ScalarEvolution::getBackedgeTakenInfo(const Loop *L) {
  ...
  ScopedScalarEvolution *ScopedSE = nullptr;
  if (!L->getExitingBlock() &&
      (ScopedSE = dyn_cast<ScopedScalarEvolution>(this)) &&
      (ScopedSE->getBackedgeTakenCountLoop() != L)) {
    return ScopedSE->getDummyBackedgeTakenCount(L);
  }
#endif // INTEL_CUSTOMIZATION
...
ScalarEvolution::computeConstantDifference(const SCEV *More, const SCEV *Less,
  ...
  APInt Zero(getTypeSizeInBits(More->getType()), 0);
#endif // INTEL_CUSTOMIZATION
  // X - X = 0.
  if (More == Less)
  ...
  if (isa<SCEVConstant>(Less) && isa<SCEVConstant>(More)) {
    ...
    return SignedOverflow ? M.ssub_ov(L, *SignedOverflow) : M - L; // INTEL
  }
  ...
  if (splitBinaryAdd(Less, LLess, RLess, Flags))
    if ((C1 = dyn_cast<SCEVConstant>(LLess)))
      if (RLess == More)
#if INTEL_CUSTOMIZATION
        return SignedOverflow ? Zero.ssub_ov(C1->getAPInt(), *SignedOverflow)
                              : -(C1->getAPInt());
#endif // INTEL_CUSTOMIZATION
  ...
  if (C1 && C2 && RLess == RMore)
    ...
    return SignedOverflow
               ? C2->getAPInt().ssub_ov(C1->getAPInt(), *SignedOverflow)
               : C2->getAPInt() - C1->getAPInt();
#endif // INTEL_CUSTOMIZATION
...
#if INTEL_CUSTOMIZATION
      if (isLoopEntryGuardedByCond(L, CondGE, OrigRHS, OrigStart, ExitCond,
                                   &RHS))
#endif // INTEL_CUSTOMIZATION
        return true;
...
#if INTEL_CUSTOMIZATION
  ScalarEvolution &NonConstSE = *const_cast<ScalarEvolution *>(this);
  ScopedScalarEvolution ScopedSE(F, TLI, AC, DT, LI, true);

  ScalarEvolution &SE = PrintScopedMode ? ScopedSE : NonConstSE;
#endif // INTEL_CUSTOMIZATION
