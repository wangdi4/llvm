...
TEST(CaptureTracking, MaxUsesToExplore) {
  ...
  auto Test = [&M](const char *FName, unsigned FalseMaxUsesLimit,
    ...
    // INTEL_CUSTOMIZATION
    // Intel version of this function has an extra bool arg before the limit.
    ASSERT_FALSE(PointerMayBeCaptured(Arg, true, true, true,
                                      FalseMaxUsesLimit));
    ASSERT_TRUE(PointerMayBeCaptured(Arg, true, true, true,
                                     TrueMaxUsesLimit));
    // INTEL_CUSTOMIZATION
