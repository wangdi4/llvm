...
def ignoreTest(test_dir_without_args):
    ignoredTests = [
#                    ".*basic.*",
#                    ".*api.*"
                    ]
    
    for pattern in ignoredTests:
        if re.match(pattern, test_dir_without_args):
            return True
...
def run_test_checking_output(current_directory, test_dir, log_file):
  ...
  if (os.path.exists(current_directory + os.sep + program_to_run)) :
    ...
    try:
      if ignoreTest(test_dir_without_args): return
...
os.environ['PATH'] = (current_directory + os.pathsep + os.environ.get('PATH'))
