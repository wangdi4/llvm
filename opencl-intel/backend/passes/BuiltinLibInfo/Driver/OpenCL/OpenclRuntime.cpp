...
const dotProdInlineData dotInlineTable[] = {
    {"_Z3dotff", 1},      {"_Z3dotdd", 1},      {"_Z3dotDv2_fS_", 2},
    {"_Z3dotDv2_dS_", 2}, {"_Z3dotDv3_fS_", 3}, {"_Z3dotDv3_dS_", 3},
    {"_Z3dotDv4_fS_", 4}, {"_Z3dotDv4_dS_", 4}, {nullptr, 0}};

const char *BuiltinReturnByPtr[] = {
    "fract", "modf", "native_fract", "native_sincos", "sincos",
};
...
Function *OpenclRuntime::findInRuntimeModule(StringRef Name) const {
  for (SmallVector<Module *, 2>::const_iterator it =
           m_runtimeModulesList.begin();
       it != m_runtimeModulesList.end(); ++it) {
    assert(*it != nullptr && "OpenclRuntime::findInRuntimeModule Encountered "
                             "NULL ptr in m_runtimeModulesList");
...
OpenclRuntime::findBuiltinFunction(StringRef mangledName) const {
  std::unique_ptr<VectorizerFunction> ret(
      new OpenClVFunction(std::string(mangledName)));
  return ret;
}
...
bool OpenclRuntime::isTIDGenerator(const Instruction *inst, bool *err,
  ...
  *err = false; // By default, no error expected..
  ...
  if (!CI)
    return false; // ID generator is a function call.
  if (!CI->getCalledFunction())
    return false;
  std::string funcName = CI->getCalledFunction()->getName().str();
  unsigned dimensionIndex =
      0; // Index of argument to CALL instruction, which is the dimension
...
unsigned OpenclRuntime::getNumJitDimensions() const { return MAX_WORK_DIM; }
...
bool OpenclRuntime::hasNoSideEffect(const std::string &func_name) const {
  ...
  // them first.
  if (isWorkItemBuiltin(func_name))
    return true;
  if (isSafeLLVMIntrinsic(func_name))
    return true;
  if (Mangler::isFakeExtract(func_name))
    return true;
  if (Mangler::isFakeInsert(func_name))
    return true;
  if (Mangler::isRetByVectorBuiltin(func_name))
    return true;
...
bool OpenclRuntime::isSafeToSpeculativeExecute(
...
bool OpenclRuntime::isExpensiveCall(const std::string &func_name) const {
  ...
  return stripped.startswith("read_image") ||
         stripped.startswith("write_image");
}
...
bool OpenclRuntime::isImageDescBuiltin(const std::string &func_name) const {
  if (0 == func_name.find("_Z16get_image_height"))
    return true;
  if (0 == func_name.find("_Z15get_image_width"))
    return true;
  if (0 == func_name.find("_Z15get_image_depth"))
    return true;
  if (0 == func_name.find("_Z27get_image_channel"))
    return true;
  if (0 == func_name.find("_Z13get_image_dim_"))
    return true;
  return false;
}

bool OpenclRuntime::isSafeLLVMIntrinsic(const std::string &func_name) const {
  if (0 == func_name.compare("llvm.var.annotation"))
    return true;
  if (0 == func_name.compare("llvm.dbg.declare"))
    return true;
  if (0 == func_name.compare("llvm.dbg.value"))
    return true;
  if (0 == func_name.compare("llvm.dbg.label"))
    return true;
  if (0 == func_name.compare("llvm.dbg.address"))
    return true;
  if (0 == func_name.compare("llvm.assume"))
    return true;
  return false;
}

bool OpenclRuntime::isReturnByPtrBuiltin(const std::string &func_name) const {
  if (!findInRuntimeModule(func_name))
    return false;
  FunctionDescriptor ret = demangle(func_name.c_str());
  if (ret == FunctionDescriptor::null())
    return false;
...
bool OpenclRuntime::isFakedFunction(StringRef fname) const {
  bool isFake = Mangler::isFakeInsert(std::string(fname)) ||
                Mangler::isFakeExtract(std::string(fname)) ||
                Mangler::isFakeBuiltin(std::string(fname));
  if (isFake)
    return true;
...
bool OpenclRuntime::isScalarMinMaxBuiltin(StringRef funcName, bool &isMin,
  ...
  isSigned = (basicType == PRIMITIVE_INT || basicType == PRIMITIVE_LONG);
  if (!isSigned && basicType != PRIMITIVE_UINT && basicType != PRIMITIVE_ULONG)
    return false;
  return true;
}
