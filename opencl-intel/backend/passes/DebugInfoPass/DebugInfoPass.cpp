...
public:
  DebugInfoPass() : ModulePass(ID), m_llvm_context(0) {}
...
private:
  ...
  //
  void insertDbgDeclaraLocalCall(DbgDeclareInst *call_instr,
                                 const FunctionContext &fContext);
...
const char *DebugInfoPass::BUILTIN_DBG_DECLARE_LOCAL_NAME =
    "__opencl_dbg_declare_local";
const char *DebugInfoPass::BUILTIN_DBG_DECLARE_GLOBAL_NAME =
    "__opencl_dbg_declare_global";
const char *DebugInfoPass::BUILTIN_DBG_ENTER_FUNCTION_NAME =
    "__opencl_dbg_enter_function";
const char *DebugInfoPass::BUILTIN_DBG_EXIT_FUNCTION_NAME =
    "__opencl_dbg_exit_function";
const char *DebugInfoPass::BUILTIN_DBG_STOPPOINT_NAME =
    "__opencl_dbg_stoppoint";
...
bool DebugInfoPass::runOnModule(Module &M) {
  ...
  for (; func_iter != m_pModule->end(); ++func_iter) {
    ...
    if (func_iter->isDeclaration() ||
        findFunctionsInModule(func_iter->getName()))
      continue;
...
void DebugInfoPass::addDebugBuiltinDeclarations() {
  ...
  FunctionType *local_functype =
      FunctionType::get(void_type, declare_params, false);
  ...
  FunctionType *enter_functype =
      FunctionType::get(void_type, enter_params, false);
...
void DebugInfoPass::insertComputeGlobalIds(Function *pFunc,
                                           FunctionContext &fContext) {
...
void DebugInfoPass::addDebugCallsToFunction(Function *pFunc,
                                            const FunctionContext &fContext) {
  ...
  //
  for (auto &BB : *pFunc) {
    for (BasicBlock::iterator II = BasicBlock::iterator(BB.getFirstNonPHI()),
                              E = BB.end();
         II != E; II++) {
      ...
      // - If the line was found and it's greater ( greater because we don't
      // want
      //   to stop on declaration of variable imported by block scope) than the
      //   previous saved C line, insert a stoppoint.
      //
...
Value *DebugInfoPass::makeAddressValueFromPointer(Metadata *ptr) const {
  ...
  return ConstantInt::get(IntegerType::getInt64Ty(m_pModule->getContext()),
                          addr);
}

void DebugInfoPass::insertDbgEnterFunctionCall(
    Function *pFunc, const FunctionContext &fContext) {
  Function *enter_function_func =
      m_pModule->getFunction(BUILTIN_DBG_ENTER_FUNCTION_NAME);
...
void DebugInfoPass::insertDbgDeclareGlobalCalls(
    Function *pFunc, const FunctionContext &fContext) {
  ...
  for (auto &globalVar : pFunc->getParent()->getGlobalList()) {
    ...
    CastInst *var_addr = CastInst::CreatePointerCast(
        GVAsValue, pointer_i8, "var_addr", fContext.original_first_instr);

    MDNode *mdn = MDNode::get(
        *m_llvm_context,
...
void DebugInfoPass::insertDbgStoppointCall(Instruction *instr,
                                           const FunctionContext &fContext) {
...
void DebugInfoPass::insertDbgDeclaraLocalCall(DbgDeclareInst *dbgDeclInstr,
                                              const FunctionContext &fContext) {
  // The first argument is an alloca for the variable.
  // Take the alloca from the metadata and cast it to i8*
  Type *pointer_i8 = IntegerType::getInt8PtrTy(*m_llvm_context);
  assert(dbgDeclInstr->getAddress() &&
         "Value address shouldn't be null for llvm.dbg.addr intrinsic");
  CastInst *var_ref_cast = CastInst::CreatePointerCast(
      dbgDeclInstr->getAddress(), pointer_i8, "", dbgDeclInstr);
  // Pass address of metadata description of the variable as an integral
  // argument.
  Value *var_metadata_addr =
      makeAddressValueFromPointer(dbgDeclInstr->getVariable());
  // Pass the metadata description of the complex expression address as an
  // integral argument.
  Value *expr_metadata_addr =
      makeAddressValueFromPointer(dbgDeclInstr->getExpression());

  Function *declare_local_func =
      m_pModule->getFunction(BUILTIN_DBG_DECLARE_LOCAL_NAME);
...
void DebugInfoPass::insertDbgExitFunctionCall(Instruction *instr,
                                              Function *pFunc,
                                              const FunctionContext &fContext) {
  Function *exit_function_func =
      m_pModule->getFunction(BUILTIN_DBG_EXIT_FUNCTION_NAME);
...
OCL_INITIALIZE_PASS_BEGIN(DebugInfoPass, "debug-info", "Debug Info", false,
                          false)
...
extern "C" {
ModulePass *createDebugInfoPass() { return new intel::DebugInfoPass(); }
}
