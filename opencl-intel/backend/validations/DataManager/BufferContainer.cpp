...
IMemoryObject *BufferContainer::CreateBuffer(const BufferDesc &buffDesc) {
  ...
  try {
    ...
    throw Exception::InvalidArgument(
        "CreateBuffer was unable to allocate buffer with size: " + oss.str() +
        " bytes.");
  }
}
...
IMemoryObject *BufferContainer::CreateImage(const ImageDesc &imDesc) {
  ...
  try {
    ...
    throw Exception::InvalidArgument(
        "CreateImage was unable to allocate image with size: " + oss.str() +
        " bytes.");
  }
}
...
BufferContainer::~BufferContainer() {
  ...
  if (!m_buffs.empty()) {
    for (MemoryObjectList::iterator it = m_buffs.begin(); it != m_buffs.end();
         ++it) {
      delete *it;
    }
    m_buffs.clear();
...
void BufferContainer::Accept(IContainerVisitor &visitor) const {
  ...
  for (MemoryObjectList::const_iterator it = m_buffs.begin();
       it != m_buffs.end(); ++it) {
    (*it)->Accept(visitor);
  }
}
