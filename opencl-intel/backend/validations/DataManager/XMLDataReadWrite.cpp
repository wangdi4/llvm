...
template <typename T>
inline void ReadWriteValueToStr(std::stringstream &isrt, T *data,
...
template <typename T>
inline void ReadWriteValueToStr8_t(std::stringstream &isrt, T *data,
                                   IXMLReadWriteBase::RWOperationType rwtype) {
  if (IXMLReadWriteBase::READ == rwtype) {
    int32_t _t;
    isrt >> _t;
...
template <>
inline void
ReadWriteValueToStr<int8_t>(std::stringstream &isrt, int8_t *data,
                            IXMLReadWriteBase::RWOperationType rwtype) {
...
template <>
inline void
ReadWriteValueToStr<uint8_t>(std::stringstream &isrt, uint8_t *data,
                             IXMLReadWriteBase::RWOperationType rwtype) {
...
template <typename T>
inline void
ReadWriteValueToStrFloat(std::stringstream &isrt, T *data,
                         IXMLReadWriteBase::RWOperationType rwtype) {
  if (IXMLReadWriteBase::READ == rwtype) {
    ...
    if (Utils::IsPInf(*data))
      isrt << "INF";
    else if (Utils::IsNInf(*data))
      isrt << "-INF";
...
template <>
inline void
ReadWriteValueToStr<float>(std::stringstream &isrt, float *data,
                           IXMLReadWriteBase::RWOperationType rwtype) {
...
template <>
inline void
ReadWriteValueToStr<double>(std::stringstream &isrt, double *data,
                            IXMLReadWriteBase::RWOperationType rwtype) {
...
template <>
inline void
ReadWriteValueToStr<CFloat16>(std::stringstream &isrt, CFloat16 *data,
                              IXMLReadWriteBase::RWOperationType rwtype) {
...
template <typename T>
inline void XMLReadWriteAttr(T *el, TiXmlElement *xml, const std::string &name,
                             const IXMLReadWriteBase::RWOperationType rwtype) {
  if (IXMLReadWriteBase::READ == rwtype) {
    if (TIXML_SUCCESS != xml->QueryValueAttribute(name, el))
      throw Exception::IOError("Error reading attribute with name " + name);
  } else if (IXMLReadWriteBase::WRITE == rwtype) { // WRITE
...
template <>
inline void
XMLReadWriteAttr<TypeDesc>(TypeDesc *el, TiXmlElement *xml,
                           const std::string &pref,
                           const IXMLReadWriteBase::RWOperationType rwtype) {
  std::string typeOfElement =
      (IXMLReadWriteBase::READ == rwtype) ? "" : el->TypeToString();
  XMLReadWriteAttr(&typeOfElement, xml, pref + "type", rwtype);

  std::size_t sizeOfElement =
      (IXMLReadWriteBase::READ == rwtype) ? 0 : el->GetSizeInBytes();
  XMLReadWriteAttr((int32_t *)&sizeOfElement, xml, pref + "type_size_in_bytes",
                   rwtype);
  ...
  if (TypeValWrapper(TypeValWrapper::ValueOf(typeOfElement)).IsAggregate() ||
    ...
    uint64_t numOfSubElements =
        (IXMLReadWriteBase::READ == rwtype) ? 0 : el->GetNumberOfElements();
    XMLReadWriteAttr(&numOfSubElements, xml, typeOfElement + "Width", rwtype);

    uint64_t numOfSubTypes =
        (IXMLReadWriteBase::READ == rwtype) ? 0 : el->GetNumOfSubTypes();
    XMLReadWriteAttr(&numOfSubTypes, xml, typeOfElement + "_number_of_subtypes",
                     rwtype);

    TypeDesc subElement = (IXMLReadWriteBase::READ == rwtype)
                              ? TypeDesc()
                              : el->GetSubTypeDesc(0);
    ...
    if (TypeValWrapper(TypeValWrapper::ValueOf(typeOfElement)).IsStruct()) {
      uint64_t offset = (IXMLReadWriteBase::READ == rwtype)
                            ? 0
                            : el->GetSubTypeDesc(0).GetOffsetInStruct();
      ...
      for (uint64_t i = 1; i < numOfSubTypes; ++i) {
        subElement = (IXMLReadWriteBase::READ == rwtype)
                         ? TypeDesc()
                         : el->GetSubTypeDesc(i);
        ostringstream oss;
        oss << i;
        XMLReadWriteAttr(&subElement, xml,
                         typeOfElement + "_sub" + oss.str() + "_", rwtype);

        offset = (IXMLReadWriteBase::READ == rwtype)
                     ? 0
                     : el->GetSubTypeDesc(i).GetOffsetInStruct();
        XMLReadWriteAttr(&offset, xml,
                         typeOfElement + "_element" + oss.str() + "_offset",
                         rwtype);
        if (IXMLReadWriteBase::READ == rwtype) {
          subElement.SetOffsetInStruct(offset);
...
template <>
inline void
XMLReadWriteAttr<BufferDesc>(BufferDesc *el, TiXmlElement *xml,
                             const std::string &,
                             const IXMLReadWriteBase::RWOperationType rwtype) {
  std::size_t numOfElements =
      (IXMLReadWriteBase::READ == rwtype) ? 0 : el->NumOfElements();
  TypeDesc elemDesc = (IXMLReadWriteBase::READ == rwtype)
                          ? TypeDesc()
                          : el->GetElementDescription();
...
template <>
inline void
XMLReadWriteAttr<ImageDesc>(ImageDesc *el, TiXmlElement *xml,
                            const std::string &,
                            const IXMLReadWriteBase::RWOperationType rwtype) {
  ImageTypeVal imageType = el->GetImageType();
  ImageSizeDesc imageSizes = (IXMLReadWriteBase::READ == rwtype)
                                 ? ImageSizeDesc()
                                 : el->GetSizesDesc();
  string dataTypeStr =
      (IXMLReadWriteBase::READ == rwtype) ? "" : el->DataTypeToString();
  string orderStr =
      (IXMLReadWriteBase::READ == rwtype) ? "" : el->OrderToString();
  size_t pixelSize =
      (IXMLReadWriteBase::READ == rwtype) ? 0 : el->GetElementSize();
  bool isNEAT = (IXMLReadWriteBase::READ == rwtype) ? false : el->IsNEAT();
  ...
  if (IXMLReadWriteBase::READ == rwtype) {
    *el = ImageDesc(imageType, imageSizes,
                    ImageChannelDataTypeValWrapper::ValueOf(dataTypeStr),
                    ImageChannelOrderValWrapper::ValueOf(orderStr), isNEAT);
  }
}
...
template <typename T>
inline void ReadWriteBufferVector(IMemoryObject *pB, TiXmlElement *pXml,
                                  IXMLReadWriteBase::RWOperationType rwtype) {
  ...
  for (uint32_t cntVec = 0; cntVec < desc.NumOfElements(); ++cntVec) {
    ...
    if (desc.GetElementDescription().IsAggregate() &&
        !desc.GetElementDescription().IsStruct()) {
      for (int32_t cntElem = 0; cntElem < (int32_t)desc.SizeOfVector();
           ++cntElem) {
    ...
    if (IXMLReadWriteBase::WRITE == rwtype)
      xmlVec->LinkEndChild(new TiXmlText(VecStr.str().c_str()));
  }
}

/// template function to read/write buffer as sequence of vector.
/// @param  typename T is dataType of Buffer Elements
inline void ReadWriteImageBinary(IMemoryObject *pB, TiXmlElement *pXml,
                                 IXMLReadWriteBase::RWOperationType rwtype) {
  ...
  if (IXMLReadWriteBase::WRITE == rwtype)
    pXml->LinkEndChild(new TiXmlText(imageSrt.str().c_str()));
}

template <typename T>
inline void ReadWriteNEATValueToStr(std::stringstream &isrt, NEATValue *data,
                                    IXMLReadWriteBase::RWOperationType rwtype) {
  if (IXMLReadWriteBase::READ == rwtype) {
...
template <typename T>
inline void
ReadWriteBufferVectorNEAT(IMemoryObject *pB, TiXmlElement *pXml,
                          IXMLReadWriteBase::RWOperationType rwtype) {
  BufferDesc desc = GetBufferDescription(pB->GetMemoryObjectDesc());
  ...
  for (uint32_t cntVec = 0; cntVec < desc.NumOfElements(); ++cntVec) {
    ...
    if (desc.GetElementDescription().IsAggregate() &&
      ...
      for (int32_t cntElem = 0; cntElem < (int32_t)desc.SizeOfVector();
           ++cntElem) {
        ReadWriteNEATValueToStr<T>(VecStr, &ba.GetElem(cntVec, cntElem),
                                   rwtype);
      }
    } else {
...
class XMLImageReadWrite : public IXMLReadWriteBase {
public:
  XMLImageReadWrite(IBufferContainer *pBC) : m_pBC(pBC) {
    if (NULL == m_pBC)
      throw Exception::InvalidArgument(
          "XMLImageReadWrite::XMLImageReadWrite constructor NULL input");
  }
...
class XMLBufferReadWrite : public IXMLReadWriteBase {
public:
  /// @brief ctor takes input parent IBufferContainer object
  /// It is passed in as argument since it is needed to create IMemoryObject
  /// objects
  XMLBufferReadWrite(IBufferContainer *pBC) : m_pBC(pBC) {
    if (NULL == m_pBC)
      throw Exception::InvalidArgument(
          "XMLBufferReadWrite::XMLBufferReadWrite constructor NULL input");
  }
  ...
  virtual IContainer *ReadWrite(IContainer *pContainer, TiXmlElement *pXml,
    ...
    if (false == desc.IsNEAT()) { // not NEAT case
#define CASE_READWRITEBUFFER(_str, _type)                                      \
  case (_str):                                                                 \
    ReadWriteBufferVector<_type>(pB, pXml, rwtype);                            \
    break;
      ...
      default:
        ...
        throw Exception::IOError(
            "XMLReader::ReadBuffer Unsupported data types");
      }
    }
#define CASE_NEATREADWRITEBUFFER(_str, _type)                                  \
  case (_str):                                                                 \
    ReadWriteBufferVectorNEAT<_type>(pB, pXml, rwtype);                        \
    break;
    else { // NEAT
      switch (elemType) {
...
class XMLMemoryObjectReadWrite : public IXMLReadWriteBase {
public:
  XMLMemoryObjectReadWrite(IBufferContainer *pBC) : m_pBC(pBC) {
    if (NULL == m_pBC)
      throw Exception::InvalidArgument(
          "XMLBufferReadWrite::XMLBufferReadWrite constructor NULL input");
  }
  ...
  virtual IContainer *ReadWrite(IContainer *pContainer, TiXmlElement *pXml,
    ...
    if (Buffer::GetBufferName() == pB->GetName()) {
      pXml = (IXMLReadWriteBase::READ == rwtype) ? pXml
...
class XMLBufferContainerReadWrite : public IXMLReadWriteBase {
public:
...
IContainer *XMLBufferContainerListReadWrite::ReadWriteBufferContainerList(
    IContainer *pContainer, TiXmlElement *pXml, const RWOperationType rwtype) {
  IBufferContainerList *pBCL = static_cast<IBufferContainerList *>(pContainer);
  if (NULL == pBCL)
    throw Exception::InvalidArgument("Invalid input/output IContainer object. "
                                     "Cannot cast to IBufferContainerList*");
...
/// @brief read data to IContainer
IContainer *XMLBufferContainerListReadWrite::ReadWrite(
    IContainer *pContainer, TiXmlElement *pXml, const RWOperationType rwtype) {
  IBufferContainerList *pBCL = static_cast<IBufferContainerList *>(pContainer);
  int32_t size_BCL =
      (WRITE == rwtype) ? (int32_t)pBCL->GetBufferContainerCount() : 0;
  ...
  for (int32_t cnt = 0; cnt < size_BCL; ++cnt) {
    ...
    IBufferContainer *pBC = (READ == rwtype) ? pBCL->CreateBufferContainer()
                                             : pBCL->GetBufferContainer(cnt);
