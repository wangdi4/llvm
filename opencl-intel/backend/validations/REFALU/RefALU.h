...
public:
  static bool GetFTZmode(void) { return FTZmode; }

  static void SetFTZmode(bool mode) { FTZmode = mode; }

  static long double precision_cast(double a) { return (long double)(a); }

  static double precision_cast(float a) { return (double)(a); }

  static float precision_cast(CFloat16 a) { return (float)a; }

  template <typename T> static bool le(const T &a, const T &b) {
    return Utils::le(a, b);
  }

  template <typename T> static bool ge(const T &a, const T &b) {
    return Utils::ge(a, b);
  }

  template <typename T> static bool eq(const T &a, const T &b) {
    return Utils::eq(a, b);
  }

  template <typename T> static bool gt(const T &a, const T &b) {
    return Utils::gt(a, b);
  }

  template <typename T> static bool lt(const T &a, const T &b) {
    return Utils::lt(a, b);
  }

  template <typename T> static T add(const T &a, const T &b) { return a + b; }

  template <typename T> static T sub(const T &a, const T &b) { return a - b; }

  template <typename T> static T div(const T &a, const T &b) {
    T ans = (T)(a / b);
    return ans;
  }

  template <typename T> static T mul(const T &a, const T &b) {
    return (T)(a * b);
  }

  template <typename T> static T neg(const T &a) { return -a; }
  ...
  /////////////// Common functions //////////////////////////////
  template <typename T>
  static T clamp(const T &x, const T &in_min, const T &in_max);

  template <typename T> static T degrees(const T &a) {
  ...
  template <typename T> static T radians(const T &a) {
  ...
  // cases where you would want a threshold function with a smooth transition.
  // Results are undefined if edge0 >= edge1.
  template <typename T>
  static T smoothstep(const T &edge0, const T &edge1, const T &x) {
    if (edge0 >= edge1)
  ...
  template <typename T> static T sign(const T &x) {
    ...
    assert(false);
    return T(
        -2.0); // we do never go here, this return is for correct compilation
  }
  ...
  template <typename T> static T round_ni(const T &a) {
  ...
  /// Returns a quiet NaN. The nancode may be placed in the significand of the
  /// resulting NaN
  ...
  static double nan(const uint32_t &x) { return Conformance::reference_nan(x); }
  static long double nan(const uint64_t &x) {
    return Utils::GetNaN<long double>();
  }

  template <typename T> static T frc(const T &a) { return sub(a, round_ni(a)); }
  ...
  template <typename T> static int isInf(const T &x) {
    return Utils::IsNInf(x) || Utils::IsPInf(x);
  }

  template <typename T> static int isNormal(const T &x) {
    return !Utils::IsDenorm(x) && !Utils::IsNaN(x) && !Utils::IsNInf(x) &&
           !Utils::IsPInf(x);
  }

  template <typename T> static int isNan(const T &x) { return Utils::IsNaN(x); }
  ...
  /* geometric functions */
  template <typename T>
  static void cross(const T *a, const T *b, T *out, const uint32_t n);
  template <typename T>
  static T distance(const T *a, const T *b, const uint32_t n);
  ...
  template <typename T>
  static void normalize(const T *a, T *out, const uint32_t n);
};
} // namespace Validation
