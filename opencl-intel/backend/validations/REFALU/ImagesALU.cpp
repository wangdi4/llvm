...
float MirroredRepeatNormalizedAddressFn(float fValue, size_t maxValue) {
  ...
  float s_prime =
      2.0f *
      rintf(fValue *
            0.5f); // Note halfway values flip flop here due to rte, but they
...
static float convert_half_to_float(unsigned short halfValue) {
  ...
  // http://en.wikipedia.org/wiki/Half_precision Note: we store these in 32-bit
  // ints to make the bit manipulations easier later
  ...
  } else if (exponent == 31) // Special-case "numbers"
  {
    ...
    // If the mantissa is 0, it's infinity, else it's NAN, but in either case,
    // the packing method is the same
    outFloat.bits = (sign << 31) | 0x7f800000 | (mantissa << 13);
    return outFloat.floatValue;
  }

  // Plain ol' normalized number, so adjust to the ranges a 32-bit float expects
  // and repack
  exponent += (127 - 15);
  mantissa <<= 13;
...
static inline void pixelMax(const float a[4], const float b[4],
                            float *results) {
  for (int i = 0; i < 4; i++)
...
FloatPixel sample_image_pixel_float_offset(
  ...
  if (imageSampler->normalized_coords) {
    ...
    case CL_MEM_OBJECT_IMAGE2D_ARRAY:
      y = unnormalize_coordinate("y", y, yAddressOffset,
    ...
    default:
      y = unnormalize_coordinate("y", y, yAddressOffset,
                                 (float)imageInfo->height,
                                 imageSampler->addressing_mode, verbose);
      z = unnormalize_coordinate("z", z, zAddressOffset,
                                 (float)imageInfo->depth,
                                 imageSampler->addressing_mode, verbose);
    }
  ...
  if (imageSampler->filter_mode == CL_FILTER_NEAREST) {
    ...
    if (verbose) {
      if (iz)
        log_info(
            "\tActual integer coords used (i = floor(x)): { %d, %d, %d }\n", ix,
            iy, iz);
      else
    ...
    if (depth == 0 || imageInfo->type == CL_MEM_OBJECT_IMAGE2D_ARRAY ||
      ...
      if (imageInfo->type == CL_MEM_OBJECT_IMAGE2D_ARRAY) {
        layer_offset =
            ...
            (size_t)calculate_array_index(z, (float)(imageInfo->arraySize - 1));
      } else if (imageInfo->type == CL_MEM_OBJECT_IMAGE1D_ARRAY) {
        layer_offset =
            imageInfo->slicePitch *
            (size_t)calculate_array_index(y, (float)(imageInfo->arraySize - 1));
      ...
      if (verbose)
        log_info("\tActual integer coords used (i = floor(x-.5)): i0:{%d, %d } "
                 "and i1:{%d, %d }\n",
                 x1, y1, x2, y2);
      ...
      for (int i = 0; i < 4; i++) {
        outData[i] =
            (float)((upLeft[i] * weights[0][0]) + (upRight[i] * weights[1][0]) +
      ...
      if (verbose) {
        ...
        log_info("\t\tp011: %f, %f, %f, %f\n\n", lowRightA[0], lowRightA[1],
                 lowRightA[2], lowRightA[3]);
        ...
        log_info("\t\tp111: %f, %f, %f, %f\n", lowRightB[0], lowRightB[1],
                 lowRightB[2], lowRightB[3]);
      }
      ...
      if (verbose)
        log_info("\tfrac( x - 0.5f ) = %f,  frac( y - 0.5f ) = %f, frac( z - "
                 "0.5f ) = %f\n",
...
template <class T>
void swizzle_vector_for_image(T *srcVector,
...
void pack_image_pixel(float *srcVector, const cl_image_format *imageFormat,
  ...
  case CLK_HALF_FLOAT: {
    ...
    throw Exception::NotImplemented(
        "pack_image_pixel::Implement Float to Half");
#if 0
  ...
  case CLK_UNORM_INT8: {
    ...
    if (imageFormat->image_channel_order == CLK_sRGBA ||
        imageFormat->image_channel_order == CLK_sBGRA) {
      for (uint64_t i = 0; i < channelCount - 1; ++i) {
        srcVector[i] = srcVector[i] <= 0.0031308f
                           ? 12.92f * srcVector[i]
                           : 1.055 * pow(srcVector[i], 1.0f / 2.4f) - 0.055;
      }
    }
    for (unsigned int i = 0; i < channelCount; i++)
  ...
  case CLK_UNSIGNED_INT32: {
    ...
    for (unsigned int i = 0; i < channelCount; i++)
      ptr[i] = (cl_uint)CONVERT_UINT(
          srcVector[i], MAKE_HEX_FLOAT(0x1.fffffep31f, 0x1fffffe, 31 - 23),
          CL_UINT_MAX);
    break;
  }
  default:
...
float get_max_relative_error(cl_image_format *format,
  ...
  case CLK_UNORM_INT_101010:
    ...
    // by reciprocal (using reciprocal generated in round to +inf mode, so
    // that 1.0 matches spec)
    break;

    // If the implementation supports these formats then it will have to allow
    // rounding error here too, because not all 32-bit ints are exactly
  ...
  if (sampler->addressing_mode == CL_ADDRESS_REPEAT ||
      ...
      sampler->filter_mode != CL_FILTER_NEAREST || sampler->normalized_coords) {
#if !defined(_WIN32)
...
float get_max_absolute_error(cl_image_format *format,
  ...
  case CLK_FLOAT:
    // disabled CL constant because of gcc  error: use of C99 hexadecimal
    // floating constant return CL_FLT_MIN
    return FLT_MIN;
  default:
    return 0.0f;
  }
...
bool get_integer_coords_offset(float x, float y, float z, float xAddressOffset,
  ...
  if (imageSampler->normalized_coords) {
    ...
    default:
      // Also, remultiply to the original coords. This simulates any truncation
      // in the pass to OpenCL
...
void sample_image_pixel<float>(void *imageData, image_descriptor *imageInfo,
  ...
  sample_image_pixel_float(imageData, imageInfo, x, y, z, imageSampler, outData,
                           0, &containsDenorms);
}
