...
/// to / flush denormals to zeros before comparison, comparison like a < b may
/// give wrong / result in "certain cases" where we do need correct compasion
/// result when operands / are denormals .... these functions comapre
/// floats/doubles using signed integer/long int / rep. In other cases, when
/// flushing to zeros is fine, these should not be used. / Also these doesn't
/// check for nans and assume nans are handled separately as special edge case /
/// by the caller which calls these functions / return 0 if both are equal, 1 if
/// x > y and -1 if x < y.
//
// inline
