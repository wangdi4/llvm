...
// dictatated by the passed arguments.
...
// returns anything other than CL_SUCCESS (0), the harness exits.
typedef int (*DeviceCheckFn)(cl_device_id device);

// Same as runTestHarness, but also supplies a function that checks the created
// device for required functionality.
extern int runTestHarnessWithCheck(
    int argc, const char *argv[], unsigned int num_fns, basefn fnList[],
    const char *fnNames[], int imageSupportRequired, int forceNoContextCreation,
...
extern int parseAndCallCommandLineTests(
    int argc, const char *argv[], cl_device_id device, unsigned int num_fns,
...
//    functionIndexToCall can be a valid index into the function list, or -1 to
//    run all of them. partialName can be a string to partially match function
//    names against and only execute functions who
//        match, or NULL to not restrict execution (ignored if
//        functionIndexToCall is not -1)
//    functionList is the actual array of functions
//    numFunctions is the number of functions in the list (which should NOT have
//    NULL at the end for "all") functionNames is an array of strings
//    representing the name of each function, to be used in partial matching
//    contextProps are used to create a testing context for each test
//    deviceToUse, deviceGroupToUse and numElementsToUse are all just passed to
//    each test function
...
extern int callTestFunctions(basefn functionList[], int numFunctions,
                             ...
                             int functionIndexToCall, const char *partialName,
                             cl_command_queue_properties queueProps);

// This function is called by callTestFunctions, once per function, to do setup,
// call, logging and cleanup
extern int callSingleTestFunction(basefn functionToCall,
...
// Given a pre-existing device type choice, check the environment for an
// override, then print what choice was made and how (and return the overridden
...
// extern void CL_CALLBACK notify_callback(const char *errinfo, const void
