...
enum TestSamplerType {
  ...
  CLAMPTOEDGE_FALSE_NEAREST =
    CLK_ADDRESS_CLAMP_TO_EDGE | CLK_NORMALIZED_COORDS_FALSE | CLK_FILTER_NEAREST,
  REPEAT_FALSE_NEAREST =
    CLK_ADDRESS_REPEAT | CLK_NORMALIZED_COORDS_FALSE | CLK_FILTER_NEAREST,
  MIRRORED_FALSE_NEAREST =
    CLK_ADDRESS_MIRRORED_REPEAT | CLK_NORMALIZED_COORDS_FALSE | CLK_FILTER_NEAREST,
  NONE_TRUE_NEAREST =
  ...
  MIRRORED_TRUE_NEAREST =
    CLK_ADDRESS_MIRRORED_REPEAT| CLK_NORMALIZED_COORDS_TRUE | CLK_FILTER_NEAREST,
  NONE_FALSE_LINEAR =
  ...
  CLAMPTOEDGE_FALSE_LINEAR =
    CLK_ADDRESS_CLAMP_TO_EDGE | CLK_NORMALIZED_COORDS_FALSE | CLK_FILTER_LINEAR,
  REPEAT_FALSE_LINEAR =
    CLK_ADDRESS_REPEAT | CLK_NORMALIZED_COORDS_FALSE | CLK_FILTER_LINEAR,
  MIRRORED_FALSE_LINEAR =
    CLK_ADDRESS_MIRRORED_REPEAT | CLK_NORMALIZED_COORDS_FALSE | CLK_FILTER_LINEAR,
  NONE_TRUE_LINEAR =
  ...
  MIRRORED_TRUE_LINEAR =
    CLK_ADDRESS_MIRRORED_REPEAT | CLK_NORMALIZED_COORDS_TRUE | CLK_FILTER_LINEAR,
...
TEST(DataVersionTest, ConvertSample_v0_v1){
    ...
    sampler = (unsigned int)(CL_DEV_SAMPLER_ADDRESS_NONE | CL_DEV_SAMPLER_FILTER_NEAREST);
    convertSampler(&sampler);
    EXPECT_EQ((unsigned int)NONE_FALSE_NEAREST, sampler);

    sampler = (unsigned int)(CL_DEV_SAMPLER_ADDRESS_NONE | CL_DEV_SAMPLER_FILTER_LINEAR);
    convertSampler(&sampler);
    EXPECT_EQ((unsigned int)NONE_FALSE_LINEAR, sampler);

    sampler =  (unsigned int)(CL_DEV_SAMPLER_ADDRESS_CLAMP | CL_DEV_SAMPLER_FILTER_NEAREST);
    convertSampler(&sampler);
    EXPECT_EQ((unsigned int)CLAMP_FALSE_NEAREST, sampler);

    sampler =  (unsigned int)(CL_DEV_SAMPLER_ADDRESS_CLAMP | CL_DEV_SAMPLER_FILTER_LINEAR);
    convertSampler(&sampler);
    EXPECT_EQ((unsigned int)CLAMP_FALSE_LINEAR, sampler);

    sampler =  (unsigned int)(CL_DEV_SAMPLER_ADDRESS_CLAMP_TO_EDGE| CL_DEV_SAMPLER_FILTER_NEAREST);
    convertSampler(&sampler);
    EXPECT_EQ((unsigned int)CLAMPTOEDGE_FALSE_NEAREST, sampler);

    sampler =  (unsigned int)(CL_DEV_SAMPLER_ADDRESS_CLAMP_TO_EDGE | CL_DEV_SAMPLER_FILTER_LINEAR);
    convertSampler(&sampler);
    EXPECT_EQ((unsigned int)CLAMPTOEDGE_FALSE_LINEAR, sampler);

    sampler =  (unsigned int)(CL_DEV_SAMPLER_ADDRESS_REPEAT | CL_DEV_SAMPLER_FILTER_NEAREST);
    convertSampler(&sampler);
    EXPECT_EQ((unsigned int)REPEAT_FALSE_NEAREST, sampler);

    sampler =  (unsigned int)(CL_DEV_SAMPLER_ADDRESS_REPEAT | CL_DEV_SAMPLER_FILTER_LINEAR);
    convertSampler(&sampler);
    EXPECT_EQ((unsigned int)REPEAT_FALSE_LINEAR, sampler);

    sampler =  (unsigned int)(CL_DEV_SAMPLER_ADDRESS_MIRRORED_REPEAT | CL_DEV_SAMPLER_FILTER_NEAREST);
    convertSampler(&sampler);
    EXPECT_EQ((unsigned int)MIRRORED_FALSE_NEAREST, sampler);

    sampler =  (unsigned int)(CL_DEV_SAMPLER_ADDRESS_MIRRORED_REPEAT | CL_DEV_SAMPLER_FILTER_LINEAR);
    convertSampler(&sampler);
...
static void  testFindSamplers(std::string sourceIn, std::vector<unsigned int >& samplerIndxs, std::string kernelName)  {
#if defined (_WIN32)
#if defined (_M_X64)
...
TEST(DataVersionTest, DISABLED_ConvertData) /* CSSD100018373 */ {
    ...
    // for kernelName1 and kernelName2 and must not change data for kernelName3
    ...
    std::string source2 = std::string("__kernel void ") + kernelName2 + 
        std::string("( read_only image1d_t input, sampler_t imageSampler) {"
        " int tidX = get_global_id(0);"
        " int offset = tidX;"
        " float coord = (float)offset;"
        " float4 results = read_imagef( input, imageSampler, coord ); }");

    // second kernel's arguments placed in the other order comparing to the first kernel
    std::string source3 = std::string("__kernel void ") + kernelName3 + 
        std::string("( sampler_t imageSampler, read_only image1d_t input) {"
        " int tidX = get_global_id(0);"
        " int offset = tidX;"
        " float coord = (float)offset;"
        " float4 results = read_imagef( input, imageSampler, coord ); }");
    ...
    for( unsigned int j = 0; j < metadata->getNumOperands(); j++ ) {
        ...
        OCLKernelArgumentsList argDescriptions = parser.KernelArgumentsParser(vecKernelNames[j], pModule.get());
        ...
        unsigned int arrSamplersIn [arrSamplersLen] = {
            (unsigned int)(CL_DEV_SAMPLER_ADDRESS_NONE | CL_DEV_SAMPLER_FILTER_NEAREST),
            (unsigned int)(CL_DEV_SAMPLER_ADDRESS_NONE | CL_DEV_SAMPLER_FILTER_LINEAR),
            (unsigned int)(CL_DEV_SAMPLER_ADDRESS_CLAMP | CL_DEV_SAMPLER_FILTER_NEAREST),
            (unsigned int)(CL_DEV_SAMPLER_ADDRESS_CLAMP | CL_DEV_SAMPLER_FILTER_LINEAR),
            (unsigned int)(CL_DEV_SAMPLER_ADDRESS_CLAMP_TO_EDGE| CL_DEV_SAMPLER_FILTER_NEAREST),
            (unsigned int)(CL_DEV_SAMPLER_ADDRESS_CLAMP_TO_EDGE | CL_DEV_SAMPLER_FILTER_LINEAR),
            (unsigned int)(CL_DEV_SAMPLER_ADDRESS_REPEAT | CL_DEV_SAMPLER_FILTER_NEAREST),
            (unsigned int)(CL_DEV_SAMPLER_ADDRESS_REPEAT | CL_DEV_SAMPLER_FILTER_LINEAR),
            (unsigned int)(CL_DEV_SAMPLER_ADDRESS_MIRRORED_REPEAT | CL_DEV_SAMPLER_FILTER_NEAREST),
            (unsigned int)(CL_DEV_SAMPLER_ADDRESS_MIRRORED_REPEAT | CL_DEV_SAMPLER_FILTER_LINEAR)
        };
        ...
        // for the first kernel in this test GetMemoryObject(j) int not a sampler, it is just integer 
        // so it must not be changed by ConvertData, the number of position is 2,
        // for the second kernel the samlper has position number 1,
        // for the third kernel sampler has position number 0
        unsigned int * sampler = NULL;
        switch(j) {
        case 0:
            sampler = (uint32_t*)input.GetBufferContainer(0)->GetMemoryObject(2)->GetDataPtr();
            break;
        case 1:
            sampler = (uint32_t*)input.GetBufferContainer(0)->GetMemoryObject(1)->GetDataPtr();
            break;
        case 2:
            sampler = (uint32_t*)input.GetBufferContainer(0)->GetMemoryObject(0)->GetDataPtr();
            break;
        default:
            break;
        }
        ...
        for (unsigned int i = 0; i<arrSamplersLen; i++) {
            *sampler = arrSamplersIn[i];
            ...
            // for second and third kernels samplers must be converted, for the thist one must not be converted
            if (j == 0)
