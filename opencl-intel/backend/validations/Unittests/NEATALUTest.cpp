...
template <typename T> class NEATAluTyped : public ::testing::Test {};
typedef ::testing::Types<float, double> FloatTypes;
TYPED_TEST_SUITE(NEATAluTyped, FloatTypes, );

typedef NEATVector (*NEATVectorBinaryOp)(const NEATVector &,
                                         const NEATVector &);
...
template <> class dT<long double> {
public:
  typedef double type;
};

// auxillary functions for cross and dot
template <typename T> T Ulp1(void);
...
/// Tests given ALU operation. Operation should has range and domain being equal
/// to whole set of real numbers.
template <typename TypeParam>
void BasicTest(NEATScalarBinaryOp ScalarOp, NEATVectorBinaryOp VectorOp,
               TypeParam(RefFunc)(const TypeParam &, const TypeParam &),
               int ulpsCount) {
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      if (ulpsCount == 0) {
        EXPECT_EQ(accRes.GetStatus(), NEATValue::ACCURATE)
            << "aSeed = " << aSeed << " bSeed = " << bSeed;
        EXPECT_EQ(*accRes.GetAcc<TypeParam>(), refRes)
            << "aSeed = " << aSeed << " bSeed = " << bSeed;
      } else
      // Operation has some inaccuracy
      {
        EXPECT_EQ(accRes.GetStatus(), NEATValue::INTERVAL)
            << "aSeed = " << aSeed << "bSeed = " << bSeed;
      ...
      SuperT allResults[RES_COUNT] = {
          RefFunc((SuperT)aMin[idx], (SuperT)bMin[idx]),
      ...
      case NEATValue::INTERVAL:
        passed = TestIntExpanded<SuperT>(minRef[i], maxRef[i], res,
                                         NEATALU::ADD_ERROR);
        break;
    ...
    for (uint32_t i = 0; i < VECTOR_SIZE; i++) {
      bool passed =
          TestIntExpanded<SuperT>(minRef[i], maxRef[i], res[i], ulpsCount);
      EXPECT_TRUE(passed) << "aSeed = " << aSeed << " bSeed = " << bSeed;
    }
  }
}

// tested functions should return false if NEATValue is wrong, i.e Unknown or
// Unwritten for Interval
...
TYPED_TEST(NEATAluTyped, TestExpanded) {
  ...
  IntervalError<TypeParam>::ExpandFPInterval(&refMinExp, &refMaxExp,
                                             IntervalError<TypeParam>(ulps));
  ...
  IntervalError<TypeParam>::ExpandFPInterval(&refAccExpMin, &refAccExpMax,
                                             IntervalError<TypeParam>(ulps));
  NEATValue accExp =
      NEATValue(TypeParam(refAccExpMin), TypeParam(refAccExpMax));
  ...
  EXPECT_FALSE(
      TestIntExpandedDotMix<SuperT>(refMinIn, refMaxIn, unwr, ref4ulps, ulps));
  EXPECT_FALSE(
      TestIntExpandedDotMix<SuperT>(refMinIn, refMaxIn, unkn, ref4ulps, ulps));
  EXPECT_FALSE(
      TestIntExpandedDotMix<SuperT>(refMinIn, refMaxIn, any, ref4ulps, ulps));

  EXPECT_TRUE(TestIntExpandedDotMix<SuperT>(refMinIn, refMaxIn, intervalExp,
                                            ref4ulps, ulps));
  ...
  EXPECT_FALSE(
      TestNeatAcc<SuperT>(unwr, accExp, refAcc, ulps, edgeVals, NUM_EDGE_VALS));
  EXPECT_FALSE(
      TestNeatAcc<SuperT>(unkn, accExp, refAcc, ulps, edgeVals, NUM_EDGE_VALS));
  EXPECT_FALSE(
      TestNeatAcc<SuperT>(any, accExp, refAcc, ulps, edgeVals, NUM_EDGE_VALS));
  EXPECT_FALSE(TestNeatAcc<SuperT>(intervalExp, accExp, refAcc, ulps, edgeVals,
                                   NUM_EDGE_VALS));

  NEATValue acc = NEATValue(TypeParam(refAcc));
  EXPECT_TRUE(
      TestNeatAcc<SuperT>(acc, accExp, refAcc, ulps, edgeVals, NUM_EDGE_VALS));
}
...
TYPED_TEST(NEATAluTyped, AddSubMul) {
  ...
  if (typeSupported) {
    BasicTest<TypeParam>(NEATALU::add<TypeParam>, NEATALU::add<TypeParam>,
                         RefALU::add<TypeParam>, NEATALU::ADD_ERROR);
    BasicTest<TypeParam>(NEATALU::mul<TypeParam>, NEATALU::mul<TypeParam>,
                         RefALU::mul<TypeParam>, NEATALU::MUL_ERROR);
    BasicTest<TypeParam>(NEATALU::sub<TypeParam>, NEATALU::sub<TypeParam>,
                         RefALU::sub<TypeParam>, NEATALU::SUB_ERROR);
...
TEST(NEATAlu, mulRegression) {
  ...
  // 0x3fd6b851e0000000 (0.35499998927116394) * 0x402274b015871a94
  // (9.2279059150144249) = 0x400a350e77ae6467 (3.2759065008254313)
  union {
    double f;
    uint64_t u;
  } a1, b1, c1;
...
TYPED_TEST(NEATAluTyped, fmod) {
  const int NUM_TESTS = 13;
  {
    TypeParam data[NUM_TESTS][4] = {
        {1.0f, 3.0f, 1.0f, 3.0f},   {11.9f, 12.1f, 1.8f, 2.2f},
        {10.9f, 11.1f, 1.9f, 2.1f}, {11.9f, 12.1f, 1.3f, 2.2f},
        {11.9f, 12.1f, 1.6f, 2.0f}, {-1.0f, 1.0f, 1.0f, 3.0f},
        {-3.0f, -1.0f, 1.0f, 3.0f}, {-3.0f, 1.0f, 1.0f, 2.0f},
        {-1.0f, 3.0f, 1.0f, 2.0f},  {-10.0f, -1.0f, 1.0f, 2.0f},
        {-5.0f, -2.0f, 0.5f, 2.0f}, {1.0f, 3.0f, -3.0f, -1.0f},
    ...
    for (int i = 0; i < NUM_TESTS; i++) {
      ...
      if (frem.GetStatus() != NEATValue::UNKNOWN) {
        ...
        TypeParam divisorStep =
            (*divisor.GetMax<TypeParam>() - *divisor.GetMin<TypeParam>()) /
            STEPS_COUNT;
        TypeParam dividendStep =
            (*dividend.GetMax<TypeParam>() - *dividend.GetMin<TypeParam>()) /
            STEPS_COUNT;
        for (TypeParam divisorIdx = *divisor.GetMin<TypeParam>();
             divisorIdx < *divisor.GetMax<TypeParam>();
             divisorIdx += divisorStep) {
          for (TypeParam dividendIdx = *dividend.GetMin<TypeParam>();
               dividendIdx < *dividend.GetMax<TypeParam>();
               dividendIdx += dividendStep) {
  ...
  }
  {
    ...
    TypeParam data[UNKNOWN_TESTS_COUNT][4] = {
        {34.0f, 36.0f, 4.9f, 5.1f}, {34.0f, 36.0f, 5.0f, 5.0f},
        {11.9f, 12.1f, 2.0f, 2.0f}, {11.9f, 12.1f, 2.0f, 2.1f},
        {11.9f, 12.1f, 1.9f, 2.0f}, {2.0f, 7.0f, -1.0f, 1.0f},
...
/// Tests NEATAlu::fcmp scalar and vector functionality with the following
/// corner cases:
/// 1. Positive and negative results for each type of comparison, including
/// ordered and unordered - without NaNs
...
TYPED_TEST(NEATAluTyped, fcmp) {
  ...
  CmpType cmpType[NUM_TESTS] = {CMP_OEQ, CMP_OGT, CMP_OGE,   CMP_OLT,
                                CMP_OLE, CMP_ONE, CMP_ORD,   CMP_UEQ,
                                CMP_UGT, CMP_UGE, CMP_ULT,   CMP_ULE,
                                CMP_UNE, CMP_UNO, CMP_FALSE, CMP_TRUE};
  ...
  for (int i = 0; i < UNIQUE_CMP_TYPES; i++) {
    {
      ...
      NEATValue trueAns2 = NEATALU::fcmp<TypeParam>(
          with, pos,
          cmpType[i + UNIQUE_CMP_TYPES +
                  1]); // Add 1 here for pure ordered/unordered comparison test
      NEATValue falseAns1 = NEATALU::fcmp<TypeParam>(with, neg, cmpType[i]);
      NEATValue falseAns2 = NEATALU::fcmp<TypeParam>(
          with, neg,
          cmpType[i + UNIQUE_CMP_TYPES +
                  1]); // Add 1 here for pure ordered/unordered comparison test
  ...
  for (int i = 0; i < UNIQUE_CMP_TYPES + 1; i++) {
    ...
    NEATValue uno1 = NEATALU::fcmp<TypeParam>(
        nanVal, tmp1, cmpType[i + UNIQUE_CMP_TYPES + 1]);
    NEATValue uno2 = NEATALU::fcmp<TypeParam>(
        tmp1, nanVal, cmpType[i + UNIQUE_CMP_TYPES + 1]);
    NEATValue uno3 = NEATALU::fcmp<TypeParam>(
        nanVal, nanVal, cmpType[i + UNIQUE_CMP_TYPES + 1]);
  ...
  for (int i = 0; i < UNIQUE_CMP_TYPES; i++) {
    {
      ...
      NEATValue unknown1 =
          NEATALU::fcmp<TypeParam>(cmpWith, cmpWith, cmpType[i]);
      NEATValue unknown2 = NEATALU::fcmp<TypeParam>(
          cmpWith, cmpWith,
          cmpType[i + UNIQUE_CMP_TYPES +
                  1]); // Add 1 here for pure ordered/unordered comparison test
    ...
    for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
      ...
      for (uint32_t i = 0; i < wrap.GetSize(); i++) {
        ...
        if (firstFloat[testIdx][i] < secondFloat[testIdx][i])
          interval[i] =
              NEATValue(firstFloat[testIdx][i], secondFloat[testIdx][i]);
        else
          interval[i] =
              NEATValue(secondFloat[testIdx][i], firstFloat[testIdx][i]);
      }
...
/* test for atomic_xchg (NEATValue *p, NEATValue val) instruction.  It swaps the
old value stored at location p with new value given by val. Returns old value.
*/
TYPED_TEST(NEATAluTyped, atomic_xchg) {
  ...
  if (sizeof(TypeParam) == sizeof(float)) {
    ...
    // currently atomic_xchg is used for the single precision floating point
    // data only
    return;
  } else {
    GTEST_FAIL();
...
TYPED_TEST(NEATAluTyped, shufflevector) {
      ...
      for (int widthSrc2Idx = 0; widthSrc2Idx < WIDTHS_COUNT; widthSrc2Idx++)
        for (int widthDstIdx = 0; widthDstIdx < WIDTHS_COUNT; widthDstIdx++) {
          ...
          for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
            ...
            DataGenerator::SetBufferContainerFillMethod<uint32_t>(
                bcfm, dataType, wrapDst, 1,
                DataGenerator::FILL_RANDOM_FROM_RANGE, 0, 0, maxSrcIndex, 0);
            DataGenerator::SetBufferContainerListFillMethod(bclfm, bcfm);
            DataGenerator dg(bclfm);
            dg.Read(&list);

            uint32_t *data = (uint32_t *)list.GetBufferContainer(0)
                                 ->GetMemoryObject(0)
                                 ->GetDataPtr();
  ...
  {
    // test to make dst vector from one src vector - second vector can have zero
    // width, mask indexes can't be grater than the width of first vector
    const int WIDTHS_COUNT = 6;
    VectorWidth widths[] = {V1, V2, V3, V4, V8, V16};
...
static bool areEqualFloatDouble(const NEATValue &val_1 /*float*/,
                                const NEATValue &val_2 /*double*/)
// the first operand must correspond to float data, the second operand must
// correspond to double data
{
...
static bool areEqualDoubleFloat(const NEATValue &val_1 /*double*/,
                                const NEATValue &val_2 /*float*/)
// the first operand must correspond to float data, the second operand must
// correspond to double data
{
  NEATValue val1 = val_1;
...
TEST(NEATAlu, fpext) {
...

  {
    ...
    for (int i = 0; i < NUM_TESTS * 2; i++) {
      DataGenerator::SetBufferContainerFillMethod<float>(
          bcfm, dataType, curWidth, 1,
          DataGenerator::FILL_RANDOM_FLOAT_WITH_SPECIAL_VALUES, 0, 0, 0, 1.0);
    }
    ...
    VectorWidthWrapper wrap(curWidth);
    for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
      float *firstFloat = (float *)list.GetBufferContainer(0)
                              ->GetMemoryObject(testIdx)
                              ->GetDataPtr();
      float *secondFloat = (float *)list.GetBufferContainer(0)
                               ->GetMemoryObject(testIdx + NUM_TESTS)
                               ->GetDataPtr();
...
TEST(NEATAlu, fptrunc) {
    ...
    for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
      ...
      for (uint32_t i = 0; i < wrap.GetSize(); i++) {
        ...
        if (firstDouble[testIdx][i] < secondDouble[testIdx][i])
          interval[i] =
              NEATValue(firstDouble[testIdx][i], secondDouble[testIdx][i]);
        else
          interval[i] =
              NEATValue(secondDouble[testIdx][i], firstDouble[testIdx][i]);
      }
...

  {
    ...
    for (int i = 0; i < NUM_TESTS * 2; i++) {
      DataGenerator::SetBufferContainerFillMethod<double>(
          bcfm, dataType, curWidth, 1,
          DataGenerator::FILL_RANDOM_FLOAT_WITH_SPECIAL_VALUES, 0, 0, 0, 1.0);
    }
    ...
    VectorWidthWrapper wrap(curWidth);
    for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
      double *firstDouble = (double *)list.GetBufferContainer(0)
                                ->GetMemoryObject(testIdx)
                                ->GetDataPtr();
      double *secondDouble = (double *)list.GetBufferContainer(0)
                                 ->GetMemoryObject(testIdx + NUM_TESTS)
                                 ->GetDataPtr();
...

  {
...

    {
      ...
      for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
        ...
        try {
          NEATValue testVal =
              NEATALU::bitcast<float, double, NEATValue>(accurate);
        ...
        try {
          NEATValue testVal =
              NEATALU::bitcast<float, double, NEATValue>(interval);
          NEATValue ref = NEATValue(NEATValue::UNKNOWN);
          if (!(NEATValue::areEqual<double>(testVal, ref)))
            passed &= false;
        } catch (Exception::InvalidArgument) {
...

  {
    ...
    for (int widthSrcIdx = 0; widthSrcIdx < WIDTHS_COUNT; widthSrcIdx++) {
      ...
      for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
        ...
        for (int widthSrcIdx2 = 0; widthSrcIdx2 < WIDTHS_COUNT;
             widthSrcIdx2++) {
...
template <typename T> class NEATAluTypedMath : public ALUTest {
public:
  DataTypeVal dataTypeVal;

  NEATAluTypedMath() {

    RefALU::SetFTZmode(T::mode); // we use ValueTypeContainer type here, T::mode
                                 // is FTZ mode, can be true or false
...
typedef ::testing::Types<
    ValueTypeContainer<float, true>, ValueTypeContainer<float, false>,
    ValueTypeContainer<double, true>, ValueTypeContainer<double, false>>
    FloatTypesMathFTZ;
typedef ::testing::Types<ValueTypeContainer<float, true>,
                         ValueTypeContainer<float, false>>
    Float32TypeMathFTZ;
TYPED_TEST_SUITE(NEATAluTypedMath, FloatTypesMathFTZ, );
TYPED_TEST_SUITE(NEATAluTypedFastRelaxedMath, Float32TypeMathFTZ, );

template <typename T> class NEATDivTest {
public:
  ...
  NEATDivTest() {
    ...
    for (unsigned int i = 0; i < NUM_TESTS * vectorWidth; ++i) {
      ...
      if (Arg2Min[i] > Arg2Max[i])
        std::swap(Arg2Min[i], Arg2Max[i]);
    }
  }

  void TestDiv(NEATFuncP NEATFunc, NEATFuncVecP NEATFuncVec, RefFuncP RefFunc,
               float ulps) {
    // Test corner cases first
    NEATValue unkn = NEATValue(NEATValue::UNKNOWN);
    NEATValue unwr = NEATValue(NEATValue::UNWRITTEN);
    ...
    for (uint32_t testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
      ...
      for (uint32_t i = 0; i < vectorWidth; i++) {
        ...
        // Test for accurate operation (input is accurate values, output -
        // interval)
        ...
        const uint32_t RES_COUNT = 4;
        sT allResults[RES_COUNT] = {sT(RefFunc(Arg1Min[idx], Arg2Min[idx])),
                                    sT(RefFunc(Arg1Max[idx], Arg2Min[idx])),
                                    sT(RefFunc(Arg1Min[idx], Arg2Max[idx])),
                                    sT(RefFunc(Arg1Max[idx], Arg2Max[idx]))};
  ...
  } // void TestDiv(NEATFuncP NEATFunc, NEATFuncVecP NEATFuncVec, RefFuncP
    // RefFunc, float ulps)
};

template <typename T> class NEATDivFrmTest : public NEATDivTest<T> {
public:
  typedef IntervalError<T> (*GetDivErrorFuncP)(const NEATValue &,
                                               const NEATValue &);

  void TestDiv(typename NEATDivFrmTest<T>::NEATFuncP NEATFunc,
               typename NEATDivFrmTest<T>::NEATFuncVecP NEATFuncVec,
               typename NEATDivFrmTest<T>::RefFuncP RefFunc,
               typename NEATDivFrmTest<T>::GetDivErrorFuncP error) {
    // Test corner cases first
    NEATValue unkn = NEATValue(NEATValue::UNKNOWN);
    NEATValue unwr = NEATValue(NEATValue::UNWRITTEN);
    NEATValue any = NEATValue(NEATValue::ANY);
    ...
    for (uint32_t testIdx = 0; testIdx < this->NUM_TESTS; testIdx++) {
      ...
      for (uint32_t i = 0; i < this->vectorWidth; i++) {
        ...
        // NEAT division for two accurate values
        NEATValue accRes = NEATFunc(aAcc, bAcc);
        typename NEATDivFrmTest<T>::sT refRes = typename NEATDivFrmTest<T>::sT(
            RefFunc(this->Arg1Min[idx], this->Arg2Min[idx]));
        EXPECT_TRUE(TestAccExpanded<typename NEATDivFrmTest<T>::sT>(
            refRes, accRes, error(aAcc, bAcc)));

        // Now test intervals as input
        // Calc expected result
        const uint32_t RES_COUNT = 4;
        typename NEATDivFrmTest<T>::sT allResults[RES_COUNT] = {
            typename NEATDivFrmTest<T>::sT(
                RefFunc(this->Arg1Min[idx], this->Arg2Min[idx])),
            typename NEATDivFrmTest<T>::sT(
                RefFunc(this->Arg1Max[idx], this->Arg2Min[idx])),
            typename NEATDivFrmTest<T>::sT(
                RefFunc(this->Arg1Min[idx], this->Arg2Max[idx])),
            typename NEATDivFrmTest<T>::sT(
                RefFunc(this->Arg1Max[idx], this->Arg2Max[idx]))};
        ...
        bool passed = false;
        if (res.GetStatus() != NEATValue::UNKNOWN &&
            res.GetStatus() != NEATValue::ANY)
          passed = TestIntExpanded<typename NEATDivFrmTest<T>::sT>(
              minRef[i], maxRef[i], res, error(a, b));
        else
          // result unknown.
          passed = (!(a.GetStatus() == NEATValue::ACCURATE &&
                      *a.GetAcc<float>() == float(1.0f) &&
      ...
      for (uint32_t i = 0; i < this->vectorWidth; i++) {
        bool passed;
        if (res[i].GetStatus() != NEATValue::UNKNOWN &&
            res[i].GetStatus() != NEATValue::ANY)
          passed = TestIntExpanded<typename NEATDivFrmTest<T>::sT>(
              minRef[i], maxRef[i], res[i], error(a[i], b[i]));
        else {
          passed = !(b[i].IsInterval() && a[i].IsAcc() &&
                     (*a[i].GetAcc<float>() == float(1.0f)) &&
                     interval1.Includes((double)*b[i].GetMin<float>()) &&
                     interval1.Includes((double)*b[i].GetMax<float>()));
          passed &= !(a[i].IsInterval() && b[i].IsInterval() &&
                      interval2.Includes((double)*a[i].GetMin<float>()) &&
                      interval2.Includes((double)*a[i].GetMax<float>()) &&
                      interval2.Includes((double)*b[i].GetMin<float>()) &&
                      interval2.Includes((double)*b[i].GetMax<float>()));
        }
...
template <typename T> class NEATDivTestRun : public ALUTest {};
template <typename T> class NEATDivFrmTestRun : public ALUTest {};

TYPED_TEST_SUITE(NEATDivTestRun, FloatTypesMathFTZ, );
TYPED_TEST(NEATDivTestRun, div) {
  RefALU::SetFTZmode(
      TypeParam::mode); // we use ValueTypeContainer type here, T::mode is FTZ
...
TYPED_TEST(NEATDivFrmTestRun, div_frm) {
  ...
  typedef NEATValue (*NEATFuncP)(const NEATValue &, const NEATValue &);
  typedef NEATVector (*NEATFuncVecP)(const NEATVector &, const NEATVector &);
  typedef IntervalError<TypeP> (*GetDivErrorFuncP)(const NEATValue &,
                                                   const NEATValue &);
  // we use the same precision reference for div
  typedef TypeP (*RefFuncP)(const TypeP &, const TypeP &);
...
TYPED_TEST(NEATDivTestRun, half_divide) {
  ...
  divTest.TestDiv(NEATFunc, NEATFuncVec, RefFunc,
                  float(NEATALU::HALF_DIVIDE_ERROR));
}
...
TYPED_TEST(NEATAluTypedMath, ExpandFPInterval) {
  ...
  GenerateRangedVectorsAutoSeed(F64, &firstFloatRanged[0], curWidth, NUM_TESTS,
                                -1000.0, 1000.0);

  GenerateRangedVectorsAutoSeed(F64, &secondFloatRanged[0], curWidth, NUM_TESTS,
                                -1000.0, 1000.0);
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
...

      {
        ...
        if (firstFloat[testIdx * wrap.GetSize() + i] <
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
...

      {
        ...
        if (firstFloat[testIdx * wrap.GetSize() + i] <
            secondFloatRanged[testIdx * wrap.GetSize() + i]) {
...
TYPED_TEST(NEATAluTypedMath, sin) {
  ...
  GenerateRandomVectorsAutoSeed(this->dataTypeVal, &firstFloat[0], curWidth,
                                NUM_TESTS);
  GenerateRandomVectorsAutoSeed(this->dataTypeVal, &secondFloat[0], curWidth,
                                NUM_TESTS);

  GenerateRangedVectorsAutoSeed(
      this->dataTypeVal, &firstFloatRanged[0], curWidth, NUM_TESTS,
      TypeP(-NEATALU::pi_2 + 0.01), TypeP(NEATALU::pi_2 - 0.01));

  // +- 0.01 to avoid including PI/2 to interval max amd min points are tested
  // on not ranged vectors (firstFloat, secondFloat)
  GenerateRangedVectorsAutoSeed(
      this->dataTypeVal, &secondFloatRanged[0], curWidth, NUM_TESTS,
      TypeP(-NEATALU::pi_2 + 0.01), TypeP(NEATALU::pi_2 - 0.01));
  ...
  testAccVal = NEATALU::sin<TypeP>(
      NEATValue(Utils::GetPInf<TypeP>(), Utils::GetPInf<TypeP>()));
  EXPECT_TRUE(testAccVal.IsNaN<TypeP>());
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      accurateRanged[i] =
          NEATValue(firstFloatRanged[testIdx * wrap.GetSize() + i]);
      if (firstFloatRanged[testIdx * wrap.GetSize() + i] <
          secondFloatRanged[testIdx * wrap.GetSize() + i])
        intervalRanged[i] =
            NEATValue(firstFloatRanged[testIdx * wrap.GetSize() + i],
                      secondFloatRanged[testIdx * wrap.GetSize() + i]);
        ...
        intervalRanged[i] =
            NEATValue(secondFloatRanged[testIdx * wrap.GetSize() + i],
                      firstFloatRanged[testIdx * wrap.GetSize() + i]);
    }
...

    {
      ...
      if (Utils::ge(diff, (TypeP)NEATALU::two_pi)) {
        ...
        TypeP offStart = RefALU::mul<TypeP>(
            RefALU::frc<TypeP>(RefALU::div<TypeP>(*intVal.GetMin<TypeP>(),
                                                  (TypeP)NEATALU::two_pi)),
            (TypeP)NEATALU::two_pi);
      ...
      TypeP dMin = NEATALU::castDown(
          refIntValMin - error.getExpandValue(ComputeUlp(refIntValMin), true));
      TypeP dMax = NEATALU::castDown(
          refIntValMax + error.getExpandValue(ComputeUlp(refIntValMax), false));

      if (hasMinPoint) {
        if (*testIntVal.GetMin<TypeP>() != TypeP(-1.0))
          passed = false;
      } else if (!(Utils::IsDenorm<TypeP>(dMin)) &&
                 *testIntVal.GetMin<TypeP>() == 0)
        if (fabs(Utils::ulpsDiff(refIntValMin, *testIntVal.GetMin<TypeP>())) >
            NEATALU::SIN_ERROR)
          passed = false;
      ...
      if (hasMaxPoint) {
        if (*testIntVal.GetMax<TypeP>() != TypeP(1.0))
          passed = false;
      } else if (!(Utils::IsDenorm<TypeP>(dMax)) &&
                 *testIntVal.GetMax<TypeP>() == 0)
        if (fabs(Utils::ulpsDiff(refIntValMax, *testIntVal.GetMax<TypeP>())) >
            NEATALU::SIN_ERROR)
          passed = false;
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      if (Utils::ge(diff, (TypeP)NEATALU::two_pi)) {
        ...
        TypeP offStart = RefALU::mul<TypeP>(
            RefALU::frc<TypeP>(RefALU::div<TypeP>(*interval[i].GetMin<TypeP>(),
                                                  (TypeP)NEATALU::two_pi)),
            (TypeP)NEATALU::two_pi);
      ...
      if (hasMinPoint) {
        if (*testIntVec1[i].GetMin<TypeP>() != TypeP(-1.0))
          passed = false;
                 ...
                 *testIntVec1[i].GetMin<TypeP>() == 0)
        if (fabs(Utils::ulpsDiff(refIntValMin,
                                 *testIntVec1[i].GetMin<TypeP>())) >
            NEATALU::SIN_ERROR)
          passed = false;
      ...
      if (hasMaxPoint) {
        if (*testIntVec1[i].GetMax<TypeP>() != TypeP(1.0))
          passed = false;
                 ...
                 *testIntVec1[i].GetMax<TypeP>() == 0)
        if (fabs(Utils::ulpsDiff(refIntValMax,
                                 *testIntVec1[i].GetMax<TypeP>())) >
            NEATALU::SIN_ERROR)
          passed = false;
    }
    EXPECT_TRUE(passed);
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      if (Utils::eq(*accurateRanged[i].GetAcc<TypeP>(), TypeP(+0.0))) {
        ...
        refAccValFloat =
            RefALU::sin(SuperT(*accurateRanged[i].GetAcc<TypeP>()));
        bool passed = TestAccExpanded<SuperT>(refAccValFloat, testAccVec[i],
                                              NEATALU::SIN_ERROR);
        EXPECT_TRUE(passed);
      }
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed = TestIntExpanded<SuperT>(refIntValMin, refIntValMax,
                                            testIntVec[i], NEATALU::SIN_ERROR);
      EXPECT_TRUE(passed);
    }
  }
}
...
TYPED_TEST(NEATAluTypedFastRelaxedMath, sin_frm) {
  ...
  sinErr =
      IntervalError<TypeP>::sinFrmSetUlps(NEATValue(TypeP(+0.0), TypeP(+0.0)));
  EXPECT_TRUE(TestIntExpanded<SuperT>(RefALU::sin(SuperT(+0.0)),
                                      RefALU::sin(SuperT(+0.0)), testAccVal,
                                      sinErr));
  ...
  testAccVal = NEATALU::sin_frm<TypeP>(
      NEATValue(Utils::GetPInf<TypeP>(), Utils::GetPInf<TypeP>()));
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    refAccValFloat = RefALU::sin(SuperT(*accVal.GetAcc<TypeP>()));
    bool passed =
        TestAccExpanded<SuperT>(refAccValFloat, testAccVal,
                                IntervalError<TypeP>::sinFrmSetUlps(accVal));
    EXPECT_TRUE(passed);
...

    {
      ...
      if (!error.isInfiniteError()) {
        if (fabs(refIntValMin - *testIntVal.GetMin<TypeP>()) >
            error.getAbsoluteError())
          passed = false;
        if (fabs(refIntValMax - *testIntVal.GetMax<TypeP>()) >
            error.getAbsoluteError())
          passed = false;
      }
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      IntervalError<TypeP> error =
          IntervalError<TypeP>::sinFrmSetUlps(interval[i]);
      if (!error.isInfiniteError()) {
        if (fabs(refIntValMin - *testIntVec1[i].GetMin<TypeP>()) >
            error.getAbsoluteError())
          passed = false;
        if (fabs(refIntValMax - *testIntVec1[i].GetMax<TypeP>()) >
            error.getAbsoluteError())
          passed = false;
      }
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed = TestAccExpanded<SuperT>(
          refAccValFloat, testAccVec[i],
          IntervalError<TypeP>::sinFrmSetUlps(accurateRanged[i]));
      EXPECT_TRUE(passed);
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed = TestIntExpanded<SuperT>(
          refIntValMin, refIntValMax, testIntVec[i],
          IntervalError<TypeP>::sinFrmSetUlps(intervalRanged[i]));
      EXPECT_TRUE(passed);
    }
  }
}
...
TYPED_TEST(NEATAluTypedMath, native_sin) {
  ...
  testAccVal = NEATALU::native_sin<TypeP>(
      NEATValue(Utils::GetPInf<TypeP>(), Utils::GetPInf<TypeP>()));
  EXPECT_TRUE(testAccVal.IsNaN<TypeP>());
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    if (Utils::eq(*accVal.GetAcc<TypeP>(), TypeP(+0.0))) {
      ...
      bool passed = TestAccExpanded<SuperT>(refAccValFloat, testAccVal,
                                            NEATALU::NATIVE_SIN_ERROR);
      EXPECT_TRUE(passed);
    }
...

    {
      ...
      TypeP dMin = NEATALU::castDown(
          refIntValMin - NEATALU::NATIVE_SIN_ERROR * ComputeUlp(refIntValMin));
      TypeP dMax = NEATALU::castDown(
          refIntValMax + NEATALU::NATIVE_SIN_ERROR * ComputeUlp(refIntValMax));
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed = TestIntExpanded<SuperT>(
          refIntValMin, refIntValMax, testIntVec[i], NEATALU::NATIVE_SIN_ERROR);
      EXPECT_TRUE(passed);
    }
  }
}
...
TYPED_TEST(NEATAluTypedMath, half_sin) {
  ...
  testAccVal = NEATALU::half_sin<TypeP>(
      NEATValue(Utils::GetPInf<TypeP>(), Utils::GetPInf<TypeP>()));
  EXPECT_TRUE(testAccVal.IsNaN<TypeP>());
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
...

    {
      ...
      TypeP dMin = NEATALU::castDown(
          refIntValMin - NEATALU::HALF_SIN_ERROR * ComputeUlp(refIntValMin));
      TypeP dMax = NEATALU::castDown(
          refIntValMax + NEATALU::HALF_SIN_ERROR * ComputeUlp(refIntValMax));
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed = TestIntExpanded<SuperT>(
          refIntValMin, refIntValMax, testIntVec[i], NEATALU::HALF_SIN_ERROR);
      EXPECT_TRUE(passed);
    }
  }
}
...
TYPED_TEST(NEATAluTypedMath, cos) {
  ...
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &firstFloatRanged[0],
                                curWidth, NUM_TESTS, TypeP(-NEATALU::pi + 0.01),
                                TypeP(0 - 0.01));
  // +- 0.01 to avoid including PI to interval max amd min points are tested
  // on not ranged vectors (firstFloat, secondFloat)
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &secondFloatRanged[0],
                                curWidth, NUM_TESTS, TypeP(-NEATALU::pi + 0.01),
                                TypeP(0 - 0.01));
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
...

    {
      ...
      TypeP dMin = NEATALU::castDown(
          refIntValMin - NEATALU::COS_ERROR * ComputeUlp(refIntValMin));
      TypeP dMax = NEATALU::castDown(
          refIntValMax + NEATALU::COS_ERROR * ComputeUlp(refIntValMax));
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      if ((Utils::eq(*accurateRanged[i].GetAcc<TypeP>(), TypeP(+0.0))) ||
        ...
        refAccValFloat =
            RefALU::cos(SuperT(*accurateRanged[i].GetAcc<TypeP>()));
        bool passed = TestAccExpanded<SuperT>(refAccValFloat, testAccVec[i],
                                              NEATALU::COS_ERROR);
        EXPECT_TRUE(passed);
      }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed = TestIntExpanded<SuperT>(refIntValMin, refIntValMax,
                                            testIntVec[i], NEATALU::COS_ERROR);
      EXPECT_TRUE(passed);
    }
  }
}
...
TYPED_TEST(NEATAluTypedFastRelaxedMath, cos_frm) {
  ...
  cosErr =
      IntervalError<TypeP>::cosFrmSetUlps(NEATValue(TypeP(+0.0), TypeP(+0.0)));
  EXPECT_TRUE(TestIntExpanded<SuperT>(RefALU::cos(SuperT(+0.0)),
                                      RefALU::cos(SuperT(+0.0)), testAccVal,
                                      cosErr));
  ...
  testAccVal = NEATALU::cos_frm<TypeP>(
      NEATValue(Utils::GetPInf<TypeP>(), Utils::GetPInf<TypeP>()));
  EXPECT_TRUE(testAccVal.IsAny());
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    bool passed =
        TestAccExpanded<SuperT>(refAccValFloat, testAccVal,
                                IntervalError<TypeP>::cosFrmSetUlps(accVal));
    EXPECT_TRUE(passed);
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      IntervalError<TypeP> error =
          IntervalError<TypeP>::cosFrmSetUlps(interval[i]);
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed = TestAccExpanded<SuperT>(
          refAccValFloat, testAccVec[i],
          IntervalError<TypeP>::cosFrmSetUlps(accurateRanged[i]));
      EXPECT_TRUE(passed);
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed = TestIntExpanded<SuperT>(
          refIntValMin, refIntValMax, testIntVec[i],
          IntervalError<TypeP>::cosFrmSetUlps(intervalRanged[i]));
      EXPECT_TRUE(passed);
    }
  }
}
...
TYPED_TEST(NEATAluTypedMath, native_cos) {
  ...
  testAccVal = NEATALU::native_cos<TypeP>(
      NEATValue(Utils::GetPInf<TypeP>(), Utils::GetPInf<TypeP>()));
  EXPECT_TRUE(testAccVal.IsNaN<TypeP>());
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
...

    {
      ...
      TypeP dMin = NEATALU::castDown(
          refIntValMin - NEATALU::NATIVE_COS_ERROR * ComputeUlp(refIntValMin));
      TypeP dMax = NEATALU::castDown(
          refIntValMax + NEATALU::NATIVE_COS_ERROR * ComputeUlp(refIntValMax));
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      refIntValMax = RefALU::cos(SuperT(*intervalRanged[i].GetMax<TypeP>()));
      bool passed = TestIntExpanded<SuperT>(
          refIntValMin, refIntValMax, testIntVec[i], NEATALU::NATIVE_COS_ERROR);
      EXPECT_TRUE(passed);
    }
  }
}
...
TYPED_TEST(NEATAluTypedMath, half_cos) {
  ...
  testAccVal = NEATALU::half_cos<TypeP>(
      NEATValue(Utils::GetPInf<TypeP>(), Utils::GetPInf<TypeP>()));
  EXPECT_TRUE(testAccVal.IsNaN<TypeP>());
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
...

    {
      ...
      TypeP dMin = NEATALU::castDown(
          refIntValMin - NEATALU::HALF_COS_ERROR * ComputeUlp(refIntValMin));
      TypeP dMax = NEATALU::castDown(
          refIntValMax + NEATALU::HALF_COS_ERROR * ComputeUlp(refIntValMax));
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      refIntValMax = RefALU::cos(SuperT(*intervalRanged[i].GetMax<TypeP>()));
      bool passed = TestIntExpanded<SuperT>(
          refIntValMin, refIntValMax, testIntVec[i], NEATALU::HALF_COS_ERROR);
      EXPECT_TRUE(passed);
    }
  }
}
...
TYPED_TEST(NEATAluTypedMath, tan) {
  ...
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &firstFloatRanged[0],
                                curWidth, NUM_TESTS, TypeP(-NEATALU::pi_2),
                                TypeP(NEATALU::pi_2));
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &secondFloatRanged[0],
                                curWidth, NUM_TESTS, TypeP(-NEATALU::pi_2),
                                TypeP(NEATALU::pi_2));
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      if (Utils::eq(*accurate[i].GetAcc<TypeP>(), TypeP(+0.0))) {
        ...
        passed = TestAccExpanded<SuperT>(refAccValFloat, testAccVec[i],
                                         NEATALU::TAN_ERROR);
        EXPECT_TRUE(passed);
      }
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed = TestIntExpanded<SuperT>(refIntValMin, refIntValMax,
                                            testIntVec[i], NEATALU::TAN_ERROR);
      EXPECT_TRUE(passed);
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      if (Utils::eq(*accurateRanged[i].GetAcc<TypeP>(), TypeP(+0.0))) {
        ...
        refAccValFloat =
            RefALU::tan(SuperT(*accurateRanged[i].GetAcc<TypeP>()));
        bool passed = TestAccExpanded<SuperT>(refAccValFloat, testAccVec[i],
                                              NEATALU::TAN_ERROR);
        EXPECT_TRUE(passed);
      }
...
TYPED_TEST(NEATAluTypedMath, half_tan) {
  ...
  testAccVal = NEATALU::half_tan<TypeP>(
      NEATValue(Utils::GetPInf<TypeP>(), Utils::GetPInf<TypeP>()));
  EXPECT_TRUE(testAccVal.IsNaN<TypeP>());
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    bool passed = TestIntExpanded<SuperT>(refIntValMin, refIntValMax,
                                          testIntVal, NEATALU::HALF_TAN_ERROR);
    EXPECT_TRUE(passed);
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed = TestIntExpanded<SuperT>(
          refIntValMin, refIntValMax, testIntVec[i], NEATALU::HALF_TAN_ERROR);
      EXPECT_TRUE(passed);
    }
...
TYPED_TEST(NEATAluTypedMath, native_tan) {
  ...
  testAccVal = NEATALU::native_tan<TypeP>(
      NEATValue(Utils::GetPInf<TypeP>(), Utils::GetPInf<TypeP>()));
  EXPECT_TRUE(testAccVal.IsNaN<TypeP>());
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    bool passed = TestIntExpanded<SuperT>(
        refIntValMin, refIntValMax, testIntVal, NEATALU::NATIVE_TAN_ERROR);
    EXPECT_TRUE(passed);
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed = TestIntExpanded<SuperT>(
          refIntValMin, refIntValMax, testIntVec[i], NEATALU::NATIVE_TAN_ERROR);
      EXPECT_TRUE(passed);
    }
...
TYPED_TEST(NEATAluTypedMath, tanpi) {
  ...
  EXPECT_TRUE(testAccVal.IsAcc() &&
              Utils::eq<TypeP>(*testAccVal.GetAcc<TypeP>(),
                               RefALU::copysign(TypeP(0.0), evenTestVal)));
  TypeP oddTestVal = 15647321;
  testAccVal = NEATALU::tanpi<TypeP>(NEATValue(oddTestVal));
  EXPECT_TRUE(testAccVal.IsAcc() &&
              Utils::eq<TypeP>(*testAccVal.GetAcc<TypeP>(),
                               RefALU::copysign(TypeP(0.0), -oddTestVal)));
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    if (Utils::IsEven(acc)) {
      EXPECT_TRUE(Utils::eq<TypeP>(*testAccVal.GetAcc<TypeP>(),
                                   RefALU::copysign(TypeP(0.0), acc)));
      ...
      passed = TestAccExpanded<SuperT>(refAccValFloat, testAccVal,
                                       NEATALU::TANPI_ERROR);
      EXPECT_TRUE(passed);
    }
    ...
    if (Utils::le(intValMin, TypeP(-0.5)) || Utils::ge(intValMax, TypeP(0.5)))
      ...
      passed = TestIntExpanded<SuperT>(refIntValMin, refIntValMax, testIntVal,
                                       NEATALU::TANPI_ERROR);
      EXPECT_TRUE(passed);
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      if (Utils::IsEven(acc)) {
        EXPECT_TRUE(Utils::eq<TypeP>(*testAccVec[i].GetAcc<TypeP>(),
                                     RefALU::copysign(TypeP(0.0), acc)));
      } else if (Utils::IsOdd(acc)) {
        EXPECT_TRUE(Utils::eq<TypeP>(*testAccVec[i].GetAcc<TypeP>(),
                                     RefALU::copysign(TypeP(0.0), -acc)));
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      if (Utils::le(intValMin, TypeP(-0.5)) || Utils::ge(intValMax, TypeP(0.5)))
        ...
        bool passed = TestIntExpanded<SuperT>(
            refIntValMin, refIntValMax, testIntVec[i], NEATALU::TANPI_ERROR);
        EXPECT_TRUE(passed);
      }
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      refAccValFloat =
          RefALU::tanpi(SuperT(*accurateRanged[i].GetAcc<TypeP>()));
      passed = TestAccExpanded<SuperT>(refAccValFloat, testAccVec[i],
                                       NEATALU::TANPI_ERROR);
      EXPECT_TRUE(passed);
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      if (Utils::le(intValMin, TypeP(-0.5)) || Utils::ge(intValMax, TypeP(0.5)))
        ...
        refIntValMin =
            RefALU::tanpi(SuperT(*intervalRanged[i].GetMin<TypeP>()));
        refIntValMax =
            RefALU::tanpi(SuperT(*intervalRanged[i].GetMax<TypeP>()));
...
TYPED_TEST(NEATAluTypedMath, asin) {
  ...
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &firstFloatRanged[0],
                                curWidth, NUM_TESTS, TypeP(-1.0 + 0.001),
                                TypeP(1.0 - 0.001));
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &secondFloatRanged[0],
                                curWidth, NUM_TESTS, TypeP(-1.0 + 0.001),
                                TypeP(1.0 - 0.001));
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    if ((*accVal.GetAcc<TypeP>() > TypeP(1.0)) ||
      ...
      EXPECT_TRUE(
          TestAccValue<TypeP>(testAccVal, TypeP(+0.0))); // asin(+0) == +0
    } else if (Utils::eq(*accVal.GetAcc<TypeP>(), TypeP(-0.0))) {
      EXPECT_TRUE(
          TestAccValue<TypeP>(testAccVal, TypeP(-0.0))); // asin(-0) == -0
    } else {
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      if ((*accurate[i].GetAcc<TypeP>() > TypeP(1.0)) ||
        ...
        EXPECT_TRUE(
            TestAccValue<TypeP>(testAccVec[i], TypeP(+0.0))); // asin(+0) == +0
      } else if (Utils::eq(*accurate[i].GetAcc<TypeP>(), TypeP(-0.0))) {
        EXPECT_TRUE(
            TestAccValue<TypeP>(testAccVec[i], TypeP(-0.0))); // asin(-0) == -0
      } else {
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      if ((*interval[i].GetMax<TypeP>() < TypeP(-1.0)) ||
        ...
        bool passed = TestIntExpanded<SuperT>(
            refIntValMin, refIntValMax, testIntVec[i], NEATALU::ASIN_ERROR);
        EXPECT_TRUE(passed);
      }
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      if (Utils::eq(*accurateRanged[i].GetAcc<TypeP>(), TypeP(+0.0))) {
        ...
        refAccValFloat =
            RefALU::asin(SuperT(*accurateRanged[i].GetAcc<TypeP>()));
        bool passed = TestAccExpanded<SuperT>(refAccValFloat, testAccVec[i],
                                              NEATALU::ASIN_ERROR);
        EXPECT_TRUE(passed);
      }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed = TestIntExpanded<SuperT>(refIntValMin, refIntValMax,
                                            testIntVec[i], NEATALU::ASIN_ERROR);
      EXPECT_TRUE(passed);
    }
  }
}
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      if ((*interval[i].GetMax<TypeP>() < TypeP(-1.0)) ||
        ...
        bool passed = TestIntExpanded<SuperT>(
            refIntValMin, refIntValMax, testIntVec[i], NEATALU::ACOS_ERROR);
        EXPECT_TRUE(passed);
      }
    }

    /* test for vector of NEAT accurate in range */
    // accurateRanged[i]==1 is not tested, because generated interval not
    // includes it
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed = TestIntExpanded<SuperT>(refIntValMin, refIntValMax,
                                            testIntVec[i], NEATALU::ACOS_ERROR);
      EXPECT_TRUE(passed);
    }
  }
}
...
TYPED_TEST(NEATAluTypedMath, atan) {
  ...
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &firstFloatRanged[0],
                                curWidth, NUM_TESTS, TypeP(-100.0),
                                TypeP(100.0));
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &secondFloatRanged[0],
                                curWidth, NUM_TESTS, TypeP(-100.0),
                                TypeP(100.0));
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    if (Utils::eq(*accVal.GetAcc<TypeP>(), TypeP(+0.0))) {
      ...
      EXPECT_TRUE(TestAccExpanded<SuperT>(refAccValFloat, testAccVal,
                                          NEATALU::ATAN_ERROR));
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed = TestIntExpanded<SuperT>(refIntValMin, refIntValMax,
                                            testIntVec[i], NEATALU::ATAN_ERROR);
      EXPECT_TRUE(passed);
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      if (Utils::eq(*accurateRanged[i].GetAcc<TypeP>(), TypeP(+0.0))) {
        ...
        refAccValFloat =
            RefALU::atan(SuperT(*accurateRanged[i].GetAcc<TypeP>()));
        bool passed = TestAccExpanded<SuperT>(refAccValFloat, testAccVec[i],
                                              NEATALU::ATAN_ERROR);
        EXPECT_TRUE(passed);
      }
...
TYPED_TEST(NEATAluTypedMath, sinh) {
  ...
  // result of sinh(x) with |x| > 10 is close to INF, so test for ranged
  // argument is enough
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed = TestIntExpanded<SuperT>(refIntValMin, refIntValMax,
                                            testIntVec[i], NEATALU::SINH_ERROR);
      EXPECT_TRUE(passed);
    }
  }
}
...
TYPED_TEST(NEATAluTypedMath, cosh) {
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed = TestIntExpanded<SuperT>(refIntValMin, refIntValMax,
                                            testIntVec[i], NEATALU::COSH_ERROR);
      EXPECT_TRUE(passed);
    }
  }
}
...
TYPED_TEST(NEATAluTypedMath, tanh) {
  ...
  // we use data range from -7 to 7 because with greater data tanh result
  // reaches its limit = 1.0 (or -1.0) (at least for float data)
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed = TestIntExpanded<SuperT>(refIntValMin, refIntValMax,
                                            testIntVec[i], NEATALU::TANH_ERROR);
      EXPECT_TRUE(passed);
    }
  }
}
...
TYPED_TEST(NEATAluTypedMath, sincos) {
  ...
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &firstFloatRanged[0],
                                curWidth, NUM_TESTS, TypeP(0.1),
                                TypeP(NEATALU::pi_2 - 0.1));
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &secondFloatRanged[0],
                                curWidth, NUM_TESTS, TypeP(0.1),
                                TypeP(NEATALU::pi_2 - 0.1));
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    bool passed = TestAccExpanded<SuperT>(refAccValFloatSin, testAccValSin,
                                          NEATALU::SIN_ERROR);
    EXPECT_TRUE(passed);

    passed = TestAccExpanded<SuperT>(refAccValFloatCos, testAccValCos,
                                     NEATALU::COS_ERROR);
    EXPECT_TRUE(passed);
    ...
    passed = TestIntExpanded<SuperT>(refIntValSinMin, refIntValSinMax,
                                     testIntValSin, NEATALU::SIN_ERROR);
    EXPECT_TRUE(passed);

    passed = TestIntExpanded<SuperT>(refIntValCosMin, refIntValCosMax,
                                     testIntValCos, NEATALU::COS_ERROR);
    EXPECT_TRUE(passed);

    /* test for vector of NEAT accurate */
    NEATVector testAccVecCos = accurateRanged;
    NEATVector testAccVecSin =
        NEATALU::sincos<TypeP>(accurateRanged, testAccVecCos);
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      refAccValFloatSin =
          RefALU::sin(SuperT(*accurateRanged[i].GetAcc<TypeP>()));
      refAccValFloatCos =
      ...
      bool passed = TestAccExpanded<SuperT>(refAccValFloatSin, testAccVecSin[i],
                                            NEATALU::SIN_ERROR);
      EXPECT_TRUE(passed);

      passed = TestAccExpanded<SuperT>(refAccValFloatCos, testAccVecCos[i],
                                       NEATALU::COS_ERROR);
      EXPECT_TRUE(passed);
    }
    ...
    NEATVector testIntVecSin =
        NEATALU::sincos<TypeP>(intervalRanged, testIntVecCos);
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed =
          TestIntExpanded<SuperT>(refIntValSinMin, refIntValSinMax,
                                  testIntVecSin[i], NEATALU::SIN_ERROR);
      EXPECT_TRUE(passed);

      passed = TestIntExpanded<SuperT>(refIntValCosMin, refIntValCosMax,
                                       testIntVecCos[i], NEATALU::COS_ERROR);
      EXPECT_TRUE(passed);
    }
  }
}
...
TYPED_TEST(NEATAluTypedMath, sinpi) {
  ...
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &firstFloatRanged[0],
                                curWidth, NUM_TESTS, TypeP(-0.49), TypeP(0.49));
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    /* load data for monotonic interval from -pi/2 to pi/2,i.e. from -1/2 to 1/2
     * for sinpi */
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      accurateRanged[i] =
          NEATValue(firstFloatRanged[testIdx * wrap.GetSize() + i]);
...

    {
      ...
      if (Utils::ge(diff, (TypeP)2.0)) {
        ...
        TypeP offStart =
            RefALU::mul<TypeP>(RefALU::frc<TypeP>(RefALU::div<TypeP>(
                                   *intVal.GetMin<TypeP>(), (TypeP)2.0)),
                               (TypeP)2.0);
      ...
      if (hasMinPoint) {
        if (*testIntVal.GetMin<TypeP>() != TypeP(-1.0))
          passed = false;
      } else if (fabs(Utils::ulpsDiff(refIntValMin,
                                      *testIntVal.GetMin<TypeP>())) >
                 NEATALU::SINPI_ERROR)
        passed = false;
      ...
      if (hasMaxPoint) {
        if (*testIntVal.GetMax<TypeP>() != TypeP(1.0))
          passed = false;
      } else if (fabs(Utils::ulpsDiff(refIntValMax,
                                      *testIntVal.GetMax<TypeP>())) >
                 NEATALU::SINPI_ERROR)
        passed = false;
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      if (Utils::ge(diff, (TypeP)2.0)) {
        ...
        TypeP offStart =
            RefALU::mul<TypeP>(RefALU::frc<TypeP>(RefALU::div<TypeP>(
                                   *interval[i].GetMin<TypeP>(), (TypeP)2.0)),
                               (TypeP)2.0);
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      if (Utils::eq<TypeP>(*accurateRanged[i].GetAcc<TypeP>(), -0.0f) ||
        ...
        refAccValFloat =
            RefALU::sinpi(SuperT(*accurateRanged[i].GetAcc<TypeP>()));
        passed = TestAccExpanded<SuperT>(refAccValFloat, testAccVec[i],
                                         NEATALU::SINPI_ERROR);
        EXPECT_TRUE(passed);
      }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed = TestIntExpanded<SuperT>(
          refIntValMin, refIntValMax, testIntVec[i], NEATALU::SINPI_ERROR);
      EXPECT_TRUE(passed);
    }
  }
}
...
TYPED_TEST(NEATAluTypedMath, cospi) {
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    if (data == 0)
      ...
      if (fractpart == TypeP(0.5))
        ...
        if ((SuperT)data >= limit) {
          ...
          // bool passed =
          // TestAccExpanded<SuperT>(SuperT(1.0),testAccVal,NEATALU::COSPI_ERROR);
          // EXPECT_TRUE(passed);
        } else {
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      if (data == 0)
        ...
        if (fractpart == TypeP(0.5))
          ...
          refAccValFloat =
              RefALU::cospi(SuperT(*accurateRanged[i].GetAcc<TypeP>()));
          bool passed = TestAccExpanded<SuperT>(refAccValFloat, testAccVec[i],
                                                NEATALU::COSPI_ERROR);
          EXPECT_TRUE(passed);
        }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed = TestIntExpanded<SuperT>(
          refIntValMin, refIntValMax, testIntVec[i], NEATALU::COSPI_ERROR);
      EXPECT_TRUE(passed);
    }
  }
}
...
TYPED_TEST(NEATAluTypedMath, asinpi) {
  ...
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &firstFloatRanged[0],
                                curWidth, NUM_TESTS, TypeP(-0.999),
                                TypeP(0.999));
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &secondFloatRanged[0],
                                curWidth, NUM_TESTS, TypeP(-0.999),
                                TypeP(0.999));
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    if (*intVal.GetMin<TypeP>() < TypeP(-1.0) ||
      ...
      bool passed = TestIntExpanded<SuperT>(refIntValMin, refIntValMax,
                                            testIntVal, NEATALU::ASINPI_ERROR);
      EXPECT_TRUE(passed);
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      if ((*interval[i].GetMax<TypeP>() > TypeP(1.0)) ||
        ...
        bool passed = TestIntExpanded<SuperT>(
            refIntValMin, refIntValMax, testIntVec[i], NEATALU::ASINPI_ERROR);
        EXPECT_TRUE(passed);
      }
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      if (Utils::eq(*accurateRanged[i].GetAcc<TypeP>(), TypeP(+0.0))) {
        ...
        refAccValFloat =
            RefALU::asinpi(SuperT(*accurateRanged[i].GetAcc<TypeP>()));
        bool passed = TestAccExpanded<SuperT>(refAccValFloat, testAccVec[i],
                                              NEATALU::ASINPI_ERROR);
        EXPECT_TRUE(passed);
      }
...
TYPED_TEST(NEATAluTypedMath, acospi) {
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    if (*intVal.GetMin<TypeP>() < TypeP(-1.0) ||
      ...
      bool passed = TestIntExpanded<SuperT>(refIntValMin, refIntValMax,
                                            testIntVal, NEATALU::ACOSPI_ERROR);
      EXPECT_TRUE(passed);
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      if ((*interval[i].GetMax<TypeP>() > TypeP(1.0)) ||
        ...
        bool passed = TestIntExpanded<SuperT>(
            refIntValMin, refIntValMax, testIntVec[i], NEATALU::ACOSPI_ERROR);
        EXPECT_TRUE(passed);
      }
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      if ((*accurateRanged[i].GetAcc<TypeP>() > TypeP(1.0)) ||
        ...
        /* if random src < -1.0 or src > 1.0 acospi returns NaN */
        EXPECT_TRUE(testAccVec[i].IsNaN<TypeP>());
      } else if (*accurateRanged[i].GetAcc<TypeP>() ==
                 TypeP(1.0)) // special accurate value
      {
        EXPECT_TRUE(TestAccValue<TypeP>(testAccVec[i], TypeP(0.0)));
      } else {
        refAccValFloat =
            RefALU::acospi(SuperT(*accurateRanged[i].GetAcc<TypeP>()));
        bool passed = TestAccExpanded<SuperT>(refAccValFloat, testAccVec[i],
                                              NEATALU::ACOSPI_ERROR);
        EXPECT_TRUE(passed);
      }
...
TYPED_TEST(NEATAluTypedMath, atanpi) {
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    if ((fabs(refIntValMin) != (SuperT)0.5) &&
      ...
      bool passed = TestIntExpanded<SuperT>(refIntValMin, refIntValMax,
                                            testIntVal, NEATALU::ATANPI_ERROR);
      EXPECT_TRUE(passed);
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      if ((fabs(refIntValMin) != (SuperT)0.5) &&
          (fabs(refIntValMax) != (SuperT)0.5)) {
        bool passed = TestIntExpanded<SuperT>(
            refIntValMin, refIntValMax, testIntVec[i], NEATALU::ATANPI_ERROR);
        EXPECT_TRUE(passed);
      }
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      if (Utils::eq(*accurateRanged[i].GetAcc<TypeP>(), TypeP(+0.0))) {
        ...
        refAccValFloat =
            RefALU::atanpi(SuperT(*accurateRanged[i].GetAcc<TypeP>()));
        bool passed = TestAccExpanded<SuperT>(refAccValFloat, testAccVec[i],
                                              NEATALU::ATANPI_ERROR);
        EXPECT_TRUE(passed);
      }
...
TYPED_TEST(NEATAluTypedMath, atan2) {
  ...
  GenerateRandomVectorsAutoSeed(this->dataTypeVal, &firstFloatX[0], curWidth,
                                NUM_TESTS);
  GenerateRandomVectorsAutoSeed(this->dataTypeVal, &secondFloatX[0], curWidth,
                                NUM_TESTS);

  /* just to reduce the interval from the full range of current data type */
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &firstFloatRangedX[0],
                                curWidth, NUM_TESTS, TypeP(-100.0),
                                TypeP(100.0));
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &secondFloatRangedX[0],
                                curWidth, NUM_TESTS, TypeP(-100.0),
                                TypeP(100.0));

  GenerateRandomVectorsAutoSeed(this->dataTypeVal, &firstFloatY[0], curWidth,
                                NUM_TESTS);
  GenerateRandomVectorsAutoSeed(this->dataTypeVal, &secondFloatY[0], curWidth,
                                NUM_TESTS);
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &firstFloatRangedY[0],
                                curWidth, NUM_TESTS, TypeP(-100.0),
                                TypeP(100.0));
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &secondFloatRangedY[0],
                                curWidth, NUM_TESTS, TypeP(-100.0),
                                TypeP(100.0));
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      if (firstFloatX[testIdx * wrap.GetSize() + i] <
          secondFloatX[testIdx * wrap.GetSize() + i])
        intervalX[i] = NEATValue(firstFloatX[testIdx * wrap.GetSize() + i],
                                 secondFloatX[testIdx * wrap.GetSize() + i]);
      else
        intervalX[i] = NEATValue(secondFloatX[testIdx * wrap.GetSize() + i],
                                 firstFloatX[testIdx * wrap.GetSize() + i]);
    }

    /* load data for interval from -100 to 100 */
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      accurateRangedX[i] =
          NEATValue(firstFloatRangedX[testIdx * wrap.GetSize() + i]);
      if (firstFloatRangedX[testIdx * wrap.GetSize() + i] <
          secondFloatRangedX[testIdx * wrap.GetSize() + i])
        intervalRangedX[i] =
            NEATValue(firstFloatRangedX[testIdx * wrap.GetSize() + i],
                      secondFloatRangedX[testIdx * wrap.GetSize() + i]);
      else
        intervalRangedX[i] =
            NEATValue(secondFloatRangedX[testIdx * wrap.GetSize() + i],
                      firstFloatRangedX[testIdx * wrap.GetSize() + i]);
    }

    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      accurateY[i] = NEATValue(firstFloatY[testIdx * wrap.GetSize() + i]);
      if (firstFloatY[testIdx * wrap.GetSize() + i] <
          secondFloatY[testIdx * wrap.GetSize() + i])
        intervalY[i] = NEATValue(firstFloatY[testIdx * wrap.GetSize() + i],
                                 secondFloatY[testIdx * wrap.GetSize() + i]);
      else
        intervalY[i] = NEATValue(secondFloatY[testIdx * wrap.GetSize() + i],
                                 firstFloatY[testIdx * wrap.GetSize() + i]);
    }

    /* load data for interval from -100 to 100 */
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      accurateRangedY[i] =
          NEATValue(firstFloatRangedY[testIdx * wrap.GetSize() + i]);
      if (firstFloatRangedY[testIdx * wrap.GetSize() + i] <
          secondFloatRangedY[testIdx * wrap.GetSize() + i])
        intervalRangedY[i] =
            NEATValue(firstFloatRangedY[testIdx * wrap.GetSize() + i],
                      secondFloatRangedY[testIdx * wrap.GetSize() + i]);
      else
        intervalRangedY[i] =
            NEATValue(secondFloatRangedY[testIdx * wrap.GetSize() + i],
                      firstFloatRangedY[testIdx * wrap.GetSize() + i]);
    }
    ...
    if (Utils::eq<TypeP>(*intValX.GetMin<TypeP>(), TypeP(0.0)) ||
      ...
      bool passed = TestIntExpanded<SuperT>(refIntValMin, refIntValMax,
                                            testIntVal, NEATALU::ATAN2_ERROR);
      EXPECT_TRUE(passed);
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      if (Utils::eq<TypeP>(*intervalX[i].GetMin<TypeP>(), TypeP(0.0)) ||
        ...
        bool passed = TestIntExpanded<SuperT>(
            refIntValMin, refIntValMax, testIntVec[i], NEATALU::ATAN2_ERROR);
        EXPECT_TRUE(passed);
      }
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      if (Utils::eq<TypeP>(*accurateRangedX[i].GetAcc<TypeP>(), TypeP(0.0))) {
        ...
        refAccValFloat =
            RefALU::atan2(SuperT(*accurateRangedY[i].GetAcc<TypeP>()),
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      if (Utils::eq<TypeP>(*intervalRangedX[i].GetMin<TypeP>(), TypeP(0.0)) ||
        ...
        refIntValMin =
            RefALU::atan2(SuperT(*intervalRangedY[i].GetMin<TypeP>()),
                          SuperT(*intervalRangedX[i].GetMin<TypeP>()));
        refIntValMax =
            RefALU::atan2(SuperT(*intervalRangedY[i].GetMax<TypeP>()),
...
TYPED_TEST(NEATAluTypedMath, atan2pi) {
  ...
  testAccVal =
      NEATALU::atan2pi<TypeP>(NEATValue(NEATValue::UNWRITTEN), accValX);
  EXPECT_TRUE(testAccVal.IsUnknown());
  testAccVal =
      NEATALU::atan2pi<TypeP>(accValY, NEATValue(NEATValue::UNWRITTEN));
  EXPECT_TRUE(testAccVal.IsUnknown());
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    if (Utils::eq<TypeP>(*intValX.GetMin<TypeP>(), TypeP(0.0)) ||
      ...
      refIntValMin = RefALU::atan2pi(SuperT(*intValY.GetMin<TypeP>()),
                                     SuperT(*intValX.GetMin<TypeP>()));
      bool passed = TestIntExpanded<SuperT>(refIntValMin, refIntValMax,
                                            testIntVal, NEATALU::ATAN2PI_ERROR);
      EXPECT_TRUE(passed);
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      if (Utils::eq<TypeP>(*intervalX[i].GetMin<TypeP>(), TypeP(0.0)) ||
        ...
        bool passed = TestIntExpanded<SuperT>(
            refIntValMin, refIntValMax, testIntVec[i], NEATALU::ATAN2PI_ERROR);
        EXPECT_TRUE(passed);
      }
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      if (Utils::eq<TypeP>(*accurateRangedX[i].GetAcc<TypeP>(), TypeP(0.0))) {
        ...
        refAccValFloat =
            RefALU::atan2pi(SuperT(*accurateRangedY[i].GetAcc<TypeP>()),
                            SuperT(*accurateRangedX[i].GetAcc<TypeP>()));
        bool passed = TestAccExpanded<SuperT>(refAccValFloat, testAccVec[i],
                                              NEATALU::ATAN2PI_ERROR);
        EXPECT_TRUE(passed);
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      if (Utils::eq<TypeP>(*intervalRangedX[i].GetMin<TypeP>(), TypeP(0.0)) ||
        ...
        refIntValMax =
            RefALU::atan2pi(SuperT(*intervalRangedY[i].GetMax<TypeP>()),
                            SuperT(*intervalRangedX[i].GetMax<TypeP>()));
        refIntValMin =
            RefALU::atan2pi(SuperT(*intervalRangedY[i].GetMin<TypeP>()),
                            SuperT(*intervalRangedX[i].GetMin<TypeP>()));
...
TYPED_TEST(NEATAluTypedMath, sqrt) {
  ...
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &firstFloat[0], curWidth,
                                NUM_TESTS, TypeP(0.01),
                                std::numeric_limits<TypeP>::max());
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &secondFloat[0], curWidth,
                                NUM_TESTS, TypeP(0.01),
                                std::numeric_limits<TypeP>::max());
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    passed =
        TestIntExpanded<SuperT>(refIntValMin, refIntValMax, testIntVal, ulpErr);
    EXPECT_TRUE(passed);
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      bool passed =
          TestAccExpanded<SuperT>(refAccValFloat, testAccVec[i], ulpErr);
      EXPECT_TRUE(passed);
    }
...
TYPED_TEST(NEATAluTypedMath, native_rsqrt) {
  ...
  /* test for specific values */
  NEATValue testAccVal =
      NEATALU::native_rsqrt<TypeP>(NEATValue(NEATValue::ANY));
  EXPECT_TRUE(testAccVal.IsUnknown());
...
TYPED_TEST(NEATAluTypedMath, DISABLED_dot) {
  typedef typename TypeParam::Type TypeP;
  typedef typename superT<TypeP>::type SuperT;
  ...
  GenerateRandomVectorsAutoSeed(this->dataTypeVal, &firstFloat0[0], curWidth,
                                NUM_TESTS);
  GenerateRandomVectorsAutoSeed(this->dataTypeVal, &secondFloat0[0], curWidth,
                                NUM_TESTS);
  GenerateRandomVectorsAutoSeed(this->dataTypeVal, &firstFloat1[0], curWidth,
                                NUM_TESTS);
  GenerateRandomVectorsAutoSeed(this->dataTypeVal, &secondFloat1[0], curWidth,
                                NUM_TESTS);

  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &firstFloatRanged0[0],
                                curWidth, NUM_TESTS, TypeP(-1000.0),
                                TypeP(+1000.0));
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &secondFloatRanged0[0],
                                curWidth, NUM_TESTS, TypeP(-1000.0),
                                TypeP(+1000.0));
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &firstFloatRanged1[0],
                                curWidth, NUM_TESTS, TypeP(-1000.0),
                                TypeP(+1000.0));
  GenerateRangedVectorsAutoSeed(this->dataTypeVal, &secondFloatRanged1[0],
                                curWidth, NUM_TESTS, TypeP(-1000.0),
                                TypeP(+1000.0));
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      if (firstFloat0[testIdx * wrap.GetSize() + i] <
          secondFloat0[testIdx * wrap.GetSize() + i])
        interval0[i] = NEATValue(firstFloat0[testIdx * wrap.GetSize() + i],
                                 secondFloat0[testIdx * wrap.GetSize() + i]);
      else
        interval0[i] = NEATValue(secondFloat0[testIdx * wrap.GetSize() + i],
                                 firstFloat0[testIdx * wrap.GetSize() + i]);
    }

    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      accurate1[i] = NEATValue(firstFloat1[testIdx * wrap.GetSize() + i]);
      if (firstFloat1[testIdx * wrap.GetSize() + i] <
          secondFloat1[testIdx * wrap.GetSize() + i])
        interval1[i] = NEATValue(firstFloat1[testIdx * wrap.GetSize() + i],
                                 secondFloat1[testIdx * wrap.GetSize() + i]);
      else
        interval1[i] = NEATValue(secondFloat1[testIdx * wrap.GetSize() + i],
                                 firstFloat1[testIdx * wrap.GetSize() + i]);
    }

    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      accurateRanged0[i] =
          NEATValue(firstFloatRanged0[testIdx * wrap.GetSize() + i]);
      if (firstFloatRanged0[testIdx * wrap.GetSize() + i] <
          secondFloatRanged0[testIdx * wrap.GetSize() + i])
        intervalRanged0[i] =
            NEATValue(firstFloatRanged0[testIdx * wrap.GetSize() + i],
                      secondFloatRanged0[testIdx * wrap.GetSize() + i]);
      else
        intervalRanged0[i] =
            NEATValue(secondFloatRanged0[testIdx * wrap.GetSize() + i],
                      firstFloatRanged0[testIdx * wrap.GetSize() + i]);
    }

    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      accurateRanged1[i] =
          NEATValue(firstFloatRanged1[testIdx * wrap.GetSize() + i]);
      if (firstFloatRanged1[testIdx * wrap.GetSize() + i] <
          secondFloatRanged1[testIdx * wrap.GetSize() + i])
        intervalRanged1[i] =
            NEATValue(firstFloatRanged1[testIdx * wrap.GetSize() + i],
                      secondFloatRanged1[testIdx * wrap.GetSize() + i]);
      else
        intervalRanged1[i] =
            NEATValue(secondFloatRanged1[testIdx * wrap.GetSize() + i],
                      firstFloatRanged1[testIdx * wrap.GetSize() + i]);
    }

    /* test for NEAT value accurate */
    NEATValue testAcc = NEATALU::dot<TypeP>(accurate0[0], accurate1[0]);
    refAccValFloat = RefALU::mul(SuperT(*accurate0[0].GetAcc<TypeP>()),
                                 SuperT(*accurate1[0].GetAcc<TypeP>()));
    EXPECT_TRUE(TestAccExpanded<SuperT>(
        refAccValFloat, testAcc,
        1.0f)); // ulpErr = 1.0f here, becasue it is vector of 1 element

    /* test for vector of NEAT accurate */
    testAcc = NEATALU::dot<TypeP>(accurate0, accurate1);

    refForUlps =
        (SuperT)GetMaxAbsAccVec<TypeP>(wrap.GetSize(), accurate0, accurate1);
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      if (Utils::IsInf<TypeP>(refAccValFloat) && Utils::IsInf<TypeP>(r) &&
          !Utils::eq_float<TypeP>(refAccValFloat, r))
    ...
    if (isResultUnknown)
      ...
      EXPECT_TRUE(
          TestAccExpanded<SuperT>(refAccValFloat, testAcc,
                                  IntervalError<TypeP>::fromAbsoluteError(
                                      refForUlps * ulpErr * Ulp1<TypeP>())));

    /* test for NEAT value interval */
    NEATValue testInt = NEATALU::dot<TypeP>(interval0[0], interval1[0]);
    {
      SuperT r[4];
      r[0] = RefALU::mul(SuperT(*interval0[0].GetMin<TypeP>()),
                         SuperT(*interval1[0].GetMin<TypeP>()));
      r[1] = RefALU::mul(SuperT(*interval0[0].GetMax<TypeP>()),
                         SuperT(*interval1[0].GetMax<TypeP>()));
      r[2] = RefALU::mul(SuperT(*interval0[0].GetMin<TypeP>()),
                         SuperT(*interval1[0].GetMax<TypeP>()));
      r[3] = RefALU::mul(SuperT(*interval0[0].GetMax<TypeP>()),
                         SuperT(*interval1[0].GetMin<TypeP>()));

      refIntValMax = GetMaxValue<SuperT>(r, 4);
      refIntValMin = GetMinValue<SuperT>(r, 4);
    }
    EXPECT_TRUE(TestIntExpanded<SuperT>(
        refIntValMin, refIntValMax, testInt,
        1.0f)); // ulpErr = 1.0f here, becasue it is vector of 1 element
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      SuperT r[4];
      r[0] = RefALU::mul(SuperT(*interval0[i].GetMin<TypeP>()),
                         SuperT(*interval1[i].GetMin<TypeP>()));
      r[1] = RefALU::mul(SuperT(*interval0[i].GetMax<TypeP>()),
                         SuperT(*interval1[i].GetMax<TypeP>()));
      r[2] = RefALU::mul(SuperT(*interval0[i].GetMin<TypeP>()),
                         SuperT(*interval1[i].GetMax<TypeP>()));
      r[3] = RefALU::mul(SuperT(*interval0[i].GetMax<TypeP>()),
                         SuperT(*interval1[i].GetMin<TypeP>()));

      if (Utils::IsInf<TypeP>(refIntValMax) &&
          Utils::IsInf<TypeP>(GetMaxValue<SuperT>(r, 4)) &&
          !Utils::eq_float<TypeP>(refIntValMax, GetMaxValue<SuperT>(r, 4)))
        isResultUnknown = true;
      if (Utils::IsInf<TypeP>(refIntValMin) &&
          Utils::IsInf<TypeP>(GetMinValue<SuperT>(r, 4)) &&
          !Utils::eq_float<TypeP>(refIntValMin, GetMinValue<SuperT>(r, 4)))
        isResultUnknown = true;
    ...
    refForUlps =
        (SuperT)GetMaxAbsIntVec<TypeP>(wrap.GetSize(), interval0, interval1);
    refForUlps = refForUlps * refForUlps;
    if (isResultUnknown)
      EXPECT_TRUE(testInt.IsUnknown());
    else
      EXPECT_TRUE(
          TestIntExpanded<SuperT>(refIntValMin, refIntValMax, testInt,
                                  IntervalError<TypeP>::fromAbsoluteError(
                                      refForUlps * ulpErr * Ulp1<TypeP>())));
    ...
    refForUlps = (SuperT)GetMaxAbsAccVec<TypeP>(wrap.GetSize(), accurateRanged0,
                                                accurateRanged1);
    refForUlps = refForUlps * refForUlps;
    ...
    EXPECT_TRUE(
        TestIntExpanded<SuperT>(refIntValMin, refIntValMax, testInt, 0.5f));
    ...
    refForUlps = (SuperT)GetMaxAbsIntVec<TypeP>(wrap.GetSize(), intervalRanged0,
                                                intervalRanged1);
    refForUlps = refForUlps * refForUlps;
    EXPECT_TRUE(
        TestIntExpanded<SuperT>(refIntValMin, refIntValMax, testInt,
...
template <typename T> class NEATGeometricTestOneArg {
public:
  // Number of different random inputs to test.
  static const uint32_t NUM_TESTS = 500;
...
template <typename T> class NEATGeometricTestTwoArgs {
public:
  // Number of different random inputs to test.
  static const uint32_t NUM_TESTS = 500;
...
template <typename T> class NEATLengthTest : public NEATGeometricTestOneArg<T> {
public:
  typedef typename superT<T>::type sT;
  ...
  NEATLengthTest() {
    GenerateRangedVectorsAutoSeed<uint32_t>(U32, &indexArr[0], currWidth, 1, 0,
                                            vectorWidth);
  }
  ...
  void TestLength(NEATFuncP NEATFunc, NEATFuncVecP NEATFuncVec,
                  float baseUlpErr) {
    for (uint32_t testIdx = 0; testIdx < NUM_TESTS; ++testIdx) {
      ...
      ulpErr =
          ...
          0.5f * // effect on e of taking sqrt( x + e )
              (0.5f *
                   (float)vectorWidth + // cumulative error for multiplications
               0.5f *
                   (float)(vectorWidth - 1)); // cumulative error for additions
...
template <typename T> class NEATLengthRun : public ALUTest {};
...
public:
  ...
  void TestDistance(NEATFuncP NEATFunc, NEATFuncVecP NEATFuncVec,
                    float baseUlpErr) {
    for (uint32_t testIdx = 0; testIdx < NUM_TESTS; ++testIdx) {
      // Test for accurate value
      ...
      // Test for accurate vector
      ulpErr =
          baseUlpErr +                // error in sqrt
          (1.5 * (float)vectorWidth + // cumulative error for multiplications
                                      // (a-b+0.5ulp)**2 = (a-b)**2 + a*0.5ulp +
                                      // b*0.5 ulp + 0.5 ulp for multiplication
...
template <typename T> class NEATDistanceRun : public ALUTest {};
...
public:
  ...
  NEATCrossTest() {
    ...
    // Fill up argument values with random data
    GenerateRangedVectorsAutoSeed(dataTypeVal, &Arg1Min[0], currWidth,
                                  NUM_TESTS, T(-1000), T(1000));
    GenerateRangedVectorsAutoSeed(dataTypeVal, &Arg1Max[0], currWidth,
                                  NUM_TESTS, T(-1000), T(1000));
    GenerateRangedVectorsAutoSeed(dataTypeVal, &Arg2Min[0], currWidth,
                                  NUM_TESTS, T(-1000), T(1000));
    GenerateRangedVectorsAutoSeed(dataTypeVal, &Arg2Max[0], currWidth,
                                  NUM_TESTS, T(-1000), T(1000));
  ...
  static void oneItem(const NEATValue &in0, const NEATValue &in1,
                      const NEATValue &in2, const NEATValue &in3, sT &min,
                      sT &max) {
  ...
  void TestCross(uint32_t num) {
    ...
    for (uint32_t testIdx = 0; testIdx < NUM_TESTS; ++testIdx) {
      ...
      refVec[0] = RefALU::flush(sT(*in1[1].GetAcc<T>())) *
                      RefALU::flush(sT(*in2[2].GetAcc<T>())) -
                  RefALU::flush(sT(*in1[2].GetAcc<T>())) *
                      RefALU::flush(sT(*in2[1].GetAcc<T>()));
      refVec[1] = RefALU::flush(sT(*in1[2].GetAcc<T>())) *
                      RefALU::flush(sT(*in2[0].GetAcc<T>())) -
                  RefALU::flush(sT(*in1[0].GetAcc<T>())) *
                      RefALU::flush(sT(*in2[2].GetAcc<T>()));
      refVec[2] = RefALU::flush(sT(*in1[0].GetAcc<T>())) *
                      RefALU::flush(sT(*in2[1].GetAcc<T>())) -
                  RefALU::flush(sT(*in1[1].GetAcc<T>())) *
                      RefALU::flush(sT(*in2[0].GetAcc<T>()));

      sT refForUlps[numMin];
      refForUlps[0] = RefALU::flush(
          RefALU::fmax(RefALU::fabs((sT)GetMaxAbsAcc<T>(in1[1], in2[2])),
                       RefALU::fabs((sT)GetMaxAbsAcc<T>(in1[2], in2[1]))));
      refForUlps[1] = RefALU::flush(
          RefALU::fmax(RefALU::fabs((sT)GetMaxAbsAcc<T>(in1[2], in2[0])),
                       RefALU::fabs((sT)GetMaxAbsAcc<T>(in1[0], in2[2]))));
      refForUlps[2] = RefALU::flush(
          RefALU::fmax(RefALU::fabs((sT)GetMaxAbsAcc<T>(in1[0], in2[1])),
                       RefALU::fabs((sT)GetMaxAbsAcc<T>(in1[1], in2[0]))));
      ...
      for (uint32_t i = 0; i < numMin; i++) {
        EXPECT_TRUE(TestAccExpanded<sT>(
            refVec[i], tst[i],
            IntervalError<T>::fromAbsoluteError(refForUlps[i])));
      }
      ...
      refForUlps[0] = RefALU::flush(
          RefALU::fmax(RefALU::fabs((sT)GetMaxAbsInt<T>(in1[1], in2[2])),
                       RefALU::fabs((sT)GetMaxAbsInt<T>(in1[2], in2[1]))));
      refForUlps[1] = RefALU::flush(
          RefALU::fmax(RefALU::fabs((sT)GetMaxAbsInt<T>(in1[2], in2[0])),
                       RefALU::fabs((sT)GetMaxAbsInt<T>(in1[0], in2[2]))));
      refForUlps[2] = RefALU::flush(
          RefALU::fmax(RefALU::fabs((sT)GetMaxAbsInt<T>(in1[0], in2[1])),
                       RefALU::fabs((sT)GetMaxAbsInt<T>(in1[1], in2[0]))));
      ...
      for (uint32_t i = 0; i < numMin; i++) {
        EXPECT_TRUE(TestIntExpanded<sT>(
            refMin[i], refMax[i], tst[i],
            IntervalError<T>::fromAbsoluteError(refForUlps[i])));
      }
    }
  }
...
template <typename T> class NEATCrossRun : public ALUTest {};
...
TYPED_TEST(NEATAluTypedMath, mix) {
  ...
  GenerateRangedVectorsAutoSeed<TypeP>(this->dataTypeVal, &aArrFirst[0],
                                       curWidth, NUM_TESTS, TypeP(0.0),
                                       TypeP(1.0));
  GenerateRangedVectorsAutoSeed<TypeP>(this->dataTypeVal, &aArrSecond[0],
                                       curWidth, NUM_TESTS, TypeP(0.0),
                                       TypeP(1.0));
  ...
  for (int testIdx = 0; testIdx < NUM_TESTS; testIdx++) {
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      if (aArrFirst[testIdx * wrap.GetSize() + i] <
          aArrSecond[testIdx * wrap.GetSize() + i])
        intA[i] = NEATValue(aArrFirst[testIdx * wrap.GetSize() + i],
                            aArrSecond[testIdx * wrap.GetSize() + i]);
      else
        intA[i] = NEATValue(aArrSecond[testIdx * wrap.GetSize() + i],
                            aArrFirst[testIdx * wrap.GetSize() + i]);
    }
    ...
    NEATValue testAccVal =
        NEATALU::mix<TypeP>(accurate0[0], accurate1[0], accA[0]);
    refAccValFloat = RefALU::sub(SuperT(*accurate1[0].GetAcc<TypeP>()),
                                 SuperT(*accurate0[0].GetAcc<TypeP>()));
    refAccValFloat =
        RefALU::mul(refAccValFloat, SuperT(*accA[0].GetAcc<TypeP>()));
    refAccValFloat =
        RefALU::add(refAccValFloat, SuperT(*accurate0[0].GetAcc<TypeP>()));

    EXPECT_TRUE(TestAccExpandedDotMix<SuperT>(refAccValFloat, testAccVal,
                                              refForUlps, ulpErr));

    /* test for NEAT value interval */
    NEATValue testIntVal =
        NEATALU::mix<TypeP>(interval0[0], interval1[0], intA[0]);
    {
      refForUlps = (SuperT)GetMaxAbsInt<TypeP>(interval0[0], interval1[0]);

      SuperT r[4];
      r[0] = RefALU::sub(SuperT(*interval1[0].GetMin<TypeP>()),
                         SuperT(*interval0[0].GetMin<TypeP>()));
      r[1] = RefALU::sub(SuperT(*interval1[0].GetMax<TypeP>()),
                         SuperT(*interval0[0].GetMax<TypeP>()));
      ...
      refIntValMax =
          RefALU::add(refIntValMax, SuperT(*interval0[0].GetMax<TypeP>()));
      refIntValMin =
          RefALU::add(refIntValMin, SuperT(*interval0[0].GetMin<TypeP>()));

      EXPECT_TRUE(TestIntExpandedDotMix<SuperT>(
          refIntValMin, refIntValMax, testIntVal, refForUlps, ulpErr));
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      refAccValFloat = RefALU::sub(SuperT(*accurate1[i].GetAcc<TypeP>()),
                                   SuperT(*accurate0[i].GetAcc<TypeP>()));
      refAccValFloat =
          RefALU::mul(refAccValFloat, SuperT(*accA[0].GetAcc<TypeP>()));
      refAccValFloat =
          RefALU::add(refAccValFloat, SuperT(*accurate0[i].GetAcc<TypeP>()));

      EXPECT_TRUE(TestAccExpandedDotMix<SuperT>(refAccValFloat, testAccVec[i],
                                                refForUlps, ulpErr));
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      SuperT r[4];
      r[0] = RefALU::sub(SuperT(*interval1[i].GetMin<TypeP>()),
                         SuperT(*interval0[i].GetMin<TypeP>()));
      r[1] = RefALU::sub(SuperT(*interval1[i].GetMax<TypeP>()),
                         SuperT(*interval0[i].GetMax<TypeP>()));
      ...
      refIntValMax =
          RefALU::add(refIntValMax, SuperT(*interval0[i].GetMax<TypeP>()));
      refIntValMin =
          RefALU::add(refIntValMin, SuperT(*interval0[i].GetMin<TypeP>()));

      EXPECT_TRUE(TestIntExpandedDotMix<SuperT>(
          refIntValMin, refIntValMax, testIntVec[i], refForUlps, ulpErr));
    }
    ...
    for (uint32_t i = 0; i < wrap.GetSize(); i++) {
      ...
      refAccValFloat = RefALU::sub(SuperT(*accurate1[i].GetAcc<TypeP>()),
                                   SuperT(*accurate0[i].GetAcc<TypeP>()));
      refAccValFloat =
          RefALU::mul(refAccValFloat, SuperT(*accA[i].GetAcc<TypeP>()));
      refAccValFloat =
          RefALU::add(refAccValFloat, SuperT(*accurate0[i].GetAcc<TypeP>()));
      EXPECT_TRUE(TestAccExpandedDotMix<SuperT>(refAccValFloat, testAccVec[i],
                                                refForUlps, ulpErr));
...
public:
  ...
  void TestInf(NEATFuncVecP NEATFuncVec) {
    ...
    // if the elements in v were replaced as follows: for( i = 0; i < sizeof(v)
    // / sizeof(v[0] ); i++ ) v[i] = isinf(v[i] ) ? copysign(1.0, v[i]) : 0.0 *
    // v [i];
  ...
  void TestNormalize(NEATFuncP NEATFunc, NEATFuncVecP NEATFuncVec,
                     float baseUlpErr) {
    NEATVector a0(currWidth);
    ...
    for (uint32_t testIdx = 0; testIdx < NUM_TESTS; ++testIdx) {
      ...
      float ulpErr =
          baseUlpErr +                 // error in rsqrt + error in multiply
      ...
      refTotalAcc = sT(0.0);
      for (uint32_t i = 0; i < vectorWidth; i++) {
        sT localRes = RefALU::mul(sT(*accurate[i].GetAcc<T>()),
                                  sT(*accurate[i].GetAcc<T>()));
      ...
      for (uint32_t i = 0; i < vectorWidth; i++) {
        if (testAccVec[i].IsInterval() || testAccVec[i].IsAcc()) {
          refAccValFloat =
              RefALU::mul(sT(*accurate[i].GetAcc<T>()), refTotalAcc);
          EXPECT_TRUE(
              TestAccExpanded<sT>(refAccValFloat, testAccVec[i], ulpErr));
        } else {
      ...
      for (uint32_t i = 0; i < vectorWidth; i++) {
        ...
        r[0] = RefALU::mul(sT(*interval[i].GetMin<T>()),
                           sT(*interval[i].GetMin<T>()));
        r[1] = RefALU::mul(sT(*interval[i].GetMax<T>()),
                           sT(*interval[i].GetMax<T>()));

        if (*interval[i].GetMin<T>() <= T(0) &&
            *interval[i].GetMax<T>() >= T(0)) {
          r[2] = sT(0);
      ...
      for (uint32_t i = 0; i < vectorWidth; i++) {
        if (testIntVec[i].IsInterval() || testIntVec[i].IsAcc()) {
          ...
          EXPECT_TRUE(TestIntExpanded<sT>(refIntValMin, refIntValMax,
                                          testIntVec[i], ulpErr));
        } else {
      ...
      sT refMul = RefALU::mul(sT(*accurate[0].GetAcc<T>()),
                              sT(*accurate[0].GetAcc<T>()));
      ...
      if (testAccVal.IsAny()) {
        ...
        EXPECT_TRUE(
            TestAccExpanded<sT>(refAccValFloat, testAccVal, ulpsErrVal));
      }
      ...
      NEATValue testIntVal = NEATFunc(interval[0]);
      {
        if (testIntVal.IsAny()) {
          sT r[2];
          r[0] = RefALU::mul(sT(*interval[0].GetMin<T>()),
                             sT(*interval[0].GetMin<T>()));
          r[1] = RefALU::mul(sT(*interval[0].GetMax<T>()),
                             sT(*interval[0].GetMax<T>()));
...
template <typename T> class NEATNormalizeRun : public ALUTest {};
...
TYPED_TEST(NEATNormalizeRun, normalize) {
  ...
  normalizeTest.TestInf(
      NEATFuncVec); // is used for normalaize function only, fast_normalize has
...
TYPED_TEST(NEATAluTypedMath, copysign) {
  ...
  // positive data
  GenerateRangedVectorsAutoSeed<T>(dataTypeVal, &Arg1Min[0], currWidth,
                                   NUM_TESTS, 0, 1000);
  GenerateRangedVectorsAutoSeed<T>(dataTypeVal, &Arg1Max[0], currWidth,
                                   NUM_TESTS, 0, 1000);
  GenerateRangedVectorsAutoSeed<T>(dataTypeVal, &Arg2Min[0], currWidth,
                                   NUM_TESTS, 0, 1000);
  GenerateRangedVectorsAutoSeed<T>(dataTypeVal, &Arg2Max[0], currWidth,
                                   NUM_TESTS, 0, 1000);
...
template <typename T, typename I> class SelectTypeContainer {
public:
  typedef T ABType;
  typedef I CType;
};

template <typename T> class NEATRelationalTestSelect : public ALUTest {
public:
  // Number of different random inputs to test.
  ...
  NEATRelationalTestSelect() {
    ...
    // Fill up argument values with random data
    GenerateRandomVectorsAutoSeed(dataTypeValAB, &Arg1Min[0], currWidth,
                                  NUM_TESTS);
    GenerateRandomVectorsAutoSeed(dataTypeValAB, &Arg1Max[0], currWidth,
                                  NUM_TESTS);
    GenerateRandomVectorsAutoSeed(dataTypeValAB, &Arg2Min[0], currWidth,
                                  NUM_TESTS);
    GenerateRandomVectorsAutoSeed(dataTypeValAB, &Arg2Max[0], currWidth,
                                  NUM_TESTS);
...
typedef ::testing::Types<
    SelectTypeContainer<float, int8_t>, SelectTypeContainer<float, uint8_t>,
    ...
    SelectTypeContainer<double, int64_t>, SelectTypeContainer<double, uint64_t>>
    SelectTypes;
...
TYPED_TEST(NEATRelationalTestSelect, select) {
  ...
  for (uint32_t testIdx = 0; testIdx < this->NUM_TESTS; ++testIdx) {
    ...
    for (uint32_t i = 0; i < this->vectorWidth; ++i) {
      TypeC localCond =
          this->Arg3[testIdx * this->vectorWidth + i] & getMSB<TypeC>();
      passed = false;
      if (localCond)
    ...
    for (uint32_t i = 0; i < this->vectorWidth; ++i) {
      a[i] = NEATValue(this->Arg1Min[testIdx * this->vectorWidth + i],
                       this->Arg1Max[testIdx * this->vectorWidth + i]);
      b[i] = NEATValue(this->Arg2Min[testIdx * this->vectorWidth + i],
                       this->Arg2Max[testIdx * this->vectorWidth + i]);
    }

    // Test scalar result = c ? b : a
    tstVal = NEATALU::select<TypeAB>(a[0], b[0],
                                     this->Arg3[testIdx * this->vectorWidth]);
