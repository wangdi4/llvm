...
template <typename T> bool my_isnan(T x, T y) { return x != y; }

template <typename T>
DataGenerator::FillDataType SetFillDataType(DataTypeVal dataType, T value) {
...
void CompareFillValue(DataTypeValWrapper dataType,
  ...
  case F16:
    ...
    {
      uint16_t *data = (uint16_t *)buf->GetDataPtr();
      for (uint32_t j = 0; j < desc.NumOfElements() * desc.SizeOfVector();
           j++) {
...
TEST(DataGenerator, FillValueTest) {
  ...
  typedef std::pair<DataTypeValWrapper, DataGenerator::FillDataType>
      FillValueTestPair;
  ...
  for (uint32_t buffLen = 1; buffLen <= NUM_TESTS; buffLen++) {
    for (int32_t vecSize = int32_t(V1); vecSize < int32_t(INVALID_WIDTH);
         vecSize++) {
      ...
      DataGenerator::SetValueMethod<int8_t>(bcfm, testPair.first, vecWidth,
                                            buffLen, randVal);
      ...
      DataGenerator::SetValueMethod<int16_t>(bcfm, testPair.first, vecWidth,
                                             buffLen, randVal);
      ...
      DataGenerator::SetValueMethod<int32_t>(bcfm, testPair.first, vecWidth,
                                             buffLen, randVal);
      ...
      DataGenerator::SetValueMethod<int64_t>(bcfm, testPair.first, vecWidth,
                                             buffLen, randVal);
      ...
      DataGenerator::SetValueMethod<uint8_t>(bcfm, testPair.first, vecWidth,
                                             buffLen, randVal);
      ...
      DataGenerator::SetValueMethod<uint16_t>(bcfm, testPair.first, vecWidth,
                                              buffLen, randVal);
      ...
      DataGenerator::SetValueMethod<uint32_t>(bcfm, testPair.first, vecWidth,
                                              buffLen, randVal);
      ...
      DataGenerator::SetValueMethod<uint64_t>(bcfm, testPair.first, vecWidth,
                                              buffLen, randVal);
      ...
      // a bit different from other cases
      {
        ...
        DataGenerator::SetValueMethod<CFloat16>(bcfm, testPair.first, vecWidth,
                                                buffLen, valueFloat16);
        ...
        testPair.second =
            SetFillDataType(testPair.first.GetValue(), valueFloat16);
        testVector.push_back(testPair);
      }
      // F32
      randVal = rand();
      testPair.first.SetValue(F32);
      DataGenerator::SetValueMethod<float>(bcfm, testPair.first, vecWidth,
                                           buffLen, randVal);
      ...
      DataGenerator::SetValueMethod<double>(bcfm, testPair.first, vecWidth,
                                            buffLen, randVal);
      ...
      for (uint32_t i = 0; i < testVector.size(); i++) {
        ...
        CompareFillValue(testPair_n.first, testPair_n.second,
...
// Each element of buffer "bufRef" should be equal to the appropriate element of
// "bufSame" and buffer "bufDiff" should not be the same with "bufRef"
void CompareSeed(DataTypeValWrapper dataType, IMemoryObject *bufRef,
                 IMemoryObject *bufSame, IMemoryObject *bufDiff,
                 uint64_t seed) {
  ...
  if ((descRef.NumOfElements() * descRef.SizeOfVector() !=
       descSame.NumOfElements() * descSame.SizeOfVector()) ||
      (descRef.NumOfElements() * descRef.SizeOfVector() !=
       descDiff.NumOfElements() * descDiff.SizeOfVector())) {
  ...
  case I8: {
    ...
    diffCnt = 0;
    for (uint32_t j = 0; j < descRef.NumOfElements() * descRef.SizeOfVector();
         j++) {
  ...
  case F16: {
    ...
    for (uint32_t j = 0; j < descRef.NumOfElements() * descRef.SizeOfVector();
      ...
      EXPECT_EQ(dataRefFloat, dataSameFloat)
          << "[CompareSeed] seed = " << seed << "\n";
...
void CompareRandomRange(DataTypeValWrapper dataType,
                        ...
                        DataGenerator::FillDataType high, IMemoryObject *buf,
                        uint64_t seed) {
  BufferDesc desc = GetBufferDescription(buf->GetMemoryObjectDesc());
  ...
  case I8: {
    int8_t *data = (int8_t *)buf->GetDataPtr();
    for (uint32_t j = 0; j < desc.NumOfElements() * desc.SizeOfVector(); j++) {
      EXPECT_GE(high.dI8, data[j])
          << "[RandomRangeTest] seed = " << seed << "\n";
      EXPECT_LE(low.dI8, data[j])
          << "[RandomRangeTest] seed = " << seed << "\n";
    }
  } break;
  case U8: {
    uint8_t *data = (uint8_t *)buf->GetDataPtr();
    for (uint32_t j = 0; j < desc.NumOfElements() * desc.SizeOfVector(); j++) {
      EXPECT_GE(high.dU8, data[j])
          << "[RandomRangeTest] seed = " << seed << "\n";
      EXPECT_LE(low.dU8, data[j])
          << "[RandomRangeTest] seed = " << seed << "\n";
    }
  } break;
  case I16: {
    int16_t *data = (int16_t *)buf->GetDataPtr();
    for (uint32_t j = 0; j < desc.NumOfElements() * desc.SizeOfVector(); j++) {
      EXPECT_GE(high.dI16, data[j])
          << "[RandomRangeTest] seed = " << seed << "\n";
      EXPECT_LE(low.dI16, data[j])
          << "[RandomRangeTest] seed = " << seed << "\n";
    }
  } break;
  case U16: {
    uint16_t *data = (uint16_t *)buf->GetDataPtr();
    for (uint32_t j = 0; j < desc.NumOfElements() * desc.SizeOfVector(); j++) {
      EXPECT_GE(high.dU16, data[j])
          << "[RandomRangeTest] seed = " << seed << "\n";
      EXPECT_LE(low.dU16, data[j])
          << "[RandomRangeTest] seed = " << seed << "\n";
    }
  } break;
  case I32: {
    int32_t *data = (int32_t *)buf->GetDataPtr();
    for (uint32_t j = 0; j < desc.NumOfElements() * desc.SizeOfVector(); j++) {
      EXPECT_GE(high.dI32, data[j])
          << "[RandomRangeTest] seed = " << seed << "\n";
      EXPECT_LE(low.dI32, data[j])
          << "[RandomRangeTest] seed = " << seed << "\n";
    }
  } break;
  case U32: {
    uint32_t *data = (uint32_t *)buf->GetDataPtr();
    for (uint32_t j = 0; j < desc.NumOfElements() * desc.SizeOfVector(); j++) {
      EXPECT_GE(high.dU32, data[j])
          << "[RandomRangeTest] seed = " << seed << "\n";
      EXPECT_LE(low.dU32, data[j])
          << "[RandomRangeTest] seed = " << seed << "\n";
    }
  } break;
  case I64: {
    int64_t *data = (int64_t *)buf->GetDataPtr();
    for (uint32_t j = 0; j < desc.NumOfElements() * desc.SizeOfVector(); j++) {
      EXPECT_GE(high.dI64, data[j])
          << "[RandomRangeTest] seed = " << seed << "\n";
      EXPECT_LE(low.dI64, data[j])
          << "[RandomRangeTest] seed = " << seed << "\n";
    }
  } break;
  case U64: {
    uint64_t *data = (uint64_t *)buf->GetDataPtr();
    for (uint32_t j = 0; j < desc.NumOfElements() * desc.SizeOfVector(); j++) {
      EXPECT_GE(high.dU64, data[j])
          << "[RandomRangeTest] seed = " << seed << "\n";
      EXPECT_LE(low.dU64, data[j])
          << "[RandomRangeTest] seed = " << seed << "\n";
    }
  } break;
  case F16: {
    ...
    for (uint32_t j = 0; j < desc.NumOfElements() * desc.SizeOfVector(); j++) {
      ...
      EXPECT_GE(highFloat, dataFloat)
          << "[RandomRangeTest] seed = " << seed << "\n";
      EXPECT_LE(lowFloat, dataFloat)
          << "[RandomRangeTest] seed = " << seed << "\n";
    }
  } break;
  case F32: {
    float *data = (float *)buf->GetDataPtr();
    for (uint32_t j = 0; j < desc.NumOfElements() * desc.SizeOfVector(); j++) {
      EXPECT_GE(high.dF32, data[j])
          << "[RandomRangeTest] seed = " << seed << "\n";
      EXPECT_LE(low.dF32, data[j])
          << "[RandomRangeTest] seed = " << seed << "\n";
    }
  } break;
  case F64: {
    double *data = (double *)buf->GetDataPtr();
    for (uint32_t j = 0; j < desc.NumOfElements() * desc.SizeOfVector(); j++) {
      EXPECT_GE(high.dF64, data[j])
          << "[RandomRangeTest] seed = " << seed << "\n";
      EXPECT_LE(low.dF64, data[j])
          << "[RandomRangeTest] seed = " << seed << "\n";
    }
  } break;
  default:
  ...
  for (uint32_t buffLen = 1; buffLen <= NUM_TESTS; buffLen++) {
    for (int32_t vecSize = (int32_t)V1; vecSize < (int32_t)INVALID_WIDTH;
      ...
      // U8
      {
        ...
        do {
          ...
          lowValue = (uint8_t)(rand() % std::numeric_limits<uint8_t>::max());
                 ...
                 lowValue); // in order to avoid (lowValue == highValue)
        testData.low.dU8 = lowValue;
        testData.high.dU8 = highValue;
        testData.dataType.SetValue(U8);

        DataGenerator::SetRandomFromRangeMethod<uint8_t>(
            bcfm, testData.dataType, vecWidth, buffLen, lowValue, highValue);
      ...
      // U16
      {
        ...
        DataGenerator::SetRandomFromRangeMethod<uint16_t>(
            bcfm, testData.dataType, vecWidth, buffLen, lowValue, highValue);
      ...
      // U32
      {
        ...
        DataGenerator::SetRandomFromRangeMethod<uint32_t>(
            bcfm, testData.dataType, vecWidth, buffLen, lowValue, highValue);
      ...
      // U64
      {
        ...
        DataGenerator::SetRandomFromRangeMethod<uint64_t>(
            bcfm, testData.dataType, vecWidth, buffLen, lowValue, highValue);
      ...
      // I8
      {
        ...
        DataGenerator::SetRandomFromRangeMethod<int8_t>(
            bcfm, testData.dataType, vecWidth, buffLen, lowValue, highValue);
      ...
      // I16
      {
        ...
        do {
          highValue =
              (int16_t)(rand() % std::numeric_limits<int16_t>::max() + 1);
          lowValue =
              (int16_t)(highValue -
                        (rand() % (std::numeric_limits<int16_t>::max() + 1)));
        } while (highValue <= lowValue);
        testData.low.dI16 = lowValue;
        testData.high.dI16 = highValue;
        testData.dataType.SetValue(I16);

        DataGenerator::SetRandomFromRangeMethod<int16_t>(
            bcfm, testData.dataType, vecWidth, buffLen, lowValue, highValue);
      ...
      // I32
      {
        ...
        do {
          highValue =
              (int32_t)(rand() % std::numeric_limits<int32_t>::max() + 1);
          lowValue = (int32_t)(highValue -
                               (rand() % std::numeric_limits<int32_t>::max()));
        } while (highValue <= lowValue);
        testData.low.dI32 = lowValue;
        testData.high.dI32 = highValue;
        testData.dataType.SetValue(I32);

        DataGenerator::SetRandomFromRangeMethod<int32_t>(
            bcfm, testData.dataType, vecWidth, buffLen, lowValue, highValue);
      ...
      // I64
      {
        ...
        DataGenerator::SetRandomFromRangeMethod<int64_t>(
            bcfm, testData.dataType, vecWidth, buffLen, lowValue, highValue);
      ...
      // I8
      {
        ...
        do {
          highValue =
              (int8_t)(-(rand() % std::numeric_limits<int8_t>::max() + 1));
          lowValue =
              (int8_t)(highValue -
                       (rand() % (std::numeric_limits<int8_t>::max() + 1)));
        } while (highValue <= lowValue);
      ...
      // I64
      {
        ...
        do {
          lowValue =
              (0 - std::numeric_limits<int64_t>::max()) + (int64_t)rand();
      ...
      // F16
      {
        ...
        DataGenerator::SetRandomFromRangeMethod<CFloat16>(
            bcfm, testData.dataType, vecWidth, buffLen, lowValue, highValue);
      ...
      // F32
      {
        ...
        DataGenerator::SetRandomFromRangeMethod<float>(
            bcfm, testData.dataType, vecWidth, buffLen, lowValue, highValue);
      ...
      // F64
      {
        ...
        DataGenerator::SetRandomFromRangeMethod<double>(
            bcfm, testData.dataType, vecWidth, buffLen, lowValue, highValue);
      ...
      for (uint32_t i = 0; i < testVector.size(); i++) {
        ...
        CompareRandomRange(testData_n.dataType, testData_n.low, testData_n.high,
                           list.GetBufferContainer(0)->GetMemoryObject(i),
                           seed);
      }
    }
...
bool IsDenormValue(double a) {
  ...
  l.dF64 = a;
  return ((l.dU64 & DOUBLE_EXP_MASK) == 0) &&
         ((l.dU64 & DOUBLE_MANT_MASK) != 0);
}
// DataGenerator::FillDataType

void CompareSpecialValue_CFloat16(float specialValuesProb, IMemoryObject *buf,
                                  uint64_t seed) {
  BufferDesc desc = GetBufferDescription(buf->GetMemoryObjectDesc());
  ...
  int32_t cntSum =
      cntNaN + cntPosZero + cntNegZero + cntPosInf + cntNegInf + cntDenorm;
  ...
  if (specialValuesProb == 0.0f) {
    ...
    EXPECT_EQ(dataNumLen, cntSum)
        << "[CompareSpecialValue] seed = " << seed << "\n";
  } else if (numExpected >= dataNumLen / 4) {
    int32_t numCorrected = (int32_t)((float)numExpected * 1.2f);
    EXPECT_LE(numExpected, numCorrected)
        << "[CompareSpecialValue] seed = " << seed << "\n";
  }
}
// function to compare results of using FILL_RANDOM_FLOAT_WITH_SPECIAL_VALUES
// method for data generation
template <typename T>
void CompareSpecialValue(float specialValuesProb, IMemoryObject *buf,
                         uint64_t seed) {
  BufferDesc desc = GetBufferDescription(buf->GetMemoryObjectDesc());
  ...
  if (specialValuesProb == 0.0f) {
    ...
    EXPECT_EQ(dataNumLen, cntSum)
        << "[CompareSpecialValue_CFloat16] seed = " << seed;
  } else if (numExpected >= dataNumLen / 4) {
    int32_t numCorrected = (int32_t)((float)numExpected * 1.2f);
    EXPECT_LE(numExpected, numCorrected)
        << "[CompareSpecialValue_CFloat16] seed = " << seed;
  ...
  for (uint32_t buffLen = 1; buffLen <= NUM_TESTS; buffLen++) {
    for (int32_t i = 0; i < probNum; i++) {
      for (int32_t vecSize = (int32_t)V1; vecSize < (int32_t)INVALID_WIDTH;
        ...
        dataType.SetValue(F16);
        DataGenerator::SetSpecialValuesMethod(bcfm, dataType, vecWidth, buffLen,
                                              prob[i]);
        ...
        uint64_t seed = dg.GetSeed();
        CompareSpecialValue_CFloat16(
            prob[i], list.GetBufferContainer(0)->GetMemoryObject(0), seed);
        CompareSpecialValue<float>(
            prob[i], list.GetBufferContainer(0)->GetMemoryObject(1), seed);
        CompareSpecialValue<double>(
            prob[i], list.GetBufferContainer(0)->GetMemoryObject(2), seed);
      }
    }
  }
}
...
TEST(DataGenerator, MultipleBufferContainersTest) {
  ...
  for (uint32_t testId = 0; testId < NUM_TESTS; ++testId) {
    for (int32_t vecSize = (int32_t)V1; vecSize < (int32_t)INVALID_WIDTH;
      ...
      EXPECT_EQ(BCList.GetBufferContainer(0)->GetMemoryObjectCount(),
                (std::size_t)2);
      EXPECT_EQ(BCList.GetBufferContainer(1)->GetMemoryObjectCount(),
                (std::size_t)3);
    }
  }
}
...
TEST(DataGenerator, ArbitraryTest) {
  ...
  DataGenerator::SetBufferContainerFillMethod<int32_t>(
      bcfm, tS.dataType, vectorWidth, 319, tS.fillMethod, tS.value.dI32,
      tS.low.dI32, tS.high.dI32, tS.prob);
  testVector.push_back(tS);
  // to fill buffer of 319 vectors of 8 elements of int32_t type by const value
  // 999
  tS.dataType.SetValue(I32);
  tS.value.dI32 = 999;
  ...
  tS.value.dF32 =
      999; // doesn't matter if FILL_RANDOM_FLOAT_WITH_SPECIAL_VALUES
  tS.low.dF32 =
      -2090; // doesn't matter if FILL_RANDOM_FLOAT_WITH_SPECIAL_VALUES
  tS.high.dF32 =
      5762; // doesn't matter if FILL_RANDOM_FLOAT_WITH_SPECIAL_VALUES
  ...
  DataGenerator::SetBufferContainerFillMethod<float>(
      bcfm, tS.dataType, vectorWidth, 319, tS.fillMethod, tS.value.dF32,
  ...
  for (uint32_t i = 0; i < testVector.size(); i++) {
    ...
    case DataGenerator::FILL_RANDOM_FLOAT_WITH_SPECIAL_VALUES:
      if (t.dataType.GetValue() == F16) {
        CompareSpecialValue_CFloat16(
            t.prob, list.GetBufferContainer(0)->GetMemoryObject(i), seed);
...
template <typename T> class DataGeneratorTypedFloat : public ::testing::Test {};
...
template <typename T> void TestGenerateRandomVectors(DataTypeVal dataTypeVal) {
  ...
  for (uint32_t buffLen = 1; buffLen <= NUM_TESTS; buffLen++) {
    for (uint32_t vecSize = uint32_t(V2); vecSize < uint32_t(INVALID_WIDTH);
         vecSize++) {
      VectorWidthWrapper vecWidth;
      vecWidth.SetValue(VectorWidth(vecSize));
      /// generates buffLen number of random vectors of vecSize width of
      /// dataTypeVal data type
      seed1 = GenerateRandomVectors(dataTypeVal, &data1[0],
                                    VectorWidth(vecSize), buffLen);
      seed2 = seed1;
      GenerateRandomVectorsSeed(dataTypeVal, &data2[0], VectorWidth(vecSize),
                                buffLen, seed2);

      for (uint32_t i = 0; i < buffLen; i++) {
        for (uint32_t j = 0; j < vecWidth.GetSize(); j++) {
          EXPECT_EQ(data1[i * vecWidth.GetSize() + j],
                    data2[i * vecWidth.GetSize() + j])
...
template <typename T>
void TestGenerateRangedVectors(DataTypeVal dataTypeVal, T low, T high) {
  const uint32_t NUM_TESTS = 10;
  T data1[NUM_TESTS * 16], data2[NUM_TESTS * 16];
  uint64_t seed1, seed2;

  for (uint32_t buffLen = 1; buffLen <= NUM_TESTS; buffLen++) {
    for (uint32_t vecSize = uint32_t(V1); vecSize < uint32_t(INVALID_WIDTH);
         vecSize++) {
      seed1 = GenerateRangedVectors(dataTypeVal, &data1[0],
                                    VectorWidth(vecSize), buffLen, low, high);
      seed2 = seed1;
      GenerateRangedVectorsSeed(dataTypeVal, &data2[0], VectorWidth(vecSize),
                                buffLen, low, high, seed2);
...
TYPED_TEST(DataGeneratorTypedSigned, GenerateRangedVectors) {
  ...
  do {
    ...
    lowValue = (TypeParam)(highValue -
                           (rand() % std::numeric_limits<TypeParam>::max()));
  } while (highValue <= lowValue);
...
template <typename T>
void TestGenerateSpecialVectors(DataTypeVal dataTypeVal, float prob) {
  const uint32_t NUM_TESTS = 10;
  T data1[NUM_TESTS * 16], data2[NUM_TESTS * 16];
  uint64_t seed1, seed2;
  ...
  for (uint32_t buffLen = 1; buffLen <= NUM_TESTS; buffLen++) {
    for (uint32_t vecSize = uint32_t(V1); vecSize < uint32_t(INVALID_WIDTH);
         vecSize++) {
      seed1 = GenerateSpecialVectors(dataTypeVal, &data1[0],
                                     VectorWidth(vecSize), buffLen, prob);
      seed2 = seed1;
...
template <typename T>
void TestGenerateRandomVectorsAutoSeed(DataTypeVal dataTypeVal) {
  const uint32_t NUM_TESTS = 10;
  T data1[NUM_TESTS * 16], data2[NUM_TESTS * 16];
  uint64_t seed1, seed2;
  ...
  for (uint32_t vecSize = uint32_t(V2); vecSize < uint32_t(INVALID_WIDTH);
    ...
    for (uint32_t buffLen = 1; buffLen <= NUM_TESTS; buffLen++) {
      ...
      /// generates buffLen number of random vectors of vecSize width of
      /// dataTypeVal data type
      GenerateRandomVectorsAutoSeed(dataTypeVal, &data1[0],
                                    VectorWidth(vecSize), buffLen);
      seed1 = GetCurrentSeed();
      GenerateRandomVectorsAutoSeed(dataTypeVal, &data2[0],
                                    VectorWidth(vecSize), buffLen);
      seed2 = GetCurrentSeed();
      EXPECT_EQ(seed2,
                (seed1 + GetUpdateConst())); // new seed should be equal to
                                             // previous seed plus constant

      uint32_t diffCnt = 0;
      for (uint32_t i = 0; i < buffLen; i++) {
        for (uint32_t j = 0; j < vecWidth.GetSize(); j++) {
          if (data1[i * vecWidth.GetSize() + j] !=
              data2[i * vecWidth.GetSize() + j])
            diffCnt++;
        }
      }
      if (diffCnt == 0)
        llvm::errs() << "[DATA GENERATOR WARNING] zero differences from "
                     << buffLen * vecWidth.GetSize() << " elements\n";
    }
  }
}

template <typename T>
void TestGenerateRandomVectorsAutoSeedFloat(DataTypeVal dataTypeVal) {
