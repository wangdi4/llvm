...
protected:
  virtual void SetUp() {
    ...
    // Buffer 0 F32
    BufferDesc arg1Desc = BufferDesc(
        BufF32VectorsNum,
        VectorWidthWrapper::ValueOf((uint32_t)BufF32VectorWidth), F32);
    IMemoryObject *buffF32 = args->CreateBuffer(arg1Desc);
    ::memcpy(buffF32->GetDataPtr(), DataBufF32, DataBufF32Size);

    // Buffer 1 F64
    BufferDesc arg2Desc = BufferDesc(
        BufF64VectorsNum,
        VectorWidthWrapper::ValueOf((uint32_t)BufF64VectorWidth), F64);
    IMemoryObject *buffF64 = args->CreateBuffer(arg2Desc);
    ::memcpy(buffF64->GetDataPtr(), DataBufF64, DataBufF64Size);

    // Buffer 2 I8
    BufferDesc argI8Desc =
        BufferDesc(BufI8VectorsNum,
                   VectorWidthWrapper::ValueOf((uint32_t)BufI8VectorWidth), I8);
    IMemoryObject *buffI8 = args->CreateBuffer(argI8Desc);
    ::memcpy(buffI8->GetDataPtr(), DataBufI8, DataBufI8Size);

    // Buffer 3 U8
    BufferDesc argU8Desc =
        BufferDesc(BufU8VectorsNum,
                   VectorWidthWrapper::ValueOf((uint32_t)BufU8VectorWidth), U8);
    IMemoryObject *buffU8 = args->CreateBuffer(argU8Desc);
    ::memcpy(buffU8->GetDataPtr(), DataBufU8, DataBufU8Size);

    // Buffer 4 I16
    BufferDesc argI16Desc = BufferDesc(
        BufI16VectorsNum,
        VectorWidthWrapper::ValueOf((uint32_t)BufI16VectorWidth), I16);
    IMemoryObject *buffI16 = args->CreateBuffer(argI16Desc);
    ::memcpy(buffI16->GetDataPtr(), DataBufI16, DataBufI16Size);

    // Buffer 5 U16
    BufferDesc argU16Desc = BufferDesc(
        BufU16VectorsNum,
        VectorWidthWrapper::ValueOf((uint32_t)BufU16VectorWidth), U16);
    IMemoryObject *buffU16 = args->CreateBuffer(argU16Desc);
    ::memcpy(buffU16->GetDataPtr(), DataBufU16, DataBufU16Size);

    // Buffer 6 I32
    BufferDesc argI32Desc = BufferDesc(
        BufI32VectorsNum,
        VectorWidthWrapper::ValueOf((uint32_t)BufI32VectorWidth), I32);
    IMemoryObject *buffI32 = args->CreateBuffer(argI32Desc);
    ::memcpy(buffI32->GetDataPtr(), DataBufI32, DataBufI32Size);

    // Buffer 7 U32
    BufferDesc argU32Desc = BufferDesc(
        BufU32VectorsNum,
        VectorWidthWrapper::ValueOf((uint32_t)BufU32VectorWidth), U32);
    IMemoryObject *buffU32 = args->CreateBuffer(argU32Desc);
    ::memcpy(buffU32->GetDataPtr(), DataBufU32, DataBufU32Size);

    // Buffer 8 I64
    BufferDesc argI64Desc = BufferDesc(
        BufI64VectorsNum,
        VectorWidthWrapper::ValueOf((uint32_t)BufI64VectorWidth), I64);
    IMemoryObject *buffI64 = args->CreateBuffer(argI64Desc);
    ::memcpy(buffI64->GetDataPtr(), DataBufI64, DataBufI64Size);

    // Buffer 9 U64
    BufferDesc argU64Desc = BufferDesc(
        BufU64VectorsNum,
        VectorWidthWrapper::ValueOf((uint32_t)BufU64VectorWidth), U64);
    IMemoryObject *buffU64 = args->CreateBuffer(argU64Desc);
    ::memcpy(buffU64->GetDataPtr(), DataBufU64, DataBufU64Size);

    // Buffer 10 F16
    BufferDesc argF16Desc = BufferDesc(
        BufF16VectorsNum,
        VectorWidthWrapper::ValueOf((uint32_t)BufF16VectorWidth), F16);
  ...
  void CheckValues(BufferContainerList &inp) {
    // buffer 0
    {
      ...
      for (int32_t i = 1; i < DataBufF32Elems; ++i)
        EXPECT_EQ(DataBufF32[i],
                  baf32.GetElem(i / BufF32VectorWidth, i % BufF32VectorWidth));
    }
    ...
    // buffer 1
    {
      ...
      for (int32_t i = 1; i < DataBufF64Elems; ++i)
        EXPECT_EQ(DataBufF64[i],
                  baF64.GetElem(i / BufF64VectorWidth, i % BufF64VectorWidth));
    }
    ...
    // buffer 2 I8
    {
      ...
      for (int32_t i = 0; i < DataBufI8Elems; ++i)
        EXPECT_EQ(DataBufI8[i],
                  baI8.GetElem(i / BufI8VectorWidth, i % BufI8VectorWidth));
    }
    ...
    // buffer 3 U8
    {
      ...
      for (int32_t i = 0; i < DataBufU8Elems; ++i)
        EXPECT_EQ(DataBufU8[i],
                  baU8.GetElem(i / BufU8VectorWidth, i % BufU8VectorWidth));
    }
    ...
    // buffer 4 I16
    {
      ...
      for (int32_t i = 0; i < DataBufI16Elems; ++i)
        EXPECT_EQ(DataBufI16[i],
                  baI16.GetElem(i / BufI16VectorWidth, i % BufI16VectorWidth));
    }
    ...
    // buffer 5 U16
    {
      ...
      for (int32_t i = 0; i < DataBufU16Elems; ++i)
        EXPECT_EQ(DataBufU16[i],
                  baU16.GetElem(i / BufU16VectorWidth, i % BufU16VectorWidth));
    }
    ...
    // buffer 6 I32
    {
      ...
      for (int32_t i = 0; i < DataBufI32Elems; ++i)
        EXPECT_EQ(DataBufI32[i],
                  baI32.GetElem(i / BufI32VectorWidth, i % BufI32VectorWidth));
    }
    ...
    // buffer 7 U32
    {
      ...
      for (int32_t i = 0; i < DataBufU32Elems; ++i)
        EXPECT_EQ(DataBufU32[i],
                  baU32.GetElem(i / BufU32VectorWidth, i % BufU32VectorWidth));
    }
    ...
    // buffer 8 I64
    {
      ...
      for (int32_t i = 0; i < DataBufI64Elems; ++i)
        EXPECT_EQ(DataBufI64[i],
                  baI64.GetElem(i / BufI64VectorWidth, i % BufI64VectorWidth));
    }
    ...
    // buffer 9 U64
    {
      ...
      for (int32_t i = 0; i < DataBufU64Elems; ++i)
        EXPECT_EQ(DataBufU64[i],
                  baU64.GetElem(i / BufU64VectorWidth, i % BufU64VectorWidth));
    }
    ...
    // buffer 10 F16
    {
      ...
      for (int32_t i = 1; i < DataBufF16Elems; ++i)
        EXPECT_TRUE(DataBufF16[i] == baF16.GetElem(i / BufF16VectorWidth,
                                                   i % BufF16VectorWidth));
    }
  }
};

const CFloat16 DataManagerDataTypes::DataBufF16[] = {CFloat16::GetNaN(),
                                                     +0.24f,
                                                     CFloat16::GetPInf(),
                                                     CFloat16::GetNInf(),
                                                     CFloat16::GetMax(),
...
const int32_t DataManagerDataTypes::DataBufF16Size =
    sizeof(DataManagerDataTypes::DataBufF16);
const int32_t DataManagerDataTypes::DataBufF16Elems =
    sizeof(DataManagerDataTypes::DataBufF16) / sizeof(CFloat16);
const int32_t DataManagerDataTypes::BufF16VectorWidth = 2;
const int32_t DataManagerDataTypes::BufF16VectorsNum =
    DataManagerDataTypes::DataBufF16Elems /
    DataManagerDataTypes::BufF16VectorWidth;
...
const int32_t DataManagerDataTypes::DataBufF32Elems =
    sizeof(DataBufF32) / sizeof(float);
const int32_t DataManagerDataTypes::BufF32VectorWidth = 2;
const int32_t DataManagerDataTypes::BufF32VectorsNum =
    DataManagerDataTypes::DataBufF32Elems /
    DataManagerDataTypes::BufF32VectorWidth;
...
const double DataManagerDataTypes::DataBufF64[] = {
    ...
    1234.65,
    -4003423.452};
const int32_t DataManagerDataTypes::DataBufF64Size =
    sizeof(DataManagerDataTypes::DataBufF64);
const int32_t DataManagerDataTypes::DataBufF64Elems =
    sizeof(DataManagerDataTypes::DataBufF64) / sizeof(double);
const int32_t DataManagerDataTypes::BufF64VectorWidth = 2;
const int32_t DataManagerDataTypes::BufF64VectorsNum =
    DataManagerDataTypes::DataBufF64Elems /
    DataManagerDataTypes::BufF64VectorWidth;
...
const int8_t DataManagerDataTypes::DataBufI8[] = {
    ...
    -5,
    88};
const int32_t DataManagerDataTypes::DataBufI8Size =
    sizeof(DataManagerDataTypes::DataBufI8);
const int32_t DataManagerDataTypes::DataBufI8Elems =
    sizeof(DataManagerDataTypes::DataBufI8) / sizeof(int8_t);
const int32_t DataManagerDataTypes::BufI8VectorWidth = 1;
const int32_t DataManagerDataTypes::BufI8VectorsNum =
    DataManagerDataTypes::DataBufI8Elems /
    DataManagerDataTypes::BufI8VectorWidth;
...
const uint8_t DataManagerDataTypes::DataBufU8[] = {
    ...
    127,
    32};
const int32_t DataManagerDataTypes::DataBufU8Size =
    sizeof(DataManagerDataTypes::DataBufU8);
const int32_t DataManagerDataTypes::DataBufU8Elems =
    sizeof(DataManagerDataTypes::DataBufU8) / sizeof(uint8_t);
const int32_t DataManagerDataTypes::BufU8VectorWidth = 4;
const int32_t DataManagerDataTypes::BufU8VectorsNum =
    DataManagerDataTypes::DataBufU8Elems /
    DataManagerDataTypes::BufU8VectorWidth;
...
const int32_t DataManagerDataTypes::DataBufI16Size =
    sizeof(DataManagerDataTypes::DataBufI16);
const int32_t DataManagerDataTypes::DataBufI16Elems =
    sizeof(DataManagerDataTypes::DataBufI16) / sizeof(int16_t);
const int32_t DataManagerDataTypes::BufI16VectorWidth = 1;
const int32_t DataManagerDataTypes::BufI16VectorsNum =
    DataManagerDataTypes::DataBufI16Elems /
    DataManagerDataTypes::BufI16VectorWidth;
...
const int32_t DataManagerDataTypes::DataBufU16Size =
    sizeof(DataManagerDataTypes::DataBufU16);
const int32_t DataManagerDataTypes::DataBufU16Elems =
    sizeof(DataManagerDataTypes::DataBufU16) / sizeof(uint16_t);
const int32_t DataManagerDataTypes::BufU16VectorWidth = 8;
const int32_t DataManagerDataTypes::BufU16VectorsNum =
    DataManagerDataTypes::DataBufU16Elems /
    DataManagerDataTypes::BufU16VectorWidth;
...
const int32_t DataManagerDataTypes::DataBufI32[] = {
    ...
    -5,
    88};
const int32_t DataManagerDataTypes::DataBufI32Size =
    sizeof(DataManagerDataTypes::DataBufI32);
const int32_t DataManagerDataTypes::DataBufI32Elems =
    sizeof(DataManagerDataTypes::DataBufI32) / sizeof(int32_t);
const int32_t DataManagerDataTypes::BufI32VectorWidth = 1;
const int32_t DataManagerDataTypes::BufI32VectorsNum =
    DataManagerDataTypes::DataBufI32Elems /
    DataManagerDataTypes::BufI32VectorWidth;
...
const uint32_t DataManagerDataTypes::DataBufU32[] = {
    ...
    32,
    58000};
const int32_t DataManagerDataTypes::DataBufU32Size =
    sizeof(DataManagerDataTypes::DataBufU32);
const int32_t DataManagerDataTypes::DataBufU32Elems =
    sizeof(DataManagerDataTypes::DataBufU32) / sizeof(uint32_t);
const int32_t DataManagerDataTypes::BufU32VectorWidth = 2;
const int32_t DataManagerDataTypes::BufU32VectorsNum =
    DataManagerDataTypes::DataBufU32Elems /
    DataManagerDataTypes::BufU32VectorWidth;
...
const int64_t DataManagerDataTypes::DataBufI64[] = {
    ...
    -5,
    88};
const int32_t DataManagerDataTypes::DataBufI64Size =
    sizeof(DataManagerDataTypes::DataBufI64);
const int32_t DataManagerDataTypes::DataBufI64Elems =
    sizeof(DataManagerDataTypes::DataBufI64) / sizeof(int64_t);
const int32_t DataManagerDataTypes::BufI64VectorWidth = 1;
const int32_t DataManagerDataTypes::BufI64VectorsNum =
    DataManagerDataTypes::DataBufI64Elems /
    DataManagerDataTypes::BufI64VectorWidth;
...
const uint64_t DataManagerDataTypes::DataBufU64[] = {
    ...
    32,
    58000};
const int32_t DataManagerDataTypes::DataBufU64Size =
    sizeof(DataManagerDataTypes::DataBufU64);
const int32_t DataManagerDataTypes::DataBufU64Elems =
    sizeof(DataManagerDataTypes::DataBufU64) / sizeof(uint64_t);
const int32_t DataManagerDataTypes::BufU64VectorWidth = 2;
const int32_t DataManagerDataTypes::BufU64VectorsNum =
    DataManagerDataTypes::DataBufU64Elems /
    DataManagerDataTypes::BufU64VectorWidth;
...
TEST(DataManager, DenormCheck) {
  ...
  // to suppress gcc warning: dereferencing type-punned pointer will break
...

  {
    ...
    // check that we load correct values
    EXPECT_EQ(d1, (reinterpret_cast<float *>(
                      pBC->GetMemoryObject(0)->GetDataPtr()))[0]);
    EXPECT_EQ(d2, (reinterpret_cast<float *>(
                      pBC->GetMemoryObject(0)->GetDataPtr()))[1]);
    EXPECT_EQ(d3, (reinterpret_cast<float *>(
                      pBC->GetMemoryObject(0)->GetDataPtr()))[2]);
    EXPECT_EQ(+0.f, (reinterpret_cast<float *>(
                        pBC->GetMemoryObject(0)->GetDataPtr()))[3]);
  }
}

/// This test checks for creation/write/load multiple BufferConatiner objects in
/// BufferContainerList object
...
const int32_t DataManagerContainerList::DataBufF32Elems =
    sizeof(DataBufF32) / sizeof(float);
const int32_t DataManagerContainerList::BufF32VectorWidth = 2;
const int32_t DataManagerContainerList::BufF32VectorsNum =
    DataBufF32Elems / BufF32VectorWidth;
...
TEST_F(DataManagerContainerList, BufferContainerListXMLReadWriteCheck) {
  ...
  const char OUTPUT_FILE_NAME[] =
      "testBufferContainerListXMLReadWriteCheck.xml";
...
TEST_F(DataManagerContainerList, BufferContainerListBinaryReadWriteCheck) {
  ...
  const char OUTPUT_FILE_NAME[] =
      "testBufferContainerListXMLReadWriteCheck.bin";
...
int32_t DataManagerContainer::DataBufF32Size =
    sizeof(DataManagerContainer::DataBufF32);
int32_t DataManagerContainer::DataBufF32Elems = DataBufF32Size / sizeof(float);

/// This test checks for creation/write/load multiple Buffer objects in
/// BufferContainer object
...
protected:
  virtual void SetUp() {
    ...
    BufferDesc argI32Desc = BufferDesc(
        ...
        VectorWidthWrapper::ValueOf((uint32_t)BufI32VectorWidth), I32);
    IMemoryObject *buffI32 = args->CreateBuffer(argI32Desc);
    ::memcpy(buffI32->GetDataPtr(), DataBufI32,
             DataBufI32Elems * sizeof(int32_t));
  }

  virtual void TearDown() { delete[] DataBufI32; }
...
/// This test checks for creation/write/load multiple Vector(Elements) in Buffer
/// object
...
protected:
  virtual void SetUp() {
    const float DataBufF32[32] = {std::numeric_limits<float>::quiet_NaN(),
                                  +0.24f,
                                  std::numeric_limits<float>::infinity(),
                                  -std::numeric_limits<float>::infinity(),
                                  std::numeric_limits<float>::max(),
  ...
  void CheckValues(BufferContainerList &inp) {
    ...
    // V3
    {
      ...
      const int32_t numElems =
          (int32_t)GetBufferDescription(pB->GetMemoryObjectDesc())
              .NumOfElements() *
          (int32_t)GetBufferDescription(pB->GetMemoryObjectDesc())
              .SizeOfVector();
      // check that we load correct values
...
void CheckFloatValueDesc(float acc, float minInt, float maxInt,
                         const NEATValue &IntervalVD,
                         const NEATValue &AccurateVD) {
...
TEST(DataManager, BufferContainers) {
  ...
  /// Test NEAT containers
  {
    ...
    /// Check data
    {
      ...
      for (int i = 0; i < 2; i++)
        for (int j = 0; j < 2; j++) {
          CheckFloatValueDesc(acc[i][j], min[i][j], max[i][j],
                              bAcc1.GetElem(i, j), bAcc2.GetElem(i, j));
        }
    }
  }
}
  ...
  for (int count = 0; count < argc; count++) {
    ...
    if (found != std::string::npos) {
      ...
      if (strIn.size() > key.size() && found < strIn.size()) {
        ...
        std::string seedToParse =
            strIn.substr(found, maxNumOfDigitsIn64bitString);
