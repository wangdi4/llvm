...
template <typename T> class NEATAluTypedCommon : public ALUTest {
public:
...
TYPED_TEST(NEATAluTypedCommon, step) {
  ...
  for (int testIdx = 0; testIdx < this->NUM_TESTS; ++testIdx) {
    ...
    refAccValFloat = RefALU::step(sT(*edgeVal.GetAcc<TypeParam>()),
                                  sT(*xVal.GetAcc<TypeParam>()));
    EXPECT_TRUE(testVal.IsAcc() &&
                *testVal.GetAcc<TypeParam>() == (TypeParam)refAccValFloat);

    /* test for vector of NEAT accurate */
    NEATVector testVec = NEATALU::step<TypeParam>(edge, x);
    for (uint32_t i = 0; i < this->vectorWidth; ++i) {
      refAccValFloat = RefALU::step(sT(*edge[i].GetAcc<TypeParam>()),
                                    sT(*x[i].GetAcc<TypeParam>()));
      EXPECT_TRUE(testVec[i].IsAcc() &&
                  *testVec[i].GetAcc<TypeParam>() == (TypeParam)refAccValFloat);
    }
    ...
    zeroCase = RefALU::step(sT(*edgeVal.GetMin<TypeParam>()),
                            sT(*xVal.GetMax<TypeParam>()));
    oneCase = RefALU::step(sT(*edgeVal.GetMax<TypeParam>()),
                           sT(*xVal.GetMin<TypeParam>()));
    if (zeroCase == oneCase) {
      EXPECT_TRUE(testVal.IsAcc() &&
                  *testVal.GetAcc<TypeParam>() == (TypeParam)zeroCase);
    } else {
    ...
    for (uint32_t i = 0; i < this->vectorWidth; ++i) {
      zeroCase = RefALU::step(sT(*edge[i].GetMin<TypeParam>()),
                              sT(*x[i].GetMax<TypeParam>()));
      oneCase = RefALU::step(sT(*edge[i].GetMax<TypeParam>()),
                             sT(*x[i].GetMin<TypeParam>()));
      if (zeroCase == oneCase) {
        EXPECT_TRUE(testVec[i].IsAcc() &&
                    *testVec[i].GetAcc<TypeParam>() == (TypeParam)zeroCase);
      } else {
        EXPECT_TRUE(zeroCase == sT(1) && oneCase == sT(0) &&
                    testVec[i].IsUnknown());
      }
    }
  }
...
static NEATValue min_ref(const NEATValue &xVal, const NEATValue &yVal) {
  ...
  if (intvOverlap)
    ...
    throw Exception::IllegalFunctionCall(
        "min_ref is called with wrong arguments");
...
TYPED_TEST(NEATAluTypedCommon, min) {
  ...
  EXPECT_TRUE(nres.IsInterval() && *nres.GetMin<TypeParam>() == xmin &&
              *nres.GetMax<TypeParam>() == xmax);
  ...
  ny.SetIntervalVal<TypeParam>(ymin, ymax);
  nres = NEATALU::min<TypeParam>(nx, ny);
  EXPECT_TRUE(nres.IsInterval() && *nres.GetMin<TypeParam>() == ymin &&
              *nres.GetMax<TypeParam>() == xmax);
  ...
  for (int testIdx = 0; testIdx < this->NUM_TESTS; ++testIdx) {
    ...
    refAccValFloat = RefALU::min(superT(*xVal.GetAcc<TypeParam>()),
                                 superT(*yVal.GetAcc<TypeParam>()));
    EXPECT_TRUE(testVal.IsAcc() &&
                *testVal.GetAcc<TypeParam>() == (TypeParam)refAccValFloat);
    ...
    for (uint32_t i = 0; i < this->vectorWidth; ++i) {
      refAccValFloat = RefALU::min(superT(*x[i].GetAcc<TypeParam>()),
                                   superT(*y[i].GetAcc<TypeParam>()));
      EXPECT_TRUE(testVec[i].IsAcc() &&
                  *testVec[i].GetAcc<TypeParam>() == (TypeParam)refAccValFloat);
    }
...
static NEATValue max_ref(const NEATValue &xVal, const NEATValue &yVal) {
  ...
  if (intvOverlap)
    ...
    // should not be here
    throw Exception::IllegalFunctionCall(
        "max_ref is called with wrong arguments");
  return NEATValue(refOut);
}
...
TYPED_TEST(NEATAluTypedCommon, max) {
  ...
  for (int testIdx = 0; testIdx < this->NUM_TESTS; ++testIdx) {
    ...
    refAccValFloat = RefALU::max(superT(*xVal.GetAcc<TypeParam>()),
                                 superT(*yVal.GetAcc<TypeParam>()));
    EXPECT_TRUE(testVal.IsAcc() &&
                *testVal.GetAcc<TypeParam>() == (TypeParam)refAccValFloat);
    ...
    for (uint32_t i = 0; i < this->vectorWidth; ++i) {
      refAccValFloat = RefALU::max(superT(*x[i].GetAcc<TypeParam>()),
                                   superT(*y[i].GetAcc<TypeParam>()));
      EXPECT_TRUE(testVec[i].IsAcc() &&
                  *testVec[i].GetAcc<TypeParam>() == (TypeParam)refAccValFloat);
    }
...
template <typename T> class NEATDegRadTest {
public:
  // Number of different random inputs to test.
  ...
  void Test(NEATFuncP NEATFunc, NEATFuncVecP NEATFuncVec, RefFuncP RefFunc,
            float ulps) {
    // Test special NEAT values: UNKNOWN, UNWRITTEN and ANY.
    ...
    for (uint32_t testIdx = 0; testIdx < NUM_TESTS; ++testIdx) {
      ...
      EXPECT_TRUE(
          TestIntExpanded<sT>(refIntValMin, refIntValMax, testVal, ulps));
      ...
      for (uint32_t i = 0; i < vectorWidth; ++i) {
        ...
        EXPECT_TRUE(
            TestIntExpanded<sT>(refIntValMin, refIntValMax, testVec[i], ulps));
      }
      ...
      for (uint32_t i = 0; i < vectorWidth; ++i)
        interval[i] = NEATValue(Arg2Min[testIdx * vectorWidth + i],
                                Arg2Max[testIdx * vectorWidth + i]);
...
template <typename T> class NEATDegRadTestRun : public ALUTest {};
...
TYPED_TEST(NEATDegRadTestRun, radians) {
  ...
  NEATDegRadTest<TypeP> radiansTest(0.0,
                                    360.0); // from 0 degrees to 360 degrees

  radiansTest.Test(NEATFunc, NEATFuncVec, RefFunc,
                   float(NEATALU::RADIANS_ERROR));
}
...
TYPED_TEST(NEATDegRadTestRun, degrees) {
  ...
  NEATDegRadTest<TypeP> degreesTest(0.0,
                                    (2.0 * M_PIL)); // from 0 to 2*pi radians

  degreesTest.Test(NEATFunc, NEATFuncVec, RefFunc,
                   float(NEATALU::DEGREES_ERROR));
}

template <typename T> class NEATCommonTestOneArg : public ALUTest {
public:
  // Number of different random inputs to test.
  static const uint32_t NUM_TESTS = 500;
  // Vector data type length.
...
template <typename T> class NEATCommonTestThreeArgs : public ALUTest {
public:
  // Number of different random inputs to test.
  static const uint32_t NUM_TESTS = 500;
  // Vector data type length.
...
TYPED_TEST(NEATCommonTestOneArg, sign) {
  ...
  EXPECT_TRUE(TestPreciseRes<TypeP>(NEATALU::sign<TypeP>,
                                    Utils::GetNaN<TypeP>(), TypeP(0.0)));
  ...
  for (uint32_t testIdx = 0; testIdx < this->NUM_TESTS; ++testIdx) {
    ...
    EXPECT_TRUE(TestPreciseRes<TypeP>(NEATALU::sign<TypeP>,
                                      *x[0].GetAcc<TypeP>(), refAccValFloat));
...
template <typename T>
static void testSmoothstepAccResult(NEATValue edge0Val, NEATValue edge1Val,
                                    NEATValue xVal, NEATValue testVal) {
  typedef typename superT<T>::type sT;
  if (*edge0Val.GetAcc<T>() >= *edge1Val.GetAcc<T>()) {
    EXPECT_TRUE(testVal.IsAny());
  } else {
    sT refAcc = RefALU::flush(
        RefALU::smoothstep<sT>(RefALU::flush(sT(*edge0Val.GetAcc<T>())),
                               RefALU::flush(sT(*edge1Val.GetAcc<T>())),
                               RefALU::flush(sT(*xVal.GetAcc<T>()))));
    EXPECT_TRUE(NEATALU::castDown(refAcc) >= *testVal.GetMin<T>() &&
                NEATALU::castDown(refAcc) <= *testVal.GetMax<T>());
  }
}

template <typename T>
static void testSmoothstepIntervalResult(NEATValue edge0Val, NEATValue edge1Val,
                                         NEATValue xVal, NEATValue testVal) {
  typedef typename superT<T>::type sT;
  const int num = 50;
  ...
  if (*edge0Val.GetMax<T>() >= *edge1Val.GetMin<T>()) {
    ...
    GenerateRangedVectorsAutoSeed(dataTypeVal, &arrEdge0[0], V1, num,
                                  *edge0Val.GetMin<T>(), *edge0Val.GetMax<T>());
    GenerateRangedVectorsAutoSeed(dataTypeVal, &arrEdge1[0], V1, num,
                                  *edge1Val.GetMin<T>(), *edge1Val.GetMax<T>());
    GenerateRangedVectorsAutoSeed(dataTypeVal, &arrX[0], V1, num,
                                  *xVal.GetMin<T>(), *xVal.GetMax<T>());

    for (int i = 0; i < num; i++) {
      sT refVal = RefALU::flush(RefALU::smoothstep<sT>(
          RefALU::flush(sT(arrEdge0[i])), RefALU::flush(sT(arrEdge1[i])),
          RefALU::flush(sT(arrX[i]))));
      EXPECT_TRUE(NEATALU::castDown(refVal) >= *testVal.GetMin<T>() &&
                  NEATALU::castDown(refVal) <= *testVal.GetMax<T>());
    }
  }
}
