...
// !!! Default value for tolerance in accurate mode for comparison of floating
// point values.
...
public:
  ...
  ///
  /// @brief Compares actual output with reference using optionally NEAT
  /// information. Comparison type will be chosen depending on the input.
  /// Reference or NEAT pointers passed to Compare can be NULL, but not both. If
  /// Reference is NULL then NEAT structures will be used to validate actual
  /// output. Another option is to pass only reference and leave NEAT to be
  /// NULL. In this case precise comparison will be performed. You can also
  /// specify both NEAT and Reference values. Then firstly NEAT structures will
  /// be used to validate output. Afterwards reference values will be validated
  /// to fit within NEAT intervals and corresponding warnings will be reported.
  /// Validation of reference values will not influence output and test is
  /// supposed to be passed even if reference doesn't fit within NEAT intervals.
  ///
  ...
  /// @param  [in]  pRef      Reference output values. If NULL specified then
  /// pNEAT should be provided.
  /// @param  [in]  pNEAT     NEAT intervals. If NULL specified then pRef should
  /// be provided.
  /// @return     PASSED if comparison succeeded. NOT_PASSED if comparison
  /// failed.
  ...
  /// @brief tolerance in ULPs used in accurate comparison of floating point
  /// types
  ...
  /// @brief tolerance in ULPs used in accurate comparison of floating point
  /// types
  inline double GetULPTolerance() const { return m_ULP_TOLERANCE; }
...
protected:
  ...
  COMP_RESULT CompareBuffers(const Buffer *pAct, const Buffer *pRef,
                             const Buffer *pNEAT);

  /// @brief Performs comparison of images
  COMP_RESULT CompareImages(const Image *pImg, const Image *pImgRef,
  ...
  COMP_RESULT CompareNElements(const int8_t *pAct, const int8_t *pRef,
                               const NEATValue *pNEAT, const TypeDesc &typeDesc,
                               const std::size_t &N);
  ...
  inline void ReportMismatch(const T *pAct, const T *pRef,
    ...
    IComparisonResults::MismatchedVal val(ind, m_pMemObjDescCurrent.get(), pRef,
                                          pAct, pNeat, refRes, actRes);
  ...
  template <typename T>
  inline bool DetectAndCheckInfsNaNs(const T &Act, const T &Ref,
                                     COMP_RESULT &res) {
    IsFloatType<T> _notUsed;
    UNUSED_ARGUMENT(_notUsed);
    bool ret = true;

    if (Utils::IsInf<T>(Act) || Utils::IsInf<T>(Ref)) { // infinity detected
      // both are + INF
      ...
      // both are - INF
        ...
        res = NOT_PASSED;
    } else if (Utils::IsNaN<T>(Act) || Utils::IsNaN<T>(Ref)) { // check NaNs
      ...
      if (Utils::IsNaN<T>(Act) && Utils::IsNaN<T>(Ref))
        ...
        res = NOT_PASSED;
    } else { // no special values were detected
      ret = false;
    }
    return ret;
  }
  ...
  template <typename T>
  inline COMP_RESULT CompareScalarAccurateFloatingPoint(const T &Act,
                                                        const T &Ref) {
    IsFloatType<T> _notUsed;
    UNUSED_ARGUMENT(_notUsed);
    T _Act = Act, _Ref = Ref;
  ...
  template <typename T>
  inline COMP_RESULT CheckInterval(const T &intervalMin, const T &intervalMax,
                                   const T &val) {
  ...
  COMP_RESULT CompareNEAT(const T &actData, const NEATValue &in_vd) {
    ...
    if (NEATValue::UNKNOWN == st) {
      LLVM_DEBUG(llvm::dbgs()
                 << "[COMPARATOR WARNING] Value with UNKNOWN status found!!\n");
      return PASSED;
    }
    ...
    if (NEATValue::UNWRITTEN == st) {
      LLVM_DEBUG(
          llvm::dbgs()
          << "[COMPARATOR WARNING] Value with UNWRITTEN status found!!\n");
      return PASSED;
    }
    ...
    if (Utils::IsNaN(maxVal) || Utils::IsNaN(minVal)) {
      LLVM_DEBUG(llvm::dbgs()
                 << "[COMPARATOR WARNING] NEAT intervals contain NaN.\n");
      return PASSED;
    }
    /// Check infinities
    if ((Utils::IsNInf(minVal)) && (Utils::IsPInf(maxVal))) {
      LLVM_DEBUG(llvm::dbgs()
                 << "[COMPARATOR WARNING] NEAT interval is infinite.\n");
      return PASSED;
    }
    if (maxVal < minVal) {
      LLVM_DEBUG(llvm::dbgs()
                 << "[COMPARATOR WARNING] Neat interval is invalid.\n");
      return NOT_PASSED;
    }
    if (in_vd.GetStatus() ==
        NEATValue::ACCURATE) { // in accurate case use accurate comparison
...
private:
  ...
  /// general index into BuffferContainerList/BufferContainer/{Buffer,
  /// Image}/Element used during execution of comparison
...
template <>
inline COMP_RESULT
Comparator::CompareScalarAccurateFloatingPoint<CFloat16>(const CFloat16 &Act,
                                                         const CFloat16 &Ref) {
...
#define DEF_COMPARE_SCALAR_ACCURATE_INTEGER(__TYPE)                            \
  template <>                                                                  \
  inline COMP_RESULT Comparator::CompareScalarAccurate<__TYPE>(                \
      const __TYPE &Act, const __TYPE &Ref) {                                  \
...
template <typename T>
inline COMP_RESULT Comparator::CompareScalarNEAT(const T &Act,
                                                 const NEATValue &NEAT) {
  throw Exception::InvalidArgument(
      "CompareScalarNEAT instantiated for non floating point type");
}

// macro for specialization of CompareScalarNEAT function for floating point
// types
#define DEF_COMPARE_SCALAR_NEAT_IMPL(__TYPE)                                   \
  template <>                                                                  \
