...
Validation::COMP_RESULT
Comparator::Compare(IComparisonResults &res, const std::vector<bool> *pIgnore,
                    const IBufferContainerList &pActual,
                    const IBufferContainerList *pReference,
                    const IBufferContainerList *pNEAT) {
  // set object local pointer to IComparisonResluts used by ReportMismatch
  // function
  ...
  if (m_haveReference) {
    if (pActual.GetBufferContainerCount() !=
        pReference->GetBufferContainerCount())
      return NOT_PASSED;
  }
  ...
  for (uint32_t BCIndex = 0; BCIndex < pActual.GetBufferContainerCount();
       ++BCIndex) {
    // push BC index
    ...
    if (pIgnore && !(pIgnore->size() == count)) {
      throw Exception::InvalidArgument(
          "Comparator::CompareInternal invalid size of ignore list");
    }
    ...
    for (uint32_t bufInd = 0; bufInd < count; bufInd++) {
      ...
      const IMemoryObject *pMemObjRef =
          (m_haveReference) ? pBCRef->GetMemoryObject(bufInd) : NULL;
      // set flag if current memobject supports NEAT format
      // and NEAT should be used for this memobject
      m_IsNEATSupportedMemObj = (m_haveNEAT) ? pBCNEAT->GetMemoryObject(bufInd)
                                                   ->GetMemoryObjectDesc()
                                                   ->IsNEAT()
                                             : false;
      ...
      const IMemoryObject *pMemObjNEAT = (m_haveNEAT && m_IsNEATSupportedMemObj)
                                             ? pBCNEAT->GetMemoryObject(bufInd)
                                             : NULL;

      if (Image::GetImageName() == pMemObjAct->GetName()) { // Image object
        // obtain Image
        const Image *pImageAct = static_cast<const Image *>(pMemObjAct);
        // obtain Reference Image
        const Image *pImageRef =
            (m_haveReference) ? static_cast<const Image *>(pMemObjRef) : NULL;
        // obtain NEAT Image
        const Image *pImageNEAT = (m_IsNEATSupportedMemObj)
                                      ? static_cast<const Image *>(pMemObjNEAT)
                                      : NULL;
        ...
        if (NOT_PASSED == CompareImages(pImageAct, pImageRef, pImageNEAT))
          CompRes = NOT_PASSED;
      } else if (Buffer::GetBufferName() ==
                 pMemObjAct->GetName()) { // Buffer object
        // obtain actual buffer
        const Buffer *pBufAct = static_cast<const Buffer *>(pMemObjAct);
        // obtain Reference buffer
        const Buffer *pBufRef =
            (m_haveReference) ? static_cast<const Buffer *>(pMemObjRef) : NULL;
        // obtain NEAT Image
        const Buffer *pBufNEAT = (m_IsNEATSupportedMemObj)
                                     ? static_cast<const Buffer *>(pMemObjNEAT)
                                     : NULL;
        ...
        throw Exception::InvalidArgument(
            "Comparator::CompareInternal Unknown type of MemObj");
    }
    // check index stack
...
COMP_RESULT Comparator::CompareBuffers(const Buffer *pAct, const Buffer *pRef,
                                       const Buffer *pNEAT) {
  const BufferDesc *pActDesc =
      static_cast<const BufferDesc *>(pAct->GetMemoryObjectDesc());

  const BufferDesc *pRefDesc =
      (m_haveReference)
          ? static_cast<const BufferDesc *>(pRef->GetMemoryObjectDesc())
          : NULL;

  const BufferDesc *pNEATDesc =
      (m_IsNEATSupportedMemObj)
          ? static_cast<const BufferDesc *>(pNEAT->GetMemoryObjectDesc())
          : NULL;
  ...
  const int8_t *pRefData =
      (m_haveReference) ? (int8_t *)pRef->GetDataPtr() : NULL;
  // obtain pointer to NEAT data
  const NEATValue *pNEATData =
      (m_IsNEATSupportedMemObj) ? (NEATValue *)pNEAT->GetDataPtr() : NULL;

  // call comparator for N sequential elements
  return CompareNElements(pActData, pRefData, pNEATData,
                          pActDesc->GetElementDescription(),
                          pActDesc->NumOfElements());
}

Validation::COMP_RESULT
Comparator::CompareNElements(const int8_t *pAct, const int8_t *pRef,
                             const NEATValue *pNEAT, const TypeDesc &Type,
...
#define CASE_TYPE(__TYPEDESC, __TYPE)                                          \
  ...
  case TVECTOR: {
    ...
    for (std::size_t cntBufferElem = 0; cntBufferElem < NumElems;
         ++cntBufferElem) {
      ...
      // obtain pointer to ref data
      const int8_t *pRefData =
          (m_haveReference) ? pRef + cntBufferElem * BufElemSize : NULL;
      // obtain pointer to NEAT data
      const NEATValue *pNEATData =
          (m_IsNEATSupportedMemObj)
              ? reinterpret_cast<NEATValue *>(const_cast<uint8_t *>(
                    (const uint8_t *)pNEAT + cntBufferElem * BufElemNEATSize))
              : NULL; // NEATValue *
  ...
  case TSTRUCT: {
    ...
    for (std::size_t cntBufferElem = 0; cntBufferElem < NumElems;
      ...
      const int8_t *pNEATDataInt8 =
          (m_IsNEATSupportedMemObj)
              ? pNeatInt8 + cntBufferElem * sizeof(BufElemNEATSize)
              : NULL;

      // loop over struct members
      for (uint64_t cntStructElem = 0; cntStructElem < Type.GetNumOfSubTypes();
           ++cntStructElem) {
        ...
        const int8_t *pActS = pActData + subType.GetOffsetInStruct();
        const int8_t *pRefS =
            (m_haveReference) ? pRefData + subType.GetOffsetInStruct() : NULL;
        const NEATValue *pNEATS =
            (m_IsNEATSupportedMemObj)
                ? (const NEATValue *)(pNEATDataInt8 +
                                      TypeNEAT.GetSubTypeDesc(cntStructElem)
                                          .GetOffsetInStruct())
                : NULL;
  ...
  default:
    throw Exception::InvalidArgument(
        "Comparator can't process elements of the following data type: " +
        Type.TypeToString());
  }
...
template <typename T>
COMP_RESULT Comparator::CompareNScalarElements(const T *pAct, const T *pRef,
  ...
  for (std::size_t cntBufferElem = 0; cntBufferElem < NumElems;
    ...
    const NEATValue *pNEATElem =
        (m_IsNEATSupportedMemObj) ? pNEAT + cntBufferElem : NULL;
    ...
    if (m_IsNEATSupportedMemObj) {
      ...
      if (pRef && (NOT_PASSED == CompareScalarNEAT<T>(*pRefElem, *pNEATElem))) {
        localres = NOT_PASSED;
        refMissed = true;
      }
    } else {
...
Validation::COMP_RESULT Comparator::CompareImages(const Image *pImgAct,
                                                  const Image *pImgRef,
                                                  const Image *pImgNEAT) {
  COMP_RESULT toReturn = PASSED;

  const ImageDesc *pActDesc =
      static_cast<const ImageDesc *>(pImgAct->GetMemoryObjectDesc());

  const ImageDesc *pRefDesc =
      (m_haveReference)
          ? static_cast<const ImageDesc *>(pImgRef->GetMemoryObjectDesc())
          : NULL;

  const ImageDesc *pNEATDesc =
      (m_IsNEATSupportedMemObj)
          ? static_cast<const ImageDesc *>(pImgNEAT->GetMemoryObjectDesc())
          : NULL;
  ...
  // check descriptors in actual and NEAT
  if (m_IsNEATSupportedMemObj) {
    std::unique_ptr<ImageDesc> pNeatModifiedForSizeComparison(
        (ImageDesc *)pNEATDesc->Clone());
    pNeatModifiedForSizeComparison->SetNeat(false);
    if (m_IsNEATSupportedMemObj &&
        (*pActDesc != *pNeatModifiedForSizeComparison))
      return NOT_PASSED;
  }
  ...
  const bool IsArr2D =
      (pActDesc->GetImageType() == OpenCL_MEM_OBJECT_IMAGE2D_ARRAY);
  const bool IsArr1D =
      (pActDesc->GetImageType() == OpenCL_MEM_OBJECT_IMAGE1D_ARRAY);

  const int8_t *ptrBaseAct = (const int8_t *)pImgAct->GetDataPtr();
  const int8_t *ptrBaseRef =
      (m_haveReference) ? (const int8_t *)pImgRef->GetDataPtr() : NULL;
  const int8_t *ptrBaseNEAT =
      (m_IsNEATSupportedMemObj) ? (const int8_t *)pImgNEAT->GetDataPtr() : NULL;

  const size_t pixelSize = pActDesc->GetElementSize();
  const uint64_t Depth =
      Is3D ? pActDesc->GetSizesDesc().depth
           : (IsArr2D ? pActDesc->GetSizesDesc().array_size : 1);
  const uint64_t slicePitch =
      (Is3D || IsArr2D) ? pActDesc->GetSizesDesc().slice
                        : (IsArr2D ? pActDesc->GetSizesDesc().row : 0);
  const uint64_t Height =
      (Is2D || IsArr2D) ? pActDesc->GetSizesDesc().height
                        : (IsArr1D ? pActDesc->GetSizesDesc().array_size : 1);
  ...
  const ImageChannelDataTypeVal ImageChannelDataType =
      pActDesc->GetImageChannelDataType();
  const size_t pixelSizeNEAT =
      (m_IsNEATSupportedMemObj) ? pNEATDesc->GetElementSize() : 0;
  const uint64_t rowPitchNEAT =
      (m_IsNEATSupportedMemObj) ? pNEATDesc->GetSizesDesc().row : 0;
  const uint64_t slicePitchNEAT =
      (m_IsNEATSupportedMemObj) ? pNEATDesc->GetSizesDesc().slice : 0;

  // loop over pixels in image
  for (uint64_t z = 0; z < Depth; ++z) {
    if (Is3D || IsArr2D)
      m_indexStack.pushIdx(z);
    ...
    for (uint64_t y = 0; y < Height; ++y) {
      ...
      for (uint64_t x = 0; x < Width; ++x) {
        ...
        const int8_t *ptrAct =
            ptrBaseAct + z * slicePitch + y * rowPitch + x * pixelSize;
        const int8_t *ptrRef =
            (m_haveReference)
                ? ptrBaseRef + z * slicePitch + y * rowPitch + x * pixelSize
                : NULL;
        const NEATValue *ptrNEAT =
            (m_IsNEATSupportedMemObj)
                ? (const NEATValue *)(ptrBaseNEAT + z * slicePitchNEAT +
                                      y * rowPitchNEAT + x * pixelSizeNEAT)
                : NULL;
        ...
        case OpenCL_UNORM_SHORT_555: { // special case. Treat
                                       // OpenCL_UNORM_SHORT_555 as
          // one element. All channels are packed into one element.
          typedef ImageChannelDataTypeValToCType<OpenCL_UNORM_SHORT_555>::type
              CType;
          res = CompareNScalarElements<CType>((const CType *)ptrAct,
                                              (const CType *)ptrRef, ptrNEAT,
                                              1 // one channel
          );
          break;
        }
        case OpenCL_UNORM_SHORT_565: { // special case. Treat
                                       // OpenCL_UNORM_SHORT_565 as
          // one element. All channels are packed into one element.
        ...
        case OpenCL_UNORM_INT_101010: { // special case. Treat
                                        // OpenCL_UNORM_INT_101010 as
          // one element. All channels are packed into one element.
        ...
        // TODO: enable CFloat16 when Validation::Utils function support
        // CFloat16
