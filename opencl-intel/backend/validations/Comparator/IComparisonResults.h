...
public:
  ...
  /// @param  [in]  index                 Indexes of mismatched value in
  /// containers(BufferContainer index, Buffer index, Vector index, element
  /// index)
  /// @param  [in]  bufferDesc            Description of the buffer containing
  /// the mismatched element
  /// @param  [in]  referenceDataPtr      Reference element
  /// @param  [in]  actualDataPtr         Actual element
  /// @param  [in]  neat                  Neat interval for element ('Accurate'
  ...
  struct Index {
    ...
    void pushIdx(uint32_t idx) { m_stack.push_back(idx); }
    void popIdx() {
      assert(!m_stack.empty());
      m_stack.pop_back();
    }
    uint32_t GetBufId() const { return m_stack.at(BUF_IDX); }
  ...
  struct NEATValueContainer {
    ...
    template <typename T> NEATValueContainer(T in_val) { SetVal<T>(in_val); }

    template <typename T> void SetVal(const NEATValue &in_val) {
      val = in_val;
      m_str = in_val.ToString<T>();
      isValid = true;
    }

    NEATValue GetVal() { return val; }
  ...
  struct ValueContainer {
    ...
    template <typename T> ValueContainer(T in_val) { SetVal<T>(in_val); }
    ...
    template <typename T> void SetVal(const T &in_val) {
...
// TODO: delete MaxBytes = sizeof(long double);
  ...
  /// Mismatch information will be stored as the list of the following
  /// structures
  struct MismatchedVal {
    template <typename T>
    MismatchedVal(const Index &in_index, const IMemoryObjectDesc *in_pDesc,
                  const T *in_referenceDataPtr, const T *in_actualDataPtr,
                  const NEATValue *in_pNEAT, bool actMissed, bool refMissed) {
      pDesc = in_pDesc;
    ...
    template <typename T> void SetReference(T val) { ref.SetVal(val); }

    template <typename T> void SetActual(T val) { act.SetVal(val); }

    template <typename T> void SetNEAT(NEATValue val) { neat.SetVal<T>(val); }
    ...
    std::string ToString() const {
      ...
      if (ref.IsValid())
        ss << "; Reference "
           << (neat.IsValid() ? (m_refMissed ? "(Fail, outside NEAT) = "
                                             : "(Ok, inside NEAT) = ")
                              : " ")
           << ref.ToString();
      if (act.IsValid())
        ss << "; Actual "
           << (neat.IsValid() ? (m_actMissed ? "(Fail, outside NEAT) = "
                                             : "(Ok, inside NEAT) = ")
                              : " ")
           << act.ToString();
      ...
      if (neat.IsValid()) {
        ...
        if (m_refMissed)
          ss << "; SATest is not able to produce correct reference results; a "
                "bug in reference compiler/NEAT is supposed.\n";
      }
      return ss.str();
    }
    ...
    double ComputeDiff(ValueContainer ref, ValueContainer act) const {
      if ((!ref.IsValid()) || (!act.IsValid()))
        throw Exception::InvalidArgument(
            "Input value container was invalid during comparator mismatch "
            "reporting");
    ...
    int GetBufIdx() const { return idx.GetBufId(); }

    const IMemoryObjectDesc *GetDesc() const { return pDesc.get(); }
    ...
    TypeVal GetTypeVal() const {
      ...
      if ("BufferDesc" == GetDesc()->GetName()) {
        ...
        if (typeDesc.IsAggregate()) { // aggregate type
          if (typeVal == TVECTOR || typeVal == TARRAY) {
            ret = typeDesc.GetSubTypeDesc(0).GetType();
          } else if (typeVal == TSTRUCT) {
            // std::cout << "Mismatches in struct data type is not supported
            // yet.";
            ret = TSTRUCT;
          } else {
            throw Exception::NotImplemented(
                "this Aggregate type is not implemented");
          }
        } else
          ret = typeVal; // scalar type
      } else if (ImageDesc::GetImageDescName() == GetDesc()->GetName()) {
        const ImageDesc *pImgDesc = static_cast<const ImageDesc *>(GetDesc());
        ret =
            ImageChannelDataTypeToTypeVal(pImgDesc->GetImageChannelDataType());
      } else
        throw Exception::InvalidArgument("Unsupported descriptor");
...
template <>
inline double IComparisonResults::MismatchedVal::ComputeDiff<float>(
    ValueContainer ref, ValueContainer act) const {
  double res =
      Utils::ulpsDiffSamePrecision(ref.GetVal<float>(), act.GetVal<float>());
  return res;
}

template <>
inline double IComparisonResults::MismatchedVal::ComputeDiff<double>(
    ValueContainer ref, ValueContainer act) const {
  double res =
      Utils::ulpsDiffSamePrecision(ref.GetVal<double>(), act.GetVal<double>());
  return res;
}

template <>
inline double IComparisonResults::MismatchedVal::ComputeDiff<CFloat16>(
    ValueContainer ref, ValueContainer act) const {
  float res = Utils::ulpsDiffSamePrecision((float)ref.GetVal<CFloat16>(),
                                           (float)act.GetVal<CFloat16>());
  return res;
}
