...
#define VEC4           4
#define VEC8           8
#define NUM_OF_VALUES  8               //Quantity of values that we review and therefore the size of the array
#define D1            (1.234567)
...
bool opencl_mul_add_calc( T * a, T * b, T * c, T * results, std::string *program_source, int vec_size )
{
  ...
  if( err ) return false;
  clFinish( cmd_queue );
  err = clEnqueueReadBuffer( cmd_queue, r_mem, CL_TRUE, 0, buffer_size,results, 0, NULL, NULL );
  if( err ) return false;
  clFinish( cmd_queue );
...
bool FMAtest_scalar_floatA(){
  std::string test_program_scalar_float = "__kernel void mul_add (__global float *a,  __global float *b, __global float *c, __global float *result) { int gid = get_global_id(0); result[gid] = (a[gid] * b[gid])+c[gid];}" ;
...
bool FMAtest_scalar_doubleA(){
  std::string test_program_scalar_double= "__kernel void mul_add (__global double *a,  __global double *b, __global double *c, __global double *result) { int gid = get_global_id(0); result[gid] = (a[gid] * b[gid])+c[gid];}";
...
bool FMAtest_v8_floatA(){
  std::string test_program_v8_float= "__kernel void mul_add (__global float8 *a,  __global float8 *b, __global float8 *c, __global float8 *result) { int gid = get_global_id(0); result[gid] = (a[gid] * b[gid])+c[gid];}" ;
...
bool FMAtest_v4_doubleA(){
  std::string test_program_v4_double= "__kernel void mul_add (__global double4 *a,  __global double4 *b, __global double4 *c, __global double4 *result) { int gid = get_global_id(0); result[gid] = (a[gid] * b[gid])+c[gid];}" ;
