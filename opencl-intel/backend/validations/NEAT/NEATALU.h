...
/******************************************************************************\
    BIs with _frm suffix like func_frm is the fast-relaxed-math version of the
same BI like func. If fast-relaxed-math enabled and specified cl version is 2.0
then func_frm will be called instead of func in the single kernel. func_frm
differ from func in result accuracy(for detailed info see section 7.4)
\******************************************************************************/
...
template <> class superT<float> {
public:
  typedef double type;
};

template <> class superT<double> {
public:
  typedef long double type;
};

template <typename T> class downT;

template <> class downT<double> {
public:
  typedef float type;
};

template <> class downT<long double> {
public:
  typedef double type;
};

// Wrapper class to specialize that float and double values are represented in
// NEAT by NEATValue. The rest of the types are treated as they are.
template <class T> class NEATType {
public:
  typedef T type;
};
...
typedef NEATValue (*NEATScalarTernaryOp)(const NEATValue &, const NEATValue &,
                                         const NEATValue &);
...
/// This class is similar to the ref ALU (refalu.cpp), only it works on
/// NEATValues and NEATVectors The results of its operations can store either
/// the range of allowed values or vector of value ranges.
class NEATALU {
public:
  /// @brief the maximal error (rel or abs) in various inaccurate operations
  static const int ADD_ERROR = 0;
  static const int ACOS_ERROR = 4;
  static const int ACOSPI_ERROR = 5;
  static const int ASINPI_ERROR = 5;
  static const int ASIN_ERROR = 4;
  static const int ATAN_ERROR = 5;
  static const int ATAN2_ERROR = 6;
  static const int ATAN2PI_ERROR = 6;
  static const int ATANPI_ERROR = 5;
  ...
  static const int COSH_ERROR = 4;
  static const int COSPI_ERROR = 4;
  static const int TAN_ERROR = 5;
  static const int TANH_ERROR = 5;
  ...
  static const int SINH_ERROR = 4;
  static const int SINPI_ERROR = 4;
  static const int DIV_ERROR_DOUBLE =
      0;                         // division is correctly rounded for doubles
  static const double DIV_ERROR; // = 2.5f
  static const double
      NORMALIZE_ERROR; // 2f + 0.5f  error in rsqrt + error in multiply
  ...
  static const double
      SQRT_ERROR_DOUBLE; // sqrt is correctly rounded for doubles
...
#define NATIVE_FUNCTIONS_ERROR_DOUBLE 134217728 // 2^27
  ...
  static const long double divpiby_180;
...
private:
  ...
  /// element of vector it calls NEATValueFunction f, that works with pair of
  /// NEAT values.
  /// @param [in]     val    NEATValue to take as first parameter of
  /// NEATValueFunction
  /// @param [in]     vec    Vector of elements to take as second parameter of
  /// NEATValueFunction
  /// @param [in]     NEATScalarBinaryOp  Binary operation function pointer that
  /// takes two NEAT values as an input
  /// @return         Resulting vector with the same width as for vec
  /// @throws InvalidArgument
  static NEATVector processVector(const NEATValue &val, const NEATVector &vec,
                                  NEATScalarBinaryOp f);

  /// @brief Goes through input vectors vec1, vec2 and vec3 for each
  /// three elements it calls NEATValueFunction f, that works with three NEAT
  /// values.
  /// @param [in]     vec1    Vector of elements to take as first parameter of
  /// NEATValueFunction
  /// @param [in]     vec2    Vector of elements to take as second parameter of
  /// NEATValueFunction
  /// @param [in]     vec3    Vector of elements to take as third parameter of
  /// NEATValueFunction
  /// @param [in]     NEATScalarTernaryOp  Ternary operation function pointer
  /// that takes three NEAT values as an input
  /// @return         Resulting vector with the same width as for vec1, vec2 and
  /// vec3
  /// @throws InvalidArgument
  static NEATVector processVector(const NEATVector &vec1,
  ...
  /// pair of elements it calls NEATValueFunction f, that works with pair of
  /// NEAT values.
  /// @param [in]     vec1    Vector of elements to take as first parameter of
  /// NEATValueFunction
  /// @param [in]     vec2    Vector of elements to take as second parameter of
  /// NEATValueFunction
  /// @param [in]     val     NEATValue to take as third parameter of
  /// NEATValueFunction
  /// @param [in]     NEATScalarTernaryOp  Ternary operation function pointer
  /// that takes three NEAT values as an input
  ...
  /// @param [in]     vec1    Vector of elements to take as first parameter of
  /// NEATValueFunction
  /// @param [in]     val2    Value to take as second parameter of
  /// NEATValueFunction
  /// @param [in]     val3    Value to take as third parameter of
  /// NEATValueFunction
  /// @param [in]     NEATScalarTernaryOp  Ternary operation function pointer
  /// that takes two NEAT values as an input
  /// @return         Resulting vector with the same width as for vec1
  /// @throws InvalidArgument
  static NEATVector processVector(const NEATVector &vec1, const NEATValue &val2,
                                  const NEATValue &val3, NEATScalarTernaryOp f);
  ...
  /// @return         Resulting vector with the same width as for vec
  /// @throws InvalidArgument
  static NEATVector processVector(const NEATVector &vec, const NEATValue &val,
                                  NEATScalarBinaryOp f);
  ...
  /// @throws InvalidArgument
  static NEATVector processVector(const NEATVector &vec1,
                                  const NEATVector &vec2, NEATScalarBinaryOp f);

  /// @brief Goes through input vector vec and calls for each
  /// element NEATValueFunction f, that works with single NEAT value.
  /// @param [in]     vec    Vector of elements to take as parameter of
  /// NEATValueFunction
  /// @param [in]     NEATScalarUnaryOp  Unary operation function pointer that
  /// takes single NEAT value as an input
  /// @return         Resulting vector with the same width as for vec
  ...
  /// @param [in]     vals    Contains values that should be combined to one
  /// interval
  ...
  static NEATValue::Status Combine(T vals[], size_t num, T &minOut, T &maxOut) {
    ...
    // we want -0.0 to be smaller than +0.0, so we use our internal methods (and
    // not the ALU's methods)
    T min = vals[0];
    T max = vals[0];

    for (uint32_t i = 1; i < num; i++) {
      if (Utils::lt<T>(vals[i], min))
        min = vals[i];
      if (Utils::lt<T>(max, vals[i]))
        max = vals[i];
    }
  ...
  static double roundingUlps(double ulps) { return (ulps - 0.5); }

  static bool IsCmpOrdered(CmpType type) {
    if ((type == CMP_OEQ) || (type == CMP_OGE) || (type == CMP_OGT) ||
        (type == CMP_OLE) || (type == CMP_OLT) || (type == CMP_ONE) ||
        (type == CMP_ORD))
      return true;
    else
      return false;
  ...
  static NEATValue eq(const NEATValue &a, const NEATValue &b) {
    ...
    if ((a.GetStatus() == NEATValue::ACCURATE) &&
        (b.GetStatus() == NEATValue::ACCURATE) &&
        (*a.GetAcc<T>() == *b.GetAcc<T>())) {
      return NEATValue(true);
    } else if (RefALU::lt(*a.GetMax<T>(), *b.GetMin<T>()) ||
               RefALU::lt(*b.GetMax<T>(), *a.GetMin<T>())) {
  ...
  template <typename T>
  static bool PowEdgePowerAcc(const T &xMin, const T &xMax, const T &y,
                              NEATValue &res) {
    if (Utils::IsNInf(y)) {
      /// [C99] 9
  ...
  template <typename T>
  static bool PowEdgeBaseAcc(const T &x, const T &yMin, const T &yMax,
                             NEATValue &res) {
    /// C99
  ...
  template <typename T>
  static bool PowEdgeCases(const T &xMin, const T &xMax, const T &yMin,
                           const T &yMax, NEATValue &res) {
    /// x is accurate
...
public:
  static double castDown(long double a) { return (double)(a); }
  static float castDown(double a) { return (float)(a); }
  static CFloat16 castDown(float a) { return (CFloat16)(a); }

  template <typename T> static double sqrtSetUlps(void);
  template <typename T> static double divSetUlps(void);

  /// @brief Converts integer value of type IntT to NEATValue that contains
  /// converted accurate value of type FloatT.
  /// @param  [in]    intVal  Integer that should be converted to floating point
  /// value.
  ...
  static NEATVector ToFloat(std::vector<IntT> intVals) {
    ...
    IsIntegerType<IntT> __notUsed;
    UNUSED_ARGUMENT(__notUsed);
    const VectorWidth vWidths[MAX_VECTOR_WIDTH + 1] = {INVALID_WIDTH,
                                                       V1,
                                                       V2,
                                                       V3,
                                                       V4,
  ...
  template <typename T> static void JustAddUlps(T *InOut, double ulps) {
  ...
  template <typename T>
  static NEATValue
  CreateNEATValue(T vals[], const int valsCount,
                  IntervalError<typename downT<T>::type> ulps) {
    typedef typename downT<T>::type DownT;
    T min = T(0), max = T(0);
    NEATValue res;
  ...
  static NEATValue add(const NEATValue &a, const NEATValue &b) {
    ...
    // Therefore we can suppose that C++ add handles infinities and NaNs
    // correctly No need in high precision also as add result is correctly
    // rounded
  ...
  static NEATValue mul(const NEATValue &a, const NEATValue &b) {
    ...
    // Assume that compiler is C99 compliant and edge cases will be handled
    // correctly
  ...
  // Back-end may generates fma instruction at stage of code generation, so in
  // this case NEATALU has no information about was fma instruction generated or
  // just common multiplication was used. in order to provide NEAT support for
  // it, mul_fma function perform performs multiplication in high precision and
  // adds 1 ULP to result. the using of mul_fma is turned on by --fma-neat
  // command line parameter of SATest
  template <typename T>
  ...
  /// @param [in] divisor
  template <typename T>
  static NEATVector native_divide(const NEATVector &vec1,
                                  const NEATVector &vec2) {
  ...
  static NEATValue half_divide(const NEATValue &a, const NEATValue &b) {
    return InternalDiv<T>(a, b, T(NEATALU::HALF_DIVIDE_ERROR));
  }

  template <typename T>
  static NEATVector half_divide(const NEATVector &vec1,
                                const NEATVector &vec2) {
  ...
  template <typename T> static NEATValue native_recip(const NEATValue &a) {
    return InternalRecip<T>(a, double(NEATALU::NATIVE_RECIP_ERROR));
  }

  template <typename T> static NEATVector native_recip(const NEATVector &vec) {
    return processVector(vec, native_recip<T>);
  }

  template <typename T> static NEATValue half_recip(const NEATValue &a) {
    return InternalRecip<T>(a, double(NEATALU::HALF_RECIP_ERROR));
  }

  template <typename T> static NEATVector half_recip(const NEATVector &vec) {
    return processVector(vec, half_recip<T>);
  }

  template <typename T> static NEATValue neg(const NEATValue &a) {
  ...
  static NEATValue fmod(const NEATValue &dividend, const NEATValue &divisor) {
    ...
    if (dividend.Includes((T)-0.0)) {
      ...
      T bounds[4] = {*positiveRes.GetMin<T>(), *positiveRes.GetMax<T>(),
                     -*negativeRes.GetMin<T>(), -*negativeRes.GetMax<T>()};
      NEATValue preResult;
      T ansMin = (T)0.0, ansMax = (T)0.0;
      preResult.SetStatus(Combine(bounds, 4, ansMin, ansMax));
      if (preResult.GetStatus() == NEATValue::ACCURATE)
        preResult.SetAccurateVal(
            ansMin); // Doesn't really matter use min or max
    ...
    // check if divisor interval is negative. Different signs of interval
    // boundaries were handled earlier
    ...
    // If length of dividend interval exceeds max divisor value then result is
    // widest interval - from zero to max divisor.
    ...
    if ((minRem1 < maxRem1) || (minRem1 == maxRem1)) {
      ...
      // 1. "reminder loop" - answer is one interval from 0.0 to max value (not
      // equal to maxDivisor)
      // 2. single interval - reminder value should fit an interval from minDiv1
      // to maxDiv2
      // 3. two intervals - for smaller minDivisor we have smaller reminder, but
      // not greater than minDiv1. Answer is unknown in this case.
  ...
  template <typename T> static T round2nearestEven(const T &x) {
    if (x < T(0.0))
  ...
  static NEATValue remainder(const NEATValue &x, const NEATValue &y) {
    ...
    if (x.IsNaN<T>() || y.IsNaN<T>() ||
        (y.IsAcc() && (Utils::eq<T>(*y.GetAcc<T>(), T(0.0)) ||
    ...
    if (divisor.IsAcc() && dividend.IsAcc())
      return NEATValue(
          RefALU::remainder<T>(*dividend.GetAcc<T>(), *divisor.GetAcc<T>()));
    ...
    // widest interval - from zero to max divisor.
    if (maxDividend - minDividend >= maxDivisor) {
      return NEATValue(-RefALU::fabs(maxDivisor / T(2.0)),
                       RefALU::fabs(maxDivisor / T(2.0)));
    }
    ...
    if ((minRem1 < maxRem1) || (minRem1 == maxRem1)) {
      ...
      if (Utils::IsInf<T>(maxDiv1) && Utils::IsInf<T>(maxDiv2))
        ...
        if (maxDivisor > maxDividend)
          toReturn =
              NEATValue(-RefALU::fabs(maxDividend), RefALU::fabs(maxDividend));
        else
          // case 1
          toReturn = NEATValue(-RefALU::fabs((maxDividend / maxDiv1) / T(2.0)),
                               RefALU::fabs((maxDividend / maxDiv1) / T(2.0)));
      } else if ((maxRem2 >= minRem1) || (maxRem2 == 0)) {
        // case 2
        toReturn = NEATValue(minRem1, maxRem2);
      }
    ...
    // in this case remainder diagram has a break
      ...
      if ((maxDiv2 >= maxDiv1 + 1) ||
          ((maxDiv1 == maxDiv2) && ((maxRem2 > minRem1) || (maxRem2 == 0)))) {
        toReturn = NEATValue(-RefALU::fabs(maxDivisor / T(2.0)),
                             RefALU::fabs(maxDivisor / T(2.0)));
      }
    }
  ...
  template <typename T>
  static NEATValue remquo(const NEATValue &x, const NEATValue &y,
                          int32_t *quo) {
    if (CheckAUU(x) || CheckAUU(y))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T>
  static NEATVector remquo(const NEATVector &vecX, const NEATVector &vecY,
  ...
  template <typename T>
  static NEATValue fcmp(const NEATValue &a, const NEATValue &b,
                        CmpType comparison) {
  ...
  // names like IS#name# were chosen to avoid conflict with defines "isnan" and
  // "isinf" included from conformance and to keep names of relational functions
  // looks similar
  template <typename T>
    ...
    if (CheckAUU(res1) || CheckAUU(res1))
      ...
      if (res1.IsInterval() || res2.IsInterval())
        throw Exception::NEATTrackFailure(
            "[NEATALU::ISlessgreater] Only ACCURATE NEAT value is supported\n");
      else {
        return NEATValue(*res1.GetAcc<bool>() || *res2.GetAcc<bool>());
      }
    }
  }
  template <typename T> static NEATValue ISfinite(const NEATValue &a) {
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
    if (a.IsFinite<T>())
      return NEATValue(true);
  ...
  template <typename T> static NEATValue ISinf(const NEATValue &a) {
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
    if (a.IsAcc()) {
      T tmp;
    ...
    T min = *a.GetMin<T>();
    T max = *a.GetMax<T>();
    if (Utils::IsPInf(min) || Utils::IsPInf(max) || Utils::IsNInf(min) ||
        Utils::IsNInf(max))
      return NEATValue(NEATValue::UNKNOWN);
    else
      return NEATValue(false);
  }
  template <typename T> static NEATValue ISnan(const NEATValue &a) {
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
    if (a.IsNaN<T>())
      return NEATValue(true);
  ...
  template <typename T> static NEATValue ISnormal(const NEATValue &a) {
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
    T min = *a.GetMin<T>();
    T max = *a.GetMax<T>();
    if (RefALU::isNormal<T>(min) && RefALU::isNormal<T>(max) &&
        RefALU::isNormal<T>(min) && RefALU::isNormal<T>(max))
      return NEATValue(true);
    else
      return NEATValue(false);
  }
  ...
  static NEATValue ISordered(const NEATValue &a, const NEATValue &b) {
    ...
    if (res1.IsInterval() || res2.IsInterval())
      throw Exception::NEATTrackFailure(
          "[NEATALU::ISordered] Only ACCURATE NEAT value is supported\n");
    else {
      return NEATValue(*res1.GetAcc<bool>() || *res2.GetAcc<bool>());
    }
  }
  ...
  template <typename T> static NEATValue signbit(const NEATValue &a) {
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T>
  static NEATValue bitselect(const NEATValue &a, const NEATValue &b,
                             const NEATValue &c) {
    if (CheckAUU(a) || CheckAUU(b) || CheckAUU(c))
      return NEATValue(NEATValue::UNKNOWN);
    if (a.IsAcc() && b.IsAcc() && c.IsAcc())
      return NEATValue(
          localBitselect(*a.GetAcc<T>(), *b.GetAcc<T>(), *c.GetAcc<T>()));
    else
      return NEATValue(NEATValue::UNKNOWN);
  }
  template <typename T>
  static NEATVector bitselect(const NEATVector &vec0, const NEATVector &vec1,
                              const NEATVector &vec2) {
    return processVector(vec0, vec1, vec2, bitselect<T>);
  }

  template <typename T>
  static NEATVector processVectorRelational(const NEATVector &vec1,
                                            const NEATVector &vec2,
                                            NEATScalarBinaryOp f) {
    ...
    for (uint32_t i = 0; i < vec1.GetSize(); i++) {
      ...
      if (res.IsAcc()) {
        ...
        throw Exception::InvalidArgument(
            "Only ACCURATE and UNKNOWN NEAT values are supported");
    }
    return toReturn;
  }

  template <typename T>
  static NEATVector processVectorRelational(const NEATVector &vec,
                                            NEATScalarUnaryOp f) {
  ...
  template <typename T>
  static NEATVector ISgreaterequal(const NEATVector &vec1,
                                   const NEATVector &vec2) {
  ...
  template <typename T>
  static NEATVector ISlessequal(const NEATVector &vec1,
                                const NEATVector &vec2) {
    return processVectorRelational<T>(vec1, vec2, ISlessequal<T>);
  }
  template <typename T>
  static NEATVector ISlessgreater(const NEATVector &vec1,
                                  const NEATVector &vec2) {
    return processVectorRelational<T>(vec1, vec2, ISlessgreater<T>);
  }
  template <typename T> static NEATVector ISfinite(const NEATVector &vec) {
    return processVectorRelational<T>(vec, ISfinite<T>);
  }
  template <typename T> static NEATVector ISinf(const NEATVector &vec) {
    return processVectorRelational<T>(vec, ISinf<T>);
  }
  template <typename T> static NEATVector ISnan(const NEATVector &vec) {
    return processVectorRelational<T>(vec, ISnan<T>);
  }
  template <typename T> static NEATVector ISnormal(const NEATVector &vec) {
  ...
  template <typename T>
  static NEATVector ISunordered(const NEATVector &vec1,
                                const NEATVector &vec2) {
    return processVectorRelational<T>(vec1, vec2, ISunordered<T>);
  }
  template <typename T> static NEATVector signbit(const NEATVector &vec) {
  ...
  template <typename T> static NEATValue flush(const NEATValue &a) {
    if (a.IsUnknown())
      return NEATValue(NEATValue::UNKNOWN);
    if (a.IsAny())
      return NEATValue(NEATValue::ANY);
  ...
  template <typename T> static NEATValue sin(const NEATValue &a) {
    double ulps = double(SIN_ERROR);
    return InternalSin<T>(a, IntervalError<T>(ulps));
  }

  template <typename T> static NEATVector sin(const NEATVector &vec) {
    return processVector(vec, sin<T>);
  }

  template <typename T> static NEATValue sin_frm(const NEATValue &a) {
    IntervalError<T> ulps = IntervalError<T>::sinFrmSetUlps(a);
    return InternalSin<T>(a, ulps);
  }

  template <typename T> static NEATVector sin_frm(const NEATVector &vec) {
    return processVector(vec, sin_frm<T>);
  }

  template <typename T> static NEATValue native_sin(const NEATValue &a) {
    double ulps = double(NATIVE_SIN_ERROR);
    return InternalSin<T>(a, IntervalError<T>(ulps));
  }

  template <typename T> static NEATVector native_sin(const NEATVector &vec) {
    return processVector(vec, native_sin<T>);
  }

  template <typename T> static NEATValue half_sin(const NEATValue &a) {
    double ulps = double(HALF_SIN_ERROR);
    return InternalSin<T>(a, IntervalError<T>(ulps));
  }

  template <typename T> static NEATVector half_sin(const NEATVector &vec) {
    return processVector(vec, half_sin<T>);
  }

  template <typename T> static NEATValue cos(const NEATValue &a) {
    double ulps = double(COS_ERROR);
    return InternalCos<T>(a, IntervalError<T>(ulps));
  }

  template <typename T> static NEATVector cos(const NEATVector &vec) {
    return processVector(vec, cos<T>);
  }

  template <typename T> static NEATValue cos_frm(const NEATValue &a) {
    IntervalError<T> ulps = IntervalError<T>::cosFrmSetUlps(a);
    return InternalCos<T>(a, ulps);
  }

  template <typename T> static NEATVector cos_frm(const NEATVector &vec) {
    return processVector(vec, cos_frm<T>);
  }

  template <typename T> static NEATValue native_cos(const NEATValue &a) {
    double ulps = double(NATIVE_COS_ERROR);
    return InternalCos<T>(a, IntervalError<T>(ulps));
  }

  template <typename T> static NEATVector native_cos(const NEATVector &vec) {
    return processVector(vec, native_cos<T>);
  }

  template <typename T> static NEATValue half_cos(const NEATValue &a) {
    double ulps = double(HALF_COS_ERROR);
    return InternalCos<T>(a, IntervalError<T>(ulps));
  }

  template <typename T> static NEATVector half_cos(const NEATVector &vec) {
  ...
  template <typename T> static NEATValue tan(const NEATValue &a) {
    return InternalTan<T>(a, TAN_ERROR);
  }

  template <typename T> static NEATVector tan(const NEATVector &vec) {
    return processVector(vec, tan<T>);
  }

  template <typename T> static NEATValue tan_frm(const NEATValue &a) {
    return mul<T>(sin_frm<T>(a), div_frm<T>(NEATValue(T(1.0f)), cos_frm<T>(a)));
  }

  template <typename T> static NEATVector tan_frm(const NEATVector &vec) {
    return processVector(vec, tan_frm<T>);
  }

  template <typename T> static NEATValue native_tan(const NEATValue &a) {
    return InternalTan<T>(a, NATIVE_TAN_ERROR);
  }

  template <typename T> static NEATVector native_tan(const NEATVector &vec) {
    return processVector(vec, native_tan<T>);
  }

  template <typename T> static NEATValue half_tan(const NEATValue &a) {
    return InternalTan<T>(a, HALF_TAN_ERROR);
  }

  template <typename T> static NEATVector half_tan(const NEATVector &vec) {
    return processVector(vec, half_tan<T>);
  }

  template <typename T> static NEATValue asin(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // check for ANY,UNWRITTEN,UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T> static NEATVector asin(const NEATVector &vec) {
    return processVector(vec, asin<T>);
  }

  template <typename T> static NEATValue acos(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // check for ANY,UNWRITTEN,UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T> static NEATVector acos(const NEATVector &vec) {
    return processVector(vec, acos<T>);
  }

  template <typename T> static NEATValue atan(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // check for ANY,UNWRITTEN,UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T> static NEATVector atan(const NEATVector &vec) {
    return processVector(vec, atan<T>);
  }

  template <typename T> static NEATValue sinh(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // check for ANY,UNWRITTEN,UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T> static NEATVector sinh(const NEATVector &vec) {
    return processVector(vec, sinh<T>);
  }

  template <typename T> static NEATValue cosh(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // check for ANY,UNWRITTEN,UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T> static NEATVector cosh(const NEATVector &vec) {
    return processVector(vec, cosh<T>);
  }

  template <typename T> static NEATValue tanh(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // check for ANY,UNWRITTEN,UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T> static NEATVector tanh(const NEATVector &vec) {
    return processVector(vec, tanh<T>);
  }

  template <typename T> static NEATValue asinpi(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // check for ANY,UNWRITTEN,UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T> static NEATVector asinpi(const NEATVector &vec) {
    return processVector(vec, asinpi<T>);
  }

  template <typename T> static NEATValue acospi(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // check for ANY,UNWRITTEN,UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
    ...
    // combine results, expand interval and cast down
    return CreateNEATValue(resSuperT, 2,
                           IntervalError<T>(NEATALU::ACOSPI_ERROR));
  }

  template <typename T> static NEATVector acospi(const NEATVector &vec) {
    return processVector(vec, acospi<T>);
  }

  template <typename T> static NEATValue atanpi(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // check for ANY,UNWRITTEN,UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
    ...
    // combine results, expand interval and cast down
    NEATValue res =
        CreateNEATValue(resSuperT, 2, IntervalError<T>(ATANPI_ERROR));

    bool hasMinLimit = (resSuperT[0] == (SuperT)(-0.5));
    bool hasMaxLimit = (resSuperT[1] == (SuperT)(0.5));

    return NEATValue(hasMinLimit ? (T)-0.5 : *res.GetMin<T>(),
                     hasMaxLimit ? (T)0.5 : *res.GetMax<T>());
  }

  template <typename T> static NEATVector atanpi(const NEATVector &vec) {
    return processVector(vec, atanpi<T>);
  }

  template <typename T> static NEATValue sinpi(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    ...
    if (!flushed.IsAcc()) {
      ...
      // diff = max - min
      T diff =
          RefALU::add(*flushed.GetMax<T>(), RefALU::neg(*flushed.GetMin<T>()));
      ...
      if (Utils::ge(diff, (T)2.0)) {
        ...
        T offStart = RefALU::mul<T>(
            RefALU::frc<T>(RefALU::div<T>(*flushed.GetMin<T>(), (T)2.0)),
            (T)2.0);
    ...
    return NEATValue(hasMinPoint ? (T)-1.0 : *res.GetMin<T>(),
                     hasMaxPoint ? (T) + 1.0 : *res.GetMax<T>());
  }

  template <typename T> static NEATVector sinpi(const NEATVector &vec) {
    return processVector(vec, sinpi<T>);
  }

  template <typename T> static NEATValue cospi(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    ...
    if (!flushed.IsAcc()) {
      ...
      if (Utils::ge(diff, (T)2.0)) {
        ...
        // offStart is:  min % (2*pi)
        T offStart =
            RefALU::mul<T>(RefALU::frc<T>(RefALU::div<T>(min, (T)2.0)), (T)2.0);
        T offEnd = RefALU::add<T>(offStart, diff);
  ...
  template <typename T> static NEATVector cospi(const NEATVector &vec) {
  ...
  template <typename T> static NEATValue tanpi(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // check for ANY,UNWRITTEN,UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
    ...
    // check if all points of the input interval don't belong to (n - 1/2;n +
    // 1/2), where n is integer number if so then we cannot guarantee any
    // result, return NEATValue::UNKNOWN
  ...
  template <typename T> static NEATVector tanpi(const NEATVector &vec) {
  ...
  static NEATValue atan2(const NEATValue &y, const NEATValue &x) {
    ...
    SuperT val[2];
    val[0] = RefALU::atan2(SuperT(minY), SuperT(minX));
    val[1] = RefALU::atan2(SuperT(maxY), SuperT(maxX));
  ...
  template <typename T> static NEATValue asinh(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T> static NEATValue acosh(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T> static NEATValue atanh(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T> static NEATVector atanh(const NEATVector &vecx) {
    return processVector(vecx, atanh<T>);
  }

  template <typename T> static NEATVector acosh(const NEATVector &vecx) {
    return processVector(vecx, acosh<T>);
  }

  template <typename T> static NEATVector asinh(const NEATVector &vecx) {
    return processVector(vecx, asinh<T>);
  }

  template <typename T> static NEATValue native_sqrt(const NEATValue &a) {
    return InternalSqrt<T>(a, double(NEATALU::NATIVE_SQRT_ERROR));
  }

  template <typename T> static NEATVector native_sqrt(const NEATVector &vec) {
    return processVector(vec, native_sqrt<T>);
  }

  template <typename T> static NEATValue sqrt(const NEATValue &a) {
    double ulps = sqrtSetUlps<T>();
    return InternalSqrt<T>(a, ulps);
  }

  template <typename T> static NEATVector sqrt(const NEATVector &vec) {
    return processVector(vec, sqrt<T>);
  }

  template <typename T> static NEATValue half_sqrt(const NEATValue &a) {
    return InternalSqrt<T>(a, double(NEATALU::HALF_SQRT_ERROR));
  }

  template <typename T> static NEATVector half_sqrt(const NEATVector &vec) {
    return processVector(vec, half_sqrt<T>);
  }

  template <typename T> static NEATValue rsqrt(const NEATValue &a) {
    return InternalRsqrt<T>(a, RSQRT_ERROR);
  }

  template <typename T> static NEATVector rsqrt(const NEATVector &vec) {
    return processVector(vec, rsqrt<T>);
  }

  template <typename T> static NEATValue native_rsqrt(const NEATValue &a) {
    return InternalRsqrt<T>(a, NATIVE_RSQRT_ERROR);
  }

  template <typename T> static NEATVector native_rsqrt(const NEATVector &vec) {
    return processVector(vec, native_rsqrt<T>);
  }

  template <typename T> static NEATValue half_rsqrt(const NEATValue &a) {
    return InternalRsqrt<T>(a, HALF_RSQRT_ERROR);
  }

  template <typename T> static NEATVector half_rsqrt(const NEATVector &vec) {
  ...
  template <typename T> static NEATValue lgamma(const NEATValue &a) {
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
    ...
    // logG(0)=logG(-1)=logG(-3)= +inf, and function is not defined for -1<x<0,
    // -3<x<-2, -5<x<-4 and so on, but it is defined for -2<x<-1, -4<x<-3,
    // -6<x<-5 and so on
    if (flushed.Includes<T>(T(0)) || *flushed.GetMax<T>() < T(0.0) ||
        *flushed.GetMin<T>() < T(0.0)) {
      return NEATValue(NEATValue::UNKNOWN);
    }

    // The ULP values for built-in math functions lgamma and lgamma_r is
    // currently undefined, so we return range from -inf to +inf,as it is done
    // in conformance, instead of:
    /*
            // lgamma plot can be found here
       http://mathworld.wolfram.com/LogGammaFunction.html

            // lgamma has a min at this point (if x >0), as like as gamma
       function http://en.wikipedia.org/wiki/Gamma_function SuperT xMinPosVal =
       SuperT(1.46163);
  ...
  template <typename T> static NEATVector lgamma(const NEATVector &vec) {
  ...
  static NEATValue lgamma_r(const NEATValue &a, int32_t *signp) {
    ...
    *signp = 0;
    ...
    // The ULP values for built-in math functions lgamma and lgamma_r is
    // currently undefined, so we return range form -inf to +inf,as it is done
    // in conformance, instead of:
    /*
            // lgamma plot can be found here
       http://mathworld.wolfram.com/LogGammaFunction.html
  ...
  template <typename T>
  static NEATVector lgamma_r(const NEATVector &vec,
  ...
  template <typename T>
  static NEATValue mad(const NEATValue &a, const NEATValue &b,
                       const NEATValue &c) {
    if (CheckAUU(a) || CheckAUU(b) || CheckAUU(c))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T>
  static NEATVector mad(const NEATVector &vec1, const NEATVector &vec2,
                        const NEATVector &vec3) {
  ...
  template <typename T>
  static NEATVector fcmp(const NEATVector &src1, const NEATVector &src2,
                         CmpType comparison) {
  ...
  template <typename T> static T GetSinCosUpperLimit();

  template <typename T> static T GetSinCosLowerLimit();

  static NEATValue extractelement(const NEATVector &vec, const uint32_t &idx);
  static NEATVector insertelement(NEATVector vec, const NEATValue elt,
                                  const uint32_t &idx);
  static NEATVector shufflevector(const NEATVector &vec1,
                                  const NEATVector &vec2,
                                  const std::vector<uint32_t> &mask);

  static NEATVector shuffle(const NEATVector &vec1,
                            const std::vector<uint32_t> &mask);
  static NEATVector shuffle2(const NEATVector &vec1, const NEATVector &vec2,
                             const std::vector<uint32_t> &mask);
  ...
  static NEATValue fpext(const NEATValue &a) {
    ...
    if (src.IsNaN<S>()) {
      ...
      D maxDst = D(maxSrc);
      /*
                  // TODO: test is it needed ( because D minDst = D(minSrc)
         could make it) if(Utils::IsPInf(minSrc))
                  {
  ...
  static NEATVector bitcast(const NEATValue &a) {
    ...
    if (sizeof(SRC) == sizeof(double) && sizeof(DEST) == sizeof(float)) {
      ...
      throw Exception::InvalidArgument("[NEATALU::bitcast] wrong data type\n");
    }
    return dst;
  }
  ...
  template <typename T> static NEATValue ceil(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    ...
    if (min != max)
      ...
      return NEATValue((
          T)min); // doesn't really matter what to return. min and max are equal
  }

  template <typename T> static NEATVector ceil(const NEATVector &a) {
  ...
  template <typename T>
  static NEATValue clamp(const NEATValue &a, const NEATValue &in_min,
  ...
  template <typename T>
  static NEATVector clamp(const NEATVector &a, const NEATValue &in_min,
                          const NEATValue &in_max) {
    return processVector(a, in_min, in_max, clamp<T>);
  }

  template <typename T>
  static NEATVector clamp(const NEATVector &a, const NEATVector &in_min,
                          const NEATVector &in_max) {
    return processVector(a, in_min, in_max, clamp<T>);
  }
    ...
    if (*resI.GetMin<T>() == *resI.GetMax<T>()) {
      ...
      if (diff < SuperT(1.0)) {
        ...
        if (Utils::ge(min, SuperT(0.0))) {
          ...
          // source interval includes 0 and difference between min and max
          // greater than 1.0
          if (valI[0] == SuperT(0.0)) {
            // example min=-0.3, max=100.1 result should be
            // min=-0.3, max=0.(9)
            return NEATValue(T(valF[0]),
  ...
  template <typename T> static NEATValue exp(const NEATValue &a) {
    double ulps = double(EXP_ERROR);
    return InternalExp<T>(a, IntervalError<T>(ulps));
  }

  template <typename T> static NEATVector exp(const NEATVector &a) {
    return processVector(a, exp<T>);
  }

  template <typename T> static NEATValue exp_frm(const NEATValue &a) {
    IntervalError<T> ulps = IntervalError<T>::expFrmSetUlps(a);
    return InternalExp<T>(a, ulps);
  }

  template <typename T> static NEATVector exp_frm(const NEATVector &a) {
    return processVector(a, exp_frm<T>);
  }

  template <typename T> static NEATValue native_exp(const NEATValue &a) {
    double ulps = double(NATIVE_EXP_ERROR);
    return InternalExp<T>(a, IntervalError<T>(ulps));
  }

  template <typename T> static NEATVector native_exp(const NEATVector &a) {
    return processVector(a, native_exp<T>);
  }

  template <typename T> static NEATValue half_exp(const NEATValue &a) {
    double ulps = double(HALF_EXP_ERROR);
    return InternalExp<T>(a, IntervalError<T>(ulps));
  }

  template <typename T> static NEATVector half_exp(const NEATVector &a) {
    return processVector(a, half_exp<T>);
  }

  template <typename T> static NEATValue exp2(const NEATValue &a) {
    double ulps = double(EXP2_ERROR);
    return InternalExp2<T>(a, IntervalError<T>(ulps));
  }

  template <typename T> static NEATVector exp2(const NEATVector &a) {
    return processVector(a, exp2<T>);
  }

  template <typename T> static NEATValue exp2_frm(const NEATValue &a) {
    IntervalError<T> ulps = IntervalError<T>::expFrmSetUlps(a);
    return InternalExp2<T>(a, ulps);
  }

  template <typename T> static NEATVector exp2_frm(const NEATVector &a) {
    return processVector(a, exp2_frm<T>);
  }

  template <typename T> static NEATValue native_exp2(const NEATValue &a) {
    double ulps = double(NATIVE_EXP2_ERROR);
    return InternalExp2<T>(a, IntervalError<T>(ulps));
  }

  template <typename T> static NEATVector native_exp2(const NEATVector &a) {
    return processVector(a, native_exp2<T>);
  }

  template <typename T> static NEATValue half_exp2(const NEATValue &a) {
    double ulps = double(HALF_EXP2_ERROR);
    return InternalExp2<T>(a, IntervalError<T>(ulps));
  }

  template <typename T> static NEATVector half_exp2(const NEATVector &a) {
    return processVector(a, half_exp2<T>);
  }

  template <typename T> static NEATValue exp10(const NEATValue &a) {
    double ulps = double(EXP10_ERROR);
    return InternalExp10<T>(a, ulps);
  }

  template <typename T> static NEATVector exp10(const NEATVector &a) {
    return processVector(a, exp10<T>);
  }

  template <typename T> static NEATValue exp10_frm(const NEATValue &a) {
    // return exp2(x*log2(10))
    return exp2_frm<T>(mul<T>(a, log2_frm<T>(NEATValue(T(10.0f)))));
  }

  template <typename T> static NEATVector exp10_frm(const NEATVector &a) {
    return processVector(a, exp10_frm<T>);
  }

  template <typename T> static NEATValue native_exp10(const NEATValue &a) {
    double ulps = double(NATIVE_EXP10_ERROR);
    return InternalExp10<T>(a, ulps);
  }

  template <typename T> static NEATVector native_exp10(const NEATVector &a) {
    return processVector(a, native_exp10<T>);
  }

  template <typename T> static NEATValue half_exp10(const NEATValue &a) {
    double ulps = double(HALF_EXP10_ERROR);
    return InternalExp10<T>(a, ulps);
  }

  template <typename T> static NEATVector half_exp10(const NEATVector &a) {
    return processVector(a, half_exp10<T>);
  }

  template <typename T> static NEATValue expm1(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // Check for ANY, UNWRITTEN, UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T> static NEATVector expm1(const NEATVector &a) {
    return processVector(a, expm1<T>);
  }

  template <typename T> static NEATValue log2(const NEATValue &a) {
    double ulps = double(LOG2_ERROR);
    return InternalLog2<T>(a, IntervalError<T>(ulps));
  }

  template <typename T> static NEATVector log2(const NEATVector &a) {
    return processVector(a, log2<T>);
  }

  template <typename T> static NEATValue log2_frm(const NEATValue &a) {
    IntervalError<T> ulps = IntervalError<T>::logFrmSetUlps(a);
    return InternalLog2<T>(a, ulps);
  }

  template <typename T> static NEATVector log2_frm(const NEATVector &a) {
    return processVector(a, log2_frm<T>);
  }

  template <typename T> static NEATValue native_log2(const NEATValue &a) {
    double ulps = double(NATIVE_LOG2_ERROR);
    return InternalLog2<T>(a, IntervalError<T>(ulps));
  }

  template <typename T> static NEATVector native_log2(const NEATVector &a) {
    return processVector(a, native_log2<T>);
  }

  template <typename T> static NEATValue half_log2(const NEATValue &a) {
    double ulps = double(HALF_LOG2_ERROR);
    return InternalLog2<T>(a, IntervalError<T>(ulps));
  }

  template <typename T> static NEATVector half_log2(const NEATVector &a) {
    return processVector(a, half_log2<T>);
  }

  template <typename T> static NEATValue log(const NEATValue &a) {
    double ulps = double(LOG_ERROR);
    return InternalLog<T>(a, IntervalError<T>(ulps));
  }

  template <typename T> static NEATVector log(const NEATVector &a) {
    return processVector(a, log<T>);
  }

  template <typename T> static NEATValue log_frm(const NEATValue &a) {
    IntervalError<T> ulps = IntervalError<T>::logFrmSetUlps(a);
    return InternalLog<T>(a, ulps);
  }

  template <typename T> static NEATVector log_frm(const NEATVector &a) {
    return processVector(a, log_frm<T>);
  }

  template <typename T> static NEATValue native_log(const NEATValue &a) {
    double ulps = double(NATIVE_LOG_ERROR);
    return InternalLog<T>(a, IntervalError<T>(ulps));
  }

  template <typename T> static NEATVector native_log(const NEATVector &a) {
    return processVector(a, native_log<T>);
  }

  template <typename T> static NEATValue half_log(const NEATValue &a) {
    double ulps = double(HALF_LOG_ERROR);
    return InternalLog<T>(a, IntervalError<T>(ulps));
  }

  template <typename T> static NEATVector half_log(const NEATVector &a) {
    return processVector(a, half_log<T>);
  }

  template <typename T> static NEATValue log10(const NEATValue &a) {
    double ulps = double(LOG10_ERROR);
    return InternalLog10<T>(a, ulps);
  }

  template <typename T> static NEATVector log10(const NEATVector &a) {
    return processVector(a, log10<T>);
  }

  template <typename T> static NEATValue native_log10(const NEATValue &a) {
    double ulps = double(NATIVE_LOG10_ERROR);
    return InternalLog10<T>(a, ulps);
  }

  template <typename T> static NEATVector native_log10(const NEATVector &a) {
    return processVector(a, native_log10<T>);
  }
  template <typename T> static NEATValue half_log10(const NEATValue &a) {
    double ulps = double(HALF_LOG10_ERROR);
    return InternalLog10<T>(a, ulps);
  }

  template <typename T> static NEATVector half_log10(const NEATVector &a) {
    return processVector(a, half_log10<T>);
  }

  template <typename T> static NEATValue log1p(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // Check for ANY, UNWRITTEN, UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T> static NEATVector log1p(const NEATVector &a) {
    return processVector(a, log1p<T>);
  }

  template <typename T> static NEATValue logb(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // Check for ANY, UNWRITTEN, UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
    ...
    if (flushed.IsAcc()) {
      /// logb(+-0) = -inf
      if (Utils::eq<T>(*flushed.GetAcc<T>(), (T) + 0.0) ||
          Utils::eq<T>(*flushed.GetAcc<T>(), (T)-0.0))
        return NEATValue(Utils::GetNInf<T>());
      /// logb(+-inf) = +inf
      if (Utils::IsPInf<T>(*flushed.GetAcc<T>()) ||
          Utils::IsNInf<T>(*flushed.GetAcc<T>()))
        return NEATValue(Utils::GetPInf<T>());
    }
  ...
  template <typename T> static NEATVector logb(const NEATVector &a) {
    return processVector(a, logb<T>);
  }

  template <typename T> static NEATValue cbrt(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // Check for ANY, UNWRITTEN, UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T> static NEATVector cbrt(const NEATVector &a) {
  ...
  static NEATValue copysign(const NEATValue &x, const NEATValue &y) {
    ...
    if (flushedX.IsAcc()) {
      if (Utils::lt<T>(minY, T(0.0)) && Utils::gt<T>(maxY, T(0.0)))
        return NEATValue(NEATValue::UNKNOWN);
      else {
  ...
  static NEATValue fdim(const NEATValue &x, const NEATValue &y) {
    ...
    // the low interval limit is +0.0, i.e. result is 0 if x is less than or
    // equal to y the upper interval limit is max(x)-min(y), because result is
    // x-y if x>y
    T maxRes = RefALU::sub(xMax, yMin);
    return NEATValue(T(0.0), maxRes);
  }
...
#define FP_ILOGBNAN INT_MAX
  ...
  template <typename T> static NEATValue ilogb(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // Check for ANY, UNWRITTEN, UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T> static NEATVector ilogb(const NEATVector &a) {
    return processVector(a, ilogb<T>);
  }

  /// Returns true if there is integer value in [min; max] interval or false
  /// otherwise
  template <typename T> static bool ContainsInteger(T min, T max) {
  ...
  template <typename T> static bool ContainsOdd(T min, T max) {
  ...
  static NEATValue pow_frm(const NEATValue &x, const NEATValue &y) {
    return exp2_frm<T>(mul<T>(y, log2_frm<T>(x)));
  }
  ...
  template <typename T>
  static NEATVector native_powr(const NEATVector &base,
                                const NEATVector &power) {
  ...
  static NEATValue select(const bool &cond, const NEATValue &a,
                          const NEATValue &b);
  static NEATVector select(const bool &cond, const NEATVector &a,
                           const NEATVector &b);
  static NEATVector select(const std::vector<bool> &cond, const NEATVector &a,
                           const NEATVector &b);

  // these functions are used to support OpenCL built-in 'select'
  template <typename T>
  static NEATValue select(const NEATValue &a, const NEATValue &b,
                          const int64_t &c) {
    bool cond = bool(c); // For a scalar type, result = c ? b : a
    return NEATALU::select(cond, b, a);
  }
  template <typename T>
  static NEATVector select(const NEATVector &a, const NEATVector &b,
  ...
  static NEATValue dot(const NEATValue &val1, const NEATValue &val2) {
    ...
    if (val1.IsFinite<T>() && val2.IsFinite<T>()) {
      ...
      vals[0] = RefALU::mul((SuperT)*flushedX.GetMin<T>(),
                            (SuperT)*flushedY.GetMax<T>());
      vals[1] = RefALU::mul((SuperT)*flushedX.GetMin<T>(),
                            (SuperT)*flushedY.GetMin<T>());
      vals[2] = RefALU::mul((SuperT)*flushedX.GetMax<T>(),
                            (SuperT)*flushedY.GetMin<T>());
      vals[3] = RefALU::mul((SuperT)*flushedX.GetMax<T>(),
                            (SuperT)*flushedY.GetMax<T>());
    ...
    return CreateNEATValue(
        vals, 2,
        IntervalError<T>(NEATALU::MUL_ERROR)); // mul is the only op here
  }
  ...
  static NEATValue dot(const NEATVector &vec1, const NEATVector &vec2) {
    ...
    for (uint32_t i = 0; i < vec1.GetSize(); i++) {
      ...
      if (vec1[i].IsFinite<T>() && vec2[i].IsFinite<T>()) {
        ...
        if (Utils::IsInf<T>(min) && Utils::IsInf<T>(localMin) &&
            !Utils::eq_float<T>(min, localMin))
          return NEATValue(NEATValue::UNKNOWN);

        // return unknown value in case of PINF + NINF operation
        if (Utils::IsInf<T>(max) && Utils::IsInf<T>(localMax) &&
            !Utils::eq_float<T>(max, localMax))
          return NEATValue(NEATValue::UNKNOWN);
    ...
    IntervalError<T>::ExpandFPInterval(
        &min, &max, IntervalError<T>::fromAbsoluteError(errorTolerance));
  ...
  // cross
  ///////////////////////////////////////////////////////////////////////////////
  // Returns the cross product of p0.xyz and p1.xyz. float3 and float4 inputs
  // are possible. The w component of float4 result returned will be 0.0
  ///////////////////////////////////////////////////////////////////////////////
  template <typename T>
  static NEATValue crossOneItem(const NEATValue &in0, const NEATValue &in1,
                                const NEATValue &in2, const NEATValue &in3) {
    ...
    // calculate all possible combinations of min1*min2 - min3*min4, min1*min2 -
    // min3*max4. etc. decide what is the min and max result
    ...
    sT errorTolerance =
        std::max(*in0a.GetMax<T>(),
                 std::max(*in1a.GetMax<T>(),
                          std::max(*in2a.GetMax<T>(), *in3a.GetMax<T>())));

    // expand interval
    // approach from conformance tests is used
    IntervalError<T>::ExpandFPInterval(
        &min0, &max0,
        IntervalError<T>::fromAbsoluteError(errorTolerance * errorTolerance *
                                            (ulps * ComputeUlp(sT(1.0)))));
  ...
  static NEATVector cross(const NEATVector &vec1, const NEATVector &vec2) {
    ...
    if (CheckAUU(vec1[0]) || vec1[0].IsNaN<T>() || CheckAUU(vec2[0]) ||
        vec2[0].IsNaN<T>()) {
      res[1] = NEATValue(NEATValue::UNKNOWN);
      res[2] = NEATValue(NEATValue::UNKNOWN);
    }
    if (CheckAUU(vec1[1]) || vec1[1].IsNaN<T>() || CheckAUU(vec2[1]) ||
        vec2[1].IsNaN<T>()) {
      res[0] = NEATValue(NEATValue::UNKNOWN);
      res[2] = NEATValue(NEATValue::UNKNOWN);
    }
    if (CheckAUU(vec1[2]) || vec1[2].IsNaN<T>() || CheckAUU(vec2[2]) ||
        vec2[2].IsNaN<T>()) {
      res[0] = NEATValue(NEATValue::UNKNOWN);
      res[1] = NEATValue(NEATValue::UNKNOWN);
    }
  ...
  static NEATValue localLength(T inMin, T inMax, double ulps) {
    ...
    return CreateNEATValue(val, 2,
                           IntervalError<typename downT<T>::type>(ulps));
  }

  template <typename T> static NEATValue length(const NEATValue &p) {
    typedef typename superT<T>::type SuperT;
  ...
  template <typename T> static NEATValue fast_length(const NEATValue &p) {
    typedef typename superT<T>::type SuperT;
    if (p.IsNaN<T>())
      return NEATValue(NEATValue::NaN<T>());
  ...
  template <typename T>
  static NEATValue localLengthVec(const NEATVector &vec1, double ulps) {
    typedef typename superT<T>::type SuperT;
    ...
    for (size_t i = 0; i < vec1.GetSize(); i++) {
      ...
      SuperT localMin = RefALU::mul<SuperT>((SuperT)*flushed.GetMin<T>(),
                                            (SuperT)*flushed.GetMin<T>());
      SuperT localMax = RefALU::mul<SuperT>((SuperT)*flushed.GetMax<T>(),
                                            (SuperT)*flushed.GetMax<T>());
  ...
  template <typename T> static NEATValue length(const NEATVector &vec1) {
    double maxUlps =
        NEATALU::sqrtSetUlps<T>() + // error in sqrt
        0.5 *                       // effect on e of taking sqrt( x + e )
            (0.5 * (double)
                       vec1.GetSize() + // cumulative error for multiplications
             0.5 * (double)(vec1.GetSize() -
                            1)); // cumulative error for additions

    return localLengthVec<T>(vec1, maxUlps);
  }

  template <typename T> static NEATValue fast_length(const NEATVector &vec1) {
    double maxUlps =
        NEATALU::HALF_SQRT_ERROR + // error in sqrt
        0.5 *                      // effect on e of taking sqrt( x + e )
            (0.5 * (double)
                       vec1.GetSize() + // cumulative error for multiplications
             0.5 * (double)(vec1.GetSize() -
  ...
  static NEATValue distance(const NEATValue &p0, const NEATValue &p1) {
    ...
    SuperT min = RefALU::sub((SuperT)*flushed0.GetMin<T>(),
                             (SuperT)*flushed1.GetMin<T>());
    SuperT max = RefALU::sub((SuperT)*flushed0.GetMax<T>(),
                             (SuperT)*flushed1.GetMax<T>());

    double maxUlps =
        sqrtSetUlps<T>() + 1.5; // error in sqrt and
  ...
  static NEATValue fast_distance(const NEATValue &p0, const NEATValue &p1) {
    ...
    SuperT min = RefALU::sub((SuperT)*flushedX.GetMin<T>(),
                             (SuperT)*flushedY.GetMin<T>());
    SuperT max = RefALU::sub((SuperT)*flushedX.GetMax<T>(),
                             (SuperT)*flushedY.GetMax<T>());

    double maxUlps =
        NEATALU::HALF_SQRT_ERROR + 1.5; // error in sqrt and
                                        // cumulative error for multiplication
  ...
  template <typename T>
  static NEATValue localDistanceVec(const NEATVector &vec0,
                                    const NEATVector &vec1, double ulps) {
    typedef typename superT<T>::type SuperT;
    ...
    for (size_t i = 0; i < vec1.GetSize(); i++) {
      ...
      SuperT localMin = RefALU::sub((SuperT)*flushed0.GetMin<T>(),
                                    (SuperT)*flushed1.GetMin<T>());
      SuperT localMax = RefALU::sub((SuperT)*flushed0.GetMax<T>(),
                                    (SuperT)*flushed1.GetMax<T>());
  ...
  static NEATValue distance(const NEATVector &vec0, const NEATVector &vec1) {
    ...
    double maxUlps =
        sqrtErr + // error in sqrt
        (1.5 *
             (double)vec1.GetSize() + // cumulative error for multiplications
  ...
  template <typename T>
  static NEATValue fast_distance(const NEATVector &vec0,
                                 const NEATVector &vec1) {
    double maxUlps =
        NEATALU::HALF_SQRT_ERROR + // error in sqrt
        (1.5 *
             (double)vec1.GetSize() + // cumulative error for multiplications
  ...
  // normalize
  ///////////////////////////////////////////////////////////////////////////////
  template <typename T> static NEATValue normalize(const NEATValue &p) {
    typedef typename superT<T>::type SuperT;
    ...
    float maxUlps = NEATALU::NORMALIZE_ERROR;
    SuperT val[1];
    if (p.IsAcc()) {
  ...
  template <typename T> static NEATVector normalize(const NEATVector &p) {
    typedef typename superT<T>::type SuperT;
    ...
    for (uint32_t i = 0; i < vec1.GetSize(); i++) {
      ...
      val[0] = RefALU::mul((SuperT)*vec1[i].GetMin<T>(),
                           (SuperT)*vec1[i].GetMin<T>());
      val[1] = RefALU::mul((SuperT)*vec1[i].GetMax<T>(),
                           (SuperT)*vec1[i].GetMax<T>());
    ...
    float maxUlps =
        NEATALU::NORMALIZE_ERROR +      // error in rsqrt + error in multiply
  ...
  // fast_normalize
  ///////////////////////////////////////////////////////////////////////////////
  template <typename T> static NEATValue fast_normalize(const NEATValue &p) {
    typedef typename superT<T>::type SuperT;

    NEATValue flushed = flush<T>(p);

    if (flushed.IsNaN<T>()) {
      // the behaviour for NaN is not defined in spec, so process as like as
      // normalize does
      return NEATValue(NEATValue::NaN<T>());
    }
    ...
    val[0] =
        RefALU::mul((SuperT)*flushed.GetMin<T>(), (SuperT)*flushed.GetMin<T>());
    val[1] =
        RefALU::mul((SuperT)*flushed.GetMax<T>(), (SuperT)*flushed.GetMax<T>());
    ...
    // OpenCL spec Version: 1.1, 6.11.5 : If the sum of squares is greater than
    // FLT_MAX then the value of the floating-point values in the result vector
    // are undefined.
    if (totalMin > std::numeric_limits<T>::max() ||
        totalMax > std::numeric_limits<T>::max())
      return NEATValue(NEATValue::ANY);

    float maxUlps =
        NEATALU::FAST_NORMALIZE_ERROR; // error in rsqrt + error in multiply
  ...
  template <typename T> static NEATVector fast_normalize(const NEATVector &p) {
    typedef typename superT<T>::type SuperT;
  ...
  template <typename T>
  static NEATValue mix(const NEATValue &x, const NEATValue &y,
                       const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    NEATValue res;
    ...
    if ((*flushedA.GetMin<T>() > (T)1.f) || (*flushedA.GetMax<T>() < (T)0.f)) {
      ...
      if ((*flushedA.GetMin<T>() < (T)0.f) ||
        ...
        if (x.IsNaN<T>() || y.IsNaN<T>())
          ...
          // result is x+(y-x)*a
          // so we should expand output interval, basing on max value of x and
          // y, not on result value
          ...
          IntervalError<T>::ExpandFPInterval(
              &min, &max, maxAbs, IntervalError<T>(NEATALU::MIX_ERROR));
  ...
  template <typename T>
  static NEATVector mix(const NEATVector &x, const NEATVector &y,
                        const NEATVector &a) {
  ...
  template <typename T> static NEATValue radians(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // check for ANY,UNWRITTEN,UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T> static NEATVector radians(const NEATVector &vec) {
    return processVector(vec, radians<T>);
  }

  ///////////////////////////////////////////////////////////////////////////////
  // degrees built-in function
  template <typename T> static NEATValue degrees(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // check for ANY,UNWRITTEN,UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T> static NEATVector degrees(const NEATVector &vec) {
    return processVector(vec, degrees<T>);
  }
  ///////////////////////////////////////////////////////////////////////////////
  // sign built-in function
  template <typename T> static NEATValue sign(const NEATValue &a) {
    // check for ANY,UNWRITTEN,UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T> static NEATVector sign(const NEATVector &vec) {
  ...
  template <typename T>
  static NEATValue smoothstep(const NEATValue &edge0, const NEATValue &edge1,
                              const NEATValue &x) {
    typedef typename superT<T>::type sT;
    // check for ANY,UNWRITTEN,UNKNOWN
    ...
    if (flushedEdge0.IsNaN<T>() || flushedEdge1.IsNaN<T>() ||
        flushedX.IsNaN<T>())
      return NEATValue(NEATValue::ANY);
    ...
    // edge1 - edge0
    sT minB =
        RefALU::add(sT(*flushedEdge1.GetMin<T>()), sT(*e2sub.GetMin<T>()));
    sT maxB =
        RefALU::add(sT(*flushedEdge1.GetMax<T>()), sT(*e2sub.GetMax<T>()));
    ...
    IntervalError<T>::ExpandFPInterval(
        &min, &max, 3,
        IntervalError<T>(localErr * 3.0)); // const 3 is used in 3 operations
  ...
  template <typename T>
  static NEATVector smoothstep(const NEATVector &edge0, const NEATVector &edge1,
  ...
  template <typename T>
  static NEATVector smoothstep(const NEATValue &edge0, const NEATValue &edge1,
                               const NEATVector &vec) {
    NEATVector toReturn(vec.GetWidth());
    size_t size = vec.GetSize();
    for (uint32_t i = 0; i < uint32_t(size); ++i) {
  ...
  static NEATValue min(const NEATValue &x, const NEATValue &y) {
    ...
    // x overlaps y
    if (res.IsUnknown())
      return NEATValue(RefALU::min(*x.GetMin<T>(), *y.GetMin<T>()),
  ...
  static NEATValue max(const NEATValue &x, const NEATValue &y) {
    ...
    // x overlaps y
    if (res.IsUnknown())
      return NEATValue(RefALU::max(*x.GetMin<T>(), *y.GetMin<T>()),
  ...
  template <typename T> static NEATValue fabs(const NEATValue &a) {
    typedef typename superT<T>::type sT;
    // check for ANY,UNWRITTEN,UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
    ...
    if (a.Includes<T>(T(0.0))) {
      val[0] = +0.0;
      val[1] = RefALU::fmax(RefALU::fabs(inp[0]), RefALU::fabs(inp[1]));
    } else {
  ...
  template <typename T> static NEATVector fabs(const NEATVector &vec) {
    return processVector(vec, fabs<T>);
  }

  /// floor implementation
  template <typename T> static NEATValue floor(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // check for ANY,UNWRITTEN,UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
    ...
    if (val[0] != val[1]) {
#ifdef DEBUG
      std::cout << "[NEATALU::floor] There should be more than one integer "
                   "value, but NEAT doesn't support such type of intervals.";
#endif
      return NEATValue(NEATValue::UNKNOWN);
    } else
      return NEATValue((T)val[0]); // doesn't really matter what to return. min
                                   // and max are equal
  }

  template <typename T> static NEATVector floor(const NEATVector &vec) {
  ...
  static NEATValue fract(const NEATValue &a, NEATValue *iptr) {
    ...
    *iptr = NEATALU::floor<T>(a);
    ...
    if (flushed.IsInterval()) {
      // The fract function is discontinuous. Multiple non-intersecting
      // intervals cannot be represented by current implementation To get
      // defined result we need to ensure that output values Y = {y = fract(x),
      // x from Min to Max } belong to continuous interval
      //
      // diff is difference between floor(min) and floor(max)
      // min = floor(min)+fract(min)
      SuperT diff = RefALU::fabs(RefALU::floor(SuperT(*flushed.GetMax<T>())) -
                                 RefALU::floor(SuperT(*flushed.GetMin<T>())));
      // if diff greater then 1 - output interval is continuous and defined as
      // [0; 1) if diff equal 1 and (Max - Min) greater then 1 - output interval
      // is continuous and defined as [0; 1)
      if (Utils::gt<SuperT>(diff, SuperT(1)) ||
          (Utils::eq<SuperT>(diff, SuperT(1)) &&
           Utils::ge<SuperT>(RefALU::fabs(SuperT(*flushed.GetMax<T>()) -
                                          SuperT(*flushed.GetMin<T>())),
                             SuperT(1.0)))) {
      ...
      // - Min) is less than 1 - output interval is discontinuous
      else if (Utils::eq<SuperT>(diff, SuperT(1)) &&
               Utils::lt<SuperT>(RefALU::fabs(SuperT(*flushed.GetMax<T>()) -
                                              SuperT(*flushed.GetMin<T>())),
                                 SuperT(1.0))) {
  ...
  static NEATValue hypot(const NEATValue &x, const NEATValue &y) {
    ...
    // TODO: check
    if (fx.IsAcc() && *fx.GetAcc<T>() == 0.0 && fy.IsAcc() &&
        *fy.GetAcc<T>() == 0.0)
      return NEATValue(T(+0.0));
    ...
    if (fx.Includes<T>(T(0.0))) { // set min boundary to zero
      xmin = 0.0;
    }
  ...
  static NEATValue maxmag(const NEATValue &x, const NEATValue &y) {
    ...
    T xAbsMinimum =
        RefALU::fmin<T>(RefALU::fabs<T>(xMin), RefALU::fabs<T>(xMax));
    T yAbsMaximum =
        RefALU::fmax<T>(RefALU::fabs<T>(yMin), RefALU::fabs<T>(yMax));
    T xAbsMaximum =
        RefALU::fmax<T>(RefALU::fabs<T>(xMin), RefALU::fabs<T>(xMax));
    T yAbsMinimum =
        RefALU::fmin<T>(RefALU::fabs<T>(yMin), RefALU::fabs<T>(yMax));
  ...
  template <typename T>
  static NEATValue fma(const NEATValue &a, const NEATValue &b,
                       const NEATValue &c) {
    typedef typename superT<T>::type SuperT;
    ...
    // Case when all could be any was tested. Other special statuses lead to
    // unknown result
    if (CheckAUU(a) || CheckAUU(b) || CheckAUU(c)) {
      return NEATValue(NEATValue::UNKNOWN);
    }
    ...
    // calculating all the possible combinations
    vals[0] =
        RefALU::add(RefALU::mul((SuperT)*a.GetMin<T>(), (SuperT)*b.GetMax<T>()),
                    (SuperT)*c.GetMin<T>());
    vals[1] =
        RefALU::add(RefALU::mul((SuperT)*a.GetMin<T>(), (SuperT)*b.GetMin<T>()),
                    (SuperT)*c.GetMin<T>());
    vals[2] =
        RefALU::add(RefALU::mul((SuperT)*a.GetMax<T>(), (SuperT)*b.GetMin<T>()),
                    (SuperT)*c.GetMin<T>());
    vals[3] =
        RefALU::add(RefALU::mul((SuperT)*a.GetMax<T>(), (SuperT)*b.GetMax<T>()),
                    (SuperT)*c.GetMin<T>());
    vals[4] =
        RefALU::add(RefALU::mul((SuperT)*a.GetMin<T>(), (SuperT)*b.GetMax<T>()),
                    (SuperT)*c.GetMax<T>());
  ...
  template <typename T>
  static NEATVector fma(const NEATVector &vec1, const NEATVector &vec2,
                        const NEATVector &vec3) {
    return processVector(vec1, vec2, vec3, fma<T>);
  }

  template <typename F, typename I> static NEATValue nan(const I &y) {
  ...
  /// rint implementation
  template <typename T> static NEATValue rint(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // check for ANY,UNWRITTEN,UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
    ...
    if (val[0] != val[1]) {
#ifdef DEBUG
      std::cout << "[NEATALU::rint] There should be more than one integer "
                   "value, but NEAT doesn't support such type of intervals.";
#endif
      return NEATValue(NEATValue::UNKNOWN);
    } else
  ...
  template <typename T> static NEATVector rint(const NEATVector &vec) {
    return processVector(vec, rint<T>);
  }

  /// round implementation
  template <typename T> static NEATValue round(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // check for ANY,UNWRITTEN,UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
    ...
    if (val[0] != val[1]) {
#ifdef DEBUG
      std::cout << "[NEATALU::round] There should be more than one integer "
                   "value, but NEAT doesn't support such type of intervals.";
#endif
      return NEATValue(NEATValue::UNKNOWN);
    } else
  ...
  template <typename T> static NEATVector round(const NEATVector &vec) {
    return processVector(vec, round<T>);
  }

  /// trunc implementation
  template <typename T> static NEATValue trunc(const NEATValue &a) {
    typedef typename superT<T>::type SuperT;
    // check for ANY,UNWRITTEN,UNKNOWN
    if (CheckAUU(a))
      return NEATValue(NEATValue::UNKNOWN);
    ...
    if (val[0] != val[1]) {
#ifdef DEBUG
      std::cout << "[NEATALU::trunc] There should be more than one integer "
                   "value, but NEAT doesn't support such type of intervals.";
#endif
      return NEATValue(NEATValue::UNKNOWN);
    } else
  ...
  template <typename T> static NEATVector trunc(const NEATVector &vec) {
  ...
  // vstore_halfn[_rte,_rtz,_rtp,_rtn], vloada_halfn,
  // vstorea_halfn[_rte,_rtz,_rtp,_rtn],
...
#define NEAT_VSTORE_HALF_RT(__str)                                             \
  template <typename T, uint32_t n>                                            \
  static void vstore##__str(const NEATVector &data, size_t offset,             \
                            uint16_t *p) {                                     \
    for (uint32_t i = 0; i < n; ++i) {                                         \
      /* if the status of NEAT value that is stored is UNWRITTEN or ACCURATE,  \
       */                                                                      \
      /*then NEAT can guarantee that reference value is correct, otherwise     \
       * not.*/                                                                \
      if (!data[i].IsAcc() && !data[i].IsUnwritten()) {                        \
        throw Exception::NEATTrackFailure(                                     \
            "[NEATALU::vstore_half] Only \"ACCURATE\","                        \
            " \"UNWRITTEN\" NEAT values are supported.");                      \
                                                                               ...
                                                                               \
  template <typename T>                                                        \
  static void vstore##__str(const NEATValue &data, size_t offset,              \
                            uint16_t *p) {                                     \
    /* if the status of NEAT value that is stored is UNWRITTEN or ACCURATE, */ \
    /*then NEAT can guarantee that reference value is correct, otherwise       \
     * not.*/                                                                  \
...
public:
  ...
  // Helper function to implement convert float for all data types and vector
  // sizes.
  ...
  /// @brief Calculate NEATVector for pixel read from raw image data. Source
  /// image is in format from OpenCL reference image data has no NEAT. It is
  /// accurate as it is stored in OpenCL Reference and RefALU::ImagesALU
  static NEATVector read_imagef_src_noneat(
...
protected:
  ...
  template <typename T>
  static NEATValue InternalDiv(const NEATValue &a, const NEATValue &b,
                               double ulps);
  ...
  static NEATValue InternalSin(const NEATValue &a, IntervalError<T> ulps) {
    ...
    if (!flushed.IsAcc()) {
      ...
      if (Utils::ge(diff, (T)two_pi)) {
        ...
        // offStart is:  min % (2*PI)
        T offStart = RefALU::mul<T>(
            RefALU::frc<T>(RefALU::div<T>(*flushed.GetMin<T>(), (T)two_pi)),
            (T)two_pi);
        T offEnd = RefALU::add<T>(offStart, diff);
    ...
    SuperT val[2];
    val[0] = hasMinPoint && ulps.mayViolateIEEE754()
                 ? (SuperT)-1.0
                 : RefALU::sin(SuperT(*flushed.GetMin<T>()));
    val[1] = hasMaxPoint && ulps.mayViolateIEEE754()
                 ? (SuperT) + 1.0
                 : RefALU::sin(SuperT(*flushed.GetMax<T>()));

    NEATValue res = CreateNEATValue(val, 2, ulps);

    return ulps.mayViolateIEEE754()
               ? res
               : NEATValue(hasMinPoint ? (T)-1.0 : *res.GetMin<T>(),
                           hasMaxPoint ? (T) + 1.0 : *res.GetMax<T>());
  }
  ...
  static NEATValue InternalCos(const NEATValue &a, IntervalError<T> ulps) {
    ...
    val[0] = hasMinPoint && ulps.mayViolateIEEE754()
                 ? (T)-1.0
                 : RefALU::cos(SuperT(*flushed.GetMin<T>()));
    val[1] = hasMaxPoint && ulps.mayViolateIEEE754()
                 ? (T) + 1.0
                 : RefALU::cos(SuperT(*flushed.GetMax<T>()));
  ...
  template <typename T>
  static NEATValue InternalPow(const NEATValue &x, const NEATValue &y,
                               const double &ulps) {
    typedef typename superT<T>::type SuperT;
    /// C99 ISO/IEC 9899 TC2
    /// [C99] 6
    ...
    /// [C99] 7
    if (y.IsAcc() && (Utils::eq(*y.GetAcc<T>(), (T) + 0.0) ||
                      Utils::eq(*y.GetAcc<T>(), (T)-0.0)))
      return NEATValue((T)1.0);
    /// Check for ANY, UNWRITTEN, UNKNOWN
    if (CheckAUU(x) || CheckAUU(y))
      return NEATValue(NEATValue::UNKNOWN);
  ...
  template <typename T>
  static NEATValue InternalPowr(const NEATValue &x, const NEATValue &y,
                                const double &ulps) {
    typedef typename superT<T>::type SuperT;
    /// Check for ANY, UNWRITTEN, UNKNOWN
    if (CheckAUU(x) || CheckAUU(y))
  ...
  template <typename T>
  static NEATValue InternalPown(const NEATValue &x, const int32_t &y,
                                const double &ulps) {
    typedef typename superT<T>::type SuperT;
