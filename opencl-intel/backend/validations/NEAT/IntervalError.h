...
namespace Validation
{
    ...
    public:
        ...
        /// @brief default ctor
        IntervalError() : m_ErrorType(ERROR_UNDEFINED),
            m_isUlpValueAccurate(true),
        ...
        static IntervalError<T> fromAbsoluteError(const sT ErrorValue, bool mayViolateIEEE754 = false)
        {
            IntervalError<T> ret;
        ...
        static IntervalError<T> fromUlpsError(const float ErrorValue, bool mayViolateIEEE754 = false)
        {
            IntervalError<T> ret;
            ret.m_ErrorType = ERROR_ULPS;
            ret.m_mayViolateIEEE754 = mayViolateIEEE754;
            ret.m_UlpError = ErrorValue;
            ret.m_isUlpValueAccurate = (ComputeUlp((double)ErrorValue) < 1.0f) ? true : false;
            return ret;
        }
        ...
        static IntervalError<T> fromUlpsIntervalError(const float LHErrorValue, const float &RHErrorValue, bool mayViolateIEEE754 = false)
        {
            IntervalError<T> ret;
            ...
            ret.m_isUlpValueAccurate = ( (ComputeUlp((double)LHErrorValue) < 1.0f) ? true : false)
        ...
        {
            assert(m_ErrorType == ERROR_ULPS && " getConstantUlps Error is not error in ulps");
            return m_UlpError;
        }
        ...
        /// @note could be called only for error with ERROR_ULPS or ERROR_BOUNDARY_ULPS type
        ...
        bool isInfiniteError() const
        {
            ...
            case ERROR_BOUNDARY_ULPS:
                ...
                return ret;
            default:
                throw Exception::InvalidArgument("[IntervalError] Unsupported error type");
            }
            return ret;
        }
        ...
        bool mayViolateIEEE754() const
        {
            return m_mayViolateIEEE754;
        }
        ...
        /// @param error error value
        static void ExpandFPInterval(sT * minInOut, sT * maxInOut, sT ref4Ulps, IntervalError<T> error);
    ...
    void IntervalError<T>::ExpandFPInterval(typename IntervalError<T>::sT * minInOut, typename IntervalError<T>::sT * maxInOut, IntervalError<T> error)
    {
        ...
        sT refMax_sub = resMax - error.getExpandValue(ulpMax, false); // add ulps to ref
        ...
        sT refMin_add = resMin + error.getExpandValue(ulpMin, true); // add ulps to ref
