...
namespace Validation
{
    ...
    template<>
    IntervalError<float> IntervalError<float>::divFrmSetUlps(const NEATValue &a, const NEATValue &b) {
        ...
        isGuaranteedUlp |= ( b.IsInterval() && a.IsAcc() && (*a.GetAcc<float>() == float(1.0f)) &&
            interval1.Includes(*b.GetMin<float>())  && interval1.Includes(*b.GetMax<float>()));
        // check condition "is x in the domain of 2**-62 to 2**62 and y in the domain of 2**-62 to 2**62."
    ...
    IntervalError<float> IntervalError<float>::cosFrmSetUlps(const NEATValue &a) {
        ...
        isGuaranteedUlp |= ( ( a.IsInterval() || a.IsAcc() )  && interval.Includes(*a.GetMin<float>()) && interval.Includes(*a.GetMax<float>()) );
        if(!isGuaranteedUlp)
            AbsErr = +INFINITY;
    ...
    IntervalError<float> IntervalError<float>::expFrmSetUlps(const NEATValue &a) {
        ...
        {
            ret = IntervalError<float>::fromUlpsIntervalError(3.0f + RefALU::floor(RefALU::fabs(2*(*a.GetMin<float>()))),
    ...
    IntervalError<float> IntervalError<float>::logFrmSetUlps(const NEATValue &a) {
        ...
        // for x in the domain [0.5, 2] the maximum absolute error is <= 2**-21; otherwise the maximum error is<=3 ulp for the full profile
        isAbsErr |= ( ( a.IsInterval() || a.IsAcc() )  && interval.Includes(*a.GetMin<float>()) && interval.Includes(*a.GetMax<float>()) );

        if(isAbsErr)
            return IntervalError<float>::fromAbsoluteError(HEX_DBL(+, 2, 0, -, 21), true);
