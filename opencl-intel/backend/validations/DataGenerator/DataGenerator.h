...
template <typename T> class IsDGType;
...
public:
  ...
  typedef std::vector<BufferDescAndFillMethodPair> BufferContainerFillMethod;

  /// Vector of vectors BufferContainerFillMethod.
  /// It is an input for DataGenerator ctor
  typedef std::vector<BufferContainerFillMethod> BufferContainerListFillMethod;
  ...
  DataGenerator(const BufferContainerListFillMethod &in_method)
      : m_BufferContainerListFillMethod(in_method),
  ...
  template <typename T>
  static void SetBufferContainerFillMethod(BufferContainerFillMethod &bcfm,
                                           DataTypeValWrapper dataType,
                                           VectorWidthWrapper vecWidth,
                                           const uint32_t buffLen,
                                           FillMethod fillMethod, T value,
    ...
    BufferFillMethodDesc bfmd = {
        fillMethod, SetFillDataType(dataType.GetValue(), value),
        SetFillDataType(dataType.GetValue(), lowValue),
        SetFillDataType(dataType.GetValue(), highValue), specialValuesProb};
  ...
  template <typename T>
  static void
  SetValueMethod(BufferContainerFillMethod &bcfm, DataTypeValWrapper dataType,
                 VectorWidthWrapper vecWidth, const uint32_t buffLen, T value) {
    /// Constraint template to work with numerical types only.
    /// Otherwise compiler will report an error
    IsDGType<T> _notUsed;
    UNUSED_ARGUMENT(_notUsed);
    ...
    SetBufferContainerFillMethod<T>(bcfm, dataType, vecWidth, buffLen,
                                    FILL_VALUE, value, 0.0f, 0.0f, 0.0f);
  }
  ...
  /// @param [in] highValue. The top limit to fill buffer.
  template <typename T>
  static void SetRandomFromRangeMethod(BufferContainerFillMethod &bcfm,
                                       DataTypeValWrapper dataType,
                                       VectorWidthWrapper vecWidth,
                                       const uint32_t buffLen, T lowValue,
                                       T highValue) {
    /// Constraint template to work with numerical types only.
    /// Otherwise compiler will report an error
    IsDGType<T> _notUsed;
    ...
    SetBufferContainerFillMethod<T>(bcfm, dataType, vecWidth, buffLen,
                                    FILL_RANDOM_FROM_RANGE, 0.0f, lowValue,
                                    highValue, 0.0f);
  }
  ...
  static void
  SetBufferContainerListFillMethod(BufferContainerListFillMethod &bclfm,
                                   BufferContainerFillMethod &bcfm) {
    bclfm.push_back(bcfm);
  }

  uint64_t GetSeed(void) { return m_Seed; }
...
private:
  ...
  /// other data types
  template <typename T> void FillRandom(T *arr, int32_t len, T max, T min) {
    /// Constraint template to work with numerical types only.
    /// Otherwise compiler will report an error
    IsDGType<T> _notUsed;
    UNUSED_ARGUMENT(_notUsed);
  ...
  void FillRandomSpecial(CFloat16 *arr, int32_t len, float specialValuesProb);

  template <typename T>
  void FillRandomSpecial(T *arr, int32_t len, float specialValuesProb) {
    /// Constraint template to work with numerical types only.
    /// Otherwise compiler will report an error
    IsDGType<T> _notUsed;
    UNUSED_ARGUMENT(_notUsed);
  ...
  template <typename T>
  static FillDataType SetFillDataType(DataTypeVal dataType, T value) {
    /// Constraint template to work with numerical types only.
    /// Otherwise compiler will report an error
    IsDGType<T> _notUsed;
    UNUSED_ARGUMENT(_notUsed);
  ...
  DataGenerator(const DataGenerator &in)
      : IDataReader(),
        m_BufferContainerListFillMethod(in.m_BufferContainerListFillMethod) {}
};
} // namespace Validation
