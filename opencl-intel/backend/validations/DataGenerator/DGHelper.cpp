...
// for testing
uint64_t GetUpdateConst(void) { return updateConst; }
...
uint64_t GetCurrentSeed(void) { return currentSeed; }

bool GetSeedFlag(void) { return setSeedFlag; }
...
void GenerateRandomVectorsAutoSeed(DataTypeVal dataTypeVal, const float *arr,
  ...
  uint8_t high8 = 0xfe; // max exponent value for normalized float 8 bits, 0xff
                        // is used to represent INF and NaN
  dataTypeExp.SetValue(U8);
  DataGenerator::SetRandomFromRangeMethod(bcfm, dataTypeExp, vecWidth, n, low8,
                                          high8);

  int32_t low23 = -8388608; // min mantissa for normalized float - 23 bits
  int32_t high23 =
      8388607; // max mantissa for normalized float - 23 bits (0x7fffff)
  dataTypeMant.SetValue(I32);
  DataGenerator::SetRandomFromRangeMethod(bcfm, dataTypeMant, vecWidth, n,
                                          low23, high23);
  ...
  dg.Read(&list);
  uint8_t *dataExp =
      (uint8_t *)list.GetBufferContainer(0)->GetMemoryObject(0)->GetDataPtr();
  int32_t *dataMant =
      (int32_t *)list.GetBufferContainer(0)->GetMemoryObject(1)->GetDataPtr();
...
void GenerateRandomVectorsAutoSeed(DataTypeVal dataTypeVal, const double *arr,
  ...
  uint16_t high11 = 0x07fe; // max exponent value for normalized double 11 bits,
                            // 0x07ff is used to represent INF and NaN
  dataTypeExp.SetValue(U16);
  DataGenerator::SetRandomFromRangeMethod(bcfm, dataTypeExp, vecWidth, n, low11,
                                          high11);

  int64_t low52 =
      -4503599627370496; // min mantissa for normalized double 52 bits
  int64_t high52 = 4503599627370495; // max mantissa for normalized double 52
                                     // bits (0x0FFFFFFFFFFFFF)
  ...
  dg.Read(&list);
  uint16_t *dataExp =
      (uint16_t *)list.GetBufferContainer(0)->GetMemoryObject(0)->GetDataPtr();
  int64_t *dataMant =
      (int64_t *)list.GetBufferContainer(0)->GetMemoryObject(1)->GetDataPtr();
