...
const NEATStructLayout *
NEATDataLayout ::getStructLayout(const StructType *Ty) const {
  NEATStructLayout *SL = 0;
  // Create the struct layout.  Because it is variable length, we
  // malloc it, then use placement new.
  int NumElts = Ty->getNumElements();
  NEATStructLayout *L = (NEATStructLayout *)malloc(
      sizeof(NEATStructLayout) + (NumElts - 1) * sizeof(uint64_t));
...
NEATStructLayout::NEATStructLayout(const StructType *ST,
                                   const NEATDataLayout &DL) {
  StructAlignment = 0;
  StructSize = 0;
...
#define HANDLE_EVENT(__EVENT)                                                  \
  if (GetCurEvent() != __EVENT)                                                \
    return;
...
NEATGenericValue NEATPlugIn::getConstantExprValue(ConstantExpr *CE,
  ...
  default:
    throw Exception::IllegalFunctionCall(
        "[NEATPlug-in::getConstantExprValue] : Not supported NEAT instruction");
    break;
  }
...
NEATGenericValue NEATPlugIn::getConstantValueFromValue(Value *V) {
  ...
  if (isa<UndefValue>(V)) {
    // if operand value is 'undef' then no need to initialize NEATValue. It's
    // already UNWRITTEN by default. We need just set correct vector length for
    // vector values.
    if (isa<VectorType>(V->getType())) {
      Ret.NEATVec.SetWidth(VectorWidthWrapper::ValueOf(
    ...
    case Type::PointerTyID: {
      ...
      if (NEATDataLayout ::IsNEATSupported(PT->getElementType())) {
        if (GlobalValue *GV2 = dyn_cast<GlobalValue>(V)) {
          ...
          Exception::IllegalFunctionCall(
              "[NEATPlug-in::getConstantValue] : Value is not GlobalValue");
        }
      } else {
    ...
    case Type::ScalableVectorTyID: {
      const FixedVectorType *VT = cast<FixedVectorType>(V->getType());
    ...
    default:
      ...
      throw Exception::IllegalFunctionCall(
          "[NEATPlug-in::getConstantValue] : Not supported NEAT data type");
      break;
    }
  }
  return Ret;
...
NEATGenericValue NEATPlugIn::getOperandValue(Value *V,
...
void NEATPlugIn::LoadValueFromMemory(NEATGenericValue &Result,
                                     NEATGenericValue *NEATPtr, Type *Ty) {
  switch (Ty->getTypeID()) {
  case Type::FloatTyID:
  case Type::DoubleTyID:
  ...
  case Type::ScalableVectorTyID: {
    ...
    Result.NEATVec.SetWidth(VectorWidthWrapper::ValueOf(
  ...
  case Type::StructTyID: {
    ...
    for (unsigned i = 0; i < Result.AggregateVal.size(); ++i) {
      unsigned int offset =
          SLO->getElementOffset(i); // calculate offset for the current element
      LoadValueFromMemory(Result.AggregateVal[i],
                          (NEATGenericValue *)((uint8_t *)NEATPtr + offset),
  ...
  case Type::ArrayTyID: {
    ...
    for (unsigned i = 0; i < Result.AggregateVal.size(); ++i) {
      uint64_t offset =
          elementSize * i; // calculate offset for the current element
      LoadValueFromMemory(
          Result.AggregateVal[i],
          (NEATGenericValue *)((uint8_t *)NEATArrayPtr + offset),
          ATy->getElementType());
    }
    break;
  }
...
void NEATDataLayout ::InitMemory(void *Memory, const Type *Ty) const {
  ...
  default:
    throw Validation::Exception::InvalidArgument(
        "NEATDataLayout ::InitMemory(): Unsupported type");
    break;
  }
}
...
uint64_t NEATDataLayout ::getTypeStoreSize(const Type *Ty) const {
  assert(Ty->isSized() &&
         "Cannot getTypeStoreSize() on a type that is unsized!");
  switch (Ty->getTypeID()) {
  case Type::LabelTyID:
  case Type::PointerTyID:
    return sizeof(void *);
  ...
  case Type::ScalableVectorTyID: {
    const FixedVectorType *VTy = cast<FixedVectorType>(Ty);
    if (VTy->getNumElements() == 3)
      return getTypeStoreSize(VTy->getElementType()) * 4;
    return getTypeStoreSize(VTy->getElementType()) * VTy->getNumElements();
  }
  default:
    throw Validation::Exception::InvalidArgument(
        "NEATDataLayout ::getTypeSizeInBits(): Unsupported type");
    break;
  }
  return 0;
}
  ...
  case Type::StructTyID: {
    ...
    for (unsigned i = 0; i < Val.AggregateVal.size(); ++i) {
      ...
      StoreValueToMemory(Val.AggregateVal[i],
                         (NEATGenericValue *)((uint8_t *)Ptr + offset),
                         STy->getElementType(i));
    }
    break;
  }
...
void NEATPlugIn::visitAllocaInst(AllocaInst &I) {
  ...
  LLVM_DEBUG(dbgs() << "NEATPlugin: Allocated Type: " << *Ty << " (" << TypeSize
...
void NEATPlugIn::callFunction(
  ...
  if (F->isDeclaration()) {
    ...
    // throw Exception::NotImplemented("[NEATPlugin]: Not implemented external
    // call in callFunction " + F->getNameStr() );
    return;
  }
...
void NEATPlugIn::handlePreFunctionRun(std::vector<ExecutionContext> &ECStack,
                                      InterpreterPluggable &Interp) {
...
template <typename T>
inline void _CopyAccurateBufferToNEATBuffer(const IMemoryObject &src,
                                            IMemoryObject &dst) {
...
template <typename T> void SetNEATValue(const char *src, char *dst) {
  const T *srcPtr = reinterpret_cast<const T *>(src);
  NEATValue *dstPtr = reinterpret_cast<NEATValue *>(dst);
  *dstPtr = NEATValue(*srcPtr);
}

void CopyBufferElementToNEAT(const char *src, char *dst,
                             const TypeDesc &srcElemDesc,
                             const TypeDesc &dstElemDesc) {
  ...
  case Validation::TARRAY: {
    for (uint64_t i = 0; i < srcElemDesc.GetNumberOfElements(); ++i) {
      CopyBufferElementToNEAT(src, dst, srcElemDesc.GetSubTypeDesc(0),
                              dstElemDesc.GetSubTypeDesc(0));
  ...
  case Validation::TSTRUCT: {
    for (uint64_t i = 0; i < srcElemDesc.GetNumOfSubTypes(); ++i) {
      CopyBufferElementToNEAT(src, dst, srcElemDesc.GetSubTypeDesc(i),
                              dstElemDesc.GetSubTypeDesc(i));
  ...
  case Validation::UNSPECIFIED_TYPE: {
    throw Exception::InvalidArgument(
        "Can't copy value of unspecified type to NEAT buffer!");
    break;
  }
  case Validation::INVALID_TYPE: {
    throw Exception::InvalidArgument(
        "Can't copy value of invalid type to NEAT buffer!");
    break;
  }
  }
}
...
void CopyBufferToNEAT(const IMemoryObject &src, IMemoryObject &dst) {
  ...
  // Number of elements in ordinary buffer and buffer with NEAT must be the
  // same.
  ...
  for (std::size_t i = 0; i < bufferLen; ++i) {
    CopyBufferElementToNEAT(srcPtr, dstPtr, srcDesc.GetElementDescription(),
                            dstDesc.GetElementDescription());
...
llvm::CreateNEATBufferContainer(const Validation::IBufferContainer &BC,
  ...
  for (std::size_t i = 0; i < BC.GetMemoryObjectCount(); ++i) {
    ...
    if (Image::GetImageName() == currMemObj->GetName()) { // image
      // TODO: enable creating NEAT images
      // this is stub
      ImageDesc imageDesc =
          GetImageDescription(currMemObj->GetMemoryObjectDesc());
      outputContainer->CreateImage(imageDesc);
    } else if (Buffer::GetBufferName() == currMemObj->GetName()) {
      BufferDesc buffDsc =
          GetBufferDescription(currMemObj->GetMemoryObjectDesc());
      // check data type
      ...
      if (buffDsc.IsFloatingPoint()) {
        ...
        if (buffDsc.GetElementDescription().IsStruct()) {
          ...
          if (neatBuffDesc.GetElementDescription().GetType() == TVECTOR)
            dType = neatBuffDesc.GetElementDescription()
                        .GetSubTypeDesc(0)
                        .GetType();
          else
          ...
          default:
            throw Exception::InvalidArgument(
                "Not supported floating point data type");
            break;
          }
        }
      } else {
...
void NEATPlugIn::handlePostFunctionRun() { SetCurEvent(POST_FUNC); }
...
#define IMPLEMENT_FLOAT_VAL(_TYPE_DM_)                                         \
  {                                                                            \
    NEATGenericValue neatVal;                                                  \
    if (elemDesc.GetType() != _TYPE_DM_)                                       \
      throw Exception::InvalidArgument("Floating point type in input data "    \
                                       "doesn't match kernel argument type");  \
    if (buffDsc.NumOfElements() != 1)                                          \
      throw Exception::InvalidArgument(                                        \
          "Number of elements in input buffer doesn't match kernel argument "  \
          "length");                                                           \
...
void llvm::CreateNEATBufferContainerMap(
  ...
  for (std::size_t buf = 0; arg_it != in_F->arg_end() && buf < numOfArguments;
    ...
    if (Image::GetImageName() == currMemObj->GetName()) {
      ...
      continue;
    } else if (Buffer::GetBufferName() == currMemObj->GetName()) { // buffer
      ...
      case Type::ScalableVectorTyID: {
        ...
        if (VTy->getElementType()->isFloatingPointTy()) {
          ...
          neatVal.NEATVec.SetWidth(
              VectorWidthWrapper::ValueOf(VTy->getNumElements()));
...
bool NEATDataLayout ::IsNEATSupported(const Type *Ty) {
  ...
  default:
    ...
    throw Validation::Exception::InvalidArgument(
        "NEATDataLayout ::IsNEATSupported(): Unsupported type");
    break;
  }
...
void NEATPlugIn::SwitchToNewBasicBlock(BasicBlock *Dest, ExecutionContext &inSF,
                                       NEATExecutionContext &NSF) {

  if (!isa<PHINode>(inSF.CurBB->begin()))
    return; // Nothing fancy to do

  // [LLVM 3.6 UPGRADE] FIXME: Is the copy of the execution context really
  // needed?
...
void NEATPlugIn::visitReturnInst(ReturnInst &I) {
  ...
  if (I.getNumOperands()) {
    ...
    // we can get I.getReturnValue()->getType() only if I.getNumOperands() is
    // true, if I.getNumOperands() is false, I.getReturnValue()->getType() is
    // empty
  ...
  // if return value is not NEAT supported, make popStackAndReturnValueToCaller
  // to pop the current stack frame, but ignore result
...
#define FLOAT_VECTOR_FUNCTION(OP, TY) FLOAT_VECTOR_FUNCTION_SUFX(OP, TY, _)
  ...
  if (Ty->isVectorTy()) {
    ...
    case Instruction::FAdd:
      FLOAT_VECTOR_FUNCTION(NEAT_WRAP::add,
                            dyn_cast<VectorType>(Ty)->getElementType())
      break;
    case Instruction::FSub:
      FLOAT_VECTOR_FUNCTION(NEAT_WRAP::sub,
                            dyn_cast<VectorType>(Ty)->getElementType())
      break;
      // if command line parameter of SATest --fma-neat set on, function mul_fma
      // used to perform multiplication, if --fma-neat set off, commom mul is
      // used. common mul has correctly rounded result, fma_mul makes resulting
      // adds 1 ULP to min and max values of resulting interval.
    case Instruction::FMul:
      if (m_bUseFmaNEAT) {
        FLOAT_VECTOR_FUNCTION(NEAT_WRAP::mul_fma,
                              dyn_cast<VectorType>(Ty)->getElementType())
      } else {
        FLOAT_VECTOR_FUNCTION(NEAT_WRAP::mul,
                              dyn_cast<VectorType>(Ty)->getElementType())
      }
      break;
    case Instruction::FDiv:
      if (isFRMPrecisionOn()) {
        FLOAT_VECTOR_FUNCTION_SUFX(
            NEAT_WRAP::div, dyn_cast<VectorType>(Ty)->getElementType(), _frm_)
      } else {
        FLOAT_VECTOR_FUNCTION(NEAT_WRAP::div,
                              dyn_cast<VectorType>(Ty)->getElementType())
      }
      break;
    case Instruction::FRem:
      FLOAT_VECTOR_FUNCTION(NEAT_WRAP::fmod,
                            dyn_cast<VectorType>(Ty)->getElementType())
      break;
    default:
...
#define FLOAT_OPERATOR(OP, TY) FLOAT_OPERATOR_SUFX(OP, TY, _)
    ...
    switch (I.getOpcode()) {
    case Instruction::FAdd:
      FLOAT_OPERATOR(NEAT_WRAP::add, Ty) break;
    case Instruction::FSub:
      FLOAT_OPERATOR(NEAT_WRAP::sub, Ty) break;
    case Instruction::FMul:
      if (m_bUseFmaNEAT) {
        FLOAT_OPERATOR(NEAT_WRAP::mul_fma, Ty)
      } else {
        FLOAT_OPERATOR(NEAT_WRAP::mul, Ty)
      }
      break;
    case Instruction::FDiv:
      if (isFRMPrecisionOn()) {
        FLOAT_OPERATOR_SUFX(NEAT_WRAP::div, Ty, _frm_)
      } else {
        FLOAT_OPERATOR(NEAT_WRAP::div, Ty)
      }
      break;
    case Instruction::FRem:
      FLOAT_OPERATOR(NEAT_WRAP::fmod, Ty) break;
    default:
...
void NEATPlugIn::visitExtractElementInst(ExtractElementInst &I) {
  ...
  GenericValue IdxGV =
      m_pInterp->getOperandValueAdapter(I.getOperand(1), m_pECStack->back());
  const unsigned indx = unsigned(IdxGV.IntVal.getZExtValue());

  // TODO: template parameter is meaningless here. Remove it once it deleted
  // from interface.
...
void NEATPlugIn::visitInsertElementInst(InsertElementInst &I) {
  ...
  GenericValue IdxGV =
      m_pInterp->getOperandValueAdapter(I.getOperand(2), m_pECStack->back());
  const unsigned indx = unsigned(IdxGV.IntVal.getZExtValue());

  // TODO: template parameter is meaningless here. Remove it once it deleted
  // from interface.
  Result.NEATVec =
      NEAT_WRAP::insertelement_fd(Src1.NEATVec, Src2.NEATVal, indx);
  SetValue(&I, Result, SF);
}
...
void NEATPlugIn::visitShuffleVectorInst(ShuffleVectorInst &I) {
  ...
  Dest.NEATVec =
      NEAT_WRAP::shufflevector_fd(Src1.NEATVec, Src2.NEATVec, mask_vec);
  ...
  if (isAnyUndef) {
    if (ShuffleMaskSize != Dest.NEATVec.GetSize()) {
      throw Exception::NEATTrackFailure(
          "NEATPlugin::shufflevector. Wrong mask vector size");
    }
...
#define COMPARE(TYPE, VALUE, Y)                                                \
  if (TYPE->getTypeID() == Type::FloatTyID) {                                  \
    R.NEAT##VALUE =                                                            \
        NEAT_WRAP::fcmp_f(Src1.NEAT##VALUE, Src2.NEAT##VALUE, CMP_##Y);        \
  }                                                                            \
  if (TYPE->getTypeID() == Type::DoubleTyID) {                                 \
    R.NEAT##VALUE =                                                            \
        NEAT_WRAP::fcmp_d(Src1.NEAT##VALUE, Src2.NEAT##VALUE, CMP_##Y);        \
  }
...
void NEATPlugIn::visitSelectInst(SelectInst &I) {
  ...
  const Type *Ty = I.getOperand(0)->getType();
  GenericValue MaskGV =
      m_pInterp->getOperandValueAdapter(I.getOperand(0), m_pECStack->back());
  NEATGenericValue Src2 = getOperandValue(I.getOperand(1), SF);
  ...
  if (Ty->isVectorTy()) {
    ...
    if (I.getOperand(1)->getType()->isVectorTy()) {
      R.NEATVec =
          NEAT_WRAP::select_fd(MaskGV.IntVal != 0, Src2.NEATVec, Src3.NEATVec);
    } else {
      R.NEATVal =
          NEAT_WRAP::select_fd(MaskGV.IntVal != 0, Src2.NEATVal, Src3.NEATVal);
    }
  }
...
void NEATPlugIn::visitFPTruncInst(FPTruncInst &I) {
  ...
  if (Ty->getTypeID() != Type::DoubleTyID &&
      I.getType()->getTypeID() != Type::FloatTyID) {
    throw InvalidArgument(
        "NEATPlugin:FPTrunc could truncate only double to float");
  }
  NEATGenericValue Src1 = getOperandValue(I.getOperand(0), SF);
  NEATGenericValue R;
...
APInt BitcastIntVectorToIntScalar(GenericValue &vec, unsigned int resBitLen,
                                  unsigned vecLen, bool isLittleEndian) {
  APInt result;
  ...
  unsigned int ShiftAmt =
      isLittleEndian ? 0 : vecElementBitWidth * (vecLen - 1);
...
void NEATPlugIn::visitBitCastInst(BitCastInst &I) {
  ...
  if (SrcTy->getTypeID() == DstTy->getTypeID()) {
    if (SrcTy->isFloatingPointTy()) // example: bitcast float value to float
                                    // value
    {
    ...
    // vector to vector bitcast
      ...
      Type::TypeID DstTypeID =
          dyn_cast<VectorType>(DstTy)->getElementType()->getTypeID();
      Type::TypeID SrcTypeID =
          dyn_cast<VectorType>(SrcTy)->getElementType()->getTypeID();
      if (SrcTypeID == Type::FloatTyID) {
        ...
        if (DstTypeID == Type::DoubleTyID) {
          ...
          for (size_t i = 0; i < Src1.NEATVec.GetSize(); ++i) {
            if (!Src1.NEATVec[i].IsAcc())
              throw Exception::NEATTrackFailure(
                  "NEATPlugin: Can't bitcast that floating point value to the "
                  "integer without ambiguity.");
          }
        } else {
          throw Exception::NEATTrackFailure(
              "NEATPlugin: Unsupported type of arguments for bitcast "
              "instruction.");
        }
        ...
        LLVM_DEBUG(dbgs() << "[NEATPlugin] warning : bitcast vector of "
                             "integers to NEAT vector\n");
        // <n x int32> to <n x float> or <n x int64> to <n x double>
        const unsigned int srcNum =
            cast<FixedVectorType>(SrcTy)->getNumElements();
        const unsigned int srcBitWidth =
            cast<VectorType>(SrcTy)->getElementType()->getPrimitiveSizeInBits();
        const unsigned int dstNum =
            cast<FixedVectorType>(DstTy)->getNumElements();

        R.NEATVec.SetWidth(VectorWidthWrapper::ValueOf(dstNum));
        GenericValue GV = m_pInterp->getOperandValueAdapter(I.getOperand(0),
                                                            m_pECStack->back());
        ...
        if (DstTypeID == Type::FloatTyID) {
          if (dstNum == srcNum) { // bitcast i32 to float
            ...
            for (unsigned int i = 0; i < dstNum; ++i) {
              ...
              intToBitcast = 0;
              unsigned int ShiftAmt =
                  isLittleEndian ? 0 : srcBitWidth * (ratio - 1);
              for (unsigned int j = 0; j < ratio; ++j) {
                APInt Tmp;
          ...
          throw Exception::NEATTrackFailure(
              "NEATPlugin: Can't bitcast to different bit width.");

      } else
        throw Exception::NEATTrackFailure("NEATPlugin: Unsupported type of "
                                          "arguments for bitcast instruction.");
  ...
  if (SrcTy->getTypeID() == Type::DoubleTyID && isa<VectorType>(DstTy)) {
    if (dyn_cast<VectorType>(DstTy)->getElementType()->getTypeID() ==
        Type::FloatTyID) {
      R.NEATVec = NEAT_WRAP::bitcast_d2f(Src1.NEATVal);
      SetValue(&I, R, SF);
    } else if (dyn_cast<VectorType>(DstTy)->getElementType()->getTypeID() ==
               Type::IntegerTyID) {
  ...
  if (isa<VectorType>(SrcTy)) {
    ...
    if (SrcTypeID == Type::FloatTyID) {
      ...
      } else if (DstTy->getTypeID() ==
                 Type::IntegerTyID) { // <2 x float> to i64
        for (size_t i = 0; i < Src1.NEATVec.GetSize(); ++i) {
          if (!Src1.NEATVec[i].IsAcc()) {
            throw Exception::NEATTrackFailure(
                "NEATPlugin: Can't bitcast that floating point value to the "
      ...
      if (DstTy->getTypeID() == Type::FloatTyID) {
        float fVal =
            float(BitcastIntVectorToIntScalar(GV, 32, srcNum, isLittleEndian)
                      .bitsToFloat());
        R.NEATVal = NEATValue(fVal);
      } else if (DstTy->getTypeID() == Type::DoubleTyID) {
        double dVal =
            double(BitcastIntVectorToIntScalar(GV, 64, srcNum, isLittleEndian)
                       .bitsToDouble());
      ...
      throw Exception::NEATTrackFailure(
          "NEATPlugin:Invalid arguments for bitcast instruction. Can't bitcast "
          "to non-floating point data types.");
    }
    SetValue(&I, R, SF);
    return;
  }
  ...
  if (SrcTy->getTypeID() == Type::IntegerTyID) {
    ...
    if (DstTy->getTypeID() == Type::FloatTyID &&
        srcBitWidth == 32) // bitcast i32 to float
    {
      float fVal = float(GV.IntVal.bitsToFloat());
      R.NEATVal = NEATValue(fVal);
    } else if (DstTy->getTypeID() == Type::DoubleTyID &&
               srcBitWidth == 64) // bitcast i64 to double
    {
      ...
      R.NEATVec.SetWidth(VectorWidthWrapper::ValueOf(
          cast<FixedVectorType>(DstTy)->getNumElements()));
  ...
  if (SrcTy->isFloatingPointTy()) {
    if (DstTy->isIntegerTy() ||
        (DstTy->isVectorTy() &&
         dyn_cast<VectorType>(DstTy)->getElementType()->isIntegerTy())) {
      if (!Src1.NEATVal.IsAcc())
        throw Exception::NEATTrackFailure(
            "NEATPlugin: Can't bitcast that floating point value to the "
            "integer without ambiguity.");
  ...
  // if we are here, we don't know how to bitcast
  throw Exception::NEATTrackFailure(
      "NEATPlugin:Invalid arguments for bitcast instruction.");
}
...
void NEATPlugIn::visitExtractValueInst(ExtractValueInst &I) {
  ...
  for (unsigned i = 0; i < Num; ++i) {
    ...
    ++IdxBegin;
    Type *IndexedType = ExtractValueInst::getIndexedType(
        Agg->getType(), I.getIndices().slice(i));
    if (IndexedType->isVectorTy() &&
        (dyn_cast<VectorType>(IndexedType)->getElementType()->isFloatTy() ||
         dyn_cast<VectorType>(IndexedType)->getElementType()->isDoubleTy())) {
...
void NEATPlugIn::visitInsertValueInst(InsertValueInst &I) {
  ...
  for (unsigned i = 0; i < Num; ++i) {
    unsigned idx = *IdxBegin;
    ++IdxBegin;
    const Type *IndexedType = ExtractValueInst::getIndexedType(
        Agg->getType(), I.getIndices().slice(i));
    if (IndexedType->isVectorTy() &&
        (dyn_cast<VectorType>(IndexedType)->getElementType()->isFloatTy() ||
         dyn_cast<VectorType>(IndexedType)->getElementType()->isDoubleTy())) {
  ...
  case Type::PointerTyID:
    ...
    break;
  default:
    LLVM_DEBUG(dbgs() << "Unhandled dest type for extractelement instruction: "
                      << *IndexedType << "\n");
    llvm_unreachable(0);
  }
...
void NEATPlugIn::visitCallSite(CallBase &CS) {
  ...
  LLVM_DEBUG(dbgs() << "[NEATPlugin] running : CallSite: "
  ...
  if (F && F->isDeclaration()) {
    ...
    case Intrinsic::not_intrinsic:
      break;
    default:
      LLVM_DEBUG(dbgs() << "[NEATPlugin] Warning:: running unknown intrinsic: "
                        << *CS.getCalledFunction() << "\n");
      return;
    }
  ...
  for (auto i = CS.arg_begin(), e = CS.arg_end(); (i != e) || (AI != AE);
       ++AI, ++i) {
    Argument *A = &*AI;
...
void NEATPlugIn::popStackAndReturnValueToCaller(const Type *RetTy,
                                                NEATGenericValue Result) {
  HANDLE_EVENT(PRE_INST); //
  // Pop the current stack frame.
  m_NECStack.pop_back();

  if (m_NECStack.empty()) { // Finished OpenCL kernel. Return type could be
                            // *only* void.
...
const NEATGenericValue &
GetArg(Value *arg, const std::map<Value *, NEATGenericValue> &ArgVals) {
...
void NEATPlugIn::execute_shuffle(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
  ...
  for (unsigned i = 0; i < ValArg1.AggregateVal.size(); ++i) {
    mask_vec.push_back(
        (uint32_t)(ValArg1.AggregateVal[i].IntVal.getLimitedValue()));
  }
...
void NEATPlugIn::execute_shuffle2(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  Function::arg_iterator Fit = F->arg_begin();
  Value *arg0 = &*Fit++;
  ...
  if (ValArg0.NEATVec.GetSize() != ValArg1.NEATVec.GetSize())
    throw Exception::NEATTrackFailure(
        "[NEATPlugIn::execute_shuffle2]: wrong vector size.");
  ...
  for (unsigned i = 0; i < ValArg2.AggregateVal.size(); ++i) {
    mask_vec.push_back(
        (uint32_t)(ValArg2.AggregateVal[i].IntVal.getLimitedValue()));
  }

  Result.NEATVec =
      NEAT_WRAP::shuffle2_fd(ValArg0.NEATVec, ValArg1.NEATVec, mask_vec);
}

///////////////// atomic_xchg built-in function ///////////////////////
void NEATPlugIn::execute_atomic_xchg(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  Function::arg_iterator Fit = F->arg_begin();
  Value *arg0 = &*Fit++;
...
void NEATPlugIn::execute_cross(
  ...
  if (Ty0->isVectorTy() || Ty1->isVectorTy()) {
    ...
    if (ValArg0.NEATVec.GetSize() != ValArg1.NEATVec.GetSize())
      throw Exception::NEATTrackFailure(
          "[NEATPlugIn::execute_cross]: wrong vector size.");
    ...
    if (TyElem->isFloatTy()) {
      ...
      throw Exception::IllegalFunctionCall(
          "[NEATPlug-in::execute_cross]: Not valid vector data type for "
          "built-in");
    }
  } else {
    throw Exception::IllegalFunctionCall(
        "[NEATPlug-in::execute_cross]: Not valid data type for built-in");
  }
}

///////////////// Step built-in function ///////////////////////
void NEATPlugIn::execute_step(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  Function::arg_iterator Fit = F->arg_begin();
  Value *arg0 = &*Fit++;
  ...
  if (Ty0->isFloatTy()) {
    ...
    if (TyElem->isFloatTy()) {
      ...
      throw Exception::IllegalFunctionCall(
          "[NEATPlug-in::step]: Not valid vector data type for built-in");
    }
  } else {
    throw Exception::IllegalFunctionCall(
        "[NEATPlug-in::step]: Not valid data type for built-in");
  }
}

///////////////// smoothstep built-in function ///////////////////////
void NEATPlugIn::execute_smoothstep(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  Function::arg_iterator Fit = F->arg_begin();
  Value *arg0 = &*Fit++;
  ...
  if (Ty0->isFloatTy()) {
    if (Ty1->isFloatTy() && Ty2->isVectorTy()) {
      Result.NEATVec = NEAT_WRAP::smoothstep_f(ValArg0.NEATVal, ValArg1.NEATVal,
                                               ValArg2.NEATVec);
    } else if (Ty1->isFloatTy() && Ty2->isFloatTy()) {
      Result.NEATVal = NEAT_WRAP::smoothstep_f(ValArg0.NEATVal, ValArg1.NEATVal,
                                               ValArg2.NEATVal);
    } else {
      throw Exception::IllegalFunctionCall(
          "[NEATPlug-in::smoothstep]: Not valid data type for built-in");
    }
  } else if (Ty0->isDoubleTy()) {
    if (Ty1->isDoubleTy() && Ty2->isVectorTy()) {
      Result.NEATVec = NEAT_WRAP::smoothstep_d(ValArg0.NEATVal, ValArg1.NEATVal,
                                               ValArg2.NEATVec);
    } else if (Ty1->isDoubleTy() && Ty2->isDoubleTy()) {
      Result.NEATVal = NEAT_WRAP::smoothstep_d(ValArg0.NEATVal, ValArg1.NEATVal,
                                               ValArg2.NEATVal);
    } else {
      throw Exception::IllegalFunctionCall(
          "[NEATPlug-in::smoothstep]: Not valid data type for built-in");
    ...
    const Type *TyElem = dyn_cast<VectorType>(Ty0)->getElementType();
    if (TyElem->isFloatTy()) {
      Result.NEATVec = NEAT_WRAP::smoothstep_f(ValArg0.NEATVec, ValArg1.NEATVec,
                                               ValArg2.NEATVec);
    } else if (TyElem->isDoubleTy()) {
      Result.NEATVec = NEAT_WRAP::smoothstep_d(ValArg0.NEATVec, ValArg1.NEATVec,
                                               ValArg2.NEATVec);
    } else {
      throw Exception::IllegalFunctionCall(
          "[NEATPlug-in::smoothstep]: Not valid vector data type for built-in");
    }
  } else {
    throw Exception::IllegalFunctionCall(
        "[NEATPlug-in::smoothstep]: Not valid data type for built-in");
...
void NEATPlugIn::execute_clamp(
  ...
  if (Ty1->isFloatTy()) {
    if (Ty0->isVectorTy()) {
      Result.NEATVec =
          NEAT_WRAP::clamp_f(ValArg0.NEATVec, ValArg1.NEATVal, ValArg2.NEATVal);
    } else {
      Result.NEATVal =
          NEAT_WRAP::clamp_f(ValArg0.NEATVal, ValArg1.NEATVal, ValArg2.NEATVal);
    }
  } else if (Ty1->isDoubleTy()) {
    if (Ty0->isVectorTy()) {
      Result.NEATVec =
          NEAT_WRAP::clamp_d(ValArg0.NEATVec, ValArg1.NEATVal, ValArg2.NEATVal);
    } else {
      Result.NEATVal =
          NEAT_WRAP::clamp_d(ValArg0.NEATVal, ValArg1.NEATVal, ValArg2.NEATVal);
    }
  } else if (Ty1->isVectorTy()) {
    const Type *TyElem = dyn_cast<VectorType>(Ty1)->getElementType();
    if (TyElem->isFloatTy()) {
      Result.NEATVec =
          NEAT_WRAP::clamp_f(ValArg0.NEATVec, ValArg1.NEATVec, ValArg2.NEATVec);
    } else if (TyElem->isDoubleTy()) {
      Result.NEATVec =
          NEAT_WRAP::clamp_d(ValArg0.NEATVec, ValArg1.NEATVec, ValArg2.NEATVec);
    } else if (TyElem->isIntegerTy()) {
      return;
    } else {
      throw Exception::IllegalFunctionCall(
          "[NEATPlug-in::clamp]: Not valid vector data type for built-in");
    }
  } else if (Ty1->isIntegerTy()) {
    return;
  } else {
    throw Exception::IllegalFunctionCall(
        "[NEATPlug-in::clamp]: Not valid data type for built-in");
  }
}
...
void NEATPlugIn::execute_vload_half(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
...
#define EXECUTE_VLOAD_HALF(n)                                                  \
  void NEATPlugIn::execute_vload_half##n(                                      \
      Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,         \
      NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {  \
    GenericValue ValArg0 = GetGenericArg(0);                                   \
    size_t offset = ValArg0.IntVal.getZExtValue();                             \
...
#define EXECUTE_VLOADA_HALF(n)                                                 \
  void NEATPlugIn::execute_vloada_half##n(                                     \
      Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,         \
      NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {  \
    GenericValue ValArg0 = GetGenericArg(0);                                   \
    size_t offset = ValArg0.IntVal.getZExtValue();                             \
...
void NEATPlugIn::execute_vloada_half(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
...
#define EXECUTE_VLOAD(n)                                                       \
  void NEATPlugIn::execute_vload##n(                                           \
      Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,         \
      NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {  \
    Function::arg_iterator Fit = F->arg_begin();                               \
    GenericValue ValArg0 = GetGenericArg(0);                                   \
    ...
    if (ETy->isFloatTy()) {                                                    \
      ...
      throw Exception::IllegalFunctionCall(                                    \
          "[NEATPlug-in::vload]: Invalid data type for vload built-in");       \
    }                                                                          \
  }
...
#define EXECUTE_VSTORE_HALF_RT(mode, name)                                     \
  void NEATPlugIn::execute_v##name##mode(                                      \
      Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,         \
      NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {  \
    Function::arg_iterator Fit = F->arg_begin();                               \
    Value *arg0 = &*Fit++;                                                     \
    ...
    if (Ty0->isFloatTy()) {                                                    \
      ...
      throw Exception::IllegalFunctionCall(                                    \
          "[NEATPlug-in::vstore_half]: Invalid data type"                      \
...
#define EXECUTE_VSTORE_HALF_N_RT(n, name, plugin_mode, alu_mode)               \
  void NEATPlugIn::execute_v##name##plugin_mode(                               \
      Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,         \
      NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {  \
    Function::arg_iterator Fit = F->arg_begin();                               \
    Value *arg0 = &*Fit++;                                                     \
    ...
    if (ETy->isFloatTy()) {                                                    \
      ...
      throw Exception::IllegalFunctionCall(                                    \
          "[NEATPlug-in::vstore_half]: Invalid data"                           \
...
#define EXECUTE_VSTORE(n)                                                      \
  void NEATPlugIn::execute_vstore##n(                                          \
      Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,         \
      NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {  \
    Function::arg_iterator Fit = F->arg_begin();                               \
    Value *arg0 = &*Fit++;                                                     \
    ...
    if (ETy->isFloatTy()) {                                                    \
      ...
      throw Exception::IllegalFunctionCall(                                    \
          "[NEATPlug-in::vstore]: Invalid data type for vstore built-in");     \
    }                                                                          \
  }
...
// Macro to define the convert_float functions for different vector length
// sizes.
#define EXECUTE_CONVERT_F(n)                                                   \
  template <typename T> NEATVector convert_f##n(GenericValue arg) {            \
    T src[n];                                                                  \
  ...
  void NEATPlugIn::execute_convert_float##n(                                   \
      Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,         \
      NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {  \
    Function::arg_iterator Fit = F->arg_begin();                               \
    Value *arg = &*Fit++;                                                      \
...
void NEATPlugIn::execute_convert_float(
  ...
  if (Ty->isDoubleTy()) {
    ...
    throw Exception::InvalidArgument(
        "[NEATPlug-in::convert_float]: Not valid data type.");
  }
}
...
#define EXECUTE_CONVERT_D(n)                                                   \
  template <typename T> NEATVector convert_d##n(GenericValue arg) {            \
    T src[n];                                                                  \
    for (unsigned int i = 0; i < n; ++i) {                                     \
      src[i] = T(arg.AggregateVal[i].IntVal.getLimitedValue());                \
    }                                                                          \
    return NEAT_WRAP::convert_double_##n(src);                                 \
                                                                               ...
                                                                               \
  void NEATPlugIn::execute_convert_double##n(                                  \
      Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,         \
      NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {  \
    Function::arg_iterator Fit = F->arg_begin();                               \
    Value *arg = &*Fit++;                                                      \
...
template <typename T> NEATValue convert_d(GenericValue argVal) {
  T val = argVal.IntVal.getSExtValue();
  return NEAT_WRAP::convert_double((T *)&val);
}

// Convert_double built-in executor.
void NEATPlugIn::execute_convert_double(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  Function::arg_iterator Fit = F->arg_begin();
  Value *arg = &*Fit++;
...
void NEATPlugIn::execute_async_work_group_copy(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  Function::arg_iterator Fit = F->arg_begin();
  Value *arg0 = &*Fit++;
  ...
  if (ETy->isFloatingPointTy()) {
    ...
    for (uint64_t i = 0; i < num_gentypes.IntVal.getZExtValue();
         ++i, ++dstP, ++srcP) {
      *dstP = *srcP;
    }
  } else if (ETy->isVectorTy() &&
             dyn_cast<VectorType>(ETy)->getElementType()->isFloatingPointTy()) {
    NEATValue *dstP = (NEATValue *)dst.PointerVal;
    NEATValue *srcP = (NEATValue *)src.PointerVal;
    for (uint64_t i = 0;
         i < num_gentypes.IntVal.getZExtValue() *
                 dyn_cast<FixedVectorType>(ETy)->getNumElements();
         ++i, ++dstP, ++srcP) {
      *dstP = *srcP;
    }
  }
...
void NEATPlugIn::execute_async_work_group_strided_copy(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  Function::arg_iterator Fit = F->arg_begin();
  Value *arg0 = &*Fit++;
  ...
  if (ETy->isFloatingPointTy()) {
    ...
    if (ArgList[0].ptrType.AddrSpace == OCLBuiltinParser::GLOBAL) {
      for (uint64_t i = 0; i < num_gentypes.IntVal.getZExtValue();
           ++i, dstP +=
                dyn_cast<FixedVectorType>(ETy)->getNumElements() * (stride - 1))
        for (uint64_t j = 0;
             j < dyn_cast<FixedVectorType>(ETy)->getNumElements();
             ++j, ++dstP, ++srcP)
          *dstP = *srcP;
    } else {
      for (uint64_t i = 0; i < num_gentypes.IntVal.getZExtValue();
...
#define RELATIONAL_TWO_ARGS(FuncName)                                          \
  ...
  if (Ty->isFloatTy()) {                                                       \
    ...
    if (Result.NEATVal.IsUnknown())                                            \
      throw Exception::NEATTrackFailure(                                       \
          "[NEATPlugIn::execute_##FuncName##]: the result of comparison is "   \
          "UNKNOWN.");                                                         \
    ...
    if (x.NEATVec.GetSize() != y.NEATVec.GetSize())                            \
      throw Exception::NEATTrackFailure(                                       \
          "[NEATPlugIn::execute_##FuncName##]: different vector sizes.");      \
    if (x.NEATVec.GetSize() != cast<FixedVectorType>(Ty)->getNumElements())    \
      throw Exception::NEATTrackFailure(                                       \
          "[NEATPlugIn::execute_##FuncName##]: wrong vector size.");           \
    if (ETy->isFloatTy()) {                                                    \
    ...
    for (size_t i = 0; i < Result.NEATVec.GetSize(); ++i)                      \
      if (Result.NEATVec[i].IsUnknown())                                       \
        throw Exception::NEATTrackFailure("[NEATPlugIn::##FuncName##]: the "   \
                                          "result of comparison is UNKNOWN."); \
  }
...
#define RELATIONAL_ONE_ARG(FuncName)                                           \
  ...
  if (Ty->isFloatTy()) {                                                       \
    ...
    const Type *ETy = cast<FixedVectorType>(Ty)->getElementType();             \
    if (x.NEATVec.GetSize() != cast<FixedVectorType>(Ty)->getNumElements())    \
      throw Exception::NEATTrackFailure(                                       \
          "[NEATPlugIn::execute_##FuncName##]: wrong vector size.");           \
    if (ETy->isFloatTy()) {                                                    \
...
void NEATPlugIn::execute_ISequal(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  RELATIONAL_TWO_ARGS(ISequal)
}
void NEATPlugIn::execute_ISnotequal(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  RELATIONAL_TWO_ARGS(ISnotequal)
}
void NEATPlugIn::execute_ISgreater(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  RELATIONAL_TWO_ARGS(ISgreater)
}
void NEATPlugIn::execute_ISgreaterequal(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  RELATIONAL_TWO_ARGS(ISgreaterequal)
}
void NEATPlugIn::execute_ISless(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  RELATIONAL_TWO_ARGS(ISless)
}
void NEATPlugIn::execute_ISlessequal(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  RELATIONAL_TWO_ARGS(ISlessequal)
}
void NEATPlugIn::execute_ISlessgreater(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  RELATIONAL_TWO_ARGS(ISlessgreater)
}
void NEATPlugIn::execute_ISfinite(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  RELATIONAL_ONE_ARG(ISfinite)
}
void NEATPlugIn::execute_ISinf(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  RELATIONAL_ONE_ARG(ISinf)
}
void NEATPlugIn::execute_ISnan(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  RELATIONAL_ONE_ARG(ISnan)
}
void NEATPlugIn::execute_ISnormal(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  RELATIONAL_ONE_ARG(ISnormal)
}
void NEATPlugIn::execute_ISordered(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  RELATIONAL_TWO_ARGS(ISordered)
}
void NEATPlugIn::execute_ISunordered(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  RELATIONAL_TWO_ARGS(ISunordered)
}
void NEATPlugIn::execute_signbit(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  RELATIONAL_ONE_ARG(signbit)
}

void NEATPlugIn::execute_bitselect(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  Function::arg_iterator Fit = F->arg_begin();
  Value *arg0 = &*Fit++;
  ...
  if (Ty != arg1->getType() || Ty != arg2->getType())
    throw Exception::NEATTrackFailure(
        "[NEATPlugIn::execute_bitselect]: input data types are not the same.");
  ...
  if (Ty->isFloatTy()) {
    Result.NEATVal = NEAT_WRAP::bitselect_f(ValArg0.NEATVal, ValArg1.NEATVal,
                                            ValArg2.NEATVal);
  } else if (Ty->isDoubleTy()) {
    Result.NEATVal = NEAT_WRAP::bitselect_d(ValArg0.NEATVal, ValArg1.NEATVal,
                                            ValArg2.NEATVal);
  } else if (Ty->isVectorTy()) {
    const Type *TyElem = dyn_cast<VectorType>(Ty)->getElementType();
    if (TyElem->isFloatTy()) {
      Result.NEATVec = NEAT_WRAP::bitselect_f(ValArg0.NEATVec, ValArg1.NEATVec,
                                              ValArg2.NEATVec);
    } else if (TyElem->isDoubleTy()) {
      Result.NEATVec = NEAT_WRAP::bitselect_d(ValArg0.NEATVec, ValArg1.NEATVec,
                                              ValArg2.NEATVec);
    }
  }
}

void NEATPlugIn::execute_select(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  Function::arg_iterator Fit = F->arg_begin();
  Value *arg0 = &*Fit++;
  ...
  if (Ty != arg1->getType())
    throw Exception::NEATTrackFailure(
        "[NEATPlugIn::execute_select]: input data types of first and second "
  ...
  // only floating point type values or vectors are supoorted.
  // in case of integer or boolean or other type input data just do nothing
  // both input values or vectors should have the same type (and size for
  // vectors)
  if (Ty->isFloatTy()) {
    ...
    if (ETy->isFloatTy() || ETy->isDoubleTy()) {
      if (a.NEATVec.GetSize() != b.NEATVec.GetSize() ||
          a.NEATVec.GetSize() != size_t(c.AggregateVal.size()) ||
          (a.NEATVec.GetSize() != cast<FixedVectorType>(Ty)->getNumElements()))
        throw Exception::NEATTrackFailure(
            "[NEATPlugIn::execute_select]: wrong vector size.");
...
void NEATPlugIn::execute_ilogb(
  ...
  if (Ty->isFloatTy()) {
    ...
    if (Result.NEATVal.IsUnknown())
      throw Exception::NEATTrackFailure(
          "[NEATPlugIn::execute_ilogb]: the result of operation is UNKNOWN.");
  } else if (Ty->isDoubleTy()) {
    ...
    if (x.NEATVec.GetSize() != cast<FixedVectorType>(Ty)->getNumElements())
      throw Exception::NEATTrackFailure(
          "[NEATPlugIn::execute_ilogb]: wrong vector size.");
...
void execute_lgamma_r(Function *F,
  ...
  if (Ty0->isVectorTy()) {
    ...
    // we do not support integer types in NEAT ALU so far, that's why we don't
    // use results written to vector intVec
    ...
    if (TyElem0->isFloatTy()) {
      ...
      throw Exception::IllegalFunctionCall(
          "[NEATPlug-in::lgamma_r]: Not valid vector data type for built-in");
    }
  } else if (Ty0->isDoubleTy()) {
    // we do not support integer types in NEAT ALU so far, that's why we don't
    // use result written to variable intRes
    int32_t intRes;
    ...
    throw Exception::IllegalFunctionCall(
        "[NEATPlug-in::lgamma_r]: Not valid data type for built-in");
  }
}
...
void execute_remquo(Function *F,
  ...
  if (Ty0->isVectorTy() && Ty1->isVectorTy()) {
    ...
    if (ValArg0.NEATVec.GetSize() != ValArg1.NEATVec.GetSize())
      throw Exception::NEATTrackFailure(
          "[NEATPlugIn::execute_remquo]: wrong vector size.");
    ...
    if (TyElem0->isFloatTy() && TyElem1->isFloatTy()) {
      Result.NEATVec =
          NEAT_WRAP::remquo_f(ValArg0.NEATVec, ValArg1.NEATVec, intVec);
    } else if (TyElem0->isDoubleTy() && TyElem1->isDoubleTy()) {
      Result.NEATVec =
          NEAT_WRAP::remquo_d(ValArg0.NEATVec, ValArg1.NEATVec, intVec);
    } else {
      throw Exception::IllegalFunctionCall(
          "[NEATPlug-in::execute_remquo]: Not valid vector data type for "
          "built-in");
    }
    ...
    int32_t intRes;
    Result.NEATVal =
        NEAT_WRAP::remquo_d(ValArg0.NEATVal, ValArg1.NEATVal, &intRes);
  } else if (Ty0->isFloatTy() && Ty1->isFloatTy()) {
    // we do not support integer types in NEAT ALU so far, that's why we don't
    // use result written to variable intRes
    int32_t intRes;
    Result.NEATVal =
        NEAT_WRAP::remquo_f(ValArg0.NEATVal, ValArg1.NEATVal, &intRes);
  } else {
    throw Exception::IllegalFunctionCall(
        "[NEATPlug-in::execute_remquo]: Not valid data type for built-in");
  }
}
...
void NEATPlugIn::execute_nan(Function *F,
  ...
  if (Ty->isIntegerTy()) {
    ...
    if (n == 32) {
      ...
      throw Exception::IllegalFunctionCall(
          "[NEATPlug-in::execute_nan]: Not valid data type for built-in "
          "(scalar)");
  } else if (Ty->isVectorTy()) {
    const VectorType *VTy = cast<VectorType>(Ty);
    ...
    if (n == 32) {
      ...
      for (unsigned i = 0; i < ValArg.AggregateVal.size(); ++i)
        vec0.push_back(
            (uint32_t)(ValArg.AggregateVal[i].IntVal.getZExtValue()));
      Result.NEATVec = NEAT_WRAP::nan_f(vec0);
    } else if (n == 64) {
      std::vector<uint64_t> vec0;
      for (unsigned i = 0; i < ValArg.AggregateVal.size(); ++i)
        vec0.push_back(
            (uint64_t)(ValArg.AggregateVal[i].IntVal.getZExtValue()));
    ...
    if (Result.NEATVec.GetSize() != cast<FixedVectorType>(Ty)->getNumElements())
      throw Exception::NEATTrackFailure(
          "[NEATPlugIn::execute_nan]: wrong vector size.");
...
void NEATPlugIn::execute_read_imagef(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  cl_mem_obj_descriptor *memobj =
      (cl_mem_obj_descriptor *)GetGenericArg(0).PointerVal;
  ...
  if (OCLBuiltinParser::FLOAT == CoordTy) {
    ...
    // int coordinates
    OCLBuiltins::getCoordsByImageType<int32_t, float>(objType, CoordGV, u, v,
                                                      w);
  }

  LLVM_DEBUG(dbgs() << "[NEATPlugin] Coordinates u=" << u << " v=" << v
                    << " w=" << w << "\n");

  cl_image_format im_fmt;
  Conformance::image_descriptor desc =
      OCLBuiltins::CreateConfImageDesc(*memobj, im_fmt);
...
void execute_mix(Function *F,
  ...
  if (Ty2->isFloatTy()) {
    if (Ty0->isVectorTy() && Ty1->isVectorTy()) {
      Result.NEATVec =
          NEAT_WRAP::mix_f(ValArg0.NEATVec, ValArg1.NEATVec, ValArg2.NEATVal);
    } else if (Ty0->isFloatTy() && Ty1->isFloatTy()) {
      Result.NEATVal =
          NEAT_WRAP::mix_f(ValArg0.NEATVal, ValArg1.NEATVal, ValArg2.NEATVal);
    } else {
      throw Exception::IllegalFunctionCall(
          "[NEATPlug-in::mix]: Not valid data type for built-in");
    }
  } else if (Ty2->isDoubleTy()) {
    if (Ty0->isVectorTy() && Ty1->isVectorTy()) {
      Result.NEATVec =
          NEAT_WRAP::mix_d(ValArg0.NEATVec, ValArg1.NEATVec, ValArg2.NEATVal);
    } else if (Ty0->isDoubleTy() && Ty1->isDoubleTy()) {
      Result.NEATVal =
          NEAT_WRAP::mix_d(ValArg0.NEATVal, ValArg1.NEATVal, ValArg2.NEATVal);
    } else {
      throw Exception::IllegalFunctionCall(
          "[NEATPlug-in::mix]: Not valid data type for built-in");
    }
    ...
    if (TyElem->isFloatTy()) {
      Result.NEATVec =
          NEAT_WRAP::mix_f(ValArg0.NEATVec, ValArg1.NEATVec, ValArg2.NEATVec);
    } else if (TyElem->isDoubleTy()) {
      Result.NEATVec =
          NEAT_WRAP::mix_d(ValArg0.NEATVec, ValArg1.NEATVec, ValArg2.NEATVec);
    } else {
      throw Exception::IllegalFunctionCall(
          "[NEATPlug-in::mix]: Not valid vector data type for built-in");
    }
  } else {
    throw Exception::IllegalFunctionCall(
        "[NEATPlug-in::mix]: Not valid data type for built-in");
...
void execute_frexp(Function *F,
  ...
  if (Ty0->isVectorTy()) {
    ...
    if (TyElem0->isFloatTy()) {
      ...
      throw Exception::IllegalFunctionCall(
          "[NEATPlug-in::frexp]: Not valid vector data type for built-in");
    }
  } else if (Ty0->isDoubleTy()) {
    // we do not support integer types in NEAT ALU so far, that's why we don't
    // use result written to variable intRes
    ...
    throw Exception::IllegalFunctionCall(
        "[NEATPlug-in::frexp]: Not valid data type for built-in");
  }
}

///////////////// ldexp built-in function ///////////////////////
void NEATPlugIn::execute_ldexp(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result, const OCLBuiltinParser::ArgVector &ArgList) {
  ...
  if (Ty0->isVectorTy()) {
    ...
    if (Ty1->isVectorTy()) {
      ...
      if (ValArg0.NEATVec.GetSize() != size_t(sizeIntVec))
        throw Exception::IllegalFunctionCall(
            "[NEATPlug-in::ldexp]: Not valid vector size for built-in");
      ...
      if (TyElem0->isFloatTy()) {
        ...
        throw Exception::IllegalFunctionCall(
            "[NEATPlug-in::ldexp]: Not valid vector data type for built-in");
      }
    } else if (Ty1->isIntegerTy()) {
      ...
      throw Exception::IllegalFunctionCall(
          "[NEATPlug-in::ldexp]: Not valid data type for built-in");
  ...
  if (Ty0->isVectorTy()) {
    ...
    if (Ty1->isVectorTy()) {
      ...
      if (ValArg0.NEATVec.GetSize() != sizeIntVec)
        throw Exception::IllegalFunctionCall(
            "[NEATPlug-in::pown]: Not valid vector size for built-in");
      ...
      if (TyElem0->isFloatTy()) {
        ...
        throw Exception::IllegalFunctionCall(
            "[NEATPlug-in::pown]: Not valid vector data type for built-in");
      }
    } else
      throw Exception::IllegalFunctionCall(
          "[NEATPlug-in::pown]: Not valid data type for built-in");
...
void NEATPlugIn::execute_rootn(
  ...
  if (Ty0->isVectorTy()) {
    ...
    if (Ty1->isVectorTy()) {
      ...
      if (ValArg0.NEATVec.GetSize() != size_t(sizeIntVec))
        throw Exception::IllegalFunctionCall(
            "[NEATPlug-in::rootn]: Not valid vector size for built-in");
      ...
      if (TyElem0->isFloatTy()) {
        ...
        throw Exception::IllegalFunctionCall(
            "[NEATPlug-in::rootn]: Not valid vector data type for built-in");
      }
    } else
      throw Exception::IllegalFunctionCall(
          "[NEATPlug-in::rootn]: Not valid data type for built-in");
...
#define UBER_BI_CALL(_aluname, args, argsvec)                                  \
  ...
  if (Ty->isFloatTy()) {                                                       \
    ...
    if (TyElem->isFloatTy()) {                                                 \
      ...
      throw Exception::IllegalFunctionCall(                                    \
          "[NEATPlug-in::" + std::string(#_aluname) +                          \
          "]: vector data type is not vector of floats or doubles");           \
    }                                                                          \
  } else {                                                                     \
    throw Exception::IllegalFunctionCall(                                      \
        "[NEATPlug-in::" + std::string(#_aluname) +                            \
...
#define UBER_BI_CALL_FRM(_aluname, args, argsvec)                              \
  ...
  if (Ty->isFloatTy()) {                                                       \
    ...
    if (TyElem->isFloatTy()) {                                                 \
      ...
      throw Exception::IllegalFunctionCall(                                    \
          "[NEATPlug-in]: Not valid vector data type for built-in");           \
    }                                                                          \
  } else {                                                                     \
    throw Exception::IllegalFunctionCall(                                      \
        "[NEATPlug-in]: Not valid data type for built-in");                    \
  }
...
// define macro HANDLE_BI_ONEARG for handling built-in like "gentype f(gentype
// x)"
...
// define macro HANDLE_BI_ONEARG_FRM for handling built-in like "gentype
// f(gentype x)" in fast relaxed mode
...
// define macro HANDLE_BI_ONEARG_SCALAR_OUT for handling built-in like "float
// f(floatn x)"
...
#define HANDLE_BI_THREEARG(_num, _aluname)                                     \
  if (#_aluname == BINameStr) {                                                \
    ...
    GET_ARG(1)                                                                 \
    GET_ARG(2)                                                                 \
    UBER_BI_CALL(_aluname,                                                     \
                 (ValArg0.NEATVal, ValArg1.NEATVal, ValArg2.NEATVal),          \
...
#define HANDLE_BI_ONEIN_TWOOUT_ARGS(_num, _aluname)                            \
  if (#_aluname == BINameStr) {                                                \
    ...
    GET_ARG(0)                                                                 \
    GET_ARG(1)                                                                 \
    NEATValue *pNeatVal = static_cast<NEATValue *>(ValArg1.PointerVal);        \
    VectorWidth vWidth = V1;                                                   \
    if (Arg0->getType()->isVectorTy())                                         \
      vWidth = ValArg0.NEATVec.GetWidth();                                     \
...
#define HANDLE_BI_SECOND_VECTOR_OR_SCALAR(_num, _aluname)                      \
  if (#_aluname == BINameStr) {                                                \
    Function::arg_iterator Fit = F->arg_begin();                               \
    GET_ARG(0) GET_ARG(1) if (Arg1->getType()->isVectorTy()) {                 \
      UBER_BI_CALL(_aluname, (ValArg0.NEATVal, ValArg1.NEATVal),               \
                   (ValArg0.NEATVec, ValArg1.NEATVec))                         \
    }                                                                          \
    else {                                                                     \
...
bool NEATPlugIn::DetectAndExecuteOCLBuiltins(
    Function *F, const std::map<Value *, NEATGenericValue> &ArgVals,
    NEATGenericValue &Result) {
  ...
  if (!OCLBuiltinParser::ParseOCLBuiltin(std::string(F->getName()), BINameStr,
                                         ArgList)) {
  ...
  // placed in top in order to call frm funtions instead of usual if frm
  // extension enabled
...
void NEATPlugIn::visitUIToFPInst(UIToFPInst &I) {
  ...
  if (isa<VectorType>(ST)) {
    ...
    vec.resize(cast<FixedVectorType>(RT)->getNumElements());
...
void *NEATPlugIn::getPointerToGlobalIfAvailable(const GlobalValue *GV) {
  ...
  GlobalAddressMapTy::iterator I = m_GlobalAddressMap.find(GV);
...
void *NEATPlugIn::getOrEmitGlobalVariable(const GlobalVariable *GV) {
  ...
  if (GA == NULL) {
    ...
    m_GlobalAddressMap.insert(
        std::pair<const GlobalValue *, void *const>(GV, Memory));
  }
