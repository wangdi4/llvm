...
template <class T> TypeVal InstTypeVal();

/// helper macro to instantiate InstTypeVal() function definition
#define DEF_INST_TYPEVAL(_TYPE, _TYPEVAL)                                      \
  template <> inline TypeVal InstTypeVal<_TYPE>() { return _TYPEVAL; }
...
public:
  /// default ctor
  TypeValWrapper() : m_value(UNSPECIFIED_TYPE) {
    if (!m_isStaticInit)
      initStatic();
  }
  ...
  TypeVal GetValue() const { return m_value; }
  ...
  bool IsComposite() const { return IsAggregate() || IsPointer(); }

  bool IsPointer() const { return m_value == TPOINTER; }

  bool IsStruct() const { return m_value == TSTRUCT; }

  bool IsVector() const { return m_value == TVECTOR; }

  bool IsInteger() const { return (m_value > TDOUBLE) && (m_value < TBOOL); }
...
private:
  ...
  inline void CheckValueAndThrow(const TypeVal &in_value) {
    ...
    if (m_metaData.count(in_value) < 1) {
      throw Exception::InvalidArgument(
          "Invalid arg. No metadata for this TypeDesc");
    }
  }
  ...
  class TypeMetadata {
  public:
    TypeMetadata() : m_size(0), m_isFloatingPoint(false), m_toString("") {}

    TypeMetadata(std::size_t size, bool isFloatingPoint,
...
public:
  TypeDesc()
      : m_numElements(0), m_offsetInStruct(0), m_type(UNSPECIFIED_TYPE),
        m_isNEAT(false), m_size(0) {}

  /// Constructor for aggregate data type description.
  /// \param in_type                      Type value.
  /// \param in_numOfElements             For array/vector data type - number of
  /// elements in array/vector.
  ///                                     For pointer - number of elements which
  ///                                     could be accessible through the
  ///                                     pointer, if it's known.
  /// \param in_numOfStructureElements    Number of structure members/fields.
  ///                                     Number of subtypes. It equals to 0 for
  ///                                     basic data types.
  TypeDesc(TypeVal in_type, uint64_t in_numOfElements = 0,
           uint64_t in_numOfStructureElements = 0)
      : m_numElements(in_numOfElements), m_offsetInStruct(0), m_type(in_type),
        m_isNEAT(false), m_size(0) {
    SetUpSubTypes(in_numOfStructureElements);
  }

  /// Getter/Setter for the number of the types contained into the current type.
  // If type is basic type not containing other types (except pointer) then 0
  // will be returned.
  ...
  /// Gets number of elements in array/vector or number of elements accessed by
  /// pointer.
  uint64_t GetNumberOfElements() const {
    if (m_type.GetValue() == TVECTOR || m_type.GetValue() == TARRAY ||
        m_type.GetValue() == TPOINTER)
      return m_numElements;
    return 0;
  }

  /// Sets number of elements in array/vector or number of elements accessed by
  /// pointer.
    ...
    if (num) {
      if (m_type.GetValue() == TVECTOR || m_type.GetValue() == TARRAY ||
        ...
        throw Exception::IllegalFunctionCall(
            std::string("Can't set number of elements for ") +
            m_type.ToString());
      }
    } else
      m_numElements = 0;
  ...
  void SetType(TypeVal in_type) {
    m_type.SetValue(in_type);
    SetUpSubTypes(0);
  }
  ...
  TypeDesc GetSubTypeDesc(uint64_t id) const {
    if (!IsComposite()) {
      throw Exception::IllegalFunctionCall(
          "GetSubTypeDesc: " + m_type.ToString() +
          std::string(" has no subtypes!"));
    }
    if (id >= m_subTypes.size()) {
      throw Exception::InvalidArgument(
          std::string(
              "Can't set sub-type description. Invalid sub-type index for ") +
          m_type.ToString() + std::string(" data type!"));
    }
  ...
  void SetSubTypeDesc(uint64_t id, const TypeDesc &in_typeDesc) {
    if (!IsComposite()) {
      throw Exception::IllegalFunctionCall(
          "SetSubTypeDesc: " + m_type.ToString() +
          std::string(" has no subtypes!"));
    }
    if (id >= m_subTypes.size()) {
      throw Exception::InvalidArgument(
  ...
  // It's primarily needed for 'struct' data types which assumes paddings and
  // alignment.
  void SetTypeAllocSize(std::size_t in_size) { m_size = in_size; }

  /// If current data types is used as 'struct' member, this method returns
  /// member offset in bytes.
  uint64_t GetOffsetInStruct() const { return m_offsetInStruct; }
  /// If current data types is used as 'struct' member, this method sets member
  /// offset in bytes.
  ...
  std::string TypeToString() const { return m_type.ToString(); }
  ...
  /// \return true if type is vector, array or structure
  bool IsAggregate() const { return m_type.IsAggregate(); }
  ...
  bool IsVector() const { return m_type.IsVector(); }

  bool IsInteger() const { return m_type.IsInteger(); }

  bool operator==(const TypeDesc &a) const;
  bool operator!=(const TypeDesc &a) const { return !(*this == a); }
...
private:
  ...
  // if data type is pointer - number of elements which could be accessible
  // through the pointer, if it's known. Default value = 1.
  ...
  // 1. Members of struct.
  // 2. Elements of array. Since all elements have the same type, the size of
  // m_subTypes must be = 1.
  // 3. Elements of vector. Since all elements have the same type, the size of
  // m_subTypes must be = 1.
  // 4. value which pointer refers to. Since all elements have the same type,
  // the size of m_subTypes must be = 1.
