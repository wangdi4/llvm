...
// MD5Code
//
MD5Code::MD5Code(unsigned char rawresult[16]) {
  std::copy(rawresult, rawresult + 16, m_res);
}

MD5Code::MD5Code(const MD5Code &that) { *this = that; }

MD5Code &MD5Code::operator=(const MD5Code &that) {
  if (this != &that)
    std::copy(that.m_res, that.m_res + 16, m_res);
  return *this;
}

bool MD5Code::operator==(const MD5Code &that) const {
  int i = 0;
  while (i < 16) {
    if (that.m_res[i] != m_res[i])
      return false;
    ++i;
  }
  return true;
...
std::ostream &operator<<(std::ostream &o, const MD5Code &code) {
  const unsigned char *strCode = code.code();
  char buf[49] = {0};
  int i = 0;
  while (i < 16) {
    sprintf(buf + (i * 3), "%02x ", strCode[i]);
    i++;
  }
  o << buf;
  return o;
...
void MD5::update(uint1 *input, uint4 input_length) {
  ...
  memcpy(m_buffer + buffer_index, input + input_index,
...
MD5::MD5(unsigned char *buffer, size_t len) {
  init();
  update(buffer, len);
  finalize();
}
...
MD5Code MD5::digest() {
  if (!finalized)
    throw("Can't get digest if you haven't finalized the digest!");
  MD5Code ret(m_digest);
  return ret;
}

const unsigned char *MD5Code::code() const { return m_res; }
