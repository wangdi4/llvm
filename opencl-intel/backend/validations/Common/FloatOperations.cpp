...
#define DEFINE_FLOAT_CMP(FUNC)\
        ...
        {
            uint80_t l = AsUInt(a);
            return (l.high_val == 0x7FFF && ((l.low_val & DOUBLE_80BIT_F_MASK) == 0));
        }
        ...
        {
            uint80_t l = AsUInt(a);
            return (l.high_val == 0xFFFF && ((l.low_val & DOUBLE_80BIT_F_MASK) == 0));
        }
        ...
        double ulpsDiffSamePrecision( long double reference,  long double testVal )
        {
            ...
            int x;
            if( 0.5L != CimathLibd::imf_frexp( reference, &x) )
            { // Non-power of two and NaN
