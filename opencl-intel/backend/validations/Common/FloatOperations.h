...
/// To use this trick, declare variable of type IsFloatType in the function,
/// using function template argument type. Type definition is specified only for
/// floating point types - float, double and CFloat16 This trick causes
/// Unreferenced local variable warning that can be disabled using
/// UNUSED_ARGUMENT macro defined above
template <typename T> class IsFloatType;
...
template <typename T> class IsIntegerType;
...
template <typename T> class IsScalarType;
...
template <typename T> struct IntStorage;

template <> struct IntStorage<float> {
  IntStorage() { val = 0xBAADF00D; }
  IntStorage(uint32_t in_val) { val = in_val; }
  uint32_t val;
};

template <> struct IntStorage<double> {
  IntStorage() { val = 0xBAADF00DBAADF00DUL; }
  IntStorage(uint64_t in_val) { val = in_val; }
  uint64_t val;
};

template <typename FloatT> struct FloatParts {
  ...
  void AddUlps(long ulp) {
    ...
    if (mant.val == 0 && exp.val == 0 && ulp < 0) {
      ...
      if (((uint64_t)abs((FloatT)ulp) >
           mant.val + (exp.val << GetSignificandSize())) &&
...
template <typename T> bool IsNaN(T a) {
  throw Exception::IllegalFunctionCall(
      "IsNaN was called with for integer type parameter");
}

template <typename T> bool IsPInf(T a) {
  throw Exception::IllegalFunctionCall(
      "IsPInf was called with for integer type parameter");
}

template <typename T> bool IsNInf(T a) {
  throw Exception::IllegalFunctionCall(
      "IsNInf was called with for integer type parameter");
}

template <typename T> bool IsInf(T a) {
  throw Exception::IllegalFunctionCall(
      "IsInf was called with for integer type parameter");
}

template <typename T> bool IsDenorm(T a) {
  throw Exception::IllegalFunctionCall(
      "IsDenorm was called with for integer type parameter");
}
...
/// operator >=
template <typename T> bool ge(T a, T b) {
  IsIntegerType<T> _notUsed;
  UNUSED_ARGUMENT(_notUsed);
  return (a >= b);
}
...
/// operator <=
template <typename T> bool le(T a, T b) {
  IsIntegerType<T> _notUsed;
  UNUSED_ARGUMENT(_notUsed);
  return (a <= b);
}
...
template <> bool lt(CFloat16 a, CFloat16 b);
template <> bool le(CFloat16 a, CFloat16 b);
template <> bool gt(CFloat16 a, CFloat16 b);
template <> bool ge(CFloat16 a, CFloat16 b);
template <> bool eq(CFloat16 a, CFloat16 b);
template <> bool lt(float a, float b);
template <> bool le(float a, float b);
template <> bool gt(float a, float b);
template <> bool ge(float a, float b);
template <> bool eq(float a, float b);
template <> bool lt(double a, double b);
template <> bool le(double a, double b);
template <> bool gt(double a, double b);
template <> bool ge(double a, double b);
template <> bool eq(double a, double b);
template <> bool lt(long double a, long double b);
template <> bool le(long double a, long double b);
template <> bool gt(long double a, long double b);
template <> bool ge(long double a, long double b);
template <> bool eq(long double a, long double b);

template <typename T> bool lt_float(T a, T b) {
...
/// operator ==
template <typename T> bool eq_float(T a, T b) {
  IsFloatType<T> _notUsed;
  UNUSED_ARGUMENT(_notUsed);
  if (IsNaN(a) || IsNaN(b))
    return false;
...
/// operator >=
template <typename T> bool ge_float(T a, T b) {
  IsFloatType<T> _notUsed;
  UNUSED_ARGUMENT(_notUsed);
  if (IsNaN(a) || IsNaN(b))
    return false;
...
/// operator <=
template <typename T> bool le_float(T a, T b) {
  IsFloatType<T> _notUsed;
  UNUSED_ARGUMENT(_notUsed);
  return ge_float(b, a);
}
...
/// operator >
template <typename T> bool gt_float(T a, T b) {
  IsFloatType<T> _notUsed;
  UNUSED_ARGUMENT(_notUsed);
  return lt_float(b, a);
}
...
inline uint80_t AsUInt(long double d) {
  ...
  memcpy(&x.high_val, (void *)((uint8_t *)&d + sizeof(x.low_val)),
         sizeof(x.high_val));
...
template <> bool IsNaN(CFloat16 a);

template <> bool IsInf(CFloat16 a);

template <> bool IsPInf(CFloat16 a);

template <> bool IsNInf(CFloat16 a);

template <> bool IsDenorm(CFloat16 a);
...
inline long double AsFloat(uint80_t d) {
  ...
  memcpy((void *)((uint8_t *)&x + sizeof(d.low_val)), &d.high_val,
         sizeof(d.high_val));
...
template <typename T> T Get1MinusEps();

template <> inline float Get1MinusEps() {
  return AsFloat((uint32_t)0x3F7FFFFF);
}

// TODO: test error for this function
template <> inline double Get1MinusEps() {
  return AsFloat((uint64_t)0x3FEFFFFFFFFFFFFF);
}

template <typename T> bool IsInteger(T val) { return (fmod(val, (T)1.0) == 0); }
template <typename T> bool IsOdd(T val) {
  return (fmod(fabs(val), (T)2.0) == 1.0);
}
template <typename T> bool IsEven(T val) {
  return (fmod(fabs(val), (T)2.0) == 0.0);
}

/// @brief returns negative infinity of specified type
template <typename T> T GetNInf();
/// @brief returns positive infinity of specified type
template <typename T> T GetPInf();
/// @brief returns NaN of specified type
template <typename T> T GetNaN();
...
template <> inline float GetNInf() {
  return -std::numeric_limits<float>::infinity();
}

template <> inline double GetNInf() {
  return -std::numeric_limits<double>::infinity();
}

template <> inline long double GetNInf() {
  return (long double)(-std::numeric_limits<double>::infinity());
}

template <> inline CFloat16 GetNInf() { return CFloat16::GetNInf(); }

template <> inline float GetPInf() {
  return std::numeric_limits<float>::infinity();
}

template <> inline double GetPInf() {
  return std::numeric_limits<double>::infinity();
}

template <> inline long double GetPInf() {
  return (long double)std::numeric_limits<double>::infinity();
}

template <> inline CFloat16 GetPInf() { return CFloat16::GetPInf(); }

template <> inline float GetNaN() {
  return std::numeric_limits<float>::quiet_NaN();
}

template <> inline double GetNaN() {
  return std::numeric_limits<double>::quiet_NaN();
}

template <> inline long double GetNaN() {
  return (long double)std::numeric_limits<double>::quiet_NaN();
}

template <> inline CFloat16 GetNaN() { return CFloat16::GetNaN(); }

/// "Super" precision types for floating point type
template <typename T> class superT;

/// float is "super" format for CFloat16
template <> class superT<CFloat16> {
public:
  typedef float type;
};
...
template <>
inline bool eq_tol<CFloat16>(const CFloat16 &a, const CFloat16 &b,
                             const double &) {
