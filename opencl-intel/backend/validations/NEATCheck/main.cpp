...
// Utility for NEATPlugin validation. It is command line utility for NEAT plugin
// validation.  It compares data from reference file with output data provided
// via standard input.  Reference file has comments starting with ";CHECKNEAT:"
// that contains NEAT data separated by spaces.
//
...
cl::opt<string>
    ReferenceFilename(cl::Required, "r",
                      cl::desc("Specify reference pattern filename"),
                      cl::value_desc("filename"));

cl::opt<string>
    ActualOutFilename(cl::Optional, "a",
                      cl::desc("Specify actual ouptut filename. It should "
                               "contain buffers in DataManager format"),
                      cl::value_desc("filename"), cl::init("-"));

cl::opt<unsigned> Tolerance(cl::Optional, "t",
                            cl::desc("Specify comparison tolerance. If not "
                                     "specified, precise comparison is used."),
                            cl::init(1000000000), cl::value_desc("tolerance"));
...
bool Equal(double a, double b) { return (ulpsDiff(a, b) <= Tolerance); }
...
void ReportError(const string &outputStr, const string &refStr,
  ...
  case MT_UNKNOWN_TYPE:
    cout << "Unknown type of status detected. Both reference and output "
            "statuses are equal to ";
    cout << outputStr;
    cout << ", Line # ";
    cout << refLine << endl;
    break;
...
template <typename T>
void StringifyTypedBuffer(stringstream &inout_Stream,
                          const IMemoryObject *in_pBuf) {
    ...
    for (uint32_t iV = 0; iV < desc.NumOfElements(); iV++) {
      ...
      for (uint32_t off = 0; off < desc.SizeOfVector(); off++) {
        ...
        case NEATValue::INTERVAL:
          inout_Stream << " INTERVAL " << *NeatOut.GetMin<T>() << " "
                       << *NeatOut.GetMax<T>() << " ";
          break;
        }
      }
...
void StringifyBuffer(stringstream &inout_Stream, IMemoryObject *in_pBuf,
                     const BufferDesc &in_bufDesc) {
  TypeVal elemType =
      (in_bufDesc.GetElementDescription().IsComposite() &&
       !in_bufDesc.GetElementDescription().IsStruct())
  ...
  case UNSPECIFIED_TYPE:
    throw Exception::InvalidArgument(
        "Cannot compare buffers with unspecified data type.");
    break;
  case INVALID_TYPE:
  ...
  default:
    throw Exception::InvalidArgument(
        "StringifyBuffer: Unsupported data type of buffer elements!");
    break;
  }
}
...
void DMtoStringStream(const string &in_filename, stringstream &inout_pSs) {
  ...
  for (uint32_t BCIndex = 0; BCIndex < inp.GetBufferContainerCount();
       BCIndex++) {
  ...
  if (Reference.is_open()) {
    ...
    while (Reference.good()) {
      ...
      if (found != string::npos) {
        ProcessString(outStream, referenceString, found + checkKeyword.size(),
                      lineNum);
      }
      lineNum++;
    }
