...
public:
  ...
  OCLBuiltinParser::BasicArgType operator[](const std::string &str) const {
    std::map<std::string, OCLBuiltinParser::BasicArgType>::const_iterator it =
        m_Map.find(str);
    if (it == m_Map.end())
...
public:
  ...
  reflection::TypePrimitiveEnum
  operator[](const OCLBuiltinParser::BasicArgType &type) const {
    std::map<OCLBuiltinParser::BasicArgType,
             reflection::TypePrimitiveEnum>::const_iterator it =
        m_Map.find(type);
    if (it == m_Map.end())
...
private:
  void initStatic();
  static std::map<OCLBuiltinParser::BasicArgType, reflection::TypePrimitiveEnum>
      m_Map;
};
...
void BasicTypeToStr::initStatic() {
  ...
  m_Map[OCLBuiltinParser::IMAGE_2D_DEPH_T] =
      reflection::PRIMITIVE_IMAGE_2D_DEPTH_T;
  m_Map[OCLBuiltinParser::IMAGE_3D_T] = reflection::PRIMITIVE_IMAGE_3D_T;
  m_Map[OCLBuiltinParser::IMAGE_1D_BUFFER_T] =
      reflection::PRIMITIVE_IMAGE_1D_BUFFER_T;
  m_Map[OCLBuiltinParser::IMAGE_1D_ARRAY_T] =
      reflection::PRIMITIVE_IMAGE_1D_ARRAY_T;
  m_Map[OCLBuiltinParser::IMAGE_2D_ARRAY_T] =
      reflection::PRIMITIVE_IMAGE_2D_ARRAY_T;
  m_Map[OCLBuiltinParser::IMAGE_2D_ARRAY_DEPH_T] =
...
std::map<OCLBuiltinParser::BasicArgType, reflection::TypePrimitiveEnum>
    BasicTypeToStr::m_Map;
...
struct GetTypeVisitor : reflection::TypeVisitor {
  ...
  llvm::OCLBuiltinParser::ARG getArg() { return newArg; }
  ...
  void visit(const reflection::UserDefinedType *p) override {
    ...
    if (found_1d != std::string::npos || found_2d != std::string::npos ||
        found_3d != std::string::npos) {
...
bool OCLBuiltinParser::GetOCLMangledName(const std::string &in_funcName,
  ...
  fd.Name = in_funcName;
  // the parameters pointed in function descriptor should exist at the time of
  // mangle operation, because reflection::FunctionDescriptor::parameters so we
  // create parameters and put pointers on them to the vector in order to delete
  // them later, after the mangle operation will be completed
  ...
  for (std::size_t i = 0; i < in_args.size(); ++i) {
    ...
    case OCLBuiltinParser::BASIC: {
      // create parameter
      reflection::PrimitiveType *primitiveType =
          new reflection::PrimitiveType(typeConvertor[arg.basicType]);
      fd.Parameters.push_back(primitiveType);
      break;
    }
    case OCLBuiltinParser::VECTOR: {
      reflection::RefParamType primitiveType(
          new reflection::PrimitiveType(typeConvertor[arg.vecType.elType]));
      reflection::VectorType *vectorType =
          new reflection::VectorType(primitiveType, (int)arg.vecType.elNum);
    ...
    case OCLBuiltinParser::POINTER: {
      ...
      case OCLBuiltinParser::BASIC: {
        reflection::RefParamType primitiveType(new reflection::PrimitiveType(
            typeConvertor[arg.ptrType.ptrType[0].basicType]));
      ...
      case OCLBuiltinParser::VECTOR: {
        reflection::RefParamType primitiveType(new reflection::PrimitiveType(
            typeConvertor[arg.ptrType.ptrType[0].vecType.elType]));
        reflection::VectorType *vectorType = new reflection::VectorType(
            primitiveType, (int)arg.ptrType.ptrType[0].vecType.elNum);
