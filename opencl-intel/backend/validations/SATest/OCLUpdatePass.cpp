...
extern "C" int getVectorizerFunctions(Pass *V,
...
public:
  OCLReferenceKernelUpdate(Pass *pVect,
...
protected:
  ...
  Value *SubstituteWIcall(llvm::CallInst *pCall, llvm::Argument *pWorkInfo,
                          llvm::Argument *pWGid, llvm::Argument *pBaseGlbId,
                          llvm::Argument *pLocalId);
  Value *CalcGlobalId(llvm::CallInst *pCall, llvm::Argument *pWGinfo,
                      llvm::Argument *pLocalId, bool bGlobal);
  ...
  friend void getKernelInfoMap(ModulePass *pKUPath,
                               map<const Function *, TLLVMKernelInfo> &infoMap);
...
const char *OCLReferenceKernelUpdate::builtin_printf_name = "__opencl_printf";
char OCLReferenceKernelUpdate::ID = 0;

ModulePass *createOCLReferenceKernelUpdatePass(
    Pass *pVect, SmallVectorImpl<Function *> &vectFunctions,
    LLVMContext *context,
    std::vector<std::string> &UndefinedExternalFunctions) {
  return new OCLReferenceKernelUpdate(pVect, vectFunctions, context,
                                      UndefinedExternalFunctions);
}

BasicBlock::iterator removeInstruction(llvm::BasicBlock *pBB,
                                       llvm::BasicBlock::iterator it);

void getKernelInfoMap(ModulePass *pKUPath,
                      map<const Function *, TLLVMKernelInfo> &infoMap) {
  OCLReferenceKernelUpdate *pKU =
      static_cast<OCLReferenceKernelUpdate *>(pKUPath);
...
bool OCLReferenceKernelUpdate::runOnModule(Module &M) {
  ...
  // now we need to pass every kernel and make relevant changes to match our
  // environment
  for (unsigned i = 0, e = m_pOpenCLMetadata->getNumOperands(); i != e; ++i) {
    // Obtain kernel function from annotation
    llvm::MDNode *elt = m_pOpenCLMetadata->getOperand(i);
    Function *pFunc =
        dyn_cast<Function>(elt->getOperand(0)->stripPointerCasts());
    if (NULL == pFunc) {
      continue; // Not a function pointer
    }

    NamedMDNode *localsAnchor = m_pModule->getNamedMetadata(
        dyn_cast<llvm::MDString>(elt->getOperand(5))->getString());
    ...
    Function *pVectFunc;
    if (!m_pVectFunctions->empty() &&
        ((pVectFunc = (*m_pVectFunctions)[i]) != NULL)) {
...
Instruction *CreateInstrFromConstantExpr(ConstantExpr *pCE, Value *From,
                                         Value *To) {
  ...
  if (pCE->getOpcode() == Instruction::GetElementPtr) {
    ...
    Replacement =
        GetElementPtrInst::Create(Pointer, ArrayRef<Value *>(Indices));
    ...
    Replacement = CastInst::Create((Instruction::CastOps)pCE->getOpcode(), To,
                                   pCE->getType());
    ...
    if (C2 == From)
      C2 = To;
    Replacement = CmpInst::Create((Instruction::OtherOps)pCE->getOpcode(),
                                  pCE->getPredicate(), C1, C2);
  } else {
...
bool ChangeConstantExpression(Value *pTheValue, Value *pUser, Instruction *pBC,
                              Instruction *Where) {
  ...
  while (itCE != pCE->use_end()) {
    ...
    if ((NULL == pUsrInst) || (pUsrInst->getParent()->getParent() !=
                               Where->getParent()->getParent())) {
      ++itCE;
      continue;
    }
...
// parameters
bool OCLReferenceKernelUpdate::ParseLocalBuffers(Function *pFunc,
                                                 NamedMDNode *localsAnchor,
                                                 Argument *pLocalMem) {
  if (NULL == localsAnchor)
    return true;

  // Create code for local buffer
  Instruction *pFirstInst =
      dyn_cast<Instruction>(pFunc->getEntryBlock().begin());
  ...
  for (unsigned i = 0, e = localsAnchor->getNumOperands(); i != e; ++i) {
    ...
    GetElementPtrInst *pLocalAddr = GetElementPtrInst::Create(
        ...
        ConstantInt::get(IntegerType::get(*m_pLLVMContext, 32),
                         m_sInfo.stTotalImplSize),
        "", pFirstInst);
    // Now add bitcast to required/original pointer type
    CastInst *pBitCast = CastInst::CreatePointerCast(
        pLocalAddr, pLclBuff->getType(), "", pFirstInst);
    ...
    while (itVal != pLclBuff->use_end()) {
      ...
      if (dyn_cast<Instruction>(U.getUser())->getParent()->getParent() ==
          pFunc) {
        U.set(pBitCast);
        itVal = pLclBuff->use_begin();
      } else {
...
Value *OCLReferenceKernelUpdate::SubstituteWIcall(llvm::CallInst *pCall,
                                                  llvm::Argument *pWorkInfo,
                                                  llvm::Argument *pWGid,
                                                  llvm::Argument *pBaseGlbId,
                                                  llvm::Argument *pLocalId) {
  ...
  if (pFuncName == "get_work_dim") {
    ...
    GetElementPtrInst *pDimCntAddr = GetElementPtrInst::Create(
        pWorkInfo, ArrayRef<Value *>(params), "", pCall);
    // Load the Value
  ...
  if (iTableInx > 0) {
    ...
    params.push_back(
        ConstantInt::get(IntegerType::get(*m_pLLVMContext, 32), 0));
    params.push_back(
        ConstantInt::get(IntegerType::get(*m_pLLVMContext, 32), iTableInx));
    params.push_back(pCall->getArgOperand(0));
    GetElementPtrInst *pSizeAddr = GetElementPtrInst::Create(
        pWorkInfo, ArrayRef<Value *>(params), "", pCall);
    // Load the Value
  ...
  if (pFuncName == "get_local_id") {
    ...
    params.push_back(
        ConstantInt::get(IntegerType::get(*m_pLLVMContext, 32), 0));
    params.push_back(pCall->getArgOperand(0));
    GetElementPtrInst *pIdAddr = GetElementPtrInst::Create(
        pLocalId, ArrayRef<Value *>(params), "", pCall);
    // Load the Value
  ...
  if (pFuncName == "get_global_id") {
    ...
    if (NULL != pVal) {
      ...
      if (NULL ==
          m_GlbIds[uiDim]) // Check if we already have value for this dimension
      {
...
llvm::Value *OCLReferenceKernelUpdate::CalcGlobalId(llvm::CallInst *pCall,
                                                    llvm::Argument *pBaseGlbId,
                                                    llvm::Argument *pLocalId,
                                                    bool bGlobal) {
  ...
  GetElementPtrInst *pLclIdAddr = GetElementPtrInst::Create(
      pLocalId, ArrayRef<Value *>(params), "", nextInst);
  // Load the value of local id
  Value *pLocalIdVal = new LoadInst(pLclIdAddr, "", nextInst);

  // Load the value of base global index
  GetElementPtrInst *pGlbBaseAddr = GetElementPtrInst::Create(
      pBaseGlbId, pCall->getArgOperand(0), "", nextInst);
  // Load the Value
  Value *pBaseGlbIdVal = new LoadInst(pGlbBaseAddr, "", nextInst);

  // Now add these two values
  Value *pGlbId =
      BinaryOperator::CreateAdd(pLocalIdVal, pBaseGlbIdVal, "", nextInst);

  return pGlbId;
}

void OCLReferenceKernelUpdate::AddCtxAddress(llvm::CallInst *pCall,
                                             llvm::Argument *pLocalId) {
  ...
  params.push_back(
      ConstantInt::get(IntegerType::get(*m_pLLVMContext, 32), MAX_WORK_DIM));

  m_pCtxPtr = GetElementPtrInst::Create(
      pLocalId, ArrayRef<Value *>(params), "",
      pCall->getParent()->getParent()->getEntryBlock().begin());
}

Value *OCLReferenceKernelUpdate::UpdatePrintf(CallInst *pCall,
                                              Argument *pLocalId) {
  if (NULL == m_pCtxPtr) {
  ...
  //
  assert(pCall->getNumArgOperands() > 0 &&
  ...
  AllocaInst *buf_alloca_inst =
      new AllocaInst(buf_arr_type, "temp_arg_buf", pCall);
  ...
  for (unsigned numarg = 1; numarg < pCall->getNumArgOperands(); ++numarg) {
    ...
    //
    BitCastInst *cast_instr = CastInst::CreatePointerCast(
        gep_instr, PointerType::getUnqual(argtype), "", pCall);
  ...
  Value *res = CallInst::Create(pFunc, ArrayRef<Value *>(params),
                                "translated_opencl_printf_call", pCall);
  return res;
}

Value *OCLReferenceKernelUpdate::UpdateAsyncCopy(llvm::CallInst *pCall,
                                                 llvm::Argument *pLocalId,
                                                 bool strided) {
  if (NULL == m_pCtxPtr) {
    AddCtxAddress(pCall, pLocalId);
  ...
  const PointerType *pPTy =
      dyn_cast<PointerType>(pCall->getArgOperand(0)->getType());
  ...
  params.push_back(ConstantInt::get(
      IntegerType::get(*m_pLLVMContext, sizeof(size_t) * BYTE_SIZE), uiSize));
  params.push_back(m_pCtxPtr);
  assert(pPTy && "Must by a pointer type");

  Function *pNewAsyncCopy = NULL;
  if (strided) {
    pNewAsyncCopy = m_pModule->getFunction(pPTy->getAddressSpace() == 3
                                               ? "lasync_wg_copy_strided_g2l"
                                               : "lasync_wg_copy_strided_l2g");
  } else {
    pNewAsyncCopy = m_pModule->getFunction(pPTy->getAddressSpace() == 3
                                               ? "lasync_wg_copy_g2l"
                                               : "lasync_wg_copy_l2g");
  ...
  Value *res =
      CallInst::Create(pNewAsyncCopy, ArrayRef<Value *>(params), "", pCall);
  return res;
}

void OCLReferenceKernelUpdate::UpdateWaitGroup(llvm::CallInst *pCall,
                                               llvm::Argument *pLocalId) {
  if (NULL == m_pCtxPtr) {
    AddCtxAddress(pCall, pLocalId);
  }
...
void OCLReferenceKernelUpdate::UpdatePrefetch(llvm::CallInst *pCall) {
  ...
  // push original parameters
  // Need bitcast to a general pointer
  CastInst *pBCPtr = CastInst::CreatePointerCast(
      pCall->getArgOperand(0),
      PointerType::get(IntegerType::get(*m_pLLVMContext, 8), 0), "", pCall);
  ...
  params.push_back(
      ConstantInt::get(IntegerType::get(*m_pLLVMContext, uiSizeT), uiSize));
  Function *pPrefetch = m_pModule->getFunction("__lprefetch");
  CallInst::Create(pPrefetch, ArrayRef<Value *>(params), "", pCall);
}

BasicBlock::iterator removeInstruction(BasicBlock *pBB,
...
int CalculateKernelLocalsSize(Function *pFunc) {
  ...
  if (NULL != pFuncElt) {
    ...
    NamedMDNode *localsAnchor = pModule->getNamedMetadata(
        dyn_cast<llvm::MDString>(pFuncElt->getOperand(5))->getString());
    ...
    if (NULL != localsAnchor) {
      ...
      for (unsigned i = 0, e = localsAnchor->getNumOperands(); i != e; ++i) {
        ...
        GlobalValue *pLclBuff = pModule->getGlobalVariable(
            dyn_cast<llvm::MDString>(operand->getOperand(0))->getString(),
            true);
        ...
        // Calculate required buffer size
        const ArrayType *pArray =
            dyn_cast<ArrayType>(pLclBuff->getType()->getElementType());
        unsigned int uiArraySize =
            pArray ? 1 : pLclBuff->getValueType()->getPrimitiveSizeInBits() / 8;
        assert(0 != uiArraySize);
        while (NULL != pArray) {
          uiArraySize *= (unsigned int)pArray->getNumElements();
          if (pArray->getContainedType(0)->getTypeID() != Type::ArrayTyID) {
            uiArraySize *=
                pArray->getContainedType(0)->getPrimitiveSizeInBits() / 8;
          }
  ...
  Function::BasicBlockListType::iterator bb_it =
      pFunc->getBasicBlockList().begin();
  while (bb_it != pFunc->getBasicBlockList().end()) {
    ...
    while (inst_it != bb_it->end()) {
      ...
      int iCallLocalSize =
          CalculateKernelLocalsSize(pCall->getCalledFunction());
    ...
    while (inst_it != bb_it->end()) {
      ...
      if ((("get_" == pCallee->getNameStr().substr(0, 4)) &&
          ...
          ("_Z21async_work_group_copy" ==
           pCallee->getNameStr().substr(0, 25)) ||
      ...
      while ((iter != m_nonInlinedFunctions.end()) && (*iter != pCallee))
        iter++;
...
Function *OCLReferenceKernelUpdate::RunOnKernel(Function *pFunc,
                                                NamedMDNode *localsAnchor) {
  ...
  // newArgsVec.push_back(PointerType::get(m_pModule->getTypeByName(
  // "struct.WorkDim"), 0));
  newArgsVec.push_back(
      PointerType::get(IntegerType::get(*m_pLLVMContext, uiSizeT), 0));
  newArgsVec.push_back(
      PointerType::get(IntegerType::get(*m_pLLVMContext, uiSizeT), 0));
  ...
  FunctionType *FTy = FunctionType::get(
      pFunc->getReturnType(), ArrayRef<llvm::Type *>(newArgsVec), false);
  ...
  // Our solution to move all internal local memory blocks to be allocated by
  // the execution engine and passed within additional parameters to the kernel,
  // those parameters are not exposed to the user
  std::map<Function *, int> LocalsMap;

  // Go through function blocks
  Function::BasicBlockListType::iterator bb_it =
      NewF->getBasicBlockList().begin();
    ...
    while (inst_it != bb_it->end()) {
      ...
      case Instruction::Call: {
        ...
        if ("get_" == calledFuncName.substr(0, 4)) {
          ...
          Value *pNewRes =
              SubstituteWIcall(pCall, pWorkDim, pWGId, pBaseGlbId, pLocalId);
          if (NULL != pNewRes) {
        ...
        if ((NULL != pCallee) && (pCallee->isDeclaration()) &&
            pCallee->getNameStr().compare(0, 4, "llvm")) {
          void *Ptr = llvm::sys::DynamicLibrary::SearchForAddressOfSymbol(
              pCallee->getNameStr());
          if (NULL == Ptr) {
            // report error
            m_pUndefinedExternalFunctions->push_back(
                pCallee->getNameStr() + " in function " + NewF->getNameStr());
          }
          ++inst_it;
          break;
        }
  ...
  for (; MDIter != EndMDIter; MDIter++) {
    for (int ui = 0, ue = MDIter->getNumOperands(); ui < ue; ui++) {
      ...
      MDNode *pNewMetadata =
          MDNode::get(*m_pLLVMContext, ArrayRef<Value *>(values));
...
void OCLReferenceKernelUpdate::AddWIInfoDeclarations() {
  ...
  */
  ...
  members.push_back(ArrayType::get(IntegerType::get(*m_pLLVMContext, uiSizeT),
                                   MAX_WORK_DIM)); // Local Id's
  StructType *pLocalId =
      StructType::get(*m_pLLVMContext, ArrayRef<Type *>(members), true);
  ...
  */
  ...
  members.push_back(ArrayType::get(IntegerType::get(*m_pLLVMContext, uiSizeT),
                                   MAX_WORK_DIM)); // Global offset
  members.push_back(ArrayType::get(IntegerType::get(*m_pLLVMContext, uiSizeT),
                                   MAX_WORK_DIM)); // Global size
  // Two dimensional array of Unifrom and NonUniform Local sizes
  members.push_back(ArrayType::get(
      ArrayType::get(IntegerType::get(*m_pLLVMContext, uiSizeT, MAX_WORK_DIM),
                     WG_SIZE_NUM));
  // Number of groups
  members.push_back(ArrayType::get(IntegerType::get(*m_pLLVMContext, uiSizeT),
                                   MAX_WORK_DIM));
...
void OCLReferenceKernelUpdate::AddAsyncCopyDeclaration() {
  ...
  // event_t async_work_group_copy(void* pDst, void* pSrc, size_t numElem,
  // event_t event, size_t elemSize, LLVMExecMultipleWIWithBarrier*
  // *ppExec);
  ...
  params.push_back(
      PointerType::get(IntegerType::get(*m_pLLVMContext, uiSizeT), 0));
  FunctionType *pNewType =
      FunctionType::get(IntegerType::get(*m_pLLVMContext, uiSizeT),
                        ArrayRef<Type *>(params), false);
  Function::Create(pNewType, (GlobalValue::LinkageTypes)0, "lasync_wg_copy_l2g",
                   m_pModule);
  Function::Create(pNewType, (GlobalValue::LinkageTypes)0, "lasync_wg_copy_g2l",
                   m_pModule);

  // event_t async_work_group_strided_copy(void* pDst, void* pSrc, size_t
  // numElem, size_t stride, event_t event,
  ...
  params.push_back(
      PointerType::get(IntegerType::get(*m_pLLVMContext, uiSizeT), 0));
  pNewType = FunctionType::get(IntegerType::get(*m_pLLVMContext, uiSizeT),
                               ArrayRef<Type *>(params), false);
  Function::Create(pNewType, (GlobalValue::LinkageTypes)0,
                   "lasync_wg_copy_strided_l2g", m_pModule);
  Function::Create(pNewType, (GlobalValue::LinkageTypes)0,
                   "lasync_wg_copy_strided_g2l", m_pModule);
  ...
  params.push_back(
      PointerType::get(IntegerType::get(*m_pLLVMContext, uiSizeT), 0));
  FunctionType *pWaitType = FunctionType::get(Type::getVoidTy(*m_pLLVMContext),
                                              ArrayRef<Type *>(params), false);
  Function::Create(pWaitType, (GlobalValue::LinkageTypes)0,
                   "lwait_group_events", m_pModule);
...
void OCLReferenceKernelUpdate::AddPrintfDeclaration() {
  ...
  params.push_back(PointerType::get(
      IntegerType::get(*m_pLLVMContext, sizeof(size_t) * BYTE_SIZE), 0));

  FunctionType *pNewType = FunctionType::get(Type::getInt32Ty(*m_pLLVMContext),
                                             ArrayRef<Type *>(params), false);
  Function::Create(pNewType, Function::ExternalLinkage, builtin_printf_name,
                   m_pModule);
...
void OCLReferenceKernelUpdate::AddPrefetchDeclaration() {
  ...
  // Element size
  params.push_back(IntegerType::get(*m_pLLVMContext, uiSizeT));
  FunctionType *pNewType = FunctionType::get(Type::getVoidTy(*m_pLLVMContext),
                                             ArrayRef<Type *>(params), false);
