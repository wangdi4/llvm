...
static QTreeWidgetItem *treeWidgetFactory(QTreeWidgetItem *parent,
                                          QString text) {
...
FileTree::FileTree(QWidget *parent) : QTreeWidget(parent) {
  ...
  connect(this, SIGNAL(itemDoubleClicked(QTreeWidgetItem *, int)), this,
          SLOT(catchDoubleClicked(QTreeWidgetItem *, int)));
...
void FileTree::setConfig(ConfigManager *cfg) {
  this->cfg = cfg;
  this->setHeaderLabel(
      QString::fromStdString(cfg->getReader()->GetProgramName()));
  ///
  addEntry(treeWidgetFactory(this, cfg->getCfgFileName()), Entry::configuration,
           cfg->getCfgFilePath());
  addEntry(treeWidgetFactory(this, cfg->getTstFileName()), Entry::tst,
           cfg->getTstFilePath());
  addEntry(treeWidgetFactory(
               this, getFileName(cfg->getReader()->GetProgramFilePath())),
           Entry::cl,
           QString::fromStdString(cfg->getReader()->GetProgramFilePath()));

  QString kernelsStr =
      "Kernels (" +
      QString::number(cfg->getReader()->GetNumberOfKernelConfigurations()) +
      ")";
  ...
  for (int i = 0; i < kernels->size(); i++) {
    ...
    addEntry(in, Entry::in, kernel->inFilePath());
    ...
    for (int j = 0; j < inbuffers->size(); j++) {
      addEntry(treeWidgetFactory(in, QString("Buffer") + QString::number(j)),
               Entry::buffer, Entry::inBuffer, i, j);
    }

    QTreeWidgetItem *ref = treeWidgetFactory(kernelItem, kernel->refFileName());
    addEntry(ref, Entry::ref, kernel->refFileName());
    ...
    for (int j = 0; j < refbuffers->size(); j++) {
      addEntry(treeWidgetFactory(ref, QString("Buffer") + QString::number(j)),
               Entry::buffer, Entry::refBuffer, i, j);
    }

    QTreeWidgetItem *neat =
        treeWidgetFactory(kernelItem, kernel->neatFileName());
    addEntry(neat, Entry::neat, kernel->neatFilePath());
    ...
    for (int j = 0; j < neatbuffers->size(); j++) {
      addEntry(treeWidgetFactory(neat, QString("Buffer") + QString::number(j)),
               Entry::buffer, Entry::neatBuffer, i, j);
    }
  }
}

Entry FileTree::getEntry(QTreeWidgetItem *itm) { return entries[itm]; }
...
void FileTree::addEntry(QTreeWidgetItem *itm, Entry::Type type,
                        Entry::BufferType buftype, int kernelId,
                        int bufferNum) {
  Entry entry;
  entry.type = type;
