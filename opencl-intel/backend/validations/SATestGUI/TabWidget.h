...
public:
  ...
  /**
   * @brief setSaveAction - set action from "Save" button from ToolBar from
   * MainWindow
   */
...
public slots:
  ...
  /**
   * @brief showRunVariants - show set of run options (parsed from *.tst or
   * *.ini file) in CommandArgumentsTab
   */
  ...
  /**
   * @brief catchSaveAction - cath action from "Save" button and do some actions
   * what need depends on current Tab
   */
  void catchSaveAction();

private:
  /**
   * @brief checkTab - check if tab is currently oppenned. if openned - show
   * its, else - create tab and set focus on its
