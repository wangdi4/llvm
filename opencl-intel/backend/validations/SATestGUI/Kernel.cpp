...
Kernel::Kernel(KernelInfo info) {
  ...
  qDebug() << "reading in";
  parseFile(infile, inBuffers, info.infile, info.infileType,
            &inBufferContainerList);
  qDebug() << "reading ref";
  parseFile(reffile, refBuffers, info.reffile, info.reffileType,
            &refBufferContainerList);
  qDebug() << "reading neat";
  parseFile(neatfile, neatBuffers, info.neatfile, info.neatfileType,
            &neatBufferContainerList);
}

static void saveBuffer(const QString &qstr, const DataFileType &fileType,
                       const BufferContainerList *bufferContainerList) {
...
void Kernel::parseFile(QFile *_file, QVector<BufferConverter *> *buffers,
  ...
  if (_file->exists() && _file->size() != 0) {
    ...
    *bufferContainerList = new BufferContainerList();
    ...
    BufferContainer *bufferContainer = static_cast<BufferContainer *>(
        (*bufferContainerList)->GetBufferContainer(0));
