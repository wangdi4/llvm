...
public:
  ...
  /**
   * @brief lenght
   * @return the lenght of buffer. If buffer contains 2 vecors of float 4, the
   * length is 2
   */
  const int lenght();
  /**
   * @brief width
   * @return the width of vector inside the buffer. If buffer contains 2 vecors
   * of float 4, the width is 4
   */
  const int width();
  /**
   * @brief typeName
...
private:
  ...
  Buffer *m_pBuffer; // BufferConverter is not an owner of this buffer, so we
                     // can't delete it in dtor
  /**
