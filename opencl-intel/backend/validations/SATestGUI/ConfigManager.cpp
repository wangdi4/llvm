...
ConfigManager::ConfigManager() { kernels = new QVector<Kernel *>; }

ConfigManager::~ConfigManager() { delete kernels; }
...
ConfigManager::ConfReader *ConfigManager::getReader() { return reader; }
...
QString ConfigManager::workingDirectory() { return info.absolutePath(); }

QString ConfigManager::getCfgFilePath() { return info.absoluteFilePath(); }

QString ConfigManager::getProgramName() {
  return QString::fromStdString(reader->GetProgramName());
}

QStringList ConfigManager::getArgs() { return this->params; }
...
void ConfigManager::parseTst() {
  ...
  if (!tstFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
    QMessageBox msgBox;
    QString message =
        getTstFileName() + QString(" failed to open ") + tstFile.errorString();
...
void ConfigManager::parseKernels() {
  ...
  for (it = this->getReader()->beginKernels();
       it != this->getReader()->endKernels(); ++it) {
    KernelInfo kernel;
