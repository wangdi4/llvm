...
BufferConverter::BufferConverter() : m_pBuffer(NULL) {}

BufferConverter::BufferConverter(Buffer *buf) {

  const BufferDesc *bufdesc =
      static_cast<const BufferDesc *>(buf->GetMemoryObjectDesc());
  if (bufdesc) {
    m_pBuffer = buf;
...
const void BufferConverter::setVal(const int i, const int j,
                                   const QString sval) {
  switch (m_type) {
  case THALF:
    /**
...
template <class T> static QString BufferConverter::toString(T number) {
  return QString::number(number);
}

template <class T> static QString BufferConverter::toHex(T arg) {
  QString str;
...
template <class T> static void BufferConverter::fromHex(QString str, T *arg) {
  str.remove(0, 2);
  T val_bytes;
  T val_revbytes;
...
template <class T> T BufferConverter::at(int i, int j, T *data) {
...
static double parseDouble(QString str) {
  ...
  val = sm *
        ldexp((double)_significand, (iexp * se + RefALU::ilogb((double)_log1) -
                                     RefALU::ilogb((double)_log2)));
  return val;
}
/**
...
static float parseFloat(QString str) {
  ...
  val = sm *
        ldexpf((float)_significand, (iexp * se + RefALU::ilogb((float)_log1) -
                                     RefALU::ilogb((float)_log2)));
