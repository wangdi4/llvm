...
MainWindow::MainWindow(QWidget *parent)
  ...
  //
  QString config = QFileDialog::getOpenFileName(this, "Select config file",
  ...
  connect(satest, SIGNAL(finished(int, QProcess::ExitStatus)), this,
          SLOT(updatetext(int, QProcess::ExitStatus)));
  ...
  connect(filetree, SIGNAL(showKernelInfo(Entry)), tabs,
          SLOT(showKernel(Entry)));
  connect(filetree, SIGNAL(showConfig(Entry)), tabs, SLOT(showConfig(Entry)));
  connect(filetree, SIGNAL(showRunVariants(Entry)), tabs,
          SLOT(showRunVariants(Entry)));
}
...
void MainWindow::createActions() {
  ...
  runsatestaction =
      new QAction(QIcon(":/icons/Run.png"), QString("Run SATest"), this);
  ...
  showvardialogaction = new QAction(QIcon(":/icons/Env.png"),
                                    QString("Edit Enviroment Variables"), this);
  ui->mainToolBar->addAction(showvardialogaction);
  connect(showvardialogaction, SIGNAL(triggered()), this,
          SLOT(showEnvDialog()));
  editpathsaction =
      new QAction(QIcon(":/icons/Path.png"), QString("Edit paths"), this);
  ui->mainToolBar->addAction(editpathsaction);
  connect(editpathsaction, SIGNAL(triggered()), this,
          SLOT(showEditPathDialog()));
}
...
void MainWindow::addEditTab(QString path) { tabs->addTab(path, Tab::Editor); }
