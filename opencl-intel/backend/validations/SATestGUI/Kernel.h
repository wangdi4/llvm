 ...
 * @detailed Work with kernels - save/load buffers, return pathes to
 * buffer-files
 * @TODO add KernelInfo member and create getters and setters to its values
 * (WorkOffset, etc)
 */
class Kernel {
public:
...
private:
  ...
  BufferContainerList *inBufferContainerList, *refBufferContainerList,
      *neatBufferContainerList;
  KernelInfo info;
   ...
   */
  void parseFile(QFile *_file, QVector<BufferConverter *> *buffers,
                 QString &qstr, DataFileType &fileType,
                 BufferContainerList **bufferContainerList);
  /**
