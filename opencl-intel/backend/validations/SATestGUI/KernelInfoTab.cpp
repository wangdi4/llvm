...
KernelInfoTab::KernelInfoTab(Kernel *kernel, int kernelId, QWidget *parent)
    : Tab(parent) {
  ...
  for (int i = 0; i < kernel->getInBuffers()->size(); i++) {
    QListWidgetItem *item =
        new QListWidgetItem(QString(QString("Buffer ") + QString::number(i)));
  ...
  connect(inBuffers, SIGNAL(itemDoubleClicked(QListWidgetItem *)), this,
          SLOT(bufferSelected(QListWidgetItem *)));
}
