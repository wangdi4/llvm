...
#define QUALIFIER_INFO_LIST_LENGTH                                             \
  (sizeof(g_qualifierInfoList) / sizeof(QualifierInfo))
...
DemangleParser::DemangleParser(TypeVector &parameters, bool isSpir12Name)
  ...
  m_imageTypeNameTranslate["ocl_image2d_depth_ro"] =
      PRIMITIVE_IMAGE_2D_DEPTH_RO_T;
  m_imageTypeNameTranslate["ocl_image2d_depth_wo"] =
      PRIMITIVE_IMAGE_2D_DEPTH_WO_T;
  m_imageTypeNameTranslate["ocl_image2d_depth_rw"] =
  ...
  m_imageTypeNameTranslate["ocl_image1d_buffer_ro"] =
      PRIMITIVE_IMAGE_1D_BUFFER_RO_T;
  m_imageTypeNameTranslate["ocl_image1d_buffer_wo"] =
      PRIMITIVE_IMAGE_1D_BUFFER_WO_T;
  m_imageTypeNameTranslate["ocl_image1d_buffer_rw"] =
      PRIMITIVE_IMAGE_1D_BUFFER_RW_T;
  m_imageTypeNameTranslate["ocl_image1darray"] = PRIMITIVE_IMAGE_1D_ARRAY_T;
  m_imageTypeNameTranslate["ocl_image1d_array_ro"] =
      PRIMITIVE_IMAGE_1D_ARRAY_RO_T;
  m_imageTypeNameTranslate["ocl_image1d_array_wo"] =
      PRIMITIVE_IMAGE_1D_ARRAY_WO_T;
  m_imageTypeNameTranslate["ocl_image1d_array_rw"] =
      PRIMITIVE_IMAGE_1D_ARRAY_RW_T;
  m_imageTypeNameTranslate["ocl_image2darray"] = PRIMITIVE_IMAGE_2D_ARRAY_T;
  m_imageTypeNameTranslate["ocl_image2d_array_ro"] =
      PRIMITIVE_IMAGE_2D_ARRAY_RO_T;
  m_imageTypeNameTranslate["ocl_image2d_array_wo"] =
      PRIMITIVE_IMAGE_2D_ARRAY_WO_T;
  m_imageTypeNameTranslate["ocl_image2d_array_rw"] =
      PRIMITIVE_IMAGE_2D_ARRAY_RW_T;
  m_imageTypeNameTranslate["ocl_image2darraydepth"] =
      PRIMITIVE_IMAGE_2D_ARRAY_DEPTH_T;
  m_imageTypeNameTranslate["ocl_image2d_array_depth_ro"] =
      PRIMITIVE_IMAGE_2D_ARRAY_DEPTH_RO_T;
  m_imageTypeNameTranslate["ocl_image2d_array_depth_wo"] =
      PRIMITIVE_IMAGE_2D_ARRAY_DEPTH_WO_T;
  m_imageTypeNameTranslate["ocl_image2d_array_depth_rw"] =
...
bool DemangleParser::demangle(const char *mangledString) {
  ...
  for (RefParamType type = getNextType(); !type.isNull();
       type = getNextType()) {
...
bool DemangleParser::match(TypeInfo *pTypeInfo,
                           TypePrimitiveEnum *pPrimitiveType) {
  if (!pTypeInfo)
    return false;
  if ((m_mangledStringLength - m_currentIndex) < pTypeInfo->m_tokenLength)
    return false;
  ...
  for (unsigned int i = 0; i < pTypeInfo->m_tokenLength; ++i) {
    if (m_pMangledString[m_currentIndex + i] != pTypeInfo->m_token[i])
      return false;
  }
  // Found a type, increment current index & set Primitive
  m_currentIndex += pTypeInfo->m_tokenLength;
  if (pPrimitiveType)
    *pPrimitiveType = pTypeInfo->m_primitiveType;
...
DemangleParser::createUserDefinedType(unsigned int userDefinedNameLength) {
  ...
  std::string typeName(&m_pMangledString[m_currentIndex],
                       userDefinedNameLength);
...
bool DemangleParser::getAddressSpace(TypeAttributeEnum &attrAddressSpace) {
  ...
  addressSpaceLength -= g_ADDRESS_SPACE_SUFFIX.m_tokenLength;
  std::string addressSpaceId(&m_pMangledString[m_currentIndex],
                             addressSpaceLength);
  char *endString;
...
std::vector<TypeAttributeEnum> DemangleParser::getAddressQualifiers() {
  ...
  for (unsigned int i = 0; i < QUALIFIER_INFO_LIST_LENGTH; ++i) {
    assert(
        strlen(g_qualifierInfoList[i].m_token) == 1 &&
