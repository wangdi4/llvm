...
#ifndef BACKEND_SUBGROUP_EMULATION_LOOP_CONSTRUCT_H
#define BACKEND_SUBGROUP_EMULATION_LOOP_CONSTRUCT_H
...
class SGLoopConstruct : public ModulePass {

public:
  static char ID;

  SGLoopConstruct() : ModulePass(ID), SizeAnalysis(nullptr) {}

  bool runOnModule(Module &M) override;

  StringRef getPassName() const override { return "SGLoopConstruct Pass"; }
...
private:
  ...
  /// here.
  InstSet findSyncPredecessors(Instruction *I);

  /// Find the sg loop header for every sub_group_barrier.
  void createSGLoop();
  ...
  /// Set vectorized_with as sg_emu_size for emulated kernels.
  void updateMetadata(Module &M);

  /// Map from function to all sub_group_barrier calls inside.
  MapVector<Function *, InstSet> FuncToSGBarriers;
  ...
  /// calls may reach it.
  MapVector<Instruction *, InstSet> BarrierToJumpTargets;
  ...
  FuncSet FunctionsNeedEmulation;
};
} // namespace intel
#endif // BACKEND_SUBGROUP_EMULATION_LOOP_CONSTRUCT_H
