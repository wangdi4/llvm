...
public:
  ...
  static void SetDebugLocBy(llvm::Instruction *I,
                            const llvm::Instruction *setBy);

  /// @brief set debug location of I according to SetBy.
  /// @param I instruction whose debug location will be set.
  /// @param setBy instruction from which debug location is gathered.
  static void SetDebugLocBy(std::vector<llvm::Instruction *> &insts,
                            llvm::Instruction *setBy);
  ...
  static llvm::Value *RootInputArgument(llvm::Value *arg, llvm::Type *rootType,
                                        llvm::CallInst *CI);
  ...
  /// @param CI The CALL instruction
  /// @return The root value if found, or NULL otherwise
  static llvm::Value *RootInputArgumentBySignature(llvm::Value *arg,
                                                   unsigned int paramNum,
                                                   llvm::CallInst *CI);

  /// @brief Follow thru a function return argument, until its type matches the
  /// "expected" type
  /// @param retVal The actual returned value of the CALL inst
  /// @param rootType The desired (or "real") type to find
  /// @param CI The CALL instruction
  /// @return The "proper" retval if found, or NULL otherwise
  static llvm::Value *RootReturnValue(llvm::Value *retVal, llvm::Type *rootType,
                                      llvm::CallInst *CI);
  ...
  /// @return casted value
  static llvm::Value *getCastedArgIfNeeded(llvm::Value *inputVal,
  ...
  /// @return casted value
  static llvm::Instruction *getCastedRetIfNeeded(llvm::Instruction *I,
                                                 llvm::Type *targetType);
  ...
  static llvm::CallInst *createFunctionCall(
      llvm::Module *pModule, const std::string &name, llvm::Type *retType,
      const llvm::SmallVectorImpl<llvm::Value *> &args,
      const llvm::SmallVectorImpl<llvm::Attribute::AttrKind> &attrs,
  ...
  /// @param insertAfter if true, insert after whereTo instruction, otherwise
  /// insert before it.
  /// @return new broadcast vector
  static llvm::Instruction *createBroadcast(llvm::Value *pVal,
                                            unsigned int packetWidth,
                                            llvm::Instruction *whereTo,
                                            bool insertAfter = false);

  /// @brief Calculate BSR - (bit set reverse order) if argument is not zero
  ///  it is equivalent to count-leading-zeroes + 1. Mathematically, it computes
  ///  floor(log(x)).
  ...
  /// @param insertPoint instruction to insert before
  static llvm::Instruction *ExtendValToType(llvm::Value *orig,
                                            llvm::Type *targetType,
                                            llvm::Instruction *insertPoint);
  ...
  /// @param targetType Target type to convert to
  /// @param insertPoint instruction to insert before
  static llvm::Instruction *TruncValToType(llvm::Value *orig,
                                           llvm::Type *targetType,
                                           llvm::Instruction *insertPoint);
...
private:
  ...
  /// @param targetType Target type to convert to
  /// @param insertPoint instruction to insert before
  static llvm::Instruction *BitCastValToType(llvm::Value *orig,
                                             llvm::Type *targetType,
                                             llvm::Instruction *insertPoint);

  /// @brief in case target Type is a pointer for orig allocates memory and
  /// store orig
  /// @param orig Source value
  /// @param targetType Target type to convert to
  /// @param insertPoint instruction to insert before
  /// @return  allcoated pointer
  static llvm::Instruction *convertValToPointer(llvm::Value *orig,
                                                llvm::Type *targetType,
                                                llvm::Instruction *insertPoint);

  /// @brief check if ShuffleVector instruction is used to artificially extend a
  /// vector.
  ...
  static llvm::Value *isExtendedByShuffle(llvm::ShuffleVectorInst *SI,
                                          llvm::Type *realType);

  /// @brief check if ShuffleVector instruction is used to artificially truncate
  /// a vector result
  /// @param SI shuffleVector instruction to check
  ...
  /// @brief check if I is obtained by series of insert element instructions to
  /// it's start.
  /// @param I - instruction to check.
  /// @param realType - target type to check.
  /// @returns rooted smaller vector of Type realType if possible, NULL
  /// otherwise.
  static llvm::Value *isInsertEltExtend(llvm::Instruction *I,
                                        llvm::Type *realType);
  ...
  static llvm::Instruction *convertUsingShuffle(llvm::Value *v,
                                                const llvm::Type *realType,
  ...
  /// @returns converted val if possible, NULL otherwise.
  static llvm::Value *
  canRootInputByShuffle(llvm::SmallVector<llvm::Value *, 4> &valInChain,
