...
static bool compatible(const FunctionDescriptor &l,
                       const FunctionDescriptor &r) {
...
void BuiltinKeeper::initNullStrategyEntries() {
  width::V vwidths[] = {width::SCALAR, width::TWO,   width::THREE,
                        width::FOUR,   width::EIGHT, width::SIXTEEN};
  TypePrimitiveEnum primitivesTy[] = {PRIMITIVE_CHAR,    // 1
                                      PRIMITIVE_SHORT,   // 2
                                      PRIMITIVE_INT,     // 3
                                      PRIMITIVE_LONG,    // 4
                                      PRIMITIVE_UCHAR,   // 5
  ...
  VWidthArray arrNonScalars(vwidths + 1,
                            (sizeof(vwidths) / sizeof(width::V)) - 1);
  VWidthArray arrAllWidth(vwidths);
  //
  // Adding WI and memory functions which accept a single unit.
  //
  {
    llvm::StringRef names[] = {"get_global_id",  "get_global_size",
                               "get_local_size", "get_enqueued_local_size",
  ...
  //
  {
    ...
    Cartesian<llvm::ArrayRef, llvm::StringRef, width::V> pairs(
        pointeredBuiltins, allWidths);
    do {
      auto pair = pairs.get();
  ...
  // this function cluster cannot be versioned due its OpenCL definition.
  {
    llvm::StringRef names[] = {
        "_Z21async_work_group_copy*",
        "_Z29async_work_group_strided_copy*",
        "_Z34__async_work_group_stream_to_image*",
        "_Z36__async_work_group_stream_from_image*",
        "_Z41__async_work_group_stream_to_image_direct*",
        "_Z43__async_work_group_stream_from_image_direct*",
    ...
    VWidthArray allWidths(vwidths);
    Cartesian<llvm::ArrayRef, llvm::StringRef, width::V> pairs(
        uniform_work_group_builtins, allWidths);
    do {
      auto pair = pairs.get();
      PairSW key(std::make_pair(std::string(pair.first), pair.second));
      m_exceptionsMap.insert(std::make_pair(key, &m_nullStrategy));
...
void BuiltinKeeper::addConversionGroup(const StringArray &names,
  ...
  Cartesian<llvm::ArrayRef, std::pair<llvm::StringRef, TypePrimitiveEnum>,
            width::V>
      threeWayProduct(arrTmpResult, arrNonScalars);
  do {
    FunctionDescriptor fd =
        fdFactory(threeWayProduct.get(), // name, primitive type, vector-width
                  p);
...
void BuiltinKeeper::addTransposGroup(const FunctionDescriptor &aosDescriptor) {
  width::V aosWidth[] = {width::SCALAR, width::FOUR, width::EIGHT,
                         width::SIXTEEN};
...
template <>
inline std::pair<FunctionDescriptor, RefParamType>
...
template <int w>
std::pair<FunctionDescriptor, RefParamType> createUniV_S(TypePrimitiveEnum p,
                                                         const std::string &n) {
  FunctionDescriptor fd;
  fd.Name = n;
  RefParamType scalar(new PrimitiveType(p));
...
template <int w>
std::pair<FunctionDescriptor, RefParamType> createUniV_V(TypePrimitiveEnum p,
                                                         const std::string &n) {
  FunctionDescriptor fd;
  fd.Name = n;
  RefParamType scalar(new PrimitiveType(p));
...
struct RangeUtil {
  ...
  void reset(BuiltinMap::MapRange &mr) { m_range = mr; }

  // indicates whether the given range is empty
  bool isEmpty() const { return (m_range.first == m_range.second); }
  ...
  const FunctionDescriptor &getDescriptor() const { return *m_range.first; }
...
PairSW BuiltinKeeper::getVersion(const std::string &name, width::V w) const {
  ...
  do {
    ...
    // we check that the candidate is in the right width, and that its
    // parameters are compatible (i.e., the same scalar type) as the one of the
    // original
