...
void OclBuiltinDB::processAliasMap() {
  ...
  for (const Record *pMap : aliasMaps) {
    ...
    const std::vector<Record *> &aliasTypesVec =
        pMap->getValueAsListOfDefs("Types");
    ...
    for (Init *const item : *aliasList) {
      ...
      if (types.empty()) {
        for (OclBuiltin::const_type_iterator tyIt = pBuiltin->type_begin(),
                                             tyEnd = pBuiltin->type_end();
             tyIt != tyEnd; tyIt++)
          types.insert(*tyIt);
      }

      m_AliasMap.insert(
          std::make_pair(pBuiltin, std::make_pair(aliasNames, types)));
    }
  }
}
...
OclBuiltinDB::OclBuiltinDB(RecordKeeper &R, bool CollectImplDefs)
  ...
  if (!HandleImplTargetInheritance) {
    ...
    // NativeTypes
    {
      ...
      for (std::vector<Record *>::iterator I = Rs.begin(), E = Rs.end(); I != E;
           ++I) {
...
/// OclBuiltinEmitter
OclBuiltinEmitter::OclBuiltinEmitter(RecordKeeper &R) : m_Records(R), m_DB(R) {}
