...
//
static bool get_reg_value_string(HKEY top_hkey, const string &path,
                                 const string &value_name, string &value) {
  ...
  LONG rc = RegOpenKeyExA(top_hkey,     // hkey
                          path.c_str(), // lpSubKey
                          0,            // ulOptions
                          KEY_READ,     // samDesired
                          &hkey         // phkResult
...
//
static string get_my_pid_string() { return stringify(GetCurrentProcessId()); }
...
struct FunctionStackFrame {
  struct VarDeclInfo {
    VarDeclInfo(void *addr_, const MDNode *description_,
                const MDNode *expression_)
        : addr(addr_), description(description_), expression(expression_) {
      if (expression) {
        assert(dyn_cast<DIExpression>(expression) &&
               "DIExpression is expected");
        ...
        for (unsigned i = 0; i < N; ++i) {
          ...
          if (Element == dwarf::DW_OP_plus) {
            ...
            // See http://llvm.org/docs/LangRef.html#diexpression for more
            // details Note that 3.6.0 already has DIExpression but this is not
            // described in the release related pages
...
struct DebugServer::DebugServerImpl {
  DebugServerImpl()
      : m_comm(0), m_initialized(false), m_runningmode(RUNNING_NORMAL),
        m_debugged_global_id(3, 0), m_saved_stack_level(0),
  ...
  //
  void NotifyBreakpointHit(const std::string &file, unsigned lineno,
                           const VarsMapping &vars);
  ...
  // frame.
  //
  VarsMapping CollectVisibleVars(const MDNode *line_metadata,
                                 const FunctionStackFrame &stackframe);
  VarsMapping CollectGlobalVars(const FunctionStackFrame &stackframe);
  VarsMapping CollectVarsInScope(const DIScope &scope,
                                 const FunctionStackFrame &stackframe);
  VarsMapping CollectVarsInLexicalBlock(const DILexicalBlock &block,
                                        const FunctionStackFrame &stackframe);
  VarDescription
  CreateVarDescription(const FunctionStackFrame::VarDeclInfo &var_info);
...
DebugServer::DebugServerImpl::BuildErrorMessage(string err) {
...
void DebugServer::DebugServerImpl::RegisterBreakpoints(
    const ClientToServerMessage &run_msg) {
  ...
  DEBUG_SERVER_LOG("Registered " + stringify(m_breakpoints.size()) +
                   " breakpoints");
}

bool DebugServer::DebugServerImpl::HasBreakpointAt(const std::string &file,
                                                   unsigned lineno) const {
  BreakpointInfo bpinfo = make_pair(file, lineno);
  return m_breakpoints.find(bpinfo) != m_breakpoints.end();
}

void DebugServer::DebugServerImpl::NotifyBreakpointHit(
    const std::string &file, unsigned lineno, const VarsMapping &vars) {
  ...
  for (VarsMapping::const_iterator ivar = vars.begin(); ivar != vars.end();
       ++ivar) {
    ServerToClientMessage::VarInfo *var_info =
        msg_to_client.mutable_bphit_msg()->add_vars();
...
void DebugServer::DebugServerImpl::SendStackTraceInfo() {
  ...
  for (FunctionStack::iterator i = m_stack.begin(); i != m_stack.end(); ++i) {
    assert(dyn_cast<DISubprogram>(i->function_metadata) &&
           "DISubprogram is expected");
    const DISubprogram *subprogram_descriptor =
        cast<DISubprogram>(i->function_metadata);
    ...
    if (i->calling_line_metadata) {
      assert(dyn_cast<DILocation>(i->calling_line_metadata) &&
             "DILocation is expected");
    ...
    if (i != m_stack.begin()) {
      ...
      if (iprev->calling_line_metadata) {
        VarsMapping vars_info =
            CollectVisibleVars(iprev->calling_line_metadata, *i);
...
bool DebuggingIsEnabled() {
  ...
  if (get_reg_value_string(HKEY_CURRENT_USER, pid_path, REG_VAL_NAME, val)) {
    return val == "1";
  } else if (get_reg_value_string(HKEY_CURRENT_USER, global_path, REG_VAL_NAME,
                                  val)) {
    return val == "1";
  } else
    return false;
...
void DebugServer::DebugServerImpl::SendMemoryRangeInfo(
    const ClientToServerMessage &memory_range_msg) {
  ...
  if (start_addr > end_addr || range_len > MAX_MEMORY_RANGE_SIZE) {
    ServerToClientMessage error_msg =
        BuildErrorMessage("Invalid GET_MEMORY_RANGE parameters");
...
void DebugServer::DebugServerImpl::SetDebuggedGlobalId(unsigned x, unsigned y,
                                                       unsigned z) {
...
bool DebugServer::DebugServerImpl::DebuggedGlobalIdMatch(unsigned x, unsigned y,
                                                         unsigned z) {
  return (m_initialized && m_debugged_global_id[0] == x &&
          m_debugged_global_id[1] == y && m_debugged_global_id[2] == z);
}
...
VarsMapping DebugServer::DebugServerImpl::CollectVisibleVars(
  ...
  if (const MDNode *scope =
          dyn_cast<const MDNode>(line_metadata->getOperand(0))) {
...
VarsMapping DebugServer::DebugServerImpl::CollectVarsInLexicalBlock(
  ...
  // cerr << "-- In Lexical block on line " << block.getLineNumber() << " --" <<
  // endl; cerr << "> My vars:\n"; dump_vars_mapping(myvars); cerr << "> Parent
...
VarsMapping DebugServer::DebugServerImpl::CollectVarsInScope(
  ...
  //
  vector<FunctionStackFrame::VarDeclInfo>::const_iterator i =
      stackframe.vars.begin();
...
VarDescription DebugServer::DebugServerImpl::CreateVarDescription(
  ...
  return VarDescription(var_name_str, var_type_str, var_value_str,
...
void DebugServer::WaitForStartCommand() {
  ...
  if (msg.type() == ClientToServerMessage::START_SESSION &&
      msg.has_start_session_msg()) {
    DEBUG_SERVER_LOG("START_SESSION received");
    d->SetDebuggedGlobalId(msg.start_session_msg().global_id_x(),
    ...
    DEBUG_SERVER_LOG(
        "Invalid message received while waiting for START_SESSION");
    ServerToClientMessage error_msg =
        d->BuildErrorMessage("Expected a START_SESSION command");
  ...
  if (msg.type() == ClientToServerMessage::RUN) {
    ...
    ServerToClientMessage error_msg =
        d->BuildErrorMessage("Expected a RUN command");
    d->m_comm->sendMessage(error_msg);
    return;
  }
}
...
void DebugServer::Stoppoint(const MDNode *line_metadata) {
  ...
  if (stopped) {
    ...
    VarsMapping vars_info =
        d->CollectVisibleVars(line_metadata, d->m_stack.front());
