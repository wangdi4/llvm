...
PluginManager::~PluginManager() {
  for (PluginsList::iterator it = m_listPlugins.begin();
       it != m_listPlugins.end(); it++)
    if (*it)
      delete *it;
}
...
void PluginManager::LoadPlugins() {
  ...
  PluginsList plugins;
  for (DllNamesVector::iterator it = namesVector.begin();
       it != namesVector.end(); ++it) {
    try {
 ...
 */
void PluginManager::OnCreateBinary(
    const DeviceBackend::ICLDevBackendKernel_ *pKernel,
    const _cl_work_description_type *pWorkDesc, size_t bufSize,
    void *pArgsBuffer) {
  LoadPlugins();
  for (PluginsList::iterator it = m_listPlugins.begin();
       it != m_listPlugins.end(); ++it)
    getBackendPlugin((*it)->plugin())
        ->OnCreateBinary(pKernel, pWorkDesc, bufSize, pArgsBuffer);
}

/**
 * Called by OCL Backend on CreateProgram call
 */
void PluginManager::OnCreateProgram(
    const void *pBinary, size_t uiBinarySize,
    const DeviceBackend::ICLDevBackendProgram_ *pProgram) {
  LoadPlugins();
  for (PluginsList::iterator it = m_listPlugins.begin();
       it != m_listPlugins.end(); ++it)
    getBackendPlugin((*it)->plugin())
        ->OnCreateProgram(pBinary, uiBinarySize, pProgram);
}

/**
 * Called by OCL Backend on CreateKernels call
 */
void PluginManager::OnCreateKernel(
    const DeviceBackend::ICLDevBackendProgram_ *pProgram,
    const DeviceBackend::ICLDevBackendKernel_ *pKernel, const void *pFunc) {
  LoadPlugins();
  for (PluginsList::iterator it = m_listPlugins.begin();
       it != m_listPlugins.end(); ++it)
 ...
 *
 */
void PluginManager::OnReleaseProgram(
    const DeviceBackend::ICLDevBackendProgram_ *pProgram) {
  LoadPlugins();
  for (PluginsList::iterator it = m_listPlugins.begin();
       it != m_listPlugins.end(); ++it)
...
PluginManagerException::PluginManagerException(std::string message)
...
IPlugin *PluginInfo::plugin() { return m_pPlugin; }
} // namespace OpenCL
} // namespace Intel
