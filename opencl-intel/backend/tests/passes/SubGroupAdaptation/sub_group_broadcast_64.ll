...
; CHECK: entry
; CHECK: %callInst = call spir_func i64 @_Z19get_local_linear_idv()
; CHECK: %cast = trunc i64 %callInst to i32
; CHECK: ret i32 %cast
...
; CHECK: %linid = zext i32 %1 to i64
; CHECK: %lid0 = urem i64 %linid, %lsz0
; CHECK: %lid1.op = udiv i64 %linid, %lsz0
...
; CHECK: declare spir_func i64 @_Z19get_local_linear_idv()
; CHECK: declare spir_func i64 @_Z14get_local_sizej(i32)
; CHECK: declare spir_func i32 @_Z20work_group_broadcastimmm(i32, i64, i64, i64)
