...
; CHECK: call i32 @__read_pipe_2_bl_intel({{.*}} %[[CAST_FAR_PIPE]], {{.*}}, i32 1, i32 1)
;
; CHECK: %[[LOAD_STAR_PIPE:.*]] = load {{.*}} @[[PIPE_STAR]]
; CHECK: %[[CAST_STAR_PIPE:.*]] = bitcast %opencl.pipe_rw_t{{.*}} %[[LOAD_STAR_PIPE]] to %opencl.pipe_ro_t
; CHECK: call i32 @__read_pipe_2_bl_intel({{.*}} %[[CAST_STAR_PIPE]], {{.*}}, i32 16, i32 8)
;
...
; CHECK: call i32 @__read_pipe_2_bl_intel({{.*}} %[[CAST_BAR_PIPE_ARR]], {{.*}}, i32 4, i32 4)
;
...
; CHECK: call i32 @__read_pipe_2_bl_intel({{.*}} %[[CAST_FAR_PIPE_ARR]], {{.*}}, i32 1, i32 1)
;
...
; CHECK: call i32 @__read_pipe_2_bl_intel({{.*}} %[[CAST_STAR_PIPE_ARR]], {{.*}}, i32 16, i32 8)
