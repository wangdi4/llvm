; RUN: %oclopt -mtriple=i686 -add-implicit-args -local-buffers -prepare-kernel-args -S %s -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: %oclopt -mtriple=i686 -add-implicit-args -local-buffers -prepare-kernel-args -S %s -o %t.ll
