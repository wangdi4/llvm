...
; RUN: %oclopt -runtimelib=%t.rtl.bc -cos-sin-pass -S < %s -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: %oclopt -runtimelib=%t.rtl.bc -cos-sin-pass -S < %s | FileCheck %s
; ModuleID = 'main'
