...
; channel int a;
;
; __kernel void writer_blocking() {
;   write_channel_intel(a, 10);
; }
;
; __kernel void reader_non_blocking(__global int *data) {
;   bool valid = false;
;   *data = read_channel_nb_intel(a, &valid);
; }
; ----------------------------------------------------
; Compile options:
...
; CHECK: digraph G {
; CHECK: edge [arrowtail=box,arrowhead=empty,dir=both,label="a"];
; CHECK-NEXT: "writer_blocking" -> "reader_non_blocking"
; CHECK-NEXT: }
...
define spir_kernel void @writer_blocking() #0 !kernel_arg_addr_space !4 !kernel_arg_access_qual !4 !kernel_arg_type !4 !kernel_arg_base_type !4 !kernel_arg_type_qual !4 !kernel_arg_host_accessible !4 !kernel_arg_pipe_depth !4 !kernel_arg_pipe_io !4 !kernel_arg_buffer_location !4 {
entry:
...
; Function Attrs: convergent nounwind
define spir_kernel void @reader_non_blocking(i32 addrspace(1)* %data) #0 !kernel_arg_addr_space !9 !kernel_arg_access_qual !10 !kernel_arg_type !11 !kernel_arg_base_type !11 !kernel_arg_type_qual !12 !kernel_arg_host_accessible !13 !kernel_arg_pipe_depth !14 !kernel_arg_pipe_io !12 !kernel_arg_buffer_location !12 {
entry:
  %read.dst = alloca i32
  ...
  store i8 0, i8* %valid, align 1, !tbaa !17
  %0 = load %opencl.pipe_rw_t addrspace(1)*, %opencl.pipe_rw_t addrspace(1)* addrspace(1)* @a.pipe
  %1 = load %opencl.channel_t addrspace(1)*, %opencl.channel_t addrspace(1)* addrspace(1)* @a, align 4, !tbaa !6
  %2 = bitcast %opencl.pipe_rw_t addrspace(1)* %0 to %opencl.pipe_ro_t addrspace(1)*
  %3 = addrspacecast i32* %read.dst to i8 addrspace(4)*
  %call1 = call i32 @__read_pipe_2_fpga(%opencl.pipe_ro_t addrspace(1)* %2, i8 addrspace(4)* %3, i32 4, i32 4)
  %4 = icmp eq i32 %call1, 0
  %5 = load i32, i32* %read.dst
  %6 = zext i1 %4 to i8
  store i8 %6, i8* %valid
  %7 = load i32 addrspace(1)*, i32 addrspace(1)** %data.addr, align 8, !tbaa !15
  store i32 %5, i32 addrspace(1)* %7, align 4, !tbaa !19
