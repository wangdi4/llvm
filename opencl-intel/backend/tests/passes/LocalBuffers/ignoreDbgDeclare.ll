; RUN: %oclopt -dpcpp-kernel-add-implicit-args -debugify -local-buffers -check-debugify -dpcpp-kernel-prepare-args -S < %s -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: %oclopt -dpcpp-kernel-add-implicit-args -local-buffers -dpcpp-kernel-prepare-args -S < %s | FileCheck %s
