// improve loop
__kernel void programMany_3(write_only image2d_t dest, int flipped, int4 dim, float2 st_origin, float4 st_delta, float4 l0, float4 l1, read_only image2d_t t0, sampler_t t_sampler0)
{
    const int Many = 128;
    float4       o0_x, r0_x, f0_x;
    float4       o0_y, r0_y, f0_y;
    float4       o0_z, r0_z, f0_z;
    float4       o0_w, r0_w, f0_w;
    ...
    r0_x = f0_x * l0.x + f0_y * l0.y + l0.w;
    r0_y = f0_x * l1.x + f0_y * l1.y + l1.w;
...
// stream samp
__kernel void programMany_2(write_only image2d_t dest, int flipped, int4 dim, float2 st_origin, float4 st_delta, float4 l0, float4 l1, read_only image2d_t t0, sampler_t t_sampler0)
{
    const int Many = 128;
    float4       o0_x, r0_x, f0_x;
    float4       o0_y, r0_y, f0_y;
...
// loop
__kernel void programMany(write_only image2d_t dest, int flipped, int4 dim, float2 st_origin, float4 st_delta, float4 l0, float4 l1, read_only image2d_t t0, sampler_t t_sampler0)
{
    const int Many = 128;
    float4       o0_x, r0_x, f0_x;
    float4       o0_y, r0_y, f0_y;
    float4       o0_z, r0_z, f0_z;
    float4       o0_w, r0_w, f0_w;
    int         loc_x_orig = get_global_id(0)*4; 
    int tmp = loc_x_orig;
    int4 loc_x;
//    int4        loc_x = loc_x_orig + (int4)(0, 1, 2, 3);
    int         loc_y = get_global_id(1);
...
__kernel void program4_2(write_only image2d_t dest, int flipped, int4 dim, float2 st_origin, float4 st_delta, float4 l0, float4 l1, read_only image2d_t t0, sampler_t t_sampler0)
{
    ...
    float4 r0Tx = read_imagef(t0, t_sampler0, (float2)(r0_x.x, r0_y.x));
    float4 r0Ty = read_imagef(t0, t_sampler0, (float2)(r0_x.y, r0_y.y));
    float4 r0Tz = read_imagef(t0, t_sampler0, (float2)(r0_x.z, r0_y.z));
    float4 r0Tw = read_imagef(t0, t_sampler0, (float2)(r0_x.w, r0_y.w));
...
// vect4
__kernel void program4(write_only image2d_t dest, int flipped, int4 dim, float2 st_origin, float4 st_delta, float4 l0, float4 l1, read_only image2d_t t0, sampler_t t_sampler0)
{
    float4       o0_x, r0_x, f0_x;
    float4       o0_y, r0_y, f0_y;
    float4       o0_z, r0_z, f0_z;
    ...
    r0_x.x = dot((float2)(f0_x.x, f0_y.x),l0.xy) + l0.w;
    r0_x.y = dot((float2)(f0_x.y, f0_y.y),l0.xy) + l0.w;
    r0_x.z = dot((float2)(f0_x.z, f0_y.z),l0.xy) + l0.w;
    r0_x.w = dot((float2)(f0_x.w, f0_y.w),l0.xy) + l0.w;
    r0_y.x = dot((float2)(f0_x.x, f0_y.x),l1.xy) + l1.w;
    r0_y.y = dot((float2)(f0_x.y, f0_y.y),l1.xy) + l1.w;
    r0_y.z = dot((float2)(f0_x.z, f0_y.z),l1.xy) + l1.w;
    r0_y.w = dot((float2)(f0_x.w, f0_y.w),l1.xy) + l1.w;
...
__kernel void programScalar(write_only image2d_t dest, int flipped, int4 dim, float2 st_origin, float4 st_delta, float4 l0, float4 l1, read_only image2d_t t0, sampler_t t_sampler0)
{
    float       o0_x, r0_x, f0_x;
    float       o0_y, r0_y, f0_y;
    float       o0_z, r0_z, f0_z;
    float       o0_w, r0_w, f0_w;
    ...
    r0_x = dot((float2)(f0_x, f0_y),l0.xy) + l0.w;
    r0_y = dot((float2)(f0_x, f0_y),l1.xy) + l1.w;
...
__kernel void program_trans(write_only image2d_t dest, int flipped, int4 dim, float2 st_origin, float4 st_delta, float4 l0, float4 l1, read_only image2d_t t0, sampler_t t_sampler0)
{
    ...
    // vertex start
    f0_start = loc_start;
    r0.x = dot(loc_start.xy,l0.xy) + l0.w;
    r0.y = dot(loc_start.xy,l1.xy) + l1.w;
    f1_start = r0;

    // vertex end
    f0_end = loc_end;
    r0.x = dot(loc_end.xy,l0.xy) + l0.w;
    r0.y = dot(loc_end.xy,l1.xy) + l1.w;
    f1_end = r0;
