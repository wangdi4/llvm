; RUN: llc < %s -march=x86-64 -mattr=+sse -mtriple=x86_64-linux-gnu | grep {movq	(%rdi), %rax}
; RUN: llc < %s -march=x86-64 -mattr=+sse -mtriple=x86_64-pc-win32 | grep {movq	8(%rcx), %rax}
; RUN: llc < %s -march=x86-64 -mattr=+sse -mtriple=x86_64-linux-gnu | grep {movq	(%rdi), %rax}
; RUN: llc < %s -march=x86-64 -mattr=+sse -mtriple=x86_64-pc-win32 | grep {movq	8(%rcx), %rax}
