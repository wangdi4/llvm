...
__kernel void reduce_half_type( __global half* data1,
                                __global half* data2,
                                __global half* data3,
                                __global half* data4,
                                __global half* ex_data1,
                                __global half* ex_data2,
                                __global half* in_data1,
                                __global half* in_data2) {
  int sglid = get_sub_group_local_id();
  half temp = sglid - 0.5;

  data1[sglid] = sub_group_reduce_add(temp);
  data2[sglid] = sub_group_reduce_min(temp);
  data3[sglid] = sub_group_reduce_max(temp);
  data4[sglid] = sub_group_reduce_mul(temp);
  
  ex_data1[sglid] = sub_group_scan_exclusive_add(temp);
