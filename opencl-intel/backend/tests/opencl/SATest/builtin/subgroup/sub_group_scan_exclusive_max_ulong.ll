; ModuleID = 'sub_group_scan_exclusive_max_ulong.ll'
source_filename = "sub_group_scan_exclusive_max_ulong.ll"
...
entry:
  ...
  %call2 = call spir_func i64 @_Z28sub_group_scan_exclusive_maxm(i64 noundef %0) #5
...
for.cond:                                         ; preds = %for.inc, %if.then5
  %max_val.0 = phi i64 [ %1, %if.then5 ], [ %max_val.1, %for.inc ]
...
for.body:                                         ; preds = %for.cond
  ...
  %2 = load i64, ptr addrspace(1) %arrayidx8, align 8, !tbaa !10
  %cmp9 = icmp ult i64 %max_val.0, %2
  br i1 %cmp9, label %if.then10, label %if.end

if.then10:                                        ; preds = %for.body
  %idxprom11 = zext i32 %i.0 to i64
  %arrayidx12 = getelementptr inbounds i64, ptr addrspace(1) %input, i64 %idxprom11
  %3 = load i64, ptr addrspace(1) %arrayidx12, align 8, !tbaa !10
  br label %if.end

if.end:                                           ; preds = %if.then10, %for.body
  %max_val.1 = phi i64 [ %3, %if.then10 ], [ %max_val.0, %for.body ]
  br label %for.inc
...
if.end13:                                         ; preds = %for.end, %if.else
  %max_val.2 = phi i64 [ %max_val.0, %for.end ], [ %1, %if.else ]
  br label %if.end14

if.end14:                                         ; preds = %if.end13, %if.then
  %max_val.3 = phi i64 [ 0, %if.then ], [ %max_val.2, %if.end13 ]
  %cmp15 = icmp eq i64 %max_val.3, %call2
  %cond = zext i1 %cmp15 to i32
  %arrayidx16 = getelementptr inbounds i32, ptr addrspace(1) %results, i64 %call
  store i32 %cond, ptr addrspace(1) %arrayidx16, align 4, !tbaa !14
  ret void
}
...
; Function Attrs: convergent
declare spir_func i64 @_Z28sub_group_scan_exclusive_maxm(i64 noundef) #3
