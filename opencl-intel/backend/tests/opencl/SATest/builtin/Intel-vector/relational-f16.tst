...
; RUN: SATest -BUILD --config=%s.cfg -tsize=32 -cpuarch="sapphirerapids" -llvm-option=-print-after=vplan-vec 2>&1 | FileCheck %s -check-prefix=CHECK32
; RUN: SATest -BUILD --config=%s.cfg -tsize=64 -cpuarch="sapphirerapids" -llvm-option=-print-after=vplan-vec 2>&1 | FileCheck %s -check-prefix=CHECK64

; CHECK32: call <32 x i32> @_Z16isfinite_v1widenDv32_Dh(<32 x half> {{.*}})
; CHECK32: call <32 x i32> @_Z13isinf_v1widenDv32_Dh(<32 x half> {{.*}})
; CHECK32: [[ISNAN:%.*]] = fcmp uno <32 x half> {{.*}}, zeroinitializer
; CHECK32-NEXT: zext <32 x i1> [[ISNAN]] to <32 x i32>
; CHECK32: call <32 x i32> @_Z16isnormal_v1widenDv32_Dh(<32 x half> {{.*}})
...
; CHECK64: call <64 x i32> @_Z16isfinite_v1widenDv64_Dh(<64 x half> {{.*}})
; CHECK64: call <64 x i32> @_Z13isinf_v1widenDv64_Dh(<64 x half> {{.*}})
; CHECK64: [[ISNAN:%.*]] = fcmp uno <64 x half> {{.*}}, zeroinitializer
; CHECK64-NEXT: zext <64 x i1> [[ISNAN]] to <64 x i32>
; CHECK64: call <64 x i32> @_Z16isnormal_v1widenDv64_Dh(<64 x half> {{.*}})
