 ...
 *				 assuming that the current coordinate is located in the center of the matrix:
 *				  1  4  7
 *			      2  5  8
 *			      3  6  9
 *				  If all 9 neighbors are not there for ex. pixels on the corners or edges,
 *                access only the ones that are there.
...
__kernel
void 
wlSimpleBoxBlur_GPU(__global float4 * input,
					...
					const    uint  buffer_size)
{
	...
	//printf("\n Num Dims = %d, Num groups_x = %d, Num groups_y = %d, groupIdx %d, groupIdy %d\n", 
	//		dims, get_num_groups(0), get_num_groups(1), groupIdx, groupIdy);
	//printf(" local(%d, %d), global(%d, %d)\n", 
	//		localIdx, localIdy, globalIdx, globalIdy);
	//printf(" global_szx %d, global_szy %d, local_szx %d, local_szx %d\n", 
...
__kernel
void 
wlSimpleBoxBlur_CPU(__global float4 * input,
					__global float4 * output,
					const    uint    width,
					const    uint    height,
					const    uint  buffer_size)
...
//	printf("groupIdx %d, groupIdy %d, count_x %d, count_y %d, index_x %d, index_y %d\n", 
//			groupIdx, groupIdy, count_x, count_y, index_x, index_y);
...
__kernel 
void
wlSimpleBoxBlur_image2d(
    __read_only image2d_t inputImage, // nVidia requires __read_only on image2d_t
    __global float4 *output,
    const uint rowCountPerGlobalID
    )
{
//	printf("-----------------In wlSimpleBoxBlur_image2d KERNEL code------------\n");
	...
	float denominator = 9.0;
	float4 colorAccumulator = 0.0f;//make_float4(0.0, 0.0, 0.0, 0.0);
	int2 curCrd, curLeftCrd, curRightCrd, upCrd, upLeftCrd, upRightCrd, lowCrd, lowLeftCrd, lowRightCrd;
	for (; row < lastRow; ++row)	
	{
...
__kernel
void 
wlSimpleBoxBlur_Optimized_CPU(__global float4 * input,
								__global float4 * output,
								const    uint    width,
								const    uint    height,
								const    uint  buffer_size)
...
//	printf("-----------------In wlSimpleBoxBlur KERNEL code------------\n");
	...
	//printf("ThreadId %d, global_szx %d, global_szy %d, globalIdx %d, globalIdy %d\n", 
	//		iThreadId, global_szx, global_szy, globalIdx, globalIdy);

	// Ignore edges for now ...
	if ((index_y + count_y + 1) >= height) { bottomEdge = true; count_y = height - index_y - 1; }
	if ((index_x + count_x + 1) >= width) { rightEdge = true; count_x = width - index_x - 1; }
	if (index_y < 1) { topEdge = true; index_y = 1; count_y -= 1; }
	if (index_x < 1) { leftEdge = true; index_x = 1; count_x -= 1; }
	//	printf ("index_x %d, index_y %d, count_x %d, count_y %d \n", index_x, index_y, count_x, count_y); 
	...
	// We can then just add / subtract from this value for each subsequent result in either x or y direction.
	...
	for (unsigned int row = 0; row < count_y - 1; )
	{
		...
		{	
			...
			colorAccumulator -=  input[tmpIndex1]; tmpIndex1 += width;
			colorAccumulator +=  input[tmpIndex2]; tmpIndex2 += width;
			colorAccumulator -=  input[tmpIndex1]; tmpIndex1 += width;
			colorAccumulator +=  input[tmpIndex2]; tmpIndex2 += width;
			colorAccumulator -=  input[tmpIndex1]; 
			colorAccumulator +=  input[tmpIndex2];
		...
		firstBlockAccumulator -=  input[tmpIndex1]; tmpIndex1++;
		firstBlockAccumulator +=  input[tmpIndex2]; tmpIndex2++;
		firstBlockAccumulator -=  input[tmpIndex1]; tmpIndex1++;
		firstBlockAccumulator +=  input[tmpIndex2]; tmpIndex2++;
