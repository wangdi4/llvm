...
//CHECK: Test Passed.
__kernel void test2i64(long2 p, __global long2* dst){
  *dst = popcount(p);
}
__kernel void test4i32(int4 p, __global int4* dst){
  *dst = popcount(p);
}
__kernel void test8i16(short8 p, __global short8* dst){
  *dst = popcount(p);
}
__kernel void test16i8(char16 p, __global char16* dst){
  *dst = popcount(p);
}
