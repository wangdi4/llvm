...
entry:
  ...
  %pf1 = alloca ptr, align 4                      ; <float**> [#uses=1]
  %a2 = alloca <4 x i32>, align 16                ; <<4 x i32>*> [#uses=1]
  %pf2 = alloca ptr, align 4                      ; <i32**> [#uses=1]
  %pf3 = alloca ptr, align 4                      ; <i8**> [#uses=0]
  %pf4 = alloca ptr, align 4                      ; <i16**> [#uses=0]
  store ptr addrspace(1) %input, ptr addrspace(1) %1
  store ptr addrspace(1) %output, ptr addrspace(1) %2
  store i32 %buffer_size, ptr %3
  %4 = call i32 @_Z13get_global_idj(i32 0)        ; <i32> [#uses=1]
  store i32 %4, ptr %tid
  %5 = load ptr %tid                              ; <i32> [#uses=1]
  %6 = load ptr addrspace(1) %1                   ; <ptr addrspace(1)> [#uses=1]
  %7 = getelementptr inbounds <4 x i32>, ptr addrspace(1) %6, i32 %5 ; <ptr addrspace(1)> [#uses=1]
  %8 = load ptr addrspace(1) %7                   ; <<4 x i32>> [#uses=1]
  %9 = bitcast <4 x i32> %8 to <4 x float>        ; <<4 x float>> [#uses=1]
  store <4 x float> %9, ptr %tmp
  %10 = load ptr %tmp                             ; <<4 x float>> [#uses=1]
  %11 = load ptr %tid                             ; <i32> [#uses=1]
  %12 = load ptr addrspace(1) %2                  ; <ptr addrspace(1)> [#uses=1]
  ...
  %18 = bitcast ptr %a1 to ptr                        ; <float*> [#uses=1]
  store ptr %18, ptr %pf1
  %19 = bitcast ptr %a2 to ptr                        ; <i32*> [#uses=1]
