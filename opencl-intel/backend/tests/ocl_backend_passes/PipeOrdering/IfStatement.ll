...
if.then:                                          ; preds = %for.body
  %7 = load %opencl.channel_t addrspace(1)*, %opencl.channel_t addrspace(1)* addrspace(1)* @ch, align 4, !tbaa !18
  store i32 42, i32* %write.src
  %8 = load %opencl.pipe_t addrspace(1)*, %opencl.pipe_t addrspace(1)* addrspace(1)* @pipe.ch
  %9 = bitcast %opencl.pipe_t addrspace(1)* %8 to %struct.__pipe_t addrspace(1)*
  %10 = addrspacecast i32* %write.src to i8 addrspace(4)*
  %11 = call i32 @__write_pipe_2_bl_intel(%struct.__pipe_t addrspace(1)* %9, i8 addrspace(4)* %10)
  br label %if.end
...
for.inc:                                          ; preds = %if.end
  ...
  store i32 %inc2, i32* %i, align 4, !tbaa !16
  br label %for.cond
