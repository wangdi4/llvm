...
;     write_channel_altera(ch1, 42);
;     // implicit work-group barrier here
;   }
;   write_channel_altera(ch2, 42);
; }
;
...
for.end:                                          ; preds = %for.cond.cleanup
  %10 = load %opencl.channel_t addrspace(1)*, %opencl.channel_t addrspace(1)* addrspace(1)* @ch2, align 4, !tbaa !13
  store i32 42, i32* %write.src
  %11 = load %opencl.pipe_t addrspace(1)*, %opencl.pipe_t addrspace(1)* addrspace(1)* @pipe.ch2
...
; Function Attrs: nounwind
define void @boo(i32 addrspace(1)* %iters) #0 !kernel_arg_addr_space !14 !kernel_arg_access_qual !15 !kernel_arg_type !16 !kernel_arg_base_type !16 !kernel_arg_type_qual !17 {
entry:
