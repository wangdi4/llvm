...
for.body:                                         ; preds = %for.cond
  %5 = load i32 addrspace(1)*, i32 addrspace(1)** %iters.addr, align 8, !tbaa !13
  %6 = load i32, i32 addrspace(1)* %5, align 4, !tbaa !17
  %cmp1 = icmp sgt i32 %6, 10
  br i1 %cmp1, label %if.then, label %if.else

if.then:                                          ; preds = %for.body
  %7 = load %opencl.channel_t addrspace(1)*, %opencl.channel_t addrspace(1)* addrspace(1)* @ch1, align 4, !tbaa !19
  store i32 42, i32* %write.src
  %8 = load %opencl.pipe_t addrspace(1)*, %opencl.pipe_t addrspace(1)* addrspace(1)* @pipe.ch1
...
if.else:                                          ; preds = %for.body
  %12 = load %opencl.channel_t addrspace(1)*, %opencl.channel_t addrspace(1)* addrspace(1)* @ch2, align 4, !tbaa !19
  store i32 42, i32* %write.src
  %13 = load %opencl.pipe_t addrspace(1)*, %opencl.pipe_t addrspace(1)* addrspace(1)* @pipe.ch2
  %14 = bitcast %opencl.pipe_t addrspace(1)* %13 to %struct.__pipe_t addrspace(1)*
  %15 = addrspacecast i32* %write.src to i8 addrspace(4)*
  %16 = call i32 @__write_pipe_2_bl_intel(%struct.__pipe_t addrspace(1)* %14, i8 addrspace(4)* %15)
  br label %if.end
