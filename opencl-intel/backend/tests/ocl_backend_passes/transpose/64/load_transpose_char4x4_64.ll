; XFAIL: i686-pc-win32
; RUN: oclopt -runtimelib=clbltfne9.rtl,clbltfnshared.rtl  -builtin-import -builtin-call-to-inst  -instcombine -inline -scalarrepl -S %s -o %t1.ll
; RUN: llc < %t1.ll -mattr=+avx -mtriple=x86_64-pc-Win64 | FileCheck %s -check-prefix=CHECK
; RUN: oclopt -runtimelib=clbltfnl9.rtl,clbltfnshared.rtl  -builtin-import -builtin-call-to-inst  -instcombine -inline -scalarrepl -S %s -o %t2.ll
...
;CHECK:    vpsrlw	[[REG0:[$_0-9]+]], [[XMM0]], [[XMM1:%xmm[0-9]+]]
;CHECK:    vpsrld	[[REG1:[$_0-9]+]], [[XMM0]], [[XMM2:%xmm[0-9]+]]
;CHECK:    vpsrld	[[REG2:[$_0-9]+]], [[XMM0]], [[XMM3:%xmm[0-9]+]]
