...
;
;   i = read_channel_altera(bar_arr[3]);
;   f = read_channel_altera(far_arr[3][2]);
;   st = read_channel_altera(star_arr[3][2][1]);
; }
;
; __kernel void nbfoo() {
;   bool valid;
;   int i = read_channel_nb_altera(bar, &valid);
;   float f = read_channel_nb_altera(far, &valid);
;   struct Foo st = read_channel_nb_altera(star, &valid);
;
;   i = read_channel_nb_altera(bar_arr[3], &valid);
;   f = read_channel_nb_altera(far_arr[3][2], &valid);
;   st = read_channel_nb_altera(star_arr[3][2][1], &valid);
; }
; ----------------------------------------------------
;
...
; CHECK: call void @__flush_read_pipe{{.*}} %[[FLUSH_CAST_BAR_PIPE]]

; CHECK: %[[FLUSH_LOAD_FAR_PIPE:.*]] = load {{.*}} @[[PIPE_FAR]]
; CHECK: %[[FLUSH_CAST_FAR_PIPE:.*]] = bitcast %opencl.pipe_t{{.*}} %[[FLUSH_LOAD_FAR_PIPE]]
; CHECK: call void @__flush_read_pipe{{.*}} %[[FLUSH_CAST_FAR_PIPE]]

; CHECK: %[[FLUSH_LOAD_STAR_PIPE:.*]] = load {{.*}} @[[PIPE_STAR]]
; CHECK: %[[FLUSH_CAST_STAR_PIPE:.*]] = bitcast %opencl.pipe_t{{.*}} %[[FLUSH_LOAD_STAR_PIPE]]
; CHECK: call void @__flush_read_pipe{{.*}} %[[FLUSH_CAST_STAR_PIPE]]

; CHECK: call void @__flush_read_pipe_array{{.*}} bitcast{{.*}} @[[PIPE_BAR_ARR]] to %struct.__pipe_t{{.*}}, i32 5

; CHECK: call void @__flush_read_pipe_array{{.*}} bitcast{{.*}} @[[PIPE_FAR_ARR]] to %struct.__pipe_t{{.*}}, i32 20

; CHECK: call void @__flush_read_pipe_array{{.*}} bitcast{{.*}} @[[PIPE_STAR_ARR]] to %struct.__pipe_t{{.*}}, i32 60
...
; CHECK: %[[LOAD_BAR_PIPE_ARR:.*]] = load {{.*}} @[[PIPE_BAR_ARR]]
; CHECK: %[[CAST_BAR_PIPE_ARR:.*]] = bitcast %opencl.pipe_t{{.*}} %[[LOAD_BAR_PIPE_ARR]]
; CHECK: call i32 @__read_pipe_2{{.*}} %[[CAST_BAR_PIPE_ARR]]
;
; CHECK: %[[LOAD_FAR_PIPE_ARR:.*]] = load {{.*}} @[[PIPE_FAR_ARR]]
; CHECK: %[[CAST_FAR_PIPE_ARR:.*]] = bitcast %opencl.pipe_t{{.*}} %[[LOAD_FAR_PIPE_ARR]]
; CHECK: call i32 @__read_pipe_2{{.*}} %[[CAST_FAR_PIPE_ARR]]
;
; CHECK: %[[LOAD_STAR_PIPE_ARR:.*]] = load {{.*}} @[[PIPE_STAR_ARR]]
; CHECK: %[[CAST_STAR_PIPE_ARR:.*]] = bitcast %opencl.pipe_t{{.*}} %[[LOAD_STAR_PIPE_ARR]]
; CHECK: call i32 @__read_pipe_2{{.*}} %[[CAST_STAR_PIPE_ARR]]
...
entry:
  ...
  %0 = load %opencl.channel_t addrspace(1)*, %opencl.channel_t addrspace(1)* addrspace(1)* @bar, align 4, !tbaa !20
  %call = tail call i32 @_Z19read_channel_altera11ocl_channeli(%opencl.channel_t addrspace(1)* %0) #3
  %1 = load %opencl.channel_t addrspace(1)*, %opencl.channel_t addrspace(1)* addrspace(1)* @far, align 4, !tbaa !20
  ...
  %3 = load %opencl.channel_t addrspace(1)*, %opencl.channel_t addrspace(1)* addrspace(1)* @star, align 4, !tbaa !20
  call void @_Z19read_channel_altera11ocl_channel3Foo(%struct.Foo* nonnull sret %st, %opencl.channel_t addrspace(1)* %3) #3
  %4 = load %opencl.channel_t addrspace(1)*, %opencl.channel_t addrspace(1)* addrspace(1)* getelementptr inbounds ([5 x %opencl.channel_t addrspace(1)*], [5 x %opencl.channel_t addrspace(1)*] addrspace(1)* @bar_arr, i32 0, i32 3), align 4, !tbaa !20
  %call2 = call i32 @_Z19read_channel_altera11ocl_channeli(%opencl.channel_t addrspace(1)* %4) #3
  %5 = load %opencl.channel_t addrspace(1)*, %opencl.channel_t addrspace(1)* addrspace(1)* getelementptr inbounds ([5 x [4 x %opencl.channel_t addrspace(1)*]], [5 x [4 x %opencl.channel_t addrspace(1)*]] addrspace(1)* @far_arr, i32 0, i32 3, i32 2), align 4, !tbaa !20
  %call3 = call float @_Z19read_channel_altera11ocl_channelf(%opencl.channel_t addrspace(1)* %5) #3
  %6 = load %opencl.channel_t addrspace(1)*, %opencl.channel_t addrspace(1)* addrspace(1)* getelementptr inbounds ([5 x [4 x [3 x %opencl.channel_t addrspace(1)*]]], [5 x [4 x [3 x %opencl.channel_t addrspace(1)*]]] addrspace(1)* @star_arr, i32 0, i32 3, i32 2, i32 1), align 4, !tbaa !20
  call void @_Z19read_channel_altera11ocl_channel3Foo(%struct.Foo* nonnull sret %tmp, %opencl.channel_t addrspace(1)* %6) #3
...
entry:
  ...
  %0 = load %opencl.channel_t addrspace(1)*, %opencl.channel_t addrspace(1)* addrspace(1)* @bar, align 4, !tbaa !20
  %1 = addrspacecast i8* %valid to i8 addrspace(4)*
  %call = call i32 @_Z22read_channel_nb_altera11ocl_channeliPU3AS4b(%opencl.channel_t addrspace(1)* %0, i8 addrspace(4)* %1) #3
  %2 = load %opencl.channel_t addrspace(1)*, %opencl.channel_t addrspace(1)* addrspace(1)* @far, align 4, !tbaa !20
  %call1 = call float @_Z22read_channel_nb_altera11ocl_channelfPU3AS4b(%opencl.channel_t addrspace(1)* %2, i8 addrspace(4)* %1) #3
  ...
  call void @_Z22read_channel_nb_altera11ocl_channel3FooPU3AS4b(%struct.Foo* nonnull sret %st, %opencl.channel_t addrspace(1)* %4, i8 addrspace(4)* %1) #3
  ...
  %call2 = call i32 @_Z22read_channel_nb_altera11ocl_channeliPU3AS4b(%opencl.channel_t addrspace(1)* %5, i8 addrspace(4)* %1) #3
  ...
  %call3 = call float @_Z22read_channel_nb_altera11ocl_channelfPU3AS4b(%opencl.channel_t addrspace(1)* %6, i8 addrspace(4)* %1) #3
  ...
  call void @_Z22read_channel_nb_altera11ocl_channel3FooPU3AS4b(%struct.Foo* nonnull sret %tmp, %opencl.channel_t addrspace(1)* %7, i8 addrspace(4)* %1) #3
