...
entry:
  br label %BB0
BB0: 
  %dummyBool = load i1 addrspace(3)* @foo.localBool, align 1
  store i1 %dummyBool, i1 addrspace(1)* %pBool
  br label %BB1
BB1: 
  %dummyBool2 = load <2 x i1> addrspace(3)* @foo.localBool2, align 1
  store <2 x i1> %dummyBool2, <2 x i1> addrspace(1)* %pVBool
  br label %BB2
BB2: 
  %dummyFloat3 = load <3 x float> addrspace(3)* @foo.localFloat3, align 16
...
; CHECK-NEXT:   [[VAR6:%[a-zA-Z0-9]+]] = getelementptr i8 addrspace(3)* %pLocalMemBase, i32 384
; CHECK-NEXT:   [[VAR7:%[a-zA-Z0-9]+]] = bitcast i8 addrspace(3)* [[VAR6]] to i32 addrspace(3)*
; CHECK-NEXT:   br label %BB0
; CHECK:      BB0:                                              ; preds = %entry
; CHECK-NEXT:   %dummyBool = load i1 addrspace(3)* [[VAR1]], align 1
; CHECK-NEXT:   store i1 %dummyBool, i1 addrspace(1)* %pBool
; CHECK-NEXT:   br label %BB1
; CHECK:      BB1:                                              ; preds = %BB0
; CHECK-NEXT:   %dummyBool2 = load <2 x i1> addrspace(3)* [[VAR3]], align 1
; CHECK-NEXT:   store <2 x i1> %dummyBool2, <2 x i1> addrspace(1)* %pVBool
; CHECK-NEXT:   br label %BB2
; CHECK:      BB2:                                              ; preds = %BB1
; CHECK-NEXT:   %dummyFloat3 = load <3 x float> addrspace(3)* [[VAR5]], align 16
; CHECK-NEXT:   store <3 x float> %dummyFloat3, <3 x float> addrspace(1)* %pVFloat
; CHECK-NEXT:   br label %BB3
; CHECK:      BB3:                                              ; preds = %BB2
; CHECK-NEXT:   %dummyInt = load i32 addrspace(3)* [[VAR7]], align 4
