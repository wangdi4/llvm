; RUN: %oclopt -dpcpp-kernel-add-function-attrs -jump-threading -S < %s -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: %oclopt -dpcpp-kernel-add-function-attrs -jump-threading -verify -S < %s | FileCheck %s
