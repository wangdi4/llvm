//
// Slighly modified god-rays sample from WOLF
// only evaluateRay remains (evaluateRayScalar removed)
// blend parameter removed from kernels, replaced with constant value
//

#define GOD_RAYS_BUNCH_SIZE 15 // 16//15
#define DECAY -0.01f
#define WEIGHT 3.0f
#define INVEXPOSURE 1.f / .9f
#define FTHRESHOLD 1.8f

void evaluateRay(__global float4 *inputImage, __global float4 *output,
                 int in_RayNum, int2 imgSize, uint blend) {
  ...
  for (int i = in_RayNum; i < in_RayNum + god_rays_bunch_size; i++) {
    ...
    // Use the ray number as a destination pixel of the bottom border and check
    // the range.
    int Ray = i;
    if (Ray < x_last) {
      // In this case, check to see if the ray is located outside of the image
      // or the ray from the left bottom corner pixel (also directed from the
      // left).
      if (y < 0 && (x < 0 || Ray != 0)) {
      ...
      // If you still have work to do, set up the destination points for the
      // original and shadow rays as a ray number.
      x_dst = Ray;
      y_dst = 0;
      x_dst_s = Ray - 1;
      y_dst_s = 0;
    ...
    // If the ray number is greater than the bottom border range, check other
    // borders.
    else {
      ...
      // Set the ray number as a destination pixel of the right border and check
      // the range.
      if (Ray < y_last) {
        ...
        // Fix the shadow ray destination point, in case the original ray is a
        // corner ray.
        if (Ray == 0) {
          x_dst_s = x_last - 1;
          y_dst_s = 0;
      ...
      // correspond to the right border.
        ...
        // Check left borders if the ray number doesn't correspond to the top
        // one.
        else {
          Ray -= x_last;
          if (Ray < y_last) {
    ...
    if (dx >= dy) {
      // Fix Decay by the step length computed as a ray length divided by steps
      // count.
      FixedDecay = exp(_decay *
                       sqrt(convert_float(dx) * convert_float(dx) +
                            convert_float(dy) * convert_float(dy)) /
                       convert_float(dx));
      ...
      // border, you should crop it by the y coordinate. You know the y
      // coordinate and need to find the first corresponding step in the image
      // borders and the delta error value. */
      if (y < 0 || y > y_last) {
      ...
      // At the end of cropping the horizontal ray, calculate the correct steps
      // count.
      steps = abs(x_dst - x);
    }
    ...
    // exactly the same steps but with different coordinates.
      ...
      // Fix Decay by the step length, which is computed as the ray length
      // divided by the steps count.
      FixedDecay = exp(_decay *
                       sqrt(convert_float(dx) * convert_float(dx) +
                            convert_float(dy) * convert_float(dy)) /
                       convert_float(dy));
      ...
      if (y < 0 || y > y_last) {
        ...
        // Use the omitted steps to fix the delta error value on the y
        // dimension.
        int new_di = di + (dy_crop - 1) * 2 * dx;
    ...
    // Check to see if the result can be written to the output pixel (the pixel
    // is not shaded by the shadow ray and it is not the first ray that should
    // paint the sun position).
    if (x != x_s || y != y_s ||
        (x_dst == 0 && y_dst == 0 && x == LightScreenPosX &&
         y == LightScreenPosY)) {
    ...
    for (int i = 0; i < steps; i++) {
      ...
      /// if(sample_128.x>One_128.x || sample_128.y>One_128.y ||
      /// sample_128.z>One_128.z)
      {
...
__kernel void main_kernel(__global float4 *inputImage, __global float4 *output,
  ...
  evaluateRay(inputImage, output, in_RayNum, imgSize, 1);
}
