...
extern std::string getWorkitemFocus();
...
//
static void host_multi_ndrange_internal(cl::Context context, cl::Device device,
                                        cl::Program program,
  ...
  if (extra_args.size() == 1) {
    ...
    if (cl_code.size() == 0)
      throw runtime_error("host_multi_ndrange can't read kernel from file: " +
                          extra_args[0]);
  ...
  cl::Buffer buf_mid(context, CL_MEM_READ_WRITE, sizeof(cl_uchar) * data_size,
                     0);
  ...
  queue.enqueueNDRangeKernel(kernel2, cl::NullRange,
                             cl::NDRange(ndrange_global_size),
                             cl::NDRange(ndrange_local_size));
