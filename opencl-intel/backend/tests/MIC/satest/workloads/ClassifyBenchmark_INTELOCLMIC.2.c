__kernel void /* __attribute__((vec_type_hint(float16)))*/ multTransModSPOCL (__global float* ptrSource,
                                         ...
                                         int sourceSize,
                                         int offset,
                                         __global float* ptrAlpha1,
                                         __global float* ptrBeta,
                                         __global unsigned int* ptrGamma,
                                         __global float* ptrDelta, int isFirstTimeFlag)
{
 ...
 for(int i = 0; i < sourceSize; i++)
 {
                         ...
                         f_0 = max(as_float(as_uint(Alpha[0] * x_0 + Beta[0]) | Gamma[0] ) + Delta[0], 0.0f);
                         float x_1 = ptrData[(i*5)+1];
                         f_1 = max(as_float(as_uint(Alpha[1] * x_1 + Beta[1]) | Gamma[1] ) + Delta[1], 0.0f);
                         float x_2 = ptrData[(i*5)+2];
                         f_2 = max(as_float(as_uint(Alpha[2] * x_2 + Beta[2]) | Gamma[2] ) + Delta[2], 0.0f);
                         float x_3 = ptrData[(i*5)+3];
                         f_3 = max(as_float(as_uint(Alpha[3] * x_3 + Beta[3]) | Gamma[3] ) + Delta[3], 0.0f);
                         float x_4 = ptrData[(i*5)+4];
                         f_4 = max(as_float(as_uint(Alpha[4] * x_4 + Beta[4]) | Gamma[4] ) + Delta[4], 0.0f);
