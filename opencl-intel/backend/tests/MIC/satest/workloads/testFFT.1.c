 ...
 * Matrix transpose is taking almost 60% of the total time
 * taken by 2D FFT.
 *
 * Blocks of 16X16 is taken and matrix transpose is done.
 * Matrix transose of blocks is done then.
 */
__kernel void matrixTranspose(__global float16 *pData16,
				int xDim1,
				int yDim1)
{
	int id=get_global_id(0);
	int yDim = yDim1 >> 4;
	int xDim = xDim1 >> 4;
	pData16 += (id * xDim1 * yDim1 >> 4);
	for(int i=0; i<xDim1; i+=16)
	{
		for(int j=(i>>4); j<yDim; j++)
		{
			__local float16 localBuf1[16];
			__global float16 *pDataG16 = pData16+i*yDim+j;
			for(int k=0; k<16; k++)
			{
				localBuf1[k] = *pDataG16;
				*pDataG16 = -1;
				pDataG16 += yDim;
			}
			__local float *pLocal2 = (__local float *)localBuf1;
			for(int m=0; m<16; m++)
			{
				//__local float *pLocal1 = pLocal2 + (m*16);
				for(int n=m; n<16; n++)
				{
					float temp;
					temp = pLocal2[m*16+n];
					pLocal2[m*16+n] = pLocal2[n*16+m];
					pLocal2[n*16+m] = temp;
				}
			}
			pDataG16 = pData16+i*yDim+j;
			if(i!=j)
			{
				__local float16 localBuf2[16];
				pDataG16 = pData16+((j<<4)*yDim+(i>>4));
				for(int k=0; k<16; k++)
				{
					localBuf2[k] = *pDataG16;
					pDataG16 += yDim;
				}
			__local float *pLocal2 = (__local float *)localBuf2;
				for(int m=0; m<16; m++)
				{
					for(int n=m; n<16; n++)
					{
				...
				for(int k=0; k<16; k++)
				{
					*pDataG16 = localBuf2[k]; pDataG16 += yDim;
				}
				pDataG16 = pData16+((j<<4)*yDim+(i>>4));
			}
			for(int k=0; k<16; k++)
			{
				*pDataG16 = localBuf1[k]; pDataG16 += yDim;
			}
		}
	}
}
