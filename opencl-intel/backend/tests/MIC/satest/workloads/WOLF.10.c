...
*/
#define galoisMultiplication(res,in1,in2)	\
    p = 0;									\
    a = in1;								\
    b = in2;								\
    for(unsigned int i=0; i < 8; ++i)		\
...
*/
#define mixColumns(res, block, galiosCoeff, j)								\
	unsigned char p=0, a=0, b=0;											\
    unsigned int bw = 4;													\
    uchar x, y, z, w;														\
    galoisMultiplication(x, block[0].x, galiosCoeff[(bw-j)%bw].x);			\
    galoisMultiplication(y, block[0].y, galiosCoeff[(bw-j)%bw].x);			\
    galoisMultiplication(z, block[0].z, galiosCoeff[(bw-j)%bw].x);			\
    galoisMultiplication(w, block[0].w, galiosCoeff[(bw-j)%bw].x);			\
    for(unsigned int k=1; k< 4; ++k)										\
    {																		\
		uchar x1, y1, z1, w1;												\
        galoisMultiplication(x1, block[k].x, galiosCoeff[(k+bw-j)%bw].x);	\
        galoisMultiplication(y1, block[k].y, galiosCoeff[(k+bw-j)%bw].x);	\
        galoisMultiplication(z1, block[k].z, galiosCoeff[(k+bw-j)%bw].x);	\
        galoisMultiplication(w1, block[k].w, galiosCoeff[(k+bw-j)%bw].x);	\
        x^=x1;y^=y1;z^=z1;w^=w1;											\
    }																		\
    res = (uchar4)(x, y, z, w);
uchar4
                ...
                const     uint     rounds   )
{
    ...
    for(unsigned int r=1; r < rounds; ++r)
    {
        ...
        mixColumns(block1[localIndex],block0, galiosCoeff, localIndex);
                ...
                const     uint      rounds    )
{
    ...
    for(unsigned int r=rounds -1 ; r > 0; --r)
    {
        ...
        barrier(CLK_LOCAL_MEM_FENCE);
        mixColumns(block0[localIndex],block1, galiosCoeff, localIndex);
