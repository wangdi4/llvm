/* AMD kernel */
__kernel void nbody_sim( __global float4* pos , __global float4* vel, int numBodies, float deltaTime, float epsSqr, __local float4* localPos,__global float4* updatedPos , __global float4* updatedVel)
{
...
__kernel void nbody_sim_double( __global double4* pos , __global double4* vel, int numBodies, double deltaTime, double epsSqr, __local double4* localPos,__global double4* updatedPos , __global double4* updatedVel)
{
	unsigned int tid = get_local_id(0);
	unsigned int gid = get_global_id(0);
	unsigned int localSize = get_local_size(0);
...
IntegrateSystemNonVectorized(
		...
		__global float *input_position_x, __global float *input_position_y, __global float *input_position_z,
		__global float *input_velocity_x, __global float *input_velocity_y, __global float *input_velocity_z,
		float time_delta, float damping, float softening_squared, int body_count, int body_count_per_group, __global float4 *output_position, int start_index )
{
...
IntegrateSystemVectorized(
		__global float4 *output_position_x,	__global float4 *output_position_y, __global float4 *output_position_z, __global float4 *mass,
		__global float4 *output_velocity_x, __global float4 *output_velocity_y, __global float4 *output_velocity_z,
		__global float4 *input_position_x, __global float4 *input_position_y, __global float4 *input_position_z,
		__global float4 *input_velocity_x, __global float4 *input_velocity_y, __global float4 *input_velocity_z,
		float time_delta, float damping, float softening_squared, int body_count, int body_count_per_group, __global float4 *output_position, int start_index )
{
	int index = get_global_id(0);
