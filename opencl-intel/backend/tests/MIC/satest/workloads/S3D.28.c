...
inline real POLYX(real x, real c0, real c1, real c2, real c3) {
  return (((((c3) * (x) + (c2)) * (x) + (c1)) * (x) + (c0)) * (x));
}
__kernel void rdsmh_kernel(__global const real *T, __global real *EG,
                           const real TCONV) {
