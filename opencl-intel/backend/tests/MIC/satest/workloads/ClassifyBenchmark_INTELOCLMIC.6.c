...
__kernel void multTransModOCL(__global double* ptrSource,
						...
						int offset,
                                         ...
                                         __global double* ptrDelta, int isFirstTimeFlag)
{
 ...
 for(int i = 0; i < sourceSize; i++)
 {
                         ...
                         f_0 = max(as_double(as_ulong(Alpha[0] * x_0 + Beta[0]) | Gamma[0] ) + Delta[0], 0.0);
                         double x_1 = ptrData[(i*5)+1];
                         f_1 = max(as_double(as_ulong(Alpha[1] * x_1 + Beta[1]) | Gamma[1] ) + Delta[1], 0.0);
                         double x_2 = ptrData[(i*5)+2];
                         f_2 = max(as_double(as_ulong(Alpha[2] * x_2 + Beta[2]) | Gamma[2] ) + Delta[2], 0.0);
                         double x_3 = ptrData[(i*5)+3];
                         f_3 = max(as_double(as_ulong(Alpha[3] * x_3 + Beta[3]) | Gamma[3] ) + Delta[3], 0.0);
                         double x_4 = ptrData[(i*5)+4];
                         f_4 = max(as_double(as_ulong(Alpha[4] * x_4 + Beta[4]) | Gamma[4] ) + Delta[4], 0.0);
                 myResult += ptrSource[i] *
                         (f_0) *
                         (f_1) *
                         (f_2) *
                         (f_3) *
