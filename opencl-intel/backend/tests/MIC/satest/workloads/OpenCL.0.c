...
void evalBoundaryCondTop(tfloat K, tfloat Ic, tfloat _maxz, tfloat D,
                        tfloat dT, tfloat dt, int m, tfloat* res)
{
...
void evalTermCondLastStep(tfloat* CB, tfloat K, tfloat Ic,
                                tfloat rn, tfloat minz, tfloat dz,
                                int J, tfloat* jexp)
{
    int j;
    for ( j = 0; j <= J; j++ )
...
void Sweep(int n, tfloat* v, tfloat* b,
           tfloat* alpha, tfloat* beta, tfloat* x)
{
    int i;
    alpha[1] = -v[2] / v[1];
    beta[1] = b[0] / v[1];
...
void evalOptimalConversionPrice(tfloat* CB, tfloat K, tfloat Ic,
                   tfloat rk, tfloat minz, tfloat dz, int J, tfloat* res,
                   tfloat* buff)
{
    int j, __j;
...
__kernel void
	evalConvertibleBond(
					  __global cb_params_struct* iparams,
                      __global tfloat* omin_z,
                      __global tfloat* omax_z,
                      __global tfloat* odz,
                      __global tfloat* ocb,
					  ...
					  )
{
    ...
    for (i=0; i <= _N; i++) buffs->r[i]=0;
    for( i = 0; i <= _N; i++ )
	{
        for ( j=1 ; j <= _M; j++ )
		{
			fabs_reslt = FABS((tfloat)i * dT - (tfloat)j);
            if ( fabs_reslt < EPSILON ) buffs->r[i] = params->rk;
        }
    }
    dz = ( max_z - min_z ) / (tfloat)_JGRID;
    dz2 = dz * dz;
    a[0] =  _SIG2 * dt / ( FOUR * dz2 ) - ( _RF - _D - HALF * _SIG2 ) * dt / ( FOUR * dz );
    a[1] = -_SIG2 * dt / ( TWO * dz2 ) - ONE - _RF * dt * HALF;
    a[2] =  _SIG2 * dt / ( FOUR * dz2 ) + ( _RF - _D - HALF * _SIG2 ) * dt / ( FOUR * dz );
    b[0] = -_SIG2 * dt / ( FOUR * dz2 ) + ( _RF - _D - HALF * _SIG2 ) * dt / ( FOUR * dz );
    b[1] =  _SIG2 * dt / ( TWO * dz2 ) - ONE + _RF * dt * HALF;
    b[2] = -_SIG2 * dt / ( FOUR * dz2 ) - ( _RF - _D - HALF * _SIG2 ) * dt / ( FOUR * dz );
    evalTermCondLastStep( res->cb, params->k, params->Ic, buffs->r[_N], min_z, dz, _JGRID, buffs->buff_j_1 );
    for ( k = _N - 1 ; k >= 0; k-- )
    {
        for ( p = _MGRID; p >= 0; p-- )
        {
            tridiag_mvMultiply( _JGRID+1, b, res->cb, buffs->cb_next );
            evalBoundaryCondBottom( params->k, _RF, dT, dt, buffs->r, _N, k, p, &c, buffs->buff_n);
            buffs->cb_next[0] -= a[0] * c ;
            evalBoundaryCondTop( params->k, params->Ic, max_z, _D, dT, dt, p, &c );
            buffs->cb_next[_JGRID - 2] -= a[2] * c;
            Sweep( _JGRID-1, a, buffs->cb_next, buffs->buff_j_1, buffs->buff_j_2, res->cb+1 );
		}
        evalOptimalConversionPrice(res->cb, params->k, params->Ic, buffs->r[k], min_z, dz, _JGRID, &c, buffs->buff_j_1);
        res->Vopt[k] = c;
        evalTermCond( res->cb, res->cb, params->k, params->Ic, buffs->r[k], min_z, dz,  _JGRID, buffs->buff_j_1);
	}
