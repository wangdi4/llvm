                        ...
                        const    int  pathN)
{
    ...
    double pathNinv = 1.0f / (double)(pathN + 1);
    {
        ...
        for(pos = 0; pos < boundary12; pos++)
        {
            ...
            double z = LOG(-LOG(p));
#ifdef USE_MAD
            double sample = mad(mad(mad(mad(mad(mad(mad(mad(c9,z,c8),z,c7),z,c6),z,c5),z,c4),z,c3),z,c2),z,c1);
#else
            double sample = c1 + z * (c2 + z * (c3 + z * (c4 + z * (c5 + z * (c6 + z * (c7 + z * (c8 + z * c9)))))));
#endif
            sample = -sample;
    ...
    }
    {
        ...
        for(pos = boundary12; pos < boundary23; pos++)
        {
            ...
            double z = y * y;
#ifdef USE_MAD
            double sample = y * mad(mad(mad(a4,z,a3),z,a2),z,a1) / mad(mad(mad(mad(b4,z,b3),z,b2),z,b1),z,1.0f);
#else
            double sample = y * (((a4 * z + a3) * z + a2) * z + a1) / ((((b4 * z + b3) * z + b2) * z + b1) * z + 1.0f);
#endif
            double callValue = S * EXP(sample * VBySqrtT + MuByT) - X;
    ...
    }
    {
        ...
        pOptionValue[optionid].Expected = (double)(ExpRT * totalsum / (double)pathN);
        //Standard deviation
        double stdDev = SQRT(((double)pathN * totalsum2 - totalsum * totalsum)/ ((double)pathN * (double)(pathN - 1)));
        //Confidence width; in 95% of all cases theoretical value lies within these borders
        pOptionValue[optionid].Confidence = (double)(ExpRT * 1.96f * stdDev / SQRT((double)pathN));
        //printf("Count=%d\n",count);
    }
}
__kernel
