		...
		__const int smoothLen)
{
	...
	for(size_t row = start_row; row < end_row; row++)
	{
		...
		if(smoothStart < 0 || smoothFinish >= num_rows)
		{
			...
			for(int i = smoothStart, nWeight = 0; i <= smoothFinish; i++, nWeight++)
			{
				if(i >= 0 && i < num_rows)
				{
			...
			for(int i = smoothStart, ind=0, nWeight=0; i <= smoothFinish; i++, nWeight++)
			{
				if(i >= 0 && i < num_rows)
				{
		...
		for(int rr = smoothStart, w = 0; rr <= smoothFinish; rr++, w++)
		{
			...
			if(edgeAxial) curr_weight = edge_kernel[w];
			else	      curr_weight = input_kernel[w];
			for(size_t channel = start_channel; channel < end_channel; channel++)
			{
				output[channel + row * num_channels] += input[channel + input_address] * curr_weight;
			}
		}
	}
}
__kernel
void BlankSampleKernel(
		const __global float *input,
		__global float *output,
		__const int num_channels,
		__const int num_rows,
