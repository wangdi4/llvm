...
#define galoisMultiplication(res, in1, in2)                                    \
  ...
  res = p;
inline uchar4 sbox(__global uchar *SBox, uchar4 block) {
  return (uchar4)(SBox[block.x], SBox[block.y], SBox[block.z], SBox[block.w]);
}
/*
uchar4
mixColumns(__local uchar4 * block, __private uchar4 * galiosCoeff, unsigned int
j)
{
    unsigned int bw = 4;
    uchar x, y, z, w;
...
__kernel void AESEncrypt(__global uchar4 *output, __global uchar4 *input,
                         __global uchar4 *roundKey, __global uchar *SBox,
                         __local uchar4 *block0, __local uchar4 *block1,
                         const uint width, const uint rounds) {
  ...
  unsigned int globalIndex =
      (((blockIdy * width / 4) + blockIdx) * 4) + (localIdy);
...
__kernel void AESDecrypt(__global uchar4 *output, __global uchar4 *input,
                         __global uchar4 *roundKey, __global uchar *SBox,
                         __local uchar4 *block0, __local uchar4 *block1,
                         const uint width, const uint rounds) {
  unsigned int blockIdx = get_group_id(0);
