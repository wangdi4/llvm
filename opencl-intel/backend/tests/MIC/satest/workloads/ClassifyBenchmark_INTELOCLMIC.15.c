						...
						uint offset)
{
	...
	for(int j = 0; j < fastStorageSize; j+=64)
	{
		...
		for(int k = 0; k < 64; k++)
		{
			...
			{
				curSupport *= max(( (locLevel[(k*5)+0]) * (data_0) ) - (locIndex[(k*5)+0]) + 1.0, 0.0);
			}
			else
			{
				curSupport *= max(1.0 - fabs( ( (locLevel[(k*5)+0]) * (data_0) ) - (locIndex[(k*5)+0]) ), 0.0);
			}
			...
			{
				curSupport *= max(( (locLevel[(k*5)+1]) * (data_1) ) - (locIndex[(k*5)+1]) + 1.0, 0.0);
			}
			else
			{
				curSupport *= max(1.0 - fabs( ( (locLevel[(k*5)+1]) * (data_1) ) - (locIndex[(k*5)+1]) ), 0.0);
			}
			...
			{
				curSupport *= max(( (locLevel[(k*5)+2]) * (data_2) ) - (locIndex[(k*5)+2]) + 1.0, 0.0);
			}
			else
			{
				curSupport *= max(1.0 - fabs( ( (locLevel[(k*5)+2]) * (data_2) ) - (locIndex[(k*5)+2]) ), 0.0);
			}
			...
			{
				curSupport *= max(( (locLevel[(k*5)+3]) * (data_3) ) - (locIndex[(k*5)+3]) + 1.0, 0.0);
			}
			else
			{
				curSupport *= max(1.0 - fabs( ( (locLevel[(k*5)+3]) * (data_3) ) - (locIndex[(k*5)+3]) ), 0.0);
			}
			...
			{
				curSupport *= max(( (locLevel[(k*5)+4]) * (data_4) ) - (locIndex[(k*5)+4]) + 1.0, 0.0);
			}
			else
			{
				curSupport *= max(1.0 - fabs( ( (locLevel[(k*5)+4]) * (data_4) ) - (locIndex[(k*5)+4]) ), 0.0);
			}
			myResult += curSupport;
		}
		barrier(CLK_LOCAL_MEM_FENCE);
	...
	for(int m = fastStorageSize; m < storageSize; m++)
	{
		...
		{
			curSupport *= max(( (ptrLevel[(m*5)+0]) * (data_0) ) - (ptrIndex[(m*5)+0]) + 1.0, 0.0);
		}
		else
		{
			curSupport *= max(1.0 - fabs( ( (ptrLevel[(m*5)+0]) * (data_0) ) - (ptrIndex[(m*5)+0]) ), 0.0);
		}
		...
		{
			curSupport *= max(( (ptrLevel[(m*5)+1]) * (data_1) ) - (ptrIndex[(m*5)+1]) + 1.0, 0.0);
		}
		else
		{
			curSupport *= max(1.0 - fabs( ( (ptrLevel[(m*5)+1]) * (data_1) ) - (ptrIndex[(m*5)+1]) ), 0.0);
		}
		...
		{
			curSupport *= max(( (ptrLevel[(m*5)+2]) * (data_2) ) - (ptrIndex[(m*5)+2]) + 1.0, 0.0);
		}
		else
		{
			curSupport *= max(1.0 - fabs( ( (ptrLevel[(m*5)+2]) * (data_2) ) - (ptrIndex[(m*5)+2]) ), 0.0);
		}
		...
		{
			curSupport *= max(( (ptrLevel[(m*5)+3]) * (data_3) ) - (ptrIndex[(m*5)+3]) + 1.0, 0.0);
		}
		else
		{
			curSupport *= max(1.0 - fabs( ( (ptrLevel[(m*5)+3]) * (data_3) ) - (ptrIndex[(m*5)+3]) ), 0.0);
		}
		...
		{
			curSupport *= max(( (ptrLevel[(m*5)+4]) * (data_4) ) - (ptrIndex[(m*5)+4]) + 1.0, 0.0);
		}
		else
		{
			curSupport *= max(1.0 - fabs( ( (ptrLevel[(m*5)+4]) * (data_4) ) - (ptrIndex[(m*5)+4]) ), 0.0);
		}
		myResult += curSupport;
	}
