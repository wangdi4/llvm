...
__kernel
void intel_MonteCarloKernel(__global TOptionValue * pOptionValue,
                        ...
                        const    int  pathN)
{
    ...
    float pathNinv = 1.0f / (float)(pathN + 1);
    {
        ...
        for(pos = 0; pos < boundary12; pos++)
        {
            ...
            float z = LOG(-LOG(p));
#ifdef USE_MAD
            float sample = mad(mad(mad(mad(mad(mad(mad(mad(c9,z,c8),z,c7),z,c6),z,c5),z,c4),z,c3),z,c2),z,c1);
#else
            float sample = c1 + z * (c2 + z * (c3 + z * (c4 + z * (c5 + z * (c6 + z * (c7 + z * (c8 + z * c9)))))));
#endif
            sample = -sample;
    ...
    }
    {
        ...
        for(pos = boundary12; pos < boundary23; pos++)
        {
            ...
            float z = y * y;
#ifdef USE_MAD
            float sample = y * mad(mad(mad(a4,z,a3),z,a2),z,a1) / mad(mad(mad(mad(b4,z,b3),z,b2),z,b1),z,1.0f);
#else
            float sample = y * (((a4 * z + a3) * z + a2) * z + a1) / ((((b4 * z + b3) * z + b2) * z + b1) * z + 1.0f);
#endif
            float callValue = S * EXP(sample * VBySqrtT + MuByT) - X;
    ...
    }
    {
        ...
        //Standard deviation
        float stdDev = SQRT(((float)pathN * totalsum2 - totalsum * totalsum)/ ((float)pathN * (float)(pathN - 1)));
        //Confidence width; in 95% of all cases theoretical value lies within these borders
        pOptionValue[optionid].Confidence = (float)(ExpRT * 1.96f * stdDev / SQRT((float)pathN));
        //printf("Count=%d\n",count);
    }
}
__kernel
void intel_MonteCarloKernel_RandomSamples(
    __global TOptionValue*  pOptionValue,
    __global TOptionData*   pOptionData,
    const    int            pathN)
{
    const int optionid = get_global_id(0);
...
#define RND_LOWER_MASK 0x7fffffffUL /* least significant r bits */
    ...
    for (i=1; i<RND_N; i++)mt[i] = ((1812433253UL * (mt[i-1] ^ (mt[i-1] >> 30)) + i))&0xffffffffUL;
    for(i=0;i<pathN;++i)
    {
        ...
        else if (p<boundary23)
        {
            ...
            float y = p - 0.5f;
            float z = y * y;
            sample = y * (((a4 * z + a3) * z + a2) * z + a1) / ((((b4 * z + b3) * z + b2) * z + b1) * z + 1.0f);
        }
        else
        {
            // Region 3 of Moro polynomial approximation to inverse CND
            ...
            const float c9 = 3.960315187E-07f;
            float z = LOG(-LOG(1.0f-p));
            sample = c1 + z * (c2 + z * (c3 + z * (c4 + z * (c5 + z * (c6 + z * (c7 + z * (c8 + z * c9)))))));
        }
...
__kernel
void intel_MonteCarloKernel_PrecalculatedSamples(
    __global TOptionValue * pOptionValue,
    __global TOptionData * pOptionData,
    __global float * pSamples,
    const    int  pathN)
{
    const int optionid = get_global_id(0);
    float T = pOptionData[optionid].T;
