...
#endif // AFCL
#endif // CUDACC
void _getkernel_0(__global ty *dst,
                  const __global ty *src,
                  ...
                  unsigned base, const __global unsigned *c_meta)
{
...
void _getkernel_1(__global ty *dst,
                  const __global ty *src,
                  unsigned ndst, unsigned nsrc, unsigned ngfor,
                  float start, float inc,
                  const __global void *d1,
                  ...
                  unsigned base, const __global unsigned *c_meta)
{
    ...
    __global ty *d_dst = (__global ty *)dst + x;
    const __global ty *d_src = (const __global ty *)src;
    if (x >= ngfor * ndst) return;
    const unsigned y = x2x_n(x_, base, d1, d2, d3, i, id, c_meta);
    *d_dst = (y < nsrc) ? d_src[y] : NAN; // ensure bounds
}
void _getkernel_2(__global ty *dst,
                  const __global ty *src,
                  unsigned ndst, unsigned nsrc, unsigned ngfor,
                  float start, float inc,
                  const __global void *d1,
                  ...
                  unsigned base, const __global unsigned *c_meta)
{
    ...
    unsigned id = x / ndst, x_ = x - id * ndst; // block id and index
    __global ty *d_dst = (__global ty *)dst + x;
    const __global ty *d_src = (const __global ty *)src + id * nsrc; // tile offset
    if (x >= ngfor * ndst) return;
    const unsigned y = x2x_(x_, base, d1, d2, d3, c_meta);
    *d_dst = (y < nsrc*ngfor) ? d_src[y] : NAN; // ensure bounds
}
void _getkernel_3(__global ty *dst,
                  const __global ty *src,
                  unsigned ndst, unsigned nsrc, unsigned ngfor,
                  float start, float inc,
                  const __global void *d1,
...
__kernel void getkernel(__global ty *dst,
                        const __global ty *src,
                        unsigned ndst, unsigned nsrc, unsigned ngfor,
                        float start, float inc,
                        const __global void *d1,
                        ...
                        unsigned base, const __global unsigned *c_meta)
{
    _getkernel_0(dst, src, ndst, nsrc, ngfor, start, inc, d1, d2, d3, base, c_meta);
}
