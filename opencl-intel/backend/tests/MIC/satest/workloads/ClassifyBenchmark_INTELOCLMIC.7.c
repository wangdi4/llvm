...
__kernel void multModOCL(__global double* ptrAlpha,
						...
						int storageSize,
						int offset,
                                         __global double* ptrAlpha1,
                                         __global double* ptrBeta,
                                         ...
                                         __global double* ptrDelta)
{
 ...
 double f_4 = 1.0;
	...
	{
                 f_0 = max(as_double(as_ulong(ptrAlpha1[(j*5)+0] * data_0 + ptrBeta[(j*5)+0]) | ptrGamma[(j*5)+0] ) + ptrDelta[(j*5)+0], 0.0);
                 f_1 = max(as_double(as_ulong(ptrAlpha1[(j*5)+1] * data_1 + ptrBeta[(j*5)+1]) | ptrGamma[(j*5)+1] ) + ptrDelta[(j*5)+1], 0.0);
                 f_2 = max(as_double(as_ulong(ptrAlpha1[(j*5)+2] * data_2 + ptrBeta[(j*5)+2]) | ptrGamma[(j*5)+2] ) + ptrDelta[(j*5)+2], 0.0);
                 f_3 = max(as_double(as_ulong(ptrAlpha1[(j*5)+3] * data_3 + ptrBeta[(j*5)+3]) | ptrGamma[(j*5)+3] ) + ptrDelta[(j*5)+3], 0.0);
                 f_4 = max(as_double(as_ulong(ptrAlpha1[(j*5)+4] * data_4 + ptrBeta[(j*5)+4]) | ptrGamma[(j*5)+4] ) + ptrDelta[(j*5)+4], 0.0);
                 myResult += ptrAlpha[j] *
                         (f_0) *
                         (f_1) *
                         (f_2) *
