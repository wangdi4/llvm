__kernel void multTransModSPOCL(__global float* ptrSource,
						...
						uint offset)
{
	...
	for(int i = 0; i < sourceSize; i+=64)
	{
		...
		for(int k = 0; k < 64; k++)
		{
			...
			{
				curSupport *= max(( (level_0) * (locData[(k*5)+0]) ) - (index_0) + 1.0f, 0.0f);
			}
			else
			{
				curSupport *= max(1.0f - fabs( ( (level_0) * (locData[(k*5)+0]) ) - (index_0) ), 0.0f);
			}
			if ((level_1) == 2.0f)
			{
				curSupport *= 1.0f;
			...
			{
				curSupport *= max(( (level_1) * (locData[(k*5)+1]) ) - (index_1) + 1.0f, 0.0f);
			}
			else
			{
				curSupport *= max(1.0f - fabs( ( (level_1) * (locData[(k*5)+1]) ) - (index_1) ), 0.0f);
			}
			if ((level_2) == 2.0f)
			{
				curSupport *= 1.0f;
			...
			{
				curSupport *= max(( (level_2) * (locData[(k*5)+2]) ) - (index_2) + 1.0f, 0.0f);
			}
			else
			{
				curSupport *= max(1.0f - fabs( ( (level_2) * (locData[(k*5)+2]) ) - (index_2) ), 0.0f);
			}
			if ((level_3) == 2.0f)
			{
				curSupport *= 1.0f;
			...
			{
				curSupport *= max(( (level_3) * (locData[(k*5)+3]) ) - (index_3) + 1.0f, 0.0f);
			}
			else
			{
				curSupport *= max(1.0f - fabs( ( (level_3) * (locData[(k*5)+3]) ) - (index_3) ), 0.0f);
			}
			if ((level_4) == 2.0f)
			{
				curSupport *= 1.0f;
			...
			{
				curSupport *= max(( (level_4) * (locData[(k*5)+4]) ) - (index_4) + 1.0f, 0.0f);
			}
			else
			{
				curSupport *= max(1.0f - fabs( ( (level_4) * (locData[(k*5)+4]) ) - (index_4) ), 0.0f);
			}
		myResult += curSupport;
		}
		barrier(CLK_LOCAL_MEM_FENCE);
