...
void HairCvSimEvalOpt_evaluate(
    ...
    int local_flag,
    double local_blend,
    __global double* hairStiff,
    __global double* hairDamp,
    __global int* mHairPointCount,
    __global PrevModelData* mPreviousModelData,
    __global double4* mInputMcAsArray,
    __global double4* mOutputMcAsArray,
    int mFlexProfileModel,
    int mStiffnessProfileModel,
    __global double* mSampledFlexProfile,
    __global double* mSampledStiffnessProfile,
    __global double4* local_ref_xfm_inv,
    __global double4* ref_xfm_inv,
    __global double4* local_ref_xfm,
    double mDynablend
)
{
    ...
    int idx = h * nPoints + p;
    // TODO: this indexing will not work if num points per hair is not the same for all hairs
    ...
    if (p > 0)
	{
        if (!reset_flag)
		{
            ...
            //if (mStiffnessProfileModel != 0) stiff_mult = mSampledStiffnessProfile[p];
            ...
            pt_data[idx].prevPos.xyz = x_prev + delta_x; // Copy position into prev_pos
        ...
        if (local_flag){
            ...
            pt_data[idx].prevPosLoc.xyz = pt_data[idx].curPosLoc.xyz + pt_data[idx].delta.xyz;
        }
    }
