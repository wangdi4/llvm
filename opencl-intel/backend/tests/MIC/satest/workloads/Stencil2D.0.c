...
#endif
inline int ToGlobalRow(int gidRow, int lidRow) {
...
inline int ToGlobalCol(int gidCol, int lidCol) {
  // assumes coordinates and dimensions are logical (without halo)
  // returns logical global column (without halo)
  return gidCol * LCOLS + lidCol;
}
inline int ToFlatHaloedIdx(int row, int col, int rowWidth) {
...
__kernel void StencilKernel(__global VALTYPE *data, __global VALTYPE *newData,
                            VALTYPE wCenter, VALTYPE wCardinal,
                            VALTYPE wDiagonal) {
  // determine our location in the OpenCL coordinate system
  int gidRow = get_group_id(1);
  int gidCol = get_group_id(0);
  int gszRow = get_num_groups(1);
  ...
  if (lidRow == 0) {
    ...
    sh[LROWS + 1][lidCol + 1] =
        data[ToFlatHaloedIdx(gRow + LROWS, gCol, gRowWidth)];
  }
  if (lidCol == 0) {
    sh[lidRow + 1][0] = data[ToFlatHaloedIdx(gRow, gCol - 1, gRowWidth)];
    sh[lidRow + 1][LCOLS + 1] =
        data[ToFlatHaloedIdx(gRow, gCol + LCOLS, gRowWidth)];
  }
  if ((lidRow == 0) && (lidCol == 0)) {
    ...
    sh[LROWS + 1][LCOLS + 1] =
        data[ToFlatHaloedIdx(gRow + LROWS, gCol + LCOLS, gRowWidth)];
  }
  // let all those loads finish
  barrier(CLK_LOCAL_MEM_FENCE);
  // do my part of the smoothing operation
  VALTYPE centerValue = sh[lidRow + 1][lidCol + 1];
  VALTYPE cardinalValueSum =
      sh[lidRow][lidCol + 1] + sh[lidRow + 2][lidCol + 1] +
      sh[lidRow + 1][lidCol] + sh[lidRow + 1][lidCol + 2];
  VALTYPE diagonalValueSum = sh[lidRow][lidCol] + sh[lidRow][lidCol + 2] +
                             sh[lidRow + 2][lidCol] +
                             sh[lidRow + 2][lidCol + 2];
  newData[gidx] = wCenter * centerValue + wCardinal * cardinalValueSum +
