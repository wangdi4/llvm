...
#define FPVECTYPE double4
#endif
__kernel void reduce(__global const FPTYPE *in, __global FPTYPE *isums,
                     const int n, __local FPTYPE *lmem) {
...
__kernel void top_scan(__global FPTYPE *isums, const int n,
                       __local FPTYPE *lmem) {
  FPTYPE val = 0.0f;
  if (get_local_id(0) < n) {
...
__kernel void bottom_scan(__global const FPTYPE *in,
                          __global const FPTYPE *isums, __global FPTYPE *out,
                          const int n, __local FPTYPE *lmem) {
