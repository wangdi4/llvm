...
define <16 x float> @broadcastpf32(float* %pe) nounwind readnone ssp {
; CHECK: vbroadcastss (%rdi), %zmm0
  %e = load float* %pe
...
define <8 x double> @broadcastpf64(double* %pe) nounwind readnone ssp {
; CHECK: vbroadcastsd (%rdi), %zmm0
  %e = load double* %pe
...
define <16 x i32> @broadcastpi32(i32* %pi) nounwind readnone ssp {
; CHECK: vbroadcastss (%rdi), %zmm0
  %i = load i32* %pi
...
define <8 x i64> @broadcasti64(i64* %pe) nounwind readnone ssp {
; CHECK: vbroadcastsd (%rdi), %zmm0
  %e = load i64* %pe
