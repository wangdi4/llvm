...
declare void @____Vectorized_.readGlobalMemoryCoalesced_original(float addrspace(1)* nocapture, float addrspace(1)* nocapture, i32) nounwind
...
; <label>:7                                       ; preds = %7, %SyncBB4
  %s.03 = phi i32 [ %6, %SyncBB4 ], [ %105, %7 ]
  %sum.02 = phi float [ 0.000000e+00, %SyncBB4 ], [ %102, %7 ]
  %j.01 = phi i32 [ 0, %SyncBB4 ], [ %103, %7 ]
  %8 = and i32 %s.03, %0
  %9 = sext i32 %8 to i64
...
define void @____Vectorized_.readGlobalMemoryCoalesced_separated_args(float addrspace(1)* nocapture %data, float addrspace(1)* nocapture %output, i32 %size, i8 addrspace(3)* %pLocalMem, %struct.WorkDim* %pWorkDim, i64* %pWGId, %struct.PaddedDimId* %pBaseGlbId, %struct.PaddedDimId* %pLocalIds, i64* %contextpointer, i64 %iterCount, i8* %pSpecialBuf, i64* %pCurrWI) nounwind alwaysinline {
bb.nph:
  %0 = add nsw i32 %size, -1
...
; <label>:8                                       ; preds = %8, %SyncBB
  %vectorPHI = phi <16 x i32> [ %7, %SyncBB ], [ %1082, %8 ]
  %vectorPHI1 = phi <16 x float> [ zeroinitializer, %SyncBB ], [ %1079, %8 ]
  %j.01 = phi i32 [ 0, %SyncBB ], [ %1080, %8 ]
...
; <label>:24                                      ; preds = %24, %SyncBB4.i
  %s.03.i = phi i32 [ %23, %SyncBB4.i ], [ %122, %24 ]
  %sum.02.i = phi float [ 0.000000e+00, %SyncBB4.i ], [ %119, %24 ]
  %j.01.i = phi i32 [ 0, %SyncBB4.i ], [ %120, %24 ]
...
define void @__Vectorized_.readGlobalMemoryCoalesced(i8* %pBuffer) {
entry:
  %0 = bitcast i8* %pBuffer to float addrspace(1)**
  %1 = load float addrspace(1)** %0, align 8
  %2 = getelementptr i8* %pBuffer, i64 8
...
; <label>:25                                      ; preds = %25, %SyncBB.i
  %vectorPHI.i = phi <16 x i32> [ %24, %SyncBB.i ], [ %1099, %25 ]
  %vectorPHI1.i = phi <16 x float> [ zeroinitializer, %SyncBB.i ], [ %1096, %25 ]
  %j.01.i = phi i32 [ 0, %SyncBB.i ], [ %1097, %25 ]
...
._crit_edge.i:                                    ; preds = %25
  ...
  %check.WI.iter.i = icmp ult i64 %CurrWI..0.i, %16
  br i1 %check.WI.iter.i, label %thenBB.i, label %____Vectorized_.readGlobalMemoryCoalesced_separated_args.exit

thenBB.i:                                         ; preds = %._crit_edge.i
  %"CurrWI++.i" = add nuw i64 %CurrWI..0.i, 1
  br label %SyncBB.i

____Vectorized_.readGlobalMemoryCoalesced_separated_args.exit: ; preds = %._crit_edge.i
  ret void
}
