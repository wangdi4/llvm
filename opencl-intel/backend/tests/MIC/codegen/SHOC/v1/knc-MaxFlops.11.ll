...
declare void @__MAdd2_original(float addrspace(1)* nocapture, i32) nounwind
...
define void @__MAdd2_separated_args(float addrspace(1)* nocapture %data, i32 %nIters, i8 addrspace(3)* %pLocalMem, %struct.WorkDim* %pWorkDim, i64* %pWGId, %struct.PaddedDimId* %pBaseGlbId, %struct.PaddedDimId* %pLocalIds, i64* %contextpointer, i64 %iterCount, i8* %pSpecialBuf, i64* %pCurrWI) nounwind alwaysinline {
FirstBB:
...
bb.nph:                                           ; preds = %SyncBB6, %bb.nph
  %j.03 = phi i32 [ %290, %bb.nph ], [ 0, %SyncBB6 ]
  %s2.02 = phi float [ %289, %bb.nph ], [ %9, %SyncBB6 ]
  %s.01 = phi float [ %285, %bb.nph ], [ %8, %SyncBB6 ]
  ...
  %286 = fpext float %281 to double
  %287 = fmul double %286, 9.899000e-01
  %288 = fsub double 1.000000e+01, %287
  %289 = fptrunc double %288 to float
  %290 = add nsw i32 %j.03, 1
  %exitcond = icmp eq i32 %290, %nIters
  br i1 %exitcond, label %._crit_edge, label %bb.nph

._crit_edge:                                      ; preds = %bb.nph, %SyncBB6
  %s2.0.lcssa = phi float [ %9, %SyncBB6 ], [ %289, %bb.nph ]
  %s.0.lcssa = phi float [ %8, %SyncBB6 ], [ %285, %bb.nph ]
  %291 = fadd float %s.0.lcssa, %s2.0.lcssa
  store float %291, float addrspace(1)* %7, align 4
...
bb.nph.i:                                         ; preds = %bb.nph.i, %SyncBB6.i
  %j.03.i = phi i32 [ %304, %bb.nph.i ], [ 0, %SyncBB6.i ]
  %s2.02.i = phi float [ %303, %bb.nph.i ], [ %23, %SyncBB6.i ]
  %s.01.i = phi float [ %299, %bb.nph.i ], [ %22, %SyncBB6.i ]
  ...
  %296 = fpext float %291 to double
  %297 = fmul double %296, 9.899000e-01
  %298 = fsub double 1.000000e+01, %297
  %299 = fptrunc double %298 to float
  %300 = fpext float %295 to double
  %301 = fmul double %300, 9.899000e-01
  %302 = fsub double 1.000000e+01, %301
  %303 = fptrunc double %302 to float
  %304 = add nsw i32 %j.03.i, 1
  %exitcond.i = icmp eq i32 %304, %4
  br i1 %exitcond.i, label %._crit_edge.i, label %bb.nph.i

._crit_edge.i:                                    ; preds = %bb.nph.i, %SyncBB6.i
  %s2.0.lcssa.i = phi float [ %23, %SyncBB6.i ], [ %303, %bb.nph.i ]
  %s.0.lcssa.i = phi float [ %22, %SyncBB6.i ], [ %299, %bb.nph.i ]
  %305 = fadd float %s.0.lcssa.i, %s2.0.lcssa.i
  store float %305, float addrspace(1)* %21, align 4
...
!0 = metadata !{void (float addrspace(1)*, i32, i8 addrspace(3)*, %struct.WorkDim*, i64*, %struct.PaddedDimId*, %struct.PaddedDimId*, i64*, i64, i8*, i64*)* @__MAdd2_separated_args, metadata !1, metadata !1, metadata !"", metadata !"float __attribute__((address_space(1))) *, int", metadata !"opencl_MAdd2_locals_anchor", void (i8*)* @MAdd2}
