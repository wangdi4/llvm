...
; RUN: %oclopt --ocl-vecclone --ocl-vec-clone-isa-encoding-override=AVX512Core --less-than-two-gig-max-work-group-size=false < %s -S -enable-debugify -disable-output 2>&1 | FileCheck -check-prefixes=DEBUGIFY,DEBUGIFY2 %s
