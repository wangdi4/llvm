...
; RUN: %oclopt < %s --ocl-vecclone --dpcpp-vector-variant-isa-encoding-override=AVX512Core -S -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: %oclopt < %s --ocl-vecclone --dpcpp-vector-variant-isa-encoding-override=AVX512Core -S -o - | FileCheck %s
