; RUN: %oclopt --dpcpp-vector-variant-isa-encoding-override=AVX512Core -dpcpp-kernel-vec-dim-analysis -ocl-vecclone -S %s -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: %oclopt --dpcpp-vector-variant-isa-encoding-override=AVX512Core -dpcpp-kernel-vec-dim-analysis -ocl-vecclone -S %s -o %t
