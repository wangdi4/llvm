; REQUIRES: 64bit
; RUN: %oclopt --ocl-vecclone -dpcpp-vect-info=%p/../Inputs/VectInfo64.gen --dpcpp-vector-variant-isa-encoding-override=AVX512Core < %s -S -enable-debugify -disable-output 2>&1 | FileCheck -check-prefix=DEBUGIFY %s
; RUN: %oclopt --ocl-vecclone -dpcpp-vect-info=%p/../Inputs/VectInfo64.gen -vplan-vec --dpcpp-vector-variant-isa-encoding-override=AVX512Core < %s -S -o - | FileCheck %s
