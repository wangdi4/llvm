...
entry:
	...
	%tmp1 = load i32, i32* %width.addr		; <i32> [#uses=3]
	...
	%tmp3 = load i32, i32* %tid		; <i32> [#uses=2]
	%tmp4 = load i32, i32* %width.addr		; <i32> [#uses=3]
	...
	%tmp11 = load float, float* %scale.addr		; <float> [#uses=1]
	...
	%tmp20 = load i32, i32* %j		; <i32> [#uses=1]
	%conv21 = sitofp i32 %tmp20 to float		; <float> [#uses=1]
	%tmp22 = load float, float* %scale.addr		; <float> [#uses=1]
	%mul23 = fmul float %conv21, %tmp22		; <float> [#uses=1]
	%tmp24 = load float, float* %scale.addr		; <float> [#uses=1]
	%div25 = fdiv float %tmp24, 2.000000e+000		; <float> [#uses=1]
	%tmp26 = load i32, i32* %width.addr		; <i32> [#uses=1]
	...
	%tmp30 = load i32, i32* %width.addr		; <i32> [#uses=1]
	%conv31 = sitofp i32 %tmp30 to float		; <float> [#uses=1]
	%div32 = fdiv float %sub29, %conv31		; <float> [#uses=1]
	store float %div32, float* %y0
	%tmp34 = load float, float* %x0		; <float> [#uses=1]
	store float %tmp34, float* %x
	%tmp36 = load float, float* %y0		; <float> [#uses=1]
	store float %tmp36, float* %y
	%tmp38 = load float, float* %x		; <float> [#uses=1]
	%tmp39 = load float, float* %x		; <float> [#uses=1]
	%mul40 = fmul float %tmp38, %tmp39		; <float> [#uses=1]
	store float %mul40, float* %x2
	%tmp42 = load float, float* %y		; <float> [#uses=1]
	%tmp43 = load float, float* %y		; <float> [#uses=1]
	%mul44 = fmul float %tmp42, %tmp43		; <float> [#uses=1]
	store float %mul44, float* %y2
	%tmp46 = load float, float* %scale.addr		; <float> [#uses=1]
	%tmp47 = load float, float* %scale.addr		; <float> [#uses=1]
...
for.cond:		; preds = %for.inc, %entry
	%tmp50 = load float, float* %x2		; <float> [#uses=1]
	%tmp51 = load float, float* %y2		; <float> [#uses=1]
	%add = fadd float %tmp50, %tmp51		; <float> [#uses=1]
	%tmp52 = load float, float* %scaleSquare		; <float> [#uses=1]
	%cmp53 = fcmp ole float %add, %tmp52		; <i1> [#uses=1]
	br i1 %cmp53, label %land.lhs.true, label %for.end

land.lhs.true:		; preds = %for.cond
	%tmp55 = load i32, i32* %iter		; <i32> [#uses=1]
	%tmp56 = load i32, i32* %maxIterations.addr		; <i32> [#uses=1]
	%cmp57 = icmp ult i32 %tmp55, %tmp56		; <i1> [#uses=1]
	br i1 %cmp57, label %for.body, label %for.end

for.body:		; preds = %land.lhs.true
	%tmp59 = load float, float* %x		; <float> [#uses=1]
	%mul60 = fmul float 2.000000e+000, %tmp59		; <float> [#uses=1]
	%tmp61 = load float, float* %y		; <float> [#uses=1]
	%mul62 = fmul float %mul60, %tmp61		; <float> [#uses=1]
	%tmp63 = load float, float* %y0		; <float> [#uses=1]
	%add64 = fadd float %mul62, %tmp63		; <float> [#uses=1]
	store float %add64, float* %y
	%tmp65 = load float, float* %x2		; <float> [#uses=1]
	%tmp66 = load float, float* %y2		; <float> [#uses=1]
	%sub67 = fsub float %tmp65, %tmp66		; <float> [#uses=1]
	%tmp68 = load float, float* %x0		; <float> [#uses=1]
	%add69 = fadd float %sub67, %tmp68		; <float> [#uses=1]
	store float %add69, float* %x
	%tmp70 = load float, float* %x		; <float> [#uses=1]
	%tmp71 = load float, float* %x		; <float> [#uses=1]
	%mul72 = fmul float %tmp70, %tmp71		; <float> [#uses=1]
	store float %mul72, float* %x2
	%tmp73 = load float, float* %y		; <float> [#uses=1]
	%tmp74 = load float, float* %y		; <float> [#uses=1]
	%mul75 = fmul float %tmp73, %tmp74		; <float> [#uses=1]
	store float %mul75, float* %y2
	br label %for.inc

for.inc:		; preds = %for.body
	%tmp76 = load i32, i32* %iter		; <i32> [#uses=1]
