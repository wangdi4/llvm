...
for.cond:		; preds = %for.inc, %entry
	...
	%tmp4 = load i32, i32* %BIN_SIZE.addr		; <i32> [#uses=1]
	%cmp = icmp ult i32 %tmp, %tmp4		; <i1> [#uses=1]
	br i1 %cmp, label %for.body, label %for.end

for.body:		; preds = %for.cond
	%tmp5 = load i32, i32* %localId		; <i32> [#uses=1]
	%tmp6 = load i32, i32* %BIN_SIZE.addr		; <i32> [#uses=1]
...
for.cond12:		; preds = %for.inc35, %for.end
	%tmp13 = load i32, i32* %i11		; <i32> [#uses=1]
	%tmp14 = load i32, i32* %BIN_SIZE.addr		; <i32> [#uses=1]
	%cmp15 = icmp ult i32 %tmp13, %tmp14		; <i1> [#uses=1]
	br i1 %cmp15, label %for.body16, label %for.end38

for.body16:		; preds = %for.cond12
	%tmp18 = load i32, i32* %globalId		; <i32> [#uses=1]
	%tmp19 = load i32, i32* %BIN_SIZE.addr		; <i32> [#uses=1]
...
for.inc35:		; preds = %for.body16
	%tmp36 = load i32, i32* %i11		; <i32> [#uses=1]
...
for.cond41:		; preds = %for.inc85, %for.end38
	%tmp42 = load i32, i32* %i40		; <i32> [#uses=1]
	%tmp43 = load i32, i32* %BIN_SIZE.addr		; <i32> [#uses=1]
	%tmp44 = load i32, i32* %GROUP_SIZE		; <i32> [#uses=2]
...
for.cond50:		; preds = %for.inc69, %for.body47
	%tmp51 = load i32, i32* %j		; <i32> [#uses=1]
	%tmp52 = load i32, i32* %GROUP_SIZE		; <i32> [#uses=1]
	%cmp53 = icmp ult i32 %tmp51, %tmp52		; <i1> [#uses=1]
	br i1 %cmp53, label %for.body54, label %for.end72

for.body54:		; preds = %for.cond50
	%tmp55 = load i32, i32* %binCount		; <i32> [#uses=1]
	%tmp56 = load i32, i32* %j		; <i32> [#uses=1]
	%tmp57 = load i32, i32* %BIN_SIZE.addr		; <i32> [#uses=1]
	%mul58 = mul i32 %tmp56, %tmp57		; <i32> [#uses=1]
	%tmp59 = load i32, i32* %i40		; <i32> [#uses=1]
	%tmp60 = load i32, i32* %GROUP_SIZE		; <i32> [#uses=1]
...
for.end72:		; preds = %for.cond50
	%tmp73 = load i32, i32* %groupId		; <i32> [#uses=1]
	%tmp74 = load i32, i32* %BIN_SIZE.addr		; <i32> [#uses=1]
	%mul75 = mul i32 %tmp73, %tmp74		; <i32> [#uses=1]
	%tmp76 = load i32, i32* %i40		; <i32> [#uses=1]
	%tmp77 = load i32, i32* %GROUP_SIZE		; <i32> [#uses=1]
...
for.inc85:		; preds = %for.end72
	%tmp86 = load i32, i32* %i40		; <i32> [#uses=1]
