...
; CHECK: footer{{[0-9]*}}:                                 ; preds = %header{{[0-9]*}}
; CHECK: ret
define void @BypassCase4(i32 %arg1, i32 %arg2, float addrspace(1)* nocapture %a, float addrspace(1)* nocapture %b) nounwind {
entry:
...
if.then:                                          ; preds = %entry
  ...
  %cmp6 = fcmp ogt float %mul, 5.000000e+000
