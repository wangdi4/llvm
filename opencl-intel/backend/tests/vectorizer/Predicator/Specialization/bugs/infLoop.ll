...
; RUN: %oclopt  -runtimelib %p/../../../Full/runtime.bc -O3 -inline-threshold=4096 -inline -lowerswitch -scalarize -mergereturn -loop-simplify -phicanon -predicate -mem2reg -dce -packetize -packet-size=4 -resolve -verify %t.bc -S -o %t1.ll -debug-only=dpcpp-kernel-work-item-analysis 2>&1 | FileCheck %s
; ModuleID = 'infLoop.ll'
