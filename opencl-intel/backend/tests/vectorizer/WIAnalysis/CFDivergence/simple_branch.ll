; RUN: %oclopt -runtimelib %p/../../Full/runtime.bc -O3  -print-wia-check -inline-threshold=4096 -inline -lowerswitch -scalarize -mergereturn -loop-simplify -phicanon -predicate -mem2reg -dce -packetize -packet-size=4 -resolve -verify %s -S -o %t1.ll -debug-only=dpcpp-kernel-work-item-analysis 2>&1 | FileCheck %s
...
; CHECK: WIA 4   %x = phi i32 [ %spec.select, %true ], [ %x1, %false ]
