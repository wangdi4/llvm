; RUN: %oclopt -runtimelib %p/../../Full/runtime.bc -O3  -print-wia-check -inline-threshold=4096 -inline -lowerswitch -scalarize -mergereturn -loop-simplify -phicanon -predicate -mem2reg -dce -packetize -packet-size=4 -resolve -verify %s -S -o %t1.ll | FileCheck %s
...
; CHECK: WI-RunOnFunction 4   %x = phi i32 [ %spec.select, %true ], [ %x1, %false ]
