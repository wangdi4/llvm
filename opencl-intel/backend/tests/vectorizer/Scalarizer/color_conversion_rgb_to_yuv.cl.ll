...
@opencl_metadata = appending global [2 x %opencl_metadata_type] [%opencl_metadata_type <{ i8* bitcast (void (<4 x float> addrspace(1)*, <4 x float> addrspace(1)*)* @color_conversion_rgb_to_yuv_GPU to i8*), i8* null, [4 x i32] zeroinitializer, [4 x i32] zeroinitializer, i8* bitcast ([1 x i8*]* @opencl_color_conversion_rgb_to_yuv_GPU_locals to i8*), i8* getelementptr inbounds ([87 x i8], [87 x i8]* @opencl_color_conversion_rgb_to_yuv_GPU_parameters, i32 0, i32 0) }>, %opencl_metadata_type <{ i8* bitcast (void (<4 x float> addrspace(1)*, <4 x float> addrspace(1)*, i32)* @color_conversion_rgb_to_yuv to i8*), i8* null, [4 x i32] zeroinitializer, [4 x i32] zeroinitializer, i8* bitcast ([1 x i8*]* @opencl_color_conversion_rgb_to_yuv_locals to i8*), i8* getelementptr inbounds ([99 x i8], [99 x i8]* @opencl_color_conversion_rgb_to_yuv_parameters, i32 0, i32 0) }>], section "llvm.metadata" ; <[2 x %opencl_metadata_type]*> [#uses=0]
...
entry:
  ...
  %tmp = load <4 x float>, <4 x float>* %pix.addr              ; <<4 x float>> [#uses=1]
  %tmp1 = extractelement <4 x float> %tmp, i32 0  ; <float> [#uses=1]
  %mul = fmul float 0x3FD322D0E0000000, %tmp1     ; <float> [#uses=1]
  %tmp2 = load <4 x float>, <4 x float>* %pix.addr             ; <<4 x float>> [#uses=1]
  ...
  %tmp5 = load <4 x float>, <4 x float>* %pix.addr             ; <<4 x float>> [#uses=1]
  ...
  %tmp11 = load <4 x float>, <4 x float>* %pix.addr            ; <<4 x float>> [#uses=1]
  %tmp12 = extractelement <4 x float> %tmp11, i32 2 ; <float> [#uses=1]
  %tmp13 = load <4 x float>, <4 x float>* %output              ; <<4 x float>> [#uses=1]
  ...
  %tmp16 = load <4 x float>, <4 x float>* %output              ; <<4 x float>> [#uses=1]
  %tmp17 = insertelement <4 x float> %tmp16, float %mul15, i32 1 ; <<4 x float>> [#uses=1]
  store <4 x float> %tmp17, <4 x float>* %output
  %tmp18 = load <4 x float>, <4 x float>* %pix.addr            ; <<4 x float>> [#uses=1]
  %tmp19 = extractelement <4 x float> %tmp18, i32 0 ; <float> [#uses=1]
  %tmp20 = load <4 x float>, <4 x float>* %output              ; <<4 x float>> [#uses=1]
...
entry:
  ...
  %tmp4 = load i32, i32* %gid0_curPix                  ; <i32> [#uses=1]
