...
define void @encountered_not_scal_variable(<2 x float> addrspace(1)* nocapture %in, float addrspace(1)* nocapture %out) nounwind {
  ...
  %3 = load <2 x float>, <2 x float> addrspace(1)* %2          ; <<2 x float>> [#uses=2]
