...
@opencl_metadata = appending global [1 x %opencl_metadata_type] [%opencl_metadata_type <{ i8* bitcast (void (float addrspace(1)*, <2 x float> addrspace(1)*)* @fmul to i8*), i8* null, [4 x i32] zeroinitializer, [4 x i32] zeroinitializer, i8* bitcast ([1 x i8*]* @opencl_fmul_locals to i8*), i8* getelementptr inbounds ([92 x i8], [92 x i8]* @opencl_fmul_parameters, i32 0, i32 0) }>], section "llvm.metadata" ; <[1 x %opencl_metadata_type]*> [#uses=0]
...
define void @fmul(float addrspace(1)* nocapture %in, <2 x float> addrspace(1)* nocapture %out) nounwind {
  ...
  %9 = load float, float addrspace(1)* %8                ; <float> [#uses=2]
