...
define void @func_abs_diff(double addrspace(1)* nocapture %in, <4 x double> addrspace(1)* nocapture %out) nounwind {
  ...
  %2 = getelementptr inbounds double, double addrspace(1)* %in, i32 1 ; <double addrspace(1)*> [#uses=1]
  %3 = load double, double addrspace(1)* %2               ; <double> [#uses=1]
  %4 = insertelement <4 x double> undef, double %3, i32 0 ; <<4 x double>> [#uses=1]
  %5 = getelementptr inbounds double, double addrspace(1)* %in, i32 2 ; <double addrspace(1)*> [#uses=1]
  %6 = load double, double addrspace(1)* %5               ; <double> [#uses=1]
  %7 = insertelement <4 x double> %4, double %6, i32 1 ; <<4 x double>> [#uses=1]
  %8 = getelementptr inbounds double, double addrspace(1)* %in, i32 3 ; <double addrspace(1)*> [#uses=1]
  %9 = load double, double addrspace(1)* %8               ; <double> [#uses=1]
  %10 = insertelement <4 x double> %7, double %9, i32 2 ; <<4 x double>> [#uses=1]
  %11 = getelementptr inbounds double, double addrspace(1)* %in, i32 4 ; <double addrspace(1)*> [#uses=1]
  %12 = load double, double addrspace(1)* %11             ; <double> [#uses=1]
  %13 = insertelement <4 x double> %10, double %12, i32 3 ; <<4 x double>> [#uses=1]
  %14 = getelementptr inbounds double, double addrspace(1)* %in, i32 %1 ; <double addrspace(1)*> [#uses=1]
  %15 = load double, double addrspace(1)* %14             ; <double> [#uses=2]
  %16 = insertelement <4 x double> undef, double %15, i32 0 ; <<4 x double>> [#uses=1]
  %17 = add nsw i32 %1, 2                         ; <i32> [#uses=1]
  %18 = getelementptr inbounds double, double addrspace(1)* %in, i32 %17 ; <double addrspace(1)*> [#uses=1]
  %19 = load double, double addrspace(1)* %18             ; <double> [#uses=1]
  ...
  %25 = getelementptr inbounds <4 x double>, <4 x double> addrspace(1)* %out, i32 %1 ; <<4 x double> addrspace(1)*> [#uses=1]
