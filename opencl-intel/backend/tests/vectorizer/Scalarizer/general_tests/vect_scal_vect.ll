...
@opencl_metadata = appending global [1 x %opencl_metadata_type] [%opencl_metadata_type <{ i8* bitcast (void (float addrspace(1)*, <4 x float> addrspace(1)*)* @func_abs_diff to i8*), i8* null, [4 x i32] zeroinitializer, [4 x i32] zeroinitializer, i8* bitcast ([1 x i8*]* @opencl_func_abs_diff_locals to i8*), i8* getelementptr inbounds ([92 x i8], [92 x i8]* @opencl_func_abs_diff_parameters, i32 0, i32 0) }>], section "llvm.metadata" ; <[1 x %opencl_metadata_type]*> [#uses=0]
...
define void @func_abs_diff(float addrspace(1)* nocapture %in, <4 x float> addrspace(1)* nocapture %out) nounwind {
  ...
  %5 = getelementptr inbounds float, float addrspace(1)* %in, i32 2 ; <float addrspace(1)*> [#uses=1]
  %6 = load float, float addrspace(1)* %5                ; <float> [#uses=1]
  %7 = insertelement <4 x float> %4, float %6, i32 1 ; <<4 x float>> [#uses=1]
  %8 = getelementptr inbounds float, float addrspace(1)* %in, i32 3 ; <float addrspace(1)*> [#uses=1]
  %9 = load float, float addrspace(1)* %8                ; <float> [#uses=1]
  %10 = insertelement <4 x float> %7, float %9, i32 2 ; <<4 x float>> [#uses=1]
  %11 = getelementptr inbounds float, float addrspace(1)* %in, i32 4 ; <float addrspace(1)*> [#uses=1]
  %12 = load float, float addrspace(1)* %11              ; <float> [#uses=1]
  %13 = insertelement <4 x float> %10, float %12, i32 3 ; <<4 x float>> [#uses=1]
  %14 = getelementptr inbounds <4 x float>, <4 x float> addrspace(1)* %out, i32 %1 ; <<4 x float> addrspace(1)*> [#uses=1]
