...
; This test checks that stride is computed from operands of FAdd.
...
entryvector_func:                                 ; preds = %entryvector_func, %dim_0_vector_pre_head
  ...
  %6 = fadd fast <8 x float> %5, <float -1.000000e+00, float -1.000000e+00, float -1.000000e+00, float -1.000000e+00, float -1.000000e+00, float -1.000000e+00, float -1.000000e+00, float -1.000000e+00>

; CHECK:      [[FADD:%[0-9]+]] = fadd fast <8 x float> %{{[0-9]+}}, <float -1.000000e+00,
; CHECK-NEXT: %extract.0 = extractelement <8 x float> [[FADD]], i32 0
; CHECK-NEXT: %extract.1 = extractelement <8 x float> [[FADD]], i32 1
; CHECK-NEXT: %sub.delta = fsub float %extract.1, %extract.0
; CHECK-NEXT: %mul.delta = fmul float %sub.delta, 8.000000e+00

  %scalar.gepvector_func = getelementptr inbounds float, float addrspace(1)* %kb, i64 %.extract.0.vector_func
  %7 = bitcast float addrspace(1)* %scalar.gepvector_func to <8 x float> addrspace(1)*
  store <8 x float> %6, <8 x float> addrspace(1)* %7, align 4
