...
entry:
	...
	%tmp33 = load i32, i32* %threadId		; <i32> [#uses=1]
	%tmp34 = load i32, i32* %sameDirectionBlockWidth		; <i32> [#uses=2]
...
if.then:		; preds = %entry
	%tmp40 = load i32, i32* %sortIncreasing		; <i32> [#uses=1]
	%sub41 = sub i32 1, %tmp40		; <i32> [#uses=1]
	store i32 %sub41, i32* %sortIncreasing
	br label %if.end

if.end:		; preds = %if.then, %entry
	%tmp44 = load i32, i32* %leftElement		; <i32> [#uses=1]
	%tmp45 = load i32, i32* %rightElement		; <i32> [#uses=1]
	%cmp46 = icmp ugt i32 %tmp44, %tmp45		; <i1> [#uses=1]
	br i1 %cmp46, label %if.then47, label %if.else

if.then47:		; preds = %if.end
	%tmp48 = load i32, i32* %leftElement		; <i32> [#uses=1]
	store i32 %tmp48, i32* %greater
	%tmp49 = load i32, i32* %rightElement		; <i32> [#uses=1]
	store i32 %tmp49, i32* %lesser
	br label %if.end52

if.else:		; preds = %if.end
	%tmp50 = load i32, i32* %rightElement		; <i32> [#uses=1]
	store i32 %tmp50, i32* %greater
	%tmp51 = load i32, i32* %leftElement		; <i32> [#uses=1]
	store i32 %tmp51, i32* %lesser
	br label %if.end52

if.end52:		; preds = %if.else, %if.then47
	%tmp53 = load i32, i32* %sortIncreasing		; <i32> [#uses=1]
