...
for.body:		; preds = %for.cond
	...
	%arrayidx = getelementptr i16, i16 addrspace(3)* %tmp5, i32 %add		; <i16 addrspace(3)*> [#uses=1]
...
for.body12:		; preds = %for.cond9
	...
	%arrayidx19 = getelementptr i32, i32 addrspace(1)* %tmp18, i32 %add17		; <i32 addrspace(1)*> [#uses=1]
	...
	%tmp24 = load i32, i32* %localId		; <i32> [#uses=1]
	%mul25 = mul i32 %tmp24, 256		; <i32> [#uses=1]
	%tmp26 = load i32, i32* %value		; <i32> [#uses=1]
	%add27 = add i32 %mul25, %tmp26		; <i32> [#uses=1]
	%tmp28 = load i16 addrspace(3)*, i16 addrspace(3)** %sharedArray.addr		; <i16 addrspace(3)*> [#uses=1]
	%arrayidx29 = getelementptr i16, i16 addrspace(3)* %tmp28, i32 %add27		; <i16 addrspace(3)*> [#uses=2]
	%tmp30 = load i16, i16 addrspace(3)* %arrayidx29		; <i16> [#uses=1]
	%inc31 = add i16 %tmp30, 1		; <i16> [#uses=1]
	store i16 %inc31, i16 addrspace(3)* %arrayidx29
	br label %for.inc32

for.inc32:		; preds = %for.body12
	%tmp33 = load i32, i32* %i8		; <i32> [#uses=1]
...
for.cond38:		; preds = %for.inc61, %for.end35
	%tmp39 = load i32, i32* %i37		; <i32> [#uses=1]
	%cmp40 = icmp slt i32 %tmp39, 256		; <i1> [#uses=1]
	br i1 %cmp40, label %for.body41, label %for.end64

for.body41:		; preds = %for.cond38
	%tmp43 = load i32, i32* %groupId		; <i32> [#uses=1]
	%mul44 = mul i32 %tmp43, 256		; <i32> [#uses=1]
	%mul45 = mul i32 %mul44, 16		; <i32> [#uses=1]
	%tmp46 = load i32, i32* %localId		; <i32> [#uses=1]
	%mul47 = mul i32 %tmp46, 256		; <i32> [#uses=1]
	%add48 = add i32 %mul45, %mul47		; <i32> [#uses=1]
	%tmp49 = load i32, i32* %i37		; <i32> [#uses=1]
	%add50 = add i32 %add48, %tmp49		; <i32> [#uses=1]
	store i32 %add50, i32* %bucketPos
	%tmp51 = load i32, i32* %bucketPos		; <i32> [#uses=1]
	%tmp52 = load i32 addrspace(1)*, i32 addrspace(1)** %buckets.addr		; <i32 addrspace(1)*> [#uses=1]
	%arrayidx53 = getelementptr i32, i32 addrspace(1)* %tmp52, i32 %tmp51		; <i32 addrspace(1)*> [#uses=1]
	%tmp54 = load i32, i32* %localId		; <i32> [#uses=1]
	%mul55 = mul i32 %tmp54, 256		; <i32> [#uses=1]
	%tmp56 = load i32, i32* %i37		; <i32> [#uses=1]
	%add57 = add i32 %mul55, %tmp56		; <i32> [#uses=1]
	%tmp58 = load i16 addrspace(3)*, i16 addrspace(3)** %sharedArray.addr		; <i16 addrspace(3)*> [#uses=1]
	%arrayidx59 = getelementptr i16, i16 addrspace(3)* %tmp58, i32 %add57		; <i16 addrspace(3)*> [#uses=1]
	%tmp60 = load i16, i16 addrspace(3)* %arrayidx59		; <i16> [#uses=1]
	%conv = zext i16 %tmp60 to i32		; <i32> [#uses=1]
	store i32 %conv, i32 addrspace(1)* %arrayidx53
	br label %for.inc61

for.inc61:		; preds = %for.body41
	%tmp62 = load i32, i32* %i37		; <i32> [#uses=1]
...
for.body:		; preds = %for.cond
	...
	%tmp10 = load i32, i32* %localId		; <i32> [#uses=1]
	%mul11 = mul i32 %tmp10, 256		; <i32> [#uses=1]
	%tmp12 = load i32, i32* %i		; <i32> [#uses=1]
	%add13 = add i32 %mul11, %tmp12		; <i32> [#uses=1]
	%tmp14 = load i16 addrspace(3)*, i16 addrspace(3)** %sharedBuckets.addr		; <i16 addrspace(3)*> [#uses=1]
	%arrayidx = getelementptr i16, i16 addrspace(3)* %tmp14, i32 %add13		; <i16 addrspace(3)*> [#uses=1]
	%tmp15 = load i32, i32* %bucketPos		; <i32> [#uses=1]
	%tmp16 = load i32 addrspace(1)*, i32 addrspace(1)** %prescanedBuckets.addr		; <i32 addrspace(1)*> [#uses=1]
	%arrayidx17 = getelementptr i32, i32 addrspace(1)* %tmp16, i32 %tmp15		; <i32 addrspace(1)*> [#uses=1]
	%tmp18 = load i32, i32 addrspace(1)* %arrayidx17		; <i32> [#uses=1]
...
for.body26:		; preds = %for.cond22
	...
	%arrayidx33 = getelementptr i32, i32 addrspace(1)* %tmp32, i32 %add31		; <i32 addrspace(1)*> [#uses=1]
	...
	%tmp39 = load i32, i32* %localId		; <i32> [#uses=1]
	%mul40 = mul i32 %tmp39, 256		; <i32> [#uses=1]
	%tmp41 = load i32, i32* %value		; <i32> [#uses=1]
	%add42 = add i32 %mul40, %tmp41		; <i32> [#uses=1]
	%tmp43 = load i16 addrspace(3)*, i16 addrspace(3)** %sharedBuckets.addr		; <i16 addrspace(3)*> [#uses=1]
	%arrayidx44 = getelementptr i16, i16 addrspace(3)* %tmp43, i32 %add42		; <i16 addrspace(3)*> [#uses=1]
	%tmp45 = load i16, i16 addrspace(3)* %arrayidx44		; <i16> [#uses=1]
	%conv46 = zext i16 %tmp45 to i32		; <i32> [#uses=1]
	store i32 %conv46, i32* %index
	%tmp47 = load i32, i32* %index		; <i32> [#uses=1]
	%tmp48 = load i32 addrspace(1)*, i32 addrspace(1)** %sortedData.addr		; <i32 addrspace(1)*> [#uses=1]
	%arrayidx49 = getelementptr i32, i32 addrspace(1)* %tmp48, i32 %tmp47		; <i32 addrspace(1)*> [#uses=1]
	%tmp50 = load i32, i32* %globalId		; <i32> [#uses=1]
	%mul51 = mul i32 %tmp50, 256		; <i32> [#uses=1]
	%tmp52 = load i32, i32* %i21		; <i32> [#uses=1]
	%add53 = add i32 %mul51, %tmp52		; <i32> [#uses=1]
	%tmp54 = load i32 addrspace(1)*, i32 addrspace(1)** %unsortedData.addr		; <i32 addrspace(1)*> [#uses=1]
	%arrayidx55 = getelementptr i32, i32 addrspace(1)* %tmp54, i32 %add53		; <i32 addrspace(1)*> [#uses=1]
	%tmp56 = load i32, i32 addrspace(1)* %arrayidx55		; <i32> [#uses=1]
	store i32 %tmp56, i32 addrspace(1)* %arrayidx49
	%tmp57 = load i32, i32* %localId		; <i32> [#uses=1]
	%mul58 = mul i32 %tmp57, 256		; <i32> [#uses=1]
	%tmp59 = load i32, i32* %value		; <i32> [#uses=1]
	%add60 = add i32 %mul58, %tmp59		; <i32> [#uses=1]
	%tmp61 = load i16 addrspace(3)*, i16 addrspace(3)** %sharedBuckets.addr		; <i16 addrspace(3)*> [#uses=1]
	%arrayidx62 = getelementptr i16, i16 addrspace(3)* %tmp61, i32 %add60		; <i16 addrspace(3)*> [#uses=1]
	%tmp63 = load i32, i32* %index		; <i32> [#uses=1]
...
for.inc66:		; preds = %for.body26
	%tmp67 = load i32, i32* %i21		; <i32> [#uses=1]
