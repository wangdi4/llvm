...
entry:
  ...
  %tmp = load <2 x float>, <2 x float>* %.compoundliteral      ; <<2 x float>> [#uses=1]
  store <2 x float> %tmp, <2 x float>* %pixelSize
  %tmp2 = load float, float* %twirlAngle.addr            ; <float> [#uses=1]
  %call = call float @_Z7radiansf(float %tmp2)    ; <float> [#uses=1]
  store float %call, float* %twirlAngleRadians
  %tmp4 = load float, float* %radius.addr                ; <float> [#uses=1]
  ...
  %tmp6 = load <2 x float>, <2 x float>* %pixelSize            ; <<2 x float>> [#uses=1]
  ...
  %tmp9 = load <2 x float>, <2 x float>* %outCrd.addr          ; <<2 x float>> [#uses=1]
  %tmp10 = load <2 x float>, <2 x float>* %center.addr         ; <<2 x float>> [#uses=1]
  %sub = fsub <2 x float> %tmp9, %tmp10           ; <<2 x float>> [#uses=1]
  store <2 x float> %sub, <2 x float>* %relativePos
  %tmp12 = load <2 x float>, <2 x float>* %relativePos         ; <<2 x float>> [#uses=1]
  %call13 = call float @_Z6lengthU8__vector2f(<2 x float> %tmp12) ; <float> [#uses=1]
  store float %call13, float* %distFromCenter
  %tmp14 = load float, float* %adjustedRadius            ; <float> [#uses=3]
  %tmp15 = load float, float* %distFromCenter            ; <float> [#uses=1]
  ...
  %tmp18 = load float, float* %distFromCenter            ; <float> [#uses=1]
  %call19 = call float @_Z3sinf(float %tmp18)     ; <float> [#uses=1]
  %tmp20 = load float, float* %twirlAngleRadians         ; <float> [#uses=1]
  %mul21 = fmul float %call19, %tmp20             ; <float> [#uses=1]
  %tmp22 = load float, float* %distFromCenter            ; <float> [#uses=3]
  ...
  %tmp60 = load float, float* %sinAngle                  ; <float> [#uses=1]
  %tmp61 = load <2 x float>, <2 x float>* %relativePos         ; <<2 x float>> [#uses=1]
  ...
  %tmp65 = load <2 x float>, <2 x float>* %relativePos         ; <<2 x float>> [#uses=1]
  ...
  %tmp68 = load float, float* %cosx_minus_siny           ; <float> [#uses=1]
  %tmp69 = load <2 x float>, <2 x float>* %relativePos         ; <<2 x float>> [#uses=1]
  %tmp70 = insertelement <2 x float> %tmp69, float %tmp68, i32 0 ; <<2 x float>> [#uses=1]
  store <2 x float> %tmp70, <2 x float>* %relativePos
  %tmp71 = load float, float* %sinx_plus_cosy            ; <float> [#uses=1]
  %tmp72 = load <2 x float>, <2 x float>* %relativePos         ; <<2 x float>> [#uses=1]
  ...
  %tmp76 = load %struct._image2d_t*, %struct._image2d_t** %inputImage.addr ; <%struct._image2d_t*> [#uses=1]
  %tmp77 = load <2 x float>, <2 x float>* %relativePos         ; <<2 x float>> [#uses=1]
  %tmp78 = load <2 x float>, <2 x float>* %center.addr         ; <<2 x float>> [#uses=1]
  ...
  %tmp81 = load <4 x float>, <4 x float>* %outputColor         ; <<4 x float>> [#uses=1]
...
entry:
  ...
  %tmp4 = load <2 x i32>, <2 x i32>* %dstSize.addr           ; <<2 x i32>> [#uses=1]
  %tmp5 = load <2 x i32>, <2 x i32>* %imgSize                ; <<2 x i32>> [#uses=1]
  ...
  %tmp7 = load <2 x i32>, <2 x i32>* %dstSize.addr           ; <<2 x i32>> [#uses=1]
  %tmp8 = extractelement <2 x i32> %tmp7, i32 0   ; <i32> [#uses=1]
  %cmp = icmp slt i32 %tmp6, %tmp8                ; <i1> [#uses=1]
  br i1 %cmp, label %land.lhs.true, label %if.end

land.lhs.true:                                    ; preds = %entry
  %tmp9 = load i32, i32* %gid1_row                     ; <i32> [#uses=1]
  %tmp10 = load <2 x i32>, <2 x i32>* %dstSize.addr          ; <<2 x i32>> [#uses=1]
  %tmp11 = extractelement <2 x i32> %tmp10, i32 1 ; <i32> [#uses=1]
  %cmp12 = icmp slt i32 %tmp9, %tmp11             ; <i1> [#uses=1]
  br i1 %cmp12, label %if.then, label %if.end

if.then:                                          ; preds = %land.lhs.true
  %tmp14 = load i32, i32* %gid1_row                    ; <i32> [#uses=1]
  %tmp15 = load <2 x i32>, <2 x i32>* %dstSize.addr          ; <<2 x i32>> [#uses=1]
  %tmp16 = extractelement <2 x i32> %tmp15, i32 0 ; <i32> [#uses=1]
  %mul = mul i32 %tmp14, %tmp16                   ; <i32> [#uses=1]
  %tmp17 = load i32, i32* %gid0_col                    ; <i32> [#uses=1]
  %add = add nsw i32 %mul, %tmp17                 ; <i32> [#uses=1]
  store i32 %add, i32* %index
  %tmp19 = load i32, i32* %gid0_col                    ; <i32> [#uses=1]
  %vecinit = insertelement <2 x i32> undef, i32 %tmp19, i32 0 ; <<2 x i32>> [#uses=1]
  %tmp20 = load i32, i32* %gid1_row                    ; <i32> [#uses=1]
  %vecinit21 = insertelement <2 x i32> %vecinit, i32 %tmp20, i32 1 ; <<2 x i32>> [#uses=1]
  store <2 x i32> %vecinit21, <2 x i32>* %curCrd
  %tmp22 = load %struct._image2d_t*, %struct._image2d_t** %inputImage.addr ; <%struct._image2d_t*> [#uses=1]
  %tmp23 = load <2 x i32>, <2 x i32>* %curCrd                ; <<2 x i32>> [#uses=1]
  %tmp24 = load <2 x i32>, <2 x i32>* %offset                ; <<2 x i32>> [#uses=1]
  ...
  %tmp27 = load float, float* %radius.addr               ; <float> [#uses=1]
  %tmp28 = load <2 x float>, <2 x float>* %center.addr         ; <<2 x float>> [#uses=1]
