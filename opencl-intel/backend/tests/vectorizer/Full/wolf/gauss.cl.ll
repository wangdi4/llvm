...
entry:
  ...
  %tmp = load float, float* @centerIndex                 ; <float> [#uses=1]
  %mul = fmul float -1.000000e+000, %tmp          ; <float> [#uses=1]
  store float %mul, float* %floatIndex
  %tmp2 = load <2 x float>, <2 x float>* %outCrd.addr          ; <<2 x float>> [#uses=1]
...
for.body:                                         ; preds = %for.cond
  %tmp8 = load <2 x float>, <2 x float> addrspace(1)* @blurDirection ; <<2 x float>> [#uses=1]
  %tmp9 = load float, float* %floatIndex                 ; <float> [#uses=1]
  ...
  %tmp14 = load <2 x float>, <2 x float>* %currentPixelLoc     ; <<2 x float>> [#uses=1]
  %tmp15 = load <2 x float>, <2 x float>* %sourceLoc           ; <<2 x float>> [#uses=1]
  %add = fadd <2 x float> %tmp14, %tmp15          ; <<2 x float>> [#uses=1]
  store <2 x float> %add, <2 x float>* %ReadPixelLoc
  %tmp17 = load %struct._image2d_t*, %struct._image2d_t** %inputImage.addr ; <%struct._image2d_t*> [#uses=1]
  %tmp18 = load <2 x float>, <2 x float>* %ReadPixelLoc        ; <<2 x float>> [#uses=1]
  ...
  %tmp24 = load <4 x float>, <4 x float>* %temp                ; <<4 x float>> [#uses=1]
  %mul25 = fmul <4 x float> %splat23, %tmp24      ; <<4 x float>> [#uses=1]
  %tmp26 = load <4 x float>, <4 x float>* %colAccum            ; <<4 x float>> [#uses=1]
...
for.end:                                          ; preds = %for.cond
  ...
  %tmp39 = load <4 x float>, <4 x float>* %colAccum            ; <<4 x float>> [#uses=1]
  ...
  %tmp41 = load <4 x float>, <4 x float>* %colAccum            ; <<4 x float>> [#uses=1]
...
entry:
  ...
  %tmp2 = load float, float* %tmp                        ; <float> [#uses=1]
  ...
  %tmp5 = load float, float* %location                   ; <float> [#uses=1]
  %tmp6 = load float, float* %radius.addr                ; <float> [#uses=1]
  %add = fadd float %tmp6, 1.000000e+000          ; <float> [#uses=1]
  %mul = fmul float %tmp5, %add                   ; <float> [#uses=1]
  store float %mul, float* %size
  %tmp7 = load float, float* %size                       ; <float> [#uses=1]
  %tmp8 = load float addrspace(1)*, float addrspace(1)** %wsize.addr   ; <float addrspace(1)*> [#uses=1]
...
entry:
  ...
  %tmp = load i32, i32* %direction_.addr               ; <i32> [#uses=1]
  store i32 %tmp, i32 addrspace(1)* @direction
  %tmp1 = load float, float* %radius_.addr               ; <float> [#uses=1]
...
for.body:                                         ; preds = %for.cond
  ...
  %tmp6 = load float, float* @centerIndex                ; <float> [#uses=1]
  %sub = fsub float %conv, %tmp6                  ; <float> [#uses=1]
  store float %sub, float* %val
  %tmp7 = load float, float* %val                        ; <float> [#uses=1]
  %cmp8 = fcmp olt float %tmp7, 0.000000e+000     ; <i1> [#uses=1]
  br i1 %cmp8, label %if.then, label %if.end

if.then:                                          ; preds = %for.body
  %tmp10 = load float, float* %val                       ; <float> [#uses=1]
  %sub11 = fsub float 0.000000e+000, %tmp10       ; <float> [#uses=1]
  store float %sub11, float* %val
  br label %if.end

if.end:                                           ; preds = %if.then, %for.body
  %tmp13 = load float, float* %val                       ; <float> [#uses=1]
  %tmp14 = load float, float addrspace(1)* @radius       ; <float> [#uses=1]
...
for.end:                                          ; preds = %for.cond
  %tmp35 = load i32, i32 addrspace(1)* @direction      ; <i32> [#uses=1]
...
if.then38:                                        ; preds = %for.end
  ...
  %tmp39 = load <2 x float>, <2 x float>* %.compoundliteral    ; <<2 x float>> [#uses=1]
...
if.else:                                          ; preds = %for.end
  ...
  %tmp41 = load <2 x float>, <2 x float>* %.compoundliteral40  ; <<2 x float>> [#uses=1]
...
entry:
  ...
  %tmp7 = load i32, i32* %row                          ; <i32> [#uses=1]
  %tmp8 = load i32, i32* %rowCountPerGlobalID.addr     ; <i32> [#uses=1]
...
for.cond:                                         ; preds = %for.inc42, %entry
  %tmp17 = load i32, i32* %row                         ; <i32> [#uses=1]
  %tmp18 = load i32, i32* %lastRow                     ; <i32> [#uses=1]
  %cmp = icmp ult i32 %tmp17, %tmp18              ; <i1> [#uses=1]
  br i1 %cmp, label %for.body, label %for.end45

for.body:                                         ; preds = %for.cond
  %tmp20 = load i32, i32* %row                         ; <i32> [#uses=1]
  %conv = uitofp i32 %tmp20 to float              ; <float> [#uses=1]
  %tmp21 = load <2 x float>, <2 x float>* %curCrd              ; <<2 x float>> [#uses=1]
...
for.cond24:                                       ; preds = %for.inc, %for.body
  ...
  %tmp26 = load <2 x i32>, <2 x i32>* %imgSize               ; <<2 x i32>> [#uses=1]
...
for.body30:                                       ; preds = %for.cond24
  %tmp31 = load i32, i32* %col                         ; <i32> [#uses=1]
  %conv32 = uitofp i32 %tmp31 to float            ; <float> [#uses=1]
  %tmp33 = load <2 x float>, <2 x float>* %curCrd              ; <<2 x float>> [#uses=1]
  %tmp34 = insertelement <2 x float> %tmp33, float %conv32, i32 0 ; <<2 x float>> [#uses=1]
  store <2 x float> %tmp34, <2 x float>* %curCrd
  %tmp35 = load %struct._image2d_t*, %struct._image2d_t** %inputImage.addr ; <%struct._image2d_t*> [#uses=1]
  %tmp36 = load <2 x float>, <2 x float>* %curCrd              ; <<2 x float>> [#uses=1]
  %call37 = call <4 x float> @evaluatePixel(%struct._image2d_t* %tmp35, <2 x float> %tmp36) ; <<4 x float>> [#uses=1]
  %tmp38 = load i32, i32* %index                       ; <i32> [#uses=2]
  %inc = add i32 %tmp38, 1                        ; <i32> [#uses=1]
  store i32 %inc, i32* %index
  %tmp39 = load <4 x float> addrspace(1)*, <4 x float> addrspace(1)** %output.addr ; <<4 x float> addrspace(1)*> [#uses=1]
  %arrayidx = getelementptr inbounds <4 x float>, <4 x float> addrspace(1)* %tmp39, i32 %tmp38 ; <<4 x float> addrspace(1)*> [#uses=1]
  store <4 x float> %call37, <4 x float> addrspace(1)* %arrayidx
  br label %for.inc

for.inc:                                          ; preds = %for.body30
  %tmp40 = load i32, i32* %col                         ; <i32> [#uses=1]
