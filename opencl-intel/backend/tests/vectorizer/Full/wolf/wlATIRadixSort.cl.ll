; RUN: %oclopt -runtimelib %p/../runtime.bc -inline-threshold=4096 -inline -domtree -mem2reg -instcombine -simplifycfg -lowerswitch -loop-simplify -scalarize -mergereturn -loop-simplify -phicanon -predicate -mem2reg -dce -packetize -packet-size=4 -resolve -verify %s -S -o %t1.ll
...
@opencl_metadata = appending global [2 x %opencl_metadata_type] [%opencl_metadata_type <{ i8* bitcast (void (i32 addrspace(1)*, i32 addrspace(1)*, i32, i16 addrspace(3)*)* @histogram to i8*), i8* null, [4 x i32] zeroinitializer, [4 x i32] zeroinitializer, i8* bitcast ([1 x i8*]* @opencl_histogram_locals to i8*), i8* getelementptr inbounds ([139 x i8], [139 x i8]* @opencl_histogram_parameters, i32 0, i32 0) }>, %opencl_metadata_type <{ i8* bitcast (void (i32 addrspace(1)*, i32 addrspace(1)*, i32, i16 addrspace(3)*, i32 addrspace(1)*)* @permute to i8*), i8* null, [4 x i32] zeroinitializer, [4 x i32] zeroinitializer, i8* bitcast ([1 x i8*]* @opencl_permute_locals to i8*), i8* getelementptr inbounds ([187 x i8], [187 x i8]* @opencl_permute_parameters, i32 0, i32 0) }>], section "llvm.metadata" ; <[2 x %opencl_metadata_type]*> [#uses=0]
...
for.body:                                         ; preds = %for.cond
  %tmp3 = load i32, i32* %localId                      ; <i32> [#uses=1]
...
for.body12:                                       ; preds = %for.cond9
  ...
  %tmp23 = load i32, i32* %localId                     ; <i32> [#uses=1]
...
for.inc31:                                        ; preds = %for.body12
  %tmp32 = load i32, i32* %i8                          ; <i32> [#uses=1]
...
for.inc60:                                        ; preds = %for.body40
  %tmp61 = load i32, i32* %i36                         ; <i32> [#uses=1]
...
for.body:                                         ; preds = %for.cond
  %tmp4 = load i32, i32* %groupId                      ; <i32> [#uses=1]
  %mul = mul i32 %tmp4, 256                       ; <i32> [#uses=1]
  %mul5 = mul i32 %mul, 16                        ; <i32> [#uses=1]
  %tmp6 = load i32, i32* %localId                      ; <i32> [#uses=1]
...
for.body26:                                       ; preds = %for.cond22
  ...
  %and = and i32 %shr, 255                        ; <i32> [#uses=1]
  store i32 %and, i32* %value
  %tmp38 = load i32, i32* %localId                     ; <i32> [#uses=1]
  ...
  %tmp59 = load i32, i32* %localId                     ; <i32> [#uses=1]
...
for.inc65:                                        ; preds = %for.body26
  %tmp66 = load i32, i32* %i21                         ; <i32> [#uses=1]
