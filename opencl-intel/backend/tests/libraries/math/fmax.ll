; RUN: llvm-extract %libdir/../clbltfnshared.rtl -rfunc '_Z4fmaxDv16_fS_' -S | FileCheck %s

; CHECK: define {{.*}}<16 x float> @_Z4fmaxDv16_fS_(<16 x float> {{.*}}[[A:%.*]], <16 x float> {{.*}}[[B:%.*]])
; CHECK-NEXT: entry:
; CHECK-NEXT: [[ISNaN:%.*]] = {{.*}} call <16 x i32> @_Z5isnanDv16_f(<16 x float> {{.*}}[[A]])
; CHECK-NEXT: [[CMP:%.*]] = fcmp contract olt <16 x float> [[A]], [[B]]
