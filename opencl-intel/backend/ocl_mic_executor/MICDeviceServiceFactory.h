...
File Name:  MICDeviceServiceFactory.h
...
class MICDeviceServiceFactory: public ICLDevBackendServiceFactory
{
private:
    MICDeviceServiceFactory();
    ~MICDeviceServiceFactory();
...
public:
     ...
     *
     * @param pBackendOptions pointer to class which will contain the backend compilation
     *  options (alloc\free program JIT handler address, etc ..)
