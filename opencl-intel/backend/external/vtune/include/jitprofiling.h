...
typedef enum iJIT_jvm_event {
  ...
  iJVM_EVENT_TYPE_METHOD_LOAD_FINISHED =
      13, /* issued after method code jitted
           * into memory but before code is executed
           * EventSpecificData is an iJIT_Method_Load
           */

  iJVM_EVENT_TYPE_METHOD_UNLOAD_START, /* issued before unload. Method code will
                                        * no longer be executed, but code and
                                        * info are still in memory. The VTune
                                        * profiler may capture method code only
                                        * at this point EventSpecificData is
                                        * iJIT_Method_Id
                                        */
  ...
  iJVM_EVENT_TYPE_ENTER_NIDS =
      19, /* issued when a method is about to be entered
...
typedef enum _iJIT_ModeFlags {
  ...
  iJIT_BE_NOTIFY_ON_UNLOAD =
      0x0002,                              /* when turned on the jit must call
                                            * iJIT_NotifyEvent
                                            * (
                                            *     iJVM_EVENT_TYPE_METHOD_UNLOAD_FINISHED,
                                            *  ) for all the method that are unloaded
                                            */
  iJIT_BE_NOTIFY_ON_METHOD_ENTRY = 0x0004, /* when turned on the jit must
                                            * instrument all the currently jited
                                            * code with calls on method entries
                                            */
  iJIT_BE_NOTIFY_ON_METHOD_EXIT = 0x0008   /* when turned on the jit must
                                            * instrument all   the currently jited
                                            * code with calls   on method exit
                                            */
} iJIT_ModeFlags;
...
typedef enum _iJIT_IsProfilingActiveFlags {
  iJIT_NOTHING_RUNNING =
      0x0000,                /* No profiler is running. Currently not used */
...
typedef struct _iJIT_Method_Load {
  ...
  unsigned int line_number_size; /* Line Table size in number of entries
                                  * - Zero if none
                                  */
  pLineNumberInfo line_number_table; /* Pointer to the begining of the line
                                      * numbers info array
                                      */
