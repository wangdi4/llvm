...
cl_err_code Intel::OpenCL::Utils::IsCPUSupported()
{
	if( CPUDetect::GetInstance()->IsGenuineIntel() && 
		CPUDetect::GetInstance()->IsProcessorType(PT_NEHALEM) )
	{
		return CL_SUCCESS;
	}
...
void CPUDetect::GetCPUInfo()
{
    ...
    *((int*)(vcCPUString+8)) = viCPUInfo[2];
    ...
    __cpuid(viCPUInfo, 0x80000000);
    int iValidExIDs = viCPUInfo[0];

    for (int i=0x80000000; i <= iValidExIDs; ++i)
    {
        __cpuid(viCPUInfo, i);
    ...
    m_szCPUBrandString = _strdup(vcCPUBrandString);
}
