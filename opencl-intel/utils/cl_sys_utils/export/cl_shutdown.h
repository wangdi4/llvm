...
public:
  ...
  // special support (HACK) to ensure we get atexit callback before not our
...
// Setting it to false as a workaround as context shutdown is not really
// executed on linux. So it does not enter real shutdown mode.
...
inline bool IsShuttingDown() {
  return (UseShutdownHandler::shutdown_mode > UseShutdownHandler::WORKING);
}

// call this function directly only if renaming is required (ex. when DLL is not
// loaded manually)
extern "C"
#ifdef _WIN32
...
#define USE_SHUTDOWN_HANDLER(local_dll_callback)                               \
  Intel::OpenCL::Utils::UseShutdownHandler use_shutdown_handler(               \
      local_dll_callback)
