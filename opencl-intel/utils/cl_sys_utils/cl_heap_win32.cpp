...
enum AllocApproach_t { USE_HEAP = 0, USE_VIRTUAL_ALLOC };
...
inline BOOL LargeFree(void *ptr) { return VirtualFree(ptr, 0, MEM_RELEASE); }
 ...
 * #pragma warning(disable : 4509) // MSVC warns about return after HeapValidate
 *because AutoMutex.
 */
...
int clCreateHeap(int node, size_t maxHeapSize, ClHeap *phHeap) {
  ...
  // To enable the LFH for the specified heap, set the variable pointed to by
  // the HeapInformation parameter to 2.
  ULONG heapSetInfo = 2;
  bool bResult = HeapSetInformation(hHeap, HeapCompatibilityInformation,
                                    &heapSetInfo, sizeof(ULONG));
  if (bResult != FALSE) {
    //_tprintf(TEXT("The low-fragmentation heap has been enabled.\n"));
  } else {
    char cBuf[256];
    SPRINTF_S(cBuf, 256,
              "'CPU Runtime': Failed to enable the low-fragmentation heap with "
              "LastError %d."
              " If running from debugger you need to set environment variable "
              "_NO_DEBUG_HEAP=1\n",
              GetLastError());
...
void *clAllocateFromHeap(ClHeap hHeap, size_t allocSize, size_t alignment,
  ...
  if (force_dedicated_pages ||
    ...
    assert((((size_t)user_ptr - (size_t)ptr) + allocSize) <=
           real_allocated_size);
  }
...
//
////////////////////////////////////////////////////////////////////
int clHeapMarkSafeForDMA(void *start, size_t size) { return 0; }

int clHeapUnmarkSafeForDMA(void *start, size_t size) { return 0; }
