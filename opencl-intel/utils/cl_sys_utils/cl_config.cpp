...
OPENCL_VERSION GetOpenclVerByCpuModel() { return OPENCL_VERSION_3_0; }
...
cl_ulong
BasicCLConfigWrapper::ParseStringToSize(const std::string &userStr) const {
  cl_ulong integer = 0;
...
ConfigFile::ConfigFile(const string &filename, string delimiter, string comment,
                       string sentry) {
...
cl_err_code ConfigFile::WriteFile(string fileName, ConfigFile &cf) {
  ...
  for (ConfigFile::mapci p = cf.m_mapContents.begin();
       p != cf.m_mapContents.end(); ++p) {
...
template <>
string
Intel::OpenCL::Utils::GetRegistryValue<string>(HKEY key, const string &valName,
                                               const string &defaultVal) {
  DWORD regValSize;
  LONG res = RegQueryValueExA(key, valName.c_str(), nullptr, nullptr, nullptr,
                              &regValSize);
  if (ERROR_SUCCESS != res) {
    return defaultVal;
  }
  std::vector<BYTE> str(regValSize);
  res = RegQueryValueExA(key, valName.c_str(), nullptr, nullptr, &str[0],
                         &regValSize);
...
OPENCL_VERSION BasicCLConfigWrapper::GetOpenCLVersion() const {
  ...
  string ver = m_pConfigFile->Read(
      "ForceOCLCPUVersion",
      string("")); // we are using this name to be aligned with GEN
  if ("1.2" == ver) {
...
#ifdef _WIN32
  DWORD iVer =
      m_pConfigFile->GetRegistryOrEtcValue<DWORD>("ForceOCLCPUVersion", 0);
#else
