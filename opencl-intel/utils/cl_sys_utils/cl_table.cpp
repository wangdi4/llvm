...
#define KEY_2_INDEX(key) (0x0000ffff & (key))

#define KEY_2_TABLE_ID(key) ((unsigned short)((0xffff0000 & (key)) >> 16))

#define INDEX_2_KEY(idx) ((m_usTableId << 16) | idx)
...
ClTable::ClTable(unsigned short usTableSize, float fLoadFactor)
    : m_usTableSize(usTableSize), m_usTableId(m_susTableCounter),
      m_usObjectIdx(0), m_fLoadFactor(fLoadFactor) {
  m_susTableCounter++;
