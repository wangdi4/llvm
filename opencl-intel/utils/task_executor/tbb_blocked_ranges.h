...
//
// Represents contiguous range if indices [left,right), including left and
// excluding right
//
class BlockedRange {
public:
  ...
  BlockedRange(BlockedRangeValueType left, BlockedRangeValueType right,
  ...
  BlockedRangeSizeType size() const {
    return BlockedRangeSizeType(m_max - m_min);
  };
  ...
  // make me the right side of the range, update other to be the left side of
  // the range
...
//
// Represents contiguous range if 2D indices [left,right), including left and
// excluding right, with priority for one of the for splitting
//
...
public:
  ...
  BlockedRange2d(const tbb::blocked_range<size_t> &tbb_coord_split_first,
      ...
      : m_coord_split_first(tbb_coord_split_first, 1),
        m_coord_split_second(tbb_coord_split_second, grain){};

  // if any is empty - the whole range is empty
  bool empty() const {
    return (m_coord_split_first.empty() || m_coord_split_second.empty());
  };
  bool is_divisible() const {
    return (m_coord_split_first.is_divisible() ||
            m_coord_split_second.is_divisible());
  };

  BlockedRange::BlockedRangeSizeType grainsize() const {
    return m_coord_split_second.grainsize();
  };

  // make me the right side of the range, update other to be the left side of
  // the range first try to split columns
...
//
// Represents contiguous range if 3D indices [left,right), including left and
// excluding right, with priority for one of the for splitting
//
class BlockedRange3d {
public:
  BlockedRange3d()
      : m_coord_split_first(), m_coord_split_second(), m_coord_split_third(){};
  ...
  bool empty() const {
    return (m_coord_split_first.empty() || m_coord_split_second.empty() ||
            m_coord_split_third.empty());
  };
  bool is_divisible() const {
    return (m_coord_split_first.is_divisible() ||
            m_coord_split_second.is_divisible() ||
            m_coord_split_third.is_divisible());
  };

  BlockedRange::BlockedRangeSizeType grainsize() const {
    return m_coord_split_third.grainsize();
  };
...
public:
  ...
  BlockedRangeByRow2d(const size_t dimsBegin[], const size_t dimsEnd[],
                      size_t grainsize)
      : BlockedRange2d(dimsBegin[1], dimsEnd[1], dimsBegin[0], dimsEnd[0],
                       grainsize) {}
...
public:
  ...
  // make me the right side of the range, update other to be the left side of
  // the range first try to split rows
...
public:
  ...
  bool is_divisible() const {
    return (m_rows.is_divisible() || m_cols.is_divisible());
  };
  ...
  BlockedRange::BlockedRangeSizeType grainsize() const {
    return m_cols.grainsize();
  };

  // make me the right side of the range, update other to be the left side of
  // the range first try to split cols
...
public:
  ...
  // if any is empty - the whole range is empty
  bool empty() const {
    return (m_pages.empty() || m_rows.empty() || m_cols.empty());
  };
  bool is_divisible() const {
    return (m_pages.is_divisible() || m_rows.is_divisible() ||
            m_cols.is_divisible());
  };
...
public:
  ...
  BlockedRangeByDefaultTBB2d(unsigned int min_rows, unsigned int max_rows,
                             unsigned int min_cols, unsigned int max_cols,
                             size_t grain = 1)
...
public:
  BlockedRangeByDefaultTBB3d()
      : tbb::blocked_range3d<size_t>(0, 0, 0, 0, 0, 0){};

  BlockedRangeByDefaultTBB3d(unsigned int min_pages, unsigned int max_pages,
                             unsigned int min_rows, unsigned int max_rows,
                             unsigned int min_cols, unsigned int max_cols,
                             size_t grain = 1)
