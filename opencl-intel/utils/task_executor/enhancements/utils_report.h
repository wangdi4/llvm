...
#define TRACENL                                                                \
  utils::internal::tracer.set_trace_info(0, TRACE_ORIG_INFO)->trace
...
public:
  ...
  Tracer *set_trace_info(int flags, const char *file, std::size_t line,
                         const char *func) {
    m_flags = flags;
    m_line = line;
    m_file = file;
  ...
  void trace(const char *fmt, ...) {
    ...
    va_end(argptr);
    if (m_flags & need_lf && len < MAX_TRACE_SIZE - 1 &&
        msg_fmt[len - 1] != '\n') {
      msg[len] = '\n';
      msg[len + 1] = 0;
    }
