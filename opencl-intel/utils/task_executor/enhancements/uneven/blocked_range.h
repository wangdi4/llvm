...
#ifndef __TBB_blocked_range_uneven_H
#define __TBB_blocked_range_uneven_H
...
public:
    ...
    blocked_range( blocked_range& r, split s ) : 
        my_end(r.my_end),
        my_begin(do_split(r, s.ratio())),
...
private:
    ...
    static Value do_split( blocked_range& r, float fr ) {
        __TBB_ASSERT( r.is_divisible(), "cannot split blocked_range that is not divisible" );
        Value split_pt = r.my_begin + size_type(r.size()*fr+0.5);
        r.my_end = split_pt;
        return split_pt;
    }
...
#endif /* __TBB_blocked_range_uneven_H */
