...
#ifndef __TBB_partitioner_uneven_H
#define __TBB_partitioner_uneven_H
...
class affinity_partitioner_base_v3: tbb::internal::no_copy {
    friend class tbb::uneven::affinity_partitioner;
    friend class tbb::uneven::interface6::internal::affinity_partition_type;
...
//! @cond INTERNAL
namespace internal {
using namespace tbb::uneven::internal;
template<typename Range, typename Body, typename Partitioner> class start_for;
...
struct partition_type_base {
    partition_type_base() {}
    partition_type_base(partition_type_base &src, split) {}
    // decision makers
    int portion_size() { return 1; }
    void set_affinity( task & ) {}
    void note_affinity( task::affinity_id ) {}
...
#if __USE_TSCG__
        tscg_task_start( execute );
        tscg_task_data( data, "%d + %d #%d", int(range.begin()), int(range.end()-range.begin()), derived().portion_size() );
#endif
...
struct auto_partition_type_base : partition_type_base<Partition> {
    ...
    auto_partition_type_base(auto_partition_type_base &src, split) : partition_type_base<Partition>(src) {
    ...
    int portion_size() { return my_divisor?my_divisor:1; }
...
public:
    affinity_partition_type( tbb::uneven::internal::affinity_partitioner_base_v3& ap ) {
    ...
    bool check_for_demand( task &t ) {
        if( !my_delay ) {
            if( map_mid<map_end ) {
                __TBB_ASSERT((long long)my_max_depth > __TBB_Log2(map_end-map_mid), 0);
...
class old_auto_partition_type: public tbb::uneven::internal::partition_type_base {
    size_t num_chunks;
...
public:
    ...
    old_auto_partition_type( const auto_partitioner& )
      : num_chunks(tbb::internal::get_initial_auto_partitioner_divisor()*__TBB_INITIAL_CHUNKS/4) {}
...
private:
    ...
    class partition_type: public uneven::internal::partition_type_base {
    public:
...
class affinity_partitioner: uneven::internal::affinity_partitioner_base_v3 {
public:
...
#endif /* __TBB_partitioner_uneven_H */
