...
#ifndef __TBB_parallel_for_uneven_H
#define __TBB_parallel_for_uneven_H
...
namespace internal {
    ...
    public:
        ...
        start_for( start_for& parent_, uneven::split ) :
            my_partition(parent_.my_partition, uneven::split()),
            my_range(parent_.my_range, uneven::split( parent_.my_partition.portion_size(), my_partition.portion_size() )),
...
#if __USE_TSCG__
        tscg_task_start( start_for );
        tscg_task_data( data, "%d + %d #%d", int(my_range.begin()), int(my_range.end()-my_range.begin()), my_partition.portion_size() );
#endif
        my_partition.check_being_stolen( *this );
        my_partition.execute(*this, my_range);
#if __USE_TSCG__
        tscg_task_stop( start_for, data );
#endif
        return NULL;
    } 
} // namespace internal
...
namespace internal {
    ...
    class parallel_for_body : tbb::internal::no_assign {
    ...
    public:
        ...
        void operator()( tbb::uneven::blocked_range<Index>& r ) const {
#if __INTEL_COMPILER
#pragma ivdep
#endif
...
void parallel_for( const Range& range, const Body& body ) {
    internal::start_for<Range,Body,const __TBB_DEFAULT_PARTITIONER()>::run(range,body, tbb::__TBB_DEFAULT_PARTITIONER());
}
#endif
...
void parallel_for_impl(Index first, Index last, Index step, const Function& f, Partitioner& partitioner) {
    if (step <= 0 )
        tbb::internal::throw_exception(tbb::internal::eid_nonpositive_step); // throws std::invalid_argument
    else if (last > first) {
        // Above "else" avoids "potential divide by zero" warning on some platforms
        Index end = (last - first - Index(1)) / step + Index(1);
        tbb::uneven::blocked_range<Index> range(static_cast<Index>(0), end);
        tbb::uneven::internal::parallel_for_body<Function, Index> body(f, first, step);
        tbb::uneven::parallel_for(range, body, partitioner);
    }
}    
...
void parallel_for_impl(Index first, Index last, Index step, const Function& f, Partitioner& partitioner, tbb::task_group_context &context) {
    if (step <= 0 )
        ...
        tbb::uneven::blocked_range<Index> range(static_cast<Index>(0), end);
        tbb::uneven::internal::parallel_for_body<Function, Index> body(f, first, step);
        tbb::uneven::parallel_for(range, body, partitioner, context);
    }
}
...
#endif /* __TBB_parallel_for_uneven_H */
