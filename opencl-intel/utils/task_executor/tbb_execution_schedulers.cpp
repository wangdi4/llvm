...
protected:
  ...
  TaskLoopBody(const Intel::OpenCL::Utils::SharedPtr<ITaskSet> &task)
      : m_task(task) {}
};

template <class blocked_range_3d> struct TaskLoopBody3D : public TaskLoopBody {
  TaskLoopBody3D(const Intel::OpenCL::Utils::SharedPtr<ITaskSet> &task)
      : TaskLoopBody(task) {}
  void operator()(const blocked_range_3d &r) const {
    ...
    TBB_PerActiveThreadData *tls = TBB_ThreadManager<
        TBB_PerActiveThreadData>::GetCurrentThreadDescriptor();
    if (nullptr == tls) {
      assert((nullptr != tls) &&
             "Task executes after thread disconnected from TEDevice or thread "
             "is connected after TEDevice shutdown");
      return;
    }

    void *user_local = m_task->AttachToThread(
        tls->user_tls, uiNumberOfWorkGroups, firstWGID, lastWGID);
...
template <class blocked_range_2d> struct TaskLoopBody2D : public TaskLoopBody {
  TaskLoopBody2D(const Intel::OpenCL::Utils::SharedPtr<ITaskSet> &task)
      : TaskLoopBody(task) {}
...
template <class blocked_range_1d> struct TaskLoopBody1D : public TaskLoopBody {
  TaskLoopBody1D(const Intel::OpenCL::Utils::SharedPtr<ITaskSet> &task)
      : TaskLoopBody(task) {}
  ...
  void operator()(const blocked_range_1d &r) const {
    ...
    TBB_PerActiveThreadData *tls = TBB_ThreadManager<
        TBB_PerActiveThreadData>::GetCurrentThreadDescriptor();
    assert((nullptr != tls) &&
           "Task executes after thread disconnected from TEDevice or thread is "
           "connected after TEDevice shutdown");
    if (nullptr == tls) {
      return;
    }
...
#ifdef __MIC__
// TODO: Fall back to auto-partitioner, because of the outliers when number of
// workgoups is bellow concurrency level
...
#define DEFINE_EXECUTOR_DIMS_ARRAY(name, ExecutorFuncName,                     \
                                   BlockedRangeNamePrefix)                     \
...
DEFINE_EXECUTOR_DIMS_ARRAY(affinity_block_row, affinity_executor,
                           BlockedRangeByRow);
DEFINE_EXECUTOR_DIMS_ARRAY(static_block_row, static_executor,
                           BlockedRangeByRow);

DEFINE_EXECUTOR_DIMS_ARRAY(auto_block_column, auto_executor,
                           BlockedRangeByColumn);
DEFINE_EXECUTOR_DIMS_ARRAY(affinity_block_column, affinity_executor,
                           BlockedRangeByColumn);
DEFINE_EXECUTOR_DIMS_ARRAY(static_block_column, static_executor,
                           BlockedRangeByColumn);

DEFINE_EXECUTOR_DIMS_ARRAY(auto_block_tile, auto_executor, BlockedRangeByTile);
DEFINE_EXECUTOR_DIMS_ARRAY(affinity_block_tile, affinity_executor,
                           BlockedRangeByTile);
DEFINE_EXECUTOR_DIMS_ARRAY(static_block_tile, static_executor,
                           BlockedRangeByTile);

TBB_ExecutionSchedulers::ExecutorFunc *TBB_ExecutionSchedulers::g_executor
    [TE_CMD_LIST_PREFERRED_SCHEDULING_LAST][TASK_SET_OPTIMIZE_BY_LAST] = {
        /* default by TBB tile */ /* by_row  */ /* by_column */ /* by_tile */
        /* auto     */ {auto_block_default, auto_block_row, auto_block_column,
                        auto_block_tile},
        /* affinity */
        {affinity_block_default, affinity_block_row, affinity_block_column,
         affinity_block_tile},
        /* static   */
        {static_block_default, static_block_row, static_block_column,
         static_block_tile}};
...
//
/////////////////////////////////////////////////////////////////////////////////////
bool TBB_ExecutionSchedulers::parallel_execute(
    base_command_list &cmdList,
    const Intel::OpenCL::Utils::SharedPtr<ITaskSet> &task) {
  ...
  if (taskExecutor.IsTBBNumaEnabled() && task->PreferNumaNodes()) {
    ...
    if (lastDimSize > nodesCount) {
      ...
      std::vector<std::vector<size_t>> &dimsBegin = cmdList.GetNumaDimsBegin();
      std::vector<std::vector<size_t>> &dimsEnd = cmdList.GetNumaDimsEnd();
      ...
      for (unsigned i = 0; i < nodesCount; ++i) {
        ...
        MEMCPY_S(dimsEnd[i].data(), MAX_WORK_DIM * sizeof(size_t), dims,
                 sizeof(dims));
        dimsEnd[i][lastDimIdx] =
            (i == (nodesCount - 1)) ? lastDimSize : (interval * i + interval);
      }
      for (unsigned i = 0; i < nodesCount; ++i) {
        lowLevelArenas[i].Execute([&, i] {
          taskGroups[i].run([&, i]() {
            execute(dimsBegin[i].data(), dimsEnd[i].data(), grainSize, task,
                    cmdList);
          });
        });
      }
