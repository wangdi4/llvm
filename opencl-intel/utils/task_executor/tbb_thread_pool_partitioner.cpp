
    //Copyright 2005-2011 Intel Corporation.  All Rights Reserved.

    //The source code contained or described herein and all documents related
    //to the source code ("Material") are owned by Intel Corporation or its
    //suppliers or licensors.  Title to the Material remains with Intel
    //Corporation or its suppliers and licensors.  The Material is protected
    //by worldwide copyright laws and treaty provisions.  No part of the
    //Material may be used, copied, reproduced, modified, published, uploaded,
    //posted, transmitted, distributed, or disclosed in any way without
    //Intel's prior express written permission.

    //No license under any patent, copyright, trade secret or other
    //intellectual property right is granted to or conferred upon you by
    //disclosure or delivery of the Materials, either expressly, by
    //implication, inducement, estoppel or otherwise.  Any license under such
    //intellectual property rights must be express and approved by Intel in
    //writing.


#include <cassert>
#ifdef WIN32
#include <Windows.h>
#else
#include <sched.h>
#endif

#include <tbb/task_scheduler_init.h>
#include <tbb/atomic.h>
#include <tbb/task.h>

#include "cl_utils.h"
#include "tbb_executor.h"

using namespace Intel::OpenCL::TaskExecutor;
namespace Intel { namespace OpenCL { namespace TaskExecutor {


class MyObserver 
{
public:
    MyObserver( int numWorkers )
        : m_numWorkers(numWorkers), m_init(tbb::task_scheduler_init::deferred)
    {}

    virtual ~MyObserver() {}

    void Activate()
    {
        m_init.initialize(m_numWorkers + 1);
    }
    void Deactivate()
    {
        m_init.terminate();
    }

protected:
    int m_numWorkers;
    tbb::task_scheduler_init m_init;
};

TBBThreadPoolPartitioner::TBBThreadPoolPartitioner(size_t numThreads)
{
    m_observer = new MyObserver(numThreads);
    assert(NULL != m_observer);
}

TBBThreadPoolPartitioner::~TBBThreadPoolPartitioner()
{
    Deactivate();
    delete m_observer;
}

void TBBThreadPoolPartitioner::Activate()
{
    m_observer->Activate();
}
void TBBThreadPoolPartitioner::Deactivate()
{
    m_observer->Deactivate();
}
}}}
