...
public:
  INLINE __ittx_scoped_task(const __itt_domain *domain, __itt_id taskid,
                            __itt_id parentid, __itt_string_handle *name)
      : mDomain(domain) {
    __itt_task_begin(domain, taskid, parentid, name);
  }
  INLINE ~__ittx_scoped_task() { __itt_task_end(mDomain); }
...
public:
  INLINE __ittx_scoped_task_fn(const __itt_domain *domain, __itt_id taskid,
                               __itt_id parentid, void *fn)
      : mDomain(domain) {
    __itt_task_begin_fn(domain, taskid, parentid, fn);
  }
  INLINE ~__ittx_scoped_task_fn() { __itt_task_end(mDomain); }
...
#define ITTX_SCOPED_TASK(domain, taskid, parentid, name)                       \
  static __itt_string_handle *_ittx_scoped_task_string_handle_##name =         \
      __itt_string_handle_create(name);                                        \
  __ittx_scoped_task _ittx_scoped_task_##name(                                 \
      domain, taskid, parentid, _ittx_scoped_task_string_handle_##name)
...
#define ITTX_SCOPED_TASK_NAMED(domain, name)                                   \
  static __itt_string_handle *_ittx_scoped_task_string_handle_##name =         \
      __itt_string_handle_create(name);                                        \
  __ittx_scoped_task _ittx_scoped_task_##name(                                 \
      domain, __itt_null, __itt_null, _ittx_scoped_task_string_handle_##name)

#define ITTX_SCOPED_TASK_NAMED_WITH_ID(domain, taskid, name)                   \
  static __itt_string_handle *_ittx_scoped_task_string_handle_##name =         \
      __itt_string_handle_create(name);                                        \
  __ittx_scoped_task _ittx_scoped_task_##name(                                 \
      domain, taskid, __itt_null, _ittx_scoped_task_string_handle_##name)
#else
#define ITTX_SCOPED_TASK(domain, taskid, parentid, name)
#define ITTX_SCOPED_TASK_FN(domain, taskid, parentid, fn)
#define ITTX_SCOPED_TASK_NAMED(domain, name)
...
public:
  friend __ittx_task_state *__ittx_task_state_create(const __itt_domain *domain,
                                                     const char *state_name);

  ~__ittx_task_state() { delete m_Name; }
...
static const char *const task_state_name_str =
    "<metadata uri=\"com.intel.task_state_name\" index=\"%d\"/>";
static const char *const task_state_str =
    "<metadata uri=\"com.intel.gpa.task_state\"/>";
static const char *const task_state_ex_str =
    "<metadata uri=\"com.intel.gpa.task_state_ex\"/>";

static const __itt_string_handle *const task_state_str_handle =
    __itt_string_handle_createA(task_state_str);
static const __itt_string_handle *const task_state_ex_str_handle =
    __itt_string_handle_createA(task_state_ex_str);

// Defines a task state by name.
INLINE __ittx_task_state *__ittx_task_state_create(const __itt_domain *domain,
                                                   const char *state_name) {
  ...
  __itt_metadata_str_add_with_scopeA(domain, __itt_scope_global, create_handle,
                                     newState->GetName(), len);

  return newState;
}

// For the current task in scope (or the task indicated by taskid), set the
// current task state.
INLINE void __ittx_task_set_state(const __itt_domain *domain, __itt_id taskid,
                                  __ittx_task_state *task_state) {
  if (task_state == NULL) {
    return;
  }
  ...
  __itt_metadata_add(domain, taskid,
                     const_cast<__itt_string_handle *>(task_state_str_handle),
                     __itt_metadata_u64, 2, state_data);
}

// Set state for custom clock domains.
INLINE void __ittx_task_set_state(const __itt_domain *domain,
                                  __itt_clock_domain *clockdomain,
  ...
  __itt_metadata_add(
      domain, taskid,
      const_cast<__itt_string_handle *>(task_state_ex_str_handle),
      __itt_metadata_u64, 3, state_data);
}

// Set the default task state for the current track.
INLINE void __ittx_set_default_state(const __itt_domain *domain,
                                     __ittx_task_state *task_state) {
  if (task_state == NULL) {
    return;
  }
