 ...
 *
 * Sometimes you cannot assign the name to a sync object in the
 * __itt_sync_set_name() call because it is not yet known there. In this case
 * you should use the rename call which allows to assign the name after the
 * creation has been registered. The renaming can be done multiple times. All
 * waits after a new name has been assigned will be attributed to the sync
...
/**
 * @brief Is called when sync object is destroyed (needed to track lifetime of
 * objects).
 */
 ...
 * execution time, there is no mechanism to destroy a domain. Any domain can be
 * accessed by any thread in the process, regardless of which thread created the
 * domain. This call is thread-safe.
...
#define __itt_id_create_ex(domain, clock_domain, timestamp, id)                \
  ITTNOTIFY_VOID_D3(id_create_ex, domain, clock_domain, timestamp, id)
#define __itt_id_create_ex_ptr ITTNOTIFY_NAME(id_create_ex)
#define __itt_id_destroy_ex(domain, clock_domain, timestamp, id)               \
  ITTNOTIFY_VOID_D3(id_destroy_ex, domain, clock_domain, timestamp, id)
...
#define __itt_region_begin(domain, id, parentid, name)                         \
  ITTNOTIFY_VOID_D3(region_begin, domain, id, parentid, name)
...
#define __itt_task_group(domain, id, parentid, name)                           \
  ITTNOTIFY_VOID_D3(task_group, domain, id, parentid, name)
...
#define __itt_task_begin(domain, id, parentid, name)                           \
  ITTNOTIFY_VOID_D3(task_begin, domain, id, parentid, name)
#define __itt_task_begin_ptr ITTNOTIFY_NAME(task_begin)
#define __itt_task_begin_fn(domain, id, parentid, fn)                          \
  ITTNOTIFY_VOID_D3(task_begin_fn, domain, id, parentid, fn)
...
ITT_STUBV(ITTAPI, void, task_end_ex,
          (const __itt_domain *domain, __itt_clock_domain *clock_domain,
           unsigned long long timestamp))
#define __itt_task_begin_ex(domain, clock_domain, timestamp, id, parentid,     \
                            name)                                              \
  ITTNOTIFY_VOID_D5(task_begin_ex, domain, clock_domain, timestamp, id,        \
                    parentid, name)
...
#define __itt_task_begin_fn_ex(domain, clock_domain, timestamp, id, parentid,  \
                               fn)                                             \
  ITTNOTIFY_VOID_D5(task_begin_fn_ex, domain, clock_domain, timestamp, id,     \
                    parentid, fn)
#define __itt_task_begin_fn_ex_ptr ITTNOTIFY_NAME(task_begin_fn_ex)
#define __itt_task_end_ex(domain, clock_domain, timestamp)                     \
  ITTNOTIFY_VOID_D2(task_end_ex, domain, clock_domain, timestamp)
...
ITT_STUBV(ITTAPI, void, task_end_overlapped_ex,
          (const __itt_domain *domain, __itt_clock_domain *clock_domain,
           unsigned long long timestamp, __itt_id taskid))
#define __itt_task_begin_overlapped(domain, id, parentid, name)                \
  ITTNOTIFY_VOID_D3(task_begin_overlapped, domain, id, parentid, name)
#define __itt_task_begin_overlapped_ptr ITTNOTIFY_NAME(task_begin_overlapped)
#define __itt_task_begin_overlapped_ex(domain, clock_domain, timestamp, id,    \
                                       parentid, name)                         \
  ITTNOTIFY_VOID_D5(task_begin_overlapped_ex, domain, clock_domain, timestamp, \
                    id, parentid, name)
...
#define __itt_task_end_overlapped(domain, id)                                  \
  ITTNOTIFY_VOID_D1(task_end_overlapped, domain, id)
...
#define __itt_task_end_overlapped_ex(domain, clock_domain, timestamp, id)      \
  ITTNOTIFY_VOID_D3(task_end_overlapped_ex, domain, clock_domain, timestamp, id)
...
#define __itt_marker(domain, id, name, scope)                                  \
  ITTNOTIFY_VOID_D3(marker, domain, id, name, scope)
...
#define __itt_marker_ex(domain, clock_domain, timestamp, id, name, scope)      \
  ITTNOTIFY_VOID_D5(marker_ex, domain, clock_domain, timestamp, id, name, scope)
...
#define __itt_metadata_add(domain, id, key, type, count, data)                 \
  ITTNOTIFY_VOID_D5(metadata_add, domain, id, key, type, count, data)
 ...
 * @param[in] key The name of the metadata
 * @param[in] data The metadata itself
 * @param[in] length The number of characters in the string, or 0 if the length
...
#define __itt_metadata_str_addA(domain, id, key, data, length)                 \
  ITTNOTIFY_VOID_D4(metadata_str_addA, domain, id, key, data, length)
...
#define __itt_metadata_str_addW(domain, id, key, data, length)                 \
  ITTNOTIFY_VOID_D4(metadata_str_addW, domain, id, key, data, length)
#define __itt_metadata_str_addW_ptr ITTNOTIFY_NAME(metadata_str_addW)
#else /* ITT_PLATFORM==ITT_PLATFORM_WIN */
#define __itt_metadata_str_add(domain, id, key, data, length)                  \
  ITTNOTIFY_VOID_D4(metadata_str_add, domain, id, key, data, length)
...
#define __itt_metadata_add_with_scope(domain, scope, key, type, count, data)   \
  ITTNOTIFY_VOID_D5(metadata_add_with_scope, domain, scope, key, type, count,  \
                    data)
...
#define __itt_metadata_str_add_with_scopeA(domain, scope, key, data, length)   \
  ITTNOTIFY_VOID_D4(metadata_str_add_with_scopeA, domain, scope, key, data,    \
                    length)
...
#define __itt_metadata_str_add_with_scopeW(domain, scope, key, data, length)   \
  ITTNOTIFY_VOID_D4(metadata_str_add_with_scopeW, domain, scope, key, data,    \
                    length)
#define __itt_metadata_str_add_with_scopeW_ptr                                 \
  ITTNOTIFY_NAME(metadata_str_add_with_scopeW)
#else /* ITT_PLATFORM==ITT_PLATFORM_WIN */
#define __itt_metadata_str_add_with_scope(domain, scope, key, data, length)    \
  ITTNOTIFY_VOID_D4(metadata_str_add_with_scope, domain, scope, key, data,     \
                    length)
#define __itt_metadata_str_add_with_scope_ptr                                  \
...
#define __itt_relation_add_to_current(domain, relation, tail)                  \
  ITTNOTIFY_VOID_D2(relation_add_to_current, domain, relation, tail)
...
#define __itt_relation_add(domain, head, relation, tail)                       \
  ITTNOTIFY_VOID_D3(relation_add, domain, head, relation, tail)
...
#define __itt_relation_add_to_current_ex(domain, clock_domain, timestamp,      \
                                         relation, tail)                       \
  ITTNOTIFY_VOID_D4(relation_add_to_current_ex, domain, clock_domain,          \
                    timestamp, relation, tail)
...
#define __itt_relation_add_ex(domain, clock_domain, timestamp, head, relation, \
                              tail)                                            \
  ITTNOTIFY_VOID_D5(relation_add_ex, domain, clock_domain, timestamp, head,    \
                    relation, tail)
...
/** @cond exclude_from_documentation */
typedef enum { __itt_track_group_type_normal = 0 } __itt_track_group_type;
/** @endcond */
