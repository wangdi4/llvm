...
#define ENTER(fname)     (cout << "===ENTER " << fname  << " {\n")
#define EXIT(fname)      (cout << "===EXIT " << fname  << " }\n")
#define DATA(name, value) (cout << "  " << name << " : " << value << "\n")
...
extern "C" {
    ...
    {
ENTER("__kmpc_omp_task_alloc"); //JJJ
DATA("gtid", gtid);
DATA("flags", flags);
DATA("sizeof_kmp_task_t", sizeof_kmp_task_t);
DATA("sizeof_shareds", sizeof_shareds);
DATA("task_entry", task_entry);
...
DATA("pointer to allocated thunk (kmp_task_t*)", task);
DATA("pointer to shareds", task->shareds);
EXIT("__kmpc_omp_task_alloc"); //JJJ
    ...
    (
        ...
        void * task_dup)
    {
ENTER("__kmpc_taskloop"); //JJJ
DATA("gtid", gtid); //JJJ
DATA("task", task); //JJJ
DATA("  task->shareds", task->shareds); //JJJ
DATA("  task->routine", task->routine); //JJJ
DATA("if_val", if_val); //JJJ
DATA("*lb", *lb); //JJJ
DATA("*ub", *ub); //JJJ
DATA("st", st); //JJJ
DATA("nogroup", nogroup); //JJJ
DATA("sched", sched); //JJJ
DATA("grainsize", grainsize); //JJJ
DATA("task_dup", task_dup); //JJJ
...
DATA("num_tasks", num_tasks); //JJJ
DATA("tripcount", tripcount); //JJJ
DATA("computed grainsize", grainsize); //JJJ
        }
...
ENTER("__kmpc_task_reduction_init"); //JJJ
DATA("num_data", num_data); //JJJ
DATA("data", data);

        //JJJ __TBB_ASSERT_RELEASE(gtid == 0, "Unknown gtid");
        kmp_task_red_input_t* input = (kmp_task_red_input_t*)data;

DATA("  data->reduce_shar", input->reduce_shar);
DATA("  data->reduce_size", input->reduce_size);
DATA("  data->reduce_init", input->reduce_init);
DATA("  data->reduce_fini", input->reduce_fini);
DATA("  data->reduce_comb", input->reduce_comb);
DATA("  data->flags->lazy_priv",  input->flags.lazy_priv);
DATA("  data->flags->reserved31", input->flags.reserved31);
...
EXIT("__kmpc_task_reduction_init"); //JJJ
        return nullptr;
    }
