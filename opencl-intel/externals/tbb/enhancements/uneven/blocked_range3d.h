...
#ifndef __TBB_blocked_range3d__uneven_H
#define __TBB_blocked_range3d__uneven_H
...
public:
    ...
    blocked_range3d( blocked_range3d& r, split s) : 
        my_pages(r.my_pages),
        my_rows(r.my_rows),
        my_cols(r.my_cols)
    {
        if( my_pages.size()*double(my_rows.grainsize()) < my_rows.size()*double(my_pages.grainsize()) ) {
            ...
            if ( my_pages.size()*double(my_cols.grainsize()) < my_cols.size()*double(my_pages.grainsize()) ) {
                my_cols.my_begin = col_range_type::do_split(r.my_cols, s.ratio());
                    ...
                    my_pages.my_begin = page_range_type::do_split(r.my_pages, s.ratio());
            }
        }
    }
...
#endif /* __TBB_blocked_range3d__uneven_H */
