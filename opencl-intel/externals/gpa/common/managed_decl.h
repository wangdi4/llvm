...
#define BEGIN_ENUM_DECL(name)   public enum class name
#define BEGIN_ENUM_DECL_WITH_BASE_TYPE(name, basetype)    public enum class name: basetype
#define END_ENUM_DECL(name)     ;
#define BEGIN_STRUCT_DECL(name)         [StructLayout(LayoutKind::Sequential, CharSet=CharSet::Unicode, Pack=1)] public value struct name
#define BEGIN_ANSI_STRUCT_DECL(name)    [StructLayout(LayoutKind::Sequential, CharSet=CharSet::Ansi, Pack=1)] public value struct name
#define ASSIGN_OFFSET(X)                [System.Runtime.InteropServices.FieldOffset(X)]
#define END_STRUCT_DECL(name)   ;

#define GT_CHAR_ARRAY_DECL(name, size)  [MarshalAs(UnmanagedType::ByValTStr, SizeConst=size)] String^ name;
#define GT_UINT32_ARRAY_DECL(name, size) [MarshalAs(UnmanagedType::ByValArray, ArraySubType = UnmanagedType::U4, SizeConst = size)] array<GT_UINT32>^ name;
...
#undef BEGIN_ENUM
#define BEGIN_ENUM(name)   public enum class MANAGED_##name
#undef END_ENUM	
