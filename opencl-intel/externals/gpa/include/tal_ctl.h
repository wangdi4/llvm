...
// 3. Determine whether any default behavior is required. If so, add it to __Default_talctl().
...
// ** DON'T EVEN THINK ABOUT REMOVING A TALCTL CODE OR CHANGING ANY OF THE VALUES!! **
...
typedef enum 
{
	...
	TALCTL_FORCE_REF		            = 0x19,     // Don't even THINK about filling the gap between 0x19 and 0x20!!
	TALCTL_NOP				            = 0x20,
...
// NOTE: It is possible to modify these structs in subsequent releases, but fields must only be added to the end
// of the struct, and the talctl() implementation must be written to support previous versions of the structs. The length 
// supplied in the inputSize param identifies which version of the struct is being provided. When a struct is redefined, the
// previous definition should be renamed to <old_name>_V0, or something like that, so that talctl() can cast to the correct
// definition for the given length.
...
#define talCtlInput(input, size, arg) memcpy(&arg, input,  TAL_MIN(inputSize, sizeof(arg))) 
#define talCtlOutput(arg, output, outputSize) memcpy(output, &arg, TAL_MIN(*outputSize, sizeof(arg))); *outputSize = TAL_MIN(*outputSize, sizeof(arg));


// NOTE: This solves the problem for the existing set of APIs with their current argument sizes.
// If any of the arguments are extended in the future, this structure will need to be modified then.
...
typedef TAL_BOOL TAL_CALL TALCTL_FUNC(TALCTL_CODE code, const void* input, TAL_UINT32 inputSize, void* output, TAL_UINT32* outputSize);
