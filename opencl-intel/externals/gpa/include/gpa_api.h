...
#define GPANOTIFY_NAME(n)                                                      \
  ITT_VERSIONIZE(GPANOTIFY_NAME_AUX(ITT_JOIN(n, INTEL_ITTNOTIFY_POSTFIX)))
...
#define GPANOTIFY_VOID_D0(n, d)                                                \
  (!(d)->flags)          ? (void)0                                             \
  : (!GPANOTIFY_NAME(n)) ? (void)0                                             \
                         : GPANOTIFY_NAME(n)(d)
#define GPANOTIFY_VOID_D1(n, d, x)                                             \
  (!(d)->flags)          ? (void)0                                             \
  : (!GPANOTIFY_NAME(n)) ? (void)0                                             \
                         : GPANOTIFY_NAME(n)(d, x)
#define GPANOTIFY_VOID_D2(n, d, x, y)                                          \
  (!(d)->flags)          ? (void)0                                             \
  : (!GPANOTIFY_NAME(n)) ? (void)0                                             \
                         : GPANOTIFY_NAME(n)(d, x, y)
#define GPANOTIFY_VOID_D3(n, d, x, y, z)                                       \
  (!(d)->flags)          ? (void)0                                             \
  : (!GPANOTIFY_NAME(n)) ? (void)0                                             \
                         : GPANOTIFY_NAME(n)(d, x, y, z)
#define GPANOTIFY_VOID_D6(n, d, x, y, z, a, b, c)                              \
  (!(d)->flags)          ? (void)0                                             \
  : (!GPANOTIFY_NAME(n)) ? (void)0                                             \
                         : GPANOTIFY_NAME(n)(d, x, y, z, a, b, c)
#define GPANOTIFY_DATA_D0(n, d)                                                \
  (!(d)->flags) ? 0 : (!GPANOTIFY_NAME(n)) ? 0 : GPANOTIFY_NAME(n)(d)
#define GPANOTIFY_DATA_D1(n, d, x)                                             \
  (!(d)->flags) ? 0 : (!GPANOTIFY_NAME(n)) ? 0 : GPANOTIFY_NAME(n)(d, x)
#define GPANOTIFY_DATA_D2(n, d, x, y)                                          \
  (!(d)->flags) ? 0 : (!GPANOTIFY_NAME(n)) ? 0 : GPANOTIFY_NAME(n)(d, x, y)
#define GPANOTIFY_DATA_D3(n, d, x, y, z)                                       \
  (!(d)->flags) ? 0 : (!GPANOTIFY_NAME(n)) ? 0 : GPANOTIFY_NAME(n)(d, x, y, z)
#define GPANOTIFY_DATA_D6(n, d, x, y, z, a, b, c)                              \
  (!(d)->flags)          ? 0                                                   \
  : (!GPANOTIFY_NAME(n)) ? 0                                                   \
                         : GPANOTIFY_NAME(n)(d, x, y, z, a, b, c)
 ...
 *
 * @param[in] metric - The metric description; NULL pointer is possible but not
 * expected
 */
 ...
 * @param[in] name   - The name for this marker; NULL or empty string is
 * possible but not expected
 */
#if ITT_PLATFORM == ITT_PLATFORM_WIN
void ITTAPI __gpa_gfx_set_markerA(const __itt_domain *domain,
                                  unsigned int color, const char *name);
void ITTAPI __gpa_gfx_set_markerW(const __itt_domain *domain,
                                  unsigned int color, const wchar_t *name);
#if defined(UNICODE) || defined(_UNICODE)
...
void ITTAPI __gpa_gfx_set_marker(const __itt_domain *domain, unsigned int color,
                                 const char *name);
...
#ifndef INTEL_NO_ITTNOTIFY_API
#if ITT_PLATFORM == ITT_PLATFORM_WIN
GPA_STUBV(ITTAPI, void, gfx_set_markerA,
          (const __itt_domain *domain, unsigned int color, const char *name))
GPA_STUBV(ITTAPI, void, gfx_set_markerW,
          (const __itt_domain *domain, unsigned int color, const wchar_t *name))
#else  /* ITT_PLATFORM==ITT_PLATFORM_WIN */
GPA_STUBV(ITTAPI, void, gfx_set_marker,
          (const __itt_domain *domain, unsigned int color, const char *name))
#endif /* ITT_PLATFORM==ITT_PLATFORM_WIN */
#if ITT_PLATFORM == ITT_PLATFORM_WIN
#define __gpa_gfx_set_markerA(d, x, y)                                         \
  GPANOTIFY_VOID_D2(gfx_set_markerA, d, x, y)
#define __gpa_gfx_set_markerA_ptr GPANOTIFY_NAME(gfx_set_markerA)
#define __gpa_gfx_set_markerW(d, x, y)                                         \
  GPANOTIFY_VOID_D2(gfx_set_markerW, d, x, y)
 ...
 *
 * Set named and colorized begin region marker for the Intel(R) GPA Frame
 * Analyzer.
 *
 * @param[in] domain - The domain for this marker
 * @param[in] color  - The color for this marker.
 ...
 */
#if ITT_PLATFORM == ITT_PLATFORM_WIN
void ITTAPI __gpa_gfx_begin_regionA(const __itt_domain *domain,
                                    unsigned int color, const char *name);
void ITTAPI __gpa_gfx_begin_regionW(const __itt_domain *domain,
                                    unsigned int color, const wchar_t *name);
#if defined(UNICODE) || defined(_UNICODE)
...
#endif /* UNICODE */
#else  /* ITT_PLATFORM==ITT_PLATFORM_WIN */
void ITTAPI __gpa_gfx_begin_region(const __itt_domain *domain,
                                   unsigned int color, const char *name);
#endif /* ITT_PLATFORM==ITT_PLATFORM_WIN */
...
#ifndef INTEL_NO_ITTNOTIFY_API
#if ITT_PLATFORM == ITT_PLATFORM_WIN
GPA_STUBV(ITTAPI, void, gfx_begin_regionA,
          (const __itt_domain *domain, unsigned int color, const char *name))
GPA_STUBV(ITTAPI, void, gfx_begin_regionW,
          (const __itt_domain *domain, unsigned int color, const wchar_t *name))
#else  /* ITT_PLATFORM==ITT_PLATFORM_WIN */
GPA_STUBV(ITTAPI, void, gfx_begin_region,
          (const __itt_domain *domain, unsigned int color, const char *name))
#endif /* ITT_PLATFORM==ITT_PLATFORM_WIN */
#if ITT_PLATFORM == ITT_PLATFORM_WIN
#define __gpa_gfx_begin_regionA(d, x, y)                                       \
  GPANOTIFY_VOID_D2(gfx_begin_regionA, d, x, y)
#define __gpa_gfx_begin_regionA_ptr GPANOTIFY_NAME(gfx_begin_regionA)
#define __gpa_gfx_begin_regionW(d, x, y)                                       \
  GPANOTIFY_VOID_D2(gfx_begin_regionW, d, x, y)
#define __gpa_gfx_begin_regionW_ptr GPANOTIFY_NAME(gfx_begin_regionW)
#else /* ITT_PLATFORM==ITT_PLATFORM_WIN */
#define __gpa_gfx_begin_region(d, x, y)                                        \
  GPANOTIFY_VOID_D2(gfx_begin_region, d, x, y)
 ...
 *
 * Capture graphics frame for offline analysis in the Intel(R) GPA Frame
 * Analyzer.
 *
 * @param[in] domain - The domain for this marker
 * @param[in] name   - The frame file name; must be valid; NULL or empty string
 * are possible but not expected
 */
#if ITT_PLATFORM == ITT_PLATFORM_WIN
void ITTAPI __gpa_gfx_capture_frameA(const __itt_domain *domain,
                                     const char *name);
void ITTAPI __gpa_gfx_capture_frameW(const __itt_domain *domain,
                                     const wchar_t *name);
#if defined(UNICODE) || defined(_UNICODE)
...
#endif /* UNICODE */
#else  /* ITT_PLATFORM==ITT_PLATFORM_WIN */
void ITTAPI __gpa_gfx_capture_frame(const __itt_domain *domain,
                                    const char *name);
#endif /* ITT_PLATFORM==ITT_PLATFORM_WIN */
...
#ifndef INTEL_NO_ITTNOTIFY_API
#if ITT_PLATFORM == ITT_PLATFORM_WIN
GPA_STUBV(ITTAPI, void, gfx_capture_frameA,
          (const __itt_domain *domain, const char *name))
GPA_STUBV(ITTAPI, void, gfx_capture_frameW,
          (const __itt_domain *domain, const wchar_t *name))
#else  /* ITT_PLATFORM==ITT_PLATFORM_WIN */
GPA_STUBV(ITTAPI, void, gfx_capture_frame,
          (const __itt_domain *domain, unsigned int color, const char *name))
#endif /* ITT_PLATFORM==ITT_PLATFORM_WIN */
#if ITT_PLATFORM == ITT_PLATFORM_WIN
#define __gpa_gfx_capture_frameA(d, x)                                         \
  GPANOTIFY_VOID_D1(gfx_capture_frameA, d, x)
#define __gpa_gfx_capture_frameA_ptr GPANOTIFY_NAME(gfx_capture_frameA)
#define __gpa_gfx_capture_frameW(d, x)                                         \
  GPANOTIFY_VOID_D1(gfx_capture_frameW, d, x)
 ...
 *
 * Turn off a part of a graphics frame capturing to the Intel(R) GPA Frame
 * Analyzer frame file. This allows to capture only a required part of a
 * rendered frame.
 *
 ...
 *
 * Turn on a part of a graphics frame capturing to the Intel(R) GPA Frame
 * Analyzer frame file after it has been turned off. This allows to capture only
 * a required part of a rendered frame.
 *
 * @param[in] domain - The domain for this command
 */
