 ...
 * TAL_FEATURE_* -- Enable flags for various tal features may be defined in project
 *      or some are automatically enabled for platforms/OSs at the bottom of this file
 *      -- TAL_FEATURE_LRB -- Enable lrb features Thread affinity, ring buffer transports, etc
 *      -- TAL_FEATURE_STATIC_PUSH_FUNCS -- enables linked verison of the static TAL pushers
*/
...
// Enabled TAL features that are default on different OSs(Platforms)
    ...
    #else //
        #pragma message("\tTAL is neither enabled or disabled (TAL_DISABLE | TAL_DISABLE)")
    #endif //
...
#define TAL_ON_LRB (defined(TAL_FEATURE_LARRABEE) && (TAL_PLATFORM == TAL_PLATFORM_NIX)) 
#define TAL_SUPPORT_LRB (defined(TAL_FEATURE_LARRABEE) && (TAL_PLATFORM == TAL_PLATFORM_WINDOWS)) 
