...
cl_mem(CL_API_CALL *INTELpfn_clCreateFromDX9MediaSurfaceINTEL)(
    ...
    HANDLE sharedHandle, UINT plane, cl_int *errcode_ret);

typedef CL_API_ENTRY cl_int(CL_API_CALL *INTELpfn_clGetDeviceIDsFromDX9INTEL)(
    cl_platform_id platform, cl_dx9_device_source_intel d3d_device_source,
    void *d3d_object, cl_dx9_device_set_intel d3d_device_set,
    cl_uint num_entries, cl_device_id *devices, cl_uint *num_devices);

#else  // Linux/Android
typedef CL_API_ENTRY
cl_int(CL_API_CALL *INTELpfn_clCreateFromDX9MediaSurfaceINTEL)();
typedef CL_API_ENTRY
cl_int(CL_API_CALL *INTELpfn_clEnqueueAcquireDX9ObjectsINTEL)();
typedef CL_API_ENTRY
cl_mem(CL_API_CALL *INTELpfn_clEnqueueReleaseDX9ObjectsINTEL)();
typedef CL_API_ENTRY cl_int(CL_API_CALL *INTELpfn_clGetDeviceIDsFromDX9INTEL)();
#endif // Linux and android

typedef CL_API_ENTRY cl_int(CL_API_CALL *INTELpfn_clGetImageParamsINTEL)(
    cl_context context, const cl_image_format *image_format,
    const cl_image_desc *image_desc, size_t *image_row_pitch,
    size_t *image_slice_pitch);

typedef CL_API_ENTRY cl_int(CL_API_CALL *INTELpfn_clSetDebugVariableINTEL)(
    cl_device_id device, const char *pKey, cl_uint value);

/* Performance Counter APIs */
typedef CL_API_ENTRY
cl_command_queue(CL_API_CALL *INTELpfn_clCreatePerfCountersCommandQueueINTEL)(
    cl_context context, cl_device_id device,
    cl_command_queue_properties properties, cl_uint configuration,
    cl_int *errcode_ret);

typedef CL_API_ENTRY
cl_int(CL_API_CALL *INTELpfn_clSetPerformanceConfigurationINTEL)(
    cl_device_id device, cl_uint count, cl_uint *offsets, cl_uint *values);

/* cl_intel_accelerator */
typedef CL_API_ENTRY
cl_accelerator_intel(CL_API_CALL *INTELpfn_clCreateAcceleratorINTEL)(
    cl_context context, cl_accelerator_type_intel type, size_t desc_size,
    const void *desc, cl_int *errcode_ret) CL_API_SUFFIX__VERSION_1_2;
...
typedef CL_API_ENTRY cl_int(CL_API_CALL *INTELpfn_clSetAcceleratorInfoINTEL)(
    cl_accelerator_intel accelerator, cl_accelerator_info_intel param_name,
    size_t param_value_size, void *param_value) CL_API_SUFFIX__VERSION_1_2;
...
typedef CL_API_ENTRY cl_int(CL_API_CALL *INTELpfn_clEnqueueMediaPakINTEL)(
    cl_command_queue command_queue, cl_accelerator_intel accelerator,
    const cl_pak_hevc_picture_intel *picture, cl_uint num_slices,
    cl_uint *num_slice_headers, const cl_pak_hevc_insert_intel *slice_headers,
    const cl_pak_hevc_slice_intel *slices,
    const cl_pak_hevc_ctu_intel *coding_tree_units, cl_mem coding_units,
    cl_mem current_image, cl_mem current_temporal, cl_mem current_recon,
    cl_uint num_refs, cl_mem *ref_images, cl_mem *ref_temporal,
    const cl_pak_hevc_insert_intel *tail_insert, cl_mem status_out,
    cl_mem stream_out, cl_uint num_events_in_wait_list,
...
cl_program(CL_API_CALL *INTELpfn_clCreateProfiledProgramWithSourceINTEL)(
    cl_context context, cl_uint count, const char **sources,
    const size_t *lengths, const void *configurations,
    cl_uint configurations_count, cl_int *errorCode);

typedef CL_API_ENTRY
cl_int(CL_API_CALL *INTELpfn_clCreateKernelProfilingJournalINTEL)(
    cl_context context, const void *configuration);

#ifdef LIBVA_SHARING
typedef CL_API_ENTRY
cl_int(CL_API_CALL *INTELpfn_clEnqueueAcquireVA_APIMediaSurfacesINTEL)(
    cl_command_queue command_queue, cl_uint num_objects,
    const cl_mem *mem_objects, cl_uint num_events_in_wait_list,
    const cl_event *event_wait_list, cl_event *ocl_event);

typedef CL_API_ENTRY
cl_int(CL_API_CALL *INTELpfn_clEnqueueReleaseVA_APIMediaSurfacesINTEL)(
    cl_command_queue command_queue, cl_uint num_objects,
    const cl_mem *mem_objects, cl_uint num_events_in_wait_list,
    const cl_event *event_wait_list, cl_event *ocl_event);

typedef CL_API_ENTRY
cl_int(CL_API_CALL *INTELpfn_clGetDeviceIDsFromVA_APIMediaAdapterINTEL)(
...
typedef CL_API_ENTRY
cl_mem(CL_API_CALL *INTELpfn_clCreateFromVA_APIMediaSurfaceINTEL)(
    cl_context context, cl_mem_flags flags, VASurfaceID *surface, cl_uint plane,
    cl_int *errcode_ret);

#else
typedef CL_API_ENTRY
cl_int(CL_API_CALL *INTELpfn_clCreateFromVA_APIMediaSurfaceINTEL)();
typedef CL_API_ENTRY
cl_int(CL_API_CALL *INTELpfn_clEnqueueReleaseVA_APIMediaSurfacesINTEL)();
typedef CL_API_ENTRY
cl_int(CL_API_CALL *INTELpfn_clGetDeviceIDsFromVA_APIMediaAdapterINTEL)();
typedef CL_API_ENTRY
cl_mem(CL_API_CALL *INTELpfn_clEnqueueAcquireVA_APIMediaSurfacesINTEL)();
#endif // LIBVA_SHARING
...
struct SOCLCRTDispatchTable {
  ...
  INTELpfn_clCreateFromVA_APIMediaSurfaceINTEL
      clCreateFromVA_APIMediaSurfaceINTEL;
  INTELpfn_clGetDeviceIDsFromVA_APIMediaAdapterINTEL
      clGetDeviceIDsFromVA_APIMediaAdapterINTEL;
  INTELpfn_clEnqueueReleaseVA_APIMediaSurfacesINTEL
      clEnqueueReleaseVA_APIMediaSurfacesINTEL;
  INTELpfn_clEnqueueAcquireVA_APIMediaSurfacesINTEL
  ...
  INTELpfn_clSetPerformanceConfigurationINTEL
