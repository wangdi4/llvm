...
crt_err_code OclDynamicLib::Load(const char *pLibName) {
  ...
  // Load library
  if ((strstr(pLibName, "\\") != nullptr) ||
      (strstr(pLibName, "/") != nullptr)) {
    // pLibName is a full path to the lib (assuming it's not available in
  ...
  IMAGE_NT_HEADERS *pnt =
      (IMAGE_NT_HEADERS *)&hMod[PIMAGE_DOS_HEADER(hMod)->e_lfanew];
  IMAGE_EXPORT_DIRECTORY *exp =
      (IMAGE_EXPORT_DIRECTORY
           *)&hMod[pnt->OptionalHeader.DataDirectory->VirtualAddress];
