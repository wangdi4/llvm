...
CrtObject::CrtObject() : m_refCount(1), m_pendencyCount(1) {}

long CrtObject::IncRefCnt() { return atomic_increment(&m_refCount); }
...
long CrtObject::IncPendencyCnt() { return atomic_increment(&m_pendencyCount); }
...
void CopyMemoryObject(MemCpyParams *pCopyCmd) {
  ...
  if (1 == pCopyCmd->uiDimCount) {
    memcpy_s(pCopyCmd->pDst, pCopyCmd->vDstPitch[0], pCopyCmd->pSrc,
             pCopyCmd->vRegion[0]);
    return;
  }
...
void CL_CALLBACK CrtEventCallBack(cl_event e, cl_int status, void *user_data) {
  ...
  if (0 == atomic_decrement(&(evData->numReqCalls))) {
    ...
    evData->m_eventDEV->dispatch->clSetUserEventStatus(evData->m_eventDEV,
                                                       CL_COMPLETE);
...
///
/// ------------------------------------------------------------------------------
CrtContext::CrtContext(cl_context context_handle,
                       const cl_context_properties *properties,
                       cl_uint num_devices, const cl_device_id *devices,
                       ctxt_logging_fn pfn_notify, void *user_data,
                       cl_int *errcode_ret)
  ...
  for (cl_uint i = 0; i < OCLCRT::crt_ocl_module.m_oclPlatforms.size(); i++) {
    outDevices =
        new cl_device_id[OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.size()];
    if (NULL == outDevices) {
    ...
    CrtDeviceInfo *devInfo =
        OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.GetValue(outDevices[0]);
    ...
    if (CRT_FAIL ==
        OCLCRT::ReplacePlatformId(
            properties, devInfo->m_crtPlatform->m_platformIdDEV, &props)) {
      errCode = CL_OUT_OF_HOST_MEMORY;
      break;
    }
    // It doesn't matter which device we pick from outDevices
    cl_context ctx = devInfo->m_origDispatchTable.clCreateContext(
        props, matchDevices, outDevices, pfn_notify, user_data, &errCode);
...
void CrtContext::GetDevicesByPlatformId(const cl_uint inNumDevices,
  ...
  for (cl_uint i = 0; i < inNumDevices; i++) {
    CrtDeviceInfo *devInfo =
        OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.GetValue(inDevices[i]);
...
void CrtContext::GetDevsIndicesByPlatformId(const cl_uint inNumDevices,
                                            const cl_device_id *inDevices,
                                            const cl_platform_id &pId,
...
cl_int CrtContext::GetReferenceCount(cl_uint *refCountParam) {
  ...
  SHARED_CTX_DISPATCH::iterator itr = m_contexts.begin();
  errCode = itr->second.clGetContextInfo(itr->first, CL_CONTEXT_REFERENCE_COUNT,
...
cl_int CrtContext::FlushQueues() {
  ...
  for (std::list<cl_command_queue>::iterator itr = m_HostcommandQueues.begin();
       itr != m_HostcommandQueues.end(); itr++) {
...
CrtMemObject::CrtMemObject(cl_mem_flags flags, void *hostPtr, CrtContext *ctx)
    : m_numValidContextObjs(0), m_memHandle(NULL) {
  m_flags = flags;
  m_pUsrPtr = NULL;
...
cl_int CrtMemObject::RegisterDestructorCallback(mem_dtor_fn memDtorFunc,
                                                void *user_data) {
  cl_int errCode = CL_SUCCESS;
  ...
  for (CTX_MEM_MAP::iterator itr = m_ContextToMemObj.begin();
    ...
    if ((IsValidImageFormat(itr->second) == CL_TRUE) &&
      ...
      errCode = itr->second->dispatch->clSetMemObjectDestructorCallback(
          itr->second, CrtMemDtorForwarder, (void *)memData);
...
cl_bool CrtMemObject::HasPrivateCopy() { return (m_pBstPtr && m_pUsrPtr); }

/// ------------------------------------------------------------------------------
///
/// ------------------------------------------------------------------------------
CrtBuffer::CrtBuffer(const size_t size, cl_mem_flags flags, void *host_ptr,
                     CrtContext *ctx)
...
CrtBuffer::CrtBuffer(CrtMemObject *parent_buffer, cl_mem_flags flags,
                     CrtContext *ctx)
...
cl_int CrtBuffer::Create(CrtMemObject **bufObj) {
  ...
  if ((m_flags & CL_MEM_USE_HOST_PTR) && (m_pUsrPtr != NULL) &&
      (((size_t)m_pUsrPtr &
        (m_pContext->getAlignment(CrtObject::CL_BUFFER) - 1)) == 0)) {
    m_pBstPtr = m_pUsrPtr;
    m_pUsrPtr = NULL;
  } else if (m_pBstPtr == NULL &&
             !m_pContext->memObjectAlwaysNotInSync(CrtObject::CL_BUFFER)) {
    // Convert bits to bytes
    m_pBstPtr =
        ALIGNED_MALLOC(m_size, m_pContext->getAlignment(CrtObject::CL_BUFFER));
  ...
  *bufObj = this;
  ...
  for (SHARED_CTX_DISPATCH::iterator itr = m_pContext->m_contexts.begin();
       itr != m_pContext->m_contexts.end(); itr++) {
    ...
    cl_mem memObj = ctx->dispatch->clCreateBuffer(ctx, crtCreateFlags, m_size,
                                                  m_pBstPtr, &errCode);
  ...
  for (; ctx_mem_itr != m_ContextToMemObj.end(); ctx_mem_itr++) {
    if (IsValidMemObjSize(ctx_mem_itr->second) == CL_TRUE) {
      errCode = ctx_mem_itr->second->dispatch->clSetMemObjectDestructorCallback(
          ctx_mem_itr->second, CrtMemDestructorCallBack, (void *)memData);
...
cl_int CrtBuffer::Create(CrtMemObject **bufObj,
  ...
  for (; itr != m_parentBuffer->m_ContextToMemObj.end(); itr++) {
    ...
    cl_mem memObj = itr->first->dispatch->clCreateSubBuffer(
        itr->second, crtCreateFlags, buffer_create_type, buffer_create_info,
        &errCode);
  ...
  for (; itr != m_ContextToMemObj.end(); itr++) {
    if (IsValidMemObjSize(itr->second) == CL_TRUE) {
      errCode = itr->second->dispatch->clSetMemObjectDestructorCallback(
          itr->second, CrtMemDestructorCallBack, (void *)memData);
...
///
/// ------------------------------------------------------------------------------
cl_int CrtBuffer::CheckParamsAndBounds(const size_t *origin,
                                       const size_t *region) {
...
///
/// ------------------------------------------------------------------------------
CrtGLBuffer::CrtGLBuffer(cl_bool isRenderBuffer, cl_mem_flags flags,
                         GLuint bufobj, CrtContext *ctx)
...
cl_int CrtGLBuffer::Create(CrtMemObject **bufObj) {
  ...
  *bufObj = this;
  ...
  for (SHARED_CTX_DISPATCH::iterator itr = m_pContext->m_contexts.begin();
    ...
    if (m_isRenderBuffer) {
      memObj = ctx->dispatch->clCreateFromGLRenderbuffer(ctx, m_flags,
                                                         m_glBufObj, &errCode);
    } else {
      memObj = ctx->dispatch->clCreateFromGLBuffer(ctx, m_flags, m_glBufObj,
                                                   &errCode);
    }
...
/// ------------------------------------------------------------------------------
#ifdef _WIN32
CrtDX9MediaSurface::CrtDX9MediaSurface(cl_mem_flags flags,
...
cl_int CrtDX9MediaSurface::Create(CrtMemObject **dx9Obj) {
  ...
  *dx9Obj = this;
  ...
  for (SHARED_CTX_DISPATCH::iterator itr = m_pContext->m_contexts.begin();
    ...
    memObj =
        ((SOCLEntryPointsTable *)ctx)
            ->crtDispatch->clCreateFromDX9MediaSurfaceINTEL(
                ctx, m_flags, m_resource, m_sharedHandle, m_plane, &errCode);
...
///
/// ------------------------------------------------------------------------------
CrtImage::CrtImage(cl_mem_flags flags, CrtContext *ctx)
    : CrtMemObject(flags, NULL, ctx) {
  m_imageDesc.crtBuffer = NULL;
}
CrtImage::CrtImage(const cl_image_format *image_format,
                   const cl_image_desc *image_desc, cl_mem_flags flags,
                   void *host_ptr, CrtContext *ctx)
    : CrtMemObject(flags, host_ptr, ctx), m_imageFormat(*image_format),
      m_dimCount(0) {
  m_imageDesc.desc = *image_desc;

  m_hostPtrRowPitch = m_imageDesc.desc.image_row_pitch
                          ? m_imageDesc.desc.image_row_pitch
                          : ((cl_uint)m_imageDesc.desc.image_width *
                             GetImageElementSize(&m_imageFormat));
  ...
  if (m_imageDesc.desc.image_type == CL_MEM_OBJECT_IMAGE1D_BUFFER) {
    m_imageDesc.crtBuffer = reinterpret_cast<CrtBuffer *>(
        ((_cl_mem_crt *)m_imageDesc.desc.mem_object)->object);
  ...
  if (gpuDevice != NULL) {
    ...
    cl_int RetVal = gpuDispatch->crtDispatch->clGetImageParamsINTEL(
        ctx->GetContextByDeviceID(gpuDevice), &m_imageFormat,
        (cl_image_desc *)(&m_imageDesc), &m_imageDesc.desc.image_row_pitch,
        &m_imageDesc.desc.image_slice_pitch);

    if (RetVal != CL_SUCCESS) {
      if (RetVal == CL_IMAGE_FORMAT_NOT_SUPPORTED) {
        m_imageDesc.desc.image_row_pitch =
            m_imageDesc.desc.image_width * GetImageElementSize(&m_imageFormat);
        m_imageDesc.desc.image_slice_pitch =
            m_imageDesc.desc.image_row_pitch * m_imageDesc.desc.image_height;
      } else {
        assert(0 && "clGetImageParamsINTEL has failed with non-expected error");
      }
    }

    assert((m_imageDesc.desc.image_type == CL_MEM_OBJECT_IMAGE1D) ||
...
cl_int CrtImage::CheckParamsAndBounds(const size_t *origin,
                                      const size_t *region) {
  ...
  case CL_MEM_OBJECT_IMAGE1D:
  case CL_MEM_OBJECT_IMAGE1D_BUFFER:
    if (origin[1] != 0 || region[1] != 1 || origin[2] != 0 || region[2] != 1) {
      return CL_INVALID_VALUE;
    }
    break;
  case CL_MEM_OBJECT_IMAGE2D:
...
cl_int CrtImage::Create(CrtMemObject **imageObj) {
  ...
  m_size = m_imageDesc.desc.image_slice_pitch * m_imageDesc.desc.image_depth *
           m_imageDesc.desc.image_array_size;
  size_t hostPtrSize = m_hostPtrSlicePitch * m_imageDesc.desc.image_depth *
                       m_imageDesc.desc.image_array_size;
  ...
  //     3. App provided enough space to accommodate the size requirement of
  //     underlying devices
  if (m_imageDesc.desc.image_type == CL_MEM_OBJECT_IMAGE1D_BUFFER) {
    crtBuffer = reinterpret_cast<CrtBuffer *>(
        ((_cl_mem_crt *)m_imageDesc.desc.mem_object)->object);
    m_pBstPtr = crtBuffer->m_pBstPtr;
    m_pUsrPtr = crtBuffer->m_pUsrPtr;
    m_size = crtBuffer->m_size;
  } else if ((m_flags & CL_MEM_USE_HOST_PTR) && (m_pUsrPtr) &&
             (((size_t)m_pUsrPtr &
               (m_pContext->getAlignment(CrtObject::CL_IMAGE) - 1)) == 0) &&
             (hostPtrSize >= m_size)) {
    m_pBstPtr = m_pUsrPtr;
    m_pUsrPtr = NULL;
    m_hostPtrRowPitch = m_imageDesc.desc.image_row_pitch;
    m_hostPtrSlicePitch = m_imageDesc.desc.image_slice_pitch;
  } else if ((m_pBstPtr == NULL) && (m_pContext->memObjectAlwaysNotInSync(
                                         CrtObject::CL_IMAGE) == false)) {
    // Convert bits to bytes
    m_pBstPtr =
        ALIGNED_MALLOC(m_size, m_pContext->getAlignment(CrtObject::CL_IMAGE));
    ...
    if ((m_flags & CL_MEM_COPY_HOST_PTR) || (m_flags & CL_MEM_USE_HOST_PTR)) {
      ...
      sCpyParam.vRegion[2] =
          m_imageDesc.desc.image_depth * m_imageDesc.desc.image_array_size;
  ...
  *imageObj = this;
  ...
  for (; itr != m_pContext->m_contexts.end(); itr++) {
    ...
    if (crtBuffer == NULL) {
      ...
      // - some flags are mutual execlusive, so we need to turn off
    ...
    memObj = ctx->dispatch->clCreateImage(
        ctx, crtCreateFlags, &m_imageFormat, imageDesc,
        (crtBuffer ? NULL : m_pBstPtr), &errCode);

    if ((CL_SUCCESS == errCode) && (memObj != NULL)) {
      m_ContextToMemObj[ctx] = memObj;
      m_numValidContextObjs++;
...
///
/// ------------------------------------------------------------------------------
CrtGLImage::CrtGLImage(cl_uint dim_count, cl_mem_flags flags,
  ...
  for (; itr != m_pContext->m_contexts.end(); itr++) {
    ...
    case 2:
      memObj = ctx->dispatch->clCreateFromGLTexture2D(
          ctx, m_flags, m_textureTarget, m_mipLevel, m_texture, &errCode);
      break;
    case 3:
...
CrtPipe::CrtPipe(const cl_uint packetSize, const cl_uint maxPackets,
                 const cl_pipe_properties *properties, cl_mem_flags flags,
                 CrtContext *ctx)
    : CrtMemObject(flags, NULL, ctx), m_packetSize(packetSize),
      m_maxPackets(maxPackets), m_properties(NULL) {
...
cl_int CrtPipe::Create(CrtMemObject **memObj) {
  ...
  ((SOCLEntryPointsTable *)ctx)
      ->crtDispatch->clCreatePipeINTEL(ctx, m_flags, m_packetSize, m_maxPackets,
                                       m_properties, NULL, &allocSize,
                                       &errCode);
  ...
  m_pBstPtr =
      ALIGNED_MALLOC(m_size, m_pContext->getAlignment(CrtObject::CL_PIPE));
  ...
  *memObj = this;
  ...
  // After we allocated the pipe's buffer in CRT we call both underlying
  // contexts clCreatePipeINTEL with the pointer to our allocated buffer
  for (ctx_itr = m_pContext->m_contexts.begin();
       ctx_itr != m_pContext->m_contexts.end(); ctx_itr++) {
    ctx = ctx_itr->first;

    cl_mem memObject = ((SOCLEntryPointsTable *)ctx)
                           ->crtDispatch->clCreatePipeINTEL(
                               ctx, m_flags, m_packetSize, m_maxPackets,
                               m_properties, m_pBstPtr, &allocSize, &errCode);
...
FINISH:
  if (CL_SUCCESS != errCode) {
    if ((NULL != memData) && (memData->m_count == 0)) {
      delete memData;
    }
    Release();
    *memObj = NULL;
...
///
/// ------------------------------------------------------------------------------
cl_int CrtContext::CreateImage(cl_mem_flags flags,
  ...
  if (image_desc->mem_object != NULL) {
    CrtBuffer *crtBuffer = reinterpret_cast<CrtBuffer *>(
        ((_cl_mem_crt *)image_desc->mem_object)->object);
  ...
  CrtImage *image =
      new CrtImage(image_format, image_desc, flags, host_ptr, this);
...
///
/// ------------------------------------------------------------------------------
cl_int CrtContext::CreateBuffer(cl_mem_flags flags, size_t size, void *host_ptr,
                                CrtMemObject **bufObj) {
  cl_int errCode = CL_SUCCESS;
...
cl_int CrtContext::CreateSubBuffer(_cl_mem_crt *parent_buffer,
                                   cl_mem_flags flags,
                                   cl_buffer_create_type buffer_create_type,
                                   const void *buffer_create_info,
                                   CrtMemObject **bufObj) {
  ...
  if (CL_SUCCESS != errCode) {
    ...
    if ((flags & (CL_MEM_HOST_READ_ONLY | CL_MEM_HOST_WRITE_ONLY |
      ...
      buffer->m_flags |= buffer->m_parentBuffer->m_flags &
...
///
/// ------------------------------------------------------------------------------
cl_int CrtContext::CreateCommandQueue(cl_command_queue queue_crt_handle,
                                      cl_device_id device,
  ...
  *crtQueue = NULL;
  ...
  cl_command_queue queueDEV = itr->second->dispatch->clCreateCommandQueue(
      itr->second, device, properties, &errCode);
  if (CL_SUCCESS == errCode) {
...
void CrtContext::PushCommandQueue(CrtQueue *pCrtQueue) {
...
cl_command_queue
CrtContext::GetDevCommandQueue(cl_command_queue queue_dev_handle) {
  CrtQueue *pCrtQueue = NULL;

  OCLCRT::Utils::OclAutoMutex CS(&m_mutex); // Critical section

  for (std::list<CrtQueue *>::iterator itr = m_DevCommandQueues.begin();
       itr != m_DevCommandQueues.end(); itr++) {
    pCrtQueue = *itr;
...
void CrtContext::RemoveCommandQueue(CrtQueue *pCrtQueue) {
  if (pCrtQueue->m_devQ == true) {
...
cl_int CrtContext::CreateCommandQueueWithProperties(
    cl_command_queue queue_crt_handle, cl_device_id device,
    const cl_queue_properties *properties, CrtQueue **crtQueue) {
  cl_int errCode = CL_SUCCESS;
  CrtQueue *pCrtQueue = NULL;
  *crtQueue = NULL;
  ...
  cl_command_queue queueDEV =
      itr->second->dispatch->clCreateCommandQueueWithProperties(
          itr->second, device, properties, &errCode);
  if (CL_SUCCESS == errCode) {
    pCrtQueue = new CrtQueue(this);
    if (NULL == pCrtQueue) {
    ...
    *crtQueue = pCrtQueue;
    ...
    // extract data from cl_queue_properties, to see if queue on device is being
    // created.
    ...
    if (properties != NULL) {
      while (*properties != 0) {
        ...
        if (PropertyType == CL_QUEUE_PROPERTIES) {
          if ((((cl_command_queue_properties)PropertyValue) &
               CL_QUEUE_ON_DEVICE) != 0) {
            QueueOnDevice = true;
            break;
          }
        }
...
/// ------------------------------------------------------------------------------
cl_int
CrtContext::SetDefaultDeviceCommandQueue(cl_device_id device,
  ...
  pCrtQueue = reinterpret_cast<CrtQueue *>(
      ((_cl_command_queue_crt *)command_queue)->object);

  if ((NULL == pCrtQueue) || (pCrtQueue->m_device != device)) {
    return CL_INVALID_COMMAND_QUEUE;
  }
  errCode = itr->second->dispatch->clSetDefaultDeviceCommandQueue(
      itr->second, device, pCrtQueue->m_cmdQueueDEV);
...
CrtPlatform::CrtPlatform()
    : m_icdSuffix(NULL), m_supportedExtensionsStr(NULL), m_platformIdDEV(NULL) {
}

CrtKernel::CrtKernel(CrtProgram *program) : m_programCRT(program) {
  m_programCRT->IncPendencyCnt();
}

CrtKernel::~CrtKernel() { m_programCRT->DecPendencyCnt(); }

CrtProgram::CrtProgram(CrtContext *ctx) : m_contextCRT(ctx), m_options("") {
...
CrtQueue::CrtQueue(CrtContext *ctx) : m_contextCRT(ctx) {
  m_contextCRT->IncPendencyCnt();
  m_devQ = false;
}

CrtQueue::~CrtQueue() { m_contextCRT->DecPendencyCnt(); }
...
CrtUserEvent::~CrtUserEvent() { m_pContext->DecPendencyCnt(); }

cl_int CrtQueue::Release() {
  OCLCRT::Utils::OclAutoMutex CS(&m_contextCRT->m_mutex); // Critical section
  cl_int errCode =
      m_cmdQueueDEV->dispatch->clReleaseCommandQueue(m_cmdQueueDEV);
...
cl_int CrtContext::CreateSampler(cl_bool normalized_coords,
  ...
  for (SHARED_CTX_DISPATCH::iterator itr = m_contexts.begin();
       itr != m_contexts.end(); itr++) {
    cl_context ctx = itr->first;
    cl_sampler sampObj = ctx->dispatch->clCreateSampler(
        ctx, normalized_coords, addressing_mode, filter_mode, &errCode);
...
cl_int CrtContext::clCreateSamplerWithProperties(
    const cl_sampler_properties *sampler_properties, CrtSampler **sampler) {
  cl_int errCode = CL_SUCCESS;
  CrtSampler *crtSampler = new CrtSampler;
  ...
  for (SHARED_CTX_DISPATCH::iterator itr = m_contexts.begin();
    ...
    cl_sampler sampObj = ctx->dispatch->clCreateSamplerWithProperties(
        ctx, sampler_properties, &errCode);
...
///
/// ------------------------------------------------------------------------------
cl_int CrtContext::CreatePipe(cl_mem_flags flags, cl_uint pipe_packet_size,
  ...
  pipe =
      new CrtPipe(pipe_packet_size, pipe_max_packets, properties, flags, this);
  if (NULL == pipe) {
    return CL_OUT_OF_HOST_MEMORY;
  }
...
///
/// ------------------------------------------------------------------------------
cl_int CrtContext::CreateProgramWithSource(cl_uint count, const char **strings,
  ...
  for (; itr != m_contexts.end(); itr++) {
    cl_context ctx = itr->first;
    cl_program pgmObj = ctx->dispatch->clCreateProgramWithSource(
        ctx, count, strings, lengths, &errCode);
    if (CL_SUCCESS == errCode) {
...
///
/// ------------------------------------------------------------------------------
cl_int CrtContext::CreateProgramWithIL(const void *il, size_t lengths,
                                       CrtProgram **crtProgram) {
  ...
  for (; itr != m_contexts.end(); itr++) {
    cl_context ctx = itr->first;
    cl_program pgmObj =
        ctx->dispatch->clCreateProgramWithIL(ctx, il, lengths, &errCode);
    if (CL_SUCCESS == errCode) {
      pCrtProgram->m_ContextToProgram[ctx] = pgmObj;
    } else {
      break;
...
///
/// ------------------------------------------------------------------------------
cl_int CrtContext::CreateProgramWithBinary(cl_uint num_devices,
  ...
  for (cl_uint i = 0; i < OCLCRT::crt_ocl_module.m_oclPlatforms.size(); i++) {
    ...
    cl_program prog = ctx->dispatch->clCreateProgramWithBinary(
        ctx, matchDevices, outDevices, fwdLengths, fwdBinaries, fwdBinaryStatus,
        &errCode);
...
cl_int CrtContext::CreateProgramWithBuiltInKernels(
    ...
    const char *kernel_names, CrtProgram **crtProgram) {
  cl_int errCode = CL_SUCCESS;
  cl_uint *indices = NULL;
  cl_device_id *outDevices = NULL;
  ...
  for (cl_uint i = 0; i < OCLCRT::crt_ocl_module.m_oclPlatforms.size(); i++) {
    ...
    cl_program prog = ctx->dispatch->clCreateProgramWithBuiltInKernels(
        ctx, matchDevices, outDevices, kernel_names, &errCode);
...
cl_int SyncManager::EnqueueNopCommand(CrtMemObject *memObj, CrtQueue *queue,
  ...
  if (memObj->getObjectType() == CrtObject::CL_BUFFER) {
    errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueReadBuffer(
        queue->m_cmdQueueDEV, memObj->getDeviceMemObj(queue->m_device),
        CL_FALSE, origin[0], region[0], memObj->m_pBstPtr, NumEventsInWaitList,
        inEventWaitList, outEvent);
  } else {
    errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueReadImage(
        queue->m_cmdQueueDEV, memObj->getDeviceMemObj(queue->m_device),
        CL_FALSE, origin, region, 0, 0, memObj->m_pBstPtr, NumEventsInWaitList,
        inEventWaitList, outEvent);
  }
  return errCode;
}

cl_int SyncManager::PrepareToExecute(CrtQueue *queue,
                                     cl_uint NumEventsInWaitList,
                                     const cl_event *inEventWaitList,
  ...
  for (cl_uint i = 0; i < NumEventsInWaitList; i++) {
    CrtEvent *pEvent = (CrtEvent *)((reinterpret_cast<const _cl_event_crt *>(
                                         inEventWaitList[i]))
                                        ->object);
  ...
  for (cl_uint i = 0; i < NumEventsInWaitList; i++) {
    ...
    if (nullEvent) {
      ...
      waitEvent = (CrtEvent *)((reinterpret_cast<const _cl_event_crt *>(
                                    inEventWaitList[i]))
                                   ->object);
    }
    if (waitEvent->m_isUserEvent) {
      ...
      evContext = waitEvent->m_queueCRT->m_contextCRT
                      ->m_DeviceToContext[waitEvent->m_queueCRT->m_device];
    }
  ...
  for (cl_uint i = 0; i < NumEventsInWaitList; i++) {
    ...
    // the target event (aiming for queue) is dependant on an event beloging to
    // different underlying context.
    if (!waitEvent->m_isUserEvent && (dstContext != evContext)) {
      ...
      if (!m_callBackData) {
        ...
        m_callBackData->m_eventDEV =
            queue->m_cmdQueueDEV->dispatch->clCreateUserEvent(dstContext,
                                                              &errCode);
        if (CL_SUCCESS != errCode) {
        ...
        //     1) CrtEventCallBack - guarantees we manage to call
        //     setUserEventStatus
        //                           before it fires
...
SyncManager::~SyncManager() { Release(CL_SUCCESS); }
...
#ifdef _WIN32
cl_int CrtContext::CreateFromDX9MediaSurface(cl_mem_flags flags,
                                             IDirect3DSurface9 *resource,
                                             HANDLE sharedHandle, UINT plane,
                                             CrtMemObject **memObj) {
  cl_int errCode = CL_SUCCESS;

  CrtDX9MediaSurface *dx9MediaSurfaceObj =
      new CrtDX9MediaSurface(flags, resource, sharedHandle, plane, this);
...
cl_int CrtContext::CreateGLBuffer(bool isRender, cl_mem_flags flags,
                                  GLuint bufobj, CrtMemObject **memObj) {
  cl_int errCode = CL_SUCCESS;
...
cl_int CrtContext::CreateGLImage(cl_uint dim_count, cl_mem_flags flags,
                                 cl_GLenum target, cl_GLint miplevel,
                                 cl_GLuint texture, CrtMemObject **memObj) {
  cl_int errCode = CL_SUCCESS;

  CrtGLImage *image =
      new CrtGLImage(dim_count, flags, target, miplevel, texture, this);
...
bool SVMFreeCallbackData::CopySVMPointers(void **SVMPointers,
  ...
  memcpy_s(m_SVMPointers, m_numSVMPointers * sizeof(void *), SVMPointers,
...
void CL_CALLBACK SVMFreeCallbackFunction(cl_event event, cl_int status,
  ...
  CrtQueue *crtQueue = reinterpret_cast<CrtQueue *>(
      ((_cl_command_queue_crt *)clbkData->m_queue)->object);
  ...
  if (clbkData->m_isGpuQueue) {
    ...
    for (cl_uint i = 0; i < clbkData->m_numSVMPointers; i++) {
      if (crtQueue->m_contextCRT->m_svmPointers.HasSVMAllocPtr(
              clbkData->m_SVMPointers[i])) {
        clSVMFree(crtQueue->m_contextCRT->m_context_handle,
                  clbkData->m_SVMPointers[i]);
      } else {
    ...
    if (NULL != clbkData->m_originalCallback) {
      ...
      clbkData->m_originalCallback(
          clbkData->m_queue, clbkData->m_numSVMPointers,
          clbkData->m_SVMPointers, clbkData->m_originalUserData);
    } else {
      // User didn't provide free function; Assume no system pointers here
      assert(!haveSystemPointers &&
             "System pointers were passed to callback without free function! "
