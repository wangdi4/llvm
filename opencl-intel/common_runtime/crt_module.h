...
enum MEM_RESIDENCY {
  SYNC_BUFFER_WITH_HOST = 0x1, // means device need synchronization when need to
                               // read/write to/from host
  SYNC_IMAGE_WITH_HOST = 0x2   // means device need synchronization when need to
                               // read/write to/from host
};

enum DEVICE_SYNC_ATTRIBUTES {
  INTEL_GPU = 0x2, // For Images, means Intel GPU needs synchronization; For
                   // Buffer, it doesn't need to synchronize
...
// Fixes the properties flag passed from the app to match the underlying
// platform properties Like cl_platform_id need to be fixed.
...
public:
  ...
  crt_err_code PatchClContextID(cl_context &inContextId,
                                KHRicdVendorDispatch *origDispatchTable);
