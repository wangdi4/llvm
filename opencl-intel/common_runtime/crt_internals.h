...
inline void *ALIGNED_MALLOC(size_t size, size_t alignment) {
  return memalign(alignment < sizeof(void *) ? sizeof(void *) : alignment,
                  size);
}
...
struct CrtProgram : public CrtObject {
  ...
  // Tracks which contexts a build request has been submitted to for this
  // program. We need this since the spec demands the clCreateKernel only for
  // devices the build context has been submitted to.
...
class AutoLock {
public:
  AutoLock(OclMutex &mutex) : m_mutex(mutex) { m_mutex.Lock(); }
...
template <class TKEY, class TVAL> class GuardedMap {
public:
  GuardedMap(std::map<TKEY, TVAL> &map) : m_map(map) {}

  std::map<TKEY, TVAL> &get() { return m_map; }
...
public:
  CrtMemObject(cl_mem_flags flags, void *hostPtr, CrtContext *ctx);
  ...
  virtual cl_int CheckParamsAndBounds(const size_t *origin,
  ...
  virtual cl_int RegisterDestructorCallback(mem_dtor_fn memDtorFunc,
                                            void *user_data);

  // Used whenever the device cannot share memory with host
  virtual cl_event SynchronizeFromDeviceToHost(CrtDeviceInfo *sourceDevice,
                                               CrtContext *context) {
    return NULL;
  }

  virtual cl_event SynchronizeToDeviceFromHost(CrtDeviceInfo *targetDevice,
                                               CrtContext *context,
  ...
  void SetMemHandle(cl_mem memHandle) { m_memHandle = memHandle; }

  cl_mem GetMemHandle() const { return m_memHandle; }
...
public:
  // Buffer Ctor
  CrtBuffer(const size_t size, cl_mem_flags flags, void *host_ptr,
            CrtContext *ctx);

  // Sub-buffer Ctor
  CrtBuffer(CrtMemObject *parent_buffer, cl_mem_flags flags, CrtContext *ctx);
  ...
  cl_int Create(CrtMemObject **memObj, cl_buffer_create_type buffer_create_type,
...
public:
  // Buffer Ctor
  CrtGLBuffer(cl_bool isRenderBuffer, cl_mem_flags flags, GLuint bufobj,
              CrtContext *ctx);
...
public:
  // Buffer Ctor
  CrtDX9MediaSurface(cl_mem_flags flags, IDirect3DSurface9 *resource,
                     HANDLE sharedhandle, UINT plane, CrtContext *ctx);
...
public:
  ...
  CrtImage(const cl_image_format *image_format, const cl_image_desc *image_desc,
           cl_mem_flags flags, void *host_ptr, CrtContext *ctx);

  CrtImage(cl_mem_flags flags, CrtContext *ctx);
...
public:
  CrtGLImage(cl_uint dim_count, cl_mem_flags flags, GLenum texture_target,
             GLint mipleve, GLuint texture, CrtContext *ctx);
...
public:
  CrtPipe(const cl_uint packetSize, const cl_uint maxPackets,
          const cl_pipe_properties *properties, cl_mem_flags flags,
          CrtContext *ctx);
...
public:
  ...
  void Remove(const void *pSVMPtr) { m_svmAllocations.Remove(pSVMPtr); }
...
public:
  CrtContext(cl_context context_handle, const cl_context_properties *properties,
             cl_uint num_devices, const cl_device_id *devices,
             ctxt_logging_fn pfn_notify, void *user_data, cl_int *errcode_ret);

  virtual ~CrtContext();

  // Memory APIs
  cl_int CreateBuffer(cl_mem_flags flags, size_t size, void *host_ptr,
                      CrtMemObject **memObj);

  cl_int CreateGLBuffer(bool isRender, cl_mem_flags flags, GLuint bufobj,
                        CrtMemObject **memObj);

  cl_int CreateSubBuffer(_cl_mem_crt *parent_buffer, cl_mem_flags flags,
                         cl_buffer_create_type buffer_create_type,
                         const void *buffer_create_info, CrtMemObject **memObj);

  cl_int CreateImage(cl_mem_flags flags, const cl_image_format *image_format,
                     const cl_image_desc *image_desc, void *host_ptr,
                     CrtMemObject **memObj);

  cl_int CreateGLImage(cl_uint dim_count, cl_mem_flags flags, cl_GLenum target,
                       cl_GLint miplevel, cl_GLuint texture,
                       CrtMemObject **memObj);

#ifdef _WIN32
  cl_int CreateFromDX9MediaSurface(cl_mem_flags flags,
                                   IDirect3DSurface9 *resource,
                                   HANDLE sharehandle, UINT plane,
                                   CrtMemObject **memObj);
#endif
  cl_int CreateSampler(cl_bool normalized_coords,
                       cl_addressing_mode addressing_mode,
                       cl_filter_mode filter_mode, CrtSampler **sampler);

  cl_int
  clCreateSamplerWithProperties(const cl_sampler_properties *sampler_properties,
                                CrtSampler **sampler);

  cl_int CreatePipe(cl_mem_flags flags, cl_uint pipe_packet_size,
                    cl_uint pipe_max_packets,
                    const cl_pipe_properties *properties,
                    CrtMemObject **memObj);

  // Command Queue and Build
  cl_int CreateCommandQueue(cl_command_queue queue_crt_handle,
                            cl_device_id device,
                            cl_command_queue_properties properties,
                            CrtQueue **crtQueue);

  cl_int SetDefaultDeviceCommandQueue(cl_device_id device,
                                      cl_command_queue queue_crt_handle);

  // Command Queue wit Properties and Build
  cl_int CreateCommandQueueWithProperties(cl_command_queue queue_crt_handle,
                                          cl_device_id device,
                                          const cl_queue_properties *properties,
                                          CrtQueue **crtQueue);

  void PushCommandQueue(CrtQueue *pCrtQueue);

  cl_command_queue GetDevCommandQueue(cl_command_queue queue_dev_handle);

  void RemoveCommandQueue(CrtQueue *pCrtQueue);

  cl_int CreateProgramWithSource(cl_uint count, const char **strings,
                                 const size_t *lengths,
                                 CrtProgram **crtProgram);

  cl_int CreateProgramWithIL(const void *il, size_t lengths,
                             CrtProgram **crtProgram);

  cl_int
  CreateProgramWithBinary(cl_uint num_devices, const cl_device_id *device_list,
                          const size_t *lengths, const unsigned char **binaries,
                          cl_int *binary_status, CrtProgram **crtProgram);

  cl_int CreateProgramWithBuiltInKernels(cl_uint num_devices,
  ...
  bool memObjectAlwaysInSync(CrtObject::CrtObjectType objType) const {
    return true;
  }
  bool memObjectAlwaysNotInSync(CrtObject::CrtObjectType objType) const {
    return false;
  }
  ...
  inline cl_context GetContextByDeviceID(cl_device_id devID) {
    return m_DeviceToContext[devID];
  }

  // Returns in (outDevices, outNumDevices) the devices which belong
  // to the input platform id (pId)
  void GetDevicesByPlatformId(const cl_uint inNumDevices,
                              const cl_device_id *inDevices,
                              const cl_platform_id &pId, cl_uint *outNumDevices,
                              cl_device_id *outDevices);

  void GetDevsIndicesByPlatformId(const cl_uint inNumDevices,
                                  const cl_device_id *inDevices,
                                  const cl_platform_id &pId,
                                  cl_uint *outNumIndices, cl_uint *outIndices);
  ...
  // We need to keep track of all host command queue for
...
struct CrtBuildCallBackData {
  CrtBuildCallBackData(cl_program prog, prog_logging_fn pfn_notify,
                       void *user_data)
      : m_pfnNotify(pfn_notify), m_userData(user_data), m_refCount(1),
        m_numBuild(0), m_clProgramHandle(prog){};
...
void CL_CALLBACK CrtSetEventCallBack(cl_event e, cl_int status,
                                     void *user_data);
...
class SyncManager {
public:
  enum SYNC_WAY { SYNC_FROM_BACKING_STORE, SYNC_TO_BACKING_STORE };

  SyncManager();
  ~SyncManager();
  cl_int PrepareToExecute(CrtQueue *queue, cl_uint NumEventsInWaitList,
                          const cl_event *inEventWaitList,
                          cl_uint *numOutEvents, cl_event **OutEvents);

  cl_int EnqueueNopCommand(CrtMemObject *memObj, CrtQueue *queue,
                           cl_uint NumEventsInWaitList,
                           const cl_event *inEventWaitList, cl_event *outEvent);
...
void CL_CALLBACK SVMFreeCallbackFunction(cl_event event, cl_int status,
                                         void *user_data);
