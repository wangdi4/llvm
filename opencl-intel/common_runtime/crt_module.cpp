...
crt_err_code CrtModule::PatchClDeviceID(cl_device_id &inDeviceId) {
  inDeviceId->dispatch->clCreateContext =
      crt_ocl_module.m_icdDispatchMgr.m_icdDispatchTable.clCreateContext;
  inDeviceId->dispatch->clGetDeviceInfo =
      crt_ocl_module.m_icdDispatchMgr.m_icdDispatchTable.clGetDeviceInfo;
  inDeviceId->dispatch->clCreateSubDevices =
      crt_ocl_module.m_icdDispatchMgr.m_icdDispatchTable.clCreateSubDevices;
  inDeviceId->dispatch->clReleaseDevice =
      crt_ocl_module.m_icdDispatchMgr.m_icdDispatchTable.clReleaseDevice;
  inDeviceId->dispatch->clRetainDevice =
      crt_ocl_module.m_icdDispatchMgr.m_icdDispatchTable.clRetainDevice;

  inDeviceId->dispatch->clGetDeviceAndHostTimer =
      crt_ocl_module.m_icdDispatchMgr.m_icdDispatchTable
          .clGetDeviceAndHostTimer;
  inDeviceId->dispatch->clGetHostTimer =
      crt_ocl_module.m_icdDispatchMgr.m_icdDispatchTable.clGetHostTimer;

  return CRT_SUCCESS;
}

crt_err_code
CrtModule::PatchClContextID(cl_context &inContextId,
                            KHRicdVendorDispatch *origDispatchTable) {
  inContextId->dispatch->clGetContextInfo =
      crt_ocl_module.m_icdDispatchMgr.m_icdDispatchTable.clGetContextInfo;
  inContextId->dispatch->clReleaseContext =
      crt_ocl_module.m_icdDispatchMgr.m_icdDispatchTable.clReleaseContext;
  inContextId->dispatch->clRetainContext =
      crt_ocl_module.m_icdDispatchMgr.m_icdDispatchTable.clRetainContext;
  return CRT_SUCCESS;
}
...
bool OCLCRT::isSupportedContextType(const cl_context_properties *properties,
  ...
  for (cl_uint i = 0; i < num_devices; i++) {
    ...
    cl_int error = clGetDeviceInfo(devices[i], CL_DEVICE_TYPE,
                                   sizeof(deviceType), &deviceType, NULL);
    if (CL_SUCCESS != error || (CL_DEVICE_TYPE_CPU != deviceType &&
...
crt_err_code CrtModule::Initialize() {
  ...
  if (m_initializeState == NOT_INITIALIZED) {
    ...
    for (cl_uint i = 0; i < m_crtConfig.getNumPlatforms(); i++) {
      ...
      if ((libName.find("igdrcl") != std::string::npos) &&
          (GetWinVer() >= OS_WIN_RS)) {
      ...
      KHRpfn_clGetExtensionFunctionAddress clGetExtFuncAddr =
          (KHRpfn_clGetExtensionFunctionAddress)pCrtPlatform->m_lib
              .GetFunctionPtrByName("clGetExtensionFunctionAddress");
      if (NULL == clGetExtFuncAddr) {
        res = CRT_FAIL;
        break;
      }
      KHRpfn_clGetPlatformIDs pfn_clGetPlatformIDs =
          (KHRpfn_clGetPlatformIDs)(ptrdiff_t)clGetExtFuncAddr(
              "clIcdGetPlatformIDsKHR");
      if (NULL == pfn_clGetPlatformIDs ||
          !(CL_SUCCESS ==
            pfn_clGetPlatformIDs(1, &pCrtPlatform->m_platformIdDEV, NULL))) {
        delete pCrtPlatform;
        continue;
      }
      ...
      size_t extSize = 0;
      if (CL_SUCCESS !=
          pCrtPlatform->m_platformIdDEV->dispatch->clGetPlatformInfo(
              pCrtPlatform->m_platformIdDEV, CL_PLATFORM_EXTENSIONS, 0, NULL,
              &extSize)) {
        res = CRT_FAIL;
        break;
      }
      ...
      if (CL_SUCCESS !=
          pCrtPlatform->m_platformIdDEV->dispatch->clGetPlatformInfo(
              pCrtPlatform->m_platformIdDEV, CL_PLATFORM_EXTENSIONS, extSize,
              pCrtPlatform->m_supportedExtensionsStr, NULL)) {
        res = CRT_FAIL;
        break;
      }
      pCrtPlatform->m_supportedExtensions =
          GetCrtExtension(pCrtPlatform->m_supportedExtensionsStr);

      // Query the ICD Suffix for each platform
      if (CL_SUCCESS !=
          pCrtPlatform->m_platformIdDEV->dispatch->clGetPlatformInfo(
              pCrtPlatform->m_platformIdDEV, CL_PLATFORM_ICD_SUFFIX_KHR, 0,
              NULL, &extSize)) {
        res = CRT_FAIL;
        break;
      }
      ...
      if (CL_SUCCESS !=
          pCrtPlatform->m_platformIdDEV->dispatch->clGetPlatformInfo(
              pCrtPlatform->m_platformIdDEV, CL_PLATFORM_ICD_SUFFIX_KHR,
              extSize, pCrtPlatform->m_icdSuffix, NULL)) {
        res = CRT_FAIL;
        break;
      }
      ...
      if (CL_SUCCESS !=
          pCrtPlatform->m_platformIdDEV->dispatch->clGetPlatformInfo(
              pCrtPlatform->m_platformIdDEV, CL_PLATFORM_VERSION, MAX_STRLEN,
              platformVersionStr, NULL)) {
        res = CRT_FAIL;
        break;
      }
      ...
      pCrtPlatform->m_platformIdDEV->dispatch->clGetDeviceIDs(
          pCrtPlatform->m_platformIdDEV, CL_DEVICE_TYPE_ALL, 0, NULL,
          &num_devices);
      if (num_devices == 0) {
        res = CRT_FAIL;
        break;
      ...
      if (!(CL_SUCCESS ==
            pCrtPlatform->m_platformIdDEV->dispatch->clGetDeviceIDs(
                pCrtPlatform->m_platformIdDEV, CL_DEVICE_TYPE_ALL, num_devices,
                pDevices, NULL))) {
        res = CRT_FAIL;
        break;
      }
      ...
      for (cl_uint j = 0; j < num_devices; j++) {
        ...
        if (CL_SUCCESS != pDevices[j]->dispatch->clGetDeviceInfo(
                              pDevices[j], CL_DEVICE_EXECUTION_CAPABILITIES,
                              sizeof(pDevInfo->m_deviceCapabilities),
                              &pDevInfo->m_deviceCapabilities, 0)) {
          res = CRT_FAIL;
          break;
        }

        if (CL_SUCCESS != pDevices[j]->dispatch->clGetDeviceInfo(
                              pDevices[j], CL_DEVICE_TYPE,
                              sizeof(cl_device_type), &pDevInfo->m_devType,
                              NULL)) {
          res = CRT_FAIL;
          break;
        }
...
cl_device_id CrtModule::GetDeviceByType(cl_device_type device_type) {
  ...
  for (OCLCRT::DEV_INFO_MAP::const_iterator
           itr = m_deviceInfoMapGuard.get().begin(),
           end = m_deviceInfoMapGuard.get().end();
       itr != end; ++itr) {
...
crt_err_code OCLCRT::ReplacePlatformId(const cl_context_properties *properties,
                                       cl_platform_id &pId,
...
void CrtModule::Shutdown() {
  ...
  for (DEV_INFO_MAP::iterator itr = m_deviceInfoMap.begin();
       itr != m_deviceInfoMap.end(); itr++) {
    if (itr->second) {
      delete itr->second;
      itr->second = NULL;
    }
