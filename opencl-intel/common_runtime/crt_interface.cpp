...
// than application's destructors are complete. Hence we should allocate
// CrtModule on heap to manage it's lifetime. The CrtModule will be deleted on
// the latest step of unload this library.
...
#define isValidPlatform(X)                                                     \
  ((X) == OCLCRT::crt_ocl_module.m_crtPlatformId || NULL == (X))
...
void UpdatePlatformExtensions() {
  if (NULL == OCLCRT::crt_ocl_module.m_common_extensions) {
    ...
    if (NULL == OCLCRT::crt_ocl_module.m_common_extensions) {
      ...
      for (cl_uint i = 0; i < OCLCRT::crt_ocl_module.m_oclPlatforms.size();
           i++) {
        ...
        std::vector<std::string>::iterator it = std::set_intersection(
            common_exts.begin(), common_exts.end(), platExt_vec.begin(),
            platExt_vec.end(), common_exts.begin());

        common_exts.erase(it, common_exts.end());
      }
      std::string ret = std::accumulate(common_exts.begin(), common_exts.end(),
                                        std::string(""), concat_spaced_strings);
      OCLCRT::crt_ocl_module.m_common_extensions = new char[ret.size() + 1];
      if (OCLCRT::crt_ocl_module.m_common_extensions != NULL) {
        strcpy_s(OCLCRT::crt_ocl_module.m_common_extensions, ret.size() + 1,
                 ret.c_str());
      }
    }
  }
...
cl_int CL_API_CALL clGetPlatformInfo(cl_platform_id platform,
  ...
  case CL_PLATFORM_VERSION:
    ...
    case OPENCL_1_1:
      crtVersion.assign(INTEL_OPENCL_1_1_PVER_STR,
                        strlen(INTEL_OPENCL_1_1_PVER_STR));
      break;
    case OPENCL_1_2:
      crtVersion.assign(INTEL_OPENCL_1_2_PVER_STR,
                        strlen(INTEL_OPENCL_1_2_PVER_STR));
      break;
    case OPENCL_2_0:
      crtVersion.assign(INTEL_OPENCL_2_0_PVER_STR,
                        strlen(INTEL_OPENCL_2_0_PVER_STR));
      break;
    case OPENCL_2_1:
      crtVersion.assign(INTEL_OPENCL_2_1_PVER_STR,
                        strlen(INTEL_OPENCL_2_1_PVER_STR));
      break;
    }
  ...
  case CL_PLATFORM_EXTENSIONS:
    ...
    if (param_value) {
      ...
      strncpy_s((char *)param_value, RetSize,
                OCLCRT::crt_ocl_module.m_common_extensions, RetSize);
    }
    break;
  case CL_PLATFORM_ICD_SUFFIX_KHR:
    ...
    if (param_value) {
      ...
      strncpy_s((char *)param_value, RetSize, INTEL_ICD_EXTENSIONS_STRING,
                RetSize);
    }
    break;
  case CL_PLATFORM_HOST_TIMER_RESOLUTION: {
    ...
    OCLCRT::DEV_INFO_MAP::const_iterator itr =
        OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.get().begin();
    cl_platform_id devicePlatform = itr->second->m_crtPlatform->m_platformIdDEV;

    return devicePlatform->dispatch->clGetPlatformInfo(
        devicePlatform, CL_PLATFORM_HOST_TIMER_RESOLUTION, param_value_size,
...
cl_int CL_API_CALL clGetDeviceIDs(cl_platform_id platform,
  ...
  for (OCLCRT::DEV_INFO_MAP::const_iterator itr =
           OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.get().begin();
       itr != OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.get().end(); itr++) {
    ...
    bool devFound =
        ((1 == OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.size()) ||
    ...
    if (((device_type == CL_DEVICE_TYPE_DEFAULT) && devFound) ||
      ...
      if (devices && (numRet < num_entries)) {
        ...
        if (devType == OCLCRT::crt_ocl_module.m_defaultDeviceType &&
            num_entries > 0) {
...
cl_context CL_API_CALL clCreateContext(const cl_context_properties *properties,
  ...
  for (cl_uint i = 0; i < num_devices; i++) {
    ...
    if ((NULL == devIdDEV) ||
        (NULL ==
         OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.GetValue(devIdDEV))) {
  ...
  if ((errCode == CL_SUCCESS) && (numPlatforms == 1)) {
    ...
    const KHRicdVendorDispatch *dTable =
        (KHRicdVendorDispatch *)(&OCLCRT::crt_ocl_module.m_deviceInfoMapGuard
                                      .GetValue(devices[0])
                                      ->m_origDispatchTable);
    ...
    if (CRT_FAIL ==
        OCLCRT::ReplacePlatformId(
            properties,
            OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.GetValue(devices[0])
                ->m_crtPlatform->m_platformIdDEV,
            &props)) {
      ...
      ctx = dTable->clCreateContext(props, num_devices, devices, pfn_notify,
                                    user_data, &errCode);
      ...
      if (ctx != NULL) {
        ...
        if (isGPUPlatform) {
          pContextInfo->m_contextType =
              CrtContextInfo::SinglePlatformGPUContext;
        } else {
          pContextInfo->m_contextType =
              CrtContextInfo::SinglePlatformCPUContext;
        }

        pContextInfo->m_crtPlatform =
            OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.GetValue(devices[0])
                ->m_crtPlatform;
        pContextInfo->m_object = (void *)(&(
            OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.GetValue(devices[0])
                ->m_origDispatchTable));
    ...
    CrtContext *crtContext = new CrtContext(
        ctx, properties, num_devices, devices, pfn_notify, user_data, &errCode);
...
cl_context CL_API_CALL clCreateContextFromType(
  ...
  // In case there is only one underlying device, we pick it as
  // CL_DEFAULT_DEVICE_TYPE
  OnlyOneAvailableDevice = (numPlatformDevices == 1);

  for (OCLCRT::DEV_INFO_MAP::iterator itr =
           OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.get().begin();
       itr != OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.get().end(); itr++) {
    CrtDeviceInfo *devInfo = itr->second;
    if (devInfo->m_isRootDevice == false) {
  ...
  if (CL_SUCCESS == errCode) {
    if (numDevices == 0) {
      ...
      ctx = clCreateContext(properties, numDevices, deviceList, pfn_notify,
                            user_data, &errCode);
    }
  }
FINISH:
...
cl_int CL_API_CALL clGetContextInfo(cl_context context,
  ...
  CrtContextInfo *ctxInfo =
      OCLCRT::crt_ocl_module.m_contextInfoGuard.GetValue(context);
  if (!ctxInfo) {
  ...
  if (ctxInfo->m_contextType != CrtContextInfo::SharedPlatformContext) {
    ...
    errCode = dTable->clGetContextInfo(context, param_name, param_value_size,
                                       param_value, param_value_size_ret);

    if (errCode == CL_SUCCESS && param_name == CL_CONTEXT_PROPERTIES) {
      // replace the underlying platform_id with the CRT platform id
      OCLCRT::ReplacePlatformId((cl_context_properties *)param_value,
                                OCLCRT::crt_ocl_module.m_crtPlatformId, NULL,
                                false);
    }
  } else {
    ...
    case CL_CONTEXT_REFERENCE_COUNT: {
      ...
      if (OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.GetValue(itr->first) ==
          NULL) {
        errCode = CL_INVALID_VALUE;
        break;
      }
      ...
      errCode =
          OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.GetValue(itr->first)
              ->m_origDispatchTable.clGetContextInfo(itr->second, param_name,
                                                     param_value_size,
                                                     param_value, &pValueSize);
...
cl_int CL_API_CALL clGetGLContextInfoKHR(
  ...
  for (itr = OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.get().begin();
    ...
    if (CRT_FAIL ==
        OCLCRT::ReplacePlatformId(
            properties, itr->second->m_crtPlatform->m_platformIdDEV, &props)) {
      errCode = CL_OUT_OF_HOST_MEMORY;
      goto FINISH;
    }
    ...
    case CL_CURRENT_DEVICE_FOR_GL_CONTEXT_KHR:
      if (((OCLCRT::crt_ocl_module.m_availableDeviceTypes &
            CL_DEVICE_TYPE_GPU) &&
           (itr->second->m_devType != CL_DEVICE_TYPE_GPU)) ||
          (itr->second->m_isRootDevice == false)) {
        continue;
      }
      errCode = itr->second->m_crtPlatform->m_platformIdDEV->dispatch
                    ->clGetGLContextInfoKHR(props, param_name, param_value_size,
                                            param_value, &total_value_size_ret);
    ...
    case CL_DEVICES_FOR_GL_CONTEXT_KHR:
      ...
      if (errCode == CL_SUCCESS) {
        ...
        if ((param_value != NULL) &&
            (total_value_size_ret > param_value_size)) {
          errCode = CL_INVALID_VALUE;
          goto FINISH;
        }
      } else {
...

{
  ...
  case CL_DEVICE_SVM_CAPABILITIES: {
    // When CPU and GPU are available we ask GPU for SVM capabilities due to GPU
    // will own SVM memory,
    ...
    if (deviceInfo != NULL) {
      retCode = deviceInfo->m_origDispatchTable.clGetDeviceInfo(
          device, param_name, param_value_size, param_value,
          param_value_size_ret);
    }
  } break;
  default:
    retCode = devInfo->m_origDispatchTable.clGetDeviceInfo(
        device, param_name, param_value_size, param_value,
        param_value_size_ret);

    if ((retCode == CL_SUCCESS) && (param_name == CL_DEVICE_PLATFORM) &&
        (param_value)) {
      memcpy_s(param_value, sizeof(cl_platform_id),
               &OCLCRT::crt_ocl_module.m_crtPlatformId, sizeof(cl_platform_id));
    }
  }
...
CL_API_ENTRY cl_int CL_API_CALL clGetDeviceAndHostTimer(
  ...
  if (errCode == CL_SUCCESS) {
    if (NULL == devInfo) {
      ...
      errCode = devInfo->m_origDispatchTable.clGetDeviceAndHostTimer(
...
CL_API_ENTRY cl_int CL_API_CALL clGetHostTimer(cl_device_id device,
  ...
  if (errCode == CL_SUCCESS) {
    if (NULL == devInfo) {
      ...
      errCode =
          devInfo->m_origDispatchTable.clGetHostTimer(device, host_timestamp);
    }
  }
...
cl_command_queue CL_API_CALL clCreateCommandQueueWithProperties(
  ...
  ctx = (CrtContext *)(ctxInfo->m_object);
  errCode = ctx->CreateCommandQueueWithProperties(queue_handle, device,
                                                  properties, &queue);
  if (CL_SUCCESS != errCode) {
    delete queue_handle;
    queue_handle = NULL;
  ...
  case CL_QUEUE_DEVICE_DEFAULT: {
    ...
    if (param_value && param_value_size >= pValueSize) {
      errCode = crtQueue->m_cmdQueueDEV->dispatch->clGetCommandQueueInfo(
          crtQueue->m_cmdQueueDEV, param_name, param_value_size, param_value,
          param_value_size_ret);
      if (errCode == CL_SUCCESS) {
        cl_command_queue dev_command_q = *((cl_command_queue *)param_value);
        cl_command_queue crt_handle =
            crtQueue->m_contextCRT->GetDevCommandQueue(dev_command_q);
...
cl_mem CL_API_CALL clCreateBuffer(cl_context context, cl_mem_flags flags,
  ...
  ctx = (CrtContext *)(ctxInfo->m_object);
  errCode = ctx->CreateBuffer(flags, size, host_ptr,
...
cl_mem CL_API_CALL clCreateSubBuffer(cl_mem buffer, cl_mem_flags flags,
  ...
  errCode = crtBuffer->m_pContext->CreateSubBuffer(
      (_cl_mem_crt *)buffer, flags, buffer_create_type, buffer_create_info,
      (CrtMemObject **)(&mem_handle->object));
...
inline cl_int CL_API_CALL EnqueueReadWriteBuffer(
    bool read_command, cl_command_queue command_queue, cl_mem buffer,
    cl_bool blocking_cmd, size_t offset, size_t cb, const void *ptr,
  ...
  CrtQueue *queue = reinterpret_cast<CrtQueue *>(
      ((_cl_command_queue_crt *)command_queue)->object);
  if (!queue) {
    return CL_INVALID_COMMAND_QUEUE;
  }
  ...
  CrtBuffer *crtBuffer =
      reinterpret_cast<CrtBuffer *>(((_cl_mem_crt *)buffer)->object);
  if (!crtBuffer) {
    errCode = CL_INVALID_MEM_OBJECT;
    goto FINISH;
  }
  ...
  errCode =
      synchHelper->PrepareToExecute(queue, num_events_in_wait_list,
                                    event_wait_list, &numOutEvents, &outEvents);
  ...
  if (read_command) {
    errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueReadBuffer(
        queue->m_cmdQueueDEV, crtBuffer->getDeviceMemObj(queue->m_device),
        blocking_cmd, offset, cb, const_cast<void *>(ptr), numOutEvents,
        outEvents, crtEvent ? &crtEvent->m_eventDEV : NULL);
  } else {
    errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueWriteBuffer(
        queue->m_cmdQueueDEV, crtBuffer->getDeviceMemObj(queue->m_device),
        blocking_cmd, offset, cb, ptr, numOutEvents, outEvents,
...
cl_int CL_API_CALL clEnqueueReadBuffer(cl_command_queue command_queue,
                                       ...
                                       cl_event *event) {
  return EnqueueReadWriteBuffer(true, command_queue, buffer, blocking_write,
...
cl_int CL_API_CALL clEnqueueWriteBuffer(
  ...
  return EnqueueReadWriteBuffer(false, command_queue, buffer, blocking_write,
                                offset, cb, ptr, num_events_in_wait_list,
                                event_wait_list, event);
}
...
cl_int CL_API_CALL EnqueueReadWriteBufferRect(
    bool read_command, cl_command_queue command_queue, cl_mem buffer,
    cl_bool blocking_cmd, const size_t *buffer_origin,
  ...
  if (read_command) {
    errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueReadBufferRect(
        queue->m_cmdQueueDEV, crtBuffer->getDeviceMemObj(queue->m_device),
        blocking_cmd, buffer_origin, host_origin, region, buffer_row_pitch,
        buffer_slice_pitch, host_row_pitch, host_slice_pitch,
        const_cast<void *>(ptr), numOutEvents, outEvents,
        crtEvent ? &crtEvent->m_eventDEV : NULL);
  } else {
    errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueWriteBufferRect(
        queue->m_cmdQueueDEV, crtBuffer->getDeviceMemObj(queue->m_device),
        blocking_cmd, buffer_origin, host_origin, region, buffer_row_pitch,
        buffer_slice_pitch, host_row_pitch, host_slice_pitch, ptr, numOutEvents,
        outEvents, crtEvent ? &crtEvent->m_eventDEV : NULL);
  }
...
cl_int CL_API_CALL clEnqueueReadBufferRect(
  ...
  return EnqueueReadWriteBufferRect(
      true, command_queue, buffer, blocking_read, buffer_origin, host_origin,
...
cl_int CL_API_CALL clEnqueueWriteBufferRect(
    ...
    cl_event *event) {
  return EnqueueReadWriteBufferRect(
      false, command_queue, buffer, blocking_write, buffer_origin, host_origin,
      region, buffer_row_pitch, buffer_slice_pitch, host_row_pitch,
      host_slice_pitch, ptr, num_events_in_wait_list, event_wait_list, event);
}
...
cl_int CL_API_CALL clEnqueueCopyBufferRect(
  ...
  CrtBuffer *crtSrcBuffer =
      reinterpret_cast<CrtBuffer *>(((_cl_mem_crt *)src_buffer)->object);
  CrtBuffer *crtDstBuffer =
      reinterpret_cast<CrtBuffer *>(((_cl_mem_crt *)dst_buffer)->object);
  ...
  errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueCopyBufferRect(
      queue->m_cmdQueueDEV, crtSrcBuffer->getDeviceMemObj(queue->m_device),
      crtDstBuffer->getDeviceMemObj(queue->m_device), src_origin, dst_origin,
      region, src_row_pitch, src_slice_pitch, dst_row_pitch, dst_slice_pitch,
      numOutEvents, outEvents, crtEvent ? &crtEvent->m_eventDEV : NULL);
...
cl_int CL_API_CALL clEnqueueCopyBuffer(cl_command_queue command_queue,
  ...
  errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueCopyBuffer(
      queue->m_cmdQueueDEV, crtSrcBuffer->getDeviceMemObj(queue->m_device),
      crtDstBuffer->getDeviceMemObj(queue->m_device), src_offset, dst_offset,
      cb, numOutEvents, outEvents, crtEvent ? &crtEvent->m_eventDEV : NULL);
...
void *CL_API_CALL clEnqueueMapBuffer(cl_command_queue command_queue,
  ...
  queue = reinterpret_cast<CrtQueue *>(
      ((_cl_command_queue_crt *)command_queue)->object);
  if (!queue) {
    errCode = CL_INVALID_COMMAND_QUEUE;
    goto FINISH;
  ...
  if ((map_flags & CL_MAP_WRITE) &&
      (crtBuffer->m_flags & CL_MEM_HOST_READ_ONLY)) {
    errCode = CL_INVALID_MEM_OBJECT;
    goto FINISH;
  }

  if ((map_flags & CL_MAP_WRITE) &&
      (crtBuffer->m_flags & CL_MEM_HOST_NO_ACCESS)) {
    errCode = CL_INVALID_MEM_OBJECT;
    goto FINISH;
  }

  if ((map_flags & CL_MAP_READ) &&
      (crtBuffer->m_flags & CL_MEM_HOST_WRITE_ONLY)) {
    errCode = CL_INVALID_MEM_OBJECT;
    goto FINISH;
  }
  ...
  if (!crtBuffer->isInteropObject()) {
    ...
    if (crtBuffer->HasPrivateCopy() && (crtBuffer->m_mapCount <= 1) &&
      ...
      errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueReadBuffer(
          queue->m_cmdQueueDEV, devMemObj, blocking_map, 0, crtBuffer->m_size,
          crtBuffer->m_pUsrPtr, numOutEvents, outEvents,
          crtEvent ? &crtEvent->m_eventDEV : NULL);
    } else {
      if (event) {
        errCode = synchHelper->EnqueueNopCommand(
            crtBuffer, queue, numOutEvents, outEvents,
            crtEvent ? &crtEvent->m_eventDEV : NULL);
      }

      if (blocking_map) {
        errCode =
            queue->m_cmdQueueDEV->dispatch->clFinish(queue->m_cmdQueueDEV);
        if (CL_SUCCESS != errCode) {
          errCode = CL_OUT_OF_RESOURCES;
          goto FINISH;
        }
    ...
    queue->m_cmdQueueDEV->dispatch->clEnqueueMapBuffer(
        queue->m_cmdQueueDEV, devMemObj, blocking_map, map_flags, offset, cb,
        numOutEvents, outEvents, crtEvent ? &crtEvent->m_eventDEV : NULL,
        &errCode);
  }
...
void *CL_API_CALL clEnqueueMapImage(
  ...
  if ((map_flags & CL_MAP_WRITE) &&
      (crtImage->m_flags & CL_MEM_HOST_READ_ONLY)) {
    errCode = CL_INVALID_MEM_OBJECT;
    goto FINISH;
  }

  if ((map_flags & CL_MAP_WRITE) &&
      (crtImage->m_flags & CL_MEM_HOST_NO_ACCESS)) {
    errCode = CL_INVALID_MEM_OBJECT;
    goto FINISH;
  }

  if ((map_flags & CL_MAP_READ) &&
      (crtImage->m_flags & CL_MEM_HOST_WRITE_ONLY)) {
    errCode = CL_INVALID_MEM_OBJECT;
    goto FINISH;
  }
  ...
  if (!crtImage->isInteropObject()) {
    // The pitch for CRT Backing store may be different than that for the
    // host_ptr So we need to report the correct host_ptr pitch
    if (image_row_pitch) {
      *image_row_pitch = crtImage->m_hostPtrRowPitch;
    }
    if (image_slice_pitch) {
      if ((crtImage->m_imageDesc.desc.image_type == CL_MEM_OBJECT_IMAGE3D) ||
          (crtImage->m_imageDesc.desc.image_type ==
    ...
    if (crtImage->HasPrivateCopy() && (crtImage->m_mapCount <= 1) &&
      ...
      errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueReadImage(
          queue->m_cmdQueueDEV, devMemObj, blocking_map, origin, region,
          crtImage->m_hostPtrRowPitch, crtImage->m_hostPtrSlicePitch, ptr,
          numOutEvents, outEvents, crtEvent ? &crtEvent->m_eventDEV : NULL);
    } else {
      if (event) {
        errCode = synchHelper->EnqueueNopCommand(
            crtImage, queue, numOutEvents, outEvents,
            crtEvent ? &crtEvent->m_eventDEV : NULL);
      }
    ...
    queue->m_cmdQueueDEV->dispatch->clEnqueueMapImage(
        queue->m_cmdQueueDEV, devMemObj, blocking_map, map_flags, origin,
        region, image_row_pitch, image_slice_pitch, numOutEvents, outEvents,
        crtEvent ? &crtEvent->m_eventDEV : NULL, &errCode);
  }
...
cl_int CL_API_CALL clEnqueueUnmapMemObject(cl_command_queue command_queue,
  ...
  if (!crtMemObj->isInteropObject()) {
    itr = std::find(crtMemObj->m_mappedPointers.begin(),
                    crtMemObj->m_mappedPointers.end(), mapped_ptr);
    ...
    if (crtMemObj->HasPrivateCopy()) {
      if (crtMemObj->getObjectType() == CrtObject::CL_IMAGE) {
        ...
        std::vector<size_t> mappedRegion =
            crtImage->GetMappedRegion(mapped_ptr);
        std::vector<size_t> mappedOrigin =
            crtImage->GetMappedOrigin(mapped_ptr);
        ...
        errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueWriteImage(
            queue->m_cmdQueueDEV, crtMemObj->getDeviceMemObj(queue->m_device),
            false, &mappedOrigin[0], &mappedRegion[0],
            crtImage->m_hostPtrRowPitch, crtImage->m_hostPtrSlicePitch,
            mapped_ptr, numOutEvents, outEvents,
            crtEvent ? &crtEvent->m_eventDEV : NULL);
      } else {
        // currently we copy all buffer contents and not only the mapped region.
        // copying the mapped region only will be an optimization for the future
        ...
        errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueWriteBuffer(
            queue->m_cmdQueueDEV, crtMemObj->getDeviceMemObj(queue->m_device),
            false, 0, crtBuffer->m_size, crtBuffer->m_pUsrPtr, numOutEvents,
            outEvents, crtEvent ? &crtEvent->m_eventDEV : NULL);
      }
    } else if (event) {
      errCode = synchHelper->EnqueueNopCommand(
          crtMemObj, queue, numOutEvents, outEvents,
          crtEvent ? &crtEvent->m_eventDEV : NULL);
    }
  } else {
...
cl_int CL_API_CALL clReleaseContext(cl_context context) {
  ...
  if (ctxInfo->m_contextType != CrtContextInfo::SharedPlatformContext) {
    ...
    errCode = dTable->clGetContextInfo(context, CL_CONTEXT_REFERENCE_COUNT,
...
cl_int CL_API_CALL clRetainCommandQueue(cl_command_queue command_queue) {
  ...
  queue->IncRefCnt();
  // I don't forward retains to the underlying platforms, i only send release
  // when the CRT ref counter for this queue reaches zero
  return errCode;
}
...
cl_int CL_API_CALL clReleaseMemObject(cl_mem memobj) {
  ...
  CrtMemObject *crtMemObj =
      reinterpret_cast<CrtMemObject *>(((_cl_mem_crt *)memobj)->object);
...
cl_int CL_API_CALL clRetainMemObject(cl_mem memobj) {
  ...
  crtMemObj->IncRefCnt();
  // I don't forward retains to the underlying platforms, i only send release
  // when the CRT ref counter for this mem objects reaches zero
  return CL_SUCCESS;
}
...
cl_program CL_API_CALL clCreateProgramWithSource(cl_context context,
  ...
  // Shared context.
  errCode = ctx->CreateProgramWithSource(count, strings, lengths, &pgm);
  if (CL_SUCCESS == errCode) {
...
cl_program CL_API_CALL clCreateProgramWithIL(cl_context context, const void *il,
  ...
  // Shared context.
  errCode = ctx->CreateProgramWithIL(il, lengths, &pgm);
  if (CL_SUCCESS == errCode) {
    SharedProgram = new _cl_program_crt;
    if (SharedProgram == NULL) {
      pgm->Release();
...
cl_program CL_API_CALL clCreateProgramWithBinary(
  ...
  if (NULL == device_list || 0 == num_devices || NULL == lengths ||
      NULL == binaries) {
    errCode = CL_INVALID_VALUE;
    goto FINISH;
  }
  ...
  errCode = ctx->CreateProgramWithBinary(num_devices, device_list, lengths,
                                         binaries, binary_status, &pgm);
...
CL_API_ENTRY cl_program CL_API_CALL clCreateProgramWithBuiltInKernels(
  ...
  if (OCLCRT::crt_ocl_module.m_CrtPlatformVersion < OPENCL_1_2) {
    ...
    errCode = ctx->CreateProgramWithBuiltInKernels(num_devices, device_list,
                                                   kernel_names, &pgm);
...
///
/// ------------------------------------------------------------------------------
cl_bool isRequiredObjAvailable(cl_device_id devId,
  ...
  for (cl_uint i = 0; i < in_programs.size(); i++) {
    ...
    cl_int errCode = devId->dispatch->clGetProgramBuildInfo(
        pgm, devId, CL_PROGRAM_BINARY_TYPE, sizeof(cl_program_binary_type),
        &pgm_bin_type, NULL);
...
///
/// ------------------------------------------------------------------------------
cl_int getAssocDevices(const cl_device_id *inDevList, cl_device_id **outDevList,
                       std::vector<CrtProgram *> &in_programs,
                       CrtContext *crtCtx, cl_uint bin_types,
                       cl_uint *outNumDevs) {
  cl_int errCode = CL_SUCCESS;
  cl_uint j = 0;
  ...
  if (NULL != inDevList) {
    ...
    for (cl_uint i = 0; i < *outNumDevs; i++) {
      ...
      if (isRequiredObjAvailable(inDevList[i], in_programs, bin_types)) {
    ...
    for (; itr != crtCtx->m_DeviceToContext.end(); itr++) {
      if (isRequiredObjAvailable(itr->first, in_programs, bin_types)) {
...
///
/// ------------------------------------------------------------------------------
cl_uint getNumRelevantContexts(cl_uint num_devices,
...
///
/// ------------------------------------------------------------------------------
cl_int CL_API_CALL clBuildProgram(cl_program program, cl_uint num_devices,
                                  const cl_device_id *device_list,
                                  const char *options,
                                  prog_logging_fn pfn_notify, void *user_data) {
  std::vector<CrtProgram *> in_programs;
  ...
  CrtProgram *crtProg =
      reinterpret_cast<CrtProgram *>(((_cl_program_crt *)program)->object);
  ...
  errCode = getAssocDevices(device_list, &deviceList, in_programs,
                            crtProg->m_contextCRT, CL_PROGRAM_BINARY_TYPE_NONE,
                            &num_devices);
  ...
  crtData->m_numBuild =
      getNumRelevantContexts(num_devices, deviceList, crtProg->m_contextCRT);
  ...
  for (cl_uint i = 0; i < OCLCRT::crt_ocl_module.m_oclPlatforms.size(); i++) {
    ...
    errCode = prog->dispatch->clBuildProgram(prog, matchDevices, outDevices,
                                             optReflect.c_str(),
                                             buildCompleteFn, crtData);
  ...
  if (!pfn_notify) {
    ...
    for (cl_uint i = 0; i < num_devices; i++) {
      ...
      deviceList[i]->dispatch->clGetProgramBuildInfo(
          crtProg->m_ContextToProgram[ctx], deviceList[i],
          CL_PROGRAM_BUILD_STATUS, sizeof(build_status), &build_status, NULL);
...
CL_API_ENTRY cl_program CL_API_CALL clLinkProgram(
    ...
    const cl_program *input_programs, prog_logging_fn pfn_notify,
    void *user_data, cl_int *errcode_ret) {
  std::vector<CrtProgram *> in_programs;
  ...
  for (cl_uint i = 0; i < num_input_programs; i++) {
    ...
    in_programs.push_back(
        ((CrtProgram *)((_cl_program_crt *)input_programs[i])->object));
  }
  ...
  for (; itr != crtCtx->m_contexts.end(); itr++) {
    ...
    cl_uint matchDevices = 0;
    crtCtx->GetDevicesByPlatformId(num_devices, deviceList, pId, &matchDevices,
                                   outDevices);
    ...
    int p = 0;
    for (std::vector<CrtProgram *>::iterator pItr = in_programs.begin();
         pItr != in_programs.end(); pItr++) {
    ...
    cl_program devPro = itr->second.clLinkProgram(
        itr->first, matchDevices, outDevices, options, num_input_programs,
        devPrograms, buildCompleteFn, crtData, &errCode);
...
CL_API_ENTRY cl_int CL_API_CALL clCompileProgram(
  ...
  for (cl_uint i = 0; i < OCLCRT::crt_ocl_module.m_oclPlatforms.size(); i++) {
    ...
    if (num_input_headers > 0) {
      ...
      if (inHeaders) {
        for (cl_uint j = 0; j < num_input_headers; j++) {
          headProg = reinterpret_cast<CrtProgram *>(
              ((_cl_program_crt *)input_headers[j])->object);
    ...
    errCode = prog->dispatch->clCompileProgram(
        prog, matchDevices, outDevices, optReflect.c_str(), num_input_headers,
        inHeaders, header_include_names, *buildCompleteFn, crtData);
...
cl_int CL_API_CALL clGetProgramBuildInfo(
  ...
  if (param_name == CL_PROGRAM_BUILD_OPTIONS) {
    size_t pValueSize =
        (pgm->m_options.size() == 0) ? 1 : (pgm->m_options.size() + 1);
    if (param_value != NULL) {
      ...
      if (pValueSize > 0) {
        strcpy_s((char *)param_value, pgm->m_options.size() + 1,
                 pgm->m_options.c_str());
      }
    }
    if (param_value_size_ret) {
    ...
    CTX_PGM_MAP::iterator it =
        pgm->m_ContextToProgram.find(ctx->GetContextByDeviceID(device));

    if (it != pgm->m_ContextToProgram.end()) {
      cl_program currPgm = it->second;
      errCode = currPgm->dispatch->clGetProgramBuildInfo(
          currPgm, device, param_name, param_value_size, param_value,
          param_value_size_ret);
      goto FINISH;
    }
...
cl_kernel CL_API_CALL clCreateKernel(cl_program program,
  ...
  for (cl_uint i = 0; i < pgm->m_buildContexts.size(); i++) {
    ...
    cl_kernel knlObj = ctxObj->dispatch->clCreateKernel(
        pgm->m_ContextToProgram[ctxObj], kernel_name, &errCode);
    if (CL_SUCCESS == errCode) {
      crtKernel->m_ContextToKernel[ctxObj] = knlObj;

      // As for now, we only support #difference number of arguments detection
      // for the sake of supporting CL_INVALID_KERNEL_DEFINITION
      if (i == 0) {
        knlObj->dispatch->clGetKernelInfo(knlObj, CL_KERNEL_NUM_ARGS,
                                          sizeof(cl_uint), &refNumKernelArgs,
                                          NULL);
      } else {
        cl_uint numKernelArgs = 0;
        knlObj->dispatch->clGetKernelInfo(
            knlObj, CL_KERNEL_NUM_ARGS, sizeof(cl_uint), &numKernelArgs, NULL);
...
CL_API_ENTRY cl_kernel CL_API_CALL clCloneKernel(cl_kernel source_kernel,
  ...
  crtKernel =
      reinterpret_cast<CrtKernel *>(((_cl_kernel_crt *)source_kernel)->object);

  for (auto &it : crtKernel->m_ContextToKernel) {
    cl_kernel tmpKernel =
        it.first->dispatch->clCloneKernel(it.second, errcode_ret);
...
///
/// ------------------------------------------------------------------------------
cl_int CL_API_CALL clGetKernelInfo(cl_kernel kernel, cl_sampler_info param_name,
                                   size_t param_value_size, void *param_value,
                                   size_t *param_value_size_ret) {
  cl_int retCode = CL_SUCCESS;
  CrtKernel *crtKernel =
      reinterpret_cast<CrtKernel *>(((_cl_kernel_crt *)kernel)->object);
  ...
  case CL_KERNEL_CONTEXT: {
    ...
    if (param_value && param_value_size >= pValueSize) {
      *((cl_context *)param_value) =
          crtKernel->m_programCRT->m_contextCRT->m_context_handle;
    }
  ...
  default: {
    ...
    retCode = kernelObj->dispatch->clGetKernelInfo(
        kernelObj, param_name, param_value_size, param_value,
        param_value_size_ret);
  } break;
  }
  return retCode;
...
cl_int CL_API_CALL clSetKernelArg(cl_kernel kernel, cl_uint arg_index,
  ...
  errCode = ((SOCLEntryPointsTable *)kernelDevObj)
                ->crtDispatch->clGetKernelArgInfo(kernelDevObj, arg_index,
                                                  CL_KERNEL_ARG_TYPE_NAME,
                                                  MAX_STRLEN, paramType, NULL);
  ...
  // conformance 1.2 require the image type be without prefix; as "image"; but
  // we still support 1.1 too; so we have both forms here
  isImage = (!(paramT.compare(0, strlen("image"), "image")) ||
             !(paramT.compare(0, strlen("__image"), "__image")));
  ...
  } else if ((isBuffer) && (arg_value != NULL)) {
  ...
  for (; itr != crtKernel->m_ContextToKernel.end(); itr++) {
    ...
    if (devObject == NULL) {
      errCode = itr->second->dispatch->clSetKernelArg(itr->second, arg_index,
                                                      arg_size, arg_value);
    } else {
      errCode = itr->second->dispatch->clSetKernelArg(itr->second, arg_index,
                                                      arg_size, devObject);
...
cl_int CL_API_CALL clGetKernelWorkGroupInfo(
  ...
  errCode = devCtx->dispatch->clGetKernelWorkGroupInfo(
      crtKernel->m_ContextToKernel[devCtx], device, param_name,
...
///
/// ------------------------------------------------------------------------------
cl_int CL_API_CALL updateAddedDevicesInfo(CrtDeviceInfo *parentDevInfo,
  ...
  for (cl_uint i = 0; i < num_devices; i++) {
    ...
    // for all handles (gpu), so we need to call Patch for each new created
    // handle
  ...
  if (CL_SUCCESS != errCode) {
    for (cl_uint i = 0; i < numProcessed; i++) {
      CrtDeviceInfo *pDevInfo =
          OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.GetValue(out_devices[i]);
      if (pDevInfo) {
        delete pDevInfo;
      }
...
CL_API_ENTRY cl_int CL_API_CALL clCreateSubDevices(
  ...
  errCode = parentDevInfo->m_origDispatchTable.clCreateSubDevices(
      device, properties, num_devices, out_devices, &tmp_num_devices_ret);

  if ((CL_SUCCESS == errCode) && (NULL != out_devices)) {
    errCode =
        updateAddedDevicesInfo(parentDevInfo, out_devices, tmp_num_devices_ret);
  }
...
cl_int CL_API_CALL clRetainEvent(cl_event event) {
  CrtEvent *crtEvent =
      reinterpret_cast<CrtEvent *>(((_cl_event_crt *)event)->object);
...
cl_int CL_API_CALL clWaitForEvents(cl_uint num_events,
  ...
  if (0 == num_events) {
    ...
    CrtEvent *crtEvent = reinterpret_cast<CrtEvent *>(
        ((_cl_event_crt *)(event_list[0]))->object);
    crtContext = crtEvent->getContext();

    for (cl_uint i = 1; i < num_events; i++) {
      crtEvent = reinterpret_cast<CrtEvent *>(
          ((_cl_event_crt *)(event_list[i]))->object);
  ...
  for (; itr != crtContext->m_contexts.end(); itr++) {
    for (cl_uint i = 0; i < num_events; i++) {
      ...
      if (crtEvent->m_isUserEvent) {
        ...
        pEvents[current++] =
            ((CrtUserEvent *)crtEvent)->m_ContextToEvent[itr->first];
      } else if (itr->first ==
                 crtEvent->getContext()
                     ->m_DeviceToContext[crtEvent->m_queueCRT->m_device]) {
...
cl_int CL_API_CALL clGetEventInfo(cl_event event, cl_event_info param_name,
  ...
  errCode =
      eventDEV->dispatch->clGetEventInfo(eventDEV, param_name, param_value_size,
                                         param_value, param_value_size_ret);
  ...
  if (errCode == CL_SUCCESS && param_value) {
    if (param_name == CL_EVENT_CONTEXT) {
      ...
      memcpy_s(param_value, sizeof(cl_context), &(crtContext->m_context_handle),
               sizeof(cl_context));
    } else if (!crtEvent->m_isUserEvent &&
               (param_name == CL_EVENT_COMMAND_QUEUE)) {
      memcpy_s(param_value, sizeof(cl_command_queue),
      ...
      // it from the CRT; recall CRT doesn't forward retain calls to the
...
cl_int CL_API_CALL clGetProgramInfo(cl_program program,
  ...
  case CL_PROGRAM_DEVICES: {
    ...
    if ((param_value != NULL) && (param_value_size < pValueSize)) {
      errCode = CL_INVALID_VALUE;
      goto FINISH;
    }
    ...
    for (; itr != crtProgram->m_ContextToProgram.end(); itr++) {
      ...
      errCode = itr->second->dispatch->clGetProgramInfo(
          itr->second, CL_PROGRAM_DEVICES, pValueSize,
          (param_value == NULL) ? NULL : (pParamValue + totalCount), &retSize);
  ...
  case CL_PROGRAM_BINARY_SIZES: {
    ...
    for (; itr != crtProgram->m_ContextToProgram.end(); itr++) {
      size_t retSize = 0;
      errCode = itr->second->dispatch->clGetProgramInfo(
          itr->second, CL_PROGRAM_BINARY_SIZES, pValueSize,
          (param_value == NULL) ? NULL : (pParamValue + totalCount), &retSize);
  ...
  case CL_PROGRAM_BINARIES: {
    ...
    for (; itr != crtProgram->m_ContextToProgram.end(); itr++) {
      size_t retSize = 0;
      errCode = itr->second->dispatch->clGetProgramInfo(
          itr->second, CL_PROGRAM_BINARIES, pValueSize,
          (param_value == NULL) ? NULL : (pParamValue + totalCount), &retSize);
  ...
  default: {
    ...
    errCode =
        pgmObj->dispatch->clGetProgramInfo(pgmObj, param_name, param_value_size,
                                           param_value, param_value_size_ret);
  } break;
  }
FINISH:
...
cl_event CL_API_CALL clCreateUserEvent(cl_context context,
  ...
  CrtContext *crtContext =
      reinterpret_cast<CrtContext *>(((_cl_context_crt *)context)->object);
  ...
  for (SHARED_CTX_DISPATCH::iterator itr = crtContext->m_contexts.begin();
       itr != crtContext->m_contexts.end(); itr++) {
...
cl_int CL_API_CALL clSetUserEventStatus(cl_event event,
  ...
  if (!crtEvent || false == crtEvent->m_isUserEvent) {
    ...
    std::map<cl_context, cl_event>::iterator itr =
        crtUserEVent->m_ContextToEvent.begin();
    for (; itr != crtUserEVent->m_ContextToEvent.end(); itr++) {
      errCode = itr->second->dispatch->clSetUserEventStatus(itr->second,
                                                            execution_status);
      if (CL_SUCCESS != errCode) {
        return errCode;
      }
...
cl_int CL_API_CALL clSetEventCallback(cl_event event,
  ...
  if (!crtEvent) {
    ...
    errCode = tgtEvent->dispatch->clSetEventCallback(
        tgtEvent, command_exec_callback_type, CrtSetEventCallBack,
        crtEvCBackData);
  }
FINISH:
  if (CL_SUCCESS != errCode) {
...
cl_int CL_API_CALL clEnqueueNDRangeKernel(
  ...
  if (crtKernel->m_programCRT->m_buildContexts.end() ==
      (std::find(crtKernel->m_programCRT->m_buildContexts.begin(),
  ...
  errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueNDRangeKernel(
      queue->m_cmdQueueDEV, crtKernel->m_ContextToKernel[targetContext],
...
cl_int CL_API_CALL clEnqueueTask(cl_command_queue command_queue,
  ...
  targetContext = queue->m_contextCRT->m_DeviceToContext[queue->m_device];
  if (crtKernel->m_ContextToKernel.find(targetContext) ==
      crtKernel->m_ContextToKernel.end()) {
    errCode = CL_INVALID_PROGRAM_EXECUTABLE;
    goto FINISH;
  }
...
cl_int CL_API_CALL clEnqueueMarker(cl_command_queue command_queue,
  ...
  cl_event eventDEV = NULL;
  errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueMarker(
      queue->m_cmdQueueDEV, &eventDEV);
  if (CL_SUCCESS != errCode) {
    crtEvent->DecPendencyCnt();
    delete event_handle;
    return errCode;
...
cl_int CL_API_CALL clEnqueueBarrier(cl_command_queue command_queue) {
  ...
  errCode =
      queue->m_cmdQueueDEV->dispatch->clEnqueueBarrier(queue->m_cmdQueueDEV);
  return errCode;
}
...
cl_int CL_API_CALL clEnqueueWaitForEvents(cl_command_queue command_queue,
  ...
  errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueWaitForEvents(
      queue->m_cmdQueueDEV, numOutEvents, outEvents);
...
cl_int CL_API_CALL EnqueueMarkerOrBarrierWithWaitList(
    cl_command_type cmdType, cl_command_queue command_queue,
    cl_uint num_events_in_wait_list, const cl_event *event_wait_list,
    cl_event *event) {
  cl_int errCode = CL_SUCCESS;
  CrtEvent *crtEvent = NULL;
...
CL_API_ENTRY cl_int CL_API_CALL clEnqueueMarkerWithWaitList(
    ...
    const cl_event *event_wait_list, cl_event *event) {
  return EnqueueMarkerOrBarrierWithWaitList(CL_COMMAND_MARKER, command_queue,
...
CL_API_ENTRY cl_int CL_API_CALL clEnqueueBarrierWithWaitList(
    cl_command_queue command_queue, cl_uint num_events_in_wait_list,
    const cl_event *event_wait_list, cl_event *event) {
  return EnqueueMarkerOrBarrierWithWaitList(CL_COMMAND_BARRIER, command_queue,
                                            num_events_in_wait_list,
                                            event_wait_list, event);
}
...
cl_int CL_API_CALL clEnqueueNativeKernel(
    cl_command_queue command_queue, user_func user_native_func, void *args,
  ...
  CrtDeviceInfo *devInfo =
      OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.GetValue(queue->m_device);
  ...
  errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueNativeKernel(
      queue->m_cmdQueueDEV, user_native_func, args, cb_args, num_mem_objects,
      crt_mem_list, args_mem_loc, numOutEvents, outEvents,
      crtEvent ? &crtEvent->m_eventDEV : NULL);
...
cl_mem CL_API_CALL clCreateImage2D(cl_context context, cl_mem_flags flags,
  ...
  ctx = (CrtContext *)(ctxInfo->m_object);
  errCode = ctx->CreateImage(flags, image_format, &image_desc, host_ptr,
                             (CrtMemObject **)(&mem_handle->object));
...
inline cl_int CL_API_CALL EnqueueReadWriteImage(
    bool read_command, cl_command_queue command_queue, cl_mem image,
    cl_bool blocking_cmd, const size_t *origin, const size_t *region,
  ...
  CrtImage *crtImage =
      reinterpret_cast<CrtImage *>(((_cl_mem_crt *)image)->object);
  if (!crtImage) {
    errCode = CL_INVALID_MEM_OBJECT;
    goto FINISH;
  }
  ...
  if (read_command) {
    errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueReadImage(
        queue->m_cmdQueueDEV, devMemObj, blocking_cmd, origin, region,
        row_pitch, slice_pitch, const_cast<void *>(ptr), numOutEvents,
        outEvents, crtEvent ? &crtEvent->m_eventDEV : NULL);
  } else {
    errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueWriteImage(
        queue->m_cmdQueueDEV, crtImage->getDeviceMemObj(queue->m_device),
        blocking_cmd, origin, region, row_pitch, slice_pitch, ptr, numOutEvents,
        outEvents, crtEvent ? &crtEvent->m_eventDEV : NULL);
  }
...
cl_int CL_API_CALL clEnqueueWriteImage(
    ...
    size_t slice_pitch, const void *ptr, cl_uint num_events_in_wait_list,
    const cl_event *event_wait_list, cl_event *event) {
  return EnqueueReadWriteImage(false, command_queue, image, blocking_read,
...
cl_int CL_API_CALL clEnqueueReadImage(
    ...
    const cl_event *event_wait_list, cl_event *event) {
  return EnqueueReadWriteImage(true, command_queue, image, blocking_read,
                               origin, region, row_pitch, slice_pitch, ptr,
                               num_events_in_wait_list, event_wait_list, event);
}
...
cl_int CL_API_CALL clEnqueueCopyImage(
  ...
  CrtImage *crtSrcImage =
      reinterpret_cast<CrtImage *>(((_cl_mem_crt *)src_image)->object);
  CrtImage *crtDstImage =
      reinterpret_cast<CrtImage *>(((_cl_mem_crt *)dst_image)->object);
  if ((!crtSrcImage) || (!crtDstImage)) {
    errCode = CL_INVALID_MEM_OBJECT;
    goto FINISH;
  }

  if ((crtSrcImage->m_pContext != queue->m_contextCRT) ||
      (crtDstImage->m_pContext != queue->m_contextCRT)) {
    errCode = CL_INVALID_CONTEXT;
    goto FINISH;
  }
  ...
  errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueCopyImage(
      queue->m_cmdQueueDEV, devSrcMemObj, devDstMemObj, src_origin, dst_origin,
      region, numOutEvents, outEvents, crtEvent ? &crtEvent->m_eventDEV : NULL);
...
cl_int CL_API_CALL clEnqueueCopyImageToBuffer(
  ...
  errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueCopyImageToBuffer(
      queue->m_cmdQueueDEV, devSrcMemObj,
      crtDstBuffer->getDeviceMemObj(queue->m_device), src_origin, region,
      dst_offset, numOutEvents, outEvents,
      crtEvent ? &crtEvent->m_eventDEV : NULL);
...
cl_int CL_API_CALL clEnqueueCopyBufferToImage(
  ...
  errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueCopyBufferToImage(
      queue->m_cmdQueueDEV, crtSrcBuffer->getDeviceMemObj(queue->m_device),
      devDstMemObj, src_offset, dst_origin, region, numOutEvents, outEvents,
      crtEvent ? &crtEvent->m_eventDEV : NULL);
...
clEnqueueFillBuffer(cl_command_queue command_queue, cl_mem buffer,
  ...
  errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueFillBuffer(
      queue->m_cmdQueueDEV, devMemObj, pattern, pattern_size, offset, cb,
      numOutEvents, outEvents, crtEvent ? &crtEvent->m_eventDEV : NULL);
...
CL_API_ENTRY cl_int CL_API_CALL clEnqueueFillImage(
  ...
  errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueFillImage(
      queue->m_cmdQueueDEV, devMemObj, fill_color, origin, region, numOutEvents,
      outEvents, crtEvent ? &crtEvent->m_eventDEV : NULL);
...
CL_API_ENTRY cl_int CL_API_CALL clEnqueueMigrateMemObjects(
  ...
  errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueMigrateMemObjects(
      queue->m_cmdQueueDEV, num_mem_objects, crt_mem_list, flags, numOutEvents,
      outEvents, crtEvent ? &crtEvent->m_eventDEV : NULL);
...
cl_sampler CL_API_CALL clCreateSampler(cl_context context,
  ...
  ctx = (CrtContext *)(ctxInfo->m_object);
  errCode = ctx->CreateSampler(normalized_coords, addressing_mode, filter_mode,
...
cl_sampler CL_API_CALL clCreateSamplerWithProperties(
  ...
  errCode = ctx->clCreateSamplerWithProperties(
      sampler_properties, (CrtSampler **)(&sampler_handle->object));
...
cl_int CL_API_CALL clGetSamplerInfo(cl_sampler sampler,
  ...
  CrtSampler *crtSampler =
      reinterpret_cast<CrtSampler *>(((_cl_sampler_crt *)sampler)->object);
  ...
  default: {
    ...
    retCode = samplerObj->dispatch->clGetSamplerInfo(
        samplerObj, param_name, param_value_size, param_value,
        param_value_size_ret);
  } break;
  }
  return retCode;
...
cl_int CL_API_CALL clGetImageInfo(cl_mem image, cl_image_info param_name,
  ...
  // Forward call
  retCode = memObj->dispatch->clGetImageInfo(
...
cl_int CL_API_CALL clGetMemObjectInfo(cl_mem memobj, cl_mem_info param_name,
  ...
  case CL_MEM_USES_SVM_POINTER: {
    ...
    cl_device_id gpuDevice =
        crtMemObj->m_pContext->GetDeviceByType(CL_DEVICE_TYPE_GPU);
    if (gpuDevice != NULL) {
      cl_mem devMemObj = crtMemObj->getDeviceMemObj(gpuDevice);
      errCode = devMemObj->dispatch->clGetMemObjectInfo(
          devMemObj, param_name, param_value_size, param_value,
          param_value_size_ret);
      break;
    }
...
cl_int CL_API_CALL clGetEventProfilingInfo(cl_event event,
  ...
  if (crtEvent->m_isUserEvent) {
    ...
    errCode = ev->dispatch->clGetEventProfilingInfo(
        ev, param_name, param_value_size, param_value, param_value_size_ret);
  } else {
    // Forward call
    errCode = crtEvent->m_eventDEV->dispatch->clGetEventProfilingInfo(
        crtEvent->m_eventDEV, param_name, param_value_size, param_value,
        param_value_size_ret);
  }
  return errCode;
}
...
cl_int CL_API_CALL clCreateKernelsInProgram(cl_program program,
  ...
  for (cl_uint i = 0; i < crtProgram->m_buildContexts.size(); i++) {
    ...
    errCode = ctxObj->dispatch->clCreateKernelsInProgram(
        crtProgram->m_ContextToProgram[ctxObj], num_kernels, ctxKernels,
...
FINISH:
  if (CL_SUCCESS != errCode) {
    for (cl_uint i = 0; i < num_kernels; i++) {
      if (NULL != kernels[i]) {
        CrtKernel *crtKernel =
            (CrtKernel *)((_cl_kernel_crt *)kernels[i])->object;
        crtKernel->Release();
        crtKernel->DecPendencyCnt();
        delete kernels[i];
        kernels[i] = NULL;
...
cl_int CL_API_CALL clGetSupportedImageFormats(cl_context context,
  ...
  for (SHARED_CTX_DISPATCH::iterator itr = ctx->m_contexts.begin();
    ...
    cl_context ctxObj = itr->first;
    errCode = ctxObj->dispatch->clGetSupportedImageFormats(
        ctxObj, flags, image_type, numTotalAvailable, allImageFormats + offset,
        &numAvailable);
  ...
  // Are we returning Union of supported image formats OR intersection??
  ...
  for (cl_uint i = 1; i < numTotalAvailable; i++) {
    if ((allImageFormats[i].image_channel_data_type ==
         allImageFormats[i - 1].image_channel_data_type) &&
        (allImageFormats[i].image_channel_order ==
         allImageFormats[i - 1].image_channel_order)) {
...
cl_int CL_API_CALL EnqueueAcquireReleaseInteropObjects(
    cl_command_type interop_sync_cmd_type, cl_command_queue command_queue,
  ...
  case CL_COMMAND_ACQUIRE_GL_OBJECTS:
    errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueAcquireGLObjects(
        queue->m_cmdQueueDEV, num_objects, crt_mem_list, numOutEvents,
        outEvents, crtEvent ? &crtEvent->m_eventDEV : NULL);
    break;
  ...
  case CL_COMMAND_ACQUIRE_DX9_OBJECTS_INTEL:
    errCode =
        ((SOCLEntryPointsTable *)queue->m_cmdQueueDEV)
            ->crtDispatch->clEnqueueAcquireDX9ObjectsINTEL(
                queue->m_cmdQueueDEV, num_objects, crt_mem_list, numOutEvents,
                outEvents, crtEvent ? &crtEvent->m_eventDEV : NULL);
    break;
  case CL_COMMAND_RELEASE_DX9_OBJECTS_INTEL:
    errCode =
        ((SOCLEntryPointsTable *)queue->m_cmdQueueDEV)
            ->crtDispatch->clEnqueueReleaseDX9ObjectsINTEL(
                queue->m_cmdQueueDEV, num_objects, crt_mem_list, numOutEvents,
                outEvents, crtEvent ? &crtEvent->m_eventDEV : NULL);
    break;
#endif
...
CL_API_ENTRY cl_int CL_API_CALL clGetKernelArgInfo(
  ...
  errCode = kernelDevObj->dispatch->clGetKernelArgInfo(
      kernelDevObj, arg_indx, param_name, param_value_size, param_value,
      param_value_size_ret);
...
CL_API_ENTRY cl_int CL_API_CALL clGetDeviceIDsFromDX9INTEL(
  ...
  for (itr = OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.get().begin();
    ...
    case CL_PREFERRED_DEVICES_FOR_DX9_INTEL:
      ...
      errCode = ((SOCLEntryPointsTable *)devicePlatform->m_platformIdDEV)
                    ->crtDispatch->clGetDeviceIDsFromDX9INTEL(
                        devicePlatform->m_platformIdDEV, d3d_device_source,
                        d3d_object, d3d_device_set, num_entries, devices,
                        &num_devices_ret);
    ...
    case CL_ALL_DEVICES_FOR_DX9_INTEL:
      ...
      errCode = ((SOCLEntryPointsTable *)devicePlatform->m_platformIdDEV)
                    ->crtDispatch->clGetDeviceIDsFromDX9INTEL(
                        ...
                        platform_devices, &platform_num_devices);
...
CL_API_ENTRY cl_int CL_API_CALL clGetDeviceIDsFromDX9MediaAdapterKHR(
  ...
  for (itr = OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.get().begin();
    ...
    case CL_PREFERRED_DEVICES_FOR_DX9_MEDIA_ADAPTER_KHR:
      ...
      errCode = (devicePlatform->m_platformIdDEV)
                    ->dispatch->clGetDeviceIDsFromDX9MediaAdapterKHR(
                        devicePlatform->m_platformIdDEV, num_media_adapters,
                        media_adapter_type, media_adapters, media_adapter_set,
                        num_entries, devices, &num_devices_ret);
    ...
    case CL_ALL_DEVICES_FOR_DX9_MEDIA_ADAPTER_KHR:
      ...
      errCode = (devicePlatform->m_platformIdDEV)
                    ->dispatch->clGetDeviceIDsFromDX9MediaAdapterKHR(
                        devicePlatform->m_platformIdDEV, num_media_adapters,
                        media_adapter_type, media_adapters, media_adapter_set,
                        num_entries, platform_devices, &platform_num_devices);
...
CL_API_ENTRY cl_int CL_API_CALL clGetDeviceIDsFromD3D10KHR(
  ...
  for (itr = OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.get().begin();
    ...
    case CL_PREFERRED_DEVICES_FOR_D3D10_KHR:
      ...
      errCode = (devicePlatform->m_platformIdDEV)
                    ->dispatch->clGetDeviceIDsFromD3D10KHR(
                        devicePlatform->m_platformIdDEV, d3d_device_source,
                        d3d_object, d3d_device_set, num_entries, devices,
                        &num_devices_ret);
...
CL_API_ENTRY cl_int CL_API_CALL clGetDeviceIDsFromD3D11KHR(
  ...
  for (itr = OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.get().begin();
    ...
    case CL_PREFERRED_DEVICES_FOR_D3D11_KHR:
      ...
      errCode = (devicePlatform->m_platformIdDEV)
                    ->dispatch->clGetDeviceIDsFromD3D11KHR(
                        devicePlatform->m_platformIdDEV, d3d_device_source,
                        d3d_object, d3d_device_set, num_entries, devices,
                        &num_devices_ret);
...
///
/// ------------------------------------------------------------------------------
cl_mem CL_API_CALL CreateFromDX9MediaSurface(cl_context context,
                                             cl_mem_flags flags,
  ...
  ctx = (CrtContext *)(ctxInfo->m_object);
  errCode =
      ctx->CreateFromDX9MediaSurface(flags, resource, sharedHandle, plane,
                                     (CrtMemObject **)(&mem_handle->object));
...
CL_API_ENTRY cl_mem CL_API_CALL clCreateFromDX9MediaSurfaceINTEL(
  ...
  memObj = ((SOCLEntryPointsTable *)context)
               ->crtDispatch->clCreateFromDX9MediaSurfaceINTEL(
                   context, flags, resource, sharedHandle, plane, &errCode);
...
CL_API_ENTRY cl_int CL_API_CALL clEnqueueAcquireDX9ObjectsINTEL(
  ...
  errCode = ((SOCLEntryPointsTable *)command_queue)
                ->crtDispatch->clEnqueueAcquireDX9ObjectsINTEL(
...
///
/// ------------------------------------------------------------------------------
CL_API_ENTRY cl_int CL_API_CALL clSetDebugVariableINTEL(cl_device_id device,
                                                        const char *pKey,
                                                        cl_uint value) {
  cl_int errCode = CL_SUCCESS;
  CrtDeviceInfo *devInfo =
  ...
  if (devInfo->m_devType != CL_DEVICE_TYPE_GPU) {
    ...
    errCode = ((SOCLEntryPointsTable *)devInfo->m_crtPlatform->m_platformIdDEV)
                  ->crtDispatch->clSetDebugVariableINTEL(device, pKey, value);
  }
...
CL_API_ENTRY cl_command_queue CL_API_CALL clCreatePerfCountersCommandQueueINTEL(
  ...
  if ((context == NULL) || (device == NULL)) {
    ...
    if ((errCode == CL_SUCCESS) && (devInfo->m_devType == CL_DEVICE_TYPE_GPU)) {
      CrtContextInfo *ctxInfo =
          OCLCRT::crt_ocl_module.m_contextInfoGuard.GetValue(context);
      if (ctxInfo == NULL) {
        errCode = CL_INVALID_CONTEXT;
      ...
      if (ctxInfo->m_contextType == CrtContextInfo::SinglePlatformGPUContext) {
        commandQueue =
            ((SOCLEntryPointsTable *)context)
                ->crtDispatch->clCreatePerfCountersCommandQueueINTEL(
                    context, device, properties, configuration, &errCode);
      } else {
        errCode = CL_INVALID_CONTEXT;
      }
    } else {
...
CL_API_ENTRY cl_int CL_API_CALL clSetPerformanceConfigurationINTEL(
  ...
  CrtDeviceInfo *pDeviceInfo =
      OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.GetValue(device);

  if ((count == 0) || (offsets == NULL) || (values == NULL)) {
    return CL_INVALID_VALUE;
  }

  if (pDeviceInfo && (pDeviceInfo->m_devType == CL_DEVICE_TYPE_GPU)) {
    result = ((SOCLEntryPointsTable *)device)
                 ->crtDispatch->clSetPerformanceConfigurationINTEL(
                     device, count, offsets, values);
  } else {
...
///
/// ------------------------------------------------------------------------------
cl_mem CL_API_CALL CreateFromGLBuffer(bool is_render_buffer, cl_context context,
  ...
  ctx = (CrtContext *)(ctxInfo->m_object);
  errCode = ctx->CreateGLBuffer(is_render_buffer, flags, bufobj,
                                (CrtMemObject **)(&mem_handle->object));
...
CL_API_ENTRY cl_mem CL_API_CALL clCreateFromGLBuffer(cl_context context,
                                                     ...
                                                     GLuint bufobj,
                                                     cl_int *errcode_ret) {
  return CreateFromGLBuffer(false, // False = Not RenderBuffer
...
clCreateFromGLRenderbuffer(cl_context context, cl_mem_flags flags,
  ...
  return CreateFromGLBuffer(true, // True = RenderBuffer
...
///
/// ------------------------------------------------------------------------------
cl_mem CL_API_CALL CreateFromGLTexture(cl_context context, cl_uint dim_count,
  ...
  ctx = (CrtContext *)(ctxInfo->m_object);
  errCode = ctx->CreateGLImage(dim_count, flags, target, miplevel, texture,
                               (CrtMemObject **)(&mem_handle->object));
...
/// ------------------------------------------------------------------------------
CL_API_ENTRY cl_mem CL_API_CALL
clCreateFromGLTexture(cl_context context, cl_mem_flags flags, GLenum target,
  ...
  return CreateFromGLTexture(context, 0, flags, target, miplevel, texture,
                             errcode_ret);
}
...
/// ------------------------------------------------------------------------------
CL_API_ENTRY cl_mem CL_API_CALL
clCreateFromGLTexture2D(cl_context context, cl_mem_flags flags, GLenum target,
                        GLint miplevel, GLuint texture, cl_int *errcode_ret) {
  return CreateFromGLTexture(context, 2, flags, target, miplevel, texture,
                             errcode_ret);
}
...
/// ------------------------------------------------------------------------------
CL_API_ENTRY cl_mem CL_API_CALL
clCreateFromGLTexture3D(cl_context context, cl_mem_flags flags, GLenum target,
                        GLint miplevel, GLuint texture, cl_int *errcode_ret) {
  return CreateFromGLTexture(context, 3, flags, target, miplevel, texture,
                             errcode_ret);
}
...
CL_API_ENTRY cl_int CL_API_CALL clGetGLObjectInfo(
  ...
  return devMemObj->dispatch->clGetGLObjectInfo(devMemObj, gl_object_type,
                                                gl_object_name);
}
...
CL_API_ENTRY cl_int CL_API_CALL clGetGLTextureInfo(
  ...
  return devMemObj->dispatch->clGetGLTextureInfo(devMemObj, param_name,
...
CL_API_ENTRY cl_int CL_API_CALL clEnqueueAcquireGLObjects(
  ...
  return EnqueueAcquireReleaseInteropObjects(
      CL_COMMAND_ACQUIRE_GL_OBJECTS, command_queue, num_objects, mem_objects,
      num_events_in_wait_list, event_wait_list, event);
}
...
CL_API_ENTRY cl_int CL_API_CALL clEnqueueReleaseGLObjects(
    ...
    const cl_event *event_wait_list, cl_event *event) {
  return EnqueueAcquireReleaseInteropObjects(
      CL_COMMAND_RELEASE_GL_OBJECTS, command_queue, num_objects, mem_objects,
      num_events_in_wait_list, event_wait_list, event);
}
...
CL_API_ENTRY cl_accelerator_intel CL_API_CALL clCreateAcceleratorINTEL(
    ...
    const void *desc, cl_int *errcode_ret) {
  ...
  acceleratorObj = ((SOCLEntryPointsTable *)context)
                       ->crtDispatch->clCreateAcceleratorINTEL(
                           context, type, desc_size, desc, &errCode);
...
CL_API_ENTRY cl_program CL_API_CALL clCreateProfiledProgramWithSourceINTEL(
    ...
    cl_uint configurations_count, cl_int *error_code) {
  cl_int errorCode = CL_SUCCESS;
  cl_program program = NULL;
  CrtContextInfo *pCtxInfo =
      OCLCRT::crt_ocl_module.m_contextInfoGuard.GetValue(context);
  ...
  program = ((SOCLEntryPointsTable *)context)
                ->crtDispatch->clCreateProfiledProgramWithSourceINTEL(
                    context, count, sources, lengths, configurations,
                    configurations_count, &errorCode);
...
CL_API_ENTRY cl_int CL_API_CALL clCreateKernelProfilingJournalINTEL(
  ...
  errorCode = ((SOCLEntryPointsTable *)context)
                  ->crtDispatch->clCreateKernelProfilingJournalINTEL(
...
CL_API_ENTRY cl_int CL_API_CALL clGetAcceleratorInfoINTEL(
  ...
  errCode = ((SOCLEntryPointsTable *)accelerator)
                ->crtDispatch->clGetAcceleratorInfoINTEL(
                    accelerator, param_name, param_value_size, param_value,
                    param_value_size_ret);
...
CL_API_ENTRY cl_int CL_API_CALL clSetAcceleratorInfoINTEL(
  ...
  errCode = ((SOCLEntryPointsTable *)accelerator)
                ->crtDispatch->clSetAcceleratorInfoINTEL(
                    accelerator, param_name, param_value_size, param_value);
...
cl_int CL_API_CALL clRetainAcceleratorINTEL(cl_accelerator_intel accelerator) {
  ...
  errCode = ((SOCLEntryPointsTable *)accelerator)
                ->crtDispatch->clRetainAcceleratorINTEL(accelerator);
...
CL_API_ENTRY cl_int CL_API_CALL clEnqueueMediaPakINTEL(
    ...
    const cl_event *event_wait_list, cl_event *oclevent) {
  cl_int errCode = CL_SUCCESS;
  ...
  crtQueue = reinterpret_cast<CrtQueue *>(
      ((_cl_command_queue_crt *)command_queue)->object);
  if (crtQueue == NULL) {
    errCode = CL_INVALID_CONTEXT;
    goto FINISH;
  ...
  if (((SOCLEntryPointsTable *)accelerator)
          ->crtDispatch->clEnqueueMediaPakINTEL == NULL) {
    errCode = CL_INVALID_CONTEXT;
    goto FINISH;
  }

  errCode =
      ((SOCLEntryPointsTable *)accelerator)
          ->crtDispatch->clEnqueueMediaPakINTEL(
              command_queue, accelerator, picture, num_slices,
              num_slice_headers, slice_headers, slices, coding_tree_units,
              coding_units, current_image, current_temporal, current_recon,
              num_refs, ref_images, ref_temporal, tail_insert, status_out,
              stream_out, num_events_in_wait_list, event_wait_list, oclevent);
...
cl_int CL_API_CALL clReleaseAcceleratorINTEL(cl_accelerator_intel accelerator) {
  ...
  errCode = ((SOCLEntryPointsTable *)accelerator)
                ->crtDispatch->clReleaseAcceleratorINTEL(accelerator);
...
CL_API_ENTRY cl_int CL_API_CALL clEnqueueReleaseVA_APIMediaSurfacesINTEL(
  ...
  errorCode = ((SOCLEntryPointsTable *)queue->m_cmdQueueDEV)
                  ->crtDispatch->clEnqueueReleaseVA_APIMediaSurfacesINTEL(
                      command_queue, num_objects, mem_objects,
                      num_events_in_wait_list, event_wait_list, event);
...
CL_API_ENTRY cl_int CL_API_CALL clEnqueueAcquireVA_APIMediaSurfacesINTEL(
  ...
  errorCode = ((SOCLEntryPointsTable *)queue->m_cmdQueueDEV)
                  ->crtDispatch->clEnqueueAcquireVA_APIMediaSurfacesINTEL(
                      command_queue, num_objects, mem_objects,
                      num_events_in_wait_list, event_wait_list, event);
...
CL_API_ENTRY cl_mem CL_API_CALL clCreateFromVA_APIMediaSurfaceINTEL(
  ...
  CLMem = ((SOCLEntryPointsTable *)context)
              ->crtDispatch->clCreateFromVA_APIMediaSurfaceINTEL(
                  context, flags, surface, plane, &errorCode);
...
int CL_API_CALL GetCRTInfo(crt_info param_name, size_t param_value_size,
                           void *param_value, size_t *param_value_size_ret) {
...
clGetExtensionFunctionAddress(const char *funcname) {
  ...
  if (funcname && !strcmp(funcname, "clCreateProfiledProgramWithSourceINTEL")) {
    return (
        (void *)(ptrdiff_t)GET_ALIAS(clCreateProfiledProgramWithSourceINTEL));
  }
...
#ifdef LIBVA_SHARING
  if (funcname &&
      !strcmp(funcname, "clGetDeviceIDsFromVA_APIMediaAdapterINTEL")) {
  ...
  if (funcname &&
      !strcmp(funcname, "clEnqueueAcquireVA_APIMediaSurfacesINTEL")) {
    return ((void *)(ptrdiff_t)(clEnqueueAcquireVA_APIMediaSurfacesINTEL));
  }
  if (funcname &&
      !strcmp(funcname, "clEnqueueReleaseVA_APIMediaSurfacesINTEL")) {
...
void *CL_API_CALL clSVMAlloc(cl_context context, cl_svm_mem_flags flags,
  ...
  gpuContext = pCrtCtx->GetContextByDeviceID(
      pCrtCtx->GetDeviceByType(CL_DEVICE_TYPE_GPU));
  if (NULL == gpuContext) {
    goto FINISH;
  }

  pSvmPtr =
      gpuContext->dispatch->clSVMAlloc(gpuContext, flags, size, alignment);
...
void CL_API_CALL clSVMFree(cl_context context, void *svm_pointer) {
  ...
  gpuContext->dispatch->clSVMFree(gpuContext, svm_pointer);
...
cl_int CL_API_CALL clEnqueueSVMFree(cl_command_queue command_queue,
                                    ...
                                    void *svm_pointers[],
                                    pfn_free pfn_free_func, void *user_data,
                                    cl_uint num_events_in_wait_list,
                                    const cl_event *event_wait_list,
                                    cl_event *event) {
  cl_int errCode = CL_SUCCESS;
  ...
  errCode =
      synchHelper->PrepareToExecute(crtQueue, num_events_in_wait_list,
                                    event_wait_list, &numOutEvents, &outEvents);
  if (CL_SUCCESS != errCode) {
    goto FINISH;
  }

  // This event will be freed in the SVMFreeCallbackFunction if it's not
  // returned to user
  ...
  devInfo =
      OCLCRT::crt_ocl_module.m_deviceInfoMapGuard.GetValue(crtQueue->m_device);
  if (NULL == devInfo) {
    errCode = CL_INVALID_COMMAND_QUEUE;
    goto FINISH;
  }
  ...
  if (devInfo->m_devType == CL_DEVICE_TYPE_GPU) {
    ...
    errCode = crtQueue->m_cmdQueueDEV->dispatch->clEnqueueSVMFree(
        crtQueue->m_cmdQueueDEV, num_svm_pointers, svm_pointers, pfn_free_func,
        user_data, numOutEvents, outEvents, &crtEvent->m_eventDEV);
    if (CL_SUCCESS != errCode) {
      goto FINISH;
    }
    ...
    errCode = crtQueue->m_cmdQueueDEV->dispatch->clSetEventCallback(
        crtEvent->m_eventDEV, CL_COMPLETE, SVMFreeCallbackFunction, clbkData);
    if (CL_SUCCESS != errCode) {
      errCode = CL_OUT_OF_RESOURCES;
      goto FINISH;
    }
    ...
    errCode = crtQueue->m_cmdQueueDEV->dispatch->clEnqueueSVMFree(
        crtQueue->m_cmdQueueDEV, num_svm_pointers, svm_pointers, NULL, NULL,
        numOutEvents, outEvents, &crtEvent->m_eventDEV);
    if (CL_SUCCESS != errCode) {
      goto FINISH;
    }
    errCode = crtQueue->m_cmdQueueDEV->dispatch->clSetEventCallback(
  ...
  // because any other failure AFTER clSetEventCallback could result with double
  // delete of clbkData first delete is below in FINISH: second delete is in
  // SVMFreeCallbackFunction
...
cl_int CL_API_CALL clEnqueueSVMMemcpy(cl_command_queue command_queue,
  ...
  errCode = crtQueue->m_cmdQueueDEV->dispatch->clEnqueueSVMMemcpy(
      crtQueue->m_cmdQueueDEV, blocking_copy, dst_ptr, src_ptr, size,
      numOutEvents, outEvents, crtEvent ? &crtEvent->m_eventDEV : NULL);
...
cl_int CL_API_CALL clEnqueueSVMMemFill(cl_command_queue command_queue,
  ...
  if (NULL == pattern || !IsPowerOf2((cl_uint)pattern_size) ||
      pattern_size > 128) {
    errCode = CL_INVALID_VALUE;
    goto FINISH;
  }
  ...
  errCode = crtQueue->m_cmdQueueDEV->dispatch->clEnqueueSVMMemFill(
      crtQueue->m_cmdQueueDEV, svm_ptr, pattern, pattern_size, size,
      numOutEvents, outEvents, crtEvent ? &crtEvent->m_eventDEV : NULL);
...
cl_int CL_API_CALL clEnqueueSVMMap(cl_command_queue command_queue,
  ...
  errCode = crtQueue->m_cmdQueueDEV->dispatch->clEnqueueSVMMap(
      crtQueue->m_cmdQueueDEV, blocking_map, map_flags, svm_ptr, size,
      numOutEvents, outEvents, crtEvent ? &crtEvent->m_eventDEV : NULL);
...
cl_int CL_API_CALL clEnqueueSVMUnmap(cl_command_queue command_queue,
  ...
  errCode = crtQueue->m_cmdQueueDEV->dispatch->clEnqueueSVMUnmap(
      crtQueue->m_cmdQueueDEV, svm_ptr, numOutEvents, outEvents,
      crtEvent ? &crtEvent->m_eventDEV : NULL);
...
CL_API_ENTRY cl_int CL_API_CALL clEnqueueSVMMigrateMem(
  ...
  if (devInfo->m_devType == CL_DEVICE_TYPE_GPU) {
    ...
    errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueSVMMigrateMem(
        queue->m_cmdQueueDEV, num_svm_pointers, svm_pointers, sizes, flags,
        numOutEvents, outEvents, &crtEvent->m_eventDEV);
  } else {
    for (size_t i = 0; i < num_svm_pointers; ++i) {
      if (!queue->m_contextCRT->m_svmPointers.GetSVMAllocPtrForRange(
              svm_pointers[i], (sizes) ? sizes[i] : 0)) {
        errCode = CL_INVALID_VALUE;
        goto FINISH;
      }
    }
    ...
    errCode = queue->m_cmdQueueDEV->dispatch->clEnqueueBarrierWithWaitList(
        queue->m_cmdQueueDEV, numOutEvents, outEvents, &crtEvent->m_eventDEV);
  }
...
cl_int CL_API_CALL clSetKernelArgSVMPointer(cl_kernel kernel, cl_uint arg_index,
  ...
  for (; itr != crtKernel->m_ContextToKernel.end(); itr++) {
    errCode = itr->second->dispatch->clSetKernelArgSVMPointer(
        itr->second, arg_index, arg_value);
...
cl_int CL_API_CALL clSetKernelExecInfo(cl_kernel kernel,
  ...
  gpuCtx =
      crtCtx->GetContextByDeviceID(crtCtx->GetDeviceByType(CL_DEVICE_TYPE_GPU));
  gpuKernel = crtKernel->m_ContextToKernel[gpuCtx];

  // call GPU's API only
  errCode = gpuKernel->dispatch->clSetKernelExecInfo(
      gpuKernel, param_name, param_value_size, param_value);
...
cl_mem CL_API_CALL clCreatePipe(cl_context context, cl_mem_flags flags,
  ...
  ctx = (CrtContext *)(ctxInfo->m_object);
  errCode = ctx->CreatePipe(flags, pipe_packet_size, pipe_max_packets,
                            properties, (CrtMemObject **)(&mem_handle->object));
