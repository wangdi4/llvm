...
namespace Intel { namespace OpenCL { namespace DeviceBackend {
	...
	public:
		...
		const char*	GetKernelName() const {return m_szName.c_str();}
		...
		const size_t* GetRequiredWorkgroupSize() const { return (m_pReqdWGSize[0] ? m_pReqdWGSize : NULL);}
	...
	protected:
		...
		size_t				m_pReqdWGSize[MAX_WORK_DIM];	// Required work-group size that was declared during kernel compilation
		size_t				m_pHintWGSize[MAX_WORK_DIM];	// Hint to work-group size that was declared during kernel compilation
