...
LLVMKernel::LLVMKernel(LLVMProgram* pProgram) :
	m_pFuncPtr(NULL), m_szName(""), m_uiArgCount(0), m_pArguments(NULL),
	m_uiOptWGSize(1), m_uiExplLocalMemCount(0), m_bBarrier(false),
	m_pModule(NULL), m_pFunction(NULL),
...
{
	m_pHintWGSize[0] = m_pHintWGSize[1] = m_pHintWGSize[2] = 0;
	m_pReqdWGSize[0] = m_pReqdWGSize[1] = m_pReqdWGSize[2] = 0;
}
...
cl_dev_err_code LLVMKernel::ParseArguments(Function *pFunc, ConstantArray* pFuncArgs)
{
	...
	{
		string::size_type pos = remArgString.find(", ", 0);
		if(pos == string::npos)
		{
		...
		case Type::PointerTyID:
			{
				...
				if ( -1 != inx )	// Image identifier was found
				{
					...
					m_pArguments[i].size_in_bytes = ((currArgString.find("__rd") != string::npos) == strArgs[i]) ? 0 : 1;	// Set RW/WR flag
					break;
				}
		...
		case Type::IntegerTyID:
			{
				if (currArgString.find("sampler_t") != string::npos)
				{
	...
	while ( bb_it != pFunc->getBasicBlockList().end() )
	{
		...
		while ( inst_it != bb_it->getInstList().end() )
		{
			...
			case Instruction::Call:
				...
				{
					if ( !m_pProgram->IsKernel(pCallee->getNameStr().c_str()) )
					{
...
cl_int LLVMKernel::Init(Function *pFunc, ConstantArray* pFuncArgs)
{
	...
	if ( m_bUseVTune && iJIT_SAMPLING_ON == iJIT_IsProfilingActive() )
	{
		...
		ML.method_name = m_szName.c_str();				// method name (can be with or without the class and signature, in any case the class name will be added to it)
	...
	for (unsigned i = 0, e = init->getType()->getNumElements(); i != e; ++i) 
	{
		...
		{
			// Set WG size hint
			llvm::Constant *wgsh = elt->getOperand(2);
			if(!wgsh->isNullValue())
			{
				llvm::ConstantArray *cawgsh = dyn_cast<llvm::ConstantArray>(wgsh);
				m_pHintWGSize[0] = (size_t)dyn_cast<llvm::ConstantInt>(cawgsh->getOperand(1))->getValue().getZExtValue();
				m_pHintWGSize[1] = (size_t)dyn_cast<llvm::ConstantInt>(cawgsh->getOperand(2))->getValue().getZExtValue();
				m_pHintWGSize[2] = (size_t)dyn_cast<llvm::ConstantInt>(cawgsh->getOperand(3))->getValue().getZExtValue();

				m_uiOptWGSize = 1;
				for(int i=0; i<MAX_WORK_DIM; ++i)
				{
					if(m_pHintWGSize[i]) m_uiOptWGSize*=m_pHintWGSize[i];
				}
			}

			//// Set required WG size
			llvm::Constant *rwgs = elt->getOperand(3);
			if(!rwgs->isNullValue())
			{
				llvm::ConstantArray *carwgs = dyn_cast<llvm::ConstantArray>(rwgs);
				m_pReqdWGSize[0] = (size_t)dyn_cast<llvm::ConstantInt>(carwgs->getOperand(1))->getValue().getZExtValue();
				m_pReqdWGSize[1] = (size_t)dyn_cast<llvm::ConstantInt>(carwgs->getOperand(2))->getValue().getZExtValue();
				m_pReqdWGSize[2] = (size_t)dyn_cast<llvm::ConstantInt>(carwgs->getOperand(3))->getValue().getZExtValue();

				m_uiOptWGSize = 1;
				for(int i=0; i<MAX_WORK_DIM; ++i)
				{
					if(m_pReqdWGSize[i]) m_uiOptWGSize*=m_pReqdWGSize[i];
				}
			}
			break;
		}
