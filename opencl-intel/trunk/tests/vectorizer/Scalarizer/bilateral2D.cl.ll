...
entry:
  ...
  %call = call float @_Z3dotDv4_fS_(<4 x float> %tmp, <4 x float> %tmp1) ; <float> [#uses=1]
...
for.body25:                                       ; preds = %for.cond19
  ...
  %call34 = call <4 x float> @_Z11read_imagefP10_image2d_tjDv2_i(%struct._image2d_t* %tmp31, i32 1, <2 x i32> %add) ; <<4 x float>> [#uses=1]
  ...
  %call37 = call <4 x float> @_Z4fabsDv4_f(<4 x float> %sub) ; <<4 x float>> [#uses=1]
...
if.then:                                          ; preds = %for.body25
  ...
  %call59 = call float @_Z3dotDv4_fS_(<4 x float> %tmp57, <4 x float> %tmp58) ; <float> [#uses=1]
