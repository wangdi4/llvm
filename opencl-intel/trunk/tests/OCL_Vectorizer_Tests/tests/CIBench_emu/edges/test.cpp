...
#define PIXELSWIDE 1024
	...
	__cl_float4 l0 = { 5.0, 5.0, 5.0, 5.0 };
	__cl_float4 l1 = { 1.0, 0.0, 0.0, 0.0 };
	__cl_float4 l2 = { 0.0, -1.0, 0.0, h };
	...
	err |= clSetKernelArg(kernel, 5, sizeof(cl_float4), &(l0));
	err |= clSetKernelArg(kernel, 6, sizeof(cl_float4), &(l1));
	err |= clSetKernelArg(kernel, 7, sizeof(cl_float4), &(l2));
	
	err |= clSetKernelArg(kernel, 8, sizeof(cl_mem), &src);
	err |= clSetKernelArg(kernel, 9, sizeof(cl_sampler), &sampler);	
	if(err) {
