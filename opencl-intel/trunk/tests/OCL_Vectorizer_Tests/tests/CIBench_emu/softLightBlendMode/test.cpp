...
#define PIXELSWIDE 1024
		...
		data2 = (unsigned char *)calloc(PIXELSWIDE*PIXELSHIGH*4, 1);
		{
			unsigned char *p = data2;
			for (int row = 0; row < 1024; row++)
			{
				for (int column = 0; column < 1024; column++)
				{
					int rloc = (row / 64) % 2;
					int cloc = (column / 64) % 2;
					if (rloc == cloc)
					{
						p[0] = 255;
						p[1] = 0;
	...
	format.image_channel_order = CL_ARGB;
	format.image_channel_data_type  = CL_UNORM_INT8;
	src2 = clCreateImage2D(context, CL_MEM_READ_ONLY, &format, w, h, 0, NULL, &err);
	if(!src2)
	{
		printf("src2:clCreateImage2D() failed.\n");
		return EXIT_FAILURE;
	}
	...
	err = clEnqueueWriteImage(queue, src2, CL_TRUE, origin, region, bpr, 0, data2, 0, NULL, NULL);
	if(err != CL_SUCCESS)
	{
		printf("clWriteImage() failed (%d).\n", err);
		return EXIT_FAILURE;
	...
	err |= clSetKernelArg(kernel, 9, sizeof(cl_mem), &src2);
	err |= clSetKernelArg(kernel, 10, sizeof(cl_sampler), &sampler);	
	err |= clSetKernelArg(kernel, 11, sizeof(cl_mem), &src);
	err |= clSetKernelArg(kernel, 12, sizeof(cl_sampler), &sampler);	
	if(err) {
