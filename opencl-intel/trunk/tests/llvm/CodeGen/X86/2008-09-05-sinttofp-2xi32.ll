; RUN: llc < %s -mcpu=corei7 -march=x86 -mattr=+sse2 -mattr=+mmx | grep unpcklpd
; RUN: llc < %s -mcpu=corei7 -march=x86 -mattr=+sse2 -mattr=+mmx | grep unpckhpd
; RUN: llc < %s -mcpu=corei7 -march=x86 -mattr=+sse2 | grep cvttpd2pi | count 1
; RUN: llc < %s -mcpu=corei7 -march=x86 -mattr=+sse2 | grep cvtpi2pd | count 1
