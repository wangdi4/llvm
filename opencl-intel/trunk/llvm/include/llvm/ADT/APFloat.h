...
namespace llvm {
  ...
  class APFloat {
  protected:
   // denormal handling flag
   // if true then every binary operation will flush to zero both operands before
   // actual operation.	 
   static bool eliminateDenormals;
  public:
	...
	// Toggle denormal handling flag
    static void setEliminatenateDenormals(bool flag)
    {
        eliminateDenormals = flag;
    }

    static bool getEliminatenateDenormals()
    {
        return eliminateDenormals;
    }
  ...
  private:
    ...
    /* if APFloat is denormal it will converted to zero */
    void fixDenormal();
	/* Compare two APfloats. 
      This function is required by denormal fixing, since all compare functions
      are defined with const modifiers. */ 
    static APFloat::cmpResult externalCompare(const APFloat &lhs, const APFloat &rhs);
