...
public:
  enum LocInfo {
    ...
    Expand  // The value should be splitted and filled in two location slots
  };
private:
...
public:
  ...
  static CCValAssign getRegPair(unsigned ValNo, EVT ValVT,
                            unsigned RegNo0, unsigned RegNo1, EVT LocVT,
                            LocInfo HTP) {
    CCValAssign Ret;
    Ret.ValNo = ValNo;
    Ret.Loc = RegNo0 | RegNo1 << 16;
    Ret.isMem = false;
    Ret.isCustom = false;
    Ret.HTP = HTP;
    Ret.ValVT = ValVT;
  ...
  std::pair < unsigned, unsigned > getLocRegPair() const { assert(isRegLoc()); return std::make_pair(Loc & 0xFFFF, Loc >> 16); }
