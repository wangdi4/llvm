...
TYPED_TEST(ValueMapTest, OperationsWork) {
  ...
  TypeParam* tmp = this->BitcastV.get();
  EXPECT_EQ(tmp, I->first);
  ...
  tmp = this->BitcastV.get();
  EXPECT_EQ(tmp, CI->first);
  ...
  tmp = this->AddV.get();
  EXPECT_EQ(tmp, InsertResult1.first->first);
  ...
  tmp = this->AddV.get();
  EXPECT_EQ(tmp, InsertResult2.first->first);
...
TYPED_TEST(ValueMapTest, Iteration) {
  ...
  for (typename ValueMap<TypeParam*, int>::iterator I = VM.begin(), E = VM.end();
    ...
    if (I->second == 2) {
		tp = this->BitcastV.get();
      EXPECT_EQ(tp, I->first);
      I->second = 5;
    } else if (I->second == 3) {
		tp = this->AddV.get();
      EXPECT_EQ(tp, I->first);
...
/// Test is failing on windows. Disabled from the build.
#ifndef _WIN32
#if ENABLE_THREADS
