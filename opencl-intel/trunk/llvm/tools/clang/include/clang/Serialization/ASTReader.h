...
private:
  ...
  struct PerFileData {
	...
	private:
		PerFileData(const PerFileData& p);
		PerFileData& operator=(const PerFileData& p);
  };
  ...
  ASTReadResult ReadASTCore(llvm::StringRef FileName, llvm::MemoryBuffer* PchBuff = NULL);
...
public:
  ...
  /// name.
  ASTReadResult ReadAST(const std::string &FileName, llvm::MemoryBuffer* PchBuff = NULL);
  ...
  Stmt *ReadSubStmt() {
    LLVM_ASSERT(ReadingKind == Read_Stmt &&
           "Should be called only during statement reading!");
    // Subexpressions are stored from last to first, so the next Stmt we need
    // is at the back of the stack.
    LLVM_ASSERT(!StmtStack.empty() && "Read too many sub statements!");
