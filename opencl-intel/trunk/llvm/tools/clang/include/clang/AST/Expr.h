...
protected:
  ...
  /// FpContract - Whether this expression can be contracted according to #pragma OPENCL FP_CONTRACT
  bool FpContract : 1;

  Expr(StmtClass SC, QualType T, bool TD, bool VD)
    : Stmt(SC), TypeDependent(TD), ValueDependent(VD), ValueKind(0), FpContract(true) {
    setType(T);
  }

  /// \brief Construct an empty expression.
  explicit Expr(StmtClass SC, EmptyShell) : Stmt(SC), FpContract(true) { }
...
public:
  ...
  void setType(QualType t) {
    ...
    LLVM_ASSERT((t.isNull() || !t->isReferenceType()) &&
  ...
  /// isFpContract - Returns whether this expression can be contracted according to #pragma OPENCL FP_CONTRACT
  bool isFpContract() const { return FpContract; }

  /// setFpContract - Sets whether this expression can be contracted according to #pragma OPENCL FP_CONTRACT
  void setFpContract(bool contract) { FpContract = contract; }
  ...
  public:
    ...
    ModifiableType getModifiable() const {
      LLVM_ASSERT(Modifiable != CM_Untested && "Did not test for modifiability.");
...
public:
  ...
  IntegerLiteral(ASTContext &C, const llvm::APInt &V,
    ...
    LLVM_ASSERT(type->isIntegerType() && "Illegal type in IntegerLiteral");
    setValue(C, V);
  }
...
public:
  ...
  SourceLocation getStrTokenLoc(unsigned TokNum) const {
    LLVM_ASSERT(TokNum < NumConcatenated && "Invalid tok number");
...
public:
  ...
  const OffsetOfNode &getComponent(unsigned Idx) {
    LLVM_ASSERT(Idx < NumComps && "Subscript out of range");
  ...
  Expr* getIndexExpr(unsigned Idx) {
    LLVM_ASSERT(Idx < NumExprs && "Subscript out of range");
...
public:
  SizeOfAlignOfExpr(Kind_t exprKind, TypeSourceInfo *TInfo,
  ...
  SizeOfAlignOfExpr(Kind_t exprKind, Expr *E,
                    QualType resultType, SourceLocation op,
                    SourceLocation rp) :
      Expr(SizeOfAlignOfExprClass, resultType,
           false, // Never type-dependent (C++ [temp.dep.expr]p3).
  ...
  TypeSourceInfo *getArgumentTypeInfo() const {
    LLVM_ASSERT(isArgumentType() && "calling getArgumentType() when arg is expr");
    return Argument.Ty;
  }
  Expr *getArgumentExpr() {
    LLVM_ASSERT(!isArgumentType() && "calling getArgumentExpr() when arg is type");
...
class MemberExpr : public Expr {
  ...
  MemberNameQualifier *getMemberQualifier() {
    LLVM_ASSERT(HasQualifierOrFoundDecl);
...
public:
  MemberExpr(Expr *base, bool isarrow, ValueDecl *memberdecl,
    ...
    LLVM_ASSERT(memberdecl->getDeclName() == NameInfo.getName());
  }
  ...
  ExplicitTemplateArgumentList &getExplicitTemplateArgs() {
    LLVM_ASSERT(HasExplicitTemplateArgumentList);
    if (!HasQualifierOrFoundDecl)
      return *reinterpret_cast<ExplicitTemplateArgumentList *>(this + 1);
...
private:
  ...
  void CheckBasePath() const {
    ...
    case CK_BaseToDerivedMemberPointer:
      LLVM_ASSERT(!path_empty() && "Cast kind should have a base path!");
      break;
    ...
    case CK_ObjCObjectLValueCast:
      LLVM_ASSERT(path_empty() && "Cast kind should not have a base path!");
      break;
    }
#endif
  }
...
public:
  ...
  BinaryOperator(Expr *lhs, Expr *rhs, Opcode opc, QualType ResTy,
    ...
    SubExprs[LHS] = lhs;
    SubExprs[RHS] = rhs;
    LLVM_ASSERT(!isCompoundAssignmentOp() &&
...
public:
  ...
  Expr *getExpr(unsigned Index) {
    LLVM_ASSERT((Index < NumExprs) && "Arg access out of range!");
  ...
  unsigned getShuffleMaskIdx(ASTContext &Ctx, unsigned N) {
    LLVM_ASSERT((N < NumExprs - 2) && "Shuffle idx out of range!");
...
public:
  ...
  const Expr* getInit(unsigned Init) const {
    LLVM_ASSERT(Init < getNumInits() && "Initializer access out of range!");
...
public:
  ...
  public:
    ...
    SourceLocation getLBracketLoc() const {
      LLVM_ASSERT((Kind == ArrayDesignator || Kind == ArrayRangeDesignator) &&
    ...
    SourceLocation getEllipsisLoc() const {
      LLVM_ASSERT(Kind == ArrayRangeDesignator &&
  ...
  Expr *getSubExpr(unsigned Idx) {
    LLVM_ASSERT(Idx < NumSubExprs && "Subscript out of range");
...
public:
  ...
  const Expr* getExpr(unsigned Init) const {
    LLVM_ASSERT(Init < getNumExprs() && "Initializer access out of range!");
