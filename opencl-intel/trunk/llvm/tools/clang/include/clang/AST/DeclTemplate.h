...
public:
  ...
  NamedDecl* getParam(unsigned Idx) {
    LLVM_ASSERT(Idx < size() && "Template parameter index out-of-range");
    return begin()[Idx];
  }
...
public:
  ...
  const TemplateArgument &get(unsigned Idx) const {
    LLVM_ASSERT(Idx < NumFlatArguments && "Invalid template argument index");
...
public:
  ...
  void init(NamedDecl *templatedDecl, TemplateParameterList* templateParams) {
    LLVM_ASSERT(TemplatedDecl == 0 && "TemplatedDecl already set!");
    LLVM_ASSERT(TemplateParams == 0 && "TemplateParams already set!");
...
public:
  ...
  FunctionTemplateDecl *getTemplate(unsigned I) const {
    LLVM_ASSERT(I < getNumTemplates() && "template index out of range");
  ...
  const TemplateArgumentLoc &getTemplateArg(unsigned I) const {
    LLVM_ASSERT(I < getNumTemplateArgs() && "template arg index out of range");
...
class RedeclarableTemplateDecl : public TemplateDecl {
  ...
  void setInstantiatedFromMemberTemplateImpl(RedeclarableTemplateDecl *TD) {
    LLVM_ASSERT(!getCommonPtr()->InstantiatedFromMember.getPointer());
...
protected:
  ...
  TemplateParmPosition()
    : Depth(0), Position(0)
  { /* LLVM_ASSERT(0 && "Cannot create positionless template parameter"); */ }
...
public:
  ...
  ClassTemplateSpecializationDecl *getMostRecentDeclaration() {
    ...
    if (!isa<ClassTemplateSpecializationDecl>(Recent)) {
      ...
      LLVM_ASSERT(Recent->isInjectedClassName() && Recent->getPreviousDeclaration());
  ...
  void initTemplateArgs(TemplateArgument *Args, unsigned NumArgs) {
    LLVM_ASSERT(TemplateArgs.flat_size() == 0 &&
  ...
  void setPointOfInstantiation(SourceLocation Loc) {
    LLVM_ASSERT(Loc.isValid() && "point of instantiation must be valid!");
  ...
  void setInstantiationOf(ClassTemplatePartialSpecializationDecl *PartialSpec,
                          TemplateArgumentList *TemplateArgs) {
    LLVM_ASSERT(!SpecializedTemplate.is<SpecializedPartialSpecialization*>() &&
...
public:
  ...
  void setMemberSpecialization() {
    ClassTemplatePartialSpecializationDecl *First
      = cast<ClassTemplatePartialSpecializationDecl>(getFirstDeclaration());
    LLVM_ASSERT(First->InstantiatedFromMember.getPointer() &&
           "Only member templates can be member template specializations");
  ...
  QualType getInjectedSpecializationType() const {
    LLVM_ASSERT(getTypeForDecl() && "partial specialization has no type set!");
