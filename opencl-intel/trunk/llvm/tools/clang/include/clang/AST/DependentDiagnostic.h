...
public:
  ...
  ddiag_iterator &operator++() {
    LLVM_ASSERT(Ptr && "attempt to increment past end of diag list");
  ...
  ddiag_iterator &operator+=(difference_type N) {
    LLVM_ASSERT(N >= 0 && "cannot rewind a DeclContext::ddiag_iterator");
    while (N--)
      ++*this;
    return *this;
  }
...
inline DeclContext::ddiag_iterator DeclContext::ddiag_begin() const {
  LLVM_ASSERT(isDependentContext()
