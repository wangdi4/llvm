...
public:
  ...
  friend_iterator &operator++() {
    LLVM_ASSERT(Ptr && "attempt to increment past end of friend list");
    Ptr = Ptr->NextFriend;
    return *this;
  }
  ...
  friend_iterator &operator+=(difference_type N) {
    LLVM_ASSERT(N >= 0 && "cannot rewind a CXXRecordDecl::friend_iterator");
    while (N--)
      ++*this;
    return *this;
  }
...
inline void CXXRecordDecl::pushFriendDecl(FriendDecl *FD) {
  LLVM_ASSERT(FD->NextFriend == 0 && "friend already has next friend?");
