...
public:
  ...
  TypeSourceInfo *getTypeOperandSourceInfo() const {
    LLVM_ASSERT(isTypeOperand() && "Cannot call getTypeOperand for typeid(expr)");
  ...
  Expr *getExprOperand() const {
    LLVM_ASSERT(!isTypeOperand() && "Cannot call getExprOperand for typeid(type)");
...
public:
  ...
  QualType getAllocatedType() const {
    LLVM_ASSERT(getType()->isPointerType());
  ...
  Expr *getPlacementArg(unsigned i) {
    LLVM_ASSERT(i < NumPlacementArgs && "Index out of range");
  ...
  Expr *getConstructorArg(unsigned i) {
    LLVM_ASSERT(i < NumConstructorArgs && "Index out of range");
...
public:
  ...
  static FindResult find(Expr *E) {
    LLVM_ASSERT(E->getType()->isSpecificBuiltinType(BuiltinType::Overload));

    FindResult Result;

    E = E->IgnoreParens();
    if (isa<UnaryOperator>(E)) {
      LLVM_ASSERT(cast<UnaryOperator>(E)->getOpcode() == UO_AddrOf);
...
public:
  ...
  CXXTemporary *getTemporary(unsigned i) {
    LLVM_ASSERT(i < NumTemps && "Index out of range");
    return Temps[i];
  }
...
public:
  ...
  Expr *getArg(unsigned I) {
    LLVM_ASSERT(I < NumArgs && "Argument index out-of-range");
    return *(arg_begin() + I);
  }
