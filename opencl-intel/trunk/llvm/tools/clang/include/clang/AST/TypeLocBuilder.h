...
#ifndef NDEBUG
    LLVM_ASSERT(LastTy.isNull() && "pushing copy on non-empty TypeLocBuilder");
    LastTy = L.getNextTypeLoc().getType();
#endif
    LLVM_ASSERT(Index == Capacity && "pushing copy on non-empty TypeLocBuilder");
...
#ifndef NDEBUG
    LLVM_ASSERT(T == LastTy && "type doesn't match last type pushed!");
#endif
...
private:
  ...
  void grow(size_t NewCapacity) {
    LLVM_ASSERT(NewCapacity > Capacity);
