...
public:
  ...
  void setOnlyValue(NamedDecl *ND) {
    LLVM_ASSERT(!getAsVector() && "Not inline");
    Data = ND;
    // Make sure that Data is a plain NamedDecl* so we can use its address
    // at getLookupResult.
    LLVM_ASSERT(*(NamedDecl **)&Data == ND &&
           "PointerUnion mangles the NamedDecl pointer!");
  }

  void remove(NamedDecl *D) {
    LLVM_ASSERT(!isNull() && "removing from empty list");
    if (NamedDecl *Singleton = getAsDecl()) {
      LLVM_ASSERT(Singleton == D && "list is different singleton");
      (void)Singleton;
      Data = (NamedDecl *)0;
      return;
    }
    ...
    LLVM_ASSERT(I != Vec.end() && "list does not contain decl");
    Vec.erase(I);

    LLVM_ASSERT(std::find(Vec.begin(), Vec.end(), D)
  ...
  DeclContext::lookup_result getLookupResult() {
    ...
    if (getAsDecl()) {
      LLVM_ASSERT(!isNull() && "Empty list isn't allowed");
    ...
    LLVM_ASSERT(getAsVector() && "Must have a vector at this point");
