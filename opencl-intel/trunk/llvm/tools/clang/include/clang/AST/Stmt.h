...
protected:
  void* operator new(size_t bytes) throw() {
    LLVM_ASSERT(0 && "Stmts cannot be allocated with regular 'new'.");
    return 0;
  }
  void operator delete(void* data) throw() {
    LLVM_ASSERT(0 && "Stmts cannot be released with regular 'delete'.");
  }
...
public:
  ...
  Stmt *Retain() {
    LLVM_ASSERT(RefCount >= 1);
    ++RefCount;
    return this;
  }

  StmtClass getStmtClass() const { 
    LLVM_ASSERT(RefCount >= 1 && "Referencing already-destroyed statement!");
...
public:
  ...
  void addSwitchCase(SwitchCase *SC) {
    LLVM_ASSERT(!SC->getNextSwitchCase() && "case/default already added to a switch");
    SC->Retain();
...
public:
  ...
  public:
    ...
    const std::string &getString() const {
      LLVM_ASSERT(isString());
      return Str;
    }

    unsigned getOperandNo() const {
      LLVM_ASSERT(isOperand());
      return OperandNo;
    }
