...
bool RecursiveASTVisitor<Derived>::TraverseClassInstantiations(
  ...
  LLVM_ASSERT(isa<ClassTemplateDecl>(Pattern) ||
...
bool RecursiveASTVisitor<Derived>::TraverseFunctionInstantiations(
  ...
  for (FunctionTemplateDecl::spec_iterator it = D->spec_begin(); it != end; ++it) {
    ...
    case TSK_ExplicitSpecialization:
      break;
    default:
      LLVM_ASSERT(false && "Unknown specialization kind.");
    }
  }
...
bool RecursiveASTVisitor<Derived>::TraverseFunctionHelper(FunctionDecl *D) {
  ...
  if (FunctionProtoType *FuncProto = dyn_cast<FunctionProtoType>(FuncType)) {
    ...
    LLVM_ASSERT(!D->isThisDeclarationADefinition() && "Unexpected function type");
