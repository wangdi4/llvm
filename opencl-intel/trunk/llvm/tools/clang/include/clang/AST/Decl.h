...
public:
  ...
  llvm::StringRef getName() const {
    LLVM_ASSERT(Name.isIdentifier() && "Name is not a simple identifier");
...
public:
  ...
  void setAnonymousNamespace(NamespaceDecl *D) {
    LLVM_ASSERT(!D || D->isAnonymousNamespace());
    LLVM_ASSERT(!D || D->getParent() == this);
...
public:
  ...
  TemplateParameterList *getTemplateParameterList(unsigned index) const {
    LLVM_ASSERT(index < getNumTemplateParameterLists());
...
public:
  ...
  void setStorageClass(StorageClass SC) {
    LLVM_ASSERT(isLegalForVariable(SC));
    SClass = SC;
  }
...
public:
  ...
  const ParmVarDecl *getParamDecl(unsigned i) const {
    LLVM_ASSERT(i < getNumParams() && "Illegal param #");
    return ParamInfo[i];
  }
  ...
  StorageClass getStorageClass() const { return StorageClass(SClass); }
  void setStorageClass(StorageClass SC) {
    LLVM_ASSERT(isLegalForFunction(SC));
    SClass = SC;
  }
...
protected:
  TagDecl(Kind DK, TagKind TK, DeclContext *DC,
    ...
    LLVM_ASSERT((DK != Enum || TK == TTK_Enum) &&
...
public:
  ...
  TemplateParameterList *getTemplateParameterList(unsigned i) const {
    LLVM_ASSERT(i < getNumTemplateParameterLists());
...
public:
  ...
  void setNumPositiveBits(unsigned Num) {
    NumPositiveBits = Num;
    LLVM_ASSERT(NumPositiveBits == Num && "can't store this bitcount");
  }
