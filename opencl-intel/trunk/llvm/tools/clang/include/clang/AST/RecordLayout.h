...
public:
  ...
  uint64_t getNonVirtualSize() const {
    LLVM_ASSERT(CXXInfo && "Record layout does not have C++ specific info!");
  ...
  uint64_t getBaseClassOffset(const CXXRecordDecl *Base) const {
    LLVM_ASSERT(CXXInfo && "Record layout does not have C++ specific info!");
    LLVM_ASSERT(CXXInfo->BaseOffsets.count(Base) && "Did not find base!");
  ...
  uint64_t getVBaseClassOffset(const CXXRecordDecl *VBase) const {
    LLVM_ASSERT(CXXInfo && "Record layout does not have C++ specific info!");
    LLVM_ASSERT(CXXInfo->VBaseOffsets.count(VBase) && "Did not find base!");
