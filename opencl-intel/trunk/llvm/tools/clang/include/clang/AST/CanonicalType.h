...
CanQual<T> CanQual<T>::getFromOpaquePtr(void *Ptr) {
  ...
  LLVM_ASSERT((!Result || Result.Stored.getAsOpaquePtr() == (void*)-1 ||
...
CanQual<T> CanQual<T>::CreateUnsafe(QualType Other) {
  LLVM_ASSERT((Other.isNull() || Other.isCanonical()) && "Type is not canonical!");
  LLVM_ASSERT((Other.isNull() || isa<T>(Other.getTypePtr())) &&
