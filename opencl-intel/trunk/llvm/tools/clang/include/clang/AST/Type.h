...
public:
  ...
  void setCVRQualifiers(unsigned mask) {
    LLVM_ASSERT(!(mask & ~CVRMask) && "bitmask contains non-CVR bits");
  ...
  void setAddressSpace(unsigned space) {
    LLVM_ASSERT(space <= MaxAddressSpace);
  ...
  void setFastQualifiers(unsigned mask) {
    LLVM_ASSERT(!(mask & ~FastMask) && "bitmask contains non-fast qualifier bits");
...
public:
  ...
  {
    LLVM_ASSERT(Quals.hasNonFastQualifiers()
           && "ExtQuals created with no fast qualifiers");
    LLVM_ASSERT(!Quals.hasFastQualifiers()
...
public:
  ...
  static void Profile(llvm::FoldingSetNodeID &ID,
                      const Type *BaseType,
                      Qualifiers Quals) {
    LLVM_ASSERT(!Quals.hasFastQualifiers() && "fast qualifiers in ExtQuals hash!");
...
enum CallingConv {
  ...
  CC_X86SvmlCall,  // __attribute__((svmlcc))
  CC_X86Svml_P2_F8_F8_F8_Call,  // __attribute__((svmlcc_p2_f8_f8_f8))
...
public:
  ...
  void addFastQualifiers(unsigned TQs) {
    LLVM_ASSERT(!(TQs & ~Qualifiers::FastMask)
  ...
  void removeFastQualifiers(unsigned Mask) {
    LLVM_ASSERT(!(Mask & ~Qualifiers::FastMask) && "mask has non-fast qualifiers");
...
public:
  ...
  bool isFloatingVecType() const;  // C99 6.2.5p11 (floating vector)
  bool isRealType() const;         // C99 6.2.5p17 (real floating + integer)
  bool isRealVecType() const;      // C99 6.2.5p17 (vector: floating + integer)
  ...
  /// OpenCL's version of isPromotableIntegerType
  bool isOpenCLPromotableIntegerType() const;
  ...
  bool isSignedIntOrIntVecType() const;
  ...
  bool isUnsignedIntOrIntVecType() const;
...
public:
  ...
  void Profile(llvm::FoldingSetNodeID &ID) {
    LLVM_ASSERT(0 && "Cannnot unique VariableArrayTypes.");
  }
};
...
public:
  ...
  QualType getArgType(unsigned i) const {
    LLVM_ASSERT(i < NumArgs && "Invalid argument number!");
  ...
  QualType getExceptionType(unsigned i) const {
    LLVM_ASSERT(i < NumExceptions && "Invalid exception number!");
...
protected:
  TypedefType(TypeClass tc, const TypedefDecl *D, QualType can)
    ...
    LLVM_ASSERT(!isa<TypedefType>(can) && "Invalid canonical type");
  }
  friend class ASTContext;  // ASTContext creates these.
public:
...
class InjectedClassNameType : public Type {
  ...
  InjectedClassNameType(CXXRecordDecl *D, QualType TST)
    ...
    LLVM_ASSERT(isa<TemplateSpecializationType>(TST));
    LLVM_ASSERT(!TST.hasQualifiers());
    LLVM_ASSERT(TST->isDependentType());
  }
...
class ElaboratedType : public TypeWithKeyword, public llvm::FoldingSetNode {
  ...
  ElaboratedType(ElaboratedTypeKeyword Keyword, NestedNameSpecifier *NNS,
    ...
    LLVM_ASSERT(!(Keyword == ETK_None && NNS == 0) &&
...
public:
  ...
  ObjCProtocolDecl *getProtocol(unsigned I) const {
    LLVM_ASSERT(I < getNumProtocols() && "Out-of-range protocol access");
    return qual_begin()[I];
  }
...
inline void QualType::removeCVRQualifiers(unsigned Mask) {
  LLVM_ASSERT(!(Mask & ~Qualifiers::CVRMask) && "mask has non-CVR bits");
