...
class CXXRecordDecl : public RecordDecl {
  ...
  struct DefinitionData &data() {
    LLVM_ASSERT(DefinitionData && "queried property of class with no definition");
...
public:
  ...
  static AccessSpecifier MergeAccess(AccessSpecifier PathAccess,
    ...
    LLVM_ASSERT(DeclAccess != AS_none);
...
public:
  ...
  bool isBaseVirtual() const {
    LLVM_ASSERT(isBaseInitializer() && "Must call this on base initializer!");
  ...
  void setMember(FieldDecl *Member) {
    LLVM_ASSERT(isMemberInitializer());
    BaseOrMember = Member;
  }
  ...
  void setSourceOrder(int pos) {
    LLVM_ASSERT(!IsWritten &&
           "calling twice setSourceOrder() on the same initializer");
    LLVM_ASSERT(SourceOrderOrNumArrayIndices == 0 &&
  ...
  VarDecl *getArrayIndex(unsigned I) {
    LLVM_ASSERT(I < getNumArrayIndices() && "Out of bounds member array index");
...
public:
  ...
  bool isImplicitlyDefined() const {
    LLVM_ASSERT(isThisDeclarationADefinition() &&
...
public:
  ...
  void setTargetDecl(NamedDecl* ND) {
    LLVM_ASSERT(ND && "Target decl is null!");
    Underlying = ND;
...
public:
  ...
  void addShadowDecl(UsingShadowDecl *S) {
    LLVM_ASSERT(S->getUsingDecl() == this);
    if (!Shadows.insert(S)) {
      LLVM_ASSERT(false && "declaration already in set");
    }
  }
  void removeShadowDecl(UsingShadowDecl *S) {
    LLVM_ASSERT(S->getUsingDecl() == this);
    if (!Shadows.erase(S)) {
      LLVM_ASSERT(false && "declaration not in set");
    }
  }
