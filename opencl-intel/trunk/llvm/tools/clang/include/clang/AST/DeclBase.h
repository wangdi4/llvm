...
public:
  ...
  void setPCHLevel(unsigned Level) { 
    LLVM_ASSERT(Level <= MaxPCHLevel && "PCH level exceeds the maximum");
    PCHLevel = Level;
  }
...
public:
  ...
  public:
    ...
    redecl_iterator& operator++() {
      LLVM_ASSERT(Current && "Advancing while iterator has reached end");
      // Get either previous decl or latest decl.
      Decl *Next = Current->getNextRedeclaration();
      LLVM_ASSERT(Next && "Should return next redeclaration or itself, never null!");
  ...
  void setObjectOfFriendDecl(bool PreviouslyDeclared) {
    ...
    LLVM_ASSERT((OldNS & (IDNS_Tag | IDNS_Ordinary |
                     IDNS_TagFriend | IDNS_OrdinaryFriend)) &&
           "namespace includes neither ordinary nor tag");
    LLVM_ASSERT(!(OldNS & ~(IDNS_Tag | IDNS_Ordinary | IDNS_Type |
                       IDNS_TagFriend | IDNS_OrdinaryFriend)) &&
  ...
  void setNonMemberOperator() {
    LLVM_ASSERT(getKind() == Function || getKind() == FunctionTemplate);
    LLVM_ASSERT((IdentifierNamespace & IDNS_Ordinary) &&
