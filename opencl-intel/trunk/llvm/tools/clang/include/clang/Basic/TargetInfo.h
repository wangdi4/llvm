...
protected:
  ...
  bool supportedOclExtensions[OpenCL::NUM_EXTENSIONS];
...
public:
  ...
  public:
    ...
    unsigned getTiedOperand() const {
      LLVM_ASSERT(hasTiedOperand() && "Has no tied operand!");
  ...
  virtual bool setOpenCLExtensionStatus(const std::string &Name,
                                                   bool status)
  {
    OpenCL::Extension ext = OpenCL::convertExtNameToEnum( Name);

    if (ext == OpenCL::unknownExtension){
      return false;
    }

    supportedOclExtensions[ext] = status;
    return true;
  }

  virtual bool isOpenCLExtensionSupported(const std::string &Name) const
  {
    ...
    return supportedOclExtensions[ext];
  }

  virtual bool isOpenCLExtensionSupported(int ext) const
  {
    if (ext == OpenCL::unknownExtension){
      return false;
    }
