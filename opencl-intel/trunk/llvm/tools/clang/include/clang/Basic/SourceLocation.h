...
private:
  ...
  static SourceLocation getFileLoc(unsigned ID) {
    LLVM_ASSERT((ID & MacroIDBit) == 0 && "Ran out of source locations!");
    SourceLocation L;
    L.ID = ID;
    return L;
  }
...
public:
  ...
  SourceLocation getFileLocWithOffset(int Offset) const {
    LLVM_ASSERT(((getOffset()+Offset) & MacroIDBit) == 0 && "invalid location");
    SourceLocation L;
    L.ID = ID+Offset;
    return L;
  }
...
public:
  ...
  const SourceManager &getManager() const {
    LLVM_ASSERT(SrcMgr && "SourceManager is NULL.");
    return *SrcMgr;
  }
