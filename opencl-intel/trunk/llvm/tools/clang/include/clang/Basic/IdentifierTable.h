...
public:
  ...
  void RevertTokenIDToIdentifier() {
    LLVM_ASSERT(TokenID != tok::identifier && "Already at tok::identifier");
  ...
  void setBuiltinID(unsigned ID) {
    ...
    LLVM_ASSERT(ObjCOrBuiltinID - unsigned(tok::NUM_OBJC_KEYWORDS) == ID
...
class Selector {
  ...
  Selector(IdentifierInfo *II, unsigned nArgs) {
    ...
    LLVM_ASSERT((InfoPtr & ArgFlags) == 0 &&"Insufficiently aligned IdentifierInfo");
    LLVM_ASSERT(nArgs < 2 && "nArgs not equal to 0/1");
    InfoPtr |= nArgs+1;
  }
