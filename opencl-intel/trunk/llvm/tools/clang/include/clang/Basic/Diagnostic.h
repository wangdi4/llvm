...
public:
  ...
  void setDiagnosticMapping(diag::kind Diag, diag::Mapping Map) {
    LLVM_ASSERT(Diag < diag::DIAG_UPPER_LIMIT &&
           "Can only map builtin diagnostics");
    LLVM_ASSERT((isBuiltinWarningOrExtension(Diag) ||
...
public:
  ...
  void AddString(llvm::StringRef S) const {
    LLVM_ASSERT(NumArgs < Diagnostic::MaxArguments &&
  ...
  void AddFixItHint(const FixItHint &Hint) const {
    ...
    LLVM_ASSERT(NumFixItHints < Diagnostic::MaxFixItHints &&
...
inline DiagnosticBuilder Diagnostic::Report(FullSourceLoc Loc, unsigned DiagID){
  LLVM_ASSERT(CurDiagID == ~0U && "Multiple diagnostics in flight at once!");
  CurDiagLoc = Loc;
  CurDiagID = DiagID;
...
public:
  ...
  Diagnostic::ArgumentKind getArgKind(unsigned Idx) const {
    LLVM_ASSERT(Idx < getNumArgs() && "Argument index out of range!");
    return (Diagnostic::ArgumentKind)DiagObj->DiagArgumentsKind[Idx];
  }

  /// getArgStdStr - Return the provided argument string specified by Idx.
  const std::string &getArgStdStr(unsigned Idx) const {
    LLVM_ASSERT(getArgKind(Idx) == Diagnostic::ak_std_string &&
  ...
  const char *getArgCStr(unsigned Idx) const {
    LLVM_ASSERT(getArgKind(Idx) == Diagnostic::ak_c_string &&
           "invalid argument accessor!");
  ...
  int getArgSInt(unsigned Idx) const {
    LLVM_ASSERT(getArgKind(Idx) == Diagnostic::ak_sint &&
           "invalid argument accessor!");
  ...
  unsigned getArgUInt(unsigned Idx) const {
    LLVM_ASSERT(getArgKind(Idx) == Diagnostic::ak_uint &&
           "invalid argument accessor!");
  ...
  const IdentifierInfo *getArgIdentifier(unsigned Idx) const {
    LLVM_ASSERT(getArgKind(Idx) == Diagnostic::ak_identifierinfo &&
           "invalid argument accessor!");
  ...
  const CharSourceRange &getRange(unsigned Idx) const {
    LLVM_ASSERT(Idx < DiagObj->NumDiagRanges && "Invalid diagnostic range index!");
