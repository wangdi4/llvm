...
public:
  ...
  io::Offset Emit(llvm::raw_ostream &out, Info &InfoObj) {
    ...
    for (unsigned i = 0; i < NumBuckets; ++i) {
      ...
      B.off = out.tell();
      LLVM_ASSERT(B.off && "Cannot write a bucket at offset 0. Please add padding.");
...
public:
  ...
  OnDiskChainedHashTable(unsigned numBuckets, unsigned numEntries,
    ...
    : NumBuckets(numBuckets), NumEntries(numEntries),
      Buckets(buckets), Base(base), InfoObj(InfoObj) {
        LLVM_ASSERT((reinterpret_cast<uintptr_t>(buckets) & 0x3) == 0 &&
  ...
  public:
    ...
    item_iterator& operator++() {  // Preincrement
      ...
      LLVM_ASSERT(NumItemsInBucketLeft);
      --NumItemsInBucketLeft;
      LLVM_ASSERT(NumEntriesLeft);
      --NumEntriesLeft;
      return *this;
    }
