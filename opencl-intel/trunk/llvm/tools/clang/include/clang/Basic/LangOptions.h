...
public:
  ...
  unsigned OpenCLBuiltins    : 1; // OpenCL builtins extensions.
  unsigned OpenCLKernelRefl  : 1; // OpenCL kernel reflection
...
public:
  ...
  explicit FPOptions(bool contract) {
...
#include "clang/Basic/OpenCLExtensions.def"
  ...
  bool isOpenCLExtensionEnabled(unsigned ext) {
#define OPENCLEXT(nm) \
    if ((ext == OpenCL::nm) && (nm == 1)) \
      return true;
#include "clang/Basic/OpenCLExtensions.def"
    return false;
  }
