...
namespace SrcMgr {
  ...
  public:
    ...
    void setBuffer(const llvm::MemoryBuffer *B) {
      LLVM_ASSERT(!Buffer.getPointer() && "MemoryBuffer already set.");
      Buffer.setPointer(B);
      Buffer.setInt(false);
    }
  ...
  public:
    ...
    static FileInfo get(SourceLocation IL, const ContentCache *Con,
      ...
      LLVM_ASSERT((X.Data & 7) == 0 &&"ContentCache pointer insufficiently aligned");
      LLVM_ASSERT((unsigned)FileCharacter < 4 && "invalid file character");
  ...
  public:
    ...
    const FileInfo &getFile() const {
      LLVM_ASSERT(isFile() && "Not a file SLocEntry!");
      return File;
    }

    const InstantiationInfo &getInstantiation() const {
      LLVM_ASSERT(isInstantiation() && "Not an instantiation SLocEntry!");
...
public:
  ...
  FileID createMainFileID(const FileEntry *SourceFile) {
    LLVM_ASSERT(MainFileID.isInvalid() && "MainFileID already set!");
  ...
  SourceLocation getLocForStartOfFile(FileID FID) const {
    LLVM_ASSERT(FID.ID < SLocEntryTable.size() && "FileID out of range");
    LLVM_ASSERT(getSLocEntry(FID).isFile() && "FileID is not a file");
  ...
  const SrcMgr::SLocEntry &getSLocEntry(unsigned ID) const {
    LLVM_ASSERT(ID < SLocEntryTable.size() && "Invalid id");
