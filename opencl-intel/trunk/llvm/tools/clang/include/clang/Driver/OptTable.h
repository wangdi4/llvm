...
namespace options {
  ...
  private:
    const Info &getInfo(OptSpecifier Opt) const {
      ...
      LLVM_ASSERT(id > 0 && id - 1 < getNumOptions() && "Invalid Option ID.");
      return OptionInfos[id - 1];
    }

    Option *CreateOption(unsigned id) const;

	OptTable (const OptTable &o);
	OptTable& operator=(const OptTable &o);
  ...
  public:
    ...
    const Option *getOption(OptSpecifier Opt) const {
      ...
      LLVM_ASSERT((unsigned) (id - 1) < getNumOptions() && "Invalid ID.");
