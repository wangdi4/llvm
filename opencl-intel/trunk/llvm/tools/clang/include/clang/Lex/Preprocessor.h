...
class Preprocessor {
  ...
  /// PchBuffer - An optional pre compiled header buffer
  llvm::MemoryBuffer* PchBuffer;
...
public:
  ...
  llvm::MemoryBuffer* getPCH() {return PchBuffer;}
  ...
  void RevertCachedTokens(unsigned N) {
    LLVM_ASSERT(isBacktrackEnabled() &&
           "Should only be called when tokens are cached for backtracking");
    LLVM_ASSERT(signed(CachedLexPos) - signed(N) >= signed(BacktrackPositions.back())
         && "Should revert tokens up to the last backtrack position, not more");
    LLVM_ASSERT(signed(CachedLexPos) - signed(N) >= 0 &&
  ...
  char getSpellingOfSingleCharacterNumericConstant(const Token &Tok, 
                                                   bool *Invalid = 0) const {
    LLVM_ASSERT(Tok.is(tok::numeric_constant) &&
           Tok.getLength() == 1 && "Called on unsupported token");
    LLVM_ASSERT(!Tok.needsCleaning() && "Token can't need cleaning with length 1");
