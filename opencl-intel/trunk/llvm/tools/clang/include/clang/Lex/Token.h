...
public:
  ...
  unsigned getLength() const {
    LLVM_ASSERT(!isAnnotation() && "Annotation tokens have no length field");
    return UintData;
  }
  ...
  SourceLocation getAnnotationEndLoc() const {
    LLVM_ASSERT(isAnnotation() && "Used AnnotEndLocID on non-annotation token");
  ...
  IdentifierInfo *getIdentifierInfo() const {
    LLVM_ASSERT(!isAnnotation() && "Used IdentInfo on annotation token!");
  ...
  const char *getLiteralData() const {
    LLVM_ASSERT(isLiteral() && "Cannot get literal data of non-literal");
    return reinterpret_cast<const char*>(PtrData);
  }
  void setLiteralData(const char *Ptr) {
    LLVM_ASSERT(isLiteral() && "Cannot set literal data of non-literal");
    PtrData = const_cast<char*>(Ptr);
  }

  void *getAnnotationValue() const {
    LLVM_ASSERT(isAnnotation() && "Used AnnotVal on non-annotation token");
    return PtrData;
  }
