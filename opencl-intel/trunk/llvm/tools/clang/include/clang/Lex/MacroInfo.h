...
private:
  ...
  bool IsAllowRedefinitionsWithoutWarning : 1;
   ...
   ~MacroInfo() {
    LLVM_ASSERT(ArgumentList == 0 && "Didn't call destroy before dtor!");
  }
...
public:
  ...
  void setArgumentList(IdentifierInfo* const *List, unsigned NumArgs,
    ...
    LLVM_ASSERT(ArgumentList == 0 && NumArguments == 0 &&
  ...
  const Token &getReplacementToken(unsigned Tok) const {
    LLVM_ASSERT(Tok < ReplacementTokens.size() && "Invalid token #");
  ...
  void EnableMacro() {
    LLVM_ASSERT(IsDisabled && "Cannot enable an already-enabled macro!");
    IsDisabled = false;
  }

  void DisableMacro() {
    LLVM_ASSERT(!IsDisabled && "Cannot disable an already-disabled macro!");
    IsDisabled = true;
  }
};
