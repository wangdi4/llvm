...
public:
  ...
  const SourceManager& getSourceManager() const { LLVM_ASSERT(isValid());return *SM;}

  FullSourceLoc asLocation() const;
  PathDiagnosticRange asRange() const;
  const Stmt *asStmt() const { LLVM_ASSERT(isValid()); return S; }
  const Decl *asDecl() const { LLVM_ASSERT(isValid()); return D; }
  ...
  const SourceManager& getManager() const { LLVM_ASSERT(isValid()); return *SM; }
...
public:
  ...
  : PathDiagnosticPiece(s, k), Pos(pos) {
    LLVM_ASSERT(Pos.asLocation().isValid() &&
...
public:
  ...
  PathDiagnosticLocation getLocation() const {
    LLVM_ASSERT(Size > 0 && "getLocation() requires a non-empty PathDiagnostic.");
