...
public:
  ...
  public:
    explicit ConcurrencyCheck(ASTUnit &Self)
      : Self(Self) 
    { 
      LLVM_ASSERT(Self.ConcurrencyCheckValue == CheckUnlocked && 
  ...
  Sema &getSema() const { 
    LLVM_ASSERT(TheSema && "ASTUnit does not have a Sema object!");
    return *TheSema; 
  }
  ...
  top_level_iterator top_level_begin() {
    LLVM_ASSERT(!isMainFileAST() && "Invalid call for AST based ASTUnit!");
