...
class CompilerInstance {
  ...
  llvm::MemoryBuffer*					InputBuffer;		// Force this input memory buffer

  llvm::raw_ostream*          OutputStream;		// Force this output buffer

  llvm::raw_ostream*					ErrorStream;		// Force this error stream

  llvm::MemoryBuffer*					PchBuffer;			// Force this pch buffer
public:
  CompilerInstance();
  ~CompilerInstance();
  ...
  llvm::LLVMContext &getLLVMContext() const {
    LLVM_ASSERT(LLVMContext && "Compiler instance has no LLVM context!");
    return *LLVMContext;
  }
  ...
  CompilerInvocation &getInvocation() {
    LLVM_ASSERT(Invocation && "Compiler instance has no invocation!");
    return *Invocation;
  }
  ...
  Diagnostic &getDiagnostics() const {
    LLVM_ASSERT(Diagnostics && "Compiler instance has no diagnostics!");
    return *Diagnostics;
  }
  ...
  DiagnosticClient &getDiagnosticClient() const {
    LLVM_ASSERT(Diagnostics && Diagnostics->getClient() && 
  ...
  TargetInfo &getTarget() const {
    LLVM_ASSERT(Target && "Compiler instance has no target!");
    return *Target;
  }
  ...
  FileManager &getFileManager() const {
    LLVM_ASSERT(FileMgr && "Compiler instance has no file manager!");
    return *FileMgr;
  }
  ...
  SourceManager &getSourceManager() const {
    LLVM_ASSERT(SourceMgr && "Compiler instance has no source manager!");
    return *SourceMgr;
  }
  ...
  Preprocessor &getPreprocessor() const {
    LLVM_ASSERT(PP && "Compiler instance has no preprocessor!");
    return *PP;
  }
  ...
  ASTContext &getASTContext() const {
    LLVM_ASSERT(Context && "Compiler instance has no AST context!");
    return *Context;
  }
  ...
  ASTConsumer &getASTConsumer() const {
    LLVM_ASSERT(Consumer && "Compiler instance has no AST consumer!");
    return *Consumer;
  }
  ...
  Sema &getSema() const { 
    LLVM_ASSERT(TheSema && "Compiler instance has no Sema object!");
    return *TheSema;
  }
  ...
  CodeCompleteConsumer &getCodeCompletionConsumer() const {
    LLVM_ASSERT(CompletionConsumer &&
  ...
  llvm::Timer &getFrontendTimer() const {
    LLVM_ASSERT(FrontendTimer && "Compiler instance has no frontend timer!");
  ...
  createDiagnostics(const DiagnosticOptions &Opts, int Argc, char **Argv,
						 llvm::raw_ostream* errStream = NULL);
  ...
  void SetInputBuffer(llvm::MemoryBuffer* MemBuffer)
  {
	  InputBuffer = MemBuffer;
  }

  void SetErrorStream(llvm::raw_ostream* ErrStream)
  {
	  ErrorStream = ErrStream;
  }

  void SetOutputStream(llvm::raw_ostream* OutStream)
  {
	  OutputStream = OutStream;
  }

  void SetPchBuffer(llvm::MemoryBuffer* PchBuff)
  {
      PchBuffer = PchBuff;
  }

  llvm::MemoryBuffer* GetPchBuffer() 
  {
	  return PchBuffer;
  }

  llvm::raw_ostream* GetForcedOutputStream()
  {
	  return OutputStream;
  }
};
