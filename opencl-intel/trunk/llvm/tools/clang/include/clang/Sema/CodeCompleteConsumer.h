...
public:
  ...
  Chunk &operator[](unsigned I) {
    LLVM_ASSERT(I < size() && "Chunk index out-of-range");
    return Chunks[I];
  }
...
public:
  ...
  NamedDecl *getDeclaration() const {
    LLVM_ASSERT(Kind == RK_Declaration && "Not a declaration result");
    return Declaration;
  }
    
  /// \brief Retrieve the keyword stored in this result.
  const char *getKeyword() const {
    LLVM_ASSERT(Kind == RK_Keyword && "Not a keyword result");
    return Keyword;
  }
...
public:
  ...
  public:
    ...
    FunctionTemplateDecl *getFunctionTemplate() const {
      LLVM_ASSERT(getKind() == CK_FunctionTemplate && "Not a function template");
