...
public:
  ...
  void setNotFoundInCurrentInstantiation() {
    LLVM_ASSERT(ResultKind == NotFound && Decls.empty());
  ...
  NamedDecl *getFoundDecl() const {
    LLVM_ASSERT(getResultKind() == Found
  ...
  NamedDecl *getRepresentativeDecl() const {
    LLVM_ASSERT(!Decls.empty() && "cannot get representative of empty set");
    return *begin();
  }
  ...
  public:
    ...
    NamedDecl *next() {
      LLVM_ASSERT(I != Results.end() && "next() called on empty filter");
      return *I++;
    }
    ...
    void done() {
#ifndef NDEBUG
      LLVM_ASSERT(!CalledDone && "done() called twice");
      CalledDone = true;
#endif
