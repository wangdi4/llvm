...
public:
  ...
  const IdentifierInfo *getField() const {
    LLVM_ASSERT(isFieldDesignator() && "Invalid accessor");
    return FieldInfo.II;
  }
  ...
  Expr *getArrayIndex() const {
    LLVM_ASSERT(isArrayDesignator() && "Invalid accessor");
    return ArrayInfo.Index;
  }

  Expr *getArrayRangeStart() const {
    LLVM_ASSERT(isArrayRangeDesignator() && "Invalid accessor");
  ...
  SourceLocation getLBracketLoc() const {
    LLVM_ASSERT((isArrayDesignator() || isArrayRangeDesignator()) &&
           "Invalid accessor");
...
public:
  ...
  const Designator &getDesignator(unsigned Idx) const {
    LLVM_ASSERT(Idx < Designators.size());
