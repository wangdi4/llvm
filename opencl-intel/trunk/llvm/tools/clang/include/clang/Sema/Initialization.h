...
public:
  ...
  CXXBaseSpecifier *getBaseSpecifier() const {
    LLVM_ASSERT(getKind() == EK_Base && "Not a base specifier");
  ...
  SourceLocation getReturnLoc() const {
    LLVM_ASSERT(getKind() == EK_Result && "No 'return' location!");
  ...
  SourceLocation getThrowLoc() const {
    LLVM_ASSERT(getKind() == EK_Exception && "No 'throw' location!");
    return SourceLocation::getFromRawEncoding(LocAndNRVO.Location);
  }
  ...
  void setElementIndex(unsigned Index) {
    LLVM_ASSERT(getKind() == EK_ArrayElement || getKind() == EK_VectorElement);
    this->Index = Index;
  }
};
...
public:
  ...
  SourceLocation getEqualLoc() const {
    LLVM_ASSERT(Kind == SIK_Copy && "Only copy initialization has an '='");
    return Locations[1];
  }
  ...
  SourceRange getParenRange() const {
    LLVM_ASSERT((getKind() == IK_Direct || Kind == SIK_Value) &&
...
public:
  ...
  FailureKind getFailureKind() const {
    LLVM_ASSERT(getKind() == FailedSequence && "Not an initialization failure!");
    return Failure;
  }
