...
public:
  ...
  ParsedType getRepAsType() const {
    LLVM_ASSERT(isTypeRep((TST) TypeSpecType) && "DeclSpec does not store a type");
    return TypeRep;
  }
  Decl *getRepAsDecl() const {
    LLVM_ASSERT(isDeclRep((TST) TypeSpecType) && "DeclSpec does not store a decl");
    return DeclRep;
  }
  Expr *getRepAsExpr() const {
    LLVM_ASSERT(isExprRep((TST) TypeSpecType) && "DeclSpec does not store an expr");
    return ExprRep;
  }
  ...
  void UpdateDeclRep(Decl *Rep) {
    LLVM_ASSERT(isDeclRep((TST) TypeSpecType));
    DeclRep = Rep;
  }
  void UpdateTypeRep(ParsedType Rep) {
    LLVM_ASSERT(isTypeRep((TST) TypeSpecType));
    TypeRep = Rep;
  }
  void UpdateExprRep(Expr *Rep) {
    LLVM_ASSERT(isExprRep((TST) TypeSpecType));
    ExprRep = Rep;
  }
...
public:
  ...
  UnqualifiedId(const UnqualifiedId &Other) 
    ...
    LLVM_ASSERT(Other.Kind == IK_Identifier && "Cannot copy non-identifiers");
  }
...
struct DeclaratorChunk {
  ...
  void destroy() {
    switch (Kind) {
    default: LLVM_ASSERT(0 && "Unknown decl type!");
  ...
  const AttributeList *getAttrs() const {
    switch (Kind) {
    default: LLVM_ASSERT(0 && "Unknown declarator kind!");
...
public:
  ...
  const DeclaratorChunk &getTypeObject(unsigned i) const {
    LLVM_ASSERT(i < DeclTypeInfo.size() && "Invalid type chunk");
  ...
  {
    LLVM_ASSERT(!DeclTypeInfo.empty() && "No type chunks to drop.");
  ...
  const AttributeList *getAttributes() const { return AttrList; }
  AttributeList *getAttributes() { return AttrList; }
  void setAttributes(AttributeList* AttrList) { this->AttrList = AttrList; }
