...
class Parser : public CodeCompletionHandler {
  ...
  llvm::OwningPtr<PragmaHandler> OpenCLFPContractHandler;
...
private:
  ...
  SourceLocation ConsumeToken() {
    LLVM_ASSERT(!isTokenStringLiteral() && !isTokenParen() && !isTokenBracket() &&
           !isTokenBrace() &&
  ...
  SourceLocation ConsumeParen() {
    LLVM_ASSERT(isTokenParen() && "wrong consume method");
  ...
  SourceLocation ConsumeBracket() {
    LLVM_ASSERT(isTokenBracket() && "wrong consume method");
  ...
  SourceLocation ConsumeBrace() {
    LLVM_ASSERT(isTokenBrace() && "wrong consume method");
  ...
  SourceLocation ConsumeStringToken() {
    LLVM_ASSERT(isTokenStringLiteral() &&
  ...
  SourceLocation ConsumeCodeCompletionToken() {
    LLVM_ASSERT(Tok.is(tok::code_completion));
    PrevTokLocation = Tok.getLocation();
    PP.Lex(Tok);
    return PrevTokLocation;    
  }
  ...
  public:
    ...
    void Commit() {
      LLVM_ASSERT(isActive && "Parsing action was finished!");
    ...
    ~TentativeParsingAction() {
      LLVM_ASSERT(!isActive && "Forgot to call Commit or Revert!");
    }
  };
...
private:
  ...
  ParsingClass &getCurrentClass() {
    LLVM_ASSERT(!ClassStack.empty() && "No lexed method stacks!");
  ...
  public:
    ...
    void complete(Decl *D) {
      LLVM_ASSERT(!Popped && "ParsingDeclaration has already been popped!");
      pop(D);
    }
  ...
  public:
    ...
    void Pop() {
      LLVM_ASSERT(!Popped && "Nested class has already been popped");
      Popped = true;
      P.PopParsingClass();
    }
  ...
  public:
    ...
    void EnterDeclaratorScope() {
      LLVM_ASSERT(!EnteredScope && "Already entered the scope!");
      LLVM_ASSERT(SS.isSet() && "C++ scope was not set!");
    ...
    ~DeclaratorScopeObj() {
      if (EnteredScope) {
        LLVM_ASSERT(SS.isSet() && "C++ scope was cleared ?");
