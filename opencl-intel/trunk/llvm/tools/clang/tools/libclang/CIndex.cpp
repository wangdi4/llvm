...
static void clang_parseTranslationUnit_Impl(void *UserData) {
  ...
  char tmpFileResults[L_tmpnam] = "XXXXXX"; // mkstemp replaces this string with the unique filename.
#ifdef _WIN32
  char *tmpResultsFileName =
    tmpnam(tmpFileResults);
  llvm::sys::Path DiagnosticsFile(tmpResultsFileName);
#else
  const int rc = mkstemp(tmpFileResults);
  if (rc == -1) {
      perror("mkstemp");
      return;
  }
  llvm::sys::Path DiagnosticsFile(tmpFileResults);
#endif  
