...
RValue CodeGenFunction::EmitCXXMemberCallExpr(const CXXMemberCallExpr *CE,
  ...
  if (MD->isTrivial()) {
    ...
    LLVM_ASSERT(MD->isCopyAssignment() && "unknown trivial member function");
...
CodeGenFunction::EmitCXXOperatorMemberCallExpr(const CXXOperatorCallExpr *E,
  ...
  if (MD->isCopyAssignment()) {
    ...
    if (ClassDecl->hasTrivialCopyAssignment()) {
      LLVM_ASSERT(!ClassDecl->hasUserDeclaredCopyAssignment() &&
...
CodeGenFunction::EmitCXXConstructExpr(llvm::Value *Dest,
  ...
  LLVM_ASSERT(Dest && "Must have a destination!");
...
static llvm::Value *EmitCXXNewAllocSize(ASTContext &Context,
  ...
  LLVM_ASSERT(NumElements->getType() == SizeTy && "element count not a size_t");
  ...
  // we're multiplying by one.
    ...
    LLVM_ASSERT(ArraySizeMultiplier == 1);
  ...
  if (CookieSize.isZero())
    ...
    LLVM_ASSERT(SizeWithoutCookie && "didn't set SizeWithoutCookie?");
...
static void StoreAnyExprIntoOneUnit(CodeGenFunction &CGF, const CXXNewExpr *E,
  ...
  LLVM_ASSERT(E->getNumConstructorArgs() == 1 &&
...
llvm::Value *CodeGenFunction::EmitCXXNewExpr(const CXXNewExpr *E) {
  ...
  // variadic function.
  LLVM_ASSERT((NewArg == E->placement_arg_end() || NewFTy->isVariadic()) &&
  ...
  LLVM_ASSERT((AllocSize == AllocSizeWithoutCookie) ==
         CalculateCookiePadding(*this, E).isZero());
  if (AllocSize != AllocSizeWithoutCookie) {
    LLVM_ASSERT(E->isArray());
...
void CodeGenFunction::EmitDeleteCall(const FunctionDecl *DeleteFD,
                                     llvm::Value *Ptr,
                                     QualType DeleteTy) {
  LLVM_ASSERT(DeleteFD->getOverloadedOperator() == OO_Delete);
...
namespace {
  ...
  struct CallArrayDelete : EHScopeStack::Cleanup {
    ...
    void Emit(CodeGenFunction &CGF, bool IsForEH) {
      ...
      LLVM_ASSERT(DeleteFTy->getNumArgs() == 1 || DeleteFTy->getNumArgs() == 2);
...
static void EmitArrayDelete(CodeGenFunction &CGF,
  ...
  LLVM_ASSERT(AllocatedPtr && "ReadArrayCookie didn't set AllocatedPtr");
  ...
  if (const CXXRecordDecl *RD = ElementType->getAsCXXRecordDecl()) {
    if (!RD->hasTrivialDestructor()) {
      LLVM_ASSERT(NumElements && "ReadArrayCookie didn't find element count"
...
void CodeGenFunction::EmitCXXDeleteExpr(const CXXDeleteExpr *E) {
  ...
  LLVM_ASSERT(ConvertTypeForMem(DeleteTy) ==
...
llvm::Value *CodeGenFunction::EmitDynamicCast(llvm::Value *V,
  ...
  if (ToVoid) {
    ...
    LLVM_ASSERT(SrcTy->isRecordType() && "Src type must be record type!");
    LLVM_ASSERT(DestTy->isRecordType() && "Dest type must be record type!");
