...
CGDebugInfo::~CGDebugInfo() {
  LLVM_ASSERT(RegionStack.empty() && "Region stack mismatch, stack not empty!");
}
...
llvm::DIType CGDebugInfo::CreateQualifiedType(QualType Ty, llvm::DIFile Unit) {
  ...
  if (Qc.hasConst()) {
    ...
    LLVM_ASSERT(Qc.empty() && "Unknown type qualifier for debug info");
...
llvm::DIType CGDebugInfo::CreateType(const RecordType *Ty,
  ...
  if (RD->isStruct())
    ...
    LLVM_ASSERT(RD->isClass() && "Unknown RecordType!");
...
static QualType UnwrapTypeForDebugInfo(QualType T) {
  do {
    ...
    LLVM_ASSERT(T != LastT && "Type unwrapping failed to unwrap!");
    if (T == LastT)
      return T;
  } while (true);
...
#include "clang/AST/TypeNodes.def"
    LLVM_ASSERT(false && "Dependent types cannot show up in debug information");
  ...
  LLVM_ASSERT(Diag && "Fall through without a diagnostic?");
...
void CGDebugInfo::EmitRegionEnd(CGBuilderTy &Builder) {
  LLVM_ASSERT(!RegionStack.empty() && "Region stack mismatch, stack empty!");
...
void CGDebugInfo::EmitFunctionEnd(CGBuilderTy &Builder) {
  ...
  LLVM_ASSERT(RCount <= RegionStack.size() && "Region stack mismatch");
