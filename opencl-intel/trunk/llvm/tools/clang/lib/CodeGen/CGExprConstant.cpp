...
AppendField(const FieldDecl *Field, uint64_t FieldOffset,
  ...
  LLVM_ASSERT(NextFieldOffsetInBytes <= FieldOffsetInBytes
  ...
  if (AlignedNextFieldOffsetInBytes > FieldOffsetInBytes) {
    LLVM_ASSERT(!Packed && "Alignment is wrong even with a packed struct!");
  ...
  if (Packed)
  {
    LLVM_ASSERT(LLVMStructAlignment == 1 && "Packed struct not byte-aligned!");
  }
  else
...
void ConstStructBuilder::AppendBitField(const FieldDecl *Field,
  ...
  if (FieldOffset < NextFieldOffsetInBytes * 8) {
    ...
    // byte.
    LLVM_ASSERT(!Elements.empty() && "Elements can't be empty!");
    ...
    if (llvm::ConstantInt *Val = dyn_cast<llvm::ConstantInt>(LastElt))
      ...
      LLVM_ASSERT(isa<llvm::UndefValue>(LastElt));
      ...
      if (!isa<llvm::IntegerType>(LastElt->getType())) {
        ...
        LLVM_ASSERT(isa<llvm::ArrayType>(LastElt->getType()) &&
               "Expected array padding of undefs");
        const llvm::ArrayType *AT = cast<llvm::ArrayType>(LastElt->getType());
        LLVM_ASSERT(AT->getElementType()->isIntegerTy(8) &&
               AT->getNumElements() != 0 &&
        ...
        AppendPadding(1);
        LLVM_ASSERT(isa<llvm::UndefValue>(Elements.back()) &&
  ...
  LLVM_ASSERT(FieldValue.getBitWidth() > 0 &&
         "Should have at least one bit left!");
  LLVM_ASSERT(FieldValue.getBitWidth() <= 8 &&
...
void ConstStructBuilder::AppendPadding(uint64_t NumBytes) {
  ...
  LLVM_ASSERT(getAlignment(C) == 1 && "Padding must have 1 byte alignment!");

  NextFieldOffsetInBytes += getSizeInBytes(C);
}

void ConstStructBuilder::AppendTailPadding(uint64_t RecordSize) {
  LLVM_ASSERT(RecordSize % 8 == 0 && "Invalid record size!");

  uint64_t RecordSizeInBytes = RecordSize / 8;
  LLVM_ASSERT(NextFieldOffsetInBytes <= RecordSizeInBytes && "Size mismatch!");
...
void ConstStructBuilder::ConvertStructToPacked() {
  ...
  LLVM_ASSERT(ElementOffsetInBytes == NextFieldOffsetInBytes &&
...
bool ConstStructBuilder::Build(InitListExpr *ILE) {
  ...
  if (NextFieldOffsetInBytes <= LayoutSizeInBytes && 
    ...
    LLVM_ASSERT(!Packed && "Size mismatch!");
    
    ConvertStructToPacked();
    LLVM_ASSERT(NextFieldOffsetInBytes <= LayoutSizeInBytes &&
  ...
  LLVM_ASSERT(Layout.getSize() / 8 == NextFieldOffsetInBytes &&
...
llvm::Constant *ConstStructBuilder::
  ...
  LLVM_ASSERT(llvm::RoundUpToAlignment(Builder.NextFieldOffsetInBytes,
...
public:
  ...
  llvm::Constant *VisitCastExpr(CastExpr* E) {
    ...
    case CK_ToUnion: {
      ...
      LLVM_ASSERT(E->getType()->isUnionType() &&
      ...
      LLVM_ASSERT(CurSize <= TotalSize && "Union size mismatch!");
  ...
  llvm::Constant *VisitInitListExpr(InitListExpr *ILE) {
    ...
    LLVM_ASSERT(0 && "Unable to handle InitListExpr");
  ...
  llvm::Constant *VisitCXXConstructExpr(CXXConstructExpr *E) {
    ...
    if (E->getNumArgs()) {
      LLVM_ASSERT(E->getNumArgs() == 1 && "trivial ctor with > 1 argument");
      LLVM_ASSERT(E->getConstructor()->isCopyConstructor() &&
             "trivial ctor has argument but isn't a copy ctor");

      Expr *Arg = E->getArg(0);
      LLVM_ASSERT(CGM.getContext().hasSameUnqualifiedType(Ty, Arg->getType()) &&
  ...
  llvm::Constant *VisitStringLiteral(StringLiteral *E) {
    LLVM_ASSERT(!E->getType()->isPointerType() && "Strings are always arrays");
...
public:
  llvm::Constant *EmitLValue(Expr *E) {
      ...
      if (const VarDecl* VD = dyn_cast<VarDecl>(Decl)) {
        ...
        if (!VD->hasLocalStorage()) {
          if (VD->isFileVarDecl() || VD->hasExternalStorage())
            ...
            LLVM_ASSERT(CGF && "Can't access static local vars without CGF");
    ...
    case Expr::AddrLabelExprClass: {
      LLVM_ASSERT(CGF && "Invalid address of label expression outside function.");
      llvm::Constant *Ptr =
...
llvm::Constant *CodeGenModule::EmitConstantExpr(const Expr *E,
  ...
  if (Success && !Result.HasSideEffects) {
    ...
    case APValue::Uninitialized:
      LLVM_ASSERT(0 && "Constant expressions should be initialized.");
      return 0;
    case APValue::LValue: {
...
FillInNullDataMemberPointers(CodeGenModule &CGM, QualType T,
  ...
  LLVM_ASSERT(StartOffset % 8 == 0 && "StartOffset not byte aligned!");
  ...
  if (const ConstantArrayType *CAT = 
    ...
    LLVM_ASSERT(T->isMemberPointerType() && "Should only see member pointers here!");
    LLVM_ASSERT(!T->getAs<MemberPointerType>()->getPointeeType()->isFunctionType() &&
