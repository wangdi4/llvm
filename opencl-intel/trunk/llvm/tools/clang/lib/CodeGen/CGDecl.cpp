...
void CodeGenFunction::EmitDecl(const Decl &D) {
  ...
  case Decl::Block:
    ...
    LLVM_ASSERT(0 && "Declaration not should not be in declstmts!");
  ...
  case Decl::StaticAssert: // static_LLVM_ASSERT(X, ""); [C++0x]
    // None of these decls require codegen support.
    return;

  case Decl::Var: {
    const VarDecl &VD = cast<VarDecl>(D);
    LLVM_ASSERT(VD.isBlockVarDecl() &&
...
void CodeGenFunction::EmitBlockVarDecl(const VarDecl &D) {
  ...
  LLVM_ASSERT(0 && "Unknown storage class");
}
...
static std::string GetStaticDeclName(CodeGenFunction &CGF, const VarDecl &D,
  ...
  if (const FunctionDecl *FD = dyn_cast<FunctionDecl>(CGF.CurFuncDecl)) {
    ...
    LLVM_ASSERT(0 && "Unknown context for block var decl");
...
CodeGenFunction::CreateStaticBlockVarDecl(const VarDecl &D,
  ...
  LLVM_ASSERT(Ty->isConstantSizeType() && "VLAs can't be static");
...
llvm::GlobalVariable *
CodeGenFunction::CreateOpenclLocalVarDecl(const VarDecl &D) {
  QualType Ty = D.getType();
  const FunctionDecl *FD = dyn_cast<FunctionDecl>(CurFuncDecl);

  std::string Name = std::string("opencl_") + std::string(CGM.getMangledName(FD)) + 
	std::string("_local_") + D.getNameAsString();

  const llvm::Type *LTy = CGM.getTypes().ConvertTypeForMem(Ty);
  llvm::GlobalVariable *GV =
    new llvm::GlobalVariable(CGM.getModule(), LTy,
                             Ty.isConstant(getContext()), llvm::GlobalValue::InternalLinkage,
                             CGM.EmitNullConstant(D.getType()), Name, 0,
                             D.isThreadSpecified(), Ty.getAddressSpace());
  GV->setAlignment(getContext().getDeclAlign(&D).getQuantity());
  
  std::string LocalsAnchorName = std::string("opencl_") + std::string(CGM.getMangledName(FD)) + std::string("_locals_anchor");

  llvm::Value *Elts[] = {
	  llvm::MDString::get(VMContext, llvm::StringRef(Name))
  };

  llvm::NamedMDNode *LocalsAnchor = 
      CGM.getModule().getOrInsertNamedMetadata(LocalsAnchorName);
  LocalsAnchor->addOperand(llvm::MDNode::get(VMContext, Elts, 1));
...
void CodeGenFunction::EmitStaticBlockVarDecl(const VarDecl &D,
  ...
  LLVM_ASSERT(DMEntry == 0 && "Decl already exists in localdeclmap!");
...
unsigned CodeGenFunction::getByRefValueLLVMField(const ValueDecl *VD) const {
  LLVM_ASSERT(ByRefValueInfo.count(VD) && "Did not find value!");
...
void CodeGenFunction::EmitLocalBlockVarDecl(const VarDecl &D,
  ...
  if (Ty->isConstantSizeType()) {
    if (getContext().getLangOptions().OpenCL &&
		D.getType().getAddressSpace() == 3) { // OpenCL locals
      const FunctionDecl *FD = dyn_cast<FunctionDecl>(CurFuncDecl);

      // OpenCL locals must be defined only inside an OpenCL kernel!
      if (!FD || !FD->hasAttr<OpenCLKernelAttr>()) {
          CGM.getDiags().Report(CGM.getContext().getFullLoc(D.getLocation()),
		  CGM.getDiags().getCustomDiagID(Diagnostic::Error,
                                               "OpenCL local variables must be declared inside OpenCL kernels only"));
      }

      // Create OpenCL locals as addressspace 3 globals
        llvm::GlobalValue * GV = CreateOpenclLocalVarDecl(D);
        DeclPtr = GV;
  ...
  if (CGDebugInfo *DI = getDebugInfo()) {
    LLVM_ASSERT(HaveInsertPoint() && "Unexpected unreachable point!");
  ...
  if (SpecialInit) {
    ...
    if (IsSimpleConstantInitializer) {
      ...
      LLVM_ASSERT(Init != 0 && "Wasn't a simple constant init?");
  ...
  if (const RecordType *RT = DtorTy->getAs<RecordType>())
    if (CXXRecordDecl *ClassDecl = dyn_cast<CXXRecordDecl>(RT->getDecl())) {      
      if (!ClassDecl->hasTrivialDestructor()) {
        ...
        LLVM_ASSERT(D && "EmitLocalBlockVarDecl - destructor is nul");
  ...
  if (const CleanupAttr *CA = D.getAttr<CleanupAttr>()) {
    ...
    LLVM_ASSERT(F && "Could not find function!");
...
void CodeGenFunction::EmitParmDecl(const VarDecl &D, llvm::Value *Arg) {
  ...
  LLVM_ASSERT((isa<ParmVarDecl>(D) || isa<ImplicitParamDecl>(D)) &&
