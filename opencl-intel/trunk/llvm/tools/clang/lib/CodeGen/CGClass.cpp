...
ComputeNonVirtualBaseClassOffset(ASTContext &Context, 
  ...
  for (CastExpr::path_const_iterator I = Start; I != End; ++I) {
    ...
    LLVM_ASSERT(!Base->isVirtual() && "Should not see virtual bases here!");
...
CodeGenModule::GetNonVirtualBaseClassOffset(const CXXRecordDecl *ClassDecl,
  ...
  LLVM_ASSERT(PathBegin != PathEnd && "Base path should not be empty!");
...
CodeGenFunction::GetAddressOfDirectBaseInCompleteClass(llvm::Value *This,
  ...
  LLVM_ASSERT(This->getType()->isPointerTy() &&
...
static llvm::Value *GetVTTParameter(CodeGenFunction &CGF, GlobalDecl GD,
  ...
  if (RD == Base) {
    LLVM_ASSERT(!CodeGenVTables::needsVTTParameter(CGF.CurGD) &&
           "doing no-op VTT offset in base dtor/ctor?");
    LLVM_ASSERT(!ForVirtualBase && "Can't have same class as virtual base!");
    SubVTTIndex = 0;
  } else {
    ...
    LLVM_ASSERT(SubVTTIndex != 0 && "Sub-VTT index must be greater than zero!");
  }
...
static void EmitBaseInitializer(CodeGenFunction &CGF, 
  ...
  LLVM_ASSERT(BaseInit->isBaseInitializer() &&
...
static void EmitAggMemberInitializer(CodeGenFunction &CGF,
  ...
  LLVM_ASSERT(Array && "Array initialization without the array type?");
  llvm::Value *IndexVar
    = CGF.GetAddrOfLocalVar(MemberInit->getArrayIndex(Index));
  LLVM_ASSERT(IndexVar && "Array index variable not loaded");
...
static void EmitMemberInitializer(CodeGenFunction &CGF,
  ...
  LLVM_ASSERT(MemberInit->isMemberInitializer() &&
...
void CodeGenFunction::EmitDestructorBody(FunctionArgList &Args) {
  ...
  case Dtor_Base:
    ...
    if (isTryBody)
      ...
      LLVM_ASSERT(Dtor->isImplicit() && "bodyless dtor not implicit");
...
void CodeGenFunction::EnterDtorCleanups(const CXXDestructorDecl *DD,
  ...
  LLVM_ASSERT(!DD->isTrivial() &&
         "Should not emit dtor epilogue for trivial dtor!");

  // The deleting-destructor phase just needs to call the appropriate
  // operator delete that Sema picked up.
  if (DtorType == Dtor_Deleting) {
    LLVM_ASSERT(DD->getOperatorDelete() && 
  ...
  LLVM_ASSERT(DtorType == Dtor_Base);
...
CodeGenFunction::EmitCXXAggrDestructorCall(const CXXDestructorDecl *D,
  ...
  LLVM_ASSERT(CA && "Do we support VLA for destruction ?");
...
CodeGenFunction::EmitCXXConstructorCall(const CXXConstructorDecl *D,
  ...
  if (D->isTrivial()) {
    if (ArgBeg == ArgEnd) {
      ...
      LLVM_ASSERT(D->isDefaultConstructor() &&
             "trivial 0-arg ctor not a default ctor");
      return;
    }

    LLVM_ASSERT(ArgBeg + 1 == ArgEnd && "unexpected argcount for trivial ctor");
    LLVM_ASSERT(D->isCopyConstructor() && "trivial 1-arg ctor not a copy ctor");
...
CodeGenFunction::EmitDelegateCXXConstructorCall(const CXXConstructorDecl *Ctor,
  ...
  LLVM_ASSERT(I != E && "no parameters to constructor");
  ...
  if (llvm::Value *VTT = GetVTTParameter(*this, GlobalDecl(Ctor, CtorType),
    ...
    if (CodeGenVTables::needsVTTParameter(CurGD)) {
      LLVM_ASSERT(I != E && "cannot skip vtt parameter, already done with args");
      LLVM_ASSERT(I->second == VoidPP && "skipping parameter not of vtt type");
      ++I;
    }
  }
