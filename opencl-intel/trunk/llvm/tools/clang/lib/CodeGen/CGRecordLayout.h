...
public:
  CGBitFieldInfo(unsigned Size, unsigned NumComponents, AccessInfo *_Components,
                 bool IsSigned) : Size(Size), NumComponents(NumComponents),
                                  IsSigned(IsSigned) {
    LLVM_ASSERT(NumComponents <= 3 && "invalid number of components!");
    ...
    for (unsigned i = 0, e = getNumComponents(); i != e; ++i) {
      ...
      LLVM_ASSERT(AI.TargetBitWidth > 0);

      // We can't load more bits than we accessed.
      LLVM_ASSERT(AI.FieldBitStart + AI.TargetBitWidth <= AI.AccessWidth);

      // We shouldn't put any bits outside the result size.
      LLVM_ASSERT(AI.TargetBitWidth + AI.TargetBitOffset <= Size);
    }

    // Check that the total number of target bits matches the total bit-field
    // size.
    LLVM_ASSERT(AccessedSize == Size && "Total size does not match accessed size!");
  }
...
public:
  ...
  const AccessInfo &getComponent(unsigned Index) const {
    LLVM_ASSERT(Index < getNumComponents() && "Invalid access!");
...
public:
  ...
  unsigned getLLVMFieldNo(const FieldDecl *FD) const {
    LLVM_ASSERT(!FD->isBitField() && "Invalid call for bit-field decl!");
    LLVM_ASSERT(FieldInfo.count(FD) && "Invalid field for record!");
    return FieldInfo.lookup(FD);
  }

  unsigned getNonVirtualBaseLLVMFieldNo(const CXXRecordDecl *RD) const {
    LLVM_ASSERT(NonVirtualBaseFields.count(RD) && "Invalid non-virtual base!");
    return NonVirtualBaseFields.lookup(RD);
  }

  /// \brief Return the BitFieldInfo that corresponds to the field FD.
  const CGBitFieldInfo &getBitFieldInfo(const FieldDecl *FD) const {
    LLVM_ASSERT(FD->isBitField() && "Invalid call for non bit-field decl!");
    llvm::DenseMap<const FieldDecl *, CGBitFieldInfo>::const_iterator
      it = BitFields.find(FD);
    LLVM_ASSERT(it != BitFields.end()  && "Unable to find bitfield info");
    return it->second;
  }
