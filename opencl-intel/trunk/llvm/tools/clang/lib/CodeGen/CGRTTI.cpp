...
static bool TypeInfoIsInStandardLibrary(const BuiltinType *Ty) {
  ...
  switch (Ty->getKind()) {
    ...
    case BuiltinType::UndeducedAuto:
      LLVM_ASSERT(false && "Should not see this type here!");
      
    case BuiltinType::NullPtr:
      LLVM_ASSERT(false && "FIXME: nullptr_t is not handled!");

    case BuiltinType::ObjCId:
    case BuiltinType::ObjCClass:
    case BuiltinType::ObjCSel:
      LLVM_ASSERT(false && "FIXME: Objective-C types are unsupported!");
  }
...
#include "clang/AST/TypeNodes.def"
  ...
  case Type::RValueReference:
    LLVM_ASSERT(false && "References shouldn't get here");
  ...
  case Type::ObjCObject:
    ...
    LLVM_ASSERT(isa<ObjCInterfaceType>(Ty));
    // Fall through.
...
void RTTIBuilder::BuildObjCObjectTypeInfo(const ObjCObjectType *OT) {
  ...
  LLVM_ASSERT(isa<BuiltinType>(T) || isa<ObjCInterfaceType>(T));
