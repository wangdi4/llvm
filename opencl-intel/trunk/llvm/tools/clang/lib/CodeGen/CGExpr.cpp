...
void CodeGenFunction::EmitStoreThroughLValue(RValue Src, LValue Dst,
  ...
  if (!Dst.isSimple()) {
    ...
    LLVM_ASSERT(Dst.isKVCRef() && "Unknown LValue type");
  ...
  if (Dst.isObjCStrong() && !Dst.isNonGC()) {
    ...
    llvm::Value *src = Src.getScalarVal();
    if (Dst.isObjCIvar()) {
      LLVM_ASSERT(Dst.getBaseIvarExp() && "BaseIvarExp is NULL");
  ...
  LLVM_ASSERT(Src.isScalar() && "Can't emit an agg store with this method");
  ...
  if (const VectorType *VTy = Ty->getAs<VectorType>()) {
    ...
    if (NumDstElts == NumSrcElts) {
      ...
      for (unsigned i = 0; i != NumSrcElts; ++i) {
        ...
        if(Idx < NumDstElts) //make sure not to fail on 3 elements vectors
      ...
      LLVM_ASSERT(0 && "unexpected shorten vector length");
    }
  } else {
...
static LValue EmitGlobalVarDeclLValue(CodeGenFunction &CGF,
  ...
  LLVM_ASSERT((VD->hasExternalStorage() || VD->isFileVarDecl()) &&
...
LValue CodeGenFunction::EmitDeclRefLValue(const DeclRefExpr *E) {
  ...
  if (const VarDecl *VD = dyn_cast<VarDecl>(ND)) {
    ...
    LLVM_ASSERT(V && "DeclRefExpr not entered in LocalDeclMap?");
  ...
  LLVM_ASSERT(false && "Unhandled DeclRefExpr");
...
LValue CodeGenFunction::EmitUnaryOpLValue(const UnaryOperator *E) {
  ...
  switch (E->getOpcode()) {
  default: LLVM_ASSERT(0 && "Unknown unary operator lvalue!");
  case UO_Deref: {
    QualType T = E->getSubExpr()->getType()->getPointeeType();
    LLVM_ASSERT(!T.isNull() && "CodeGenFunction::EmitUnaryOpLValue: Illegal type");
...
LValue CodeGenFunction::EmitPredefinedLValue(const PredefinedExpr *E) {
  ...
  case PredefinedExpr::PrettyFunction: {
    ...
    switch (Type) {
    default: LLVM_ASSERT(0 && "Invalid type");
    case PredefinedExpr::Func:
...
LValue CodeGenFunction::EmitArraySubscriptExpr(const ArraySubscriptExpr *E) {
  ...
  if (E->getBase()->getType()->isVectorType()) {
    ...
    LLVM_ASSERT(LHS.isSimple() && "Can only subscript lvalue vectors here!");
  ...
  if (const VariableArrayType *VAT =
    ...
    LLVM_ASSERT(Array->getType()->isArrayType() &&
...
EmitExtVectorElementExpr(const ExtVectorElementExpr *E) {
  ...
  if (E->isArrow()) {
    ...
    LLVM_ASSERT(E->getBase()->getType()->isVectorType());
    Base = EmitLValue(E->getBase());
  } else {
    // Otherwise, the base is a normal rvalue (as in (V+V).x), emit it as such.
    LLVM_ASSERT(E->getBase()->getType()->getAs<VectorType>() &&
  ...
  LLVM_ASSERT(Base.isExtVectorElt() && "Can only subscript lvalue vec elts here!");
...
LValue CodeGenFunction::EmitMemberExpr(const MemberExpr *E) {
  ...
  LLVM_ASSERT(false && "Unhandled member declaration!");
  return LValue();
}
...
CodeGenFunction::EmitLValueForFieldInitialization(llvm::Value* BaseValue, 
  ...
  LLVM_ASSERT(!FieldType.getObjCGCAttr() && "fields cannot have GC attrs");
...
CodeGenFunction::EmitConditionalOperatorLValue(const ConditionalOperator* E) {
  ...
  LLVM_ASSERT((hasAggregateLLVMType(E->getType()) &&
          !E->getType()->isAnyComplexType()) &&
  ...
  case CK_NoOp:
    if (E->getSubExpr()->Classify(getContext()).getKind() 
      ...
      if (LV.isPropertyRef() || LV.isKVCRef()) {
        ...
        LLVM_ASSERT(!RV.isScalar() && "EmitCastLValue-scalar cast of property ref");
...
LValue CodeGenFunction::EmitCallExprLValue(const CallExpr *E) {
  ...
  LLVM_ASSERT(E->getCallReturnType()->isReferenceType() &&
...
LValue CodeGenFunction::EmitObjCMessageExprLValue(const ObjCMessageExpr *E) {
  ...
  LLVM_ASSERT(E->getMethodDecl()->getResultType()->isReferenceType() &&
         "Can't have a scalar return unless the return type is a "
         "reference type!");
...
RValue CodeGenFunction::EmitCall(QualType CalleeType, llvm::Value *Callee,
  ...
  LLVM_ASSERT(CalleeType->isFunctionPointerType() &&
