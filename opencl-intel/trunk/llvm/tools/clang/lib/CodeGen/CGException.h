...
public:
  ...
  EHCleanupScope(bool IsNormal, bool IsEH, bool IsActive,
    ...
    : EHScope(EHScope::Cleanup),
      ...
      ExtInfo(0)
  {
    LLVM_ASSERT(this->CleanupSize == CleanupSize && "cleanup size overflow");
  }
  ...
  llvm::BasicBlock *getBranchAfterBlock(unsigned I) const {
    LLVM_ASSERT(I < getNumBranchAfters());
  ...
  llvm::BasicBlock *getEHBranchAfterBlock(unsigned I) const {
    LLVM_ASSERT(I < getNumEHBranchAfters());
...
inline void EHScopeStack::popCatch() {
  ...
  LLVM_ASSERT(isa<EHCatchScope>(*begin()));
  ...
  LLVM_ASSERT(CatchDepth > 0 && "mismatched catch/terminate push/pop");
  CatchDepth--;
}
...
inline void EHScopeStack::popTerminate() {
  ...
  LLVM_ASSERT(isa<EHTerminateScope>(*begin()));
...
inline EHScopeStack::iterator EHScopeStack::find(stable_iterator sp) const {
  LLVM_ASSERT(sp.isValid() && "finding invalid savepoint");
  LLVM_ASSERT(sp.Size <= stable_begin().Size && "finding savepoint after pop");
  return iterator(EndOfBuffer - sp.Size);
}

inline EHScopeStack::stable_iterator
EHScopeStack::stabilize(iterator ir) const {
  LLVM_ASSERT(StartOfData <= ir.Ptr && ir.Ptr <= EndOfBuffer);
