...
void CGObjCGNU::EmitTryStmt(CodeGen::CodeGenFunction &CGF,
  ...
  if (S.getNumCatchStmts()) {
    for (unsigned I = 0, N = S.getNumCatchStmts(); I != N; ++I) {
      ...
      LLVM_ASSERT(OPT && "Invalid @catch type.");
      const ObjCInterfaceDecl *IDecl =
        OPT->getObjectType()->getInterface();
      LLVM_ASSERT(IDecl && "Invalid @catch type.");
...
void CGObjCGNU::EmitThrowStmt(CodeGen::CodeGenFunction &CGF,
  ...
  if (const Expr *ThrowExpr = S.getThrowExpr()) {
    ...
    LLVM_ASSERT((!CGF.ObjCEHValueStack.empty() && CGF.ObjCEHValueStack.back()) &&
...
void CGObjCGNU::EmitObjCGlobalAssign(CodeGen::CodeGenFunction &CGF,
  ...
  if (!threadlocal)
    ...
    LLVM_ASSERT(false && "EmitObjCGlobalAssign - Threal Local API NYI");
}
