...
void AggExprEmitter::EmitFinalDestCopy(const Expr *E, RValue Src, bool Ignore) {
  LLVM_ASSERT(Src.isAggregate() && "value must be aggregate value!");
...
void AggExprEmitter::EmitFinalDestCopy(const Expr *E, LValue Src, bool Ignore) {
  LLVM_ASSERT(Src.isSimple() && "Can't have aggregate bitfield, vector, etc");
...
void AggExprEmitter::VisitCastExpr(CastExpr *E) {
  ...
  case CK_Dynamic: {
    LLVM_ASSERT(isa<CXXDynamicCastExpr>(E) && "CK_Dynamic without a dynamic_cast?");
  ...
  case CK_UncheckedDerivedToBase: {
    LLVM_ASSERT(0 && "cannot perform hierarchy conversion in EmitAggExpr: "
  ...
  case CK_ConstructorConversion:
    LLVM_ASSERT(CGF.getContext().hasSameUnqualifiedType(E->getSubExpr()->getType(),
                                                   E->getType()) &&
...
void AggExprEmitter::VisitConditionalOperator(const ConditionalOperator *E) {
  ...
  LLVM_ASSERT(E->getLHS() && "Must have LHS for aggregate value");
...
void AggExprEmitter::VisitInitListExpr(InitListExpr *E) {
  ...
  LLVM_ASSERT(E->getType()->isRecordType() && "Only support structs/unions here!");
  ...
  if (E->getType()->isUnionType()) {
    ...
    if (!E->getInitializedFieldInUnion()) {
      ...
      for (RecordDecl::field_iterator Field = SD->field_begin(),
        ...
        LLVM_ASSERT(Field->isUnnamedBitfield() && "Only unnamed bitfields allowed");
#endif
      return;
    }
...
void CodeGenFunction::EmitAggExpr(const Expr *E, llvm::Value *DestPtr,
                                  ...
                                  bool IsInitializer,
                                  bool RequiresGCollection) {
  LLVM_ASSERT(E && hasAggregateLLVMType(E->getType()) &&
...
LValue CodeGenFunction::EmitAggExprToLValue(const Expr *E) {
  LLVM_ASSERT(hasAggregateLLVMType(E->getType()) && "Invalid argument!");
...
void CodeGenFunction::EmitAggregateCopy(llvm::Value *DestPtr,
                                        ...
                                        bool isVolatile) {
  LLVM_ASSERT(!Ty->isAnyComplexType() && "Shouldn't happen for complex");

  if (getContext().getLangOptions().CPlusPlus) {
    if (const RecordType *RT = Ty->getAs<RecordType>()) {
      CXXRecordDecl *Record = cast<CXXRecordDecl>(RT->getDecl());
      LLVM_ASSERT((Record->hasTrivialCopyConstructor() || 
