...
public:
  ...
  OverriderInfo getOverrider(const CXXMethodDecl *MD, 
    ...
    LLVM_ASSERT(OverridersMap.count(std::make_pair(MD, BaseOffset)) && 
...
FinalOverriders::FinalOverriders(const CXXRecordDecl *MostDerivedClass,
  ...
  for (CXXFinalOverriderMap::const_iterator I = FinalOverriders.begin(),
    ...
    for (OverridingMethods::const_iterator I = Methods.begin(),
      ...
      LLVM_ASSERT(SubobjectOffsets.count(std::make_pair(MD->getParent(), 
                                                   SubobjectNumber)) &&
      ...
      LLVM_ASSERT(I->second.size() == 1 && "Final overrider is not unique!");
      const UniqueVirtualMethod &Method = I->second.front();

      const CXXRecordDecl *OverriderRD = Method.Method->getParent();
      LLVM_ASSERT(SubobjectLayoutClassOffsets.count(
      ...
      LLVM_ASSERT(!Overrider.Method && "Overrider should not exist yet!");
...
static BaseOffset ComputeBaseOffset(ASTContext &Context, 
  ...
  if (!const_cast<CXXRecordDecl *>(DerivedRD)->
    ...
    LLVM_ASSERT(false && "Class must be derived from the passed in base class!");
    return BaseOffset();
  }
...
ComputeReturnAdjustmentBaseOffset(ASTContext &Context, 
  ...
  LLVM_ASSERT(CanDerivedReturnType->getTypeClass() == 
  ...
  if (isa<ReferenceType>(CanDerivedReturnType)) {
    ...
    LLVM_ASSERT(false && "Unexpected return type!");
  }
...
FinalOverriders::ComputeBaseOffsets(BaseSubobject Base, bool IsVirtual,
  ...
  LLVM_ASSERT(!SubobjectOffsets.count(std::make_pair(RD, SubobjectNumber))
         && "Subobject offset already exists!");
  LLVM_ASSERT(!SubobjectLayoutClassOffsets.count(std::make_pair(RD, SubobjectNumber)) 
         && "Subobject offset already exists!");
...
public:
  ...
  int64_t getVCallOffset() const {
    LLVM_ASSERT(getKind() == CK_VCallOffset && "Invalid component kind!");
    
    return getOffset();
  }

  int64_t getVBaseOffset() const {
    LLVM_ASSERT(getKind() == CK_VBaseOffset && "Invalid component kind!");
    
    return getOffset();
  }

  int64_t getOffsetToTop() const {
    LLVM_ASSERT(getKind() == CK_OffsetToTop && "Invalid component kind!");
    
    return getOffset();
  }
  
  const CXXRecordDecl *getRTTIDecl() const {
    LLVM_ASSERT(getKind() == CK_RTTI && "Invalid component kind!");
    
    return reinterpret_cast<CXXRecordDecl *>(getPointer());
  }
  
  const CXXMethodDecl *getFunctionDecl() const {
    LLVM_ASSERT(getKind() == CK_FunctionPointer);
    
    return reinterpret_cast<CXXMethodDecl *>(getPointer());
  }

  const CXXDestructorDecl *getDestructorDecl() const {
    LLVM_ASSERT((getKind() == CK_CompleteDtorPointer ||
  ...
  const CXXMethodDecl *getUnusedFunctionDecl() const {
    LLVM_ASSERT(getKind() == CK_UnusedFunctionPointer);
...
private:
  VTableComponent(Kind ComponentKind, int64_t Offset) {
    LLVM_ASSERT((ComponentKind == CK_VCallOffset || 
            ComponentKind == CK_VBaseOffset ||
            ComponentKind == CK_OffsetToTop) && "Invalid component kind!");
    LLVM_ASSERT(Offset <= ((1LL << 56) - 1) && "Offset is too big!");
    
    Value = ((Offset << 3) | ComponentKind);
  }

  VTableComponent(Kind ComponentKind, uintptr_t Ptr) {
    LLVM_ASSERT((ComponentKind == CK_RTTI || 
    ...
    LLVM_ASSERT((Ptr & 7) == 0 && "Pointer not sufficiently aligned!");
    
    Value = Ptr | ComponentKind;
  }
  
  int64_t getOffset() const {
    LLVM_ASSERT((getKind() == CK_VCallOffset || getKind() == CK_VBaseOffset ||
...
bool VCallOffsetMap::MethodsCanShareVCallOffset(const CXXMethodDecl *LHS,
                                                const CXXMethodDecl *RHS) {
  LLVM_ASSERT(LHS->isVirtual() && "LHS must be virtual!");
  LLVM_ASSERT(RHS->isVirtual() && "LHS must be virtual!");
...
int64_t VCallOffsetMap::getVCallOffsetOffset(const CXXMethodDecl *MD) {
  ...
  LLVM_ASSERT(false && "Should always find a vcall offset offset!");
  return 0;
}
...
VCallAndVBaseOffsetBuilder::AddVCallAndVBaseOffsets(BaseSubobject Base,
  ...
  if (const CXXRecordDecl *PrimaryBase = Layout.getPrimaryBase()) {
    ...
    if (PrimaryBaseIsVirtual) {
      LLVM_ASSERT(Layout.getVBaseClassOffset(PrimaryBase) == 0 &&
      ...
      LLVM_ASSERT(Layout.getBaseClassOffset(PrimaryBase) == 0 &&
...
void VCallAndVBaseOffsetBuilder::AddVBaseOffsets(const CXXRecordDecl *RD,
  ...
  for (CXXRecordDecl::base_class_const_iterator I = RD->bases_begin(),
    ...
    if (I->isVirtual() && VisitedVirtualBases.insert(BaseDecl)) {
      ...
      LLVM_ASSERT(!VBaseOffsetOffsets.count(BaseDecl) &&
...
void VTableBuilder::AddThunk(const CXXMethodDecl *MD, const ThunkInfo &Thunk) {
  LLVM_ASSERT(!isBuildingConstructorVTable() && 
...
ComputeAllOverriddenMethods(const CXXMethodDecl *MD,
  ...
  LLVM_ASSERT(MD->isVirtual() && "Method is not virtual!");
...
VTableBuilder::AddMethod(const CXXMethodDecl *MD,
  ...
  if (const CXXDestructorDecl *DD = dyn_cast<CXXDestructorDecl>(MD)) {
    LLVM_ASSERT(ReturnAdjustment.isEmpty() && 
...
VTableBuilder::IsOverriderUsed(const CXXMethodDecl *Overrider,
  ...
  while (true) {
    ...
    if (!PrimaryBases.insert(PrimaryBase)) {
      LLVM_ASSERT(false && "Found a duplicate primary base!");
    }
  ...
  for (CXXRecordDecl::method_iterator I = RD->method_begin(),
    ...
    if (const CXXMethodDecl *OverriddenMD = 
      ...
      if (ComputeReturnAdjustmentBaseOffset(Context, MD, 
        ...
        // method.
        LLVM_ASSERT(MethodInfoMap.count(OverriddenMD) && 
        ...
        LLVM_ASSERT(!MethodInfoMap.count(MD) &&
...
VTableBuilder::LayoutPrimaryAndSecondaryVTables(BaseSubobject Base,
                                                ...
                                                bool BaseIsVirtualInLayoutClass,
                                                uint64_t OffsetInLayoutClass) {
  LLVM_ASSERT(Base.getBase()->isDynamicClass() && "class does not have a vtable!");
    ...
    if (AddressPointsByIndex.count(NextIndex)) {
      if (AddressPointsByIndex.count(NextIndex) == 1) {
        ...
        for (std::multimap<uint64_t, BaseSubobject>::const_iterator I =
          ...
          LLVM_ASSERT(I->second.getBaseOffset() == BaseOffset &&
                 "Invalid base offset!");
...
void CodeGenVTables::ComputeMethodVTableIndices(const CXXRecordDecl *RD) {
  ...
  if (PrimaryBase) {
    LLVM_ASSERT(PrimaryBase->isDefinition() && 
  ...
  for (CXXRecordDecl::method_iterator i = RD->method_begin(),
    ...
    if (const CXXDestructorDecl *DD = dyn_cast<CXXDestructorDecl>(MD)) {
      if (MD->isImplicit()) {
        LLVM_ASSERT(!ImplicitVirtualDtor && 
...
uint64_t CodeGenVTables::getNumVirtualFunctionPointers(const CXXRecordDecl *RD) {
  ...
  LLVM_ASSERT(I != NumVirtualFunctionPointers.end() && "Did not find entry!");
  return I->second;
}
...
uint64_t CodeGenVTables::getMethodVTableIndex(GlobalDecl GD) {
  ...
  LLVM_ASSERT(I != MethodVTableIndices.end() && "Did not find index!");
  return I->second;
}
...
int64_t CodeGenVTables::getVirtualBaseOffsetOffset(const CXXRecordDecl *RD, 
  ...
  LLVM_ASSERT(I != VirtualBaseClassOffsetOffsets.end() && "Did not find index!");
...
CodeGenVTables::getAddressPoint(BaseSubobject Base, const CXXRecordDecl *RD) {
  LLVM_ASSERT(AddressPoints.count(std::make_pair(RD, Base)) &&
         "Did not find address point!");

  uint64_t AddressPoint = AddressPoints.lookup(std::make_pair(RD, Base));
  LLVM_ASSERT(AddressPoint && "Address point must not be zero!");
...
void CodeGenVTables::EmitThunk(GlobalDecl GD, const ThunkInfo &Thunk)
{
  ...
  if (cast<llvm::GlobalValue>(Entry)->getType()->getElementType() != 
    ...
    LLVM_ASSERT(OldThunkFn->isDeclaration() &&
...
void CodeGenVTables::ComputeVTableRelatedInformation(const CXXRecordDecl *RD,
  ...
  LLVM_ASSERT(!VTableThunksMap.count(RD) && 
  ...
  for (VTableBuilder::AddressPointsMapTy::const_iterator I =
    ...
    LLVM_ASSERT(!AddressPoint && "Address point already exists for this base!");
...
GetGlobalVariable(llvm::Module &Module, llvm::StringRef Name,
  ...
  if (GV) {
    ...
    LLVM_ASSERT(GV->isDeclaration() && "Declaration has wrong type!");
...
CodeGenVTables::GenerateClassData(llvm::GlobalVariable::LinkageTypes Linkage,
  ...
  if (VTable) {
    LLVM_ASSERT(VTable->getInitializer() && "VTable doesn't have a definition!");
    return;
  }
