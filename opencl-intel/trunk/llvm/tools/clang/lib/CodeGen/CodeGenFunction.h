...
private:
  ...
  EHScopeStack(const EHScopeStack& e);
  EHScopeStack& operator=(const EHScopeStack& e);
...
public:
  ...
  BranchFixup &addBranchFixup() {
    LLVM_ASSERT(hasNormalCleanups() && "adding fixup in scope without cleanups");
  ...
  BranchFixup &getBranchFixup(unsigned I) {
    LLVM_ASSERT(I < getNumBranchFixups());
...
public:
  ...
  public:
    ...
    void ForceCleanup() {
      LLVM_ASSERT(PerformCleanup && "Already forced cleanup");
      CGF.DidCallStackSave = OldDidCallStackSave;
      CGF.PopCleanupBlocks(CleanupStackDepth);
  ...
  void EndConditionalBranch() {
    LLVM_ASSERT(ConditionalBranchLevel != 0 &&
...
public:
  ...
  llvm::Value *GetAddrOfLocalVar(const VarDecl *VD) {
    ...
    LLVM_ASSERT(Res && "Invalid argument to GetAddrOfLocalVar(), no decl!");
    return Res;
  }
  ...
  llvm::Value *LoadCXXThis() {
    LLVM_ASSERT(CXXThisValue && "no 'this' value for this function");
    return CXXThisValue;
  }
  ...
  llvm::Value *LoadCXXVTT() {
    LLVM_ASSERT(CXXVTTValue && "no VTT value for this function");
    return CXXVTTValue;
  }
  ...
  llvm::Value* EmitFmaCall(const Expr* A, const Expr* B, const Expr* C, const QualType& ParamQualType, const char* FuncName);
  ...
  /// CreateStaticBlockVarDecl - Create a zero-initialized LLVM global for an
  /// OpenCL local block var decl.
  llvm::GlobalVariable *CreateOpenclLocalVarDecl(const VarDecl &D);
  ...
  void EmitCallArgs(CallArgList& Args, const T* CallArgTypeInfo,
    ...
    if (CallArgTypeInfo) {
      for (typename T::arg_type_iterator I = CallArgTypeInfo->arg_type_begin(),
        ...
        LLVM_ASSERT(Arg != ArgEnd && "Running over edge of argument list!");
        QualType ArgType = *I;

        LLVM_ASSERT(getContext().getCanonicalType(ArgType.getNonReferenceType()).
               getTypePtr() ==
      ...
      // variadic function.
      LLVM_ASSERT((Arg == ArgEnd || CallArgTypeInfo->isVariadic()) &&
