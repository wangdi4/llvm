...
VTTBuilder::GetAddrOfVTable(BaseSubobject Base, bool BaseIsVirtual, 
  ...
  if (Base.getBase() == MostDerivedClass) {
    LLVM_ASSERT(Base.getBaseOffset() == 0 &&
...
void VTTBuilder::AddVTablePointer(BaseSubobject Base, llvm::Constant *VTable,
  ...
  if (VTableClass == MostDerivedClass) {
    LLVM_ASSERT(!SecondaryVirtualPointerIndices.count(Base) &&
  ...
  if (VTableClass != MostDerivedClass) {
    ...
    LLVM_ASSERT(AddressPoint != 0 && "Did not find ctor vtable address point!");
  } else {
    // Just get the address point for the regular vtable.
    AddressPoint = CGM.getVTables().getAddressPoint(Base, VTableClass);
    LLVM_ASSERT(AddressPoint != 0 && "Did not find vtable address point!");
  }
...
uint64_t CodeGenVTables::getSubVTTIndex(const CXXRecordDecl *RD, 
  ...
  LLVM_ASSERT(I != SubVTTIndicies.end() && "Did not find index!");
...
CodeGenVTables::getSecondaryVirtualPointerIndex(const CXXRecordDecl *RD,
  ...
  LLVM_ASSERT(I != SecondaryVirtualPointerIndices.end() && "Did not find index!");
