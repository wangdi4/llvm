...
char *EHScopeStack::allocate(size_t Size) {
  ...
  LLVM_ASSERT(StartOfBuffer + Size <= StartOfData);
  StartOfData -= Size;
  return StartOfData;
}
...
void *EHScopeStack::pushCleanup(CleanupKind Kind, size_t Size) {
  LLVM_ASSERT(((Size % sizeof(void*)) == 0) && "cleanup type is misaligned");
...
void EHScopeStack::popCleanup() {
  LLVM_ASSERT(!empty() && "popping exception stack when not empty");

  LLVM_ASSERT(isa<EHCleanupScope>(*begin()));
...
void EHScopeStack::popFilter() {
  ...
  LLVM_ASSERT(CatchDepth > 0 && "mismatched filter push/pop");
  CatchDepth--;
}
...
void EHScopeStack::popNullFixups() {
  ...
  LLVM_ASSERT(hasNormalCleanups());

  EHScopeStack::iterator it = find(InnermostNormalCleanup);
  unsigned MinSize = cast<EHCleanupScope>(*it).getFixupDepth();
  LLVM_ASSERT(BranchFixups.size() >= MinSize && "fixup stack out of order");
...
void CodeGenFunction::EmitStartEHSpec(const Decl *D) {
  ...
  LLVM_ASSERT(!Proto->hasAnyExceptionSpec() && "function with parameter pack");
...
llvm::BasicBlock *CodeGenFunction::getInvokeDestImpl() {
  LLVM_ASSERT(EHStack.requiresLandingPad());
...
llvm::BasicBlock *CodeGenFunction::EmitLandingPad() {
  ...
  for (EHScopeStack::iterator I = EHStack.begin(), E = EHStack.end();
    ...
    case EHScope::Filter: {
      LLVM_ASSERT(I.next() == EHStack.end() && "EH filter is not end of EH stack");
      LLVM_ASSERT(!CatchAll.isValid() && "EH filter reached after catch-all");
  ...
  // Otherwise, signal that we at least have cleanups.
    ...
    LLVM_ASSERT(LastToEmitInLoop > 2);
    LastToEmitInLoop--;
  }

  LLVM_ASSERT(EHSelector.size() >= 3 && "selector call has only two arguments!");
  ...
  for (unsigned I = 2; I != LastToEmitInLoop; ++I) {
    ...
    LLVM_ASSERT(Dest.isValid() && "no handler entry for value in selector?");
  ...
  if (CatchAll.isValid()) {
    LLVM_ASSERT(isa<llvm::ConstantPointerNull>(EHSelector.back()));
...
static void InitCatchParam(CodeGenFunction &CGF,
  ...
  LLVM_ASSERT(RD && "aggregate catch type was not a record!");
  ...
  LLVM_ASSERT(CD && "record has no copy constructor!");
...
void CodeGenFunction::ExitCXXTryStmt(const CXXTryStmt &S, bool IsFnTryBlock) {
  ...
  LLVM_ASSERT(CatchScope.getNumHandlers() == NumHandlers);
...
CodeGenFunction::EnterFinallyBlock(const Stmt *Body,
  ...
  LLVM_ASSERT((BeginCatchFn != 0) == (EndCatchFn != 0) &&
         "begin/end catch functions not paired");
  LLVM_ASSERT(RethrowFn && "rethrow function is required");
