...
static const CXXMethodDecl *getStructor(const CXXMethodDecl *MD) {
  LLVM_ASSERT((isa<CXXConstructorDecl>(MD) || isa<CXXDestructorDecl>(MD)) &&
...
public:
  ...
  ~CXXNameMangler() {
    ...
    LLVM_ASSERT(status == 0 && "Could not demangle mangled name!");
    free(result);
  }
#endif
...
void CXXNameMangler::mangleUnscopedTemplateName(TemplateName Template) {
  ...
  LLVM_ASSERT(Dependent && "Not a dependent template name?");
...
static const FieldDecl *FindFirstNamedDataMember(const RecordDecl *RD) {
  LLVM_ASSERT(RD->isAnonymousStructOrUnion() &&
...
void CXXNameMangler::mangleUnqualifiedName(const NamedDecl *ND,
  ...
  case DeclarationName::Identifier: {
    ...
    LLVM_ASSERT(ND && "mangling empty name without declaration");
    ...
    if (const VarDecl *VD = dyn_cast<VarDecl>(ND)) {
      ...
      if (!FD) break;
      LLVM_ASSERT(FD->getIdentifier() && "Data member name isn't an identifier!");
    ...
    if (const TypedefDecl *D = TD->getTypedefForAnonDecl()) {
      LLVM_ASSERT(TD->getDeclContext() == D->getDeclContext() &&
             "Typedef should not be in another decl context!");
      LLVM_ASSERT(D->getDeclName().getAsIdentifierInfo() &&
  ...
  case DeclarationName::ObjCMultiArgSelector:
    LLVM_ASSERT(false && "Can't mangle Objective-C selector names here!");
    break;
  ...
  case DeclarationName::CXXUsingDirective:
    LLVM_ASSERT(false && "Can't mangle a using directive name!");
    break;
  }
}
...
void CXXNameMangler::mangleTemplatePrefix(TemplateName Template) {
  ...
  DependentTemplateName *Dependent = Template.getAsDependentTemplateName();
  LLVM_ASSERT(Dependent && "Unknown template name kind?");
...
void CXXNameMangler::mangleType(TemplateName TN) {
  ...
  case TemplateName::DependentTemplate: {
    ...
    LLVM_ASSERT(Dependent->isIdentifier());
...
CXXNameMangler::mangleOperatorName(OverloadedOperatorKind OO, unsigned Arity) {
  ...
  case NUM_OVERLOADED_OPERATORS:
    LLVM_ASSERT(false && "Not an overloaded operator");
    break;
  }
}
...
void CXXNameMangler::mangleType(const BuiltinType *T) {
  ...
  case BuiltinType::UndeducedAuto:
    LLVM_ASSERT(0 && "Should not see undeduced auto here");
    break;
...
#include "clang/AST/StmtNodes.inc"
  ...
  case Expr::SizeOfAlignOfExprClass: {
    ...
    switch(SAE->getKind()) {
        ...
        Out << 's';
        break;
      case SizeOfAlignOfExpr::AlignOf:
        Out << 'a';
        break;
      case SizeOfAlignOfExpr::VecStep:
        Out << 'v';
        break;
      default:
        assert(false && "Unknown kind for SizeOfAlignOfExpr");
    }
  ...
  case Expr::DependentScopeDeclRefExprClass: {
    ...
    LLVM_ASSERT(QTy && "Qualifier was not type!");
  ...
  case Expr::StringLiteralClass: {
    ...
    Out << 'L';
    LLVM_ASSERT(isa<ConstantArrayType>(E->getType()));
    mangleType(E->getType());
    Out << 'E';
    break;
  }
...
void CXXNameMangler::mangleTemplateArg(const NamedDecl *P,
  ...
  switch (A.getKind()) {
  default:
    LLVM_ASSERT(0 && "Unknown template argument kind!");
  ...
  case TemplateArgument::Declaration: {
    LLVM_ASSERT(P && "Missing template parameter for declaration argument");
    //  <expr-primary> ::= L <mangled-name> E # external name
...
bool CXXNameMangler::mangleSubstitution(uintptr_t Ptr) {
  ...
  if (SeqID == 0)
    ...
    while (SeqID) {
      LLVM_ASSERT(BufferPtr > Buffer && "Buffer overflow!");
...
void CXXNameMangler::addSubstitution(uintptr_t Ptr) {
  LLVM_ASSERT(!Substitutions.count(Ptr) && "Substitution already exists!");
...
void MangleContext::mangleName(const NamedDecl *D,
                               llvm::SmallVectorImpl<char> &Res) {
  LLVM_ASSERT((isa<FunctionDecl>(D) || isa<VarDecl>(D)) &&
          "Invalid mangleName() call, argument is not a variable or function!");
  LLVM_ASSERT(!isa<CXXConstructorDecl>(D) && !isa<CXXDestructorDecl>(D) &&
...
void MangleContext::mangleThunk(const CXXMethodDecl *MD,
  ...
  LLVM_ASSERT(!isa<CXXDestructorDecl>(MD) &&
...
void MangleContext::mangleCXXRTTI(QualType Ty,
  ...
  LLVM_ASSERT(!Ty.hasQualifiers() && "RTTI info cannot have top-level qualifiers");
  CXXNameMangler Mangler(*this, Res);
