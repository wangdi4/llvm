...
static unsigned ClangCallConvToLLVMCallConv(CallingConv CC) {
  ...
  case CC_X86SvmlCall: return llvm::CallingConv::X86_Svml;
  case CC_X86Svml_P2_F8_F8_F8_Call: return llvm::CallingConv::X86_Svml_P2_F8_F8_F8;
...
static CallingConv getCallingConventionForDecl(const Decl *D) {
  ...
  if (D->hasAttr<SvmlCallAttr>())
    return CC_X86SvmlCall;
...
const CGFunctionInfo &CodeGenTypes::getFunctionInfo(const CXXMethodDecl *MD) {
  ...
  LLVM_ASSERT(!isa<CXXConstructorDecl>(MD) && "wrong method for contructors!");
  LLVM_ASSERT(!isa<CXXDestructorDecl>(MD) && "wrong method for destructors!");
...
const CGFunctionInfo &CodeGenTypes::getFunctionInfo(const CXXDestructorDecl *D,
  ...
  CanQual<FunctionProtoType> FTP = GetFormalType(D);
  LLVM_ASSERT(FTP->getNumArgs() == 0 && "dtor with formal parameters");
...
const CGFunctionInfo &CodeGenTypes::getFunctionInfo(const FunctionDecl *FD) {
  ...
  LLVM_ASSERT(isa<FunctionType>(FTy));
  if (isa<FunctionNoProtoType>(FTy))
    return getFunctionInfo(FTy.getAs<FunctionNoProtoType>());  
  LLVM_ASSERT(isa<FunctionProtoType>(FTy));
...
const CGFunctionInfo &CodeGenTypes::getFunctionInfo(CanQualType ResTy,
  ...
  for (llvm::SmallVectorImpl<CanQualType>::const_iterator
    ...
    LLVM_ASSERT(I->isCanonicalAsParam());
#endif
...
void CodeGenTypes::GetExpandedTypes(QualType Ty,
  ...
  LLVM_ASSERT(RT && "Can only expand structure types.");
  const RecordDecl *RD = RT->getDecl();
  LLVM_ASSERT(!RD->hasFlexibleArrayMember() &&
         "Cannot expand structure with flexible array.");

  for (RecordDecl::field_iterator i = RD->field_begin(), e = RD->field_end();
         i != e; ++i) {
    const FieldDecl *FD = *i;
    LLVM_ASSERT(!FD->isBitField() &&
...
CodeGenFunction::ExpandTypeToArgs(QualType Ty, RValue RV,
  ...
  RecordDecl *RD = RT->getDecl();
  LLVM_ASSERT(RV.isAggregate() && "Unexpected rvalue during struct expansion");
...
CodeGenTypes::GetFunctionType(const CGFunctionInfo &FI, bool IsVariadic,
  ...
  case ABIArgInfo::Expand:
    LLVM_ASSERT(0 && "Invalid ABI kind for return argument");
  ...
  case ABIArgInfo::Indirect: {
    LLVM_ASSERT(!RetAI.getIndirectAlign() && "Align unused on indirect return.");
...
void CodeGenFunction::EmitFunctionProlog(const CGFunctionInfo &FI,
  ...
  LLVM_ASSERT(FI.arg_size() == Args.size() &&
  ...
  for (FunctionArgList::const_iterator i = Args.begin(), e = Args.end();
    ...
    case ABIArgInfo::Direct: {
      ...
      if (!isa<llvm::StructType>(ArgI.getCoerceToType()) &&
        ...
        LLVM_ASSERT(AI != Fn->arg_end() && "Argument mismatch!");
        llvm::Value *V = AI;
...
RValue CodeGenFunction::EmitCall(const CGFunctionInfo &CallInfo,
  ...
  LLVM_ASSERT(CallInfo.arg_size() == CallArgs.size() &&
         "Mismatch between function signature & arguments.");
  ...
  LLVM_ASSERT(0 && "Unhandled ABIArgInfo::Kind");
  return RValue::get(0);
}
