...
CodeGenModule::getDeclVisibilityMode(const Decl *D) const {
  ...
  if (const VisibilityAttr *attr = D->getAttr<VisibilityAttr>()) {
    ...
    default: LLVM_ASSERT(0 && "Unknown visibility!");
...
llvm::StringRef CodeGenModule::getMangledName(GlobalDecl GD) {
  ...
  if (!getCXXABI().getMangleContext().shouldMangleDeclName(ND)) {
    ...
    LLVM_ASSERT(II && "Attempt to mangle unnamed decl.");
...
void CodeGenModule::EmitOCLAnnotations() {
  ...
  if(getLangOptions().OpenCLBuiltins)
    return; // don't emit the following metadata in the builtins

  llvm::NamedMDNode *OpenCLMetadata =
    TheModule.getOrInsertNamedMetadata("opencl.build.options");

  if(getLangOptions().OpenCLKernelRefl) {
    llvm::Value *ReflStr = llvm::MDString::get(VMContext, llvm::StringRef("-cl-kernel-arg-info"));
    OpenCLMetadata->addOperand(llvm::MDNode::get(VMContext, &ReflStr, 1));
  }
}
...
CodeGenModule::getFunctionLinkage(const FunctionDecl *D) {
  ...
  LLVM_ASSERT(Linkage == GVA_StrongExternal);
...
void CodeGenModule::EmitDeferred() {
  ...
  while (!DeferredDeclsToEmit.empty() || !DeferredVTables.empty()) {
    ...
    LLVM_ASSERT(CGRef && "Deferred decl wasn't referenced?");
...
void CodeGenModule::EmitGlobal(GlobalDecl GD) {
  ...
  if (const FunctionDecl *FD = dyn_cast<FunctionDecl>(Global)) {
    ...
    LLVM_ASSERT(VD->isFileVarDecl() && "Cannot emit local var decl as global.");
...
void CodeGenModule::EmitGlobalDefinition(GlobalDecl GD) {
  ...
  LLVM_ASSERT(0 && "Invalid argument to EmitGlobalDefinition()");
}
...
CodeGenModule::GetOrCreateLLVMFunction(llvm::StringRef MangledName,
  ...
  LLVM_ASSERT(F->getName() == MangledName && "name was uniqued!");
  if (D.getDecl())
  ...
  if (DDI != DeferredDecls.end()) {
    ...
    // synthesized.
      ...
      if (CD->isImplicit()) {
        LLVM_ASSERT(CD->isUsed() && "Sema doesn't consider constructor as used.");
        DeferredDeclsToEmit.push_back(D);
      }
    } else if (const CXXDestructorDecl *DD = dyn_cast<CXXDestructorDecl>(FD)) {
      if (DD->isImplicit()) {
        LLVM_ASSERT(DD->isUsed() && "Sema doesn't consider destructor as used.");
        DeferredDeclsToEmit.push_back(D);
      }
    } else if (const CXXMethodDecl *MD = dyn_cast<CXXMethodDecl>(FD)) {
      if (MD->isCopyAssignment() && MD->isImplicit()) {
        LLVM_ASSERT(MD->isUsed() && "Sema doesn't consider CopyAssignment as used.");
        DeferredDeclsToEmit.push_back(D);
      }
    }
  }
  ...
  if (!IsIncompleteFunction) {
    LLVM_ASSERT(F->getType()->getElementType() == Ty);
    return F;
  }
...
llvm::Constant *CodeGenModule::GetAddrOfGlobalVar(const VarDecl *D,
                                                  const llvm::Type *Ty) {
  LLVM_ASSERT(D->hasGlobalStorage() && "Not a global variable");
...
void CodeGenModule::EmitTentativeDefinition(const VarDecl *D) {
  LLVM_ASSERT(!D->getInit() && "Cannot emit definite definitions here!");
...
void CodeGenModule::EmitGlobalVarDefinition(const VarDecl *D) {
  ...
  if (!InitExpr) {
    ...
    // to do a RAUW.
    LLVM_ASSERT(!ASTTy->isIncompleteType() && "Unexpected incomplete type");
  ...
  if (llvm::ConstantExpr *CE = dyn_cast<llvm::ConstantExpr>(Entry)) {
    LLVM_ASSERT(CE->getOpcode() == llvm::Instruction::BitCast ||
           // all zero index gep.
...
void CodeGenModule::EmitGlobalFunctionDefinition(GlobalDecl GD) {
  ...
  if (cast<llvm::GlobalValue>(Entry)->getType()->getElementType() != Ty) {
    ...
    LLVM_ASSERT(OldFn->isDeclaration() &&
...
void CodeGenModule::EmitAliasDefinition(GlobalDecl GD) {
  ...
  if (Entry) {
    LLVM_ASSERT(Entry->isDeclaration());
...
llvm::Value *CodeGenModule::getBuiltinLibFunction(const FunctionDecl *FD,
                                                  unsigned BuiltinID) {
  LLVM_ASSERT((Context.BuiltinInfo.isLibFunction(BuiltinID) ||
...
GetConstantCFStringEntry(llvm::StringMap<llvm::Constant*> &Map,
  ...
  if (Result != conversionOK) {
    ...
    // this duplicate code.
    LLVM_ASSERT(Result == sourceIllegal && "UTF-8 to UTF-16 conversion failed");
    StringLength = NumBytes;
    return Map.GetOrCreateValue(String);
  }
...
std::string CodeGenModule::GetStringForStringLiteral(const StringLiteral *E) {
  ...
  LLVM_ASSERT(CAT && "String isn't pointer or array!");
...
void CodeGenModule::EmitObjCIvarInitializations(ObjCImplementationDecl *D) {
  ...
  LLVM_ASSERT(DC && "EmitObjCIvarInitializations - null DeclContext");
...
void CodeGenModule::EmitTopLevelDecl(Decl *D) {
  ...
  default:
    ...
    LLVM_ASSERT(isa<TypeDecl>(D) && "Unsupported decl kind");
  }
}
