...
static uint64_t LookupFieldBitOffset(CodeGen::CodeGenModule &CGM,
  ...
  LLVM_ASSERT(Index != Ivars.size() && "Ivar is not inside container!");
...
CGObjCCommonMac::EmitLegacyMessageSend(CodeGen::CodeGenFunction &CGF,
  ...
  if (Method)
  {
    LLVM_ASSERT(CGM.getContext().getCanonicalType(Method->getResultType()) ==
...
void CGObjCMac::GenerateClass(const ObjCImplementationDecl *ID) {
  ...
  if (GV) {
    LLVM_ASSERT(GV->getType()->getElementType() == ObjCTypes.ClassTy &&
    ...
    for (unsigned I = 0, N = AtTryStmt->getNumCatchStmts(); I != N; ++I) {
      ...
      if (AllMatched) {
        ...
        if (CatchParam) {
          ...
          LLVM_ASSERT(CGF.HaveInsertPoint() && "DeclStmt destroyed insert point?");
      ...
      LLVM_ASSERT(OPT && "Unexpected non-object pointer type in @catch");
      const ObjCObjectType *ObjTy = OPT->getObjectType();

      // FIXME: @catch (Class c) ?
      ObjCInterfaceDecl *IDecl = ObjTy->getInterface();
      LLVM_ASSERT(IDecl && "Catch parameter must have Objective-C type!");
...
void CGObjCMac::EmitObjCWeakAssign(CodeGen::CodeGenFunction &CGF,
  ...
  if (!isa<llvm::PointerType>(SrcTy)) {
    ...
    LLVM_ASSERT(Size <= 8 && "does not support size > 8");
...
void CGObjCMac::EmitObjCIvarAssign(CodeGen::CodeGenFunction &CGF,
                                   llvm::Value *src, llvm::Value *dst,
                                   llvm::Value *ivarOffset) {
  LLVM_ASSERT(ivarOffset && "EmitObjCIvarAssign - ivarOffset is NULL");
  const llvm::Type * SrcTy = src->getType();
  if (!isa<llvm::PointerType>(SrcTy)) {
    unsigned Size = CGM.getTargetData().getTypeAllocSize(SrcTy);
    LLVM_ASSERT(Size <= 8 && "does not support size > 8");
...
llvm::Function *CGObjCCommonMac::GetMethodDefinition(const ObjCMethodDecl *MD) {
  ...
  if (MD->hasBody() && MD->getPCHLevel() > 0) {
    ...
    LLVM_ASSERT(MethodDefinitions[MD] && "EmitTopLevelDecl didn't emit the method!");
...
void CGObjCCommonMac::BuildAggrIvarLayout(const ObjCImplementationDecl *OI,
  ...
  for (unsigned i = 0, e = RecFields.size(); i != e; ++i) {
    ...
    if (const ArrayType *Array = CGM.getContext().getAsArrayType(FQT)) {
      ...
      LLVM_ASSERT(CArray && "only array with known element size is supported");
...
bool CGObjCNonFragileABIMac::LegacyDispatchedSelector(Selector Sel) {
  ...
  default:
    LLVM_ASSERT(0 && "Invalid dispatch method!");
...
llvm::GlobalVariable * CGObjCNonFragileABIMac::BuildClassRoTInitializer(
  ...
  LLVM_ASSERT(OID && "CGObjCNonFragileABIMac::BuildClassRoTInitializer");
...
void CGObjCNonFragileABIMac::GenerateClass(const ObjCImplementationDecl *ID) {
  ...
  LLVM_ASSERT(ID->getClassInterface() &&
...
llvm::Constant *CGObjCNonFragileABIMac::EmitIvarList(
  ...
  const ObjCInterfaceDecl *OID = ID->getClassInterface();
  LLVM_ASSERT(OID && "CGObjCNonFragileABIMac::EmitIvarList - null interface");
...
CodeGen::RValue CGObjCNonFragileABIMac::EmitMessageSend(
  ...
  LLVM_ASSERT(Fn && "CGObjCNonFragileABIMac::EmitMessageSend");
  Name += '_';
...
CGObjCNonFragileABIMac::GetEHType(QualType T) {
  ...
  LLVM_ASSERT(PT && "Invalid @catch type.");
  const ObjCInterfaceType *IT = PT->getInterfaceType();
  LLVM_ASSERT(IT && "Invalid @catch type.");
...
CGObjCNonFragileABIMac::GetInterfaceEHType(const ObjCInterfaceDecl *ID,
  ...
  // initializer.
  LLVM_ASSERT((!Entry || !Entry->hasInitializer()) && "Duplicate EHType definition");
  std::string ClassName(getClassSymbolPrefix() + ID->getNameAsString());
