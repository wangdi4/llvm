...
CGBitFieldInfo CGBitFieldInfo::MakeInfo(CodeGenTypes &Types,
  ...
  LLVM_ASSERT(llvm::isPowerOf2_32(TypeSizeInBits) && "Unexpected type size!");
  ...
  while (AccessedTargetBits < FieldSize) {
    ...
    if (AccessStart + AccessWidth > ContainingTypeSizeInBits) {
      ...
      AccessWidth >>= 1;
      LLVM_ASSERT(AccessWidth >= 8 && "Cannot access under byte size!");
      continue;
    }
    ...
    LLVM_ASSERT(FieldOffset < AccessStart + AccessWidth && "Invalid access start!");
    LLVM_ASSERT(AccessStart < FieldOffset + FieldSize && "Invalid access start!");
    ...
    LLVM_ASSERT(NumComponents < 3 && "Unexpected number of components!");
  ...
  LLVM_ASSERT(AccessedTargetBits == FieldSize && "Invalid bit-field access!");
...
void CGRecordLayoutBuilder::LayoutBitField(const FieldDecl *D,
  ...
  if (FieldOffset < NextFieldOffset) {
    LLVM_ASSERT(BitsAvailableInLastField && "Bitfield size mismatch!");
    LLVM_ASSERT(NextFieldOffsetInBytes && "Must have laid out at least one byte!");

    // The bitfield begins in the previous bit-field.
    NumBytesToAppend =
      llvm::RoundUpToAlignment(FieldSize - BitsAvailableInLastField, 8) / 8;
  } else {
    LLVM_ASSERT(FieldOffset % 8 == 0 && "Field offset not aligned correctly");
    ...
    LLVM_ASSERT(NumBytesToAppend && "No bytes to append!");
  }
...
bool CGRecordLayoutBuilder::LayoutField(const FieldDecl *D,
  ...
  LLVM_ASSERT(FieldOffset % 8 == 0 && "FieldOffset is not on a byte boundary!");
  ...
  if (TypeAlignment > Alignment) {
    LLVM_ASSERT(!Packed && "Alignment is wrong even with packed struct!");
    return false;
  }
  ...
  if (FieldOffsetInBytes < AlignedNextFieldOffsetInBytes) {
    LLVM_ASSERT(!Packed && "Could not place field even with packed struct!");
    return false;
  }
...
void CGRecordLayoutBuilder::LayoutUnion(const RecordDecl *D) {
  LLVM_ASSERT(D->isUnion() && "Can't call LayoutUnion on a non-union record!");
  ...
  for (RecordDecl::field_iterator Field = D->field_begin(),
    ...
    LLVM_ASSERT(Layout.getFieldOffset(FieldNo) == 0 &&
  ...
  if (!Align) {
    LLVM_ASSERT(HasOnlyZeroSizedBitFields &&
...
CGRecordLayoutBuilder::LayoutNonVirtualBases(const CXXRecordDecl *RD,
  ...
  if (RD->isDynamicClass()) {
    if (!PrimaryBase) {
      ...
      LLVM_ASSERT(NextFieldOffsetInBytes == 0 &&
...
bool CGRecordLayoutBuilder::LayoutFields(const RecordDecl *D) {
  LLVM_ASSERT(!D->isUnion() && "Can't call LayoutFields on a union!");
  LLVM_ASSERT(Alignment && "Did not set alignment!");
...
CGRecordLayout *CodeGenTypes::ComputeRecordLayout(const RecordDecl *D) {
  ...
  LLVM_ASSERT(TypeSizeInBits == getTargetData().getTypeAllocSizeInBits(Ty) &&
         "Type size mismatch!");
  ...
  for (unsigned i = 0, e = AST_RL.getFieldCount(); i != e; ++i, ++it) {
    ...
    if (!FD->isBitField()) {
      ...
      LLVM_ASSERT(AST_RL.getFieldOffset(i) == SL->getElementOffsetInBits(FieldNo) &&
    ...
    for (unsigned i = 0, e = Info.getNumComponents(); i != e; ++i) {
      ...
      LLVM_ASSERT(AccessBitOffset + AI.AccessWidth <= TypeSizeInBits &&
