...
class ScalarExprEmitter
  ...
  Value* createLAndScalarVal(const BinaryOperator* E);
  Value* createLOrScalarVal(const BinaryOperator* E);
  Value* convertScalar2Vector(Value* scalar, const llvm::VectorType* vecType, bool isSigned);
  Value* getSignedDivSelectValue(const BinOpInfo &Ops);
public:
  ...
  Value *VisitDeclRefExpr(DeclRefExpr *E) {
    ...
    if (E->Evaluate(Result, CGF.getContext()) && Result.Val.isInt()) {
      LLVM_ASSERT(!Result.HasSideEffects && "Constant declref with side-effect?!");
      llvm::ConstantInt *CI 
...
#undef HANDLEBINOP
  ...
  static std::string GetMangledFmaFuncName(BuiltinType::Kind Ty, int NumVecElems)
  {
	  std::stringstream strstr;

	  LLVM_ASSERT(Ty == BuiltinType::Float || Ty == BuiltinType::Double);
	  strstr << "_Z3fma";
	  if (NumVecElems == 1)
		  if (Ty == BuiltinType::Float)
			  strstr << "fff";
		  else
			  strstr << "ddd";
	  else
	  {
		  strstr << "Dv" << NumVecElems << "_";
		  if (Ty == BuiltinType::Float)
			  strstr << "f";
		  else
			  strstr << "d";
  ...
  std::string GetMangledFmaFuncName(const Expr* L, const Expr* R)
  {
	  // determine the kind of built-in type of both operands and number of vector elements
	  BuiltinType::Kind LKind, RKind;
	  unsigned int NumVecElems = 1;
	  if (const BuiltinType* BuiltinTypeL = dyn_cast<BuiltinType>(L->getType()->getCanonicalTypeInternal()))
		  LKind = BuiltinTypeL->getKind();
	  else {
		  if (const VectorType* ExtVecTypeL = L->getType()->getAs<VectorType>())
		  {
			  LKind = dyn_cast<BuiltinType>(ExtVecTypeL->getElementType()->getCanonicalTypeInternal())->getKind();
			  NumVecElems = ExtVecTypeL->getNumElements();
		  }
		  else
			  LLVM_ASSERT(0);
	  }
	  if (const BuiltinType* BuiltinTypeR = dyn_cast<BuiltinType>(R->getType()->getCanonicalTypeInternal()))
		  RKind = BuiltinTypeR->getKind();
	  else
		  if (const VectorType* ExtVecTypeR = R->getType()->getAs<VectorType>())
		  {
			  RKind = dyn_cast<BuiltinType>(ExtVecTypeR->getElementType()->getCanonicalTypeInternal())->getKind();
			  if (NumVecElems == 1)
				  NumVecElems = ExtVecTypeR->getNumElements();
			  else
				  LLVM_ASSERT(NumVecElems == ExtVecTypeR->getNumElements());
		  }
		  else
			  LLVM_ASSERT(0);
	  // determine the function's mangled name
	  if (LKind == BuiltinType::Float && RKind == BuiltinType::Float)
#if 0
		  return GetMangledFmaFuncName(BuiltinType::Float, NumVecElems);
#else
		  return "";
#endif
	  if (LKind == BuiltinType::Double || RKind == BuiltinType::Double)
		  return GetMangledFmaFuncName(BuiltinType::Double, NumVecElems);
	  LLVM_ASSERT(0);
	  return std::string();
  }

  llvm::Value* EmitFmaCall(const Expr* A, const Expr* Mul, const QualType& ParamQualType, const char* FuncName)
  {
	  while (const ParenExpr* PE = dyn_cast<ParenExpr>(Mul))
		  Mul = PE->getSubExpr();
	  if (const BinaryOperator* BinOp = dyn_cast<BinaryOperator>(Mul))
	  {
		  if (BinOp->getOpcode() == BO_Mul)
			  return CGF.EmitFmaCall(A, BinOp->getLHS(), BinOp->getRHS(), ParamQualType, FuncName);
	  }
	  else
		  if (const CastExpr* CE = dyn_cast<CastExpr>(Mul))
		  {
			  const Expr* SubExpr = CE->getSubExpr();
			  while (const ParenExpr* PE = dyn_cast<ParenExpr>(SubExpr))
				  SubExpr = PE->getSubExpr();
			  if (const BinaryOperator* BinOp = dyn_cast<BinaryOperator>(SubExpr))
				  if (BinOp->getOpcode() == BO_Mul)
				  {
					  // we use ImplicitCastExpr for all sub-classes of CastExpr
					  ImplicitCastExpr* ICEL = ImplicitCastExpr::Create(CGF.getContext(), ParamQualType, CE->getCastKind(), BinOp->getLHS(), NULL, VK_RValue);
					  ImplicitCastExpr* ICER = ImplicitCastExpr::Create(CGF.getContext(), ParamQualType, CE->getCastKind(), BinOp->getRHS(), NULL, VK_RValue);
					  llvm::Value* Call = CGF.EmitFmaCall(A, ICEL, ICER, ParamQualType, FuncName);
					  CGF.getContext().Deallocate(ICEL);
					  CGF.getContext().Deallocate(ICER);
					  return Call;
				  }
		  }
		  return NULL;
  ...
  static bool isFloatingType(const QualType& T)
  {
	  if (T->isFloatingType())
		  return true;
	  if (T->isVectorType())
		  if (const ExtVectorType* VT = T->getAs<ExtVectorType>())
  ...
  {
#if 0
	  LLVM_ASSERT(E->getLHS()->isFpContract() == E->getRHS()->isFpContract());
	  if (E->getLHS()->isFpContract() && (isFloatingType(E->getLHS()->getType()) || isFloatingType(E->getRHS()->getType())))
	  {
		  const std::string& FuncName = GetMangledFmaFuncName(E->getLHS(), E->getRHS());
		  if (!FuncName.empty())
		  {
			  llvm::Value* res;
			  if (res = EmitFmaCall(E->getLHS(), E->getRHS(), E->getType(), FuncName.c_str()))
				  return res;
			  if (res = EmitFmaCall(E->getRHS(), E->getLHS(), E->getType(), FuncName.c_str()))
				  return res;
		  }
	  }
#endif
  ...
  Value* VisitBinAddAssign(const CompoundAssignOperator* E)
  {
	  ...
	  if (E->getLHS()->isFpContract() && (isFloatingType(E->getLHS()->getType()) || isFloatingType(E->getRHS()->getType())))
	  {
		  const std::string& FuncName = GetMangledFmaFuncName(E->getLHS(), E->getRHS());
		  if (!FuncName.empty())
			  if (llvm::Value* Fma = EmitFmaCall(E->getLHS(), E->getRHS(), E->getType(), FuncName.c_str()))
			  {
				  bool Ignore = TestAndClearIgnoreResultAssign();
				  const LValue& LHSLV = EmitCheckedLValue(E->getLHS());
				  CGF.EmitStoreThroughLValue(RValue::get(Fma), LHSLV, E->getLHS()->getType());
				  if (Ignore)
					  return NULL;
...
Value *ScalarExprEmitter::EmitConversionToBool(Value *Src, QualType SrcType) {
  LLVM_ASSERT(SrcType.isCanonical() && "EmitScalarConversion strips typedefs");
  ...
  LLVM_ASSERT((SrcType->isIntegerType() || isa<llvm::PointerType>(Src->getType())) &&
...
Value *ScalarExprEmitter::EmitScalarConversion(Value *Src, QualType SrcType,
  ...
  if (isa<llvm::PointerType>(DstTy)) {
    ...
    LLVM_ASSERT(SrcType->isIntegerType() && "Not ptr->ptr or int->ptr conversion?");
  ...
  if (isa<llvm::PointerType>(Src->getType())) {
    ...
    LLVM_ASSERT(isa<llvm::IntegerType>(DstTy) && "not ptr->int?");
  ...
  LLVM_ASSERT(Src->getType()->isFloatingPointTy() && "Unknown real conversion");
  if (isa<llvm::IntegerType>(DstTy)) {
  ...
  LLVM_ASSERT(DstTy->isFloatingPointTy() && "Unknown real conversion");
...
Value *ScalarExprEmitter::VisitInitListExpr(InitListExpr *E) {
  bool Ignore = TestAndClearIgnoreResultAssign();
  (void)Ignore;
  LLVM_ASSERT (Ignore == false && "init list ignored");
  ...
  // If a scalar literal value is specified, the scalar literal value will
  // be replicated to all the components of the vector type. (OpenCL 1.1 spec. 6.1.6)
  if ((CurIdx < ResElts) && CGF.getContext().getLangOptions().OpenCL)
  {
    LLVM_ASSERT(CurIdx == 1 && "Expected a single initializer scalar");
    llvm::Constant *Mask = llvm::Constant::getNullValue(llvm::VectorType::get(
       llvm::Type::getInt32Ty(CGF.getLLVMContext()), VType->getNumElements()));
    V = Builder.CreateShuffleVector(V, V, Mask, "vecinit");
    CurIdx = ResElts;
  }
...
Value *ScalarExprEmitter::EmitCastExpr(CastExpr *CE) {
  ...
  case CK_Unknown:
    ...
    //LLVM_ASSERT(0 && "Unknown cast kind!");
    break;
  ...
  case CK_ToUnion:
    LLVM_ASSERT(0 && "Should be unreachable!");
    break;

  case CK_ArrayToPointerDecay: {
    LLVM_ASSERT(E->getType()->isArrayType() &&
    ...
    if (!E->getType()->isVariableArrayType()) {
      LLVM_ASSERT(isa<llvm::PointerType>(V->getType()) && "Expected pointer");
      LLVM_ASSERT(isa<llvm::ArrayType>(cast<llvm::PointerType>(V->getType())
                                 ->getElementType()) &&
...
EmitScalarPrePostIncDec(const UnaryOperator *E, LValue LV,
  ...
  } else if (isa<llvm::IntegerType>(InVal->getType()) || InVal->getType()->isVectorTy()) {
...
Value *ScalarExprEmitter::VisitUnaryLNot(const UnaryOperator *E) {
  const llvm::Type* const resType = ConvertType(E->getType());

  if (CGF.CGM.getLangOptions().OpenCL && resType->isVectorTy()) {
      const llvm::Type* const valType = ConvertType(E->getSubExpr()->getType());
	  Value* const boolVal = CGF.EmitScalarExpr(E->getSubExpr());
      Value* const valZero = llvm::Constant::getNullValue(valType);
      Value* const resZero = llvm::Constant::getNullValue(resType);
	  Value* const cmp = valType->isFPOrFPVectorTy() ? 
          Builder.CreateFCmpUEQ(boolVal, valZero) : Builder.CreateICmpEQ(boolVal, valZero);
	  return Builder.CreateSelect(cmp, llvm::Constant::getAllOnesValue(resType), resZero);
  }
...
static llvm::Constant* getConstant(const llvm::Type* type, uint64_t v, bool isSigned) {
	if (type->isVectorTy()) {
		const llvm::VectorType *VT = dyn_cast<llvm::VectorType>(type);
		llvm::Constant *cnst = llvm::ConstantInt::get(VT->getElementType(), v, isSigned);
		std::vector<llvm::Constant *> cVec;
		for(unsigned int i = 0; i < VT->getNumElements(); i++)
			cVec.push_back(cnst);
		return llvm::ConstantVector::get(cVec);
	}
	return llvm::ConstantInt::get(type, v, isSigned);
}

Value* ScalarExprEmitter::getSignedDivSelectValue(const BinOpInfo &Ops) {
  const llvm::Type* type = Ops.LHS->getType();
  Value *Zero = llvm::ConstantExpr::getNullValue(Ops.LHS->getType());
  Value *cmp1 = Builder.CreateICmp(llvm::CmpInst::ICMP_EQ, Zero, Ops.RHS, "cmp1");		
  /* Division on integer types which results in a value that lies outside of the range bounded by the maximum and minimum representable values of the
  integer type will not cause an exception but will result in an unspecified value. Because of 2's-complement representation the only problematic case
  is MIN / -1, which equals MAX + 1. */
  Value* ones = getConstant(type, 1, true);
  Value* min = getConstant(type, (uint64_t)1 << (type->getScalarSizeInBits() - 1), true);
  Value* minusOne = getConstant(type, -1, true);
  Value* cmp2 = Builder.CreateICmp(llvm::CmpInst::ICMP_EQ, Ops.LHS, min);
  Value* cmp3 = Builder.CreateICmp(llvm::CmpInst::ICMP_EQ, Ops.RHS, minusOne);
  Value* andd = Builder.CreateAnd(cmp2, cmp3);
  Value* orr = Builder.CreateOr(cmp1, andd);
  return Builder.CreateSelect(orr, ones, Ops.RHS, "sel");
}

Value *ScalarExprEmitter::EmitDiv(const BinOpInfo &Ops) {
  if(!CGF.CGM.getLangOptions().OpenCL)
  {
    if (Ops.LHS->getType()->isFPOrFPVectorTy())
    ...
    {
      return Builder.CreateFDiv(Ops.LHS, Ops.RHS, "div");
    } else if (Ops.Ty->isUnsignedIntOrIntVecType()) {
      Value *Zero = llvm::ConstantExpr::getNullValue(Ops.LHS->getType());
      Value *cmp = Builder.CreateICmp(llvm::CmpInst::ICMP_EQ, Zero, Ops.RHS, "cmp");
      Value *ones;
      if(Ops.LHS->getType()->isVectorTy()) {
        const llvm::VectorType *VT = dyn_cast<llvm::VectorType>(Ops.LHS->getType());
        llvm::Constant *one = llvm::ConstantInt::get(VT->getElementType(), 1);
        std::vector<llvm::Constant *> cVec;
        for(unsigned int i = 0; i < VT->getNumElements(); i++) cVec.push_back(one);
        ones = llvm::ConstantVector::get(cVec);
      } else {
        ones = llvm::ConstantInt::get(Ops.LHS->getType(), 1);
      }
      Value *sel = Builder.CreateSelect(cmp, ones, Ops.RHS, "sel");
      return Builder.CreateUDiv(Ops.LHS, sel, "div");
    } else {
      Value* sel = getSignedDivSelectValue(Ops);
      return Builder.CreateSDiv(Ops.LHS, sel, "div");
    }
  }
}
  ...
  // Rem in C can't be a floating point type: C99 6.5.5p2.
	if(!CGF.CGM.getLangOptions().OpenCL)
	{
	  if (Ops.Ty->isUnsignedIntegerType())
		...
		if (Ops.Ty->isUnsignedIntOrIntVecType()) {
		  Value *Zero = llvm::ConstantExpr::getNullValue(Ops.LHS->getType());
		  Value *cmp = Builder.CreateICmp(llvm::CmpInst::ICMP_EQ, Zero, Ops.RHS, "cmp");
		  Value *ones;
		  if(Ops.LHS->getType()->isVectorTy())
		  ...
		  Value *sel = Builder.CreateSelect(cmp, ones, Ops.RHS, "sel");
		  return Builder.CreateURem(Ops.LHS, sel, "rem");
		} else {
		  Value *sel = getSignedDivSelectValue(Ops);
		  return Builder.CreateSRem(Ops.LHS, sel, "rem");
		}
	}
}
...
Value *ScalarExprEmitter::EmitOverflowCheckedBinOp(const BinOpInfo &Ops) {
  ...
  case BO_MulAssign:
    ...
    break;
  default:
    LLVM_ASSERT(false && "Unsupported operation for overflow detection");
    IID = 0;
  }
  OpID <<= 1;
  OpID |= 1;
...
Value *ScalarExprEmitter::EmitAdd(const BinOpInfo &Ops) {
  ...
  if (PT || OPT) {
    ...
    LLVM_ASSERT((PT || OPT) && "Invalid add expr");
    Ptr = Ops.RHS;
    Idx = Ops.LHS;
    IdxExp = BinOp->getLHS();
  }
...
Value *ScalarExprEmitter::EmitShl(const BinOpInfo &Ops) {
  ...
  if (CGF.getContext().getLangOptions().OpenCL) {
    if (Ops.LHS->getType()->isVectorTy()) {
      /* The result of E1 << E2 is E1 left-shifted by log2(N) least significant bits in E2 viewed as an unsigned integer value, where N is the number of bits
      used to represent the type of E1 elements, if E1 is a vector. */
      const unsigned N = Ops.LHS->getType()->getScalarSizeInBits();
      llvm::Constant* const mask = llvm::ConstantInt::get(RHS->getType(), N - 1);
      RHS = Builder.CreateAnd(Ops.RHS, mask);
    }
  }
...
Value *ScalarExprEmitter::EmitShr(const BinOpInfo &Ops) {
  ...
  if (CGF.getContext().getLangOptions().OpenCL) {
    if (Ops.LHS->getType()->isVectorTy()) {
      ...
      significant bits in E2 viewed as an unsigned integer value, where N is
      the number of bits used to represent the type of E1 elements, if E1 is
      a vector. */	  
      const unsigned N = Ops.LHS->getType()->getScalarSizeInBits();
      llvm::Constant* const mask = llvm::ConstantInt::get(RHS->getType(), N - 1);
      RHS = Builder.CreateAnd(Ops.RHS, mask);
...
Value* ScalarExprEmitter::createLAndScalarVal(const BinaryOperator * E) {
...
Value* ScalarExprEmitter::convertScalar2Vector(Value* scalar, const llvm::VectorType* vecType, bool isSigned)
{
	if (scalar->getType()->getPrimitiveSizeInBits() < vecType->getElementType()->getPrimitiveSizeInBits()) {
		if (isSigned)
			scalar = Builder.CreateSExt(scalar, vecType->getElementType(), "sext");
		else
			scalar = Builder.CreateZExt(scalar, vecType->getElementType(), "zext");
	}
	if (scalar->getType()->getPrimitiveSizeInBits() > vecType->getElementType()->getPrimitiveSizeInBits())
		scalar = Builder.CreateTrunc(scalar, vecType->getElementType(), "trunc");
	const llvm::Type* const i32 = llvm::Type::getInt32Ty(CGF.getLLVMContext());
	llvm::Value* vec = llvm::UndefValue::get(vecType);
	for (unsigned int i = 0; i < vecType->getNumElements(); i++)
		vec = Builder.CreateInsertElement(vec, scalar, llvm::ConstantInt::getSigned(i32, i));
	return vec;
}

Value *ScalarExprEmitter::VisitBinLAnd(const BinaryOperator *E) {
  const llvm::Type *ResTy = ConvertType(E->getType());
  const Type& rType = *E->getRHS()->getType(), &lType = *E->getLHS()->getType();
  const VectorType* const rVecType = rType.getAs<VectorType>(), *const lVecType = lType.getAs<VectorType>();

  if (!lType.isVectorType() && !rType.isVectorType())
	  return createLAndScalarVal(E);
  llvm::Value* lOp = CGF.EmitScalarExpr(E->getLHS()), *rOp = CGF.EmitScalarExpr(E->getRHS());
  const llvm::Type* const lLlvmType = lOp->getType(), *const rLlvmType = rOp->getType();
  if (!lType.isVectorType())
	  lOp = convertScalar2Vector(lOp, dyn_cast<llvm::VectorType>(rLlvmType), rVecType->getElementType().getTypePtr()->isSignedIntegerType());
  if (!rType.isVectorType())
	  rOp = convertScalar2Vector(rOp, dyn_cast<llvm::VectorType>(lLlvmType), lVecType->getElementType().getTypePtr()->isSignedIntegerType());
  // if this is a floating-point vector, we cannot compare to a zeroinitializer of an integer vector type
  llvm::Value *const valZero = llvm::Constant::getNullValue(lOp->getType());
  llvm::Value *const resZero = llvm::Constant::getNullValue(ResTy);
  llvm::Value *const trueVal = ResTy->isVectorTy() ? llvm::Constant::getAllOnesValue(ResTy) : llvm::ConstantInt::getSigned(ResTy, 1);
  LLVM_ASSERT(lOp->getType()->isFPOrFPVectorTy() == rOp->getType()->isFPOrFPVectorTy());
  llvm::Value *const cmp1 = lOp->getType()->isFPOrFPVectorTy() ?
      Builder.CreateFCmpUEQ(lOp, valZero, "cmp1") : Builder.CreateICmpEQ(lOp, valZero, "cmp1");
  llvm::Value *const cmp2 = lOp->getType()->isFPOrFPVectorTy() ?
      Builder.CreateFCmpUEQ(rOp, valZero, "cmp2") : Builder.CreateICmpEQ(rOp, valZero, "cmp2");
  llvm::Value *const select1 = Builder.CreateSelect(cmp1, resZero, trueVal, "select1");
  return Builder.CreateSelect(cmp2, resZero, select1, "select2");
}
Value *ScalarExprEmitter::VisitBinLOr(const BinaryOperator *E) {
  const llvm::Type *ResTy = ConvertType(E->getType());
    ...
    LLVM_ASSERT(lOp->getType()->isFPOrFPVectorTy() == rOp->getType()->isFPOrFPVectorTy());
    llvm::Value* const cmp1 = lOp->getType()->isFPOrFPVectorTy() ?
        Builder.CreateFCmpUNE(lOp, valZero, "cmp1") : Builder.CreateICmpNE(lOp, valZero, "cmp1");
    llvm::Value* const cmp2 = rOp->getType()->isFPOrFPVectorTy() ?
        Builder.CreateFCmpUNE(rOp, valZero, "cmp2") : Builder.CreateICmpNE(rOp, valZero, "cmp2");
    llvm::Value* const select1 = Builder.CreateSelect(cmp1, trueVal, resZero, "select1");
    return Builder.CreateSelect(cmp2, trueVal, select1, "select2");
}

Value* ScalarExprEmitter::createLOrScalarVal(const BinaryOperator* E)
{
	const llvm::Type *ResTy = ConvertType(E->getType());
...
VisitConditionalOperator(const ConditionalOperator *E) {
  ...
  // the normal way.
  if (E->getCond()->getType()->isVectorType() &&	/* If the condition is scalar, we can't use select, since then we would have to evaluate both values, which
														would be incorrect if the value that wasn't selected had some side-effect. In the case of a vector
														condition the ?: operator is equivalent to the select built-in function, which means that both values have
														to be evaluated prior to calling it and therefore the select instruction is suitable here. */
	  E->getLHS()) {
	  const llvm::Type* type = ConvertType(E->getType());
	  CondVal = CGF.EmitScalarExpr(E->getCond());
	  Value* LHS = Visit(E->getLHS()), *RHS = Visit(E->getRHS());
	  if (!type->isVectorTy())
		  CondVal = Builder.CreateICmpNE(CondVal, llvm::Constant::getNullValue(CondVal->getType()));
	  else {
		  const llvm::VectorType* vecType = dyn_cast<llvm::VectorType>(type);
		  if (!LHS->getType()->isVectorTy())
			  LHS = convertScalar2Vector(LHS, vecType, E->getType()->isSignedIntegerType());
		  if (!RHS->getType()->isVectorTy())
			  RHS = convertScalar2Vector(RHS, vecType, E->getType()->isSignedIntegerType());		  
		  if (!CondVal->getType()->isVectorTy())
			  CondVal = Builder.CreateICmpNE(CondVal, llvm::Constant::getNullValue(CondVal->getType()));
		  else {
			  // for each component of a vector type, result[i] = if MSB of c[i] is set ? b[i] : a[i]			  
			  Value* ashr = Builder.CreateAShr(CondVal, vecType->getScalarSizeInBits() - 1);
			  const llvm::VectorType* i1VecType = llvm::VectorType::get(llvm::Type::getInt1Ty(CondVal->getContext()), vecType->getNumElements());
			  CondVal = Builder.CreateTrunc(ashr, i1VecType);
		  }
	  }
	  return Builder.CreateSelect(CondVal, LHS, RHS);
  }
...
llvm::Value* CodeGenFunction::EmitFmaCall(const Expr* Addend, const Expr* Mul1, const Expr* Mul2, const QualType& ParamQualType, const char* FuncName)
{
	const llvm::Type* ParamType = ConvertType(ParamQualType);
	std::vector<const llvm::Type*> ArgsTys;
	for (size_t i = 0; i < 3; i++)
		ArgsTys.push_back(ParamType);
	const llvm::FunctionType* FunctionType = llvm::FunctionType::get(ParamType, ArgsTys, false);
	llvm::Function* F = cast<llvm::Function>(CGM.GetOrCreateLLVMFunction(FuncName, FunctionType, GlobalDecl()));
	F->setLinkage(llvm::Function::ExternalLinkage);
	llvm::Value* Callee = LValue::MakeAddr(F, ParamQualType, 0, CGF.getContext()).getAddress();
	CallArgList CallArgs;	
	CallArgs.push_back(std::make_pair(EmitCallArg(Mul1, ParamQualType), ParamQualType));
	CallArgs.push_back(std::make_pair(EmitCallArg(Mul2, ParamQualType), ParamQualType));
	CallArgs.push_back(std::make_pair(EmitCallArg(Addend, ParamQualType), ParamQualType));
	llvm::SmallVector<llvm::Value*, 16> Args;
	for (CallArgList::const_iterator I = CallArgs.begin(), E = CallArgs.end(); I != E; ++I) {
		RValue RV = I->first;
		Args.push_back(RV.getScalarVal());
	}
	llvm::CallInst* CI = Builder.CreateCall(Callee, Args.data(), Args.data()+Args.size());
	if (Builder.isNamePreserving())
		CI->setName("call");
	return RValue::get(CI).getScalarVal();
}
...
Value *CodeGenFunction::EmitScalarConversion(Value *Src, QualType SrcTy,
                                             QualType DstTy) {
  LLVM_ASSERT(!hasAggregateLLVMType(SrcTy) && !hasAggregateLLVMType(DstTy) &&
         "Invalid scalar expression to emit");
...
Value *CodeGenFunction::EmitComplexToScalarConversion(ComplexPairTy Src,
                                                      QualType SrcTy,
                                                      QualType DstTy) {
  LLVM_ASSERT(SrcTy->isAnyComplexType() && !hasAggregateLLVMType(DstTy) &&
...
#undef COMPOUND_OP
  ...
  case BO_Assign:
  case BO_Comma:
    LLVM_ASSERT(false && "Not valid compound assignment operators");
    break;
  }
