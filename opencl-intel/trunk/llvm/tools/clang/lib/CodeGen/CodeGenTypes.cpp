...
void CodeGenTypes::HandleLateResolvedPointers() {
  LLVM_ASSERT(!PointersToResolve.empty() && "No pointers to resolve!");
...
void CodeGenTypes::UpdateCompletedType(const TagDecl *TD) {
  ...
  LLVM_ASSERT(isa<llvm::OpaqueType>(OpaqueHolder.get()) &&
...
static const llvm::Type* getTypeForFormat(llvm::LLVMContext &VMContext,
  ...
  LLVM_ASSERT(0 && "Unknown float format!");
  return 0;
}
...
#include "clang/AST/TypeNodes.def"
    LLVM_ASSERT(false && "Non-canonical or dependent types aren't possible.");
    break;
  ...
  case Type::Builtin: {
    ...
    case BuiltinType::UndeducedAuto:
      LLVM_ASSERT(0 && "Unexpected builtin type!");
      break;
    }
    LLVM_ASSERT(0 && "Unknown builtin type!");
    break;
  }
  case Type::Complex: {
  ...
  case Type::VariableArray: {
    ...
    LLVM_ASSERT(A.getIndexTypeCVRQualifiers() == 0 &&
...
CodeGenTypes::getCGRecordLayout(const RecordDecl *TD) const {
  ...
  LLVM_ASSERT(Layout && "Unable to find record layout information for type");
  return *Layout;
}
