...
class GlobalDecl {
  ...
  void Init(const Decl *D) {
    LLVM_ASSERT(!isa<CXXConstructorDecl>(D) && "Use other ctor with ctor decls!");
    LLVM_ASSERT(!isa<CXXDestructorDecl>(D) && "Use other ctor with dtor decls!");
...
public:
  ...
  CXXCtorType getCtorType() const {
    LLVM_ASSERT(isa<CXXConstructorDecl>(getDecl()) && "Decl is not a ctor!");
    return static_cast<CXXCtorType>(Value.getInt());
  }

  CXXDtorType getDtorType() const {
    LLVM_ASSERT(isa<CXXDestructorDecl>(getDecl()) && "Decl is not a dtor!");
