...
void CodeGenFunction::EmitStmt(const Stmt *S) {
  ...
  if (!HaveInsertPoint()) {
    ...
    if (!ContainsLabel(S)) {
      ...
      LLVM_ASSERT(!isa<DeclStmt>(*S) && "Unexpected DeclStmt!");
      return;
    }
  ...
  case Stmt::ObjCAtCatchStmtClass:
    LLVM_ASSERT(0 && "@catch statements should be handled by EmitObjCAtTryStmt");
    break;
  case Stmt::ObjCAtFinallyStmtClass:
    LLVM_ASSERT(0 && "@finally statements should be handled by EmitObjCAtTryStmt");
    break;
...
void CodeGenFunction::EmitLabel(const LabelStmt &S) {
  ...
  // it from the branch-fixups list.
    ...
    LLVM_ASSERT(!Dest.getScopeDepth().isValid() && "already emitted label!");
...
void CodeGenFunction::EmitBreakStmt(const BreakStmt &S) {
  LLVM_ASSERT(!BreakContinueStack.empty() && "break stmt not in a loop or switch!");
...
void CodeGenFunction::EmitContinueStmt(const ContinueStmt &S) {
  LLVM_ASSERT(!BreakContinueStack.empty() && "continue stmt not in a loop!");
...
void CodeGenFunction::EmitCaseStmtRange(const CaseStmt &S) {
  LLVM_ASSERT(S.getRHS() && "Expected RHS value in CaseStmt");
...
void CodeGenFunction::EmitDefaultStmt(const DefaultStmt &S) {
  ...
  LLVM_ASSERT(DefaultBlock->empty() &&
  ...
  while (*Constraint) {
    ...
    case '[': {
      ...
      bool result = Target.resolveSymbolicName(Constraint,
                                               &(*OutCons)[0],
                                               OutCons->size(), Index);
      LLVM_ASSERT(result && "Could not resolve symbolic name"); result=result;
...
void CodeGenFunction::EmitAsmStmt(const AsmStmt &S) {
  ...
  for (unsigned i = 0, e = S.getNumOutputs(); i != e; i++) {
    ...
    LLVM_ASSERT(IsValid && "Failed to parse output constraint"); 
  ...
  for (unsigned i = 0, e = S.getNumInputs(); i != e; i++) {
    ...
    LLVM_ASSERT(IsValid && "Failed to parse input constraint"); (void)IsValid;
  ...
  for (unsigned i = 0, e = S.getNumOutputs(); i != e; i++) {
    ...
    if (!Info.allowsMemory() && !hasAggregateLLVMType(OutExpr->getType())) {
      ...
      if (Info.hasMatchingInput()) {
        ...
        LLVM_ASSERT(InputNo != S.getNumInputs() && "Didn't find matching input!");
