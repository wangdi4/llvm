...
llvm::Value *ItaniumCXXABI::EmitMemberDataPointerAddress(CodeGenFunction &CGF,
                                                         ...
                                                         llvm::Value *MemPtr,
                                           const MemberPointerType *MPT) {
  LLVM_ASSERT(MemPtr->getType() == getPtrDiffTy());
...
ItaniumCXXABI::EmitMemberPointerConversion(CodeGenFunction &CGF,
                                           const CastExpr *E,
                                           llvm::Value *Src) {
  LLVM_ASSERT(E->getCastKind() == CK_DerivedToBaseMemberPointer ||
...
ItaniumCXXABI::EmitMemberPointerConversion(llvm::Constant *C,
  ...
  // away the offset.
  if (SrcTy->isMemberDataPointer()) {
    LLVM_ASSERT(C->getType() == getPtrDiffTy());
...
llvm::Constant *ItaniumCXXABI::EmitMemberPointer(const CXXMethodDecl *MD) {
  LLVM_ASSERT(MD->isInstance() && "Member function must not be static!");
...
bool ItaniumCXXABI::NeedsArrayCookie(QualType ElementType) {
  ...
  LLVM_ASSERT(UsualDeallocationFunction->getNumParams() == 2 && 
...
llvm::Value *ItaniumCXXABI::InitializeArrayCookie(CodeGenFunction &CGF,
                                                  ...
                                                  llvm::Value *NumElements,
                                                  QualType ElementType) {
  LLVM_ASSERT(NeedsArrayCookie(ElementType));
