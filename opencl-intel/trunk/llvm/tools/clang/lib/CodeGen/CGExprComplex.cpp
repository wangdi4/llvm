...
public:
  ...
  ComplexPairTy EmitLoadOfLValue(const Expr *E) {
    ...
    LLVM_ASSERT(LV.isKVCRef() && "Unknown LValue type!");
  ...
  ComplexPairTy VisitStmt(Stmt *S) {
    ...
    LLVM_ASSERT(0 && "Stmt can't have complex result type!");
  ...
  ComplexPairTy VisitCXXScalarValueInitExpr(CXXScalarValueInitExpr *E) {
    LLVM_ASSERT(E->getType()->isAnyComplexType() && "Expected complex type!");
...
ComplexPairTy ComplexExprEmitter::VisitBinAssign(const BinaryOperator *E) {
  ...
  bool ignreal = TestAndClearIgnoreRealAssign();
  bool ignimag = TestAndClearIgnoreImagAssign();
  LLVM_ASSERT(CGF.getContext().hasSameUnqualifiedType(E->getLHS()->getType(), 
                                                 E->getRHS()->getType()) &&
         "Invalid assignment");
  // Emit the RHS.
...
VisitConditionalOperator(const ConditionalOperator *E) {
  ...
  LLVM_ASSERT(E->getLHS() && "Must have LHS for complex value");
...
ComplexPairTy CodeGenFunction::EmitComplexExpr(const Expr *E, bool IgnoreReal,
  ...
  LLVM_ASSERT(E && E->getType()->isAnyComplexType() &&
