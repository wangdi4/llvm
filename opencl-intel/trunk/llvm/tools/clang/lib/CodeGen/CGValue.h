...
public:
  ...
  llvm::Value *getScalarVal() const {
    LLVM_ASSERT(isScalar() && "Not a scalar!");
  ...
  llvm::Value *getAggregateAddr() const {
    LLVM_ASSERT(isAggregate() && "Not an aggregate!");
    return V1.getPointer();
  }
...
private:
  void Initialize(Qualifiers Quals, unsigned Alignment = 0) {
    ...
    LLVM_ASSERT(this->Alignment == Alignment && "Alignment exceeds allowed max!");
...
public:
  ...
  // simple lvalue
  llvm::Value *getAddress() const { LLVM_ASSERT(isSimple()); return V; }

  // vector elt lvalue
  llvm::Value *getVectorAddr() const { LLVM_ASSERT(isVectorElt()); return V; }
  llvm::Value *getVectorIdx() const { LLVM_ASSERT(isVectorElt()); return VectorIdx; }

  // extended vector elements.
  llvm::Value *getExtVectorAddr() const { LLVM_ASSERT(isExtVectorElt()); return V; }
  llvm::Constant *getExtVectorElts() const {
    LLVM_ASSERT(isExtVectorElt());
    return VectorElts;
  }

  // bitfield lvalue
  llvm::Value *getBitFieldBaseAddr() const {
    LLVM_ASSERT(isBitField());
    return V;
  }
