...
#if 0	// I've put this in "#if 0", because the signature of computeInfo has changed in clang 2.8
/// GHAL3DABIInfo -- GHAL3D ABI information
class GHAL3DABIInfo : public ABIInfo {
	// Arg size limit is 16 constant registers * 16 bytes per constant register *
	// 8 bits == 2048 bits.
	static const uint64_t maxArgSizeInBits = 2048;

	//static bool isExpandableGHAL3DType(QualType Ty);
...
public:
	...
	virtual void computeInfo(CGFunctionInfo &FI, ASTContext &Context,
		llvm::LLVMContext &VMContext) const {
	FI.getReturnInfo() = classifyReturnType(FI.getReturnType(), Context, VMContext);
	for (CGFunctionInfo::arg_iterator it = FI.arg_begin(), ie = FI.arg_end(); it != ie; ++it)
...
ABIArgInfo GHAL3DABIInfo::classifyReturnType(QualType RetTy,
											 ASTContext &Context,
											 llvm::LLVMContext &VMContext) const {
	if (RetTy->isVoidType()) {
...
ABIArgInfo GHAL3DABIInfo::classifyArgumentType(QualType Ty,
											   ASTContext &Context,
											   llvm::LLVMContext &VMContext) const {
	LLVM_ASSERT(Context.getTypeSize(Ty) <= GHAL3DABIInfo::maxArgSizeInBits && 
													   "Function arg is too large");
...
llvm::Value *GHAL3DABIInfo::EmitVAArg(llvm::Value *VAListAddr, QualType Ty,
									  CodeGenFunction &CGF) const {
	return 0;
}
#endif
    ...
    if (const Type *SeltTy = isSingleElementStruct(RetTy, getContext())) {
      if (const BuiltinType *BT = SeltTy->getAs<BuiltinType>()) {
        ...
        if (BT->getKind() == BuiltinType::Float) {
          LLVM_ASSERT(getContext().getTypeSize(RetTy) ==
...
X86_64ABIInfo::Class X86_64ABIInfo::merge(Class Accum, Class Field) {
  ...
  LLVM_ASSERT((Accum != Memory && Accum != ComplexX87) &&
...
void X86_64ABIInfo::classify(QualType Ty, uint64_t OffsetBase,
  ...
  if (const ConstantArrayType *AT = getContext().getAsConstantArrayType(Ty)) {
    ...
    if (Hi == Memory)
      Lo = Memory;
    LLVM_ASSERT((Hi != SSEUp || Lo == SSE) && "Invalid SSEUp array classification.");
    return;
  }
  ...
  if (const RecordType *RT = Ty->getAs<RecordType>()) {
    ...
    if (const CXXRecordDecl *CXXRD = dyn_cast<CXXRecordDecl>(RD)) {
      for (CXXRecordDecl::base_class_const_iterator i = CXXRD->bases_begin(),
             e = CXXRD->bases_end(); i != e; ++i) {
        LLVM_ASSERT(!i->isVirtual() && !i->getType()->isDependentType() &&
    ...
    for (RecordDecl::field_iterator i = RD->field_begin(), e = RD->field_end();
      ...
      if (BitField) {
        ...
        if (EB_Lo) {
          LLVM_ASSERT(EB_Hi == EB_Lo && "Invalid classification, type > 16 bytes.");
          FieldLo = NoClass;
          FieldHi = Integer;
        } else {
          FieldLo = Integer;
...
GetINTEGERTypeAtOffset(const llvm::Type *IRType, unsigned IROffset,
  ...
  LLVM_ASSERT(TySizeInBytes != SourceOffset && "Empty field?");
...
GetX86_64ByValArgumentPair(const llvm::Type *Lo, const llvm::Type *Hi,
  ...
  LLVM_ASSERT(HiStart != 0 && HiStart <= 8 && "Invalid x86-64 argument pair!");
  ...
  if (HiStart != 8) {
    ...
    if (Lo->isFloatTy())
      ...
      LLVM_ASSERT(Lo->isIntegerTy() && "Invalid/unknown lo type");
  ...
  LLVM_ASSERT(TD.getStructLayout(Result)->getElementOffset(1) == 8 &&
...
classifyReturnType(QualType RetTy) const {
  ...
  LLVM_ASSERT((Hi != Memory || Lo == Memory) && "Invalid memory classification.");
  LLVM_ASSERT((Hi != SSEUp || Lo == SSE) && "Invalid SSEUp classification.");
  ...
  case NoClass:
    ...
    // null.
    LLVM_ASSERT((Hi == SSE || Hi == Integer || Hi == X87Up) &&
           "Unknown missing lo part");
    break;

  case SSEUp:
  case X87Up:
    LLVM_ASSERT(0 && "Invalid classification for lo word.");
  ...
  case X87:
    ...
    // %st1.
  case ComplexX87:
    LLVM_ASSERT(Hi == ComplexX87 && "Unexpected ComplexX87 classification.");
  ...
  case Memory:
  case X87:
    LLVM_ASSERT(0 && "Invalid classification for hi word.");
  ...
  case SSEUp:
    LLVM_ASSERT(Lo == SSE && "Unexpected SSEUp classification.");
...
llvm::Value *X86_64ABIInfo::EmitVAArg(llvm::Value *VAListAddr, QualType Ty,
  ...
  if (neededInt && neededSSE) {
    // FIXME: Cleanup.
    LLVM_ASSERT(AI.isDirect() && "Unexpected ABI info for mixed regs");
    const llvm::StructType *ST = cast<llvm::StructType>(AI.getCoerceToType());
    llvm::Value *Tmp = CGF.CreateTempAlloca(ST);
    LLVM_ASSERT(ST->getNumElements() == 2 && "Unexpected ABI info for mixed regs");
    const llvm::Type *TyLo = ST->getElementType(0);
    const llvm::Type *TyHi = ST->getElementType(1);
    LLVM_ASSERT((TyLo->isFPOrFPVectorTy() ^ TyHi->isFPOrFPVectorTy()) &&
    ...
    LLVM_ASSERT(neededSSE == 2 && "Invalid number of needed registers!");
