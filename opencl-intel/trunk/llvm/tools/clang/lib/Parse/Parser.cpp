...
Parser::Parser(Preprocessor &pp, Sema &actions)
  ...
  if (getLang().OpenCL) {
    ...
    OpenCLFPContractHandler.reset(
                  new PragmaOpenCLFPContractHandler(actions,*this));
...
void Parser::ExitScope() {
  LLVM_ASSERT(getCurScope() && "Scope imbalance!");
...
Parser::~Parser() {
  ...
  if (getLang().OpenCL) {
    ...
    OpenCLExtensionHandler.reset();
    PP.RemovePragmaHandler("OPENCL", OpenCLFPContractHandler.get());
    OpenCLFPContractHandler.reset();
  }
...
void Parser::Initialize() {
  ...
  LLVM_ASSERT(getCurScope() == 0 && "A scope is already active?");
...
void Parser::ParseTranslationUnit() {
  ...
  ExitScope();
  LLVM_ASSERT(getCurScope() == 0 && "Scope imbalance!");
}
...
bool Parser::isStartOfFunctionDefinition(const ParsingDeclarator &Declarator) {
  LLVM_ASSERT(Declarator.getTypeObject(0).Kind == DeclaratorChunk::Function &&
...
Decl *Parser::ParseFunctionDefinition(ParsingDeclarator &D,
  ...
  LLVM_ASSERT(FnTypeInfo.Kind == DeclaratorChunk::Function &&
...
Parser::ExprResult Parser::ParseSimpleAsm(SourceLocation *EndLoc) {
  LLVM_ASSERT(Tok.is(tok::kw_asm) && "Not an asm!");
...
bool Parser::TryAnnotateTypeOrScopeToken(bool EnteringContext) {
  LLVM_ASSERT((Tok.is(tok::identifier) || Tok.is(tok::coloncolon)
  ...
  if (Tok.is(tok::kw_typename)) {
    ...
    if (Tok.is(tok::identifier)) {
      ...
      LLVM_ASSERT(Tok.is(tok::annot_typename) &&
