  ...
  while (true) {
    ...
    if (Tok.is(tok::annot_template_id) && NextToken().is(tok::coloncolon)) {
      ...
      if (TemplateId->Kind == TNK_Type_template ||
        ...
        Token TypeToken = Tok;
        ConsumeToken();
        LLVM_ASSERT(Tok.is(tok::coloncolon) && "NextToken() not working properly!");
        SourceLocation CCLoc = ConsumeToken();
      ...
      LLVM_ASSERT(false && "FIXME: Only type template names supported here");
    }
    ...
    if (Next.is(tok::coloncolon)) {
      ...
      LLVM_ASSERT((Tok.is(tok::coloncolon) || Tok.is(tok::colon)) &&
...
ExprResult Parser::ParseCXXCasts() {
  ...
  switch (Kind) {
  default: LLVM_ASSERT(0 && "Unknown C++ cast!"); abort();
...
ExprResult Parser::ParseCXXTypeid() {
  LLVM_ASSERT(Tok.is(tok::kw_typeid) && "Not 'typeid'!");
...
Parser::ParseCXXPseudoDestructor(ExprArg Base, SourceLocation OpLoc,
  ...
  if (Tok.is(tok::identifier)) {
    ...
    ConsumeToken();
    LLVM_ASSERT(Tok.is(tok::coloncolon) &&"ParseOptionalCXXScopeSpecifier fail");
    CCLoc = ConsumeToken();
  ...
  // Parse the tilde.
  LLVM_ASSERT(Tok.is(tok::tilde) && "ParseOptionalCXXScopeSpecifier fail");
...
ExprResult Parser::ParseThrowExpression() {
  LLVM_ASSERT(Tok.is(tok::kw_throw) && "Not throw!");
...
ExprResult Parser::ParseCXXThis() {
  LLVM_ASSERT(Tok.is(tok::kw_this) && "Not 'this'!");
...
Parser::ParseCXXTypeConstructExpression(const DeclSpec &DS) {
  ...
  LLVM_ASSERT((Exprs.size() == 0 || Exprs.size()-1 == CommaLocs.size())&&
...
void Parser::ParseCXXSimpleTypeSpecifier(DeclSpec &DS) {
  ...
  case tok::coloncolon:   // ::foo::bar
    LLVM_ASSERT(0 && "Annotation token should already be formed!");
  default:
    LLVM_ASSERT(0 && "Not a simple-type-specifier token!");
    abort();
...
bool Parser::ParseUnqualifiedIdTemplateId(CXXScopeSpec &SS,
  ...
  LLVM_ASSERT((AssumeTemplateId || Tok.is(tok::less)) &&
...
bool Parser::ParseUnqualifiedIdOperator(CXXScopeSpec &SS, bool EnteringContext,
  ...
  LLVM_ASSERT(Tok.is(tok::kw_operator) && "Expected 'operator' keyword");
...
Parser::ParseCXXNewExpression(bool UseGlobal, SourceLocation Start) {
  LLVM_ASSERT(Tok.is(tok::kw_new) && "expected 'new' token");
...
Parser::ParseCXXDeleteExpression(bool UseGlobal, SourceLocation Start) {
  LLVM_ASSERT(Tok.is(tok::kw_delete) && "Expected 'delete' keyword");
...
static UnaryTypeTrait UnaryTypeTraitFromTokKind(tok::TokenKind kind) {
  switch(kind) {
  default: LLVM_ASSERT(false && "Not a known unary type trait.");
...
Parser::ParseCXXAmbiguousParenExpression(ParenParseOption &ExprType,
                                         ...
                                         SourceLocation &RParenLoc) {
  LLVM_ASSERT(getLang().CPlusPlus && "Should only be called for C++!");
  LLVM_ASSERT(ExprType == CastExpr && "Compound literals are not ambiguous!");
  LLVM_ASSERT(isTypeIdInParens() && "Not a type-id!");
  ...
  LLVM_ASSERT(ParseAs == SimpleExpr);
