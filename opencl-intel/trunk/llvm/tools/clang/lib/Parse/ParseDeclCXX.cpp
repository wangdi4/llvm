...
Decl *Parser::ParseNamespace(unsigned Context,
  ...
  LLVM_ASSERT(Tok.is(tok::kw_namespace) && "Not a namespace!");
...
Decl *Parser::ParseNamespaceAlias(SourceLocation NamespaceLoc,
                                              ...
                                              SourceLocation &DeclEnd) {
  LLVM_ASSERT(Tok.is(tok::equal) && "Not equal token");
...
Decl *Parser::ParseLinkage(ParsingDeclSpec &DS,
                                       unsigned Context) {
  LLVM_ASSERT(Tok.is(tok::string_literal) && "Not a string literal!");
...
Decl *Parser::ParseUsingDirectiveOrDeclaration(unsigned Context,
                                                     SourceLocation &DeclEnd,
                                                     CXX0XAttributeList Attr) {
  LLVM_ASSERT(Tok.is(tok::kw_using) && "Not using token");
...
Decl *Parser::ParseUsingDirective(unsigned Context,
                                              ...
                                              SourceLocation &DeclEnd,
                                              AttributeList *Attr) {
  LLVM_ASSERT(Tok.is(tok::kw_namespace) && "Not 'namespace' token");
...
Decl *Parser::ParseStaticAssertDeclaration(SourceLocation &DeclEnd){
  LLVM_ASSERT(Tok.is(tok::kw_static_assert) && "Not a static_assert declaration");
...
void Parser::ParseDecltypeSpecifier(DeclSpec &DS) {
  LLVM_ASSERT(Tok.is(tok::kw_decltype) && "Not a decltype specifier");
...
Parser::TypeResult Parser::ParseClassName(SourceLocation &EndLocation,
  ...
  if (Tok.is(tok::annot_template_id)) {
    ...
    if (TemplateId->Kind == TNK_Type_template ||
      ...
      LLVM_ASSERT(Tok.is(tok::annot_typename) && "template-id -> type failed");
...
void Parser::ParseClassSpecifier(tok::TokenKind TagTokKind,
  ...
  if (TagTokKind == tok::kw_struct)
    ...
    LLVM_ASSERT(TagTokKind == tok::kw_union && "Not a class specifier");
  ...
  if (TemplateId) {
    ...
    // We diagnose this error in ActOnClassTemplateSpecialization.
      ...
      if (TemplateInfo.Kind == ParsedTemplateInfo::ExplicitInstantiation) {
        ...
        // the '<>' after 'template'.
        LLVM_ASSERT(TUK == Sema::TUK_Definition && "Expected a definition here");
...
void Parser::ParseBaseClause(Decl *ClassDecl) {
  LLVM_ASSERT(Tok.is(tok::colon) && "Not a base clause");
  ConsumeToken();
...
void Parser::ParseCXXClassMemberDeclaration(AccessSpecifier AS,
  ...
  if (Tok.is(tok::kw_template)) {
    LLVM_ASSERT(!TemplateInfo.TemplateParams &&
...
void Parser::ParseCXXMemberSpecification(SourceLocation RecordLoc,
  ...
  LLVM_ASSERT((TagType == DeclSpec::TST_struct ||
         TagType == DeclSpec::TST_union  ||
...
void Parser::ParseConstructorInitializer(Decl *ConstructorDecl) {
  LLVM_ASSERT(Tok.is(tok::colon) && "Constructor initializer always starts with ':'");
...
bool Parser::ParseExceptionSpecification(SourceLocation &EndLoc,
  ...
  LLVM_ASSERT(Tok.is(tok::kw_throw) && "expected throw");
...
void Parser::PushParsingClass(Decl *ClassDecl, bool NonNestedClass) {
  LLVM_ASSERT((NonNestedClass || !ClassStack.empty()) &&
...
void Parser::PopParsingClass() {
  LLVM_ASSERT(!ClassStack.empty() && "Mismatched push/pop for class parsing");
  ...
  LLVM_ASSERT(!ClassStack.empty() && "Missing top-level class?");
  ...
  LLVM_ASSERT(getCurScope()->isClassScope() && "Nested class outside of class scope?");
...
CXX0XAttributeList Parser::ParseCXX0XAttributes(SourceLocation *EndLoc) {
  LLVM_ASSERT(Tok.is(tok::l_square) && NextToken().is(tok::l_square)
...
ExprResult Parser::ParseCXX0XAlignArgument(SourceLocation Start) {
  if (isTypeIdInParens()) {
    ...
    return Actions.ActOnSizeOfAlignOfExpr(TypeLoc, SizeOfAlignOfExpr::AlignOf, true,
