...
Parser::ParseRHSOfBinaryExpression(ExprResult LHS, prec::Level MinPrec) {
  ...
  while (1) {
    ...
    LLVM_ASSERT(NextTokPrec <= ThisPrec && "Recursion didn't work!");
...
ExprResult Parser::ParseCastExpression(bool isUnaryExpression,
  ...
  case tok::kw_vec_step:   // unary-expression: 'vec_step' OpenCL specific unary-expression
...
Parser::ParsePostfixExpressionSuffix(ExprResult LHS) {
  ...
  while (1) {
    ...
    case tok::l_paren: {   // p-e: p-e '(' argument-expression-list[opt] ')'
      ...
      if (!LHS.isInvalid()) {
        LLVM_ASSERT((ArgExprs.size() == 0 || ArgExprs.size()-1 == CommaLocs.size())&&
...
Parser::ParseExprAfterTypeofSizeofAlignof(const Token &OpTok,
  ...
  LLVM_ASSERT((OpTok.is(tok::kw_typeof)    || OpTok.is(tok::kw_sizeof) ||
...
ExprResult Parser::ParseSizeofAlignofExpression() {
  LLVM_ASSERT((Tok.is(tok::kw_sizeof) || Tok.is(tok::kw___alignof)
  ...
  SizeOfAlignOfExpr::Kind_t exprKind = SizeOfAlignOfExpr::SizeOf;
  if (OpTok.is(tok::kw_alignof) || OpTok.is(tok::kw___alignof))
    exprKind = SizeOfAlignOfExpr::AlignOf;
  else if (OpTok.is(tok::kw_vec_step))
    exprKind = SizeOfAlignOfExpr::VecStep;
...
ExprResult Parser::ParseBuiltinPrimaryExpression() {
  ...
  switch (T) {
  default: LLVM_ASSERT(0 && "Not a builtin primary expression!");
  case tok::kw___builtin_va_arg: {
...
Parser::ParseParenExpression(ParenParseOption &ExprType, bool stopIfCastExpr,
                             ...
                             SourceLocation &RParenLoc) {
  LLVM_ASSERT(Tok.is(tok::l_paren) && "Not a paren expr!");
...
Parser::ParseCompoundLiteralExpression(ParsedType Ty,
                                       ...
                                       SourceLocation RParenLoc) {
  LLVM_ASSERT(Tok.is(tok::l_brace) && "Not a compound literal!");
...
ExprResult Parser::ParseStringLiteralExpression() {
  LLVM_ASSERT(isTokenStringLiteral() && "Not a string literal!");
...
ExprResult Parser::ParseBlockLiteralExpression() {
  LLVM_ASSERT(Tok.is(tok::caret) && "block literal starts with ^");
