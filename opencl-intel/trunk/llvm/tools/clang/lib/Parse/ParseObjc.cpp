...
Decl *Parser::ParseObjCAtInterfaceDeclaration(
  ...
  LLVM_ASSERT(Tok.isObjCAtKeyword(tok::objc_interface) &&
...
void Parser::ParseObjCPropertyAttribute(ObjCDeclSpec &DS, Decl *ClassDecl,
                                        Decl **Methods, 
                                        unsigned NumMethods) {
  LLVM_ASSERT(Tok.getKind() == tok::l_paren);
...
Decl *Parser::ParseObjCMethodPrototype(Decl *IDecl,
                                       tok::ObjCKeywordKind MethodImplKind) {
  LLVM_ASSERT((Tok.is(tok::minus) || Tok.is(tok::plus)) && "expected +/-");
...
void Parser::ParseObjCTypeQualifierList(ObjCDeclSpec &DS, bool IsParameter) {
  while (1) {
    ...
    for (unsigned i = 0; i != objc_NumQuals; ++i) {
      ...
      switch (i) {
      default: LLVM_ASSERT(0 && "Unknown decl qualifier");
...
ParseObjCProtocolReferences(llvm::SmallVectorImpl<Decl *> &Protocols,
  ...
  LLVM_ASSERT(Tok.is(tok::less) && "expected <");
...
void Parser::ParseObjCClassInstanceVariables(Decl *interfaceDecl,
                                             ...
                                             SourceLocation atLoc) {
  LLVM_ASSERT(Tok.is(tok::l_brace) && "expected {");
...
Decl *Parser::ParseObjCAtProtocolDeclaration(SourceLocation AtLoc,
  ...
  LLVM_ASSERT(Tok.isObjCAtKeyword(tok::objc_protocol) &&
...
Decl *Parser::ParseObjCAtImplementationDeclaration(
  SourceLocation atLoc) {
  LLVM_ASSERT(Tok.isObjCAtKeyword(tok::objc_implementation) &&
...
Decl *Parser::ParseObjCAtEndDeclaration(SourceRange atEnd) {
  LLVM_ASSERT(Tok.isObjCAtKeyword(tok::objc_end) &&
...
Decl *Parser::ParseObjCAtAliasDeclaration(SourceLocation atLoc) {
  LLVM_ASSERT(Tok.isObjCAtKeyword(tok::objc_compatibility_alias) &&
...
Decl *Parser::ParseObjCPropertySynthesize(SourceLocation atLoc) {
  LLVM_ASSERT(Tok.isObjCAtKeyword(tok::objc_synthesize) &&
...
Decl *Parser::ParseObjCPropertyDynamic(SourceLocation atLoc) {
  LLVM_ASSERT(Tok.isObjCAtKeyword(tok::objc_dynamic) &&
...
StmtResult Parser::ParseObjCTryStmt(SourceLocation atLoc) {
  ...
  while (Tok.is(tok::at)) {
    ...
    if (Tok.isObjCAtKeyword(tok::objc_catch)) {
      ...
      LLVM_ASSERT(Tok.isObjCAtKeyword(tok::objc_finally) && "Lookahead confused?");
...
bool Parser::isSimpleObjCMessageExpression() {
  LLVM_ASSERT(Tok.is(tok::l_square) && getLang().ObjC1 &&
...
ExprResult Parser::ParseObjCMessageExpression() {
  LLVM_ASSERT(Tok.is(tok::l_square) && "'[' expected");
...
Parser::ParseObjCEncodeExpression(SourceLocation AtLoc) {
  LLVM_ASSERT(Tok.isObjCAtKeyword(tok::objc_encode) && "Not an @encode expression!");
