...
AttributeList *Parser::ParseGNUAttributes(SourceLocation *EndLoc) {
  LLVM_ASSERT(Tok.is(tok::kw___attribute) && "Not a GNU attribute list!");
    ...
    while (Tok.is(tok::identifier) || isDeclarationSpecifier() ||
      ...
      if (Tok.is(tok::l_paren)) {
        ...
        if (Tok.is(tok::identifier)) {
          ...
          if(!AttrName->getName().compare("vec_type_hint"))
          {
            TypeResult Ty = ParseTypeName();

            if(!Ty.isInvalid())
            {
              Expr *expr = Actions.ActOnSizeOfAlignOfExpr(Tok.getLocation(), SizeOfAlignOfExpr::SizeOf, true,
                  Ty.get().getAsOpaquePtr(), SourceRange(Tok.getLocation(), Tok.getLocation())).get();
              ExprVector ArgExprs(Actions);
              ArgExprs.push_back(expr);
              CurrAttr = new AttributeList(AttrName, AttrNameLoc, 0, AttrNameLoc,
                                           0, SourceLocation(),
										   ArgExprs.take(), ArgExprs.size(), CurrAttr);
          ...
          if(!AttrName->getName().compare("endian"))
          {
            IdentifierInfo *ii = Tok.getIdentifierInfo();
            Expr *expr = Actions.ActOnEndianAttr(Tok.getLocation(), ii->getName().str().c_str(), ii->getName().size()).get();
            ConsumeToken();
          ...
          case tok::kw_typeof:
            if(!AttrName->getName().compare("vec_type_hint"))
            {
              ...
              Expr *expr = Actions.ActOnSizeOfAlignOfExpr(Tok.getLocation(), SizeOfAlignOfExpr::SizeOf, true,
                                            Ty.getAsOpaquePtr(), SourceRange(Tok.getLocation(), Tok.getLocation())).get();
              ExprVector ArgExprs(Actions);
              ArgExprs.push_back(expr);
              CurrAttr = new AttributeList(AttrName, AttrNameLoc, 0, AttrNameLoc,
                                           0, SourceLocation(),
...
AttributeList* Parser::ParseMicrosoftDeclSpec(AttributeList *CurrAttr) {
  LLVM_ASSERT(Tok.is(tok::kw___declspec) && "Not a declspec!");
...
Parser::DeclGroupPtrTy Parser::ParseDeclGroup(ParsingDeclSpec &DS,
  ...
  if (AllowFunctionDefinitions && D.isFunctionDeclarator() &&
    ...
    if (isStartOfFunctionDefinition(D)) {
	  ...
	  // This is a patch, since deep inside the call stack of ParseFunctionDefinition in Sema::GetTypeForDeclarator D->getAttributes() is called, but it returns NULL. 
	  D.setAttributes(DS.getAttributes());
      Decl *TheDecl = ParseFunctionDefinition(D);
	  // Set D's attributes back to NULL, otherwise DS falls on assert in its destructor.
	  D.setAttributes(NULL);
      return Actions.ConvertDeclToDeclGroup(TheDecl);
    }
...
Decl *Parser::ParseDeclarationAfterDeclarator(Declarator &D,
  ...
  if (Tok.is(tok::equal)) {
    ...
    if (ParseExpressionList(Exprs, CommaLocs)) {
      ...
      LLVM_ASSERT(!Exprs.empty() && Exprs.size()-1 == CommaLocs.size() &&
...
bool Parser::ParseImplicitInt(DeclSpec &DS, CXXScopeSpec *SS,
                              const ParsedTemplateInfo &TemplateInfo,
                              AccessSpecifier AS) {
  LLVM_ASSERT(Tok.is(tok::identifier) && "should have identifier");
  ...
  LLVM_ASSERT(!DS.hasTypeSpecifier() && "Type specifier checked above");
  ...
  while (1) {
    ...
    case tok::annot_cxxscope: {
      ...
      if (Next.is(tok::annot_template_id) &&
        ...
        LLVM_ASSERT(Tok.is(tok::annot_template_id) &&
    ...
    case tok::kw___kernel:
      {
        DS.AddAttributes(new AttributeList(PP.getIdentifierInfo("opencl_kernel_function"), Loc, 0, Loc, 0,
                                   SourceLocation(), 0, 0, NULL));
	  }
      break;
    ...
    if (isInvalid) {
      LLVM_ASSERT(PrevSpec && "Method did not return previous specifier!");
      LLVM_ASSERT(DiagID);
...
void Parser::ParseDirectDeclarator(Declarator &D) {
  ...
  if (getLang().CPlusPlus && D.mayHaveIdentifier()) {
    ...
    LLVM_ASSERT(Tok.getIdentifierInfo() && "Not an identifier?");
 ...
 PastIdentifier:
  LLVM_ASSERT(D.isPastIdentifier() &&
...
void Parser::ParseParenDeclarator(Declarator &D) {
  ...
  LLVM_ASSERT(!D.isPastIdentifier() && "Should be called before passing identifier");
...
void Parser::ParseFunctionDeclarator(SourceLocation LParenLoc, Declarator &D,
  ...
  LLVM_ASSERT(D.isPastIdentifier() && "Should not call before identifier!");
    ...
    if (getLang().CPlusPlus) {
      ...
      if (Tok.is(tok::kw_throw)) {
        ...
        ParseExceptionSpecification(EndLoc, Exceptions, ExceptionRanges,
                                    hasAnyExceptionSpec);
        LLVM_ASSERT(Exceptions.size() == ExceptionRanges.size() &&
...
void Parser::ParseTypeofSpecifier(DeclSpec &DS) {
  LLVM_ASSERT(Tok.is(tok::kw_typeof) && "Not a typeof specifier");
  Token OpTok = Tok;
