...
Decl *Parser::ParseCXXInlineMethodDef(AccessSpecifier AS, Declarator &D,
  ...
  LLVM_ASSERT(D.getTypeObject(0).Kind == DeclaratorChunk::Function &&
         "This isn't a function declarator!");
  LLVM_ASSERT((Tok.is(tok::l_brace) || Tok.is(tok::colon) || Tok.is(tok::kw_try)) &&
  ...
  for (; !Class.MethodDecls.empty(); Class.MethodDecls.pop_front()) {
    ...
    for (unsigned I = 0, N = LM.DefaultArgs.size(); I != N; ++I) {
      ...
      if (CachedTokens *Toks = LM.DefaultArgs[I].Toks) {
        ...
        // Consume the '='.
        LLVM_ASSERT(Tok.is(tok::equal) && "Default argument not starting with '='");
        ...
        LLVM_ASSERT(!PP.getSourceManager().isBeforeInTranslationUnit(origLoc,
                                                           Tok.getLocation()) &&
...
void Parser::ParseLexedMethodDefs(ParsingClass &Class) {
  ...
  for (; !Class.MethodDefs.empty(); Class.MethodDefs.pop_front()) {
    ...
    LLVM_ASSERT(!LM.Toks.empty() && "Empty body!");
