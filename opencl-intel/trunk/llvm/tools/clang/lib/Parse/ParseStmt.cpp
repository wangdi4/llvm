...
StmtResult Parser::ParseLabeledStatement(AttributeList *Attr) {
  LLVM_ASSERT(Tok.is(tok::identifier) && Tok.getIdentifierInfo() &&
         "Not an identifier!");
  ...
  LLVM_ASSERT(Tok.is(tok::colon) && "Not a label!");
...
StmtResult Parser::ParseCaseStatement(AttributeList *Attr) {
  LLVM_ASSERT(Tok.is(tok::kw_case) && "Not a case stmt!");
  // FIXME: Use attributes?
  delete Attr;
  ...
  LLVM_ASSERT(!TopLevelCase.isInvalid() && "Should have parsed at least one case!");
...
StmtResult Parser::ParseDefaultStatement(AttributeList *Attr) {
  ...
  LLVM_ASSERT(Tok.is(tok::kw_default) && "Not a default stmt!");
...
StmtResult Parser::ParseCompoundStatement(AttributeList *Attr,
  ...
  LLVM_ASSERT(Tok.is(tok::l_brace) && "Not a compount stmt!");
...
StmtResult Parser::ParseIfStatement(AttributeList *Attr) {
  ...
  LLVM_ASSERT(Tok.is(tok::kw_if) && "Not an if stmt!");
...
StmtResult Parser::ParseSwitchStatement(AttributeList *Attr) {
  ...
  LLVM_ASSERT(Tok.is(tok::kw_switch) && "Not a switch stmt!");
...
StmtResult Parser::ParseWhileStatement(AttributeList *Attr) {
  ...
  LLVM_ASSERT(Tok.is(tok::kw_while) && "Not a while stmt!");
...
StmtResult Parser::ParseDoStatement(AttributeList *Attr) {
  ...
  LLVM_ASSERT(Tok.is(tok::kw_do) && "Not a do stmt!");
...
StmtResult Parser::ParseForStatement(AttributeList *Attr) {
  ...
  LLVM_ASSERT(Tok.is(tok::kw_for) && "Not a for stmt!");
  ...
  if (!ForEach) {
    LLVM_ASSERT(!SecondPart.get() && "Shouldn't have a second expression yet.");
...
StmtResult Parser::ParseGotoStatement(AttributeList *Attr) {
  ...
  LLVM_ASSERT(Tok.is(tok::kw_goto) && "Not a goto stmt!");
...
StmtResult Parser::ParseReturnStatement(AttributeList *Attr) {
  ...
  LLVM_ASSERT(Tok.is(tok::kw_return) && "Not a return stmt!");
...
StmtResult Parser::ParseAsmStatement(bool &msAsm) {
  LLVM_ASSERT(Tok.is(tok::kw_asm) && "Not an asm stmt");
  ...
  LLVM_ASSERT(Names.size() == Constraints.size() &&
...
Decl *Parser::ParseFunctionStatementBody(Decl *Decl) {
  LLVM_ASSERT(Tok.is(tok::l_brace));
...
Decl *Parser::ParseFunctionTryBlock(Decl *Decl) {
  LLVM_ASSERT(Tok.is(tok::kw_try) && "Expected 'try'");
...
StmtResult Parser::ParseCXXCatchBlock() {
  LLVM_ASSERT(Tok.is(tok::kw_catch) && "Expected 'catch'");
