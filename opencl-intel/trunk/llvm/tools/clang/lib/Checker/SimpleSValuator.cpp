  ...
  while (1) {
    ...
    case nonloc::SymExprValKind: {
      ...
      if (op == BO_EQ && rhs.isZeroConstant()) {
        ...
        case BO_LAnd:
        case BO_LOr:
          LLVM_ASSERT(false && "Logical operators handled by branching logic.");
          return UnknownVal();
        case BO_Assign:
        case BO_MulAssign:
        case BO_DivAssign:
        ...
        case BO_OrAssign:
        case BO_Comma:
          LLVM_ASSERT(false && "'=' and ',' operators handled by GRExprEngine.");
          return UnknownVal();
        case BO_PtrMemD:
        case BO_PtrMemI:
          LLVM_ASSERT(false && "Pointer arithmetic not handled here.");
          return UnknownVal();
        case BO_LT:
        case BO_GT:
        case BO_LE:
        ...
        case BO_NE:
          ...
          LLVM_ASSERT(symIntExpr->getType(ValMgr.getContext()) == resultTy);
...
SVal SimpleSValuator::EvalBinOpLL(const GRState *state,
  ...
  if (lhs == rhs) {
    switch (op) {
    default:
      LLVM_ASSERT(false && "Unimplemented operation for two identical values");
      return UnknownVal();
    case BO_Sub:
  ...
  switch (lhs.getSubKind()) {
  default:
    LLVM_ASSERT(false && "Ordering not implemented for this Loc.");
    return UnknownVal();
  ...
  case loc::ConcreteIntKind: {
    ...
    LLVM_ASSERT(isa<loc::MemRegionVal>(rhs) || isa<loc::GotoLabel>(rhs));
  ...
  case loc::MemRegionKind: {
    ...
    LLVM_ASSERT(LeftMR && "MemRegionKind SVal doesn't have a region!");
    ...
    if (const FieldRegion *LeftFR = dyn_cast<FieldRegion>(LeftMR)) {
      ...
      LLVM_ASSERT(false && "Fields not found in parent record's definition");
    }
