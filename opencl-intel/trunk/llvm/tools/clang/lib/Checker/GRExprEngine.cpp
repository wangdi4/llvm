  ...
  do {
    ...
    if (const ObjCMethodDecl *MD = dyn_cast<ObjCMethodDecl>(D)) {
      ...
      if (const Loc *LV = dyn_cast<Loc>(&V)) {
        ...
        LLVM_ASSERT(state && "'self' cannot be null");
      }
    }
  } while (0);
  ...
  switch (S->getStmtClass()) {
    ...
    case Stmt::StmtExprClass: {
      ...
      if (SE->getSubStmt()->body_empty()) {
        ...
        LLVM_ASSERT(SE->getType() == getContext().VoidTy
...
#ifndef NDEBUG
      LLVM_ASSERT(CalleeReturnsReferenceOrRecord(C));
#endif
...
#ifndef NDEBUG
      LLVM_ASSERT(ReceiverReturnsReferenceOrRecord(ME));
#endif
...
void GRExprEngine::ProcessSwitch(GRSwitchNodeBuilder& builder) {
  ...
  for ( ; I != EI; ++I) {
    ...
    LLVM_ASSERT(b && V1.Val.isInt() && !V1.HasSideEffects
             && "Case condition must evaluate to an integer constant.");
    b = b; // silence unused variable warning
    LLVM_ASSERT(V1.Val.getInt().getBitWidth() ==
    ...
    if (const Expr* E = Case->getRHS()) {
      ...
      LLVM_ASSERT(b && V2.Val.isInt() && !V2.HasSideEffects
             && "Case condition must evaluate to an integer constant.");
      b = b; // silence unused variable warning
    }
    else
...
void GRExprEngine::ProcessCallExit(GRCallExitNodeBuilder &B) {
  ...
  if (const CXXConstructExpr *CCE = dyn_cast<CXXConstructExpr>(CE)) {
    ...
    if (V) {
      ...
      LLVM_ASSERT(isa<nonloc::LazyCompoundVal>(ObjVal));
...
void GRExprEngine::VisitLogicalExpr(const BinaryOperator* B, ExplodedNode* Pred,
  ...
  LLVM_ASSERT(B->getOpcode() == BO_LAnd ||
         B->getOpcode() == BO_LOr);

  LLVM_ASSERT(B==CurrentStmt && Pred->getLocationContext()->getCFG()->isBlkExpr(B));

  const GRState* state = GetState(Pred);
  SVal X = state->getSVal(B);
  LLVM_ASSERT(X.isUndef());
...
void GRExprEngine::VisitCommonDeclRefExpr(const Expr *Ex, const NamedDecl *D,
  ...
  if (const VarDecl* VD = dyn_cast<VarDecl>(D)) {
    ...
    LLVM_ASSERT(!asLValue && "EnumConstantDecl does not have lvalue.");
...
void GRExprEngine::EvalStore(ExplodedNodeSet& Dst, const Expr *AssignE,
  ...
  LLVM_ASSERT(Builder && "GRStmtNodeBuilder must be defined.");
...
void GRExprEngine::VisitInitListExpr(const InitListExpr* E, ExplodedNode* Pred,
  ...
  LLVM_ASSERT(0 && "unprocessed InitListExpr type");
}
...
void GRExprEngine::VisitSizeOfAlignOfExpr(const SizeOfAlignOfExpr* Ex,
  ...
  if (Ex->getKind() == SizeOfAlignOfExpr::SizeOf) {
    if (T == getContext().VoidTy) {
      // sizeof(void) == 1 byte.
      amt = CharUnits::One();
    }
    else if (!T->isConstantSizeType()) {
      LLVM_ASSERT(T->isVariableArrayType() && "Unknown non-constant-sized type.");
  ...
  else if (Ex->getKind() == SizeOfAlignOfExpr::AlignOf) {
    // Get alignment of the type.
    amt = getContext().getTypeAlignInChars(T);
  }
  else {
    // OpenCL specific vec_step.
    if (T->isVectorType()) {
      amt = CharUnits::fromQuantity(T->getAs<clang::VectorType>()->getNumElements());
    }
    else {
...
void GRExprEngine::VisitOffsetOfExpr(const OffsetOfExpr* OOE, 
  ...
  if (OOE->Evaluate(Res, getContext()) && Res.Val.isInt()) {
    ...
    LLVM_ASSERT(IV.getBitWidth() == getContext().getTypeSize(OOE->getType()));
    LLVM_ASSERT(OOE->getType()->isIntegerType());
    LLVM_ASSERT(IV.isSigned() == OOE->getType()->isSignedIntegerType());
...
void GRExprEngine::VisitUnaryOperator(const UnaryOperator* U, 
  ...
  switch (U->getOpcode()) {
    ...
    case UO_Plus: LLVM_ASSERT(!asLValue);  // FALL-THROUGH.
    case UO_Extension: {
...
//        }
        ...
        switch (U->getOpcode()) {
          default:
            LLVM_ASSERT(false && "Invalid Opcode.");
            break;
...
void GRExprEngine::VisitBinaryOperator(const BinaryOperator* B,
  ...
  for (ExplodedNodeSet::iterator I1=Tmp1.begin(), E1=Tmp1.end(); I1!=E1; ++I1) {
    ...
    for (ExplodedNodeSet::iterator I2=CheckedSet.begin(), E2=CheckedSet.end();
      ...
      switch (Op) {
        default:
          LLVM_ASSERT(0 && "Invalid opcode for compound assignment.");
