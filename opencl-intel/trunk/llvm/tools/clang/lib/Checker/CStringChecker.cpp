...
const GRState *CStringChecker::CheckLocation(CheckerContext &C,
  ...
  LLVM_ASSERT(ER->getValueType() == C.getASTContext().CharTy &&
...
const GRState *CStringChecker::SetCStringLength(const GRState *state,
                                                const MemRegion *MR,
                                                SVal StrLen) {
  LLVM_ASSERT(!StrLen.isUndef() && "Attempt to set an undefined string length");
  if (StrLen.isUnknown())
    return state;
...
void CStringChecker::PreVisitDeclStmt(CheckerContext &C, const DeclStmt *DS) {
  ...
  for (DeclStmt::const_decl_iterator I = DS->decl_begin(), E = DS->decl_end();
    ...
    LLVM_ASSERT(StrVal.isValid() && "Initializer string is unknown or undefined");
