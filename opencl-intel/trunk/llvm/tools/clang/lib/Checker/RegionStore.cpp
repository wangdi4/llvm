...
public: // Part of public interface to class.
  ...
  Store BindDefault(Store store, const MemRegion *R, SVal V) {
    ...
    LLVM_ASSERT(!Lookup(B, R, BindingKey::Default));
    LLVM_ASSERT(!Lookup(B, R, BindingKey::Direct));
...
void InvalidateRegionsWorker::VisitBaseRegion(const MemRegion *baseR) {
  ...
  LLVM_ASSERT(SymbolManager::canSymbolicate(T) || V.isUnknown());
...
SVal RegionStoreManager::EvalBinOp(BinaryOperator::Opcode Op, Loc L, NonLoc R,
  ...
  switch (MR->getKind()) {
    ...
    case MemRegion::UnknownSpaceRegionKind:
      LLVM_ASSERT(0 && "Cannot perform pointer arithmetic on a MemSpace");
      return UnknownVal();
  }
...
SVal RegionStoreManager::Retrieve(Store store, Loc L, QualType T) {
  LLVM_ASSERT(!isa<UnknownVal>(L) && "location unknown");
  LLVM_ASSERT(!isa<UndefinedVal>(L) && "location undefined");
  ...
  if (isa<CodeTextRegion>(MR)) {
    LLVM_ASSERT(0 && "Why load from a code text region?");
    return UnknownVal();
  }
...
SVal RegionStoreManager::RetrieveStruct(Store store, const TypedRegion* R) {
  ...
  LLVM_ASSERT(T->isStructureOrClassType());
  return ValMgr.makeLazyCompoundVal(store, R);
}

SVal RegionStoreManager::RetrieveArray(Store store, const TypedRegion * R) {
  LLVM_ASSERT(isa<ConstantArrayType>(R->getValueType()));
  return ValMgr.makeLazyCompoundVal(store, R);
}
