...
bool GRCoreEngine::ExecuteWorkList(const LocationContext *L, unsigned Steps,
  ...
  while (Steps && WList->hasWork()) {
    ...
    switch (Node->getLocation().getKind()) {
      ...
      default:
        LLVM_ASSERT(isa<PostStmt>(Node->getLocation()));
...
void GRCoreEngine::HandleBlockExit(const CFGBlock * B, ExplodedNode* Pred) {
  ...
  if (const Stmt* Term = B->getTerminator()) {
    switch (Term->getStmtClass()) {
      default:
        LLVM_ASSERT(false && "Analysis for this terminator not implemented.");
        break;
...
static ProgramPoint GetProgramPoint(const Stmt *S, ProgramPoint::Kind K,
  ...
  switch (K) {
    default:
      LLVM_ASSERT(false && "Unhandled ProgramPoint kind");    
...
void GRCallEnterNodeBuilder::GenerateNode(const GRState *state,
  ...
  LLVM_ASSERT(Entry->empty());
  LLVM_ASSERT(Entry->succ_size() == 1);
