...
RetainSummary* RetainSummaryManager::getSummary(const FunctionDecl* FD) {
  ...
  do {
    ...
    //  filters.
    LLVM_ASSERT(ScratchArgs.isEmpty());
...
RetainSummaryManager::getClassMethodSummary(Selector S, IdentifierInfo *ClsName,
  ...
  LLVM_ASSERT(ClsName && "Class name must be specified.");
...
PathDiagnosticPiece* CFRefReport::VisitNode(const ExplodedNode* N,
  ...
  if (!PrevT) {
    ...
    if (CurrV.isOwned()) {
      ...
      if (static_cast<CFRefBug&>(getBugType()).getTF().isGCEnabled()) {
        LLVM_ASSERT(CurrV.getObjKind() == RetEffect::CF);
        os << "  "
  ...
  do {
    ...
    if (!TF.isGCEnabled() && contains(AEffects, Dealloc)) {
      ...
      LLVM_ASSERT(!(PrevV == CurrV) && "The typestate *must* have changed.");
      // We may not have transitioned to 'release' if we hit an error.
      // This case is handled elsewhere.
      if (CurrV.getKind() == RefVal::Released) {
        LLVM_ASSERT(CurrV.getCombinedCounts() == 0);
      ...
      switch (CurrV.getKind()) {
        ...
        case RefVal::NotOwned:
          ...
          if (PrevV.getCount() == CurrV.getCount()) {
            ...
            LLVM_ASSERT(PrevV.getAutoreleaseCount() < CurrV.getAutoreleaseCount());
          ...
          if (PrevV.getKind() == RefVal::Released) {
            LLVM_ASSERT(TF.isGCEnabled() && CurrV.getCount() > 0);
  ...
  switch (RE.getKind()) {
    ...
    case RetEffect::OwnedSymbol: {
      ...
      if (RE.getKind() == RetEffect::OwnedAllocatedSymbol) {
        ...
        LLVM_ASSERT(isFeasible && "Cannot assume fresh symbol is non-null.");
      }
#endif
...
void CFRefCount::EvalObjCMessageExpr(ExplodedNodeSet& Dst,
  ...
  LLVM_ASSERT(Summ && "RetainSummary is null");
...
const GRState * CFRefCount::Update(const GRState * state, SymbolRef sym,
  ...
  switch (E) {
    ...
    case Dealloc:
      ...
      switch (V.getKind()) {
        default:
          LLVM_ASSERT(false && "Invalid case.");
        case RefVal::Owned:
...
CFRefCount::HandleAutoreleaseCounts(const GRState * state, GenericNodeBuilder Bd,
  ...
  LLVM_ASSERT(!isGCEnabled() && "Autorelease counts in GC mode?");
  ...
  if (N) {
    for (llvm::SmallVectorImpl<SymbolRef>::iterator
      ...
      CFRefBug *BT = static_cast<CFRefBug*>(Pred ? leakWithinFunction
                                                 : leakAtReturn);
      LLVM_ASSERT(BT && "BugType not initialized.");
...
void CFRefCount::ProcessNonLeakError(ExplodedNodeSet& Dst,
  ...
  switch (hasErr) {
    default:
      LLVM_ASSERT(false && "Unhandled error.");
      return;
...
void CFRefCount::RegisterChecks(GRExprEngine& Eng) {
  ...
  if (isGCEnabled())
    ...
    LLVM_ASSERT(getLangOptions().getGCMode() == LangOptions::NonGC);
