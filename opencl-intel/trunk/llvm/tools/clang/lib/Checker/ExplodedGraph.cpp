...
void ExplodedNode::NodeGroup::addNode(ExplodedNode* N, ExplodedGraph &G) {
  LLVM_ASSERT((reinterpret_cast<uintptr_t>(N) & Mask) == 0x0);
  LLVM_ASSERT(!getFlag());
  ...
  if (getKind() == Size1) {
    if (ExplodedNode* NOld = getNode()) {
      ...
      LLVM_ASSERT((reinterpret_cast<uintptr_t>(V) & Mask) == 0x0);
      V->push_back(NOld, Ctx);
      V->push_back(N, Ctx);
      P = reinterpret_cast<uintptr_t>(V) | SizeOther;
      LLVM_ASSERT(getPtr() == (void*) V);
      LLVM_ASSERT(getKind() == SizeOther);
    }
    else {
      P = reinterpret_cast<uintptr_t>(N);
      LLVM_ASSERT(getKind() == Size1);
    }
  }
  else {
    LLVM_ASSERT(getKind() == SizeOther);
