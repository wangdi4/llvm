...
PathDiagnosticBuilder::getEnclosingStmtLocation(const Stmt *S) {
  LLVM_ASSERT(S && "Null Stmt* passed to getEnclosingStmtLocation");
  ...
  LLVM_ASSERT(S && "Cannot have null Stmt for PathDiagnosticLocation");
      ...
      switch (T->getStmtClass()) {
        ...
        case Stmt::BinaryOperatorClass: {
          ...
          if (B->getOpcode() == BO_LAnd) {
            ...
            LLVM_ASSERT(B->getOpcode() == BO_LOr);
            os << "||" << "' is ";
...
bool EdgeBuilder::containsLocation(const PathDiagnosticLocation &Container,
  ...
  LLVM_ASSERT(ContainerBegLine <= ContainerEndLine);
  LLVM_ASSERT(ContaineeBegLine <= ContaineeEndLine);
...
MakeReportGraph(const ExplodedGraph* G,
  ...
  LLVM_ASSERT(!WS.empty() && "No error node found in the trimmed graph.");
  ...
  for ( const ExplodedNode *N = Root ;;) {
    ...
    LLVM_ASSERT(IMitr != InverseMap.end() && "No mapping to original node.");
  ...
  for (PathDiagnostic::iterator I = PD.begin(), E = PD.end(); I!=E; ++I) {
    ...
    if (!MacroGroup || ParentInstantiationLoc == MacroStack.back().second) {
      ...
      if (MacroGroup)
        ...
        LLVM_ASSERT(InstantiationLoc.isFileID());
...
void GRBugReporter::GeneratePathDiagnostic(PathDiagnostic& PD,
  ...
  LLVM_ASSERT(R && "No original report found for sliced graph.");
...
void BugReporter::FlushReport(BugReportEquivClass& EQ) {
  ...
  switch (End-Beg) {
    default: LLVM_ASSERT(0 && "Don't handle this many ranges yet!");
