...
SVal::symbol_iterator& SVal::symbol_iterator::operator++() {
  LLVM_ASSERT(!itr.empty() && "attempting to iterate on an 'end' iterator");
  LLVM_ASSERT(isa<SymbolData>(itr.back()));
  itr.pop_back();
  if (!itr.empty())
    while (!isa<SymbolData>(itr.back())) expand();
  return *this;
...
SymbolRef SVal::symbol_iterator::operator*() {
  LLVM_ASSERT(!itr.empty() && "attempting to dereference an 'end' iterator");
...
void SVal::symbol_iterator::expand() {
  ...
  LLVM_ASSERT(false && "unhandled expansion case");
}
...
void Loc::dumpToStream(llvm::raw_ostream& os) const {
  switch (getSubKind()) {
    ...
    case loc::MemRegionKind:
      ...
      break;
    default:
      LLVM_ASSERT(false && "Pretty-printing not implemented for this Loc.");
      break;
  }
}
