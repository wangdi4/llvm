...
#define OPENCLEXT(NAME) supportedOclExtensions[ OpenCL::NAME ] = false;
...
const char *TargetInfo::getTypeName(IntType T) {
  switch (T) {
  default: LLVM_ASSERT(0 && "not an integer!");
...
TargetInfo::getNormalizedGCCRegisterName(llvm::StringRef Name) const {
  LLVM_ASSERT(isValidGCCRegisterName(Name) && "Invalid register passed in");
  ...
  if (isdigit(Name[0])) {
    int n;
    if (!Name.getAsInteger(0, n)) {
      LLVM_ASSERT(n >= 0 && (unsigned)n < NumNames &&
...
bool TargetInfo::resolveSymbolicName(const char *&Name,
  ...
  LLVM_ASSERT(*Name == '[' && "Symbolic name did not start with '['");
  Name++;
