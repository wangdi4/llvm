...
void ContentCache::replaceBuffer(const llvm::MemoryBuffer *B,
                                 bool DoNotFree) {
  LLVM_ASSERT(B != Buffer.getPointer());
...
void LineTableInfo::AddLineNote(unsigned FID, unsigned Offset,
  ...
  LLVM_ASSERT((Entries.empty() || Entries.back().FileOffset < Offset) &&
...
void LineTableInfo::AddLineNote(unsigned FID, unsigned Offset,
  ...
  LLVM_ASSERT(FilenameID != -1 && "Unspecified filename should use other accessor");
  ...
  if (EntryExit == 0) {  // No #include stack change.
    ...
    LLVM_ASSERT(!Entries.empty() && Entries.back().IncludeOffset &&
...
const LineEntry *LineTableInfo::FindNearestLineEntry(unsigned FID,
  ...
  LLVM_ASSERT(!Entries.empty() && "No #line entries for this FID after all!");
...
void SourceManager::AddLineNote(SourceLocation Loc, unsigned LineNo,
  ...
  if (FilenameID == -1) {
    LLVM_ASSERT(!IsFileEntry && !IsFileExit && !IsSystemHeader && !IsExternCHeader &&
...
SourceManager::getOrCreateContentCache(const FileEntry *FileEnt) {
  LLVM_ASSERT(FileEnt && "Didn't specify a file entry to use?");
...
FileID SourceManager::createFileID(const ContentCache *File,
  ...
  if (PreallocatedID) {
    ...
    // entry and return.
    LLVM_ASSERT(PreallocatedID < SLocEntryLoaded.size() &&
           "Preallocate ID out-of-range");
    LLVM_ASSERT(!SLocEntryLoaded[PreallocatedID] &&
           "Source location entry already loaded");
    LLVM_ASSERT(Offset && "Preallocate source location cannot have zero offset");
  ...
  LLVM_ASSERT(NextOffset+FileSize+1 > NextOffset && "Ran out of source locations!");
...
SourceLocation SourceManager::createInstantiationLoc(SourceLocation SpellingLoc,
  ...
  LLVM_ASSERT(NextOffset+TokLength+1 > NextOffset && "Ran out of source locations!");
...
SourceManager::getMemoryBufferForFile(const FileEntry *File,
  ...
  LLVM_ASSERT(IR && "getOrCreateContentCache() cannot return NULL");
...
FileID SourceManager::getFileIDSlow(unsigned SLocOffset) const {
  LLVM_ASSERT(SLocOffset && "Invalid FileID");
...
SourceManager::getImmediateInstantiationRange(SourceLocation Loc) const {
  LLVM_ASSERT(Loc.isMacroID() && "Not an instantiation loc!");
...
SourceManager::getFileCharacteristic(SourceLocation Loc) const {
  LLVM_ASSERT(!Loc.isInvalid() && "Can't get file characteristic of invalid loc!");
  std::pair<FileID, unsigned> LocInfo = getDecomposedInstantiationLoc(Loc);
  ...
  LLVM_ASSERT(LineTable && "Can't have linetable entries without a LineTable!");
...
SourceLocation SourceManager::getLocation(const FileEntry *SourceFile,
  ...
  LLVM_ASSERT(SourceFile && "Null source file!");
  LLVM_ASSERT(Line && Col && "Line and column should start from 1!");
...
bool SourceManager::isBeforeInTranslationUnit(SourceLocation LHS,
  ...
  LLVM_ASSERT(LHS.isValid() && RHS.isValid() && "Passed invalid source location!");
  if (LHS == RHS)
    return false;
