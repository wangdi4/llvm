...
static void DefineStd(MacroBuilder &Builder, llvm::StringRef MacroName,
  ...
  LLVM_ASSERT(MacroName[0] != '_' && "Identifier should be in the user's namespace");
...
static void getDarwinDefines(MacroBuilder &Builder, const LangOptions &Opts,
  ...
  if (Triple.getEnvironmentName() == "iphoneos") {
    LLVM_ASSERT(Maj < 10 && Min < 99 && Rev < 99 && "Invalid version!");
    char Str[6];
    Str[0] = '0' + Maj;
    Str[1] = '0' + (Min / 10);
    Str[2] = '0' + (Min % 10);
    ...
    LLVM_ASSERT(Triple.getEnvironmentName().empty() && "Invalid environment!");
    LLVM_ASSERT(Maj < 99 && Min < 10 && Rev < 10 && "Invalid version!");
    char Str[5];
    Str[0] = '0' + (Maj / 10);
    Str[1] = '0' + (Maj % 10);
    Str[2] = '0' + Min;
...
public:
  X86TargetInfo(const std::string& triple)
    : TargetInfo(triple), SSELevel(NoMMXSSE), AMD3DNowLevel(NoAMD3DNow),
      HasAES(false), HasAVX(false), HasAVX2(false), IsKNF(false), IsMIC(false) {
...
void X86TargetInfo::getDefaultFeatures(const std::string &CPU,
  ...
  // X86_64 always has SSE2 except KNF and MIC.
  if ((PointerWidth == 64) && (CPU != "knf") && (CPU != "mic"))
  ...
  if (CPU == "generic" || CPU == "i386" || CPU == "i486" || CPU == "i586" ||
    ...
    setFeatureEnabled(Features, "knf", true);
  else if (CPU == "mic")
    setFeatureEnabled(Features, "mic", true);
...
void X86TargetInfo::HandleTargetFeatures(std::vector<std::string> &Features) {
  ...
  for (unsigned i = 0, e = Features.size(); i !=e; ++i) {
    ...
    else if (Features[i].substr(1) == "avx") {
      HasAVX = true;
      continue;
    }
    // TODO: when we'll have more than two MIC architectures,
    // we should define something like "MICLevel", like in SSELevel.
    if (Features[i].substr(1) == "knf") {
      IsKNF = true;
      continue;
    }
    if (Features[i].substr(1) == "mic") {
      IsMIC = true;
      continue;
    }


    LLVM_ASSERT(Features[i][0] == '+' && "Invalid target feature!");
...
void X86TargetInfo::getTargetDefines(const LangOptions &Opts,
  ...
  if (IsKNF) Builder.defineMacro("__KNF__");
  if (IsMIC) Builder.defineMacro("__MIC__");
...
public:
  X86_32TargetInfo(const std::string& triple) : X86TargetInfo(triple) {
    LongWidth = LongAlign = 64; // OCL tweak
...
TargetInfo *TargetInfo::CreateTargetInfo(Diagnostic &Diags,
  ...
  for (std::vector<std::string>::const_iterator it = Opts.Features.begin(),
    ...
    if (1 == (it->find_first_of("cl_"))){
      std::string extName = it->substr(1, std::string::npos);
      if (Name[0] == '+')
        if (false == Target->setOpenCLExtensionStatus(extName, true))
          Diags.Report(diag::err_target_invalid_feature) << extName;

      if (Name[0] == '-')
        if (false == Target->setOpenCLExtensionStatus(extName, false))
          Diags.Report(diag::err_target_invalid_feature) << extName;
