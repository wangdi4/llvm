...
static const StaticDiagInfoRec *GetDiagInfo(unsigned DiagID) {
  ...
  if (IsFirst) {
    for (unsigned i = 1; i != NumDiagEntries; ++i) {
      LLVM_ASSERT(StaticDiagInfo[i-1].DiagID != StaticDiagInfo[i].DiagID &&
             "Diag ID conflict, the enums at the start of clang::diag (in "
             "Diagnostic.h) probably need to be increased");

      LLVM_ASSERT(StaticDiagInfo[i-1] < StaticDiagInfo[i] &&
  ...
  namespace diag {
    ...
    public:
      ...
      const char *getDescription(unsigned DiagID) const {
        LLVM_ASSERT(this && DiagID-DIAG_UPPER_LIMIT < DiagInfo.size() &&
...
Diagnostic::Level Diagnostic::getDiagnosticLevel(unsigned DiagID) const {
  ...
  LLVM_ASSERT(DiagClass != CLASS_NOTE && "Cannot get diagnostic level of a note!");
...
Diagnostic::getDiagnosticLevel(unsigned DiagID, unsigned DiagClass) const {
  ...
  default: LLVM_ASSERT(0 && "Unknown mapping!");
...
static void HandleSelectModifier(const DiagnosticInfo &DInfo, unsigned ValNo,
  ...
  while (ValNo) {
    ...
    LLVM_ASSERT(NextVal != ArgumentEnd && "Value for integer select modifier was"
...
static void HandleOrdinalModifier(unsigned ValNo,
                                  llvm::SmallVectorImpl<char> &OutStr) {
  LLVM_ASSERT(ValNo != 0 && "ValNo must be strictly positive!");
...
static bool TestPluralRange(unsigned Val, const char *&Start, const char *End) {
  ...
  LLVM_ASSERT(*Start == ',' && "Bad plural expression syntax: expected ,");
  ++Start;
...
static void HandlePluralModifier(unsigned ValNo,
  ...
  const char *ArgumentEnd = Argument + ArgumentLen;
  while (1) {
    LLVM_ASSERT(Argument < ArgumentEnd && "Plural expression didn't match.");
    const char *ExprEnd = Argument;
    while (*ExprEnd != ':') {
      LLVM_ASSERT(ExprEnd != ArgumentEnd && "Plural missing expression end");
      ++ExprEnd;
    }
  ...
  while (DiagStr != DiagEnd) {
    ...
    if (!isdigit(DiagStr[0])) {
      ...
      if (DiagStr[0] == '{') {
        ...
        LLVM_ASSERT(DiagStr != DiagEnd && "Mismatched {}'s in diagnostic string!");
        ArgumentLen = DiagStr-Argument;
        ++DiagStr;  // Skip }.
      }
    }

    LLVM_ASSERT(isdigit(*DiagStr) && "Invalid format for argument in diagnostic");
    ...
    case Diagnostic::ak_std_string: {
      ...
      LLVM_ASSERT(ModifierLen == 0 && "No modifiers for strings yet");
    ...
    case Diagnostic::ak_sint: {
      ...
      if (ModifierIs(Modifier, ModifierLen, "select")) {
        ...
        LLVM_ASSERT(ModifierLen == 0 && "Unknown integer modifier");
...
PartialDiagnostic::StorageAllocator::~StorageAllocator() {
  LLVM_ASSERT(NumFreeListEntries == NumCached && "A partial is on the lamb");
}
