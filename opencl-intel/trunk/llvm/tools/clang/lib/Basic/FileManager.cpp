...
namespace {
  static std::string GetFullPath(const char *relPath) {
    ...
    LLVM_ASSERT(absPathStrPtr && "_fullpath() returned NULL!");
...
void FileManager::addStatCache(StatSysCallCache *statCache, bool AtBeginning) {
  LLVM_ASSERT(statCache && "No stat cache provided?");
...
void FileManager::removeStatCache(StatSysCallCache *statCache) {
  ...
  if (PrevCache)
    ...
    LLVM_ASSERT(false && "Stat cache not found for removal");
}
...
static void TrimString(const char* NameStart, const char*& NameEnd)
{
	// we don't trim the start of the string, since clang prefixes the source's filename with the paths of the include directories.
	while (NameStart <= NameEnd && (NameEnd[-1] == ' ' || NameEnd[-1] == '\t'))
		NameEnd--;
}
...
const FileEntry *FileManager::getFile(const char *NameStart,
  ...
  // stat in Linux doesn't trim the filename by itself
  TrimString(NameStart, NameEnd);
  // See if there is already an entry in the map.
