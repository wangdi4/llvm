...
const Builtin::Info &Builtin::Context::GetRecord(unsigned ID) const {
  ...
  LLVM_ASSERT(ID - Builtin::FirstTSBuiltin < NumTSRecords && "Invalid builtin ID!");
...
void Builtin::Context::InitializeBuiltins(IdentifierTable &Table,
  ...
  for (unsigned i = Builtin::NotBuiltin+1; i != Builtin::FirstTSBuiltin; ++i)
        ...
        (!NoBuiltins || !strchr(BuiltinInfo[i].Attributes, 'f')))
    {
      if (OpenCL)
      {
        //OpenCL doesn't allow the use of C99 standard library functions
        //from these headers
        if (BuiltinInfo[i].HeaderName != NULL)
        {
          continue;
        }
      }
...
Builtin::Context::isPrintfLike(unsigned ID, unsigned &FormatIdx,
  ...
  ++Printf;
  LLVM_ASSERT(*Printf == ':' && "p or P specifier must have be followed by a ':'");
  ++Printf;

  LLVM_ASSERT(strchr(Printf, ':') && "printf specifier must end with a ':'");
...
Builtin::Context::isScanfLike(unsigned ID, unsigned &FormatIdx,
  ...
  ++Scanf;
  LLVM_ASSERT(*Scanf == ':' && "s or S specifier must have be followed by a ':'");
  ++Scanf;

  LLVM_ASSERT(strchr(Scanf, ':') && "printf specifier must end with a ':'");
