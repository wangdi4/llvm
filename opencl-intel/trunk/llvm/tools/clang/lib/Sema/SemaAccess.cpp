...
struct AccessTarget : public AccessedEntity {
  ...
  const CXXRecordDecl *resolveInstanceContext(Sema &S) const {
    LLVM_ASSERT(HasInstanceContext);
...
static AccessResult IsDerivedFromInclusive(const CXXRecordDecl *Derived,
  ...
  LLVM_ASSERT(Derived->getCanonicalDecl() == Derived);
  LLVM_ASSERT(Target->getCanonicalDecl() == Target);
  ...
  while (true) {
    for (CXXRecordDecl::base_class_const_iterator
      ...
      if (const RecordType *RT = T->getAs<RecordType>()) {
        ...
        LLVM_ASSERT(T->isDependentType() && "non-dependent base wasn't a record?");
...
static bool MightInstantiateTo(Sema &S, DeclContext *Context,
  ...
  LLVM_ASSERT(!Friend->isDependentContext() &&
...
static AccessResult MatchesFriend(Sema &S,
  ...
  LLVM_ASSERT(isa<FunctionDecl>(Friend) && "unknown friend decl kind");
...
static AccessResult GetProtectedFriendKind(Sema &S, const EffectiveContext &EC,
                                           const CXXRecordDecl *InstanceContext,
                                           const CXXRecordDecl *NamingClass) {
  LLVM_ASSERT(InstanceContext->getCanonicalDecl() == InstanceContext);
  LLVM_ASSERT(NamingClass->getCanonicalDecl() == NamingClass);
...
static AccessResult HasAccess(Sema &S,
  ...
  LLVM_ASSERT(Access == AS_private || Access == AS_protected);
  ...
  for (EffectiveContext::record_iterator
    ...
    // [B3] and [M3]
      ...
      LLVM_ASSERT(Access == AS_protected);
...
static CXXBasePath *FindBestPath(Sema &S,
  ...
  bool isDerived = Derived->isDerivedFrom(const_cast<CXXRecordDecl*>(Base),
                                          Paths);
  LLVM_ASSERT(isDerived && "derived class not actually derived from base");
  (void) isDerived;

  CXXBasePath *BestPath = 0;

  LLVM_ASSERT(FinalAccess != AS_none && "forbidden access after declaring class");
  ...
  for (CXXBasePaths::paths_iterator PI = Paths.begin(), PE = Paths.end();
    ...
    while (I != E) {
      ...
      LLVM_ASSERT(PathAccess != AS_none);
  ...
  LLVM_ASSERT((!BestPath || BestPath->Access != AS_public) &&
...
static bool TryDiagnoseProtectedAccess(Sema &S, const EffectiveContext &EC,
  ...
  if (!Target.hasInstanceContext())
    return false;
  LLVM_ASSERT(Target.isMemberAccess());
  ...
  for (EffectiveContext::record_iterator
    ...
    const CXXRecordDecl *InstanceContext = Target.resolveInstanceContext(S);
    LLVM_ASSERT(InstanceContext && "diagnosing dependent access");
...
static AccessResult IsAccessible(Sema &S,
  ...
  LLVM_ASSERT(UnprivilegedAccess != AS_public && "public access not weeded out");
  ...
  LLVM_ASSERT(Entity.getDeclaringClass() != NamingClass);
  ...
  LLVM_ASSERT(Path->Access <= UnprivilegedAccess &&
...
static void DelayDependentAccess(Sema &S,
  ...
  LLVM_ASSERT(EC.isDependent() && "delaying non-dependent access");
  DeclContext *DC = EC.getInnerContext();
  LLVM_ASSERT(DC->isDependentContext() && "delaying non-dependent access");
...
static AccessResult CheckEffectiveAccess(Sema &S,
                                         ...
                                         SourceLocation Loc,
                                         AccessTarget &Entity) {
  LLVM_ASSERT(Entity.getAccess() != AS_public && "called for public access!");
...
Sema::AccessResult Sema::CheckMemberOperatorAccess(SourceLocation OpLoc,
  ...
  LLVM_ASSERT(RT && "found member operator but object expr not of record type");
...
void Sema::CheckLookupAccess(const LookupResult &R) {
  LLVM_ASSERT(getLangOptions().AccessControl
         && "performing access check without access control");
  LLVM_ASSERT(R.getNamingClass() && "performing access check without naming class");
...
void Sema::ActOnStartSuppressingAccessChecks() {
  LLVM_ASSERT(!SuppressAccessChecking &&
