...
Sema::BuildCXXNamedCast(SourceLocation OpLoc, tok::TokenKind Kind,
  ...
  switch (Kind) {
  default: LLVM_ASSERT(0 && "Unknown C++ cast!");
...
CastsAwayConstness(Sema &Self, QualType SrcType, QualType DestType) {
  ...
  // in C++ 5.2.11p8.
  LLVM_ASSERT((SrcType->isAnyPointerType() || SrcType->isMemberPointerType() ||
          SrcType->isBlockPointerType()) &&
         "Source type is not pointer or pointer to member.");
  LLVM_ASSERT((DestType->isAnyPointerType() || DestType->isMemberPointerType() ||
...
CheckDynamicCast(Sema &Self, Expr *&SrcExpr, QualType DestType,
  ...
  if (DestPointee->isVoidType()) {
    LLVM_ASSERT(DestPointer && "Reference to void is not possible");
  } else if (DestRecord) {
  ...
  LLVM_ASSERT((DestPointer || DestReference) &&
    "Bad destination non-ptr/ref slipped through.");
  LLVM_ASSERT((DestRecord || DestPointee->isVoidType()) &&
    "Bad destination pointee slipped through.");
  LLVM_ASSERT(SrcRecord && "Bad source pointee slipped through.");
  ...
  LLVM_ASSERT(SrcDecl && "Definition missing");
...
static TryCastResult TryReinterpretCast(Sema &Self, Expr *SrcExpr,
  ...
  if (DestType->isIntegralType(Self.Context)) {
    LLVM_ASSERT(srcIsPtr && "One type must be a pointer");
  ...
  if (SrcType->isIntegralOrEnumerationType()) {
    LLVM_ASSERT(destIsPtr && "One type must be a pointer");
