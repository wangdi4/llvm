...
DeclContext *Sema::computeDeclContext(const CXXScopeSpec &SS,
  ...
  case NestedNameSpecifier::Identifier:
    LLVM_ASSERT(false && "Dependent nested-name-specifier has no DeclContext");
    break;
  ...
  case NestedNameSpecifier::TypeSpecWithTemplate: {
    ...
    LLVM_ASSERT(Tag && "Non-tag type in nested-name-specifier");
    return Tag->getDecl();
  } break;
...
CXXRecordDecl *Sema::getCurrentInstantiationOf(NestedNameSpecifier *NNS) {
  LLVM_ASSERT(getLangOptions().CPlusPlus && "Only callable in C++");
  LLVM_ASSERT(NNS->isDependent() && "Only dependent nested-name-specifier allowed");
...
bool Sema::RequireCompleteDeclContext(CXXScopeSpec &SS,
                                      DeclContext *DC) {
  LLVM_ASSERT(DC != 0 && "given null context");
...
NamedDecl *Sema::FindFirstQualifierInScope(Scope *S, NestedNameSpecifier *NNS) {
  ...
  LookupName(Found, S);
  LLVM_ASSERT(!Found.isAmbiguous() && "Cannot handle ambiguities here yet");
...
bool Sema::ShouldEnterDeclaratorScope(Scope *S, const CXXScopeSpec &SS) {
  LLVM_ASSERT(SS.isSet() && "Parser passed invalid CXXScopeSpec.");
...
void Sema::ActOnCXXExitDeclaratorScope(Scope *S, const CXXScopeSpec &SS) {
  ...
  if (SS.isInvalid())
    return;
  LLVM_ASSERT(!SS.isInvalid() && computeDeclContext(SS, true) &&
