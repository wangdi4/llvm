...
InitListChecker::FillInValueInitializations(const InitializedEntity &Entity,
                                            InitListExpr *ILE,
                                            bool &RequiresSecondPass) {
  LLVM_ASSERT((ILE->getType() != SemaRef.Context.VoidTy) &&
...
void InitListChecker::CheckImplicitInitList(const InitializedEntity &Entity,
  ...
  if (T->isArrayType())
    ...
    LLVM_ASSERT(0 && "CheckImplicitInitList(): Illegal type");
...
void InitListChecker::CheckExplicitInitList(const InitializedEntity &Entity,
                                            ...
                                            unsigned &StructuredIndex,
                                            bool TopLevelObject) {
  LLVM_ASSERT(IList->isExplicit() && "Illegal Implicit InitListExpr");
...
void InitListChecker::CheckListElementTypes(const InitializedEntity &Entity,
  ...
  if (DeclType->isScalarType()) {
    ...
    if (DeclType->isRecordType()) {
      ...
      LLVM_ASSERT(0 && "Aggregate that isn't a structure or array?!");
...
void InitListChecker::CheckVectorType(const InitializedEntity &Entity,
  ...
  if (Index < IList->getNumInits()) {
    ...
    // OpenCL requires all elements to be initialized, or a single scalar initializer
...
static void ExpandAnonymousFieldDesignator(Sema &SemaRef,
  ...
  LLVM_ASSERT(false && "Unable to find anonymous struct/union field");
}
...
InitListChecker::CheckDesignatedInitializer(const InitializedEntity &Entity,
  ...
  LLVM_ASSERT((IsFirstDesignator || StructuredList) &&
  ...
  LLVM_ASSERT(StructuredList && "Expected a structured initializer list");
  ...
  if (D->isArrayDesignator()) {
    ...
    LLVM_ASSERT(D->isArrayRangeDesignator() && "Need array-range designator");
...
static OverloadingResult TryRefInitWithConversionFunction(Sema &S,
  ...
  bool DerivedToBase;
  bool ObjCConversion;
  LLVM_ASSERT(!S.CompareReferenceRelationship(Initializer->getLocStart(), 
                                         T1, T2, DerivedToBase,
                                         ObjCConversion) &&
...
static void TryConstructorInitialization(Sema &S, 
  ...
  LLVM_ASSERT(DestRecordType && "Constructor initialization requires record type");  
...
static void TryDefaultInitialization(Sema &S,
                                     ...
                                     const InitializationKind &Kind,
                                     InitializationSequence &Sequence) {
  LLVM_ASSERT(Kind.getKind() == InitializationKind::IK_Default);
...
static void TryUserDefinedConversion(Sema &S, 
  ...
  QualType DestType = Entity.getType();
  LLVM_ASSERT(!DestType->isReferenceType() && "References are handled elsewhere");
  QualType SourceType = Initializer->getType();
  LLVM_ASSERT((DestType->isRecordType() || SourceType->isRecordType()) &&
...
InitializationSequence::InitializationSequence(Sema &S,
  ...
  LLVM_ASSERT(NumArgs == 1 && "Zero-argument case handled above");
  
  //    - Otherwise, if the source type is a (possibly cv-qualified) class 
  //      type, conversion functions are considered.
  if (Initializer && !SourceType.isNull() && SourceType->isRecordType()) {
    TryUserDefinedConversion(S, Entity, Kind, Initializer, *this);
    return;
  }
...
InitializationSequence::Perform(Sema &S,
  ...
  LLVM_ASSERT(!Steps.empty() && "Cannot have an empty initialization sequence");
  ...
  for (step_iterator Step = step_begin(), StepEnd = step_end();
    ...
    case SK_ConstructorInitialization: {
      ...
      if (Kind.getKind() == InitializationKind::IK_Default) {
        ...
        LLVM_ASSERT(ClassDecl && "No parent class for constructor.");
...
Sema::PerformCopyInitialization(const InitializedEntity &Entity,
  ...
  LLVM_ASSERT(InitE && "No initialization expression?");
