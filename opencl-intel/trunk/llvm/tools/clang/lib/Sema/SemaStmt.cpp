...
StmtResult Sema::ActOnExprStmt(FullExprArg expr) {
  Expr *E = expr.get();
  LLVM_ASSERT(E && "ActOnExprStmt(): missing expression");
...
void Sema::DiagnoseUnusedExprResult(const Stmt *S, bool isComma) {
  ...
  if(DiagRuntimeBehavior(Loc, PDiag(DiagID) << R1 << R2)) {
    if(getLangOptions().OpenCL && isComma) {
      // maybe it's a vector literal without the cast?
       Diag(Loc, diag::note_should_be_vec_literal);     
    }
  }
}
...
Sema::ActOnCaseStmt(SourceLocation CaseLoc, Expr *LHSVal,
  ...
  LLVM_ASSERT((LHSVal != 0) && "missing expression in case statement");
...
Sema::ActOnLabelStmt(SourceLocation IdentLoc, IdentifierInfo *II,
  ...
  LLVM_ASSERT(LabelDecl->getID() == II && "Label mismatch!");
...
Sema::ActOnFinishSwitchStmt(SourceLocation SwitchLoc, Stmt *Switch,
  ...
  LLVM_ASSERT(SS == getCurFunction()->SwitchStack.back() &&
  ...
  if (!HasDependentValue) {
    ...
    if (!HasDependentValue && !TheDefaultStmt) {
      ...
      if (HasConstantCond) {
        LLVM_ASSERT(Result.Val.isInt() && "switch condition evaluated to non-int");
        ConstantCondValue = Result.Val.getInt();
        ShouldCheckConstantCond = true;

        LLVM_ASSERT(ConstantCondValue.getBitWidth() == CondWidth &&
...
Sema::ActOnDoStmt(SourceLocation DoLoc, Stmt *Body,
  ...
  LLVM_ASSERT(Cond && "ActOnDoStmt(): missing expression");
