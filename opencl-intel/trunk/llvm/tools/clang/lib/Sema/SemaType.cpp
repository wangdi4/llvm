...
static QualType ConvertDeclSpecToType(Sema &TheSema,
  ...
  case DeclSpec::TST_char:
    if (DS.getTypeSpecSign() == DeclSpec::TSS_unspecified)
      ...
      LLVM_ASSERT(DS.getTypeSpecSign() == DeclSpec::TSS_unsigned &&
             "Unknown TSS value");
  ...
  case DeclSpec::TST_char16:
      LLVM_ASSERT(DS.getTypeSpecSign() == DeclSpec::TSS_unspecified &&
        "Unknown TSS value");
  ...
  case DeclSpec::TST_unspecified:
    ...
    if (TheSema.getLangOptions().ImplicitInt) {
      ...
      if ((TheSema.getLangOptions().CPlusPlus || TheSema.getLangOptions().OpenCL)&&
  ...
  case DeclSpec::TST_struct: {
    ...
    LLVM_ASSERT(DS.getTypeSpecWidth() == 0 && DS.getTypeSpecComplex() == 0 &&
  ...
  case DeclSpec::TST_typeofType:
    ...
    Result = TheSema.GetTypeFromParser(DS.getRepAsType());
    LLVM_ASSERT(!Result.isNull() && "Didn't get a type for typeof?");
    // TypeQuals handled by caller.
    Result = Context.getTypeOfType(Result);
    break;
  case DeclSpec::TST_typeofExpr: {
    Expr *E = DS.getRepAsExpr();
    LLVM_ASSERT(E && "Didn't get an expression for typeof?");
    // TypeQuals handled by caller.
  ...
  case DeclSpec::TST_decltype: {
    Expr *E = DS.getRepAsExpr();
    LLVM_ASSERT(E && "Didn't get an expression for decltype?");
    // TypeQuals handled by caller.
  ...
  if (DS.getTypeSpecComplex() == DeclSpec::TSC_complex) {
    ...
    LLVM_ASSERT(typeSize > 0 && "type size for vector must be greater than 0 bits");
  ...
  LLVM_ASSERT(DS.getTypeSpecComplex() != DeclSpec::TSC_imaginary &&
  ...
  if (unsigned TypeQuals = DS.getTypeQualifiers()) {
    ...
    if (Result->isFunctionType() && TypeQuals) {
      ...
      if (TypeQuals & DeclSpec::TQ_const)
        ...
        LLVM_ASSERT((TypeQuals & DeclSpec::TQ_restrict) &&
...
QualType Sema::BuildPointerType(QualType T,
  ...
  LLVM_ASSERT(!T->isObjCObjectType() && "Should build ObjCObjectPointerType");
...
TypeSourceInfo *Sema::GetTypeForDeclarator(Declarator &D, Scope *S,
  ...
  if (T == Context.UndeducedAutoTy) {
    ...
    case Declarator::KNRTypeListContext:
      LLVM_ASSERT(0 && "K&R type lists aren't allowed in C++");
      break;
    ...
    case Declarator::MemberContext:
      ...
      case TTK_Enum: LLVM_ASSERT(0 && "unhandled tag kind"); break;
  ...
  for (unsigned i = 0, e = D.getNumTypeObjects(); i != e; ++i) {
    ...
    default: LLVM_ASSERT(0 && "Unknown decltype!");
    ...
    case DeclaratorChunk::Function: {
      ...
      if (T.getCVRQualifiers() && D.getDeclSpec().getTypeQualifiers() &&
        ...
        LLVM_ASSERT(NumQuals > 0 && "No known qualifiers?");
      ...
      if (!FTI.NumArgs && !FTI.isVariadic && !getLangOptions().CPlusPlus) {
        ...
        for (unsigned i = 0, e = FTI.NumArgs; i != e; ++i) {
          ...
          LLVM_ASSERT(!ArgTy.isNull() && "Couldn't parse type?");

          // Adjust the parameter type.
          LLVM_ASSERT((ArgTy == adjustParameterType(ArgTy)) && "Unadjusted type?");
  ...
  if (getLangOptions().CPlusPlus && T->isFunctionType()) {
    ...
    LLVM_ASSERT(FnTy && "Why oh why is there not a FunctionProtoType here?");
...
namespace {
  ...
  public:
    ...
    void VisitObjCObjectTypeLoc(ObjCObjectTypeLoc TL) {
      ...
      if (DS.getProtocolQualifiers()) {
        LLVM_ASSERT(TL.getNumProtocols() > 0);
        LLVM_ASSERT(TL.getNumProtocols() == DS.getNumProtocolQualifiers());
    ...
    void VisitTypeOfExprTypeLoc(TypeOfExprTypeLoc TL) {
      LLVM_ASSERT(DS.getTypeSpecType() == DeclSpec::TST_typeofExpr);
      TL.setTypeofLoc(DS.getTypeSpecTypeLoc());
      TL.setParensRange(DS.getTypeofParensRange());
    }
    void VisitTypeOfTypeLoc(TypeOfTypeLoc TL) {
      LLVM_ASSERT(DS.getTypeSpecType() == DeclSpec::TST_typeofType);
      TL.setTypeofLoc(DS.getTypeSpecTypeLoc());
      TL.setParensRange(DS.getTypeofParensRange());
      LLVM_ASSERT(DS.getRepAsType());
      TypeSourceInfo *TInfo = 0;
      Sema::GetTypeFromParser(DS.getRepAsType(), &TInfo);
  ...
  public:
    ...
    void VisitBlockPointerTypeLoc(BlockPointerTypeLoc TL) {
      LLVM_ASSERT(Chunk.Kind == DeclaratorChunk::BlockPointer);
      TL.setCaretLoc(Chunk.Loc);
    }
    void VisitPointerTypeLoc(PointerTypeLoc TL) {
      LLVM_ASSERT(Chunk.Kind == DeclaratorChunk::Pointer);
    ...
    void VisitMemberPointerTypeLoc(MemberPointerTypeLoc TL) {
      LLVM_ASSERT(Chunk.Kind == DeclaratorChunk::MemberPointer);
      TL.setStarLoc(Chunk.Loc);
      // FIXME: nested name specifier
    }
    void VisitLValueReferenceTypeLoc(LValueReferenceTypeLoc TL) {
      LLVM_ASSERT(Chunk.Kind == DeclaratorChunk::Reference);
    ...
    void VisitRValueReferenceTypeLoc(RValueReferenceTypeLoc TL) {
      LLVM_ASSERT(Chunk.Kind == DeclaratorChunk::Reference);
      LLVM_ASSERT(!Chunk.Ref.LValueRef);
      TL.setAmpAmpLoc(Chunk.Loc);
    }
    void VisitArrayTypeLoc(ArrayTypeLoc TL) {
      LLVM_ASSERT(Chunk.Kind == DeclaratorChunk::Array);
      TL.setLBracketLoc(Chunk.Loc);
      TL.setRBracketLoc(Chunk.EndLoc);
      TL.setSizeExpr(static_cast<Expr*>(Chunk.Arr.NumElts));
    }
    void VisitFunctionTypeLoc(FunctionTypeLoc TL) {
      LLVM_ASSERT(Chunk.Kind == DeclaratorChunk::Function);
...
Sema::GetTypeSourceInfoForDeclarator(Declarator &D, QualType T,
  ...
  if (ReturnTypeInfo) {
    ...
    LLVM_ASSERT(TL.getFullDataSize() == CurrTL.getFullDataSize());
...
ParsedType Sema::CreateParsedType(QualType T, TypeSourceInfo *TInfo) {
  ...
  LLVM_ASSERT(LocT->getTypeClass() != T->getTypeClass() &&
...
void LocInfoType::getAsStringInternal(std::string &Str,
  ...
  LLVM_ASSERT(false && "LocInfoType leaked into the type system; an opaque TypeTy*"
...
TypeResult Sema::ActOnTypeName(Scope *S, Declarator &D) {
  ...
  // the parser.
  LLVM_ASSERT(D.getIdentifier() == 0 && "Type name should have no identifier!");
...
bool ProcessFnAttr(Sema &S, QualType &Type, const AttributeList &Attr) {
  ...
  case AttributeList::AT_svmlcall: CC = CC_X86SvmlCall; break;
  case AttributeList::AT_svml_p2_f8_f8_f8_call: CC = CC_X86Svml_P2_F8_F8_F8_Call; break;
...
void ProcessTypeAttributeList(Sema &S, QualType &Result,
  ...
  for (; AL; AL = AL->getNext()) {
    ...
    case AttributeList::AT_svmlcall:
    case AttributeList::AT_svml_p2_f8_f8_f8_call:
...
bool Sema::RequireCompleteType(SourceLocation Loc, QualType T,
  ...
  //  LLVM_ASSERT(!Loc.isInvalid() && "Invalid location in RequireCompleteType");
  // FIXME: Add this assertion to help us flush out problems with
  // checking for dependent types and type-dependent expressions.
  //
  //  LLVM_ASSERT(!T->isDependentType() &&
  ...
  if (const RecordType *Record = MaybeTemplate->getAs<RecordType>()) {
    if (ClassTemplateSpecializationDecl *ClassTemplateSpec
      ...
      if (CXXRecordDecl *Pattern = Rec->getInstantiatedFromMemberClass()) {
        ...
        LLVM_ASSERT(MSInfo && "Missing member specialization information?");
