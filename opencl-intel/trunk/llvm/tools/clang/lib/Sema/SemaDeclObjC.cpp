...
void Sema::ActOnStartOfObjCMethodDef(Scope *FnBodyScope, Decl *D) {
  LLVM_ASSERT(getCurMethodDecl() == 0 && "Method parsing confused");
...
ActOnStartClassInterface(SourceLocation AtInterfaceLoc,
  ...
  LLVM_ASSERT(ClassName && "Missing class identifier");
...
Sema::ActOnStartProtocolInterface(SourceLocation AtProtoInterfaceLoc,
  ...
  LLVM_ASSERT(ProtocolName && "Missing protocol identifier");
...
void Sema::CheckImplementationIvars(ObjCImplementationDecl *ImpDecl,
  ...
  LLVM_ASSERT(ImpDecl && "missing implementation decl");
  ...
  LLVM_ASSERT(ivars && "missing @implementation ivars");
...
void Sema::ImplMethodsVsClassMethods(Scope *S, ObjCImplDecl* IMPDecl,
  ...
  if (ObjCInterfaceDecl *I = dyn_cast<ObjCInterfaceDecl> (CDecl)) {
    ...
    LLVM_ASSERT(false && "invalid ObjCContainerDecl type.");
}
...
Sema::ActOnForwardClassDeclaration(SourceLocation AtClassLoc,
  ...
  LLVM_ASSERT(Interfaces.size() == NumElts);
...
bool Sema::MatchTwoMethodDeclarations(const ObjCMethodDecl *Method,
  ...
  for (; ParamI != E; ++ParamI, ++PrevI) {
    LLVM_ASSERT(PrevI != PrevMethod->param_end() && "Param mismatch");
...
Sema::GlobalMethodPool::iterator Sema::ReadMethodPool(Selector Sel) {
  LLVM_ASSERT(ExternalSource && "We need an external AST source");
  LLVM_ASSERT(MethodPool.find(Sel) == MethodPool.end() &&
...
void Sema::CompareMethodParamsInBaseAndSuper(Decl *ClassDecl,
  ...
  while (ObjCInterfaceDecl *SD = ID->getSuperClass()) {
    ...
    for (; ParamI != E; ++ParamI, ++PrevI) {
      ...
      LLVM_ASSERT(PrevI != SuperMethodDecl->param_end() && "Param mismatch");
