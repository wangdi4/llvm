...
DeduceNonTypeTemplateArgument(Sema &S,
  ...
  LLVM_ASSERT(NTTP->getDepth() == 0 &&
         "Cannot deduce non-type template argument with depth > 0");
  LLVM_ASSERT((Value->isTypeDependent() || Value->isValueDependent()) &&
...
DeduceTemplateArguments(Sema &S,
  ...
  if (TemplateTemplateParmDecl *TempParam
    ...
    LLVM_ASSERT(ExistingArg.getKind() == TemplateArgument::Template);
...
DeduceTemplateArguments(Sema &S,
                        ...
                        TemplateDeductionInfo &Info,
                    llvm::SmallVectorImpl<DeducedTemplateArgument> &Deduced) {
  LLVM_ASSERT(Arg.isCanonical() && "Argument type must be canonical");
...
DeduceTemplateArguments(Sema &S,
  ...
  if (const TemplateTypeParmType *TemplateTypeParm
    ...
    LLVM_ASSERT(TemplateTypeParm->getDepth() == 0 && "Can't deduce with depth > 0");
    LLVM_ASSERT(Arg != S.Context.OverloadTy && "Unresolved overloaded function");
  ...
  switch (Param->getTypeClass()) {
    ...
    case Type::InjectedClassName: {
      ...
      LLVM_ASSERT(isa<TemplateSpecializationType>(Param) &&
        ...
        if (const RecordType *RecordT = Arg->getAs<RecordType>()) {
          ...
          while (!ToVisit.empty()) {
            ...
            for (CXXRecordDecl::base_class_iterator Base = Next->bases_begin(),
                 ...
                 Base != BaseEnd; ++Base) {
              LLVM_ASSERT(Base->getType()->isRecordType() &&
...
DeduceTemplateArguments(Sema &S,
  ...
  case TemplateArgument::Null:
    LLVM_ASSERT(false && "Null template argument in parameter list");
    break;
...
DeduceTemplateArguments(Sema &S,
                        ...
                        TemplateDeductionInfo &Info,
                    llvm::SmallVectorImpl<DeducedTemplateArgument> &Deduced) {
  LLVM_ASSERT(ParamList.size() == ArgList.size());
...
static bool isSameTemplateArg(ASTContext &Context,
  ...
  switch (X.getKind()) {
    case TemplateArgument::Null:
      LLVM_ASSERT(false && "Comparing NULL template argument");
      break;
...
Sema::SubstituteExplicitTemplateArguments(
  ...
  if (FunctionType) {
    ...
    LLVM_ASSERT(Proto && "Function template does not have a prototype?");
...
Sema::FinishTemplateArgumentDeduction(FunctionTemplateDecl *FunctionTemplate,
  ...
  LLVM_ASSERT(Specialization->getPrimaryTemplate()->getCanonicalDecl() == 
...
Sema::DeduceTemplateArguments(FunctionTemplateDecl *FunctionTemplate,
  ...
  //   If A is not a reference type:
    ...
    LLVM_ASSERT(!A->isReferenceType() && "Reference types were handled above");
...
static bool isAtLeastAsSpecializedAs(Sema &S,
  ...
  LLVM_ASSERT(Proto1 && Proto2 && "Function templates must have prototypes");
...
Sema::getMoreSpecializedTemplate(FunctionTemplateDecl *FT1,
  ...
  LLVM_ASSERT(!(Better1 && Better2) && "Should have broken out in the loop above");
  if (Better1)
    return FT1;
  else if (Better2)
    return FT2;
...
Sema::getMostSpecialized(UnresolvedSetIterator SpecBegin,
  ...
  LLVM_ASSERT(BestTemplate && "Not a function template specialization?");
  for (UnresolvedSetIterator I = SpecBegin + 1; I != SpecEnd; ++I) {
    FunctionTemplateDecl *Challenger
      = cast<FunctionDecl>(*I)->getPrimaryTemplate();
    LLVM_ASSERT(Challenger && "Not a function template specialization?");
