...
ParsedType Sema::getDestructorName(SourceLocation TildeLoc,
  ...
  if (SS.isSet()) {
    ...
    LookupCtx = computeDeclContext(SearchType);
    isDependent = SearchType->isDependentType();
    LLVM_ASSERT((isDependent || !SearchType->isIncompleteType()) && 
...
Sema::ActOnCXXBoolLiteral(SourceLocation OpLoc, tok::TokenKind Kind) {
  LLVM_ASSERT((Kind == tok::kw_true || Kind == tok::kw_false) &&
...
Sema::ActOnCXXTypeConstructExpr(SourceRange TypeRange, ParsedType TypeRep,
  ...
  if (NumExprs == 1) {
    ...
    if (CheckCastTypes(TypeRange, Ty, Exprs[0], Kind, BasePath,
                       /*isParenthesizedExprSet=*/false,/*FunctionalStyle=*/true))
      return ExprError();
  ...
  LLVM_ASSERT(NumExprs == 0 && "Expected 0 expressions");
  // C++ [expr.type.conv]p2:
...
bool Sema::FindAllocationOverload(SourceLocation StartLoc, SourceRange Range,
  ...
  LLVM_ASSERT(false && "Unreachable, bad result from BestViableFunction");
  return true;
}
...
void Sema::DeclareGlobalAllocationFunction(DeclarationName Name,
  ...
  if (HasBadAllocExceptionSpec) {
    LLVM_ASSERT(StdBadAlloc && "Must have std::bad_alloc declared");
...
bool Sema::FindDeallocationFunction(SourceLocation StartLoc, CXXRecordDecl *RD,
  ...
  LLVM_ASSERT(Operator && "Did not find a deallocation function!");
  return false;
}
...
static ExprResult BuildCXXCastArgument(Sema &S, 
                                       ...
                                       Expr *From) {
  switch (Kind) {
  default: LLVM_ASSERT(0 && "Unhandled cast kind!");
  ...
  case CK_UserDefinedConversion: {
    LLVM_ASSERT(!From->getType()->isPointerType() && "Arg can't have pointer type!");
...
Sema::PerformImplicitConversion(Expr *&From, QualType ToType,
  ...
  case ImplicitConversionSequence::UserDefinedConversion: {
      ...
      if (const CXXConversionDecl *Conv = dyn_cast<CXXConversionDecl>(FD)) {
        ...
        LLVM_ASSERT(0 && "Unknown conversion function kind!");
  ...
  case ImplicitConversionSequence::EllipsisConversion:
    LLVM_ASSERT(false && "Cannot perform an ellipsis conversion");
    return false;
...
Sema::PerformImplicitConversion(Expr *&From, QualType ToType,
  ...
  if (SCS.CopyConstructor) {
    ...
    LLVM_ASSERT(!ToType->isReferenceType());
  ...
  default:
    LLVM_ASSERT(false && "Improper first standard conversion");
    break;
  }
  ...
  case ICK_Num_Conversion_Kinds:
    LLVM_ASSERT(false && "Improper second standard conversion");
    break;
  }
  ...
  default:
    LLVM_ASSERT(false && "Improper third standard conversion");
    break;
  }
...
static bool FindConditionalOverload(Sema &Self, Expr *&LHS, Expr *&RHS,
  ...
  switch (CandidateSet.BestViableFunction(Self, Loc, Best)) {
    ...
    case OR_Deleted:
      LLVM_ASSERT(false && "Conditional operator has only built-in overloads");
      break;
  }
  return true;
}
...
QualType Sema::FindCompositePointerType(SourceLocation Loc, 
  ...
  LLVM_ASSERT(getLangOptions().CPlusPlus && "This function assumes C++");
...
ExprResult Sema::MaybeBindToTemporary(Expr *E) {
  ...
  LLVM_ASSERT(!isa<CXXBindTemporaryExpr>(E) && "Double-bound temporary?");
...
Expr *Sema::MaybeCreateCXXExprWithTemporaries(Expr *SubExpr) {
  LLVM_ASSERT(SubExpr && "sub expression can't be null!");

  // Check any implicit conversions within the expression.
  CheckImplicitConversions(SubExpr);

  unsigned FirstTemporary = ExprEvalContexts.back().NumTemporaries;
  LLVM_ASSERT(ExprTemporaries.size() >= FirstTemporary);
...
ExprResult Sema::ActOnPseudoDestructorExpr(Scope *S, Expr *Base,
                                                 ...
                                                 UnqualifiedId &SecondTypeName,
                                                       bool HasTrailingLParen) {
  LLVM_ASSERT((FirstTypeName.getKind() == UnqualifiedId::IK_TemplateId ||
          FirstTypeName.getKind() == UnqualifiedId::IK_Identifier) &&
         "Invalid first type name in pseudo-destructor");
  LLVM_ASSERT((SecondTypeName.getKind() == UnqualifiedId::IK_TemplateId ||
...
CXXMemberCallExpr *Sema::BuildCXXMemberCallExpr(Expr *Exp, 
  ...
  if (PerformObjectArgumentInitialization(Exp, /*Qualifier=*/0,
                                          FoundDecl, Method))
  {
    LLVM_ASSERT(0 && "Calling BuildCXXMemberCallExpr with invalid call?");
  }
