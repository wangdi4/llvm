...
Sema::getTemplateInstantiationArgs(NamedDecl *D, 
  ...
  while (!Ctx->isFileContext()) {
    ...
    if (ClassTemplateSpecializationDecl *Spec
      ...
      LLVM_ASSERT(Spec->getSpecializedTemplate() && "No class template?");
    ...
    // Add template arguments from a function template specialization.
      ...
      if (const TemplateArgumentList *TemplateArgs
        ...
        LLVM_ASSERT(Function->getPrimaryTemplate() && "No function template?");
...
Sema::InstantiatingTemplate::InstantiatingTemplate(Sema &SemaRef,
  ...
  LLVM_ASSERT(!Inst.isInstantiationRecord());
  ++SemaRef.NonInstantiationEntries;
}
...
void Sema::InstantiatingTemplate::Clear() {
  if (!Invalid) {
    if (!SemaRef.ActiveTemplateInstantiations.back().isInstantiationRecord()) {
      LLVM_ASSERT(SemaRef.NonInstantiationEntries > 0);
      --SemaRef.NonInstantiationEntries;
    }
...
bool Sema::InstantiatingTemplate::CheckInstantiationDepth(
                                        SourceLocation PointOfInstantiation,
                                           SourceRange InstantiationRange) {
  LLVM_ASSERT(SemaRef.NonInstantiationEntries <=
...
Decl *TemplateInstantiator::TransformDecl(SourceLocation Loc, Decl *D) {
  ...
  if (TemplateTemplateParmDecl *TTP = dyn_cast<TemplateTemplateParmDecl>(D)) {
    if (TTP->getDepth() < TemplateArgs.getNumLevels()) {
      ...
      LLVM_ASSERT(!Template.isNull() && Template.getAsTemplateDecl() &&
...
TemplateInstantiator::TransformPredefinedExpr(PredefinedExpr *E) {
  ...
  LLVM_ASSERT(currentDecl && "Must have current function declaration when "
                        "instantiating.");
...
TemplateInstantiator::TransformTemplateParmRefExpr(DeclRefExpr *E,
  ...
  if (Arg.getKind() == TemplateArgument::Declaration) {
    ...
    QualType TargetType = SemaRef.SubstType(NTTP->getType(), TemplateArgs,
                                            E->getLocation(), 
                                            DeclarationName());
    LLVM_ASSERT(!TargetType.isNull() && "type substitution failed for param type");
    LLVM_ASSERT(!TargetType->isDependentType() && "param type still dependent");
...
ExprResult TemplateInstantiator::TransformCXXDefaultArgExpr(
    CXXDefaultArgExpr *E) {
  LLVM_ASSERT(!cast<FunctionDecl>(E->getParam()->getDeclContext())->
...
TemplateInstantiator::TransformTemplateTypeParmType(TypeLocBuilder &TLB,
  ...
  if (T->getDepth() < TemplateArgs.getNumLevels()) {
    ...
    LLVM_ASSERT(TemplateArgs(T->getDepth(), T->getIndex()).getKind()
...
TypeSourceInfo *Sema::SubstType(TypeSourceInfo *T,
                                ...
                                SourceLocation Loc,
                                DeclarationName Entity) {
  LLVM_ASSERT(!ActiveTemplateInstantiations.empty() &&
...
Sema::InstantiateClassMembers(SourceLocation PointOfInstantiation,
  ...
  for (DeclContext::decl_iterator D = Instantiation->decls_begin(),
    ...
    if (FunctionDecl *Function = dyn_cast<FunctionDecl>(*D)) {
      ...
      LLVM_ASSERT(Pattern && "Missing instantiated-from-template information");
...
void LocalInstantiationScope::InstantiatedLocal(const Decl *D, Decl *Inst) {
  ...
  LLVM_ASSERT((!Stored || Stored == Inst)&& "Already instantiated this local");
  Stored = Inst;
}
