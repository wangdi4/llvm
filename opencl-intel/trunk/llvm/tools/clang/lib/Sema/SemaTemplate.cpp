...
TemplateNameKind Sema::isTemplateName(Scope *S,
  ...
  LLVM_ASSERT(getLangOptions().CPlusPlus && "No template names in C!");
  ...
  if (ResultCount > 1) {
    ...
    if (isa<FunctionTemplateDecl>(TD)) {
      ...
      LLVM_ASSERT(isa<ClassTemplateDecl>(TD) || isa<TemplateTemplateParmDecl>(TD));
...
void Sema::LookupTemplateName(LookupResult &Found,
  ...
  if (!ObjectType.isNull()) {
    ...
    LLVM_ASSERT(!SS.isSet() && "ObjectType and scope specifier cannot coexist");
    LookupCtx = computeDeclContext(ObjectType);
    isDependent = ObjectType->isDependentType();
    LLVM_ASSERT((isDependent || !ObjectType->isIncompleteType()) && 
...
bool Sema::DiagnoseTemplateParameterShadow(SourceLocation Loc, Decl *PrevDecl) {
  LLVM_ASSERT(PrevDecl->isTemplateParameter() && "Not a template parameter");
...
Decl *Sema::ActOnTypeParameter(Scope *S, bool Typename, bool Ellipsis,
  ...
  LLVM_ASSERT(S->isTemplateParamScope() &&
  ...
  if (DefaultArg) {
    ...
    LLVM_ASSERT(DefaultTInfo && "expected source information for type");
...
Sema::CheckClassTemplate(Scope *S, unsigned TagSpec, TagUseKind TUK,
                         ...
                         AccessSpecifier AS) {
  LLVM_ASSERT(TemplateParams && TemplateParams->size() > 0 &&
         "No template parameters");
  LLVM_ASSERT(TUK != TUK_Reference && "Can only declare or define class templates");
  bool Invalid = false;
  ...
  LLVM_ASSERT(Kind != TTK_Enum && "can't build template of enumerated type");
  ...
  LLVM_ASSERT(T->isDependentType() && "Class template type is not dependent?");
  (void)T;
  ...
  for (TemplateParameterList::iterator NewParam = NewParams->begin(),
    ...
    if (TemplateTypeParmDecl *NewTypeParm
      ...
      if (NewTypeParm->isParameterPack()) {
        LLVM_ASSERT(!NewTypeParm->hasDefaultArgument() &&
...
QualType Sema::CheckTemplateIdType(TemplateName Name,
  ...
  LLVM_ASSERT((Converted.structuredSize() ==
  ...
  if (Name.isDependent() ||
    ...
    LLVM_ASSERT(isa<RecordType>(CanonType) &&
...
TypeResult Sema::ActOnTagTemplateIdType(TypeResult TypeResult,
  ...
  if (const RecordType *RT = Type->getAs<RecordType>()) {
    ...
    LLVM_ASSERT(Id && "templated class must have an identifier");
...
ExprResult Sema::BuildTemplateIdExpr(const CXXScopeSpec &SS,
  ...
  LLVM_ASSERT(!R.empty() && "empty lookup results when building templateid");
  LLVM_ASSERT(!R.isAmbiguous() && "ambiguous lookup when building templateid");
...
TemplateNameKind Sema::ActOnDependentTemplateName(Scope *S, 
  ...
  case UnqualifiedId::IK_LiteralOperatorId:
    LLVM_ASSERT(false && "We don't support these; Parse shouldn't have allowed propagation");
...
bool Sema::CheckTemplateArgument(NamedDecl *Param,
  ...
  if (NonTypeTemplateParmDecl *NTTP =dyn_cast<NonTypeTemplateParmDecl>(Param)) {    
    ...
    case TemplateArgument::Null:
      LLVM_ASSERT(false && "Should never see a NULL template argument here");
      return true;
  ...
  for (TemplateParameterList::iterator Param = Params->begin(),
    ...
    if (TemplateTypeParmDecl *TTP = dyn_cast<TemplateTypeParmDecl>(*Param)) {
      if (!TTP->hasDefaultArgument()) {
        LLVM_ASSERT((Invalid || PartialTemplateArgs) && "Missing default argument");
        break;
      }
...
bool Sema::CheckTemplateArgument(TemplateTypeParmDecl *Param,
  ...
  LLVM_ASSERT(ArgInfo && "invalid TypeSourceInfo");
...
bool Sema::CheckTemplateArgumentPointerToMember(Expr *Arg, 
  ...
  if (isa<FieldDecl>(DRE->getDecl()) || isa<CXXMethodDecl>(DRE->getDecl())) {
    LLVM_ASSERT((isa<FieldDecl>(DRE->getDecl()) ||
...
bool Sema::CheckTemplateArgument(NonTypeTemplateParmDecl *Param,
  ...
  if (ParamType->isPointerType()) {
    ...
    LLVM_ASSERT(ParamType->getPointeeType()->isIncompleteOrObjectType() &&
  ...
  if (const ReferenceType *ParamRefType = ParamType->getAs<ReferenceType>()) {
    ...
    LLVM_ASSERT(ParamRefType->getPointeeType()->isIncompleteOrObjectType() &&
  ...
  LLVM_ASSERT(ParamType->isMemberPointerType() && "Only pointers to members remain");
...
bool Sema::CheckTemplateArgument(TemplateTemplateParmDecl *Param,
  ...
  if (!Template) {
    ...
    LLVM_ASSERT(Name.isDependent() && "Non-dependent template isn't a declaration?");
    return false;
  }
  ...
  if (!isa<ClassTemplateDecl>(Template) &&
    ...
    LLVM_ASSERT(isa<FunctionTemplateDecl>(Template) &&
...
Sema::BuildExpressionFromDeclTemplateArgument(const TemplateArgument &Arg,
                                              QualType ParamType,
                                              SourceLocation Loc) {
  LLVM_ASSERT(Arg.getKind() == TemplateArgument::Declaration &&
...
Sema::BuildExpressionFromIntegralTemplateArgument(const TemplateArgument &Arg,
                                                  SourceLocation Loc) {
  LLVM_ASSERT(Arg.getKind() == TemplateArgument::Integral &&
...
Sema::TemplateParameterListsAreEqual(TemplateParameterList *New,
  ...
  for (TemplateParameterList::iterator OldParm = Old->begin(),
    ...
    if (TemplateTypeParmDecl *OldTTP
      ...
      LLVM_ASSERT(isa<TemplateTemplateParmDecl>(*OldParm) &&
...
Sema::ActOnClassTemplateSpecialization(Scope *S, unsigned TagSpec,
  ...
  LLVM_ASSERT(TUK != TUK_Reference && "References are not specializations");
  ...
  // original template.
  TagTypeKind Kind = TypeWithKeyword::getTagTypeKindForTypeSpec(TagSpec);
  LLVM_ASSERT(Kind != TTK_Enum && "Invalid enum tag in class template spec!");
...
Sema::CheckSpecializationInstantiationRedecl(SourceLocation NewLoc,
  ...
  case TSK_ImplicitInstantiation:
    LLVM_ASSERT(false && "Don't check implicit instantiations here");
    return false;
  ...
  case TSK_ExplicitSpecialization:
    ...
    case TSK_ExplicitInstantiationDefinition:
      LLVM_ASSERT((PrevTSK == TSK_ImplicitInstantiation || 
  ...
  case TSK_ExplicitInstantiationDeclaration:
    ...
    case TSK_ExplicitInstantiationDefinition:
      ...
      LLVM_ASSERT(PrevPointOfInstantiation.isValid() &&
             "Explicit instantiation without point of instantiation?");
      HasNoEffect = true;
      return false;
    }
  ...
  LLVM_ASSERT(false && "Missing specialization/instantiation case?");
...
Sema::CheckFunctionTemplateSpecialization(FunctionDecl *FD,
  ...
  LLVM_ASSERT(SpecInfo && "Function template specialization info missing?");
...
Sema::CheckMemberSpecialization(NamedDecl *Member, LookupResult &Previous) {
  LLVM_ASSERT(!isa<TemplateDecl>(Member) && "Only for non-template members");
  ...
  //   instantiation to take place, in every translation unit in which such a 
  //   use occurs; no diagnostic is required.
  LLVM_ASSERT(MSInfo && "Member specialization info missing?");
  ...
  if (isa<FunctionDecl>(Member)) {
    ...
    LLVM_ASSERT(isa<CXXRecordDecl>(Member) && "Only member classes remain");
...
Sema::ActOnExplicitInstantiation(Scope *S,
  ...
  Decl *TagD = ActOnTag(S, TagSpec, Sema::TUK_Reference,
                        ...
                        Owned, IsDependent);
  LLVM_ASSERT(!IsDependent && "explicit instantiation of dependent name not yet handled");
  ...
  if (PrevDecl) {
    ...
    bool HasNoEffect = false;
    LLVM_ASSERT(MSInfo && "No member specialization information?");
...
DeclResult Sema::ActOnExplicitInstantiation(Scope *S,
  ...
  if (!R->isFunctionType()) {
    ...
    LLVM_ASSERT(MSInfo && "Missing static data member specialization info?");
    bool HasNoEffect = false;
...
Sema::ActOnDependentTag(Scope *S, unsigned TagSpec, TagUseKind TUK,
  ...
  LLVM_ASSERT(Name && "Expected a name in a dependent tag");
...
Sema::ActOnTypenameType(Scope *S, SourceLocation TypenameLoc, 
  ...
  LLVM_ASSERT(isa<TemplateSpecializationType>(T) && 
  ...
  LLVM_ASSERT(DTN && "dependent template has non-dependent name?");
  LLVM_ASSERT(DTN->getQualifier()
         == static_cast<NestedNameSpecifier*>(SS.getScopeRep()));
