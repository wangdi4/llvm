...
static bool hasFunctionProto(const Decl *d) {
  if (const FunctionType *FnTy = getFunctionType(d))
    ...
    LLVM_ASSERT(isa<ObjCMethodDecl>(d) || isa<BlockDecl>(d));
    return true;
  }
}
...
static void HandleNoReturnAttr(Decl *d, const AttributeList &Attr, Sema &S) {
  ...
  LLVM_ASSERT(Attr.isInvalid() == false);
...
static void HandleSentinelAttr(Decl *d, const AttributeList &Attr, Sema &S) {
  ...
  if (FunctionDecl *FD = dyn_cast<FunctionDecl>(d)) {
    ...
    LLVM_ASSERT(FT && "FunctionDecl has non-function type?");
...
static void HandleReqdExtension(Decl *D, const AttributeList &Attr, Sema &S) {
  // check the attribute arguments.
  if (Attr.getNumArgs() != 1) {
    S.Diag(Attr.getLoc(), diag::err_attribute_wrong_number_arguments) << 1;
    return;
  ...
  /* required extension only applies to function declarations */
  if (!isa<FunctionDecl>(D)) {
    S.Diag(Attr.getLoc(), diag::warn_attribute_wrong_decl_type)
    << Attr.getName() << 0 /*function*/;
    return;
  ...
  if (!SE) {
    S.Diag(ArgExpr->getLocStart(), diag::err_attribute_not_string) <<"reqd_extension";
    return;
  }

  llvm::StringRef extName = SE->getString();

  OpenCL::Extension ext = OpenCL::convertExtNameToEnum(extName);

  if (ext == OpenCL::unknownExtension) {
    S.Diag(Attr.getLoc(), diag::warn_attribute_unknown_extension) << extName;
    return;
  }

  D->addAttr(::new (S.Context) ReqdExtensionAttr(Attr.getLoc(),S.Context, ext));
}
...
static void HandleVecTypeHint(Decl *d, const AttributeList &Attr, Sema &S) {
  ...
  Expr *ArgExpr = static_cast<Expr *>(Attr.getArg(0));
  SizeOfAlignOfExpr *SAE = dyn_cast<SizeOfAlignOfExpr>(ArgExpr);

  // Make sure that there is a string literal as the annotation's single
  // argument.
  if (!SAE) {
    S.Diag(ArgExpr->getLocStart(), diag::err_attribute_not_string) <<"vec_type_hint";
    return;
  }
  d->addAttr(::new (S.Context) VecTypeHintAttr(Attr.getLoc(), S.Context, SAE->getArgumentType()));
}
...
static void HandleCallConvAttr(Decl *d, const AttributeList &Attr, Sema &S) {
  ...
  case AttributeList::AT_svmlcall:
    d->addAttr(::new (S.Context) SvmlCallAttr(Attr.getLoc(), S.Context));
    return;
...
static void HandleOpenCLKernelAttr(Decl *d, const AttributeList &Attr, Sema &S) {
  ...
  if (!FD) {
    S.Diag(Attr.getLoc(), diag::err_kernel_attribute_on_non_function);
    return;
  }

  if (!FD->getResultType().getTypePtr()->isVoidType()) {
    S.Diag(Attr.getLoc(), diag::err_kernel_must_return_void);
    return;
  }
  ...
  for (unsigned i = 0; i < numParams; ++i) {
    ...
    if (QT->isStructureType()) {
      RecordDecl *RD = QT->getAsStructureType()->getDecl();
      DeclContext::decl_iterator i, e;

      for (i = RD->decls_begin(), e =  RD->decls_end(); i != e; i++) {
        DeclaratorDecl* DD = dyn_cast<DeclaratorDecl>(*i);
        if (!DD)
          break;

        if (DD->getType()->isPointerType()) {
          S.Diag(param->getLocation(), diag::err_OpenCL_object_in_struct_argument);
          return;
        }

        std::string typeName = DD->getType().getAsString();

        if (S.IsOpenCLImageType(typeName) ||
            typeName == "sampler_t" ||
            typeName == "event_t") {
           S.Diag(param->getLocation(), diag::err_OpenCL_object_in_struct_argument);
           return;
    ...
    if (typeName == "_Bool") {
      S.Diag(param->getLocation(), diag::err_bool_as_kernel_argument);
      return;
    }
    if (typeName == "half") {
      S.Diag(param->getLocation(), diag::err_half_as_kernel_argument);
      return;
    }
    if (typeName == "size_t") {
      S.Diag(param->getLocation(), diag::err_size_t_as_kernel_argument);
      return;
    }
    if (typeName == "ptrdiff_t") {
      S.Diag(param->getLocation(), diag::err_ptrdiff_t_as_kernel_argument);
      return;
    }
    if (typeName == "intptr_t") {
      S.Diag(param->getLocation(), diag::err_intptr_t_as_kernel_argument);
      return;
    }
    if (typeName == "uintptr_t") {
      S.Diag(param->getLocation(), diag::err_uintptr_t_as_kernel_argument);
      return;
    }
    if (typeName == "event_t") {
      S.Diag(param->getLocation(), diag::err_event_t_as_kernel_argument);
      return;
    }
    ...
    if (param->getType()->isPointerType()) {
      ...
      if (QT->getPointeeType()->isPointerType()) {
        S.Diag(Attr.getLoc(), diag::err_kernel_argument_pointer_to_pointer);
        return;
      }

      //check for adress space
      if (!QT->getPointeeType().getQualifiers().hasAddressSpace()) {
        std::string typeName = QT->getPointeeType().getAsString();
        if ( (typeName != "struct _image2d_t") &&
             (typeName != "struct _image3d_t") &&
             (typeName != "struct _image2d_array_t")) {
          S.Diag(Attr.getLoc(), diag::err_pointer_with_no_address_space);
          return;
        }
      }
    }
...
static void HandleLLVMAttr(Decl *d, const AttributeList &Attr, Sema &S) {
  switch(Attr.getKind()) {
  default:
    break;
  case AttributeList::AT_llvm_alignstack:
    {
      unsigned alignment;
      ...
      if (E->isTypeDependent() || E->isValueDependent() ||
          !E->isIntegerConstantExpr(ArgNum, S.Context)) {
        S.Diag(Attr.getLoc(), diag::err_attribute_argument_not_int)
          << "llvm_alignstack" << E->getSourceRange();
        return;
      }
      alignment = (unsigned) ArgNum.getZExtValue();

      d->addAttr(::new (S.Context) LLVMAlignStackAttr(Attr.getLoc(), S.Context,
                                                       alignment));
      break;
    }
  case AttributeList::AT_llvm_alwaysinline:
    d->addAttr(::new (S.Context) LLVMAlwaysInlineAttr(Attr.getLoc(), S.Context));
    break;
  case AttributeList::AT_llvm_hotpatch:
    d->addAttr(::new (S.Context) LLVMHotpatchAttr(Attr.getLoc(), S.Context));
    break;
  case AttributeList::AT_llvm_nonlazybind:
    d->addAttr(::new (S.Context) LLVMNonLazyBindAttr(Attr.getLoc(), S.Context));
    break;
  case AttributeList::AT_llvm_inlinehint:
    d->addAttr(::new (S.Context) LLVMInlineHintAttr(Attr.getLoc(), S.Context));
    break;
  case AttributeList::AT_llvm_naked:
    d->addAttr(::new (S.Context) LLVMNakedAttr(Attr.getLoc(), S.Context));
    break;
  case AttributeList::AT_llvm_noimplicitfloat:
    d->addAttr(::new (S.Context) LLVMNoImplicitFloatAttr(Attr.getLoc(), S.Context));
    break;
  case AttributeList::AT_llvm_noinline:
    d->addAttr(::new (S.Context) LLVMNoInlineAttr(Attr.getLoc(), S.Context));
    break;
  case AttributeList::AT_llvm_noredzone:
    d->addAttr(::new (S.Context) LLVMNoRedZoneAttr(Attr.getLoc(), S.Context));
    break;
  case AttributeList::AT_llvm_noreturn:
    d->addAttr(::new (S.Context) LLVMNoReturnAttr(Attr.getLoc(), S.Context));
    break;
  case AttributeList::AT_llvm_nounwind:
    d->addAttr(::new (S.Context) LLVMNoUnwindAttr(Attr.getLoc(), S.Context));
    break;
  case AttributeList::AT_llvm_optsize:
    d->addAttr(::new (S.Context) LLVMOptSizeAttr(Attr.getLoc(), S.Context));
    break;
  case AttributeList::AT_llvm_readnone:
    d->addAttr(::new (S.Context) LLVMReadNoneAttr(Attr.getLoc(), S.Context));
    break;
  case AttributeList::AT_llvm_readonly:
    d->addAttr(::new (S.Context) LLVMReadOnlyAttr(Attr.getLoc(), S.Context));
    break;
  case AttributeList::AT_llvm_ssp:
    d->addAttr(::new (S.Context) LLVMSSPAttr(Attr.getLoc(), S.Context));
    break;
  case AttributeList::AT_llvm_sspreq:
    d->addAttr(::new (S.Context) LLVMSSPReqAttr(Attr.getLoc(), S.Context));
    break;
  }
}
...
static void HandleEndianAttr(Decl *d, const AttributeList &Attr, Sema &S) {
  if (Attr.getNumArgs() != 1) {
    S.Diag(Attr.getLoc(), diag::err_attribute_wrong_number_arguments) << 1;
    return;
  }

  StringLiteral *Arg = static_cast<StringLiteral*>(Attr.getArg(0));

  OCLEndianAttr::EndianType type;

  if (Arg->getString() == "host")
    type = OCLEndianAttr::host;
  else if (Arg->getString() == "device")
    type = OCLEndianAttr::device;
  else {
    S.Diag(Attr.getLoc(), diag::warn_attribute_unknown_endian);
    return;
  }

  d->addAttr(::new (S.Context) OCLEndianAttr(Attr.getLoc(), S.Context, type));  
}
...
static void HandleNSReturnsRetainedAttr(Decl *d, const AttributeList &Attr,
  ...
  switch (Attr.getKind()) {
    default:
      LLVM_ASSERT(0 && "invalid ownership attribute");
      return;
...
static void ProcessDeclAttribute(Scope *scope, Decl *D,
  ...
  case AttributeList::AT_reqd_extension:
    HandleReqdExtension(D, Attr, S); break;
  ...
  case AttributeList::AT_llvm_ssp:
  case AttributeList::AT_llvm_sspreq:
    HandleLLVMAttr(D, Attr, S);
    break;
  case AttributeList::AT_endian:
    HandleEndianAttr(D, Attr, S);
    break;
  default:
...
NamedDecl * Sema::DeclClonePragmaWeak(NamedDecl *ND, IdentifierInfo *II) {
  LLVM_ASSERT(isa<FunctionDecl>(ND) || isa<VarDecl>(ND));
  NamedDecl *NewD = 0;
...
void Sema::PopParsingDeclaration(ParsingDeclStackState S, Decl *D) {
  LLVM_ASSERT(ParsingDeclDepth > 0 && "empty ParsingDeclaration stack");
  ParsingDeclDepth--;
  ...
  LLVM_ASSERT(SavedIndex <= DelayedDiagnostics.size() &&
