...
void ResultBuilder::MaybeAddResult(Result R, DeclContext *CurContext) {
  LLVM_ASSERT(!ShadowMaps.empty() && "Must enter into a results scope");
...
void ResultBuilder::AddResult(Result R) {
  LLVM_ASSERT(R.Kind != Result::RK_Declaration && 
...
static void AddTemplateParameterChunks(ASTContext &Context,
  ...
  for (TemplateParameterList::iterator P = Params->begin(); P != PEnd; ++P) {
    ...
    if (TemplateTypeParmDecl *TTP = dyn_cast<TemplateTypeParmDecl>(*P)) {
      ...
      LLVM_ASSERT(isa<TemplateTemplateParmDecl>(*P));
...
CodeCompletionResult::CreateCodeCompletionString(Sema &S,
  ...
  LLVM_ASSERT(Kind == RK_Declaration && "Missed a result kind?");
    ...
    for (LastDeducibleArgument = Deduced.size(); LastDeducibleArgument > 0;
      ...
      if (!Deduced[LastDeducibleArgument - 1]) {
        ...
        if (TemplateTypeParmDecl *TTP = dyn_cast<TemplateTypeParmDecl>(Param))
          ...
          LLVM_ASSERT(isa<TemplateTemplateParmDecl>(Param));
          HasDefaultArg 
...
void Sema::CodeCompleteMemberReferenceExpr(Scope *S, ExprTy *BaseE,
  ...
  if (const RecordType *Record = BaseType->getAs<RecordType>()) {
    ...
    LLVM_ASSERT(ObjCPtr && "Non-NULL pointer guaranteed above!");
...
void Sema::CodeCompleteTag(Scope *S, unsigned TagSpec) {
  ...
  default:
    LLVM_ASSERT(false && "Unknown type specifier kind in CodeCompleteTag");
    return;
  }
