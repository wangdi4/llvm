...
unsigned JumpScopeChecker::GetDeepestCommonScope(unsigned A, unsigned B) {
  while (A != B) {
    ...
    // higher indices.
    if (A < B) {
      LLVM_ASSERT(Scopes[B].ParentScope < B);
      B = Scopes[B].ParentScope;
    } else {
      LLVM_ASSERT(Scopes[A].ParentScope < A);
...
void JumpScopeChecker::VerifyJumps() {
  while (!Jumps.empty()) {
    ...
    for (SwitchCase *SC = SS->getSwitchCaseList(); SC;
      ...
      LLVM_ASSERT(LabelAndGotoScopes.count(SC) && "Case not visited?");
...
void JumpScopeChecker::VerifyIndirectJumps() {
  ...
  llvm::SmallVector<JumpScope, 32> JumpScopes;
  {
    ...
    for (llvm::SmallVectorImpl<IndirectGotoStmt*>::iterator
      ...
      LLVM_ASSERT(LabelAndGotoScopes.count(IG) &&
  ...
  for (llvm::SmallVectorImpl<LabelStmt*>::iterator
    ...
    LLVM_ASSERT(LabelAndGotoScopes.count(TheLabel) &&
...
void JumpScopeChecker::DiagnoseIndirectJump(IndirectGotoStmt *Jump,
  ...
  LLVM_ASSERT(JumpScope != TargetScope);
...
void JumpScopeChecker::CheckJump(Stmt *From, Stmt *To,
                                 SourceLocation DiagLoc, unsigned JumpDiag) {
  LLVM_ASSERT(LabelAndGotoScopes.count(From) && "Jump didn't get added to scopes?");
  unsigned FromScope = LabelAndGotoScopes[From];

  LLVM_ASSERT(LabelAndGotoScopes.count(To) && "Jump didn't get added to scopes?");
