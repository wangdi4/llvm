...
TemplateDeclInstantiator::VisitTranslationUnitDecl(TranslationUnitDecl *D) {
  LLVM_ASSERT(false && "Translation units cannot be instantiated");
  return D;
}

Decl *
TemplateDeclInstantiator::VisitNamespaceDecl(NamespaceDecl *D) {
  LLVM_ASSERT(false && "Namespaces cannot be instantiated");
  return D;
}
...
Decl *TemplateDeclInstantiator::VisitFriendDecl(FriendDecl *D) {
  ...
  LLVM_ASSERT(ND && "friend decl must be a decl or a type!");
...
Decl *TemplateDeclInstantiator::VisitEnumConstantDecl(EnumConstantDecl *D) {
  LLVM_ASSERT(false && "EnumConstantDecls can only occur within EnumDecls.");
  return 0;
}
...
Decl *TemplateDeclInstantiator::VisitFunctionDecl(FunctionDecl *D,
  ...
  if (!isFriend && D->isOutOfLine()) {
    LLVM_ASSERT(D->getDeclContext()->isFileContext());
  ...
  if (DependentFunctionTemplateSpecializationInfo *Info
    ...
    LLVM_ASSERT(isFriend && "non-friend has dependent specialization info?");
...
TemplateDeclInstantiator::VisitCXXMethodDecl(CXXMethodDecl *D,
  ...
  if (!isa<FunctionProtoType>(T)) {
    LLVM_ASSERT(!Params.size() && "Instantiating type could not yield parameters");
...
Decl *TemplateDeclInstantiator::VisitTemplateTypeParmDecl(
  ...
  LLVM_ASSERT(T->isTemplateTypeParmType());
...
TemplateDeclInstantiator::SubstFunctionType(FunctionDecl *D,
  ...
  LLVM_ASSERT(OldTInfo && "substituting function without type source info");
  LLVM_ASSERT(Params.empty() && "parameter vector is non-empty at start");
  ...
  if (NewTInfo != OldTInfo) {
    ...
    if (FunctionProtoTypeLoc *OldProtoLoc
      ...
      LLVM_ASSERT(NewProtoLoc && "Missing prototype?");
...
TemplateDeclInstantiator::InitFunctionInstantiation(FunctionDecl *New,
  ...
  if (ActiveInst.Kind == ActiveInstType::ExplicitTemplateArgumentSubstitution ||
    ...
    if (FunctionTemplateDecl *FunTmpl
      ...
      LLVM_ASSERT(FunTmpl->getTemplatedDecl() == Tmpl &&
  ...
  LLVM_ASSERT(Proto && "Function template without prototype?");
  ...
  if (Proto->hasExceptionSpec() || Proto->hasAnyExceptionSpec() ||
    ...
    LLVM_ASSERT(NewProto && "Template instantiation without function prototype?");
...
void Sema::InstantiateStaticDataMemberDefinition(
  ...
  LLVM_ASSERT(Def && "This data member was not instantiated from a template?");
  LLVM_ASSERT(Def->isStaticDataMember() && "Not a static data member?");  
...
static bool isInstantiationOfStaticDataMember(VarDecl *Pattern,
                                              VarDecl *Instance) {
  LLVM_ASSERT(Instance->isStaticDataMember());
...
NamedDecl *Sema::FindInstantiatedDecl(SourceLocation Loc, NamedDecl *D,
  ...
  if (CXXRecordDecl *Record = dyn_cast<CXXRecordDecl>(D)) {
    ...
    if (ClassTemplate) {
      ...
      LLVM_ASSERT(isa<InjectedClassNameType>(T) &&
    ...
    if (!T.isNull()) {
      ...
      LLVM_ASSERT(!T.isNull() && "Instantiation of injected-class-name cannot fail.");
    
      if (!T->isDependentType()) {
        LLVM_ASSERT(T->isRecordType() && "Instantiation must produce a record type");
      ...
      LLVM_ASSERT(!SawNonDependentContext && 
  ...
  if (ParentDC != D->getDeclContext()) {
    ...
    if (CXXRecordDecl *Spec = dyn_cast<CXXRecordDecl>(ParentDC)) {
      if (!Spec->isDependentContext()) {
        ...
        LLVM_ASSERT(Tag && "type of non-dependent record is not a RecordType");
    ...
    LLVM_ASSERT((Result || isa<UsingShadowDecl>(D) || D->isInvalidDecl() ||
...
void Sema::PerformPendingInstantiations(bool LocalOnly) {
  while (!PendingLocalImplicitInstantiations.empty() ||
    ...
    case TSK_Undeclared:
      LLVM_ASSERT(false && "Cannot instantitiate an undeclared specialization.");
