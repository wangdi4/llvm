...
ImplicitConversionRank GetConversionRank(ImplicitConversionKind Kind) {
  static const ImplicitConversionRank
    ...
    ICR_Promotion,
	ICR_Widening,
	ICR_Promotion_Widening,
	ICR_Promotion_Widening,
    ICR_Conversion,
    ICR_Conversion,
    ICR_Conversion,
    ICR_Conversion,
	ICR_Conversion_Widening,
	ICR_Conversion_Widening,
	ICR_Conversion_Widening,
    ICR_Conversion,
    ICR_Conversion,
...
Sema::CheckOverload(Scope *S, FunctionDecl *New, const LookupResult &Old,
  ...
  for (LookupResult::iterator I = Old.begin(), E = Old.end();
    ...
    if (FunctionTemplateDecl *OldT = dyn_cast<FunctionTemplateDecl>(OldD)) {
      ...
      LLVM_ASSERT(Old.getLookupKind() == LookupUsingDeclName);
...
static bool IsStandardConversion(Sema &S, Expr* From, QualType ToType,
  ...
  if (FromType == S.Context.OverloadTy) {
    ...
    if (FunctionDecl *Fn
      ...
      LLVM_ASSERT(S.Context.hasSameType(FromType,
  ...
  if (S.getLangOptions().OpenCL && ToType->isVectorType()) {
	  const QualType& ToElemType = (ToType->getAs<ExtVectorType>())->getElementType();
	  if (FromType->isVectorType()) {
		  const QualType& FromElemType = (FromType->getAs<ExtVectorType>())->getElementType();
		  const ExtVectorType& FromVecType = *FromType->getAs<ExtVectorType>(), &ToVecType = *ToType->getAs<ExtVectorType>();
		  if (FromVecType.getNumElements() != ToVecType.getNumElements() ||
			  FromElemType->getCanonicalTypeUnqualified() != ToElemType->getCanonicalTypeUnqualified())
			  return false;
		  SCS.Second = ICK_Identity;
	  } else {
		  if (FromType == ToElemType)
			  SCS.Second = ICK_Widening;
		  else if (S.IsIntegralPromotion(From, FromType, ToElemType))
			  SCS.Second = ICK_Integral_Promotion_Widening;
		  else if (S.IsFloatingPointPromotion(FromType, ToElemType))
			  SCS.Second = ICK_Floating_Promotion_Widening;
		  else if (FromType->isIntegerType() && ToElemType->isIntegerType())
			  SCS.Second = ICK_Integral_Conversion_Widening;
		  else if (FromType->isFloatingType() && ToElemType->isFloatingType())
			  SCS.Second = ICK_Floating_Conversion_Widening;
		  else if ((FromType->isFloatingType() && ToElemType->isIntegralType(S.Context)) ||
			  (FromType->isIntegralType(S.Context) && ToElemType->isFloatingType()))
			  SCS.Second = ICK_Floating_Integral_Widening;
		  else
			  return false;
	  }
...
bool Sema::IsIntegralPromotion(Expr *From, QualType FromType, QualType ToType) {
  ...
  // int (C++ 4.5p1).
  if (((getLangOptions().OpenCL && FromType->isOpenCLPromotableIntegerType()) || (!getLangOptions().OpenCL && FromType->isPromotableIntegerType())) &&
	  !FromType->isBooleanType() && !FromType->isEnumeralType()) {
...
bool Sema::CheckMemberPointerConversion(Expr *From, QualType ToType,
  ...
  if (!FromPtrType) {
    ...
    LLVM_ASSERT(From->isNullPointerConstant(Context, 
                                       Expr::NPC_ValueDependentIsNull) &&
  ...
  LLVM_ASSERT(ToPtrType && "No member pointer cast has a target type "
  ...
  LLVM_ASSERT(FromClass->isRecordType() && "Pointer into non-class.");
  LLVM_ASSERT(ToClass->isRecordType() && "Pointer into non-class.");

  CXXBasePaths Paths(/*FindAmbiguities=*/true, /*RecordPaths=*/true,
                     /*DetectVirtual=*/true);
  bool DerivationOkay = IsDerivedFrom(ToClass, FromClass, Paths);
  LLVM_ASSERT(DerivationOkay &&
                                   ...
                                   const StandardConversionSequence& SCS2)
{
  ...
  if (Rank1 < Rank2)
    ...
    return ImplicitConversionSequence::Worse;
	  ...
	  const QualType& T1 = SCS1.getToType(1), &T2 = SCS2.getToType(1);
	  if (Rank1 == ICR_Widening || Rank1 == ICR_Promotion_Widening || Rank1 == ICR_Conversion_Widening) {
		  // Sema::IsStandardConversion puts the "to-type" we need at index 1
		  unsigned int numElem1 = T1.getTypePtr()->getAs<ExtVectorType>()->getNumElements();
		  unsigned int numElem2 = T2.getTypePtr()->getAs<ExtVectorType>()->getNumElements();
		  if (numElem1 < numElem2)
			  return ImplicitConversionSequence::Better;
		  if (numElem2 < numElem1)
			  return ImplicitConversionSequence::Worse;
	  }
	  if (T1.getTypePtr()->isFloatingType() && T2.getTypePtr()->isFloatingType() &&
		  (Rank1 == ICR_Conversion || Rank1 == ICR_Conversion_Widening)) {
		  if ((T1 == S.Context.FloatTy && T2 == S.Context.DoubleTy && S.getOpenCLOptions().cl_khr_fp64) ||
			  (T1 == S.Context.DoubleTy && T2 == S.Context.FloatTy && !S.getOpenCLOptions().cl_khr_fp64))
			  return ImplicitConversionSequence::Worse;
		  if ((T1 == S.Context.DoubleTy && T2 == S.Context.FloatTy && S.getOpenCLOptions().cl_khr_fp64) ||
			  (T1 == S.Context.FloatTy && T2 == S.Context.DoubleTy && !S.getOpenCLOptions().cl_khr_fp64))
			  return ImplicitConversionSequence::Better;
	  }
...
Sema::CompareReferenceRelationship(SourceLocation Loc,
                                   ...
                                   bool &DerivedToBase,
                                   bool &ObjCConversion) {
  LLVM_ASSERT(!OrigT1->isReferenceType() &&
    "T1 must be the pointee type of the reference type");
  LLVM_ASSERT(!OrigT2->isReferenceType() && "T2 cannot be a reference type");
...
FindConversionForRefInit(Sema &S, ImplicitConversionSequence &ICS,
                         ...
                         bool AllowExplicit) {
  LLVM_ASSERT(T2->isRecordType() && "Can only find conversions of record types.");
  ...
  case OR_Success:
    ...
    LLVM_ASSERT(ICS.UserDefined.After.ReferenceBinding &&
...
TryReferenceInit(Sema &S, Expr *&Init, QualType DeclType,
                 ...
                 bool SuppressUserConversions,
                 bool AllowExplicit) {
  LLVM_ASSERT(DeclType->isReferenceType() && "Reference init needs a reference");
...
TryObjectArgumentInitialization(Sema &S, QualType OrigFromType,
  ...
  LLVM_ASSERT(FromType->isRecordType());
...
Sema::AddOverloadCandidate(FunctionDecl *Function,
  ...
  LLVM_ASSERT(Proto && "Functions without a prototype cannot be overloaded");
  LLVM_ASSERT(!Function->getDescribedFunctionTemplate() &&
...
void Sema::AddMethodCandidate(DeclAccessPair FoundDecl,
  ...
  if (FunctionTemplateDecl *TD = dyn_cast<FunctionTemplateDecl>(Decl)) {
    LLVM_ASSERT(isa<CXXMethodDecl>(TD->getTemplatedDecl()) &&
...
Sema::AddMethodCandidate(CXXMethodDecl *Method, DeclAccessPair FoundDecl,
  ...
  LLVM_ASSERT(Proto && "Methods without a prototype cannot be overloaded");
  LLVM_ASSERT(!isa<CXXConstructorDecl>(Method) &&
...
Sema::AddMethodTemplateCandidate(FunctionTemplateDecl *MethodTmpl,
  ...
  LLVM_ASSERT(Specialization && "Missing member function template specialization?");
  LLVM_ASSERT(isa<CXXMethodDecl>(Specialization) &&
...
Sema::AddTemplateOverloadCandidate(FunctionTemplateDecl *FunctionTemplate,
  ...
  // Add the function template specialization produced by template argument
  // deduction as a candidate.
  LLVM_ASSERT(Specialization && "Missing function template specialization?");
...
Sema::AddConversionCandidate(CXXConversionDecl *Conversion,
                             ...
                             OverloadCandidateSet& CandidateSet) {
  LLVM_ASSERT(!Conversion->getDescribedFunctionTemplate() &&
...
Sema::AddTemplateConversionCandidate(FunctionTemplateDecl *FunctionTemplate,
                                     ...
                                     Expr *From, QualType ToType,
                                     OverloadCandidateSet &CandidateSet) {
  LLVM_ASSERT(isa<CXXConversionDecl>(FunctionTemplate->getTemplatedDecl()) &&
...
void Sema::AddBuiltinCandidate(QualType ResultTy, QualType *ParamTys,
  ...
  for (unsigned ArgIdx = 0; ArgIdx < NumArgs; ++ArgIdx) {
    ...
    if (ArgIdx < NumContextualBoolArguments) {
      LLVM_ASSERT(ParamTys[ArgIdx] == Context.BoolTy &&
...
BuiltinCandidateTypeSet::AddPointerWithMoreQualifiedTypeVariants(QualType Ty,
  ...
  if (!PointerTy) {
    if (const ObjCObjectPointerType *PTy = Ty->getAs<ObjCObjectPointerType>()) {
      ...
      LLVM_ASSERT(false && "type was not a pointer type!");
  }
  else
...
BuiltinCandidateTypeSet::AddMemberPointerWithMoreQualifiedTypeVariants(
  ...
  LLVM_ASSERT(PointerTy && "type was not a member pointer type!");
...
// FIXME:   Context.Char16Ty, Context.Char32Ty,
  ...
  LLVM_ASSERT(ArithmeticTypes[FirstPromotedIntegralType] == Context.IntTy &&
         "Invalid first promoted integral type");
  LLVM_ASSERT(ArithmeticTypes[LastPromotedIntegralType - 1] 
           == Context.UnsignedLongLongTy &&
         "Invalid last promoted integral type");
  LLVM_ASSERT(ArithmeticTypes[FirstPromotedArithmeticType] == Context.IntTy &&
         "Invalid first promoted arithmetic type");
  LLVM_ASSERT(ArithmeticTypes[LastPromotedArithmeticType - 1] 
  ...
  case NUM_OVERLOADED_OPERATORS:
    LLVM_ASSERT(false && "Expected an overloaded operator");
    break;
  ...
  case OO_Call:
    LLVM_ASSERT(false && "Special operators don't use AddBuiltinOperatorCandidates");
    break;
...
isBetterOverloadCandidate(Sema &S,
  ...
  size_t NumArgs = Cand1.Conversions.size();
  assert(Cand2.Conversions.size() == NumArgs && "Overload candidate mismatch");
  // OpenCL 1.2: Sequences that match with ellipsis are the worst.
  for (size_t i = StartArg; i < NumArgs; i++) {
	  if (Cand1.Conversions[i].isEllipsis() && !Cand2.Conversions[i].isEllipsis())
		  return false;
	  if (Cand2.Conversions[i].isEllipsis() && !Cand1.Conversions[i].isEllipsis())
		  return true;
  }
...
OverloadCandidateKind ClassifyOverloadCandidate(Sema &S,
  ...
  if (CXXMethodDecl *Meth = dyn_cast<CXXMethodDecl>(Fn)) {
    ...
    LLVM_ASSERT(Meth->isCopyAssignment()
...
void DiagnoseBadConversion(Sema &S, OverloadCandidate *Cand, unsigned I) {
  ...
  LLVM_ASSERT(Conv.isBad());
  LLVM_ASSERT(Cand->Function && "for now, candidate must be a function");
  ...
  if (FromTy == S.Context.OverloadTy) {
    LLVM_ASSERT(FromExpr && "overload set argument came from implicit argument?");
  ...
  if (CToTy.getUnqualifiedType() == CFromTy.getUnqualifiedType() &&
    ...
    LLVM_ASSERT(CVR && "unexpected qualifiers mismatch");
...
void DiagnoseArityMismatch(Sema &S, OverloadCandidate *Cand,
  ...
  if (NumFormalArgs < MinParams) {
    LLVM_ASSERT((Cand->FailureKind == ovl_fail_too_few_arguments) ||
    ...
    LLVM_ASSERT((Cand->FailureKind == ovl_fail_too_many_arguments) ||
           (Cand->FailureKind == ovl_fail_bad_deduction &&
...
void DiagnoseBadDeduction(Sema &S, OverloadCandidate *Cand,
  ...
  case Sema::TDK_Incomplete: {
    LLVM_ASSERT(ParamD && "no parameter found for incomplete deduction result");
  ...
  case Sema::TDK_Underqualified: {
    LLVM_ASSERT(ParamD && "no parameter found for bad qualifiers deduction result");
    ...
    LLVM_ASSERT(S.Context.hasSameType(Param, NonCanonParam));
  ...
  case Sema::TDK_Inconsistent: {
    LLVM_ASSERT(ParamD && "no parameter found for inconsistent deduction result");    
    int which = 0;
  ...
  case Sema::TDK_InvalidExplicitArguments:
    LLVM_ASSERT(ParamD && "no parameter found for invalid explicit arguments");    
...
void NoteBuiltinOperatorCandidate(Sema &S,
  ...
  LLVM_ASSERT(Cand->Conversions.size() <= 2 && "builtin operator is not binary");
  ...
  bool operator()(const OverloadCandidate *L,
    ...
    if (!L->Viable) {
      ...
      if (L->FailureKind == ovl_fail_bad_conversion) {
        ...
        LLVM_ASSERT(L->Conversions.size() == R->Conversions.size());
...
void CompleteNonViableCandidate(Sema &S, OverloadCandidate *Cand,
  ...
  while (true) {
    LLVM_ASSERT(ConvIdx != ConvCount && "no bad conversion in candidate");
    ConvIdx++;
  ...
  LLVM_ASSERT(!Cand->Conversions[ConvIdx].isInitialized() &&
...
Sema::ResolveAddressOfOverloadedFunction(Expr *From, QualType ToType,
  ...
  LLVM_ASSERT(From->getType() == Context.OverloadTy);
  ...
  if (!FoundNonTemplateFunction) {
    ...
    LLVM_ASSERT(Result != MatchesCopy.end() && "no most-specialized template");
    MarkDeclarationReferenced(From->getLocStart(), *Result);
...
static void AddOverloadedCallCandidate(Sema &S,
  ...
  if (FunctionDecl *Func = dyn_cast<FunctionDecl>(Callee)) {
    LLVM_ASSERT(!ExplicitTemplateArgs && "Explicit template arguments?");
  ...
  LLVM_ASSERT(false && "unhandled case in overloaded call candidate");
...
void Sema::AddOverloadedCallCandidates(UnresolvedLookupExpr *ULE,
  ...
  if (ULE->requiresADL()) {
    for (UnresolvedLookupExpr::decls_iterator I = ULE->decls_begin(),
      ...
      LLVM_ASSERT(!(*I)->getDeclContext()->isRecord());
      LLVM_ASSERT(isa<UsingShadowDecl>(*I) ||
             !(*I)->getDeclContext()->isFunctionOrMethod());
      LLVM_ASSERT((*I)->getUnderlyingDecl()->isFunctionOrFunctionTemplate());
    }
  }
#endif
...
BuildRecoveryCallExpr(Sema &SemaRef, Scope *S, Expr *Fn,
  ...
  LLVM_ASSERT(!R.empty() && "lookup results empty despite recovery");
...
Sema::BuildOverloadedCallExpr(Scope *S, Expr *Fn, UnresolvedLookupExpr *ULE,
  ...
  if (ULE->requiresADL()) {
    ...
    LLVM_ASSERT(!ULE->getQualifier() && "qualified name with ADL");
    ...
    if (ULE->decls_begin() + 1 == ULE->decls_end() &&
      ...
      LLVM_ASSERT(0 && "performing ADL for builtin");
      
    // We don't perform ADL in C.
    LLVM_ASSERT(getLangOptions().CPlusPlus && "ADL enabled in C");
  }
#endif
...
Sema::CreateOverloadedUnaryOp(SourceLocation OpLoc, unsigned OpcIn,
  ...
  LLVM_ASSERT(Op != OO_None && "Invalid opcode for overloaded unary operator");
  DeclarationName OpName = Context.DeclarationNames.getCXXOperatorName(Op);
...
Sema::BuildCallToMemberFunction(Scope *S, Expr *MemExprE,
  ...
  LLVM_ASSERT(Method && "Member call to something that isn't a method?");
...
Sema::BuildCallToObjectOfClassType(Scope *S, Expr *Object,
                                   ...
                                   SourceLocation *CommaLocs,
                                   SourceLocation RParenLoc) {
  LLVM_ASSERT(Object->getType()->isRecordType() && "Requires object type argument");
...
Sema::BuildOverloadedArrowExpr(Scope *S, Expr *Base, SourceLocation OpLoc) {
  LLVM_ASSERT(Base->getType()->isRecordType() && "left-hand side must have class type");
...
Expr *Sema::FixOverloadedFunctionReference(Expr *E, DeclAccessPair Found,
  ...
  if (ImplicitCastExpr *ICE = dyn_cast<ImplicitCastExpr>(E)) {
    Expr *SubExpr = FixOverloadedFunctionReference(ICE->getSubExpr(),
                                                   Found, Fn);
    LLVM_ASSERT(Context.hasSameType(ICE->getSubExpr()->getType(), 
                               SubExpr->getType()) &&
           "Implicit cast type cannot be determined from overload");
    LLVM_ASSERT(ICE->path_empty() && "fixing up hierarchy conversion?");
  ...
  if (UnaryOperator *UnOp = dyn_cast<UnaryOperator>(E)) {
    LLVM_ASSERT(UnOp->getOpcode() == UO_AddrOf &&
    ...
    if (CXXMethodDecl *Method = dyn_cast<CXXMethodDecl>(Fn)) {
      if (Method->isStatic()) {
        ...
        LLVM_ASSERT(isa<DeclRefExpr>(SubExpr)
               && "fixed to something other than a decl ref");
        LLVM_ASSERT(cast<DeclRefExpr>(SubExpr)->getQualifier()
  ...
  LLVM_ASSERT(false && "Invalid reference to overloaded function");
  return E->Retain();
}
