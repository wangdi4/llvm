...
#include "clang/AST/StmtNodes.inc"
  ...
  ExprResult RebuildSizeOfAlignOf(TypeSourceInfo *TInfo,
                                        ...
                                        SizeOfAlignOfExpr::Kind_t exprKind, SourceRange R) {
    return getSema().CreateSizeOfAlignOfExpr(TInfo, OpLoc, exprKind, R);
  }
  ...
  ExprResult RebuildSizeOfAlignOf(Expr *SubExpr, SourceLocation OpLoc,
                                        SizeOfAlignOfExpr::Kind_t exprKind, SourceRange R) {
    ExprResult Result
      = getSema().CreateSizeOfAlignOfExpr(SubExpr, OpLoc, exprKind, R);
    if (Result.isInvalid())
      return ExprError();
  ...
  ExprResult RebuildMemberExpr(Expr *Base, SourceLocation OpLoc,
    ...
    if (!Member->getDeclName()) {
      ...
      LLVM_ASSERT(!Qualifier && "Can't have an unnamed field with a qualifier!");
  ...
  ExprResult RebuildCXXNamedCastExpr(SourceLocation OpLoc,
    ...
    default:
      LLVM_ASSERT(false && "Invalid C++ named cast");
      break;
    }
  ...
  ExprResult RebuildShuffleVectorExpr(SourceLocation BuiltinLoc,
    ...
    LLVM_ASSERT(Lookup.first != Lookup.second && "No __builtin_shufflevector?");
...
TreeTransform<Derived>::TransformNestedNameSpecifier(NestedNameSpecifier *NNS,
  ...
  case NestedNameSpecifier::Identifier:
    LLVM_ASSERT((Prefix || !ObjectType.isNull()) &&
...
::TransformDeclarationNameInfo(const DeclarationNameInfo &NameInfo,
  ...
  LLVM_ASSERT(0 && "Unknown name kind.");
  return DeclarationNameInfo();
}
...
TreeTransform<Derived>::TransformTemplateName(TemplateName Name,
  ...
  if (QualifiedTemplateName *QTN = Name.getAsQualifiedTemplateName()) {
    ...
    LLVM_ASSERT(false && "overloaded template name survived to here");
  }
  ...
  LLVM_ASSERT(false && "overloaded function decl survived to here");
  return TemplateName();
}
...
TreeTransform<Derived>::TransformSwitchCase(SwitchCase *S) {
  LLVM_ASSERT(false && "SwitchCase is abstract and cannot be transformed");
  return SemaRef.Owned(S->Retain());
}
...
TreeTransform<Derived>::TransformDesignatedInitExpr(DesignatedInitExpr *E) {
  ...
  for (DesignatedInitExpr::designators_iterator D = E->designators_begin(),
    ...
    LLVM_ASSERT(D->isArrayRangeDesignator() && "New kind of designator?");
    ExprResult Start
...
TreeTransform<Derived>::TransformCXXOperatorCallExpr(CXXOperatorCallExpr *E) {
  ...
  case OO_Call: {
    ...
    LLVM_ASSERT(E->getNumArgs() >= 1 && "Object call is missing arguments");
...
TreeTransform<Derived>::TransformObjCMessageExpr(ObjCMessageExpr *E) {
  ...
  LLVM_ASSERT(E->getReceiverKind() == ObjCMessageExpr::Instance &&
...
QualType TreeTransform<Derived>::RebuildUnresolvedUsingType(Decl *D) {
  ...
  if (isa<UsingDecl>(D)) {
    ...
    LLVM_ASSERT(Using->isTypeName() &&
           "UnresolvedUsingTypenameDecl transformed to non-typename using");

    // A valid resolved using typename decl points to exactly one type decl.
    LLVM_ASSERT(++Using->shadow_begin() == Using->shadow_end());
    Ty = cast<TypeDecl>((*Using->shadow_begin())->getTargetDecl());
    
  } else {
    LLVM_ASSERT(isa<UnresolvedUsingTypenameDecl>(D) &&
...
TreeTransform<Derived>::RebuildNestedNameSpecifier(NestedNameSpecifier *Prefix,
  ...
  if (T->isDependentType() || T->isRecordType() ||
    ...
    LLVM_ASSERT(!T.hasLocalQualifiers() && "Can't get cv-qualifiers here");
