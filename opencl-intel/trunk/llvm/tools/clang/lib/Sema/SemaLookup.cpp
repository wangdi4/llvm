...
namespace {
  ...
  public:
    ...
    void visitScopeChain(Scope *S, Scope *InnermostFileScope) {
      ...
      LLVM_ASSERT(InnermostFileDC && InnermostFileDC->isFileContext());
...
static inline unsigned getIDNS(Sema::LookupNameKind NameKind,
  ...
  case Sema::LookupOperatorName:
    ...
    LLVM_ASSERT(!Redeclaration && "cannot do redeclaration operator lookup");
...
void LookupResult::sanity() const {
  LLVM_ASSERT(ResultKind != NotFound || Decls.size() == 0);
  LLVM_ASSERT(ResultKind != Found || Decls.size() == 1);
  LLVM_ASSERT(ResultKind != FoundOverloaded || Decls.size() > 1 ||
         (Decls.size() == 1 &&
          isa<FunctionTemplateDecl>((*begin())->getUnderlyingDecl())));
  LLVM_ASSERT(ResultKind != FoundUnresolvedValue || sanityCheckUnresolved());
  LLVM_ASSERT(ResultKind != Ambiguous || Decls.size() > 1 ||
         (Decls.size() == 1 && Ambiguity == AmbiguousBaseSubobjects));
  LLVM_ASSERT((Paths != NULL) == (ResultKind == Ambiguous &&
...
void LookupResult::resolveKind() {
  ...
  if (N == 0) {
    LLVM_ASSERT(ResultKind == NotFound || ResultKind == NotFoundInCurrentInstantiation);
    return;
  }
...
static bool LookupDirect(Sema &S, LookupResult &R, const DeclContext *DC) {
  ...
  for (UnresolvedSetImpl::iterator U = Unresolved->begin(), 
    ...
    LLVM_ASSERT(ConvProto && "Nonsensical conversion function template type");
...
CppNamespaceLookup(Sema &S, LookupResult &R, ASTContext &Context, 
  ...
  LLVM_ASSERT(NS && NS->isFileContext() && "CppNamespaceLookup() requires namespace!");
...
bool Sema::CppLookupName(LookupResult &R, Scope *S) {
  LLVM_ASSERT(getLangOptions().CPlusPlus && "Can perform only C++ lookup");
...
static bool LookupQualifiedNameInUsingDirectives(Sema &S, LookupResult &R,
                                                 DeclContext *StartDC) {
  LLVM_ASSERT(StartDC->isFileContext() && "start context is not a file context");
...
bool Sema::LookupQualifiedName(LookupResult &R, DeclContext *LookupCtx,
  ...
  LLVM_ASSERT(LookupCtx && "Sema::LookupQualifiedName requires a lookup context");

  if (!R.getLookupName())
    return false;

  // Make sure that the declaration context is complete.
  LLVM_ASSERT((!isa<TagDecl>(LookupCtx) ||
      ...
      if (isa<CXXMethodDecl>(FirstDecl)) {
        ...
        for (DeclContext::lookup_iterator Func = Path->Decls.first;
          ...
          if (!isa<CXXMethodDecl>(*Func)) {
            LLVM_ASSERT(isa<TagDecl>(*Func) && "Non-function must be a tag decl");
            break;
          }
...
bool Sema::DiagnoseAmbiguousLookup(LookupResult &Result) {
  LLVM_ASSERT(Result.isAmbiguous() && "Lookup result must be ambiguous");
...
void Sema::LookupOverloadedOperatorName(OverloadedOperatorKind Op, Scope *S,
  ...
  LLVM_ASSERT(!Operators.isAmbiguous() && "Operator lookup cannot be ambiguous");
