...
void UnqualifiedId::setTemplateId(TemplateIdAnnotation *TemplateId) {
  LLVM_ASSERT(TemplateId && "NULL template-id annotation?");
  Kind = IK_TemplateId;
...
bool DeclSpec::SetStorageClassSpec(SCS S, SourceLocation Loc,
  ...
  LLVM_ASSERT((unsigned)S == StorageClassSpec && "SCS constants overflow bitfield");
  return false;
}
...
bool DeclSpec::SetTypeSpecType(TST T, SourceLocation Loc,
                               ...
                               unsigned &DiagID,
                               ParsedType Rep) {
  LLVM_ASSERT(isTypeRep(T) && "T does not store a type");
  LLVM_ASSERT(Rep && "no type provided!");
...
bool DeclSpec::SetTypeSpecType(TST T, SourceLocation Loc,
                               ...
                               unsigned &DiagID,
                               Expr *Rep) {
  LLVM_ASSERT(isExprRep(T) && "T does not store an expr");
  LLVM_ASSERT(Rep && "no expression provided!");
  if (TypeSpecType != TST_unspecified) {
    PrevSpec = DeclSpec::getSpecifierName((TST) TypeSpecType);
    DiagID = diag::err_invalid_decl_spec_combination;
    return true;
...
bool DeclSpec::SetTypeSpecType(TST T, SourceLocation Loc,
                               ...
                               unsigned &DiagID,
                               Decl *Rep, bool Owned) {
  LLVM_ASSERT(isDeclRep(T) && "T does not store a decl");
...
bool DeclSpec::SetTypeSpecType(TST T, SourceLocation Loc,
                               const char *&PrevSpec,
                               unsigned &DiagID) {
  LLVM_ASSERT(!isDeclRep(T) && !isTypeRep(T) && !isExprRep(T) &&
...
bool DeclSpec::SetTypeQual(TQ T, SourceLocation Loc, const char *&PrevSpec,
  ...
  switch (T) {
  default: LLVM_ASSERT(0 && "Unknown type qualifier!");
...
void DeclSpec::Finish(Diagnostic &D, Preprocessor &PP) {
  ...
  case TSW_longlong: // long long int
	//long long type is not allowed in OpenCL
	if ((TypeSpecWidth == TSW_longlong) && PP.getLangOptions().OpenCL){
	  Diag(D, TSWLoc, SrcMgr, diag::err_invalid_longlong_in_opencl);
	  break;
	}
    if (TypeSpecType == TST_unspecified)
  ...
  LLVM_ASSERT(!TypeSpecOwned || isDeclRep((TST) TypeSpecType));
