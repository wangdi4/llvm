...
ParsedType Sema::getTypeName(IdentifierInfo &II, SourceLocation NameLoc,
  ...
  LLVM_ASSERT(IIDecl && "Didn't find decl");
...
void Sema::PushDeclContext(Scope *S, DeclContext *DC) {
  LLVM_ASSERT(getContainingDC(DC) == CurContext &&
...
void Sema::PopDeclContext() {
  LLVM_ASSERT(CurContext && "DeclContext imbalance!");

  CurContext = getContainingDC(CurContext);
  LLVM_ASSERT(CurContext && "Popped translation unit!");
}
...
void Sema::EnterDeclaratorContext(Scope *S, DeclContext *DC) {
  ...
  LLVM_ASSERT(!S->getEntity() && "scope already has entity");

#ifndef NDEBUG
  Scope *Ancestor = S->getParent();
  while (!Ancestor->getEntity()) Ancestor = Ancestor->getParent();
  LLVM_ASSERT(Ancestor->getEntity() == CurContext && "ancestor context mismatch");
#endif
...
void Sema::ExitDeclaratorContext(Scope *S) {
  LLVM_ASSERT(S->getEntity() == CurContext && "Context imbalance!");
...
void Sema::ActOnPopScope(SourceLocation Loc, Scope *S) {
  ...
  LLVM_ASSERT((S->getFlags() & (Scope::DeclScope | Scope::TemplateParamScope)) &&
         "Scope shouldn't contain decls!");

  for (Scope::decl_iterator I = S->decl_begin(), E = S->decl_end();
       I != E; ++I) {
    Decl *TmpD = (*I);
    LLVM_ASSERT(TmpD && "This decl didn't get pushed??");

    LLVM_ASSERT(isa<NamedDecl>(TmpD) && "Decl isn't NamedDecl?");
...
Sema::CXXSpecialMember Sema::getSpecialMember(const CXXMethodDecl *MD) {
  ...
  LLVM_ASSERT(MD->isCopyAssignment() && "Must have copy assignment operator");
...
bool Sema::MergeFunctionDecl(FunctionDecl *New, Decl *OldD) {
  ...
  if (OldType->getNoReturnAttr() && !NewType->getNoReturnAttr()) {
    ...
    New->setType(NewQType);
    LLVM_ASSERT(NewQType.isCanonical());
  }
  ...
  if (!getLangOptions().CPlusPlus &&
    ...
    if (isa<FunctionNoProtoType>(NewFuncType) &&
      ...
      LLVM_ASSERT(!OldProto->hasExceptionSpec() && "Exception spec in C");
...
Decl *Sema::BuildAnonymousStructOrUnion(Scope *S, DeclSpec &DS,
  ...
  if (getLangOptions().CPlusPlus) {
    ...
    for (DeclContext::decl_iterator Mem = Record->decls_begin(),
      ...
      if (FieldDecl *FD = dyn_cast<FieldDecl>(*Mem)) {
        ...
        //   members (clause 11).
        LLVM_ASSERT(FD->getAccess() != AS_none);
  ...
  LLVM_ASSERT(TInfo && "couldn't build declarator info for anonymous struct/union");
  ...
  if (RecordDecl *OwningClass = dyn_cast<RecordDecl>(Owner)) {
    ...
    LLVM_ASSERT(SCSpec != DeclSpec::SCS_typedef &&
...
Sema::GetNameFromUnqualifiedId(const UnqualifiedId &Name) {
  ...
  LLVM_ASSERT(false && "Unknown name kind");
  return DeclarationNameInfo();
}
...
Sema::RegisterLocallyScopedExternCDecl(NamedDecl *ND,
                                       ...
                                       Scope *S) {
  LLVM_ASSERT(ND->getLexicalDeclContext()->isFunctionOrMethod() &&
...
Sema::ActOnVariableDeclarator(Scope* S, Declarator& D, DeclContext* DC,
                              ...
                              MultiTemplateParamsArg TemplateParamLists,
                              bool &Redeclaration) {
  if (getLangOptions().OpenCL && !getLangOptions().OpenCLBuiltins && D.getContext() == Declarator::FileContext) {
    const std::string typeName = QualType(R.getTypePtr(), 0).getAsString();
    if (typeName != "sampler_t" && R.getAddressSpace() != 2) {
        Diag(D.getIdentifierLoc(), diag::err_program_scope_variables_must_be_declared_in_constant_address_spce);
        D.setInvalidType();
    } else {
        if (typeName == "sampler_t" && !R.isConstQualified() && R.getAddressSpace() != 2) {
            Diag(D.getIdentifierLoc(), diag::err_sampler_t_variables_must_be_constant_or_const);
            D.setInvalidType();
        }
    }
  }

  //image types and sampler_t types for local variables,
  //are not allowed in OpenCL 
  std::string str = R.getAsString();
  size_t space = str.find(" ");
  str = str.substr(0, space);
  if ((getLangOptions().OpenCL) && (3 == R.getAddressSpace()) && 
	  (IsOpenCLImageType(str) || "sampler_t" == str)) {
	  Diag(D.getIdentifierLoc(), diag::err_invalid_local_variable_in_opencl);
	  D.setInvalidType();
  }
  //sampler_t arrays and pointers are not allowed in OpenCL 
  if ((getLangOptions().OpenCL) && "sampler_t" == str && 
	  (R->isPointerType() || (R->isArrayType()))) {
	  Diag(D.getIdentifierLoc(), diag::err_invalid_sampler_type_in_opencl);
	  D.setInvalidType();
  }
  ...
  LLVM_ASSERT(SCSpec != DeclSpec::SCS_typedef &&
         "Parser allowed 'typedef' as storage class VarDecl.");
  VarDecl::StorageClass SC = StorageClassSpecToVarDeclStorageClass(SCSpec);
  //extern, static, auto and register storage are not allowed in OpenCL
  if ((getLangOptions().OpenCL) && (SC == SC_Auto || SC == SC_Register || 
      SC == SC_Extern || SC == SC_Static)) {
	Diag(D.getIdentifierLoc(), diag::err_invalid_storage_class_in_opencl);
    D.setInvalidType();
    SC = SC_None;
  }
  if (SCSpec == DeclSpec::SCS_mutable) {
...
void Sema::CheckVariableDeclaration(VarDecl *NewVD,
  ...
  if ( getLangOptions().OpenCL ) {
	  if(T->isVariableArrayType()) {
	    Diag(NewVD->getLocation(), diag::err_no_val_decl_in_opencl);
	    return NewVD->setInvalidDecl();
	  }

    std::string type = T.getAsString();

    if(!LangOpts.OpenCLBuiltins && type == "half") {
	    Diag(NewVD->getLocation(), diag::err_half_variables);
	    return NewVD->setInvalidDecl();
    }
  }
  ...
  //
  // OpenCL local, global and constant address spaces are allowed 
  if (NewVD->hasLocalStorage() && (T.getAddressSpace() != 0) && (!getLangOptions().OpenCL)) {
  ...
  if (getLangOptions().OpenCL && !LangOpts.OpenCLBuiltins) {
    if (!Pointee.isNull() && Pointee->isFunctionProtoType()) {
      Diag(NewVD->getLocation(), diag::err_functions_pointers_not_allowed_in_openCL)
        << PtrOrRef;
      return NewVD->setInvalidDecl();
    }
  }
...
Sema::ActOnFunctionDeclarator(Scope* S, Declarator& D, DeclContext* DC,
  ...
  LLVM_ASSERT(R.getTypePtr()->isFunctionType());

  // TODO: consider using NameInfo for diagnostic.
  DeclarationNameInfo NameInfo = GetNameForDeclarator(D);
  DeclarationName Name = NameInfo.getName();
  FunctionDecl::StorageClass SC = SC_None;
  //samplers cannot be used as a return value in OpenCL
  std::string str = R.getAsString();
  size_t space = str.find(" ");
  str = str.substr(0, space);
  if(getLangOptions().OpenCL && "sampler_t" == str) {
    Diag(D.getIdentifierLoc(),
	diag::err_invalid_sampler_rtype_in_opencl);
	D.setInvalidType();
  }

  switch (D.getDeclSpec().getStorageClassSpec()) {
  default: LLVM_ASSERT(0 && "Unknown storage class!");
  ...
  case DeclSpec::SCS_extern: 
	if(getLangOptions().OpenCL) {
      Diag(D.getDeclSpec().getStorageClassSpecLoc(),
      diag::err_invalid_storage_class_in_opencl);
      D.setInvalidType();
	  break;
	}
	SC = SC_Extern; break;
  ...
  if (isFriend) {
    ...
    LLVM_ASSERT((DC->isFileContext() || !Previous.empty()) &&
  ...
  if (D.getNumTypeObjects() > 0) {
	...
	//variadic functions are not allowed in OpenCL
	if(FTI.isVariadic && getLangOptions().OpenCL && 
    (NewFD->getName().find("printf") == llvm::StringRef::npos)) {
	  Diag(FTI.getEllipsisLoc(), diag::err_no_vararg_func_in_opencl);
	}
    ...
    if (FTI.NumArgs == 1 && !FTI.isVariadic && FTI.ArgInfo[0].Ident == 0 &&
      ...
      for (unsigned i = 0, e = FTI.NumArgs; i != e; ++i) {
        ...
        LLVM_ASSERT(Param->getDeclContext() != NewFD && "Was set before ?");
    ...
    LLVM_ASSERT(R->isFunctionNoProtoType() && NewFD->getNumParams() == 0 &&
  ...
  if (isFunctionTemplateSpecialization && isFriend &&
    ...
    LLVM_ASSERT(HasExplicitTemplateArgs &&
  ...
  LLVM_ASSERT((NewFD->isInvalidDecl() || !Redeclaration ||
  ...
  if (!Previous.empty()) {
    ...
    if (Redeclaration) {
      ...
      if (FunctionTemplateDecl *OldTemplateDecl
        ...
        LLVM_ASSERT(NewTemplateDecl && "Template/non-template mismatch");
        ...
        if (IsExplicitSpecialization && 
          ...
          LLVM_ASSERT(OldTemplateDecl->isMemberSpecialization());
        }
      } else {
...
void Sema::CheckMain(FunctionDecl* FD) {
  ...
  LLVM_ASSERT(T->isFunctionType() && "function decl is not of function type");
  ...
  LLVM_ASSERT(FD->getNumParams() == nparams);
...
#if 0   // the kernel in "test_basic kernel_memory_alignment_constant" has uninitialized constant variables
    if (2 == Var->getType().getAddressSpace() && NULL == Var->getInit()) {
        Diag(Var->getLocation(), diag::err_constant_vars_must_be_initialized);
    }
#endif
...
Decl *Sema::ActOnParamDeclarator(Scope *S, Declarator &D) {
  ...
  if (DS.getStorageClassSpec() == DeclSpec::SCS_register) {
	//register storage class specifier is not allowed in OpenCL
	if(getLangOptions().OpenCL) {
	  Diag(DS.getStorageClassSpecLoc(),
        diag::err_invalid_storage_class_in_OCL_func_decl);
	} else {
...
Decl *Sema::ActOnStartOfFunctionDef(Scope *FnBodyScope,
                                         Declarator &D) {
  LLVM_ASSERT(getCurFunctionDecl() == 0 && "Function parsing confused");
  LLVM_ASSERT(D.getTypeObject(0).Kind == DeclaratorChunk::Function &&
         "Not a function declarator!");
  DeclaratorChunk::FunctionTypeInfo &FTI = D.getTypeObject(0).Fun;
...
Decl *Sema::ActOnFinishFunctionBody(Decl *dcl, Stmt *Body,
  ...
  if (FD) {
    ...
    LLVM_ASSERT(FD == getCurFunctionDecl() && "Function parsing confused");
  } else if (ObjCMethodDecl *MD = dyn_cast_or_null<ObjCMethodDecl>(dcl)) {
    LLVM_ASSERT(MD == getCurMethodDecl() && "Method parsing confused");
    MD->setBody(Body);
  ...
  if (Body) {
    ...
    LLVM_ASSERT(ExprTemporaries.empty() && "Leftover temporaries in function");
  }
...
NamedDecl *Sema::ImplicitlyDefineFunction(SourceLocation Loc,
  ...
  LLVM_ASSERT(!Error && "Error setting up implicit decl!");
...
TypedefDecl *Sema::ParseTypedefDecl(Scope *S, Declarator &D, QualType T,
                                    TypeSourceInfo *TInfo) {
  LLVM_ASSERT(D.getIdentifier() && "Wrong callback for declspec without declarator");
  LLVM_ASSERT(!T.isNull() && "GetTypeForDeclarator() returned null type");

  if (!TInfo) {
    LLVM_ASSERT(D.isInvalidType() && "no declarator info for valid type");
...
Decl *Sema::ActOnTag(Scope *S, unsigned TagSpec, TagUseKind TUK,
  ...
  LLVM_ASSERT((Name != 0 || TUK == TUK_Definition) &&
  ...
  if (!Previous.empty()) {
    ...
    if (TagDecl *PrevTagDecl = dyn_cast<TagDecl>(PrevDecl)) {
      ...
      if (TUK == TUK_Reference || TUK == TUK_Friend ||
        ...
        if (!Invalid) {
          ...
          // Diagnose attempts to redefine a tag. Ignore '__va_list_tag' built-in (Guy)
          if (TUK == TUK_Definition && (Name->getName().compare("__va_list_tag"))) {
    ...
    // things like templates still live in the tag namespace.
      ...
      LLVM_ASSERT(getLangOptions().CPlusPlus);
...
void Sema::ActOnStartCXXMemberDeclarations(Scope *S, Decl *TagD,
  ...
  LLVM_ASSERT(InjectedClassName->isInjectedClassName() &&
...
void Sema::ActOnTagFinishDefinition(Scope *S, Decl *TagD,
  ...
  if (getLangOptions().OpenCL) {
    if (Tag->isStruct() || Tag->isUnion()) {
      unsigned adressSpace = 0;
      DeclContext::decl_iterator i, e;
      for (i = Tag->decls_begin(), e =  Tag->decls_end(); i != e; i++) {
        DeclaratorDecl* DD = dyn_cast<DeclaratorDecl>(*i);
        if (!DD)
          break;

        if (IsOpenCLImageType(DD->getType().getAsString())) {
          Diag(DD->getLocation(), diag::err_image_as_struct_field);
        }

        if (adressSpace != 0) {
          if (adressSpace != DD->getType().getAddressSpace()) {
            Diag(DD->getLocation(), diag::err_struct_fields_must_have_same_address_space);
          }
        }

        adressSpace = DD->getType().getAddressSpace();
      }
    }
  }
...
bool Sema::VerifyBitField(SourceLocation FieldLoc, IdentifierInfo *FieldName,
  ...
  // OpenCL doesn't support bitfields
  if(getLangOptions().OpenCL) {
    return Diag(FieldLoc, diag::err_no_bitfields_in_opencl);
  }
...
FieldDecl *Sema::HandleField(Scope *S, RecordDecl *Record,
  ...
  LookupName(Previous, S);
  LLVM_ASSERT((Previous.empty() || Previous.isOverloadedResult() || 
...
bool Sema::CheckNontrivialField(FieldDecl *FD) {
  LLVM_ASSERT(FD);
  LLVM_ASSERT(getLangOptions().CPlusPlus && "valid check only for C++");
...
void Sema::DiagnoseNontrivial(const RecordType* T, CXXSpecialMember member) {
  ...
  case CXXConstructor:
    if (RD->hasUserDeclaredConstructor()) {
      ...
      LLVM_ASSERT(0 && "found no user-declared constructors");
      return;
    }
    break;
  ...
  default:
    LLVM_ASSERT(0 && "unexpected special member"); return;
  }

  // Check for nontrivial bases (and recurse).
  for (base_iter bi = RD->bases_begin(), be = RD->bases_end(); bi != be; ++bi) {
    const RecordType *BaseRT = bi->getType()->getAs<RecordType>();
    LLVM_ASSERT(BaseRT && "Don't know how to handle dependent bases");
  ...
  LLVM_ASSERT(0 && "found no explanation for non-trivial member");
}
...
TranslateIvarVisibility(tok::ObjCKeywordKind ivarVisibility) {
  ...
  default: LLVM_ASSERT(0 && "Unknown visitibility kind");
...
Decl *Sema::ActOnIvar(Scope *S,
  ...
  if (ObjCImplementationDecl *IMPDecl =
    ...
    // Case of ivar declared in an implementation. Context is that of its class.
      ...
      LLVM_ASSERT(EnclosingContext && "Implementation has no class interface!");
    }
    else
...
void Sema::ActOnFields(Scope* S,
                       ...
                       AttributeList *Attr) {
  LLVM_ASSERT(EnclosingDecl && "missing record or interface decl");
    ...
    if (ObjCInterfaceDecl *ID = dyn_cast<ObjCInterfaceDecl>(EnclosingDecl)) {
      ...
      if (ID->getSuperClass())
        DiagnoseDuplicateIvars(ID, ID->getSuperClass());
                  ...
                  dyn_cast<ObjCImplementationDecl>(EnclosingDecl)) {
      LLVM_ASSERT(IMPDecl && "ActOnFields - missing ObjCImplementationDecl");
...
static bool isRepresentableIntegerValue(ASTContext &Context,
                                        llvm::APSInt &Value,
                                        QualType T) {
  LLVM_ASSERT(T->isIntegralType(Context) && "Integral type required!");
...
Decl *Sema::ActOnEnumConstant(Scope *S, Decl *theEnumDecl,
  ...
  if (PrevDecl) {
    ...
    LLVM_ASSERT((getLangOptions().CPlusPlus || !isa<TagDecl>(PrevDecl)) &&
...
void Sema::ActOnEnumBody(SourceLocation EnumLoc, SourceLocation LBraceLoc,
  ...
  if (NumNegativeBits) {
    ...
    if (Packed && NumPositiveBits <= CharWidth) {
      ...
      BestWidth = Context.Target.getLongLongWidth();
      LLVM_ASSERT(NumPositiveBits <= BestWidth &&
