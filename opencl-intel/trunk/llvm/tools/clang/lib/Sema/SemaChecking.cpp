...
SourceLocation Sema::getLocationOfStringLiteralByte(const StringLiteral *SL,
  ...
  LLVM_ASSERT(!SL->isWide() && "This doesn't work for wide strings yet");

  // Loop over all of the tokens in this string until we find the one that
  // contains the byte we're looking for.
  unsigned TokNo = 0;
  while (1) {
    LLVM_ASSERT(TokNo < SL->getNumConcatenated() && "Invalid byte number!");
...
Sema::CheckBuiltinFunctionCall(unsigned BuiltinID, CallExpr *TheCall) {
  ...
  case Builtin::BI__builtin___CFStringMakeConstantString:
    LLVM_ASSERT(TheCall->getNumArgs() == 1 &&
  ...
  if (BuiltinID >= Builtin::BI__builtin_as_char &&
    ...
    if (getLangOptions().OpenCL) {
      if (SemaBuiltinAsTypeN(BuiltinID, TheCallResult))
        return ExprError();
    } else {
      Diag(TheCall->getLocStart(), diag::err_as_typen_no_opencl);
      return ExprError();
    }
  }
...
static unsigned RFT(unsigned t, bool shift = false) {
  ...
  switch (t & 0x7) {
    ...
    case 4: // f32
      LLVM_ASSERT(!shift && "cannot shift float types!");
      return (2 << (int)quad) - 1;
    case 5: // poly8
      LLVM_ASSERT(!shift && "cannot shift polynomial types!");
      return (8 << (int)quad) - 1;
    case 6: // poly16
      LLVM_ASSERT(!shift && "cannot shift polynomial types!");
      return (4 << (int)quad) - 1;
...
#undef BUILTIN_ROW
  ...
  switch (BuiltinID) {
  default: LLVM_ASSERT(0 && "Unknown overloaded atomic builtin!");
...
bool Sema::SemaBuiltinAsTypeN(unsigned BuiltinID, ExprResult TheCallResult)
{  
  CallExpr *TheCall = (CallExpr *)TheCallResult.get();

  if (TheCall->getNumArgs() != 1) {
    Diag(TheCall->getLocStart(), diag::err_as_typen_wrong_number_of_args);
    return true;
  }

  QualType ReturnTypeElement;
  unsigned ReturnTypeDimension = 0;

  // Select the appropriate return type and dimension from the builtin's
  // name.
#define AS_TYPEN_SELECT(builtin_name, returntype, returndimension) \
  case Builtin::BI##builtin_name:\
    ReturnTypeElement = returntype;\
    ReturnTypeDimension = returndimension;\
    break;

  switch (BuiltinID) {
      AS_TYPEN_SELECT(__builtin_as_char, Context.CharTy, 1)
      AS_TYPEN_SELECT(__builtin_as_char2, Context.CharTy, 2)
      AS_TYPEN_SELECT(__builtin_as_char3, Context.CharTy, 3)
      AS_TYPEN_SELECT(__builtin_as_char4, Context.CharTy, 4)
      AS_TYPEN_SELECT(__builtin_as_char8, Context.CharTy, 8)
      AS_TYPEN_SELECT(__builtin_as_char16, Context.CharTy, 16)
      AS_TYPEN_SELECT(__builtin_as_uchar, Context.UnsignedCharTy, 1)
      AS_TYPEN_SELECT(__builtin_as_uchar2, Context.UnsignedCharTy, 2)
      AS_TYPEN_SELECT(__builtin_as_uchar3, Context.UnsignedCharTy, 3)
      AS_TYPEN_SELECT(__builtin_as_uchar4, Context.UnsignedCharTy, 4)
      AS_TYPEN_SELECT(__builtin_as_uchar8, Context.UnsignedCharTy, 8)
      AS_TYPEN_SELECT(__builtin_as_uchar16, Context.UnsignedCharTy, 16)
      AS_TYPEN_SELECT(__builtin_as_short, Context.ShortTy, 1)
      AS_TYPEN_SELECT(__builtin_as_short2, Context.ShortTy, 2)
      AS_TYPEN_SELECT(__builtin_as_short3, Context.ShortTy, 3)
      AS_TYPEN_SELECT(__builtin_as_short4, Context.ShortTy, 4)
      AS_TYPEN_SELECT(__builtin_as_short8, Context.ShortTy, 8)
      AS_TYPEN_SELECT(__builtin_as_short16, Context.ShortTy, 16)
      AS_TYPEN_SELECT(__builtin_as_ushort, Context.UnsignedShortTy, 1)
      AS_TYPEN_SELECT(__builtin_as_ushort2, Context.UnsignedShortTy, 2)
      AS_TYPEN_SELECT(__builtin_as_ushort3, Context.UnsignedShortTy, 3)
      AS_TYPEN_SELECT(__builtin_as_ushort4, Context.UnsignedShortTy, 4)
      AS_TYPEN_SELECT(__builtin_as_ushort8, Context.UnsignedShortTy, 8)
      AS_TYPEN_SELECT(__builtin_as_ushort16, Context.UnsignedShortTy, 16)
      AS_TYPEN_SELECT(__builtin_as_int, Context.IntTy, 1)
      AS_TYPEN_SELECT(__builtin_as_int2, Context.IntTy, 2)
      AS_TYPEN_SELECT(__builtin_as_int3, Context.IntTy, 3)
      AS_TYPEN_SELECT(__builtin_as_int4, Context.IntTy, 4)
      AS_TYPEN_SELECT(__builtin_as_int8, Context.IntTy, 8)
      AS_TYPEN_SELECT(__builtin_as_int16, Context.IntTy, 16)
      AS_TYPEN_SELECT(__builtin_as_uint, Context.UnsignedIntTy, 1)
      AS_TYPEN_SELECT(__builtin_as_uint2, Context.UnsignedIntTy, 2)
      AS_TYPEN_SELECT(__builtin_as_uint3, Context.UnsignedIntTy, 3)
      AS_TYPEN_SELECT(__builtin_as_uint4, Context.UnsignedIntTy, 4)
      AS_TYPEN_SELECT(__builtin_as_uint8, Context.UnsignedIntTy, 8)
      AS_TYPEN_SELECT(__builtin_as_uint16, Context.UnsignedIntTy, 16)
      AS_TYPEN_SELECT(__builtin_as_long, Context.LongTy, 1)
      AS_TYPEN_SELECT(__builtin_as_long2, Context.LongTy, 2)
      AS_TYPEN_SELECT(__builtin_as_long3, Context.LongTy, 3)
      AS_TYPEN_SELECT(__builtin_as_long4, Context.LongTy, 4)
      AS_TYPEN_SELECT(__builtin_as_long8, Context.LongTy, 8)
      AS_TYPEN_SELECT(__builtin_as_long16, Context.LongTy, 16)
      AS_TYPEN_SELECT(__builtin_as_ulong, Context.UnsignedLongTy, 1)
      AS_TYPEN_SELECT(__builtin_as_ulong2, Context.UnsignedLongTy, 2)
      AS_TYPEN_SELECT(__builtin_as_ulong3, Context.UnsignedLongTy, 3)
      AS_TYPEN_SELECT(__builtin_as_ulong4, Context.UnsignedLongTy, 4)
      AS_TYPEN_SELECT(__builtin_as_ulong8, Context.UnsignedLongTy, 8)
      AS_TYPEN_SELECT(__builtin_as_ulong16, Context.UnsignedLongTy, 16)
      AS_TYPEN_SELECT(__builtin_as_float, Context.FloatTy, 1)
      AS_TYPEN_SELECT(__builtin_as_float2, Context.FloatTy, 2)
      AS_TYPEN_SELECT(__builtin_as_float3, Context.FloatTy, 3)
      AS_TYPEN_SELECT(__builtin_as_float4, Context.FloatTy, 4)
      AS_TYPEN_SELECT(__builtin_as_float8, Context.FloatTy, 8)
      AS_TYPEN_SELECT(__builtin_as_float16, Context.FloatTy, 16)
      AS_TYPEN_SELECT(__builtin_as_double, Context.DoubleTy, 1)
      AS_TYPEN_SELECT(__builtin_as_double2, Context.DoubleTy, 2)
      AS_TYPEN_SELECT(__builtin_as_double3, Context.DoubleTy, 3)
      AS_TYPEN_SELECT(__builtin_as_double4, Context.DoubleTy, 4)
      AS_TYPEN_SELECT(__builtin_as_double8, Context.DoubleTy, 8)
      AS_TYPEN_SELECT(__builtin_as_double16, Context.DoubleTy, 16)
      default: assert(0 && "Invalid case in switch");
  }

  QualType ReturnType;
  if (ReturnTypeDimension == 1) {
    ReturnType = ReturnTypeElement;
  } else {
    ReturnType = Context.getExtVectorType(ReturnTypeElement, 
                                          ReturnTypeDimension);
  }
  uint64_t ReturnSize = Context.getTypeSize(ReturnType);

  QualType OperandType = TheCall->getArg(0)->getType();
  uint64_t OperandSize = Context.getTypeSize(OperandType);

  // OpenCL disallows conversions between types of different sizes
  if (ReturnSize != OperandSize) {
    Diag(TheCall->getArg(0)->getLocStart(), diag::err_as_typen_size_mismatch);
    return true;
  }

  // Change the return type of the call to match the specific builtin
  // used, in order to succeed in consequent type checking.
...
bool Sema::SemaBuiltinFPClassification(CallExpr *TheCall, unsigned NumArgs) {
  ...
  if (ImplicitCastExpr *Cast = dyn_cast<ImplicitCastExpr>(OrigArg)) {
    ...
    if (CastArg->getType()->isSpecificBuiltinType(BuiltinType::Float)) {
      LLVM_ASSERT(Cast->getType()->isSpecificBuiltinType(BuiltinType::Double) &&
...
void CheckFormatHandler::DoneProcessing() {
  ...
  if (!HasVAListArg) {
    ...
    if (notCoveredArg >= 0) {
      LLVM_ASSERT((unsigned)notCoveredArg < NumDataArgs);
...
static DeclRefExpr* EvalAddr(Expr *E) {
  ...
  LLVM_ASSERT((E->getType()->isAnyPointerType() ||
...
struct IntRange {
  ...
  static IntRange forCanonicalType(ASTContext &C, const Type *T) {
    LLVM_ASSERT(T->isCanonicalUnqualified());
...
bool IsSameFloatAfterCast(const APValue &value,
  ...
  LLVM_ASSERT(value.isComplexFloat());
...
void AnalyzeComparison(Sema &S, BinaryOperator *E) {
  ...
  LLVM_ASSERT(S.Context.hasSameUnqualifiedType(T, E->getRHS()->getType())
  ...
  if (lex->getType()->hasSignedIntegerRepresentation()) {
    LLVM_ASSERT(!rex->getType()->hasSignedIntegerRepresentation() &&
  ...
  if (E->isEqualityOp()) {
    ...
    // non-negative.
    LLVM_ASSERT(unsignedRange.NonNegative && "unsigned range includes negative?");
...
bool Sema::IsOpenCLImageType(const std::string& str)
{
    return str == "image2d_t" || str == "image3d_t" || str == "image2d_array_t";
}
