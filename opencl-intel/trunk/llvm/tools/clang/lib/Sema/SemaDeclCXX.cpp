...
bool Sema::isCurrentClassName(const IdentifierInfo &II, Scope *,
  ...
  LLVM_ASSERT(getLangOptions().CPlusPlus && "No class names in C!");
...
Sema::CheckBaseSpecifier(CXXRecordDecl *Class,
  ...
  LLVM_ASSERT(BaseDecl && "Record type has no declaration");
  BaseDecl = BaseDecl->getDefinition();
  LLVM_ASSERT(BaseDecl && "Base type is not incomplete, but has no definition");
  CXXRecordDecl * CXXBaseDecl = cast<CXXRecordDecl>(BaseDecl);
  LLVM_ASSERT(CXXBaseDecl && "Base type is not a C++ type");
...
void Sema::BuildBasePathArray(const CXXBasePaths &Paths, 
  ...
  LLVM_ASSERT(BasePathArray.empty() && "Base path array must be empty!");
  LLVM_ASSERT(Paths.isRecordingPaths() && "Must record paths!");
...
Sema::CheckDerivedToBaseConversion(QualType Derived, QualType Base,
  ...
  LLVM_ASSERT(StillOkay && "Can only be used with a derived-to-base conversion");
  (void)StillOkay;
...
Decl *Sema::ActOnAccessSpecifier(AccessSpecifier Access,
  ...
  LLVM_ASSERT(Access != AS_none && "Invalid kind for syntactic access specifier!");
...
Sema::ActOnCXXMemberDeclarator(Scope *S, AccessSpecifier AS, Declarator &D,
  ...
  LLVM_ASSERT(isa<CXXRecordDecl>(CurContext));
  LLVM_ASSERT(!DS.isFriendSpecified());
  ...
  if (isInstField) {
    ...
    Member = HandleField(S, cast<CXXRecordDecl>(CurContext), Loc, D, BitWidth,
                         AS);
    LLVM_ASSERT(Member && "HandleField never returns null");
  } else {
  ...
  LLVM_ASSERT((Name || isInstField) && "No identifier for non-field ?");
...
BuildImplicitBaseInitializer(Sema &SemaRef, CXXConstructorDecl *Constructor,
  ...
  case IIK_Move:
    LLVM_ASSERT(false && "Unhandled initializer kind!");
  }
...
BuildImplicitMemberInitializer(Sema &SemaRef, CXXConstructorDecl *Constructor,
  ...
  if (ImplicitInitKind == IIK_Copy) {
    ...
    while (const ConstantArrayType *Array
      ...
      LLVM_ASSERT(!IterationVarRef.isInvalid() &&
  ...
  LLVM_ASSERT(ImplicitInitKind == IIK_Default && "Unhandled implicit init kind!");
...
static bool CollectFieldInitializer(BaseAndFieldInfo &Info,
  ...
  if (Info.IIK == IIK_Default && Field->isAnonymousStructOrUnion()) {
    ...
    LLVM_ASSERT(FieldClassType && "anonymous struct/union without record type");
...
Sema::SetBaseOrMemberInitializers(CXXConstructorDecl *Constructor,
  ...
  for (CXXRecordDecl::field_iterator Field = ClassDecl->field_begin(),
    ...
    if ((*Field)->getType()->isIncompleteArrayType()) {
      LLVM_ASSERT(ClassDecl->hasFlexibleArrayMember() &&
...
DiagnoseBaseOrMemInitializerOrder(Sema &SemaRef,
  ...
  for (unsigned InitIndex = 0; InitIndex != NumInits; ++InitIndex) {
    ...
    if (IdealIndex == NumIdealInits && PrevInit) {
      ...
      LLVM_ASSERT(IdealIndex != NumIdealInits &&
...
bool CheckRedundantInit(Sema &S,
  ...
  if (FieldDecl *Field = Init->getMember())
    ...
    LLVM_ASSERT(BaseClass && "neither field nor base");
    S.Diag(Init->getSourceLocation(),
...
Decl *Sema::ActOnConversionDeclarator(CXXConversionDecl *Conversion) {
  LLVM_ASSERT(Conversion && "Expected to receive a conversion function declaration");
    ...
    if (FunctionTemplateDecl *ConversionTemplate
      ...
      if (ClassDecl->replaceConversion(
        ...
        return ConversionTemplate;
                                            ...
                                            Conversion))
      return Conversion;
    LLVM_ASSERT(Conversion->isInvalidDecl() && "Conversion should not get here.");
...
Decl *Sema::ActOnStartNamespaceDef(Scope *NamespcScope,
  ...
  if (II) {
    ...
    LLVM_ASSERT(Namespc->isAnonymousNamespace());
    ...
    if (PrevDecl) {
      LLVM_ASSERT(PrevDecl->isAnonymousNamespace());
      LLVM_ASSERT(!PrevDecl->getNextNamespace());
...
void Sema::ActOnFinishNamespaceDef(Decl *Dcl, SourceLocation RBrace) {
  ...
  LLVM_ASSERT(Namespc && "Invalid parameter, expected NamespaceDecl");
...
Decl *Sema::ActOnUsingDirective(Scope *S,
                                          ...
                                          AttributeList *AttrList) {
  LLVM_ASSERT(!SS.isInvalid() && "Invalid CXXScopeSpec.");
  LLVM_ASSERT(NamespcName && "Invalid NamespcName.");
  LLVM_ASSERT(IdentLoc.isValid() && "Invalid NamespceName location.");
  LLVM_ASSERT(S->getFlags() & Scope::DeclScope && "Invalid Scope.");
  ...
  if (!R.empty()) {
    ...
    LLVM_ASSERT((isa<NamespaceDecl>(Named) || isa<NamespaceAliasDecl>(Named))
...
UsingShadowDecl *Sema::BuildUsingShadowDecl(Scope *S,
  ...
  if (isa<UsingShadowDecl>(Target)) {
    Target = cast<UsingShadowDecl>(Target)->getTargetDecl();
    LLVM_ASSERT(!isa<UsingShadowDecl>(Target) && "nested shadow declaration");
  }
...
NamedDecl *Sema::BuildUsingDeclaration(Scope *S, AccessSpecifier AS,
  ...
  LLVM_ASSERT(IdentLoc.isValid() && "Invalid TargetName location.");
  ...
  if (S) {
    ...
    LLVM_ASSERT(IsInstantiation && "no scope in non-instantiation");
    LLVM_ASSERT(CurContext->isRecord() && "scope not record in instantiation");
...
CXXConstructorDecl *Sema::DeclareImplicitDefaultConstructor(
  ...
  LLVM_ASSERT(!ClassDecl->hasUserDeclaredConstructor() && 
...
void Sema::DefineImplicitDefaultConstructor(SourceLocation CurrentLocation,
                                            CXXConstructorDecl *Constructor) {
  LLVM_ASSERT((Constructor->isImplicit() && Constructor->isDefaultConstructor() &&
          !Constructor->isUsed(false)) &&
    "DefineImplicitDefaultConstructor - call it for implicit default ctor");

  CXXRecordDecl *ClassDecl = Constructor->getParent();
  LLVM_ASSERT(ClassDecl && "DefineImplicitDefaultConstructor - invalid constructor");
...
void Sema::DefineImplicitDestructor(SourceLocation CurrentLocation,
                                    CXXDestructorDecl *Destructor) {
  LLVM_ASSERT((Destructor->isImplicit() && !Destructor->isUsed(false)) &&
         "DefineImplicitDestructor - call it for implicit default dtor");
  CXXRecordDecl *ClassDecl = Destructor->getParent();
  LLVM_ASSERT(ClassDecl && "DefineImplicitDestructor - invalid destructor");
...
BuildSingleCopyAssign(Sema &S, SourceLocation Loc, QualType T, 
  ...
  LLVM_ASSERT(IterationVarRef && "Reference to invented variable cannot fail!");
...
static bool hasConstCopyAssignment(Sema &S, const CXXRecordDecl *CClass) {
  ...
  LLVM_ASSERT(Class->isInvalidDecl() &&
...
CXXMethodDecl *Sema::DeclareImplicitCopyAssignment(CXXRecordDecl *ClassDecl) {
  ...
  for (CXXRecordDecl::base_class_iterator Base = ClassDecl->bases_begin(),
    ...
    LLVM_ASSERT(!Base->getType()->isDependentType() &&
...
void Sema::DefineImplicitCopyAssignment(SourceLocation CurrentLocation,
  ...
  LLVM_ASSERT((CopyAssignOperator->isImplicit() && 
  ...
  LLVM_ASSERT(OtherRef && "Reference to parameter cannot fail!");
  
  // Construct the "this" pointer. We'll be using this throughout the generated
  // ASTs.
  Expr *This = ActOnCXXThis(Loc).takeAs<Expr>();
  LLVM_ASSERT(This && "Reference to this cannot fail!");
  ...
  for (CXXRecordDecl::field_iterator Field = ClassDecl->field_begin(),
    ...
    ExprResult To = BuildMemberReferenceExpr(This, This->getType(),
                                                   Loc, /*IsArrow=*/true,
                                                   SS, 0, MemberLookup, 0);
    LLVM_ASSERT(!From.isInvalid() && "Implicit field reference cannot fail");
    LLVM_ASSERT(!To.isInvalid() && "Implicit field reference cannot fail");
    ...
    if (FieldType->isArrayType() &&
      ...
      if (NeedsCollectableMemCpy) {
        if (!CollectableMemCpyRef) {
          ...
          CollectableMemCpyRef = BuildDeclRefExpr(CollectableMemCpy, 
                                                  ...
                                                  Loc, 0).takeAs<Expr>();
          LLVM_ASSERT(CollectableMemCpyRef && "Builtin reference cannot fail");
        }
      }
      // Create a reference to the __builtin_memcpy builtin function.
        ...
        BuiltinMemCpyRef = BuildDeclRefExpr(BuiltinMemCpy, 
                                            ...
                                            Loc, 0).takeAs<Expr>();
        LLVM_ASSERT(BuiltinMemCpyRef && "Builtin reference cannot fail");
      }
      ...
      LLVM_ASSERT(!Call.isInvalid() && "Call to __builtin_memcpy cannot fail!");
  ...
  StmtResult Body = ActOnCompoundStmt(Loc, Loc, move_arg(Statements),
                                            /*isStmtExpr=*/false);
  LLVM_ASSERT(!Body.isInvalid() && "Compound statement creation cannot fail");
...
void Sema::DefineImplicitCopyConstructor(SourceLocation CurrentLocation,
                                   CXXConstructorDecl *CopyConstructor,
                                   unsigned TypeQuals) {
  LLVM_ASSERT((CopyConstructor->isImplicit() &&
          CopyConstructor->isCopyConstructor(TypeQuals) &&
          !CopyConstructor->isUsed(false)) &&
         "DefineImplicitCopyConstructor - call it for implicit copy ctor");

  CXXRecordDecl *ClassDecl = CopyConstructor->getParent();
  LLVM_ASSERT(ClassDecl && "DefineImplicitCopyConstructor - invalid constructor");
...
void Sema::AddCXXDirectInitializerToDecl(Decl *RealDecl,
                                         ...
                                         SourceLocation *CommaLocs,
                                         SourceLocation RParenLoc) {
  LLVM_ASSERT(Exprs.size() != 0 && Exprs.get() && "missing expressions");
...
Sema::CompleteConstructorCall(CXXConstructorDecl *Constructor,
  ...
  LLVM_ASSERT(Proto && "Constructor without a prototype?");
...
bool Sema::CheckOverloadedOperatorDeclaration(FunctionDecl *FnDecl) {
  LLVM_ASSERT(FnDecl && FnDecl->isOverloadedOperator() &&
...
#include "clang/Basic/OperatorKinds.def"
  ...
  if (Op != OO_Call &&
    ...
    if (CanBeUnaryOperator && CanBeBinaryOperator) {
      ...
      LLVM_ASSERT(CanBeBinaryOperator &&
  ...
  if (Op == OO_Equal) {
    ...
    LLVM_ASSERT(isa<CXXMethodDecl>(FnDecl) &&
...
Decl *Sema::ActOnExceptionDeclarator(Scope *S, Declarator &D) {
  ...
  if (NamedDecl *PrevDecl = LookupSingleName(S, II, D.getIdentifierLoc(),
    ...
    LLVM_ASSERT(!S->isDeclScope(PrevDecl));
...
FriendDecl *Sema::CheckFriendTypeDecl(SourceLocation FriendLoc, 
  ...
  LLVM_ASSERT(TSInfo && "NULL TypeSourceInfo for friend type declaration");
...
Decl *Sema::ActOnFriendTypeDecl(Scope *S, const DeclSpec &DS,
  ...
  LLVM_ASSERT(DS.isFriendSpecified());
  LLVM_ASSERT(DS.getStorageClassSpec() == DeclSpec::SCS_unspecified);
...
Decl *Sema::ActOnFriendFunctionDecl(Scope *S,
  ...
  LLVM_ASSERT(ND->getDeclContext() == DC);
  LLVM_ASSERT(ND->getLexicalDeclContext() == CurContext);
...
DeclResult Sema::ActOnCXXConditionDeclaration(Scope *S, Declarator &D) {
  ...
  LLVM_ASSERT(D.getDeclSpec().getStorageClassSpec() != DeclSpec::SCS_typedef &&
