...
static bool areSameSizeInt(BuiltinType::Kind k1, BuiltinType::Kind k2, BuiltinType::Kind signedKind, BuiltinType::Kind unsignedKind)
{
	return (k1 == signedKind && k2 == unsignedKind) || (k1 == unsignedKind && k2 == signedKind);
}

static bool areAllowedVectorTypes(const VectorType& l, const VectorType& r)
{
	const BuiltinType* const lElemType = l.getElementType().getTypePtr()->getAs<BuiltinType>();
	const BuiltinType* const rElemType = r.getElementType().getTypePtr()->getAs<BuiltinType>();
	assert(lElemType && rElemType && "found vector whose element type is not a built-in one");
	const BuiltinType::Kind lKind = lElemType->getKind();
	const BuiltinType::Kind rKind = rElemType->getKind();
	if (lKind == rKind)
		return true;
	if (areSameSizeInt(lKind, rKind, BuiltinType::Char_S, BuiltinType::UChar))
		return true;
	if (areSameSizeInt(lKind, rKind, BuiltinType::Short, BuiltinType::UShort))
		return true;
	if (areSameSizeInt(lKind, rKind, BuiltinType::Int, BuiltinType::UInt))
		return true;
	if (areSameSizeInt(lKind, rKind, BuiltinType::Long, BuiltinType::ULong))
		return true;
	return false;
}

static QualType getSignedVectorType(const QualType& vecType, ASTContext& Context)
{
	const BuiltinType::Kind kind = vecType->getAs<VectorType>()->getElementType().getTypePtr()->getAs<BuiltinType>()->getKind();

	if (kind == BuiltinType::UChar)
		return Context.getExtVectorType(Context.CharTy, vecType->getAs<VectorType>()->getNumElements());
	if (kind == BuiltinType::UShort)
		return Context.getExtVectorType(Context.ShortTy, vecType->getAs<VectorType>()->getNumElements());
	if (kind == BuiltinType::UInt || kind == BuiltinType::Float)
		return Context.getExtVectorType(Context.IntTy, vecType->getAs<VectorType>()->getNumElements());
    if (kind == BuiltinType::ULong || kind == BuiltinType::Double)
		return Context.getExtVectorType(Context.LongTy, vecType->getAs<VectorType>()->getNumElements());
	// kind is already signed
	return vecType;
}
...
void Sema::DefaultFunctionArrayConversion(Expr *&E) {
  ...
  LLVM_ASSERT(!Ty.isNull() && "DefaultFunctionArrayConversion - missing type");
...
void Sema::DefaultFunctionArrayLvalueConversion(Expr *&E) {
  ...
  QualType Ty = E->getType();
  LLVM_ASSERT(!Ty.isNull() && "DefaultFunctionArrayLvalueConversion - missing type");
...
Expr *Sema::UsualUnaryConversions(Expr *&Expr) {
  ...
  LLVM_ASSERT(!Ty.isNull() && "UsualUnaryConversions - missing type");
...
void Sema::DefaultArgumentPromotion(Expr *&Expr_ref) {
  QualType Ty = Expr_ref->getType();
  LLVM_ASSERT(!Ty.isNull() && "DefaultArgumentPromotion - missing type");

  // If this is a 'float' (CVR qualified or typedef) promote to double.
  if (Ty->isSpecificBuiltinType(BuiltinType::Float))
    return ImpCastExprToType(Expr_ref, Context.DoubleTy,
                             CK_FloatingCast);

  // In OpenCL, vectors of promotable types have to be converted similarly
  // to the scalar case. An appropriate conversion function is used.
  //
  const ExtVectorType *VT = Ty->getAs<ExtVectorType>();
  if (VT && getLangOptions().OpenCL) 
  {
      const BuiltinType* elem_type = VT->getElementType()->getAs<BuiltinType>();

      if (elem_type && (elem_type->getKind() == BuiltinType::Float ||
                        elem_type->isPromotableIntegerType()))
      {
          // Prepare function name of the appropriate vector size.
          //
          std::string conversion_func_name = vec_conversion_function_for_type(elem_type->getKind());
          std::stringstream num_formatter;
          num_formatter << VT->getNumElements();
          conversion_func_name += num_formatter.str();

          // Create a DeclarationName for the function
          // 
          IdentifierInfo& II = Context.Idents.get(conversion_func_name.c_str());
          DeclarationName declname(&II);

          SourceLocation sourceloc = Expr_ref->getExprLoc();

          // Using LookupName to find the function's declaration since 
          // conversion functions are overloaded, and we need them all in order
          // to resolve to the right one with BuildOverloadedCallExpr.
          //
          LookupResult R(*this, declname, sourceloc, LookupOrdinaryName);
          LookupName(R, TUScope, false);

          // Build an UnresolvedLookupExpr (since the conversion function is 
          // overloaded).
          // This function is usually used for C++ (because it handles 
          // overloaded functions). We reuse it here for OpenCL, setting the
          // C++ related args (scope_spec, NeedsADL) to dummies.
          //
          CXXScopeSpec scope_spec;
          ExprResult declnameexpr = BuildDeclarationNameExpr(
              scope_spec, R, false);

          // Make sure the lookup expression is of the expected type and was
          // indeed found to be overloaded.
          //
          UnresolvedLookupExpr* unresolvedlookup = 
              dyn_cast<UnresolvedLookupExpr>(declnameexpr.takeAs<Expr>());
          if (!unresolvedlookup || !unresolvedlookup->isOverloaded()) 
          {
              Diag(sourceloc, diag::err_invalid_vector_promotion);
              return;
          }

          // Build the CallExpr, and replace the argument expression with it.
          //
          ExprResult callresult = BuildOverloadedCallExpr(
              NULL, unresolvedlookup, unresolvedlookup, sourceloc, 
              &Expr_ref, 1, 0, sourceloc);

          Expr_ref = callresult.takeAs<Expr>();
          if (!Expr_ref) 
              Diag(sourceloc, diag::err_invalid_vector_promotion);
  ...
  UsualUnaryConversions(Expr_ref);
}
...
Sema::ActOnStringLiteral(const Token *StringToks, unsigned NumStringToks) {
  LLVM_ASSERT(NumStringToks && "Must have at least one string!");
  ...
  // All string literal storage shall be in the __constant address space (OpenCL 1.1 specs, sec. 6.5.3)
  if (getLangOptions().OpenCL)
  {
    StrTy = Context.getAddrSpaceQualType(StrTy, 2);
  }
...
VarDecl *Sema::BuildAnonymousStructUnionMemberPath(FieldDecl *Field,
  ...
  LLVM_ASSERT(Field->getDeclContext()->isRecord() &&
...
Sema::BuildAnonymousStructUnionMemberReference(SourceLocation Loc,
  ...
  for (llvm::SmallVector<FieldDecl *, 4>::reverse_iterator
    ...
    LLVM_ASSERT(!MemberTypeQuals.hasAddressSpace());
...
static IMAKind ClassifyImplicitMemberAccess(Sema &SemaRef,
  ...
  LLVM_ASSERT(!R.empty() && (*R.begin())->isCXXClassMember());
...
ExprResult Sema::ActOnIdExpression(Scope *S,
  ...
  LLVM_ASSERT(!(isAddressOfOperand && HasTrailingLParen) &&
...
Sema::PerformObjectMemberConversion(Expr *&From,
  ...
  if (Qualifier) {
    ...
    LLVM_ASSERT(!QType.isNull() && "lookup done with dependent qualifier?");
    LLVM_ASSERT(QType->isRecordType() && "lookup done with non-record type");
  ...
  if (FoundDecl->getDeclContext() != Member->getDeclContext()) {
    LLVM_ASSERT(isa<UsingShadowDecl>(FoundDecl));
    ...
    if (!Context.hasSameUnqualifiedType(FromRecordType, URecordType)) {
      LLVM_ASSERT(IsDerivedFrom(FromRecordType, URecordType));
      CXXCastPath BasePath;
...
Sema::BuildImplicitMemberExpr(const CXXScopeSpec &SS,
  ...
  LLVM_ASSERT(!R.empty() && !R.isAmbiguous());
...
Sema::BuildDeclarationNameExpr(const CXXScopeSpec &SS,
                               const DeclarationNameInfo &NameInfo,
                               NamedDecl *D) {
  LLVM_ASSERT(D && "Cannot refer to a NULL declaration");
...
ExprResult Sema::ActOnPredefinedExpr(SourceLocation Loc,
  ...
  switch (Kind) {
  default: LLVM_ASSERT(0 && "Unknown simple primary expr!");
...
ExprResult Sema::ActOnNumericConstant(const Token &Tok) {
  ...
  if (Literal.isFloatingLiteral()) {
	...
	// long long is not allowed in OpenCL
	if (getLangOptions().OpenCL && Literal.isLongLong)
      Diag(Tok.getLocation(), diag::err_longlong_num_opencl);
    ...
    if (Literal.GetIntegerValue(ResultVal)) {
      ...
      LLVM_ASSERT(Context.getTypeSize(Ty) == ResultVal.getBitWidth() &&
...
ExprResult Sema::ActOnParenExpr(SourceLocation L,
  ...
  LLVM_ASSERT((E != 0) && "ActOnParenExpr() missing expr");
...
bool Sema::CheckSizeOfAlignOfOperand(QualType exprType,
                                     ...
                                     SizeOfAlignOfExpr::Kind_t exprKind) {
  ...
  if (exprType->isFunctionType()) {
    ...
    if (exprKind != SizeOfAlignOfExpr::AlignOf)
  ...
  if (LangOpts.ObjCNonFragileABI && exprType->isObjCObjectType()) {
    Diag(OpLoc, diag::err_sizeof_nonfragile_interface)
      << exprType << (exprKind == SizeOfAlignOfExpr::SizeOf) << ExprRange;
    return true;
  }
...
bool Sema::CheckAlignOfExpr(Expr *E, SourceLocation OpLoc,
  ...
  return CheckSizeOfAlignOfOperand(E->getType(), OpLoc, ExprRange, SizeOfAlignOfExpr::AlignOf);
}
...
bool Sema::CheckVecStepExpr(Expr *E, SourceLocation OpLoc,
  ...
  return CheckSizeOfAlignOfOperand(E->getType(), OpLoc, ExprRange, SizeOfAlignOfExpr::VecStep);
}
...
Sema::CreateSizeOfAlignOfExpr(TypeSourceInfo *TInfo,
  ...
  if (!T->isDependentType() &&
      CheckSizeOfAlignOfOperand(T, OpLoc, R, exprKind))
    return ExprError();

  // C99 6.5.3.4p4: the type (an unsigned integer type) is size_t.
  return Owned(new (Context) SizeOfAlignOfExpr(exprKind, TInfo,
...
Sema::CreateSizeOfAlignOfExpr(Expr *E, SourceLocation OpLoc,
  ...
  if (E->isTypeDependent()) {
  ...
  } else if (exprKind == SizeOfAlignOfExpr::AlignOf) {
    isInvalid = CheckAlignOfExpr(E, OpLoc, R);
  } else if (exprKind == SizeOfAlignOfExpr::VecStep) {
    ...
    isInvalid = CheckSizeOfAlignOfOperand(E->getType(), OpLoc, R, SizeOfAlignOfExpr::SizeOf);
  }
  ...
  // C99 6.5.3.4p4: the type (an unsigned integer type) is size_t.
  return Owned(new (Context) SizeOfAlignOfExpr(exprKind, E,
                                               Context.getSizeType(), OpLoc,
                                               R.getEnd()));
}
...
ExprResult
Sema::ActOnSizeOfAlignOfExpr(SourceLocation OpLoc, SizeOfAlignOfExpr::Kind_t exprKind, bool isType,
  ...
  if (isType) {
    ...
    return CreateSizeOfAlignOfExpr(TInfo, OpLoc, exprKind, ArgRange);
  }

  Expr *ArgEx = (Expr *)TyOrEx;
  ExprResult Result
    = CreateSizeOfAlignOfExpr(ArgEx, OpLoc, exprKind, ArgEx->getSourceRange());

  return move(Result);
}

ExprResult
Sema::ActOnEndianAttr(SourceLocation OpLoc, const char* param, unsigned int len) {
  StringLiteral *liter = StringLiteral::Create(Context, param, len, false, Context.CharTy, OpLoc);
  ExprResult Result(liter);
...
Sema::ActOnPostfixUnaryOp(Scope *S, SourceLocation OpLoc,
  ...
  switch (Kind) {
  default: LLVM_ASSERT(0 && "Unknown unary op!");
...
#if 0   // disable until BE fix their sources
    if (getLangOptions().OpenCL)
        return ExprError(Diag(LLoc, diag::err_opencl_vector_square_bracket_for_vector) <<
...
CheckExtVectorComponent(QualType baseType, SourceLocation OpLoc,
  ...
  if (CompSize == 1)
      return Context.getQualifiedType(vecType->getElementType().getTypePtr(), baseType.getQualifiers());

  // this means that we have an expression of the form "s[0]"
  if (CompSize == 0) {
	  Diag(OpLoc, diag::err_ext_vector_component_missing_element_specifier) << SourceRange(CompLoc);
	  return QualType();
  }
  ...
  for (unsigned i = 0, E = ExtVectorDecls.size(); i != E; ++i) {
    if (ExtVectorDecls[i]->getUnderlyingType() == VT)
        return Context.getQualifiedType(Context.getTypedefType(ExtVectorDecls[i]).getTypePtr(), baseType.getQualifiers());
  }
...
Sema::ActOnDependentMemberExpr(Expr *BaseExpr, QualType BaseType,
  ...
  if (!IsArrow) {
    ...
    if (PT && (!getLangOptions().ObjC1 ||
      ...
      LLVM_ASSERT(BaseExpr && "cannot happen with implicit member accesses");
  ...
  LLVM_ASSERT(BaseType->isDependentType() ||
...
LookupMemberExprInRecord(Sema &SemaRef, LookupResult &R,
  ...
  if (SS.isSet()) {
    ...
    LLVM_ASSERT(DC && "Cannot handle non-computable dependent contexts in lookup");
...
Sema::BuildMemberReferenceExpr(Expr *BaseExpr, QualType BaseExprType,
  ...
  if (IsArrow) {
    LLVM_ASSERT(BaseType->isPointerType());
...
Sema::LookupMemberExpr(LookupResult &R, Expr *&BaseExpr,
  ...
  LLVM_ASSERT(BaseExpr && "no base expression");
  ...
  LLVM_ASSERT(!BaseType.isNull() && "no type for member expression");
...
bool Sema::GatherArgumentsForCall(SourceLocation CallLoc,
  ...
  bool call_to_builtin_as_typen = (
    FDecl != 0 && 
    FDecl->getBuiltinID() >= Builtin::BI__builtin_as_char &&
    FDecl->getBuiltinID() <= Builtin::BI__builtin_as_uintptr_t);
  ...
  if (CallType != VariadicDoesNotApply) {
    ...
    for (unsigned i = ArgIx; i != NumArgs; ++i) {
      ...
      // OpenCL 1.1 standard (6.2.4.2) states that type promotion for as_typen
      // arguments shall not be performed.
      if (!call_to_builtin_as_typen)
...
Sema::ActOnCallExpr(Scope *S, Expr *Fn, SourceLocation LParenLoc,
  ...
  if (getLangOptions().CPlusPlus) {
    ...
    if (UnresolvedMemberExpr *MemE = dyn_cast<UnresolvedMemberExpr>(NakedFn)) {
      ...
      // method template.
      LLVM_ASSERT((MemE->getNumDecls() > 1) ||
  ...
  if (getLangOptions().OpenCL) {
    if (NDecl->hasAttr<ReqdExtensionAttr>()) {
      unsigned ext = NDecl->getAttr<ReqdExtensionAttr>()->getExtension();

      if (!OpenCLFeatures.isOpenCLExtensionEnabled(ext)) {
        return ExprError(Diag(Fn->getLocStart(), diag::err_extension_disabled)
            << OpenCL::convertEnumToString(ext));
      }
    }
  }
...
Sema::BuildResolvedCallExpr(Expr *Fn, NamedDecl *NDecl,
  ...
  if (const FunctionProtoType *Proto = dyn_cast<FunctionProtoType>(FuncT)) {
    ...
    LLVM_ASSERT(isa<FunctionNoProtoType>(FuncT) && "Unknown FunctionType!");
...
Sema::ActOnCompoundLiteral(SourceLocation LParenLoc, ParsedType Ty,
  ...
  LLVM_ASSERT((Ty != 0) && "ActOnCompoundLiteral(): missing type");
  // FIXME: put back this assert when initializers are worked out.
  //LLVM_ASSERT((InitExpr != 0) && "ActOnCompoundLiteral(): missing expression");
...
static CastKind getScalarCastKind(ASTContext &Context,
  ...
  // FIXME: Assert here.
  // LLVM_ASSERT(false && "Unhandled cast combination!");
  return CK_Unknown;
}
...
bool Sema::CheckCastTypes(SourceRange TyR, QualType castType, Expr *&castExpr,
                          CastKind& Kind,
                          CXXCastPath &BasePath, bool isParenthesizedExprSet,
  ...
  if (castType->isExtVectorType())
    return CheckExtVectorCast(TyR, castType, castExpr, Kind, isParenthesizedExprSet);
...
bool Sema::CheckVectorCast(SourceRange R, QualType VectorTy, QualType Ty,
                           CastKind &Kind) {
  LLVM_ASSERT(VectorTy->isVectorType() && "Not a vector type!");
...
bool Sema::CheckExtVectorCast(SourceRange R, QualType DestTy, Expr *&CastExpr,
                              CastKind &Kind, bool isParenthesizedExprSet) {
  LLVM_ASSERT(DestTy->isExtVectorType() && "Not an extended vector type!");
  ...
  if (SrcTy->isVectorType()) {
    if (getLangOptions().OpenCL && isParenthesizedExprSet && !Context.hasSameUnqualifiedType(SrcTy, DestTy))
        return Diag(R.getBegin(), diag::err_elem_type_mismatch_in_vector_literal) << R;
    if (Context.getTypeSize(DestTy) != Context.getTypeSize(SrcTy)) {
      if(LangOpts.OpenCL)
        return Diag(R.getBegin(), diag::err_opencl_vector_to_vector_cast)
      << SrcTy << DestTy;
...
Sema::ActOnCastExpr(Scope *S, SourceLocation LParenLoc, ParsedType Ty,
  ...
  if (isa<ParenListExpr>(castExpr))
    return ActOnCastOfParenListExpr(S, LParenLoc, RParenLoc, castExpr,
                                    castTInfo);
  /* This is to distinguish in OpenCL between the following expressions (x's type is int2):
     1. (float2)(x)     - vector initialization - don't allow this initialization of float2 from int2
     2. (float2)x       - vector bit cast - not allowed in OpenCL at all, but we support it for the built-in functions and we don't check the type of x
   */
  return BuildCStyleCastExpr(LParenLoc, castTInfo, RParenLoc, castExpr, false);
}

ExprResult
Sema::BuildCStyleCastExpr(SourceLocation LParenLoc, TypeSourceInfo *Ty,
                          SourceLocation RParenLoc, Expr *castExpr, bool isParenthesizedExprSet) {
  CastKind Kind = CK_Unknown;
  CXXCastPath BasePath;
  if (CheckCastTypes(SourceRange(LParenLoc, RParenLoc), Ty->getType(), castExpr,
                     Kind, BasePath, isParenthesizedExprSet))
    return ExprError();

  if(LangOpts.OpenCL && !LangOpts.OpenCLBuiltins &&
    Ty->getType()->isVectorType() && castExpr->getType()->isVectorType() &&
    !Context.hasSameUnqualifiedType(Ty->getType(), castExpr->getType())) {

    Diag(LParenLoc, diag::err_opencl_vector_to_vector_cast)
      << castExpr->getType() << Ty->getType();
...
Sema::ActOnCastOfParenListExpr(Scope *S, SourceLocation LParenLoc,
  ...
  if (isAltiVecLiteral) {
    ...
    if (Result.isInvalid()) return ExprError();
    return BuildCStyleCastExpr(LParenLoc, TInfo, RParenLoc, Result.take(), true);
  }
}
...
ExprResult Sema::ActOnParenOrParenListExpr(SourceLocation L,
  ...
  LLVM_ASSERT((exprs != 0) && "ActOnParenOrParenListExpr() missing expr list");
  Expr *expr;
...
QualType Sema::CheckConditionalOperands(Expr *&Cond, Expr *&LHS, Expr *&RHS,
  ...
  /* If any of the operands is a vector, don't do the usual unary conversions, since if the vector element type is smaller than int and the scalar operands are converted to int,
  they cannot be widened to the vector type. */
  if (getLangOptions().OpenCL){
    if (!Cond->getType()->isVectorType() && !LHS->getType()->isVectorType() && !RHS->getType()->isVectorType()) {
  ...
  if (!getLangOptions().OpenCL && !CondTy->isScalarType()) { // C99 6.5.15p2
    Diag(Cond->getLocStart(), diag::err_typecheck_cond_expect_scalar)
      << CondTy;
    return QualType();
  }
  if (getLangOptions().OpenCL && (CondTy->isFloatingType() || (CondTy->isVectorType() &&
        CondTy->getAs<ExtVectorType>()->getElementType()->isFloatingType()))) {
      Diag(Cond->getLocStart(), diag::err_typecheck_float_types_not_allowed) << Cond->getSourceRange();
      return QualType();
  }
  if (getLangOptions().OpenCL && CondTy->isVectorType()) {
      if (LHSTy->isVectorType() || RHSTy->isVectorType()) {
          const QualType& valType = CheckVectorOperands(QuestionLoc, LHS, RHS, true);
          if (valType == QualType())
              return valType;
          if ((valType->getAs<ExtVectorType>())->getNumElements() !=
              (CondTy->getAs<ExtVectorType>())->getNumElements()) {
              Diag(QuestionLoc, diag::err_trinary_op_cond_and_values_num_elems_not_match) <<
                  Cond->getSourceRange() << LHS->getSourceRange() << RHS->getSourceRange();
              return QualType();
          }
          return valType;
      }
      // values are scalar, but the condition is a vector. Try to convert the values to the type of the condition.
      // TODO: Try to find a common vector type of the values - it doesn't have to be connected to the element type of the condition
	  const QualType& condAndLeft = CheckVectorOperands(QuestionLoc, Cond, LHS, false);
	  if (condAndLeft.isNull())
		  return condAndLeft;
	  const QualType& condAndRight = CheckVectorOperands(QuestionLoc, Cond, RHS, false);
	  if (condAndLeft == condAndRight) {	// if both checks yield the same type, it is actually the type of the condition
		  return CondTy;
	  }
	  return QualType();
  }
  ...
  if (LHSTy->isVectorType() || RHSTy->isVectorType())
    return CheckVectorOperands(QuestionLoc, LHS, RHS, true);
...
Sema::CheckPointerTypesForAssignment(QualType lhsType, QualType rhsType) {
  ...
  if (lhptee->isVoidType()) {
    ...
    LLVM_ASSERT(rhptee->isFunctionType());
  ...
  if (rhptee->isVoidType()) {
    ...
    // As an extension, we allow cast to/from void* to function pointer.
    LLVM_ASSERT(lhptee->isFunctionType());
    return FunctionVoidPointer;
  }
...
QualType Sema::CheckVectorOperands(SourceLocation Loc, Expr *&lex, Expr *&rex, bool allowFloatingPointTypes) {
  // For conversion purposes, we ignore any qualifiers.
  // For example, "const float" and "float" are equivalent.
  QualType lhsType =
  ...
  if (const ExtVectorType *LV = lhsType->getAs<ExtVectorType>()) {
    ...
    if (EltTy->isIntegralType(Context) && rhsType->isIntegralType(Context)) {
      if (Context.getIntegerTypeOrder(EltTy, rhsType) >= 0 || getLangOptions().OpenCL) {
    ...
    if (EltTy->isRealFloatingType() && rhsType->isScalarType() &&
      ...
      if (Context.getFloatingTypeOrder(EltTy, rhsType) >= 0 || getLangOptions().OpenCL) {
		if (!allowFloatingPointTypes) {
			Diag(Loc, diag::err_typecheck_float_types_not_allowed) << lex->getSourceRange() << rex->getSourceRange();
			return QualType();
		}
	...
	if (EltTy->isRealFloatingType() && rhsType->isIntegralType(Context)) {
		if (!allowFloatingPointTypes) {
			Diag(Loc, diag::err_typecheck_float_types_not_allowed) << lex->getSourceRange();
			return QualType();
		}
		ImpCastExprToType(rex, lhsType, CK_IntegralToFloating);
		if (swapped)
			std::swap(rex, lex);
		return lhsType;
	}
	if (EltTy->isIntegerType() && rhsType->isScalarType() && rhsType->isRealFloatingType()) {
		if (!allowFloatingPointTypes) {
			Diag(Loc, diag::err_typecheck_float_types_not_allowed) << rex->getSourceRange();
			return QualType();
		}
		ImpCastExprToType(rex, lhsType, CK_FloatingToIntegral);
		if (swapped)
			std::swap(rex, lex);
		return lhsType;
	}
...
QualType Sema::CheckMultiplyDivideOperands(
  ...
  if (lex->getType()->isVectorType() || rex->getType()->isVectorType())
    return CheckVectorOperands(Loc, lex, rex, true);
...
QualType Sema::CheckRemainderOperands(
  ...
  if (lex->getType()->isVectorType() || rex->getType()->isVectorType()) {
    if (lex->getType()->hasIntegerRepresentation() && 
      ...
      return CheckVectorOperands(Loc, lex, rex, false);
    return InvalidOperands(Loc, lex, rex);
  }
...
QualType Sema::CheckAdditionOperands( // C99 6.5.6
  ...
  if (lex->getType()->isVectorType() || rex->getType()->isVectorType()) {
    QualType compType = CheckVectorOperands(Loc, lex, rex, true);
...
QualType Sema::CheckCompareOperands(Expr *&lex, Expr *&rex, SourceLocation Loc,
  ...
  if (!lType->hasFloatingRepresentation() &&
    ...
    if (literalString) {
      ...
      default: LLVM_ASSERT(false && "Invalid comparison operator");
      }
...
QualType Sema::CheckVectorCompareOperands(Expr *&lex, Expr *&rex,
  ...
  QualType vType = CheckVectorOperands(Loc, lex, rex, true);
  if (vType.isNull())
    return vType;
  ...
  if (lType->hasIntegerRepresentation()) {
	  if (getLangOptions().OpenCL)
		  return getSignedVectorType(lType, Context);
      return lType;
  }
  ...
  LLVM_ASSERT(TypeSize == Context.getTypeSize(Context.LongLongTy) &&
...
inline QualType Sema::CheckLogicalOperands( // C99 6.5.[13,14]
  ...
  if (!Context.getLangOptions().CPlusPlus) {
    ...
    UsualUnaryConversions(rex);
	if (getLangOptions().OpenCL) {
		const QualType& ltype = lex->getType(), &rtype = rex->getType();
		if ((!ltype->isScalarType() && !ltype->isVectorType()) ||
			(!rtype->isScalarType() && !rtype->isVectorType()) ||
			(ltype->isVectorType() && rtype->isVectorType() && ltype->getAs<VectorType>()->getNumElements() != rtype->getAs<VectorType>()->getNumElements()) ||
			(ltype->isVectorType() && rtype->isVectorType() && !areAllowedVectorTypes(*ltype->getAs<VectorType>(), *rtype->getAs<VectorType>())))
			return InvalidOperands(Loc, lex, rex);
		if (ltype->isScalarType() && rtype->isScalarType())
			return Context.IntTy;
		if (ltype->isScalarType() && rtype->isVectorType())
			return getSignedVectorType(rtype, Context);
		if (ltype->isVectorType() && rtype->isScalarType())
			return getSignedVectorType(ltype, Context);
		const BuiltinType::Kind lKind = ltype->getAs<VectorType>()->getElementType().getTypePtr()->getAs<BuiltinType>()->getKind();
		const BuiltinType::Kind rKind = rtype->getAs<VectorType>()->getElementType().getTypePtr()->getAs<BuiltinType>()->getKind();
		if (lKind == rKind)
		  return getSignedVectorType(ltype, Context);
		return InvalidOperands(Loc, lex, rex);
	}
...
static bool CheckForModifiableLvalue(Expr *E, SourceLocation Loc, Sema &S) {
  ...
  if (S.IsOpenCLImageType(E->getType().getAsString())) {
      Diag = diag::error_opencl_image_cannot_be_modified;
      NeedType = true;
  } else {
...
QualType Sema::CheckCommaOperands(Expr *LHS, Expr *&RHS, SourceLocation Loc) {
  DiagnoseUnusedExprResult(LHS, true);
...
QualType Sema::CheckIncrementDecrementOperand(Expr *Op, SourceLocation OpLoc,
  ...
  LLVM_ASSERT(!ResType.isNull() && "no type for increment/decrement expression");
  ...
  } else if (ResType->isRealType() || ResType->isRealVecType()) {
	  //increment/decrement of floats is not allowed in openCL
	  if (getLangOptions().OpenCL && (ResType->isFloatingType() || ResType->isFloatingVecType())) {
		  Diag(OpLoc, diag::err_increment_decrement_float_in_opencl) << Op->getSourceRange();
		  return QualType();
	  }
	  // else OK!
...
QualType Sema::CheckAddressOfOperand(Expr *OrigOp, SourceLocation OpLoc) {
  ...
  if (lval == Expr::LV_ClassTemporary) {
    ...
    if (const VarDecl *vd = dyn_cast<VarDecl>(dcl)) {
      ...
      LLVM_ASSERT(0 && "Unknown/unexpected decl type");
    }
  }
...
static inline BinaryOperatorKind ConvertTokenKindToBinaryOpcode(
  ...
  switch (Kind) {
  default: LLVM_ASSERT(0 && "Unknown binop!");
...
static bool CheckOpenCLHalf(Expr *operand) {
  std::string typeName = operand->getType().getAsString();
...
ExprResult Sema::CreateBuiltinBinOp(SourceLocation OpLoc,
  ...
  if(LangOpts.OpenCL && !LangOpts.OpenCLBuiltins) {
    if(CheckOpenCLHalf(lhs)||CheckOpenCLHalf(rhs)) {
      Diag(OpLoc, diag::err_half_arithmetics);
      return ExprError();
    }
  }
  ...
  case BO_Add:
	...
	lhs->setFpContract(getFPOptions().fp_contract);
	rhs->setFpContract(getFPOptions().fp_contract);
    break;
  case BO_Sub:
...
ExprResult Sema::ActOnBinOp(Scope *S, SourceLocation TokLoc,
  ...
  LLVM_ASSERT((lhs != 0) && "ActOnBinOp(): missing left expression");
  LLVM_ASSERT((rhs != 0) && "ActOnBinOp(): missing right expression");
...
ExprResult Sema::CreateBuiltinUnaryOp(SourceLocation OpLoc,
  ...
  case UO_LNot: // logical negation
    ...
    if (resultType->isDependentType())
      break;
	if ((getLangOptions().OpenCL && !resultType->isScalarType() && !resultType->isVectorType())
		|| (!getLangOptions().OpenCL && !resultType->isScalarType())) // C99 6.5.3.3p1)
      return ExprError(Diag(OpLoc, diag::err_typecheck_unary_expr)
        << resultType << Input->getSourceRange());
	...
	if (!getLangOptions().OpenCL)
	  ...
	  if (Input->getType().getTypePtr()->isVectorType())
	    resultType = getSignedVectorType(Input->getType(), Context);
	  else
...
Sema::ActOnStmtExpr(SourceLocation LPLoc, Stmt *SubStmt,
  ...
  LLVM_ASSERT(SubStmt && isa<CompoundStmt>(SubStmt) && "Invalid action invocation!");
...
ExprResult Sema::ActOnTypesCompatibleExpr(SourceLocation BuiltinLoc,
  ...
  LLVM_ASSERT((!argT1.isNull() && !argT2.isNull()) && "Missing type argument(s)");
...
ExprResult Sema::ActOnChooseExpr(SourceLocation BuiltinLoc,
                                             ...
                                             SourceLocation RPLoc) {
  LLVM_ASSERT((CondExpr && LHSExpr && RHSExpr) && "Missing type argument(s)");
...
void Sema::ActOnBlockArguments(Declarator &ParamInfo, Scope *CurScope) {
  LLVM_ASSERT(ParamInfo.getIdentifier()==0 && "block-id should have no identifier!");
...
bool Sema::DiagnoseAssignmentResult(AssignConvertType ConvTy,
  ...
  switch (ConvTy) {
  default: LLVM_ASSERT(0 && "Unknown conversion type");
  case Compatible: return false;
  case PointerToInt:
    if (getLangOptions().OpenCL && !LangOpts.OpenCLBuiltins) {
  ...
  case FunctionVoidPointer:
    if (getLangOptions().OpenCL && !LangOpts.OpenCLBuiltins) {
      DiagKind = diag::err_functions_pointers_not_allowed_in_openCL;
      Diag(Loc, DiagKind) << 0;
      return true;
    }
