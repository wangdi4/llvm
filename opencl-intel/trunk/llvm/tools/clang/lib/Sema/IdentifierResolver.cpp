...
void IdentifierResolver::IdDeclInfo::RemoveDecl(NamedDecl *D) {
  ...
  LLVM_ASSERT(0 && "Didn't find this decl on its identifier's chain!");
}
...
bool IdentifierResolver::isDeclInScope(Decl *D, DeclContext *Ctx,
  ...
  if (Ctx->isFunctionOrMethod()) {
    ...
    if (LangOpt.CPlusPlus) {
      ...
      //
      LLVM_ASSERT(S->getParent() && "No TUScope?");
...
void IdentifierResolver::RemoveDecl(NamedDecl *D) {
  LLVM_ASSERT(D && "null param passed");
  DeclarationName Name = D->getDeclName();
  if (IdentifierInfo *II = Name.getAsIdentifierInfo())
    II->setIsFromAST(false);
  ...
  LLVM_ASSERT(Ptr && "Didn't find this decl on its identifier's chain!");

  if (isDeclPtr(Ptr)) {
    LLVM_ASSERT(D == Ptr && "Didn't find this decl on its identifier's chain!");
    Name.setFETokenInfo(NULL);
    return;
  }
...
bool IdentifierResolver::ReplaceDecl(NamedDecl *Old, NamedDecl *New) {
  LLVM_ASSERT(Old->getDeclName() == New->getDeclName() &&
...
void IdentifierResolver::iterator::incrementSlowCase() {
  ...
  LLVM_ASSERT(!isDeclPtr(InfoPtr) && "Decl with wrong id ?");
