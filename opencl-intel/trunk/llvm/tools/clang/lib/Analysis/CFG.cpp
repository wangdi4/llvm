...
CFGBlock *CFGBuilder::VisitConditionalOperator(ConditionalOperator *C,
  ...
  if (LHSBlock) {
    ...
    if (KnownVal.isFalse()) {
      ...
      LLVM_ASSERT(ConfluenceBlock->pred_size() == 1);
    } else {
      ...
      LLVM_ASSERT(ConfluenceBlock->pred_size() == 2);
...
CFGBlock* CFGBuilder::VisitLabelStmt(LabelStmt* L) {
  ...
  LLVM_ASSERT(LabelMap.find(L) == LabelMap.end() && "label already in map");
...
CFGBlock* CFGBuilder::VisitForStmt(ForStmt* F) {
  ...
  if (Stmt* C = F->getCond()) {
    ...
    LLVM_ASSERT(Block == EntryConditionBlock);
  ...
  // Now create the loop body.
  {
   ...
   SaveAndRestore<CFGBlock*> save_Block(Block), save_Succ(Succ),
    ...
    if (Stmt* I = F->getInc()) {
      ...
      LLVM_ASSERT(Succ == EntryConditionBlock);
      Succ = createBlock();
    }

    // Finish up the increment (or empty) block if it hasn't been already.
    if (Block) {
      LLVM_ASSERT(Block == Succ);
      if (!FinishBlock(Block))
        return 0;
      Block = 0;
    }
...
CFGBlock* CFGBuilder::VisitSwitchStmt(SwitchStmt* Terminator) {
  ...
  LLVM_ASSERT(Terminator->getBody() && "switch must contain a non-NULL body");
  Block = NULL;
  ...
  LLVM_ASSERT(Terminator->getCond() && "switch condition must be non-NULL");
...
CFGBlock* CFGBuilder::VisitCaseStmt(CaseStmt* CS) {
  ...
  // statement.
  LLVM_ASSERT(SwitchTerminatedBlock);
...
CFGBlock *CFGBuilder::VisitCXXTryStmt(CXXTryStmt *Terminator) {
  ...
  LLVM_ASSERT(Terminator->getTryBlock() && "try must contain a non-NULL body");
  Block = NULL;
  ...
  void VisitBinaryOperator(BinaryOperator* B) {
    ...
    switch (B->getOpcode()) {
      ...
      case BO_LAnd:
        ...
        return;
      default:
        LLVM_ASSERT(false && "Invalid logical operator.");
    }
  }
...
static void print_block(llvm::raw_ostream& OS, const CFG* cfg,
  ...
  if (Stmt* Label = const_cast<Stmt*>(B.getLabel())) {
    ...
    if (LabelStmt* L = dyn_cast<LabelStmt>(Label))
      ...
      LLVM_ASSERT(false && "Invalid label statement in CFGBlock.");
