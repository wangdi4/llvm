...
Option::Option(OptionClass _Kind, OptSpecifier _ID, const char *_Name,
  ...
  // inherent limitation.
  LLVM_ASSERT((!Alias || (!Alias->Alias && !Group)) &&
...
Arg *OptionGroup::accept(const ArgList &Args, unsigned &Index) const {
  LLVM_ASSERT(0 && "accept() should never be called on an OptionGroup");
  return 0;
}
...
Arg *InputOption::accept(const ArgList &Args, unsigned &Index) const {
  LLVM_ASSERT(0 && "accept() should never be called on an InputOption");
  return 0;
}
...
Arg *UnknownOption::accept(const ArgList &Args, unsigned &Index) const {
  LLVM_ASSERT(0 && "accept() should never be called on an UnknownOption");
  return 0;
}
...
MultiArgOption::MultiArgOption(OptSpecifier ID, const char *Name,
  ...
  LLVM_ASSERT(NumArgs > 1  && "Invalid MultiArgOption!");
}
