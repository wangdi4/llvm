...
void Driver::BuildActions(const ToolChain &TC, const ArgList &Args,
  ...
  for (ArgList::const_iterator it = Args.begin(), ie = Args.end();
    ...
    if (isa<InputOption>(A->getOption())) {
      ...
      if (InputType == types::TY_Nothing) {
        ...
        LLVM_ASSERT(InputTypeArg && "InputType set w/o InputTypeArg");
        InputTypeArg->claim();
        Ty = InputType;
      }
  ...
  for (unsigned i = 0, e = Inputs.size(); i != e; ++i) {
    ...
    LLVM_ASSERT(NumSteps && "Invalid number of steps!");
    ...
    for (unsigned i = 0; i != NumSteps; ++i) {
      ...
      if (Phase == phases::Link) {
        LLVM_ASSERT(i + 1 == NumSteps && "linking must be final compilation step.");
...
Action *Driver::ConstructPhaseAction(const ArgList &Args, phases::ID Phase,
  ...
  switch (Phase) {
  case phases::Link: LLVM_ASSERT(0 && "link action invalid here.");
  case phases::Preprocess: {
    ...
    if (Args.hasArg(options::OPT_M) || Args.hasArg(options::OPT_MM)) {
      ...
      LLVM_ASSERT(OutputTy != types::TY_INVALID &&
  ...
  LLVM_ASSERT(0 && "invalid phase in ConstructPhaseAction");
  return 0;
}
...
const char *Driver::GetNamedOutputPath(Compilation &C,
  ...
  if (JA.getType() == types::TY_Image) {
    ...
    LLVM_ASSERT(Suffix && "All types used for output should have a suffix.");
