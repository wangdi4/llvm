...
static inline bool operator<(const OptTable::Info &A, const OptTable::Info &B) {
  ...
  LLVM_ASSERT(((A.Kind == Option::JoinedClass) ^ (B.Kind == Option::JoinedClass)) &&
    ...
    TheInputOption(0), TheUnknownOption(0), FirstSearchableIndex(0)
{
  ...
  for (unsigned i = 0, e = getNumOptions(); i != e; ++i) {
    ...
    if (Kind == Option::InputClass) {
      LLVM_ASSERT(!TheInputOption && "Cannot have multiple input options!");
      TheInputOption = getOption(i + 1);
    } else if (Kind == Option::UnknownClass) {
      LLVM_ASSERT(!TheUnknownOption && "Cannot have multiple input options!");
  ...
  LLVM_ASSERT(FirstSearchableIndex != 0 && "No searchable options?");
  ...
  for (unsigned i = FirstSearchableIndex, e = getNumOptions(); i != e; ++i) {
    ...
    LLVM_ASSERT((Kind != Option::InputClass && Kind != Option::UnknownClass &&
  ...
  for (unsigned i = FirstSearchableIndex+1, e = getNumOptions(); i != e; ++i) {
    if (!(getInfo(i) < getInfo(i + 1))) {
      getOption(i)->dump();
      getOption(i + 1)->dump();
      LLVM_ASSERT(0 && "Options are not in order!");
    }
  }
#endif
}
...
Option *OptTable::CreateOption(unsigned id) const {
  ...
  if (info.Flags & RenderJoined) {
    LLVM_ASSERT((info.Kind == Option::JoinedOrSeparateClass ||
...
InputArgList *OptTable::ParseArgs(const char **ArgBegin, const char **ArgEnd,
  ...
  while (Index < End) {
    ...
    LLVM_ASSERT(Index > Prev && "Parser failed to consume argument.");

    // Check for missing argument error.
    if (!A) {
      LLVM_ASSERT(Index >= End && "Unexpected parser error.");
      LLVM_ASSERT(Index - Prev - 1 && "No missing arguments!");
...
static std::string getOptionHelpName(const OptTable &Opts, OptSpecifier Id) {
  ...
  case Option::GroupClass: case Option::InputClass: case Option::UnknownClass:
    LLVM_ASSERT(0 && "Invalid option with help text.");

  case Option::MultiArgClass:
    LLVM_ASSERT(0 && "Cannot print metavar for this kind of option.");
