...
void Clang::AddARMTargetArgs(const ArgList &Args,
  ...
  if (FloatABI == "soft") {
    ...
    LLVM_ASSERT(FloatABI == "hard" && "Invalid float abi!");
    CmdArgs.push_back("-mfloat-abi");
...
void Clang::AddX86TargetArgs(const ArgList &Args,
  ...
  for (arg_iterator it = Args.filtered_begin(options::OPT_m_x86_Features_Group),
    ...
    // Skip over "-m".
    LLVM_ASSERT(Name.startswith("-m") && "Invalid feature name.");
    Name = Name.substr(2);
...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  LLVM_ASSERT(Inputs.size() == 1 && "Unable to handle multiple inputs.");
  ...
  if (isa<AnalyzeJobAction>(JA)) {
    LLVM_ASSERT(JA.getType() == types::TY_Plist && "Invalid output type.");
    ...
    LLVM_ASSERT(isa<CompileJobAction>(JA) && "Invalid action for clang tool.");
    ...
    if (JA.getType() == types::TY_Nothing) {
      ...
      LLVM_ASSERT(JA.getType() == types::TY_PP_Asm &&
  ...
  Args.AddLastArg(CmdArgs, options::OPT_cl_kernel_arg_info);
  ...
  if (Output.getType() == types::TY_Dependencies) {
    ...
    LLVM_ASSERT(Output.isNothing() && "Invalid output.");
  }
...
void ClangAs::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  LLVM_ASSERT(Inputs.size() == 1 && "Unexpected number of inputs.");
  ...
  LLVM_ASSERT(Output.isFilename() && "Unexpected lipo output.");
  CmdArgs.push_back("-o");
  CmdArgs.push_back(Output.getFilename());

  LLVM_ASSERT(Input.isFilename() && "Invalid input.");
...
void gcc::Common::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (Output.isFilename()) {
    ...
    LLVM_ASSERT(Output.isNothing() && "Unexpected output");
    CmdArgs.push_back("-fsyntax-only");
  }
...
const char *darwin::CC1::getCC1Name(types::ID Type) const {
  switch (Type) {
  default:
    LLVM_ASSERT(0 && "Unexpected type for Darwin CC1 tool.");
  case types::TY_Asm:
...
void darwin::Preprocess::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  LLVM_ASSERT(Output.isFilename() && "Unexpected CC1 output.");
...
void darwin::Compile::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (Output.getType() == types::TY_PCH) {
    LLVM_ASSERT(Output.isFilename() && "Invalid PCH output.");
...
void darwin::Link::AddLinkArgs(const ArgList &Args,
  ...
  LLVM_ASSERT((Args.hasArg(options::OPT_mmacosx_version_min_EQ) ||
...
void darwin::Link::ConstructJob(Compilation &C, const JobAction &JA,
                                ...
                                const ArgList &Args,
                                const char *LinkingOutput) const {
  LLVM_ASSERT(Output.getType() == types::TY_Image && "Invalid linker output type.");
...
void darwin::Lipo::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  for (InputInfoList::const_iterator
         it = Inputs.begin(), ie = Inputs.end(); it != ie; ++it) {
    const InputInfo &II = *it;
    LLVM_ASSERT(II.isFilename() && "Unexpected lipo input.");
    CmdArgs.push_back(II.getFilename());
  }
  const char *Exec =
...
void darwin::Dsymutil::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  LLVM_ASSERT(Inputs.size() == 1 && "Unable to handle multiple inputs.");
  const InputInfo &Input = Inputs[0];
  LLVM_ASSERT(Input.isFilename() && "Unexpected dsymutil input.");
  CmdArgs.push_back(Input.getFilename());
