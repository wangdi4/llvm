...
void ASTTypeWriter::VisitBuiltinType(const BuiltinType *T) {
  LLVM_ASSERT(false && "Built-in types are never serialized");
}
...
void ASTTypeWriter::VisitTypedefType(const TypedefType *T) {
  Writer.AddDeclRef(T->getDecl(), Record);
  LLVM_ASSERT(!T->isCanonicalUnqualified() && "Invalid typedef ?");
...
ASTTypeWriter::VisitDependentSizedExtVectorType(
  ...
  LLVM_ASSERT(false && "Cannot serialize dependent sized extended vector types");
}
...
adjustFilenameForRelocatablePCH(const char *Filename, const char *isysroot) {
  LLVM_ASSERT(Filename && "No file name to adjust?");
...
public:
  ...
  void EmitData(llvm::raw_ostream& Out, key_type_ref,
    ...
    LLVM_ASSERT(Out.tell() - Start == DataLen && "Wrong data length");
  }
};
...
public:
  ...
  void EmitKey(llvm::raw_ostream& Out, Selector Sel, unsigned) {
    ...
    LLVM_ASSERT((Start >> 32) == 0 && "Selector key offset too large");
  ...
  void EmitData(llvm::raw_ostream& Out, key_type_ref,
    ...
    LLVM_ASSERT(Out.tell() - Start == DataLen && "Data length is wrong");
  }
};
} // end anonymous namespace
...
void ASTWriter::WriteIdentifierTable(Preprocessor &PP) {
  ...
  // strings.
  {
    ...
    for (llvm::DenseMap<const IdentifierInfo *, IdentID>::iterator
         ...
         ID != IDEnd; ++ID) {
      LLVM_ASSERT(ID->first && "NULL identifier in identifier table");
...
public:
  ...
  void EmitKey(llvm::raw_ostream& Out, DeclarationName Name, unsigned) {
    ...
    LLVM_ASSERT(Name.getNameKind() < 0x100 && "Invalid name kind ?");
    ...
    case DeclarationName::CXXConversionFunctionName:
      ...
      break;
    case DeclarationName::CXXOperatorName:
      LLVM_ASSERT(Name.getCXXOverloadedOperator() < 0x100 && "Invalid operator ?");
...
void ASTWriter::WriteDeclContextVisibleUpdate(const DeclContext *DC) {
  LLVM_ASSERT((DC->isTranslationUnit() || DC->isNamespace()) &&
...
void ASTWriter::SetSelectorOffset(Selector Sel, uint32_t Offset) {
  ...
  LLVM_ASSERT(ID && "Unknown selector");
...
void ASTWriter::WriteASTCore(Sema &SemaRef, MemorizeStatCalls *StatCalls,
  ...
  LLVM_ASSERT(SemaRef.PendingLocalImplicitInstantiations.empty() &&
...
void ASTWriter::WriteASTChain(Sema &SemaRef, MemorizeStatCalls *StatCalls,
  ...
  for (FirstLatestDeclMap::iterator
    ...
    LLVM_ASSERT(I->first->getPCHLevel() > I->second->getPCHLevel() &&
...
TypeIdx ASTWriter::getTypeIdx(QualType T) const {
  ...
  LLVM_ASSERT(I != TypeIdxs.end() && "Type not emitted!");
  return I->second;
}
...
DeclID ASTWriter::getDeclID(const Decl *D) {
  ...
  LLVM_ASSERT(DeclIDs.find(D) != DeclIDs.end() && "Declaration not emitted!");
  return DeclIDs[D];
}
...
ASTWriter::AddTemplateParameterList(const TemplateParameterList *TemplateParams,
                                    RecordData &Record) {
  LLVM_ASSERT(TemplateParams && "No TemplateParams!");
...
ASTWriter::AddTemplateArgumentList(const TemplateArgumentList *TemplateArgs,
                                   RecordData &Record) {
  LLVM_ASSERT(TemplateArgs && "No TemplateArgs!");
...
void ASTWriter::SetReader(ASTReader *Reader) {
  LLVM_ASSERT(Reader && "Cannot remove chain");
  LLVM_ASSERT(FirstDeclID == NextDeclID &&
