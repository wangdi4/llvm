...
void ASTDeclReader::VisitFunctionDecl(FunctionDecl *FD) {
  ...
  default: LLVM_ASSERT(false && "Unhandled TemplatedKind!");
    break;
...
void ASTDeclReader::VisitCXXRecordDecl(CXXRecordDecl *D) {
  ...
  default:
    LLVM_ASSERT(0 && "Out of sync with ASTDeclWriter or messed up reading");
  case Data_NoDefData:
    break;
  case Data_Owner:
  ...
  if (OwnsDefinitionData) {
    ...
    LLVM_ASSERT(Data.Definition && "Data.Definition should be already set!");
    Data.FirstFriend
  ...
  default:
    LLVM_ASSERT(false && "Out of sync with ASTDeclWriter::VisitCXXRecordDecl?");
...
void ASTDeclReader::VisitRedeclarableTemplateDecl(RedeclarableTemplateDecl *D) {
  ...
  LLVM_ASSERT((PrevDecl == 0 || PrevDecl->getKind() == D->getKind()) &&
  ...
  if (PrevDecl == 0) {
    if (RedeclarableTemplateDecl *RTD
          = cast_or_null<RedeclarableTemplateDecl>(Reader.GetDecl(Record[Idx++]))) {
      LLVM_ASSERT(RTD->getKind() == D->getKind() &&
    ...
    LLVM_ASSERT(Reader.GetDecl(ThisDeclID) == D && "Invalid ThisDeclID ?");
    ASTReader::FirstLatestDeclIDMap::iterator I
        = Reader.FirstLatestDeclIDs.find(ThisDeclID);
    if (I != Reader.FirstLatestDeclIDs.end()) {
      Decl *NewLatest = Reader.GetDecl(I->second);
      LLVM_ASSERT((LatestDecl->getLocation().isInvalid() ||
    ...
    LLVM_ASSERT(LatestDecl->getKind() == D->getKind() && "Latest kind mismatch");
...
void ASTDeclReader::VisitRedeclarable(Redeclarable<T> *D) {
  ...
  switch (Kind) {
  default:
    LLVM_ASSERT(0 && "Out of sync with ASTDeclWriter::VisitRedeclarable or messed up"
                " reading");
  ...
  LLVM_ASSERT(!(Kind == PointsToPrevious &&
           Reader.FirstLatestDeclIDs.find(ThisDeclID) !=
  ...
  LLVM_ASSERT(Reader.GetDecl(ThisDeclID) == static_cast<T*>(D) &&
         "Invalid ThisDeclID ?");
  ASTReader::FirstLatestDeclIDMap::iterator I
      = Reader.FirstLatestDeclIDs.find(ThisDeclID);
  if (I != Reader.FirstLatestDeclIDs.end()) {
    Decl *NewLatest = Reader.GetDecl(I->second);
    LLVM_ASSERT((D->getMostRecentDeclaration()->getLocation().isInvalid() ||
            NewLatest->getLocation().isInvalid() ||
            Reader.SourceMgr.isBeforeInTranslationUnit(
...
void ASTReader::ReadAttributes(llvm::BitstreamCursor &DeclsCursor,
  ...
  LLVM_ASSERT(Code == llvm::bitc::UNABBREV_RECORD &&
         "Expected unabbreviated record"); (void)Code;

  RecordData Record;
  unsigned Idx = 0;
  unsigned RecCode = DeclsCursor.ReadRecord(Code, Record);
  LLVM_ASSERT(RecCode == DECL_ATTR && "Expected attribute record");
  (void)RecCode;
...
#include "clang/Serialization/AttrPCHRead.inc"
    ...
    LLVM_ASSERT(New && "Unable to decode attribute?");
    if(New != NULL) {
...
inline void ASTReader::LoadedDecl(unsigned Index, Decl *D) {
  LLVM_ASSERT(!DeclsLoaded[Index] && "Decl loaded twice?");
  DeclsLoaded[Index] = D;
}
...
ASTReader::DeclCursorForIndex(unsigned Index, DeclID ID) {
  ...
  LLVM_ASSERT(F && F->LocalNumDecls > Index && "Broken chain");
...
Decl *ASTReader::ReadDeclRecord(unsigned Index, DeclID ID) {
  ...
  case DECL_CONTEXT_VISIBLE:
    LLVM_ASSERT(false && "Record cannot be de-serialized with ReadDeclRecord");
    break;
  case DECL_TRANSLATION_UNIT:
    LLVM_ASSERT(Index == 0 && "Translation unit must be at index 0");
  ...
  LLVM_ASSERT(D && "Unknown declaration reading AST file");
  if(D != NULL) {
    LoadedDecl(Index, D);
    Reader.Visit(D);
  }
