...
void ASTStmtReader::VisitStmt(Stmt *S) {
  LLVM_ASSERT(Idx == NumStmtFields && "Incorrect statement field count");
}
...
void ASTStmtReader::VisitExpr(Expr *E) {
  ...
  LLVM_ASSERT(Idx == NumExprFields && "Incorrect expression field count");
}
...
void ASTStmtReader::VisitStringLiteral(StringLiteral *E) {
  ...
  LLVM_ASSERT(Record[Idx] == E->getNumConcatenated() &&
...
void ASTStmtReader::VisitOffsetOfExpr(OffsetOfExpr *E) {
  ...
  VisitExpr(E);
  LLVM_ASSERT(E->getNumComponents() == Record[Idx]);
  ++Idx;
  LLVM_ASSERT(E->getNumExpressions() == Record[Idx]);
  ++Idx;
  E->setOperatorLoc(SourceLocation::getFromRawEncoding(Record[Idx++]));
...
void ASTStmtReader::VisitSizeOfAlignOfExpr(SizeOfAlignOfExpr *E) {
  VisitExpr(E);
  E->setKind((SizeOfAlignOfExpr::Kind_t)Record[Idx++]);
  if (Record[Idx] == 0) {
...
void ASTStmtReader::VisitMemberExpr(MemberExpr *E) {
  ...
  LLVM_ASSERT(E->getStmtClass() == Stmt::MemberExprClass &&
...
void ASTStmtReader::VisitCastExpr(CastExpr *E) {
  ...
  LLVM_ASSERT(NumBaseSpecs == E->path_size());
  E->setSubExpr(Reader.ReadSubExpr());
...
void ASTStmtReader::VisitDesignatedInitExpr(DesignatedInitExpr *E) {
  ...
  LLVM_ASSERT(NumSubExprs == E->getNumSubExprs() && "Wrong number of subexprs");
...
void ASTStmtReader::VisitObjCMessageExpr(ObjCMessageExpr *E) {
  VisitExpr(E);
  LLVM_ASSERT(Record[Idx] == E->getNumArgs());
  ++Idx;
  ...
  LLVM_ASSERT(Kind == E->getReceiverKind());
...
void ASTStmtReader::VisitObjCAtTryStmt(ObjCAtTryStmt *S) {
  VisitStmt(S);
  LLVM_ASSERT(Record[Idx] == S->getNumCatchStmts());
  ++Idx;
...
void ASTStmtReader::VisitCXXTryStmt(CXXTryStmt *S) {
  VisitStmt(S);
  LLVM_ASSERT(Record[Idx] == S->getNumHandlers() && "NumStmtFields is wrong ?");
  ++Idx;
...
void ASTStmtReader::VisitCXXDefaultArgExpr(CXXDefaultArgExpr *E) {
  ...
  LLVM_ASSERT(Record[Idx] == (uint64_t)(E->Param.getInt()) && "We messed up at creation ?");
...
ASTStmtReader::VisitCXXUnresolvedConstructExpr(CXXUnresolvedConstructExpr *E) {
  VisitExpr(E);
  LLVM_ASSERT(Record[Idx] == E->arg_size() && "Read wrong record during creation ?");
  ++Idx; // NumArgs;
...
Stmt *ASTReader::ReadStmtFromStream(llvm::BitstreamCursor &Cursor) {
  ...
  while (true) {
    ...
    LLVM_ASSERT(Idx == Record.size() && "Invalid deserialization of statement");
    StmtStack.push_back(S);
  }

#ifndef NDEBUG
  LLVM_ASSERT(StmtStack.size() > PrevNumStmts && "Read too many sub stmts!");
  LLVM_ASSERT(StmtStack.size() == PrevNumStmts + 1 && "Extra expressions on stack!");
#endif
