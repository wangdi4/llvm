...
static bool EqualConcatenations(llvm::SmallVector<llvm::StringRef, 2> L,
  ...
  for (;;) {
    ...
    if (LR.size() == RR.size()) {
      ...
      // length.
      if (LI == LN) {
        LLVM_ASSERT(RI == RN && "Strings not the same length after all?");
        return true;
      }
      LR = L[LI];
      RR = R[RI].Data;
      ...
      if (!RR.startswith(LR))
        return false;
      ++LI;
      LLVM_ASSERT(LI != LN && "Strings not the same length after all?");
...
bool PCHValidator::ReadPredefinesBuffer(const PCHPredefinesBlocks &Buffers,
  ...
  for (unsigned I = 0, N = MissingPredefines.size(); I != N; ++I) {
    ...
    LLVM_ASSERT(EndOfMacroName != std::string::npos &&
    ...
    for (; ConflictPos != CmdLineLines.end(); ++ConflictPos) {
      ...
      LLVM_ASSERT(ConflictPos->size() > MacroDefLen &&
    ...
    if (ConflictPos != CmdLineLines.end()) {
      ...
      LLVM_ASSERT(MacroLoc.second!=llvm::StringRef::npos && "Unable to find macro!");
    ...
    for (unsigned I = 0; I != NumInstanceMethods; ++I) {
	  ...
	  if (Prev){
	    ...
	    Prev = Prev->Next;
	     ...
	     Prev = new (Mem) ObjCMethodList(Method, 0);
    }
...
public:
  ...
  IdentifierInfo *ReadData(const internal_key_type& k,
    ...
    LLVM_ASSERT(Bits == 0 && "Extra bits in the identifier?");
    DataLen -= 6;
    ...
    LLVM_ASSERT(II->isExtensionToken() == ExtensionToken &&
           "Incorrect extension token flag");
    (void)ExtensionToken;
    II->setIsPoisoned(Poisoned);
    LLVM_ASSERT(II->isCPlusPlusOperatorKeyword() == CPlusPlusOperatorKeyword &&
...
bool ASTReader::ParseLineTable(llvm::SmallVectorImpl<uint64_t> &Record) {
  ...
  while (Idx < Record.size()) {
    ...
    LLVM_ASSERT(NumEntries && "Numentries is 00000");
    Entries.clear();
...
private:
	ASTStatCache(const ASTStatCache& a);
	ASTStatCache& operator=(const ASTStatCache& a);
public:
...
llvm::BitstreamCursor &ASTReader::SLocCursorForID(unsigned ID) {
  LLVM_ASSERT(ID != 0 && ID <= TotalNumSLocEntries &&
  ...
  LLVM_ASSERT(F && F->LocalNumSLocEntries > ID && "Chain corrupted");
...
void ASTReader::ReadMacroRecord(llvm::BitstreamCursor &Stream, uint64_t Offset){
  LLVM_ASSERT(PP && "Forgot to set Preprocessor ?");
...
MacroDefinition *ASTReader::getMacroDefinition(IdentID ID) {
  ...
  if (!MacroDefinitionsLoaded[ID]) {
    ...
    LLVM_ASSERT(MacroDefinitionsLoaded[ID] && "Broken chain");
  }
...
ASTReader::ReadASTBlock(PerFileData &F) {
  ...
  while (!Stream.AtEndOfStream()) {
    ...
    case REDECLS_UPDATE_LATEST: {
      LLVM_ASSERT(Record.size() % 2 == 0 && "Expected pairs of DeclIDs");
      for (unsigned i = 0, e = Record.size(); i < e; i += 2) {
        DeclID First = Record[i], Latest = Record[i+1];
        LLVM_ASSERT((FirstLatestDeclIDs.find(First) == FirstLatestDeclIDs.end() ||
...
ASTReader::ASTReadResult ASTReader::ReadAST(const std::string &FileName, llvm::MemoryBuffer* PchBuff) {
  switch(ReadASTCore(FileName, PchBuff)) {
  case Failure: return Failure;
  case IgnorePCH: return IgnorePCH;
  case Success: break;
  }
...
ASTReader::ASTReadResult ASTReader::ReadASTCore(llvm::StringRef FileName, llvm::MemoryBuffer* PchBuff) {
...
void ASTReader::InitializeContext(ASTContext &Ctx) {
  Context = &Ctx;
  LLVM_ASSERT(Context && "Passed null context!");
  ...
  if (unsigned Sigjmp_buf = SpecialTypes[SPECIAL_TYPE_sigjmp_buf]) {
    ...
    if (const TypedefType *Typedef = Sigjmp_bufType->getAs<TypedefType>())
      ...
      LLVM_ASSERT(Tag && "Invalid sigjmp_buf type in AST file");
...
ASTReader::RecordLocation ASTReader::TypeCursorForIndex(unsigned Index) {
  ...
  LLVM_ASSERT(F && F->LocalNumTypes > Index && "Broken chain");
...
QualType ASTReader::GetType(TypeID ID) {
  ...
  if (Index < NUM_PREDEF_TYPE_IDS) {
    ...
    LLVM_ASSERT(!T.isNull() && "Unknown predefined type");
    return T.withFastQualifiers(FastQuals);
  }

  Index -= NUM_PREDEF_TYPE_IDS;
  LLVM_ASSERT(Index < TypesLoaded.size() && "Type index out-of-range");
...
TypeIdx ASTReader::GetTypeIdx(QualType T) const {
  if (T.isNull())
    return TypeIdx();
  LLVM_ASSERT(!T.getLocalFastQualifiers());
...
bool ASTReader::FindExternalLexicalDecls(const DeclContext *DC,
  ...
  LLVM_ASSERT(DC->hasExternalLexicalStorage() &&
...
ASTReader::FindExternalVisibleDeclsByName(const DeclContext *DC,
  ...
  LLVM_ASSERT(DC->hasExternalVisibleStorage() &&
...
void ASTReader::InitializeSema(Sema &S) {
  ...
  LLVM_ASSERT(PendingInstantiations.size() % 2 == 0 && "Expected pairs of entries");
  ...
  if (!SemaDeclRefs.empty()) {
    LLVM_ASSERT(SemaDeclRefs.size() == 2 && "More decl refs than expected!");
...
void ASTReader::SetIdentifierInfo(unsigned ID, IdentifierInfo *II) {
  LLVM_ASSERT(ID && "Non-zero identifier ID required");
  LLVM_ASSERT(ID <= IdentifiersLoaded.size() && "identifier ID out of range");
...
ASTReader::ReadTemplateName(const RecordData &Record, unsigned &Idx) {
  ...
  LLVM_ASSERT(0 && "Unhandled template name kind!");
  return TemplateName();
}
...
ASTReader::ReadTemplateArgument(llvm::BitstreamCursor &DeclsCursor,
  ...
  LLVM_ASSERT(0 && "Unhandled template argument kind!");
  return TemplateArgument();
}
...
void ASTReader::RecordSwitchCaseID(SwitchCase *SC, unsigned ID) {
  LLVM_ASSERT(SwitchCaseStmts[ID] == 0 && "Already have a SwitchCase with this ID");
  SwitchCaseStmts[ID] = SC;
}

/// \brief Retrieve the switch-case statement with the given ID.
SwitchCase *ASTReader::getSwitchCaseWithID(unsigned ID) {
  LLVM_ASSERT(SwitchCaseStmts[ID] != 0 && "No SwitchCase with this ID");
...
void ASTReader::RecordLabelStmt(LabelStmt *S, unsigned ID) {
  LLVM_ASSERT(LabelStmts.find(ID) == LabelStmts.end() &&
...
void ASTReader::FinishedDeserializing() {
  LLVM_ASSERT(NumCurrentElementsDeserializing &&
