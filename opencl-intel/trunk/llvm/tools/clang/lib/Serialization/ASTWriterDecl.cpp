...
void ASTDeclWriter::VisitParmVarDecl(ParmVarDecl *D) {
  ...
  // just us assuming it.
  LLVM_ASSERT(!D->isInvalidDecl() && "Shouldn't emit invalid decls");
  LLVM_ASSERT(!D->isThreadSpecified() && "PARM_VAR_DECL can't be __thread");
  LLVM_ASSERT(D->getAccess() == AS_none && "PARM_VAR_DECL can't be public/private");
  LLVM_ASSERT(!D->isExceptionVariable() && "PARM_VAR_DECL can't be exception var");
  LLVM_ASSERT(D->getPreviousDeclaration() == 0 && "PARM_VAR_DECL can't be redecl");
  LLVM_ASSERT(!D->isStaticDataMember() &&
...
void ASTDeclWriter::VisitCXXRecordDecl(CXXRecordDecl *D) {
  ...
  if (D->DefinitionData) {
    LLVM_ASSERT(D->DefinitionData->Definition &&
...
void ASTDeclWriter::VisitRedeclarableTemplateDecl(RedeclarableTemplateDecl *D) {
  ...
  if (D->getPreviousDeclaration() == 0) {
    ...
    if (First->getMostRecentDeclaration() == D &&
      ...
      LLVM_ASSERT(Writer.FirstLatestDecls.find(First)==Writer.FirstLatestDecls.end()
...
void ASTDeclWriter::VisitClassTemplateDecl(ClassTemplateDecl *D) {
  ...
  if (D->getPreviousDeclaration() == 0) {
    ...
    for (CTSDSetTy::iterator I=CTSDSet.begin(), E = CTSDSet.end(); I!=E; ++I) {
      LLVM_ASSERT(I->isCanonicalDecl() && "Expected only canonical decls in set");
      Writer.AddDeclRef(&*I, Record);
    }
...
void ASTDeclWriter::VisitFunctionTemplateDecl(FunctionTemplateDecl *D) {
  ...
  if (D->getPreviousDeclaration() == 0) {
    ...
    for (llvm::FoldingSet<FunctionTemplateSpecializationInfo>::iterator
      ...
      LLVM_ASSERT(I->Function->isCanonicalDecl() &&
