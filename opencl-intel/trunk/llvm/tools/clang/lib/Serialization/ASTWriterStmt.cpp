...
void ASTStmtWriter::VisitDeclRefExpr(DeclRefExpr *E) {
  ...
  LLVM_ASSERT((NumTemplateArgs != 0) == E->hasExplicitTemplateArgs() &&
...
void ASTStmtWriter::VisitDesignatedInitExpr(DesignatedInitExpr *E) {
  ...
  for (DesignatedInitExpr::designators_iterator D = E->designators_begin(),
    ...
    if (D->isFieldDesignator()) {
      ...
      LLVM_ASSERT(D->isArrayRangeDesignator() && "Unknown designator");
...
ASTStmtWriter::VisitCXXDependentScopeMemberExpr(CXXDependentScopeMemberExpr *E){
  ...
  if (E->hasExplicitTemplateArgs()) {
    ...
    LLVM_ASSERT(Args.NumTemplateArgs &&
...
unsigned ASTWriter::RecordSwitchCaseID(SwitchCase *S) {
  LLVM_ASSERT(SwitchCaseIDs.find(S) == SwitchCaseIDs.end() &&
...
void ASTWriter::WriteSubStmt(Stmt *S) {
  ...
  if (Writer.Code == serialization::STMT_NULL_PTR) {
    ...
    S->dump(SrcMgr);
    LLVM_ASSERT(0 && "Unhandled sub statement writing AST file");
  }
#endif
...
void ASTWriter::FlushStmts() {
  ...
  for (unsigned I = 0, N = StmtsToEmit.size(); I != N; ++I) {
    ...
    LLVM_ASSERT(N == StmtsToEmit.size() &&
