...
CompilerInstance::CompilerInstance()
  : Invocation(new CompilerInvocation()), InputBuffer(NULL), OutputStream(NULL), ErrorStream(NULL), PchBuffer(NULL) {
}
...
void CompilerInstance::createDiagnostics(int Argc, char **Argv) {
  Diagnostics = createDiagnostics(getDiagnosticOpts(), Argc, Argv, ErrorStream);
}
...
CompilerInstance::createDiagnostics(const DiagnosticOptions &Opts,
  ...
  if (Opts.BinaryOutput) {
    ...
    if(errStream) {
      Diags->setClient(new TextDiagnosticPrinter(*errStream, Opts));
	} else {
      Diags->setClient(new TextDiagnosticPrinter(llvm::errs(), Opts));
	}
  }
...
CompilerInstance::createPCHExternalASTSource(llvm::StringRef Path,
  ...
  ASTReader::ASTReadResult res = ASTReader::Success;

  res = Reader->ReadAST(Path, PP.getPCH());
...
void CompilerInstance::addOutputFile(llvm::StringRef Path,
                                     llvm::raw_ostream *OS) {
  LLVM_ASSERT(OS && "Attempt to add empty stream to output list!");
...
bool CompilerInstance::InitializeSourceManager(llvm::StringRef InputFile) {
  if (NULL != InputBuffer) {
    getSourceManager().createMainFileIDForMemBuffer(InputBuffer);
    if (getSourceManager().getMainFileID().isInvalid()) {
      getDiagnostics().Report(diag::err_fe_error_reading_memory);
      return false;
    } 
...
bool CompilerInstance::ExecuteAction(FrontendAction &Act) {
  LLVM_ASSERT(hasDiagnostics() && "Diagnostics engine is not initialized!");
  LLVM_ASSERT(!getFrontendOpts().ShowHelp && "Client must handle '-help'!");
  LLVM_ASSERT(!getFrontendOpts().ShowVersion && "Client must handle '-version'!");
