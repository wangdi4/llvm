...
bool FrontendAction::BeginSourceFile(CompilerInstance &CI,
                                     ...
                                     InputKind InputKind) {
  LLVM_ASSERT(!Instance && "Already processing a source file!");
  LLVM_ASSERT(!Filename.empty() && "Unexpected empty filename!");
  ...
  // AST unit.
  if (InputKind == IK_AST) {
    LLVM_ASSERT(!usesPreprocessorOnly() &&
           "Attempt to pass AST file to preprocessor only action!");
    LLVM_ASSERT(hasASTFileSupport() &&
  ...
  if (!usesPreprocessorOnly()) {
    ...
    /// Use PCH?
    if (( NULL != CI.GetPchBuffer() ) || (!CI.getPreprocessorOpts().ImplicitPCHInclude.empty())) {
      LLVM_ASSERT(hasPCHSupport() && "This action does not have PCH support!");
