...
const std::string &ASTUnit::getASTFileName() {
  LLVM_ASSERT(isMainFileAST() && "Not an ASTUnit from an AST file!");
...
bool ASTUnit::Parse(llvm::MemoryBuffer *OverrideMainBuffer) {
  ...
  LLVM_ASSERT(Clang.getFrontendOpts().Inputs.size() == 1 &&
         "Invocation must have exactly one source file!");
  LLVM_ASSERT(Clang.getFrontendOpts().Inputs[0].first != IK_AST &&
         "FIXME: AST inputs not yet supported here!");
  LLVM_ASSERT(Clang.getFrontendOpts().Inputs[0].first != IK_LLVM_IR &&
