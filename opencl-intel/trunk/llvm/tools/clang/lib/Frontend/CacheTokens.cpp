...
public:
  ...
  Offset getOffset() const { LLVM_ASSERT(valid); return off; }
...
PTHEntry PTHWriter::LexTokens(Lexer& L) {
  ...
  NextToken:
    ...
    if (Tok.is(tok::hash) && Tok.isAtStartOfLine()) {
      ...
      // the next token.
      LLVM_ASSERT(!ParsingPreprocessorDirective);
      ...
      case tok::pp_include_next: {
        ...
        LLVM_ASSERT(!Tok.isAtStartOfLine());
        if (Tok.is(tok::identifier))
          PP.LookUpIdentifierInfo(Tok);
      ...
      case tok::pp_endif: {
        ...
        LLVM_ASSERT(!PPStartCond.empty());
        LLVM_ASSERT(PPCond.size() > PPStartCond.back());
        LLVM_ASSERT(PPCond[PPStartCond.back()].second == 0);
  ...
  LLVM_ASSERT(PPStartCond.empty() && "Error: imblanced preprocessor conditionals.");
  ...
  for (unsigned i = 0, e = PPCond.size(); i!=e; ++i) {
    ...
    LLVM_ASSERT(x != 0 && "PPCond entry not backpatched.");
...
std::pair<Offset,Offset> PTHWriter::EmitIdentifierTable() {
  ...
  for (IDMap::iterator I = IM.begin(), E = IM.end(); I != E; ++I) {
    ...
    LLVM_ASSERT(I->second > 0);
    LLVM_ASSERT(I->second-1 < idcount);
