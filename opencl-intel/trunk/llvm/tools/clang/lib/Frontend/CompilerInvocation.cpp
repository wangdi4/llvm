...
static void CodeGenOptsToArgs(const CodeGenOptions &Opts,
  ...
  if (Opts.OptimizeSize) {
    LLVM_ASSERT(Opts.OptimizationLevel == 2 && "Invalid options!");
    Res.push_back("-Os");
...
static void FrontendOptsToArgs(const FrontendOptions &Opts,
  ...
  for (unsigned i = 0, e = Opts.Inputs.size(); i != e; ++i) {
    LLVM_ASSERT((!NeedLang || Opts.Inputs[i].first == Opts.Inputs[0].first) &&
  ...
  for (unsigned i = 0, e = Opts.UserEntries.size(); i != e; ++i) {
    ...
    if (E.IsUserSupplied) {
      if (E.Group == frontend::After) {
        ...
        LLVM_ASSERT(E.Group == frontend::Angled && "Invalid group!");
...
static void LangOptsToArgs(const LangOptions &Opts,
  ...
  if (Opts.getGCMode() != LangOptions::NonGC) {
    if (Opts.getGCMode() == LangOptions::HybridGC) {
      ...
      LLVM_ASSERT(Opts.getGCMode() == LangOptions::GCOnly && "Invalid GC mode!");
  ...
  if (Opts.getVisibilityMode() != LangOptions::Default) {
    ...
    if (Opts.getVisibilityMode() == LangOptions::Hidden) {
      ...
      LLVM_ASSERT(Opts.getVisibilityMode() == LangOptions::Protected &&
             "Invalid visibility!");
...
static void PreprocessorOptsToArgs(const PreprocessorOptions &Opts,
  ...
  if (!Opts.TokenCache.empty()) {
    if (Opts.ImplicitPTHInclude.empty()) {
      ...
      LLVM_ASSERT(Opts.ImplicitPTHInclude == Opts.TokenCache &&
...
static InputKind ParseFrontendArgs(FrontendOptions &Opts, ArgList &Args,
  ...
  if (const Arg *A = Args.getLastArg(OPT_Action_Group)) {
    ...
    default:
      LLVM_ASSERT(0 && "Invalid option in group!");
    case OPT_ast_dump:
...
#include "clang/Frontend/LangStandards.def"
  ...
  if (LangStd == LangStandard::lang_unspecified) {
    ...
    case IK_AST:
    case IK_LLVM_IR:
      LLVM_ASSERT(0 && "Invalid input kind!");
    case IK_OpenCL:
  ...
  if (LangStd == LangStandard::lang_opencl) {
    ...
    Opts.LaxVectorConversions = 1;  // we need this for compiling our built-in function files

    Opts.OpenCLBuiltins = 0;
    Opts.OpenCLKernelRefl = 0;

    if(Args.hasArg(OPT_opencl_builtins))
      Opts.OpenCLBuiltins = 1;

    if(Args.hasArg(OPT_cl_kernel_arg_info))
      Opts.OpenCLKernelRefl = 1;
  }
