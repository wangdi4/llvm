...
void InitHeaderSearch::AddPath(const llvm::Twine &Path,
                               ...
                               bool IgnoreSysRoot) {
  LLVM_ASSERT(!Path.isTriviallyEmpty() && "can't handle empty path here");
    ...
    if (lResult == ERROR_SUCCESS) {
      ...
      for (index = 0; RegEnumKeyEx(hTopKey, index, keyName, &size, NULL,
        ...
        if (value > bestValue) {
          bestIndex = (int)index;
          bestValue = value;
		  strncpy(bestName, keyName, sizeof(bestName) - 1);
...
static void RemoveDuplicates(std::vector<DirectoryLookup> &SearchList,
  ...
  for (unsigned i = 0; i != SearchList.size(); ++i) {
    ...
    if (CurEntry.isNormalDir()) {
      ...
      LLVM_ASSERT(CurEntry.isHeaderMap() && "Not a headermap or normal dir?");
    ...
    if (CurEntry.getDirCharacteristic() != SrcMgr::C_User) {
      ...
      for (FirstDir = 0; ; ++FirstDir) {
        LLVM_ASSERT(FirstDir != i && "Didn't find dupe?");
  ...
  if (Verbose) {
    ...
    for (unsigned i = 0, e = SearchList.size(); i != e; ++i) {
      ...
      if (SearchList[i].isNormalDir())
        ...
        LLVM_ASSERT(SearchList[i].isHeaderMap() && "Unknown DirectoryLookup");
        Suffix = " (headermap)";
      }
