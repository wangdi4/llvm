...
void TextDiagnosticPrinter::HighlightRange(const CharSourceRange &R,
  ...
  LLVM_ASSERT(CaretLine.size() == SourceLine.size() &&
  ...
  LLVM_ASSERT(StartColNo <= EndColNo && "Invalid range!");
  ...
  if (R.isTokenRange()) {
    ...
    LLVM_ASSERT(StartColNo <= EndColNo && "Trying to highlight whitespace??");
  }
...
static void SelectInterestingSourceRegion(std::string &SourceLine,
  ...
  while ((CaretEnd - CaretStart) < TargetColumns) {
    ...
    if (CaretEnd != SourceLength) {
      LLVM_ASSERT(CaretEnd < SourceLength && "Unexpected caret position!");
...
void TextDiagnosticPrinter::EmitCaretDiagnostic(SourceLocation Loc,
  ...
  LLVM_ASSERT(LangOpts && "Unexpected diagnostic outside source file processing");
  LLVM_ASSERT(!Loc.isInvalid() && "must have a valid source location here");
  ...
  for (unsigned i = 0; i != SourceLine.size(); ++i) {
    ...
    LLVM_ASSERT(0 < TabStop && TabStop <= DiagnosticOptions::MaxTabStop &&
           "Invalid -ftabstop value");
    unsigned NumSpaces = ((i+TabStop)/TabStop * TabStop) - (i+1);
    LLVM_ASSERT(NumSpaces < TabStop && "Invalid computation of space amt");
...
static unsigned findEndOfWord(unsigned Start,
                              ...
                              unsigned Columns) {
  LLVM_ASSERT(Start < Str.size() && "Invalid start position!");
...
void TextDiagnosticPrinter::HandleDiagnostic(Diagnostic::Level Level,
  ...
  if (Info.getLocation().isValid()) {
    ...
    if (PLoc.isInvalid()) {
      ...
      if (!FID.isInvalid()) {
        ...
        if (FE && FE->getName()) {
          OS << FE->getName();
          if (FE->getDevice() == 0 && FE->getInode() == 0 && FE->getFileMode() == 0) {
  ...
  if (DiagOpts->ShowColors) {
    ...
    switch (Level) {
    case Diagnostic::Ignored: LLVM_ASSERT(0 && "Invalid diagnostic type");
  ...
  if (!OptionName.empty() || DiagCategory != 0) {
    ...
    if (DiagCategory) {
      ...
      if (DiagOpts->ShowCategories == 1)
        ...
        LLVM_ASSERT(DiagOpts->ShowCategories == 2 && "Invalid ShowCategories value");
  ...
  if (DiagOpts->ShowCarets && Info.getLocation().isValid() &&
    ...
    LLVM_ASSERT(NumRanges < 20 && "Out of space");
    for (unsigned i = 0; i != NumRanges; ++i)
