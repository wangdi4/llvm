...
void RewriteObjC::RewritePropertyImplDecl(ObjCPropertyImplDecl *PID,
  ...
  LLVM_ASSERT((*startBuf == '@') && "bogus @synthesize location");
  const char *semiBuf = strchr(startBuf, ';');
  LLVM_ASSERT((*semiBuf == ';') && "@synthesize: can't find ';'");
...
void RewriteObjC::RewriteForwardProtocolDecl(ObjCForwardProtocolDecl *PDecl) {
  ...
  {
    LLVM_ASSERT(false && "Invalid SourceLocation");
  }
...
Stmt *RewriteObjC::RewriteObjCIvarRefExpr(ObjCIvarRefExpr *IV,
  ...
  if (CurMethodDef) {
    if (BaseExpr->getType()->isObjCObjectPointerType()) {
      ...
      LLVM_ASSERT(iFaceDecl && "RewriteObjCIvarRefExpr - iFaceDecl is null");
      // lookup which class implements the instance variable.
      ObjCInterfaceDecl *clsDeclared = 0;
      iFaceDecl->getDecl()->lookupInstanceVariable(D->getIdentifier(),
                                                   clsDeclared);
      LLVM_ASSERT(clsDeclared && "RewriteObjCIvarRefExpr(): Can't find class");
      ...
      RecordDecl *RD = RecordDecl::Create(*Context, TTK_Struct, TUDecl,
                                          SourceLocation(), II);
      LLVM_ASSERT(RD && "RewriteObjCIvarRefExpr(): Can't find RecordDecl");
    ...
    LLVM_ASSERT(!IV->isFreeIvar() && "Cannot have a free standing ivar outside a method");
...
Stmt *RewriteObjC::RewriteObjCForCollectionStmt(ObjCForCollectionStmt *S,
                                                SourceLocation OrigEnd) {
  LLVM_ASSERT(!Stmts.empty() && "ObjCForCollectionStmt - Statement stack empty");
  LLVM_ASSERT(isa<ObjCForCollectionStmt>(Stmts.back()) &&
         "ObjCForCollectionStmt Statement stack mismatch");
  LLVM_ASSERT(!ObjCBcLabelNo.empty() &&
  ...
  if (isa<CompoundStmt>(S->getBody())) {
    ...
    LLVM_ASSERT(semiBuf && "Can't find ';'");
...
Stmt *RewriteObjC::RewriteObjCSynchronizedStmt(ObjCAtSynchronizedStmt *S) {
  ...
  LLVM_ASSERT((*startBuf == '@') && "bogus @synchronized location");
  ...
  LLVM_ASSERT((*startBuf == '}') && "bogus @synchronized block");
...
void RewriteObjC::RewriteTryReturnStmts(Stmt *S) {
 ...
 if (isa<ReturnStmt>(S)) {
   ...
   const char *semiBuf = strchr(startBuf, ';');
   LLVM_ASSERT((*semiBuf == ';') && "RewriteTryReturnStmts: can't find ';'");
...
void RewriteObjC::RewriteSyncReturnStmts(Stmt *S, std::string syncExitBuf) {
  ...
  if (isa<ReturnStmt>(S)) {
    ...
    const char *semiBuf = strchr(startBuf, ';');
    LLVM_ASSERT((*semiBuf == ';') && "RewriteSyncReturnStmts: can't find ';'");
    SourceLocation onePastSemiLoc = startLoc.getFileLocWithOffset(semiBuf-startBuf+1);
...
Stmt *RewriteObjC::RewriteObjCTryStmt(ObjCAtTryStmt *S) {
  ...
  LLVM_ASSERT((*startBuf == '@') && "bogus @try location");
  ...
  LLVM_ASSERT((*startBuf == '}') && "bogus @try block");
  ...
  for (unsigned I = 0, N = S->getNumCatchStmts(); I != N; ++I) {
    ...
    LLVM_ASSERT((*startBuf == '@') && "bogus @catch location");
    ...
    if (Catch->hasEllipsis()) {
      ...
      LLVM_ASSERT(*SM->getCharacterData(Catch->getRParenLoc()) == ')' &&
             "bogus @catch paren location");
      LLVM_ASSERT((*bodyBuf == '{') && "bogus @catch body location");
      ...
      LLVM_ASSERT((*rParenBuf == ')') && "bogus @catch paren location");
      LLVM_ASSERT((*bodyBuf == '{') && "bogus @catch body location");
      ...
      LLVM_ASSERT(false && "@catch rewrite bug");
    }
  }
  // Complete the catch list...
  if (lastCatchBody) {
    SourceLocation bodyLoc = lastCatchBody->getLocEnd();
    LLVM_ASSERT(*SM->getCharacterData(bodyLoc) == '}' &&
  ...
  if (ObjCAtFinallyStmt *finalStmt = S->getFinallyStmt()) {
    ...
    startBuf = SM->getCharacterData(startLoc);
    LLVM_ASSERT((*startBuf == '@') && "bogus @finally start");
    ...
    LLVM_ASSERT(*SM->getCharacterData(startLoc) == '{' &&
           "bogus @finally body location");
    LLVM_ASSERT(*SM->getCharacterData(endLoc) == '}' &&
           "bogus @finally body location");
...
Stmt *RewriteObjC::RewriteObjCThrowStmt(ObjCAtThrowStmt *S) {
  ...
  LLVM_ASSERT((*startBuf == '@') && "bogus @throw location");
  ...
  LLVM_ASSERT((*wBuf == 'w') && "@throw: can't find 'w'");
  ReplaceText(startLoc, wBuf-startBuf+1, buf);

  const char *semiBuf = strchr(startBuf, ';');
  LLVM_ASSERT((*semiBuf == ';') && "@throw: can't find ';'");
...
Stmt *RewriteObjC::RewriteAtSelector(ObjCSelectorExpr *Exp) {
  ...
  LLVM_ASSERT(SelGetUidFunctionDecl && "Can't find sel_registerName() decl");
...
static void scanToNextArgument(const char *&argRef) {
  ...
  LLVM_ASSERT(angle == 0 && "scanToNextArgument - bad protocol type syntax");
}
...
void RewriteObjC::RewriteObjCQualifiedInterfaceTypes(Decl *Dcl) {
  ...
  if (VarDecl *VD = dyn_cast<VarDecl>(Dcl)) {
    ...
    LLVM_ASSERT(funcType && "missing function type");
...
void RewriteObjC::SynthSuperContructorFunctionDecl() {
  ...
  LLVM_ASSERT(!argT.isNull() && "Can't find 'id' type");
...
void RewriteObjC::SynthMsgSendFunctionDecl() {
  ...
  LLVM_ASSERT(!argT.isNull() && "Can't find 'SEL' type");
  ArgTys.push_back(argT);
  QualType msgSendType = Context->getFunctionType(Context->getObjCIdType(),
                                                  &ArgTys[0], ArgTys.size(),
                                                  true /*isVariadic*/, 0,
...
void RewriteObjC::SynthMsgSendSuperFunctionDecl() {
  ...
  LLVM_ASSERT(!argT.isNull() && "Can't build 'struct objc_super *' type");
  ArgTys.push_back(argT);
  argT = Context->getObjCSelType();
  LLVM_ASSERT(!argT.isNull() && "Can't find 'SEL' type");
  ArgTys.push_back(argT);
...
Stmt *RewriteObjC::SynthMessageExpr(ObjCMessageExpr *Exp,
  ...
  case ObjCMessageExpr::SuperClass: {
    ...
    LLVM_ASSERT(MsgSendFlavor && "MsgSendFlavor is NULL!");
  ...
  if (MsgSendStretFlavor) {
	...
	SizeOfAlignOfExpr *sizeofExpr = new (Context) SizeOfAlignOfExpr(SizeOfAlignOfExpr::SizeOf,
...
void RewriteObjC::SynthesizeObjCInternalStruct(ObjCInterfaceDecl *CDecl,
                                               std::string &Result) {
  LLVM_ASSERT(CDecl && "Class missing in SynthesizeObjCInternalStruct");
  LLVM_ASSERT(CDecl->getName() != "" &&
  ...
  {
    LLVM_ASSERT(false && "struct already synthesize- SynthesizeObjCInternalStruct");
  }
}
...
RewriteObjCProtocolMetaData(ObjCProtocolDecl *PDecl, llvm::StringRef prefix,
  ...
  {
    LLVM_ASSERT(false && "protocol already synthesized");
  }
...
void RewriteObjC::RewriteByRefString(std::string &ResultStr, 
                                     ...
                                     ValueDecl *VD) {
  LLVM_ASSERT(BlockByRefDeclNo.count(VD) && 
...
std::string RewriteObjC::SynthesizeBlockFunc(BlockExpr *CE, int i,
  ...
  if (isa<FunctionNoProtoType>(AFT)) {
    ...
    LLVM_ASSERT(FT && "SynthesizeBlockFunc: No function proto");
    S += '(';
...
Stmt *RewriteObjC::SynthesizeBlockCall(CallExpr *Exp, const Expr *BlockExp) {
  ...
  if (const DeclRefExpr *DRE = dyn_cast<DeclRefExpr>(BlockExp)) {
    ...
    LLVM_ASSERT(1 && "RewriteBlockClass: Bad type");
  }
  LLVM_ASSERT(CPT && "RewriteBlockClass: Bad type");
  const FunctionType *FT = CPT->getPointeeType()->getAs<FunctionType>();
  LLVM_ASSERT(FT && "RewriteBlockClass: Bad type");
  const FunctionProtoType *FTP = dyn_cast<FunctionProtoType>(FT);
  // FTP will be null for closures that don't take arguments.
...
void RewriteObjC::RewriteBlockPointerFunctionArgs(FunctionDecl *FD) {
  ...
  LLVM_ASSERT((*startArgList == '(') && "Rewriter fuzzy parser confused");

  parenCount++;
  // advance the location to startArgList.
  DeclLoc = DeclLoc.getFileLocWithOffset(startArgList-startBuf);
  LLVM_ASSERT((DeclLoc.isValid()) && "Invalid DeclLoc");
...
bool RewriteObjC::PointerTypeTakesAnyBlockArguments(QualType QT) {
  ...
  if (PT) {
    ...
    LLVM_ASSERT(BPT && "BlockPointerTypeTakeAnyBlockArguments(): not a block pointer type");
...
void RewriteObjC::GetExtentOfArgList(const char *Name, const char *&LParen,
  ...
  LLVM_ASSERT((*argPtr == '(') && "Rewriter fuzzy parser confused");
...
void RewriteObjC::RewriteBlockPointerDecl(NamedDecl *ND) {
  ...
  if (VarDecl *VD = dyn_cast<VarDecl>(ND))
    ...
    LLVM_ASSERT(0 && "RewriteBlockPointerDecl(): Decl type not yet handled");
...
void RewriteObjC::RewriteByRefVar(VarDecl *ND) {
  ...
  if (CurFunctionDef)
    ...
    LLVM_ASSERT(CurMethodDef && "RewriteByRefVar - CurMethodDef is null");
  ...
  if (!hasInit) {
    ...
    LLVM_ASSERT((*semiBuf == ';') && "RewriteByRefVar: can't find ';'");
 ...
 int countOfInnerDecls = 0;
  ...
  if (BlockDeclRefs.size()) {
    ...
    for (llvm::SmallVector<ValueDecl*,8>::iterator I = BlockByRefDecls.begin(),
      ...
      RecordDecl *RD = RecordDecl::Create(*Context, TTK_Struct, TUDecl,
                                          SourceLocation(), II);
      LLVM_ASSERT(RD && "SynthBlockInitExpr(): Can't find RecordDecl");
      QualType castT = Context->getPointerType(Context->getTagDeclType(RD));
...
Stmt *RewriteObjC::RewriteFunctionBodyOrGlobalInitializer(Stmt *S) {
      ...
      if (ValueDecl *ND = dyn_cast<ValueDecl>(SD)) {
        ...
        if (VarDecl *VD = dyn_cast<VarDecl>(SD)) {
          if (VD->hasAttr<BlocksAttr>()) {
            ...
            LLVM_ASSERT(!BlockByRefDeclNo.count(ND) &&
  ...
  if (isa<SwitchStmt>(S) || isa<WhileStmt>(S) ||
      isa<DoStmt>(S) || isa<ForStmt>(S)) {
    LLVM_ASSERT(!Stmts.empty() && "Statement stack is empty");
