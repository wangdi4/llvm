...
namespace {
  ...
  public:
    ...
    const SourceDelta &getValue(unsigned i) const {
      LLVM_ASSERT(i < NumValuesUsed && "Invalid value #");
      return Values[i];
    }
...
namespace {
  ...
  public:
    ...
    const DeltaTreeNode *getChild(unsigned i) const {
      LLVM_ASSERT(i < getNumValuesUsed()+1 && "Invalid child");
      return Children[i];
    }
...
bool DeltaTreeNode::DoInsertion(unsigned FileIndex, int Delta,
  ...
  if (isLeaf()) {
    ...
    LLVM_ASSERT(InsertRes && "No result location specified");
    DoSplit(*InsertRes);
...
void DeltaTreeNode::DoSplit(InsertResult &InsertRes) {
  LLVM_ASSERT(isFull() && "Why split a non-full node?");
...
static void VerifyTree(const DeltaTreeNode *N) {
  ...
  if (IN == 0) {
    ...
    for (unsigned i = 0, e = N->getNumValuesUsed(); i != e; ++i) {
      if (i)
        LLVM_ASSERT(N->getValue(i-1).FileLoc < N->getValue(i).FileLoc);
      FullDelta += N->getValue(i).Delta;
    }
    LLVM_ASSERT(FullDelta == N->getFullDelta());
    return;
  }
...
DeltaTree::DeltaTree(const DeltaTree &RHS) {
  ...
  LLVM_ASSERT(getRoot(RHS.Root)->getNumValuesUsed() == 0 &&
...
void DeltaTree::AddDelta(unsigned FileIndex, int Delta) {
  LLVM_ASSERT(Delta && "Adding a noop?");
