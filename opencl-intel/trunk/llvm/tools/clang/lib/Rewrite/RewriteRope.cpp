...
namespace {
  ...
  public:
    ...
    const RopePiece &getPiece(unsigned i) const {
      LLVM_ASSERT(i < getNumPieces() && "Invalid piece ID");
      return Pieces[i];
    }

    const RopePieceBTreeLeaf *getNextLeafInOrder() const { return NextLeaf; }
    void insertAfterLeafInOrder(RopePieceBTreeLeaf *Node) {
      LLVM_ASSERT(PrevLeaf == 0 && NextLeaf == 0 && "Already in ordering");
...
RopePieceBTreeNode *RopePieceBTreeLeaf::insert(unsigned Offset,
  ...
  if (!isFull()) {
    ...
    if (Offset == size()) {
      ...
      LLVM_ASSERT(SlotOffs == Offset && "Split didn't occur before insertion!");
    }
...
void RopePieceBTreeLeaf::erase(unsigned Offset, unsigned NumBytes) {
  ...
  LLVM_ASSERT(PieceOffs == Offset && "Split didn't occur before erase!");
  ...
  LLVM_ASSERT(getPiece(StartPiece).size() > NumBytes);
...
namespace {
  ...
  public:
    ...
    const RopePieceBTreeNode *getChild(unsigned i) const {
      LLVM_ASSERT(i < NumChildren && "invalid child #");
      return Children[i];
    }
...
RopePieceBTreeNode *RopePieceBTreeNode::split(unsigned Offset) {
  LLVM_ASSERT(Offset <= size() && "Invalid offset to split!");
  if (RopePieceBTreeLeaf *Leaf = dyn_cast<RopePieceBTreeLeaf>(this))
...
RopePieceBTreeNode *RopePieceBTreeNode::insert(unsigned Offset,
                                               const RopePiece &R) {
  LLVM_ASSERT(Offset <= size() && "Invalid offset to insert!");
  if (RopePieceBTreeLeaf *Leaf = dyn_cast<RopePieceBTreeLeaf>(this))
...
void RopePieceBTreeNode::erase(unsigned Offset, unsigned NumBytes) {
  LLVM_ASSERT(Offset+NumBytes <= size() && "Invalid offset to erase!");
  if (RopePieceBTreeLeaf *Leaf = dyn_cast<RopePieceBTreeLeaf>(this))
...
RopePieceBTree::RopePieceBTree(const RopePieceBTree &RHS) {
  LLVM_ASSERT(RHS.empty() && "Can't copy non-empty tree yet");
  Root = new RopePieceBTreeLeaf();
}
RopePieceBTree::~RopePieceBTree() {
...
RopePiece RewriteRope::MakeRopeString(const char *Start, const char *End) {
  ...
  LLVM_ASSERT(Len && "Zero length RopePiece is invalid!");
