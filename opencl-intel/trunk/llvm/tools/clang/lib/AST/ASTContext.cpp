...
ASTContext::getCanonicalTemplateTemplateParmDecl(
  ...
  LLVM_ASSERT(Canonical == 0 && "Shouldn't be in the map!");
  (void)Canonical;
...
void ASTContext::InitBuiltinTypes() {
  LLVM_ASSERT(VoidTy.isNull() && "Context reinitialized?");
...
ASTContext::getInstantiatedFromStaticDataMember(const VarDecl *Var) {
  LLVM_ASSERT(Var->isStaticDataMember() && "Not a static data member");
...
ASTContext::setInstantiatedFromStaticDataMember(VarDecl *Inst, VarDecl *Tmpl,
  ...
  LLVM_ASSERT(Inst->isStaticDataMember() && "Not a static data member");
  LLVM_ASSERT(Tmpl->isStaticDataMember() && "Not a static data member");
  LLVM_ASSERT(!InstantiatedFromStaticDataMember[Inst] &&
...
ASTContext::setInstantiatedFromUsingDecl(UsingDecl *Inst, NamedDecl *Pattern) {
  LLVM_ASSERT((isa<UsingDecl>(Pattern) ||
          isa<UnresolvedUsingValueDecl>(Pattern) ||
          isa<UnresolvedUsingTypenameDecl>(Pattern)) && 
         "pattern decl is not a using decl");
  LLVM_ASSERT(!InstantiatedFromUsingDecl[Inst] && "pattern already exists");
...
ASTContext::setInstantiatedFromUsingShadowDecl(UsingShadowDecl *Inst,
  ...
  LLVM_ASSERT(!InstantiatedFromUsingShadowDecl[Inst] && "pattern already exists");
...
void ASTContext::setInstantiatedFromUnnamedFieldDecl(FieldDecl *Inst,
                                                     FieldDecl *Tmpl) {
  LLVM_ASSERT(!Inst->getDeclName() && "Instantiated field decl is not unnamed");
  LLVM_ASSERT(!Tmpl->getDeclName() && "Template field decl is not unnamed");
  LLVM_ASSERT(!InstantiatedFromUnnamedFieldDecl[Inst] &&
...
const llvm::fltSemantics &ASTContext::getFloatTypeSemantics(QualType T) const {
  ...
  LLVM_ASSERT(BT && "Not a floating point type!");
  switch (BT->getKind()) {
  default: LLVM_ASSERT(0 && "Not a floating point type!");
...
#include "clang/AST/TypeNodes.def"
    LLVM_ASSERT(false && "Should not see dependent types");
    break;
  ...
  case Type::Builtin:
    ...
    default: LLVM_ASSERT(0 && "Unknown builtin type!");
  ...
  case Type::TemplateSpecialization:
    LLVM_ASSERT(getCanonicalType(T) != T &&
  ...
  LLVM_ASSERT(Align && (Align & (Align-1)) == 0 && "Alignment must be power of 2");
...
void ASTContext::setObjCImplementation(ObjCInterfaceDecl *IFaceD,
  ...
  LLVM_ASSERT(IFaceD && ImplD && "Passed null params");
  ObjCImpls[IFaceD] = ImplD;
}
/// \brief Set the implementation of ObjCCategoryDecl.
void ASTContext::setObjCImplementation(ObjCCategoryDecl *CatD,
                           ObjCCategoryImplDecl *ImplD) {
  LLVM_ASSERT(CatD && ImplD && "Passed null params");
...
TypeSourceInfo *ASTContext::CreateTypeSourceInfo(QualType T,
  ...
  if (!DataSize)
    ...
    LLVM_ASSERT(DataSize == TypeLoc::getFullDataSizeForType(T) &&
...
QualType ASTContext::getExtQualType(const Type *TypeNode, Qualifiers Quals) {
  ...
  if (ExtQuals *EQ = ExtQualNodes.FindNodeOrInsertPos(ID, InsertPos)) {
    LLVM_ASSERT(EQ->getQualifiers() == Quals);
...
QualType ASTContext::getAddrSpaceQualType(QualType T, unsigned AddressSpace) {
  ...
  // another one.
  LLVM_ASSERT(!Quals.hasAddressSpace() &&
...
QualType ASTContext::getObjCGCQualType(QualType T,
  ...
  // another one.
  LLVM_ASSERT(!Quals.hasObjCGCAttr() &&
...
QualType ASTContext::getComplexType(QualType T) {
  ...
  if (!T.isCanonical()) {
    ...
    LLVM_ASSERT(NewIP == 0 && "Shouldn't be in the map!"); NewIP = NewIP;
  }
...
QualType ASTContext::getBlockPointerType(QualType T) {
  LLVM_ASSERT(T->isFunctionType() && "block of function types only");
...
QualType ASTContext::getConstantArrayType(QualType EltTy,
  ...
  LLVM_ASSERT((EltTy->isDependentType() ||
...
QualType ASTContext::getDependentSizedArrayType(QualType EltTy,
                                                ...
                                                unsigned EltTypeQuals,
                                                SourceRange Brackets) {
  LLVM_ASSERT((!NumElts || NumElts->isTypeDependent() || 
  ...
  if (Canon) {
    ...
    if (CanonEltTy == EltTy) {
      ...
      if (NumElts) {
        ...
        LLVM_ASSERT(!CanonCheck && "Dependent-sized canonical array type broken");
        (void)CanonCheck;
...
QualType ASTContext::getVectorType(QualType vecType, unsigned NumElts,
  ...
  LLVM_ASSERT(baseType != 0 && "getVectorType(): Expecting a built-in type");
...
QualType ASTContext::getExtVectorType(QualType vecType, unsigned NumElts) {
  ...
  baseType = dyn_cast<BuiltinType>(getCanonicalType(vecType).getTypePtr());
  LLVM_ASSERT(baseType != 0 && "getExtVectorType(): Expecting a built-in type");
  ...
  if (Canon) {
    ...
    if (CanonVecTy == vecType) {
      ...
      DependentSizedExtVectorType *CanonCheck
        = DependentSizedExtVectorTypes.FindNodeOrInsertPos(ID, InsertPos);
      LLVM_ASSERT(!CanonCheck && "Dependent-sized ext_vector canonical type broken");
      (void)CanonCheck;
...
QualType ASTContext::getInjectedClassNameType(CXXRecordDecl *Decl,
                                              QualType TST) {
#ifndef NDEBUG
  LLVM_ASSERT(NeedsInjectedClassNameType(Decl));
#endif
  if (Decl->TypeForDecl) {
    LLVM_ASSERT(isa<InjectedClassNameType>(Decl->TypeForDecl));
  } else if (CXXRecordDecl *PrevDecl = Decl->getPreviousDeclaration()) {
    LLVM_ASSERT(PrevDecl->TypeForDecl && "previous declaration has no type");
...
QualType ASTContext::getTypeDeclTypeSlow(const TypeDecl *Decl) {
  LLVM_ASSERT(Decl && "Passed null for Decl param");
  LLVM_ASSERT(!Decl->TypeForDecl && "TypeForDecl present in slow case");

  if (const TypedefDecl *Typedef = dyn_cast<TypedefDecl>(Decl))
    return getTypedefType(Typedef);

  LLVM_ASSERT(!isa<TemplateTypeParmDecl>(Decl) &&
         "Template type parameter types are always available.");

  if (const RecordDecl *Record = dyn_cast<RecordDecl>(Decl)) {
    LLVM_ASSERT(!Record->getPreviousDeclaration() &&
           "struct/union has previous declaration");
#ifndef NDEBUG
    LLVM_ASSERT(!NeedsInjectedClassNameType(Record));
#endif
    return getRecordType(Record);
  } else if (const EnumDecl *Enum = dyn_cast<EnumDecl>(Decl)) {
    LLVM_ASSERT(!Enum->getPreviousDeclaration() &&
...
ASTContext::getSubstTemplateTypeParmType(const TemplateTypeParmType *Parm,
  ...
  LLVM_ASSERT(Replacement.isCanonical()
...
QualType ASTContext::getTemplateTypeParmType(unsigned Depth, unsigned Index,
  ...
  if (Name) {
    ...
    TemplateTypeParmType *TypeCheck 
      = TemplateTypeParmTypes.FindNodeOrInsertPos(ID, InsertPos);
    LLVM_ASSERT(!TypeCheck && "Template type parameter canonical type broken");
    (void)TypeCheck;
  } else
    TypeParm = new (*this, TypeAlignment)
...
ASTContext::getCanonicalTemplateSpecializationType(TemplateName Template,
  ...
  LLVM_ASSERT(Spec->isDependentType() &&
...
ASTContext::getElaboratedType(ElaboratedTypeKeyword Keyword,
  ...
  if (!Canon.isCanonical()) {
    ...
    LLVM_ASSERT(!CheckT && "Elaborated canonical type broken");
    (void)CheckT;
  }
...
QualType ASTContext::getDependentNameType(ElaboratedTypeKeyword Keyword,
                                          ...
                                          QualType Canon) {
  LLVM_ASSERT(NNS->isDependent() && "nested-name-specifier must be dependent");
...
TemplateName ASTContext::getCanonicalTemplateName(TemplateName Name) {
  ...
  LLVM_ASSERT(!Name.getAsOverloadedTemplate());

  DependentTemplateName *DTN = Name.getAsDependentTemplateName();
  LLVM_ASSERT(DTN && "Non-dependent template names must refer to template decls.");
...
ASTContext::getCanonicalTemplateArgument(const TemplateArgument &Arg) {
  ...
  // Silence GCC warning
  LLVM_ASSERT(false && "Unhandled template argument kind");
...
QualType ASTContext::getArrayDecayedType(QualType Ty) {
  ...
  LLVM_ASSERT(PrettyArrayType && "Not an array type!");
...
static FloatingRank getFloatingRank(QualType T) {
  ...
  LLVM_ASSERT(T->getAs<BuiltinType>() && "getFloatingRank(): not a floating type");
  switch (T->getAs<BuiltinType>()->getKind()) {
  default: LLVM_ASSERT(0 && "getFloatingRank(): not a floating type");
...
QualType ASTContext::getFloatingTypeOfSizeWithinDomain(QualType Size,
  ...
  if (Domain->isComplexType()) {
    switch (EltRank) {
    default: LLVM_ASSERT(0 && "getFloatingRank(): illegal value for rank");
  ...
  LLVM_ASSERT(Domain->isRealFloatingType() && "Unknown domain!");
  switch (EltRank) {
  default: LLVM_ASSERT(0 && "getFloatingRank(): illegal value for rank");
...
unsigned ASTContext::getIntegerRank(Type *T) {
  LLVM_ASSERT(T->isCanonicalUnqualified() && "T should be canonicalized");
  ...
  switch (cast<BuiltinType>(T)->getKind()) {
  default: LLVM_ASSERT(0 && "getIntegerRank(): not a built-in integer");
  case BuiltinType::Bool:
...
QualType ASTContext::getPromotedIntegerType(QualType Promotable) {
  LLVM_ASSERT(!Promotable.isNull());
  LLVM_ASSERT(Promotable->isPromotableIntegerType());
  ...
  uint64_t IntSize = getTypeSize(IntTy);
  LLVM_ASSERT(Promotable->isUnsignedIntegerType() && PromotableSize <= IntSize);
...
void ASTContext::setCFConstantStringType(QualType T) {
  ...
  LLVM_ASSERT(Rec && "Invalid CFConstantStringType");
...
void ASTContext::setNSConstantStringType(QualType T) {
  const RecordType *Rec = T->getAs<RecordType>();
  LLVM_ASSERT(Rec && "Invalid NSConstantStringType");
...
void ASTContext::setBlockDescriptorType(QualType T) {
  const RecordType *Rec = T->getAs<RecordType>();
  LLVM_ASSERT(Rec && "Invalid BlockDescriptorType");
...
QualType ASTContext::BuildByRefType(llvm::StringRef DeclName, QualType Ty) {
  ...
  LLVM_ASSERT(getIntWidth(IntTy) == 32 && "non-32bit int not supported");
  QualType FieldTypes[] = {
    getPointerType(VoidPtrTy),
...
QualType ASTContext::getBlockParmType(
  ...
  for (unsigned i = 0; i < Layout.size(); ++i) {
    ...
    if (isa<CXXThisExpr>(E)) {
      ...
      // Padding.
      LLVM_ASSERT(isa<ConstantArrayType>(FieldType) &&
             isa<DeclRefExpr>(E) &&
...
void ASTContext::setObjCFastEnumerationStateType(QualType T) {
  const RecordType *Rec = T->getAs<RecordType>();
  LLVM_ASSERT(Rec && "Invalid ObjCFAstEnumerationStateType");
...
static char ObjCEncodingForPrimitiveKind(const ASTContext *C, QualType T) {
    switch (T->getAs<BuiltinType>()->getKind()) {
    default: LLVM_ASSERT(0 && "Unhandled builtin type kind");
...
static void EncodeBitField(const ASTContext *Context, std::string& S,
  ...
  LLVM_ASSERT(E && "bitfield width not there - getObjCEncodingForTypeImpl");
...
void ASTContext::getObjCEncodingForTypeImpl(QualType T, std::string& S,
  ...
  if (const ArrayType *AT =
    ...
    if (isa<IncompleteArrayType>(AT)) {
      ...
      if (const ConstantArrayType *CAT = dyn_cast<ConstantArrayType>(AT))
        ...
        LLVM_ASSERT(isa<VariableArrayType>(AT) && "Unknown array type!");
        S += '0';
      }
  ...
  LLVM_ASSERT(0 && "@encode for type not implemented!");
}
...
void ASTContext::setBuiltinVaListType(QualType T) {
  LLVM_ASSERT(BuiltinVaListType.isNull() && "__builtin_va_list type already set!");
...
void ASTContext::setObjCConstantStringInterface(ObjCInterfaceDecl *Decl) {
  LLVM_ASSERT(ObjCConstantStringType.isNull() &&
...
TemplateName ASTContext::getOverloadedTemplateName(UnresolvedSetIterator Begin,
  ...
  LLVM_ASSERT(size > 1 && "set is not overloaded!");
  ...
  for (UnresolvedSetIterator I = Begin; I != End; ++I) {
    NamedDecl *D = *I;
    LLVM_ASSERT(isa<FunctionTemplateDecl>(D) ||
...
TemplateName ASTContext::getDependentTemplateName(NestedNameSpecifier *NNS,
                                                  const IdentifierInfo *Name) {
  LLVM_ASSERT((!NNS || NNS->isDependent()) &&
  ...
  if (CanonNNS == NNS) {
    ...
    DependentTemplateName *CheckQTN =
      DependentTemplateNames.FindNodeOrInsertPos(ID, InsertPos);
    LLVM_ASSERT(!CheckQTN && "Dependent type name canonicalization broken");
    (void)CheckQTN;
  }
...
ASTContext::getDependentTemplateName(NestedNameSpecifier *NNS,
  ...
  if (CanonNNS == NNS) {
    ...
    DependentTemplateName *CheckQTN
      = DependentTemplateNames.FindNodeOrInsertPos(ID, InsertPos);
    LLVM_ASSERT(!CheckQTN && "Dependent template name canonicalization broken");
    (void)CheckQTN;
  }
...
CanQualType ASTContext::getFromTargetType(unsigned Type) const {
  ...
  LLVM_ASSERT(false && "Unhandled TargetInfo::IntType value");
  return CanQualType();
}
...
static bool areCompatVectorTypes(const VectorType *LHS,
  ...
  LLVM_ASSERT(LHS->isCanonicalUnqualified() && RHS->isCanonicalUnqualified());
...
bool ASTContext::areCompatibleVectorTypes(QualType FirstVec,
                                          QualType SecondVec) {
  LLVM_ASSERT(FirstVec->isVectorType() && "FirstVec should be a vector type");
  LLVM_ASSERT(SecondVec->isVectorType() && "SecondVec should be a vector type");
...
bool ASTContext::ObjCQualifiedIdTypesAreCompatible(QualType lhs, QualType rhs,
  ...
  LLVM_ASSERT(rhsQID && "One of the LHS/RHS should be id<x>");
...
void getIntersectionOfProtocols(ASTContext &Context,
  ...
  const ObjCObjectType* LHS = LHSOPT->getObjectType();
  const ObjCObjectType* RHS = RHSOPT->getObjectType();
  LLVM_ASSERT(LHS->getInterface() && "LHS must have an interface base");
  LLVM_ASSERT(RHS->getInterface() && "RHS must have an interface base");
...
bool ASTContext::canAssignObjCInterfaces(const ObjCObjectType *LHS,
  ...
  LLVM_ASSERT(LHS->getInterface() && "LHS is not an interface type");
  LLVM_ASSERT(RHS->getInterface() && "RHS is not an interface type");
...
QualType ASTContext::mergeFunctionTypes(QualType lhs, QualType rhs, 
  ...
  if (lproto && rproto) { // two C99 style function prototypes
    LLVM_ASSERT(!lproto->hasExceptionSpec() && !rproto->hasExceptionSpec() &&
           "C++ shouldn't be here");
  ...
  if (proto) {
    LLVM_ASSERT(!proto->hasExceptionSpec() && "C++ shouldn't be here");
...
QualType ASTContext::mergeTypes(QualType LHS, QualType RHS, 
  ...
  LLVM_ASSERT(!LHS->getAs<ReferenceType>() && "LHS is a reference type?");
  LLVM_ASSERT(!RHS->getAs<ReferenceType>() && "RHS is a reference type?");
  ...
  if (LQuals != RQuals) {
    ...
    LLVM_ASSERT((GC_L != GC_R) && "unequal qualifier sets had only equal elements");
...
#define DEPENDENT_TYPE(Class, Base) case Type::Class:
#include "clang/AST/TypeNodes.def"
    LLVM_ASSERT(false && "Non-canonical and dependent types shouldn't get here");
    return QualType();

  case Type::LValueReference:
  case Type::RValueReference:
  case Type::MemberPointer:
    LLVM_ASSERT(false && "C++ should never be in mergeTypes");
    return QualType();
  ...
  case Type::FunctionProto:
  case Type::ExtVector:
    LLVM_ASSERT(false && "Types are eliminated above");
    return QualType();
...
QualType ASTContext::getCorrespondingUnsignedType(QualType T) {
  LLVM_ASSERT(T->hasSignedIntegerRepresentation() && "Unexpected type");
  ...
  LLVM_ASSERT(BTy && "Unexpected signed integer type");
  switch (BTy->getKind()) {
  case BuiltinType::Char_S:
  case BuiltinType::SChar:
  ...
  default:
    LLVM_ASSERT(0 && "Unexpected signed integer type");
    return QualType();
  }
}
...
static QualType DecodeTypeFromStr(const char *&Str, ASTContext &Context,
  ...
  while (!Done) {
    ...
    case 'S':
      LLVM_ASSERT(!Unsigned && "Can't use both 'S' and 'U' modifiers!");
      LLVM_ASSERT(!Signed && "Can't use 'S' modifier multiple times!");
      Signed = true;
      break;
    case 'U':
      LLVM_ASSERT(!Signed && "Can't use both 'S' and 'U' modifiers!");
      LLVM_ASSERT(!Unsigned && "Can't use 'S' modifier multiple times!");
      Unsigned = true;
      break;
    case 'L':
      LLVM_ASSERT(HowLong <= 2 && "Can't have LLLL modifier");
      ++HowLong;
      break;
    }
  }
  ...
  // Read the base type.
  switch (*Str++) {
  default: LLVM_ASSERT(0 && "Unknown builtin type letter!");
  case 'v':
    LLVM_ASSERT(HowLong == 0 && !Signed && !Unsigned &&
  ...
  case 'f':
    ...
    break;
  case 'd':
    LLVM_ASSERT(HowLong < 2 && !Signed && !Unsigned &&
    ...
    break;
  case 's':
    LLVM_ASSERT(HowLong == 0 && "Bad modifiers used with 's'!");
    if (Unsigned)
  ...
  case 'i':
    ...
    break;
  case 'c':
    LLVM_ASSERT(HowLong == 0 && "Bad modifiers used with 'c'!");
    if (Signed)
    ...
    break;
  case 'b': // boolean
    LLVM_ASSERT(HowLong == 0 && !Signed && !Unsigned && "Bad modifiers for 'b'!");
    Type = Context.BoolTy;
    break;
  case 'z':  // size_t.
    LLVM_ASSERT(HowLong == 0 && !Signed && !Unsigned && "Bad modifiers for 'z'!");
  ...
  case 'a':
    ...
    LLVM_ASSERT(!Type.isNull() && "builtin va list type not initialized!");
    break;
  case 'A':
  ...
  case 'V': {
    ...
    LLVM_ASSERT(End != Str && "Missing vector size");
...
QualType ASTContext::GetBuiltinType(unsigned id,
  ...
  LLVM_ASSERT((TypeStr[0] != '.' || TypeStr[1] == 0) &&
...
ASTContext::UsualArithmeticConversionsType(QualType lhs, QualType rhs) {
  ...
  if (lhs->isRealFloatingType() || rhs->isRealFloatingType()) {
    ...
    if (result > 0) // convert the rhs
      return lhs;
    LLVM_ASSERT(result < 0 && "illegal float comparison");
...
bool ASTContext::DeclMustBeEmitted(const Decl *D) {
  ...
  LLVM_ASSERT(VD->isFileVarDecl() && "Expected file scoped var");
