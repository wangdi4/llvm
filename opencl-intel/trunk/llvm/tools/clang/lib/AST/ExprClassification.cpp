...
Cl Expr::ClassifyImpl(ASTContext &Ctx, SourceLocation *Loc) const {
  LLVM_ASSERT(!TR->isReferenceType() && "Expressions can't have reference type.");
...
static Cl::Kinds ClassifyBinaryOp(ASTContext &Ctx, const BinaryOperator *E) {
  LLVM_ASSERT(Ctx.getLangOptions().CPlusPlus &&
...
Expr::isModifiableLvalue(ASTContext &Ctx, SourceLocation *Loc) const {
  ...
  LLVM_ASSERT(VC.getKind() == Cl::CL_LValue && "Unhandled kind");
#if 0   // disable until BE fix their sources
  if (Ctx.getLangOptions().OpenCL && Ctx.getQualifiedType(getType(), getType()->getCanonicalTypeInternal().getQualifiers()).getAddressSpace() == 2)
