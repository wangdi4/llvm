...
CXXMethodDecl *CXXRecordDecl::getCopyAssignmentOperator(bool ArgIsConst) const {
  ...
  for (llvm::tie(Op, OpEnd) = this->lookup(Name); Op != OpEnd; ++Op) {
    ...
    LLVM_ASSERT(FnType && "Overloaded operator has no prototype.");
...
CXXRecordDecl::addedConstructor(ASTContext &Context,
  ...
  LLVM_ASSERT(!ConDecl->isImplicit() && "addedConstructor - not for implicit decl");
...
void CXXRecordDecl::addedAssignmentOperator(ASTContext &Context,
  ...
  LLVM_ASSERT(FnType && "Overloaded operator has no proto function type.");
  LLVM_ASSERT(FnType->getNumArgs() == 1 && !FnType->isVariadic());
...
void CXXRecordDecl::CheckConversionFunction(NamedDecl *ConvDecl) {
  LLVM_ASSERT(ConvDecl->getDeclContext() == this &&
  ...
  if (FunctionTemplateDecl *Temp = dyn_cast<FunctionTemplateDecl>(ConvDecl)) {
    LLVM_ASSERT(isa<CXXConversionDecl>(Temp->getTemplatedDecl()));
  } else {
    LLVM_ASSERT(isa<CXXConversionDecl>(ConvDecl));
  }
}
#endif
...
void CXXRecordDecl::removeConversion(const NamedDecl *ConvDecl) {
  ...
  for (unsigned I = 0, E = Convs.size(); I != E; ++I) {
    if (Convs[I].getDecl() == ConvDecl) {
      Convs.erase(I);
      LLVM_ASSERT(std::find(Convs.begin(), Convs.end(), ConvDecl) == Convs.end()
...
CXXRecordDecl::setInstantiationOfMemberClass(CXXRecordDecl *RD,
  ...
  LLVM_ASSERT(TemplateOrInstantiation.isNull() && 
         "Previous template or instantiation?");
  LLVM_ASSERT(!isa<ClassTemplateSpecializationDecl>(this));
  TemplateOrInstantiation 
...
CXXRecordDecl::setTemplateSpecializationKind(TemplateSpecializationKind TSK) {
  ...
  LLVM_ASSERT(false && "Not a class template or member class specialization");
}
...
CXXDestructorDecl *CXXRecordDecl::getDestructor() const {
  ...
  LLVM_ASSERT(++I == E && "Found more than one destructor!");
...
void CXXMethodDecl::addOverriddenMethod(const CXXMethodDecl *MD) {
  LLVM_ASSERT(MD->isCanonicalDecl() && "Method is not canonical!");
  LLVM_ASSERT(!MD->getParent()->isDependentContext() &&
...
QualType CXXMethodDecl::getThisType(ASTContext &C) const {
  ...
  LLVM_ASSERT(isInstance() && "No 'this' for static methods!");
...
CXXConstructorDecl::Create(ASTContext &C, CXXRecordDecl *RD,
  ...
  LLVM_ASSERT(NameInfo.getName().getNameKind()
...
static const char *getAccessName(AccessSpecifier AS) {
  switch (AS) {
    default:
    case AS_none:
      LLVM_ASSERT("Invalid access specifier!");
      return 0;
    case AS_public:
      return "public";
    case AS_private:
