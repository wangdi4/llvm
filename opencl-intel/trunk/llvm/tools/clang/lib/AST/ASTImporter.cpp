...
bool StructuralEquivalenceContext::Finish() {
  while (!DeclsToCheck.empty()) {
    ...
    LLVM_ASSERT(D2 && "Unrecorded tentative equivalence?");
...
Expr *ASTNodeImporter::VisitSizeOfAlignOfExpr(SizeOfAlignOfExpr *E) {
  ...
  if (E->isArgumentType()) {
    ...
    return new (Importer.getToContext()) SizeOfAlignOfExpr(E->getKind(),
                                                           TInfo, ResultType,
                                           Importer.Import(E->getOperatorLoc()),
...
FileID ASTImporter::Import(FileID FromID) {
  ...
  LLVM_ASSERT(FromSLoc.isFile() && "Cannot handle macro instantiations yet");
