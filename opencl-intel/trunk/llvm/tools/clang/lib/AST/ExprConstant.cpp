...
static bool EvaluatePointer(const Expr* E, LValue& Result, EvalInfo &Info) {
  LLVM_ASSERT(E->getType()->hasPointerRepresentation());
...
public:
  ...
  bool Success(const llvm::APSInt &SI, const Expr *E) {
    LLVM_ASSERT(E->getType()->isIntegralOrEnumerationType() && 
           "Invalid evaluation result.");
    LLVM_ASSERT(SI.isSigned() == E->getType()->isSignedIntegerType() &&
           "Invalid evaluation result.");
    LLVM_ASSERT(SI.getBitWidth() == Info.Ctx.getIntWidth(E->getType()) &&
           "Invalid evaluation result.");
    Result = APValue(SI);
    return true;
  }
  ...
  bool Success(const llvm::APInt &I, const Expr *E) {
    LLVM_ASSERT(E->getType()->isIntegralOrEnumerationType() && 
           "Invalid evaluation result.");
    LLVM_ASSERT(I.getBitWidth() == Info.Ctx.getIntWidth(E->getType()) &&
           "Invalid evaluation result.");
  ...
  bool VisitStmt(Stmt *) {
    LLVM_ASSERT(0 && "This should be called on integers, stmts are not integers");
    return false;
  }
...
static int EvaluateBuiltinClassifyType(const CallExpr *E) {
  ...
  if (ArgTy->isVoidType())
    ...
    LLVM_ASSERT(0 && "CallExpr::isBuiltinClassifyType(): unimplemented type");
  return -1;
}
...
bool IntExprEvaluator::VisitBinaryOperator(const BinaryOperator *E) {
  ...
  if (LHSTy->isAnyComplexType()) {
    LLVM_ASSERT(RHSTy->isAnyComplexType() && "Invalid comparison");
  ...
  if (LHSTy->isRealFloatingType() &&
    ...
    switch (E->getOpcode()) {
    default:
      LLVM_ASSERT(0 && "Invalid binary operator!");
    case BO_LT:
...
bool IntExprEvaluator::VisitSizeOfAlignOfExpr(const SizeOfAlignOfExpr *E) {
  ...
  if (E->getKind() == SizeOfAlignOfExpr::AlignOf) {
    if (E->isArgumentType())
  ...
  if (E->getKind() == SizeOfAlignOfExpr::VecStep) {
    clang::QualType Ty;
    if (E->isArgumentType())
...
bool IntExprEvaluator::VisitOffsetOfExpr(const OffsetOfExpr *E) {
  ...
  for (unsigned i = 0; i != n; ++i) {
    ...
    case OffsetOfExpr::OffsetOfNode::Field: {
      ...
      LLVM_ASSERT(i < RL.getFieldCount() && "offsetof field in wrong type");
...
static bool EvaluateFloat(const Expr* E, APFloat& Result, EvalInfo &Info) {
  LLVM_ASSERT(E->getType()->isRealFloatingType());
...
static bool EvaluateComplex(const Expr *E, ComplexValue &Result,
                            EvalInfo &Info) {
  LLVM_ASSERT(E->getType()->isAnyComplexType());
...
bool ComplexExprEvaluator::VisitImaginaryLiteral(ImaginaryLiteral *E) {
  ...
  if (SubExpr->getType()->isRealFloatingType()) {
    ...
    LLVM_ASSERT(SubExpr->getType()->isIntegerType() &&
...
bool ComplexExprEvaluator::VisitCastExpr(CastExpr *E) {
  ...
  if (SubType->isRealFloatingType()) {
    ...
    if (Result.isComplexFloat()) {
      ...
      LLVM_ASSERT(Result.isComplexInt() && "Invalid evaluate result.");
      if (EltType->isRealFloatingType()) {
        Result.makeComplexFloat();
...
bool ComplexExprEvaluator::VisitBinaryOperator(const BinaryOperator *E) {
  ...
  LLVM_ASSERT(Result.isComplexFloat() == RHS.isComplexFloat() &&
...
APSInt Expr::EvaluateAsInt(ASTContext &Ctx) const {
  ...
  Result = Result;
  LLVM_ASSERT(Result && "Could not evaluate expression");
  LLVM_ASSERT(EvalResult.Val.isInt() && "Expression did not evaluate to integer");

  return EvalResult.Val.getInt();
}

 bool Expr::EvalResult::isGlobalLValue() const {
   LLVM_ASSERT(Val.isLValue());
...
static ICEDiag CheckICE(const Expr* E, ASTContext &Ctx) {
  LLVM_ASSERT(!E->isValueDependent() && "Should not see value dependent exprs!");
  if (!E->getType()->isIntegralOrEnumerationType()) {
    return ICEDiag(2, E->getLocStart());
  }
...
#include "clang/AST/StmtNodes.inc"
  ...
  case Expr::SizeOfAlignOfExprClass: {
    ...
    if ((Exp->getKind() ==  SizeOfAlignOfExpr::SizeOf ) && Exp->getTypeOfArgument()->isVariableArrayType())
      return ICEDiag(2, E->getLocStart());
    return NoDiag();
  }
...
bool Expr::isIntegerConstantExpr(llvm::APSInt &Result, ASTContext &Ctx,
  ...
  LLVM_ASSERT(!EvalResult.HasSideEffects && "ICE with side effects!");
  LLVM_ASSERT(EvalResult.Val.isInt() && "ICE that isn't integer!");
