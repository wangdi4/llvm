...
void CXXNewExpr::AllocateArgsArray(ASTContext &C, bool isArray,
  ...
  LLVM_ASSERT(SubExprs == 0 && "SubExprs already allocated");
  Array = isArray;
...
void OverloadExpr::initializeResults(ASTContext &C,
                                     UnresolvedSetIterator Begin,
                                     UnresolvedSetIterator End) {
  LLVM_ASSERT(Results == 0 && "Results already initialized!");
...
bool UnaryTypeTraitExpr::EvaluateTrait(ASTContext& C) const {
  switch(UTT) {
  default: LLVM_ASSERT(false && "Unknown type trait or not implemented");
    ...
    if (const RecordType *RT = QueriedType->getAs<RecordType>()) {
      ...
      for (llvm::tie(Op, OpEnd) = RD->lookup(Name);
        ...
        if (Operator->isCopyAssignmentOperator()) {
          ...
          if (CPT && !CPT->hasEmptyExceptionSpec()) {
            AllNoThrow = false;
            break;
          }
        }
  ...
  case UTT_HasNothrowConstructor:
    ...
    if (const RecordType *RT =
      ...
      if (CXXConstructorDecl *Constructor = RD->getDefaultConstructor()) {
        ...
        if (CPT && CPT->hasEmptyExceptionSpec() && CPT->getNumArgs() == 0)
          return true;
      }
    }
    return false;
...
CXXBindTemporaryExpr *CXXBindTemporaryExpr::Create(ASTContext &C,
                                                   CXXTemporary *Temp,
                                                   Expr* SubExpr) {
  LLVM_ASSERT(SubExpr->getType()->isRecordType() &&
  ...
  Args(0), NumArgs(numargs) 
{
  if (NumArgs) {
    ...
    for (unsigned i = 0; i != NumArgs; ++i) {
      LLVM_ASSERT(args[i] && "NULL argument in CXXConstructExpr");
      Args[i] = args[i];
    }
  }
}
...
void CXXExprWithTemporaries::setNumTemporaries(ASTContext &C, unsigned N) {
  LLVM_ASSERT(Temps == 0 && "Cannot resize with this");
  NumTemps = N;
...
CXXRecordDecl *UnresolvedMemberExpr::getNamingClass() const {
  ...
  if (getQualifier()) {
    ...
    LLVM_ASSERT(T && "qualifier in member expression does not name type");
    Record = T->getAsCXXRecordDecl();
    LLVM_ASSERT(Record && "qualifier in member expression does not name record");
  }
  // Otherwise the naming class must have been the base class.
    ...
    if (isArrow()) {
      ...
      LLVM_ASSERT(PT && "base of arrow member access is not pointer");
      BaseType = PT->getPointeeType();
    }
    
    Record = BaseType->getAsCXXRecordDecl();
    LLVM_ASSERT(Record && "base of member expression does not name record");
  }
