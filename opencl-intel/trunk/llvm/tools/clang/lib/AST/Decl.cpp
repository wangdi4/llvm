...
static Linkage getLinkageForNamespaceScopeDecl(const NamedDecl *D) {
  LLVM_ASSERT(D->getDeclContext()->getRedeclContext()->isFileContext() &&
...
bool NamedDecl::declarationReplaces(NamedDecl *OldD) const {
  LLVM_ASSERT(getDeclName() == OldD->getDeclName() && "Declaration name mismatch");
...
void DeclaratorDecl::setQualifierInfo(NestedNameSpecifier *Qualifier,
  ...
  if (Qualifier) {
    ...
    LLVM_ASSERT(QualifierRange.isInvalid());
    if (hasExtInfo()) {
...
QualifierInfo::setTemplateParameterListsInfo(ASTContext &Context,
  ...
  LLVM_ASSERT((NumTPLists == 0 || TPLists != 0) &&
         "Empty array of template parameters with positive size!");
  LLVM_ASSERT((NumTPLists == 0 || NNS) &&
...
const char *VarDecl::getStorageClassSpecifierString(StorageClass SC) {
  ...
  LLVM_ASSERT(0 && "Invalid storage class");
  return 0;
}
...
void VarDecl::setTemplateSpecializationKind(TemplateSpecializationKind TSK,
  ...
  LLVM_ASSERT(MSI && "Not an instantiated static data member?");
...
Expr *ParmVarDecl::getDefaultArg() {
  LLVM_ASSERT(!hasUnparsedDefaultArg() && "Default argument is not yet parsed!");
  LLVM_ASSERT(!hasUninstantiatedDefaultArg() &&
...
CXXTemporary *ParmVarDecl::getDefaultArgTemporary(unsigned i) {
  LLVM_ASSERT(getNumDefaultArgTemporaries() && 
...
FunctionDecl::setPreviousDeclaration(FunctionDecl *PrevDecl) {
  ...
  if (FunctionTemplateDecl *FunTmpl = getDescribedFunctionTemplate()) {
    ...
    LLVM_ASSERT((!PrevDecl || PrevFunTmpl) && "Function/function template mismatch");
...
void FunctionDecl::setParams(ParmVarDecl **NewParamInfo, unsigned NumParams) {
  LLVM_ASSERT(ParamInfo == 0 && "Already has param info!");
  LLVM_ASSERT(NumParams == getNumParams() && "Parameter count mismatch!");
...
bool FunctionDecl::isInlineDefinitionExternallyVisible() const {
  LLVM_ASSERT(isThisDeclarationADefinition() && "Must have the function definition");
  LLVM_ASSERT(isInlined() && "Function must be inline");
  ASTContext &Context = getASTContext();
...
FunctionDecl::TemplatedKind FunctionDecl::getTemplatedKind() const {
  ...
  LLVM_ASSERT(false && "Did we miss a TemplateOrSpecialization type?");
  return TK_NonTemplate;
}
...
FunctionDecl::setInstantiationOfMemberFunction(FunctionDecl *FD,
  ...
  LLVM_ASSERT(TemplateOrSpecialization.isNull() && 
...
FunctionDecl::setFunctionTemplateSpecialization(FunctionTemplateDecl *Template,
                        ...
                        const TemplateArgumentListInfo *TemplateArgsAsWritten,
                                          SourceLocation PointOfInstantiation) {
  LLVM_ASSERT(TSK != TSK_Undeclared && 
  ...
  if (InsertPos)
    ...
    (void)Existing;
    LLVM_ASSERT((!Existing || Existing->Function->isCanonicalDecl()) &&
...
FunctionDecl::setTemplateSpecializationKind(TemplateSpecializationKind TSK,
  ...
  if (FunctionTemplateSpecializationInfo *FTSInfo
    ...
    LLVM_ASSERT(false && "Function cannot have a template specialization kind");
}
...
void TagDecl::completeDefinition() {
  LLVM_ASSERT((!isa<CXXRecordDecl>(this) ||
...
void EnumDecl::completeDefinition(QualType NewType,
  ...
  LLVM_ASSERT(!isDefinition() && "Cannot redefine enums!");
  IntegerType = NewType;
...
RecordDecl::RecordDecl(Kind DK, TagKind TK, DeclContext *DC, SourceLocation L,
  ...
  LLVM_ASSERT(classof(static_cast<Decl*>(this)) && "Invalid Kind!");
}
...
void RecordDecl::completeDefinition() {
  LLVM_ASSERT(!isDefinition() && "Cannot redefine record!");
  TagDecl::completeDefinition();
}
...
ValueDecl *RecordDecl::getAnonymousStructOrUnionObject() {
  ...
  LLVM_ASSERT(isAnonymousStructOrUnion());
  ValueDecl *D = cast<ValueDecl>(getNextDeclInContext());
  LLVM_ASSERT(D->getType()->isRecordType());
  LLVM_ASSERT(D->getType()->getAs<RecordType>()->getDecl() == this);
  return D;
}
