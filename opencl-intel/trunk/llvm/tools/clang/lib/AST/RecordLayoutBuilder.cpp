...
void EmptySubobjectMap::AddSubobjectAtOffset(const CXXRecordDecl *RD, 
  ...
  LLVM_ASSERT(std::find(Classes.begin(), Classes.end(), RD) == Classes.end() &&
...
void RecordLayoutBuilder::IdentifyPrimaryBases(const CXXRecordDecl *RD) {
  ...
  for (CXXRecordDecl::base_class_const_iterator i = RD->bases_begin(),
         e = RD->bases_end(); i != e; ++i) {
    LLVM_ASSERT(!i->getType()->isDependentType() &&
...
void RecordLayoutBuilder::DeterminePrimaryBase(const CXXRecordDecl *RD) {
  ...
  LLVM_ASSERT(!PrimaryBase && "Should not get here with a primary base!");

  // Allocate the virtual table pointer at offset zero.
  LLVM_ASSERT(DataSize == 0 && "Vtable pointer must be at offset zero!");
...
RecordLayoutBuilder::ComputeBaseSubobjectInfo(const CXXRecordDecl *RD, 
  ...
  if (IsVirtual) {
    ...
    if (InfoSlot) {
      LLVM_ASSERT(InfoSlot->Class == RD && "Wrong class for virtual base info!");
      return InfoSlot;
    }
  ...
  if (RD->getNumVBases()) {
    ...
    if (Layout.getPrimaryBaseWasVirtual()) {
      ...
      LLVM_ASSERT(PrimaryVirtualBase && "Didn't have a primary virtual base!");
  ...
  if (PrimaryVirtualBase && !PrimaryVirtualBaseInfo) {
    ...
    // virtual base.
    PrimaryVirtualBaseInfo = VirtualBaseInfo.lookup(PrimaryVirtualBase);
    LLVM_ASSERT(PrimaryVirtualBaseInfo &&
...
void RecordLayoutBuilder::ComputeBaseSubobjectInfo(const CXXRecordDecl *RD) {
  for (CXXRecordDecl::base_class_const_iterator I = RD->bases_begin(),
    ...
    if (IsVirtual) {
      ...
      LLVM_ASSERT(VirtualBaseInfo.count(BaseDecl) &&
             "Did not add virtual base!");
    } else {
      // Add the base info to the map of non-virtual bases.
      LLVM_ASSERT(!NonVirtualBaseInfo.count(BaseDecl) &&
...
RecordLayoutBuilder::LayoutNonVirtualBases(const CXXRecordDecl *RD) {
  ...
  if (PrimaryBase) {
    if (PrimaryBaseIsVirtual) {
      ...
      LLVM_ASSERT(!VisitedVirtualBases.count(PrimaryBase) &&
  ...
  for (CXXRecordDecl::base_class_const_iterator I = RD->bases_begin(),
    ...
    LLVM_ASSERT(BaseInfo && "Did not find base info for non-virtual base!");
...
void RecordLayoutBuilder::LayoutNonVirtualBase(const BaseSubobjectInfo *Base) {
  ...
  LLVM_ASSERT(!Bases.count(Base->Class) && "base offset already exists!");
...
RecordLayoutBuilder::AddPrimaryVirtualBaseOffsets(const BaseSubobjectInfo *Info, 
  ...
  if (Info->PrimaryVirtualBaseInfo) {
    LLVM_ASSERT(Info->PrimaryVirtualBaseInfo->IsVirtual && 
           "Primary virtual base is not virtual!");
    if (Info->PrimaryVirtualBaseInfo->Derived == Info) {
      // Add the offset.
      LLVM_ASSERT(!VBases.count(Info->PrimaryVirtualBaseInfo->Class) && 
    ...
    if (I->isVirtual()) {
      if (PrimaryBase != BaseDecl || !PrimaryBaseIsVirtual) {
        ...
        if (!IndirectPrimaryBase) {
          ...
          LLVM_ASSERT(BaseInfo && "Did not find virtual base info!");
...
void RecordLayoutBuilder::LayoutVirtualBase(const BaseSubobjectInfo *Base) {
  LLVM_ASSERT(!Base->Derived && "Trying to lay out a primary virtual base!");
  
  // Layout the base.
  uint64_t Offset = LayoutBase(Base);

  // Add its base class offset.
  LLVM_ASSERT(!VBases.count(Base->Class) && "vbase offset already exists!");
...
void RecordLayoutBuilder::Layout(const CXXRecordDecl *RD) {
  ...
  for (CXXRecordDecl::base_class_const_iterator I = RD->bases_begin(),
    ...
    LLVM_ASSERT(Bases.count(BaseDecl) && "Did not find base offset!");
  }
  ...
  for (CXXRecordDecl::base_class_const_iterator I = RD->vbases_begin(),
    ...
    LLVM_ASSERT(VBases.count(BaseDecl) && "Did not find base offset!");
  }
#endif
}
...
void RecordLayoutBuilder::LayoutWideBitField(uint64_t FieldSize,
                                                uint64_t TypeSize) {
  LLVM_ASSERT(Context.getLangOptions().CPlusPlus &&
  ...
  LLVM_ASSERT(!Type.isNull() && "Did not find a type!");
...
void RecordLayoutBuilder::UpdateAlignment(unsigned NewAlignment) {
  ...
  LLVM_ASSERT(llvm::isPowerOf2_32(NewAlignment && "Alignment not a power of 2"));
...
const ASTRecordLayout &ASTContext::getASTRecordLayout(const RecordDecl *D) {
  D = D->getDefinition();
  LLVM_ASSERT(D && "Cannot get layout of forward declarations!");
...
const CXXMethodDecl *ASTContext::getKeyFunction(const CXXRecordDecl *RD) {
  ...
  LLVM_ASSERT(RD && "Cannot get key function for forward declarations!");

  const CXXMethodDecl *&Entry = KeyFunctions[RD];
  if (!Entry)
    Entry = RecordLayoutBuilder::ComputeKeyFunction(RD);
  else
    LLVM_ASSERT(Entry == RecordLayoutBuilder::ComputeKeyFunction(RD) &&
...
ASTContext::getObjCLayout(const ObjCInterfaceDecl *D,
  ...
  LLVM_ASSERT(!D->isForwardDecl() && "Invalid interface decl!");
...
static void DumpCXXRecordLayout(llvm::raw_ostream &OS,
  ...
  for (CXXRecordDecl::base_class_const_iterator I = RD->vbases_begin(),
         E = RD->vbases_end(); I != E; ++I) {
    LLVM_ASSERT(I->isVirtual() && "Found non-virtual class!");
