...
void RedeclarableTemplateDecl::setPreviousDeclarationImpl(
  ...
  if (Prev) {
    ...
    LLVM_ASSERT(CommonOrPrev.is<CommonBase*>() && "Cannot reset TemplateDecl Prev");
  }
}
...
void ClassTemplateDecl::getPartialSpecializations(
  ...
  for (llvm::FoldingSet<ClassTemplatePartialSpecializationDecl>::iterator
       ...
       P != PEnd; ++P) {
    LLVM_ASSERT(!PS[P->getSequenceNumber()]);
...
void TemplateArgumentListBuilder::Append(const TemplateArgument &Arg) {
  LLVM_ASSERT((Arg.getKind() != TemplateArgument::Type ||
          Arg.getAsType().isCanonical()) && "Type must be canonical!");
  LLVM_ASSERT(FlatArgs.size() < MaxFlatArgs && "Argument list builder is full!");
  LLVM_ASSERT(!StructuredArgs &&
...
void TemplateArgumentListBuilder::BeginPack() {
  LLVM_ASSERT(!AddingToPack && "Already adding to pack!");
  LLVM_ASSERT(!StructuredArgs && "Argument list already contains a pack!");
...
void TemplateArgumentListBuilder::EndPack() {
  LLVM_ASSERT(AddingToPack && "Not adding to pack!");
  LLVM_ASSERT(!StructuredArgs && "Argument list already contains a pack!");
...
void TemplateArgumentList::init(ASTContext &Context,
                           const TemplateArgument *Args,
                           unsigned NumArgs) {
LLVM_ASSERT(NumFlatArguments == 0 && NumStructuredArguments == 0 &&
       "Already initialized!");
...
initTemplateArgsAsWritten(const TemplateArgumentListInfo &ArgInfos) {
  LLVM_ASSERT(ArgsAsWritten == 0 && "ArgsAsWritten already set");
  unsigned N = ArgInfos.size();
