...
void StmtPrinter::VisitSwitchCase(SwitchCase*) {
  LLVM_ASSERT(0 && "SwitchCase is an abstract class");
}
...
void StmtPrinter::VisitIntegerLiteral(IntegerLiteral *Node) {
  ...
  if (!Node->getType()->getAs<BuiltinType>())
    return;
  
  switch (Node->getType()->getAs<BuiltinType>()->getKind()) {
  default: LLVM_ASSERT(0 && "Unexpected type for integer literal!");
...
#include "clang/Basic/OperatorKinds.def"
  ...
  if (Kind == OO_PlusPlus || Kind == OO_MinusMinus) {
    ...
    LLVM_ASSERT(false && "unknown overloaded operator");
  }
}
...
static const char *getTypeTraitName(UnaryTypeTrait UTT) {
  switch (UTT) {
  default: LLVM_ASSERT(false && "Unknown type trait");
