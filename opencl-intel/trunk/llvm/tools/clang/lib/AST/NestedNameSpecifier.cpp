...
NestedNameSpecifier::Create(ASTContext &Context, NestedNameSpecifier *Prefix,
                            IdentifierInfo *II) {
  LLVM_ASSERT(II && "Identifier cannot be NULL");
  LLVM_ASSERT((!Prefix || Prefix->isDependent()) && "Prefix must be dependent");
...
NestedNameSpecifier::Create(ASTContext &Context, NestedNameSpecifier *Prefix,
                            NamespaceDecl *NS) {
  LLVM_ASSERT(NS && "Namespace cannot be NULL");
  LLVM_ASSERT((!Prefix ||
...
NestedNameSpecifier::Create(ASTContext &Context, NestedNameSpecifier *Prefix,
  ...
  LLVM_ASSERT(T && "Type cannot be NULL");
  NestedNameSpecifier Mockup;
  Mockup.Prefix.setPointer(Prefix);
...
NestedNameSpecifier::print(llvm::raw_ostream &OS,
  ...
  case TypeSpec: {
    ...
    LLVM_ASSERT(!isa<ElaboratedType>(T) &&
