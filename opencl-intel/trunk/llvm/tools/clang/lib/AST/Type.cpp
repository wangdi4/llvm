...
ObjCObjectType::ObjCObjectType(QualType Canonical, QualType Base,
  ...
  : Type(ObjCObject, Canonical, false),
    ...
    BaseType(Base) {
  LLVM_ASSERT(this->NumProtocols == NumProtocols &&
...
bool Type::isSignedIntOrIntVecType() const {
  if (const BuiltinType *BT = dyn_cast<BuiltinType>(CanonicalType)) {
    return BT->getKind() >= BuiltinType::Char_S &&
           BT->getKind() <= BuiltinType::Int128;
  }
...
bool Type::isUnsignedIntOrIntVecType() const {
  if (const BuiltinType *BT = dyn_cast<BuiltinType>(CanonicalType)) {
    return BT->getKind() >= BuiltinType::Bool &&
           BT->getKind() <= BuiltinType::UInt128;
  }
...
bool Type::isConstantSizeType() const {
  LLVM_ASSERT(!isIncompleteType() && "This doesn't make sense for incomplete types");
  LLVM_ASSERT(!isDependentType() && "This doesn't make sense for dependent types");
...
bool Type::isOpenCLPromotableIntegerType() const
{
	if (const BuiltinType* BT = getAs<BuiltinType>())
		if (BT->getKind() == BuiltinType::Int || BT->getKind() == BuiltinType::UInt)
			return true;
	return isPromotableIntegerType();
}
...
const char *Type::getTypeClassName() const {
  switch (TC) {
  default: LLVM_ASSERT(0 && "Type class not in TypeNodes.def!");
...
llvm::StringRef FunctionType::getNameForCallConv(CallingConv CC) {
  ...
  case CC_X86SvmlCall: return "svmlcall";
...
static bool isDependent(const TemplateArgument &Arg) {
  ...
  case TemplateArgument::Null:
    LLVM_ASSERT(false && "Should not have a NULL template argument");
    return false;
...
QualType QualifierCollector::apply(QualType QT) const {
  ...
  LLVM_ASSERT(Context && "extended qualifiers but no context!");
