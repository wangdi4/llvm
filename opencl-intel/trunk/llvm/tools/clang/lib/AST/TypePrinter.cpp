...
void TypePrinter::PrintFunctionProto(const FunctionProtoType *T, 
  ...
  case CC_X86StdCall:
    ...
    break;
  case CC_X86SvmlCall:
    S += " __attribute__((svmlcc))";
    break;
  case CC_X86Svml_P2_F8_F8_F8_Call:
    S += " __attribute__((svmlcc_p2_f8_f8_f8))";
    break;
  case CC_X86FastCall:
...
void TypePrinter::PrintTag(TagDecl *D, std::string &InnerString) {
  ...
  if (const IdentifierInfo *II = D->getIdentifier())
    ...
    LLVM_ASSERT(Typedef->getIdentifier() && "Typedef without identifier?");
...
static void PrintTemplateArgument(std::string &Buffer,
  ...
  switch (Arg.getKind()) {
    case TemplateArgument::Null:
      LLVM_ASSERT(false && "Null template argument");
      break;
