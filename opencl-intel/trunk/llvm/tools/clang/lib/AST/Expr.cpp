...
UnaryOperator::getOverloadedOpcode(OverloadedOperatorKind OO, bool Postfix) {
  switch (OO) {
  default: LLVM_ASSERT(false && "No unary operator for overloaded function");
...
IdentifierInfo *OffsetOfExpr::OffsetOfNode::getFieldName() const {
  LLVM_ASSERT(getKind() == Field || getKind() == Identifier);
  if (getKind() == Field)
...
const char *CastExpr::getCastKindName() const {
  ...
  LLVM_ASSERT(0 && "Unhandled cast kind!");
  return 0;
}
...
void CastExpr::setCastPath(const CXXCastPath &Path) {
  LLVM_ASSERT(Path.size() == path_size());
...
BinaryOperator::getOverloadedOpcode(OverloadedOperatorKind OO) {
  switch (OO) {
  default: LLVM_ASSERT(false && "Not an overloadable binary operator");
...
bool Expr::isNullPointerConstant(ASTContext &Ctx,
  ...
  if (isValueDependent()) {
    ...
    case NPC_NeverValueDependent:
      LLVM_ASSERT(false && "Unexpected value dependent expression!");
...
IdentifierInfo *DesignatedInitExpr::Designator::getFieldName() {
  LLVM_ASSERT(Kind == FieldDesignator && "Only valid on a field designator");
...
DesignatedInitExpr::DesignatedInitExpr(ASTContext &C, QualType Ty, 
  ...
  *Child++ = Init;
  ...
  LLVM_ASSERT(IndexIdx == NumIndexExprs && "Wrong number of index expressions");
}
...
Expr *DesignatedInitExpr::getArrayIndex(const Designator& D) {
  LLVM_ASSERT(D.Kind == Designator::ArrayDesignator && "Requires array designator");
...
Expr *DesignatedInitExpr::getArrayRangeStart(const Designator& D) {
  LLVM_ASSERT(D.Kind == Designator::ArrayRangeDesignator &&
