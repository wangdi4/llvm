...
const char *Decl::getDeclKindName() const {
  switch (DeclKind) {
  default: LLVM_ASSERT(0 && "Declaration not in DeclNodes.inc!");
...
const char *DeclContext::getDeclKindName() const {
  switch (DeclKind) {
  default: LLVM_ASSERT(0 && "Declaration context not in DeclNodes.inc!");
...
TranslationUnitDecl *Decl::getTranslationUnitDecl() {
  ...
  LLVM_ASSERT(DC && "This decl is not contained in a translation unit!");
...
void Decl::setAttrs(const AttrVec &attrs) {
  LLVM_ASSERT(!HasAttrs && "Decl already contains attrs.");

  AttrVec &AttrBlank = getASTContext().getDeclAttrs(this);
  LLVM_ASSERT(AttrBlank.empty() && "HasAttrs was wrong?");
...
const AttrVec &Decl::getAttrs() const {
  LLVM_ASSERT(HasAttrs && "No attrs to get!");
...
#define DECL_CONTEXT_BASE(NAME)                  \
      if (DK >= first##NAME && DK <= last##NAME) \
        return static_cast<NAME##Decl*>(const_cast<DeclContext*>(D));
#include "clang/AST/DeclNodes.inc"
      LLVM_ASSERT(false && "a decl that inherits DeclContext isn't handled");
      return 0;
  }
}
...
DeclContext *DeclContext::getPrimaryContext() {
  ...
  default:
    if (DeclKind >= Decl::firstTag && DeclKind <= Decl::lastTag) {
      ...
      LLVM_ASSERT(isa<TagType>(Tag->TypeForDecl) ||
    ...
    LLVM_ASSERT(DeclKind >= Decl::firstFunction && DeclKind <= Decl::lastFunction &&
...
DeclContext::LoadLexicalDeclsFromExternalStorage() const {
  ...
  LLVM_ASSERT(hasExternalLexicalStorage() && Source && "No external storage?");
...
void DeclContext::removeDecl(Decl *D) {
  LLVM_ASSERT(D->getLexicalDeclContext() == this &&
         "decl being removed from non-lexical context");
  LLVM_ASSERT((D->NextDeclInContext || D == LastDecl) &&
  ...
  if (D == FirstDecl) {
    ...
    for (Decl *I = FirstDecl; true; I = I->NextDeclInContext) {
      LLVM_ASSERT(I && "decl not found in linked list");
  ...
  if (isa<NamedDecl>(D)) {
    ...
    LLVM_ASSERT(Pos != Map->end() && "no lookup entry for decl");
...
void DeclContext::addHiddenDecl(Decl *D) {
  ...
  LLVM_ASSERT(!D->getNextDeclInContext() && D != LastDecl &&
...
void DeclContext::MaterializeVisibleDeclsFromExternalStorage() {
  ExternalASTSource *Source = getParentASTContext().getExternalSource();
  LLVM_ASSERT(hasExternalVisibleStorage() && Source && "No external storage?");
...
StoredDeclsMap *DeclContext::CreateStoredDeclsMap(ASTContext &C) const {
  LLVM_ASSERT(!LookupPtr && "context already has a decls map");
  LLVM_ASSERT(getPrimaryContext() == this &&
...
DependentDiagnostic *DependentDiagnostic::Create(ASTContext &C,
  ...
  LLVM_ASSERT(Parent->isDependentContext()
