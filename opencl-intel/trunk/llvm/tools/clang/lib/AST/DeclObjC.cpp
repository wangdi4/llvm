...
ObjCInterfaceDecl *ObjCMethodDecl::getClassInterface() {
  ...
  LLVM_ASSERT(!isa<ObjCProtocolDecl>(getDeclContext()) && "It's a protocol method");
  LLVM_ASSERT(false && "unknown method context");
  return 0;
}
...
ObjCIvarDecl *ObjCIvarDecl::Create(ASTContext &C, ObjCContainerDecl *DC,
  ...
  if (DC) {
    ...
    // fixed.
    LLVM_ASSERT((isa<ObjCInterfaceDecl>(DC) || isa<ObjCImplementationDecl>(DC) ||
            isa<ObjCCategoryDecl>(DC)) &&
...
const ObjCInterfaceDecl *ObjCIvarDecl::getContainingInterface() const {
  ...
  case ObjCProtocol:
    LLVM_ASSERT(0 && "invalid ivar container!");
    return 0;

    // Ivars can only appear in class extension categories.
  case ObjCCategory: {
    const ObjCCategoryDecl *CD = cast<ObjCCategoryDecl>(DC);
    LLVM_ASSERT(CD->IsClassExtension() && "invalid container for ivar!");
    return CD->getClassInterface();
  }
