...
DeclarationName::DeclarationName(Selector Sel) {
  ...
  default:
    ...
    LLVM_ASSERT((Ptr & PtrMask) == 0 && "Improperly aligned MultiKeywordSelector");
...
DeclarationName::NameKind DeclarationName::getNameKind() const {
  ...
  LLVM_ASSERT(0 && "This should be unreachable!");
  return Identifier;
}
...
#include "clang/Basic/OperatorKinds.def"
    ...
    LLVM_ASSERT(OpName && "not an overloaded operator");
  ...
  LLVM_ASSERT(false && "Unexpected declaration name kind");
}
...
void *DeclarationName::getFETokenInfoAsVoid() const {
  ...
  default:
    LLVM_ASSERT(false && "Declaration name has no FETokenInfo");
  }
  return 0;
}
...
DeclarationNameTable::getCXXSpecialName(DeclarationName::NameKind Kind,
                                        CanQualType Ty) {
  LLVM_ASSERT(Kind >= DeclarationName::CXXConstructorName &&
  ...
  case DeclarationName::CXXConstructorName:
    ...
    LLVM_ASSERT(!Ty.hasQualifiers() &&"Constructor type must be unqualified");
    break;
  case DeclarationName::CXXDestructorName:
    EKind = DeclarationNameExtra::CXXDestructor;
    LLVM_ASSERT(!Ty.hasQualifiers() && "Destructor type must be unqualified");
    break;
  case DeclarationName::CXXConversionFunctionName:
