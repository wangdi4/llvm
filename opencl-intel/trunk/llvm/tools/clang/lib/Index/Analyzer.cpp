...
public:
  ...
  virtual void Handle(TranslationUnit *TU) {
    LLVM_ASSERT(TU && "Passed null translation unit");

    Decl *D = Ent.getDecl(TU->getASTContext());
    LLVM_ASSERT(D && "Couldn't resolve Entity");
...
public:
  RefSelectorAnalyzer(ObjCMethodDecl *MD,
    ...
    LLVM_ASSERT(!isa<ObjCProtocolDecl>(MD->getDeclContext()) &&
...
public:
  MessageAnalyzer(ObjCMessageExpr *Msg,
    ...
    LLVM_ASSERT(CanBeInstanceMethod || CanBeClassMethod);

    Selector sel = Msg->getSelector();
    LLVM_ASSERT(!sel.isNull());
  ...
  bool ValidMethod(ObjCMethodDecl *D, ObjCInterfaceDecl *MsgIFace,
    ...
    if (ReceiverIFace) {
      ...
      LLVM_ASSERT(ReceiverIFaceEnt.isInvalid());
...
void Analyzer::FindDeclarations(Decl *D, TULocationHandler &Handler) {
  LLVM_ASSERT(D && "Passed null declaration");
