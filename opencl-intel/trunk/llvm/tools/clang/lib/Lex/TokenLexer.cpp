...
void TokenLexer::Init(Token &Tok, SourceLocation ILEnd, MacroArgs *Actuals) {
  ...
  if (Macro && Macro->isFunctionLike() && Macro->getNumArgs())
...
void TokenLexer::ExpandFunctionArguments() {
  ...
  for (unsigned i = 0, e = NumTokens; i != e; ++i) {
    ...
    if (CurTok.is(tok::hash) || CurTok.is(tok::hashat)) {
      ...
      LLVM_ASSERT(ArgNo != -1 && "Token following # is not an argument?");
    ...
    LLVM_ASSERT(PasteBefore && ResultToks.back().is(tok::hashhash));
  ...
  if (MadeChange) {
    LLVM_ASSERT(!OwnsTokens && "This would leak if we already own the token list");
...
bool TokenLexer::PasteTokens(Token &Tok) {
  ...
  do {
    ...
    ++CurToken;
    LLVM_ASSERT(!isAtEnd() && "No token on the RHS of a paste operator!");
    ...
    if (Tok.is(tok::identifier) && RHS.is(tok::identifier)) {
      ...
      LLVM_ASSERT(ResultTokLoc.isFileID() &&
...
void TokenLexer::HandleMicrosoftCommentPaste(Token &Tok) {
  ...
  // being expanded.
  LLVM_ASSERT(Macro && "Token streams can't paste comments");
  Macro->EnableMacro();
