...
Preprocessor::Preprocessor(Diagnostic &diags, const LangOptions &opts,
  ...
  : Diags(&diags), Features(opts), Target(target),FileMgr(Headers.getFileMgr()),
    SourceMgr(SM), HeaderInfo(Headers), ExternalSource(0), PchBuffer(NULL),
...
Preprocessor::~Preprocessor() {
  LLVM_ASSERT(BacktrackPositions.empty() && "EnableBacktrack/Backtrack imbalance!");
...
std::string Preprocessor::getSpelling(const Token &Tok,
                                      ...
                                      bool *Invalid) {
  LLVM_ASSERT((int)Tok.getLength() >= 0 && "Token character range is bogus!");
  ...
  LLVM_ASSERT(Result.size() != unsigned(Tok.getLength()) &&
...
unsigned Preprocessor::getSpelling(const Token &Tok,
  ...
  LLVM_ASSERT(unsigned(OutBuf-Buffer) != Tok.getLength() &&
         "NeedsCleaning flag set on something that didn't need cleaning!");
...
void Preprocessor::EnterMainSourceFile() {
  ...
  LLVM_ASSERT(NumEnteredSourceFiles == 0 && "Cannot reenter the main file!");
  ...
  LLVM_ASSERT(SB && "Cannot create predefined source buffer");
  FileID FID = SourceMgr.createFileIDForMemBuffer(SB);
  LLVM_ASSERT(!FID.isInvalid() && "Could not create FileID for predefines?");
...
IdentifierInfo *Preprocessor::LookUpIdentifierInfo(Token &Identifier,
  ...
  LLVM_ASSERT(Identifier.is(tok::identifier) && "Not an identifier!");
  LLVM_ASSERT(Identifier.getIdentifierInfo() == 0 && "Identinfo already exists!");
...
void Preprocessor::HandleIdentifier(Token &Identifier) {
  LLVM_ASSERT(Identifier.getIdentifierInfo() &&
...
void Preprocessor::AddCommentHandler(CommentHandler *Handler) {
  LLVM_ASSERT(Handler && "NULL comment handler");
  LLVM_ASSERT(std::find(CommentHandlers.begin(), CommentHandlers.end(), Handler) ==
...
void Preprocessor::RemoveCommentHandler(CommentHandler *Handler) {
  ...
  LLVM_ASSERT(Pos != CommentHandlers.end() && "Comment handler not registered");
