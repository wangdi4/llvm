...
void PragmaNamespace::AddPragma(PragmaHandler *Handler) {
  LLVM_ASSERT(!Handlers.lookup(Handler->getName()) &&
...
void Preprocessor::Handle_Pragma(Token &Tok) {
  ...
  LLVM_ASSERT(StrVal[0] == '"' && StrVal[StrVal.size()-1] == '"' &&
...
void Preprocessor::HandlePragmaMark() {
  LLVM_ASSERT(CurPPLexer && "No current lexer?");
  if (CurLexer)
...
void Preprocessor::AddPragmaHandler(llvm::StringRef Namespace,
  ...
  if (!Namespace.empty()) {
    ...
    if (PragmaHandler *Existing = PragmaHandlers->FindHandler(Namespace)) {
      ...
      LLVM_ASSERT(InsertNS != 0 && "Cannot have a pragma namespace and pragma"
  ...
  LLVM_ASSERT(!InsertNS->FindHandler(Handler->getName()) &&
...
void Preprocessor::RemovePragmaHandler(llvm::StringRef Namespace,
  ...
  if (!Namespace.empty()) {
    ...
    LLVM_ASSERT(Existing && "Namespace containing handler does not exist!");

    NS = Existing->getIfNamespace();
    LLVM_ASSERT(NS && "Invalid namespace, registered as a regular pragma handler!");
  }
...
struct PragmaDebugHandler : public PragmaHandler {
  ...
  virtual void HandlePragma(Preprocessor &PP, Token &DepToken) {
    ...
    if (II->isStr("assert")) {
      LLVM_ASSERT(0 && "This is an assertion!");
  ...
  void DebugOverflowStack(volatile int *dummy = NULL) {
    if(!dummy) DebugOverflowStack(dummy);
  }
};
