...
const Token *MacroArgs::getUnexpArgument(unsigned Arg) const {
  ...
  // Scan to find Arg.
  for (; Arg; ++Result) {
    LLVM_ASSERT(Result < Start+NumUnexpArgTokens && "Invalid arg #");
...
MacroArgs::getPreExpArgument(unsigned Arg, const MacroInfo *MI, 
                             Preprocessor &PP) {
  LLVM_ASSERT(Arg < MI->getNumArgs() && "Invalid argument number!");
...
const Token &MacroArgs::getStringifiedArgument(unsigned ArgNo,
                                               Preprocessor &PP) {
  LLVM_ASSERT(ArgNo < NumUnexpArgTokens && "Invalid argument number!");
