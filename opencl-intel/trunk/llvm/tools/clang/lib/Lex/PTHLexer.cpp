...
void PTHLexer::getEOF(Token& Tok) {
  LLVM_ASSERT(EofToken.is(tok::eof));
  Tok = EofToken;
}

void PTHLexer::DiscardToEndOfLine() {
  LLVM_ASSERT(ParsingPreprocessorDirective && ParsingFilename == false &&
...
bool PTHLexer::SkipBlock() {
  LLVM_ASSERT(CurPPCondPtr && "No cached PP conditional information.");
  LLVM_ASSERT(LastHashTokPtr && "No known '#' token.");
  ...
  do {
    ...
    if (HashEntryI < LastHashTokPtr && TableIdx) {
      ...
      LLVM_ASSERT(NextPPCondPtr >= CurPPCondPtr);
  ...
  LLVM_ASSERT(HashEntryI == LastHashTokPtr && "No PP-cond entry found for '#'");
  LLVM_ASSERT(TableIdx && "No jumping from #endifs.");
  ...
  if (CurPtr > HashEntryI) {
    LLVM_ASSERT(CurPtr == HashEntryI + DISK_TOKEN_SIZE);
  ...
  // Skip the '#' token.
  LLVM_ASSERT(((tok::TokenKind)*CurPtr) == tok::hash);
...
public:
  ...
  static PTHFileData ReadData(const internal_key_type& k,
    ...
    LLVM_ASSERT(k.first == 0x1 && "Only file lookups can match!");
...
IdentifierInfo* PTHManager::LazilyCreateIdentifierInfo(unsigned PersistentID) {
  ...
  LLVM_ASSERT(IDData < (const unsigned char*)Buf->getBufferEnd());
  ...
  Mem->second = IDData;
  LLVM_ASSERT(IDData[0] != '\0');
  IdentifierInfo *II = new ((void*) Mem) IdentifierInfo();

  // Store the new IdentifierInfo in the cache.
  PerIDCache[PersistentID] = II;
  LLVM_ASSERT(II->getNameStart() && II->getNameStart()[0] != '\0');
  return II;
}
...
IdentifierInfo* PTHManager::get(llvm::StringRef Name) {
  ...
  LLVM_ASSERT(Name.empty() || Name.data()[Name.size()-1] != '\0');
...
PTHLexer *PTHManager::CreateLexer(FileID FID) {
  ...
  LLVM_ASSERT(PP && "No preprocessor set yet!");
