...
MacroArgs *Preprocessor::ReadFunctionLikeMacroArgs(Token &MacroName,
  ...
  LLVM_ASSERT(Tok.is(tok::l_paren) && "Error computing l-paren-ness?");
  ...
  while (Tok.isNot(tok::r_paren)) {
    LLVM_ASSERT((Tok.is(tok::l_paren) || Tok.is(tok::comma)) &&
    ...
    ++NumActuals;
    LLVM_ASSERT(NumFixedArgsLeft != 0 && "Too many arguments parsed");
    --NumFixedArgsLeft;
  }
...
void Preprocessor::ExpandBuiltinMacro(Token &Tok) {
  ...
  LLVM_ASSERT(II && "Can't be a macro without id info!");
  ...
  if (II == Ident__LINE__) {
	...
	if (const char* fName = PLoc.getFilename())
	  FN += fName;
    ...
    if (!IsValid)
      ...
      LLVM_ASSERT(II == Ident__has_feature && "Must be feature check");
    ...
    LLVM_ASSERT(0 && "Unknown identifier!");
  }
