...
NumericLiteralParser(const char *begin, const char *end,
  ...
  LLVM_ASSERT(!isalnum(*end) && *end != '.' && *end != '_' &&
...
void NumericLiteralParser::ParseNumberStartingWithZero(SourceLocation TokLoc) {
  LLVM_ASSERT(s[0] == '0' && "Invalid method call");
  s++;
...
bool NumericLiteralParser::GetIntegerValue(llvm::APInt &Val) {
  ...
  while (s < SuffixBegin) {
    ...
    LLVM_ASSERT(C < radix && "NumericLiteralParser ctor should have rejected this");
...
CharLiteralParser::CharLiteralParser(const char *begin, const char *end,
  ...
  LLVM_ASSERT(begin[0] == '\'' && "Invalid token lexed");
  ++begin;

  // FIXME: The "Value" is an uint64_t so we can handle char literals of
  // upto 64-bits.
  // FIXME: This extensively assumes that 'char' is 8-bits.
  LLVM_ASSERT(PP.getTargetInfo().getCharWidth() == 8 &&
         "Assumes char is 8 bits");
  LLVM_ASSERT(PP.getTargetInfo().getIntWidth() <= 64 &&
         (PP.getTargetInfo().getIntWidth() & 7) == 0 &&
         "Assumes sizeof(int) on target is <= 64 and a multiple of char");
  LLVM_ASSERT(PP.getTargetInfo().getWCharWidth() <= 64 &&
...
StringLiteralParser(const Token *StringToks, unsigned NumStringToks,
  ...
  if (AnyWide) {
    ...
    LLVM_ASSERT((wchar_tByteWidth & 7) == 0 && "Assumes wchar_t is byte multiple!");
    wchar_tByteWidth /= 8;
  }
  ...
  for (unsigned i = 0, e = NumStringToks; i != e; ++i) {
    ...
    LLVM_ASSERT(ThisTokBuf[0] == '"' && "Expected quote, lexer broken?");
    ++ThisTokBuf;
...
unsigned StringLiteralParser::getOffsetOfStringByte(const Token &Tok,
  ...
  LLVM_ASSERT(SpellingPtr[0] != 'L' && "Doesn't handle wide strings yet");
  ...
  LLVM_ASSERT(SpellingPtr[0] == '"' && "Should be a string literal!");
  ++SpellingPtr;

  // Skip over bytes until we find the offset we're looking for.
  while (ByteNo) {
    LLVM_ASSERT(SpellingPtr < SpellingEnd && "Didn't find byte offset!");
    ...
    LLVM_ASSERT(!HadError && "This method isn't valid on erroneous strings");
    --ByteNo;
  }
