...
static bool EvaluateValue(PPValue &Result, Token &PeekTok, DefinedTracker &DT,
  ...
  case tok::numeric_constant: {
    ...
    LLVM_ASSERT(Literal.isIntegerLiteral() && "Unknown ppnumber");
  ...
  case tok::char_constant: {   // 'x'
    ...
    if (Result.Val.getBitWidth() > Val.getBitWidth()) {
      ...
      LLVM_ASSERT(Result.Val.getBitWidth() == Val.getBitWidth() &&
...
static bool EvaluateDirectiveSubExpr(PPValue &LHS, unsigned MinPrec,
  ...
  while (1) {
    ...
    LLVM_ASSERT(PeekPrec <= ThisPrec && "Recursion didn't work!");
    ...
    bool Overflow = false;
    switch (Operator) {
    default: LLVM_ASSERT(0 && "Unknown operator token!");
    case tok::percent:
      if (RHS.Val != 0)
        Res = LHS.Val % RHS.Val;
      else if (ValueLive) {
