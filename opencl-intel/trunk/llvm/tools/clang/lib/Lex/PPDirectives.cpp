...
void Preprocessor::SkipExcludedConditionalBlock(SourceLocation IfTokenLoc,
                                                ...
                                                bool FoundElse) {
  ++NumSkipped;
  LLVM_ASSERT(CurTokenLexer == 0 && CurPPLexer && "Lexing a macro, not a file?");
  ...
  while (1) {
    ...
    if (Directive.startswith("if")) {
      ...
      if (Sub == "ndif") {  // "endif"
        ...
        LLVM_ASSERT(!InCond && "Can't be skipping if not in a conditional!");
        ...
        if (CondInfo.WasSkipping || CondInfo.FoundNonSkip) {
          ...
          LLVM_ASSERT(CurPPLexer->LexingRawMode && "We have to be skipping here!");
...
void Preprocessor::PTHSkipExcludedConditionalBlock() {
  ...
  while (1) {
    LLVM_ASSERT(CurPTHLexer);
    LLVM_ASSERT(CurPTHLexer->LexingRawMode == false);
...
void Preprocessor::HandleLineDirective(Token &Tok) {
  ...
  if (StrTok.is(tok::eom))
    ...
    LLVM_ASSERT(!Literal.AnyWide && "Didn't allow wide strings in");
    if (Literal.hadError)
      return DiscardUntilEndOfDirective();
    if (Literal.Pascal) {
...
bool Preprocessor::GetIncludeFilenameSpelling(SourceLocation Loc,
  ...
  LLVM_ASSERT(!Buffer.empty() && "Can't have tokens with empty spellings!");
...
void Preprocessor::HandleIncludeMacrosDirective(Token &IncludeMacrosTok) {
  ...
  do {
    Lex(TmpTok);
    LLVM_ASSERT(TmpTok.isNot(tok::eof) && "Didn't find end of -imacros!");
...
bool Preprocessor::ReadMacroDefinitionArgList(MacroInfo *MI) {
  ...
  while (1) {
    ...
    case tok::ellipsis:  // #define X(... -> C99 varargs
      //variadic macros are not allowed in OpenCL
	  if(getLangOptions().OpenCL) {
		Diag(Tok, diag::err_no_vararg_macro_in_opencl);
		return true;
	  }
...
void Preprocessor::HandleDefineDirective(Token &DefineTok) {
  ...
  if (Tok.is(tok::eom)) {
    ...
    LLVM_ASSERT(Ident__VA_ARGS__->isPoisoned() && "__VA_ARGS__ should be poisoned!");
    if (MI->isC99Varargs())
    ...
    if (!getDiagnostics().getSuppressSystemWarnings() ||
      ...
      if(getLangOptions().OpenCL) {
        if(!MI->isIdenticalTo(*OtherMI, *this)) {
          Diag(MI->getDefinitionLoc(), diag::err_macro_redefinition_in_opencl)
...
void Preprocessor::HandleIfdefDirective(Token &Result, bool isIfndef,
  ...
  if (CurPPLexer->getConditionalStackDepth() == 0) {
    ...
    if (!ReadAnyTokensBeforeDirective && MI == 0) {
      LLVM_ASSERT(isIfndef && "#ifdef shouldn't reach here");
...
void Preprocessor::HandleEndifDirective(Token &EndifToken) {
  ...
  LLVM_ASSERT(!CondInfo.WasSkipping && CurPPLexer && !CurPPLexer->LexingRawMode &&
