...
Lexer *Lexer::Create_PragmaLexer(SourceLocation SpellingLoc,
  ...
  LLVM_ASSERT(L->BufferEnd[0] == 0 && "Buffer is not nul terminated!");
...
static void InitCharacterInfo() {
  ...
  LLVM_ASSERT(CHAR_HORZ_WS == CharInfo[(int)' ']);
  LLVM_ASSERT(CHAR_HORZ_WS == CharInfo[(int)'\t']);
  LLVM_ASSERT(CHAR_HORZ_WS == CharInfo[(int)'\f']);
  LLVM_ASSERT(CHAR_HORZ_WS == CharInfo[(int)'\v']);
  LLVM_ASSERT(CHAR_VERT_WS == CharInfo[(int)'\n']);
  LLVM_ASSERT(CHAR_VERT_WS == CharInfo[(int)'\r']);
  LLVM_ASSERT(CHAR_UNDER   == CharInfo[(int)'_']);
  LLVM_ASSERT(CHAR_PERIOD  == CharInfo[(int)'.']);
  for (unsigned i = 'a'; i <= 'z'; ++i) {
    LLVM_ASSERT(CHAR_LETTER == CharInfo[i]);
    LLVM_ASSERT(CHAR_LETTER == CharInfo[i+'A'-'a']);
  }
  for (unsigned i = '0'; i <= '9'; ++i)
    LLVM_ASSERT(CHAR_NUMBER == CharInfo[i]);
...
static SourceLocation GetMappedTokenLoc(Preprocessor &PP,
  ...
  LLVM_ASSERT(FileLoc.isMacroID() && "Must be an instantiation");
...
SourceLocation Lexer::getSourceLocation(const char *Loc,
  ...
  LLVM_ASSERT(Loc >= BufferStart && Loc <= BufferEnd &&
  ...
  LLVM_ASSERT(PP && "This doesn't work on raw lexers");
...
bool Lexer::SaveBCPLComment(Token &Result, const char *CurPtr) {
  ...
  LLVM_ASSERT(Spelling[0] == '/' && Spelling[1] == '/' && "Not bcpl comment?");
...
std::string Lexer::ReadToEndOfLine() {
  ...
  while (1) {
    ...
    case '\n':
      ...
      LLVM_ASSERT(CurPtr[-1] == Char && "Trigraphs for newline?");
      BufferPtr = CurPtr-1;
      ...
      LLVM_ASSERT(Tmp.is(tok::eom) && "Unexpected token!");
...
unsigned Lexer::isNextPPTokenLParen() {
  LLVM_ASSERT(!LexingRawMode && "How can we expand a macro from a skipping buffer?");
...
bool Lexer::IsStartOfConflictMarker(const char *CurPtr) {
  ...
  if (FindConflictEnd(CurPtr, BufferEnd)) {
    ...
    while (*CurPtr != '\r' && *CurPtr != '\n') {
      LLVM_ASSERT(CurPtr != BufferEnd && "Didn't find end of line");
      ++CurPtr;
    }
    BufferPtr = CurPtr;
    return true;
...
LexNextToken:
  ...
  case 0:  // Null.
    ...
    if (CurPtr-1 == BufferEnd) {
      ...
      LLVM_ASSERT(PPCache && "Raw buffer::LexEndOfFile should return a token");
