...
bool Preprocessor::isInPrimaryFile() const {
  ...
  LLVM_ASSERT(IsFileLexer(IncludeMacroStack[0]) &&
...
void Preprocessor::EnterSourceFile(FileID FID, const DirectoryLookup *CurDir,
                                   SourceLocation Loc) {
  LLVM_ASSERT(CurTokenLexer == 0 && "Cannot #include a file inside a macro!");
...
bool Preprocessor::HandleEndOfFile(Token &Result, bool isEndOfMacro) {
  ...
  if (CurLexer) {
    ...
    LLVM_ASSERT(CurPTHLexer && "Got EOF but no current lexer set!");
...
bool Preprocessor::HandleEndOfTokenLexer(Token &Result) {
  LLVM_ASSERT(CurTokenLexer && !CurPPLexer &&
...
void Preprocessor::RemoveTopOfLexerStack() {
  LLVM_ASSERT(!IncludeMacroStack.empty() && "Ran out of stack entries to load");
...
void Preprocessor::HandleMicrosoftCommentPaste(Token &Tok) {
  ...
  if (Tok.is(tok::eom)) {
    LLVM_ASSERT(FoundLexer && "Can't get end of line without an active lexer");
  ...
  LLVM_ASSERT(!FoundLexer && "Lexer should return EOM before EOF in PP mode");
}
