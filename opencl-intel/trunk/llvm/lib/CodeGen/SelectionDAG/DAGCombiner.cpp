...
void DAGCombiner::Run(CombineLevel AtLevel) {
  ...
  // Add almost all the dag nodes to the worklist.
...
SDValue DAGCombiner::visitTRUNCATE(SDNode *N) {
  ...
  // This is currently performed only on scalars because vector elements may 
  // have different number of active low bits.
  if (!VT.isVector()) {
    SDValue Shorter =
...
SDValue DAGCombiner::visitEXTRACT_VECTOR_ELT(SDNode *N) {
 ...
 if (InVec.getOpcode() == ISD::SCALAR_TO_VECTOR) {
  ...
  if (isa<ConstantSDNode>(EltNo)) {
    ...
    // LLVM Machine instructions have multiple values for users to use. Example, DIV produces two values.
    // The Load instruction also has two values (results). One Is the data which is fetched from the memory,
    // and the second value is the ‘chain’, which is LLVM’s mechanism for ordering load/store instructions. 
    // If there is a store after this load, then there is a chain, which means that there is another user.  
    // Thus, asking hasOneUser would fail. Instead we ask  hasNUsesOfValue on the 'data' value.
    if (!LN0 || !LN0->hasNUsesOfValue(1,0) || LN0->isVolatile()) return SDValue();
  ...
  if (cast<ShuffleVectorSDNode>(N)->isSplat()) {
    ...
    if (V->getOpcode() == ISD::BUILD_VECTOR) {
      ...
      if (NumElems > BaseIdx) {
        ...
        // This condition ensures there are no nodes such as BIT_CAST intervening
        if (N0->getOpcode() == ISD::BUILD_VECTOR) {
          SDValue Op = V->getOperand(BaseIdx);
          SmallVector<SDValue, 8> Ops(V->getNumOperands(), Op);
          return DAG.getNode(ISD::BUILD_VECTOR, V->getDebugLoc(), VT,
                       &Ops[0], Ops.size());
        }
...
SDValue DAGCombiner::visitEXTRACT_SUBVECTOR(SDNode* N) {
  ...
  if (V->getOpcode() == ISD::INSERT_SUBVECTOR) {
    EVT V0VT = V->getOperand(0).getValueType();
    EVT V1VT = V->getOperand(1).getValueType();
    // Handle only simple case where vector being inserted and vector
    // being extracted are of same type, and are half size of larger vectors.
    if (NVT != V1VT || NVT.getSizeInBits()*2 != V0VT.getSizeInBits()) {
      assert(false && "Target requires generalization of this optimization");
      return SDValue();
    }

    bool IdxEqual = false;
    ConstantSDNode *N_Idx = dyn_cast<ConstantSDNode>(N->getOperand(1));
    ConstantSDNode *V_Idx = dyn_cast<ConstantSDNode>(V->getOperand(2));
    // If indeces are both constants, do a more relaxed comparison by comparing
    // their values which might be of different types.
    if (N_Idx && V_Idx)
      IdxEqual = N_Idx->getAPIntValue().getZExtValue() ==
                 V_Idx->getAPIntValue().getZExtValue();
    else
      IdxEqual = N->getOperand(1) == V->getOperand(2);

    // EXTRACT_SV( INSERT_SV( V1, V2, I ), I), return V2
    if (IdxEqual)
      return V->getOperand(1);
    // EXTRACT_SV( INSERT_SV( V1, V2, I1 ), I2), return EXTRACT_SV( V1, I2 )
