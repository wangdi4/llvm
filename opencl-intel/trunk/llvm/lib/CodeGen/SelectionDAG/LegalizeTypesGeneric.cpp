...
void DAGTypeLegalizer::SplitRes_SELECT(SDNode *N, SDValue &Lo,
                                       SDValue &Hi) {
  SDValue CL = N->getOperand(0), CH = N->getOperand(0), LL, LH, RL, RH;
  DebugLoc dl = N->getDebugLoc();
  const EVT& condType = N->getOperand(0).getValueType();
  if (condType.isVector())
  {
	  if (SplitVectors[N->getOperand(0)].first.getNode())
		{
		  GetSplitOp(N->getOperand(0), CL, CH);
		}
	  else {
		  const EVT& i1SubVecType = EVT::getVectorVT(*DAG.getContext(), condType.getVectorElementType(), condType.getVectorNumElements() / 2);
		  CL = DAG.getNode(ISD::EXTRACT_SUBVECTOR, dl, i1SubVecType, N->getOperand(0), DAG.getIntPtrConstant(0));
		  CH = DAG.getNode(ISD::EXTRACT_SUBVECTOR, dl, i1SubVecType, N->getOperand(0), DAG.getIntPtrConstant(i1SubVecType.getVectorNumElements()));
		  SetSplitVector(N->getOperand(0), CL, CH);
	  }
  }
  GetSplitOp(N->getOperand(1), LL, LH);
  GetSplitOp(N->getOperand(2), RL, RH);

  Lo = DAG.getNode(ISD::SELECT, dl, LL.getValueType(), CL, LL, RL);
  Hi = DAG.getNode(ISD::SELECT, dl, LH.getValueType(), CH, LH, RH);
}
...
void DAGTypeLegalizer::SplitRes_SELECT_CC(SDNode *N, SDValue &Lo,
  ...
  SDValue C0L, C0H;
  GetSplitOp(N->getOperand(0), C0L, C0H);
  SDValue C1L, C1H;
  GetSplitOp(N->getOperand(0), C1L, C1H);
  Lo = DAG.getNode(ISD::SELECT_CC, dl, LL.getValueType(), C0L,
                   C1L, LL, RL, N->getOperand(4));
  Hi = DAG.getNode(ISD::SELECT_CC, dl, LH.getValueType(), C0H,
                   C1H, LH, RH, N->getOperand(4));
}
