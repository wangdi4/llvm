...
SDValue DAGTypeLegalizer::ScalarizeVecRes_SELECT(SDNode *N) {
	// the condition may still be scalar
	SDValue cond = N->getOperand(0).getValueType().isVector() ? GetScalarizedVector(N->getOperand(0)) : N->getOperand(0);
	SDValue LHS = GetScalarizedVector(N->getOperand(1));
	SDValue RHS = GetScalarizedVector(N->getOperand(2));
	return DAG.getNode(ISD::SELECT, N->getDebugLoc(), LHS.getValueType(), cond, LHS, RHS);
}

SDValue DAGTypeLegalizer::ScalarizeVecRes_SELECT_CC(SDNode *N) {
  SDValue LHS = GetScalarizedVector(N->getOperand(2));
  return DAG.getNode(ISD::SELECT_CC, N->getDebugLoc(), LHS.getValueType(),
                     GetScalarizedVector(N->getOperand(0)),
...
SDValue DAGTypeLegalizer::SplitVecOp_SELECT(SDNode* N)
{
	const EVT& condVecType = N->getOperand(0).getValueType(), &valVecType = N->getOperand(1).getValueType();
	assert(condVecType.getVectorNumElements() == valVecType.getVectorNumElements());
	const unsigned int numElems = valVecType.getVectorNumElements();
	SmallVector<SDValue, 16> condElems(numElems), val1Elems(numElems), val2Elems(numElems), selects(numElems);
	const DebugLoc& dl = N->getDebugLoc();

	for (unsigned int i = 0; i < numElems; i++) {
		condElems[i] = DAG.getNode(ISD::EXTRACT_VECTOR_ELT, dl, condVecType.getVectorElementType(), N->getOperand(0), DAG.getIntPtrConstant(i));
		val1Elems[i] = DAG.getNode(ISD::EXTRACT_VECTOR_ELT, dl, valVecType.getVectorElementType(), N->getOperand(1), DAG.getIntPtrConstant(i));
		val2Elems[i] = DAG.getNode(ISD::EXTRACT_VECTOR_ELT, dl, valVecType.getVectorElementType(), N->getOperand(2), DAG.getIntPtrConstant(i));
		selects[i] = DAG.getNode(ISD::SELECT, dl, valVecType.getVectorElementType(), condElems[i], val1Elems[i], val2Elems[i]);
	}
	return DAG.getNode(ISD::BUILD_VECTOR, dl, valVecType, selects.data(), numElems);
}
...
SDValue DAGTypeLegalizer::WidenVecRes_EXTRACT_SUBVECTOR(SDNode *N) {
  ...
  if (CIdx) {
    ...
    if (IdxVal == 0 && (InVT == WidenVT ||
		InOp.getNode()->getOpcode() == ISD::INTRINSIC_WO_CHAIN ||
		InOp.getNode()->getOpcode() == ISD::INTRINSIC_W_CHAIN))
      return InOp;
...
SDValue DAGTypeLegalizer::GenWidenVectorLoads(SmallVector<SDValue, 16>& LdChain,
  ...
  while (LdWidth > 0) {
    ...
    if (LdWidth < NewVTWidth) {
      ...
      L = DAG.getLoad(NewVT, dl, Chain, BasePtr, SV,
