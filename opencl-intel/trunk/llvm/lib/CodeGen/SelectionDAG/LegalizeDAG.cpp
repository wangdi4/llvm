...
SDValue SelectionDAGLegalize::LegalizeOp(SDValue Op) {
  ...
  ISD::NodeType nodeType = (ISD::NodeType)Node->getOpcode();
...
SDValue SelectionDAGLegalize::ExpandBUILD_VECTOR(SDNode *Node) {
  ...
  if (isOnlyLowElement) {
    // Do not generate a SCALAR_TO_VECTOR for 256-bit type - there is no way to lower it
    if (VT.getSizeInBits() == 256) {
      EVT HalfVT = EVT::getVectorVT(*DAG.getContext(),  VT.getVectorElementType(), 
        VT.getVectorNumElements() / 2);

      SDValue Vec128 = DAG.getNode(ISD::SCALAR_TO_VECTOR, dl, HalfVT, Node->getOperand(0));
      return DAG.getNode(ISD::INSERT_SUBVECTOR, dl, VT, DAG.getNode(ISD::UNDEF, dl, VT), Vec128,
