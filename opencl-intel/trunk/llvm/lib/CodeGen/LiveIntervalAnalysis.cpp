...
void LiveIntervals::handleRegisterDef(MachineBasicBlock *MBB,
  ...
  if (TargetRegisterInfo::isVirtualRegister(MO.getReg()))
    ...
    bool isAllocatableOrHasAllocatableAlias = allocatableRegs_[MO.getReg()];
    for (const unsigned* AS = tri_->getAliasSet(MO.getReg()); *AS && !isAllocatableOrHasAllocatableAlias; ++AS)
      isAllocatableOrHasAllocatableAlias = isAllocatableOrHasAllocatableAlias || allocatableRegs_[*AS];
    if (isAllocatableOrHasAllocatableAlias) {
      MachineInstr *CopyMI = NULL;
      if (MI->isCopyLike())
        CopyMI = MI;
