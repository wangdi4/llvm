...
static bool GetCpuIDAndInfo(unsigned valueEAX, unsigned valueECX, unsigned *rEAX,
...
void X86Subtarget::AutoDetectSubtargetFeatures() {
  ...
  if (GetCpuIDAndInfo(0, 0, &EAX, text.u+0, text.u+2, text.u+1))
    return;

  GetCpuIDAndInfo(0x1, 0, &EAX, &EBX, &ECX, &EDX);
  ...
  if (IsIntel || IsAMD) {
    ...
    GetCpuIDAndInfo(0x80000001, 0, &EAX, &EBX, &ECX, &EDX);
  ...
  if (HasAVX) {
    GetCpuIDAndInfo(0x7, 0, &EAX, &EBX, &ECX, &EDX);
...
X86Subtarget::X86Subtarget(const std::string &TT, const std::string &FS, 
  ...
  // stack alignment was set explicitly
  if (StackAlignment)
