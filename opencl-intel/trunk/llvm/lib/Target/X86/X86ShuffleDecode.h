...
/*
static void DecodeVSHUFPSMask(unsigned NElts, unsigned Imm,
                             SmallVectorImpl<unsigned> &ShuffleMask) {
  assert(NElts == 8);
  // Part that reads from src1[0-3].
  for (unsigned i = 0; i != NElts/4; ++i) {
    ShuffleMask.push_back(Imm % (NElts/2));
    Imm /= (NElts/2);
  }
  
  // Part that reads from src2[0-3].
  for (unsigned i = 0; i != NElts/4; ++i) {
    ShuffleMask.push_back(Imm % (NElts/2));
    Imm /= (NElts/2);
  }

  //Upper lane uses same mask as lower lane
  for (unsigned i=0; i<NElts/2; ++i)
      ShuffleMask.push_back(ShuffleMask[i]+NElts/2);
}*/
