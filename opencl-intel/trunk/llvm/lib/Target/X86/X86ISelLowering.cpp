...
/* utility, helps to debug */
/*
static void PrintSmallVecInt(const SmallVectorImpl<int> &a, char *str)
{
  printf("%s = { ", str);
...
static SDValue Concat128BitVectors(SDValue Lower, SDValue Upper, SelectionDAG &DAG);


/// Generate a DAG to grab 128-bits from a vector > 128 bits.  This
/// sets things up to match to an AVX VEXTRACTF128 instruction or a
/// simple sub-register reference.  Idx is an index in the 128 bits we
/// want.  It need not be aligned to a 128-bit boundary.  That makes
...
static SDValue Extract128BitVector(SDValue Vec,
  ...
  if (isa<ConstantSDNode>(Idx)) {
    ...
    assert((NormalizedIdxVal <= ElemsPerChunk) && "Illegal index value in extract128 operation");
...
static SDValue Insert128BitVector(SDValue Result,
  ...
  if (isa<ConstantSDNode>(Idx)) {
    ...
    assert(NormalizedIdxVal <= ElemsPerChunk && "Illegal index value in insert128 operation");
    SDValue VecIdx = DAG.getConstant(NormalizedIdxVal, MVT::i32);
...
/// Given two 128-bit vectors, concat them.
static SDValue Concat128BitVectors(SDValue Lower, SDValue Upper, SelectionDAG &DAG) {
  ...
  SDValue Vec = DAG.getNode(ISD::UNDEF, dl, VT);

  if (Upper.getOpcode() != ISD::UNDEF) {
    Vec = Insert128BitVector(DAG.getNode(ISD::UNDEF, dl, VT), Upper,
      DAG.getConstant(
...
X86TargetLowering::X86TargetLowering(X86TargetMachine &TM)
  ...
  if (!UseSoftFloat && Subtarget->hasAVX()) {
    ...
    // Promote v16i16, v32i8, v8i32 load, select, and, or, xor to v4i64.
    for (unsigned i = (unsigned)MVT::FIRST_VECTOR_VALUETYPE; i <= (unsigned)MVT::LAST_VECTOR_VALUETYPE; i++) {
      MVT::SimpleValueType SVT = (MVT::SimpleValueType)i;
      EVT VT = SVT;
    ...
    if (!UseSoftFloat && Subtarget->hasAVX2()) {

      setOperationAction(ISD::VECTOR_SHUFFLE,     MVT::v32i8,  Custom);
      setOperationAction(ISD::VECTOR_SHUFFLE,     MVT::v16i16, Custom);
      ...
      setOperationAction(ISD::ZERO_EXTEND,        MVT::v16i16, Legal);
      setOperationAction(ISD::ZERO_EXTEND,        MVT::v8i32,  Legal);
      setOperationAction(ISD::ZERO_EXTEND,        MVT::v4i64,  Legal);

      setOperationAction(ISD::SIGN_EXTEND,        MVT::v16i16, Legal);
      setOperationAction(ISD::SIGN_EXTEND,        MVT::v8i32,  Legal);
      setOperationAction(ISD::SIGN_EXTEND,        MVT::v4i64,  Legal);

      setOperationAction(ISD::AND,                MVT::v4i64,  Legal);
      setOperationAction(ISD::XOR,                MVT::v4i64,  Legal);
      setOperationAction(ISD::OR,                 MVT::v4i64,  Legal);
    } else {
      ...
      setOperationAction(ISD::VECTOR_SHUFFLE,     MVT::v8i32, Promote);
      AddPromotedToType (ISD::VECTOR_SHUFFLE,     MVT::v8i32, MVT::v8f32);
      setOperationAction(ISD::VECTOR_SHUFFLE,     MVT::v4i64, Promote);
      AddPromotedToType (ISD::VECTOR_SHUFFLE,     MVT::v4i64, MVT::v4f64);
    }
    ...
    // The operation SINT_TO_FP is registered by source operand
    // The operation FP_TO_SINT is registered by destination operand 
    //  -- see  VectorLegalizer::LegalizeOp() --
  ...
  if (Subtarget->isTargetWindows()) {
    if (!Subtarget->is64Bit()) {
      ...
      //MSVCRT has it own library functions
...
X86TargetLowering::getOptimalMemOpType(uint64_t Size,
  ...
  if (NonScalarIntSafe &&
    ...
    if (Size >= 16 &&
      ...
      if (Subtarget->hasAVX())
        return Size > 16 ? MVT::v8i32 : MVT::v4i32;
      if (Subtarget->hasSSE2())
        return MVT::v4i32;
      if (Subtarget->hasSSE1())
        return MVT::v4f32;
...
X86TargetLowering::findRepresentativeClass(EVT VT) const{
  ...
  case MVT::v4f32: case MVT::v2f64:
	...
	break;
  case MVT::v32i8: case MVT::v8i32: case MVT::v4i64: case MVT::v16i16:
  case MVT::v8f32: case MVT::v4f64:
    RRC = X86::VR256RegisterClass;
    break;
  }
...
X86TargetLowering::LowerCallResult(SDValue Chain, SDValue InFlag,
  ...
  for (unsigned i = 0; i != RVLocs.size(); ++i) {
    ...
    } else if (VA.getLocInfo() == CCValAssign::Expand) {
	    assert (VA.getLocVT().getSizeInBits() == 256);
        std::pair <unsigned, unsigned> LocRegs = VA.getLocRegPair();
        assert((LocRegs.first == X86::XMM0 && LocRegs.second == X86::XMM1) ||
               (LocRegs.first == X86::XMM2 && LocRegs.second == X86::XMM3));
        EVT RegVT = VA.getLocVT();

        EVT HalfRegVT = EVT::getVectorVT(*DAG.getContext(),
          RegVT.getVectorElementType(),
          RegVT.getVectorNumElements() / 2);

        SDValue CHLo = DAG.getCopyFromReg(Chain, dl, LocRegs.first, HalfRegVT).getValue(1);
        SDValue CHHi = DAG.getCopyFromReg(Chain, dl, LocRegs.second, HalfRegVT).getValue(1);

        SDValue ArgValueLo = CHLo.getValue(0);
        SDValue ArgValueHi = CHHi.getValue(0);
	      
	      
	      Val = DAG.getNode(ISD::CONCAT_VECTORS, dl, RegVT ,ArgValueLo, ArgValueHi);

        Chain = DAG.getNode(ISD::TokenFactor, dl,MVT::Other, CHLo, CHHi);
...
bool X86TargetLowering::IsCalleePop(bool IsVarArg, CallingConv::ID CallingConv) const {
  if (IsVarArg)
    return false;
...
CCAssignFn *X86TargetLowering::CCAssignFnForNode(CallingConv::ID CC) const {
  if (Subtarget->is64Bit()) {
    ...
    else if (CC == CallingConv::X86_Svml)
      return CC_X86_64_Svml;
    else if (CC == CallingConv::X86_Svml_P2_F8_F8_F8)
      return CC_X86_64_Svml_P2_F8_F8_F8;
    else if (CC == CallingConv::X86_SvmlThunk)
      return CC_X86_64_SvmlThunk;
    else if (CC == CallingConv::X86_OCL_Kernel)
      return CC_OCL_X86_64;
  ...
  if (CC == CallingConv::X86_FastCall)
    ...
    return CC_X86_32_Svml;
  else if (CC == CallingConv::X86_Svml_P2_F8_F8_F8)
    return CC_X86_32_Svml_P2_F8_F8_F8;
  else if (CC == CallingConv::X86_OCL_Kernel)
    return CC_OCL_X86_32;
...
X86TargetLowering::LowerFormalArguments(SDValue Chain,
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (RegVT == MVT::i32)
        ...
        RC = (VA.getLocInfo() == CCValAssign::Expand)? X86::VR128RegisterClass : X86::VR256RegisterClass;
      ...
      if ((VA.getLocInfo() == CCValAssign::Expand) && (RegVT.getSizeInBits() == 256)) {
        std::pair <unsigned, unsigned> LocRegs = VA.getLocRegPair();
        MF.addLiveIn(LocRegs.first, RC);
        MF.addLiveIn(LocRegs.second, RC);
        
        EVT HalfRegVT = EVT::getVectorVT(*DAG.getContext(),
          RegVT.getVectorElementType(),
          RegVT.getVectorNumElements() / 2);

        SDValue ArgValueLo = DAG.getCopyFromReg(Chain, dl, LocRegs.first, HalfRegVT);
        SDValue ArgValueHi = DAG.getCopyFromReg(Chain, dl, LocRegs.second, HalfRegVT);

	      SmallVector<SDValue, 2> Ops;
	      Ops.push_back(ArgValueLo);
	      Ops.push_back(ArgValueHi);

	      ArgValue =  DAG.getNode(ISD::CONCAT_VECTORS, dl, RegVT ,&Ops[0], Ops.size());
      }
      else {
  ...
  if (Subtarget->IsCalleePop(isVarArg, CallConv)) {
    ...
    if (!Is64Bit && !IsWindows && !IsTailCallConvention(CallConv) && ArgsAreStructReturn(Ins))
...
static void 
PassShadowReg(unsigned Reg, SDValue& Arg, SmallVector<std::pair<unsigned, SDValue>, 8>& RegsToPass) {
...
X86TargetLowering::LowerCall(SDValue Chain, SDValue Callee,
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    case CCValAssign::Expand: {
      assert(VA.isRegLoc());

      // Special case: passing AVX256 values in XMM registers.
      ...
      std::pair < unsigned, unsigned >RegLocPair = VA.getLocRegPair();

      OpLo = DAG.getNode(ISD::EXTRACT_SUBVECTOR, dl, newVT, Arg,
                          DAG.getIntPtrConstant(0));
      
      RegsToPass.push_back(std::make_pair(RegLocPair.first, OpLo));

      OpHi = DAG.getNode(ISD::EXTRACT_SUBVECTOR, dl, newVT, Arg,
                          DAG.getIntPtrConstant(RegVT.getVectorNumElements()/2));

      RegsToPass.push_back(std::make_pair(RegLocPair.second, OpHi));

      if (isVarArg && Subtarget->isTargetWin64()) {
        // Win64 ABI requires argument XMM reg to be copied to the corresponding
        // shadow reg if callee is a varargs function.
        PassShadowReg(RegLocPair.first, OpLo, RegsToPass);
        PassShadowReg(RegLocPair.second, OpHi, RegsToPass);
      }
      continue;
    }
    }
    if (VA.isRegLoc()) {
      ...
      if (isVarArg && Subtarget->isTargetWin64()) {
        // Win64 ABI requires argument XMM reg to be copied to the corresponding
        // shadow reg if callee is a varargs function.
        PassShadowReg(VA.getLocReg(), Arg, RegsToPass);
      }
  ...
  else if (!Is64Bit && !IsWindows && !IsTailCallConvention(CallConv) && IsStructRet)
...
/*
 * A shuffle node with 1 source operand
 */
...
/*
 * A shuffle node with 1 source operand and a shuffle mask
 */
static SDValue getTargetShuffleNode(unsigned Opc, DebugLoc dl, EVT VT,
...
/*
 * A shuffle node with 2 source operands and a shuffle mask
 */
static SDValue getTargetShuffleNode(unsigned Opc, DebugLoc dl, EVT VT,
  ...
  case X86ISD::PALIGN:
  case X86ISD::SHUFPD:
  case X86ISD::VSHUFPDY:
  case X86ISD::SHUFPS:
  case X86ISD::VSHUFPSY:
  case X86ISD::VPERM2F128:
  case X86ISD::VPERM2I128:
    ConstantInt *C = ConstantInt::get(*DAG.getContext(), APInt(8, TargetMask));
    return DAG.getNode(Opc, dl, VT, V1, V2, DAG.getConstant(*C, MVT::i8));
  }
  return SDValue();
}

/*
 * A shuffle node with 2 source operands 
 */
static SDValue getTargetShuffleNode(unsigned Opc, DebugLoc dl, EVT VT,
...
/* all mask els are 0 */
static bool isVBROADCASTMask(const SmallVectorImpl<int> &Mask, EVT VT) {
	return (VT == MVT::v4f32 || VT == MVT::v4i32 || VT == MVT::v8f32 || VT == MVT::v8i32) &&
           (Mask.end() == std::find_if(Mask.begin(), Mask.end(), std::bind2nd(std::not_equal_to<int>(), 0)));
}

bool X86::isVBROADCASTMask(ShuffleVectorSDNode *N) {
  SmallVector<int, 8> M;
  N->getMask(M);
  return ::isVBROADCASTMask(M, N->getValueType(0));
}
...
/*
 * isVSHUFPSMask()
 * return true if the shuffle may be implemented with 
 * vshufps ymm1, ymm2, imm8
 */
static bool isVSHUFPSMask(const SmallVectorImpl<int> &Mask) {
  // The legal mask:
  // {0-3} {0-3} {8-11} {8-11} {4-7} {4-7} {12-15} {12-15}
  // And the mask should be symetric, Mask[0]&3 == Mask[4]&3
  
  // Check ranges
  if (
    !isUndefOrInRange(Mask[0], 0, 4) ||
    !isUndefOrInRange(Mask[1], 0, 4) ||
    !isUndefOrInRange(Mask[2], 8, 12) ||
    !isUndefOrInRange(Mask[3], 8, 12) ||
    !isUndefOrInRange(Mask[4], 4, 8) ||
    !isUndefOrInRange(Mask[5], 4, 8) ||
    !isUndefOrInRange(Mask[6], 12, 16) ||
    !isUndefOrInRange(Mask[7], 12, 16)
    ) return false;
  
  // Check symetric: Mask[0]&3 == Mask[4]&3, Mask[1]&3 == Mask[5]&3...
  for (unsigned i=0; i<4; i++) {
    if ((Mask[i+4] >= 0) && (Mask[i] >= 0))
      if ((Mask[i]&3) != (Mask[i+4]&3))
        return false;
  }
  return true;
}
 ...
 * isVSHUFPDMask()
 * return true if the shuffle may be implemented with 
 * vshufpd ymm1, ymm2, imm8
 */
static bool isVSHUFPDMask(const SmallVectorImpl<int> &Mask) {
  // The legal mask:
  // {0-1} {4-5} {2-3} {6-7} 

  // Check ranges
  if (
    !isUndefOrInRange(Mask[0], 0, 2) ||
    !isUndefOrInRange(Mask[1], 4, 6) ||
    !isUndefOrInRange(Mask[2], 2, 4) ||
    !isUndefOrInRange(Mask[3], 6, 8) 
    ) return false;
...
/* 
 * isVSHUFPMask()
 * returns true if vshufpd or vshufps may be used. 
 * For 256-bit vectors only.
 */
bool X86::isVSHUFPMask(ShuffleVectorSDNode *N) {
  SmallVector<int, 8> M;
  N->getMask(M);
  EVT VT = N->getValueType(0);
  if (VT.getSizeInBits() != 256)
    return false;
  return (VT.getVectorNumElements() == 8)?isVSHUFPSMask(M) : isVSHUFPDMask(M);
}
...
static bool isPSHUFHWMask(const SmallVectorImpl<int> &Mask, EVT VT) {
  ...
  if (VT == MVT::v16i16) {
    // Expected mask
    // {0, 1, 2, 3, [4-7],[4-7],[4-7],[4-7],8,9,10,11,[12-15],[12-15],[12-15],[12-15]
    // + in-lane symetric
    for (unsigned i=0; i<4; i++) {
      if (!isUndefOrEqual(Mask[i],i) ||
        !isUndefOrEqual(Mask[i+8],i+8))
        return false;
    }
    for (unsigned i=4; i<8; i++) {
      if (!isUndefOrInRange(Mask[i], 4, 8) ||
        !isUndefOrInRange(Mask[i+8],12, 16))
        return false;
      //symetric

      if ((Mask[i] >=0) && (Mask[i+8] >=0) && (Mask[i+8]-Mask[i] !=8))
        return false;
    }
    return true;
...
static bool isPSHUFLWMask(const SmallVectorImpl<int> &Mask, EVT VT) {
  ...
  if (VT == MVT::v16i16) {
    // Expected mask
    // {[0-3],[0-3],[0-3],[0-3], 4,5,6,7, [8-11],[8-11],[8-11],[8-11],12,13,14,15
    // + in-lane symetric
    for (unsigned i=4; i<8; i++) {
      if (!isUndefOrEqual(Mask[i],i) ||
        !isUndefOrEqual(Mask[i+8],i+8))
    ...
    for (unsigned i=0; i<4; i++) {
      if (!isUndefOrInRange(Mask[i], 0, 4) ||
        !isUndefOrInRange(Mask[i+8], 8, 12))
        return false;
      //symetric
...
/// getPALIGNRMaskType - Return 0 if the specified VECTOR_SHUFFLE operand
/// specifies a shuffle of elements is not suitable for input to PALIGNR.
/// Return ~0U if the mask uses both source operands.
/// Return 1 if the mask selects only components from operand 1.
/// Return 2 if the mask selects only components from operand 2.
static unsigned getPALIGNRMaskType(const SmallVectorImpl<int> &Mask, EVT VT,
                          bool hasSSSE3) {
  ...
  if (!NeedsUnary) return ~0U;
  
  // Unary case - determine which of the operands to select.
  // If shift amount is larger then number of components of vector, take 
  // the second operand, otherwise take the first operand.  
  return s < e ? 1 : 2;
}

unsigned X86::getPALIGNRMaskType(ShuffleVectorSDNode *N) {
  SmallVector<int, 8> M;
  N->getMask(M);
  return ::getPALIGNRMaskType(M, N->getValueType(0), true);
}
...
/* 
 * isCommutedSHUFP()
 * returns true if swapping operands will allow to use 
 * the "shufp" instruction
 */
 ...
 * isCommutedVSHUFP()
 * returns true if swapping operands will allow to use 
 * the "vshufp" instruction
 */
static bool isCommutedVSHUFP(ShuffleVectorSDNode *N) {
  SmallVector<int, 8> M;
  N->getMask(M);
  EVT VT = N->getValueType(0);
  
  CommuteVectorShuffleMask(M, VT);
  return (VT.getVectorNumElements() == 8)? isVSHUFPSMask(M) : isVSHUFPDMask(M);
}
...
bool X86::isMOVHLPSMask(ShuffleVectorSDNode *N) {
  if ((N->getValueType(0).getVectorNumElements() != 4) ||
    !N->getValueType(0).is128BitVector())
    return false;
...
bool X86::isMOVHLPS_v_undef_Mask(ShuffleVectorSDNode *N) {
  ...
  if ((NumElems != 4) || !N->getValueType(0).is128BitVector())
    return false;
...
static bool isUNPCKLMask(const SmallVectorImpl<int> &Mask, EVT VT,
  ...
  if (NumElts != 2 && NumElts != 4 && NumElts != 8 && NumElts != 16 && NumElts != 32)
    return false;
...
/*
 * isVUNPCKLPDMask()
 * return true if the shuffle can be implemented with instruction
 * vunpcklpd ymm1, ymm2, ymm3/mem
 * The legal is mask { 0, 4, 2, 6 } or undef (-1) at any place
 */
static bool isVUNPCKLPDMask(const SmallVectorImpl<int> &Mask, EVT VT) {
  ...
  if (
    ...
    !isUndefOrEqual(Mask[1], 4) ||
    !isUndefOrEqual(Mask[2], 2) ||
    !isUndefOrEqual(Mask[3], 6))
    return false;
  return true;
}
 ...
 * isVUNPCKHPDMask()
 * return true if the shuffle can be implemented with instruction
 * vunpckhpd ymm1, ymm2, ymm3/mem
 * The legal is mask { 1, 5, 3, 7 } or undef (-1) at any place
 */
static bool isVUNPCKHPDMask(const SmallVectorImpl<int> &Mask, EVT VT) {
  ...
  if (
    !isUndefOrEqual(Mask[0], 1) ||
    !isUndefOrEqual(Mask[1], 5) ||
    !isUndefOrEqual(Mask[2], 3) ||
    !isUndefOrEqual(Mask[3], 7))
    return false;
  return true;
}
 ...
 * isVUNPCKLPSMask()
 * return true if the shuffle can be implemented with instruction
 * vunpcklps ymm1, ymm2, ymm3/mem
 * The legal is mask { 0, 8, 1, 9, 4, 12, 5, 13 } or undef (-1) at any place
 */
static bool isVUNPCKLPSMask(const SmallVectorImpl<int> &Mask, EVT VT) {

  int NumElts = VT.getVectorNumElements();
  if (NumElts != 8 || VT.getSizeInBits() != 256)
    return false;

  // The legal mask { 0, 8, 1, 9, 4, 12, 5, 13 }
  // or may be seen as { 0, 0+8, 1, 1+8, 4, 4+8, 5, 5+8 }

  if (
    !isUndefOrEqual(Mask[0], 0) ||
    !isUndefOrEqual(Mask[1], 8) ||
    !isUndefOrEqual(Mask[2], 1) ||
    !isUndefOrEqual(Mask[3], 9) ||
    !isUndefOrEqual(Mask[4], 4) ||
    !isUndefOrEqual(Mask[5], 12) ||
    !isUndefOrEqual(Mask[6], 5) ||
    !isUndefOrEqual(Mask[7], 13))
    return false;
  return true;
}
 ...
 * isVUNPCKHPSMask()
 * return true if the shuffle can be implemented with instruction
 * vunpckhps ymm1, ymm2, ymm3/mem
 * The legal is mask { 2, 10, 3, 11, 6, 14, 7, 15 } or undef (-1) at any place
 */
static bool isVUNPCKHPSMask(const SmallVectorImpl<int> &Mask, EVT VT) {

  int NumElts = VT.getVectorNumElements();
  if (NumElts != 8 || VT.getSizeInBits() != 256)
    return false;

  // The legal mask { 2, 10, 3, 11, 6, 14, 7, 15 }
  // or may be seen as { 2, 2+8, 3, 3+8, 6, 6+8, 7, 7+8 }

  if (
    !isUndefOrEqual(Mask[0], 2) ||
    !isUndefOrEqual(Mask[1], 10) ||
    !isUndefOrEqual(Mask[2], 3) ||
    !isUndefOrEqual(Mask[3], 11) ||
    !isUndefOrEqual(Mask[4], 6) ||
    !isUndefOrEqual(Mask[5], 14) ||
    !isUndefOrEqual(Mask[6], 7) ||
    !isUndefOrEqual(Mask[7], 15))
    return false;
  return true;
}
...
/*
 * isVUNPCKLMask()
 * return true if vunpcklps or vunpcklpd may be used
 */
bool X86::isVUNPCKLMask(ShuffleVectorSDNode *N) {
  SmallVector<int, 8> M;
  N->getMask(M);
  return ::isVUNPCKLPDMask(M, N->getValueType(0)) || ::isVUNPCKLPSMask(M, N->getValueType(0));
}

/*
 * isVUNPCKHMask()
 * return true if vunpckhps or vunpckhpd may be used
 */
bool X86::isVUNPCKHMask(ShuffleVectorSDNode *N) {
  SmallVector<int, 8> M;
  N->getMask(M);
  return ::isVUNPCKHPDMask(M, N->getValueType(0)) || ::isVUNPCKHPSMask(M, N->getValueType(0));
}
...
/*
 *  isMOVSHDUPMask() 
 *  return true if the shuffle can be implemented with
 *   movshdup xmm1, xmm2
 *   the legal mask { 1, 1, 3, 3 } or undef(-1) at any place
 *  Or the similar AVX-256 instruction
 *   vmovshdup ymm1, ymm2
 *   the legal mask { 1, 1, 3, 3, 5, 5, 7, 7 } or undef(-1) at any place
 */
bool X86::isMOVSHDUPMask(ShuffleVectorSDNode *N) {

  unsigned NumElems = N->getValueType(0).getVectorNumElements();
  EVT VT = N->getValueType(0);
  if (!(VT.getSizeInBits() == 256 &&  NumElems == 8) &&
    !(VT.getSizeInBits() == 128 &&  NumElems == 4))
    return false;
 ...
 *  return true if the shuffle can be implemented with
 *   movsldup xmm1, xmm2
 *   the legal mask { 0, 0, 2, 2 } or undef(-1) at any place
 *  Or the similar AVX-256 instruction
 *   vmovsldup ymm1, ymm2
 *   the legal mask { 0, 0, 2, 2, 4, 4, 6, 6 } or undef(-1) at any place
 */
...
bool X86::isMOVDDUPMask(ShuffleVectorSDNode *N) {
  if (N->getValueType(0).getSizeInBits() > 128)
    return false;
...
/// isVMOVDDUPYMask - Return true if the specified VECTOR_SHUFFLE operand
/// specifies a shuffle of elements that is suitable for input to VMOVDDUPY.
bool X86::isVMOVDDUPYMask(ShuffleVectorSDNode *N) {
  int NumElements = N->getValueType(0).getVectorNumElements();

  if  ((NumElements != 4) || !N->getValueType(0).is256BitVector())
    return false;
 ...
 *
 * The legal mask: {0-3},{0-3},{0-3},{0-3},{4-7},{4-7},{4-7},{4-7}
 *   or undef at any place
 * And the mask should be symetric.
 */

static 
bool isVPERMIL8Mask(const SmallVectorImpl<int> &Mask) {
  // Check the range
  for (unsigned i=0; i< 4; i++) {
    if (!isUndefOrInRange(Mask[i], 0, 4))
      return false;
    if (!isUndefOrInRange(Mask[i+4], 4, 8))
      return false;
  }
  // Check symetric: Mask[0]&3 == Mask[4]&3, Mask[1]&3 == Mask[5]&3...
  for (unsigned i=0; i<4; i++) {
 ...
 * isVPERMILPDMask() 
 * return true if the 
 * vpermilpd ymm1, ymm2, imm8
 * instruction may be used.
 *
 * The legal mask: {0-1},{0-1},{2-3},{2-3} or undef at any place
 * And the mask should be symetric.
 */

static 
bool isVPERMIL4Mask(const SmallVectorImpl<int> &Mask) {
  if (
    !isUndefOrInRange(Mask[0], 0, 2) ||
    !isUndefOrInRange(Mask[1], 0, 2) ||
    !isUndefOrInRange(Mask[2], 2, 4) ||
 ...
 * instruction may be used.
 */
bool X86::isVPERMILMask(ShuffleVectorSDNode *N) {
  EVT VT = N->getValueType(0);
  ...
  return (NumElements == 4)?isVPERMIL4Mask(Mask):isVPERMIL8Mask(Mask);
}

/* 
 * getVPERMILImmediate() 
 * return an immediate for vpermilps or vpermilpd instruction
 */
unsigned X86::getVPERMILImmediate(ShuffleVectorSDNode *N) {
  EVT VT = N->getValueType(0);
  SmallVector<int, 8> Mask;
  N->getMask(Mask);
  unsigned NumElements = VT.getVectorNumElements();
  ...
  // The D-form (4 elements):
  // {0-1},{0-1},{2-3},{2-3} 
  // imm8[0] = Mask[0], imm8[1] = Mask[1], imm8[2] = Mask[2]&1,imm8[3] = Mask[3]&1

  if (NumElements == 4) {
    for (unsigned i=0; i<4; ++i)
      if (Mask[i] >=0) ImmMask |= (Mask[i]&1) << i;
  }

  // The S-form (8 elements):
  // {0-3},{0-3},{0-3},{0-3},{4-7},{4-7},{4-7},{4-7} - symetric
  // imm8[0:1] = Mask[0] or Mask[4] & 3
  // imm8[2:3] = Mask[1] or Mask[5] & 3 
  // imm8[4:5] = Mask[2] or Mask[6] & 3
  // imm8[6:7] = Mask[3] or Mask[7] & 3

  else {
    for (unsigned i=0; i<4; ++i) {
      if (Mask[i] >=0) ImmMask |= (Mask[i]&3) << i*2;
      else if (Mask[i+4] >=0) ImmMask |= (Mask[i+4]&3) << i*2;
    }
  }
...
/// isVPERM2F128Mask - Return true if the specified VECTOR_SHUFFLE operand
/// specifies a shuffle of elements that is suitable for input to VPERM2F128.
/// TODO: fix case such as  <undef, 1 ,2 ,3, 12, 13, 14, 15>
bool X86::isVPERM128Mask(ShuffleVectorSDNode *N) {
  EVT VT = N->getValueType(0);
  SmallVector<int, 8> Mask;
  N->getMask(Mask);
  ...
  for (unsigned i=0; i < NumElements; i+=NumElements/2) {  
    unsigned j=0;    
    // find first element which is not undef
	while ((j < NumElements/2)  && ( Mask[i+j] < 0) )
      ++j;
    
    // All undefs?
    if (j == NumElements/2) continue;

    int AnchorVal = Mask[i+j] - j;
    if (AnchorVal % (NumElements/2) != 0)
      return false;
    
    for (++j;j < NumElements/2; ++j)
      if (!isUndefOrEqual(Mask[i+j], AnchorVal+j))
        return false;
  }
...
unsigned X86::getVPERMCLImmediate(ShuffleVectorSDNode *N) {
  EVT VT = N->getValueType(0);
  SmallVector<int, 8> Mask;
  N->getMask(Mask);
  unsigned targetMask = 0;

  assert(VT.getVectorNumElements() <= 8);
  if (VT.getVectorNumElements() == 4) {
    for (unsigned i=0; i < 4; ++i) {
      if (Mask[i] > 0)
        targetMask |= Mask[i] << (i*2);
    }
  }
  else {
    for (unsigned i=0; i < 8; ++i) {
      if (Mask[i] > 0)
        targetMask |= Mask[i] << (i*3);
    }
  }
  return targetMask;
}

bool X86::isAllZeroSplatMask(ShuffleVectorSDNode *N) {
  EVT VT = N->getValueType(0);
  SmallVector<int, 8> Mask;
  N->getMask(Mask);
...
/// getVSHUFImmediate() - Return the appropriate immediate to shuffle
/// the specified VECTOR_SHUFFLE mask wiVSHUFP* instruction.
unsigned X86::getVSHUFImmediate(ShuffleVectorSDNode *N) {
  EVT VT = N->getValueType(0);
  SmallVector<int, 8> Mask;
  N->getMask(Mask);
  unsigned NumElements = VT.getVectorNumElements();
...
/// getVPERM2F128Immediate - Return the appropriate immediate to shuffle
/// the specified VECTOR_SHUFFLE mask with VPERM2F128 instruction.
unsigned X86::getVPERM128Immediate(ShuffleVectorSDNode *SVOp) {  
  ...
  unsigned ImmMask = 0;
  if (NumElems == 4) {
    if (Mask[0] > 0) ImmMask = (Mask[0]&6) >> 1;
    else if (Mask[1] > 0) ImmMask = (Mask[1]&6) >> 1;

    if (Mask[2] > 0) ImmMask |= ((Mask[2]&6) >> 1) << 4;
    else if (Mask[3] > 0) ImmMask |= ((Mask[3]&6) >> 1) << 4;

  }
  else { // 8 elements
    if (Mask[0] > 0) ImmMask = (Mask[0]&0xc) >> 2;
    else if (Mask[1] > 0) ImmMask = (Mask[1]&0xc) >> 2;
    else if (Mask[2] > 0) ImmMask = (Mask[2]&0xc) >> 2;
    else if (Mask[3] > 0) ImmMask = (Mask[3]&0xc) >> 2;

    if (Mask[4] > 0) ImmMask |= ((Mask[4]&0xc) >> 2) << 4;
    else if (Mask[5] > 0) ImmMask |= ((Mask[5]&0xc) >> 2) << 4;
    else if (Mask[6] > 0) ImmMask |= ((Mask[6]&0xc) >> 2) << 4;
    else if (Mask[7] > 0) ImmMask |= ((Mask[7]&0xc) >> 2) << 4;
...
static bool ShouldXformToMOVHLPS(ShuffleVectorSDNode *Op) {
  if ((Op->getValueType(0).getVectorNumElements() != 4) ||
    Op->getValueType(0).getSizeInBits() == 256)
    return false;
...
static SDValue getZeroVector(EVT VT, SelectionDAG &DAG, DebugLoc dl, 
  ...
  else if (VT.getSizeInBits() == 256) { 
    if (Subtarget->hasAVX2()) {
      SDValue Cst = DAG.getTargetConstant(0, MVT::i64);
      Vec = DAG.getNode(ISD::BUILD_VECTOR, dl, MVT::v4i64, Cst, Cst, Cst, Cst);
    
    } else {
      // AVX1
      // 256-bit logic and arithmetic instructions in AVX1 are
...
static SDValue getOnesVector(EVT VT, SelectionDAG &DAG, DebugLoc dl) {
  assert((VT.isVector() && (VT.getSizeInBits() >= 128)) && "Expected a vector type");

  // Always build ones vectors as <4 x i32> or <4 x i64> bitcasted to their dest
  // type.  This ensures they get CSE'd
  EVT constVectType;
  SDValue Cst;

  if (VT.getSizeInBits() == 128) {
    constVectType = MVT::v4i32;
    Cst = DAG.getTargetConstant(~0U, MVT::i32);
  }
  else { // (VT.getSizeInBits() == 256)
    constVectType = MVT::v4i64;
    Cst = DAG.getTargetConstant(~0ULL, MVT::i64);
  }

  SDValue Vec = DAG.getNode(ISD::BUILD_VECTOR, dl, constVectType, Cst, Cst, Cst, Cst);
  return DAG.getNode(ISD::BIT_CONVERT, dl, VT, Vec);
}
...
static SDValue getShuffleVectorZeroOrUndef(SDValue V2, unsigned Idx,
  ...
  SDValue V1 = isZero
    ? getZeroVector(VT, DAG, V2.getDebugLoc(), Subtarget) : DAG.getUNDEF(VT);
  unsigned NumElems = VT.getVectorNumElements();
...
SDValue getShuffleScalarElt(SDNode *N, int Index, SelectionDAG &DAG,
  ...
  assert (VT.getSizeInBits() != 256 || 
    "getShuffleScalarElt is not suppoted for 256-bit vectors");
...
static SDValue LowerBuildVectorv16i8(SDValue Op, unsigned NonZeros,
  ...
  for (unsigned i = 0; i < 16; ++i) {
    ...
    if (ThisIsNonZero && First) {
      if (NumZero)
        V = getZeroVector(MVT::v8i16, DAG, dl, Subtarget);
      else
...
/*
 * BuildVectorWithBroadcast_AVX2()
 * Create VBROADCAST SDNode if possible
 * AVX2 only
 */
static 
SDValue BuildVectorWithBroadcast_AVX2(SDValue Op, SDValue Op0, SelectionDAG &DAG) {
  DebugLoc dl = Op.getDebugLoc();
  EVT VT = Op.getValueType();
  EVT ElmVT = VT.getVectorElementType();

  EVT VT128 = EVT::getVectorVT(*DAG.getContext(),  ElmVT, 128/ElmVT.getSizeInBits());

  Op0 = DAG.getNode(ISD::SCALAR_TO_VECTOR, dl, VT128, Op0);
  return DAG.getNode(X86ISD::VBROADCAST_REG, dl, VT, Op0);
  
}

/*
 * BuildVectorWithBroadcast()
 * Create VBROADCAST SDNode if possible
 * AVX only
 */
static 
SDValue BuildVectorWithBroadcast(SDValue Op, SelectionDAG &DAG) {
  DebugLoc dl = Op.getDebugLoc();
  EVT VT = Op.getValueType();
  EVT ExtVT = VT.getVectorElementType();

  SDNode *Op0 = Op.getOperand(0).getNode();
  LoadSDNode *LD = dyn_cast<LoadSDNode>(Op0);
  if ((LD!=0 || Op0->getOpcode() != ISD::ConstantFP)&& !Op.getValue(0)->isOnlyUserOf(Op0)) {
    return SDValue();
  }
  
  unsigned NumElems = Op.getNumOperands();

  // Check for VBROADCAST opportunity
  // Need to refine the condition, since splat doesn't cover case where we broadcast v2f32
  if ((VT == MVT::v4f32 || VT == MVT::v8f32 || VT == MVT::v4f64 ||
       VT == MVT::v4i32 || VT == MVT::v8i32 || VT == MVT::v4i64) ) {

    unsigned i = 0;
  
    // Find first element which is not undef
    while ( i != NumElems && Op.getOperand(i).getOpcode() == ISD::UNDEF) 
      ++i;

    unsigned numUndefs = i;
  
    // All non-undef must be same load splatted across the vector
    LoadSDNode *LD = dyn_cast<LoadSDNode>(Op.getOperand(i));
    if (LD && RelaxedMayFoldVectorLoad(Op.getOperand(i)) && 
        // Instruction selector refuses to select when there is a chain from the load
        !(LD->hasAnyUseOfValue(1))) {
      SDValue LoadValue = Op.getOperand(i);  
      bool isVBROADCAST = true;

      for (; i != NumElems && isVBROADCAST; ++i)
        if (Op.getOperand(i).getOpcode() == ISD::UNDEF)
          ++numUndefs;
        else if (Op.getOperand(i) != LoadValue)
          isVBROADCAST = false;
  
      // Omit VMOVSS case
      bool isMOVSS = NumElems-1 == numUndefs && Op.getOperand(0).getOpcode() != ISD::UNDEF;
      isVBROADCAST = isVBROADCAST && !isMOVSS;

      if (isVBROADCAST) {
        SDValue Item = DAG.getNode(ISD::SCALAR_TO_VECTOR, dl, VT, LoadValue);
        return DAG.getNode(X86ISD::VBROADCAST_MEM, dl, VT, Item);
      }
    }
  }
  return SDValue();
...
static 
SDValue Split256BitVector(SDValue Op, SelectionDAG &DAG)
{
  DebugLoc dl = Op.getDebugLoc();
  ...
  return Concat128BitVectors(Lower, Upper, DAG);
}  
...
X86TargetLowering::LowerBUILD_VECTOR(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (ISD::isBuildVectorAllZeros(Op.getNode())) {
    ...
    if (Subtarget->hasAVX2() && (OpVT == MVT::v4i64))
      return Op;

    if (Subtarget->hasAVX() && (OpVT == MVT::v8f32))
      return Op;

    return getZeroVector(Op.getValueType(), DAG, dl, Subtarget);
  }
  ...
  if (ISD::isBuildVectorAllOnes(Op.getNode())) {
    ...
    if (!Subtarget->hasAVX2() && (OpVT == MVT::v4i64) )
      return Split256BitVector(Op, DAG);
  ...
  SmallSet<SDValue, 32> Values;
  unsigned FirstDefinedOp = NumElems;
  for (unsigned i = 0; i < NumElems; ++i) {
    SDValue Elt = Op.getOperand(i);
    if (Elt.getOpcode() == ISD::UNDEF)
      continue;
    Values.insert(Elt);
    if (FirstDefinedOp == NumElems) FirstDefinedOp = i;
  ...
  // Try to build vector with BROADCAST
  
  if (Subtarget->hasAVX2() && !IsAllConstants && Values.size() == 1) {
    SDValue resNode = BuildVectorWithBroadcast_AVX2(Op, Op.getOperand(FirstDefinedOp), DAG);
    if (resNode != SDValue())
      return resNode;
  }

  if (Subtarget->hasAVX() && !IsAllConstants) {
    SDValue resNode = BuildVectorWithBroadcast(Op, DAG);
    if (resNode != SDValue())
      return resNode;
  } 

  if ((VT.getSizeInBits() == 256) && !IsAllConstants)
    return Split256BitVector(Op, DAG);
  ...
  if (NumNonZero == 1) {
    ...
    if (ExtVT == MVT::i64 && !Subtarget->is64Bit() &&
      ...
      if (DAG.MaskedValueIsZero(Item, APInt::getBitsSet(64, 32, 64))) {
        ...
        Item = getShuffleVectorZeroOrUndef(Item, 0, true, DAG, Subtarget);
    ...
    if (Idx == 0) {
      if (NumZero == 0) {
        ...
        return getShuffleVectorZeroOrUndef(Item, 0, true, DAG, Subtarget);
    ...
    if (EVTBits == 32) {
      ...
      Item = getShuffleVectorZeroOrUndef(Item, 0, NumZero > 0, DAG, Subtarget);
      SmallVector<int, 8> MaskVec;
      for (unsigned i = 0; i < NumElems; i++)
  ...
  if (EVTBits == 64) {
    if (NumNonZero == 1) {
      ...
      SDValue V2 = DAG.getNode(ISD::SCALAR_TO_VECTOR, dl, VT,
                                 Op.getOperand(Idx));
      return getShuffleVectorZeroOrUndef(V2, Idx, true, DAG, Subtarget);
    }
    return SDValue();
  }
  ...
  if (NumElems == 4 && NumZero > 0) {
    for (unsigned i = 0; i < 4; ++i) {
      ...
      if (isZero)
        V[i] = getZeroVector(VT, DAG, dl, Subtarget);
      else
...
X86TargetLowering::LowerCONCAT_VECTORS(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (ResVT.getSizeInBits() == 256){
    return Concat128BitVectors(Op.getOperand(0), Op.getOperand(1), DAG);
  }
...
/*
 * Shuffle_4_of_2x64_Vectors_Into_1()
 * The function returns a shuffle node that shuffles 2 vectors v2f64 into one
 *
 * At input we have 4 vectors v2f64 and a mask with 2 elements
 * According to this mask we choose 2 relevant input vectors and create a shuffle
 * node that merges these 2 input vectors into one.
 */
static SDValue
Shuffle_4_of_2x64_Vectors_Into_1(ShuffleVectorSDNode *SVOp, EVT VT, SmallVector <SDValue, 4>& Ops, SmallVector <int, 2>& Mask, SelectionDAG &DAG)
{

  DebugLoc dl = SVOp->getDebugLoc();

  SmallVector<int, 2> DstMask(4U, -1);

  SDValue Op0 = DAG.getUNDEF(VT);
  SDValue Op1 = DAG.getUNDEF(VT);

  if (Mask[0] != -1)
  {
    unsigned Op0Idx = Mask[0]>>1; // Mask[0]/2
    DstMask[0] = Mask[0] & 1 ; // Mask[0] mod 2
    Op0 = Ops[Op0Idx];
  }

  if (Mask[1] != -1) {
    unsigned Op1Idx = Mask[1]>>1; // Mask[1]/4
    DstMask[1] = (Mask[1] & 1)+2 ; // Mask[1] mod 4
    Op1 = Ops[Op1Idx];
  }

  assert((Mask[0] != -1) || (Mask[1] != -1));

  return DAG.getVectorShuffle(VT, dl, Op0, Op1, &DstMask[0]);
  
}

/*
 * LowerVECTOR_SHUFFLE_4x64()
 *
 * This function is called when we can't replace a given v4f64 shuffle 
 * with one instruction.
 * Now we are going to split the v4f64 shuffle into many smaller shuffles
 */

static SDValue
LowerVECTOR_SHUFFLE_4x64(ShuffleVectorSDNode *SVOp, SelectionDAG &DAG) {
  SDValue V1 = SVOp->getOperand(0);
  SDValue V2 = SVOp->getOperand(1);
  DebugLoc dl = SVOp->getDebugLoc();
  EVT VT = SVOp->getValueType(0);
  ...
  bool Src0InUse = false;
  bool Src1InUse = false;
  bool Src2InUse = false;
  bool Src3InUse = false;
  bool Dst0Written = false;
  bool Dst1Written = false;

  SmallVector<int, 4> PermMask;
  SVOp->getMask(PermMask);

  SmallVector<int, 2> Dst0Mask(4U, -1);
  SmallVector<int, 2> Dst1Mask(4U, -1);

  // PermMask is an array of 4 elements, each element from 0 to 7 (or -1)

  for (unsigned i=0; i<2; i++)
  {
    int Idx0 = PermMask[i];
    int Idx1 = PermMask[i+2];
    Dst0Mask[i] = Idx0;
    Dst1Mask[i] = Idx1;

    if (Idx0 >= 6) Src3InUse = true;
    else if (Idx0 >= 4) Src2InUse = true;
    else if (Idx0 >= 2) Src1InUse = true;
    else if (Idx0 >= 0) Src0InUse = true;

    if (Idx1 >= 6) Src3InUse = true;
    else if (Idx1 >= 4) Src2InUse = true;
    else if (Idx1 >= 2) Src1InUse = true;
    else if (Idx1 >= 0) Src0InUse = true;

    if (PermMask[i] >= 0) Dst0Written = true;
    if (PermMask[i+2] >= 0) Dst1Written = true;
  }


  /* extract src0 (src0 is a lower part of the original Operand_0) and add it as input SD node to dst0 and dst1 */
  SmallVector <SDValue, 4> InputOps4wide;

  if (Src0InUse)
  {
    InputOps4wide.push_back(Extract128BitVector(V1, DAG.getConstant(0, MVT::i32), DAG, dl));
  }
  else
  {
    InputOps4wide.push_back(DAG.getUNDEF(HalfVT));
  }

  /* extract src1 (src1 is a higher part of the original Operand_0) and add it as input SD node to dst0 and dst1 */
  if (Src1InUse)
  {
    InputOps4wide.push_back(Extract128BitVector(V1, DAG.getConstant(2, MVT::i32), DAG, dl));
  }
  else
  {
    InputOps4wide.push_back(DAG.getUNDEF(HalfVT));
  }
  /* extract src2 (src2 is a lower part of the original Operand_1) and add it as input SD node to dst0 and dst1 */
  if (Src2InUse)
  {
    InputOps4wide.push_back(Extract128BitVector(V2, DAG.getConstant(0, MVT::i32), DAG, dl));
  }
  else
  {
    InputOps4wide.push_back(DAG.getUNDEF(HalfVT));
  }
  /* extract src3 (src3 is a higher part of the original Operand_1) and add it as input SD node to dst0 and dst1 */
  if (Src3InUse)
  {
    InputOps4wide.push_back(Extract128BitVector(V2, DAG.getConstant(2, MVT::i32), DAG, dl));
  }
  else
  {
    InputOps4wide.push_back(DAG.getUNDEF(HalfVT));
  ...
  SDValue DstLo = DAG.getNode(ISD::UNDEF, dl, HalfVT);
  SDValue DstHi = DAG.getNode(ISD::UNDEF, dl, HalfVT);
  if (Dst0Written) {
    DstLo = Shuffle_4_of_2x64_Vectors_Into_1(SVOp, HalfVT, InputOps4wide, Dst0Mask, DAG);
  }
  if (Dst1Written) {
    DstHi = Shuffle_4_of_2x64_Vectors_Into_1(SVOp, HalfVT, InputOps4wide, Dst1Mask, DAG);
  }

  SDValue allTogether = Concat128BitVectors(DstLo, DstHi, DAG);
...
/*
 * Shuffle_4_of_4x32_Vectors_Into_1()
 * The function returns a shuffle node that shuffles 2 vectors v4f32 into one
 *
 * At input we have 4 vectors v4f32 and a mask with 4 elements
 * According to this mask we choose 2 relevant input vectors and create a shuffle
 * node that merges these 2 input vectors into one.
 */
static SDValue
Shuffle_4_of_4x32_Vectors_Into_1(ShuffleVectorSDNode *SVOp, EVT VT, SmallVector <SDValue, 4>& Ops, SmallVector <int, 4>& Mask, SelectionDAG &DAG)
{

  DebugLoc dl = SVOp->getDebugLoc();

  // Masks like [0 1 2 3] don't need any shuffles. Just return the op
  bool isPassThru = true;
  for (int i=0; i<4; ++i) {
    if (Mask[i] != -1 && Mask[i] != i) isPassThru = false;
  }
  if (isPassThru) return Ops[0];

  SmallVector<int, 4> DstLoMask(4U, -1);
  SmallVector<int, 4> DstHiMask(4U, -1);
  ...
  if (Mask[0] != -1)
  {
    Op0Idx = Mask[0]>>2; // Mask[0]/4
    DstLoMask[0] = Mask[0] & 3 ; // Mask[0] mod 4
    Op0 = Ops[Op0Idx];
  }

  if (Mask[1] != -1) {
    Op1Idx = Mask[1]>>2; // Mask[1]/4
    DstLoMask[1] = (Mask[1] & 3)+4 ; // Mask[1] mod 4
    Op1 = Ops[Op1Idx];
  }

  SDValue DstLo = DAG.getUNDEF(VT);

  if ((Mask[0] != -1) || (Mask[1] != -1)) {
    DstLo = DAG.getVectorShuffle(VT, dl, Op0, Op1, &DstLoMask[0]);
  }
  ...
  if (Mask[2] != -1)
  {
    Op0Idx = Mask[2]>>2; // Mask[2]/4
    DstHiMask[0] = Mask[2] & 3 ; // Mask[2] mod 4
    Op0 = Ops[Op0Idx];
  }

  if (Mask[3] != -1) {
    Op1Idx = Mask[3]>>2; // Mask[3]/4
    DstHiMask[1] = (Mask[3] & 3)+4 ; // Mask[3] mod 4
    Op1 = Ops[Op1Idx];
  }

  SDValue DstHi = DAG.getUNDEF(VT);

  if ((Mask[2] != -1) || (Mask[3] != -1)) {
    DstHi = DAG.getVectorShuffle(VT, dl, Op0, Op1, &DstHiMask[0]);
  }

  SmallVector<int, 4> JoinMask ;
  JoinMask.push_back(0);
  JoinMask.push_back(1);
  JoinMask.push_back(4);
  JoinMask.push_back(5);
  return DAG.getVectorShuffle(VT, dl, DstLo, DstHi, &JoinMask[0]);
}

/*
 * LowerVECTOR_SHUFFLE_8x32()
 *
 * This function is called when we can't replace a given v8f32 shuffle 
 * with one instruction.
 * Now we are going to split the v8f32 shuffle into many smaller shuffles
 */
static SDValue
LowerVECTOR_SHUFFLE_8x32(ShuffleVectorSDNode *SVOp, SelectionDAG &DAG) {
  SDValue V1 = SVOp->getOperand(0);
  SDValue V2 = SVOp->getOperand(1);
  DebugLoc dl = SVOp->getDebugLoc();
  EVT VT = SVOp->getValueType(0);
  ...
  // We consider 2 vectors eight element each as 4 sub-vectors.
  ...
  SmallVector<int, 4> Dst0Mask(4U, -1);
  SmallVector<int, 4> Dst1Mask(4U, -1);

  // PermMask is an array of 8 elements, each element from 0 to 15 (or -1)

  for (unsigned i=0; i<4; i++)
  {
    int Idx0 = PermMask[i];
    int Idx1 = PermMask[i+4];
    Dst0Mask[i] = Idx0;
    Dst1Mask[i] = Idx1;

    if (Idx0 >= 12) Src3InUse = true;
    else if (Idx0 >= 8) Src2InUse = true;
    else if (Idx0 >= 4) Src1InUse = true;
    else if (Idx0 >= 0) Src0InUse = true;

    if (Idx1 >= 12) Src3InUse = true;
    else if (Idx1 >= 8) Src2InUse = true;
    else if (Idx1 >= 4) Src1InUse = true;
    else if (Idx1 >= 0) Src0InUse = true;

    if (PermMask[i] >= 0) Dst0Written = true;
    if (PermMask[i+4] >= 0) Dst1Written = true;
  }
  ...
  if (Src1InUse)
  {
    InputOps4wide.push_back(Extract128BitVector(V1, DAG.getConstant(4, MVT::i32), DAG, dl));
  }
  else
  {
    InputOps4wide.push_back(DAG.getUNDEF(HalfVT));
  ...
  if (Src3InUse)
  {
    InputOps4wide.push_back(Extract128BitVector(V2, DAG.getConstant(4, MVT::i32), DAG, dl));
  }
  else
  {
    InputOps4wide.push_back(DAG.getUNDEF(HalfVT));
  ...
  SDValue DstHi = DAG.getNode(ISD::UNDEF, dl, HalfVT);
  if (Dst0Written) {
    DstLo = Shuffle_4_of_4x32_Vectors_Into_1(SVOp, HalfVT, InputOps4wide, Dst0Mask, DAG);
  }
  if (Dst1Written) {
    DstHi = Shuffle_4_of_4x32_Vectors_Into_1(SVOp, HalfVT, InputOps4wide, Dst1Mask, DAG);
  }
...
static unsigned getPERMCLOpcode(EVT VT) {
  switch(VT.getSimpleVT().SimpleTy) {
  case MVT::v8i32: return X86ISD::PERMD;
  case MVT::v4i64: return X86ISD::PERMQ;
  case MVT::v8f32: return X86ISD::PERMPS;
  case MVT::v4f64: return X86ISD::PERMPD;
  default:
    llvm_unreachable("Unknown type for permcl");
  }
  return 0;
}
...
SDValue NormalizeVectorShuffle(SDValue Op, SelectionDAG &DAG,
  ...
  if (isZeroShuffle(SVOp))
    return getZeroVector(VT, DAG, dl, Subtarget);
...
X86TargetLowering::LowerVECTOR_SHUFFLE(SDValue Op, SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
  if (VT.getSizeInBits() == 256) {
    return Subtarget->hasAVX2()? LowerVECTOR_SHUFFLE_256_AVX2(Op, DAG):
                                 LowerVECTOR_SHUFFLE_256_AVX1(Op, DAG);
  }
  ShuffleVectorSDNode *SVOp = cast<ShuffleVectorSDNode>(Op);
  SDValue V1 = Op.getOperand(0);
  SDValue V2 = Op.getOperand(1);
  ...
  if (unsigned Res = getPALIGNRMaskType(M, VT, HasSSSE3)) {
    if (~0U != Res) {
      assert((Res==1 || Res==2) && "Unrecognized return value from isPALIGNRMask");
      if (1 == Res)
        // Build a unary node using V1 for both operands
        V2 = V1;
      else
        // Build a unary node using V2 for both operands
        V1 = V2;
	}
...
static bool isMatchMask(ShuffleVectorSDNode *shuff, int* MaskPattern) {
  SmallVector<int, 8> Mask;
  shuff->getMask(Mask);

  for (int i=0; i<8; i++) {
    if (!isUndefOrEqual(Mask[i], MaskPattern[i])) return false; 
  }
  return true;
}

SDValue
X86TargetLowering::LowerVECTOR_SHUFFLE_256_AVX2(SDValue Op, SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
  assert (VT.getSizeInBits() == 256);
  ShuffleVectorSDNode *SVOp = cast<ShuffleVectorSDNode>(Op);
  SDValue V1 = Op.getOperand(0);
  ...
  bool V1IsUndef = V1.getOpcode() == ISD::UNDEF;
  bool V2IsUndef = V2.getOpcode() == ISD::UNDEF;
  bool V1IsSplat = isSplatVector(V1.getNode());
  bool V2IsSplat = isSplatVector(V2.getNode());
  ...
  /*****
  In-lane and cross-lane permutation for 8x32 and 4x64 vectors
             | 8x32         | 4x64       
  ---------------------------------------
  Int IL     | VPSHUFD      | VPERMQ
  FP  IL     | VPERMILPS    | VPERMILPD
  Int CL     | VPERMD       | VPERMQ
  FP  CL     | VPERMPS      | VPERMPD

  * The closs-lane permutations for 8x32 vectors have mask in reg-mem
    form. All others have imm8 mask
  */

  if (X86::isVPERM128Mask(SVOp)) {
    unsigned TargetMask = X86::getVPERM128Immediate(SVOp);
    return getTargetShuffleNode(VT.isInteger()?X86ISD::VPERM2I128:X86ISD::VPERM2F128, 
      dl, VT, V1, V2, TargetMask, DAG);
  }

  // In-lane one source permutation for v8i32
  if ((VT == MVT::v8i32) && X86::isVPERMILMask(SVOp)) {
    unsigned TargetMask = X86::getVPERMILImmediate(SVOp);
    return getTargetShuffleNode(X86ISD::PSHUFD, dl, VT, V1, TargetMask, DAG);
  }

  // In-lane one source permutation for v8f32
  if ((VT == MVT::v8f32) && X86::isVPERMILMask(SVOp)) {
    unsigned TargetMask = X86::getVPERMILImmediate(SVOp);
    return getTargetShuffleNode(X86ISD::VPERMILPSY, dl, VT, V1, TargetMask, DAG);
  }

  // In-lane High-Words shuffle
  if ((VT == MVT::v16i16) && X86::isPSHUFHWMask(SVOp)) {
    unsigned TargetMask = X86::getShufflePSHUFHWImmediate(SVOp);
    return getTargetShuffleNode(X86ISD::PSHUFHW, dl, VT, V1, TargetMask, DAG);
  }

  // In lane Low-Words shuffle
  if ((VT == MVT::v16i16) && X86::isPSHUFLWMask(SVOp)) {
    unsigned TargetMask = X86::getShufflePSHUFLWImmediate(SVOp);
    return getTargetShuffleNode(X86ISD::PSHUFLW, dl, VT, V1, TargetMask, DAG);
  }
  ...
  // Cross-lane permutation for the both: integer and fp
  // 8-element shuffle is in register
  // 4-element shuffle is in immediate
  if (V2IsUndef) {
    if ((VT == MVT::v8i32)|| (VT == MVT::v8f32)){
      unsigned TargetMask = X86::getVPERMCLImmediate(SVOp);

      // 8x32 cross-lane mask in the reg-mem form
      SmallVector<SDValue,8> clMask;
      for (unsigned i=0; i<8; i++) {
        clMask.push_back(DAG.getConstant((TargetMask >>(i*3))&7, MVT::i32));
      }
      SDValue clConstPool = DAG.getNode(ISD::BUILD_VECTOR, dl,
                                  MVT::v8i32, &clMask[0], 8);
  
      return getTargetShuffleNode(getPERMCLOpcode(VT), dl, VT, V1, clConstPool, DAG);
    }
    else if ((VT == MVT::v4i64)|| (VT == MVT::v4f64)){
      unsigned TargetMask = X86::getVPERMCLImmediate(SVOp);
      return getTargetShuffleNode(getPERMCLOpcode(VT), dl, VT, V1, TargetMask, DAG);
    }
  }

  return LowerVECTOR_SHUFFLE_256_AVX1(Op, DAG);

}

SDValue
X86TargetLowering::LowerVECTOR_SHUFFLE_256_AVX1(SDValue Op, SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
  assert (VT.getSizeInBits() == 256);
  ShuffleVectorSDNode *SVOp = cast<ShuffleVectorSDNode>(Op);
  SDValue V1 = Op.getOperand(0);
  ...
  if (V2IsUndef && X86::isVMOVDDUPYMask(SVOp) && 
      RelaxedMayFoldVectorLoad(V1)) {
      return getTargetShuffleNode(X86ISD::VMOVDDUPY, dl, VT, V1, DAG);
  }

  if (X86::isMOVHLPSMask(SVOp))
    return getMOVHighToLow(Op, dl, DAG);

  if (X86::isMOVSHDUPMask(SVOp) && V2IsUndef)
    return getTargetShuffleNode(X86ISD::MOVSHDUP, dl, VT, V1, DAG);

  if (X86::isMOVSLDUPMask(SVOp) && V2IsUndef)
    return getTargetShuffleNode(X86ISD::MOVSLDUP, dl, VT, V1, DAG);



  if (X86::isVUNPCKLMask(SVOp))
    return getTargetShuffleNode(getUNPCKLOpcode(VT),
                                dl, VT, V1, V2, DAG);

  if (X86::isVUNPCKHMask(SVOp))
    return getTargetShuffleNode(getUNPCKHOpcode(VT), dl, VT, V1, V2, DAG);

  if (X86::isVSHUFPMask(SVOp)) {
    unsigned TargetMask = X86::getVSHUFImmediate(SVOp);
    unsigned Op = (VT.getVectorNumElements() == 8) ? X86ISD::VSHUFPSY : X86ISD::VSHUFPDY;
    return getTargetShuffleNode(Op, dl, VT, V1, V2,
                                  TargetMask, DAG);
  }

  // Try to swap operands in the node to match x86 shuffle ops
  if (!V2IsUndef && isCommutedVSHUFP(SVOp)) {
    SVOp = cast<ShuffleVectorSDNode>(CommuteVectorShuffle(SVOp, DAG));
    assert (X86::isVSHUFPMask(SVOp) || "Error in VSHUFP commute ");
    V1 = SVOp->getOperand(0);
    V2 = SVOp->getOperand(1);
    unsigned TargetMask = X86::getVSHUFImmediate(SVOp);
    unsigned Op = (VT.getVectorNumElements() == 8) ? X86ISD::VSHUFPSY : X86ISD::VSHUFPDY;
  ...
  if (V2IsUndef && X86::isVPERMILMask(SVOp) && RelaxedMayFoldVectorLoad(V1)) {
    unsigned TargetMask = X86::getVPERMILImmediate(SVOp);
    unsigned Opcode = (VT.getVectorNumElements() == 8) ? X86ISD::VPERMILPSY : X86ISD::VPERMILPDY;
    return getTargetShuffleNode(Opcode, dl, VT, V1, TargetMask, DAG);
  }

  // We are here because all optimization attempts failed.

  if (X86::isAllZeroSplatMask(SVOp)) {
    unsigned Opcode = (VT.getVectorNumElements() == 8) ? X86ISD::VPERMILPSY : X86ISD::VPERMILPDY;
    SDValue NewNode = getTargetShuffleNode(Opcode, dl, VT, V1, 0, DAG);
    return getTargetShuffleNode(X86ISD::VPERM2F128, dl, VT, NewNode, DAG.getUNDEF(VT), 0, DAG);
  }

  int Seq32L_seq[] = {0,8,2,10,4,12,6,14};
  int Seq32H_seq[] = {1,9,3,11,5,13,7,15};

  if (isMatchMask(SVOp, Seq32H_seq)) {
    SDValue Shuff = getTargetShuffleNode(X86ISD::VSHUFPSY, dl, VT, V1, V2, 0xdd, DAG);
    return getTargetShuffleNode(X86ISD::VPERMILPSY, dl, VT, Shuff, 0xd8, DAG);
  }

  if (isMatchMask(SVOp, Seq32L_seq)) {
    SDValue Shuff = getTargetShuffleNode(X86ISD::VSHUFPSY, dl, VT, V1, V2, 0x88, DAG);
    return getTargetShuffleNode(X86ISD::VPERMILPSY, dl, VT, Shuff, 0xd8, DAG);
  }

  if (NumElems == 8)
    return LowerVECTOR_SHUFFLE_8x32(SVOp, DAG);
  if (NumElems == 4)
    return LowerVECTOR_SHUFFLE_4x64(SVOp, DAG);

  assert(false || "Suffle not handled");
  return SDValue();
}
...
SDValue
X86TargetLowering::LowerEXTRACT_VECTOR_ELT_256(SDValue Op,
                                           SelectionDAG &DAG) const {
...
X86TargetLowering::LowerEXTRACT_VECTOR_ELT(SDValue Op,
  ...
  if (VecVT.getSizeInBits() == 256) 
      return LowerEXTRACT_VECTOR_ELT_256(Op, DAG);
...
X86TargetLowering::LowerINSERT_VECTOR_ELT(SDValue Op, SelectionDAG &DAG) const {
  ...
  DebugLoc dl = Op.getDebugLoc();
  SDValue N0 = Op.getOperand(0);  // The vector, where the element will be inserted
  SDValue N1 = Op.getOperand(1);  // The single element to insert
  SDValue N2 = Op.getOperand(2);  // Insertion position
  ...
  if (VT.getSizeInBits() > 128) {
    ...
    // Extract the 128-bit sub-vector from the original vector N0 
	// according to the insertion position N2
	// The extracted sub-vector is an upper or lower part of the 
	// original vector N0
    SDValue SubN0 = Extract128BitVector(N0, N2, DAG, dl);

	// Now we should insert an element into SubN0
	// Calculate the new insertion position for SubN0
	// For example, if the N2 was '3' in 4-elements vector N0,
	// the SubN0 is an upper part of N0 and 
	// the ScaledN2 will be '1' for 2-elements vector SubN0

	unsigned NumElems = VT.getVectorNumElements();
	unsigned IdxVal = cast<ConstantSDNode>(N2)->getZExtValue();
	bool Upper = IdxVal >= NumElems / 2;

    // Build a node that calculates ScaledN2 form the given N2 
	// and the original width of N0
    SDValue ScaledN2 = N2;
    if (Upper)
	...
	// Insert the given element N1 into the sub-vector SubN0
    SDValue SubN0AfterInsertion = DAG.getNode(ISD::INSERT_VECTOR_ELT, dl, SubN0.getValueType(), SubN0,
                     N1, ScaledN2);

    // Insert the 128-bit vector
    return Insert128BitVector(N0, SubN0AfterInsertion, N2, DAG, dl);
  }
...
X86TargetLowering::LowerEXTRACT_SUBVECTOR(SDValue Op, SelectionDAG &DAG) const {
  if (Subtarget->hasAVX()) {
    ...
    if (Op.getValueType().getSizeInBits() == 128
        && Vec.getValueType().getSizeInBits() == 256) {
...
SDValue X86TargetLowering::LowerFNEG(SDValue Op, SelectionDAG &DAG) const {
  ...
  EVT EltVT = VT;
  unsigned EltCount = 1;
  unsigned EltSizeInBits = EltVT.getSizeInBits();
  if (VT.isVector()) {
    EltVT = VT.getVectorElementType();
    EltSizeInBits = EltVT.getSizeInBits();
    EltCount = VT.getSizeInBits()/EltSizeInBits;
  }
  
  Constant *C = ConstantFP::get(*Context, APFloat(APInt(EltSizeInBits, 1ULL << (EltSizeInBits-1))));
  std::vector<Constant*> CV(EltCount, C);
  ...
  SDValue Mask = DAG.getLoad(VT, dl, DAG.getEntryNode(), CPIdx,
                             ...
                             false, false, 16);
  if (VT.isVector()) {
    EVT T = VT.getSizeInBits()==128 ? MVT::v2i64 : MVT::v4i64;
    return DAG.getNode(ISD::BIT_CONVERT, dl, VT,
                       DAG.getNode(ISD::XOR, dl, T,
                    DAG.getNode(ISD::BIT_CONVERT, dl, T,
                                Op.getOperand(0)),
                    DAG.getNode(ISD::BIT_CONVERT, dl, T, Mask)));
  } else {
...
SDValue X86TargetLowering::LowerVSETCC(SDValue Op, SelectionDAG &DAG) const {
  ...
  // AVX does not support integer compares on 256-bit operands
  if (!isFP && Op1.getValueType().is256BitVector() && !Subtarget->hasAVX2()) {
    return LowerOpTo128(Op, DAG);
  }
  
  if (isFP) {
    unsigned SSECC = 8;
    EVT VT0 = Op0.getValueType();
    unsigned Opc = X86ISD::CMPPS;

    if ((VT0 == MVT::v4f32) || (VT0 == MVT::v8f32)) {
      Opc = X86ISD::CMPPS;
    }
    else if ((VT0 == MVT::v2f64) || (VT0 == MVT::v4f64)) {
      Opc = X86ISD::CMPPD;
    }
    else assert(false && "Illegal input type for VSETCC operation");
...
/*
 * LowerOpTo128()
 * Lowering on 256-bit operation to two 128-bit operations
 * It is used for AVX integer operations, which provided in 128-bit form only.
 */
SDValue X86TargetLowering::LowerOpTo128(SDValue Op, SelectionDAG &DAG) const {
  ...
  assert(Subtarget->hasAVX() && "Can lower operation for AVX256 only");
  ...
  // Extract lower and upper parts of the first operand

  Op1Lo = DAG.getNode(ISD::EXTRACT_SUBVECTOR, dl, newVT, Op1,
                      DAG.getIntPtrConstant(0));
  Op1Hi = DAG.getNode(ISD::EXTRACT_SUBVECTOR, dl, newVT, Op1,
                      DAG.getIntPtrConstant(VT.getVectorNumElements()/2));

  // Extract lower and upper parts of the second operand

  Op2Lo = DAG.getNode(ISD::EXTRACT_SUBVECTOR, dl, newVT, Op2,
                      DAG.getIntPtrConstant(0));
  Op2Hi = DAG.getNode(ISD::EXTRACT_SUBVECTOR, dl, newVT, Op2,
                      DAG.getIntPtrConstant(VT.getVectorNumElements()/2));
					  
  // Some operations have the 3rd operand. It should not be splitted
  SDValue ResLo;
  SDValue ResHi;
  switch (Opcode) {
    case ISD::VSETCC:
      ResLo = DAG.getNode(Opcode, dl, newVT, Op1Lo, Op2Lo, Op.getOperand(2));
      ResHi = DAG.getNode(Opcode, dl, newVT, Op1Hi, Op2Hi, Op.getOperand(2));
      break;
    default:
      ResLo = DAG.getNode(Opcode, dl, newVT, Op1Lo, Op2Lo);
      ResHi = DAG.getNode(Opcode, dl, newVT, Op1Hi, Op2Hi);
    }
  ...
  return DAG.getNode(ISD::CONCAT_VECTORS, dl, VT,&Ops[0], Ops.size());

}

SDValue X86TargetLowering::GenerateFixedShift(SDNode* N, SelectionDAG &DAG, SDValue BaseShAmt) const {
...
SDValue X86TargetLowering::GenerateVariableShift(SDNode* N, SelectionDAG &DAG) const {

  EVT VT = N->getValueType(0);
  EVT EltVT = VT.getVectorElementType();
  DebugLoc DL = N->getDebugLoc();

  assert(Subtarget->hasAVX2() || " The variable shift supported on HSW"); 
  ...
  if ((VT == MVT::v8i32) || (VT== MVT::v4i64) ||
    ...
    default:
      ...
      break;
    case ISD::SHL:
      return DAG.getNode(X86ISD::VSHIFTLL, DL, VT, ValOp, ShAmt);
    case ISD::SRL:
      return DAG.getNode(X86ISD::VSHIFTRL, DL, VT, ValOp, ShAmt);
    case ISD::SRA:
      if ((VT == MVT::v4i32) ||(VT == MVT::v8i32)) 
         return DAG.getNode(X86ISD::VSHIFTRA, DL, VT, ValOp, ShAmt);
      break;
    }
  }
  ...
  {
    return LowerOpTo128(SDValue(N, 0), DAG);
  }

  if ((VT == MVT::v8i16))
  {
    // zero-extend the operand from v8i16 to v8i32
    ValOp = DAG.getNode(X86ISD::ZEXT_L, DL, MVT::v8i32, ValOp);

    // zero-extend the shift constant from v8i16 to v8i32
    ShAmt = DAG.getNode(X86ISD::ZEXT_L, DL, MVT::v8i32, ShAmt);

    // do the same shift operation on v8i32 
    ValOp = DAG.getNode(N->getOpcode(), DL, MVT::v8i32, ValOp, ShAmt);
    ...
    for (unsigned i=0; i<2; i++) {
      ...
      for (unsigned j=0; j<8; ++j)
        pshufbMask.push_back(DAG.getConstant(0xff, MVT::i8));
    }
    SDValue pshufbConstPool = DAG.getNode(ISD::BUILD_VECTOR, DL,
                                 MVT::v32i8, &pshufbMask[0], 32);

    ValOp = DAG.getNode(ISD::BIT_CONVERT, DL, MVT::v32i8, ValOp);
    ValOp = DAG.getNode(X86ISD::PSHUFB, DL, MVT::v32i8, ValOp, pshufbConstPool);

    ValOp = DAG.getNode(ISD::BIT_CONVERT, DL, MVT::v4i64, ValOp);

    unsigned TargetMask = 8; // 00 00 10 00
    ValOp = getTargetShuffleNode(X86ISD::PERMQ, DL, MVT::v4i64, ValOp, TargetMask, DAG);

    ValOp =  DAG.getNode(ISD::EXTRACT_SUBVECTOR, DL, MVT::v2i64, ValOp, DAG.getIntPtrConstant(0));
 
    return DAG.getNode(ISD::BIT_CONVERT, DL, VT, ValOp);
...
SDValue X86TargetLowering::LowerShift(SDValue Op, SelectionDAG &DAG) const {
  assert(Op.getNode()->getNumOperands() == 2 && "Shift Op must have two operands");
  EVT VT = Op.getValueType();
  DebugLoc dl = Op.getDebugLoc();
  SDValue R = Op.getOperand(0);
  SDValue SH = Op.getOperand(1);
  ...
  if (VT.is128BitVector() && Amt.getNode()->getOpcode() == ISD::EXTRACT_SUBVECTOR) {
    Amt = Amt.getOperand(0);
  }

  //TODO:  <i32 5, i32 0, i32 5, i32 5, i32 5, i32 5, i32 5, i32 5>
  //       We can split and treat each half separately

  // Optimize shl/srl with constant shift amount
  if (isSplatVector(Amt.getNode())) {
    SDValue SclrAmt = Amt->getOperand(0);
    if (dyn_cast<ConstantSDNode>(SclrAmt)) {
 
      if (VT.is256BitVector() && !Subtarget->hasAVX2()) 
        return LowerOpTo128(Op, DAG);
   
      SDValue ShNode = GenerateFixedShift(Op.getNode(), DAG, SclrAmt);
      if (ShNode.getNode() != 0)
        return ShNode;
    }
  }
  ...
  if (Subtarget->hasAVX2()) {
    SDValue res = GenerateVariableShift(Op.getNode(), DAG);
    if (res.getNode()) return res;
  }

  if (VT == MVT::v4i32 && Op->getOpcode() == ISD::SHL) {
    ConstantInt *MI = ConstantInt::get(*Context, APInt(32, 0x0000001FU));   
    std::vector<Constant*> MV(4, MI);
    Constant *MS = ConstantVector::get(MV);
    SDValue MSIdx = DAG.getConstantPool(MS, getPointerTy(), 16);
    SDValue Mask = DAG.getLoad(VT, dl, DAG.getEntryNode(), MSIdx,
                                 PseudoSourceValue::getConstantPool(), 0,
                                 false, false, 16);

    SH = DAG.getNode(ISD::AND, dl, VT, Mask, SH);


    Op = DAG.getNode(ISD::INTRINSIC_WO_CHAIN, dl, VT,
                     DAG.getConstant(Intrinsic::x86_sse2_pslli_d, MVT::i32),
                     SH, DAG.getConstant(23, MVT::i32));
  ...
  if (VT == MVT::v8i32 && Op->getOpcode() == ISD::SHL) {
    MVT VTI_Half = MVT::v4i32;
    MVT VTF_Half = MVT::v4f32;
    std::vector<Constant*> MV(VT.getVectorNumElements());
    
    bool isAmountConstant = SH.getOpcode() == ISD::BUILD_VECTOR;
    
    // If shift amount is constant, fold the adjusted shift amount (Amt & 0x1f)
    if (isAmountConstant) {
      for (size_t i=0; i<MV.size(); ++i) {
        SDValue ScalarOp = SH.getOperand(i);
        if (ScalarOp.getOpcode() != ISD::UNDEF && ScalarOp.getOpcode() != ISD::Constant) {
            //TODO: BUILD_VECTOR node can have operands which are operations such sext, zext, trunc...
            //      Need to handle all these cases and remove this condition.
            isAmountConstant = false;
            break;
        }
        uint64_t val = 0;
        if (ConstantSDNode* ScalarConstOp = cast<ConstantSDNode>(ScalarOp.getNode())) {
          val &= ScalarConstOp->getZExtValue() & 0x0000001FU;
        }
        ConstantInt *MI = ConstantInt::get(*Context, APInt(32, val ));
        MV[i] = MI;
      }
    } 
    if (!isAmountConstant) {
      ConstantInt *MI = ConstantInt::get(*Context, APInt(32, 0x0000001FU));
      std::fill(MV.begin(), MV.end(), MI);
    }
    ...
    ConstantInt *CI = ConstantInt::get(*Context, APInt(32, 0x3f800000U));
    std::vector<Constant*> CV(VTI_Half.getVectorNumElements(), CI);
    Constant *C = ConstantVector::get(CV);
    SDValue CPIdx = DAG.getConstantPool(C, getPointerTy(), 16);
    SDValue Addend = DAG.getLoad(VTI_Half, dl, DAG.getEntryNode(), CPIdx,
                               PseudoSourceValue::getConstantPool(), 0,
                               false, false, 16);
    ...
    for (unsigned i=0; i<2; ++i) {
      ...
      SDValue SH_Half = DAG.getNode(ISD::EXTRACT_SUBVECTOR, dl, VTI_Half, SH,
                      DAG.getIntPtrConstant(i * VTI_Half.getVectorNumElements()));
      SDValue R_Half = DAG.getNode(ISD::EXTRACT_SUBVECTOR, dl, VTI_Half, R,
                      DAG.getIntPtrConstant(i * VTI_Half.getVectorNumElements()));

      Op = DAG.getNode(ISD::INTRINSIC_WO_CHAIN, dl, VTI_Half,
                     DAG.getConstant(Intrinsic::x86_sse2_pslli_d, MVT::i32),
                     SH_Half, DAG.getConstant(23, MVT::i32));    
      
      Op = DAG.getNode(ISD::ADD, dl, VTI_Half, Op, Addend);
      Op = DAG.getNode(ISD::BIT_CONVERT, dl, VTF_Half, Op);
      Op = DAG.getNode(ISD::FP_TO_SINT, dl, VTI_Half, Op);
      Op = DAG.getNode(ISD::MUL, dl, VTI_Half, Op, R_Half);
      Ops[i] = Op;
    }
    return DAG.getNode(ISD::CONCAT_VECTORS, dl, VT, &Ops[0], Ops.size());
...
SDValue X86TargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  ...
  case ISD::ADD:                
  case ISD::SUB:                return LowerOpTo128(Op, DAG);
  }
}
...
const char *X86TargetLowering::getTargetNodeName(unsigned Opcode) const {
  ...
  case X86ISD::PERMD:              return "X86ISD::PERMD";
  case X86ISD::PERMQ:              return "X86ISD::PERMQ";
  ...
  case X86ISD::VSHUFPSY:            return "X86ISD::VSHUFPSY";
  case X86ISD::SHUFPD:             return "X86ISD::SHUFPD";
  case X86ISD::VSHUFPDY:            return "X86ISD::VSHUFPDY";
  ...
  case X86ISD::VMOVDDUPY:          return "X86ISD::VMOVDDUPY";
  ...
  case X86ISD::VUNPCKHPSY:         return "X86ISD::VUNPCKHPSY";
  case X86ISD::VUNPCKHPDY:         return "X86ISD::VUNPCKHPDY";
  ...
  case X86ISD::VBROADCAST_MEM:     return "X86ISD::VBROADCAST_MEM";
  case X86ISD::VBROADCAST_REG:     return "X86ISD::VBROADCAST_REG";
  ...
  case X86ISD::ZEXT_L:             return "X86ISD::ZEXT_L";
  case X86ISD::VSHIFTLL:           return "X86ISD::VSHIFTLL";
  case X86ISD::VSHIFTRL:           return "X86ISD::VSHIFTRL";
  case X86ISD::VSHIFTRA:           return "X86ISD::VSHIFTRA";
...
X86TargetLowering::isShuffleMaskLegal(const SmallVectorImpl<int> &M,
  ...
  if (VT.getSizeInBits() == 64)
    return getPALIGNRMaskType(M, VT, Subtarget->hasSSSE3());
  ...
  return (VT.getVectorNumElements() == 2 ||
          ...
          isPSHUFHWMask(M, VT) ||
          isPSHUFLWMask(M, VT) ||
          getPALIGNRMaskType(M, VT, Subtarget->hasSSSE3())!=0 ||
          isUNPCKLMask(M, VT) ||
          isUNPCKHMask(M, VT) ||
...
X86TargetLowering::EmitPCMP(MachineInstr *MI, MachineBasicBlock *BB,
  ...
  Opc = Subtarget->hasAVX() ? X86::VMOVAPSrr : X86::MOVAPSrr;

  BuildMI(BB, dl, TII->get(Opc), MI->getOperand(0).getReg())
    .addReg(X86::XMM0);
  ...
  for (int i = 3, e = MI->getNumOperands(); i != e; ++i) {
    ...
    MachineMemOperand *MMO =
      F->getMachineMemOperand(
        ...
        /*Size=*/16, /*Align=*/16);
    BuildMI(XMMSaveMBB, DL, TII->get(Subtarget->hasAVX() ? X86::VMOVAPSmr : X86::MOVAPSmr))
...
// Truncate in AVX2
static SDValue PerformTruncateCombine_AVX2(SDNode *N, SelectionDAG &DAG,
                                     const X86Subtarget *Subtarget)  {
  ...
  if ((VT == MVT::v4i32) && (OpVT == MVT::v4i64)) {
    ...
    Op = DAG.getNode(ISD::BIT_CONVERT, dl, MVT::v8i32, Op);
    // The mask: 0, 2, 4, 6
    SDValue Ops[] = { 
        DAG.getConstant(0, MVT::i32), 
        DAG.getConstant(2, MVT::i32), 
    ...
    SDValue  CstPool = DAG.getNode(ISD::BUILD_VECTOR, dl, MVT::v8i32, Ops, 8 );

    Op = DAG.getNode(X86ISD::PERMD, dl, MVT::v8i32, Op, CstPool);
  ...
  if ((VT == MVT::v8i16) && (OpVT == MVT::v8i32)) {
    ...
    Op = DAG.getNode(ISD::BIT_CONVERT, dl, MVT::v32i8, Op);
    Op = DAG.getNode(X86ISD::PSHUFB, dl, MVT::v32i8, Op, pshufbConstPool);

    Op = DAG.getNode(ISD::BIT_CONVERT, dl, MVT::v4i64, Op);

    unsigned TargetMask = 8; // 00 00 10 00
    Op = getTargetShuffleNode(X86ISD::PERMQ, dl, MVT::v4i64, Op, TargetMask, DAG);
...
SDValue X86TargetLowering::PerformTruncateCombine(SDNode *N, SelectionDAG &DAG) const
{
  if (!Subtarget->hasAVX()) return SDValue();

  if (Subtarget->hasAVX2()) {
    SDValue val = PerformTruncateCombine_AVX2(N, DAG, Subtarget);
    if (val.getNode() != 0)
      return val;
  }
  ...
  if ((VT == MVT::v4i32) && (OpVT == MVT::v4i64))
  {
    ...
    OpLo = DAG.getNode(ISD::BIT_CONVERT, dl, MVT::v4i32, OpLo);
    OpHi = DAG.getNode(ISD::BIT_CONVERT, dl, MVT::v4i32, OpHi);

    unsigned TargetMask = 8; // 00 00 10 00 [0, 0, 2, 0]
    OpLo = getTargetShuffleNode(X86ISD::PSHUFD, dl, VT, OpLo, TargetMask, DAG);
    OpHi = getTargetShuffleNode(X86ISD::PSHUFD, dl, VT, OpHi, TargetMask, DAG);

    return getTargetShuffleNode(X86ISD::MOVLHPS, dl, VT, OpLo, OpHi, DAG);
  }
  if ((VT == MVT::v8i16) && (OpVT == MVT::v8i32))
  {
    ...
    OpLo = DAG.getNode(ISD::BIT_CONVERT, dl, MVT::v16i8, OpLo);
    OpHi = DAG.getNode(ISD::BIT_CONVERT, dl, MVT::v16i8, OpHi);
    OpLo = DAG.getNode(X86ISD::PSHUFB, dl, MVT::v16i8, OpLo, pshufbConstPool);
    OpHi = DAG.getNode(X86ISD::PSHUFB, dl, MVT::v16i8, OpHi, pshufbConstPool);

    OpLo = DAG.getNode(ISD::BIT_CONVERT, dl, MVT::v4i32, OpLo);
    OpHi = DAG.getNode(ISD::BIT_CONVERT, dl, MVT::v4i32, OpHi);

    SDValue res = getTargetShuffleNode(X86ISD::MOVLHPS, dl, MVT::v4i32, OpLo, OpHi, DAG);
    return DAG.getNode(ISD::BIT_CONVERT, dl, MVT::v8i16, res);
...
// Get base amount of Bits to be shifted
static SDValue ShiftBaseAmount(SDNode* N, SelectionDAG &DAG) {
  ...
  if (ShAmtOp.getOpcode() == ISD::VECTOR_SHUFFLE &&
...
///                       when possible.
SDValue X86TargetLowering::PerformShiftCombine(SDNode* N, SelectionDAG &DAG) const {
  EVT VT = N->getValueType(0);
  ...
  if (!VT.isInteger() || (VT.getSizeInBits() < 128)  || !VT.isVector())
    return SDValue();

  // 128-bit vectors or 256-bit for AVX2
  if ((VT.getSizeInBits() == 256) && (!Subtarget->hasAVX2()))
    return SDValue();

  SDValue ShAmt = ShiftBaseAmount(N, DAG);
  if (ShAmt.getNode() != 0) {
    return GenerateFixedShift(N, DAG, ShAmt);
  }
  else if (Subtarget->hasAVX2())
    return GenerateVariableShift(N, DAG);
  return SDValue();
...
static SDValue PerformSTORECombine(SDNode *N, SelectionDAG &DAG,
  ...
  // If we are saving a concatination of two XMM registers, perform two saves
  if (VT.getSizeInBits() == 256 && 
...
SDValue X86TargetLowering::PerformZExtCombine(SDNode *N, SelectionDAG &DAG) const {
  ...
  if (N0.getOpcode() == ISD::AND &&  N0.hasOneUse()) {
    ...
    if (N00.hasOneUse() && (N00.getOpcode() == X86ISD::SETCC_CARRY)) {
  ...
  if (Subtarget->hasAVX2())
  {
    if (((VT == MVT::v8i32) && (OpVT == MVT::v8i16))  ||
      ...
      return DAG.getNode(X86ISD::ZEXT_L, dl, VT, N0);
    }
    return SDValue();
  }
  /* Optimize vectors in SSE41 mode
     zero extend v4i8 to v4i32 use vpmovzxbd
     zero extend v4i16 to v4i32 use vpmovzxwd     
  */

  if (Subtarget->hasSSE41()) {

    if ((VT == MVT::v4i32) && (OpVT == MVT::v4i8)) {

      if ((N0.getOpcode() == ISD::EXTRACT_SUBVECTOR) &&
          (N0.getOperand(0).getValueType().getSizeInBits() == 128))
      {
        N0 = DAG.getNode(ISD::BIT_CONVERT, dl, MVT::v16i8, N0.getOperand(0));
      } else {
        N0 = DAG.getNode(ISD::BIT_CONVERT, dl, MVT::i32, N0);
        N0 = DAG.getNode(ISD::SCALAR_TO_VECTOR, dl, MVT::v4i32, N0);
        N0 = DAG.getNode(ISD::BIT_CONVERT, dl, MVT::v16i8, N0);
      }
      return DAG.getNode(ISD::INTRINSIC_WO_CHAIN, dl, MVT::v4i32,
        DAG.getConstant(Intrinsic::x86_sse41_pmovzxbd, MVT::i32), N0);
    }
    if ((VT == MVT::v4i32) && (OpVT == MVT::v4i16)) {
      ...
      if ((N0.getOpcode() == ISD::EXTRACT_SUBVECTOR) &&
          (N0.getOperand(0).getValueType().getSizeInBits() == 128))
      {
        N0 = DAG.getNode(ISD::BIT_CONVERT, dl, MVT::v8i16, N0.getOperand(0));
      } else {
        N0 = DAG.getNode(ISD::BIT_CONVERT, dl, MVT::i64, N0);
        N0 = DAG.getNode(ISD::SCALAR_TO_VECTOR, dl, MVT::v2i64, N0);
        N0 = DAG.getNode(ISD::BIT_CONVERT, dl, MVT::v8i16, N0);
      }
      return DAG.getNode(ISD::INTRINSIC_WO_CHAIN, dl, MVT::v4i32,
        DAG.getConstant(Intrinsic::x86_sse41_pmovzxwd, MVT::i32), N0);
    }
  }

  /* Optimize vectors in AVX mode
     v8i8 -> v8i32 
     1) zero extend v8i8 to v8i16 , use pmovzxbw
     2) zero extend v8i16 to v8i32
  ...
  */
  if (Subtarget->hasAVX()) {
    ...
    if (((VT == MVT::v8i32) && (OpVT == MVT::v8i16))  ||
      ...
      SDValue ZeroVec = getZeroVector(OpVT, DAG, dl, Subtarget);
      SDValue OpLo = getTargetShuffleNode(getUNPCKLOpcode(OpVT), dl, OpVT, N0, ZeroVec, DAG);
      SDValue OpHi = getTargetShuffleNode(getUNPCKHOpcode(OpVT), dl, OpVT, N0, ZeroVec, DAG);

      EVT HVT = EVT::getVectorVT(*DAG.getContext(), VT.getVectorElementType(), 
        VT.getVectorNumElements()/2);

      OpLo = DAG.getNode(ISD::BIT_CONVERT, dl, HVT, OpLo);
      OpHi = DAG.getNode(ISD::BIT_CONVERT, dl, HVT, OpHi);

      return Concat128BitVectors(OpLo, OpHi, DAG);
    }
  }
...
SDValue X86TargetLowering::PerformDAGCombine(SDNode *N,
  ...
  case ISD::TRUNCATE: return PerformTruncateCombine(N, DAG);
  }
