...
void X86RegisterInfo::emitPrologue(MachineFunction &MF) const {
  ...
  if (Is64Bit && !Fn->hasFnAttr(Attribute::NoRedZone) &&
    ...
    unsigned RedZoneSize = 32;
    // We take the following max in order to preserve alignment
    // in the case where MaxAlign>RedZoneSize
    StackSize += std::max(MaxAlign, RedZoneSize);
    MFI->setStackSize(StackSize);
  }
