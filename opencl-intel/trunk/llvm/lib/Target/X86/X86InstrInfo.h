...
namespace X86II {
  enum {
    ...
    // MRMDstIsSrcReg - Used for AVX instructions where dest and src 0 are the 
    // same register, and src 1 is a register.
    MRMDstIsSrcReg = 44,

	// MRMDstIsSrcReg - Used for AVX instructions where dest and src 0 are the 
    // same, and src 1 is a memory addrress.
    MRMDstIsSrcMem = 45,
  ...
  static inline int getMemoryOperandNo(uint64_t TSFlags) {
	...
	case X86II::MRMSrcMem: {
      ...
      if (HasVEX_4V && MRMDstIsSrcMem != Form)
