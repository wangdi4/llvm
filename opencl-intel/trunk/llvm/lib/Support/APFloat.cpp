...
namespace llvm {
  ...
  // by default flush to zero is disabled.
  bool APFloat::eliminateDenormals = false;
}
...
void APFloat::fixDenormal()
{
    if (semantics->minExponent == exponent && category == fcNormal)  // if min. exponent convert to 0
        *this = getZero(getSemantics(), isNegative());
}

/* Normalized addition or subtraction.  */
APFloat::opStatus
APFloat::addOrSubtract(const APFloat &_rhs, roundingMode rounding_mode,
                       bool subtract)
{
  opStatus fs;
...
/* Normalized multiply.  */
APFloat::opStatus
APFloat::multiply(const APFloat &_rhs, roundingMode rounding_mode)
{
  opStatus fs;
...
/* Normalized divide.  */
APFloat::opStatus
APFloat::divide(const APFloat &_rhs, roundingMode rounding_mode)
{
  opStatus fs;
...
APFloat::cmpResult APFloat::externalCompare(const APFloat &lhs, const APFloat &rhs)
{
  cmpResult result;
  assert(lhs.semantics == rhs.semantics);
  ...
  if (lhs.sign != rhs.sign) {
    ...
    result = lhs.compareAbsoluteValue(rhs);
...
APFloat::cmpResult
APFloat::compare(const APFloat &_rhs) const
{
  ...
  return externalCompare(lhs, rhs);
}
