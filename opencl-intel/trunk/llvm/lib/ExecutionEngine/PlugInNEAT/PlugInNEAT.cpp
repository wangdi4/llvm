                                  ...
                                  std::map<Value *, NEATGenericValue>& out_NEATArgValues )
{
    ...
    // WARNING!!!: Dirty hack.
    // OCLUpdatePass Transform pass adds 4 additional
    // implicit arguments to kernel. We take them into consideration at
    // number of arguments check
    // TODO: remove hack dependency on Update pass
    const size_t IMPLICIT_ARG_COUNT = 4;
    if(in_F->arg_size() - IMPLICIT_ARG_COUNT != in_BC.GetMemoryObjectCount())
    {
        ...
        ++arg_it, ++buf)
    {
        ...
        {   // buffer
            Buffer * currBuffer = dynamic_cast<Buffer*>(currMemObj);
            BufferDesc buffDsc = GetBufferDescription(currMemObj->GetMemoryObjectDesc());
...
// TODO: implement
  ...
  case Type::OpaqueTyID:
    {
/*
            NEAT do not support for opaque TypeID
            TODO: verify it does not break floating point data store in OpaqueTyID
            Example of generating this type is images:

            %struct._image2d_t = type opaque
           define void @test_fn(%struct._image2d_t* %srcimg, i32 %sampler, <4 x float> addrspace(1)* %results) nounwind {
...
void NEATPlugIn::visitExtractValueInst( ExtractValueInst &I )
{
  ...
  for (unsigned i = 0 ; i < Num; ++i) {
    ...
    ++IdxBegin;
    const Type *IndexedType = ExtractValueInst::getIndexedType(Agg->getType(), I.idx_begin(), IdxBegin);
                  ...
                  const OCLBuiltinParser::ArgVector& ArgList)
{
    ...
    uint32_t sampler = GetGenericArg(1).IntVal.getZExtValue();
    ...
    const GenericValue& CoordGV = GetGenericArg(2);
...
void execute_dot(Function *F,
    ...
    if (Ty0->isVectorTy() && Ty1->isVectorTy()) {
        ...
        const Type *TyElem1 = dyn_cast<VectorType>(Ty1)->getElementType();
        if (TyElem0->isFloatTy() && TyElem1->isFloatTy()) {
            Result.NEATVal = NEATALU::dot<float>(ValArg0.NEATVec, ValArg1.NEATVec);
        } else if (TyElem0->isDoubleTy() && TyElem1->isDoubleTy()) {
            Result.NEATVal = NEATALU::dot<double>(ValArg0.NEATVec, ValArg1.NEATVec);
        } else {
            throw Exception::IllegalFunctionCall("[NEATPlug-in::dot]: Not valid vector data type for built-in");}
    } else if (Ty0->isDoubleTy() && Ty1->isDoubleTy()) {
            Result.NEATVal = NEATALU::dot<double>(ValArg0.NEATVal, ValArg1.NEATVal);
    } else if (Ty0->isFloatTy() && Ty1->isFloatTy()) {
            Result.NEATVal = NEATALU::dot<float>(ValArg0.NEATVal, ValArg1.NEATVal);
    } else{
        throw Exception::IllegalFunctionCall("[NEATPlug-in::dot]: Not valid data type for built-in");
    }
}
...
//    HANDLE_BI_ONEARG( 79,     tgamma)
    ...
    HANDLE_BI_EXECUTE(96, dot)
    HANDLE_BI_ONEARG( 97, normalize)
    HANDLE_BI_ONEARG( 98, radians)
    HANDLE_BI_ONEARG( 99, degrees)
    ...
    HANDLE_BI_EXECUTE( 110,     clamp)
    HANDLE_BI_ONEARG( 111,     sign)
    HANDLE_BI_EXECUTE( 112,     smoothstep)
...
void NEATPlugIn::visitUIToFPInst( UIToFPInst &I )
{
  ...
  if (Type::VectorTyID == ST->getTypeID())
  {
    ...
    const Type* DstVecType = RT->getScalarType();
