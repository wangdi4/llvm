...
struct float3 {float a;float b;float c;};
struct double3 {double a;double b;double c;};
struct m256bit {__m128 a;__m128 b;};
struct m512bit {__m128 a;__m128 b;__m128 c;__m128 d;};
struct m1024bit {__m128 a;__m128 b;__m128 c;__m128 d; __m128 e;__m128 f;__m128 g;__m128 h;};

ffi_type ffi_type_f2 = {sizeof(float)*2, sizeof(__m128),FFI_TYPE_FLOAT, NULL};
ffi_type ffi_type_f3 = {sizeof(float)*3, sizeof(__m128),FFI_TYPE_FLOAT, NULL};
ffi_type ffi_type_d3 = {sizeof(double)*3, sizeof(__m128),FFI_TYPE_FLOAT, NULL};
FFI_TYPEDEF(m128, __m128, FFI_TYPE_FLOAT);
FFI_TYPEDEF(m256, m256bit, FFI_TYPE_FLOAT);
FFI_TYPEDEF(m512, m512bit, FFI_TYPE_FLOAT);
FFI_TYPEDEF(m1024, m1024bit, FFI_TYPE_FLOAT);
...
static ffi_type *ffiTypeFor(const Type *Ty) {
  switch (Ty->getTypeID()) {
    ...
    case Type::VectorTyID:
      if (cast<VectorType>(Ty)->getElementType()->getTypeID() == Type::IntegerTyID) {
        switch (cast<VectorType>(Ty)->getNumElements()) {
          case 2:  return &ffi_type_f2;
          case 3:  return &ffi_type_f3;
          case 4:  return &ffi_type_m128;
          case 8:  return &ffi_type_m256;
          case 16:  return &ffi_type_m512;
        }
      } else if (cast<VectorType>(Ty)->getElementType()->getTypeID() == Type::FloatTyID)
      {
        switch (cast<VectorType>(Ty)->getNumElements()) {
          case 2:  return &ffi_type_f2;
          case 3:  return &ffi_type_f3;
      ...
      } else if (cast<VectorType>(Ty)->getElementType()->getTypeID() == Type::DoubleTyID)
      {
        switch (cast<VectorType>(Ty)->getNumElements()) {
          case 2:  return &ffi_type_m128;
          case 3:  return &ffi_type_d3;
          case 4:  return &ffi_type_m256;
          case 8:  return &ffi_type_m512;
          //case 16:  return &ffi_type_m1024; //Doesn't work yet.
        }
      }
    default: break;
  }
    ...
    case Type::VectorTyID: {
      if (cast<VectorType>(Ty)->getElementType()->getTypeID() == Type::IntegerTyID) {
            ...
            for (unsigned i = 0; i < cast<VectorType>(Ty)->getNumElements(); ++i) {
              I32Ptr[i] = (int32_t) AV.AggregateVal[i].IntVal.getZExtValue();
            }
            return ArgDataPtr;
      } else if (cast<VectorType>(Ty)->getElementType()->getTypeID() == Type::FloatTyID)
      {
            float *FloatPtr = (float *) ArgDataPtr;
            for (unsigned i = 0; i < cast<VectorType>(Ty)->getNumElements(); ++i) {
              FloatPtr[i] = AV.AggregateVal[i].FloatVal;
            }
            return ArgDataPtr;
      } else if (cast<VectorType>(Ty)->getElementType()->getTypeID() == Type::DoubleTyID)
      {
          double *DoublePtr = (double *) ArgDataPtr;
          for (unsigned i = 0; i < cast<VectorType>(Ty)->getNumElements(); ++i) {
            DoublePtr[i] = AV.AggregateVal[i].DoubleVal;
          return ArgDataPtr;
      }
    }
                           }
...
static bool ffiInvoke(RawFunc Fn, Function *F,
  ...
  if (ffi_prep_cif(&cif, FFI_DEFAULT_ABI, NumArgs, rtype, &args[0]) == FFI_OK) {
    ...
    switch (RetTy->getTypeID()) {
      ...
      case Type::VectorTyID:{
        for (unsigned i = 0; i < cast<VectorType>(RetTy)->getNumElements(); ++i) {
          GenericValue currRetVal;
          switch (cast<VectorType>(RetTy)->getElementType()->getTypeID()) {
      case Type::FloatTyID: currRetVal.FloatVal = ((float *) ret.data())[i]; break;
      case Type::DoubleTyID: currRetVal.DoubleVal = ((double *) ret.data())[i]; break;
      case Type::IntegerTyID: currRetVal.IntVal = APInt(32, ((int32_t *) ret.data())[i]); break;
          }
          Result.AggregateVal.push_back(currRetVal);
        }
        break;
                            }
      default: break;
...
#if defined(VALIDATION_ON)
  Validation::OCLBuiltins::FillOCLBuiltins(FuncNames);
#endif
}
