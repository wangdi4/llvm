...
#define MASK_VECTOR_NANS_T(X,Y, TZ, FLAG)                                   \
    ...
    for( uint32_t _i=0;_i<X.AggregateVal.size();_i++){                      \
        ...
        else Dest.AggregateVal[_i].IntVal = APInt(1,!FLAG);                 \
    }
...
static GenericValue executeSelectInst(GenericValue Src1, GenericValue Src2,
    ...
    if(Ty->isVectorTy())
    {
        ...
        for (size_t i = 0; i < Src1.AggregateVal.size(); ++i)
            Dest.AggregateVal[i] =
                (Src1.AggregateVal[i].IntVal == 0) ?
                Src3.AggregateVal[i] : Src2.AggregateVal[i];
    }
    else Dest = (Src1.IntVal == 0) ? Src3 : Src2;
    return Dest;
}
...
unsigned int nextHighestPowerOfTwo(unsigned int a)
{
    unsigned nextHighestPowerOf2 = a;
    nextHighestPowerOf2--;
    nextHighestPowerOf2 |= nextHighestPowerOf2 >> 1;
    nextHighestPowerOf2 |= nextHighestPowerOf2 >> 2;
    nextHighestPowerOf2 |= nextHighestPowerOf2 >> 4;
    nextHighestPowerOf2 |= nextHighestPowerOf2 >> 8;
    nextHighestPowerOf2 |= nextHighestPowerOf2 >> 16;
    nextHighestPowerOf2++;
    return nextHighestPowerOf2;
}
...
void Interpreter::visitShl(BinaryOperator &I) {
  ...
  if (Ty->isVectorTy()) {
  // vector
  // src1 and src2 must be the same size and integer type
      assert(Src1.AggregateVal.size() == Src2.AggregateVal.size());
      for( unsigned i=0;i<Src1.AggregateVal.size();i++) {
          GenericValue Result;
          if (Src2.AggregateVal[i].IntVal.getZExtValue() < Src1.AggregateVal[i].IntVal.getBitWidth())
              Result.IntVal = Src1.AggregateVal[i].IntVal.shl(Src2.AggregateVal[i].IntVal.getZExtValue());
          else
          {
              unsigned bitMask = nextHighestPowerOfTwo(Src1.AggregateVal[i].IntVal.getBitWidth()) - 1;
              Result.IntVal = Src1.AggregateVal[i].IntVal.shl(bitMask & Src2.AggregateVal[i].IntVal.getZExtValue());
          }
  ...
  // scalar
    ...
    {
        unsigned bitMask = nextHighestPowerOfTwo(Src1.IntVal.getBitWidth()) - 1;
        Dest.IntVal = Src1.IntVal.shl(bitMask & Src2.IntVal.getZExtValue());
    }
  }
...
void Interpreter::visitLShr(BinaryOperator &I) {
  ...
  // src1 and src2 must be the same size and integer type
      ...
      for( unsigned i=0;i<Src1.AggregateVal.size();i++) {
          GenericValue Result;
          if (Src2.AggregateVal[i].IntVal.getZExtValue() < Src1.AggregateVal[i].IntVal.getBitWidth())
              Result.IntVal = Src1.AggregateVal[i].IntVal.lshr(Src2.AggregateVal[i].IntVal.getZExtValue());
          else
          {
              unsigned bitMask = nextHighestPowerOfTwo(Src1.AggregateVal[i].IntVal.getBitWidth()) - 1;
              Result.IntVal = Src1.AggregateVal[i].IntVal.lshr(bitMask & Src2.AggregateVal[i].IntVal.getZExtValue());
          }
          Dest.AggregateVal.push_back(Result);
      }
  } else {
  // scalar
      ...
      {
          unsigned bitMask = nextHighestPowerOfTwo(Src1.IntVal.getBitWidth()) - 1;
          Dest.IntVal = Src1.IntVal.lshr(bitMask & Src2.IntVal.getZExtValue());
      }
  }
...
void Interpreter::visitAShr(BinaryOperator &I) {
  ...
  // src1 and src2 must be the same size and integer type
      ...
      for( unsigned i=0;i<Src1.AggregateVal.size();i++) {
          GenericValue Result;
          if (Src2.AggregateVal[i].IntVal.getZExtValue() < Src1.AggregateVal[i].IntVal.getBitWidth())
              Result.IntVal = Src1.AggregateVal[i].IntVal.ashr(Src2.AggregateVal[i].IntVal.getZExtValue());
          else
          {
              unsigned bitMask = nextHighestPowerOfTwo(Src1.AggregateVal[i].IntVal.getBitWidth()) - 1;
              Result.IntVal = Src1.AggregateVal[i].IntVal.ashr(bitMask & Src2.AggregateVal[i].IntVal.getZExtValue());
          }
          Dest.AggregateVal.push_back(Result);
      }
  } else {
  // scalar
      ...
      {
          unsigned bitMask = nextHighestPowerOfTwo(Src1.IntVal.getBitWidth()) - 1;
          Dest.IntVal = Src1.IntVal.ashr(bitMask & Src2.IntVal.getZExtValue());
      }
  }
...
GenericValue Interpreter::executeTruncInst(Value *SrcVal, const Type *DstTy,
  ...
  if (SrcTy->getTypeID() == Type::VectorTyID) {
      ...
      // the sizes of src and dst vectors must be equal. llvm-asm should check it
    ...
    if (DstTy->getTypeID() == Type::VectorTyID) {
      ...
      if(SrcElemTy->isFloatTy()) {
          for (unsigned i = 0; i < SrcNum; i++) {
             TempSrc.AggregateVal[i].IntVal.zext(SrcBitSize);
             TempSrc.AggregateVal[i].IntVal.floatToBits(SrcVec.AggregateVal[i].FloatVal);
          }
      } else if (SrcElemTy->isDoubleTy()) {
          for (unsigned i = 0; i < SrcNum; i++) {
              TempSrc.AggregateVal[i].IntVal.zext(SrcBitSize);
              TempSrc.AggregateVal[i].IntVal.doubleToBits(SrcVec.AggregateVal[i].DoubleVal);
          }
      ...
      // Example: bitcast <2 x i64> <i64 0, i64 1> to <4 x i32>
           ...
           for (unsigned i = 0; i < SrcNum; i++) {
               ...
               for (unsigned j = 0; j < Ratio; j++) {
                   GenericValue Elt;
                   Elt.IntVal.zext(SrcBitSize);
                   Elt.IntVal = TempSrc.AggregateVal[i].IntVal;
                   Elt.IntVal = Elt.IntVal.lshr(ShiftAmt);
                   if(DstBitSize < SrcBitSize) // it could be DstBitSize == SrcBitSize, so check it
                       Elt.IntVal.trunc(DstBitSize);
      ...
      // conver result from integer to specified type
      if (DstTy->getTypeID() == Type::VectorTyID) {
...
void Interpreter::visitExtractElementInst(ExtractElementInst &I) {
  ...
  if(Src1.AggregateVal.size() > indx) {
      switch (Ty->getTypeID()) {
          ...
          break;
          default:
              dbgs() << "Unhandled dest type for extractelement instruction: " << *Ty << "\n";
              llvm_unreachable(0);
      }
  } else {
...
void Interpreter::visitInsertElementInst(InsertElementInst &I) {
  ...
  if(Ty->getTypeID() != Type::VectorTyID) {
    dbgs() << "Unhandled dest type for insertelement instruction: " << *Ty << "\n";
    llvm_unreachable(0);
  }
  ...
  const Type *TyContained = Ty->getContainedType(0);
  ...
  if(Src1.AggregateVal.size() > indx) {
      switch (TyContained->getTypeID()) {
          ...
          break;
          default:
              dbgs() << "Unhandled dest type for insertelement instruction: " << *TyContained << "\n";
              llvm_unreachable(0);
      }
  } else {
      dbgs() << "Invalid index in insertelement instruction\n";
  }
  SetValue(&I, Dest, SF);
}
...
void Interpreter::visitShuffleVectorInst(ShuffleVectorInst &I){
  ...
  const Type *Ty = I.getType();
  if(Ty->getTypeID() != Type::VectorTyID) {
    dbgs() << "Unhandled dest type for shufflevector instruction: " << *Ty << "\n";
    llvm_unreachable(0);
  }
...
// I suppose it's no need to check src1 and src2 types, because compiled bytecode
// can't contain different types for src1 and src2 for shufflevector instruction
        ...
        for( unsigned i=0; i<src3Size; i++) {
            ...
            if(j < src1Size) {
                ...
                if(j < src1Size + src2Size) {
                    ...
                    // the selector may not be grater than sum of lengths of first and second operands and
                    // llasm should not allow situation like
                    // %tmp = shufflevector <2 x i32> <i32 3, i32 4>, <2 x i32> undef, <2 x i32> < i32 0, i32 5 >
                    // where i32 5 is invalid, but let it be additional check here
                    dbgs() << "Invalid mask in shufflevector instruction\n";
                    llvm_unreachable(0);
                }
            }
        }
...
void Interpreter::visitExtractValueInst(ExtractValueInst &I) {
    ...
    const Type *IndexedType = ExtractValueInst::getIndexedType(Agg->getType(), I.idx_begin(), I.idx_end());
    switch (IndexedType->getTypeID()) {
        case Type::IntegerTyID:
            Dest.IntVal = TmpVal.IntVal;
        break;
        case Type::FloatTyID:
            Dest.FloatVal = TmpVal.FloatVal;
        break;
        case Type::DoubleTyID:
            Dest.DoubleVal = TmpVal.DoubleVal;
        break;
        case Type::ArrayTyID:
        ...
        break;
        default:
            dbgs() << "Unhandled dest type for extractelement instruction: " << *IndexedType << "\n";
            llvm_unreachable(0);
    }
