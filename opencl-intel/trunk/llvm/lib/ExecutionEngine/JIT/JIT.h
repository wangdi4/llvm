...
private:
  ...
  /// Jit information for generated functions
  std::map<const Function*, std::pair<size_t, uint64_t> >	JitInfoTable;
...
public:
  ...
  std::map<const Function*, std::pair<size_t, uint64_t> >& getJitInfoTable(const MutexGuard &L) {
	  return JitInfoTable;
  }
};
...
public:
  ...
  void	addJitInfo(const Function* Fn, size_t FnSize, uint64_t FnStackSize);
