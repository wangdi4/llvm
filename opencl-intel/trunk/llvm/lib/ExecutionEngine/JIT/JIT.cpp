...
/// Add JIT information about the function
void JIT::addJitInfo(const Function* Fn, size_t FnSize, uint64_t FnStackSize)
{
	MutexGuard locked(lock);
	jitstate->getJitInfoTable(locked)[Fn]=std::pair<size_t, uint64_t>(FnSize, FnStackSize);
}

size_t JIT::getJitFunctionSize(const Function* Fn)
{
	MutexGuard locked(lock);
	std::map<const Function*, std::pair<size_t, uint64_t> >::const_iterator it;

	it = jitstate->getJitInfoTable(locked).find(Fn);
	if ( it != jitstate->getJitInfoTable(locked).end())
	{
		return it->second.first;
	}
	return 0;
}
uint64_t JIT::getJitFunctionStackSize(const Function* Fn)
{
	MutexGuard locked(lock);
	std::map<const Function*, std::pair<size_t, uint64_t> >::const_iterator it;
