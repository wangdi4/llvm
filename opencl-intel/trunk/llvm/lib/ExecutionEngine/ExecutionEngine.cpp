...
//  return new char[GVSize];
  ...
  size_t TA = getTargetData()->getPreferredAlignment(GV);
  size_t GVA = GV->getAlignment();
  size_t A = std::max(TA, GVA);
  if (A <= 4) {
	  return new char[GVSize];
  } else {
	  // Allocate GVSize+A bytes of memory, then use an aligned pointer within that
	  // space.
	  //TODO: Solve this memory leak!!! (Guy)
	  char *Ptr = new char[GVSize+A];
...
GenericValue ExecutionEngine::getConstantValue(const Constant *C) {
    ...
    case Type::VectorTyID:
      {
        ...
        if (VTy->getElementType()->isIntegerTy())
        {
          ...
          {
            Result.AggregateVal[i].IntVal = APInt(VTy->getElementType()->getPrimitiveSizeInBits(), 0);
          }
        }
      }
      break;
    case Type::ArrayTyID:
      {
        ...
        Result.AggregateVal.resize(elemNum);
        // Go over all a elements and get 'undef' constant value for each of them independently.
        for (unsigned i = 0; i < elemNum; ++i)
        {
          Result.AggregateVal[i] = getConstantValue(UndefValue::get(ATy->getElementType()));
        }
        break;
      }
    case Type::StructTyID:
      {
        // if the whole structure is 'undef'
        const StructType* STy = dyn_cast<StructType>(C->getType());
        unsigned elemNum = STy->getNumElements();
        Result.AggregateVal.resize(elemNum);
        // Go through all structure elements and get 'undef' constant value for them recursively.
        for (unsigned i = 0; i < elemNum; ++i)
        {
          Result.AggregateVal[i] = getConstantValue(UndefValue::get(STy->getElementType(i)));
        }
        break;
      }
    default:
  ...
  case Type::VectorTyID:
    {
      // WARNING! Doesn't support vector with only one element (e.g. <1 x i32>).
      ...
      {
        if (dyn_cast<ConstantAggregateZero>(C))
        {
          // fill vector with zeros
          GenericValue floatZero;
          floatZero.FloatVal = 0.f;
          std::fill(Result.AggregateVal.begin(), Result.AggregateVal.end(), floatZero);
          break;
        }
        for (unsigned i = 0; i < elemNum; ++i)
        {
          if (!isa<UndefValue>(CV->getOperand(i))) {
            Result.AggregateVal[i].FloatVal = 
              cast<ConstantFP>(CV->getOperand(i))->getValueAPF().convertToFloat();
          }
        }
        break;
      }
      // Check if vector holds doubles.
      if (VTy->getElementType()->isDoubleTy())
      {
        if (dyn_cast<ConstantAggregateZero>(C))
        {
          // fill vector with zeros
          ...
          std::fill(Result.AggregateVal.begin(), Result.AggregateVal.end(), doubleZero);
          break;
        }
        for (unsigned i = 0; i < elemNum; ++i)
        {
          if (!isa<UndefValue>(CV->getOperand(i))) {
            Result.AggregateVal[i].DoubleVal = 
              cast<ConstantFP>(CV->getOperand(i))->getValueAPF().convertToDouble();
          }
        }
        break;
      }
      ...
      if (VTy->getElementType()->isIntegerTy())
      {
        if (dyn_cast<ConstantAggregateZero>(C))
        {
          ...
          intZero.IntVal = APInt(VTy->getScalarSizeInBits(), 0ull);
          std::fill(Result.AggregateVal.begin(), Result.AggregateVal.end(), intZero);
          break;
        }
        for (unsigned i = 0; i < elemNum; ++i)
        {
          if (!isa<UndefValue>(CV->getOperand(i))) {
            Result.AggregateVal[i].IntVal = cast<ConstantInt>(CV->getOperand(i))->getValue();
          } else {
            Result.AggregateVal[i].IntVal = APInt(CV->getOperand(i)->getType()->getPrimitiveSizeInBits(), 0);
          }
        }
        break;
      }
  ...
  case Type::StructTyID:
    {
      ...
      {
        // if we have 'zero' constant then we have to create 'zero' constant of element type and run getConstantValue recursively
        const Constant *opConst = dyn_cast<ConstantAggregateZero>(C) != 0 ? Constant::getNullValue(CSTy->getElementType(i)) : CS->getOperand(i);
  ...
  case Type::ArrayTyID:
    {
      ...
      Result.AggregateVal.resize(CATy->getNumElements());
      // if we have 'zero' constant then we have to create 'zero' constant of element type and run getConstantValue recursively
      for (unsigned int i = 0; i < CATy->getNumElements(); ++i)
      {
        const Constant *opConst = dyn_cast<ConstantAggregateZero>(C) != 0 ? Constant::getNullValue(CATy->getElementType()) : CA->getOperand(i);
        Result.AggregateVal[i] = getConstantValue(opConst);
      }
      break;
    }
...
void ExecutionEngine::StoreValueToMemory(const GenericValue &Val,
      ...
      for (unsigned i = 0; i < Val.AggregateVal.size(); ++i)
      {
        ...
        if (cast<VectorType>(Ty)->getElementType()->isIntegerTy())
        {
          ...
          StoreIntToMemory(Val.AggregateVal[i].IntVal, 
            (uint8_t*)Ptr + (Val.AggregateVal[i].IntVal.getBitWidth()+7)/8*i,
  ...
  case Type::StructTyID: {
      ...
      const StructLayout *SLO = getTargetData()->getStructLayout(STy);

      for (unsigned i = 0; i < Val.AggregateVal.size(); ++i)
      {
        unsigned int offset = SLO->getElementOffset(i); // calculate offset for the current element
        StoreValueToMemory(Val.AggregateVal[i], (GenericValue*)((uint8_t*)Ptr+offset), STy->getElementType(i));
      }
      break;
    }
  case Type::ArrayTyID: {
      ...
      for (unsigned i = 0; i < Val.AggregateVal.size(); ++i)
      {
        unsigned int offset = getTargetData()->getTypeStoreSize(ATy->getElementType())*i; // calculate offset for the current element
        StoreValueToMemory(Val.AggregateVal[i], (GenericValue*)((uint8_t*)Ptr+offset), ATy->getElementType());
      }
      break;
    }
  default:
...
void ExecutionEngine::LoadValueFromMemory(GenericValue &Result,
  ...
  case Type::VectorTyID: {
    if (cast<VectorType>(Ty)->getElementType()->isFloatTy())
    {
      Result.AggregateVal.resize(cast<VectorType>(Ty)->getNumElements());
      for (unsigned i = 0; i < cast<VectorType>(Ty)->getNumElements(); ++i)
      {
    ...
    {
      GenericValue intZero;
      intZero.IntVal = APInt(cast<IntegerType>(cast<VectorType>(Ty)->getElementType())->getBitWidth(), 0);
      Result.AggregateVal.resize(cast<VectorType>(Ty)->getNumElements(), intZero);
      for (unsigned i = 0; i < cast<VectorType>(Ty)->getNumElements(); ++i)
      {
        // getBitWidth() returns number of bits for integer value, but memory
        // is allocated in bytes i.e. 8-bit pieces. So if integer type is i5
        ...
        LoadIntFromMemory(Result.AggregateVal[i].IntVal, 
          (uint8_t*)Ptr+((cast<IntegerType>(cast<VectorType>(Ty)->getElementType())->getBitWidth()+7)/8)*i, 
          (cast<IntegerType>(cast<VectorType>(Ty)->getElementType())->getBitWidth()+7)/8);
      }
    }
    break;
  }
  case Type::StructTyID: {
      ...
      {
        unsigned int offset = SLO->getElementOffset(i); // calculate offset for the current element
        LoadValueFromMemory(Result.AggregateVal[i], (GenericValue*)((uint8_t*)Ptr+offset), STy->getElementType(i));
      }
      break;
    }
  case Type::ArrayTyID: {
      ...
      {
        unsigned int offset = getTargetData()->getTypeStoreSize(ATy->getElementType())*i; // calculate offset for the current element
        LoadValueFromMemory(Result.AggregateVal[i], (GenericValue*)((uint8_t*)Ptr+offset), ATy->getElementType());
      }
      break;
    }
  default:
