...
Value *InstCombiner::EvaluateInDifferentType(Value *V, const Type *Ty, 
  ...
  // Make sure that we are comparing scalars to scalars
  assert(V->getType()->isVectorTy() == Ty->isVectorTy() && "Wrong type to evaluate");
  ...
  case Instruction::ShuffleVector: {
    Value *In0 = EvaluateInDifferentType(I->getOperand(0), Ty, isSigned);
    Value *In1 = EvaluateInDifferentType(I->getOperand(1), Ty, isSigned);
    Res = new ShuffleVectorInst(In0, In1, I->getOperand(2));
    break;
  }
  case Instruction::ExtractElement:{
    assert(!Ty->isVectorTy() && "Wrong incoming scalar type");
    const VectorType* VTy = cast<VectorType>(I->getOperand(0)->getType());
    const VectorType* NewVTy = VectorType::get(Ty, VTy->getNumElements());
    Value *In0 = EvaluateInDifferentType(I->getOperand(0), NewVTy, isSigned);    
    Res = ExtractElementInst::Create(In0, I->getOperand(1));
    break;
  }
  ...
  case Instruction::InsertElement:{
    ...
    Value *In0 = EvaluateInDifferentType(I->getOperand(0), Ty, isSigned);
    Value *In1 = EvaluateInDifferentType(I->getOperand(1),ElTy, isSigned);
    Res = InsertElementInst::Create(In0, In1, I->getOperand(2));
    break;
  }
...
static bool CanEvaluateTruncated(Value *V, const Type *Ty) {
  ...
  case Instruction::ShuffleVector: {
    ...
    return CanEvaluateTruncated(SVI->getOperand(0), Ty) &&
           CanEvaluateTruncated(SVI->getOperand(1), Ty);
  }
  case Instruction::ExtractElement: {
    assert(!Ty->isVectorTy() && "Wrong incoming scalar type");
    const VectorType* VTy = cast<VectorType>(I->getOperand(0)->getType());
    ...
    return CanEvaluateTruncated(EEI->getOperand(0), NewVTy);
  }
  case Instruction::InsertElement: {
    InsertElementInst *IEI = cast<InsertElementInst>(I);
    assert(Ty->isVectorTy() && "Wrong incoming scalar type");
    const VectorType* VTy = cast<VectorType>(Ty);    
    return CanEvaluateTruncated(IEI->getOperand(0), Ty) &&
      CanEvaluateTruncated(IEI->getOperand(1), VTy->getElementType());
  }
...
static bool CanEvaluateZExtd(Value *V, const Type *Ty, unsigned &BitsToClear) {
  ...
  case Instruction::ShuffleVector: {
    ShuffleVectorInst *SVI = cast<ShuffleVectorInst>(I);
    if (!CanEvaluateZExtd(SVI->getOperand(0), Ty, Tmp) ||
        !CanEvaluateZExtd(SVI->getOperand(1), Ty, BitsToClear) ||
      Tmp != BitsToClear) return false;
    return true;
  }
  case Instruction::ExtractElement: {
    ExtractElementInst *EEI = cast<ExtractElementInst>(I);
    const VectorType* VTy = cast<VectorType>(EEI->getOperand(0)->getType());
    const VectorType* NewVTy = VectorType::get(Ty, VTy->getNumElements());
    return CanEvaluateZExtd(EEI->getOperand(0), NewVTy, BitsToClear);
  }
  case Instruction::InsertElement: {
    InsertElementInst *IEI = cast<InsertElementInst>(I);
    const VectorType* VTy = cast<VectorType>(Ty);
    if (!CanEvaluateZExtd(IEI->getOperand(0), Ty, Tmp) ||
        !CanEvaluateZExtd(IEI->getOperand(1), VTy->getElementType(), BitsToClear) ||
        Tmp != BitsToClear) return false;
    return true;
  }
  case Instruction::LShr:
...
static bool CanEvaluateSExtd(Value *V, const Type *Ty) {
  ...
  case Instruction::ShuffleVector: {
    ShuffleVectorInst *SVI = cast<ShuffleVectorInst>(I);
    return CanEvaluateSExtd(SVI->getOperand(0), Ty) &&
           CanEvaluateSExtd(SVI->getOperand(1), Ty);
  }
  case Instruction::ExtractElement: {
    ExtractElementInst *EEI = cast<ExtractElementInst>(I);
    const VectorType* VTy = cast<VectorType>(EEI->getOperand(0)->getType());
    const VectorType* NewVTy = VectorType::get(Ty, VTy->getNumElements());
    return CanEvaluateSExtd(EEI->getOperand(0), NewVTy);
  }
  case Instruction::InsertElement: {
    InsertElementInst *IEI = cast<InsertElementInst>(I);
    assert(Ty->isVectorTy() && "Wrong incoming scalar type");
