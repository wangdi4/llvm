...
Instruction *InstCombiner::visitAnd(BinaryOperator &I) {
  ...
  bool Swapped = false;
  bool SwappedOp0 = false;
  bool SwappedOp1 = false;
...

  {
    ...
    if (Op0->hasOneUse() &&
      ...
      if (A == Op1) {                                // (A^B)&A -> A&(A^B)
        ...
        Swapped ^= true;
        std::swap(Op0, Op1);
        std::swap(SwappedOp0, SwappedOp1);
      }
    }
  ...
  // If we swapped the operands of this instruction in an attempt to find
  // a pattern to optimize and found no pattern, swap it back. If we do not
  // swap it back, we may enter an endless loop in case some other
  // optimization fixes the swap.
  if (SwappedOp0) cast<BinaryOperator>(I.getOperand(0))->swapOperands();
  if (SwappedOp1) cast<BinaryOperator>(I.getOperand(1))->swapOperands();
  if (Swapped) I.swapOperands();
