...
Instruction *InstCombiner::visitExtractElementInst(ExtractElementInst &EI) {
  ...
  if (Instruction *I = dyn_cast<Instruction>(EI.getOperand(0))) {
    ...
    if (BinaryOperator *BO = dyn_cast<BinaryOperator>(I)) {
      ...
      if (CI->hasOneUse() && EI.hasOneUse() && (CI->getOpcode() != Instruction::BitCast)) {
        Value *EE = Builder->CreateExtractElement(CI->getOperand(0),EI.getIndexOperand());
        return CastInst::Create(CI->getOpcode(), cast<Instruction>(EE), EI.getType());
      }
    }
  }
  return 0;
