...
public:
  ...
  Instruction *ReplaceInstUsesWith(Instruction &I, Value *V) {
    ...
    // martiom: There are cases when instruction is vector type and constant is scalar
    // in this case we convert constant to vector type too.
	Constant* cnst = dyn_cast<Constant>(V);
    if (I.getType()->isVectorTy() && cnst && !cnst->getType()->isVectorTy()) {
        const VectorType *vt = dyn_cast<VectorType>(I.getType());
        std::vector<Constant*> vecElems;
        for (unsigned int i = 0; i < vt->getNumElements(); i++)
            vecElems.push_back(cnst);
        V = ConstantVector::get(vecElems);
    }
