...
Instruction *InstCombiner::visitGetElementPtrInst(GetElementPtrInst &GEP) {
  ...
  ////(gep (extract (vcast index)))  -> (gep (cast( extract index)))
  for (unsigned idx = 0; idx < GEP.getNumIndices(); ++idx) {
    Value* Index = GEP.getOperand(idx + 1);
    // try to eliminate vector casts
    
    if (ExtractElementInst* EEI = dyn_cast<ExtractElementInst>(Index)) {
      CastInst* CI = dyn_cast<CastInst>(EEI->getOperand(0));
      if (CI && CI->getOpcode() != Instruction::BitCast) {
        Value* WideEEI = Builder->CreateExtractElement(CI->getOperand(0),EEI->getIndexOperand());
        Value* NarrowCI = Builder->CreateCast(CI->getOpcode(), WideEEI, EEI->getType());
        assert(NarrowCI->getType() == GEP.getOperand(idx + 1)->getType());
        GEP.setOperand(idx + 1, NarrowCI);
        return &GEP;
      }
    }
  }
