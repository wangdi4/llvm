...
void Verifier::visitFunction(Function &F) {
  ...
  case CallingConv::X86_Svml:
  case CallingConv::X86_Svml_P2_F8_F8_F8:
...
void Verifier::VerifyCallSite(CallSite CS) {
  ...
  for (unsigned i = 0, e = FTy->getNumParams(); i != e; ++i)
    Assert3(CS.getArgument(i)->getType() == FTy->getParamType(i) ||
            (dyn_cast<PointerType>(CS.getArgument(i)->getType()) && dyn_cast<PointerType>(FTy->getParamType(i)) &&
            (dyn_cast<PointerType>(CS.getArgument(i)->getType())->getElementType() == dyn_cast<PointerType>(FTy->getParamType(i))->getElementType())),
