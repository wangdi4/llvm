...
void CallInst::init(Value *Func, Value* const *Params, unsigned NumParams) {
  ...
  for (unsigned i = 0; i != NumParams; ++i) {
    assert((i >= FTy->getNumParams() || 
            ...
            (dyn_cast<PointerType>(Params[i]->getType()) && dyn_cast<PointerType>(FTy->getParamType(i)) &&
            (dyn_cast<PointerType>(Params[i]->getType())->getElementType() == dyn_cast<PointerType>(FTy->getParamType(i))->getElementType()))
            ) &&
