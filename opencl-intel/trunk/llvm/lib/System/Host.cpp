...
// Parameters: 
//    valueEAX [in] - value of EAX register  
//    valueECX [in] - value of ECX register 
// detection of AVX2 requires EAX and ECX values to be set before calling cpuid.
// detection of SSE/AVX1/FMA requires only EAX
//    rEAX [out] - value of EAX after the call
//    rEBX [out] - value of EBX after the call
//    rECX [out] - value of ECX after the call
//    rEDX [out] - value of EDX after the call
static bool GetX86CpuIDAndInfo(unsigned valueEAX, unsigned valueECX, unsigned *rEAX,
    ...
    int registers[4];
    __cpuidex(registers, valueEAX, valueECX);
    *rEAX = registers[0];
    *rEBX = registers[1];
    *rECX = registers[2];
    *rEDX = registers[3];
...
std::string sys::getHostCPUName() {
  ...
  if (GetX86CpuIDAndInfo(0x1, 0, &EAX, &EBX, &ECX, &EDX))
    return "generic";
  unsigned Family = 0;
  unsigned Model  = 0;
  DetectX86FamilyModel(EAX, Family, Model);

  GetX86CpuIDAndInfo(0x80000001, 0, &EAX, &EBX, &ECX, &EDX);
  ...
  GetX86CpuIDAndInfo(0, 0, &EAX, text.u+0, text.u+2, text.u+1);
  if (memcmp(text.c, "GenuineIntel", 12) == 0) {
    ...
    case 6:
      ...
      case 0x2a:
        {
          unsigned EAX = 0, EBX = 0, ECX = 0, EDX = 0;
          GetX86CpuIDAndInfo(7, 0, &EAX, &EBX, &ECX, &EDX);
          if ((EBX & 0x20) == 0x20)
            return "haswell";
        }
        return "sandybridge";
