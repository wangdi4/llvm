...
typedef Validation::auto_ptr_ex<
    ICLDevBackendCompilationService,
    Validation::ReleaseDP<ICLDevBackendCompilationService>>
    ICLDevBackendCompileServicePtr;
typedef Validation::auto_ptr_ex<
    ICLDevBackendExecutionService,
    Validation::ReleaseDP<ICLDevBackendExecutionService>>
    ICLDevBackendExecutionServicePtr;
typedef Validation::auto_ptr_ex<
    ICLDevBackendSerializationService,
    Validation::ReleaseDP<ICLDevBackendSerializationService>>
    ICLDevBackendSerializationServicePtr;

/// @brief the plugin exported function
typedef bool (*PLUGIN_EXPORT_F)();

/** @brief BackendWrapper: This class hides the internals of loading and calling
 * OCL CPU Backend. +base class for FactoryMethds, CompilationsService & Plugins
 * test cases classes
 */
...
public:
  ...
  /// @brief Init(): creating instance, loading the dll and (decided by caller)
  ...
  void CreateProgramContainer(const std::string &programFile,
                              std::vector<char> &buffer);

protected:
  /** @brief SetUp(), TearDown(): called by the gtest, shared for FactoryMethods
   * tests and CompilationService tests, responsible for initiating the
   * BackendWrapper class and terminating it overload them if necessary (in
   * PluginTest for example)
   */
  virtual void SetUp();
