...
TEST_F(DISABLED_BackEndTests_KernelSubGroupInfo, SubGroupInfoSuccess)
{
    ...
    cl_dev_err_code ret = funcGetFactory->GetCompilationService(nullptr, spCompileService.getOutPtr());
    ...
    const ICLDevBackendKernelProporties* pKernelProps = pIKernel->GetKernelProporties();
