...
[ "KernelCompile",     0, 1, # OpenCL-only
  ["ocl_kernel",                  \&findmin,     "BuildProgram"]
],
[ "QueueDelay",        0, 1, # OpenCL-only
...
[ "Stencil2D",         1, 1,
  ["stencil",                     \&findmin,     "SP_Sten2D"],
  ["stencil_dp",                  \&findmin,     "DP_Sten2D"]
],
[ "Triad",             1, 1,
...
foreach my $bench (@$benchmarks)
{
    ...
    my $command = buildCommand($program, $logbase);
...
sub usage() {
   ...
   print "-d        - Comma-separates list of device numbers on each node\n";
