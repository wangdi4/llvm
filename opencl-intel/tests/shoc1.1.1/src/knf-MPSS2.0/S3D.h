...
__declspec(target(mic)) T1 DIV(T1 x, T2 y)
{
   return x * (1.0f / y);
}
#else
...
// POW
template<class T>
__declspec(target(mic)) T POW (T in, T in2);

template<>
__declspec(target(mic)) double POW<double>(double in, double in2)
{
    return pow(in, in2);
}

template<>
__declspec(target(mic)) float POW<float>(float in, float in2)
{
    return powf(in, in2);
}
// EXP
...
template<>
__declspec(target(mic)) double EXP<double>(double in)
{
    return exp(in);
}

template<>
__declspec(target(mic)) float EXP<float>(float in)
{
    return expf(in);
}
...
// EXP10
template<class T>
__declspec(target(mic)) T EXP10(T in);

template<>
__declspec(target(mic)) double EXP10<double>(double in)
{
    return exp10(in);
}

template<>
__declspec(target(mic)) float EXP10<float>(float in)
{
    return exp10f(in);
}
...
// EXP2
template<class T>
__declspec(target(mic)) T EXP2(T in);

template<>
__declspec(target(mic)) double EXP2<double>(double in)
{
    return exp2(in);
}

template<>
__declspec(target(mic)) float EXP2<float>(float in)
{
    return exp2f(in);
}
...
// FMAX
template<class T>
__declspec(target(mic)) T MAX(T in, T in2);

template<>
__declspec(target(mic)) double MAX<double>(double in, double in2)
{
    return fmax(in, in2);
}

template<>
__declspec(target(mic)) float MAX<float>(float in, float in2)
{
    return fmaxf(in, in2);
}
...
// FMIN
template<class T>
__declspec(target(mic)) T MIN(T in, T in2);

template<>
__declspec(target(mic)) double MIN<double>(double in, double in2)
{
    return fmin(in, in2);
}

template<>
__declspec(target(mic)) float MIN<float>(float in, float in2)
{
    return fminf(in, in2);
}
...
// LOG
template<class T>
__declspec(target(mic)) T LOG(T in);

template<>
__declspec(target(mic)) double LOG<double>(double in)
{
    return log(in);
}

template<>
__declspec(target(mic)) float LOG<float>(float in)
{
    return logf(in);
}
...
// LOG10
template<class T>
__declspec(target(mic)) T LOG10(T in);

template<>
__declspec(target(mic)) double LOG10<double>(double in)
{
    return log10(in);
}

template<>
__declspec(target(mic)) float LOG10<float>(float in)
{
    return log10f(in);
}
