...
#include "CommonKNFStencilFactory.cpp"
...
template<class T>
void
KNFValidate(const Matrix2D<T>& s, const Matrix2D<T>& t,double valErrThreshold,unsigned int nValErrsToPrint)
{
...
DoTest( const char* timerDesc, ResultDatabase& resultDB, OptionParser& opts )
{
    ...
    //try
    {
        ...
        testStencilFactory = new KNFStencilFactory<T>;
        ...
        {
#endif // defined(PARALLEL)
        std::cout << "\nPerforming stencil operation on host for later comparison with KNF output\n"
        ...
        // compute the result on the KNF device
...
#endif // defined(PARALLEL)
	   KNFValidate(exp,data,valErrThreshold,nValErrsToPrint);
