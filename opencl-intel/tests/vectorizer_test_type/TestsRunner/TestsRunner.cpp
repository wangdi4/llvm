...
void TestsRunner::runTestsInDir(const string& dirName, TestResults& results) {
	...
	bool cantFindDir = (handleFind = FindFirstFile((LPCSTR)dirName.c_str(),
...
#ifdef WINDOWS
	bool canFindTest = (handleFind = FindFirstFile((LPCSTR)string(dirName + "/*").c_str(),
		&FindFileData)) != INVALID_HANDLE_VALUE;
#else
...
bool TestsRunner::isInTestsToRun(string testName, string path) {
	...
	for (it = testsToRun.begin(); it != testsToRun.end(); it++) {
		...
		if ((pathToRun == path) || (pathToRun + "/" == path.substr(0,
				pathToRun.size() + 1)) || (pathToRun == path + "/" + testName)) {
			return true;
		}
	}
