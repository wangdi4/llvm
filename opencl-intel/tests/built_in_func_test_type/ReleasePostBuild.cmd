...
copy /y %PLATFORM_NAME%\%CONFIG_NAME%\%TARGET_NAME%.exe %MTV_LOCAL_BIN_DIR%.opt >> %PLATFORM_NAME%\%CONFIG_NAME%\customBuild.log 2>&1

REM -------- copy .pdb file ------------
copy /y %PLATFORM_NAME%\%CONFIG_NAME%\%TARGET_NAME%.pdb %MTV_LOCAL_BIN_DIR% >> %PLATFORM_NAME%\%CONFIG_NAME%\customBuild.log 2>&1
copy /y %PLATFORM_NAME%\%CONFIG_NAME%\%TARGET_NAME%.pdb %MTV_LOCAL_BIN_DIR%.opt >> %PLATFORM_NAME%\%CONFIG_NAME%\customBuild.log 2>&1
...
copy /b /y %TARGET_NAME%.cl %MTV_LOCAL_BIN_DIR%.opt >> %PLATFORM_NAME%\%CONFIG_NAME%\customBuild.log 2>&1
