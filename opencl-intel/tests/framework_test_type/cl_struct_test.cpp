...
bool clStructTest()
{
    ...
    \
    __kernel void test(TEST_STRUCT src1, TEST_UNION src2, __global TEST_STRUCT* dst)\
    {\
	...
	{
        if( (dst1[i].a != src1.a - 1) || (dst1[i].b != src1.b - 1) || (dst1[i].c != (src1.c - 1 + src2.i)) || (dst1[i].d != src1.d - 1) )
        {
        ...
        if( (dst2[i].a != src1.a) || (dst2[i].b != src1.b) || (dst2[i].c != (src1.c + src2.i)) || (dst2[i].d != src1.d) )
        {
            printf("Result did not validate:\n\
                   \\tdst[%d].a = %c\n\
                   \tdst[%d].b = %d\n\
