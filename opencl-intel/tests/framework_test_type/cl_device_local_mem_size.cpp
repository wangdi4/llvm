...
bool cl_device_local_mem_size_test() {
  ...
  EXIT_IF_FAILED(
      CheckCondition("trySetLocalMemSize", expectedLocalMemSize != 0));

  return cl_device_local_mem_size_test_body(expectedLocalMemSize,
                                            programSources);
}
#ifndef _WIN32
...
bool cl_device_local_mem_size_test_body(cl_ulong expectedLocalMemSize,
  ...
  iRet = clGetDeviceInfo(device, CL_DEVICE_LOCAL_MEM_SIZE, sizeof(cl_ulong),
                         &localMemSize, nullptr);
  EXIT_IF_FAILED(Check("clGetDeviceInfo", CL_SUCCESS, iRet));
  EXIT_IF_FAILED(
      CheckInt("CL_DEVICE_LOCAL_MEM_SIZE", expectedLocalMemSize, localMemSize));
  ...
  EXIT_IF_FAILED(BuildProgramSynch(context, 1, (const char **)&ps, nullptr,
                                   options.c_str(), &program));
  ...
  iRet = clEnqueueReadBuffer(queue, buffer, CL_TRUE, 0,
