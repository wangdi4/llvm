...
bool run_kernel(cl_context& context,cl_device_id& device,cl_command_queue& cmd_queue, const char* prog, int *out){
	cl_int err;
	cl_kernel kernel;
	cl_program program;
	bool res;
...
bool fission_commandQ_test(){
	...
	bResult = run_kernel(context, out_devices[0], cmd_queue, ocl_test_program, &p);
	printf("\n---------------------------------------\n");
