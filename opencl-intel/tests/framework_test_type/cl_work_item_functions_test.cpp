...
//| -------
//|
//| Test all the work items functions (get_global_id\get_local_id ..) in OCL 6.11.1
//| in both cases in-bound and out-of-bound dimindx, in order to get full coverage
//| for all the cases there's a need for constant and var versions of dimindx because
//| in constant version the in-bound check will be done in compile time (offline) and
//| in var version the inbound check will be done in kernel execution (online).
//|
//| Method
//| ------
//|
...
//| 4. Enqueue execution of the selected kernel that tests the WI function (in all versions)
//| 5. Enqueue a read from the output buffer
//| 6. Compare if the output the same as the expected output
//|
//| Pass criteria
//| -------------
//|
...
bool clWorkItemFunctionsTest()
{
	...
	"{\n"\
	"pOutputs[get_global_id(0) + 2 * get_global_size(0) ] = get_global_size(pOutputs[get_global_id(0) + 0 * get_global_size(0) ]);\n"\
	"pOutputs[get_global_id(0) + 3 * get_global_size(0) ] = get_global_size(pOutputs[get_global_id(0) + 1 * get_global_size(0) ]);\n"\
	"pOutputs[get_global_id(0) + 0 * get_global_size(0) ] = get_global_size(0);\n"\
	"pOutputs[get_global_id(0) + 1 * get_global_size(0) ] = get_global_size(3);\n"\
	"}"\
	"__kernel void get_global_id_test(__global size_t* pOutputs)\n"\
	"{\n"\
	"pOutputs[get_global_id(0) + 2 * get_global_size(0) ] = get_global_id(pOutputs[get_global_id(0) + 0 * get_global_size(0) ]);\n"\
	"pOutputs[get_global_id(0) + 3 * get_global_size(0) ] = get_global_id(pOutputs[get_global_id(0) + 1 * get_global_size(0) ]);\n"\
	"pOutputs[get_global_id(0) + 0 * get_global_size(0) ] = get_global_id(0);\n"\
	"pOutputs[get_global_id(0) + 1 * get_global_size(0) ] = get_global_id(3);\n"\
	"}"\
	"__kernel void get_local_size_test(__global size_t* pOutputs)\n"\
	"{\n"\
	"pOutputs[get_global_id(0) + 2 * get_global_size(0) ] = get_local_size(pOutputs[get_global_id(0) + 0 * get_global_size(0) ]);\n"\
	"pOutputs[get_global_id(0) + 3 * get_global_size(0) ] = get_local_size(pOutputs[get_global_id(0) + 1 * get_global_size(0) ]);\n"\
	"pOutputs[get_global_id(0) + 0 * get_global_size(0) ] = get_local_size(0);\n"\
	"pOutputs[get_global_id(0) + 1 * get_global_size(0) ] = get_local_size(3);\n"\
	"}"\
	"__kernel void get_local_id_test(__global size_t* pOutputs)\n"\
	"{\n"\
	"pOutputs[get_global_id(0) + 2 * get_global_size(0) ] = get_local_id(pOutputs[get_global_id(0) + 0 * get_global_size(0) ]);\n"\
	"pOutputs[get_global_id(0) + 3 * get_global_size(0) ] = get_local_id(pOutputs[get_global_id(0) + 1 * get_global_size(0) ]);\n"\
	"pOutputs[get_global_id(0) + 0 * get_global_size(0) ] = get_local_id(0);\n"\
	"pOutputs[get_global_id(0) + 1 * get_global_size(0) ] = get_local_id(3);\n"\
	"}"\
	"__kernel void get_num_groups_test(__global size_t* pOutputs)\n"\
	"{\n"\
	"pOutputs[get_global_id(0) + 2 * get_global_size(0) ] = get_num_groups(pOutputs[get_global_id(0) + 0 * get_global_size(0) ]);\n"\
	"pOutputs[get_global_id(0) + 3 * get_global_size(0) ] = get_num_groups(pOutputs[get_global_id(0) + 1 * get_global_size(0) ]);\n"\
	"pOutputs[get_global_id(0) + 0 * get_global_size(0) ] = get_num_groups(0);\n"\
	"pOutputs[get_global_id(0) + 1 * get_global_size(0) ] = get_num_groups(3);\n"\
	"}"\
	"__kernel void get_group_id_test(__global size_t* pOutputs)\n"\
	"{\n"\
	"pOutputs[get_global_id(0) + 2 * get_global_size(0) ] = get_group_id(pOutputs[get_global_id(0) + 0 * get_global_size(0) ]);\n"\
	"pOutputs[get_global_id(0) + 3 * get_global_size(0) ] = get_group_id(pOutputs[get_global_id(0) + 1 * get_global_size(0) ]);\n"\
	"pOutputs[get_global_id(0) + 0 * get_global_size(0) ] = get_group_id(0);\n"\
	"pOutputs[get_global_id(0) + 1 * get_global_size(0) ] = get_group_id(3);\n"\
	"}"\
	"__kernel void get_global_offset_test(__global size_t* pOutputs)\n"\
	"{\n"\
	"pOutputs[get_global_id(0) + 2 * get_global_size(0) ] = get_global_offset(pOutputs[get_global_id(0) + 0 * get_global_size(0) ]);\n"\
	"pOutputs[get_global_id(0) + 3 * get_global_size(0) ] = get_global_offset(pOutputs[get_global_id(0) + 1 * get_global_size(0) ]);\n"\
	"pOutputs[get_global_id(0) + 0 * get_global_size(0) ] = get_global_offset(0);\n"\
	"pOutputs[get_global_id(0) + 1 * get_global_size(0) ] = get_global_offset(3);\n"\
	"}"\
	};
