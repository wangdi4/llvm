...
TEST_F(CL21, EnqueueSVMMigrateMem_Positive)
{
    ...
    iRet = clEnqueueSVMMigrateMem(m_queue, svmBuffers.size(), &svmBuffers[0], &svmBuffers_sizes[0], CL_MIGRATE_MEM_OBJECT_HOST, 0, NULL, NULL);
...
TEST_F(CL21, EnqueueSVMMigrateMem_Negative)
{
    ...
    iRet = clEnqueueSVMMigrateMem(NULL, svmBuffers.size(), &svmBuffers[0],
            &svmBuffers_sizes[0], CL_MIGRATE_MEM_OBJECT_HOST, 0, NULL, NULL);
    ASSERT_EQ(CL_INVALID_COMMAND_QUEUE, iRet)
        << " clEnqueueSVMMigrateMem with invalid queue failed. ";

    iRet = clEnqueueSVMMigrateMem(m_queue, 0, &svmBuffers[0],
            &svmBuffers_sizes[0], CL_MIGRATE_MEM_OBJECT_HOST, 0, NULL, NULL);
    ...
    std::vector<size_t> svmBuffers_wrong_sizes(svmBuffers.size(), svmBuffer_size + 12);
    iRet = clEnqueueSVMMigrateMem(m_queue, svmBuffers.size(), &svmBuffers[0],
            &svmBuffers_wrong_sizes[0], CL_MIGRATE_MEM_OBJECT_HOST, 0, NULL, NULL);
    ASSERT_EQ(CL_INVALID_VALUE, iRet)
    ...
    iRet = clEnqueueSVMMigrateMem(m_queue, svmBuffers.size(), &wrong_svmBuffers[0],
            &svmBuffers_sizes[0], CL_MIGRATE_MEM_OBJECT_HOST, 0, NULL, NULL);
    ASSERT_EQ(CL_INVALID_VALUE, iRet)
        << " clEnqueueSVMMigrateMem with invalid pointers failed. ";

    iRet = clEnqueueSVMMigrateMem(m_queue, svmBuffers.size(), &svmBuffers[0],
            &svmBuffers_sizes[0], CL_MIGRATE_MEM_OBJECT_HOST, 2, NULL, NULL);
    ...
    iRet = clEnqueueSVMMigrateMem(m_queue, svmBuffers.size(), &svmBuffers[0],
            &svmBuffers_sizes[0], CL_MIGRATE_MEM_OBJECT_HOST, 0, &user_event, NULL);
    ASSERT_EQ(CL_INVALID_EVENT_WAIT_LIST, iRet)
        << " clEnqueueSVMMigrateMem with invalid numbers of events failed. ";
