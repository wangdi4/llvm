...
bool fission_two_queues_test() {
  ...
  cmd_queue[1] =
      clCreateCommandQueueWithProperties(context, subdevice_id, NULL, &err);
  bResult = SilentCheck("clCreateCommandQueueWithProperties - second queue",
                        CL_SUCCESS, err);
  if (!bResult)
    return bResult;
  ...
  err = clEnqueueNDRangeKernel(cmd_queue[0], kernel, 1, NULL, &globalSize, NULL,
                               0, NULL, NULL);
  bResult = SilentCheck("clEnqueueNDRangeKernel", CL_SUCCESS, err);
  if (!bResult)
    return bResult;

  err = clEnqueueReadBuffer(cmd_queue[0], buf, CL_TRUE, 0, WORK_SIZE, input, 0,
                            NULL, NULL);
  ...
  bResult = SilentCheck("clReleaseCommandQueue [0]", CL_SUCCESS, err);

  memset(input, 0, WORK_SIZE);
  err = clEnqueueWriteBuffer(cmd_queue[1], buf, CL_FALSE, 0, WORK_SIZE, input,
                             0, NULL, NULL);
  bResult = SilentCheck("clEnqueueReadBuffer", CL_SUCCESS, err);
  if (!bResult)
    return bResult;

  err = clEnqueueNDRangeKernel(cmd_queue[1], kernel, 1, NULL, &globalSize, NULL,
                               0, NULL, NULL);
  bResult = SilentCheck("clEnqueueNDRangeKernel", CL_SUCCESS, err);
  if (!bResult)
    return bResult;

  err = clEnqueueReadBuffer(cmd_queue[1], buf, CL_TRUE, 0, WORK_SIZE, input, 0,
                            NULL, NULL);
  bResult = SilentCheck("clEnqueueReadBuffer", CL_SUCCESS, err);
  if (!bResult)
    return bResult;
