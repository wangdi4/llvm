...
#define TEMPLATE_DECLERATION_FIX 	size_t& ImagesWidth = this->ImagesWidth; \
...
#define SIMPLE_READING_KERNEL __kernel void imageRead(__read_only %s /*1 type of image */ srcImg, __global uchar4  *pPixels)	\
{	\
	const sampler_t sampler = %s | CLK_ADDRESS_NONE |CLK_FILTER_NEAREST;	\
	size_t x = get_global_id(0);	\
	size_t y = get_global_id(1);	\
	size_t z = get_global_id(2);	\
	size_t pxlOff = x + y * DEFAULT_WIDTH + z * DEFAULT_WIDTH * DEFAULT_HEIGHT;	\
...
#define SIMPLE_GET_SIZE_KERNEL __kernel void imageSize(__read_only %s /*1 type of image */ srcImg, __global int *outputSize)	\
{	\
	*outputSize = %s(srcImg); \
}
