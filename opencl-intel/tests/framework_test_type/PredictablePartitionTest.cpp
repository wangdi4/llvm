...
//| 2. On each sub-device, execute a kernel several times and validate the results
//|
//| Pass criteria
//| -------------
//|
//| The kernel returns the correct results during all iterations on all sub-devices
...
bool test_subdevice(cl_device_id subdevice_id, cl_uint subdevice_size, bool useFinish)
{
	...
	if (CL_SUCCESS != err || CL_BUILD_ERROR == build_status)
	{
		...
		err = clGetProgramBuildInfo(program,subdevice_id,CL_PROGRAM_BUILD_LOG,MAX_SOURCE_SIZE,err_str_ptr,NULL);
		if (err!=CL_SUCCESS)
...
bool predictable_partition_test()
{
    ...
    for (size_t prop = 0; prop < paramSize / sizeof(cl_device_partition_property); ++prop)
    {
	...
	{
    	printf("Device doesn't support fission BY_COUNTS. Test passing vacuously\n");
		return true;
	}
