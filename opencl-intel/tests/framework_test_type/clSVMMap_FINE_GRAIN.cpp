...
TEST_F(CL20, clSVMMap_FINE_GRAIN)
{
    ...
    iRet = clEnqueueSVMMap(m_queue,
                           ...
                           /*map_flags*/0,
                           &host_allocated_mem[0],
                           host_allocated_mem.size(),
    ...
    ASSERT_TRUE((CL_SUBMITTED | CL_QUEUED) & event_status) << " clEnqueueSVMMap failed. ";

    iRet = clSetUserEventStatus(user_event,
                                CL_COMPLETE);
    ASSERT_EQ(CL_SUCCESS, iRet) << " clGetEventInfo  failed. ";
...
TEST_F(CL20, clSVMUnmap_FINE_GRAIN)
{
    ...
    cl_int event_status = -1;
    iRet = clGetEventInfo(unmap_event,
                          CL_EVENT_COMMAND_EXECUTION_STATUS,
                          sizeof(event_status),
                          &event_status,
                          nullptr);
    ASSERT_EQ(CL_SUCCESS, iRet)         << " clGetEventInfo  failed. ";
    // The command should have CL_SUBMITTED or CL_QUEUED execution status.
    ASSERT_TRUE((CL_SUBMITTED | CL_QUEUED) & event_status) << " clEnqueueSVMUnmap failed. ";
