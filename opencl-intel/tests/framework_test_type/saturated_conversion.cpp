...
cl_int check_scalar(std::vector<float> h_rhs, std::vector<char> h_res) {
    ...
    const char *ocl_test_program[] = { \
        "__kernel void test_convert_char_sat_float(__global float *src, __global char *res)"\
        "{"\
...
cl_int check_vector(std::vector<float> h_rhs, std::vector<char> h_res) {
    ...
    const char *ocl_test_program[] = { \
        "__kernel void test_convert_char8_sat_float8(__global float8 *src, __global char8 *res)"\
        "{"\
        "    size_t i = get_global_id(0);"\
