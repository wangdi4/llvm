...
bool UnloadPlatformCompiler()
{
    ...
    CheckException(L"clUnloadPlatformCompiler with invalid platform", CL_INVALID_PLATFORM, iRet);

    //valid call
    iRet = clUnloadPlatformCompiler(platform);
    CheckException(L"clUnloadPlatformCompiler", CL_SUCCESS, iRet);

    //Create and build program to check that RT will "reload" compiler.
    cl_program program = 0;
    {
        size_t dummy_kernel_lengths[] = {strlen(dummy_kernel[0])};
        program = clCreateProgramWithSource(context, 1, dummy_kernel, dummy_kernel_lengths, &iRet);
