...
//| 5. Enqueue execution of a kernel on all the created sub devices.
//| 6. read the results.
//|
//| Pass criteria
//| -------------
...
bool run_kernel(cl_context& context,cl_device_id& device,cl_command_queue& cmd_queue, const char* prog, struct dummy *out1){
	...
	err = clEnqueueReadBuffer(cmd_queue, buff, CL_TRUE, 0, sizeof(struct dummy), out1, 0, NULL, NULL);
...
bool fission_options_test(){
	...
	for (size_t i = 0; i < num_devices; i++)
	{
		bResult |= run_kernel(context, out_devices[i], cmd_queue[i], ocl_test_program, &p);
	}
