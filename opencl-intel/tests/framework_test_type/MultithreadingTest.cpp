...
public:
  RetainReleaseTestThread(size_t iterations, size_t print_period,
                          cl_context contextHandle)
      : m_iterations(iterations), m_print_period(print_period),
...
public:
  ReleaseKernelArrayThread(cl_kernel *kernels, size_t count)
      : m_count(count), m_kernels(kernels) {}
...
public:
  ReleaseProgramArrayThread(cl_program *programs, size_t count)
      : m_count(count), m_programs(programs) {}
...
public:
  ReleaseQueueArrayThread(cl_command_queue *queues, size_t count)
      : m_count(count), m_queues(queues) {}
...
public:
  ReleaseContextArrayThread(cl_context *contexts, size_t count)
      : m_count(count), m_contexts(contexts) {}
...
public:
  HelloWorldTestThread(cl_context context, cl_command_queue queue,
                       cl_program program, cl_mem param, int expected)
      : m_context(context), m_queue(queue), m_program(program), m_param(param),
...
void HelloWorldTestThread::ThreadRoutine() {
  ...
  err = clEnqueueReadBuffer(m_queue, m_param, CL_TRUE, 0, sizeof(int),
                            &kernelResult, 0, NULL, NULL);
...
void ConcurrentExecutionTestThread::ThreadRoutine() {
  ...
  const char *programSource =
      "__kernel void k(__global int* p) { p[get_global_id(0)] = 8; }";
  ...
  iRet = clEnqueueNDRangeKernel(queue, kernel, 1, NULL, &globalSize, NULL, 0,
                                NULL, NULL);
  ...
  iRet = clEnqueueReadBuffer(queue, param, CL_TRUE, 0, sizeof(int),
                             &kernelResult, 0, NULL, NULL);
...
bool MultithreadedContextRefCount() {
  ...
  for (size_t i = 0; i < numThreads; ++i) {
    threads[i] =
        new RetainReleaseTestThread(numIterations, print_period, context);
  }
  printf("Running\n");
  fflush(0);
...
bool MultithreadedReleaseObjects() {
  ...
  const char *programSource =
      "__kernel void stam(__global int* p) { p[get_global_id(0)] += 2; }";
  ...
  for (size_t iteration = 0; iteration < numIterations; ++iteration) {
    ...
    for (size_t object = 0; object < numObjects; ++object) {
      ...
      bResult &= SilentCheck("Create Context from type (gDeviceType)",
                             CL_SUCCESS, iRet);
      ...
      programs[object] = clCreateProgramWithSource(contexts[object], 1,
                                                   &programSource, NULL, &iRet);
      bResult &= SilentCheck("Create program with source", CL_SUCCESS, iRet);
...
bool MultithreadedHelloWorld() {
  ...
  const size_t expectedResult =
      8; // remember to change it in programSource if you don't like this value
  ...
  for (size_t i = 0; i < numThreads; ++i) {
    params[i] =
        clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int), NULL, &iRet);
    bResult &= SilentCheck("Build clCreateBuffer", CL_SUCCESS, iRet);
  }
  if (!bResult) {
    return false;
  ...
  std::vector<SynchronizedThread *> threads(numThreads);
  for (size_t i = 0; i < numThreads; ++i) {
    threads[i] = new HelloWorldTestThread(context, queue, program, params[i],
                                          expectedResult);
  }
...
public:
  OrderViolationThread(int id, cl_context context, cl_command_queue queue,
                       cl_program program, cl_mem param)
      : m_context(context), m_queue(queue), m_program(program), m_param(param),
        m_id(id), m_bResult(false) {
    cl_int err;
...
void OrderViolationThread::ThreadRoutine() {
  ...
  cl_char *mapPtr =
      (cl_char *)clEnqueueMapBuffer(m_queue, m_param, CL_TRUE, CL_MAP_WRITE, 0,
                                    sizeof(cl_char), 0, NULL, NULL, &err);
  ...
  *mapPtr = (cl_char)(0x80 + m_id);
  ...
  err = clEnqueueNDRangeKernel(m_queue, m_kernel1, 1, NULL, &globalSize, NULL,
                               0, NULL, NULL);
  SilentCheck("clEnqueueNDRangeKernel(1)", CL_SUCCESS, err);
  err = clEnqueueNDRangeKernel(m_queue, m_kernel2, 1, NULL, &globalSize, NULL,
                               0, NULL, NULL);
  SilentCheck("clEnqueueNDRangeKernel(2)", CL_SUCCESS, err);

  mapPtr =
      (cl_char *)clEnqueueMapBuffer(m_queue, m_param, CL_TRUE, CL_MAP_READ, 0,
                                    sizeof(cl_char), 0, NULL, NULL, &err);
  SilentCheck("clEnqueueMapBuffer", CL_SUCCESS, err);
  cl_char val = *mapPtr;
  ...
  if (!m_bResult) {
    printf("Validation failed on id=%d, expected %x, got %x\n", m_id,
           (unsigned int)m_id, (unsigned int)val);
  }
...
bool MultithreadedOrderViolation() {
  ...
  for (size_t i = 0; i < numThreads; ++i) {
    params[i] = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR,
                               sizeof(cl_char), &data[i], &iRet);
    bResult &= SilentCheck("Build clCreateBuffer", CL_SUCCESS, iRet);
  }
  if (!bResult) {
    return false;
  ...
  for (size_t i = 0; i < numThreads; ++i) {
    threads[i] =
        new OrderViolationThread((int)i, context, queue, program, params[i]);
  }
...
void BuildProgramThread::ThreadRoutine() {
  ...
  program =
      clCreateProgramWithSource(m_context, 1, &s_programSource, NULL, &iRet);
