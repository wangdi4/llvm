...
bool clLinkProgramTest()
{
    ...
    // test that clLinkProgram generates a build log for program in case of linking failure
    // try clLinkProgram without clTestLib
    clTestFailProg = clLinkProgram(context, uiNumDevices, pDevices, NULL, 1, &(clLibBin[1]), NULL, NULL, &iRet);
    if (CL_LINK_PROGRAM_FAILURE != iRet)
    {
        ...
        printf("clLinkProgram did not fail with CL_LINK_PROGRAM_FAILURE as expected, instead it returned = %s\n",ClErrTxt(iRet));
        return false;
    }
    ...
    for (unsigned int i = 0; i < uiNumDevices; i++)
    {
        ...
        iRet = clGetProgramBuildInfo(clTestFailProg, pDevices[i], CL_PROGRAM_BUILD_LOG, 0, NULL, &buildLogSize);
        if (CL_SUCCESS != iRet)
        {
            clReleaseProgram(clHeaders[0]);
            clReleaseProgram(clHeaders[1]);
        ...
        iRet = clGetProgramBuildInfo(clTestFailProg, pDevices[i], CL_PROGRAM_BUILD_LOG, buildLogSize, buildLog, NULL);
        if (CL_SUCCESS != iRet)
        {
            clReleaseProgram(clHeaders[0]);
            clReleaseProgram(clHeaders[1]);
