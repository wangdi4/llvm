...
bool run_kernel(cl_context& context,cl_device_id& device,cl_command_queue& cmd_queue, const char* prog,char *out){
	cl_int err;
	cl_kernel kernel;
	cl_program program;
	bool res;
	...
	clCreateSubBuffer(0, CL_MEM_WRITE_ONLY, CL_BUFFER_CREATE_TYPE_REGION ,NULL, &err); 
	res = SilentCheck("clCreateSubBuffer, CSSD100006061",CL_INVALID_MEM_OBJECT,err);
	if (!res) 
	{
		clReleaseKernel(kernel);
		clReleaseProgram(program);	
	...
	res = SilentCheck("SilentCheck, CSSD100006062",CL_INVALID_EVENT_WAIT_LIST,err);
	//enqueue read buffer - should fail
	err &= clEnqueueReadBuffer(cmd_queue, buff, CL_TRUE, 0, sizeof(char), out, 1, NULL, NULL);
	res &= SilentCheck("clEnqueueReadBuffer, CSSD100006062",CL_INVALID_EVENT_WAIT_LIST,err);
	...
	res &= SilentCheck("clEnqueueCopyBuffer, CSSD100006063",CL_MEM_COPY_OVERLAP,err);


	clFinish(cmd_queue);
    clReleaseEvent(ndEvent);
    clReleaseMemObject(buff);
	SilentCheck("Second clReleaseMemObject(buff)", CL_INVALID_MEM_OBJECT, clReleaseMemObject(buff)); // CSSD100005934
	clReleaseKernel(kernel);
	SilentCheck("Second clReleaseKernel()", CL_INVALID_KERNEL, clReleaseKernel(kernel)); // CSSD100005934
	clReleaseProgram(program);
	SilentCheck("Second clReleaseProgram()", CL_INVALID_PROGRAM, clReleaseProgram(program)); // CSSD100005934
	return res;
}
...
bool api_test(){
	...
	bResult = SilentCheck("clGetDeviceIDs, CSSD100006051:1",CL_INVALID_VALUE,err);
	if (!bResult)	return bResult;

	// init Devices (only one CPU...) should fail
	err = clGetDeviceIDs(platform,CL_DEVICE_TYPE_ALL,0,NULL,NULL);
	bResult = SilentCheck("clGetDeviceIDs, CSSD100006051:2",CL_INVALID_VALUE,err);
	if (!bResult)	return bResult;
	...
	bResult = CheckSize("clGetExtensionFunctionAddress(NULL)", (size_t)NULL, (size_t)invFunc);
	if (!bResult)	return bResult;
	...
	bResult = SilentCheck("clGetDeviceInfo, CSSD100006052",CL_INVALID_VALUE,err);
	if (!bResult)	return bResult;
	...
	bResult = SilentCheck("clGetDeviceInfo, CSSD100011955",CL_INVALID_VALUE,err);
	bResult &= SilentCheckSize("clGetDeviceInfo, CSSD100011955 ulRetValCheck=-1",ulRetValCheck, -1);
	bResult &= SilentCheckSize("clGetDeviceInfo(CL_DEVICE_MAX_PARAMETER_SIZE), CSSD100011955 actual_size=0", 0, actual_size);
	if (!bResult)	return bResult;

	err = clGetDeviceInfo(device, CL_DEVICE_PLATFORM, 1, &ulRetValCheck, &actual_size);
	bResult = SilentCheck("clGetDeviceInfo, CSSD100011955",CL_INVALID_VALUE,err);
	bResult &= SilentCheckSize("clGetDeviceInfo, CSSD100011955 ulRetValCheck=-1",ulRetValCheck, -1);
	bResult &= SilentCheckSize("clGetDeviceInfo(CL_DEVICE_PLATFORM), CSSD100011955 actual_size=0", 0, actual_size);
	if (!bResult)	return bResult;

	err = clGetPlatformInfo(platform, CL_PLATFORM_VERSION, 1, &ulRetValCheck, &actual_size);
	bResult = SilentCheck("clGetPlatformInfo, CSSD100011955",CL_INVALID_VALUE,err);
	bResult &= SilentCheckSize("clGetPlatformInfo, CSSD100011955 ulRetValCheck=-1",ulRetValCheck, -1);
	bResult &= SilentCheckSize("clGetPlatformInfo, CSSD100011955 actual_size=0", 0, actual_size);
	if (!bResult)	return bResult;
	...
	bResult = SilentCheck("clCreateContextFromType, CSSD100012446",CL_INVALID_PROPERTY,err);
	if (!bResult)	return bResult;

	//init context should fail
	context = clCreateContext(NULL,1,&device,NULL,user_data,&err);
	bResult = SilentCheck("clCreateContext, CSSD100006053",CL_INVALID_VALUE,err);
	if (!bResult)	return bResult;

	//init context should fail
	context = clCreateContext(NULL,1,NULL,NULL,NULL,&err);
	bResult = SilentCheck("clCreateContext, CSSD100006054",CL_INVALID_VALUE,err);
	if (!bResult)	return bResult;

	//init context should fail
	context = clCreateContextFromType(NULL,CL_DEVICE_TYPE_ALL,NULL,user_data,&err);
	bResult = SilentCheck("clCreateContext, CSSD100006055",CL_INVALID_VALUE,err);
	if (!bResult)	return bResult;
	...
	SilentCheck("Second clReleaseCommandQueue()", CL_INVALID_COMMAND_QUEUE, clReleaseCommandQueue(cmd_queue)); // CSSD100005934
	clReleaseContext(context);
	err = clReleaseContext(context);
	bResult = SilentCheck("Second clReleaseContext()",CL_INVALID_CONTEXT,err); // CSSD100006057, CSSD100005934
