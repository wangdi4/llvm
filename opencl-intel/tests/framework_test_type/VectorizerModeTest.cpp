...
static bool vectorizerModeTest(std::string const& mode)
{
    ...
    if (bResult)
    {
        ...
        if(mode == "1")
        {
            ...
            const std::string actualMsg = strLog.substr(place+kernelName.length(), expectedMsg.length());
        ...
        else
        {
            ...
            if(mode.length() > 0 && mode != "0") // do not check the width in the autonomous mode
              vecWidthMsg += " (" + mode + ")";  // the vectorized width is surrounted by brackets

            const std::string expectedMsg = std::string(" was successfully vectorized") + vecWidthMsg;
            const std::string actualMsg = strLog.substr(place+kernelName.length(), expectedMsg.length());
            if(actualMsg != expectedMsg)
            {
...
//#else
//    ::testing::FLAGS_gtest_death_test_style = "threadsafe";
//    EXPECT_EXIT( { vectorizerModeTest( mode ); exit(0); }, ::testing::ExitedWithCode(1), "" );
//#endif
//}
