...
static void BuildProgramAndExecWithQueue(cl_command_queue target_queue,
  ...
  size_t gws = 1;
  iRet =
      clEnqueueNDRangeKernel(m_queue, kern, 1, NULL, &gws, NULL, 0, NULL, NULL);
  ASSERT_EQ(CL_SUCCESS, iRet) << " clEnqueueNDRangeKernel failed. ";

  iRet = clFinish(m_queue);
  ASSERT_EQ(CL_SUCCESS, iRet) << " clFinish failed. ";

  int *success = (int *)clEnqueueMapBuffer(m_queue, arg, CL_TRUE, CL_MAP_READ,
                                           0, arg_size, 0, NULL, NULL, &iRet);
  ASSERT_EQ(CL_SUCCESS, iRet) << " clEnqueueMapBuffer failed. ";

  ASSERT_EQ(1, *success) << "Queue setted from host and queue returned by "
                            "get_default_queue() are different";
}
...
TEST_F(CL21, SetDefaultDeviceCommandQueueOOO) {
  ...
  cl_queue_properties prop_for_default[] = {
      ...
      0};
  clCreateCommandQueueWithProperties(m_context, m_device, prop_for_default,
                                     &iRet);
  ASSERT_EQ(CL_SUCCESS, iRet) << " clCreateCommandQueueWithProperties(create "
                                 "default device queue) failed. ";

  cl_queue_properties prop_for_not_default[] = {
      CL_QUEUE_PROPERTIES,
      CL_QUEUE_ON_DEVICE | CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE, 0};
  cl_command_queue queue_created_not_as_default =
      clCreateCommandQueueWithProperties(m_context, m_device,
                                         prop_for_not_default, &iRet);
  ASSERT_EQ(CL_SUCCESS, iRet) << " clCreateCommandQueueWithProperties(create "
                                 "simple OOO queue) failed. ";

  ASSERT_NO_FATAL_FAILURE(BuildProgramAndExecWithQueue(
      queue_created_not_as_default, m_device, m_context, m_queue));
}
...
TEST_F(CL21, SetDefaultDeviceCommandQueueOOO_Profiling) {
  ...
  cl_queue_properties prop_for_default[] = {
      CL_QUEUE_PROPERTIES,
      CL_QUEUE_PROFILING_ENABLE | CL_QUEUE_ON_DEVICE_DEFAULT |
  ...
  ASSERT_EQ(CL_SUCCESS, iRet)
      << " clCreateCommandQueueWithProperties(create profiling queue) failed. ";
...
TEST_F(CL21, SetDefaultDeviceCommandQueueOOO_SubDevice) {
  ...
  cl_device_partition_property prop_sub_device[] = {CL_DEVICE_PARTITION_EQUALLY,
                                                    1, 0};
  unsigned int num_sub_devices = 0;
  iRet =
      clCreateSubDevices(m_device, prop_sub_device, 0, NULL, &num_sub_devices);
  ASSERT_EQ(CL_SUCCESS, iRet) << " clCreateSubDevice failed. ";

  std::vector<cl_device_id> sub_devices(num_sub_devices, 0);
  iRet = clCreateSubDevices(m_device, prop_sub_device, sub_devices.size(),
                            &sub_devices[0], NULL);
  ASSERT_EQ(CL_SUCCESS, iRet) << " clCreateSubDevice failed. ";
  ...
  cl_context context_ =
      clCreateContext(prop, 1, &sub_devices[0], NULL, NULL, &iRet);
  ...
  cl_queue_properties prop_for_default[] = {
      ...
      CL_QUEUE_PROFILING_ENABLE | CL_QUEUE_ON_DEVICE_DEFAULT |
          CL_QUEUE_ON_DEVICE | CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE,
      0};
  clCreateCommandQueueWithProperties(context_, sub_devices[0], prop_for_default,
                                     &iRet);
  ASSERT_EQ(CL_SUCCESS, iRet) << " clCreateCommandQueueWithProperties(create "
                                 "default device queue) failed. ";
  ...
  cl_queue_properties prop_for_not_default[] = {
      ...
      0};
  cl_command_queue queue_created_not_as_default =
      clCreateCommandQueueWithProperties(context_, sub_devices[0],
                                         prop_for_not_default, &iRet);
  ASSERT_EQ(CL_SUCCESS, iRet) << " clCreateCommandQueueWithProperties(create "
                                 "simple OOO queue) failed. ";

  cl_queue_properties host_queue_prop[] = {
      CL_QUEUE_PROPERTIES, CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE, 0};
  cl_command_queue host_queue = clCreateCommandQueueWithProperties(
      context_, sub_devices[0], host_queue_prop, &iRet);

  ASSERT_NO_FATAL_FAILURE(BuildProgramAndExecWithQueue(
      queue_created_not_as_default, sub_devices[0], context_, host_queue));
}
...
TEST_F(CL21, SetDefaultDeviceCommandQueue_Get_Default_Queue_Query) {
  ...
  iRet = clSetDefaultDeviceCommandQueue(m_context, m_device,
                                        queue_created_not_as_default);
  ASSERT_EQ(CL_SUCCESS, iRet) << " clSetDefaultDeviceCommandQueue failed. ";
  ...
  iRet = clGetCommandQueueInfo(m_queue, CL_QUEUE_DEVICE_DEFAULT,
                               sizeof(cl_command_queue), &default_queue,
                               &data_being_queried);
  ASSERT_EQ(CL_SUCCESS, iRet) << " clGetCommandQueueInfo during quering "
                                 "CL_QUEUE_DEVICE_DEFAULT failed. ";

  ASSERT_EQ(queue_created_not_as_default, default_queue)
      << " Queue setted as default and returned by CL_QUEUE_DEVICE_DEFAULT "
         "query don't match. ";
}
...
TEST_F(CL21, SetDefaultDeviceCommandQueue_Negative) {
  ...
  iRet =
      clSetDefaultDeviceCommandQueue((cl_context)m_device, m_device, m_queue);
  ASSERT_EQ(CL_INVALID_CONTEXT, iRet)
      << " clSetDefaultDeviceCommandQueue with invalid context failed. ";

  iRet = clSetDefaultDeviceCommandQueue(m_context, (cl_device_id)m_context,
                                        m_queue);
  ASSERT_EQ(CL_INVALID_DEVICE, iRet)
      << " clSetDefaultDeviceCommandQueue with invalid device failed. ";

  iRet = clSetDefaultDeviceCommandQueue(m_context, m_device,
                                        (cl_command_queue)m_context);
