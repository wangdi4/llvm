...
// WARNING!!!
// This test computes ULPs incorrectly. This may lead to failures on native_
// functions which are false. ULPs are calculated here by substracting floating
// point casted to integers. However the ULPs should be in more complex way as
// they are calculated in OpenCL conformance. See
// /conform1.2/src/test_common/harness/errorHelpers.c
...
#define NATIVE_TEST_FLOAT(func, vec)                                           \
  kernel void native_test(__global float##vec *pBuff,                          \
                          __global int##vec *pULP) {                           \
    ...
    pULP[index] =                                                              \
        (-as_int##vec(abs(isgreater(pBuff[index], fabs(fres / two))))) |       \
        as_int##vec(abs(res - native_res));                                    \
  }

#define NATIVE_RECIP(vec)                                                      \
  kernel void native_recip_test(__global float##vec *pBuff,                    \
                                __global int##vec *pULP) {                     \
    size_t index = get_global_id(0);                                           \
    float##vec one = 1.0;                                                      \
...
#define NATIVE_DIVIDE(vec)                                                     \
  kernel void native_divide_test(__global float##vec *pBuffX,                  \
                                 __global float##vec *pBuffY,                  \
                                 __global int##vec *pULP) {                    \
    size_t index = get_global_id(0);                                           \
    ...
    float##vec two = 2;                                                        \
    pULP[index] =                                                              \
        (-as_int##vec(abs(isgreater(pBuffX[index], fabs(fres / two))))) |      \
        as_int##vec(abs(res - native_res));                                    \
  }

#define NATIVE_POWR(vec)                                                       \
  kernel void native_powr_test(__global float##vec *pBuffX,                    \
                               __global float##vec *pBuffY,                    \
                               __global int##vec *pULP) {                      \
    size_t index = get_global_id(0);                                           \
...
#ifndef _WIN32
// I don't know where this is defined, but apparently it is defined somewhere as
// "__builtin_isgreater".
#undef isgreater
#endif

#define RUNTESTS_FLOATS(name, func, buff, buffnum)                             \
  RunFunctionTest(NAME_VEC(name, float, ), XSTR(NATIVE_TEST_FLOAT(func, )), 1, \
                  context, queue, buff, buffnum, stBuffSize, bResult,          \
                  NATIVE_SP_MAX_ERROR_ULP);

#define RUNTESTS_SPECIAL_FLOATS(name, func, buff, buffnum)                     \
  RunFunctionTest(NAME_VEC(name, float, ), XSTR(NATIVE_##name()), 1, context,  \
                  queue, buff, buffnum, stBuffSize, bResult,                   \
                  NATIVE_SP_MAX_ERROR_ULP);                                    \
  RunFunctionTest(NAME_VEC(name, float, 2), XSTR(NATIVE_##name(2)), 2,         \
                  context, queue, buff, buffnum, stBuffSize, bResult,          \
                  NATIVE_SP_MAX_ERROR_ULP);                                    \
  RunFunctionTest(NAME_VEC(name, float, 3), XSTR(NATIVE_##name(3)), 3,         \
                  context, queue, buff, buffnum, stBuffSize, bResult,          \
                  NATIVE_SP_MAX_ERROR_ULP);                                    \
  RunFunctionTest(NAME_VEC(name, float, 4), XSTR(NATIVE_##name(4)), 4,         \
                  context, queue, buff, buffnum, stBuffSize, bResult,          \
                  NATIVE_SP_MAX_ERROR_ULP);                                    \
  RunFunctionTest(NAME_VEC(name, float, 8), XSTR(NATIVE_##name(8)), 8,         \
                  context, queue, buff, buffnum, stBuffSize, bResult,          \
                  NATIVE_SP_MAX_ERROR_ULP);                                    \
  RunFunctionTest(NAME_VEC(name, float, 16), XSTR(NATIVE_##name(16)), 16,      \
                  context, queue, buff, buffnum, stBuffSize, bResult,          \
                  NATIVE_SP_MAX_ERROR_ULP);

// runs a kernel and check it's results vs targetULP
template <typename T>
void RunFunctionTest(const char *FuncName, const char *ocl_test_program,
                     int vec, cl_context &context, cl_command_queue &queue,
                     T **pBuff, int numBuffs, size_t &stBuffSize, bool &bResult,
                     int target_ulp) {
