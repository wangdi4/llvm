...
bool clImageExecuteTest() {
  ...
  const char *ocl_test_program[] = {
      "__kernel void image_test(__read_only image2d_t srcImg, sampler_t samp, "
      "__global uint4* pPixels)\n"
      "{\n"
...
#if 0
    cl_program program =
      clCreateProgramWithSource(context, 1, (const char**)&ocl_test_program,
                                NULL, &iRet);
...

    {
      ...
      if (gDeviceType != CL_DEVICE_TYPE_ACCELERATOR) {
        ...
        printf(" - Creating src image %d by %d...\n", (int)szSrcWidth,
               (int)szSrcHeight);
        cl_mem clSrcImg =
            clCreateImage2D(context, CL_MEM_READ_ONLY, &clFormat, szSrcWidth,
                            szSrcHeight, 0, NULL, &iRet);
        ...
        cl_mem clDstBuff =
            clCreateBuffer(context, CL_MEM_WRITE_ONLY, stBuffSize, NULL, &iRet);
...

        {
          ...
          cl_sampler sampler =
              clCreateSamplerWithProperties(context, props, &iRet);
          bResult &= Check("clCreateSamplerWithProperties", CL_SUCCESS, iRet);
          if (!bResult)
            goto release_image;

          cl_sampler_properties propsLinear[] = {CL_SAMPLER_NORMALIZED_COORDS,
                                                 CL_FALSE,
                                                 CL_SAMPLER_ADDRESSING_MODE,
                                                 CL_ADDRESS_NONE,
                                                 CL_SAMPLER_FILTER_MODE,
          ...
          cl_sampler samplerLinear =
              clCreateSamplerWithProperties(context, propsLinear, &iRet);
          bResult &= Check("clCreateSamplerWithProperties", CL_SUCCESS, iRet);
          if (!bResult)
          ...
          iRet = clEnqueueWriteImage(queue, clSrcImg, CL_TRUE, origin, region,
          ...
          // Verification phase
          //
          iRet = clEnqueueReadBuffer(queue, clDstBuff, CL_TRUE, 0, stBuffSize,
                                     pDstBuffer, 0, NULL, NULL);
          ...
          printf(
              "Testing that invalid sampler usage don't lead to app crash..\n");
          iRet = clSetKernelArg(kernel, 1, sizeof(cl_sampler), &samplerLinear);
          bResult &= Check("clSetKernelArg - sampler", CL_SUCCESS, iRet);
          if (!bResult)
            goto release_linear_sampler;

          // we don't really care about image content here, so just left it as
          // in previous test
          {
