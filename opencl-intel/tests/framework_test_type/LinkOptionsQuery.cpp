...
void clGetProgramBuildInfoTest()
{
    ...
    // Create context
    const cl_context_properties prop[5] =
    ...
    iRet = clCompileProgram(
      Pr0, 1, &device, "-cl-fast-relaxed-math", 0, NULL, NULL, NULL, NULL);
    ASSERT_EQ(CL_SUCCESS, iRet) << " clCompileProgram for Pr0 failed.";

    iRet = clCompileProgram(
      Pr1, 1, &device, "-cl-fast-relaxed-math", 0, NULL, NULL, NULL, NULL);
    ASSERT_EQ(CL_SUCCESS, iRet) << " clCompileProgram for Pr1 failed.";

    iRet = clCompileProgram(
      Pr2, 1, &device, "-cl-denorms-are-zero", 0, NULL, NULL, NULL, NULL);
    ...
    cl_program Pr01Linked = clLinkProgram(
      context, 1, &device, "-cl-denorms-are-zero",
    ...
    iRet = clGetProgramBuildInfo(
      Pr01Linked, device, CL_PROGRAM_BUILD_OPTIONS, 0, NULL, &SizePr01Linked);
    ASSERT_EQ(CL_SUCCESS, iRet)
        << " Device failed to return compile options size.";

    char* OptsPr01Linked = (char *)malloc(SizePr01Linked);
    iRet = clGetProgramBuildInfo(
      Pr01Linked, device, CL_PROGRAM_BUILD_OPTIONS, SizePr01Linked,
      OptsPr01Linked, NULL);
    ASSERT_EQ(CL_SUCCESS, iRet)
        << " clGetProgramBuildInfo CL_PROGRAM_BUILD_OPTIONS failed";
    ASSERT_TRUE(OptsPr01Linked)
        << " No LinkOptions";
    ...
    cl_program Pr01LinkedToLib = clLinkProgram(
      context, 1, &device, "-create-library -cl-denorms-are-zero",
      2, ToLink, NULL, NULL, &iRet);
    ASSERT_EQ(CL_SUCCESS, iRet) << " clCreateProgramWithSource failed.";
    ...
    cl_program Pr012Linked = clLinkProgram(
      context, 1, &device, "-cl-fast-relaxed-math",
      2, ToLink2, NULL, NULL, &iRet);
    ASSERT_EQ(CL_SUCCESS, iRet) << " clCreateProgramWithSource failed.";
    ...
    iRet = clGetProgramBuildInfo(
      Pr012Linked, device, CL_PROGRAM_BUILD_OPTIONS, 0, NULL, &SizePr012Linked);
    ASSERT_EQ(CL_SUCCESS, iRet)
        << " Device failed to return compile options size.";

    char* OptsPr012Linked = (char *)malloc(SizePr01Linked);
    iRet = clGetProgramBuildInfo(
      Pr012Linked, device, CL_PROGRAM_BUILD_OPTIONS, SizePr012Linked,
      OptsPr012Linked, NULL);
    ASSERT_EQ(CL_SUCCESS, iRet)
        << " clGetProgramBuildInfo CL_PROGRAM_BUILD_OPTIONS failed";
    ASSERT_TRUE(OptsPr012Linked)
        << " No LinkOptions";
