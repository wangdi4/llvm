...
#define TITLE( prefix ) get_title( title, prefix, sizeof(prefix), dev_name, (4*sizeof(char)) )
    

inline char* get_title( char* buffer, 
                           const char* prefix, unsigned int psize, 
...
bool run_the_test(const char* test_name, bool include_migration)
{
    ...
    for (unsigned int curr_dev = 0; curr_dev < sizeof(dev_types)/sizeof(dev_types[0]); ++curr_dev )
    {
        ...
        if ((CL_DEVICE_NOT_FOUND == iRet) && (CL_DEVICE_TYPE_CPU != dev_types[curr_dev]))
        {
            ...
            bResult &= SilentCheck(TITLE("clGetDeviceIDs for "), CL_DEVICE_NOT_FOUND, iRet);    
            return bResult;
        }
    ...
    for (unsigned int round = 0; round < number_of_rounds; ++round)
    {
        for (unsigned int curr_dev = 0; curr_dev < sizeof(devices)/sizeof(devices[0]); ++curr_dev )
    	{
            ...
            if (include_migration)
            {
                ...
                bResult &= SilentCheck(TITLE("clEnqueueMigrateMemObjects from HOST to "), CL_SUCCESS, iRet);    
                if (!bResult) break;
            }
    		...
    		bResult &= SilentCheck(TITLE("clEnqueueNDRangeKernel for "), CL_SUCCESS, iRet);    
            if (!bResult) break;
            ...
            if (include_migration)
            {
                ...
                bResult &= SilentCheck(TITLE("clEnqueueMigrateMemObjects to HOST from "), CL_SUCCESS, iRet);    
                if (!bResult) break;
            }
	...
	bResult &= SilentCheck(TITLE("clEnqueueReadBuffer - Dst for "), CL_SUCCESS, iRet);
	if (!bResult) goto main_error_exit;

	for( unsigned y=0; y < stBuffSize; ++y )
	{
		if ( pDstBuff[y] != (pBuff[y] + 10 * number_of_rounds * (int)(sizeof(devices)/sizeof(devices[0]))))
		{
			bResult = false;
            break;
		}
