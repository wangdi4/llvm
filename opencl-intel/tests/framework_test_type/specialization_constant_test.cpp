...
//         %34 = OpString "kernel_arg_type.foo.char*,short*,int*,long*,half*,float*,double*,"
...
//         %17 = OpTypeFunction %void %_ptr_CrossWorkgroup_uchar %_ptr_CrossWorkgroup_ushort %_ptr_CrossWorkgroup_uint %_ptr_CrossWorkgroup_ulong %_ptr_CrossWorkgroup_half %_ptr_CrossWorkgroup_float %_ptr_CrossWorkgroup_double
...
protected:
    ...
    virtual void SetUp()
    {
        ASSERT_OCL_SUCCESS(clGetPlatformIDs(1, &m_platform, NULL), "clGetPlatformIDs");
        ASSERT_OCL_SUCCESS(clGetDeviceIDs(m_platform, gDeviceType, 1, &m_device, NULL), "clGetDeviceIDs");
        ...
        m_program = clCreateProgramWithIL(m_context, spirv.data(), spirv.size(), &iRet);
        ...
        mem_c = clCreateBuffer(m_context, CL_MEM_USE_HOST_PTR, sizeof(c), &c, &iRet);
        ASSERT_OCL_SUCCESS(iRet, "clCreateBuffer");
        mem_s = clCreateBuffer(m_context, CL_MEM_USE_HOST_PTR, sizeof(s), &s, &iRet);
        ASSERT_OCL_SUCCESS(iRet, "clCreateBuffer");
        mem_i = clCreateBuffer(m_context, CL_MEM_USE_HOST_PTR, sizeof(i), &i, &iRet);
        ASSERT_OCL_SUCCESS(iRet, "clCreateBuffer");
        mem_l = clCreateBuffer(m_context, CL_MEM_USE_HOST_PTR, sizeof(l), &l, &iRet);
        ASSERT_OCL_SUCCESS(iRet, "clCreateBuffer");
        mem_h = clCreateBuffer(m_context, CL_MEM_USE_HOST_PTR, sizeof(h), &h, &iRet);
        ASSERT_OCL_SUCCESS(iRet, "clCreateBuffer");
        mem_f = clCreateBuffer(m_context, CL_MEM_USE_HOST_PTR, sizeof(f), &f, &iRet);
        ASSERT_OCL_SUCCESS(iRet, "clCreateBuffer");
        mem_d = clCreateBuffer(m_context, CL_MEM_USE_HOST_PTR, sizeof(d), &d, &iRet);
        ASSERT_OCL_SUCCESS(iRet, "clCreateBuffer");
    }
    ...
    void buildAndRun() {
        ...
        {
            std::string log("", 1000);
            clGetProgramBuildInfo(m_program, m_device, CL_PROGRAM_BUILD_LOG, log.size(), &log[0], nullptr);
        ...
        ASSERT_OCL_SUCCESS(clSetKernelArg(kern, 0, sizeof(cl_mem), &mem_c), "clSetKernelArg");
        ASSERT_OCL_SUCCESS(clSetKernelArg(kern, 1, sizeof(cl_mem), &mem_s), "clSetKernelArg");
        ASSERT_OCL_SUCCESS(clSetKernelArg(kern, 2, sizeof(cl_mem), &mem_i), "clSetKernelArg");
        ASSERT_OCL_SUCCESS(clSetKernelArg(kern, 3, sizeof(cl_mem), &mem_l), "clSetKernelArg");
        ASSERT_OCL_SUCCESS(clSetKernelArg(kern, 4, sizeof(cl_mem), &mem_h), "clSetKernelArg");
        ASSERT_OCL_SUCCESS(clSetKernelArg(kern, 5, sizeof(cl_mem), &mem_f), "clSetKernelArg");
        ASSERT_OCL_SUCCESS(clSetKernelArg(kern, 6, sizeof(cl_mem), &mem_d), "clSetKernelArg");

        size_t gws = 1;
        iRet = clEnqueueNDRangeKernel(m_queue, kern, 1, nullptr, &gws, nullptr, 0, nullptr, nullptr);
...
TEST_F(SpecializationConstant, CheckDefaultValue)
{
    ...
    ASSERT_EQ(h, 0x3800) << "Wrong result!"; //0x3800 is binary representaion of 0.5 for half
