...
TEST_TYPE_EXECUTABLE_NAME = os.getenv('EXE_DIRECTORY') + '/framework_test_type' + exeSuffix
#define the test type's xml schema file name
TEST_TYPE_SCHEMA_NAME     = os.getenv('EXE_DIRECTORY') + '/framework_test_type.xsd'
...
def execute_test(exe, test_case_name):
	...
	if sys.platform == 'win32':
            args = [os.path.normpath(exe), test_case_name]
        else:
            args = './' + os.path.normpath(exe) + " " + test_case_name

	outputDirectory = os.getenv('OUTPUT_DIRECTORY')

	exp_out = outputDirectory + '/result_' + test_case_name + '.out'
	exp_out_file = open(exp_out,'w')

	exp_err_out = outputDirectory+ '/result_err_' + test_case_name + '.out'
...
def main():
	...
	outputDirectory = os.getenv('OUTPUT_DIRECTORY')
        ...
        final_log_out = open(outputDirectory + '/' + 'framework_test_type.out', 'w')
	for tc in data.get_test_cases():
            single_out = outputDirectory + '/result_' + tc.get_name() + '.out'
            if os.path.isfile(single_out) == False:
                continue
