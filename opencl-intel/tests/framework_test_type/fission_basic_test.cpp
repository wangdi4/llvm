...
bool run_kernel(cl_context& context,cl_device_id& device,cl_command_queue& cmd_queue, const char* prog, struct preferences *out1, struct foo *out2){
	...
	out2->counter = 0;
	out2->flag = 0;
	err = clEnqueueReadBuffer(cmd_queue, buff1, CL_TRUE, 0, sizeof(struct preferences), out1, 0, NULL, NULL);
	err = clEnqueueReadBuffer(cmd_queue, buff2, CL_TRUE, 0, sizeof(struct foo), out2, 0, NULL, NULL);
...
bool fission_basic_test(){
	...
	bResult = run_kernel(context, out_devices[0], cmd_queue, ocl_test_program, &p, &f);
	printf("\n---------------------------------------\n");
