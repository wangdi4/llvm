...
int test_get_sampler_info(cl_device_id deviceID, cl_context context, cl_command_queue queue, int num_elements)
{
    ...
    sampler = clCreateSampler( NULL, CL_TRUE, CL_ADDRESS_CLAMP, CL_FILTER_LINEAR, &error );
    sampler = clCreateSampler( context, CL_TRUE, CL_ADDRESS_CLAMP, CL_FILTER_LINEAR, &error );
    test_error( error, "Unable to create sampler to test with" );
    
    cl_uint refCount;
    error = clGetSamplerInfo( sampler, CL_SAMPLER_REFERENCE_COUNT, NULL, &refCount, &size );
...
int test_get_command_queue_info(cl_device_id deviceID, cl_context context, cl_command_queue ignoreQueue, int num_elements)
{
    ...
    cl_uint refCount;
    error = clGetCommandQueueInfo( queue, CL_QUEUE_REFERENCE_COUNT, NULL, &refCount, &size );
