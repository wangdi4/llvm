...
#define PROPERTY_TYPES 8
    ...
    for (cl_uint i = starting_property;i < ending_property;i++) {
      ...
      err = clCreateSubDevices(NULL, partitionProp[i], deviceCount, subDevices, &deviceCount);
      ...
      err = clRetainDevice(subDevices[0]);

      err = clReleaseDevice(NULL);
      err = clReleaseDevice(subDevices[0]);

      err = clReleaseDevice(subDevices[0]);
      test_error( err, "\n Releasing sub-device failed \n" );

      for (cl_uint j=1;j < deviceCount;j++)
      {
