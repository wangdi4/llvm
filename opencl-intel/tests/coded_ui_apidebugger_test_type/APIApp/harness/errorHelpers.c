...
int check_opencl_version_with_testname(const char *subtestname, cl_device_id device)
{
    ...
    for(i=0; i < nRequiring12; ++i) {
        if(!strcmp(subtestname, subtests_requiring_opencl_1_2[i])) {
            return check_opencl_version(device);
        }
    }
    return 0;
}

int check_opencl_version(cl_device_id device) {
    int error;
    char device_version[1024];
    const char * required_version="OpenCL 1.2 ";
    ...
    // The device version string could contain vendor specific information
    return strncmp(device_version, required_version, strlen(required_version));
}
