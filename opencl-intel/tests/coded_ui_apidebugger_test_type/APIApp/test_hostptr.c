...
test_hostptr(cl_device_id device, cl_context context, cl_command_queue queue, int num_elements)
{
    ...
    // Block to mark deletion of streams before deletion of host_ptr
    {
        ...
        cl_float *data;
        data = (cl_float*) clEnqueueMapBuffer( queue, NULL, CL_TRUE, CL_MAP_READ, 0, sizeof(cl_float) * num_elements, 0, NULL, NULL, &err );
        data = (cl_float*) clEnqueueMapBuffer( queue, streams[2], CL_TRUE, CL_MAP_READ, 0, sizeof(cl_float) * num_elements, 0, NULL, NULL, &err );
        ...
        err = clEnqueueCopyImage(queue, NULL, streams[5], origin, origin, region,  0, NULL, NULL);
        err = clEnqueueCopyImage(queue, streams[3], streams[5], origin, origin, region,  0, NULL, NULL);
        ...
        err = clEnqueueReadImage(queue, NULL, CL_TRUE, origin, region, 4*img_width, 0, rgba8_outptr, 0, NULL, NULL);
