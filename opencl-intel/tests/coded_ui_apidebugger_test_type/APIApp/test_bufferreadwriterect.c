...
int read_verify_region(size_t src, size_t soffset[3], size_t sregion[3], size_t dst, size_t doffset[3], size_t dregion[3]) {
    ...
    // Copy the source region of the cl buffer, to the destination region of the temporary buffer.
		err = clEnqueueReadBufferRect(queue,
                                          NULL,
                                          CL_TRUE,
                                          soffset,doffset,
                                          sregion,
    ...
    err = clEnqueueReadBufferRect(queue,
                                          ...
                                          tmp_buffer,
                                          0, NULL, NULL);
    CL_EXIT_ERROR(err, "clEnqueueReadBufferRect failed between %u and %u",(unsigned)src,(unsigned)dst);
...
int write_region(size_t src, size_t soffset[3], size_t sregion[3], size_t dst, size_t doffset[3], size_t dregion[3]) {
    ...
    // Copy the source region of the cl buffer, to the destination region of the temporary buffer.
		err = clEnqueueWriteBufferRect(queue,
                                           NULL,
                                           CL_TRUE,
                                           doffset,soffset,
    /*sregion,*/dregion,
    ...
    /*sregion,*/dregion,
                                           ...
                                           tmp_buffer,
                                           0, NULL, NULL);
    CL_EXIT_ERROR(err, "clEnqueueWriteBufferRect failed between %u and %u",(unsigned)src,(unsigned)dst);
...
void CL_CALLBACK mem_obj_destructor_callback2( cl_mem, void *data )
{
    free( data );
}
...
test_bufferreadwriterect(cl_device_id device, cl_context context, cl_command_queue queue_, int num_elements)
{
    ...
    for (unsigned i=0; i != TotalImages; ++i) {
        ...
        err = clSetMemObjectDestructorCallback( buffer[i], mem_obj_destructor_callback2, backing[i] );
    ...
    for (size_t iter = 0; iter < num_tries; ++iter) {
        ...
        size_t operation;//get_random_size_t(0,TotalOperations,mt);
				if (iter == 0) operation = 0;
				else if (iter == 1) operation = 1;
				else if (iter == 2) operation = 2;
				else  operation = 3;
