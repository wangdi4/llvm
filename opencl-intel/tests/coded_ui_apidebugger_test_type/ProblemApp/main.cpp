...
int main(int argc, const char** argv)
{
	...
	cl_device_id fake_object = (cl_device_id)0;

	err = clCreateSubDevices(devices[CPU], prop, 2, &subdevices[0], NULL);
	checkOpenCLErrors(err);
	
	/*clCreateSubDevices error generation*/
	cl_device_partition_property error_prop[] = { CL_DEVICE_PARTITION_EQUALLY, 2, CL_DEVICE_PARTITION_BY_COUNTS_LIST_END, 0 };
	err = clCreateSubDevices((cl_device_id)fake_object, prop, 2, &subdevices[0], NULL);
	/******************************************************************/
	...
	contexts[0] = clCreateContext(0, 1, &(cl_device_id)fake_object, NULL, NULL, &err);
	/******************************************************************/
	...
	/**clCreateContext error ******************************************/
	clCreateCommandQueue(contexts[CPU], devices[CPU], CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE | 123, &err);
	/******************************************************************/

	/**clCreateSamplerqq error ******************************************/
	clCreateSampler((cl_context)fake_object, true, CL_ADDRESS_CLAMP_TO_EDGE, CL_FILTER_NEAREST, &err);
	/******************************************************************/
	// Create sampler
	...
	vector<cl_kernel> kernels(num_of_devices); cl_kernel fake_kernel;
	vector<cl_program> programs(num_of_devices); cl_program fake_program;
	fake_kernel = create_kernel("queues_comp_errors.cl", "QueueKernel", contexts[CPU], devices[CPU], &fake_program, true);
	...
	fake_kernel = create_kernel("images_warnings.cl", "OverLap", contexts[GPU], devices[GPU], &fake_program, true);
	kernels[MIC] = create_kernel("queues.cl", "QueueKernel", contexts[MIC], devices[MIC], &programs[MIC]);
	fake_kernel = create_kernel("queues_link_errors.cl", "QueueKernel", contexts[MIC], devices[MIC], &fake_program, true);
	fake_kernel = create_kernel("queues.cl", "AbsentKernel", contexts[MIC], devices[MIC], &fake_program, true);
