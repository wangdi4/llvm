...
cl_kernel create_kernel(string file_name, string kernel_name, cl_context context, cl_device_id device, cl_program * prog, bool ignore_errors){
	cl_int err;
	...
	*prog = clCreateProgramWithSource(context, 1, &raw_text, 0, &err);
	if(!ignore_errors)checkOpenCLErrors(err);

	const string build_options("-g -s "+string(file_name));
