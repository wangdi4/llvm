...
/******************************************************************************
 * Test checks atomic_add in kernel code sums float/double values correctly.
 ******************************************************************************/
...
const char *OCL20_KERNEL_LOCAL_ADDRSPACE_ARIFM_TEST_CODE_STR =
    ...
    "  int tid = get_global_id(0);"
    "  __local T sum;"
    "  atomic_store_explicit((volatile __local AT*)&sum, src_a[tid],"
    "                      memory_order_relaxed);"
    "  r_old[tid] = atomic_fetch_add_explicit((volatile __local AT*)&sum,"
    "                                         src_b[tid], memory_order_relaxed,"
    "                                         memory_scope_device); "
    "  atomic_store_explicit(&(r_sum[tid]), sum, memory_order_relaxed);"
    "}";
...
const char *OCL20_KERNEL_LOCAL_ADDRSPACE_ATOM_TEST_CODE_STR =
    ...
    "  atomic_fetch_add_explicit((volatile __local AT*)&sum, *addend,"
    ...
    "  barrier(CLK_LOCAL_MEM_FENCE);"
    "  if (get_local_id(0) == 0) {"
    "    atomic_fetch_add_explicit(r_sum, sum, memory_order_relaxed,"
    "                              memory_scope_device);"
    "  }"
    "}";
} // namespace
...
template <typename T>
cl_int check_atomic_add_arifm(bool isOCL20, bool isLocal,
...
template <typename T> bool atomic_add_float_arifm_test(bool isOCL20) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    iRet = check_atomic_add_arifm(isOCL20, i == 0 ? false : true, src_a, src_b,
                                  r_old, r_sum);
...
template <typename T>
cl_int check_atomic_add_atomicity(bool isOCL20, bool isLocal, size_t global_ws,
...
template <typename T>
bool check_atomicity_results_with_ocl20_local_addrspace(size_t global_ws,
                                                        size_t local_ws,
                                                        T augend, T addend,
                                                        T r_sum) {
  T exp_sum = augend;
  for (size_t i = 0; i < global_ws / local_ws; i++) {
    T sum = 0.0;
    for (size_t j = 0; j < local_ws; j++) {
  ...
  size_t remainder = global_ws % local_ws;
  if (remainder != 0) {
    T sum = 0.0;
...
template <typename T> bool atomic_add_float_atomicity_test(bool isOCL20) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    iRet =
        check_atomic_add_atomicity(isOCL20, (i == 0) ? false : true, global_ws,
                                   local_ws, augend, addend, &r_sum);
    ...
    if (i == 0 || !isOCL20) {
      ...
      if (!check_atomicity_results_with_ocl20_local_addrspace(
              global_ws, local_ws, augend, addend, r_sum)) {
        printf("Results differ from expected in check_atomic_add_atomicity\n");
        return false;
      }
    }
