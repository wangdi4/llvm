//
// Copyright 2020 Intel Corporation.
//
// This software and the related documents are Intel copyrighted materials, and
// your use of them is governed by the express license under which they were
// provided to you (License). Unless the License provides otherwise, you may not
// use, modify, copy, publish, distribute, disclose or transmit this software or
// the related documents without Intel's prior written permission.
//
// This software and the related documents are provided as is, with no express
// or implied warranties, other than those that are expressly stated in the
// License.

#include "SGEmulationTest.h"

TEST_P(SGEmulationTest, SubRoutineTestsWithTID) {

  const char *kernel = "__attribute__((noinline))"
                       "size_t foo() {"
                       "  return get_local_id(0);"
                       "}"
                       "size_t foo_sg() {"
                       "  return get_sub_group_local_id();"
                       "}"
                       "__kernel void basic(__global unsigned long* local_id, "
                       "__global unsigned long* sg_local_id) {"
                       "  size_t lid = foo();"
                       "  local_id[lid] = lid;"
                       "  sg_local_id[lid] = foo_sg();"
                       "}";
  const size_t kernel_size = strlen(kernel);
  cl_int iRet = CL_SUCCESS;

  cl_program program =
      clCreateProgramWithSource(m_context, 1, &kernel, &kernel_size, &iRet);
  ASSERT_OCL_SUCCESS(iRet, " clCreateProgramWithSource");

  iRet = clBuildProgram(program, 0, nullptr, "", nullptr, nullptr);
  if (CL_SUCCESS != iRet) {
    std::string log;
    ASSERT_NO_FATAL_FAILURE(GetBuildLog(m_device, program, log));
    FAIL() << log;
  }

  cl_kernel kern = clCreateKernel(program, "basic", &iRet);
  ASSERT_OCL_SUCCESS(iRet, " clCreateKernel");

  constexpr size_t gsize = 16;
  constexpr size_t lsize = 16;

  size_t max_sg_size = 0;
  iRet = clGetKernelSubGroupInfo(
      kern, m_device, CL_KERNEL_MAX_SUB_GROUP_SIZE_FOR_NDRANGE, sizeof(lsize),
      &lsize, sizeof(max_sg_size), &max_sg_size, nullptr);
  ASSERT_OCL_SUCCESS(iRet, " clGetKernelSubGroupInfo");

  cl_ulong lid[lsize] = {0};
  cl_mem mem_obj_lid = clCreateBuffer(m_context, CL_MEM_USE_HOST_PTR,
                                      sizeof(cl_ulong) * lsize, lid, &iRet);
  ASSERT_OCL_SUCCESS(iRet, " clCreateBuffer");

  iRet = clSetKernelArg(kern, 0, sizeof(cl_mem), &mem_obj_lid);
  ASSERT_OCL_SUCCESS(iRet, " clSetKernelArg");

  cl_ulong sg_lid[lsize] = {0};
  cl_mem mem_obj_sg_lid = clCreateBuffer(
      m_context, CL_MEM_USE_HOST_PTR, sizeof(cl_ulong) * lsize, sg_lid, &iRet);
  ASSERT_OCL_SUCCESS(iRet, " clCreateBuffer");

  iRet = clSetKernelArg(kern, 1, sizeof(cl_mem), &mem_obj_sg_lid);
  ASSERT_OCL_SUCCESS(iRet, " clSetKernelArg");

  iRet = clEnqueueNDRangeKernel(m_queue, kern, 1, nullptr, &gsize, &lsize, 0,
                                nullptr, nullptr);
  ASSERT_OCL_SUCCESS(iRet, " clEnqueueNDRangeKernel");

  iRet = clFinish(m_queue);
  ASSERT_OCL_SUCCESS(iRet, " clFinish");

  for (size_t i = 0; i < lsize; ++i) {
    ASSERT_EQ(i % max_sg_size, sg_lid[i])
        << "Mismatch at work-item: " << i << " Expect: " << i % max_sg_size
        << " Result: " << sg_lid[i];
    ASSERT_EQ(i, lid[i]) << "Mismatch at work-item: " << i << " Expect: " << i
                         << " Result: " << lid[i];
  }

  iRet = clReleaseKernel(kern);
  ASSERT_OCL_SUCCESS(iRet, " clReleaseKernel");
  iRet = clReleaseProgram(program);
  ASSERT_OCL_SUCCESS(iRet, " clReleaseProgram");
  iRet = clReleaseMemObject(mem_obj_lid);
  ASSERT_OCL_SUCCESS(iRet, " clReleaseMemObject");
  iRet = clReleaseMemObject(mem_obj_sg_lid);
  ASSERT_OCL_SUCCESS(iRet, " clReleaseMemObject");
}

// this test is to check if the builtin is called in two different functions and
// is widened correctly.
TEST_P(SGEmulationTest, SubRoutineTestsWithSameBuiltins) {
  const char *kernel = "size_t foo(__global int *a) {"
                       "  int lid = get_local_id(0);"
                       "  a[lid] = sub_group_scan_inclusive_add(lid);"
                       "  return lid;"
                       "}"
                       "__attribute__((noinline)) "
                       "size_t foo_sg(__global int *a) {"
                       "  int lid = get_local_id(0);"
                       "  a[lid] = sub_group_scan_inclusive_add(lid);"
                       "  return get_sub_group_local_id();"
                       "}"
                       "__attribute__((intel_reqd_sub_group_size(8)))"
                       "__kernel void basic(__global unsigned long* local_id, "
                       "  __global int *scan_add) {"
                       "  size_t lid = foo(scan_add);"
                       "  local_id[lid] = lid;"
                       "}"
                       "__attribute__((intel_reqd_sub_group_size(8)))"
                       "__kernel void basic_sg("
                       "  __global unsigned long* sg_local_id,"
                       "  __global int *sg_scan_add) {"
                       "  size_t lid = get_local_id(0);"
                       "  sg_local_id[lid] = foo_sg(sg_scan_add);"
                       "}";

  const size_t kernel_size = strlen(kernel);
  cl_int iRet = CL_SUCCESS;

  cl_program program =
      clCreateProgramWithSource(m_context, 1, &kernel, &kernel_size, &iRet);
  ASSERT_OCL_SUCCESS(iRet, " clCreateProgramWithSource");

  iRet = clBuildProgram(program, 0, nullptr, "", nullptr, nullptr);
  if (CL_SUCCESS != iRet) {
    std::string log;
    ASSERT_NO_FATAL_FAILURE(GetBuildLog(m_device, program, log));
    FAIL() << log;
  }

  cl_kernel kern = clCreateKernel(program, "basic", &iRet);
  ASSERT_OCL_SUCCESS(iRet, " clCreateKernel");
  cl_kernel kern_sg = clCreateKernel(program, "basic_sg", &iRet);
  ASSERT_OCL_SUCCESS(iRet, " clCreateKernel");

  constexpr size_t gsize = 16;
  constexpr size_t lsize = 16;

  size_t max_sg_size = 0;
  iRet = clGetKernelSubGroupInfo(
      kern, m_device, CL_KERNEL_MAX_SUB_GROUP_SIZE_FOR_NDRANGE, sizeof(lsize),
      &lsize, sizeof(max_sg_size), &max_sg_size, nullptr);
  ASSERT_OCL_SUCCESS(iRet, " clGetKernelSubGroupInfo");

  cl_ulong lid[lsize] = {0};
  cl_mem mem_obj_lid = clCreateBuffer(m_context, CL_MEM_USE_HOST_PTR,
                                      sizeof(cl_ulong) * lsize, lid, &iRet);
  ASSERT_OCL_SUCCESS(iRet, " clCreateBuffer");
  cl_int scan_add[lsize] = {0};
  cl_mem mem_obj_scan_add = clCreateBuffer(
      m_context, CL_MEM_USE_HOST_PTR, sizeof(cl_int) * lsize, scan_add, &iRet);
  ASSERT_OCL_SUCCESS(iRet, " clCreateBuffer");
  iRet = clSetKernelArg(kern, 0, sizeof(cl_mem), &mem_obj_lid);
  ASSERT_OCL_SUCCESS(iRet, " clSetKernelArg");
  iRet = clSetKernelArg(kern, 1, sizeof(cl_mem), &mem_obj_scan_add);
  ASSERT_OCL_SUCCESS(iRet, " clSetKernelArg");

  cl_ulong sg_lid[lsize] = {0};
  cl_mem mem_obj_sg_lid = clCreateBuffer(
      m_context, CL_MEM_USE_HOST_PTR, sizeof(cl_ulong) * lsize, sg_lid, &iRet);
  ASSERT_OCL_SUCCESS(iRet, " clCreateBuffer");
  cl_int sg_scan_add[lsize] = {0};
  cl_mem mem_obj_sg_scan_add =
      clCreateBuffer(m_context, CL_MEM_USE_HOST_PTR, sizeof(cl_int) * lsize,
                     sg_scan_add, &iRet);
  ASSERT_OCL_SUCCESS(iRet, " clCreateBuffer");
  iRet = clSetKernelArg(kern_sg, 0, sizeof(cl_mem), &mem_obj_sg_lid);
  ASSERT_OCL_SUCCESS(iRet, " clSetKernelArg");

  iRet = clSetKernelArg(kern_sg, 1, sizeof(cl_mem), &mem_obj_sg_scan_add);
  ASSERT_OCL_SUCCESS(iRet, " clSetKernelArg");

  iRet = clEnqueueNDRangeKernel(m_queue, kern, 1, nullptr, &gsize, &lsize, 0,
                                nullptr, nullptr);
  ASSERT_OCL_SUCCESS(iRet, " clEnqueueNDRangeKernel");

  iRet = clEnqueueNDRangeKernel(m_queue, kern_sg, 1, nullptr, &gsize, &lsize, 0,
                                nullptr, nullptr);
  ASSERT_OCL_SUCCESS(iRet, " clEnqueueNDRangeKernel");

  iRet = clFinish(m_queue);
  ASSERT_OCL_SUCCESS(iRet, " clFinish");

  for (size_t i = 0; i < lsize; ++i) {
    ASSERT_EQ(i % max_sg_size, sg_lid[i])
        << "Mismatch at work-item: " << i << " Expect: " << i % max_sg_size
        << " Result: " << sg_lid[i];
    ASSERT_EQ(i, lid[i]) << "Mismatch at work-item: " << i << " Expect: " << i
                         << " Result: " << lid[i];
  }

  int scan_add_ref = 0;
  for (size_t i = 0; i < lsize; ++i) {
    if (i % max_sg_size == 0)
      scan_add_ref = 0;
    scan_add_ref += i;
    ASSERT_EQ(scan_add_ref, scan_add[i])
        << "Mismatch at work-item: " << i << " Expect: " << scan_add_ref
        << " Result: " << scan_add[i];

    ASSERT_EQ(scan_add_ref, sg_scan_add[i])
        << "Mismatch at work-item: " << i << " Expect: " << scan_add_ref
        << " Result: " << sg_scan_add[i];
  }

  iRet = clReleaseKernel(kern);
  ASSERT_OCL_SUCCESS(iRet, " clReleaseKernel");
  iRet = clReleaseKernel(kern_sg);
  ASSERT_OCL_SUCCESS(iRet, " clReleaseKernel");
  iRet = clReleaseProgram(program);
  ASSERT_OCL_SUCCESS(iRet, " clReleaseProgram");
  iRet = clReleaseMemObject(mem_obj_lid);
  ASSERT_OCL_SUCCESS(iRet, " clReleaseMemObject");
  iRet = clReleaseMemObject(mem_obj_sg_lid);
  ASSERT_OCL_SUCCESS(iRet, " clReleaseMemObject");
  iRet = clReleaseMemObject(mem_obj_scan_add);
  ASSERT_OCL_SUCCESS(iRet, " clReleaseMemObject");
  iRet = clReleaseMemObject(mem_obj_sg_scan_add);
  ASSERT_OCL_SUCCESS(iRet, " clReleaseMemObject");
}
