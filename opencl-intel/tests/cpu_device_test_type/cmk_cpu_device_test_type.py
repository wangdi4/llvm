...
TEST_TYPE_EXECUTABLE_NAME = os.getenv('EXE_DIRECTORY') + '/cpu_device_test_type' + exeSuffix
#define the test type's xml schema file name
TEST_TYPE_SCHEMA_NAME     = os.getenv('EXE_DIRECTORY') + '/cpu_device_test_type.xsd'

class XmlCPUDeviceTestCase(XMLEntity):
    """
...
class XmlCPUDeviceTest(XMLEntity):
    """
    this class implements the test-type specific requirements from its XML file.
    It inherits from XMLEntity that gives access to basic methods such as
    parsing and validating against a schema, and gives access to elements by
    ...
    def get_test_cases(self):
        ...
        for tc in test_cases:
            test_cases_list.append(XmlCPUDeviceTestCase(doc_root = tc))
...
def execute_test(exe, test_case_name):
    ...
    exp_out = outputDirectory + '/result.out'
    exp_out_file = open(exp_out,'w')

    exp_err_out = outputDirectory + '/result_err.out'
...
def main():
    ...
    data = XmlCPUDeviceTest(test_config_file, test_schema)
