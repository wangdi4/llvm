...
// Test for omp simd last private
int simd_looplastprivate(int *arr) {
  int index;
  int lpi;

#pragma omp simd lastprivate(lpi)
  for (index = 0; index < SIZE; index++) {
    lpi = arr[index];
    dummy_noop2(arr);
    arr[index] = lpi + 2;
...
__kernel void simd_lastprivate(__global int* error) {
  *error = -1;
