...
TEST_TYPE_EXECUTABLE_NAME = 'workloads_test_type.exe'
#define the test type's xml schema file name
TEST_TYPE_SCHEMA_NAME     = 'workloads_test_type.xsd'
...
def execute_test(exe, output_file_name, workload_name, workload_configuration):
	...
	# prepare the arguments list
        args = [exe, output_file_name, workload_name, '1', workload_configuration]

	print args

	exp_out = 'result_' + workload_name + '.out'
	exp_out_file = open(exp_out,'w')

	exp_err_out = 'result_err_' + workload_name + '.out'
...
def main():
	...
	for workload in data.get_workloads():
            ...
            if workload.get_file_to_copy() <> '':
                file_to_copy = mtv_env.search_for_file_in_bin_dir(workload.get_file_to_copy())
                print 'copy ' + file_to_copy + ' to ' + os.getcwd()
                shutil.copy2(file_to_copy, os.getcwd())
            test_status = 'FAIL'
