...
# Test usage of setting value for some variable types (global,local,private,volatile,regular and pointers) in gdb
# 
class TestVariableSetting(DebuggerTestCase):
    ...
    @skipNotGDB
    @expectedFailureGDB
    # Expected to fail due to ClearQuest defect CSSD100014001 (setting __local variables has no effect on program execution)
    def test_all_variable_setting(self):
    #
    # Test - some variable types (global, local, arguments, volatile, private)
