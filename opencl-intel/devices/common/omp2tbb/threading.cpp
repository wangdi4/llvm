...
public:
	fork_runner(unsigned int _n_thr, kmpc_micro _pkfn, int _gtid, int _argc, void* _argv[]) :
	  n_thr(_n_thr), pkfn(_pkfn), gtid(_gtid), argc(_argc), argv(_argv), master_slot_id(-1) {}
	...
	void operator()(void) const
	{
		...
		tbb::uneven::parallel_for(tbb::uneven::blocked_range<unsigned int>(0,n_thr,1),
			*this,
			tbb::opencl_partitioner(master_slot_id, n_thr, false)
		);
	}
};
...
extern "C" LIBOMP2TBB_API void __kmpc_fork_call (ident_t* loc, kmp_int32 argc, kmpc_micro microtask, ...)
{
	...
	// Reset MT execition control flags
	...
	if ( (entry->my_concurency > 0) && (entry->my_concurency != current_arena_concurency) ) {
		...
		fork_runner runner(entry->my_concurency, microtask, t_tgid, argc, argv);
		...
		fork_runner runner(current_arena_concurency, microtask, t_tgid, argc, argv);
		runner();
	}
