...
cl_dev_err_code FillMemObject::init(vector<COIBUFFER>& outCoiBuffsArr, vector<COI_ACCESS_FLAGS>& outAccessFlagArr)
{
    ...
    do {
		...
		{
			if ((cmdParams->offset[i] != 0) || ((fillMemObjDispatcherData.dim_count == 1) && (cmdParams->region[i] != pMemObj.dimensions.buffer_size)) ||
				((fillMemObjDispatcherData.dim_count > 1) && (cmdParams->region[i] != pMemObj.dimensions.dim[i])))
			{
...
cl_dev_err_code FillMemObject::execute()
{
	...
	do
	{
		...
		COIFUNCTION func = m_pCommandList->getDeviceFunction( DeviceServiceCommunication::FILL_MEM_OBJECT );

		// Set command type for the tracer.
		m_commandTracer.set_command_type((char*)"FillMemObject");
...
void FillMemObject::fireCallBack(void* arg)
{
