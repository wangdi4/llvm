...
namespace Intel { namespace OpenCL { namespace UtilsNative {
    ...
    class AtomicPointerNative
    {
    public:
        AtomicPointerNative(T* ptr = NULL) : m_ptr(ptr) {}
        ~AtomicPointerNative() {}
    ...
    private:
        AtomicPointerNative(const AtomicPointerNative& ac) {m_ptr = ac.m_ptr; }
        T* volatile m_ptr;
    };

    class AtomicCounterNative
    {
    public:
        AtomicCounterNative(long initVal = 0) : m_val(initVal) {}
     ...
     *
     ************************************************************************/
    class OclAutoMutexNative
    {
    public:
        OclAutoMutexNative(IMutex* mutexObj, bool bAutoLock = true);
        ~OclAutoMutexNative();
     ...
     *
     ************************************************************************/
    class OclMutexNative: public IMutex
    {
    public:
        OclMutexNative(unsigned int uiSpinCount = 4000);
        virtual ~OclMutexNative ();
        void Lock();
        void Unlock();
    protected:
        pthread_mutex_t m_mutexHndl;
    private:
        void spinCountMutexLock();
        unsigned int m_uiSpinCount;
    };

    class OclSpinMutexNative: public IMutex
    {
    public:
        OclSpinMutexNative();
        void Lock();
        void Unlock();
    protected:
        AtomicCounterNative lMutex;
        long threadId;
    };

}}}    // Intel::OpenCL::UtilsNative
