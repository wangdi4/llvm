...
/**
 * This interface class is responsible for the serilization service e.g. turn
 * the given object into blob and restore from blob;
 */
...
public:
   ...
   *
   * @returns CL_DEV_SUCCESS and pSize will be modified to the required size in
   * case of sucess CL_DEV_INVALID_VALUE in case pSize == NULL;
   * CL_DEV_ERROR_FAIL otherwise
   */
   ...
   * @param serializationType to specify the required serialization type
   * @param pKernel pointer to the kernel you want to serialize
   * @param pBlob pointer to already allocated buffer to get the serialization
   * data
   * @param bufferSize the size of the allocated blob
   *
   * @returns
   *  if bufferSize >= required Blob Size
   *      CL_DEV_SUCCESS and pBlob will get the serialized data in case of
   * sucess CL_DEV_INVALID_VALUE in case pBlob == NULL; CL_DEV_ERROR_FAIL
   * otherwise else if the bufferSize don't have the required size
   * CL_DEV_ERROR_FAIL will be returned
   */
  virtual cl_dev_err_code
   ...
   * @param pKernel pointer to the kernel which will be resotored from the
   * buffer
   * @param pBlob pointer to buffer which contains the serialization data
   * @param blobSize the size of the blob
   *
   * @returns
   *  in case of success CL_DEV_SUCCESS and pKernel will point to the restored
   * kernel else CL_DEV_ERROR_FAIL will be returned and pKernel will point to
   * NULL
   */
  virtual cl_dev_err_code
  DeSerializeProgram(cl_serialization_type serializationType,
                     ICLDevBackendProgram_ **ppProgram, const void *pBlob,
                     size_t blobSize, unsigned int binaryVersion) const = 0;
