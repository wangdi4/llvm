...
namespace DeviceBackend {
  ...
  // The back-end compiler is not required to compile the code at this point, it
  // is only expected to cache the byte code.
  //
  // Input
  //    pContainer  - A container of the program byte code to compile. The
  //                  container is expected to be persistent only during the
  //                  method call. The implementation is required to cache this
  //                  information.
  //    pProgram  - The new program object.
  // Returns
  //    CL_DEV_BE_SUCCESS if the program object is created successfully and a
  //                      valid non zero program object.
  //    Otherwise it returns a NULL program object with one of the following
  //    error values:
  //      CL_DEV_BE_INVALID_BINARY - provided binary within the container is not
  //                                 supported or binary data is broken
  //      CL_DEV_BE_OUT_OF_MEMORY  - there is not enough memory to create the
  //                                 program
  //
  // Note:
  // An important part of the IR is the required / hinted work group size per
  // kernel obtained from the front end
  // __attribute__((reqd_work_group_size(X, Y, Z))),
  // __attribute__(work_group_size_hint(X, Y, Z))
  // __attribute__((vec_type_hint (type)))
  // static cl_int CreateProgram(const cl_prog_container* IN pContainer,
  //                             ICLDevBackendProgram** OUT pProgram ) = 0;

  class ICLDevBackendKernel;
  // An interface class to an OpenCL program object provided by the Back-end
  // Compiler
  class ICLDevBackendProgram
  {
  public:
    // Builds the program, the generated program byte code is compiler specific
    // and is not necessarily the final target machine code.
    //
    // Input
    //    pOptions    - A pointer to a string that describes the build options
    //                  to be used for building the program executable.
    //
    // Returns
    //    CL_DEV_BUILD_ERROR - the last call to clBuildProgram generated errors
    //    CL_DEV_BUILD_WARNING - the last call to clBuildProgram generated
    //                           warnings
    ...
    //
    // Dasher Comments:
    //    1. Dasher performs LLVM-> Multiple DI (one per kernel)->Partial
    //       Compilation (w/o state specialization phase)
    //    2. all errors would happen during source->llvm
    //    3. we are fine with compilation errors but what about link errors?
    //    4. we need to make sure we allocate all of the resources based on the
    //       return values of the build program API (OUT_OF_MEMORY)
    ...
    // Input
    //    pSize - A pointer to the size of the log buffer. if pLog is NULL,
    //            pSize will contain the size of the buffer
    //    pLog  - A pointer to the a null terminated string containing the build
    //            log
    // Returns
    //      CL_DEV_SUCCESS        - upon success
    virtual cl_dev_err_code GetBuildLog(size_t INOUT *pSize, char* OUT pLog)
      const = 0;

    // get a container of the program
    // Input
    //    pDescriptor - A pointer to valid program binary descriptor that is
    //                  requested
    // Returns
    //    Returns a pointer to the internally stored container, if this
    //    descriptor does not exists returns NULL
    virtual cl_dev_err_code GetContainer(size_t INOUT *pSize,
                                         cl_prog_container_header* OUT
                                           pContainer) const = 0;

    // Retrieves a pointer to a kernel object by kernel name
    // Input
    //    pKernelName - A pointer to null terminated string that specified
    //                  kernel name
    // Output
    ...
    virtual cl_dev_err_code  GetKernel(const char* IN pKernelName, const
                                       ICLDevBackendKernel** OUT pKernel)
      const = 0;

    // Retrieves a vector of pointers to a function descriptors
    // User should provide a buffer for vector storage
    // Input
    //    pKernels  - A pointer to buffer that will hold pointers to kernel
    //                objects
    //    puiRetCount  - A pointer to the number of kernels which are pointer by
    //                   pKernels
    // Returns
    //    CL_DEV_SUCCESS    - if vector successfully was retrieved
    //    CL_DEV_ERROR_FAIL  - if provided buffer is not enough or one of the
    //                         parameters is invalid
    virtual cl_dev_err_code  GetAllKernels(const ICLDevBackendKernel** IN
                                           pKernels, cl_uint* INOUT puiRetCount)
      const = 0;
  ...
  public:
    ...
    // Input
    //    pArgsBuffer     - This buffer contains the list of arguments of the
    //                      kernel ( see protocol for explanation of how this is
    //                      organized )
    //    BufferSize      - indicates the size of the arguments buffer
    //    WorkDimension   - is the number of dimensions used to specify the
    //                      global work-items and work-items in the work-group.
    //                      work_dim must be greater than zero and less than or
    //                      equal to three.
    //    pGlobalWorkOffset - points to an array of work_dim unsigned values
    //                        that describe the offset of global work-items in
    //                        work_dim dimensions that will execute the kernel
    //                        function. The total number of global work-items is
    //                        computed as global_work_size[0] * … *
    //                        global_work_size[work_dim – 1].
    //    pGlobalWorkSize - points to an array of work_dim unsigned values that
    //                      describe the number of global work-items in work_dim
    //                      dimensions that will execute the kernel function.
    //                      The total number of global work-items is computed as
    //                      global_work_size[0] * … *
    //                      global_work_size[work_dim – 1].
    //    pLocalWorkSize  - points to an array of work_dim unsigned values that
    //                      describe the number of work-items that make up a
    //                      work-group (also referred to as the size of the
    //                      work-group) that will execute the kernel specified
    //                      by kernel.
    //    pBinary     - the pointer to the executable object.
    //
    // Returns
    //      CL_DEV_SUCCESS if the arguments were set successfully and a valid
    //                     non zero executable object
    //      Otherwise it returns a NULL executable object with one of the
    //      following error values:
    //      CL_DEV_OUT_OF_MEMORY  - there is not enough memory to perform the
    //      operation
    //      ... more need to fill that (TBD)
    // Notes:
    //      In Dasher this will cause final specialization of the code based on
    //      the state.
    ...
    virtual cl_dev_err_code GetKernelParams(const KernelArgument* OUT
                                            *pArgsBuffer, cl_uint* OUT ArgCount
                                            ) const = 0;

    // Returns a pointer to the kernel name
    virtual const char*  GetKernelName() const = 0;

    // Returns the size in bytes of the local memory buffer required by this
    // kernel
  ...
  public:
    ...
    //  pGlobalId - a 3 dimension array which containing the global id to work
    //              on in each dimension
    //  pLocalId - a 3 dimension array which containing the local id to work on
    //             in each dimension
    //  pItemsToProcess - a 3 dimension array which contains the number of work
    //                    items to process in each dimension
    // Returns
    ...
    // Returns the required number of memory buffers, their sizes and their
    // types
    // Input
    ...
    virtual cl_dev_err_code
      GetMemoryBuffersDescriptions(size_t* IN pBuffersSizes,
                                   cl_exec_mem_type* IN pBuffersTypes,
                                   size_t* INOUT pBufferCount ) const = 0;

    // Returns the actual number of Work Items handled by each executable
    // instance
  ...
  public:
    ...
    // Input
    //  pGroupId - a 3 dimension array which containing the group id to be
    //             executed
    //  pLocalOffset - a 3 dimension array which containing the local offset in
    //                 each dimension where to start execution
    //  pItemsToProcess - a 3 dimension array which contains the number of work
    //                    items to process in each dimension
    // Returns
    //    CL_DEV_SUCCESS - the execution completed successfully
