...
bool CompileTask::Execute() {
...

  {
    ...
    if (pIL) {
      ...
      m_pFECompiler->ParseSpirv(
          ...
          pIL->GetSpecConstIds(), pIL->GetSpecConstValues(),
      ...
      if (IsSPIR) {
        ...
        assert(szSource != NULL && "Invalid source code");
        m_pFECompiler->CompileProgram(szSource, m_uiNumHeaders, m_pszHeaders,
                                      m_pszHeadersNames, m_sOptions.c_str(),
  ...
  m_pDeviceProgram->SetBuildLogInternal("Compilation done\n");
  m_pDeviceProgram->SetBinaryInternal(uiBinarySize, pBinary.get(),
                                      CL_PROGRAM_BINARY_TYPE_COMPILED_OBJECT);
  SetComplete(CL_BUILD_SUCCESS);
  return true;
}
