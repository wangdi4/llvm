...
public:
  ...
  Allocate(const cl_context_properties *clProperties, cl_uint uiNumDevices,
           ...
           logging_fn pfnNotify, void *pUserData, cl_err_code *pclErr,
           ocl_entry_points *pOclEntryPoints, ocl_gpa_data *pGPAData,
           ContextModule &contextModule) {
    return SharedPtr<Context>(new Context(
        clProperties, uiNumDevices, uiNumRootDevices, ppDevice, pfnNotify,
        pUserData, pclErr, pOclEntryPoints, pGPAData, contextModule));
  }
   ...
   * Arguments:    clProperties [in] - context's properties
   *               uiNumDevices [in] - number of devices associated to the
   *                                   context
   *               ppDevice [in] - list of devices
   *               pfnNotify [in] - error notification function's pointer
   *               pUserData [in] - user date
   ...
   * Arguments: pProgram[inout] - pointer to the program created from a SPIR-V
   *                              module.
   *            uiSpecId[in] - id of the specialization constant whose value
   *                           will be set.
   *            szSpecSize[in] - size in bytes of the data pointed to by
   *                             spec_value.
   *            pSpecValue[in] - pointer to the memory location that contains
   *                             the value of the specialization constant.
   ...
   * Return value: CL_OUT_OF_HOST_MEMORY if not enough memory for allocate
   *                                     Program object
  ...
  CompileProgram(cl_program IN clProgram, cl_uint IN uiNumDevices,
                 const cl_device_id *IN pclDeviceList, cl_uint IN uiNumHeaders,
                 const cl_program *IN pclHeaders,
                 const char **IN pszHeadersNames, const char *IN szOptions,
                 pfnNotifyBuildDone IN pfn_notify, void *IN user_data);
  ...
  // Get a set of all root devices associated with the context - implicitly, and
  // explicitly.
   ...
   * Description: Get a device associated with the context according to the
   *              device index
  ...
  cl_err_code CreateBuffer(cl_mem_flags clFlags, size_t szSize, void *pHostPtr,
                           SharedPtr<MemoryObject> *ppBuffer);

  // create new sub buffer object
  cl_err_code
  CreateSubBuffer(SharedPtr<MemoryObject> buffer, cl_mem_flags clFlags,
  ...
  cl_err_code
  CreateImage(cl_mem_flags clFlags, const cl_image_format *pclImageFormat,
              void *pHostPtr, const size_t *szImageDims,
              const size_t *szImagePitches, SharedPtr<MemoryObject> *ppImage,
              bool bIsImageBuffer);
  ...
  cl_err_code setDestructorCallback(cl_context context,
                                    void(CL_CALLBACK *funcNotify)(cl_context,
                                                                  void *),
                                    void *userData);

  // get memory object according the mem id
  SharedPtr<MemoryObject> GetMemObject(cl_mem clMemId) {
    return m_mapMemObjects.GetOCLObject((_cl_mem_int *)clMemId)
        .StaticCast<MemoryObject>();
  }

  MemoryObject *GetMemObjectPtr(cl_mem clMemId) {
    return (MemoryObject *)(m_mapMemObjects.GetOCLObjectPtr(
        (_cl_mem_int *)clMemId));
  }

  // create new sampler object
  cl_err_code CreateSampler(cl_bool bNormalizedCoords,
  ...
  bool GetDevicesFromList(cl_uint uiNumDevices, const cl_device_id *pclDevices,
                          SharedPtr<FissionableDevice> *ppDevices);

  ocl_gpa_data *GetGPAData() const;
  /*****************************************************************************
   * Function:     NotifyError
   * Description: Report information on errors that occur in this context using
   *              the callback function registered by the application
   * Arguments: pcErrInfo [in] - pointer to an error string
   *            pPrivateInfo [in] - represent a pointer to binary data that is
   *                                returned that can be used to log additional
  ...
  Intel::OpenCL::Utils::ClHeap GetMemoryObjectsHeap(void) const {
    return m_MemObjectsHeap;
  };
   ...
   * @return an SVMBuffer that contains the address pointed to by ptr or NULL if
   * no such SVMBuffer exists
   */
  ...
  // return true only if ptr points to a location inside an SVM buffer that was
  // created by SVMAlloc
  bool IsSVMPointer(const void *ptr) const;

  cl_int SetKernelArgSVMPointer(const SharedPtr<Kernel> &pKernel,
                                cl_uint uiArgIndex, const void *pArgValue);

  cl_int SetKernelExecInfo(const SharedPtr<Kernel> &pKernel,
                           cl_kernel_exec_info paramName,
                           size_t szParamValueSize, const void *pParamValue);
  ...
  cl_err_code CreatePipe(cl_mem_flags flags, cl_uint uiPipePacketSize,
                         ...
                         SharedPtr<MemoryObject> &pPipe, void *pHostPtr);

  void *MapPipe(SharedPtr<Pipe> &pPipe, cl_map_flags flags,
                size_t requestedSize, size_t *pMappedSize, cl_err_code *pError);
  ...
  cl_err_code GetProfileDataDevice(cl_device_id device_id, cl_program program,
  ...
  bool DoesSupportUsmSharedSingle() const { return m_bSupportsUsmSharedSingle; }

  // check if cross-device shared USM allocation is supported
  bool DoesSupportUsmSharedCross() const { return m_bSupportsUsmSharedCross; }

  // check if shared system USM allocation is supported
  bool DoesSupportUsmSharedSystem() const { return m_bSupportsUsmSharedSystem; }

  /**
   * @param ptr some pointer
   * @return an USMBuffer that
   * contains the address pointed to by ptr or
   *  nullptr if no such
   * USMBuffer exists
   */
  ConstSharedPtr<USMBuffer> GetUSMBufferContainingAddr(const void *ptr) const;
  ...
  /**
   * Allocate host USM
   * @param properties an optional list of allocation properties and their
   ...
   */
  void *USMHostAlloc(const cl_mem_properties_intel *properties, size_t size,
  ...
  /**
   * Allocate USM with shared ownership between the host and the specified
   * OpenCL device.
   * @param device OpenCL device ID to associate with the allocation.
   * @param properties an optional list of allocation properties and their
   *                   corresponding values.
   ...
   * @return CL_SUCCESS if the function executes
   * successfully and
   ...
   * @param param_value_size specify the size in bytes of memory pointed to by
   *                         param_value.
   * @param param_value
   * @param param_value_size_ret
   * @return CL_SUCCESS if the function executes successfully and error codes if
   *                    failed.
   */
  cl_int GetMemAllocInfoINTEL(const void *ptr, cl_mem_info_intel param_name,
                              size_t param_value_size, void *param_value,
                              size_t *param_value_size_ret);

  /**
   * Allocate device USM to represent global variable pointer queried from
   * OpenCL program.
   * @param device OpenCL device ID to associate with the allocation.
   * @param gvSize Size of the global variable
   * @param gvPtr Pointer to the global variable queried from program.
   * @return CL_SUCCESS if the function executes successfully and error  code if
   *                    failed.
   */
  cl_int USMDeviceAllocGlobalVariable(cl_device_id device, size_t gvSize,
                                      void *gvPtr);
  /// Get backend library program.
  cl_program GetLibraryProgram() { return m_backendLibraryProgram; }

  /// Get backend library kernels.
  std::map<threadid_t, std::map<std::string, cl_kernel>> &GetLibraryKernels() {
...
protected:
  ...
  cl_err_code GetMaxImageDimensions(size_t &psz2dWidth, size_t &psz2dHeight,
                                    size_t &psz3dWidth, size_t &psz3dHeight,
                                    size_t &psz3dDepth, size_t &pszArraySize,
                                    size_t &psz1dImgBufSize);

  /**
   * Compate against UNION of all device capabilities in the context (see
   ...
   */
  cl_err_code CheckSupportedImageFormat(const cl_image_format *pclImageFormat,
                                        cl_mem_flags clMemFlags,
                                        cl_mem_object_type clObjType);

  /**
   * Calculate the supported file formats for context. UNION of all device
   * capabilities (see clGetSupportedImageFormats).
   * @param clMemFlags
   * @param clObjType
   * @return size of supported image formats list.
   */
  size_t CalculateSupportedImageFormats(const cl_mem_flags clMemFlags,
                                        cl_mem_object_type clObjType);

  cl_err_code
  CheckSupportedImageFormatByMemFlags(cl_mem_flags clFlags,
   ...
   * @param
   * properties list of allocation properties and their corresponding values.
   * @errcode_ret may return an appropriate error code.
   * @return allocation flag.
   */
  cl_mem_alloc_flags_intel
  ParseUSMAllocProperties(const cl_mem_properties_intel *properties,
                          cl_int *errcode_ret);
   ...
   * @param type USM type
   * @param
   * device OpenCL device ID to associate with the allocation.
   * @param
   * properties an optional list of allocation properties and their
   * corresponding values. The list is terminated with the special property 0.
   * @param size the size in bytes of the requested device allocation.
   ...
   * @param errcode may return an appropriate error code.
   */
  void *USMAlloc(cl_unified_shared_memory_type_intel type, cl_device_id device,
                 const cl_mem_properties_intel *properties, size_t size,
  ...
  OCLObjectsMap<_cl_device_id_int, _cl_platform_id_int>
      m_mapDevices; // holds the devices that associated to the program
  SharedPtr<Device> *m_ppExplicitRootDevices; // list of all explicit root
                                              // devices in the context
  SharedPtr<FissionableDevice> *m_ppAllDevices;
  tSetOfDevices
      m_allRootDevices; // set of all root devices implicitly/explicitly defined
                        // in the context.
  ...
  OCLObjectsMap<_cl_program_int>
      m_mapPrograms; // holds the programs that related to this context
  OCLObjectsMap<_cl_mem_int>
      m_mapMemObjects; // holds the memory objects that belongs to the context
  OCLObjectsMap<_cl_sampler_int>
      m_mapSamplers; // holds the sampler objects that belongs to the context
