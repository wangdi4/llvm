...
{
	LOG_DEBUG(L"ContextModule::CheckDevices enter. uiNumDevices=%d, pclDeviceIds=%d, ppDevices=%d", uiNumDevices, pclDeviceIds, ppDevices);
	cl_err_code clErrRet = CL_SUCCESS;
...
{
	LOG_DEBUG(L"ContextModule::GetContextInfo enter. context=%d, param_name=%d, param_value_size=%d, param_value=%d, param_value_size_ret=%d", 
...
{
	LOG_DEBUG(L"Enter SetKernelArg (clKernel=%d, uiArgIndex=%d, szArgSize=%d, pszArgValue=%d)", 
...
cl_mem ContextModule::CreateBuffer(cl_context clContext, 
								   ...
								   cl_int * pErrcodeRet)
{
	LOG_DEBUG(L"Enter CreateBuffer (clContext=%d, clFlags=%d, szSize=%d, pHostPtr=%d, pErrcodeRet=%d)", 
...
cl_mem ContextModule::CreateSubBuffer(cl_mem				clBuffer,
								   ...
								   cl_int *					pErrcodeRet)
{
	LOG_INFO(L"Enter CreateSubBuffer (clFlags=%d, cl_buffer_create_type=%d, pErrcodeRet=%d)", 
	...
	{
		LOG_ERROR(L"GetOCLObject(%d, %d) returned %ws", clBuffer, &pMemObj, ClErrTxt(clErr));
...
cl_mem ContextModule::CreateImage2D(cl_context clContext, 
									...
									cl_int * pErrcodeRet)
{
	LOG_DEBUG(L"Enter CreateImage2D (clContext=%d, clFlags=%d, clImageFormat=%d, szImageWidth=%d, szImageHeight=%d, szImageRowPitch=%d, pHostPtr=%d, pErrcodeRet=%d)", 
...
cl_mem ContextModule::CreateImage3D(cl_context clContext, 
									...
									cl_int * pErrcodeRet)
{
	LOG_DEBUG(L"Enter CreateImage3D (clContext=%d, clFlags=%d, clImageFormat=%d, szImageWidth=%d, szImageHeight=%d, szImageDepth=%d, szImageRowPitch=%d, szImageSlicePitch=%d, pHostPtr=%d, pErrcodeRet=%d)", 
...
{
	LOG_DEBUG(L"Enter RetainMemObject (clMemObj=%d)", clMemObj);
													  ...
													  void *pUserData)
{
	...
	if (CL_FAILED(clErr) || NULL == pMemObj)
	{
		LOG_ERROR(L"GetOCLObject(%d, %d) returned %ws", memObj, &pMemObj, ClErrTxt(clErr));
		return CL_INVALID_MEM_OBJECT;
	}
...
cl_sampler ContextModule::CreateSampler(cl_context clContext, 
										...
										cl_int * pErrcodeRet)
{
	LOG_DEBUG(L"Enter CreateSampler (clContext=%d, bNormalizedCoords=%d, clAddressingMode=%d, clFilterMode=%d, pErrcodeRet=%d)", 
...
{
	LOG_DEBUG(L"Enter RetainSampler (clSampler=%d)", clSampler);
...
{
	LOG_DEBUG(L"Enter RetainMemObject (clMemObj=%d)", clSampler);
...
cl_mem ContextModule::CreateFromGLBuffer(cl_context clContext, 
						  ...
						  int * pErrcodeRet)
{
	LOG_DEBUG(L"Enter CreateFromGLBuffer (clContext=%d, clFlags=%d, pErrcodeRet=%d)", 
	...
	if (CL_FAILED(clErr))
	{
		LOG_ERROR(L"pGLContext->CreateGLBuffer(%d, %d, %d, %d) = %ws", clMemFlags, glBufObj, &pBuffer, ClErrTxt(clErr))
		if (NULL != pErrcodeRet)
		{
			*pErrcodeRet = CL_ERR_OUT(clErr);
		}
...
cl_mem ContextModule::CreateFromGLTexture2D(cl_context clContext, 
							 ...
							 cl_int * pErrcodeRet)
{
	LOG_DEBUG(L"Enter CreateFromGLTexture2D (clContext=%d, clFlags=%d, glTextureTarget=%d, glMipLevel=%d, glTexture=%, pErrcodeRet=%d)", 
	...
	if (CL_FAILED(clErr))
	{
		LOG_ERROR(L"pGLContext->CreateGLTexture2D(%d, %d, %d, %d, %d) = %ws", clMemFlags, glTextureTarget, glMipLevel, glTexture,
	...
	if (CL_FAILED(clErr))
	{
		LOG_ERROR(L"m_pMemObjects->AddObject(%d, %d, false) = %ws", pMemObj, pMemObj->GetHandle(), ClErrTxt(clErr))
			if (NULL != pErrcodeRet)
			{
				*pErrcodeRet = CL_ERR_OUT(clErr);
			}
...
cl_mem ContextModule::CreateFromGLTexture3D(cl_context clContext, 
							 ...
							 cl_int * pErrcodeRet)
{
	LOG_DEBUG(L"Enter CreateFromGLTexture3D (clContext=%d, clFlags=%d, glTextureTarget=%d, glMipLevel=%d, glTexture=%, pErrcodeRet=%d)", 
		clContext, clMemFlags, glTextureTarget, glMipLevel, glTexture, pErrcodeRet);
	...
	if (CL_FAILED(clErr))
	{
		LOG_ERROR(L"pGLContext->CreateGLTexture3D(%d, %d, %d, %d, %d) = %ws", clMemFlags, glTextureTarget, glMipLevel, glTexture,
			&pMemObj, ClErrTxt(clErr))
		if (NULL != pErrcodeRet)
		{
			*pErrcodeRet = CL_ERR_OUT(clErr);
...
cl_mem ContextModule::CreateFromGLRenderbuffer(cl_context clContext, 
								...
								cl_int * pErrcodeRet)
{
	LOG_DEBUG(L"Enter CreateFromGLRenderbuffer (clContext=%d, clFlags=%d, glRenderBuffer=%d, pErrcodeRet=%d)", 
	...
	if (CL_FAILED(clErr))
	{
		LOG_ERROR(L"pGLContext->CreateGLRenderBuffer(%d, %d, %d) = %ws", clMemFlags, glRenderBuffer, &pMemObj, ClErrTxt(clErr))
			if (NULL != pErrcodeRet)
			{
				*pErrcodeRet = CL_ERR_OUT(clErr);
			}
...
{
	LOG_DEBUG(L"Enter GetGLObjectInfo (clMemObj=%d, pglObjectType=%d, pglObjectName=%d)", 
	...
	{
		LOG_ERROR(L"Object %d isn't a valid object", clMemObj);
		return CL_INVALID_MEM_OBJECT;
	}
...
cl_int ContextModule::GetGLTextureInfo(cl_mem clMemObj, 
									   ...
									   size_t * pszParamValueSizeRet)
{
	LOG_DEBUG(L"Enter GetGLTextureInfo (clMemObj=%d, cl_gl_texture_info=%d)", 
