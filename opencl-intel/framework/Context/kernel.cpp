...
DeviceKernel::DeviceKernel(Kernel *pKernel,
                           ...
                           LoggerClient *pLoggerClient, cl_err_code *pErr)
    : OCLObjectBase("DeviceKernel"), m_clDevKernel(CL_INVALID_HANDLE),
      m_pKernel(pKernel), m_pDevice(pDevice), m_CL_KERNEL_WORK_GROUP_SIZE(0),
      m_CL_KERNEL_LOCAL_MEM_SIZE(0),
...
bool DeviceKernel::CacheRequiredInfo() {
  ...
  err = pDevAgent->clDevGetKernelInfo(m_clDevKernel, CL_DEV_KERNEL_MAX_WG_SIZE,
                                      0, nullptr,
                                      sizeof(m_CL_KERNEL_WORK_GROUP_SIZE),
                                      &m_CL_KERNEL_WORK_GROUP_SIZE, nullptr);
...
cl_err_code Kernel::SetKernelArgumentInfo(const DeviceKernel *pDeviceKernel) {
  ...
  const ConstSharedPtr<FrontEndCompiler> &pFECompiler =
      pDevice->GetRootDevice()->GetFrontEndCompiler();
  ...
  if ((NULL != pFECompiler.GetPtr()) && (NULL != pBin)) {
    // First use Front-end compiler for the information
    ClangFE::IOCLFEKernelArgInfo *pArgsInfo;
    clErrCode = pFECompiler->GetKernelArgInfo(
        pBin, uiBinSize, m_sKernelPrototype.m_szKernelName.c_str(), &pArgsInfo);
    if (CL_FAILED(clErrCode)) {
      // If no kernel arg info, so just ignore.
      // Otherwise, free internal data
      // [OpenCL2.1 Development] Temporary disable assert below due to in case
      // of SPIRV FE compiler returns CL_OUT_OF_HOST_MEMORY instead of
      // CL_KERNEL_ARG_INFO_NOT_AVAILABLE.
      // assert(  CL_KERNEL_ARG_INFO_NOT_AVAILABLE == clErrCode && "other
      // codes indicates logical errors and should never occure");
