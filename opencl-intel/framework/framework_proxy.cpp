...
void FrameworkProxy::InitOCLEntryPoints() {
  ...
  /// ICD functions
  ICDDispatchTable.clGetPlatformIDs =
      (cl_api_clGetPlatformIDs)GET_ALIAS(clGetPlatformIDs);
  ICDDispatchTable.clGetPlatformInfo =
      (cl_api_clGetPlatformInfo)GET_ALIAS(clGetPlatformInfo);
  ICDDispatchTable.clGetDeviceIDs =
      (cl_api_clGetDeviceIDs)GET_ALIAS(clGetDeviceIDs);
  ICDDispatchTable.clGetDeviceInfo =
      (cl_api_clGetDeviceInfo)GET_ALIAS(clGetDeviceInfo);
  ICDDispatchTable.clCreateContext =
      (cl_api_clCreateContext)GET_ALIAS(clCreateContext);
  ICDDispatchTable.clCreateContextFromType =
      (cl_api_clCreateContextFromType)GET_ALIAS(clCreateContextFromType);
  ...
  ICDDispatchTable.clRetainContext =
      (cl_api_clRetainContext)GET_ALIAS(clRetainContext);
  ICDDispatchTable.clReleaseContext =
      (cl_api_clReleaseContext)GET_ALIAS(clReleaseContext);
  ICDDispatchTable.clGetContextInfo =
      (cl_api_clGetContextInfo)GET_ALIAS(clGetContextInfo);
  ICDDispatchTable.clCreateCommandQueue =
      (cl_api_clCreateCommandQueue)GET_ALIAS(clCreateCommandQueue);
  ICDDispatchTable.clCreateCommandQueueWithProperties =
      (cl_api_clCreateCommandQueueWithProperties)GET_ALIAS(
          clCreateCommandQueueWithProperties);
  ICDDispatchTable.clRetainCommandQueue =
      (cl_api_clRetainCommandQueue)GET_ALIAS(clRetainCommandQueue);
  ICDDispatchTable.clReleaseCommandQueue =
      (cl_api_clReleaseCommandQueue)GET_ALIAS(clReleaseCommandQueue);
  ICDDispatchTable.clGetCommandQueueInfo =
      (cl_api_clGetCommandQueueInfo)GET_ALIAS(clGetCommandQueueInfo);
  ICDDispatchTable.clSetCommandQueueProperty = nullptr;
  ICDDispatchTable.clCreateBuffer =
      (cl_api_clCreateBuffer)GET_ALIAS(clCreateBuffer);
  ICDDispatchTable.clCreateBufferWithProperties =
      (cl_api_clCreateBufferWithProperties)GET_ALIAS(
          clCreateBufferWithProperties);
  ICDDispatchTable.clCreateImage =
      (cl_api_clCreateImage)GET_ALIAS(clCreateImage);
  ICDDispatchTable.clCreateImageWithProperties =
      (cl_api_clCreateImageWithProperties)GET_ALIAS(
          clCreateImageWithProperties);
  ICDDispatchTable.clCreateImage2D =
      (cl_api_clCreateImage2D)GET_ALIAS(clCreateImage2D);
  ICDDispatchTable.clCreateImage3D =
      (cl_api_clCreateImage3D)GET_ALIAS(clCreateImage3D);
  ICDDispatchTable.clRetainMemObject =
      (cl_api_clRetainMemObject)GET_ALIAS(clRetainMemObject);
  ICDDispatchTable.clReleaseMemObject =
      (cl_api_clReleaseMemObject)GET_ALIAS(clReleaseMemObject);
  ICDDispatchTable.clGetSupportedImageFormats =
      (cl_api_clGetSupportedImageFormats)GET_ALIAS(clGetSupportedImageFormats);
  ICDDispatchTable.clGetMemObjectInfo =
      (cl_api_clGetMemObjectInfo)GET_ALIAS(clGetMemObjectInfo);
  ICDDispatchTable.clGetImageInfo =
      (cl_api_clGetImageInfo)GET_ALIAS(clGetImageInfo);
  ICDDispatchTable.clCreateSampler =
      (cl_api_clCreateSampler)GET_ALIAS(clCreateSampler);
  ICDDispatchTable.clCreateSamplerWithProperties =
      (cl_api_clCreateSamplerWithProperties)GET_ALIAS(
          clCreateSamplerWithProperties);
  ICDDispatchTable.clRetainSampler =
      (cl_api_clRetainSampler)GET_ALIAS(clRetainSampler);
  ICDDispatchTable.clReleaseSampler =
      (cl_api_clReleaseSampler)GET_ALIAS(clReleaseSampler);
  ICDDispatchTable.clGetSamplerInfo =
      (cl_api_clGetSamplerInfo)GET_ALIAS(clGetSamplerInfo);
  ICDDispatchTable.clCreateProgramWithSource =
      (cl_api_clCreateProgramWithSource)GET_ALIAS(clCreateProgramWithSource);
  ICDDispatchTable.clSetDefaultDeviceCommandQueue =
      (cl_api_clSetDefaultDeviceCommandQueue)GET_ALIAS(
          clSetDefaultDeviceCommandQueue);
  ICDDispatchTable.clCreateProgramWithBinary =
      (cl_api_clCreateProgramWithBinary)GET_ALIAS(clCreateProgramWithBinary);
  ICDDispatchTable.clCreateProgramWithBuiltInKernels =
      (cl_api_clCreateProgramWithBuiltInKernels)GET_ALIAS(
          clCreateProgramWithBuiltInKernels);
  ICDDispatchTable.clCreateProgramWithIL =
      (cl_api_clCreateProgramWithIL)GET_ALIAS(clCreateProgramWithIL);
  ICDDispatchTable.clRetainProgram =
      (cl_api_clRetainProgram)GET_ALIAS(clRetainProgram);
  ICDDispatchTable.clReleaseProgram =
      (cl_api_clReleaseProgram)GET_ALIAS(clReleaseProgram);
  ICDDispatchTable.clBuildProgram =
      (cl_api_clBuildProgram)GET_ALIAS(clBuildProgram);
  ICDDispatchTable.clCompileProgram =
      (cl_api_clCompileProgram)GET_ALIAS(clCompileProgram);
  ICDDispatchTable.clLinkProgram =
      (cl_api_clLinkProgram)GET_ALIAS(clLinkProgram);
  ICDDispatchTable.clUnloadCompiler =
      (cl_api_clUnloadCompiler)GET_ALIAS(clUnloadCompiler);
  ICDDispatchTable.clUnloadPlatformCompiler =
      (cl_api_clUnloadPlatformCompiler)GET_ALIAS(clUnloadPlatformCompiler);
  ICDDispatchTable.clGetProgramInfo =
      (cl_api_clGetProgramInfo)GET_ALIAS(clGetProgramInfo);
  ICDDispatchTable.clGetProgramBuildInfo =
      (cl_api_clGetProgramBuildInfo)GET_ALIAS(clGetProgramBuildInfo);
  ICDDispatchTable.clCreateKernel =
      (cl_api_clCreateKernel)GET_ALIAS(clCreateKernel);
  ICDDispatchTable.clCreateKernelsInProgram =
      (cl_api_clCreateKernelsInProgram)GET_ALIAS(clCreateKernelsInProgram);
  ICDDispatchTable.clRetainKernel =
      (cl_api_clRetainKernel)GET_ALIAS(clRetainKernel);
  ICDDispatchTable.clReleaseKernel =
      (cl_api_clReleaseKernel)GET_ALIAS(clReleaseKernel);
  ICDDispatchTable.clSetKernelArg =
      (cl_api_clSetKernelArg)GET_ALIAS(clSetKernelArg);
  ICDDispatchTable.clGetKernelInfo =
      (cl_api_clGetKernelInfo)GET_ALIAS(clGetKernelInfo);
  ICDDispatchTable.clCloneKernel =
      (cl_api_clCloneKernel)GET_ALIAS(clCloneKernel);
  ICDDispatchTable.clGetHostTimer =
      (cl_api_clGetHostTimer)GET_ALIAS(clGetHostTimer);
  ICDDispatchTable.clGetDeviceAndHostTimer =
      (cl_api_clGetDeviceAndHostTimer)GET_ALIAS(clGetDeviceAndHostTimer);
  ICDDispatchTable.clGetKernelWorkGroupInfo =
      (cl_api_clGetKernelWorkGroupInfo)GET_ALIAS(clGetKernelWorkGroupInfo);
  ICDDispatchTable.clGetKernelSubGroupInfo =
      (cl_api_clGetKernelSubGroupInfo)GET_ALIAS(clGetKernelSubGroupInfo);
  ICDDispatchTable.clGetKernelSubGroupInfoKHR =
      (cl_api_clGetKernelSubGroupInfoKHR)GET_ALIAS(clGetKernelSubGroupInfoKHR);
  ICDDispatchTable.clWaitForEvents =
      (cl_api_clWaitForEvents)GET_ALIAS(clWaitForEvents);
  ICDDispatchTable.clGetEventInfo =
      (cl_api_clGetEventInfo)GET_ALIAS(clGetEventInfo);
  ICDDispatchTable.clRetainEvent =
      (cl_api_clRetainEvent)GET_ALIAS(clRetainEvent);
  ICDDispatchTable.clReleaseEvent =
      (cl_api_clReleaseEvent)GET_ALIAS(clReleaseEvent);
  ICDDispatchTable.clGetEventProfilingInfo =
      (cl_api_clGetEventProfilingInfo)GET_ALIAS(clGetEventProfilingInfo);
  ICDDispatchTable.clFlush = (cl_api_clFlush)GET_ALIAS(clFlush);
  ICDDispatchTable.clFinish = (cl_api_clFinish)GET_ALIAS(clFinish);
  ICDDispatchTable.clEnqueueReadBuffer =
      (cl_api_clEnqueueReadBuffer)GET_ALIAS(clEnqueueReadBuffer);
  ICDDispatchTable.clEnqueueWriteBuffer =
      (cl_api_clEnqueueWriteBuffer)GET_ALIAS(clEnqueueWriteBuffer);
  ICDDispatchTable.clEnqueueCopyBuffer =
      (cl_api_clEnqueueCopyBuffer)GET_ALIAS(clEnqueueCopyBuffer);
  ICDDispatchTable.clEnqueueFillBuffer =
      (cl_api_clEnqueueFillBuffer)GET_ALIAS(clEnqueueFillBuffer);
  ICDDispatchTable.clEnqueueFillImage =
      (cl_api_clEnqueueFillImage)GET_ALIAS(clEnqueueFillImage);
  ICDDispatchTable.clEnqueueReadImage =
      (cl_api_clEnqueueReadImage)GET_ALIAS(clEnqueueReadImage);
  ICDDispatchTable.clEnqueueWriteImage =
      (cl_api_clEnqueueWriteImage)GET_ALIAS(clEnqueueWriteImage);
  ICDDispatchTable.clEnqueueCopyImage =
      (cl_api_clEnqueueCopyImage)GET_ALIAS(clEnqueueCopyImage);
  ICDDispatchTable.clEnqueueCopyImageToBuffer =
      (cl_api_clEnqueueCopyImageToBuffer)GET_ALIAS(clEnqueueCopyImageToBuffer);
  ICDDispatchTable.clEnqueueCopyBufferToImage =
      (cl_api_clEnqueueCopyBufferToImage)GET_ALIAS(clEnqueueCopyBufferToImage);
  ICDDispatchTable.clEnqueueMapBuffer =
      (cl_api_clEnqueueMapBuffer)GET_ALIAS(clEnqueueMapBuffer);
  ICDDispatchTable.clEnqueueMapImage =
      (cl_api_clEnqueueMapImage)GET_ALIAS(clEnqueueMapImage);
  ICDDispatchTable.clEnqueueUnmapMemObject =
      (cl_api_clEnqueueUnmapMemObject)GET_ALIAS(clEnqueueUnmapMemObject);
  ICDDispatchTable.clEnqueueNDRangeKernel =
      (cl_api_clEnqueueNDRangeKernel)GET_ALIAS(clEnqueueNDRangeKernel);
  ICDDispatchTable.clEnqueueTask =
      (cl_api_clEnqueueTask)GET_ALIAS(clEnqueueTask);
  ICDDispatchTable.clEnqueueNativeKernel =
      (cl_api_clEnqueueNativeKernel)GET_ALIAS(clEnqueueNativeKernel);
  ICDDispatchTable.clEnqueueMarker =
      (cl_api_clEnqueueMarker)GET_ALIAS(clEnqueueMarker);
  ICDDispatchTable.clEnqueueMarkerWithWaitList =
      (cl_api_clEnqueueMarkerWithWaitList)GET_ALIAS(
          clEnqueueMarkerWithWaitList);
  ICDDispatchTable.clEnqueueBarrierWithWaitList =
      (cl_api_clEnqueueBarrierWithWaitList)GET_ALIAS(
          clEnqueueBarrierWithWaitList);
  ICDDispatchTable.clEnqueueWaitForEvents =
      (cl_api_clEnqueueWaitForEvents)GET_ALIAS(clEnqueueWaitForEvents);
  ICDDispatchTable.clEnqueueBarrier =
      (cl_api_clEnqueueBarrier)GET_ALIAS(clEnqueueBarrier);
  ICDDispatchTable.clGetExtensionFunctionAddress =
      (cl_api_clGetExtensionFunctionAddress)GET_ALIAS(
          clGetExtensionFunctionAddress);
  ICDDispatchTable.clGetExtensionFunctionAddressForPlatform =
      (cl_api_clGetExtensionFunctionAddressForPlatform)GET_ALIAS(
  ...
  ICDDispatchTable.clSetEventCallback =
      (cl_api_clSetEventCallback)GET_ALIAS(clSetEventCallback);
  ICDDispatchTable.clCreateSubBuffer =
      (cl_api_clCreateSubBuffer)GET_ALIAS(clCreateSubBuffer);
  ICDDispatchTable.clSetMemObjectDestructorCallback =
      (cl_api_clSetMemObjectDestructorCallback)GET_ALIAS(
          clSetMemObjectDestructorCallback);
  ICDDispatchTable.clCreateUserEvent =
      (cl_api_clCreateUserEvent)GET_ALIAS(clCreateUserEvent);
  ICDDispatchTable.clSetUserEventStatus =
      (cl_api_clSetUserEventStatus)GET_ALIAS(clSetUserEventStatus);
  ICDDispatchTable.clEnqueueReadBufferRect =
      (cl_api_clEnqueueReadBufferRect)GET_ALIAS(clEnqueueReadBufferRect);
  ICDDispatchTable.clEnqueueWriteBufferRect =
      (cl_api_clEnqueueWriteBufferRect)GET_ALIAS(clEnqueueWriteBufferRect);
  ICDDispatchTable.clEnqueueCopyBufferRect =
      (cl_api_clEnqueueCopyBufferRect)GET_ALIAS(clEnqueueCopyBufferRect);
  ICDDispatchTable.clEnqueueMigrateMemObjects =
      (cl_api_clEnqueueMigrateMemObjects)GET_ALIAS(clEnqueueMigrateMemObjects);
  ICDDispatchTable.clCreateSubDevices =
      (cl_api_clCreateSubDevices)GET_ALIAS(clCreateSubDevices);
  ICDDispatchTable.clRetainDevice =
      (cl_api_clRetainDevice)GET_ALIAS(clRetainDevice);
  ICDDispatchTable.clReleaseDevice =
      (cl_api_clReleaseDevice)GET_ALIAS(clReleaseDevice);
  ICDDispatchTable.clGetKernelArgInfo =
      (cl_api_clGetKernelArgInfo)GET_ALIAS(clGetKernelArgInfo);
  ...
  ICDDispatchTable.clEnqueueSVMFree =
      (cl_api_clEnqueueSVMFree)GET_ALIAS(clEnqueueSVMFree);
  ICDDispatchTable.clEnqueueSVMMemcpy =
      (cl_api_clEnqueueSVMMemcpy)GET_ALIAS(clEnqueueSVMMemcpy);
  ICDDispatchTable.clEnqueueSVMMemFill =
      (cl_api_clEnqueueSVMMemFill)GET_ALIAS(clEnqueueSVMMemFill);
  ICDDispatchTable.clEnqueueSVMMap =
      (cl_api_clEnqueueSVMMap)GET_ALIAS(clEnqueueSVMMap);
  ICDDispatchTable.clEnqueueSVMMigrateMem =
      (cl_api_clEnqueueSVMMigrateMem)GET_ALIAS(clEnqueueSVMMigrateMem);
  ICDDispatchTable.clEnqueueSVMUnmap =
      (cl_api_clEnqueueSVMUnmap)GET_ALIAS(clEnqueueSVMUnmap);
  ICDDispatchTable.clSetKernelArgSVMPointer =
      (cl_api_clSetKernelArgSVMPointer)GET_ALIAS(clSetKernelArgSVMPointer);
  ICDDispatchTable.clSetKernelExecInfo =
      (cl_api_clSetKernelExecInfo)GET_ALIAS(clSetKernelExecInfo);

  ICDDispatchTable.clCreatePipe = (cl_api_clCreatePipe)GET_ALIAS(clCreatePipe);
  ICDDispatchTable.clGetPipeInfo =
      (cl_api_clGetPipeInfo)GET_ALIAS(clGetPipeInfo);

  ICDDispatchTable.clSetProgramSpecializationConstant =
      (cl_api_clSetProgramSpecializationConstant)GET_ALIAS(
          clSetProgramSpecializationConstant);

  /// Extra functions for Common Runtime
  CRTDispatchTable.clGetKernelArgInfo =
      (cl_api_clGetKernelArgInfo)GET_ALIAS(clGetKernelArgInfo);
  ...
  CRTDispatchTable.clCreatePipeINTEL =
      (INTELpfn_clCreatePipeINTEL)GET_ALIAS(clCreatePipeINTEL);
