...
Command::Command(const SharedPtr<IOclCommandQueueBase> &cmdQueue)
    : m_Event(QueueEvent::Allocate(cmdQueue)), m_clDevCmdListId(0),
      m_pDevice(nullptr), m_pCommandQueue(cmdQueue), m_returnCode(CL_SUCCESS),
...
Command::~Command() {
  ...
  if (m_bEventDetached) {
    ...
    // In this case Command is being deleted explicitly, not through the event,
    // so we need to free the event ourselves.
    m_Event = nullptr;
  }
...
void Command::GPA_DestroyCommand() {
  ...
  if ((nullptr != pGPAData) && (pGPAData->bUseGPA) &&
      (pGPAData->bEnableContextTracing)) {
    // not an error, the internal m_pGpaCommand depends on global pGPAData is
    // active.
...
CopyBufferCommand::CopyBufferCommand(
    ...
    : CopyMemObjCommand(cmdQueue, pOclEntryPoints, pSrcBuffer, pDstBuffer,
...
CopyBufferRectCommand::CopyBufferRectCommand(
    ...
    : CopyMemObjCommand(cmdQueue, pOclEntryPoints, pSrcBuffer, pDstBuffer,
                        pszSrcOrigin, pszDstOrigin, pszRegion, szSrcRowPitch,
                        szSrcSlicePitch, szDstRowPitch, szDstSlicePitch) {
...
CopyImageCommand::CopyImageCommand(
    ...
    const SharedPtr<MemoryObject> &pDstImage, const size_t *pszSrcOrigin,
    const size_t *pszDstOrigin, const size_t *pszRegion)
    : CopyMemObjCommand(cmdQueue, pOclEntryPoints, pSrcImage, pDstImage,
                        pszSrcOrigin, pszDstOrigin, pszRegion) {
...
CopyBufferToImageCommand::CopyBufferToImageCommand(
    ...
    : CopyMemObjCommand(cmdQueue, pOclEntryPoints, pSrcBuffer, pDstImage,
                        pszSrcOffset, pszDstOrigin, pszDstRegion) {
...
CopyImageToBufferCommand::CopyImageToBufferCommand(
    ...
    : CopyMemObjCommand(cmdQueue, pOclEntryPoints, pSrcImage, pDstBuffer,
                        pszSrcOrigin, pszDstOffset, pszSrcRegion) {
...
MapBufferCommand::MapBufferCommand(
    ...
    : MapMemObjCommand(cmdQueue, pOclEntryPoints, pBuffer, clMapFlags, nullptr,
...
MapImageCommand::MapImageCommand(
    const SharedPtr<IOclCommandQueueBase> &cmdQueue,
    ocl_entry_points *pOclEntryPoints, const SharedPtr<MemoryObject> &pImage,
    cl_map_flags clMapFlags, const size_t *pOrigin, const size_t *pRegion,
    size_t *pszImageRowPitch, size_t *pszImageSlicePitch)
    : MapMemObjCommand(cmdQueue, pOclEntryPoints, pImage, clMapFlags, pOrigin,
                       pRegion, pszImageRowPitch, pszImageSlicePitch) {
...
MapMemObjCommand::MapMemObjCommand(
    const SharedPtr<IOclCommandQueueBase> &cmdQueue,
    ocl_entry_points *pOclEntryPoints, const SharedPtr<MemoryObject> &pMemObj,
    cl_map_flags clMapFlags, const size_t *pOrigin, const size_t *pRegion,
    size_t *pszImageRowPitch, size_t *pszImageSlicePitch)
...
ReadBufferCommand::ReadBufferCommand(
    const SharedPtr<IOclCommandQueueBase> &cmdQueue,
    ocl_entry_points *pOclEntryPoints, const SharedPtr<MemoryObject> &pBuffer,
    const size_t pszOffset[3], const size_t pszCb[3], void *pDst)
    : ReadMemObjCommand(cmdQueue, pOclEntryPoints, pBuffer, pszOffset, pszCb, 0,
                        0, pDst) {
...
ReadBufferRectCommand::ReadBufferRectCommand(
    ...
    : ReadMemObjCommand(cmdQueue, pOclEntryPoints, pBuffer, szBufferOrigin,
                        szRegion, szBufferRowPitch, szBufferSlicePitch, pDst,
                        szDstOrigin, szDstRowPitch, szDstSlicePitch) {
...
ReadImageCommand::ReadImageCommand(
    ...
    : ReadMemObjCommand(cmdQueue, pOclEntryPoints, pImage, pszOrigin, pszRegion,
                        0, 0, pDst, nullptr, szRowPitch, szSlicePitch) {
 ...
 *
 ******************************************************************/
TaskCommand::TaskCommand(const SharedPtr<IOclCommandQueueBase> &cmdQueue,
                         ocl_entry_points *pOclEntryPoints,
                         const SharedPtr<Kernel> &pKernel)
    : NDRangeKernelCommand(cmdQueue, pOclEntryPoints, pKernel, 1, nullptr,
...
WriteBufferCommand::WriteBufferCommand(
    ...
    const SharedPtr<MemoryObject> &pBuffer, const size_t *pszOffset,
    const size_t *pszCb, const void *cpSrc)
    : WriteMemObjCommand(cmdQueue, pOclEntryPoints, bBlocking, pBuffer,
                         pszOffset, pszCb, 0, 0, cpSrc) {
...
FillBufferCommand::FillBufferCommand(
    ...
    : FillMemObjCommand(cmdQueue, pOclEntryPoints, pBuffer, offset, size,
...
FillImageCommand::FillImageCommand(
    const SharedPtr<IOclCommandQueueBase> &cmdQueue,
    ocl_entry_points *pOclEntryPoints, const SharedPtr<MemoryObject> &pImg,
    const void *pattern, size_t pattern_size, const cl_uint num_of_dimms,
    const size_t *offset, const size_t *size)
    : FillMemObjCommand(cmdQueue, pOclEntryPoints, pImg, offset, size,
                        num_of_dimms, pattern, pattern_size) {
...
WriteBufferRectCommand::WriteBufferRectCommand(
    ...
    : WriteMemObjCommand(cmdQueue, pOclEntryPoints, bBlocking, pBuffer,
                         szBufferOrigin, szRegion, szBufferRowPitch,
                         szBufferSlicePitch, pDst, szSrcOrigin, szDstRowPitch,
                         szDstSlicePitch) {
...
WriteImageCommand::WriteImageCommand(
    const SharedPtr<IOclCommandQueueBase> &cmdQueue,
    ocl_entry_points *pOclEntryPoints, cl_bool bBlocking,
    const SharedPtr<MemoryObject> &pImage, const size_t *pszOrigin,
    const size_t *pszRegion, size_t szRowPitch, size_t szSlicePitch,
    const void *cpSrc)
    : WriteMemObjCommand(cmdQueue, pOclEntryPoints, bBlocking, pImage,
                         pszOrigin, pszRegion, 0, 0, cpSrc, nullptr, szRowPitch,
                         szSlicePitch) {
...
MigrateSVMMemCommand::MigrateSVMMemCommand(
    ...
    const void **pMemObjects, const size_t *sizes)
    : Command(cmdQueue), m_pMemObjects(pMemObjects), m_pSizes(sizes) {
...
AdviseUSMMemCommand::AdviseUSMMemCommand(
    const SharedPtr<IOclCommandQueueBase> &cmdQueue, const void *ptr,
...
PrePostFixRuntimeCommand::PrePostFixRuntimeCommand(
    Command *relatedUserCommand, Mode working_mode,
    const SharedPtr<IOclCommandQueueBase> &cmdQueue)
    : RuntimeCommand(cmdQueue), m_relatedUserCommand(relatedUserCommand),
      m_working_mode(working_mode), m_force_error_return(CL_SUCCESS),
