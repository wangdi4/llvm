 ...
 *
 ******************************************************************/
ExecutionModule::~ExecutionModule() { RELEASE_LOGGER_CLIENT; }
 ...
 *
 ******************************************************************/
cl_err_code ExecutionModule::SetCommandQueueProperty(
    cl_command_queue clCommandQueue, cl_command_queue_properties clProperties,
    cl_bool bEnable, cl_command_queue_properties *pclOldProperties) {
  SharedPtr<OclCommandQueue> pOclCommandQueue = GetCommandQueue(clCommandQueue);
  if (NULL == pOclCommandQueue.GetPtr()) {
    return CL_INVALID_COMMAND_QUEUE;
  }
  ...
  if ((0 == clProperties) && (CL_TRUE == bEnable)) {
    ...
    cl_command_queue_properties mask =
...
cl_err_code ExecutionModule::EnqueueFillBuffer(
  ...
  if (NULL == pattern || 0 >= pattern_size || 128 < pattern_size ||
      (pattern_size % 3 != 0 && !IsPowerOf2(pattern_size)) ||
      (pattern_size % 3 == 0 &&
       (pattern_size > sizeof(cl_long3) || !IsPowerOf2(pattern_size / 3)))) {
    return CL_INVALID_VALUE;
  }
...
cl_err_code ExecutionModule::EnqueueCopyBuffer(
  ...
  if (clSrcBuffer == clDstBuffer) {
    // Check overlapping
    if (CheckMemoryObjectOverlapping(pSrcBuffer, pszSrcOrigin, pszDstOrigin,
                                     pszRegion)) {
...
cl_err_code ExecutionModule::EnqueueCopyBufferRect(
  ...
  if (CheckIfAnyDimIsZero(region) ||
      ...
      (src_buffer_slice_pitch != 0 &&
       src_buffer_slice_pitch < (region[1] * src_buffer_row_pitch)) ||
      (dst_buffer_slice_pitch != 0 &&
       dst_buffer_slice_pitch < (region[1] * dst_buffer_row_pitch))) {
  ...
  if (CL_SUCCESS != (errVal = pSrcBuffer->CheckBoundsRect(
                         szSrcOrigin, region, src_buffer_row_pitch,
                         src_buffer_slice_pitch)) ||
      CL_SUCCESS != (errVal = pDstBuffer->CheckBoundsRect(
                         szDstOrigin, region, dst_buffer_row_pitch,
                         dst_buffer_slice_pitch)))
  ...
  if (clSrcBuffer == clDstBuffer) {
    // Check overlapping
    if (CheckMemoryObjectOverlapping(pSrcBuffer, szSrcOrigin, szDstOrigin,
                                     region)) {
      // Rami todo
...
cl_err_code ExecutionModule::EnqueueFillImage(
  ...
  errVal = img->GetImageInfo(CL_IMAGE_FORMAT, sizeof(cl_image_format), &format,
                             NULL);
  ...
  errVal =
      img->GetImageInfo(CL_IMAGE_ARRAY_SIZE, sizeof(size_t), &dim_sz, NULL);
  if (CL_SUCCESS == errVal) {
    if (dim_sz)
      ++img_dim_count;
  } else
...
cl_err_code ExecutionModule::EnqueueNDRangeKernel(
  ...
  const SharedPtr<FissionableDevice> &pDevice =
      pCommandQueue->GetDefaultDevice();
...
cl_err_code ExecutionModule::EnqueueCopyImage(
  ...
  if (clSrcImage == clDstImage) {
    if (CheckMemoryObjectOverlapping(pSrcImage, szSrcOrigin, szDstOrigin,
                                     szRegion)) {
      return CL_MEM_COPY_OVERLAP;
    }
  }
  if (!AreImageDimsSupportedByDevice(*pSrcImage,
                                     *pCommandQueue->GetDefaultDevice()) ||
      !AreImageDimsSupportedByDevice(*pDstImage,
                                     *pCommandQueue->GetDefaultDevice())) {
    return CL_INVALID_IMAGE_SIZE;
  }
  if (CL_SUCCESS != (errVal = CheckImageFormatSupportedByDevice(
                         *pCommandQueue->GetDefaultDevice(), *pSrcImage))) {
    return errVal;
  }
  if (CL_SUCCESS != (errVal = CheckImageFormatSupportedByDevice(
                         *pCommandQueue->GetDefaultDevice(), *pDstImage))) {
    return errVal;
  }
...
cl_err_code ExecutionModule::EnqueueCopyImageToBuffer(
  ...
  // pDstBuffer
  if (pSrcImage->GetType() == CL_MEM_OBJECT_IMAGE1D_BUFFER &&
      pSrcImage->GetBackingStoreData() == pDstBuffer->GetBackingStoreData()) {
    return CL_INVALID_MEM_OBJECT;
  }
...
cl_err_code ExecutionModule::EnqueueCopyBufferToImage(
  ...
  // pSrcBuffer
  if (pDstImage->GetType() == CL_MEM_OBJECT_IMAGE1D_BUFFER &&
      pDstImage->GetBackingStoreData() == pSrcBuffer->GetBackingStoreData()) {
    return CL_INVALID_MEM_OBJECT;
  }
...
void *ExecutionModule::EnqueueMapImage(
  ...
  MapImageCommand *pMapImageCmd = new MapImageCommand(
      pCommandQueue, m_pOclEntryPoints, pImage, clMapFlags, szOrigin, szRegion,
...
cl_int ExecutionModule::EnqueueSVMMemcpy(
  ...
  if (NULL == pSrcSvmBuffer.GetPtr()) {
    if (NULL == pDstSvmBuffer.GetPtr()) {
      ...
      pCmd = new WriteSvmBufferCommand(pQueue, m_pOclEntryPoints, bBlockingCopy,
                                       pDstSvmBuffer, pszDstOrigin, pszRegion,
                                       pSrcPtr);
    }
  } else {
    if (NULL == pDstSvmBuffer.GetPtr()) {
      pCmd = new ReadSvmBufferCommand(pQueue, m_pOclEntryPoints, pSrcSvmBuffer,
                                      pszSrcOrigin, pszRegion, pDstPtr);
    } else {
      pCmd = new CopySvmBufferCommand(pQueue, m_pOclEntryPoints, pSrcSvmBuffer,
...
cl_int ExecutionModule::EnqueueSVMMap(cl_command_queue clCommandQueue,
  ...
  Command *pCmd = NULL;
  if (pSvmBuf) {
    pCmd = new MapSvmBufferCommand(pQueue, m_pOclEntryPoints, pSvmBuf, mapflags,
...
cl_int ExecutionModule::EnqueueSVMUnmap(cl_command_queue clCommandQueue,
  ...
  if (pSvmBuf) {
    pCmd =
        new UnmapSvmBufferCommand(pQueue, m_pOclEntryPoints, pSvmBuf, pSvmPtr);
  } else {
    // if it's a system pointer, it must work like a marker.
...
cl_err_code ExecutionModule::EnqueueUSMMemset(
  ...
  if (nullptr != usmBuf.GetPtr())
    memsetCommand = new MemFillUsmBufferCommand(
        queue, m_pOclEntryPoints, usmBuf, pattern, pattern_size,
        (ptrdiff_t)dst_ptr - (ptrdiff_t)usmBuf->GetAddr(), size);
  else
    memsetCommand =
        new RuntimeUSMMemFillCommand(dst_ptr, pattern, pattern_size, size,
...
cl_err_code ExecutionModule::EnqueueUSMMemcpy(
  ...
  if (nullptr == srcBuffer.GetPtr()) {
    if (nullptr == dstBuffer.GetPtr())
      memcpyCommand = new RuntimeUSMMemcpyCommand(dst_ptr, src_ptr, size, queue,
                                                  num_events_in_wait_list > 0);
    else
      memcpyCommand =
          new WriteUsmBufferCommand(queue, m_pOclEntryPoints, blocking,
                                    dstBuffer, dstOrigin, region, src_ptr);
  } else {
    if (nullptr == dstBuffer.GetPtr())
      memcpyCommand = new ReadUsmBufferCommand(
          queue, m_pOclEntryPoints, srcBuffer, srcOrigin, region, dst_ptr);
    else
      memcpyCommand =
          new CopyUsmBufferCommand(queue, m_pOclEntryPoints, srcBuffer,
                                   dstBuffer, srcOrigin, dstOrigin, region);
  }
