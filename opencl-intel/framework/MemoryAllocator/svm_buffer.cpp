...
cl_err_code BufferPointerArg::LockOnDevice(
...
cl_err_code
BufferPointerArg::UnLockOnDevice(IN const SharedPtr<FissionableDevice> &dev,
...
size_t BufferPointerArg::GetPixelSize() const { return m_pBuf->GetPixelSize(); }

void BufferPointerArg::GetLayout(OUT size_t *dimensions, OUT size_t *rowPitch,
                                 OUT size_t *slicePitch) const {
  m_pBuf->GetLayout(dimensions, rowPitch, slicePitch);
}

cl_err_code BufferPointerArg::CheckBounds(const size_t *pszOrigin,
                                          const size_t *pszRegion) const {
  return m_pBuf->CheckBounds(pszOrigin, pszRegion);
}

cl_err_code BufferPointerArg::CheckBoundsRect(const size_t *pszOrigin,
                                              const size_t *pszRegion,
                                              size_t szRowPitch,
                                              size_t szSlicePitch) const {
  return m_pBuf->CheckBoundsRect(pszOrigin, pszRegion, szRowPitch,
                                 szSlicePitch);
}
...
cl_err_code BufferPointerArg::GetDeviceDescriptor(
    const SharedPtr<FissionableDevice> &IN pDevice,
    IOCLDevMemoryObject *OUT *ppDevObject, SharedPtr<OclEvent> OUT *ppEvent) {
  IOCLDevMemoryObject *pBufDevObj;
  const cl_err_code err = m_pBuf->GetDeviceDescriptor(
      pDevice, &pBufDevObj, ppEvent); // what about ppEvent?
  if (CL_FAILED(err)) {
    return err;
  }
...
cl_err_code BufferPointerArg::UpdateDeviceDescriptor(
    const SharedPtr<FissionableDevice> &IN pDevice,
...
cl_dev_err_code
SharedPointerArg::PointerArgDevMemoryObject::clDevMemObjUpdateBackingStore(
    void *operation_handle, cl_dev_bs_update_state *pUpdateState) {
  if (nullptr != m_pBufDevMemObj) {
    return m_pBufDevMemObj->clDevMemObjUpdateBackingStore(operation_handle,
                                                          pUpdateState);
  }
  return CL_DEV_SUCCESS;
}
...
cl_dev_err_code
SharedPointerArg::PointerArgDevMemoryObject::clDevMemObjUpdateFromBackingStore(
    void *operation_handle, cl_dev_bs_update_state *pUpdateState) {
  if (nullptr != m_pBufDevMemObj) {
    return m_pBufDevMemObj->clDevMemObjUpdateFromBackingStore(operation_handle,
                                                              pUpdateState);
  }
  return CL_DEV_SUCCESS;
}
...
cl_dev_err_code
SharedPointerArg::PointerArgDevMemoryObject::clDevMemObjInvalidateData() {
  if (nullptr != m_pBufDevMemObj) {
...
cl_dev_err_code
SharedPointerArg::PointerArgDevMemoryObject::clDevMemObjRelease() {
  if (nullptr != m_pBufDevMemObj) {
