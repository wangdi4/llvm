...
; INTEL_CUSTOMIZATION
; RUN: ld.lld -plugin-opt=opaque-pointers %t1a.o %t2a.o -o %t3a -save-temps --lto-whole-program-visibility \
...
; INTEL_CUSTOMIZATION
; RUN: ld.lld -plugin-opt=opaque-pointers %t1b.o %t2b.o -o %t3b -save-temps --lto-whole-program-visibility \
; RUN: %intel_mllvm %intel_devirt_options \
; end INTEL_CUSTOMIZATION
; RUN:   -mllvm -pass-remarks=. 2>&1 | FileCheck %s --check-prefix=REMARK
; INTEL_CUSTOMIZATION
...
; INTEL_CUSTOMIZATION
; RUN: ld.lld -plugin-opt=opaque-pointers %t1c.o %t2c.o -o %t3c -save-temps --lto-whole-program-visibility \
; RUN: %intel_mllvm %intel_devirt_options \
; end INTEL_CUSTOMIZATION
; RUN:   -mllvm -pass-remarks=. 2>&1 | FileCheck %s --check-prefix=REMARK
; INTEL_CUSTOMIZATION
...
; INTEL_CUSTOMIZATION
; RUN: ld.lld -plugin-opt=opaque-pointers %t2c.o -o %t.so -shared
; end INTEL_CUSTOMIZATION

;; Index based WPD
; INTEL_CUSTOMIZATION
; RUN: ld.lld -plugin-opt=opaque-pointers %t1a.o %t.so -o %t4a --lto-whole-program-visibility \
; end INTEL_CUSTOMIZATION
; RUN:   -mllvm -pass-remarks=. 2>&1 | count 0

;; Hybrid WPD
; INTEL_CUSTOMIZATION
; RUN: ld.lld -plugin-opt=opaque-pointers %t1b.o %t.so -o %t4b --lto-whole-program-visibility \
; end INTEL_CUSTOMIZATION
; RUN:   -mllvm -pass-remarks=. 2>&1 | count 0

;; Regular LTO WPD
; INTEL_CUSTOMIZATION
; RUN: ld.lld -plugin-opt=opaque-pointers %t1c.o %t.so -o %t4c --lto-whole-program-visibility \
; end INTEL_CUSTOMIZATION
; RUN:   -mllvm -pass-remarks=. 2>&1 | count 0
