...
; INTEL_CUSTOMIZATION
; RUN: ld.lld -plugin-opt=opaque-pointers %t2.o -o %t3 -save-temps --lto-whole-program-visibility \
...
; INTEL_CUSTOMIZATION
; RUN: ld.lld -plugin-opt=opaque-pointers %t.o -o %t3 -save-temps --lto-whole-program-visibility \
; RUN: %intel_mllvm %intel_devirt_options \
; end INTEL_CUSTOMIZATION
; RUN:   -mllvm -pass-remarks=. 2>&1 | FileCheck %s --check-prefix=REMARK
; INTEL_CUSTOMIZATION
...
; RUN: opt -o %t4.o %s
; INTEL_CUSTOMIZATION
; RUN: ld.lld -plugin-opt=opaque-pointers %t4.o -o %t3 -save-temps --lto-whole-program-visibility \
; RUN: %intel_mllvm %intel_devirt_options \
; end INTEL_CUSTOMIZATION
; RUN:   -mllvm -pass-remarks=. 2>&1 | FileCheck %s --check-prefix=REMARK
; INTEL_CUSTOMIZATION
...
; INTEL_CUSTOMIZATION
; RUN: ld.lld -plugin-opt=opaque-pointers %t5.o -o %t5.so -shared
; RUN: ld.lld -plugin-opt=opaque-pointers %t5.o %t5.so -o %t5 -save-temps --lto-whole-program-visibility \
; RUN: %intel_mllvm %intel_devirt_options \
; end INTEL_CUSTOMIZATION
