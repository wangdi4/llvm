; REQUIRES: asserts
; This test checks that whole program was detected since the aliases @aliassub
; and @aliassub2 were resolved. The goal of this test is to check that an alias
; that aliases another alias is resolved.

; RUN: llvm-as -o %t_wp2alias.bc %s
; RUN: lld-link /out:%t_wp2alias.exe /entry:main %t_wp2alias.bc /subsystem:console  \
