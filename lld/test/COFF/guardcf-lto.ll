...
; INTEL_CUSTOMIZATION
; RUN: lld-link /opt:opaque-pointers -def:%t.def -out:%t.lib -machine:x64
...
; INTEL_CUSTOMIZATION
; RUN: lld-link /opt:opaque-pointers -entry:main -guard:cf -dll %t.bc %t.lib %t.ldcfg.obj -out:%t2.dll
; end INTEL_CUSTOMIZATION
